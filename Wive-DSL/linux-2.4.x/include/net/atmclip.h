...
struct clip_vcc {
	...
	unsigned long ip;	/* local WAN IP address */
//	unsigned long arp_srv;	/* atm arp server IP address */
};
...
int clip_create(struct atm_vcc *atmvcc, int number);	// Jenny
int clip_mkip(struct atm_vcc *vcc,int timeout);
int clip_ip(struct atm_vcc *vcc,int ip);					// Jenny
int clip_entryset(struct atm_vcc *vcc,u32 ip);			// Jenny
