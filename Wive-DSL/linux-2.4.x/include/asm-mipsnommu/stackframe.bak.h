...
8:                                                       \
		...
		ori	$28, sp, 0x1fff;                 \
		xori	$28, 0x1fff;                     \
	/*#ifdef NO_MM ku check*/ #if 0						\
		lw	k0, TASK_DEEPTH($28);				\
		sll	k0, k0, 1;						\
		sw	k0, TASK_DEEPTH($28);			\
	#endif									\
		.set	pop
...
/*#ifdef NO_MM ku check */ #if 0					\
		...
		ori		k0, k0,0x1fff;		\
		xori	k0, 0x1fff;			\
		lw		k1, TASK_DEEPTH(k0); \
		nop;							\
		srl		k1, k1,1;				\
		sw		k1, TASK_DEEPTH(k0);	\
#endif								\
		lw	k0, PT_EPC(sp);                  \
		lw	sp,  PT_R29(sp);                 \
		jr	k0;                              \
