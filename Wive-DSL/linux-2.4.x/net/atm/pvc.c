    ...
    int sockaddr_len)
{
	...
	{
		// Mason Yu
		//printk("sap_family error\n");
		return -EAFNOSUPPORT;
	}	
	vcc = ATM_SD(sock);

	// Modified by Mason Yu for modify snmp agent
	if ( addr->sap_addr.vpi != 0 && addr->sap_addr.vci != 16) 
	{
	
		if (!test_bit(ATM_VF_HASQOS,&vcc->flags))
		{
			// Mason Yu
			//printk("test_bit error\n");
			return -EBADFD;
		}	
...
{
int ret;
	ret = pvc_bind(sock,sockaddr,sockaddr_len);
	return ret;
