...
#ifdef CONFIG_RTL867X_IPTABLES_FAST_PATH
	// Kaohj --- fastpath entry point for PPPoA
	if(fp_on) {
		if (pppoa_fastpath(&pvcc->chan, skb)==1)
			return;
	}
#endif
	//printk("normal path\n");
...
static int pppoatm_send(struct ppp_channel *chan, struct sk_buff *skb)
{
	...
	//    /*pvcc->chan.unit*/ 0, skb, pvcc->atmvcc);
	...
	DPRINTK("(unit %d): atm_skb(%p)->vcc(%p)->dev(%p)\n",
	    /* pvcc->chan.unit */ 0, skb, ATM_SKB(skb)->vcc,
	    ATM_SKB(skb)->vcc->dev);
