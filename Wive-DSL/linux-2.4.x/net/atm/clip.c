...
#define DPRINTK(format, args...) printk("clip: " format, ##args)
#else
...
static void send_arp(struct atm_vcc *vcc, struct net_device *dev, unsigned short op, uint32_t local_ip, uint32_t remote_ip)
{
	...
	DPRINTK("send_arp: skb->data 0x%04X%04X%04X%04X%04X%04X%04X%04X%04X%04X%04X%04X\n"
		, ((u16 *)skb->data)[0], ((u16 *)skb->data)[1], ((u16 *)skb->data)[2], ((u16 *)skb->data)[3], ((u16 *)skb->data)[4], ((u16 *)skb->data)[5], ((u16 *)skb->data)[6], ((u16 *)skb->data)[7]
		, ((u16 *)skb->data)[8], ((u16 *)skb->data)[9], ((u16 *)skb->data)[10], ((u16 *)skb->data)[11], ((u16 *)skb->data)[12], ((u16 *)skb->data)[13], ((u16 *)skb->data)[14], ((u16 *)skb->data)[15]);
...
//	if (!atmarpd) return -EUNATCH;
	...
	switch (type) {
		...
		case act_change:
			break;
		default:
			DPRINTK("invalid control msg type 0x%x", type);
	}
#if 0
	DPRINTK("to_atmarpd(type %d itf %d ip 0x%X)\n",ctrl->type, ctrl->itf_num, ctrl->ip);
	DPRINTK("to_atmarpd(atmarpd vpi %d vci %d)\n", atmarpd->vpi, atmarpd->vci);
...
{
	DPRINTK("link_vcc %p to entry %p (neigh %p neigh->type %d)\n",clip_vcc,entry,
	    entry->neigh, entry->neigh->type);
	clip_vcc->entry = entry;
//	clip_vcc->xoff = 0; /* @@@ may overrun buffer by one packet */	// Jenny
...
static void unlink_clip_vcc(struct clip_vcc *clip_vcc)
{
	...
	spin_lock_bh(&entry->neigh->dev->xmit_lock);   /* block clip_start_xmit() */	// Jenny
	entry->neigh->used = jiffies;
	for (walk = &entry->vccs; *walk; walk = &(*walk)->next)
		if (*walk == clip_vcc) {
			...
			*walk = clip_vcc->next; /* atomic */
			clip_vcc->entry = NULL;
//			if (clip_vcc->xoff)	// Jenny
//				netif_wake_queue(entry->neigh->dev);
//			if (entry->vccs) return;	// Jenny
			if (entry->vccs)
				goto out;
...
//	DPRINTK("idle_timer_check clip_tbl %p clip_tbl.id %s\n", &clip_tbl, clip_tbl.id);
			...
			for (clip_vcc = entry->vccs; clip_vcc;
				...
				if (clip_vcc->idle_timeout &&
					...
					DPRINTK("releasing vcc %p->%p of "
					    ...
					    entry);
					send_arp(clip_vcc->vcc, clip_vcc->vcc->net_dev, ARPOP_InREQUEST, clip_vcc->ip, 0);
...
static int clip_arp_rcv(struct sk_buff *skb)
{
	...
	DPRINTK("clip_arp_rcv skb=%p skb->dev %p skb->dev->type=%d\n", skb, skb->dev, skb->dev->type);
	DPRINTK("clip_arp_rcv: skb->data 0x%04X%04X%04X%04X%04X%04X%04X%04X\n", ((u16 *)skb->data)[0], ((u16 *)skb->data)[1], ((u16 *)skb->data)[2], ((u16 *)skb->data)[3], ((u16 *)skb->data)[4], ((u16 *)skb->data)[5], ((u16 *)skb->data)[6], ((u16 *)skb->data)[7]);
	vcc = ATM_SKB(skb)->vcc;
	if (!vcc || !atm_charge(vcc,skb->truesize)) {
		dev_kfree_skb_any(skb);
		return 0;
	}
//	DPRINTK("pushing to vcc %p\n",vcc);
//	DPRINTK("using %p->old_push %p\n", CLIP_VCC(vcc), CLIP_VCC(vcc)->old_push);
//	CLIP_VCC(vcc)->old_push(vcc,skb);
	...
	src_ip = get_ip(spa);
	tgt_ip = get_ip(tpa);
	DPRINTK("  SRC IP: 0x%x, DST IP: 0x%x\n",src_ip, tgt_ip);
	switch (ntohs(arp->ar_op)) {
		case ARPOP_InREQUEST:
			DPRINTK("got InARP_REQ, my ip 0x%X\n", CLIP_VCC(vcc)->ip);
...
//			clip_entryset(vcc, src_ip);
		...
		case ARPOP_InREPLY:
			...
			break;
		case ARPOP_REQUEST:
			DPRINTK("got ARPOP_REQUEST, my ip 0x%X\n", CLIP_VCC(vcc)->ip);
...
//			clip_entryset(vcc, src_ip);
		...
		case ARPOP_REPLY:
			...
			break;
		default:
			DPRINTK("unrecognized ARP op 0x%x\n", ntohs(arp->ar_op));
    }
	dev_kfree_skb_any(skb);
	return 0;
}
...
//		if (skb->protocol == htons(ETH_P_ARP)) {
		if (skb->protocol == htons(ETH_P_ARP) && ((u16 *) skb->data)[0] == htons(ARPHRD_ATM)) {
	...
	PRIV(skb->dev)->stats.rx_packets++;
	PRIV(skb->dev)->stats.rx_bytes += skb->len;
//	memset(ATM_SKB(skb), 0, sizeof(struct atm_skb_data));	// Jenny
...
static void clip_pop(struct atm_vcc *vcc,struct sk_buff *skb)
{
	...
	DPRINTK("clip_pop(vcc %p) dev %p dev->type=%d\n",vcc, dev, dev->type);
...
static int clip_constructor(struct neighbour *neigh)
{
	...
	DPRINTK("clip_constructor (neigh %p, entry %p in_dev %p dev %p dev->type=%d in_dev->dev %p)\n",neigh, entry,in_dev, dev, dev->type, in_dev->dev);
...
#if 0
int srv_ip(struct atm_vcc *vcc,int ip)
{
	CLIP_VCC(vcc)->arp_srv = ip;
	return 0;
}
#endif
...
//	unsigned long flags;
	...
	DPRINTK("clip_start_xmit (skb %p skb->dev %p skb->dev->type=%d dev %p skb->dst->neighbour %p skb->dst->dev %p)\n",skb, skb->dev, skb->dev->type, dev, skb->dst->neighbour, skb->dst->dev);
...
//		return 0;	// Jenny
	...
	DPRINTK("clip_start_xmit neigh %p, vccs %p\n",entry,entry->vccs);
	ATM_SKB(skb)->vcc = vcc = entry->vccs->vcc;
	DPRINTK("clip_start_xmit using neighbour %p, vcc %p\n",skb->dst->neighbour,vcc);
	if (entry->vccs->encap) {
		void *here;
...
int clip_mkip(struct atm_vcc *vcc,int timeout)
{
	...
	DPRINTK("mkip clip_vcc %p vcc %p vcc->dev %p clip_devs %p vcc->push %p\n",clip_vcc,vcc, vcc->dev, clip_devs, vcc->push);
	clip_vcc->vcc = vcc;
	vcc->user_back = clip_vcc;
	clip_vcc->entry = NULL;
//	clip_vcc->xoff = 0;	// Jenny
	clip_vcc->encap = 1;
	clip_vcc->last_use = jiffies;
	clip_vcc->idle_timeout = timeout*HZ;
	clip_vcc->old_push = vcc->push;
	barrier();	// Jenny
//	clip_vcc->old_pop = vcc->pop;	// Jenny
	vcc->push = clip_push;
//	vcc->pop = clip_pop;	// Jenny
	skb_queue_head_init(&copy);
	skb_migrate(&vcc->recvq,&copy);
	// Kaohj -- notify encapsulation to sar driver, added by Jenny
	...
	DPRINTK("registered (net:%s) %p\n", clip_devs->name, vcc->net_dev);
	return 0;
}


int clip_entryset(struct atm_vcc *vcc,u32 ip)
{
	...
	if (entry != clip_vcc->entry) {
		if (!clip_vcc->entry) DPRINTK("clip_entryset: add\n");
		else {
			DPRINTK("clip_entryset: update\n");
			unlink_clip_vcc(clip_vcc);
		}
		link_vcc(clip_vcc,entry);
	}
	DPRINTK("clip_entryset: clip_vcc %p vcc->user_back %p entry->vccs->vcc %p\n", clip_vcc, vcc->user_back, entry->vccs->vcc);
...
static int clip_init(struct net_device *dev)
{
	...
	dev->tx_queue_len = 100; /* "normal" queue (packets) */
	    ...
	    /* against memory hogs. */
	dev->flags = 0;
	// Kaohj, used to avoid bridging between WAN connections, added by Jenny
	dev->priv_flags = IFF_DOMAIN_WAN;
	return 0;
}

//int clip_create(int number)
int clip_create(struct atm_vcc *atmvcc, int number)
{
...
//	sprintf(dev->name,"atm%d",number);
	...
	// Kaohj -- put net_device into atm_vcc, added by Jenny
...
{
	DPRINTK("atmarpd_close vcc %p\n", vcc);
	del_timer(&idle_timer);	// Jenny
...
int atm_init_atmarp(struct atm_vcc *vcc)
{
	...
	vcc->user_back = CLIP_VCC(vcc);	// Jenny
