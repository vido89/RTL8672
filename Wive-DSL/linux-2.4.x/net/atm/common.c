    ...
    int vci)
{
	...
	DPRINTK("  TX: %d, PCR %d %d..%d, SDU %d\n",vcc->qos.txtp.traffic_class,
	    vcc->qos.txtp.pcr, vcc->qos.txtp.min_pcr,vcc->qos.txtp.max_pcr,vcc->qos.txtp.max_sdu);
	DPRINTK("  RX: %d, PCR %d %d..%d, SDU %d\n",vcc->qos.rxtp.traffic_class,
	    vcc->qos.rxtp.pcr, vcc->qos.rxtp.min_pcr,vcc->qos.rxtp.max_pcr,vcc->qos.rxtp.max_sdu);
	fops_get (dev->ops);
	if (dev->ops->open) {
...
static int atm_do_connect(struct atm_vcc *vcc,int itf,int vpi,int vci)
{
	...
	if (!dev) {
		DPRINTK("atm_do_connect(): atm_find_dev() error = %d\n", -ENODEV);
		return_val =  -ENODEV;
	}
	else
		return_val = atm_do_connect_dev(vcc,dev,vpi,vci);

	DPRINTK("atm_do_connect(): atm_do_connect_dev() = %d\n", return_val);
...
int atm_connect_vcc(struct atm_vcc *vcc,int itf,short vpi,int vci)
{
	...
	// Modified by Mason Yu for modify snmp agent
	if ( vpi != 0 && vci != 16 ) 
	{
...
int atm_connect(struct socket *sock,int itf,short vpi,int vci)
{
	...
	//allow PVC 0/0 to create, UI must npt allow this case
    ...
    int flags,struct scm_cookie *scm)
{
	...
	// Mason Yu for Test write()
	//printk("atm_recvmsg : Do atm_recvmsg\n");
    ...
    struct scm_cookie *scm)
{
	...
	// Mason Yu for Test write()
	//printk("atm_sendmsg : Do atm_sendmsg\n");
...
//				ret_val = clip_create(arg);
				ret_val = clip_create(vcc, arg);	// Jenny
			goto done;
/*		case ATMARPD_CTRL:
			if (!capable(CAP_NET_ADMIN)) {
				ret_val = -EPERM;
...
*/
		...
		case ATMARP_IP:	// Jenny, local host IP address
			if (!capable(CAP_NET_ADMIN)) 
				...
				ret_val = clip_ip(vcc, arg);
			goto done;
/*		case ATMARP_SRV:	// Jenny, arp server IP address
			if (!capable(CAP_NET_ADMIN)) 
				ret_val = -EPERM;
			else
				ret_val = srv_ip(vcc, arg);
			goto done;
*/
#endif
#if defined(CONFIG_ATM_LANE) || defined(CONFIG_ATM_LANE_MODULE)
