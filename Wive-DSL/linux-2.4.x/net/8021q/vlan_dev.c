...
#ifdef VLAN_DEBUG
	printk(VLAN_DBG ": skb: %p vlan_id: %hx\n",
	       skb, vid);
#endif
...
#ifdef VLAN_DEBUG
		printk(VLAN_DBG ": ERROR:	No net_device for VID: %i on dev: %s [%i]\n",
...
#ifdef VLAN_DEBUG
		printk(VLAN_DBG ": dropping skb: %p because came in on wrong device, dev: %s  real_dev: %s, skb_dev: %s\n",
...
#ifdef VLAN_DEBUG
	printk(VLAN_DBG ": priority: %lu  for TCI: %hu (hbo)\n",
...
#ifdef VLAN_DEBUG
	printk(VLAN_DBG ": skb: %p type: %hx len: %x vlan_id: %hx, daddr: %p\n",
...
#ifdef VLAN_DEBUG
		printk(VLAN_DBG ": %s: had to grow skb.\n", vdev->name);
#endif
	}
...
#ifdef VLAN_DEBUG
		printk(VLAN_DBG ": proto to encap: 0x%hx (hbo)\n",
...
#ifdef VLAN_DEBUG
	printk(VLAN_DBG ": about to send skb: %p to dev: %s\n",
	       skb, skb->dev->name);
	...
	if (dev) {
		if (dev->priv_flags & IFF_802_1Q_VLAN) {
			...
			if (flag == 1) {
				...
				printk(KERN_ERR ": flag %i is not valid.\n",
				       (int)(flag));
				dev_put(dev);
				return -EINVAL;
			}
		...
		printk(KERN_ERR ": Could not find device: %s\n", dev_name);
	}
