					       ...
					       unsigned short VLAN_ID)
{
	...
	if (real_dev->features & NETIF_F_VLAN_CHALLENGED) {
		printk(VLAN_DBG ": VLANs not supported on %s.\n",
		       real_dev->name);
		goto out_put_dev;
	}
	...
	if ((real_dev->features & NETIF_F_HW_VLAN_RX) &&
		...
		printk(VLAN_DBG ": Device %s has buggy VLAN hw accel.\n",
		       real_dev->name);
		goto out_put_dev;
	}
	...
	if (r) {
		...
		printk(VLAN_DBG ": ALREADY had VLAN registered\n");
		goto out_unlock;
	}
...
int vlan_ioctl_handler(unsigned long arg)
{
	...
	default:
		...
		printk(VLAN_DBG ": Unknown VLAN CMD: %x \n",
		       args.cmd);
		return -EINVAL;
	};
