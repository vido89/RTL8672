...
{
	if (!vlandev) {
		printk(VLAN_ERR ": invalid argument: %p\n",
		       vlandev);
		return -EINVAL;
	}

	if (!(vlandev->priv_flags & IFF_802_1Q_VLAN)) {
		printk(VLAN_DBG ": invalid argument, device: %s is not a VLAN device, priv_flags: 0x%4hX.\n",
		       vlandev->name, vlandev->priv_flags);
		return -EINVAL;
	}

#ifdef VLAN_DEBUG
	printk(VLAN_DBG ": dev: %p\n", vlandev);
#endif
