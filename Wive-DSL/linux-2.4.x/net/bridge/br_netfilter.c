...
static int br_nf_pre_routing_finish_ipv6(struct sk_buff *skb)
{
	...
	if (skb->protocol == __constant_htons(ETH_P_PPP_SES)) {
		skb_push(skb, PPPOE_HDR_LEN);
		skb->nh.raw -= PPPOE_HDR_LEN;
	}
#endif
...
#ifdef SUPPORT_PPPOE
	if (skb->protocol == __constant_htons(ETH_P_PPP_SES)|| skb->protocol == __constant_htons(ETH_P_IP) \
		|| IS_VLAN_IP)
#else
	if (skb->protocol == __constant_htons(ETH_P_IP) || IS_VLAN_IP)
#endif		
...
#ifdef SUPPORT_PPPOE
	if (skb->protocol == __constant_htons(ETH_P_PPP_SES)) {
		skb_pull(*pskb, PPPOE_HDR_LEN);
		(*pskb)->nh.raw += PPPOE_HDR_LEN;
	}
#endif 
   ...
   int (*okfn)(struct sk_buff *))
{
	...
	//add by ramen to accept the packet from the device that doesn't belong to the bridge,example the router model pvc .
...
int br_netfilter_onoff( char * arg )
{
