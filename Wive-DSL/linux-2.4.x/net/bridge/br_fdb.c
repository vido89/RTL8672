 ...
 *
 *	$Id: br_fdb.c,v 1.6 2007/09/11 08:56:53 jiunming Exp $
 *
...
struct net_bridge_fdb_entry *br_fdb_get2(struct net_bridge *br, unsigned char *addr)
{
	struct net_bridge_fdb_entry *fdb;
		   ...
		   int is_local)
{
	...
	// Kao
	if (!is_local && fdb_count >= FDB_MAX_COUNT)
	{
		// fdb full, cannot insert it
		write_unlock_bh(&br->hash_lock);
		return;
	}
