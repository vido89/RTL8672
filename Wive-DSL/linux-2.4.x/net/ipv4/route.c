 ...
 *
 * Version:	$Id: route.c,v 1.9 2008/10/28 03:58:46 jenny Exp $
 *
...
int ip_route_output_slow(struct rtable **rp, const struct rt_key *oldkey)
{
	...
	if (fib_lookup(&key, &res)) {
		...
		err = -ENETUNREACH;
		#if 0
		// Mason Yu. dial on demand for local-out packet.
		{
		extern int ppp_dial_pid;
			if(ppp_dial_pid>0) {
				sys_kill(ppp_dial_pid, SIGUSR1); /* Raise SIGUSR1 in current process */
				ppp_dial_pid = 0;
				printk("sys kill dial for local-out packet\n");
			}
		}
		#endif
		goto out;
