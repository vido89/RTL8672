 ...
 *
 * Version:	$Id: ip_output.c,v 1.8 2008/10/22 06:55:44 cathy Exp $
 *
...
#endif
// Kaohj --- for Telefonica special case: source IP passthrough
...
int ip_output(struct sk_buff *skb)
{
	...
	if (skb->dst->dev->br_port == 0 && !(skb->dst->dev->priv_flags & IFF_EBRIDGE)) {
...
int ip_fragment(struct sk_buff *skb, int (*output)(struct sk_buff*))
{
	...
	while(left > 0)	{
		...
		skb2->vlan_member = skb->vlan_member;
		// restore from_dev (been kept at skbuff.c::pskb_copy())
		skb2->from_dev = skb->from_dev;
