 ...
 *
 * Version:	$Id: fib_frontend.c,v 1.3 2007/09/26 03:39:47 yachang Exp $
 *
...
void __SWAP Route_Fastpath_Init() {
	int t;
	struct fib_table *tb;
	...
	#ifdef CONFIG_IP_MULTIPLE_TABLES
	for (t=RT_TABLE_MIN; t<=RT_TABLE_MAX; t++) {
	...
	#else
	fib_iterate(main_table, 0);
	fib_iterate(local_table, 0);
	#endif	
}
#endif
