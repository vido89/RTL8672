...
ip_packet_match(const struct iphdr *ip,
		const char *indev,
#if defined(CONFIG_BRIDGE) || defined(CONFIG_BRIDGE_MODULE)  // ebtables
...
// ebtables	
	...
	//if (FWINV(ret != 0, IPT_INV_VIA_IN) && (strcmp(switch_port, ipinfo->iniface)!=0) ) {
	// Kaohj
	if (switch_port == NULL)
		switch_port = "";
	if (FWINV(ret != 0 && ret2 != 0, IPT_INV_VIA_IN) && (strcmp(switch_port, ipinfo->iniface)!=0) ) { // ebtables		
	     ...
	     void *userdata)
{
	...
	do {
		...
		//if (ip_packet_match(ip, indev, outdev, &e->ip, offset, (*pskb)->switch_port)) {
		// ebtables
