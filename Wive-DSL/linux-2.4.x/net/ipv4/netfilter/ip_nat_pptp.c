...
// The function lack a parameter. We should add a new parameter of struct ip_conntrack_expect *exp.  
			 	...
			 	struct sk_buff **pskb)
{
	...
	if(master == NULL || master->help.ct_pptp_info.pptp_magic != PPTP_TCP_PORT) {
		...
		greh = (struct pptp_gre_hdr*) ((void *) iph + iph->ihl * 4);
	}
...
static struct ip_nat_helper pptp = 
// Commented by Mason Yu
#if 0
                  { { NULL, NULL },
				    { { 0, {.tcp = { __constant_htons(1723) }} },
				      { 0, { 0 }, IPPROTO_TCP } },
				    { { 0, { .tcp ={0xFFFF }} },
				      { 0, { 0 }, 0xFFFF } },
				    pptp_help, "pptp" };
...
static struct ip_nat_helper gre = 
// Commented by Mason Yu
#if 0
                  { { NULL, NULL },
				    { { 0, { 0 } },
