...
static void init_tcp_state_hash()
{
	int i;
...
static inline void recalculate() {
	int i;
...
int Get_ip_conntrack_count()
{
return ip_conntrack_count.counter;
}
extern int dst_garbage_num_g;
#ifdef CONFIG_IP_CONNTRACK_GARBAGE_NEW
static unsigned int
...
#if 0
asmlinkage void sys_GetConntrackNum(void)
{
     
#ifdef CONFIG_IP_CONNTRACK_GARBAGE_NEW1	  
#define PRINTLENGTH  512
     READ_LOCK(&ip_conntrack_lock);
     	  struct list_head* it=NULL;
           int seq = 1;
	  int i = 0,length=0;
	  char *buffer=(char*)kmalloc(PRINTLENGTH*sizeof(char),GFP_KERNEL);
	  if(!buffer) return;
	  for(i=1; i<TCP_CONNTRACK_MAX;i++ )
	  {
	  {
	  	unsigned int status;
		asm("mfc0 %0, $12": "=d"(status):);
		printk("reg status %x\n", status);
	  }
	       printk("print %d list \n",Tcp_State_Hash_Head[i].state);
	      if(Tcp_State_Hash_Head[i].state_hash_head->next == Tcp_State_Hash_Head[i].state_hash_end)
	      	{
	      	  printk("%d is empty\n",Tcp_State_Hash_Head[i].state);
	      	}
	     else{
		 	int num = 0;
              for(it=Tcp_State_Hash_Head[i].state_hash_head->next;it!=Tcp_State_Hash_Head[i].state_hash_end;it=it->next)
		 	{
		 		num++;				
				memset(buffer,0,PRINTLENGTH);
 			         length=print_conntrack_1(buffer,(struct ip_conntrack*)((char *)it-STATE_TUPLE_OFFSET));
	 			if(length||length<=PRINTLENGTH)
	 			printk("seq=%d:   %s",seq++,buffer);
				else
						{
						printk("seq=%d:      @@@@@@@@@@@@@@@@@@@@\n",seq);
						}
	  ...
	  for(i=0;i<2;i++)
		...
		printk("reg status %x\n", status);
	  	   ...
	  	   printk("print %d list \n",Udp_State_Hash_Head[i].state);
		  	  if(Udp_State_Hash_Head[i].state_hash_head->next == Udp_State_Hash_Head[i].state_hash_end)
		      	{
		      	  printk("%d is empty\n",Udp_State_Hash_Head[i].state);
		      	}
		     else{
			 	int num = 0;
	              for(it=Udp_State_Hash_Head[i].state_hash_head->next;it!=Udp_State_Hash_Head[i].state_hash_end;it=it->next)
			 	{
			 		num++;					
					memset(buffer,0,PRINTLENGTH);
					length=print_conntrack_1(buffer,(struct ip_conntrack*)((char *)it-STATE_TUPLE_OFFSET));
	 ...
	 kfree(buffer);
     	 READ_UNLOCK(&ip_conntrack_lock);
#endif
 //printk("Now:ip_conntrack_count=%d,ipv4_dst_ops.entries=%d,arp_tbl.entries=%d ip_conntrack_max=%d,dst_garbage_num_g=%d\n",Get_ip_conntrack_count(),Get_ipv4_dst_ops_entries(),GetArpTbl(),ip_conntrack_max,0/*dst_garbage_num_g*/);
...
destroy_conntrack(struct nf_conntrack *nfct)
{
	...
	// Kao added, remove its expectations
	remove_expectations(ct);
		    ...
		    const struct ip_conntrack *ignored_conntrack)
{
	...
	// Kaohj -- ip_conntrack optimization
	// calls it once before looping
...
//printk("protcol---%d\n",(skb)->nh.iph->protocol);
	...
	DEBUGP("Confirming conntrack %p\n", ct);
        // #define DEBUGP printk
             DEBUGP("__ip_conntrack_confirm:");
             DUMP_TUPLE(&(ct->tuplehash[0].tuple));
             DUMP_TUPLE(&(ct->tuplehash[1].tuple));
         #define DEBUGP
	WRITE_LOCK(&ip_conntrack_lock);
...
static struct ip_conntrack_tuple_hash *oldest_h =NULL;
static inline int iterate(const struct ip_conntrack_tuple_hash *i)
{
	//return !(i->ctrack->status & IPS_ASSURED);
	if (i->ctrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum != IPPROTO_TCP)
		goto BYEBYE;
	...
	if (!oldest_h)
		...
		if (oldest_h->ctrack->timeout.expires > i->ctrack->timeout.expires)
			oldest_h = i;
	}
...
int drop_oldest () 
{
	...
	for(i=0; i<ip_conntrack_htable_size; i++) {	
		h = LIST_FIND(&ip_conntrack_hash[i], iterate, struct ip_conntrack_tuple_hash *);
...
int drop_unreply( )
{
	...
	int i;
	for(i=0; i<ip_conntrack_htable_size; i++) {	
		if (early_drop(&ip_conntrack_hash[i]))
			return 1;
	}
...
__NOMIPS16
#endif
//static inline int __drop_one_by_proto(int proto){
static int __drop_one_by_proto(int proto){
	int i = 0;
	LIST_HEAD(empty);
	struct list_head *ptr = &empty;
	...
	for(i =0; i<sizeof(drop_priority)/sizeof(struct DROP_PRORITY); i++)
	{
 		...
 		if (!list_empty(ptr))
		{
			...
			ct=list_entry(ptr->next,struct ip_conntrack, state_tuple);
			atomic_inc(&ct->ct_general.use);
			break;
		}
	}
...
#if 0
static inline int __drop_one_udp(){
	int i = 0;
...
static inline int __drop_one_tcp(){
	int i = 0;
	struct list_head *ptr;
	READ_LOCK(&ip_conntrack_lock);       
	for(i =0; i<sizeof(drop_priority)/sizeof(struct DROP_PRORITY); i++)
...
static int __drop_one_conntrack(int factor)
{
       //printk("enter the drop_one_tcp_conntrack.............\n");
	int i = 0;
        struct list_head *ptr;
        struct list_head* it;
	struct ip_conntrack* tmp, *ct = NULL;
	int dropped = 0;
	READ_LOCK(&ip_conntrack_lock);       
	for(i =0; i<sizeof(drop_priority)/sizeof(struct DROP_PRORITY); i++)
	{
...
int drop_one_conntrack() {
	if ((_prot_limit[PROT_TCP] < atomic_read(&prot_counters[PROT_TCP])) && __drop_one_by_proto(PROT_TCP)) {
		return 1;
	}

	if ((_prot_limit[PROT_UDP] < atomic_read(&prot_counters[PROT_UDP])) && __drop_one_by_proto(PROT_UDP)) {
		return 1;
	}
...
void drop_all_dns_conntrack()
{
	...
	for ( i=0; i< ip_conntrack_htable_size; i++) {
		...
		list_for_each(current, head) {			
			h = ( struct ip_conntrack_tuple_hash *)current;
			
			if (h->ctrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == 53) {
				printk("drop_all_dns_conntrack: Find %d session\n", i+1);
				// Add user account
				//atomic_inc(&h->ctrack->ct_general.use);			
...
// Kill all ip conntrack session that protocol is ICMP.
void drop_all_icmp_conntrack()
{
	int i;	
	struct list_head *head, *current;	
	struct ip_conntrack_tuple_hash *h;	
	...
	for ( i=0; i< ip_conntrack_htable_size; i++) {
		...
		list_for_each(current, head) {			
			...
			if (h->ctrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_ICMP) {
				//printk("drop_all_icmp_conntrack: Find %d session\n", i+1);
				// Add user account
				//atomic_inc(&h->ctrack->ct_general.use);			
...
// Kill all ip conntrack that protocil is TCP or UDP.
void drop_all_tcp_udp_conntrack()
{
	int i;	
	struct list_head *head, *current;	
	struct ip_conntrack_tuple_hash *h;	
	...
	for ( i=0; i< ip_conntrack_htable_size; i++) {
		...
		list_for_each(current, head) {			
			...
			if ( (h->ctrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_TCP) || 
			     (h->ctrack->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_UDP)    ) {
				printk("drop_all_tcp_udp_conntrack: Find %d session\n", i+1);
				// Add user account
				//atomic_inc(&h->ctrack->ct_general.use);			
...
// Mason Yu
// Kill all ip conntrack sessions.
#ifdef CONFIG_RTL867X_KERNEL_MIPS16_NET
__NOMIPS16
#endif
void drop_all_conntrack()
{
	int i;	
	struct list_head *head, *current;	
	struct ip_conntrack_tuple_hash *h;		
...
int __SWAP
ct_iterate(const struct ip_conntrack_tuple_hash *hash)
{
	struct ip_conntrack *ct;
	struct ip_nat_helper *helper;
	struct ip_nat_info *info;
				...
				struct ip_conntrack_expect *expect)
{
	...
	if (old) {
		...
		// Kaohj, list_for_each --> list_for_each_prev
...
#ifdef CONFIG_IP_CONNTRACK_GARBAGE_NEW	
static int conntrack_min = 0;
static int conntrack_max= 100;
#endif
#ifdef CONFIG_SYSCTL
...
static ctl_table ip_conntrack_table[] = {
	...
	{ NET_IP_CONNTRACK_MAX, NET_IP_CONNTRACK_MAX_NAME, &ip_conntrack_max,
	  sizeof(ip_conntrack_max), 0644,  NULL, conntrack_dointvec },	  
	{ NET_IP_CONNTRACK_MAX+1, "ip_conntrack_tcp", &prot_limit[PROT_TCP], 	  
	  sizeof(prot_limit[PROT_TCP]), 0644, NULL, &conntrack_dointvec_minmax, NULL, 0, &conntrack_min, &conntrack_max },
	{ NET_IP_CONNTRACK_MAX+2, "ip_conntrack_udp", &prot_limit[PROT_UDP], 
	  sizeof(prot_limit[PROT_UDP]), 0644, NULL, &conntrack_dointvec_minmax, NULL, 0, &conntrack_min, &conntrack_max },
	{ NET_IP_CONNTRACK_MAX+3, "ip_conntrack_count", &ip_conntrack_count, 
