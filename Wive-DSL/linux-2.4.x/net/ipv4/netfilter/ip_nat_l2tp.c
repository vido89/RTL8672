...
static char flag='1';
// Added by Mason Yu
static u_int16_t last_local_port=0;
...
unsigned int l2tpInbound(u_int32_t srcip, struct iphdr *iph,struct l2tp_header *dptr)
{
	...
	//printk("udph->dest=%d last_local_port=%d dptr->tunnnel_id=%d dptr->flag=%x last_ip=%x \n", udph->dest, last_local_port, dptr->tunnnel_id, ntohs(dptr->flag), last_ip);
	...
	for(i=0; i< MaxSession; i++)
	  {
	    ...
	    if(session[i].tunnel_id==dptr->tunnnel_id && dptr->tunnnel_id !=0  && session[i].local_port==udph->dest )
	      {
	      LocalIp=session[i].redirect_ip;
	      //printk("l2tpInbound(1): LocalIp = %x\n", LocalIp);
	      return SUCCESS;
	      }
	  }
	...
	{	   
	   ...
	   if(i== MaxSession)
	     {	     
             ...
             // Added by Mason Yu
             session[session_cnt].local_port=last_local_port;
	 ...
	 LocalIp=last_ip;
	 //printk("l2tpInbound(2): LocalIp = %x\n", LocalIp);
	 return SUCCESS;
...
// Line 173(AP source code) : { { 0, { __constant_htons(1701) } }
// Line 175(AP source code) : { { 0,{ 0xFFFF } }
