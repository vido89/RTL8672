...
unsigned char ftp_algonoff= 0;
#define PROCFS_NAME "ftp_algonoff"
static struct proc_dir_entry* FP_Proc_ftp_algonoff;
#ifdef CONFIG_RTL867X_KERNEL_MIPS16_NET
__NOMIPS16
#endif
int ftp_proc_debug_write(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
	...
	if(!ftp_algonoff)
        	{
        	ftp_alginit();
	 DEBUGP("*****Enable ftp ALG function!*****\n");
	 ftp_algonoff=1;
	}
    }
	...
	if(ftp_algonoff){
		fini();
		DEBUGP("*****Disable ftp ALG function!*****\n");
		ftp_algonoff=0;
		}
    }
    else
...
int ftp_proc_debug_read(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
         if(ftp_algonoff==1)
        printk("ftp ALG ON!\n");
...
int  Ftp_Alg_OnOff_init()
{

	FP_Proc_ftp_algonoff= create_proc_entry(PROCFS_NAME, 0644, &proc_root);
    if (FP_Proc_ftp_algonoff == NULL) {
        remove_proc_entry(PROCFS_NAME, &proc_root);
        printk(KERN_ALERT "Error: Could not initialize /proc/%s\n",
            PROCFS_NAME);
        return -ENOMEM;
    }
    FP_Proc_ftp_algonoff->read_proc  = (read_proc_t *)ftp_proc_debug_read;
    FP_Proc_ftp_algonoff->write_proc  = (write_proc_t *)ftp_proc_debug_write;
    //FP_Proc_Debug->owner       = THIS_MODULE;
    FP_Proc_ftp_algonoff->mode       = S_IFREG | S_IRUGO;
    FP_Proc_ftp_algonoff->uid       = 0;
    FP_Proc_ftp_algonoff->gid       = 0;
    FP_Proc_ftp_algonoff->size       = 37;  
...
static void ftp_alginit(void)
{
	...
	#ifdef CONFIG_IP_NF_ALG_ONOFF
	DEBUGP("load ftp ALG mould sucessfully!\n");
	ftp_algonoff=1;	
	#endif
}
