...
static void irc_alginit(void);
#ifdef CONFIG_IP_NF_ALG_ONOFF
static unsigned char ginitflag=0;
unsigned char irc_algonoff= 0;
#define PROCFS_NAME "irc_algonoff"
static struct proc_dir_entry* FP_Proc_irc_algonoff;
int irc_proc_debug_write(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
	...
	if(!irc_algonoff)
       {
      	 DEBUGP("*****Enable irc ALG function!*****\n");
     	irc_alginit();
	irc_algonoff=1;
	}
    }
       ...
       if(irc_algonoff){
	   	DEBUGP("*****Disable irc ALG function!*****\n");       
		fini();
		irc_algonoff=0;
    	}
    }
...
int irc_proc_debug_read(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
         if(irc_algonoff==1)
        printk("irc ALG ON!\n");
...
int  irc_Alg_OnOff_init()
{

	FP_Proc_irc_algonoff= create_proc_entry(PROCFS_NAME, 0644, &proc_root);
    if (FP_Proc_irc_algonoff == NULL) {
        remove_proc_entry(PROCFS_NAME, &proc_root);
        printk(KERN_ALERT "Error: Could not initialize /proc/%s\n",
            PROCFS_NAME);
        return -ENOMEM;
    }
    FP_Proc_irc_algonoff->read_proc  = (read_proc_t *)irc_proc_debug_read;
    FP_Proc_irc_algonoff->write_proc  = (write_proc_t *)irc_proc_debug_write;
    //FP_Proc_Debug->owner       = THIS_MODULE;
    FP_Proc_irc_algonoff->mode       = S_IFREG | S_IRUGO;
    FP_Proc_irc_algonoff->uid       = 0;
    FP_Proc_irc_algonoff->gid       = 0;
    FP_Proc_irc_algonoff->size       = 37;  
