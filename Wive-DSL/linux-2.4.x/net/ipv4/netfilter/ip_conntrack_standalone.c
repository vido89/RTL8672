			      ...
			      int (*okfn)(struct sk_buff *))
{
	struct rtable *rt = (struct rtable *)(*pskb)->dst;
		...
		//if (qosRuleExist && (*pskb)->dev->br_port)//bridge
		if (enable_imq && (*pskb)->dev->br_port && (*pskb)->imq_br)
		{
			//printk("%s bridge pkt!\n", __FUNCTION__);
			/*ql:20080804 START: remove clear action for further use*/
			(*pskb)->imq_br = 0;
			/*ql:20080804 END*/
			return NF_ACCEPT;
		}
...
extern int ct_iterate(const struct ip_conntrack_tuple_hash *hash);
void __SWAP IPCT_Fastpath_Init() {
	int i;
	READ_LOCK(&ip_conntrack_lock);
	/* Traverse hash; print originals then reply. */
	for (i = 0; i < ip_conntrack_htable_size; i++) {

		if (LIST_FIND(&ip_conntrack_hash[i], ct_iterate, struct ip_conntrack_tuple_hash *))
			goto finished;
	}
