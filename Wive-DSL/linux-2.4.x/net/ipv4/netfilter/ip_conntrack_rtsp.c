...
#define INFOP(args...) printk(KERN_INFO __FILE__ ":" args)
...
static void rtsp_alginit(void);
static void fini(void);
#ifdef CONFIG_IP_NF_ALG_ONOFF
static unsigned char ginitflag=0;
unsigned char rtsp_algonoff= 0;
#define PROCFS_NAME "rtsp_algonoff"
static struct proc_dir_entry* FP_Proc_rtsp_algonoff;
static int rtsp_proc_debug_write(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
	...
	if(!rtsp_algonoff)
        {
        DEBUGP("*****Enable rtsp ALG function!*****\n");
     	rtsp_alginit();
	rtsp_algonoff=1;
	}
    }
        ...
        if(rtsp_algonoff){
	DEBUGP("*****Disable rtsp ALG function!*****\n");       
	fini();
	rtsp_algonoff=0;
        	}
    }
...
static int rtsp_proc_debug_read(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
         if(rtsp_algonoff==1)
        printk("rtsp ALG ON!\n");
...
static int  rtsp_Alg_OnOff_init()
{

	FP_Proc_rtsp_algonoff= create_proc_entry(PROCFS_NAME, 0644, &proc_root);
    if (FP_Proc_rtsp_algonoff == NULL) {
        remove_proc_entry(PROCFS_NAME, &proc_root);
        printk(KERN_ALERT "Error: Could not initialize /proc/%s\n",
            PROCFS_NAME);
        return -ENOMEM;
    }
    FP_Proc_rtsp_algonoff->read_proc  = (read_proc_t *)rtsp_proc_debug_read;
    FP_Proc_rtsp_algonoff->write_proc  = (write_proc_t *)rtsp_proc_debug_write;
    //FP_Proc_Debug->owner       = THIS_MODULE;
    FP_Proc_rtsp_algonoff->mode       = S_IFREG | S_IRUGO;
    FP_Proc_rtsp_algonoff->uid       = 0;
    FP_Proc_rtsp_algonoff->gid       = 0;
    FP_Proc_rtsp_algonoff->size       = 37;  
...
num_ports=0;
	...
	#ifdef CONFIG_IP_NF_ALG_ONOFF
	DEBUGP("load rtsp ALG mould sucessfully!\n");
	rtsp_algonoff=1;	
	#endif
	return 0;
}
