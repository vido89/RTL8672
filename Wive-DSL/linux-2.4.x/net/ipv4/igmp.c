 ...
 *
 *	Version: $Id: igmp.c,v 1.6 2007/10/09 12:22:34 jiunming Exp $
 *
 *	Authors:
...
static int igmp_send_report(struct net_device *dev, u32 group, int type)
{
	...
	if (u) {
		...
		// This packet should not be looped back. We tag it PACKET_OUTGOING
		// in order not to be looped back on ip_mc_output() (ip_output.c)
