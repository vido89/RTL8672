...
int dev_queue_xmit(struct sk_buff *skb)
{
	...
	if (!enable_imq || (dev->priv_flags!=IFF_DOMAIN_WAN)) {
#endif
	q = dev->qdisc;
	if (q->enqueue) {
...
#endif //end of CONFIG_RTK_VOIP_RX_INPUT_QUEUE
...
static void net_rx_action(struct softirq_action *h)
{
	...
	for (;;) {
		...
		// Mason Yu. Because we have NAT and DiffServ use tc's u32 classfier, it must use orignal Src IP to parser tc rule.
		memcpy(&(skb->srcIP), skb->nh.raw+12, 4);		
		{
			...
			if (skb->dev->br_port != NULL &&
				...
				// Kaohj
				//if (!skb->dev->brpppoe ||
				if ((!skb->dev->brpppoe && (!(skb->dev->priv_flags & IFF_DOMAIN_WAN) || memcmp(skb->mac.ethernet->h_dest, skb->dev->dev_addr, ETH_ALEN))) ||
				    ((skb->mac.ethernet->h_proto == 0x8863 ||
				      skb->mac.ethernet->h_proto == 0x8864) &&
				     memcmp(skb->mac.ethernet->h_dest, skb->dev->dev_addr, ETH_ALEN)) &&
				     skb->pkt_type != PACKET_LOOPBACK ) {  // ebtables
				     	int ret; //ebtables
				     	
				     	// handle_bridge(skb, pt_prev);  // org
					ret=handle_bridge(skb, pt_prev);  // ebtables
					//if (br_handle_frame_hook(skb) == 0) //ebtables
					br_handle_frame_hook(skb); //ebtables
					//	return ret; // ebtables
					//pt_prev = NULL; // ebtables
					dev_put(rx_dev);
					continue;
				}
			}
...
int dev_change_flags(struct net_device *dev, unsigned flags)
{
	...
	if (m_flags=(old_flags^flags)&(IFF_BROADCAST | IFF_POINTOPOINT))	/* Bits is different  ? */
	{
		dev->flags ^= m_flags;
	}
	/*
...
static int dev_ifsioc(struct ifreq *ifr, unsigned int cmd)
{
	...
	switch(cmd) 
	{
		case SIOCGIFFLAGS:	/* Get interface flags */
			...
			// Mason Yu. for link status check
			if ( strcmp("eth0", ifr->ifr_name) == 0 ) {
				int status;
				
				//printk("get eth0 flags\n");
				status = link_test(0);
				//printk("status=%d\n", status);
				if ( status == 0 )
			...
			if ( strncmp(ifr->ifr_name, "eth0_sw", 7) == 0 ) {
				int status, idx;
				
				idx = ifr->ifr_name[7] - '0' + 1;
				status = link_test(idx);
				if ( status >= 1 )
					ifr->ifr_flags |= IFF_RUNNING;
				return 0;
			}
			// end of link state check
		...
		default:
			if ((cmd >= SIOCDEVPRIVATE &&
			    ...
			    cmd == SIOCGMEDIALS ||
			    cmd == SIOCETHTEST  || 
			    cmd == SIOCSIPQOS) {   // Mason Yu. For Set IPQOS
				if (dev->do_ioctl) {
...
int dev_ioctl(unsigned int cmd, void *arg)
{
	...
	switch(cmd) 
	{
		...
		case SIOCGMEDIALS:
		case SIOCETHTEST:
		case SIOCSIPQOS: // Mason Yu. For Set IPQOS
			if (!capable(CAP_NET_ADMIN))
				return -EPERM;
			dev_load(ifr.ifr_name);
			dev_probe_lock();
		...
		extern int br_netfilter_onoff( char * arg );
		case SIOCBRIDGENF:
		// Mason Yu Test
		#if 1	
			// Added by Mason Yu for brnf enable/disable buttom
			 /*patch from linux 2.4*/
			if( cmd==SIOCBRIDGENF ) printk( "\nit's SIOCBRIDGENF ioctl\n" );
			ret = br_netfilter_onoff((char *) arg);
			return ret;
		#else
			if (strcmp( (char *)arg, "off") == 0 ) {	
