 ...
 *
 *	Version:	$Id: skbuff.c,v 1.22 2008/11/27 08:06:49 ql Exp $
 *
 *	Fixes:	
...
__NOMIPS16
#endif
__IRAM static /*__inline__*/ struct sk_buff *skb_head_from_pool(void)
{
...
__NOMIPS16
#endif
__IRAM static /*__inline__ */void skb_head_to_pool(struct sk_buff *skb)
{
	struct sk_buff_head *list = &skb_head_pool[smp_processor_id()].list;
...
#define DATA_BUFFER_SIZE (1600+24)//modified for tkip issue
#define DATA_POOL_SIZE 64
static u8 *private_data_pool[DATA_POOL_SIZE];
static int pvt_data_pool_producer=0;
static int pvt_data_pool_consumer=DATA_POOL_SIZE-1;
//get preallocated data bfr
#ifdef CONFIG_RTL867X_KERNEL_MIPS16_NET
__NOMIPS16
#endif
__IRAM static u8 *get_skb_data_bfr(void)
{
	//jim add context protection to avoid...
	unsigned int flags;
	local_irq_save(flags);
  int tmp=(pvt_data_pool_consumer+1)&(DATA_POOL_SIZE-1);
  u8 *data=NULL;
  
    if (tmp!=pvt_data_pool_producer) {
    	data = private_data_pool[tmp];
    	pvt_data_pool_consumer = tmp;
    	//printk("get p:%d c:%d\n", pvt_data_pool_producer, pvt_data_pool_consumer);
    };
	local_irq_restore(flags);
    return data;
}

//return preallocated data bfr
#ifdef CONFIG_RTL867X_KERNEL_MIPS16_NET
__NOMIPS16
#endif
static int put_skb_data_bfr(u8 *data)
{
	unsigned int flags;
	local_irq_save(flags);
  int tmp=(pvt_data_pool_producer+1)&(DATA_POOL_SIZE-1);
  int rtn_val=0;
...
/*__IRAM */struct sk_buff *alloc_skb(unsigned int size,int gfp_mask)
{
	struct sk_buff *skb;
	u8 *data;
	size+=8;//for wlan tkip
				  ...
				  unsigned long flags)
{
	...
	skb->tc_index = 0;
#endif
	skb->src_port = IF_NONE;
// Kaohj
	// Kaohj
	skb->switch_port = 0;
	//memset( skb->switch_port, 0, sizeof(skb->switch_port) ); //jiunming, set to default
	skb->vlan_member = 0;
	skb->vlan_passthrough=0;
	skb->from_dev = NULL;
	skb->pvid = 0;
}
...
__IRAM static void skb_release_data(struct sk_buff *skb)
{
	if (!skb->cloned ||
...
// Kaohj
	...
	//memcpy(n->switch_port, skb->switch_port, sizeof(skb->switch_port));
	C(vlan_member);
	C(vlan_passthrough);
	C(from_dev);
	C(pvid);
