...
next_knode:
	if (n) {
		...
		//  key->off == 12 , it means that it is a Src IP.		
		if ( key->off == 12 ) {			
			memcpy(&old_srcIP, skb->nh.raw+12, 4);
			memcpy(skb->nh.raw+12, &(skb->srcIP), 4);
		...
		for (i = n->sel.nkeys; i>0; i--, key++) {
			if ((*(u32*)(ptr+key->off+(off2&key->offmask))^key->val)&key->mask) {
				...
				// Mason Yu. We should reset the Src IP in skb.
				if ( key->off == 12 ) {			
					memcpy(skb->nh.raw+12, &old_srcIP, 4);
				}
				goto next_knode;
			}
