...
// Kaohj
static int real_init_squashfs_fs(void)
{
#ifdef CONFIG_SQUASHFS_LZMA	
...
static void real_exit_squashfs_fs(void)
{
#ifndef CONFIG_SQUASHFS_LZMA
...
static int fsinit_read_proc(char *page, char **start, off_t off,
...
static int fsinit_write_proc(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
      if (count < 2)
         return -EFAULT;
      if (buffer && !copy_from_user(&init_flag, buffer, 1)) {
      	 if (init_flag == '1') {
      	 	real_exit_squashfs_fs();
      	 	real_init_squashfs_fs();
      	 }
      	 init_flag = '0';
         return count;
         }
...
static int __init init_squashfs_fs(void)
{
	...
	res = create_proc_entry("fs_init", S_IFREG|S_IRUGO|S_IWUSR, proc_root_fs);
        if (res) {
                res->read_proc=fsinit_read_proc;
	        res->write_proc=fsinit_write_proc;
	}
	
	return real_init_squashfs_fs();
/*
#ifdef CONFIG_SQUASHFS_LZMA	
	printk(KERN_INFO "Squashfs 2.1-r2 (released 2004/12/15) (C) 2002-2004 Phillip Lougher\n");
	printk(KERN_INFO "LZMA decompressor support based on LZMA SDK 4.05 by Oleg I.Vdovikin\n");
