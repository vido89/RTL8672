...
volatile struct packet p;
volatile int d;

#ifdef CONFIG_CRAMFS
char root[40]="root=/dev/mtdblock2";
#endif
...
#ifndef NO_MM //command 	
	printk("Kernel command line: %s\n", saved_command_line);
	parse_options(command_line);
#endif	
#ifdef CONFIG_CRAMFS
	command_line=root;	
	printk("commad_line %s\n",command_line);
	parse_options(command_line);
#endif
	trap_init();
	/*
...
*/
	...
	/*hrchen, switch ISR from 0xBFC00180 to 0x80000080 */
...
static int init(void * unused)
{
	...
	free_initmem();
	// Kao added
	// Get rid of the boot loader segment.
	//free_btcodemem();
	unlock_kernel();
...
extern asmlinkage long sys_mount(char * dev_name, char * dir_name, 
	char * type, unsigned long new_flags, void * data);
...
#endif
#ifdef CONFIG_RAMFS
        if ((err=sys_mount("ramfs","/var","ramfs",flags,NULL)))
...
//	execve("/etc/rc",argv_init,envp_init);
