...
#else
uint32 rtpHold[MAX_SESS_NUM] = {0};
uint32 rtpConfigOK[MAX_SESS_NUM] = {0};
#endif

#ifndef AUDIOCODES_VOIP
uint32 nTxRtpStatsCountByte[MAX_SLIC_CH_NUM];
uint32 nTxRtpStatsCountPacket[MAX_SLIC_CH_NUM];
#endif
...
struct RTK_TRAP_profile Rtp_send[MAX_SLIC_CH_NUM][FIFO_NUM];
char Rtp_fifo[MAX_SLIC_CH_NUM][FIFO_NUM*512];
static unsigned long length = sizeof(struct RTK_TRAP_profile);
static unsigned int pload_len = 0;
static unsigned char rtp_w[MAX_SLIC_CH_NUM] = {0}, rtp_r[MAX_SLIC_CH_NUM] = {0};
static unsigned char pload_w[MAX_SLIC_CH_NUM] ={0}, pload_r[MAX_SLIC_CH_NUM] = {0};

#endif

static void send(struct RTK_TRAP_profile *ptr, void *ptr_data, Tuint32 data_len)
{
	Tuint8 *tmp;
	Tuint32 tos;
	Tsudphdr *udphdr;
...
//	printk("enter send function\n");
	...
	ProfileDump(PROFILE_INDEX_TEMP, PROFILE_INDEX_TEMP,1000);
	restore_flags(flags);
#endif	
	if (stat) {
...
#endif	
	if((rst = ip_route_output(&rt, ptr->ip_src_addr, ptr->ip_dst_addr,(Tuint32)(RT_TOS(tos)), 0)))
...
/* ip */
	...
	iphdr->check = ip_fast_csum((Tuint8 *)(iphdr), 5);
...
#if defined (AUDIOCODES_VOIP)
void rtp_send_aux(Tuint32 ip_src_addr, Tuint32 ip_dst_addr, Tuint16 udp_src_port, Tuint16 udp_dst_port, void *ptr_data, Tuint32 data_len)
{
...
{	
	unsigned char chid;
	unsigned long flags;
        //unsigned char rtp_w_now[MAX_SLIC_CH_NUM], pload_w_now[MAX_SLIC_CH_NUM];
...
void system_process_rtp_tx(unsigned char CH, unsigned int sid, void *ptr_data, unsigned int data_len)
