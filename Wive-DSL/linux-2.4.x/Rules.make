...
%.o16: %.c
	#$(CC) $(CFLAGS) $(EXTRA_CFLAGS_nostdinc) $(EXTRA_MIPS16) -DKBUILD_BASENAME=$(subst $(comma),_,$(subst -,_,$(*F))) $(CFLAGS_$@) -S $< -o $@.s 
	$(CC) $(CFLAGS) $(EXTRA_CFLAGS_nostdinc) $(EXTRA_MIPS16) -DKBUILD_BASENAME=$(subst $(comma),_,$(subst -,_,$(*F))) $(CFLAGS_$@) -c -o $@ $<
	@ ( \
	    echo 'ifeq ($(strip $(subst $(comma),:,$(CFLAGS) $(EXTRA_CFLAGS_nostdinc) $(CFLAGS_$@))),$$(strip $$(subst $$(comma),:,$$(CFLAGS) $$(EXTRA_CFLAGS_nostdinc) $$(CFLAGS_$@))))' ; \
	    echo 'FILES_FLAGS_UP_TO_DATE += $@' ; \
	    echo 'endif' \
...
ifdef CONFIG_RTL867X_KERNEL_MIPS16
	echo $(obj-y) >.objlist
	$(TOPDIR)/scripts/mkdep -mips16 .objlist $(CFLAGS) $(EXTRA_CFLAGS_nostdinc) -- $(wildcard *.[chS]) > .depend
	rm -f .objlist
else
