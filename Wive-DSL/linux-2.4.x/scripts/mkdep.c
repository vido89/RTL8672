...
static int getObjList(const char* filename, char* buffer, int length)
{
    FILE* fp;
    int retval;
    ...
    retval=fread(buffer, 1, length, fp);
    fclose(fp);
    if( retval<=0)
	 return -1;
     else 
...
int main(int argc, char **argv)
{
	int len;
	const char *hpath;
	const char *objListFile=NULL;
	char   objList[LISTSIZE];
	int mips16=0;
	...
	while (++argv, --argc > 0) {
		...
		else if (strncmp(*argv, "-mips16 ", 7) == 0) {
			mips16=1;
			--argc;
			++argv;
			objListFile=*argv;							
	...
	if(mips16==1)
	{
          getObjList(objListFile, objList, LISTSIZE);
	}
	...
	while (--argc > 0) {
		...
		if (len > 2 && filename[len-2] == '.') {
			if (filename[len-1] == 'c' || filename[len-1] == 'S') {
				...
				if(mips16==1)
				{ 						 
				   depname[len+2]= depname[len];
				   depname[len] = '1';
				   depname[len+1] = '6';
				   if((strstr(objList, depname))==NULL)
				   {				  				      
				      depname[len] =  depname[len+2]; 								
				   }
				}
			    g_filename = filename;
			    command = "";
