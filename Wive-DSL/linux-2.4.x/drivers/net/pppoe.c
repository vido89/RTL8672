...
//		sock_queue_rcv_skb(sk, skb);
		if (sock_queue_rcv_skb(sk, skb))	// linux-2.4.25 patch
			goto abort_kfree;
	}
...
int pppoe_fastpath(struct sk_buff *skb)
{
	...
	sk = po->sk;
	bh_lock_sock(sk);
	if (sk->lock.users != 0 || !(sk->state & PPPOX_BOUND)) return -2;	
	pch = &sk->protinfo.pppox->chan;
		  ...
		  int sockaddr_len, int flags)
{
	...
	if (po->pppoe_pa.sid) {
		...
		if(po->pppoe_dev)	// linux-2.4.22 patch
			dev_put(po->pppoe_dev);
	...
	return error;
err_put:
	if (po->pppoe_dev) {	// linux-2.4.23 patch
		dev_put(po->pppoe_dev);
		po->pppoe_dev = NULL;
	}
	goto end;
