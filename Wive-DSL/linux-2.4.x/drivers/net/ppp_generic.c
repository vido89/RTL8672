...
int
ppp_fastpath(struct ppp_channel *chan, struct sk_buff *skb)
{
...
// Kaohj -- remove ppp header, update skb->dev for fastpath
int pppoa_fastpath(struct ppp_channel *chan, struct sk_buff *skb)
{
	int ret=0;
	struct channel *pch = chan->ppp;
	struct ppp *ppp;
	...
	if (FastPath_Enter(skb)==1) {
		...
		// restore and back to the normal path
		skb_push(skb,2);
	}
...
#endif // of CONFIG_RTL867X_IPTABLES_FAST_PATH
#ifdef CONFIG_RTL867X_KERNEL_MIPS16_DRIVERS_NET
__NOMIPS16
#endif
void
