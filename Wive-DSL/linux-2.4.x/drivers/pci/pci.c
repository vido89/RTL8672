/*
 *	$Id: pci.c,v 1.3 2007/09/10 12:16:02 jiunming Exp $
 *
 ...
 *
 * 	History:
 *	2007/06/05 SH		Add rtl_pci_init to launch pci scan and pci_base_address_fix_up
 *				to make sure the correctness of BARs.
 *			
 */
...
#ifdef CONFIG_RTL867X_KERNEL_MIPS16_DRIVERS_PCI
__NOMIPS16
#endif
PCI_OP(read, byte, u8 *)
#ifdef CONFIG_RTL867X_KERNEL_MIPS16_DRIVERS_PCI
...
static void pci_read_bases(struct pci_dev *dev, unsigned int howmany, int rom)
{
	...
	for(pos=0; pos<howmany; pos = next) {
		...
		if ((l & PCI_BASE_ADDRESS_SPACE) == PCI_BASE_ADDRESS_SPACE_MEMORY) {
			...
			DBG("memory space, res start %x sz %x\n", res->start, sz);	
		} else {
			res->start = l & PCI_BASE_ADDRESS_IO_MASK;
			res->flags |= l & ~PCI_BASE_ADDRESS_IO_MASK;
			sz = pci_size(sz, PCI_BASE_ADDRESS_IO_MASK & 0xffff);
			DBG("io space, res start %x sz %x\n", res->start, sz);		
		}
...
#ifdef CONFIG_PCI
DBG("Before pci_scan_slot, make sure BAR setting is correct\n");
