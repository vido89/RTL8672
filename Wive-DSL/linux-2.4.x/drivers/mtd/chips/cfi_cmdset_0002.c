 ...
 * This code is GPL
 *
 * $Id: cfi_cmdset_0002.c,v 1.3 2007/09/10 12:03:49 jiunming Exp $
 *
 */
...
static int do_write_oneword(struct map_info *map, struct flchip *chip, unsigned long adr, __u32 datum, int fast)
{
 ...
 retry:
	...
	for (Count = 3; Last[(Count - 1) % 4] != Last[(Count - 2) % 4] && Count < 500000; Count++){
		cfi_spin_unlock(chip->mutex);
		cfi_udelay(10);
		cfi_spin_lock(chip->mutex);
...
static int cfi_amdstd_erase_varsize(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	*/
	...
	//10/14/05' hrchen, disable for erase size seldom to align to erase sector boundary
	...
	while(len) {
		...
		if (len<0) 
		    break;  //10/14/05' hrchen, sectors are all erased
