...
static struct notifier_block nettel_notifier_block = {nettel_reboot_notifier, NULL, 0 };
...
mod_init_t init_nettel(void)
{
	...
	cpu_mmcrp = (void *) ioremap_nocache(0xfffef000, 4096);
	if (cpu_mmcrp == NULL) {
	...
	*((unsigned char *) (cpu_mmcrp + 0xc64)) = 0x01;

	amdpar = (volatile unsigned long *) (cpu_mmcrp + 0xc4);

#ifdef CONFIG_MTD_CFI_INTELEXT
	intelboot = 0;
	intel0cs = SC520_PAR_ROMCS1;
	intel0par = (volatile unsigned long *) (cpu_mmcrp + 0xc0);
	intel1cs = SC520_PAR_ROMCS2;
	intel1par = (volatile unsigned long *) (cpu_mmcrp + 0xbc);
	...
	*amdpar = SC520_PAR(SC520_PAR_BOOTCS, amdaddr, maxsize);
	...
	if ((amd_mtd = do_map_probe("jedec_probe", &nettel_amd_map))) {
		...
		if (!orig_romcs1par) {
			...
			intel0par = (volatile unsigned long *) (cpu_mmcrp + 0xc4);
			intel1cs = SC520_PAR_ROMCS1;
			intel1par = (volatile unsigned long *) (cpu_mmcrp + 0xc0);
			...
			intel1par = (volatile unsigned long *) (cpu_mmcrp + 0xc4);
...
mod_exit_t cleanup_nettel(void)
{
#ifdef CONFIG_MTD_CFI_INTELEXT
...
module_init(init_nettel);
module_exit(cleanup_nettel);
