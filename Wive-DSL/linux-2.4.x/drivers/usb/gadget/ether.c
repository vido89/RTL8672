...
//		skb->protocol = eth_type_trans (skb, dev->net);	//cathy
		 ...
		 */
#if 0 //loopback test		
		if(skb->data[12] == 0x86 && skb->data[13] == 0x72) {
...
struct sk_buff *
skb_realloc_headroom2(struct sk_buff *skb, unsigned int headroom)
{
	struct sk_buff *skb2;
...
static struct usb_request *eth_req_alloc (struct usb_ep *, unsigned, int);
...
#ifdef OLD_USBLED_STYLE
	if(IS_6085 || IS_RLE0315)
		REG32(USBLED) = 0x0000048f;
#else 
...
eth_req_alloc (struct usb_ep *ep, unsigned size, int gfp_flags)
{
	struct usb_request	*req;
...
eth_bind (struct usb_gadget *gadget)
{
	...
	if (iSerialNumber) {
		...
		strncpy(serial_number, iSerialNumber, sizeof serial_number);
	}
