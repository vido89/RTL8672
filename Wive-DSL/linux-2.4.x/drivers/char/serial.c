...
struct serial_state rs_table[1] = { 
	 SERIAL_PORT_DFNS	
};
...
static struct timer_list sendbuf_timer;
static struct timer_list  polltimer;
//#define SETNETLOGGER 0x5495               /*if should be used for ioctrl*/
#define LOG_BUF_LEN	(32*1024)			/* This must be a power of two */
#define LOG_BUF_MASK	(LOG_BUF_LEN-1)
static char log_buf[LOG_BUF_LEN];
#define LOG_BUF(idx) (log_buf[(idx) & LOG_BUF_MASK])
static unsigned long net_start;			/* Index into log_buf: next char to be sent to net */
static unsigned long log_end;			/* Index into log_buf: most-recently-written-char + 1 */
static spinlock_t logbuf_lock = SPIN_LOCK_UNLOCKED;
static spinlock_t console_lock = SPIN_LOCK_UNLOCKED;
/*send data by nic*/
static unsigned short ip_seq;
static int serial_irq;
static int txflag;
static unsigned int mibsip;
static char smac[MAC_ADDR_LEN];
static int Pollflag;
static struct tq_struct netsendwork;
static unsigned long SIP;
static unsigned long DIP;
#if 1
static char netlogpkthead[]={/*00*/       0xff, 0xff, 0xff, 0xff, 0xff, 0xff,      /*dmac*/
                                           /*06*/      0x00, 0x00, 0x00,0x00,0x00,0x00,  /*smac*/
                                           /*12*/      0x08, 0x00,                             /*ip*/
                                           /*14*/      0x45, 0x00,                             /*v4,length 20*/
                                           /*16*/      0x00, 0x00,                             /*ip total length*/    
                                           /*18*/      0x00, 0x00,                             /*idectification*/ 
                                           /*20*/      0x00, 0x00,                             /*fragment and fragment offset*/
                                           /*22*/      0x80,                                     /*time to live*/
                                           /*23*/      0x11,                                     /*udp*/
                                           /*24*/      0x00, 0x00,                               /*checksum if want hw compute checksum,now it should not be zero*/
                                           /*26*/      0xc0, 0xa8, 0x01, 0x01,                /*sip*/
                                           /*30*/      0xc0, 0xa8, 0x01, 0xff,                /*dip*/
                                           /*34*/      0x12, 0x34,                               /*s port*/
                                           /*36*/      0x12, 0x34,                               /*d port*/
                                           /*38*/      0x00, 0x00,                               /*udp total length*/
                                           /*40*/      0x00, 0x00,                               /*udp check sum.hw do should not be zero*/
                                           };
...
#define IP_SEQ 18
#define UDP_LEN 38
#define SMAC_OFFSET 6
#define BDINFO_ADDR 0xbfc04000
#define SET_IP_LEN(x) (*(unsigned short*)(netlogpkthead+IP_LEN) =x)
#define SET_IP_SEQ(x) (*(unsigned short*)(netlogpkthead+IP_SEQ) =x)
#define SET_UDP_LEN(x) (*(unsigned short*)(netlogpkthead+UDP_LEN)=x)


#if 0
static int set_smac(void)
{
	char* ptr=HW_CONFIG_SETTING;
	char mac[MAC_ADDR_LEN];
	if( memcmp(ptr, HS_CONF_SETTING_SIGNATURE_TAG, SIGNATURE_LEN) == 0 ) {
		  HW_MIB_Tp pHWMIB;
	    ptr+=sizeof(PARAM_HEADER_T);
	    pHWMIB = (HW_MIB_Tp)ptr;
		memcpy(mac, pHWMIB->elanMacAddr, MAC_ADDR_LEN);
	  
	}else
		memcpy(mac, BOARD_PARAM_MAC, MAC_ADDR_LEN);
	netlogpkthead[6]=mac[0];
	netlogpkthead[7]=mac[1];
	netlogpkthead[8]=mac[2];
	netlogpkthead[9]=mac[3];
	netlogpkthead[10]=mac[4];	
	netlogpkthead[11]=mac[5];

	return 0;
}
#endif
static int set_smac(char *mac)
{
	netlogpkthead[6]=mac[0];
	netlogpkthead[7]=mac[1];
	netlogpkthead[8]=mac[2];
...
static int set_ip(unsigned int boardip)
{

   
//read IP of runtime config
	char *ptrIP = CURRENT_CONFIG_SETTING;
       char  sip[IP_ADDR_LEN];
	char  dip[IP_ADDR_LEN];
	
	sip[0] =   *(char *)(&boardip);
	sip[1] =   *(((char *)(&boardip))+1);
	sip[2] =   *(((char *)(&boardip))+2);
	sip[3] =   *(((char *)(&boardip))+3);
	
       
	netlogpkthead[26]=sip[0];
	netlogpkthead[27]=sip[1];
	netlogpkthead[28]=sip[2];
	netlogpkthead[29]=sip[3];
	dip[0] = sip[0];
	dip[1] = sip[1];
	dip[2] = sip[2];
	dip[3] = 255;
	netlogpkthead[30]=dip[0];
	netlogpkthead[31]=dip[1];
	netlogpkthead[32]=dip[2];
	netlogpkthead[33]=dip[3];
	DIP = *((unsigned long *)dip);
	SIP = *((unsigned long *)sip);
	 
	return 0;
}


#ifdef CONFIG_RTL867X_KERNEL_MIPS16_DRIVERS_CHAR
__NOMIPS16
#endif
static void emit_char(char c)
{
	...
	LOG_BUF(log_end) = c;
	log_end++;
	if (log_end - net_start > LOG_BUF_LEN)
		net_start = log_end - LOG_BUF_LEN;
	spin_unlock_irqrestore(&logbuf_lock, flags);
}

void receive_buffers(struct async_struct *info,	const char *buf, unsigned int  len);
...
{	
	...
	unsigned long flags;
	 if(!IRQ_ports[serial_irq])
	 	return;
	  spin_lock_irqsave(&console_lock, flags);
	 info=(struct async_struct *)IRQ_ports[serial_irq];
	 /*maybe need to mutex*/
	 /*windows CLRF is \n\r now only need \n so buf len should be len-1*/
	  
	 receive_buffers(info,buf,len-1);
	  spin_unlock_irqrestore(&console_lock, flags);
}
//EXPORT_SYMBOL(console_input);

 void receive_buffers(struct async_struct *info,
...
void bit_reverse(char *buf,unsigned long len)
{
     char* tempbuf=buf;
     while(len-- >0)
     {
...
__NOMIPS16
#endif
int nic_send(const char *buf, unsigned long len)
{
	unsigned short length;
	struct sk_buff *skb;
	struct net_device *dev;
	if(!(skb=dev_alloc_skb(1600)))
		return -1;
	...
	ip_seq++;
	SET_IP_SEQ(ip_seq);
	length+=14; /*mac header*/

       skb_put(skb,length);
	memcpy(skb->data, netlogpkthead, sizeof(netlogpkthead));	
	bit_reverse(buf,len);
	/*jim copy udp data to skb*/
	memcpy(skb->data+sizeof(netlogpkthead), buf, len);
	/*calc udp csum*/
//	skb->csum=csum_tcpudp_nofold(0xc0a80101, 0xc0a801ff, 8+len, 17, 0);
	skb->csum=csum_tcpudp_nofold(SIP, DIP, 8+len, 17, 0);
	*(unsigned short*)(skb->data+14+20+6) = csum_fold(skb_checksum(skb, 14+20, len+8, skb->csum));
	/*calc ip csum*/
	*(unsigned short*)(skb->data+14+10) = csum_fold(skb_checksum(skb, 14, 20, 0));
	dev=dev_get_by_name("eth0");
	if(dev->hard_start_xmit(skb, dev))
		dev_kfree_skb(skb); //drop packet	

	return 0;
}
//EXPORT_SYMBOL(nic_send);

int __call_net_send(unsigned long start, unsigned long end)
{
        int err;
        err =nic_send(&LOG_BUF(start),end-start);
        return err;
}
int _call_net_send(unsigned long start, unsigned long end)
{
	if (start != end) {
		if ((start & LOG_BUF_MASK) > (end & LOG_BUF_MASK)) {
			/* wrapped write */
			__call_net_send(start & LOG_BUF_MASK, LOG_BUF_LEN);
			__call_net_send(0, end & LOG_BUF_MASK);
		} else {
			__call_net_send(start, end);
		}
	}
}
#ifdef CONFIG_RTL867X_KERNEL_MIPS16_DRIVERS_CHAR
__NOMIPS16
#endif
int call_net_send(unsigned long start, unsigned long end,unsigned int ISTIMEOUT)
{
	...
	start_print = start;
        ...
        while((cur_index+1000) < end ){
                 
                cur_index += 1000;
                _call_net_send(start_print, cur_index);
                start_print = cur_index;
		   /*if send packet too fast and too much.client will lost packet*/
	        spin_lock_irqsave(&logbuf_lock, flags);
                net_start=cur_index;
                spin_unlock_irqrestore(&logbuf_lock, flags);
	        mdelay(20);
	...
	if(ISTIMEOUT){
	        _call_net_send(start_print, end);
	        spin_lock_irqsave(&logbuf_lock, flags);
                net_start=end;
                spin_unlock_irqrestore(&logbuf_lock, flags);
	}
...
__NOMIPS16
#endif
void need_net_send(unsigned int ISTIMEOUT)
{
	unsigned long flags;
	unsigned long _net_start, _log_end;
//alex_huang
        ...
        _log_end = log_end;
        spin_unlock_irqrestore(&logbuf_lock, flags);
        if(_net_start < _log_end)
            call_net_send(_net_start,_log_end,ISTIMEOUT);
...
*/
//EXPORT_SYMBOL(start_netlog_tx);

static void sendlogbuf_function(unsigned long data)
{
     need_net_send(1);
//    printk("timer out\n");
     if(txflag)
       mod_timer(&sendbuf_timer,jiffies+20*HZ/100);
}

static void poll_function(unsigned long data)
{
     if(txflag){  	
        if(Pollflag==0){
   	      txflag=0;
              printk("netlogger is closed\n");
        }else {
          Pollflag=0;
          mod_timer(&polltimer,jiffies+200*HZ);
        }
     }  
}
int netlog_init()
{
     printk("netlog start\n");
     INIT_TQUEUE(&netsendwork,net_send_task,NULL);
   //  set_smac();
     //set_ip();
     sendbuf_timer.data=0;
     sendbuf_timer.function=sendlogbuf_function;
     polltimer.data=0;
     polltimer.function = poll_function;
				...
				int value)
{
        ...
        if(UART_TX == offset)
        {
       		emit_char((unsigned char)value);
			if((unsigned char)value=='\n' && txflag /*||(unsigned char)value == '#'*/ )
				queue_task(&netsendwork,&tq_serial);
        }
#endif
		    ...
		    unsigned int cmd, unsigned long arg)
{
...
#ifdef CONFIG_RTL867X_NETLOG
      int length;
      memset((void *)&netlogIOcmd, 0, sizeof( struct netlogIO));  
#endif
	...
	switch (cmd) {
		...
		case TIOCSERSWILD:
			...
			return 0;
#ifdef CONFIG_RTL867X_NETLOG  
           /*add by alex_huang for userspace ioctl */
		case NLGSETNETLOGGER:
			 if(copy_from_user(&txflag,(void *)arg,sizeof(int)))
			      return -EFAULT;
			 if(txflag){
				printk("netlogger is up\n");
				
				mod_timer(&sendbuf_timer,jiffies+30*HZ/100);
				mod_timer(&polltimer,jiffies+200*HZ);
				need_net_send(0);					
				}
				break;
		case NLGSENTDATA:
			if(copy_from_user(&netlogIOcmd,(void*)arg,sizeof(struct netlogIO))){
			  	return -EFAULT;
			}
			if(txflag)
			       console_input(netlogIOcmd.netlog_recv_buf+6,netlogIOcmd.buflen-6);
			break;			  
		...
		case NLGSENDCMD:
		      ...
		      if(copy_from_user(&netlogIOcmd,(void*)arg,sizeof(struct netlogIO))){
			  	return -EFAULT;
                    ...
                    if(txflag){
			   length=nic_send(netlogIOcmd.netlog_recv_buf,netlogIOcmd.buflen);
			   }
			break;
              case NLGSENDPOLL:
                      Pollflag =1;
			 break;
		case NLGSETIP:
			  if(copy_from_user(&mibsip,(void*)arg,sizeof(mibsip))){
			  	return -EFAULT;
		      	}
			set_ip(mibsip);
			break;
...
#ifdef CONFIG_RTL8601
	state->baud_base = CPU_CLOCK_RATE*3/16;
#else
...
#endif
	info->io_type = state->io_type;
	info->iomem_base = state->iomem_base=(char*)UART_REGS_BASE;
	info->iomem_reg_shift = state->iomem_reg_shift=2;
#if defined(CONFIG_RTL8671)
	quot = state->baud_base / baud-1;//tylo
#else
