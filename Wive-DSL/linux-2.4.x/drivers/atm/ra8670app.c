...
void  Alloc_desc(sar_private *cp)
{
	...
	size=sizeof(RV_CMD_DESC)*SAR_RX_RING_SIZE*(Enable_VC_CNT+1)+Desc_Align; 
	tmp=kmalloc(size, GFP_KERNEL);	
	cp->pRVDescBuf=(char *)tmp;
	cp->RVDesc=(RV_STS_DESC *) (((UINT32)((UINT32)tmp+Desc_Align)&0xFFFFFF00)|Uncache_Mask );	
	memset(cp->RVDesc, 0 , size);
...
{
  int val, timeRatioPCR,pkt_exp=1,j;
...
extern void EnterHeavyTraffic();
extern void ExitHeavyTraffic();
...
#ifdef CONFIG_FLOW_CTRL
	TDFI_Value=REG32(SAR_TDFI_Addr);
	for(i=0;i<Enable_VC_CNT ;i++){
...
#else
	extern struct net_device *imq_devs;
	extern int enable_imq;
	if (enable_imq && imq_devs) {
		int avail_desc;
		
		avail_desc = (cp->vcc[ch_no].TV.desc_pf- 1 - cp->vcc[ch_no].TV.desc_pw + SAR_TX_DESC_NUM)&(SAR_TX_DESC_NUM-1);
		if (netif_queue_stopped(imq_devs)) {
			if (avail_desc >= SAR_TX_THRESHOLD)
				netif_wake_queue(imq_devs);
		}
	} else {
		int avail_desc;
...
void sendTERM() {
	...
	for (i=0; i<Enable_VC_CNT; i++) {
		if (cp->vcc[i].created==VC_CREATED) { // fake PPPoA TERM-REQ
			...
			memcpy(skb_put(skb, TERM_LEN), ppp_term, TERM_LEN);
...
static unsigned long  crc32tab[256] __SWAP_DATA =
{
...
int OutBandAAL5Rx(struct atm_vcc *atm_vcc, char *rx_bfr)
{
  ...
  unsigned int upushAddr;
	...
	//last cell
    crc = crc32(crc, outband_rx_skb->data, outband_rx_skb->len)^0xc704dd7b;
    if (crc) {  //crc error
