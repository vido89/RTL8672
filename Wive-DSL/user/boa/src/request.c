...
#if defined(WLAN_WEB_REDIRECT) || defined(QSETUP_WEB_REDIRECT) || defined(WEB_REDIRECT_BY_MAC) || defined(_SUPPORT_CAPTIVEPORTAL_PROFILE_)
#include "./LINUX/mib.h"
#endif
#endif
//ql
...
extern int qsetup_server_s;
void get_qsetup_request(void)
{
	request *new_req;
	new_req = get_sock_request(qsetup_server_s);
	if(new_req)
		new_req->request_qsetup = 1;
}
#endif
...
int process_header_end(request * req)
{
	...
	if(req->request_qsetup == 1) {
		...
		if (!adsl_drv_get(RLCM_GET_LINK_SPEED, (void *)&vLs, RLCM_GET_LINK_SPEED_SIZE) || vLs.upstreamRate == 0)
				sprintf(tmpbuf, "http://%s/popup.asp", ipaddr);	// No ADSL link page
		else
				sprintf(tmpbuf, "http://%s/qsetup.asp", ipaddr);	// PPPoE dial up fail page
