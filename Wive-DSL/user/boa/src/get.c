...
int init_get2(request * req)
{
	...
	if (strstr(req->request_uri, "fc-page1-ch.asp"))
	{//clear fast configure parameter
		memset(&fcEntry, 0, sizeof(fcEntry));
		memset(&fcEntry2, 0, sizeof(fcEntry2));
		fcEntry.ipDhcp = 1;
		fcEntry.napt = 1;
		fcEntry.dgw = 1;
	}
#endif
#if defined(ZTE_531B_BRIDGE_SC) || defined(ZTE_GENERAL_ROUTER_SC)
	if (strstr(req->request_uri, "status.asp"))
		{
		websRedirect( req, "status_sc.asp");
		return 0;
		}
	#endif
	...
	if (getAccPriv(req->user) != (int)PRIV_ROOT && strstr(req->request_uri, "userconfig.asp")) {
		websRedirect( req, "/admin/user-password.asp");
		return 0;
	}
#endif
	...
	if (strstr(req->request_uri, "status.asp")) {
		websRedirect( req, "status_pl.asp");
		return 0;
	}
#endif
