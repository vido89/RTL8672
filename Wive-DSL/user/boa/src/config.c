...
void c_add_digest			(char *v1, char *v2, void *t);
...
{
#ifdef USE_AUTH
	// v2 is in a form of path:realm, so break it.
	char *realm;

	realm = strchr(v2, ':');
	if (!realm) {
		fprintf(stderr, "Malformed arg: %s\n", v2);
		return;
	}
	*realm++ = '\0';
	auth_add_digest(v1, v2, realm);
#endif
}
#endif
