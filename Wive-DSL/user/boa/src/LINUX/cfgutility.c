...
int resetIGMProxy(int restart, char * pIfname)
{
	...
	if(restart){
		if(getIfModeByName(pIfname) <= ADSL_BR1483){
...
int setIgmpProxyCapability(char enabled, char * pIfname)
{
	...
	if(enabled){
		...
		if(getIfModeByIndex(ifIndex) <= ADSL_BR1483){
			printf("No such route interface!\n");
			return -1;
		}
...
int addGroupInterface(enum PortMappingGrp grp, char *pIfname)
{
	...
	ifnum = get_group_ifinfo(itfs, 16, PM_DEFAULTGRP);
	if(ifnum == 0){
...
int addPortForwarding(MIB_CE_PORT_FW_Tp pEntry)
{
	...
	if(pEntry->ifIndex != 0xff){
		if(getIfModeByIndex(pEntry->ifIndex) <= ADSL_BR1483){
			printf("no such route interface!\n");
			return -1;
		}
	}
...
int addRipInterface(MIB_CE_RIP_Tp pEntry)
{
	...
	if(pEntry->ifIndex != 0xff){
		if((pEntry->ifIndex == -1) || (getIfModeByIndex(pEntry->ifIndex) <= ADSL_BR1483)){
			printf("no such route interface!\n");
			return -1;
		}
	}
...
int addWanInterface(MIB_CE_ATM_VC_Tp pEntry)
{
	...
	do {
		...
		if (cnt == 0)	// pvc not exists
		{
			...
			pEntry->ConDevInstNum = 1 + findMaxConDevInstNum();
		...
		}else	// pvc exists
		{
			...
			if (Entry.cmode == ADSL_PPPoE && pEntry->cmode == ADSL_PPPoE)
			{
				...
				{	// get the pvc info.
					...
					pEntry->ConPPPInstNum = 1 + findMaxPPPConInstNum( Entry.ConDevInstNum );
#endif
				}
				else
				{
