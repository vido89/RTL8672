...
"\"lan1\", \"lan2\", \"lan3\", \"lan4\","
#else
#if defined(CONFIG_RTL867X_COMBO_PORTMAPPING)
"\"eth0\", "
#else
"\"lan0\", "
#endif
...
#if 0
#ifdef DHCPS_DNS_OPTIONS
	{"dns",		END,	CMD_DHCP_DNS,			0,					dhcp_dns_args,	cmdSetDhcpDnsOption,	listCommandArgs,		"DNS option"},
#endif
#endif
...
#endif
		entry.cdvt=0;
	} else if (entry.cmode == ADSL_PPPoE || entry.cmode == ADSL_PPPoA){ 	// PPP connection
		if(getArgValue(pCmdArg,"username",(void *)buf) == 1){	
#ifdef CONFIG_GUI_WEB
			if(strlen(buf) > P_MAX_NAME_LEN-1)
#else
...
static void cmdRestoreDefault(int cmdType, struct cmd_arg *pCmdArg)
{
	...
	printf("System is rebooting, please wait...\n");
	/*reboot*/
