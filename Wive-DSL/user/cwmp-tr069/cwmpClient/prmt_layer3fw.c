...
int setFwEntity(char *name, struct CWMP_LEAF *entity, int type, void *data)
{
	...
	}else if( strcmp( lastname, "Interface" )==0 )
	{
		...
		unsigned char newifindex;
		if( buf==NULL ) return ERR_9007;
		if( strlen(buf)==0 ) 
			fw->ifIndex=0xff;//return ERR_9007;
		else{
			newifindex = transfer2IfIndex( buf );
			if( newifindex==0xff ) return ERR_9007;
	...
	if(p)
	{
		...
		if(fp)
		{
			...
			while( fgets(buff, sizeof(buff), fp) != NULL )
			{
				...
				if(  (p->Enable) &&
				     ( (p->ifIndex==0xff) || (strcmp(ifname, ifname2)==0) ) && //interface: any or specific
