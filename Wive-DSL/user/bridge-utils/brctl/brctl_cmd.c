...
// Added by Mason Yu for brnf enable/disable buttom
void br_cmd_brnf(struct bridge *br, char *brnfflag, char *arg1)
{		
	//printf("brnfflag=%s  arg1=%s\n", brnfflag, arg1);	
	return ioctl(br_socket_fd, SIOCBRIDGENF, brnfflag);	
}

// Kaohj -- set wirelss blocking
void br_cmd_wlanblock(struct bridge *br, char *arg0, char *arg1)
{
	int mode;
	
	sscanf(arg0, "%i", &mode);
	br_set_wlan_block(br, mode);
}
...
#endif
	{0, 1, "brnf", br_cmd_brnf},   // Added by Mason Yu for brnf enable/disable buttom
	// Kaohj --- wireless blocking
	{1, 1, "wlanblock", br_cmd_wlanblock},
};
