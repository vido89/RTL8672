...
int parse_CTC_Vendor_Class(struct dhcpMessage *packet, unsigned char* option60, struct dhcp_ctc_client_info *pstClientInfo)
{
	...
	pstClientInfo->iCategory = CTC_Computer;
...
int sendOffer(struct dhcpMessage *oldpacket)
{
  ...
  //       		  printf("No host use the public IP\n"); 
...
#ifdef IP_BASED_CLIENT_TYPE
				&& ((lease->yiaddr==myipaddr)||((ip_addr != 0) || (!check_type(lease->yiaddr, devicetype))))
#endif
				// Mason Yu
	         	...
	         	{	         		
	         		goto getip;
		         	   ...
		         	   /* and it isn't on the network */
		    	     	     && !check_ip(req_align)
#ifdef IP_BASED_CLIENT_TYPE
					&& (!check_type(req_align, devicetype))
#endif
	    	     	     ) {	    	     	     		      	   	        
	...
	//			packet.yiaddr = find_IP_by_Mac(packet.chaddr);
		        	...
		        	if (!packet.yiaddr){
#ifdef IP_BASED_CLIENT_TYPE
					 packet.yiaddr = find_address(0, devicetype);
#else
		        		 packet.yiaddr = find_address(0);	 
#endif
		         	}
		         	...
		         	if (!packet.yiaddr){	
#ifdef IP_BASED_CLIENT_TYPE
					packet.yiaddr = find_address(1, devicetype);
#else
    ...
    if (NULL != pstLease)
    {
		...
		if (NULL != (classVendor=get_option(oldpacket, DHCP_VENDOR)))
		{
            ...
            if (NULL != pstClientInfo)
            {
                ...
                switch (pstClientInfo->iCategory)
                {
                    case CTC_STB:
...
void find_match_serving_pool(struct dhcpMessage *dhcppacket){
	...
	unsigned char len=0;
	unsigned char vendormachflag=0,clientidmachflag=0,userclassmatchflag=0,chaddrmatchflag=0;
	...
	while(p_servingpool_tmp!=NULL){
		...
		if(vendormachflag==1 && clientidmachflag==1 && userclassmatchflag==1 && chaddrmatchflag==1)
			break;
