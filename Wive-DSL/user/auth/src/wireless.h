...
#include <linux/if.h>			/* for IF_NAMESIZE and co... */
...
struct	iwreq 
{
	union
	{
		char	ifrn_name[IF_NAMESIZE];	/* if name, e.g. "eth0" */
	} ifr_ifrn;
...
struct	iw_priv_args
{
	...
	char		name[IF_NAMESIZE];	/* Name of the extension */
};
...
#define IW_EV_CHAR_LEN	(IW_EV_LCP_LEN + IF_NAMESIZE)
...
#define IW_EV_CHAR_PK_LEN	(IW_EV_LCP_PK_LEN + IF_NAMESIZE)
