...
#define MC_VIEWPOS ".mc/view.pos"
/* temporary file */
#define MC_VIEWPOS_TMP ".mc/viewpos.tmp"
/* maximum entries in MC_EDITPOS */
#define MC_MAX_VIEWPOS_ENTRIES 1024
...
/* "$Id: view.c,v 1.14 1998/05/05 06:19:03 unammx Exp $" */
...
static void move_right (WView *, int quantity);
...
view_status (WView *view)
{
    ...
    static int i18n_adjust = 0, c1 = 0, c2 = 0;
    static char *file_label, *temp_label, msg_tmp[1024];
	...
	attrset (MENU_ENTRY_COLOR);
    	printw (_("%s"), name_trunc (view->filename ? view->filename:
					view->command ? view->command:"", i));

	if (w > 46) {
	    widget_move (view, view->have_frame, 25 + view->have_frame);

	    if (view->hex_mode)
		{
		temp_label = _("Offset:");
		c1 = strlen (temp_label);

		attrset (MENU_HOT_COLOR);
                printw (_("Offset:"));

		attrset (MENU_ENTRY_COLOR);
                sprintf (msg_tmp,"0x%08x", view->edit_cursor);
		c2 = c1 + strlen (msg_tmp)+1;
                printw (msg_tmp);
		}
	    else
		{
		...
		attrset (MENU_ENTRY_COLOR);
		sprintf (msg_tmp, "%d", -view->start_col);
		c2 = c1 + strlen (msg_tmp)+1;
                printw (msg_tmp);
		}
	}
	if (w > 62) {
	    ...
	    widget_move (view, view->have_frame, 25 + (c2 ? c2 : 19) + view->have_frame);

	    attrset (MENU_HOT_COLOR);
	    printw ("=");

	    attrset (MENU_ENTRY_COLOR);
	    printw ("%s", size_trunc (view->s.st_size));
...
display (WView *view)
{
    ...
    int row = (view->fullscreen ? 0 : 1) + frame_shift;
    int height, width;
    long from;
    int c;
    int boldflag = 0;
	...
	/* Start of text column */
	    ...
	    sprintf (hex_buff, "%08X", (int) (from - view->first)); // CHECK IT'S SIZE!!! //--Olegarch
...
view_update (WView *view)
{
	...
	display (view);
	if (!view->fullscreen) view_status (view);
	view->dirty = 0;
...
/*
 ...
 // This code makes error... very, very strange: when i move hex text down, it moves up!
 //back old code yet
...
/* --Olegarch */
 q = 
    current + 
    ( (view->have_frame ? 
	(cpanel->widget.lines - (view->fullscreen ? 2: 3))
	:
	(LINES - (view->fullscreen ? 0: 2))
	) 
	* view->bytes_per_line); 
/*-----------------------------------------------------------*/
...
move_right (WView *view, int i)
{
	...
	view->start_col = view->start_col - i;
	if (view->start_col > 0)
	    view->start_col = 0;
    }
    view->dirty++;
...
//    direction = view->direction;
    ...
    i = ch = 0;
    for (;pos >= 0 && (ch = get_byte (view, pos))!= -1; pos +=  view->direction, i++){
...
void toggle_wrap_mode (WView *view)
{
	...
	if(!view->fullscreen) view_labels (view);
        view->dirty++;
        view_update (view);
		return;
    } 
...
check_left_right_keys (WView *view, int c)
{
	...
	move_right (view, 1);
    else if (c == (KEY_M_CTRL | KEY_LEFT))
	move_left (view, 10);
    else if (c == (KEY_M_CTRL | KEY_RIGHT))
	move_right (view, 10);
    else return 0;
...
int view_get_saved_line(const char *filename)
{
    ...
    if ((fn = concat_dir_and_file (home_dir, MC_VIEWPOS)) == NULL)
       return 1;
    f = fopen (fn, "r");
    free(fn);
    if (!f) return 1;
...
void save_view_line(const char *filename, const int line)
{
    ...
    if ((tmp = concat_dir_and_file (home_dir, MC_VIEWPOS_TMP)) == NULL)
       return;
    if ((fn = concat_dir_and_file (home_dir, MC_VIEWPOS)) == NULL) {
       free(tmp);
       return;
    ...
    if (t) {
       ...
       if (f) {
           while (fgets (buf, sizeof (buf), f)) {
               ...
               fprintf (t, "%s", buf);
               if (++i>MC_MAX_VIEWPOS_ENTRIES) break;
           }
           fclose (f);
       }
    }
...
view_handle_key (WView *view, int c)
{
    ...
    if (check_movement_keys (c, 1, (view->fullscreen ? vheight+1 : vheight), view, (movefn) view_move_backward, (movefn) view_move_forward,
    ...
    case XCTRL('E'):
    case KEY_F(11):
	view->fullscreen = view->fullscreen ? 0 : 1;
	if (!view->have_frame) widget_set_size (&view->widget, 0, 0, view->fullscreen ? LINES : LINES-1, COLS);
	do_refresh();
        return 1;
    ...
    case '\n':
    case 'f':
        view_move_forward (view, vheight - (view->fullscreen? 0 : 1));
//	view_move_forward (view, vheight - 1);
    ...
    case 'b':
        view_move_backward (view, vheight - (view->fullscreen? 0 : 1));
//    	view_move_backward (view, vheight - 1);
    ...
    case ALT('x'):
    case ESC_CHAR:
// dirty hack - if we have frame, we can't exit from editor by ESC, because it crashes mc    --Olegarch
     if (!view->have_frame)
	if (view_ok_to_quit (view)){
	    if (view_save_line) {
	        char *fullfilename = (char *) vfs_canon (view->filename);
// here we save name and current line of viewed file           --Olegarch
		int i, cur_line = 1;
		int saved_wrap_mode = view->wrap_mode;
		...
		for (i = view->first; i < view->start_display; i++)
		if (get_byte (view, i) == '\n') cur_line ++;
		    save_view_line ( fullfilename, cur_line );
		free (fullfilename);
		view->dirty++;
		view->wrap_mode = saved_wrap_mode;
		}
...
#ifdef HAVE_DLGSWITCH
void finish_view( Dlg_head *our_dlg, int *move_dir_p, WView *wview )
{
    if (move_dir_p)
...
void run_viewer( Dlg_head *our_dlg, int *move_dir_p, WView *wview )
{
    run_dlg (our_dlg);

    if (our_dlg->soft_exit)
	return;

    finish_view( our_dlg, move_dir_p, wview );
}
#endif /* HAVE_DLGSWITCH */
...
view_adjust_size (Dlg_head *h)
{
    ...
    widget_set_size (&view->widget, 0, 0, view->fullscreen ? LINES : LINES-1, COLS);
...
view (char *_command, char *_file, int *move_dir_p, int start_line)
{
	...
	*move_dir_p = 0;
    ...
    if (!error){
	update_xterm_title ("MC view", _file);
	run_dlg (our_dlg);
	if (move_dir_p)
	    *move_dir_p = wview->move_dir;
    }
...
#else
    if (!error) {
	dlglist_add_viewer( our_dlg, _file, move_dir_p, wview );
	run_viewer( our_dlg, move_dir_p, wview );
    }
#endif /* HAVE_DLGSWITCH */
