...
char *fix_mib_names[FIX_END] = {
	"VOIP.PORT[0].DISPLAY_NAME",
	"VOIP.PORT[0].NUMBER",
	"VOIP.PORT[0].LOGIN_ID",
	"VOIP.PORT[0].PASSWORD"
};
...
int do_batch_converting(char *text, int text_len, char *filename)
{
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		// create text file
//		sprintf(inFilename, "%s\\%s.txt", ((CCVVoIPApp *) AfxGetApp())->m_curdir, mac);
		sprintf(inFilename, "%s.txt", mac);
		create_txtfile(text, text_len,
			mac, display, username, authname, authpwd, inFilename);

		// create config file
		sprintf(outFilename, "%s.dat", mac);
		if (text2config(inFilename, outFilename) != 0)
		{
			fprintf(stderr, "VoIP Converting Error: text2config failed\n");
			continue;
		}
	}
...
void CPage1::OnBatchConvert() 
{
	...
	// read file
	if (flash_voip_read_file(cfg_filename, &buf, &buf_len) != 0)
	{
	...
	status = do_batch_converting(text, text_len, mac_filename);
	free(text);
