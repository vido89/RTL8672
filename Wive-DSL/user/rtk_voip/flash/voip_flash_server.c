...
int voip_flash_server_start()
{
	...
	voip_feature = stVoipCfg.cfg;
	if (voip_feature != VOIP_FLASH_FEATURE)
	{
		fprintf(stderr, "VoIP Feature is invalid (%x vs %x)\n",
			VOIP_FLASH_FEATURE, voip_feature);
		return -1;
	}
#endif
...
label_do_write_ack:
		...
		case VFWM_CMD_MIBGET:
			...
			shs.msgMibGetAck.len = VFWM_PAYLOAD_SIZE( shs.msgMibGetAck ) - sizeof( shs.msgMibGetAck.len )
									- MAX_MIBMSG_LENGTH + shs.msgMibGetAck.data_len;
