...
#ifdef X86_SIMULATE
	g_VoIP_Feature = VOIP_FLASH_FEATURE;
...
int mib_get_8186( unsigned long id, void *data, unsigned long data_len )
{
	...
	if( sh.msgMibgetAck.type == VFWM_CMD_MIBGET_ACK ) {
		...
		memcpy( data, sh.msgMibgetAck.data, sh.msgMibgetAck.data_len );
	} else {
