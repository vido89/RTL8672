...
static const args_t aTstVoipMgrSession[] = {
	...
	{ ANAME( TstVoipMgrSession, rfc2833_payload_type_local ), ATYPE_IN },
	{ ANAME( TstVoipMgrSession, rfc2833_payload_type_remote ), ATYPE_IN },
...
static const args_t aTstVoipRtpStatistics[] = {
	{ ANAME( TstVoipRtpStatistics, chid ), ATYPE_IN },
...
static const args_t aTstVoip2833[] = {
	...
	{ ANAME( TstVoip2833, sid ), ATYPE_IN },
...
static const args_t aTstVoipSlicReg[] = {
	...
	{ ANAME( TstVoipSlicReg, reg_val ), ATYPE_IN },
	{ NULL, 0, 0, ATYPE_NONE },
};
...
static const node_t nDspFaxModem[] = {
	{ NID( VOIP_MGR_FAX_MODEM_PASS_CFG ), NTYPE_LEAF, NULL, NAT( TstVoipCfg ) },
...
static const node_t nDspDTMF[] = {
	...
	{ NID( VOIP_MGR_DTMF_GET ), NTYPE_LEAF, NULL, NAT( TstVoipValue ) },
...
static const node_t nDriverSLIC[] = {
	{ NID( VOIP_MGR_SLIC_HOOK ), NTYPE_LEAF, NULL, NAT( TstVoipSlicHook ) },
	...
	{ NID( VOIP_MGR_FLUSH_FIFO ), NTYPE_LEAF, NULL, NAT( TstVoipValue ) },
...
static const node_t nDriverNetwork[] = {
	...
	{ NID( VOIP_MGR_SET_DSCP ), NTYPE_LEAF, NULL, aTstVoipFDT_Int, sizeof( int ) },
...
static const node_t nMisc[] = {
	...
	{ NID( VOIP_MGR_GET_FEATURE ), NTYPE_LEAF, NULL, NAT( TstVoipCfg ) },
...
#define SETSOCKOPT(optid, varptr, varsize, qty) \
        { \
                int     sockfd; \
                if ((sockfd = socket(AF_INET, SOCK_RAW, IPPROTO_RAW)) == -1) { \
                        return -1; \
                } \
                if (setsockopt(sockfd, IPPROTO_IP, optid, (void *)varptr, (varsize)*qty) != 0) { \
                        close(sockfd); \
                        return -2; \
                } \
                close(sockfd); \
