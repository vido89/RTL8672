...
extern int char_replace(const char *src, char old_char, char new_char, char *result);
...
void asp_voip_FxoSet(webs_t wp, char_t *path, char_t *query)
{
	...
	char_replace(key_display, '*', '.', pCfg->funckey_pstn);
...
#endif
{
	...
	{
		char_replace(pCfg->funckey_pstn, '.', '*', key_display);
