...
{	
	...
	while(head_offset < status.st_size){
		...
		if(len > 0x400000){ // rock: fix image size > 2M problem
                        print_msg(RTK_DBG_WARNING, "MISC", "Image len exceed max size 0x400000 ! len=0x%x", len);
			free(Readbuf);
			return -1;
    		}
		if ( (flag == 1) || (flag == 3)) {
...
static int downloadFwFile(int fwNum)
{
	...
	while(index < fwNum){
		...
		if(index == fwNum - 1)
		{
			Reboot = (status.st_size/60000)+5;
                        print_msg(RTK_DBG_INFO, "MISC", "Set WTD :%d\n", Reboot);
			if(rtk_WTD_Reboot(Reboot)){
                                print_msg(RTK_DBG_INFO, "MISC", "WTD_Reboot() fail\n");                              
				system("reboot");
				return 0;
			}
		}
