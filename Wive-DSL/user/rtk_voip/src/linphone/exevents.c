...
int linphone_inc_new_call(LinphoneCore *lc, guint32 ssid, eXosip_event_t *ev)
{
	...
	{
		// caller id need all digit and length >= 3
		for (i=0; from_url->url->username[i]; i++)
			if (!isdigit(from_url->url->username[i]))
				break;

		if (from_url->url->username[i] == 0 && i >=3)
		{
			strncpy(caller_id, from_url->url->username, sizeof(caller_id) - 1);
			caller_id[sizeof(caller_id) - 1] = 0;
		}
	}
	...
	if (caller_id[0] == 0)
	{
		...
		for (i=0; caller_id_name[i]; i++)
			if (!isdigit(caller_id_name[i]))
				break;

		// caller id need all digit and length >= 3,
		// or private caller id (start with '-')
		if (caller_id_name[i] == 0 && i >= 3)
		{
...
/* try the codec change here!! eric wu in 10.22 */
	...
	while(elem!=NULL){
		...
		#ifdef AUTO_RESOURCE
			if (VOIP_RESOURCE_UNAVAILABLE == rtk_VoIP_resource_check(payload.pt))
...
int linphone_accept_audio_offer(sdp_context_t *ctx,sdp_payload_t *payload)
{
	...
	if (supported == SupportedAndValid)	
	{
		...
		#ifdef AUTO_RESOURCE
			if (VOIP_RESOURCE_UNAVAILABLE == rtk_VoIP_resource_check(params->static_pt))
			{
				g_warning("no resource for payload %s.\n", payload->a_rtpmap);
				return Unsupported;
			}
