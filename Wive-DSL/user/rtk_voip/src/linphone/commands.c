...
#if 0
void linphone_proxy_config_create_routes(LinphoneProxyConfig *cfg, voipCfgPortParam_t *voip_ptr)
{
	...
	if (cfg->reg_routes)
	{
		osip_routes_free(cfg->reg_routes);
		cfg->reg_routes = NULL;
	}

	i = osip_routes_init(&cfg->reg_routes);
	if (i != 0)
		return;

	if (voip_ptr->outbound_proxy_enable) // add outbound proxy to pre-existing routes
	{
		if (voip_ptr->outbound_proxy_port && voip_ptr->outbound_proxy_port != 5060)
			sprintf(proxy_addr, "sip:%s:%d", voip_ptr->outbound_proxy, voip_ptr->outbound_proxy_port);
		else
			sprintf(proxy_addr, "sip:%s", voip_ptr->outbound_proxy);

		osip_routes_set(cfg->reg_routes, proxy_addr);
	}
}
#endif
...
//		linphone_core_terminate_dialog(lc, 1, ( const char * )1 );
		rtk_SetPlayTone(lc->chid, 1, DSPCODEC_TONE_BUSY, TRUE, DSPCODEC_TONEDIRECTION_LOCAL);
	}
	...
	{		
		...
		if( ( syss = GetSysState(lc->chid) ) == SYS_STATE_DAA_CONNECT ) {
			rtk_Set_DAA_ISR_FLOW(lc->chid, ssid, DAA_FLOW_NORMAL);
...
label_ignore_dtmf_command:
		...
		sprintf( arg1, "sip:127.0.0.1:%d", 
					( lc ->parent ->ports[ RTK_VOIP_SLIC_NUM(g_VoIP_Feature) ] ) 
						.sip_conf.sip_port );
		...
		if( arg1[ 0 ] == '0' ) {
			...
			switch( GetSysState(lc->chid) ) {
			case SYS_STATE_EDIT:
				if (rtk_Set_DAA_ISR_FLOW(lc->chid, ssid, DAA_FLOW_3WAY_CONFERENCE) == 0xff ) /* PSTN line not connect or line busy*/
			    {
			...
			case SYS_STATE_CONNECT_EDIT:
				...
				if (((g_VoIP_Feature & DAA_TYPE_MASK) != REAL_DAA_NEGO) ||
			    	(rtk_Set_DAA_ISR_FLOW(lc->chid, ssid, DAA_FLOW_3WAY_CONFERENCE) == 0xff )) /* PSTN line not connect or line busy*/
			    {
	...
	else if( strcmp( command , "ping" ) == 0 )
	{
		...
		if (linphone_core_interpret_url(lc, arg1, &to, NULL))
		{
			...
			if (from == NULL)
			{
				linphone_core_get_current_contact(lc, to, &from);
			}

			eXosip_ping(lc->chid, from, to, routes, &tr);
			eXosip.pings[lc->chid] = tr;
			osip_free(to);
		}
    }
#endif
