...
extern int g_VoIP_Feature;	/* declared in voip_manager() */
...
static void IvrPlaySpeechFormat( unsigned int chid, const unsigned char *format, ... )
{
	...
	rtk_IvrStartPlaying( chid, buffer );
}
									  ...
									  cmd_operand_t *pOperand )
{
	...
	switch( ivr_cmd ) {
		...
		case IVR_COMMAND_SET_DNS0:
			...
			if( !CheckIpAddressOperand( cmd_len, pDialCode, ( ivr_cmd == IVR_COMMAND_SET_NETMASK ) ) )
