	...
	const char *media_port, char *orig_local_ip)
{
	...
	if (tr->ctx_type == IST)
	{
		...
		if (strcmp(call->sdpctx->remote->c_connection->c_addr, "0.0.0.0") == 0)
		{
			sdp_message_free(sdp);
			return 0;
		}
	...
	if (call->sdpctx->remote) // update local ip from SDP
	{
		...
		i = eXosip_get_localip_for(
			call->sdpctx->remote->c_connection->c_addr, 
			&local_ip_tmp
		);
	...
	if (strcmp(sdp->c_connection->c_addr, call->sdpctx->localip) == 0 &&
	...
	sdp->c_connection->c_addr = osip_strdup(call->sdpctx->localip);
