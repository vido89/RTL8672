...
//			VOIP FLASH Write
//------------------------------------------------------------------
#define VOIP_FLASH_WRITE_PATHNAME		"/bin/webs"		// use for ipc 
...
#define MAX_MIBMSG_LENGTH		( 8192 - 32 )	/* MSGMAX = 8192; - 32 for header, but header is not such large now. */
...
typedef struct {
	...
	unsigned long data_len;
	unsigned char data[ MAX_MIBMSG_LENGTH ];
