...
static int sighup, sigusr1, sigterm;
...
"-w, --wait-timeout=to		 The amount of time to wait before returning the besr answer.\n"
...
#ifdef CONFIG_TR069_DNS_ISOLATION
static int reload_servers(char *fname,char *fname1, struct irec *interfaces,
...
#ifdef CONFIG_TR069_DNS_ISOLATION
int tr069_care = 0;
char *resolve_tr069=NULL;
static struct server *tr069_server;
#endif
...
#ifdef HAVE_GETOPT_LONG
#ifdef CONFIG_TR069_DNS_ISOLATION
 option = getopt_long(argc, argv, "fFnbvhdr:m:p:c:l:s:i:t:u:a:x:w:T:", opts, NULL);
#else 
      option = getopt_long(argc, argv, "fFnbvhdr:m:p:c:l:s:i:t:u:a:x:w:", opts, NULL);
#endif
#else
#ifdef CONFIG_TR069_DNS_ISOLATION
      option = getopt(argc, argv, "fFnbvhdr:m:p:c:l:s:i:t:u:a:x:w:T:");
#else 
	...
	cache_failures = 0;	/*don't cache failures*/
       ...
       if(option == 'T')
        {
         tr069_care =1;
	  resolve_tr069= safe_string_alloc(optarg); 
	  printf("**************care tr069 ------\n");	 
        }
#endif	   
      if (option == '?')
  ...
  //10/15/05' hrchen, disable for IPV6 is not supported
#if 0
  ...
  if (daemon)
      ...
      if ((ent_pw = getpwnam(username)))
	{
	  ...
	  setuid(ent_pw->pw_uid);
    	...
    	// Kaohj -- If created by tty, stdin, stdout and stderr may be associated to tty.
  ...
  while (1)
      ...
      if (sighup)
	{
	  ...
	  {
#ifdef CONFIG_TR069_DNS_ISOLATION
	    num_servers = reload_servers(resolv, resolve_tr069,interfaces, peerfd, peerfd6);
//	    reload_tr069_servers(resolve_tr069,interfaces, peerfd, peerfd6);
#else
      ...
      for (iface = interfaces; iface; iface = iface->next)
	{
	  if (FD_ISSET(iface->fd, &rset))
	      ...
	      if (n >= (int)sizeof(HEADER) && !header->qr)
		{
		  ...
		  // Jenny, trigger PPP dial-on-demand
		  if (defaultGWAddr(pingaddr) == 0 && !strcmp(pingaddr, "64.64.64.64")) {
			if ((fp = fopen("/var/run/spppd.pid", "r"))) {
				fscanf(fp, "%d\n", &spppd_pid);
				fclose(fp);
			}
			kill(spppd_pid, SIGUSR1); /* Raise SIGUSR1 in current process */
			printf("dnsmasq %s kill dial\n", __FUNCTION__);
			sleep(3);
		  }
...
static int reload_servers(char *fname, char *fname_tr069, struct irec *interfaces,
			   int peerfd, int peerfd6)
{
  FILE *f,*f1;
...
#endif
      if (last_server) {
      		// first server look up
      		struct server *first_server = last_server;
		while(first_server->next != last_server)
			first_server = first_server->next;

		printf("1st server=%08x, new server=%08x, last server=%08x\n"
		...
		serv->next = last_server;
		last_server = serv;
		first_server->next = last_server;		
      } else {
		last_server = serv;
		serv->next = last_server;
		printf("1st server=%08x\n",(unsigned int)serv);
      }
...
//      syslog(LOG_INFO, "using nameserver %s", token); 
 ...
 if(f1!= 0){ 
  while ((line = fgets(buff, MAXLIN, f1)))
{
      union  mysockaddr addr;
      char *token = strtok(line, " \t\n");
      struct irec *iface;
...
//      syslog(LOG_INFO, "using nameserver %s", token); 
  ...
  /* Count the servers and return it*/
  
  fclose(f);
  if (!last_server)
      return 0;
  for(i=1,srv=last_server;srv->next!=last_server;srv=srv->next,i++);
  return i;
}
			  ...
			  int plen)
{
  ...
  if (forward)
	...
	{ 
	  ...
	  // Mason Yu
	  //printf("Send to 0x%x DNS Server\n", ((struct sockaddr_in *)&(forward->sentto->addr.sa))->sin_addr.s_addr);
	  
	  if (sendto(fd, (char *)header, plen, 0, 
		     &forward->sentto->addr.sa, 
		     sa_len(&forward->sentto->addr)) != -1) {
	    // Mason Yu
	    //printf("sendto DNS Server is ok !!\n");
  ...
  header->arcount = htons(0);
    ...
    {	// Jenny, trigger PPP dial-on-demand
	FILE *fp;
	if ((fp = fopen("/tmp/ppp_dod_flag", "r"))) {
		fscanf(fp, "%d\n", &ppp_dod);
		fclose(fp);
	}
	if(ppp_dod == 1) {
		...
		kill(spppd_pid, SIGUSR1); /* Raise SIGUSR1 in current process */
		ppp_dod = 0;
		fp = fopen("/tmp/ppp_dod_flag", "w");
		fprintf(fp, "%d", ppp_dod);
		fclose(fp);
		printf("dnsmasq %s kill dial\n", __FUNCTION__);
		sleep(3);		
	}
  ...
  //printf("Send to DNS Server Error!\n");
