...
static char CMD_MOUNT_FMT[] = "/bin/mount -o sync -t %s /dev/%s %s";
...
static int try_mount(const char *devnode, const char *mnt) {
	...
	for (idx = 0; fstypes[idx] != 0; idx++) {
		...
		rv = mount(source, mnt, fstype, 
			MS_SYNCHRONOUS | MS_NODIRATIME | MS_NOATIME, 
			0);
		if (rv==0)
			break;
		usleep(rand() % 500000);
		...
		*/
		...
		snprintf(cmd_bufferx, sizeof(cmd_bufferx), 
			"echo \"(%u) E mount %s to %s(%s) ret %d (%d)\" >> /tmp/log", 
			++cmd_counter, devnode, mnt, fstype,  rv, retry);
		system(cmd_bufferx); 
		#endif
	}
...
static unsigned int fs_mount(unsigned int parition_id) {
	unsigned int mnt_map = 0;
...
static unsigned int fs_umount(unsigned int parition_id) {
	unsigned int mnt_map = 0;
