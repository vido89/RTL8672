...
int tail_main(int argc, char **argv)
{
	...
	if ((argc>1) && (strlen(argv[1])>=2) && (argv[1][0]=='-') &&
		...
		count = parse_number(argv[1]+1, tail_suffixes);
		argc--;
		argv++;
	}
#endif
	...
	for (i = 0; i < nfiles; i++) {
		...
		if (show_headers || (!hide_headers && nfiles > 1))
			printf("%s==> %s <==\n", i == 0 ? "" : "\n", argv[optind + i]);
#if 1
		if ((!from_top) && (units == LINES) && (fds[i] != STDIN_FILENO)) {
			special_case = 1;
			fpos = lseek(fds[i], 0, SEEK_END);
		}
		else {
			special_case = 0;
		}
		...
		if (special_case) {
			fpos -= 4096;
			if (fpos < 0) fpos = 0;
			fpos = lseek(fds[i], fpos, SEEK_SET);
		}
#endif
...
#if 1
		if (special_case && (seen <= count) && (fpos != 0)) {
			cont = 1;
			tailbuf_destroy();
		}
		} while (cont);
