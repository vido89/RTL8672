...
/*ping_zhang:20081223 END*/
  ...
  /*ping_zhang:20081223 END*/
    ...
    ret = sleep (wait);
    if (ret != 0)
    	sleep(2); // interrupted by SIGUSR1, wait for interface up
  }
...
#endif
{
	...
	if (r == 0)
	{
		...
		snprintf (servername, len, "%s (%hhu.%hhu.%hhu.%hhu)", (char *) p,
...
// Kaohj
  ...
  while (1)
   {
  #ifdef SNTP_MULTI_SERVER
    c = getopt_long (argc, argv, "i:p:s:hv", longopts, &longindex);
  #else
    c = getopt_long (argc, argv, "i:p:hv", longopts, &longindex);
  #endif
    if (c == -1)
      break;
