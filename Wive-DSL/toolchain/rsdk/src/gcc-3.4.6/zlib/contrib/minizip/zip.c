     ...
     linkedlist_data *ll;
{
  ...
  while (ldi != NULL)
	...
	if (fwrite (ldi->data, (uInt) ldi->filled_in_this_block, 1, fout) !=
	    1)
	  return ZIP_ERRNO;
      ldi = ldi->next_datablock;
    }
...
local int
ziplocal_putValue (file, x, nbByte)
     FILE *file;
     uLong x;
     int nbByte;
{
     ...
     int level;
{
  ...
  if ((err == ZIP_OK) && (size_extrafield_local > 0))
    if (fwrite
	(extrafield_local, (uInt) size_extrafield_local, 1, zi->filezip) != 1)
      err = ZIP_ERRNO;
     ...
     unsigned len;
{
  ...
  while ((err == ZIP_OK) && (zi->ci.stream.avail_in > 0))
	...
	{
	  if (fwrite
	      (zi->ci.buffered_data, (uInt) zi->ci.pos_in_buffered_data, 1,
	       zi->filezip) != 1)
	    err = ZIP_ERRNO;
	  zi->ci.pos_in_buffered_data = 0;
	  zi->ci.stream.avail_out = (uInt) Z_BUFSIZE;
     ...
     zipFile file;
{
  ...
  if (err == ZIP_OK)
	...
	err =
	  ziplocal_putValue (zi->filezip, (uLong) zi->ci.stream.total_out, 4);
