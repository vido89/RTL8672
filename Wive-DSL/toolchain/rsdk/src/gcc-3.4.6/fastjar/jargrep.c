...
create_regexp (const char *regstr, int options)
{
  ...
  if ((exp = (regex_t *) malloc (sizeof (regex_t))))
    {
      if ((errcode =
	   regcomp (exp, regstr, (options & JG_IGNORE_CASE) ? REG_ICASE : 0)))
	{
	  fprintf (stderr, "regcomp of regex failed,\n");
	  if ((errmsg =
	       (char *) malloc (msgsize =
				regerror (errcode, exp, NULL, 0) + 1)))
	    {
...
*/
static int
check_sig (ub1 * scratch, pb_file * pbfp)
{
  ub4 signature;
  int retflag = 0;
...
static void
decd_siz (ub4 * csize, ub4 * usize, ub2 * fnlen, ub2 * eflen, ub2 * flags,
	  ub2 * method, ub1 * file_header)
{
...
static char *
read_string (pb_file * pbf, int size)
{
  char *page;
...
static char *
extract_line (const char *stream, regoff_t begin, regoff_t end, int *b)
{
  int e;
  int length;
  char *retstr;
  ...
  (*b)++;
  for (e = end; stream[e] == '\t' || !iscntrl ((unsigned char) stream[e]);
       e++);
  length = e - *b;
...
static int
chk_wrd (regex_t * exp, const char *str)
{
  int wrd_fnd = FALSE;
  int frnt_ok;
  int bck_ok;
...
static void
prnt_mtchs (regex_t * exp, const char *filename, const char *stream,
	    regmatch_t * pmatch, regmatch_t * nl_offset, int num, int lines,
	    int options)
{
  int i;
  int j = 0;
      ...
      if (begin > o_begin)
	{
	  if (!(options & JG_WORD_EXPRESSIONS) || chk_wrd (exp, str))
	      ...
	      if (!(options & JG_PRINT_COUNT))
		{
		  ...
		  if (options & JG_PRINT_BYTEOFFSET)
		    printf ("%d:", begin);
		  printf ("%s\n", str);
		}
	    }
...
static void
check_crc (pb_file * pbf, const char *stream, ub4 usize)
{
  ub4 crc = 0;
  ub4 lcrc;
  ub1 scratch[16];
...
static void
mk_ascii (char *stream, size_t usize)
{
  size_t i;
...
static regmatch_t *
fnd_match (regex_t * exp, const char *str_stream, int *i)
{
  int regflag;
  regmatch_t match;
  ...
  for (*i = 0, regflag = regexec (exp, str_stream, 1, &match, 0); !regflag;
       regflag =
       regexec (exp, &(str_stream[match.rm_eo]), 1, &match, 0), (*i)++)
    {
      if ((tmp = (regmatch_t *)
...
static int
cont_grep (regex_t * exp, regex_t * nl_exp, int fd, pb_file * pbf,
	   int options)
{
  int retflag = TRUE;
  int i;
  ...
  if (pb_read (pbf, (file_header + 4), 26) != 26)
      ...
      if (filename[fnlen - 1] != '/')
	{
	  ...
	  if (flags & 0x008)
	    check_crc (pbf, str_stream, usize);
	  ...
	  prnt_mtchs (exp, filename, str_stream, match_array, nl_offsets, i,
		      j, options);
	  if (match_array)
	    free (match_array);
	  free (str_stream);
...
static void
jargrep (regex_t * exp, regex_t * nl_exp, const char *jarfile, int options)
{
  int fd;
  int floop = TRUE;
  pb_file pbf;
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "bce:insVw", option_vec, NULL)) != -1)
    {
      switch (c)
	{
	case 'b':
