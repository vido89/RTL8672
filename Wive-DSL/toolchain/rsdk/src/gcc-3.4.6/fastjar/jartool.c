...
main (int argc, char **argv)
{
  ...
  while ((opt = getopt_long (argc, argv, OPTION_STRING, options, NULL)) != -1)
    {
      switch (opt)
	{
	case 'C':
  ...
  if (action == ACTION_CREATE)
	...
	{
	  jarfd =
	    open (jarfile, O_CREAT | O_BINARY | O_WRONLY | O_TRUNC, 0666);
...
int
make_manifest (int jfd, const char *mf_name)
{
  time_t current_time;
  ...
  if (mf_name == NULL)
    {
      ...
      int mf_len =
	strlen (MANIFEST_STR) + strlen (VERSION) + strlen (MANIFEST_END);
      char *mf;
...
int
add_file_to_jar (int jfd, int ffd, const char *fname, struct stat *statbuf)
{
  ...
  if (verbose)
    printf ("(in=%d) (out=%d) (%s %d%%)\n",
	    ...
	    (do_compress ? ((int) ((1 - ze->csize / (float) ze->usize) * 100))
	     : 0));
...
int
extract_jar (int fd, char **files, int file_num)
{
  int rdamt;
  int out_a, in_a;
  ub4 signature;
  ...
  for (;;)
	...
	printf ("%10s: %s\n",
		(method == 8 ? "inflated" : "extracted"), filename);
    }
...
int
list_jar (int fd, char **files, int file_num)
{
  ub4 signature;
  ub4 csize;
  ub4 usize;
