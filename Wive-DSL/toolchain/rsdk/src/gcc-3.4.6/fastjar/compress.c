...
int
compress_file (int in_fd, int out_fd, struct zipentry *ze)
{
  Bytef in_buff[RDSZ];
  Bytef out_buff[RDSZ];
...
int
inflate_file (pb_file * pbf, int out_fd, struct zipentry *ze)
{
  Bytef in_buff[RDSZ];
  Bytef out_buff[RDSZ];
  unsigned int rdamt;
...
static Bytef *
ez_inflate_str (pb_file * pbf, ub4 csize, ub4 usize)
{
  Bytef *out_buff;
  Bytef *in_buff;
  unsigned int rdamt;
...
static Bytef *
hrd_inflate_str (pb_file * pbf, ub4 * csize, ub4 * usize)
{
  Bytef *out_buff;
  Bytef *tmp;
  Bytef in_buff[RDSZ];
...
Bytef *
inflate_string (pb_file * pbf, ub4 * csize, ub4 * usize)
{
  Bytef *ret_buf;

  if (*csize && *usize)
    ret_buf = ez_inflate_str (pbf, *csize, *usize);
  else
    ret_buf = hrd_inflate_str (pbf, csize, usize);
