...
} preds[] =
{
...
#endif
  {
    ...
    {
  CONST_INT, CONST_DOUBLE, CONST, SYMBOL_REF,
	LABEL_REF, SUBREG, REG, MEM, ADDRESSOF, PLUS, MINUS, MULT}},
  {
    "register_operand",
    {
  SUBREG, REG, ADDRESSOF}},
  {
...
static struct decision *new_decision (const char *, struct decision_head *);
...
static struct decision *write_switch (struct decision *, int);
...
extern void debug_decision (struct decision *);
extern void debug_decision_list (struct decision *);

...
find_operand (rtx pattern, int n, rtx stop)
{
  ...
  if ((code == MATCH_SCRATCH
       ...
       || code == MATCH_OPERAND
       || code == MATCH_OPERATOR
       || code == MATCH_PARALLEL) && XINT (pattern, 0) == n)
    return pattern;
...
validate_pattern (rtx pattern, rtx insn, rtx set, int set_code)
{
  ...
  switch (code)
		...
		for (j = 0; preds[i].codes[j] != 0; j++)
		  {
			...
			&& c != CONST_INT
			&& c != CONST_DOUBLE
			&& c != CONST && c != HIGH && c != CONSTANT_P_RTX)
		      allows_non_const = 1;
	...
	if (set && pred_name[0] != '\0' && allows_non_lvalue)
	  {
	    message_with_line (pattern_lineno,
	...
	/* If only one of the operands is VOIDmode, and PC or CC0 is
	   not involved, it's probably a mistake.  */
		 ...
		 && GET_CODE (dest) != CC0
		 && GET_CODE (src) != PC
		 && GET_CODE (src) != CC0 && GET_CODE (src) != CONST_INT)
	  {
	    const char *which;
...
add_to_sequence (rtx pattern, struct decision_head *last,
		 const char *position, enum routine_type insn_type, int top)
{
  RTX_CODE code;
...
static int
maybe_both_true (struct decision *d1, struct decision *d2, int toplevel)
{
...
factor_tests (struct decision_head *head)
{
  ...
  for (first = head->first; first && first->next; first = next)
	...
	continue;
	  ...
	  && type != DT_elt_one_int && type != DT_elt_zero_wide_safe)
	continue;
...
write_switch (struct decision *start, int depth)
{
  ...
  if (!p->next
      ...
      || p->next->tests->next || nodes_identical_1 (p->tests, p->next->tests))
    return p;
  ...
  if (type == DT_code)
	...
	ret = p;
      ...
      while (p && p->tests->type == DT_pred && p->tests->u.pred.index >= 0)
	{
	  const RTX_CODE *c;
...
static int
write_node (struct decision *p, int depth, enum routine_type subroutine_type)
{
