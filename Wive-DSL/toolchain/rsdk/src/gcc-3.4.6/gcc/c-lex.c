...
static enum integer_type_kind narrowest_unsigned_type (tree, unsigned int);
static enum integer_type_kind narrowest_signed_type (tree, unsigned int);
...
init_c_lex (void)
{
  ...
  file_info_tree = splay_tree_new ((splay_tree_compare_fn) strcmp,
				   0, (splay_tree_delete_value_fn) free);
...
{
  print_time ((const char *) n->key, ((struct c_fileinfo *) n->value)->time);
  return 0;
}
...
interpret_integer (const cpp_token * token, unsigned int flags)
{
  ...
  if (flags & CPP_N_IMAGINARY)
    value =
      build_complex (NULL_TREE, convert (type, integer_zero_node), value);
  ...
  if (cpp_interpret_string (parse_in, strs, count, &istr, wide))
  ...
  else
    {
      /* Callers cannot generally handle error_mark_node in this context,
         ...
         issued an error.  */
      if (wide)
	value = build_string (TYPE_PRECISION (wchar_type_node) / TYPE_PRECISION (char_type_node), "\0\0\0");	/* widest supported wchar_t
														   is 32 bits */
      else
...
lex_charconst (const cpp_token * token)
{
  ...
  result = cpp_interpret_charconst (parse_in, token, &chars_seen, &unsignedp);
