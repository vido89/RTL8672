			 ...
			 enum machine_mode cmp_mode, rtx op0, rtx op1)
{
  ...
  if (cmp_mode != VOIDmode && !VECTOR_MODE_P (mode))
    {
			  ...
			  rtx op, enum machine_mode op_mode)
{
  ...
  /* We can do some operations on integer CONST_DOUBLEs.  Also allow
	...
	case FIX:
	  ...
	  if (width > HOST_BITS_PER_WIDE_INT)
	    {
	      th =
		(HOST_WIDE_INT) - 1 << (width - HOST_BITS_PER_WIDE_INT - 1);
	      tl = 0;
	    }
	  else
	    {
  ...
  /* This was formerly used only for non-IEEE float.
	...
	case NEG:
	  ...
	  if (GET_CODE (op) == MULT && !HONOR_SIGN_DEPENDENT_ROUNDING (mode))
	    {
	      temp = simplify_gen_unary (NEG, mode, XEXP (op, 0), mode);
				...
				rtx op0, rtx op1)
{
  ...
  if (GET_CODE (op0) == code && associative_constant_p (XEXP (op0, 1)))
    {
  ...
  if (GET_CODE (op1) == code && associative_constant_p (XEXP (op1, 1)))
    {
  ...
  if (GET_CODE (op0) != CONST_INT || GET_CODE (op1) != CONST_INT
	...
	case PLUS:
	  ...
	  if (INTEGRAL_MODE_P (mode)
	      && GET_CODE (op0) == NOT && trueop1 == const1_rtx)
	...
	case MULT:
	  ...
	  /* In IEEE floating point, x*1 is not equivalent to x for
	     signalling NaNs.  */
	  if (!HONOR_SNANS (mode) && trueop1 == CONST1_RTX (mode))
	    return op0;
	...
	case DIV:
	  ...
	  if (!HONOR_NANS (mode)
	      && !HONOR_SIGNED_ZEROS (mode)
	      && trueop0 == CONST0_RTX (mode) && !side_effects_p (op1))
	    return op0;
	...
	case ASHIFTRT:
	  ...
	  if (GET_CODE (trueop0) == CONST_INT
	      && width <= HOST_BITS_PER_WIDE_INT
	      && (unsigned HOST_WIDE_INT) INTVAL (trueop0) ==
	      GET_MODE_MASK (mode) && !side_effects_p (op1))
	    return op0;
	...
	case VEC_SELECT:
	  if (!VECTOR_MODE_P (mode))
		...
		return CONST_VECTOR_ELT (trueop0,
					 INTVAL (XVECEXP (trueop1, 0, 0)));
	    }
	  else
	    {
	      if (!VECTOR_MODE_P (GET_MODE (trueop0))
	      ...
	      if (GET_CODE (trueop0) == CONST_VECTOR)
		{
		  ...
		  for (i = 0; i < n_elts; i++)
			...
			abort ();
		      RTVEC_ELT (v, i) =
			CONST_VECTOR_ELT (trueop0, INTVAL (x));
		    }
	...
	for (j = i + 1; j < n_ops; j++)
	  {
		...
		if (tem
		    ...
		    && !(first
			 && GET_CODE (tem) == NOT && XEXP (tem, 0) == rhs))
		  {
		    lneg &= rneg;
		    if (GET_CODE (tem) == NEG)
			       ...
			       rtx op0, rtx op1)
{
  ...
  if (!HONOR_NANS (GET_MODE (trueop0))
      && rtx_equal_p (trueop0, trueop1) && !side_effects_p (trueop0))
  ...
  /* Otherwise, see if the operands are both integers.  */
      ...
      if (width != 0 && width < HOST_BITS_PER_WIDE_INT)
	{
      ...
      if (width != 0 && width <= HOST_BITS_PER_WIDE_INT)
	h0u = h1u = 0, h0s = HWI_SIGN_EXTEND (l0s), h1s =
	  HWI_SIGN_EXTEND (l1s);
  ...
  /* Otherwise, there are some code-specific tests we can make.  */
	...
	case LEU:
	  ...
	  if (GET_CODE (trueop1) == CONST_INT
	      && (unsigned HOST_WIDE_INT) INTVAL (trueop1) ==
	      GET_MODE_MASK (mode) && INTEGRAL_MODE_P (mode))
	    return const_true_rtx;
	  break;
			    ...
			    rtx op2)
{
  ...
  switch (code)
      ...
      if (GET_RTX_CLASS (GET_CODE (op0)) == '<' && !side_effects_p (op0))
	{
	  ...
	  if (GET_CODE (op1) == CONST_INT && GET_CODE (op2) == CONST_INT)
		...
		break;
	      ...
	      return gen_rtx_fmt_ee (code, mode, XEXP (op0, 0),
				     XEXP (op0, 1));
	    }
	}
      break;
    ...
    case VEC_MERGE:
      if (GET_MODE (op0) != mode
	  || GET_MODE (op1) != mode || !VECTOR_MODE_P (mode))
	abort ();
		       ...
		       enum machine_mode innermode, unsigned int byte)
{
  ...
  for (elem = 0; elem < num_elem; elem++)
	...
	case CONST_INT:
	  for (i = 0;
	       i < HOST_BITS_PER_WIDE_INT && i < elem_bitsize; i += value_bit)
	    *vp++ = INTVAL (el) >> i;
		 ...
		 enum machine_mode innermode, unsigned int byte)
{
  ...
  if (byte % GET_MODE_SIZE (outermode) || byte >= GET_MODE_SIZE (innermode))
    abort ();
  ...
  if (GET_CODE (op) == CONST_INT
      || GET_CODE (op) == CONST_DOUBLE || GET_CODE (op) == CONST_VECTOR)
  ...
  if (GET_CODE (op) == SUBREG)
    {
      ...
      if (outermode == innermostmode && byte == 0 && SUBREG_BYTE (op) == 0)
      ...
      else
	{
	  int offset = 0;
	  int difference =
	    (GET_MODE_SIZE (innermostmode) - GET_MODE_SIZE (outermode));
      ...
      new = simplify_subreg (outermode, SUBREG_REG (op),
			     GET_MODE (SUBREG_REG (op)), final_offset);
      if (new)
	return new;
  ...
  if (GET_CODE (op) == MEM && !mode_dependent_address_p (XEXP (op, 0))
