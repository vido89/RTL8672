...
fr30_compute_frame_size (int from_reg, int to_reg)
{
  ...
  current_frame_info.total_size =
    args_size + var_size + reg_size + pretend_size;
...
fr30_expand_prologue (void)
{
  ...
  if (current_frame_info.total_size == 0 && current_frame_info.gmask)
    abort ();
  ...
  if (current_frame_info.pretend_size)
    {
      ...
      for (regno = FIRST_ARG_REGNUM + FR30_NUM_ARG_REGS;
	   regno--, regs_to_save--;)
	{
...
fr30_expand_epilogue (void)
{
  ...
  if (current_frame_info.frame_size > 0)
      ...
      else
	{
	  rtx tmp = gen_rtx_REG (Pmode, PROLOGUE_TMP_REGNUM);
	  emit_insn (gen_movsi
		     (tmp, GEN_INT (current_frame_info.frame_size)));
...
fr30_print_operand (FILE * file, rtx x, int code)
{
  ...
  switch (code)
	...
	default:
	  output_operand_lossage
	    ("fr30_print_operand: unrecognized %%b code");
	  break;
	}
      return;
    ...
    case 'x':
      if (GET_CODE (x) != CONST_INT || INTVAL (x) < 16 || INTVAL (x) > 32)
...
fr30_pass_by_reference (tree valist, tree type)
{
  ...
  t =
    build (POSTINCREMENT_EXPR, va_list_type_node, valist,
...
stack_add_operand (rtx operand, Mmode mode ATTRIBUTE_UNUSED)
{
  return
    (GET_CODE (operand) == CONST_INT
     && INTVAL (operand) >= -512
     && INTVAL (operand) <= 508 && ((INTVAL (operand) & 3) == 0));
}
...
add_immediate_operand (rtx operand, Mmode mode ATTRIBUTE_UNUSED)
{
  return
    (GET_CODE (operand) == REG
     || (GET_CODE (operand) == CONST_INT
	 && INTVAL (operand) >= -16 && INTVAL (operand) <= 15));
}
...
high_register_operand (rtx operand, Mmode mode ATTRIBUTE_UNUSED)
{
  return
    (GET_CODE (operand) == REG
     && REGNO (operand) <= 15 && REGNO (operand) >= 8);
}
...
{
  return (GET_CODE (operand) == REG && REGNO (operand) <= 7);
}
...
fr30_move_double (rtx * operands)
{
  ...
  if (dest_code == REG)
      ...
      else if (src_code == MEM)
	{
	  ...
	  int reverse =
	    (refers_to_regno_p (dregno, dregno + 1, addr, 0) != 0);
      ...
      else
	{
	  ...
	  emit_insn (gen_addsi_small_int
		     (src0, src0, GEN_INT (UNITS_PER_WORD)));
