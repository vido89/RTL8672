...
static
GTY (())
     bool rs6000_always_hint;

/* Schedule instructions for group formation.  */
     static GTY (())
     bool rs6000_sched_groups;
...
/* Opaque types.  */
     static GTY (())
     tree opaque_V2SI_type_node;
     static GTY (())
     tree opaque_V2SF_type_node;
     static GTY (())
     tree opaque_p_V2SI_type_node;

/* AltiVec requires a few more basic types in addition to the vector
   types already defined in tree.c.  */
     static GTY (())
     tree bool_char_type_node;	/* __bool char */
     static GTY (())
     tree bool_short_type_node;	/* __bool short */
     static GTY (())
     tree bool_int_type_node;	/* __bool int */
     static GTY (())
     tree pixel_type_node;	/* __pixel */
     static GTY (())
     tree bool_V16QI_type_node;	/* __vector __bool char */
     static GTY (())
     tree bool_V8HI_type_node;	/* __vector __bool short */
     static GTY (())
     tree bool_V4SI_type_node;	/* __vector __bool int */
     static GTY (())
     tree pixel_V8HI_type_node;	/* __vector __pixel */
...
/* String from -malign-XXXXX.  */
     ...
     static tree rs6000_handle_longcall_attribute (tree *, tree, tree, int,
						   bool *);
     static tree rs6000_handle_altivec_attribute (tree *, tree, tree, int,
						  bool *);
     ...
     static void rs6000_output_mi_thunk (FILE *, tree, HOST_WIDE_INT,
					 HOST_WIDE_INT, tree);
     static rtx rs6000_emit_set_long_const (rtx, HOST_WIDE_INT,
					    HOST_WIDE_INT);
     static bool rs6000_return_in_memory (tree, tree);
     static void rs6000_file_start (void);
#if TARGET_ELF
     static unsigned int rs6000_elf_section_type_flags (tree, const char *,
							int);
     static void rs6000_elf_asm_out_constructor (rtx, int);
     static void rs6000_elf_asm_out_destructor (rtx, int);
     static void rs6000_elf_end_indicate_exec_stack (void) ATTRIBUTE_UNUSED;
     static void rs6000_elf_select_section (tree, int,
  ...
  ATTRIBUTE_UNUSED;
     ...
     static void rs6000_xcoff_select_section (tree, int,
					      unsigned HOST_WIDE_INT);
     ...
     static unsigned int rs6000_xcoff_section_type_flags (tree, const char *,
							  int);
     ...
     static rtx rs6000_expand_builtin (tree, rtx, rtx, enum machine_mode,
				       int);
     ...
     static void rs6000_parse_yes_no_option (const char *, const char *,
					     int *);
...
static
GTY ((param_is (struct toc_hash_struct)))
...
rs6000_override_options (const char *default_cpu)
{
  ...
  if (rs6000_traceback_name)
	...
	error
	  ("unknown -mtraceback arg `%s'; expecting `full', `partial' or `none'",
  ...
  rs6000_parse_yes_no_option ("isel", rs6000_isel_string, &rs6000_isel);
  ...
  if (TARGET_E500)
	...
	target_flags = target_flags & ~MASK_STRING;
      ...
      if (rs6000_spe_string != 0 && !strcmp (rs6000_spe_string, "no"))
	rs6000_long_double_type_size = 64;
    }
  ...
  rs6000_sched_restricted_insns_priority = (rs6000_sched_groups ? 1 : 0);
...
rs6000_file_start (void)
{
  ...
  if (flag_verbose_asm)
	...
	{
	case SDATA_NONE:
	  fprintf (file, "%s -msdata=none", start);
	  start = "";
	  break;
	case SDATA_DATA:
	  fprintf (file, "%s -msdata=data", start);
	  start = "";
	  break;
	case SDATA_SYSV:
	  fprintf (file, "%s -msdata=sysv", start);
	  start = "";
	  break;
	case SDATA_EABI:
	  fprintf (file, "%s -msdata=eabi", start);
	  start = "";
	  break;
	}
  ...
  if (reload_completed)
    {
	  ...
	  && !info->lr_save_p
	  && !info->cr_save_p && info->vrsave_mask == 0 && !info->push_p)
	return 1;
    }
...
int
any_operand (rtx op ATTRIBUTE_UNUSED, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  return 1;
}
...
{
  return (gpc_reg_operand (op, mode) || (GET_CODE (op) == CONST_INT
#if HOST_BITS_PER_WIDE_INT != 32
					 &&
					 ((unsigned
					   HOST_WIDE_INT) (INTVAL (op) +
							   0x80000000) <
					  (unsigned HOST_WIDE_INT)
					  0x100000000ll)
#endif
...
reg_or_logical_cint_operand (rtx op, enum machine_mode mode)
{
  if (GET_CODE (op) == CONST_INT)
    ...
    {
      if (GET_MODE_BITSIZE (mode) <= HOST_BITS_PER_WIDE_INT || mode != DImode)
	abort ();
...
got_operand (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  return (GET_CODE (op) == SYMBOL_REF
	  || GET_CODE (op) == CONST || GET_CODE (op) == LABEL_REF);
}
...
easy_fp_constant (rtx op, enum machine_mode mode)
{
  ...
  if ((TARGET_SOFT_FLOAT || !TARGET_FPRS) && mode != DImode)
    return 1;
...
easy_vector_splat_const (int cst, enum machine_mode mode)
{
  switch (mode)
    {
    case V4SImode:
      if (EASY_VECTOR_15 (cst) || EASY_VECTOR_15_ADD_SELF (cst))
	return cst;
...
easy_vector_constant (rtx op, enum machine_mode mode)
{
  ...
  if (GET_CODE (op) != CONST_VECTOR || (!TARGET_ALTIVEC && !TARGET_SPE))
    return 0;
  ...
  if (TARGET_SPE && mode == V2SImode
      && cst >= -0x7fff && cst <= 0x7fff && cst2 >= -0x7fff && cst2 <= 0x7fff)
    return 1;

  if (TARGET_ALTIVEC && easy_vector_same (op, mode))
    {
      cst = easy_vector_splat_const (cst, mode);
      if (EASY_VECTOR_15_ADD_SELF (cst) || EASY_VECTOR_15 (cst))
	return 1;
    }
  return 0;
}
...
easy_vector_constant_add_self (rtx op, enum machine_mode mode)
{
  int cst;
  if (TARGET_ALTIVEC
      && GET_CODE (op) == CONST_VECTOR && easy_vector_same (op, mode))
    {
...
gen_easy_vector_constant_add_self (rtx op)
{
  ...
  for (i = 0; i < units; i++)
    RTVEC_ELT (v, i) = GEN_INT (INTVAL (CONST_VECTOR_ELT (op, i)) >> 1);
...
input_operand (rtx op, enum machine_mode mode)
{
  ...
  if (GET_MODE_CLASS (mode) == MODE_FLOAT
      && CONSTANT_P (op) && easy_fp_constant (op, mode))
    return 1;
  ...
  if (GET_CODE (op) == CONST_VECTOR && easy_vector_constant (op, mode))
    return 1;
...
{
  if (!TARGET_MACHO || !flag_pic || mode != SImode || GET_CODE (x) != MEM)
    return false;
  x = XEXP (x, 0);
			   ...
			   enum machine_mode mode)
{
  ...
  if (GET_CODE (x) == PLUS
      ...
      && (unsigned HOST_WIDE_INT) (INTVAL (XEXP (x, 1)) + 0x8000) >= 0x10000)
    {
      ...
      return gen_rtx_PLUS (Pmode, sum, GEN_INT (low_int));
	   ...
	   && (TARGET_POWERPC64 || mode != DImode) && mode != TImode)
    {
  ...
  else if (TARGET_MACHO && TARGET_32BIT && TARGET_NO_TOC && !flag_pic
#if TARGET_MACHO
				  ...
				  int ind_levels ATTRIBUTE_UNUSED, int *win)
{
  ...
  if (GET_CODE (x) == PLUS
      ...
      && GET_CODE (XEXP (x, 1)) == CONST_INT
      && (INTVAL (XEXP (x, 1)) & 3) != 0
      && GET_MODE_SIZE (mode) >= UNITS_PER_WORD && TARGET_POWERPC64)
    {
  ...
  if (GET_CODE (x) == PLUS
      ...
      && REG_MODE_OK_FOR_BASE_P (XEXP (x, 0), mode)
      && GET_CODE (XEXP (x, 1)) == CONST_INT
      && !SPE_VECTOR_MODE (mode) && !ALTIVEC_VECTOR_MODE (mode))
    {
  ...
  if (GET_CODE (x) == SYMBOL_REF
      ...
      && !ALTIVEC_VECTOR_MODE (mode) && (flag_pic || MACHO_DYNAMIC_NO_PIC_P)
	...
	x = gen_rtx_LO_SUM (GET_MODE (x), gen_rtx_HIGH (Pmode, x), x);
  ...
  if (!TARGET_POWERPC64)
    {
      ...
      operand1 = operand_subword_force (dest, WORDS_BIG_ENDIAN == 0, DImode);
      operand2 = operand_subword_force (dest, WORDS_BIG_ENDIAN != 0, DImode);
...
rs6000_emit_move (rtx dest, rtx source, enum machine_mode mode)
{
  ...
  if (mode == SFmode && !TARGET_POWERPC
      && TARGET_HARD_FLOAT && TARGET_FPRS && GET_CODE (operands[0]) == MEM)
    {
      int regnum;
  ...
  switch (mode)
    ...
    case TImode:
      if (GET_CODE (operands[0]) == MEM
	  && GET_CODE (XEXP (operands[0], 0)) != REG && !reload_in_progress)
	operands[0]
	  = replace_equiv_address (operands[0],
				   copy_addr_to_reg (XEXP (operands[0], 0)));

      if (GET_CODE (operands[1]) == MEM
	  && GET_CODE (XEXP (operands[1], 0)) != REG && !reload_in_progress)
	operands[1]
  ...
  if (TARGET_ALTIVEC_ABI && ALTIVEC_VECTOR_MODE (mode))
      ...
      if (stack)
	{
	  ...
	  if (TARGET_DEBUG_ARG)
	   ...
	   && !cum->stdarg && cum->sysv_gregno <= GP_ARG_MAX_REG)
    cum->sysv_gregno++;
...
rs6000_mixed_function_arg (enum machine_mode mode, tree type, int align_words)
{
  ...
  /* Optimize the simple case where the arg fits in one gpr, except in
     ...
     BITS_PER_WORD wide.  */
  if (n_units == 0 || (n_units == 1 && mode != BLKmode))
	      ...
	      if (align_words < GP_ARG_NUM_REG)
		{
		  ...
		  if (align_words + n_words > GP_ARG_NUM_REG
		      ...
		      do
			{
		      ...
		      while (++align_words < GP_ARG_NUM_REG
			     && --n_words != 0);
		    }
		  else
		    {
			    ...
			    tree type, int named)
{
  ...
  if (USE_FP_FOR_ARG_P (cum, mode, type)
      ...
      && !(type
	   && (cum->nargs_prototype <= 0
	       || (DEFAULT_ABI == ABI_AIX
		   && TARGET_XL_COMPAT && align_words >= GP_ARG_NUM_REG))))
    {
      if (cum->fregno + ((GET_MODE_SIZE (mode) + 7) >> 3) >
	  FP_ARG_MAX_REG + 1)
			...
			int no_rtl)
{
  ...
  if (DEFAULT_ABI == ABI_V4
      && TARGET_HARD_FLOAT && TARGET_FPRS
      && !no_rtl && next_cum.fregno <= FP_ARG_V4_MAX_REG)
    {
...
rs6000_va_arg (tree valist, tree type)
{
  ...
  if (TARGET_ALTIVEC_ABI && ALTIVEC_VECTOR_MODE (TYPE_MODE (type)))
	...
	(expand_expr (u, NULL_RTX, QImode, EXPAND_NORMAL),
	 GEN_INT (8 - n_reg + 1), GE, const1_rtx, QImode, 1, lab_false);
...
static const struct builtin_description bdesc_3arg[] = {
  {MASK_ALTIVEC, CODE_FOR_altivec_vmaddfp, "__builtin_altivec_vmaddfp",
   ALTIVEC_BUILTIN_VMADDFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vmhaddshs, "__builtin_altivec_vmhaddshs",
   ALTIVEC_BUILTIN_VMHADDSHS},
  {MASK_ALTIVEC, CODE_FOR_altivec_vmhraddshs, "__builtin_altivec_vmhraddshs",
   ALTIVEC_BUILTIN_VMHRADDSHS},
  {MASK_ALTIVEC, CODE_FOR_altivec_vmladduhm, "__builtin_altivec_vmladduhm",
   ALTIVEC_BUILTIN_VMLADDUHM},
  {MASK_ALTIVEC, CODE_FOR_altivec_vmsumubm, "__builtin_altivec_vmsumubm",
   ALTIVEC_BUILTIN_VMSUMUBM},
  {MASK_ALTIVEC, CODE_FOR_altivec_vmsummbm, "__builtin_altivec_vmsummbm",
   ALTIVEC_BUILTIN_VMSUMMBM},
  {MASK_ALTIVEC, CODE_FOR_altivec_vmsumuhm, "__builtin_altivec_vmsumuhm",
   ALTIVEC_BUILTIN_VMSUMUHM},
  {MASK_ALTIVEC, CODE_FOR_altivec_vmsumshm, "__builtin_altivec_vmsumshm",
   ALTIVEC_BUILTIN_VMSUMSHM},
  {MASK_ALTIVEC, CODE_FOR_altivec_vmsumuhs, "__builtin_altivec_vmsumuhs",
   ALTIVEC_BUILTIN_VMSUMUHS},
  {MASK_ALTIVEC, CODE_FOR_altivec_vmsumshs, "__builtin_altivec_vmsumshs",
   ALTIVEC_BUILTIN_VMSUMSHS},
  {MASK_ALTIVEC, CODE_FOR_altivec_vnmsubfp, "__builtin_altivec_vnmsubfp",
   ALTIVEC_BUILTIN_VNMSUBFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vperm_4sf, "__builtin_altivec_vperm_4sf",
   ALTIVEC_BUILTIN_VPERM_4SF},
  {MASK_ALTIVEC, CODE_FOR_altivec_vperm_4si, "__builtin_altivec_vperm_4si",
   ALTIVEC_BUILTIN_VPERM_4SI},
  {MASK_ALTIVEC, CODE_FOR_altivec_vperm_8hi, "__builtin_altivec_vperm_8hi",
   ALTIVEC_BUILTIN_VPERM_8HI},
  {MASK_ALTIVEC, CODE_FOR_altivec_vperm_16qi, "__builtin_altivec_vperm_16qi",
   ALTIVEC_BUILTIN_VPERM_16QI},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsel_4sf, "__builtin_altivec_vsel_4sf",
   ALTIVEC_BUILTIN_VSEL_4SF},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsel_4si, "__builtin_altivec_vsel_4si",
   ALTIVEC_BUILTIN_VSEL_4SI},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsel_8hi, "__builtin_altivec_vsel_8hi",
   ALTIVEC_BUILTIN_VSEL_8HI},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsel_16qi, "__builtin_altivec_vsel_16qi",
   ALTIVEC_BUILTIN_VSEL_16QI},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsldoi_16qi,
   "__builtin_altivec_vsldoi_16qi", ALTIVEC_BUILTIN_VSLDOI_16QI},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsldoi_8hi, "__builtin_altivec_vsldoi_8hi",
   ALTIVEC_BUILTIN_VSLDOI_8HI},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsldoi_4si, "__builtin_altivec_vsldoi_4si",
   ALTIVEC_BUILTIN_VSLDOI_4SI},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsldoi_4sf, "__builtin_altivec_vsldoi_4sf",
...
static const struct builtin_description bdesc_dst[] = {
  {MASK_ALTIVEC, CODE_FOR_altivec_dst, "__builtin_altivec_dst",
   ALTIVEC_BUILTIN_DST},
  {MASK_ALTIVEC, CODE_FOR_altivec_dstt, "__builtin_altivec_dstt",
   ALTIVEC_BUILTIN_DSTT},
  {MASK_ALTIVEC, CODE_FOR_altivec_dstst, "__builtin_altivec_dstst",
   ALTIVEC_BUILTIN_DSTST},
  {MASK_ALTIVEC, CODE_FOR_altivec_dststt, "__builtin_altivec_dststt",
...
static struct builtin_description bdesc_2arg[] = {
  {MASK_ALTIVEC, CODE_FOR_addv16qi3, "__builtin_altivec_vaddubm",
   ALTIVEC_BUILTIN_VADDUBM},
  {MASK_ALTIVEC, CODE_FOR_addv8hi3, "__builtin_altivec_vadduhm",
  ...
  {MASK_ALTIVEC, CODE_FOR_andv4si3, "__builtin_altivec_vand",
  ...
  {MASK_ALTIVEC, CODE_FOR_altivec_vcfux, "__builtin_altivec_vcfux",
   ALTIVEC_BUILTIN_VCFUX},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcfsx, "__builtin_altivec_vcfsx",
   ALTIVEC_BUILTIN_VCFSX},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpbfp, "__builtin_altivec_vcmpbfp",
   ALTIVEC_BUILTIN_VCMPBFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpequb, "__builtin_altivec_vcmpequb",
   ALTIVEC_BUILTIN_VCMPEQUB},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpequh, "__builtin_altivec_vcmpequh",
   ALTIVEC_BUILTIN_VCMPEQUH},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpequw, "__builtin_altivec_vcmpequw",
   ALTIVEC_BUILTIN_VCMPEQUW},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpeqfp, "__builtin_altivec_vcmpeqfp",
   ALTIVEC_BUILTIN_VCMPEQFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpgefp, "__builtin_altivec_vcmpgefp",
   ALTIVEC_BUILTIN_VCMPGEFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpgtub, "__builtin_altivec_vcmpgtub",
   ALTIVEC_BUILTIN_VCMPGTUB},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpgtsb, "__builtin_altivec_vcmpgtsb",
   ALTIVEC_BUILTIN_VCMPGTSB},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpgtuh, "__builtin_altivec_vcmpgtuh",
   ALTIVEC_BUILTIN_VCMPGTUH},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpgtsh, "__builtin_altivec_vcmpgtsh",
   ALTIVEC_BUILTIN_VCMPGTSH},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpgtuw, "__builtin_altivec_vcmpgtuw",
   ALTIVEC_BUILTIN_VCMPGTUW},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpgtsw, "__builtin_altivec_vcmpgtsw",
   ALTIVEC_BUILTIN_VCMPGTSW},
  {MASK_ALTIVEC, CODE_FOR_altivec_vcmpgtfp, "__builtin_altivec_vcmpgtfp",
   ALTIVEC_BUILTIN_VCMPGTFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vctsxs, "__builtin_altivec_vctsxs",
   ALTIVEC_BUILTIN_VCTSXS},
  {MASK_ALTIVEC, CODE_FOR_altivec_vctuxs, "__builtin_altivec_vctuxs",
   ALTIVEC_BUILTIN_VCTUXS},
  {MASK_ALTIVEC, CODE_FOR_umaxv16qi3, "__builtin_altivec_vmaxub",
   ALTIVEC_BUILTIN_VMAXUB},
  {MASK_ALTIVEC, CODE_FOR_smaxv16qi3, "__builtin_altivec_vmaxsb",
   ALTIVEC_BUILTIN_VMAXSB},
  {MASK_ALTIVEC, CODE_FOR_umaxv8hi3, "__builtin_altivec_vmaxuh",
   ALTIVEC_BUILTIN_VMAXUH},
  {MASK_ALTIVEC, CODE_FOR_smaxv8hi3, "__builtin_altivec_vmaxsh",
   ALTIVEC_BUILTIN_VMAXSH},
  {MASK_ALTIVEC, CODE_FOR_umaxv4si3, "__builtin_altivec_vmaxuw",
   ALTIVEC_BUILTIN_VMAXUW},
  {MASK_ALTIVEC, CODE_FOR_smaxv4si3, "__builtin_altivec_vmaxsw",
   ALTIVEC_BUILTIN_VMAXSW},
  {MASK_ALTIVEC, CODE_FOR_smaxv4sf3, "__builtin_altivec_vmaxfp",
  ...
  {MASK_ALTIVEC, CODE_FOR_uminv16qi3, "__builtin_altivec_vminub",
   ALTIVEC_BUILTIN_VMINUB},
  {MASK_ALTIVEC, CODE_FOR_sminv16qi3, "__builtin_altivec_vminsb",
   ALTIVEC_BUILTIN_VMINSB},
  {MASK_ALTIVEC, CODE_FOR_uminv8hi3, "__builtin_altivec_vminuh",
   ALTIVEC_BUILTIN_VMINUH},
  {MASK_ALTIVEC, CODE_FOR_sminv8hi3, "__builtin_altivec_vminsh",
   ALTIVEC_BUILTIN_VMINSH},
  {MASK_ALTIVEC, CODE_FOR_uminv4si3, "__builtin_altivec_vminuw",
   ALTIVEC_BUILTIN_VMINUW},
  {MASK_ALTIVEC, CODE_FOR_sminv4si3, "__builtin_altivec_vminsw",
   ALTIVEC_BUILTIN_VMINSW},
  {MASK_ALTIVEC, CODE_FOR_sminv4sf3, "__builtin_altivec_vminfp",
  ...
  {MASK_ALTIVEC, CODE_FOR_iorv4si3, "__builtin_altivec_vor",
   ALTIVEC_BUILTIN_VOR},
  ...
  {MASK_ALTIVEC, CODE_FOR_altivec_vsplth, "__builtin_altivec_vsplth",
   ALTIVEC_BUILTIN_VSPLTH},
  {MASK_ALTIVEC, CODE_FOR_altivec_vspltw, "__builtin_altivec_vspltw",
  ...
  {MASK_ALTIVEC, CODE_FOR_subv8hi3, "__builtin_altivec_vsubuhm",
   ALTIVEC_BUILTIN_VSUBUHM},
  {MASK_ALTIVEC, CODE_FOR_subv4si3, "__builtin_altivec_vsubuwm",
   ALTIVEC_BUILTIN_VSUBUWM},
  {MASK_ALTIVEC, CODE_FOR_subv4sf3, "__builtin_altivec_vsubfp",
  ...
  {MASK_ALTIVEC, CODE_FOR_altivec_vsum4ubs, "__builtin_altivec_vsum4ubs",
   ALTIVEC_BUILTIN_VSUM4UBS},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsum4sbs, "__builtin_altivec_vsum4sbs",
   ALTIVEC_BUILTIN_VSUM4SBS},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsum4shs, "__builtin_altivec_vsum4shs",
   ALTIVEC_BUILTIN_VSUM4SHS},
  {MASK_ALTIVEC, CODE_FOR_altivec_vsum2sws, "__builtin_altivec_vsum2sws",
  ...
  {0, CODE_FOR_spe_evmergehi, "__builtin_spe_evmergehi",
   SPE_BUILTIN_EVMERGEHI},
  {0, CODE_FOR_spe_evmergehilo, "__builtin_spe_evmergehilo",
   SPE_BUILTIN_EVMERGEHILO},
  {0, CODE_FOR_spe_evmergelo, "__builtin_spe_evmergelo",
   SPE_BUILTIN_EVMERGELO},
  {0, CODE_FOR_spe_evmergelohi, "__builtin_spe_evmergelohi",
   SPE_BUILTIN_EVMERGELOHI},
  {0, CODE_FOR_spe_evmhegsmfaa, "__builtin_spe_evmhegsmfaa",
   SPE_BUILTIN_EVMHEGSMFAA},
  {0, CODE_FOR_spe_evmhegsmfan, "__builtin_spe_evmhegsmfan",
   SPE_BUILTIN_EVMHEGSMFAN},
  {0, CODE_FOR_spe_evmhegsmiaa, "__builtin_spe_evmhegsmiaa",
   SPE_BUILTIN_EVMHEGSMIAA},
  {0, CODE_FOR_spe_evmhegsmian, "__builtin_spe_evmhegsmian",
   SPE_BUILTIN_EVMHEGSMIAN},
  {0, CODE_FOR_spe_evmhegumiaa, "__builtin_spe_evmhegumiaa",
   SPE_BUILTIN_EVMHEGUMIAA},
  {0, CODE_FOR_spe_evmhegumian, "__builtin_spe_evmhegumian",
   SPE_BUILTIN_EVMHEGUMIAN},
  {0, CODE_FOR_spe_evmhesmf, "__builtin_spe_evmhesmf", SPE_BUILTIN_EVMHESMF},
  {0, CODE_FOR_spe_evmhesmfa, "__builtin_spe_evmhesmfa",
   SPE_BUILTIN_EVMHESMFA},
  {0, CODE_FOR_spe_evmhesmfaaw, "__builtin_spe_evmhesmfaaw",
   SPE_BUILTIN_EVMHESMFAAW},
  {0, CODE_FOR_spe_evmhesmfanw, "__builtin_spe_evmhesmfanw",
   SPE_BUILTIN_EVMHESMFANW},
  {0, CODE_FOR_spe_evmhesmi, "__builtin_spe_evmhesmi", SPE_BUILTIN_EVMHESMI},
  {0, CODE_FOR_spe_evmhesmia, "__builtin_spe_evmhesmia",
   SPE_BUILTIN_EVMHESMIA},
  {0, CODE_FOR_spe_evmhesmiaaw, "__builtin_spe_evmhesmiaaw",
   SPE_BUILTIN_EVMHESMIAAW},
  {0, CODE_FOR_spe_evmhesmianw, "__builtin_spe_evmhesmianw",
   SPE_BUILTIN_EVMHESMIANW},
  {0, CODE_FOR_spe_evmhessf, "__builtin_spe_evmhessf", SPE_BUILTIN_EVMHESSF},
  {0, CODE_FOR_spe_evmhessfa, "__builtin_spe_evmhessfa",
   SPE_BUILTIN_EVMHESSFA},
  {0, CODE_FOR_spe_evmhessfaaw, "__builtin_spe_evmhessfaaw",
   SPE_BUILTIN_EVMHESSFAAW},
  {0, CODE_FOR_spe_evmhessfanw, "__builtin_spe_evmhessfanw",
   SPE_BUILTIN_EVMHESSFANW},
  {0, CODE_FOR_spe_evmhessiaaw, "__builtin_spe_evmhessiaaw",
   SPE_BUILTIN_EVMHESSIAAW},
  {0, CODE_FOR_spe_evmhessianw, "__builtin_spe_evmhessianw",
   SPE_BUILTIN_EVMHESSIANW},
  {0, CODE_FOR_spe_evmheumi, "__builtin_spe_evmheumi", SPE_BUILTIN_EVMHEUMI},
  {0, CODE_FOR_spe_evmheumia, "__builtin_spe_evmheumia",
   SPE_BUILTIN_EVMHEUMIA},
  {0, CODE_FOR_spe_evmheumiaaw, "__builtin_spe_evmheumiaaw",
   SPE_BUILTIN_EVMHEUMIAAW},
  {0, CODE_FOR_spe_evmheumianw, "__builtin_spe_evmheumianw",
   SPE_BUILTIN_EVMHEUMIANW},
  {0, CODE_FOR_spe_evmheusiaaw, "__builtin_spe_evmheusiaaw",
   SPE_BUILTIN_EVMHEUSIAAW},
  {0, CODE_FOR_spe_evmheusianw, "__builtin_spe_evmheusianw",
   SPE_BUILTIN_EVMHEUSIANW},
  {0, CODE_FOR_spe_evmhogsmfaa, "__builtin_spe_evmhogsmfaa",
   SPE_BUILTIN_EVMHOGSMFAA},
  {0, CODE_FOR_spe_evmhogsmfan, "__builtin_spe_evmhogsmfan",
   SPE_BUILTIN_EVMHOGSMFAN},
  {0, CODE_FOR_spe_evmhogsmiaa, "__builtin_spe_evmhogsmiaa",
   SPE_BUILTIN_EVMHOGSMIAA},
  {0, CODE_FOR_spe_evmhogsmian, "__builtin_spe_evmhogsmian",
   SPE_BUILTIN_EVMHOGSMIAN},
  {0, CODE_FOR_spe_evmhogumiaa, "__builtin_spe_evmhogumiaa",
   SPE_BUILTIN_EVMHOGUMIAA},
  {0, CODE_FOR_spe_evmhogumian, "__builtin_spe_evmhogumian",
   SPE_BUILTIN_EVMHOGUMIAN},
  {0, CODE_FOR_spe_evmhosmf, "__builtin_spe_evmhosmf", SPE_BUILTIN_EVMHOSMF},
  {0, CODE_FOR_spe_evmhosmfa, "__builtin_spe_evmhosmfa",
   SPE_BUILTIN_EVMHOSMFA},
  {0, CODE_FOR_spe_evmhosmfaaw, "__builtin_spe_evmhosmfaaw",
   SPE_BUILTIN_EVMHOSMFAAW},
  {0, CODE_FOR_spe_evmhosmfanw, "__builtin_spe_evmhosmfanw",
   SPE_BUILTIN_EVMHOSMFANW},
  {0, CODE_FOR_spe_evmhosmi, "__builtin_spe_evmhosmi", SPE_BUILTIN_EVMHOSMI},
  {0, CODE_FOR_spe_evmhosmia, "__builtin_spe_evmhosmia",
   SPE_BUILTIN_EVMHOSMIA},
  {0, CODE_FOR_spe_evmhosmiaaw, "__builtin_spe_evmhosmiaaw",
   SPE_BUILTIN_EVMHOSMIAAW},
  {0, CODE_FOR_spe_evmhosmianw, "__builtin_spe_evmhosmianw",
   SPE_BUILTIN_EVMHOSMIANW},
  {0, CODE_FOR_spe_evmhossf, "__builtin_spe_evmhossf", SPE_BUILTIN_EVMHOSSF},
  {0, CODE_FOR_spe_evmhossfa, "__builtin_spe_evmhossfa",
   SPE_BUILTIN_EVMHOSSFA},
  {0, CODE_FOR_spe_evmhossfaaw, "__builtin_spe_evmhossfaaw",
   SPE_BUILTIN_EVMHOSSFAAW},
  {0, CODE_FOR_spe_evmhossfanw, "__builtin_spe_evmhossfanw",
   SPE_BUILTIN_EVMHOSSFANW},
  {0, CODE_FOR_spe_evmhossiaaw, "__builtin_spe_evmhossiaaw",
   SPE_BUILTIN_EVMHOSSIAAW},
  {0, CODE_FOR_spe_evmhossianw, "__builtin_spe_evmhossianw",
   SPE_BUILTIN_EVMHOSSIANW},
  {0, CODE_FOR_spe_evmhoumi, "__builtin_spe_evmhoumi", SPE_BUILTIN_EVMHOUMI},
  {0, CODE_FOR_spe_evmhoumia, "__builtin_spe_evmhoumia",
   SPE_BUILTIN_EVMHOUMIA},
  {0, CODE_FOR_spe_evmhoumiaaw, "__builtin_spe_evmhoumiaaw",
   SPE_BUILTIN_EVMHOUMIAAW},
  {0, CODE_FOR_spe_evmhoumianw, "__builtin_spe_evmhoumianw",
   SPE_BUILTIN_EVMHOUMIANW},
  {0, CODE_FOR_spe_evmhousiaaw, "__builtin_spe_evmhousiaaw",
   SPE_BUILTIN_EVMHOUSIAAW},
  {0, CODE_FOR_spe_evmhousianw, "__builtin_spe_evmhousianw",
   SPE_BUILTIN_EVMHOUSIANW},
  {0, CODE_FOR_spe_evmwhsmf, "__builtin_spe_evmwhsmf", SPE_BUILTIN_EVMWHSMF},
  {0, CODE_FOR_spe_evmwhsmfa, "__builtin_spe_evmwhsmfa",
   SPE_BUILTIN_EVMWHSMFA},
  {0, CODE_FOR_spe_evmwhsmi, "__builtin_spe_evmwhsmi", SPE_BUILTIN_EVMWHSMI},
  {0, CODE_FOR_spe_evmwhsmia, "__builtin_spe_evmwhsmia",
   SPE_BUILTIN_EVMWHSMIA},
  {0, CODE_FOR_spe_evmwhssf, "__builtin_spe_evmwhssf", SPE_BUILTIN_EVMWHSSF},
  {0, CODE_FOR_spe_evmwhssfa, "__builtin_spe_evmwhssfa",
   SPE_BUILTIN_EVMWHSSFA},
  {0, CODE_FOR_spe_evmwhumi, "__builtin_spe_evmwhumi", SPE_BUILTIN_EVMWHUMI},
  {0, CODE_FOR_spe_evmwhumia, "__builtin_spe_evmwhumia",
   SPE_BUILTIN_EVMWHUMIA},
  {0, CODE_FOR_spe_evmwlsmiaaw, "__builtin_spe_evmwlsmiaaw",
   SPE_BUILTIN_EVMWLSMIAAW},
  {0, CODE_FOR_spe_evmwlsmianw, "__builtin_spe_evmwlsmianw",
   SPE_BUILTIN_EVMWLSMIANW},
  {0, CODE_FOR_spe_evmwlssiaaw, "__builtin_spe_evmwlssiaaw",
   SPE_BUILTIN_EVMWLSSIAAW},
  {0, CODE_FOR_spe_evmwlssianw, "__builtin_spe_evmwlssianw",
   SPE_BUILTIN_EVMWLSSIANW},
  {0, CODE_FOR_spe_evmwlumi, "__builtin_spe_evmwlumi", SPE_BUILTIN_EVMWLUMI},
  {0, CODE_FOR_spe_evmwlumia, "__builtin_spe_evmwlumia",
   SPE_BUILTIN_EVMWLUMIA},
  {0, CODE_FOR_spe_evmwlumiaaw, "__builtin_spe_evmwlumiaaw",
   SPE_BUILTIN_EVMWLUMIAAW},
  {0, CODE_FOR_spe_evmwlumianw, "__builtin_spe_evmwlumianw",
   SPE_BUILTIN_EVMWLUMIANW},
  {0, CODE_FOR_spe_evmwlusiaaw, "__builtin_spe_evmwlusiaaw",
   SPE_BUILTIN_EVMWLUSIAAW},
  {0, CODE_FOR_spe_evmwlusianw, "__builtin_spe_evmwlusianw",
  ...
  {0, CODE_FOR_spe_evmwsmfaa, "__builtin_spe_evmwsmfaa",
   SPE_BUILTIN_EVMWSMFAA},
  {0, CODE_FOR_spe_evmwsmfan, "__builtin_spe_evmwsmfan",
  ...
  {0, CODE_FOR_spe_evmwsmiaa, "__builtin_spe_evmwsmiaa",
   SPE_BUILTIN_EVMWSMIAA},
  {0, CODE_FOR_spe_evmwsmian, "__builtin_spe_evmwsmian",
  ...
  {0, CODE_FOR_spe_evmwssfaa, "__builtin_spe_evmwssfaa",
   SPE_BUILTIN_EVMWSSFAA},
  {0, CODE_FOR_spe_evmwssfan, "__builtin_spe_evmwssfan",
  ...
  {0, CODE_FOR_spe_evmwumiaa, "__builtin_spe_evmwumiaa",
   SPE_BUILTIN_EVMWUMIAA},
  {0, CODE_FOR_spe_evmwumian, "__builtin_spe_evmwumian",
  ...
  {0, CODE_FOR_spe_evmwhssfaa, "__builtin_spe_evmwhssfaa",
   SPE_BUILTIN_EVMWHSSFAA},
  {0, CODE_FOR_spe_evmwhssmaa, "__builtin_spe_evmwhssmaa",
   SPE_BUILTIN_EVMWHSSMAA},
  {0, CODE_FOR_spe_evmwhsmfaa, "__builtin_spe_evmwhsmfaa",
   SPE_BUILTIN_EVMWHSMFAA},
  {0, CODE_FOR_spe_evmwhsmiaa, "__builtin_spe_evmwhsmiaa",
   SPE_BUILTIN_EVMWHSMIAA},
  {0, CODE_FOR_spe_evmwhusiaa, "__builtin_spe_evmwhusiaa",
   SPE_BUILTIN_EVMWHUSIAA},
  {0, CODE_FOR_spe_evmwhumiaa, "__builtin_spe_evmwhumiaa",
   SPE_BUILTIN_EVMWHUMIAA},
  {0, CODE_FOR_spe_evmwhssfan, "__builtin_spe_evmwhssfan",
   SPE_BUILTIN_EVMWHSSFAN},
  {0, CODE_FOR_spe_evmwhssian, "__builtin_spe_evmwhssian",
   SPE_BUILTIN_EVMWHSSIAN},
  {0, CODE_FOR_spe_evmwhsmfan, "__builtin_spe_evmwhsmfan",
   SPE_BUILTIN_EVMWHSMFAN},
  {0, CODE_FOR_spe_evmwhsmian, "__builtin_spe_evmwhsmian",
   SPE_BUILTIN_EVMWHSMIAN},
  {0, CODE_FOR_spe_evmwhusian, "__builtin_spe_evmwhusian",
   SPE_BUILTIN_EVMWHUSIAN},
  {0, CODE_FOR_spe_evmwhumian, "__builtin_spe_evmwhumian",
   SPE_BUILTIN_EVMWHUMIAN},
  {0, CODE_FOR_spe_evmwhgssfaa, "__builtin_spe_evmwhgssfaa",
   SPE_BUILTIN_EVMWHGSSFAA},
  {0, CODE_FOR_spe_evmwhgsmfaa, "__builtin_spe_evmwhgsmfaa",
   SPE_BUILTIN_EVMWHGSMFAA},
  {0, CODE_FOR_spe_evmwhgsmiaa, "__builtin_spe_evmwhgsmiaa",
   SPE_BUILTIN_EVMWHGSMIAA},
  {0, CODE_FOR_spe_evmwhgumiaa, "__builtin_spe_evmwhgumiaa",
   SPE_BUILTIN_EVMWHGUMIAA},
  {0, CODE_FOR_spe_evmwhgssfan, "__builtin_spe_evmwhgssfan",
   SPE_BUILTIN_EVMWHGSSFAN},
  {0, CODE_FOR_spe_evmwhgsmfan, "__builtin_spe_evmwhgsmfan",
   SPE_BUILTIN_EVMWHGSMFAN},
  {0, CODE_FOR_spe_evmwhgsmian, "__builtin_spe_evmwhgsmian",
   SPE_BUILTIN_EVMWHGSMIAN},
  {0, CODE_FOR_spe_evmwhgumian, "__builtin_spe_evmwhgumian",
...
static const struct builtin_description_predicates bdesc_altivec_preds[] = {
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v4sf, "*vcmpbfp.",
   "__builtin_altivec_vcmpbfp_p", ALTIVEC_BUILTIN_VCMPBFP_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v4sf, "*vcmpeqfp.",
   "__builtin_altivec_vcmpeqfp_p", ALTIVEC_BUILTIN_VCMPEQFP_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v4sf, "*vcmpgefp.",
   "__builtin_altivec_vcmpgefp_p", ALTIVEC_BUILTIN_VCMPGEFP_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v4sf, "*vcmpgtfp.",
   "__builtin_altivec_vcmpgtfp_p", ALTIVEC_BUILTIN_VCMPGTFP_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v4si, "*vcmpequw.",
   "__builtin_altivec_vcmpequw_p", ALTIVEC_BUILTIN_VCMPEQUW_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v4si, "*vcmpgtsw.",
   "__builtin_altivec_vcmpgtsw_p", ALTIVEC_BUILTIN_VCMPGTSW_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v4si, "*vcmpgtuw.",
   "__builtin_altivec_vcmpgtuw_p", ALTIVEC_BUILTIN_VCMPGTUW_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v8hi, "*vcmpgtuh.",
   "__builtin_altivec_vcmpgtuh_p", ALTIVEC_BUILTIN_VCMPGTUH_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v8hi, "*vcmpgtsh.",
   "__builtin_altivec_vcmpgtsh_p", ALTIVEC_BUILTIN_VCMPGTSH_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v8hi, "*vcmpequh.",
   "__builtin_altivec_vcmpequh_p", ALTIVEC_BUILTIN_VCMPEQUH_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v16qi, "*vcmpequb.",
   "__builtin_altivec_vcmpequb_p", ALTIVEC_BUILTIN_VCMPEQUB_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v16qi, "*vcmpgtsb.",
   "__builtin_altivec_vcmpgtsb_p", ALTIVEC_BUILTIN_VCMPGTSB_P},
  {MASK_ALTIVEC, CODE_FOR_altivec_predicate_v16qi, "*vcmpgtub.",
...
static struct builtin_description bdesc_spe_predicates[] = {
  ...
  {0, CODE_FOR_spe_evfscmpeq, "__builtin_spe_evfscmpeq",
   SPE_BUILTIN_EVFSCMPEQ},
  {0, CODE_FOR_spe_evfscmpgt, "__builtin_spe_evfscmpgt",
   SPE_BUILTIN_EVFSCMPGT},
  {0, CODE_FOR_spe_evfscmplt, "__builtin_spe_evfscmplt",
   SPE_BUILTIN_EVFSCMPLT},
  {0, CODE_FOR_spe_evfststeq, "__builtin_spe_evfststeq",
   SPE_BUILTIN_EVFSTSTEQ},
  {0, CODE_FOR_spe_evfststgt, "__builtin_spe_evfststgt",
   SPE_BUILTIN_EVFSTSTGT},
  /* Place-holder.  Leave as last.  */
  {0, CODE_FOR_spe_evfststlt, "__builtin_spe_evfststlt",
...
static struct builtin_description bdesc_spe_evsel[] = {
  /* Place-holder.  Leave as first.  */
  {0, CODE_FOR_spe_evcmpgts, "__builtin_spe_evsel_gts",
   SPE_BUILTIN_EVSEL_CMPGTS},
  {0, CODE_FOR_spe_evcmpgtu, "__builtin_spe_evsel_gtu",
   SPE_BUILTIN_EVSEL_CMPGTU},
  {0, CODE_FOR_spe_evcmplts, "__builtin_spe_evsel_lts",
   SPE_BUILTIN_EVSEL_CMPLTS},
  {0, CODE_FOR_spe_evcmpltu, "__builtin_spe_evsel_ltu",
   SPE_BUILTIN_EVSEL_CMPLTU},
  {0, CODE_FOR_spe_evcmpeq, "__builtin_spe_evsel_eq",
   SPE_BUILTIN_EVSEL_CMPEQ},
  {0, CODE_FOR_spe_evfscmpgt, "__builtin_spe_evsel_fsgt",
   SPE_BUILTIN_EVSEL_FSCMPGT},
  {0, CODE_FOR_spe_evfscmplt, "__builtin_spe_evsel_fslt",
   SPE_BUILTIN_EVSEL_FSCMPLT},
  {0, CODE_FOR_spe_evfscmpeq, "__builtin_spe_evsel_fseq",
   SPE_BUILTIN_EVSEL_FSCMPEQ},
  {0, CODE_FOR_spe_evfststgt, "__builtin_spe_evsel_fststgt",
   SPE_BUILTIN_EVSEL_FSTSTGT},
  {0, CODE_FOR_spe_evfststlt, "__builtin_spe_evsel_fststlt",
   SPE_BUILTIN_EVSEL_FSTSTLT},
  /* Place-holder.  Leave as last.  */
  {0, CODE_FOR_spe_evfststeq, "__builtin_spe_evsel_fststeq",
...
static const struct builtin_description bdesc_abs[] = {
  {MASK_ALTIVEC, CODE_FOR_absv4si2, "__builtin_altivec_abs_v4si",
   ALTIVEC_BUILTIN_ABS_V4SI},
  {MASK_ALTIVEC, CODE_FOR_absv8hi2, "__builtin_altivec_abs_v8hi",
   ALTIVEC_BUILTIN_ABS_V8HI},
  {MASK_ALTIVEC, CODE_FOR_absv4sf2, "__builtin_altivec_abs_v4sf",
   ALTIVEC_BUILTIN_ABS_V4SF},
  {MASK_ALTIVEC, CODE_FOR_absv16qi2, "__builtin_altivec_abs_v16qi",
   ALTIVEC_BUILTIN_ABS_V16QI},
  {MASK_ALTIVEC, CODE_FOR_altivec_abss_v4si, "__builtin_altivec_abss_v4si",
   ALTIVEC_BUILTIN_ABSS_V4SI},
  {MASK_ALTIVEC, CODE_FOR_altivec_abss_v8hi, "__builtin_altivec_abss_v8hi",
   ALTIVEC_BUILTIN_ABSS_V8HI},
  {MASK_ALTIVEC, CODE_FOR_altivec_abss_v16qi, "__builtin_altivec_abss_v16qi",
...
static struct builtin_description bdesc_1arg[] = {
  {MASK_ALTIVEC, CODE_FOR_altivec_vexptefp, "__builtin_altivec_vexptefp",
   ALTIVEC_BUILTIN_VEXPTEFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vlogefp, "__builtin_altivec_vlogefp",
   ALTIVEC_BUILTIN_VLOGEFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vrefp, "__builtin_altivec_vrefp",
   ALTIVEC_BUILTIN_VREFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vrfim, "__builtin_altivec_vrfim",
   ALTIVEC_BUILTIN_VRFIM},
  {MASK_ALTIVEC, CODE_FOR_altivec_vrfin, "__builtin_altivec_vrfin",
   ALTIVEC_BUILTIN_VRFIN},
  {MASK_ALTIVEC, CODE_FOR_altivec_vrfip, "__builtin_altivec_vrfip",
   ALTIVEC_BUILTIN_VRFIP},
  {MASK_ALTIVEC, CODE_FOR_ftruncv4sf2, "__builtin_altivec_vrfiz",
   ALTIVEC_BUILTIN_VRFIZ},
  {MASK_ALTIVEC, CODE_FOR_altivec_vrsqrtefp, "__builtin_altivec_vrsqrtefp",
   ALTIVEC_BUILTIN_VRSQRTEFP},
  {MASK_ALTIVEC, CODE_FOR_altivec_vspltisb, "__builtin_altivec_vspltisb",
   ALTIVEC_BUILTIN_VSPLTISB},
  {MASK_ALTIVEC, CODE_FOR_altivec_vspltish, "__builtin_altivec_vspltish",
   ALTIVEC_BUILTIN_VSPLTISH},
  {MASK_ALTIVEC, CODE_FOR_altivec_vspltisw, "__builtin_altivec_vspltisw",
   ALTIVEC_BUILTIN_VSPLTISW},
  {MASK_ALTIVEC, CODE_FOR_altivec_vupkhsb, "__builtin_altivec_vupkhsb",
   ALTIVEC_BUILTIN_VUPKHSB},
  {MASK_ALTIVEC, CODE_FOR_altivec_vupkhpx, "__builtin_altivec_vupkhpx",
   ALTIVEC_BUILTIN_VUPKHPX},
  {MASK_ALTIVEC, CODE_FOR_altivec_vupkhsh, "__builtin_altivec_vupkhsh",
   ALTIVEC_BUILTIN_VUPKHSH},
  {MASK_ALTIVEC, CODE_FOR_altivec_vupklsb, "__builtin_altivec_vupklsb",
   ALTIVEC_BUILTIN_VUPKLSB},
  {MASK_ALTIVEC, CODE_FOR_altivec_vupklpx, "__builtin_altivec_vupklpx",
   ALTIVEC_BUILTIN_VUPKLPX},
  {MASK_ALTIVEC, CODE_FOR_altivec_vupklsh, "__builtin_altivec_vupklsh",
   ALTIVEC_BUILTIN_VUPKLSH},

  /* The SPE unary builtins must start with SPE_BUILTIN_EVABS and
     end with SPE_BUILTIN_EVSUBFUSIAAW.  */
  {0, CODE_FOR_spe_evabs, "__builtin_spe_evabs", SPE_BUILTIN_EVABS},
  {0, CODE_FOR_spe_evaddsmiaaw, "__builtin_spe_evaddsmiaaw",
   SPE_BUILTIN_EVADDSMIAAW},
  {0, CODE_FOR_spe_evaddssiaaw, "__builtin_spe_evaddssiaaw",
   SPE_BUILTIN_EVADDSSIAAW},
  {0, CODE_FOR_spe_evaddumiaaw, "__builtin_spe_evaddumiaaw",
   SPE_BUILTIN_EVADDUMIAAW},
  {0, CODE_FOR_spe_evaddusiaaw, "__builtin_spe_evaddusiaaw",
  ...
  {0, CODE_FOR_spe_evfsctsiz, "__builtin_spe_evfsctsiz",
  ...
  {0, CODE_FOR_spe_evfsctuiz, "__builtin_spe_evfsctuiz",
  ...
  {0, CODE_FOR_spe_evsubfsmiaaw, "__builtin_spe_evsubfsmiaaw",
   SPE_BUILTIN_EVSUBFSMIAAW},
  {0, CODE_FOR_spe_evsubfssiaaw, "__builtin_spe_evsubfssiaaw",
   SPE_BUILTIN_EVSUBFSSIAAW},
  {0, CODE_FOR_spe_evsubfumiaaw, "__builtin_spe_evsubfumiaaw",
   SPE_BUILTIN_EVSUBFUMIAAW},

  /* Place-holder.  Leave as last unary SPE builtin.  */
  {0, CODE_FOR_spe_evsubfusiaaw, "__builtin_spe_evsubfusiaaw",
...
rs6000_expand_unop_builtin (enum insn_code icode, tree arglist, rtx target)
{
  ...
  if (icode == CODE_FOR_altivec_vspltisb
      ...
      || icode == CODE_FOR_spe_evsplatfi || icode == CODE_FOR_spe_evsplati)
    {
      /* Only allow 5-bit *signed* literals.  */
      if (GET_CODE (op0) != CONST_INT
	  || INTVAL (op0) > 0x1f || INTVAL (op0) < -0x1f)
	{
...
rs6000_expand_binop_builtin (enum insn_code icode, tree arglist, rtx target)
{
  ...
  if (icode == CODE_FOR_altivec_vcfux
      ...
      || icode == CODE_FOR_spe_evsubifw || icode == CODE_FOR_spe_evsrwiu)
    {
...
spe_expand_stv_builtin (enum insn_code icode, tree arglist)
{
  ...
  if (arg0 == error_mark_node
      || arg1 == error_mark_node || arg2 == error_mark_node)
    return const0_rtx;
...
rs6000_expand_ternop_builtin (enum insn_code icode, tree arglist, rtx target)
{
  ...
  if (icode == CODE_FOR_altivec_vsldoi_4sf
      ...
      || icode == CODE_FOR_altivec_vsldoi_16qi)
    {
      ...
      if (TREE_CODE (arg2) != INTEGER_CST || TREE_INT_CST_LOW (arg2) & ~0xf)
	{
			    ...
			    bool * expandedp)
{
  ...
  *expandedp = false;
  ...
  for (i = 0; i < ARRAY_SIZE (bdesc_dst); i++, d++)
	...
	*expandedp = true;
	STRIP_NOPS (arg2);
	if (TREE_CODE (arg2) != INTEGER_CST || TREE_INT_CST_LOW (arg2) & ~0x3)
	  {
  ...
  *expandedp = true;
  ...
  switch (fcode)
	...
	return const0_rtx;
      ...
      if (TREE_CODE (arg0) != INTEGER_CST || TREE_INT_CST_LOW (arg0) & ~0x3)
	{
  ...
  for (i = 0; i < ARRAY_SIZE (bdesc_altivec_preds); i++, dp++)
    if (dp->code == fcode)
      return altivec_expand_predicate_builtin (dp->icode, dp->opcode, arglist,
					       target);
...
static struct builtin_description bdesc_2arg_spe[] = {
  ...
  {0, CODE_FOR_spe_evlwwsplatx, "__builtin_spe_evlwwsplatx",
   SPE_BUILTIN_EVLWWSPLATX},
  {0, CODE_FOR_spe_evlwhsplatx, "__builtin_spe_evlwhsplatx",
   SPE_BUILTIN_EVLWHSPLATX},
  {0, CODE_FOR_spe_evlhhesplatx, "__builtin_spe_evlhhesplatx",
   SPE_BUILTIN_EVLHHESPLATX},
  {0, CODE_FOR_spe_evlhhousplatx, "__builtin_spe_evlhhousplatx",
   SPE_BUILTIN_EVLHHOUSPLATX},
  {0, CODE_FOR_spe_evlhhossplatx, "__builtin_spe_evlhhossplatx",
  ...
  {0, CODE_FOR_spe_evlwwsplat, "__builtin_spe_evlwwsplat",
   SPE_BUILTIN_EVLWWSPLAT},
  {0, CODE_FOR_spe_evlwhsplat, "__builtin_spe_evlwhsplat",
   SPE_BUILTIN_EVLWHSPLAT},
  {0, CODE_FOR_spe_evlhhesplat, "__builtin_spe_evlhhesplat",
   SPE_BUILTIN_EVLHHESPLAT},
  {0, CODE_FOR_spe_evlhhousplat, "__builtin_spe_evlhhousplat",
   SPE_BUILTIN_EVLHHOUSPLAT},
  {0, CODE_FOR_spe_evlhhossplat, "__builtin_spe_evlhhossplat",
...
rs6000_init_builtins (void)
{
  ...
  (*lang_hooks.decls.
   pushdecl) (build_decl (TYPE_DECL, get_identifier ("__vector signed char"),
			  V16QI_type_node));
  (*lang_hooks.decls.
   pushdecl) (build_decl (TYPE_DECL, get_identifier ("__vector __bool char"),
  ...
  (*lang_hooks.decls.
   pushdecl) (build_decl (TYPE_DECL, get_identifier ("__vector signed short"),
			  V8HI_type_node));
  (*lang_hooks.decls.
   pushdecl) (build_decl (TYPE_DECL, get_identifier ("__vector __bool short"),
			  bool_V8HI_type_node));
  ...
  (*lang_hooks.decls.
   pushdecl) (build_decl (TYPE_DECL, get_identifier ("__vector signed int"),
			  V4SI_type_node));
  (*lang_hooks.decls.
   pushdecl) (build_decl (TYPE_DECL, get_identifier ("__vector __bool int"),
			  bool_V4SI_type_node));
...
spe_init_builtins (void)
{
  ...
  tree v2si_ftype_4_v2si = build_function_type (opaque_V2SI_type_node,
						tree_cons (NULL_TREE,
  ...
  tree v2sf_ftype_4_v2sf = build_function_type (opaque_V2SF_type_node,
						tree_cons (NULL_TREE,
  ...
  tree int_ftype_int_v2si_v2si = build_function_type (integer_type_node,
						      tree_cons (NULL_TREE,
								 integer_type_node,
								 tree_cons
								 (NULL_TREE,
  ...
  tree int_ftype_int_v2sf_v2sf = build_function_type (integer_type_node,
						      tree_cons (NULL_TREE,
								 integer_type_node,
								 tree_cons
								 (NULL_TREE,
  ...
  tree void_ftype_v2si_puint_int = build_function_type (void_type_node,
							tree_cons (NULL_TREE,
								   opaque_V2SI_type_node,
								   tree_cons
								   (NULL_TREE,
  ...
  tree void_ftype_v2si_puint_char = build_function_type (void_type_node,
							 tree_cons (NULL_TREE,
								    opaque_V2SI_type_node,
								    tree_cons
								    (NULL_TREE,
  ...
  tree void_ftype_v2si_pv2si_int = build_function_type (void_type_node,
							tree_cons (NULL_TREE,
								   opaque_V2SI_type_node,
								   tree_cons
								   (NULL_TREE,
  ...
  tree void_ftype_v2si_pv2si_char = build_function_type (void_type_node,
							 tree_cons (NULL_TREE,
								    opaque_V2SI_type_node,
								    tree_cons
								    (NULL_TREE,
  ...
  tree void_ftype_int = build_function_type (void_type_node,
					     tree_cons (NULL_TREE,
							integer_type_node,
							endlink));

  tree int_ftype_void = build_function_type (integer_type_node, endlink);

  tree v2si_ftype_pv2si_int = build_function_type (opaque_V2SI_type_node,
						   tree_cons (NULL_TREE,
							      opaque_p_V2SI_type_node,
							      tree_cons
							      (NULL_TREE,
  ...
  tree v2si_ftype_puint_int = build_function_type (opaque_V2SI_type_node,
						   tree_cons (NULL_TREE,
							      puint_type_node,
							      tree_cons
							      (NULL_TREE,
  ...
  tree v2si_ftype_pushort_int = build_function_type (opaque_V2SI_type_node,
						     tree_cons (NULL_TREE,
								pushort_type_node,
								tree_cons
								(NULL_TREE,
  ...
  tree v2si_ftype_signed_char = build_function_type (opaque_V2SI_type_node,
						     tree_cons (NULL_TREE,
  ...
  enable_mask_for_builtins ((struct builtin_description *) bdesc_2arg,
			    ...
			    SPE_BUILTIN_EVADDW, SPE_BUILTIN_EVXOR);
  enable_mask_for_builtins ((struct builtin_description *) bdesc_1arg,
			    ARRAY_SIZE (bdesc_1arg),
			    SPE_BUILTIN_EVABS, SPE_BUILTIN_EVSUBFUSIAAW);
  enable_mask_for_builtins ((struct builtin_description *)
			    bdesc_spe_predicates,
			    ARRAY_SIZE (bdesc_spe_predicates),
			    SPE_BUILTIN_EVCMPEQ, SPE_BUILTIN_EVFSTSTLT);
  ...
  def_builtin (target_flags, "__builtin_spe_mtspefscr", void_ftype_int,
	       SPE_BUILTIN_MTSPEFSCR);
  def_builtin (target_flags, "__builtin_spe_mfspefscr", int_ftype_void,
	       SPE_BUILTIN_MFSPEFSCR);
  def_builtin (target_flags, "__builtin_spe_evstddx",
	       void_ftype_v2si_pv2si_int, SPE_BUILTIN_EVSTDDX);
  def_builtin (target_flags, "__builtin_spe_evstdhx",
	       void_ftype_v2si_pv2si_int, SPE_BUILTIN_EVSTDHX);
  def_builtin (target_flags, "__builtin_spe_evstdwx",
	       void_ftype_v2si_pv2si_int, SPE_BUILTIN_EVSTDWX);
  def_builtin (target_flags, "__builtin_spe_evstwhex",
	       void_ftype_v2si_puint_int, SPE_BUILTIN_EVSTWHEX);
  def_builtin (target_flags, "__builtin_spe_evstwhox",
	       void_ftype_v2si_puint_int, SPE_BUILTIN_EVSTWHOX);
  def_builtin (target_flags, "__builtin_spe_evstwwex",
	       void_ftype_v2si_puint_int, SPE_BUILTIN_EVSTWWEX);
  def_builtin (target_flags, "__builtin_spe_evstwwox",
	       void_ftype_v2si_puint_int, SPE_BUILTIN_EVSTWWOX);
  def_builtin (target_flags, "__builtin_spe_evstdd",
	       void_ftype_v2si_pv2si_char, SPE_BUILTIN_EVSTDD);
  def_builtin (target_flags, "__builtin_spe_evstdh",
	       void_ftype_v2si_pv2si_char, SPE_BUILTIN_EVSTDH);
  def_builtin (target_flags, "__builtin_spe_evstdw",
	       void_ftype_v2si_pv2si_char, SPE_BUILTIN_EVSTDW);
  def_builtin (target_flags, "__builtin_spe_evstwhe",
	       void_ftype_v2si_puint_char, SPE_BUILTIN_EVSTWHE);
  def_builtin (target_flags, "__builtin_spe_evstwho",
	       void_ftype_v2si_puint_char, SPE_BUILTIN_EVSTWHO);
  def_builtin (target_flags, "__builtin_spe_evstwwe",
	       void_ftype_v2si_puint_char, SPE_BUILTIN_EVSTWWE);
  def_builtin (target_flags, "__builtin_spe_evstwwo",
	       void_ftype_v2si_puint_char, SPE_BUILTIN_EVSTWWO);
  def_builtin (target_flags, "__builtin_spe_evsplatfi",
	       v2si_ftype_signed_char, SPE_BUILTIN_EVSPLATFI);
  def_builtin (target_flags, "__builtin_spe_evsplati", v2si_ftype_signed_char,
	       SPE_BUILTIN_EVSPLATI);

  /* Loads.  */
  def_builtin (target_flags, "__builtin_spe_evlddx", v2si_ftype_pv2si_int,
	       SPE_BUILTIN_EVLDDX);
  def_builtin (target_flags, "__builtin_spe_evldwx", v2si_ftype_pv2si_int,
	       SPE_BUILTIN_EVLDWX);
  def_builtin (target_flags, "__builtin_spe_evldhx", v2si_ftype_pv2si_int,
	       SPE_BUILTIN_EVLDHX);
  def_builtin (target_flags, "__builtin_spe_evlwhex", v2si_ftype_puint_int,
	       SPE_BUILTIN_EVLWHEX);
  def_builtin (target_flags, "__builtin_spe_evlwhoux", v2si_ftype_puint_int,
	       SPE_BUILTIN_EVLWHOUX);
  def_builtin (target_flags, "__builtin_spe_evlwhosx", v2si_ftype_puint_int,
	       SPE_BUILTIN_EVLWHOSX);
  def_builtin (target_flags, "__builtin_spe_evlwwsplatx",
	       v2si_ftype_puint_int, SPE_BUILTIN_EVLWWSPLATX);
  def_builtin (target_flags, "__builtin_spe_evlwhsplatx",
	       v2si_ftype_puint_int, SPE_BUILTIN_EVLWHSPLATX);
  def_builtin (target_flags, "__builtin_spe_evlhhesplatx",
	       v2si_ftype_pushort_int, SPE_BUILTIN_EVLHHESPLATX);
  def_builtin (target_flags, "__builtin_spe_evlhhousplatx",
	       v2si_ftype_pushort_int, SPE_BUILTIN_EVLHHOUSPLATX);
  def_builtin (target_flags, "__builtin_spe_evlhhossplatx",
	       v2si_ftype_pushort_int, SPE_BUILTIN_EVLHHOSSPLATX);
  def_builtin (target_flags, "__builtin_spe_evldd", v2si_ftype_pv2si_int,
	       SPE_BUILTIN_EVLDD);
  def_builtin (target_flags, "__builtin_spe_evldw", v2si_ftype_pv2si_int,
	       SPE_BUILTIN_EVLDW);
  def_builtin (target_flags, "__builtin_spe_evldh", v2si_ftype_pv2si_int,
	       SPE_BUILTIN_EVLDH);
  def_builtin (target_flags, "__builtin_spe_evlhhesplat",
	       v2si_ftype_pushort_int, SPE_BUILTIN_EVLHHESPLAT);
  def_builtin (target_flags, "__builtin_spe_evlhhossplat",
	       v2si_ftype_pushort_int, SPE_BUILTIN_EVLHHOSSPLAT);
  def_builtin (target_flags, "__builtin_spe_evlhhousplat",
	       v2si_ftype_pushort_int, SPE_BUILTIN_EVLHHOUSPLAT);
  def_builtin (target_flags, "__builtin_spe_evlwhe", v2si_ftype_puint_int,
	       SPE_BUILTIN_EVLWHE);
  def_builtin (target_flags, "__builtin_spe_evlwhos", v2si_ftype_puint_int,
	       SPE_BUILTIN_EVLWHOS);
  def_builtin (target_flags, "__builtin_spe_evlwhou", v2si_ftype_puint_int,
	       SPE_BUILTIN_EVLWHOU);
  def_builtin (target_flags, "__builtin_spe_evlwhsplat", v2si_ftype_puint_int,
	       SPE_BUILTIN_EVLWHSPLAT);
  def_builtin (target_flags, "__builtin_spe_evlwwsplat", v2si_ftype_puint_int,
	       SPE_BUILTIN_EVLWWSPLAT);
...
altivec_init_builtins (void)
{
  ...
  tree pcshort_type_node =
    build_pointer_type (build_qualified_type
			(short_integer_type_node, TYPE_QUAL_CONST));
  ...
  tree pcvoid_type_node =
    build_pointer_type (build_qualified_type
			(void_type_node, TYPE_QUAL_CONST));

  tree int_ftype_int_v4si_v4si = build_function_type_list (integer_type_node,
							   integer_type_node,
							   V4SI_type_node,
							   V4SI_type_node,
							   NULL_TREE);
  ...
  tree void_ftype_pint_v4si = build_function_type_list (void_type_node,
							pint_type_node,
							V4SI_type_node,
							NULL_TREE);
  tree v8hi_ftype_pcshort =
    build_function_type_list (V8HI_type_node, pcshort_type_node, NULL_TREE);
  tree void_ftype_pshort_v8hi = build_function_type_list (void_type_node,
							  pshort_type_node,
							  V8HI_type_node,
							  NULL_TREE);
  ...
  tree v8hi_ftype_void = build_function_type (V8HI_type_node, void_list_node);
  tree void_ftype_void = build_function_type (void_type_node, void_list_node);
  tree void_ftype_int
    = build_function_type_list (void_type_node, integer_type_node, NULL_TREE);

  tree v16qi_ftype_long_pcvoid = build_function_type_list (V16QI_type_node,
							   long_integer_type_node,
							   pcvoid_type_node,
							   NULL_TREE);
  tree v8hi_ftype_long_pcvoid = build_function_type_list (V8HI_type_node,
							  long_integer_type_node,
							  pcvoid_type_node,
							  NULL_TREE);
  tree v4si_ftype_long_pcvoid = build_function_type_list (V4SI_type_node,
							  long_integer_type_node,
							  pcvoid_type_node,
							  NULL_TREE);

  tree void_ftype_v4si_long_pvoid = build_function_type_list (void_type_node,
							      V4SI_type_node,
							      long_integer_type_node,
							      pvoid_type_node,
							      NULL_TREE);
  tree void_ftype_v16qi_long_pvoid = build_function_type_list (void_type_node,
							       V16QI_type_node,
							       long_integer_type_node,
							       pvoid_type_node,
							       NULL_TREE);
  tree void_ftype_v8hi_long_pvoid = build_function_type_list (void_type_node,
							      V8HI_type_node,
							      long_integer_type_node,
							      pvoid_type_node,
							      NULL_TREE);
  tree int_ftype_int_v8hi_v8hi = build_function_type_list (integer_type_node,
							   integer_type_node,
							   V8HI_type_node,
							   V8HI_type_node,
							   NULL_TREE);
  ...
  tree int_ftype_int_v4sf_v4sf = build_function_type_list (integer_type_node,
							   integer_type_node,
							   V4SF_type_node,
							   V4SF_type_node,
							   NULL_TREE);
  ...
  tree void_ftype_pcvoid_int_int = build_function_type_list (void_type_node,
							     pcvoid_type_node,
							     integer_type_node,
							     integer_type_node,
							     NULL_TREE);
  tree int_ftype_pcchar = build_function_type_list (integer_type_node,
						    pcchar_type_node,
						    NULL_TREE);

  def_builtin (MASK_ALTIVEC, "__builtin_altivec_ld_internal_4sf",
	       v4sf_ftype_pcfloat, ALTIVEC_BUILTIN_LD_INTERNAL_4sf);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_st_internal_4sf",
	       void_ftype_pfloat_v4sf, ALTIVEC_BUILTIN_ST_INTERNAL_4sf);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_ld_internal_4si",
	       v4si_ftype_pcint, ALTIVEC_BUILTIN_LD_INTERNAL_4si);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_st_internal_4si",
	       void_ftype_pint_v4si, ALTIVEC_BUILTIN_ST_INTERNAL_4si);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_ld_internal_8hi",
	       v8hi_ftype_pcshort, ALTIVEC_BUILTIN_LD_INTERNAL_8hi);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_st_internal_8hi",
	       void_ftype_pshort_v8hi, ALTIVEC_BUILTIN_ST_INTERNAL_8hi);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_ld_internal_16qi",
	       v16qi_ftype_pcchar, ALTIVEC_BUILTIN_LD_INTERNAL_16qi);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_st_internal_16qi",
	       void_ftype_pchar_v16qi, ALTIVEC_BUILTIN_ST_INTERNAL_16qi);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_mtvscr", void_ftype_v4si,
	       ALTIVEC_BUILTIN_MTVSCR);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_mfvscr", v8hi_ftype_void,
	       ALTIVEC_BUILTIN_MFVSCR);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_dssall", void_ftype_void,
	       ALTIVEC_BUILTIN_DSSALL);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_dss", void_ftype_int,
	       ALTIVEC_BUILTIN_DSS);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_lvsl",
	       v16qi_ftype_long_pcvoid, ALTIVEC_BUILTIN_LVSL);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_lvsr",
	       v16qi_ftype_long_pcvoid, ALTIVEC_BUILTIN_LVSR);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_lvebx",
	       v16qi_ftype_long_pcvoid, ALTIVEC_BUILTIN_LVEBX);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_lvehx",
	       v8hi_ftype_long_pcvoid, ALTIVEC_BUILTIN_LVEHX);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_lvewx",
	       v4si_ftype_long_pcvoid, ALTIVEC_BUILTIN_LVEWX);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_lvxl", v4si_ftype_long_pcvoid,
	       ALTIVEC_BUILTIN_LVXL);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_lvx", v4si_ftype_long_pcvoid,
	       ALTIVEC_BUILTIN_LVX);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_stvx",
	       void_ftype_v4si_long_pvoid, ALTIVEC_BUILTIN_STVX);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_stvewx",
	       void_ftype_v4si_long_pvoid, ALTIVEC_BUILTIN_STVEWX);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_stvxl",
	       void_ftype_v4si_long_pvoid, ALTIVEC_BUILTIN_STVXL);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_stvebx",
	       void_ftype_v16qi_long_pvoid, ALTIVEC_BUILTIN_STVEBX);
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_stvehx",
	       void_ftype_v8hi_long_pvoid, ALTIVEC_BUILTIN_STVEHX);

  /* See altivec.h for usage of "__builtin_altivec_compiletime_error".  */
  def_builtin (MASK_ALTIVEC, "__builtin_altivec_compiletime_error",
	       int_ftype_pcchar, ALTIVEC_BUILTIN_COMPILETIME_ERROR);
...
rs6000_common_init_builtins (void)
{
  ...
  tree v4sf_ftype_v4sf_v4sf_v16qi = build_function_type_list (V4SF_type_node,
							      V4SF_type_node,
							      V4SF_type_node,
							      V16QI_type_node,
							      NULL_TREE);
  tree v4si_ftype_v4si_v4si_v16qi = build_function_type_list (V4SI_type_node,
							      V4SI_type_node,
							      V4SI_type_node,
							      V16QI_type_node,
							      NULL_TREE);
  tree v8hi_ftype_v8hi_v8hi_v16qi = build_function_type_list (V8HI_type_node,
							      V8HI_type_node,
							      V8HI_type_node,
							      V16QI_type_node,
							      NULL_TREE);
  ...
  tree v2si_ftype_v2si_v2si = build_function_type_list (opaque_V2SI_type_node,
							opaque_V2SI_type_node,
							opaque_V2SI_type_node,
							NULL_TREE);

  tree v2sf_ftype_v2sf_v2sf = build_function_type_list (opaque_V2SF_type_node,
							opaque_V2SF_type_node,
							opaque_V2SF_type_node,
							NULL_TREE);

  tree v2si_ftype_int_int = build_function_type_list (opaque_V2SI_type_node,
						      integer_type_node,
						      integer_type_node,
						      NULL_TREE);

  tree v2si_ftype_v2si = build_function_type_list (opaque_V2SI_type_node,
						   opaque_V2SI_type_node,
						   NULL_TREE);

  tree v2sf_ftype_v2sf = build_function_type_list (opaque_V2SF_type_node,
						   opaque_V2SF_type_node,
						   NULL_TREE);

  tree v2sf_ftype_v2si = build_function_type_list (opaque_V2SF_type_node,
						   opaque_V2SI_type_node,
						   NULL_TREE);

  tree v2si_ftype_v2sf = build_function_type_list (opaque_V2SI_type_node,
						   opaque_V2SF_type_node,
						   NULL_TREE);

  tree v2si_ftype_v2si_char = build_function_type_list (opaque_V2SI_type_node,
							opaque_V2SI_type_node,
							char_type_node,
							NULL_TREE);

  tree v2si_ftype_int_char = build_function_type_list (opaque_V2SI_type_node,
						       integer_type_node,
						       char_type_node,
						       NULL_TREE);

  tree v2si_ftype_char = build_function_type_list (opaque_V2SI_type_node,
						   char_type_node, NULL_TREE);

  tree int_ftype_int_int = build_function_type_list (integer_type_node,
						     integer_type_node,
						     integer_type_node,
						     NULL_TREE);
  ...
  tree v4si_ftype_v4si_v4si = build_function_type_list (V4SI_type_node,
							...
							V4SI_type_node,
							NULL_TREE);
  tree v4sf_ftype_v4si_int = build_function_type_list (V4SF_type_node,
						       V4SI_type_node,
						       integer_type_node,
						       NULL_TREE);
  tree v4si_ftype_v4sf_int = build_function_type_list (V4SI_type_node,
						       V4SF_type_node,
						       integer_type_node,
						       NULL_TREE);
  ...
  tree v8hi_ftype_v8hi_int = build_function_type_list (V8HI_type_node,
						       ...
						       integer_type_node,
						       NULL_TREE);
  tree v16qi_ftype_v16qi_int = build_function_type_list (V16QI_type_node,
							 V16QI_type_node,
							 integer_type_node,
							 NULL_TREE);
  ...
  tree v8hi_ftype_v8hi_v8hi_int = build_function_type_list (V8HI_type_node,
							    V8HI_type_node,
							    V8HI_type_node,
							    integer_type_node,
							    NULL_TREE);
  tree v4si_ftype_v4si_v4si_int = build_function_type_list (V4SI_type_node,
							    V4SI_type_node,
							    V4SI_type_node,
							    integer_type_node,
							    NULL_TREE);
  ...
  tree v4sf_ftype_v4sf_v4sf = build_function_type_list (V4SF_type_node,
							...
							V4SF_type_node,
							NULL_TREE);
  tree v4sf_ftype_v4sf_v4sf_v4si = build_function_type_list (V4SF_type_node,
							     V4SF_type_node,
							     V4SF_type_node,
							     V4SI_type_node,
							     NULL_TREE);
  tree v4sf_ftype_v4sf_v4sf_v4sf = build_function_type_list (V4SF_type_node,
							     V4SF_type_node,
							     V4SF_type_node,
							     V4SF_type_node,
							     NULL_TREE);
  tree v4si_ftype_v4si_v4si_v4si = build_function_type_list (V4SI_type_node,
							     V4SI_type_node,
							     V4SI_type_node,
							     V4SI_type_node,
							     NULL_TREE);
  tree v8hi_ftype_v8hi_v8hi = build_function_type_list (V8HI_type_node,
							...
							V8HI_type_node,
							NULL_TREE);
  tree v8hi_ftype_v8hi_v8hi_v8hi = build_function_type_list (V8HI_type_node,
							     V8HI_type_node,
							     V8HI_type_node,
							     V8HI_type_node,
							     NULL_TREE);
  tree v4si_ftype_v8hi_v8hi_v4si = build_function_type_list (V4SI_type_node,
							     V8HI_type_node,
							     V8HI_type_node,
							     V4SI_type_node,
							     NULL_TREE);
  tree v4si_ftype_v16qi_v16qi_v4si = build_function_type_list (V4SI_type_node,
							       V16QI_type_node,
							       V16QI_type_node,
							       V4SI_type_node,
							       NULL_TREE);
  ...
  tree v4si_ftype_v4sf_v4sf = build_function_type_list (V4SI_type_node,
							...
							V4SF_type_node,
							NULL_TREE);
  tree v8hi_ftype_v16qi_v16qi = build_function_type_list (V8HI_type_node,
							  V16QI_type_node,
							  V16QI_type_node,
							  NULL_TREE);
  tree v4si_ftype_v8hi_v8hi = build_function_type_list (V4SI_type_node,
							...
							V8HI_type_node,
							NULL_TREE);
  tree v8hi_ftype_v4si_v4si = build_function_type_list (V8HI_type_node,
							V4SI_type_node,
							V4SI_type_node,
							NULL_TREE);
  tree v16qi_ftype_v8hi_v8hi = build_function_type_list (V16QI_type_node,
							 V8HI_type_node,
							 V8HI_type_node,
							 NULL_TREE);
  tree v4si_ftype_v16qi_v4si = build_function_type_list (V4SI_type_node,
							 V16QI_type_node,
							 V4SI_type_node,
							 NULL_TREE);
  tree v4si_ftype_v16qi_v16qi = build_function_type_list (V4SI_type_node,
							  V16QI_type_node,
							  V16QI_type_node,
							  NULL_TREE);
  tree v4si_ftype_v8hi_v4si = build_function_type_list (V4SI_type_node,
							V8HI_type_node,
							V4SI_type_node,
							NULL_TREE);
  tree v4si_ftype_v8hi =
    build_function_type_list (V4SI_type_node, V8HI_type_node, NULL_TREE);
  tree int_ftype_v4si_v4si = build_function_type_list (integer_type_node,
						       V4SI_type_node,
						       V4SI_type_node,
						       NULL_TREE);
  tree int_ftype_v4sf_v4sf = build_function_type_list (integer_type_node,
						       ...
						       V4SF_type_node,
						       NULL_TREE);
  tree int_ftype_v16qi_v16qi = build_function_type_list (integer_type_node,
							 V16QI_type_node,
							 V16QI_type_node,
							 NULL_TREE);
  tree int_ftype_v8hi_v8hi = build_function_type_list (integer_type_node,
						       V8HI_type_node,
						       V8HI_type_node,
						       NULL_TREE);
...
expand_block_move (rtx operands[])
{
  ...
  for (offset = 0; bytes > 0; offset += move_bytes, bytes -= move_bytes)
	...
	rtx (*mov) (rtx, rtx);
      ...
      if (TARGET_STRING && bytes > 24	/* move up to 32 bytes at a time */
	  && !fixed_regs[5]
	  && !fixed_regs[6]
	  && !fixed_regs[7]
	  && !fixed_regs[8]
	  && !fixed_regs[9]
	  && !fixed_regs[10] && !fixed_regs[11] && !fixed_regs[12])
	{
	  move_bytes = (bytes > 32) ? 32 : bytes;
	  gen_func.movstrsi = gen_movstrsi_8reg;
	}
      else if (TARGET_STRING && bytes > 16	/* move up to 24 bytes at a time */
	       && !fixed_regs[5]
	       && !fixed_regs[6]
	       && !fixed_regs[7]
	       && !fixed_regs[8] && !fixed_regs[9] && !fixed_regs[10])
	{
	  move_bytes = (bytes > 24) ? 24 : bytes;
	  gen_func.movstrsi = gen_movstrsi_6reg;
	}
      else if (TARGET_STRING && bytes > 8	/* move up to 16 bytes at a time */
	       && !fixed_regs[5]
	       && !fixed_regs[6] && !fixed_regs[7] && !fixed_regs[8])
	{
  ...
  for (i = 1; i < count; i++)
    {
      ...
      if (GET_CODE (elt) != CLOBBER && GET_CODE (elt) != SET)
	return 0;
    }
  ...
  for (i = 0; i < count; i++)
    {
	  ...
	  || GET_MODE (src_reg) != CCmode || !CR_REGNO_P (REGNO (src_reg)))
	return 0;
...
mtcrf_operation (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  if (GET_CODE (src_reg) != REG
      || GET_MODE (src_reg) != SImode || !INT_REGNO_P (REGNO (src_reg)))
    return 0;
...
lmw_operation (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  if (dest_regno > 31 || count != 32 - (int) dest_regno)
    return 0;
  ...
  for (i = 0; i < count; i++)
	...
	return 0;
      if (REGNO (addr_reg) != base_regno || newoffset != offset + 4 * i)
	return 0;
    }
...
stmw_operation (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  if (src_regno > 31 || count != 32 - (int) src_regno)
    return 0;
...
{
  if (GET_RTX_CLASS (code) != '<' || GET_MODE_CLASS (mode) != MODE_CC)
    abort ();
  ...
  if (mode != CCFPmode
	  ...
	  || code == UNEQ || code == LTGT
	  || code == UNGT || code == UNLT || code == UNGE || code == UNLE))
    abort ();
  ...
  if (mode == CCFPmode
      && !flag_finite_math_only
      && (code == LE || code == GE
	  || code == UNEQ || code == LTGT || code == UNGT || code == UNLT))
    abort ();

  /* These are invalid; the information is not there.  */
  if (mode == CCEQmode && code != EQ && code != NE)
    abort ();
}
...
branch_positive_comparison_operator (rtx op, enum machine_mode mode)
{
  ...
  return (code == EQ || code == LT || code == GT
	  || code == LTU || code == GTU || code == UNORDERED);
}
...
secondary_reload_class (enum reg_class class, enum machine_mode mode, rtx in)
{
  ...
  if (TARGET_ELF || (DEFAULT_ABI == ABI_DARWIN
      ...
      ))
    {
	  ...
	  && (GET_CODE (in) == SYMBOL_REF
	      || GET_CODE (in) == HIGH
	      || GET_CODE (in) == LABEL_REF || GET_CODE (in) == CONST))
	return BASE_REGS;
    }
  ...
  if ((regno == -1 || ALTIVEC_REGNO_P (regno)) && class == ALTIVEC_REGS)
    return NO_REGS;
...
ccr_bit (rtx op, int scc_p)
{
  ...
  if (GET_CODE (reg) != REG || !CR_REGNO_P (REGNO (reg)))
    abort ();
...
print_operand (FILE * file, rtx x, int code)
{
  ...
  switch (code)
    ...
    case 'p':
      ...
      if (!INT_P (x)
	  || INT_LOWPART (x) < 0 || (i = exact_log2 (INT_LOWPART (x))) < 0)
	  ...
	  && GET_CODE (x) != CONST_DOUBLE && CONSTANT_P (x))
	{
      ...
      /* Remove initial .'s to turn a -mcall-aixdesc function
         ...
         itself.  */
	       ...
	       && XSTR (x, 0)[0] == '.' && DEFAULT_ABI == ABI_AIX)
	{
...
rs6000_generate_compare (enum rtx_code code)
{
  ...
  if (rs6000_compare_fp_p)
    comp_mode = CCFPmode;
  else if (code == GTU || code == LTU || code == GEU || code == LEU)
    comp_mode = CCUNSmode;
  else
    comp_mode = CCmode;
...
rs6000_emit_sCOND (enum rtx_code code, rtx result)
{
  ...
  if (TARGET_E500 && rs6000_compare_fp_p && !TARGET_FPRS && TARGET_HARD_FLOAT)
    {
      rtx t;
  ...
  if (note != NULL_RTX)
    {
      ...
      if (rs6000_always_hint || abs (prob) > REG_BR_PROB_BASE / 100 * 48)
	{
  ...
  switch (code)
    ...
    case LE:
      ...
      emit_insn (gen_rtx_SET
		 (VOIDmode, temp, gen_rtx_NEG (compare_mode, op0)));
      op0 = temp;
      break;
    ...
    case ORDERED:
      temp = gen_reg_rtx (compare_mode);
      emit_insn (gen_rtx_SET
		 (VOIDmode, temp, gen_rtx_ABS (compare_mode, op0)));
      op0 = temp;
      break;
...
rs6000_stack_info (void)
{
  ...
  if (rs6000_ra_ever_killed ()
      ...
      || (DEFAULT_ABI == ABI_DARWIN
	  && flag_pic
	  && current_function_uses_pic_offset_table) || info_ptr->calls_p)
    {
  ...
  if (regs_ever_live[CR2_REGNO]
      || regs_ever_live[CR3_REGNO] || regs_ever_live[CR4_REGNO])
    {
  ...
  switch (DEFAULT_ABI)
      ...
      if (TARGET_ALTIVEC_ABI)
	{
	  ...
	  info_ptr->altivec_save_offset
	    = info_ptr->vrsave_save_offset
	    - info_ptr->altivec_padding_size - info_ptr->altivec_size;
      ...
      if (TARGET_SPE_ABI && info_ptr->spe_64bit_regs_used != 0)
	{
	  ...
	  if (info_ptr->spe_gp_size != 0)
	    info_ptr->spe_padding_size = 8 - (-info_ptr->cr_save_offset % 8);
	  else
	    info_ptr->spe_padding_size = 0;

	  info_ptr->spe_gp_save_offset
	    = info_ptr->cr_save_offset
	    - info_ptr->spe_padding_size - info_ptr->spe_gp_size;
	...
	info_ptr->toc_save_offset =
	  info_ptr->cr_save_offset - info_ptr->toc_size;
  ...
  info_ptr->save_size = RS6000_ALIGN (info_ptr->fp_size
				      ...
				      + info_ptr->toc_size, save_align);

  non_fixed_size = (info_ptr->vars_size
		    + info_ptr->parm_size
		    + info_ptr->save_size + info_ptr->varargs_size);
  ...
  if (!TARGET_SPE_ABI
      || info_ptr->spe_64bit_regs_used == 0 || info_ptr->spe_gp_size == 0)
...
spe_func_has_64bit_regs_p (void)
{
  ...
  for (insn = NEXT_INSN (insns); insn != NULL_RTX; insn = NEXT_INSN (insn))
      ...
      if (INSN_P (insn))
	{
	  ...
	  i = PATTERN (insn);
	  if (GET_CODE (i) == SET && SPE_VECTOR_MODE (GET_MODE (SET_SRC (i))))
	    return true;
	}
    }
...
rs6000_function_ok_for_sibcall (tree decl, tree exp ATTRIBUTE_UNUSED)
{
  ...
  if (decl)
      ...
      if (TARGET_ALTIVEC_VRSAVE)
	{
      ...
      if (DEFAULT_ABI == ABI_DARWIN || (*targetm.binds_local_p) (decl))
	{
  ...
  if (TARGET_ELF && DEFAULT_ABI == ABI_V4 && flag_pic == 1)
  ...
  else if (TARGET_ELF && DEFAULT_ABI != ABI_AIX && flag_pic == 2)
    {
      ...
      rtx tempLR = (fromprolog
		    ? gen_rtx_REG (Pmode, LINK_REGISTER_REGNUM)
		    : gen_reg_rtx (Pmode));
      rtx temp0 = (fromprolog ? gen_rtx_REG (Pmode, 0) : gen_reg_rtx (Pmode));
  ...
  if (info->lr_save_p)
    {
	  ...
	  || current_function_calls_alloca || info->total_size > 32767)
	{
  ...
  for (insn = get_insns (); insn; insn = NEXT_INSN (insn))
	...
	if (GET_CODE (pat) == PARALLEL)
	  for (i = 0; i < XVECLEN (pat, 0); i++)
		...
		{
		  sub = XEXP (sub, 0);
		  if (GET_CODE (sub) == UNSPEC && XINT (sub, 1) == UNSPEC_TOC)
		    return 1;
		}
	    }
      }
...
rs6000_aix_emit_builtin_unwind_init (void)
{
  ...
  do_compare_rtx_and_jump (opcode, tocompare, EQ, 1,
			   SImode, NULL_RTX, NULL_RTX, no_toc_save_needed);
  ...
  if (current_function_limit_stack)
    {
	  ...
	  && REGNO (stack_limit_rtx) > 1 && REGNO (stack_limit_rtx) <= 31)
	{
	  emit_insn (TARGET_32BIT
		     ? gen_addsi3 (tmp_reg,
				   stack_limit_rtx,
	  ...
	  emit_insn (gen_cond_trap (LTU, stack_reg, tmp_reg, const0_rtx));
	       ...
	       && TARGET_32BIT && DEFAULT_ABI == ABI_V4)
	{
...
spe_synthesize_frame_save (rtx real)
{
  ...
  if (GET_CODE (real) != SET || GET_MODE (SET_SRC (real)) != V2SImode)
    return real;
  ...
  synth = replace_rtx (synth, reg, gen_rtx_REG (SImode, REGNO (reg) + 1200));
		 ...
		 unsigned int regno, int offset, HOST_WIDE_INT total_size)
{
  ...
  if ((TARGET_ALTIVEC_ABI && ALTIVEC_VECTOR_MODE (mode))
      || (TARGET_SPE_ABI
	  && SPE_VECTOR_MODE (mode) && !SPE_CONST_OFFSET_OK (offset)))
    {
...
rs6000_emit_prologue (void)
{
  ...
  if (current_function_calls_eh_return)
      ...
      for (i = 0;; ++i)
	{
	  ...
	  emit_frame_save (frame_reg_rtx, frame_ptr_rtx, reg_mode, regno,
			   ...
			   + reg_size * (int) i, info->total_size);
	}
    }
  ...
  if (DEFAULT_ABI == ABI_DARWIN
      && flag_pic && current_function_uses_pic_offset_table)
    {
      ...
      rs6000_maybe_dead (emit_move_insn
			 (gen_rtx_REG (Pmode, RS6000_PIC_OFFSET_TABLE_REGNUM),
			  gen_rtx_REG (Pmode, LINK_REGISTER_REGNUM)));
    }
#endif
}
...
rs6000_emit_epilogue (int sibcall)
{
  ...
  using_mfcr_multiple = (rs6000_cpu == PROCESSOR_PPC601
			 ...
			 || rs6000_cpu == PROCESSOR_PPC750 || optimize_size);
  ...
  if (use_backchain_to_restore_sp)
	...
	frame_reg_rtx = gen_rtx_REG (Pmode, 11);
      ...
      emit_move_insn (frame_reg_rtx, gen_rtx_MEM (Pmode, sp_reg_rtx));
  ...
  if (using_load_multiple)
    ...
    for (i = 0; i < 32 - info->first_gp_reg_save; i++)
      if ((regs_ever_live[info->first_gp_reg_save + i]
	   && (!call_used_regs[info->first_gp_reg_save + i]
	       || (i + info->first_gp_reg_save ==
		   RS6000_PIC_OFFSET_TABLE_REGNUM && TARGET_TOC
		   && TARGET_MINIMAL_TOC)))
	  || (i + info->first_gp_reg_save == RS6000_PIC_OFFSET_TABLE_REGNUM
	      && ((DEFAULT_ABI == ABI_V4 && flag_pic != 0)
		  || (DEFAULT_ABI == ABI_DARWIN && flag_pic))))
  ...
  if (info->cr_save_p)
	...
	{
	  for (i = 0; i < 8; i++)
	    if (regs_ever_live[CR0_REGNO + i]
		&& !call_used_regs[CR0_REGNO + i])
	      count++;
	  if (count == 0)
	    abort ();
	}
				 ...
				 HOST_WIDE_INT size ATTRIBUTE_UNUSED)
{
  ...
  if (DEFAULT_ABI == ABI_AIX && !flag_inhibit_size_directive
	...
	abort ();
      ...
      fprintf (file, "%d,",
	       (optional_tbtab << 5) | ((info->first_fp_reg_save != 64) <<
					1));
			...
			tree function)
{
  ...
  insn = emit_call_insn (gen_rtx_PARALLEL (VOIDmode,
					   gen_rtvec (4,
  ...
  if (rs6000_sched_groups)
    {
	  ...
	  || type == TYPE_LOAD_UX
	  || type == TYPE_STORE_UX || type == TYPE_MFCR)
	return true;
    }
...
is_dispatch_slot_restricted (rtx insn)
{
  ...
  switch (type)
    ...
    case TYPE_LDIV:
      return 2;
    default:
      if (rs6000_cpu == PROCESSOR_POWER5 && is_cracked_insn (insn))
	return 2;
      return 0;
    }
}
			     ...
			     int distance)
{
  ...
  if (rs6000_sched_costly_dep == store_to_load_dep_costly
      && is_load_insn (next) && is_store_insn (insn))
    /* Prevent load after store in the same group.  */
    return true;

  if (rs6000_sched_costly_dep == true_store_to_load_dep_costly
      && is_load_insn (next)
      && is_store_insn (insn) && (!link || (int) REG_NOTE_KIND (link) == 0))
	  ...
	  if (next == next_insn)
	    {
	      ...
	      if (rs6000_is_costly_dependence
		  (insn, next_insn, link, cost, 0))
		return true;
	    }
	}
    }
				 ...
				 bool * no_add_attrs)
{
  ...
  switch (altivec_type)
	...
	case SImode:
	case V4SImode:
	  result = bool_V4SI_type_node;
	  break;
	case HImode:
	case V8HImode:
	  result = bool_V8HI_type_node;
	  break;
	case QImode:
	case V16QImode:
	  result = bool_V16QI_type_node;
	default:
	  break;
	}
      break;
...
{
  if (type == bool_char_type_node)
    return "U6__boolc";
  if (type == bool_short_type_node)
    return "U6__bools";
  if (type == pixel_type_node)
    return "u7__pixel";
  if (type == bool_int_type_node)
    return "U6__booli";
...
static void
rs6000_elf_select_section (tree decl, int reloc, unsigned HOST_WIDE_INT align)
{
...
rs6000_elf_unique_section (tree decl, int reloc)
{
  ...
  default_unique_section_1 (decl, reloc, flag_pic || DEFAULT_ABI == ABI_AIX);
}

...
rs6000_elf_encode_section_info (tree decl, rtx rtl, int first)
{
  ...
  if (first
      ...
      && !TARGET_AIX && DEFAULT_ABI == ABI_AIX)
    {
...
find_addr_reg (rtx addr)
{
  while (GET_CODE (addr) == PLUS)
    {
      if (GET_CODE (XEXP (addr, 0)) == REG && REGNO (XEXP (addr, 0)) != 0)
	addr = XEXP (addr, 0);
...
macho_branch_islands (void)
{
  ...
  for (branch_island = branch_island_list;
       branch_island; branch_island = TREE_CHAIN (branch_island))
    {
      const char *label =
	IDENTIFIER_POINTER (BRANCH_ISLAND_LABEL_NAME (branch_island));
      const char *name =
	darwin_strip_name_encoding (IDENTIFIER_POINTER
...
rs6000_elf_declare_function_name (FILE * file, const char *name, tree decl)
{
  ...
  if (TARGET_RELOCATABLE
      && (get_pool_size () != 0 || current_function_profile) && uses_TOC ())
    {
      char buf[256];
		  ...
		  int *total)
{
  switch (code)
	...
	case PROCESSOR_PPC7450:
	  *total = (GET_CODE (XEXP (x, 1)) != CONST_INT
		    ? COSTS_N_INSNS (4) : COSTS_N_INSNS (3));
	  return true;
	...
	case PROCESSOR_RS64A:
	  *total = (GET_MODE (XEXP (x, 1)) != DImode
		    ? COSTS_N_INSNS (65) : COSTS_N_INSNS (67));
	  return true;
	...
	case PROCESSOR_PPC630:
	  *total = (GET_MODE (XEXP (x, 1)) != DImode
		    ? COSTS_N_INSNS (21) : COSTS_N_INSNS (37));
	  return true;
	...
	case PROCESSOR_POWER5:
	  *total = (GET_MODE (XEXP (x, 1)) != DImode
		    ? COSTS_N_INSNS (18) : COSTS_N_INSNS (34));
	  return true;
...
rs6000_complex_function_value (enum machine_mode mode)
{
  ...
  r1 = gen_rtx_EXPR_LIST (inner, gen_rtx_REG (inner, regno), const0_rtx);
...
rs6000_libcall_value (enum machine_mode mode)
{
  ...
  if (GET_MODE_CLASS (mode) == MODE_FLOAT && TARGET_HARD_FLOAT && TARGET_FPRS)
    regno = FP_ARG_RETURN;
  else if (ALTIVEC_VECTOR_MODE (mode) && TARGET_ALTIVEC && TARGET_ALTIVEC_ABI)
    regno = ALTIVEC_ARG_RETURN;
