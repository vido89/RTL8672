...
static unsigned int
xtensa_multibss_section_type_flags (tree, const char *, int)
  ATTRIBUTE_UNUSED;
  ...
  if (GET_CODE (op) == CONST_INT)
    {
      return (BITS_BIG_ENDIAN
	      ? (INTVAL (op) == BITS_PER_WORD - 1) : (INTVAL (op) == 0));
    }
  return FALSE;
}
...
call_insn_operand (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  if (CONSTANT_ADDRESS_P (op))
      ...
      if (flag_pic)
	{
	  ...
	  if (flag_function_sections || DECL_ONE_ONLY (current_function_decl))
	    return FALSE;
...
constantpool_address_p (rtx addr)
{
  ...
  if (GET_CODE (addr) == CONST)
	...
	return FALSE;
      ...
      offset = XEXP (addr, 1);
      if ((GET_CODE (offset) != CONST_INT) || ((INTVAL (offset) & 3) != 0))
	return FALSE;
		      ...
		      rtx cmp1 /* second operand to compare */ )
{
  ...
  switch (test_code)
    ...
    default:
      ...
      reverse_regs = 0;
      invert = 0;
      gen_fn = 0;		/* avoid compiler warnings */
    }
...
xtensa_expand_conditional_move (rtx * operands, int isflt)
{
  ...
  if (isflt)
    gen_fn = (branch_type == CMP_SI
	      ? gen_movsfcc_internal0 : gen_movsfcc_internal1);
  else
    gen_fn = (branch_type == CMP_SI
	      ? gen_movsicc_internal0 : gen_movsicc_internal1);
...
fixup_subreg_mem (rtx x)
{
  if (GET_CODE (x) == SUBREG
    ...
    {
      rtx temp = gen_rtx_SUBREG (GET_MODE (x),
...
xtensa_find_mode_for_size (unsigned item_size)
{
  ...
  while (1)
	...
	abort ();
      ...
      if (xtensa_ld_opcodes[(int) mode] && xtensa_st_opcodes[(int) mode])
	break;
...
xtensa_expand_nonlocal_goto (rtx * operands)
{
  ...
  goto_handler = replace_rtx (copy_rtx (goto_handler),
			      virtual_stack_vars_rtx, containing_fp);

  emit_library_call (gen_rtx_SYMBOL_REF (Pmode, "__xtensa_nonlocal_goto"),
		     0, VOIDmode, 2,
		     containing_fp, Pmode, goto_handler, Pmode);
}
...
void
xtensa_output_literal (FILE * file, rtx x, enum machine_mode mode,
		       int labelno)
{
  long value_long[2];
  REAL_VALUE_TYPE r;
...
xtensa_build_builtin_va_list (void)
{
  ...
  f_stk = build_decl (FIELD_DECL, get_identifier ("__va_stk"), ptr_type_node);
  f_reg = build_decl (FIELD_DECL, get_identifier ("__va_reg"), ptr_type_node);
...
xtensa_va_arg (tree valist, tree type)
{
  ...
  addr_tree = build (PLUS_EXPR, ptr_type_node,
		     make_tree (ptr_type_node, array), ndx);
...
xtensa_rtx_costs (rtx x, int code, int outer_code, int *total)
{
  switch (code)
	...
	case SET:
	  ...
	  break;
	case PLUS:
	  if (xtensa_simm8 (INTVAL (x)) || xtensa_simm8x256 (INTVAL (x)))
	    {
	      *total = 0;
	      return true;
	    }
