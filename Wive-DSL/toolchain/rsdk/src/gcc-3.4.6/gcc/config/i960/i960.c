...
fp_literal_one (op, mode)
     ...
     enum machine_mode mode;
{
  return (TARGET_NUMERICS && mode == GET_MODE (op)
	  && op == CONST1_RTX (mode));
}
...
signed_literal (op, mode)
     ...
     enum machine_mode mode ATTRIBUTE_UNUSED;
{
  return ((GET_CODE (op) == CONST_INT) && INTVAL (op) > -32
	  && INTVAL (op) < 32);
}
     ...
     enum machine_mode mode;
{
  ...
  if (GET_CODE (operands[0]) == MEM && GET_CODE (operands[1]) != REG
	  ...
	  || current_function_stdarg || rtx_equal_function_value_matters))
     ...
     rtx dst, src;
{
  ...
  if (GET_CODE (dst) == REG && GET_CODE (src) == REG)
      ...
      if ((REGNO (src) & 3) || (REGNO (dst) & 3))
	{
	  ...
	  if (REGNO (dst) <= REGNO (src) + 3 && REGNO (dst) >= REGNO (src))
	    return
	      "mov	%F1,%F0\n\tmov	%E1,%E0\n\tmov	%D1,%D0\n\tmov	%1,%0";
	  else
	    return
     ...
     tree fndecl;
{
  ...
  if (leaf_proc_ok)
    for (i = 0, j = 0; i < FIRST_PSEUDO_REGISTER; i++)
      if (regs_ever_live[i] && ((!call_used_regs[i]) || (i > 7 && i < 12)))
	{
	  /* Global registers.  */
	  if (i < 16 && i > 7 && i != 13)
	    leaf_proc_ok = 0;
     ...
     HOST_WIDE_INT size;
{
  ...
  lvar_size =
    actual_fsize - compute_frame_size (0) - n_remaining_saved_regs * 4;
  ...
  for (i = 0, j = n_remaining_saved_regs; j > 0 && i < 16; i++)
	...
	nr = 4;
      ...
      fprintf (file, "\tst%s	%s," HOST_WIDE_INT_PRINT_DEC "(fp)\n",
	       ((nr == 4) ? "q" :
		(nr == 3) ? "t" :
		(nr == 2) ? "l" : ""), reg_names[i], offset);
      sprintf (tmpstr, "\tld%s	" HOST_WIDE_INT_PRINT_DEC "(fp),%s\n",
	       ((nr == 4) ? "q" :
		(nr == 3) ? "t" :
		(nr == 2) ? "l" : ""), offset, reg_names[i]);
      strcat (epilogue_string, tmpstr);
      i += nr - 1;
      j -= nr;
      offset += nr * 4;
     ...
     int labelno;
{
  ...
  for (last_parm_reg = 7;
       last_parm_reg >= 0 && !regs_ever_live[last_parm_reg]; last_parm_reg--)
    ;
     ...
     int size;
{
  ...
  i = MIN (i960_expr_alignment (base, 4), i960_expr_alignment (offset, 4));
