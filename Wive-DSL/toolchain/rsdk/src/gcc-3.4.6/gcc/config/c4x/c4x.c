...
c4x_override_options (void)
{
  ...
  switch (c4x_cpu_version)
    {
    case 30:
      target_flags |= C30_FLAG;
      break;
    case 31:
      target_flags |= C31_FLAG;
      break;
    case 32:
      target_flags |= C32_FLAG;
      break;
    case 33:
      target_flags |= C33_FLAG;
      break;
    case 40:
      target_flags |= C40_FLAG;
      break;
    case 44:
      target_flags |= C44_FLAG;
      break;
    default:
...
c4x_hard_regno_mode_ok (unsigned int regno, enum machine_mode mode)
{
  switch (mode)
    ...
    case HImode:		/* Long Long (64 bits).  */
      ...
      return IS_INT_REGNO (regno)
	&& IS_INT_REGNO (regno + 1) && (regno & 1) == 0;
...
c4x_init_cumulative_args (CUMULATIVE_ARGS * cum, tree fntype, rtx libname)
{
  ...
  if (TARGET_DEBUG)
    fprintf (stderr, "%s%s, args = %d)\n",
	     ...
	     cum->var ? ", variable args" : "", cum->args);
}
			  ...
			  tree type, int named)
{
  ...
  if (!TARGET_MEMPARM && named && type && !MUST_PASS_IN_STACK (mode, type))
    {
      /* Look for float, double, or long double argument.  */
      if (mode == QFmode || mode == HFmode)
	cum->floats++;
...
c4x_isr_reg_used_p (unsigned int regno)
{
  ...
  if (regno == FRAME_POINTER_REGNUM || IS_ST_REGNO (regno))
    return 0;
...
c4x_interrupt_function_p (void)
{
  ...
  return cfun_name[0] == 'c'
    ...
    && cfun_name[3] == 'n'
    && cfun_name[4] == 't'
    && ISDIGIT (cfun_name[5]) && ISDIGIT (cfun_name[6]);
}
...
c4x_expand_prologue (void)
{
  ...
  if (c4x_interrupt_function_p ())
      ...
      for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
	{
	  ...
	  || regs_ever_live[RS_REGNO] || regs_ever_live[RE_REGNO])
	{
	...
	{
	  if ((size != 0)
	      || (current_function_args_size != 0) || (optimize < 2))
	    {
	      insn = emit_insn (gen_pushqi (gen_rtx_REG (QImode, AR3_REGNO)));
	      RTX_FRAME_RELATED_P (insn) = 1;
	      insn = emit_insn (gen_movqi (gen_rtx_REG (QImode, AR3_REGNO),
  ...
  if (c4x_interrupt_function_p ())
      ...
      for (regno = FIRST_PSEUDO_REGISTER - 1; regno >= 0; --regno)
	{
	  ...
	  if (regno == DP_REGNO)
	      ...
	      if (IS_EXT_REGNO (regno))
		{
	      ...
	      insn =
		emit_insn (gen_popqi_unspec (gen_rtx_REG (QImode, regno)));
	      RTX_FRAME_RELATED_P (insn) = 1;
	    }
	}
      if (size)
	{
	  insn = emit_insn (gen_subqi3 (gen_rtx_REG (QImode, SP_REGNO),
					...
					GEN_INT (size)));
	  RTX_FRAME_RELATED_P (insn) = 1;
	  insn = emit_insn (gen_popqi (gen_rtx_REG (QImode, AR3_REGNO)));
	  RTX_FRAME_RELATED_P (insn) = 1;
	}
...
c4x_null_epilogue_p (void)
{
  ...
  if (reload_completed
      ...
      && !current_function_args_size && !(optimize < 2) && !get_frame_size ())
    {
      for (regno = FIRST_PSEUDO_REGISTER - 1; regno >= 0; regno--)
	if (regs_ever_live[regno] && !call_used_regs[regno]
	    && (regno != AR3_REGNO))
...
c4x_emit_move_sequence (rtx * operands, enum machine_mode mode)
{
  ...
  if (GET_CODE (op0) == SUBREG && mixed_subreg_operand (op0, mode))
    {
  ...
  if (GET_CODE (op1) == SUBREG && mixed_subreg_operand (op1, mode))
    {
      /* We should only generate these mixed mode patterns
         during RTL generation.  If we need do it later on
         then we'll have to emit patterns that won't clobber CC.  */
  ...
  if (mode == QImode
      ...
      && !IS_INT16_CONST (INTVAL (op1)) && !IS_HIGH_CONST (INTVAL (op1)))
    {
  ...
  if (mode == HImode
      && reg_operand (op0, mode) && const_int_operand (op1, mode))
    {
...
c4x_print_operand (FILE * file, rtx op, int letter)
{
  ...
  switch (code)
    {
    case REG:
      if (GET_MODE_CLASS (GET_MODE (op)) == MODE_FLOAT && !TARGET_TI)
	...
	if (REG_P (op0))
	  {
		...
		if (IS_INDEX_REG (op0))
		  {
		    fprintf (file, "*+%s(%s)", reg_names[REGNO (op1)], reg_names[REGNO (op0)]);	/* Index + base.  */
		  }
		else
		  {
		    fprintf (file, "*+%s(%s)", reg_names[REGNO (op0)], reg_names[REGNO (op1)]);	/* Base + index.  */
		  }
	      }
	    else if (INTVAL (op1) < 0)
	      {
		fprintf (file, "*-%s(" HOST_WIDE_INT_PRINT_DEC ")", reg_names[REGNO (op0)], -INTVAL (op1));	/* Base - displacement.  */
	      }
	    else
	      {
		fprintf (file, "*+%s(" HOST_WIDE_INT_PRINT_DEC ")", reg_names[REGNO (op0)], INTVAL (op1));	/* Base + displacement.  */
  ...
  if (!c4x_rptb_valid_p (insn, start_label))
    {
      ...
      emit_insn_before (gen_addqi3 (count_reg, count_reg, GEN_INT (-1)),
			insn);
  ...
  for (; insn; insn = PREV_INSN (insn))
    {
      if (insn == start_label)
	break;
      if (GET_CODE (insn) == JUMP_INSN && JUMP_LABEL (insn) == start_label)
...
c4x_R_indirect (rtx op)
{
  ...
  switch (GET_CODE (op))
	...
	return REG_P (op0)
	  ...
	  && GET_CODE (op1) == CONST_INT && IS_UINT5_CONST (INTVAL (op1));
      }
      break;
	...
	if (REG_P (op0))
	  {
	    ...
	    return IS_ADDR_OR_PSEUDO_REG (op0)
	      && GET_CODE (op1) == CONST_INT && IS_DISP1_CONST (INTVAL (op1));
	  }
      }
      break;
...
c4x_T_constraint (rtx op)
{
  ...
  if (GET_CODE (op) != LO_SUM)
	...
	&& GET_MODE (op) == Pmode && SYMBOL_REF_FUNCTION_P (op);
    }
  ...
  if ((GET_CODE (XEXP (op, 0)) == REG) && (REGNO (XEXP (op, 0)) == DP_REGNO))
  ...
  if (GET_CODE (op) == MEM)
    {
	  ...
	  || code == PRE_DEC
	  || code == POST_INC
	  || code == POST_DEC || code == PRE_MODIFY || code == POST_MODIFY)
	return 1;
    }
  return 0;
}
...
const_operand (register rtx op, register enum machine_mode mode)
{
  switch (mode)
    ...
    case HFmode:
      if (GET_CODE (op) != CONST_DOUBLE
	  || GET_MODE (op) != mode || GET_MODE_CLASS (mode) != MODE_FLOAT)
	return 0;
...
{
  if (GET_CODE (op) == SUBREG && GET_MODE (op) == QFmode)
    return 0;
...
{
  if (GET_CODE (op) == SUBREG && mixed_subreg_operand (op, mode))
    return 0;
...
c4x_valid_operands (enum rtx_code code, rtx * operands,
		    enum machine_mode mode ATTRIBUTE_UNUSED, int force)
{
  rtx op1;
  rtx op2;
  enum rtx_code code1;
  ...
  if (code == ASHIFTRT || code == LSHIFTRT
    ...
    return code2 == REG && (c4x_S_indirect (op1) || c4x_R_indirect (op1));
...
int
valid_operands (enum rtx_code code, rtx * operands, enum machine_mode mode)
{
...
int
legitimize_operands (enum rtx_code code, rtx * operands,
		     enum machine_mode mode)
{
  ...
  if (code == COMPARE)
    {
	  ...
	  && preserve_subexpressions_p () && rtx_cost (operands[1], code) > 1)
  ...
  if (!reload_in_progress
      ...
      && preserve_subexpressions_p () && rtx_cost (operands[2], code) > 1)
...
c4x_rtx_costs (rtx x, int code, int outer_code, int *total)
{
  ...
  switch (code)
	...
	*total = 0;
	       ...
	       && outer_code == AND && (val == 255 || val == 65535))
	*total = 0;
      else if (!TARGET_C3X
