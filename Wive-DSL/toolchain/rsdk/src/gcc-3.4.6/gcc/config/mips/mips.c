...
static bool mips_symbolic_address_p (enum mips_symbol_type,
				     enum machine_mode);
...
static unsigned int mips_build_shift (struct mips_integer_op *,
				      HOST_WIDE_INT);
...
static void mips_select_rtx_section (enum machine_mode, rtx,
				     unsigned HOST_WIDE_INT);
static void
mips_select_section (tree, int, unsigned HOST_WIDE_INT)
  ATTRIBUTE_UNUSED;
...
/* Strings to hold which cpu and instruction set architecture to use.  */
     ...
     const char *mips_no_data_in_code_string;	/* for -mno-data-in-code */
...
/* Array giving truth value on whether or not a given hard register
   can support a given mode.  */
     char mips_hard_regno_mode_ok[(int)
				  MAX_MACHINE_MODE][FIRST_PSEUDO_REGISTER];
...
/* Hardware names for the registers.  If -mrnames is used, this
   will be overwritten with mips_sw_reg_names.  */
     ...
     char mips_reg_names[][8] = {
       ...
       "$c0r8", "$c0r9", "$c0r10", "$c0r11", "$c0r12", "$c0r13", "$c0r14",
	 "$c0r15",
       "$c0r16", "$c0r17", "$c0r18", "$c0r19", "$c0r20", "$c0r21", "$c0r22",
	 "$c0r23",
       "$c0r24", "$c0r25", "$c0r26", "$c0r27", "$c0r28", "$c0r29", "$c0r30",
	 "$c0r31",
       "$c2r0", "$c2r1", "$c2r2", "$c2r3", "$c2r4", "$c2r5", "$c2r6", "$c2r7",
       "$c2r8", "$c2r9", "$c2r10", "$c2r11", "$c2r12", "$c2r13", "$c2r14",
	 "$c2r15",
       "$c2r16", "$c2r17", "$c2r18", "$c2r19", "$c2r20", "$c2r21", "$c2r22",
	 "$c2r23",
       "$c2r24", "$c2r25", "$c2r26", "$c2r27", "$c2r28", "$c2r29", "$c2r30",
	 "$c2r31",
       "$c3r0", "$c3r1", "$c3r2", "$c3r3", "$c3r4", "$c3r5", "$c3r6", "$c3r7",
       "$c3r8", "$c3r9", "$c3r10", "$c3r11", "$c3r12", "$c3r13", "$c3r14",
	 "$c3r15",
       "$c3r16", "$c3r17", "$c3r18", "$c3r19", "$c3r20", "$c3r21", "$c3r22",
	 "$c3r23",
       "$c3r24", "$c3r25", "$c3r26", "$c3r27", "$c3r28", "$c3r29", "$c3r30",
	 "$c3r31"
     };
...
mips_offset_within_object_p (rtx symbol, HOST_WIDE_INT offset)
{
  ...
  if (CONSTANT_POOL_ADDRESS_P (symbol)
      && offset >= 0 && offset < (int) GET_MODE_SIZE (get_pool_mode (symbol)))
    return true;
...
mips_const_insns (rtx x)
{
  ...
  switch (GET_CODE (x))
	...
	return (INTVAL (x) >= 0 && INTVAL (x) < 256 ? 1
		...
		: SMALL_OPERAND_UNSIGNED (-INTVAL (x)) ? 3 : 0);
...
reg_or_const_float_1_operand (rtx op, enum machine_mode mode)
{
  ...
  switch (GET_CODE (op))
    {
    case CONST_DOUBLE:
      if (mode != GET_MODE (op) || (mode != DFmode && mode != SFmode))
	return 0;
...
const_float_1_operand (rtx op, enum machine_mode mode)
{
  ...
  if (GET_CODE (op) != CONST_DOUBLE
      || mode != GET_MODE (op) || (mode != DFmode && mode != SFmode))
    return 0;
...
stack_operand (rtx op, enum machine_mode mode)
{
  ...
  return ((mode == VOIDmode || mode == GET_MODE (op))
	  ...
	  && addr.type == ADDRESS_REG && addr.reg == stack_pointer_rtx);
}
		    ...
		    unsigned HOST_WIDE_INT value)
{
  if (SMALL_OPERAND (value)
      || SMALL_OPERAND_UNSIGNED (value) || LUI_OPERAND (value))
    {
...
mips_legitimize_const_move (enum machine_mode mode, rtx dest, rtx src)
{
  ...
  if (!TARGET_MIPS16
      && offset != 0 && (!no_new_pseudos || SMALL_OPERAND (offset)))
    {
  ...
  if (!memory_operand (src, VOIDmode))
    src =
      replace_equiv_address (src, mips_split_symbol (dest, XEXP (src, 0)));
...
mips_legitimize_move (enum machine_mode mode, rtx dest, rtx src)
{
  ...
  if (GET_MODE_SIZE (mode) <= UNITS_PER_WORD
      && CONSTANT_P (src) && !move_operand (src, mode))
    {
...
m16_check_op (rtx op, int low, int high, int mask)
{
  return (GET_CODE (op) == CONST_INT
	  && INTVAL (op) >= low
	  && INTVAL (op) <= high && (INTVAL (op) & mask) == 0);
}
...
mips_rtx_costs (rtx x, int code, int outer_code, int *total)
{
  ...
  switch (code)
	  ...
	  || (outer_code) == LSHIFTRT)
	{
      ...
      if ((outer_code) == XOR && INTVAL (x) >= 0 && INTVAL (x) < 0x10000)
	{
	  *total = 0;
	  return true;
	}
	  ...
	  && INTVAL (x) >= -0x8000 && INTVAL (x) < 0x8000)
	{
	  *total = 0;
      ...
      if (((outer_code) == EQ || (outer_code) == NE) && INTVAL (x) == 0)
	{
	  *total = 0;
	  return true;
	}
	...
	*total = COSTS_N_INSNS (4);
    ...
    case MULT:
      if (mode == SFmode)
	{
	  if (TUNE_MIPS3000 || TUNE_MIPS3900 || TUNE_MIPS5000)
	    *total = COSTS_N_INSNS (4);
	  else if (TUNE_MIPS6000 || TUNE_MIPS5400 || TUNE_MIPS5500)
...
mips_gen_conditional_trap (rtx * operands)
{
  ...
  emit_insn (gen_rtx_TRAP_IF (VOIDmode,
			      gen_rtx (cmp_code, GET_MODE (operands[0]), op0,
				       op1), operands[1]));
}

...
mips_load_call_address (rtx dest, rtx addr, int sibcall_p)
{
  ...
  if (TARGET_EXPLICIT_RELOCS
      && !(sibcall_p && TARGET_NEWABI) && global_got_operand (addr, VOIDmode))
    {
  ...
  if (info->fpr_p)
    {
      ...
      even_reg_p = (GET_MODE_SIZE (mode) > UNITS_PER_HWFPVALUE
		    || (mips_abi == ABI_O64 && mode == SFmode) || FP_INC > 1);
    }
	      ...
	      tree type, int named)
{
  ...
  if (type != 0
      ...
      && TYPE_SIZE_UNIT (type)
      && host_integerp (TYPE_SIZE_UNIT (type), 1) && named)
    {
      ...
      if (field != 0)
	{
	  ...
	  for (i = 0; i < info.reg_words; i++)
		...
		if (TREE_CODE (field) == FIELD_DECL
		  ...
		  && TYPE_PRECISION (TREE_TYPE (field)) == BITS_PER_WORD)
		reg =
		  gen_rtx_REG (DFmode, FP_ARG_FIRST + info.reg_offset + i);
	      else
		reg =
		  gen_rtx_REG (DImode, GP_ARG_FIRST + info.reg_offset + i);
			     ...
			     enum machine_mode mode, tree type, int no_rtl)
{
  ...
  fp_saved = (EABI_FLOAT_VARARGS_P
	      ? MAX_ARGS_IN_REGISTERS - local_cum.num_fprs : 0);
  ...
  if (mips_abi == ABI_EABI)
      ...
      else
	{
	  ...
	  r = expand_expr (off, NULL_RTX, TYPE_MODE (TREE_TYPE (off)),
			   EXPAND_NORMAL);
	  emit_cmp_and_jump_insns (r, const0_rtx, EQ, const1_rtx,
				   GET_MODE (r), 1, lab_false);
	  ...
	  t =
	    build (MINUS_EXPR, TREE_TYPE (off), off, build_int_2 (rsize, 0));
	  t = build (MODIFY_EXPR, TREE_TYPE (off), off, t);
	  expand_expr (t, const0_rtx, VOIDmode, EXPAND_NORMAL);
...
mips_expand_unaligned_load (rtx dest, rtx src, unsigned int width, int bitpos)
{
  ...
  if (GET_CODE (dest) == SUBREG
      ...
      && SUBREG_BYTE (dest) == 0 && GET_MODE (SUBREG_REG (dest)) == SImode)
...
bool
mips_expand_unaligned_store (rtx dest, rtx src, unsigned int width,
			     int bitpos)
{
  rtx left, right;
...
override_options (void)
{
  ...
  mips_section_threshold =
    g_switch_set ? g_switch_value : MIPS_DEFAULT_GVALUE;
  ...
  if (mips_isa_string != 0)
	...
	error ("-mips%s conflicts with the other architecture options, "
	       "which specify a MIPS%d processor", mips_isa_string, mips_isa);
  ...
  if (TARGET_GAS && !TARGET_MIPS16 && TARGET_SPLIT_ADDRESSES
      && optimize && !flag_pic && !ABI_HAS_64BIT_SYMBOLS)
  ...
  mips_char_to_class['c'] = (TARGET_ABICALLS ? PIC_FN_ADDR_REG :
			     TARGET_MIPS16 ? M16_NA_REGS : GR_REGS);
...
mips_debugger_offset (rtx addr, HOST_WIDE_INT offset)
{
  ...
  /* sdbout_parms does not want this to crash for unrecognized cases.  */
    ...
    fatal_insn
      ("mips_debugger_offset called with non stack/frame/arg pointer", addr);
#endif
		   ...
		   const char *prefix)
{
  size_t i;
  int cur_pos = 17;
  register const unsigned char *string = (const unsigned char *) string_param;
...
void
mips_finish_declare_object (FILE * stream, tree decl, int top_level,
			    int at_end)
{
  const char *name;
...
mips_global_pointer (void)
{
  ...
  if (!regs_ever_live[GLOBAL_POINTER_REGNUM]
      && !current_function_uses_const_pool && !mips_function_has_gp_insn ())
    return 0;
  ...
  if (TARGET_NEWABI && current_function_is_leaf)
    for (regno = GP_REG_FIRST; regno <= GP_REG_LAST; regno++)
	  ...
	  && !fixed_regs[regno] && regno != PIC_FUNCTION_ADDR_REGNUM)
	return regno;
...
compute_frame_size (HOST_WIDE_INT size)
{
  ...
  for (regno = (FP_REG_LAST - FP_INC + 1);
       regno >= FP_REG_FIRST; regno -= FP_INC)
    {
      if (mips_save_reg_p (regno))
	{
  ...
  if (fmask)
    {
      ...
      offset = (args_size + cprestore_size + var_size
		+ gp_reg_rounded + fp_reg_size - FP_INC * UNITS_PER_FPREG);
  ...
  if (!flag_inhibit_size_directive)
    {
      ...
      fprintf (file,
	       ...
	       (reg_names[(frame_pointer_needed)
			  ...
			  STACK_POINTER_REGNUM]), ((frame_pointer_needed
						    && TARGET_MIPS16) ? tsize
						   -
						   cfun->machine->frame.
						   args_size : tsize),
	       reg_names[GP_REG_FIRST + 31], cfun->machine->frame.var_size,
	       cfun->machine->frame.num_gp, cfun->machine->frame.num_fp,
...
mips_save_reg (rtx reg, rtx mem)
{
  if (GET_MODE (reg) == DFmode && !TARGET_FLOAT64)
	...
	emit_move_insn (mem, reg);
      ...
      mips_set_frame_expr (gen_rtx_PARALLEL
			   (VOIDmode, gen_rtvec (2, x1, x2)));
    }
  else
    {
      if (TARGET_MIPS16
	  && REGNO (reg) != GP_REG_FIRST + 31 && !M16_REG_P (REGNO (reg)))
	{
...
mips_encode_section_info (tree decl, rtx rtl, int first)
{
  ...
  if (TARGET_MIPS16)
    {
      if (first && TREE_CODE (decl) == STRING_CST && !flag_writable_strings
		     ...
		     enum machine_mode mode)
{
  ...
  if (mode == TFmode)
    return mips_return_fpr_pair (mode,
				 DImode, 0, DImode, GET_MODE_SIZE (mode) / 2);
...
mips_preferred_reload_class (rtx x, enum reg_class class)
{
  ...
  if (TARGET_HARD_FLOAT
      && FLOAT_MODE_P (GET_MODE (x)) && reg_class_subset_p (FP_REGS, class))
    return FP_REGS;
  ...
  if (cfun->machine->mips16_gp_pseudo_rtx == NULL_RTX)
    {
      ...
      start_sequence ();
      unspec =
	gen_rtx_UNSPEC (VOIDmode, gen_rtvec (1, const0_rtx), UNSPEC_GP);
  ...
  if (l == NULL)
    {
      ...
      sprintf (secname, ".mips16.call.%s%s", fpret ? "fp." : "", fnname);
      stubname = (char *) alloca (strlen (fnname) + 20);
      sprintf (stubname, "__call_stub_%s%s", fpret ? "fp_" : "", fnname);
      stubid = get_identifier (stubname);
      stubdecl = build_decl (FUNCTION_DECL, stubid,
			     build_function_type (void_type_node, NULL_TREE));
      DECL_SECTION_NAME (stubdecl) = build_string (strlen (secname), secname);
...
mips16_lay_out_constants (void)
{
  ...
  for (insn = first; insn; insn = NEXT_INSN (insn))
      ...
      if (GET_CODE (insn) == INSN && GET_CODE (PATTERN (insn)) == SET)
	{
	  ...
	  if (src != NULL_RTX && CONSTANT_POOL_ADDRESS_P (src))
	  ...
	  else if (src != NULL_RTX && SYMBOL_REF_FLAG (src))
	    {
	      ...
	      if (((insns_len - addr)
		   + max_internal_pool_size
		   + pool_size + mips_string_length) >= 0x8000)
		{
		  val = src;
		  mode = Pmode;
		}
      ...
      if (GET_CODE (insn) == BARRIER)
	{
      ...
      if (constants != NULL && (NEXT_INSN (insn) == NULL || (first_constant_ref >= 0 && (((addr - first_constant_ref) + 2	/* for alignment */
