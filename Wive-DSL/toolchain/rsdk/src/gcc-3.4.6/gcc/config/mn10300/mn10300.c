	...
	case CONST_DOUBLE:
	  {
		...
		break;;
	      case VOIDmode:
	      case DImode:
		print_operand_address (file, GEN_INT (CONST_DOUBLE_LOW (x)));
		break;
	      default:
		break;
	      }
	...
	case CONST_DOUBLE:
	  {
		...
		abort ();
	      case VOIDmode:
	      case DImode:
		print_operand_address (file, GEN_INT (CONST_DOUBLE_HIGH (x)));
		break;
	      default:
		break;
	      }
...
print_operand_address (FILE * file, rtx addr)
{
  switch (GET_CODE (addr))
    ...
    case PLUS:
      {
	rtx base, index;
	if (REG_P (XEXP (addr, 0)) && REG_OK_FOR_BASE_P (XEXP (addr, 0)))
	  base = XEXP (addr, 0), index = XEXP (addr, 1);
	else if (REG_P (XEXP (addr, 1)) && REG_OK_FOR_BASE_P (XEXP (addr, 1)))
...
can_use_return_insn (void)
{
  ...
  return (reload_completed
	  ...
	  && !regs_ever_live[16]
	  && !regs_ever_live[17]
	  && fp_regs_to_save () == 0 && !frame_pointer_needed);
}
		...
		if (xsize)
		  {
		    addr = gen_rtx_PLUS (SImode,
					 stack_pointer_rtx, GEN_INT (xsize));
		  }
		else
...
expand_epilogue (void)
{
  ...
  if (size || regs_ever_live[2] || regs_ever_live[3]
      ...
      || regs_ever_live[16] || regs_ever_live[17] || frame_pointer_needed)
...
store_multiple_operation (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  for (i = 1; i < count; i++)
    {
	  ...
	  || GET_CODE (SET_SRC (elt)) != REG || REGNO (SET_SRC (elt)) >= last)
	return 0;
  ...
  if ((mask & 0x3c000) != 0 && (mask & 0x3c000) != 0x3c000)
    return 0;
  ...
  if (from == ARG_POINTER_REGNUM && to == FRAME_POINTER_REGNUM)
    {
	  ...
	  || regs_ever_live[14] || regs_ever_live[15]
	  || regs_ever_live[16] || regs_ever_live[17]
	  || fp_regs_to_save () || frame_pointer_needed)
	return REG_SAVE_BYTES + 4 * fp_regs_to_save ();
      else
	return 0;
    }
	      ...
	      tree type, int named ATTRIBUTE_UNUSED)
{
  ...
  if (type == NULL_TREE && cum->nbytes + size > nregs * UNITS_PER_WORD)
    return 0;
  ...
  while (optimize && temp)
    {
	  ...
	  || GET_CODE (temp) == JUMP_INSN || GET_CODE (temp) == BARRIER)
	break;
      ...
      if (!set)
	{
	  ...
	  && (!past_call || !call_used_regs[REGNO (SET_DEST (set))]))
	{
	  rtx xoperands[2];
	  xoperands[0] = operand;
...
impossible_plus_operand (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  if (XEXP (op, 0) == stack_pointer_rtx || XEXP (op, 1) == stack_pointer_rtx)
    return 1;
...
mask_ok_for_mem_btst (int len, int bit)
{
  ...
  return (((mask & 0xff) == mask)
	  || ((mask & 0xff00) == mask)
	  || ((mask & 0xff0000) == mask) || ((mask & 0xff000000) == mask));
}
		    ...
		    enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  if (GET_CODE (x) == PLUS && symbolic_operand (XEXP (x, 1), VOIDmode))
    {
...
legitimize_pic_address (rtx orig, rtx reg)
{
  if (GET_CODE (orig) == LABEL_REF
	  ...
	  && (CONSTANT_POOL_ADDRESS_P (orig) || !MN10300_GLOBAL_P (orig))))
    {
      if (reg == 0)
...
legitimate_pic_operand_p (rtx x)
{
  ...
  if (GET_CODE (x) == UNSPEC
      && (XINT (x, 1) == UNSPEC_PIC
	  || XINT (x, 1) == UNSPEC_GOT
	  || XINT (x, 1) == UNSPEC_GOTOFF || XINT (x, 1) == UNSPEC_PLT))
    return 1;
