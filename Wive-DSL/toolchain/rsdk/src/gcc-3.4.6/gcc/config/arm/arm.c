...
static Mnode *move_minipool_fix_backward_ref (Mnode *, Mnode *,
					      HOST_WIDE_INT);
...
static const struct processors all_cores[] = {
  ...
  {"strongarm",
   FL_MODE26 | FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_LDSCHED | FL_STRONG},
  {"strongarm110",
   FL_MODE26 | FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_LDSCHED | FL_STRONG},
  {"strongarm1100",
   FL_MODE26 | FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_LDSCHED | FL_STRONG},
  ...
  {"arm10tdmi",
   FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_THUMB | FL_LDSCHED | FL_ARCH5},
  {"arm1020t",
   FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_THUMB | FL_LDSCHED | FL_ARCH5},
  {"arm926ejs",
   FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_THUMB | FL_ARCH5 | FL_ARCH5E},
  {"arm1026ejs",
   FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_THUMB | FL_ARCH5 | FL_ARCH5E},
  {"xscale",
   FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_THUMB | FL_LDSCHED | FL_STRONG |
   FL_ARCH5 | FL_ARCH5E | FL_XSCALE},
  {"iwmmxt",
   FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_THUMB | FL_LDSCHED | FL_STRONG |
   FL_ARCH5 | FL_ARCH5E | FL_XSCALE | FL_IWMMXT},
...
static const struct processors all_architectures[] = {
  ...
  {"armv5",
   FL_CO_PROC | FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_THUMB | FL_ARCH5},
  {"armv5t",
   FL_CO_PROC | FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_THUMB | FL_ARCH5},
  {"armv5te",
   FL_CO_PROC | FL_MODE32 | FL_FAST_MULT | FL_ARCH4 | FL_THUMB | FL_ARCH5 |
...
arm_override_options (void)
{
  ...
  if (TARGET_ARM
      ...
      && !TARGET_APCS_FRAME && (TARGET_DEFAULT & ARM_FLAG_APCS_FRAME))
...
use_return_insn (int iscond, rtx sibling)
{
  ...
  if (current_function_pretend_args_size || cfun->machine->uses_anonymous_args
...
arm_canonicalize_comparison (enum rtx_code code, rtx * op1)
{
  ...
  switch (code)
    ...
    case GEU:
    case LTU:
      if (i != 0 && (const_ok_for_arm (i - 1) || const_ok_for_arm (-(i - 1))))
	{
	  *op1 = GEN_INT (i - 1);
			  ...
			  tree fndecl ATTRIBUTE_UNUSED)
{
  ...
  pcum->nregs =
    ((fntype && aggregate_value_p (TREE_TYPE (fntype), fntype)) ? 1 : 0);
  pcum->iwmmxt_nregs = 0;
  ...
  if (TARGET_REALLY_IWMMXT && fntype)
    {
      ...
      for (fn_arg = TYPE_ARG_TYPES (fntype);
	   fn_arg; fn_arg = TREE_CHAIN (fn_arg))
	pcum->named_count += 1;
...
static tree
arm_handle_fndecl_attribute (tree * node, tree name,
	...
	emit_insn (gen_pic_load_addr_thumb (address, orig));
      ...
      if ((GET_CODE (orig) == LABEL_REF
	   || (GET_CODE (orig) == SYMBOL_REF &&
	       SYMBOL_REF_LOCAL_P (orig))) && NEED_GOT_RELOC)
...
arm_finalize_pic (int prologue ATTRIBUTE_UNUSED)
{
  ...
  if (GOT_PCREL)
    pic_tmp2 = gen_rtx_CONST (VOIDmode,
			      gen_rtx_PLUS (Pmode, global_offset_table,
					    pc_rtx));
  else
  ...
  /* Reload currently can't handle MINUS, so disable this for now */
  else if (GET_CODE (x) == MINUS)
    {
      ...
      return (arm_address_register_rtx_p (xop0, strict_p)
	   ...
	   && !(flag_pic && symbol_mentioned_p (get_pool_constant (x))))
    return 1;
...
arm_legitimate_index_p (enum machine_mode mode, rtx index, int strict_p)
{
  ...
  if (TARGET_HARD_FLOAT && GET_MODE_CLASS (mode) == MODE_FLOAT)
    return (code == CONST_INT && INTVAL (index) < 1024
	    && INTVAL (index) > -1024 && (INTVAL (index) & 3) == 0);

  if (TARGET_CIRRUS
      && (GET_MODE_CLASS (mode) == MODE_FLOAT || mode == DImode))
    return (code == CONST_INT
	    && INTVAL (index) < 255 && INTVAL (index) > -255);
  ...
  if (TARGET_REALLY_IWMMXT && VALID_IWMMXT_REG_MODE (mode))
    return (code == CONST_INT
	    && INTVAL (index) < 256 && INTVAL (index) > -256);
...
thumb_legitimate_offset_p (enum machine_mode mode, HOST_WIDE_INT val)
{
  switch (GET_MODE_SIZE (mode))
    ...
    default:
      return (val >= 0
	      && (val + GET_MODE_SIZE (mode)) <= 128 && (val & 3) == 0);
    }
}
...
arm_legitimize_address (rtx x, rtx orig_x, enum machine_mode mode)
{
  if (GET_CODE (x) == PLUS)
	...
	xop1 = force_reg (SImode, xop1);
      ...
      if (ARM_BASE_REGISTER_RTX_P (xop0) && GET_CODE (xop1) == CONST_INT)
	{
...
arm_rtx_costs_1 (rtx x, enum rtx_code code, enum rtx_code outer)
{
  ...
  if (TARGET_THUMB)
	...
	case CONST_INT:
	  if (outer == SET)
	    ...
	    return 0;
	  else if (outer == AND && INTVAL (x) < 256 && INTVAL (x) >= -256)
	    return COSTS_N_INSNS (1);
  ...
  switch (code)
	...
	return outer == SET ? 2 : -1;
      else if (outer == AND && const_ok_for_arm (~INTVAL (x)))
	return -1;
...
arm_adjust_cost (rtx insn, rtx link, rtx dep, int cost)
{
  ...
  if (arm_tune_xscale
      ...
      && recog_memoized (insn) >= 0 && recog_memoized (dep) >= 0)
    {
...
{
  return (GET_CODE (op) == CONST_INT && INTVAL (op) >= 0 && INTVAL (op) < 64);
}
...
arm_memory_load_p (rtx insn)
{
  ...
  if (GET_CODE (lhs) != REG || REGNO_REG_CLASS (REGNO (lhs)) != GENERAL_REGS)
    return false;
...
di_operand (rtx op, enum machine_mode mode)
{
  ...
  if (mode != VOIDmode && GET_MODE (op) != VOIDmode
      && GET_MODE (op) != DImode)
    return FALSE;
...
arm_comparison_operator (rtx x, enum machine_mode mode)
{
  return (comparison_operator (x, mode)
	  && GET_CODE (x) != LTGT && GET_CODE (x) != UNEQ);
}
...
cc_register (rtx x, enum machine_mode mode)
{
  ...
  if (GET_MODE (x) == mode && GET_CODE (x) == REG && REGNO (x) == CC_REGNUM)
    return TRUE;
  ...
  for (i = 0; in_words_to_go >= 2; i += 4)
      ...
      if (out_words_to_go)
	{
	  if (out_words_to_go > 4)
	    ...
	    {
	      mem =
		adjust_automodify_address (dstbase, SImode, dst, dstoffset);
...
arm_gen_rotated_half_load (rtx memref)
{
  ...
  if (TARGET_MMU_TRAPS && ((BYTES_BIG_ENDIAN ? 1 : 0) ^ ((offset & 2) == 0)))
    return NULL;
...
arm_select_cc_mode (enum rtx_code op, rtx x, rtx y)
{
  ...
  if (TARGET_THUMB
      && GET_MODE (x) == SImode
      && (op == EQ || op == NE) && (GET_CODE (x) == ZERO_EXTRACT))
    return CC_Nmode;
...
arm_reload_in_hi (rtx * operands)
{
  ...
  if (GET_CODE (base) == MINUS
  ...
  else if (GET_CODE (base) == PLUS)
    {
      ...
      lo = (offset >= 0 ? (offset & 0xfff) : -((-offset) & 0xfff));
...
add_minipool_forward_ref (Mfix * fix)
{
  ...
  for (mp = minipool_vector_head; mp != NULL; mp = mp->next)
	...
	max_mp = mp;
      ...
      if (TARGET_REALLY_IWMMXT
	  && max_mp == NULL && fix->fix_size == 8 && mp->fix_size != 8)
	{
	  max_mp = mp;
  ...
  for (insn = next_nonnote_insn (insn); insn; insn = next_nonnote_insn (insn))
    {
	  ...
	  && (arm_cirrus_insn_p (insn)
	      || GET_CODE (insn) == JUMP_INSN || arm_memory_load_p (insn)))
	cirrus_reorg (insn);
...
print_multi_reg (FILE * stream, const char *instr, int reg, int mask)
{
  ...
  if (!TARGET_APCS_32 && ((mask & (1 << PC_REGNUM)) != 0))
    fprintf (stream, "^");
  ...
  if (code0 == REG)
      ...
      else if (code1 == CONST_VECTOR)
	{
	  ...
	  switch (GET_MODE (operands[1]))
	    ...
	    case V2SImode:
	      otherops[1] =
		GEN_INT (INTVAL (CONST_VECTOR_ELT (operands[1], 1)));
	      operands[1] =
		GEN_INT (INTVAL (CONST_VECTOR_ELT (operands[1], 0)));
	      break;
...
output_mov_immediate (rtx * operands)
{
  ...
  /* Try to use one MVN.  */
	...
	output_multi_immediate (operands, "mvn%?\t%0, %1",
				"bic%?\t%0, %0, %1", 1, ~n);
      else
	output_multi_immediate (operands, "mov%?\t%0, %1",
				"orr%?\t%0, %0, %1", 1, n);
    }
...
static const char *
output_multi_immediate (rtx * operands, const char *instr1,
			const char *instr2, int immed_op, HOST_WIDE_INT n)
{
#if HOST_BITS_PER_WIDE_INT > 32
  n &= 0xffffffff;
#endif
...
arm_compute_save_reg_mask (void)
{
  ...
  if (regs_ever_live[LR_REGNUM]
      || (save_reg_mask
	  && optimize_size && ARM_FUNC_TYPE (func_type) == ARM_FT_NORMAL))
  ...
  if (live_regs_mask)
      ...
      if ((live_regs_mask & (1 << IP_REGNUM)) == (1 << IP_REGNUM))
	{
	  ...
	  if (frame_pointer_needed)
	    ...
	    {
	      if (!IS_INTERRUPT (func_type) && !TARGET_REALLY_IWMMXT)
		abort ();
	    }
	}
...
arm_output_epilogue (rtx sibling)
{
  ...
  if (ARM_FUNC_TYPE (func_type) == ARM_FT_EXCEPTION_HANDLER && !really_return)
...
arm_expand_prologue (void)
{
  ...
  if (frame_pointer_needed)
	...
	{
	  insn =
	    gen_rtx_PLUS (SImode, stack_pointer_rtx, GEN_INT (fp_offset));
  ...
  if ((func_type == ARM_FT_ISR || func_type == ARM_FT_FIQ)
      && (live_regs_mask & (1 << LR_REGNUM)) != 0 && !frame_pointer_needed)
  ...
  if (frame_pointer_needed)
      ...
      if (IS_NESTED (func_type))
	{
	  ...
	  if (regs_ever_live[3] == 0 || saved_pretend_args)
	    insn = gen_rtx_REG (SImode, 3);
	  else			/* if (current_function_pretend_args_size == 0) */
	    {
  ...
  switch (code)
    ...
    case 'M':
      asm_fprintf (stream, "{%r-%r}",
		   REGNO (x), REGNO (x) + ARM_NUM_REGS (GET_MODE (x)) - 1);
      return;
	...
	fprintf (stream, "mv%s%s",
		 mode == DFmode ? "d"
		 : mode == SImode ? "fx"
		 : mode == DImode ? "dx" : "f", reg_names[REGNO (x)] + 2);
    ...
    case 'w':
      if (GET_CODE (x) != CONST_INT || INTVAL (x) < 0 || INTVAL (x) >= 16)
		  ...
		  && GET_CODE (SET_DEST (scanbody)) == PC)
		{
	      ...
	      /* Fail if a conditional return is undesirable (eg on a
	         StrongARM), but still allow this if optimizing for size.  */
		       ...
		       && !use_return_insn (TRUE, NULL) && !optimize_size)
		fail = TRUE;
	      else if (GET_CODE (scanbody) == RETURN && seeking_return)
		{
		  arm_ccfsm_state = 2;
		  succeed = TRUE;
		}
...
arm_hard_regno_mode_ok (unsigned int regno, enum machine_mode mode)
{
  ...
  return GET_MODE_CLASS (mode) == MODE_FLOAT
    && regno >= FIRST_ARM_FP_REGNUM && regno <= LAST_ARM_FP_REGNUM;
}
...
arm_debugger_arg_offset (int value, rtx addr)
{
  ...
  if ((TARGET_THUMB || !frame_pointer_needed) && REGNO (addr) == SP_REGNUM)
    return 0;
...
arm_init_iwmmxt_builtins (void)
{
  ...
  tree int_ftype_int = build_function_type (integer_type_node,
					    tree_cons (NULL_TREE,
						       integer_type_node,
						       endlink));
  tree v8qi_ftype_v8qi_v8qi_int = build_function_type (V8QI_type_node,
						       tree_cons (NULL_TREE,
								  V8QI_type_node,
								  tree_cons
								  (NULL_TREE,
								   ...
								   (NULL_TREE,
								    integer_type_node,
								    endlink))));
  tree v4hi_ftype_v4hi_int = build_function_type (V4HI_type_node,
						  tree_cons (NULL_TREE,
							     V4HI_type_node,
							     tree_cons
							     (NULL_TREE,
							      integer_type_node,
							      endlink)));
  tree v2si_ftype_v2si_int = build_function_type (V2SI_type_node,
						  tree_cons (NULL_TREE,
							     V2SI_type_node,
							     tree_cons
							     (NULL_TREE,
							      integer_type_node,
							      endlink)));
  tree v2si_ftype_di_di = build_function_type (V2SI_type_node,
					       tree_cons (NULL_TREE,
							  ...
							  (NULL_TREE,
							   long_long_integer_type_node,
							   endlink)));
  tree di_ftype_di_int = build_function_type (long_long_integer_type_node,
					      tree_cons (NULL_TREE,
							 long_long_integer_type_node,
							 tree_cons (NULL_TREE,
								    integer_type_node,
								    endlink)));
  tree di_ftype_di_int_int = build_function_type (long_long_integer_type_node,
						  tree_cons (NULL_TREE,
							     long_long_integer_type_node,
							     tree_cons
							     (NULL_TREE,
							      ...
							      (NULL_TREE,
							       integer_type_node,
							       endlink))));
  tree int_ftype_v8qi = build_function_type (integer_type_node,
					     tree_cons (NULL_TREE,
							V8QI_type_node,
							endlink));
  tree int_ftype_v4hi = build_function_type (integer_type_node,
					     tree_cons (NULL_TREE,
							V4HI_type_node,
							endlink));
  tree int_ftype_v2si = build_function_type (integer_type_node,
					     tree_cons (NULL_TREE,
							V2SI_type_node,
							endlink));
  tree int_ftype_v8qi_int = build_function_type (integer_type_node,
						 tree_cons (NULL_TREE,
							    V8QI_type_node,
							    tree_cons
							    (NULL_TREE,
							     integer_type_node,
							     endlink)));
  tree int_ftype_v4hi_int = build_function_type (integer_type_node,
						 tree_cons (NULL_TREE,
							    V4HI_type_node,
							    tree_cons
							    (NULL_TREE,
							     integer_type_node,
							     endlink)));
  tree int_ftype_v2si_int = build_function_type (integer_type_node,
						 tree_cons (NULL_TREE,
							    V2SI_type_node,
							    tree_cons
							    (NULL_TREE,
							     integer_type_node,
							     endlink)));
  tree v8qi_ftype_v8qi_int_int = build_function_type (V8QI_type_node,
						      tree_cons (NULL_TREE,
								 V8QI_type_node,
								 tree_cons
								 (NULL_TREE,
								  ...
								  (NULL_TREE,
								   integer_type_node,
								   endlink))));
  tree v4hi_ftype_v4hi_int_int = build_function_type (V4HI_type_node,
						      tree_cons (NULL_TREE,
								 V4HI_type_node,
								 tree_cons
								 (NULL_TREE,
								  ...
								  (NULL_TREE,
								   integer_type_node,
								   endlink))));
  tree v2si_ftype_v2si_int_int = build_function_type (V2SI_type_node,
						      tree_cons (NULL_TREE,
								 V2SI_type_node,
								 tree_cons
								 (NULL_TREE,
								  ...
								  (NULL_TREE,
								   ...
								   endlink))));
  /* Miscellaneous.  */
  tree v8qi_ftype_v4hi_v4hi = build_function_type (V8QI_type_node,
						   tree_cons (NULL_TREE,
							      V4HI_type_node,
							      tree_cons
							      (NULL_TREE,
							       V4HI_type_node,
							       endlink)));
  tree v4hi_ftype_v2si_v2si = build_function_type (V4HI_type_node,
						   tree_cons (NULL_TREE,
							      V2SI_type_node,
							      tree_cons
							      (NULL_TREE,
							       V2SI_type_node,
							       endlink)));
  tree v2si_ftype_v4hi_v4hi = build_function_type (V2SI_type_node,
						   tree_cons (NULL_TREE,
							      V4HI_type_node,
							      tree_cons
							      (NULL_TREE,
							       V4HI_type_node,
							       endlink)));
  tree v2si_ftype_v8qi_v8qi = build_function_type (V2SI_type_node,
						   tree_cons (NULL_TREE,
							      V8QI_type_node,
							      tree_cons
							      (NULL_TREE,
							       V8QI_type_node,
							       endlink)));
  tree v4hi_ftype_v4hi_di = build_function_type (V4HI_type_node,
						 tree_cons (NULL_TREE,
							    V4HI_type_node,
							    tree_cons
							    (NULL_TREE,
							     long_long_integer_type_node,
							     endlink)));
  tree v2si_ftype_v2si_di = build_function_type (V2SI_type_node,
						 tree_cons (NULL_TREE,
							    V2SI_type_node,
							    tree_cons
							    (NULL_TREE,
							     long_long_integer_type_node,
							     endlink)));
  tree void_ftype_int_int = build_function_type (void_type_node,
						 tree_cons (NULL_TREE,
							    integer_type_node,
							    tree_cons
							    (NULL_TREE,
  ...
  tree di_ftype_v8qi = build_function_type (long_long_integer_type_node,
					    tree_cons (NULL_TREE,
						       V8QI_type_node,
						       endlink));
  tree di_ftype_v4hi = build_function_type (long_long_integer_type_node,
					    tree_cons (NULL_TREE,
						       V4HI_type_node,
						       endlink));
  tree di_ftype_v2si = build_function_type (long_long_integer_type_node,
					    tree_cons (NULL_TREE,
						       V2SI_type_node,
						       endlink));
  tree v2si_ftype_v4hi = build_function_type (V2SI_type_node,
					      tree_cons (NULL_TREE,
							 V4HI_type_node,
							 endlink));
  tree v4hi_ftype_v8qi = build_function_type (V4HI_type_node,
					      tree_cons (NULL_TREE,
							 V8QI_type_node,
							 endlink));
  ...
  tree di_ftype_v4hi_v4hi = build_function_type (long_long_unsigned_type_node,
						 tree_cons (NULL_TREE,
							    V4HI_type_node,
							    tree_cons
							    (NULL_TREE,
  ...
  tree v8qi_ftype_v8qi_v8qi = build_function_type (V8QI_type_node,
						   tree_cons (NULL_TREE,
							      V8QI_type_node,
							      tree_cons
							      (NULL_TREE,
							       V8QI_type_node,
							       endlink)));
  tree v4hi_ftype_v4hi_v4hi = build_function_type (V4HI_type_node,
						   tree_cons (NULL_TREE,
							      V4HI_type_node,
							      tree_cons
							      (NULL_TREE,
							       V4HI_type_node,
							       endlink)));
  tree v2si_ftype_v2si_v2si = build_function_type (V2SI_type_node,
						   tree_cons (NULL_TREE,
							      V2SI_type_node,
							      tree_cons
							      (NULL_TREE,
							       V2SI_type_node,
							       endlink)));
  tree di_ftype_di_di = build_function_type (long_long_unsigned_type_node,
					     tree_cons (NULL_TREE,
							long_long_unsigned_type_node,
							tree_cons (NULL_TREE,
								   long_long_unsigned_type_node,
  ...
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wzero", di_ftype_void,
		ARM_BUILTIN_WZERO);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_setwcx", void_ftype_int_int,
		ARM_BUILTIN_SETWCX);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_getwcx", int_ftype_int,
		ARM_BUILTIN_GETWCX);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsllh", v4hi_ftype_v4hi_di,
		ARM_BUILTIN_WSLLH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsllw", v2si_ftype_v2si_di,
		ARM_BUILTIN_WSLLW);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wslld", di_ftype_di_di,
		ARM_BUILTIN_WSLLD);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsllhi", v4hi_ftype_v4hi_int,
		ARM_BUILTIN_WSLLHI);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsllwi", v2si_ftype_v2si_int,
		ARM_BUILTIN_WSLLWI);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wslldi", di_ftype_di_int,
		ARM_BUILTIN_WSLLDI);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrlh", v4hi_ftype_v4hi_di,
		ARM_BUILTIN_WSRLH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrlw", v2si_ftype_v2si_di,
		ARM_BUILTIN_WSRLW);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrld", di_ftype_di_di,
		ARM_BUILTIN_WSRLD);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrlhi", v4hi_ftype_v4hi_int,
		ARM_BUILTIN_WSRLHI);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrlwi", v2si_ftype_v2si_int,
		ARM_BUILTIN_WSRLWI);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrldi", di_ftype_di_int,
		ARM_BUILTIN_WSRLDI);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrah", v4hi_ftype_v4hi_di,
		ARM_BUILTIN_WSRAH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsraw", v2si_ftype_v2si_di,
		ARM_BUILTIN_WSRAW);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrad", di_ftype_di_di,
		ARM_BUILTIN_WSRAD);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrahi", v4hi_ftype_v4hi_int,
		ARM_BUILTIN_WSRAHI);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsrawi", v2si_ftype_v2si_int,
		ARM_BUILTIN_WSRAWI);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsradi", di_ftype_di_int,
		ARM_BUILTIN_WSRADI);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wrorh", v4hi_ftype_v4hi_di,
		ARM_BUILTIN_WRORH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wrorw", v2si_ftype_v2si_di,
		ARM_BUILTIN_WRORW);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wrord", di_ftype_di_di,
		ARM_BUILTIN_WRORD);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wrorhi", v4hi_ftype_v4hi_int,
		ARM_BUILTIN_WRORHI);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wrorwi", v2si_ftype_v2si_int,
		ARM_BUILTIN_WRORWI);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wrordi", di_ftype_di_int,
		ARM_BUILTIN_WRORDI);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wshufh", v4hi_ftype_v4hi_int,
		ARM_BUILTIN_WSHUFH);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsadb", v2si_ftype_v8qi_v8qi,
		ARM_BUILTIN_WSADB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsadh", v2si_ftype_v4hi_v4hi,
		ARM_BUILTIN_WSADH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsadbz", v2si_ftype_v8qi_v8qi,
		ARM_BUILTIN_WSADBZ);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wsadhz", v2si_ftype_v4hi_v4hi,
		ARM_BUILTIN_WSADHZ);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_textrmsb", int_ftype_v8qi_int,
		ARM_BUILTIN_TEXTRMSB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_textrmsh", int_ftype_v4hi_int,
		ARM_BUILTIN_TEXTRMSH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_textrmsw", int_ftype_v2si_int,
		ARM_BUILTIN_TEXTRMSW);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_textrmub", int_ftype_v8qi_int,
		ARM_BUILTIN_TEXTRMUB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_textrmuh", int_ftype_v4hi_int,
		ARM_BUILTIN_TEXTRMUH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_textrmuw", int_ftype_v2si_int,
		ARM_BUILTIN_TEXTRMUW);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tinsrb", v8qi_ftype_v8qi_int_int,
		ARM_BUILTIN_TINSRB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tinsrh", v4hi_ftype_v4hi_int_int,
		ARM_BUILTIN_TINSRH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tinsrw", v2si_ftype_v2si_int_int,
		ARM_BUILTIN_TINSRW);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_waccb", di_ftype_v8qi,
		ARM_BUILTIN_WACCB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wacch", di_ftype_v4hi,
		ARM_BUILTIN_WACCH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_waccw", di_ftype_v2si,
		ARM_BUILTIN_WACCW);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tmovmskb", int_ftype_v8qi,
		ARM_BUILTIN_TMOVMSKB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tmovmskh", int_ftype_v4hi,
		ARM_BUILTIN_TMOVMSKH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tmovmskw", int_ftype_v2si,
		ARM_BUILTIN_TMOVMSKW);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wpackhss", v8qi_ftype_v4hi_v4hi,
		ARM_BUILTIN_WPACKHSS);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wpackhus", v8qi_ftype_v4hi_v4hi,
		ARM_BUILTIN_WPACKHUS);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wpackwus", v4hi_ftype_v2si_v2si,
		ARM_BUILTIN_WPACKWUS);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wpackwss", v4hi_ftype_v2si_v2si,
		ARM_BUILTIN_WPACKWSS);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wpackdus", v2si_ftype_di_di,
		ARM_BUILTIN_WPACKDUS);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wpackdss", v2si_ftype_di_di,
		ARM_BUILTIN_WPACKDSS);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckehub", v4hi_ftype_v8qi,
		ARM_BUILTIN_WUNPCKEHUB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckehuh", v2si_ftype_v4hi,
		ARM_BUILTIN_WUNPCKEHUH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckehuw", di_ftype_v2si,
		ARM_BUILTIN_WUNPCKEHUW);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckehsb", v4hi_ftype_v8qi,
		ARM_BUILTIN_WUNPCKEHSB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckehsh", v2si_ftype_v4hi,
		ARM_BUILTIN_WUNPCKEHSH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckehsw", di_ftype_v2si,
		ARM_BUILTIN_WUNPCKEHSW);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckelub", v4hi_ftype_v8qi,
		ARM_BUILTIN_WUNPCKELUB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckeluh", v2si_ftype_v4hi,
		ARM_BUILTIN_WUNPCKELUH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckeluw", di_ftype_v2si,
		ARM_BUILTIN_WUNPCKELUW);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckelsb", v4hi_ftype_v8qi,
		ARM_BUILTIN_WUNPCKELSB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckelsh", v2si_ftype_v4hi,
		ARM_BUILTIN_WUNPCKELSH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wunpckelsw", di_ftype_v2si,
		ARM_BUILTIN_WUNPCKELSW);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wmacs", di_ftype_di_v4hi_v4hi,
		ARM_BUILTIN_WMACS);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wmacsz", di_ftype_v4hi_v4hi,
		ARM_BUILTIN_WMACSZ);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wmacu", di_ftype_di_v4hi_v4hi,
		ARM_BUILTIN_WMACU);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_wmacuz", di_ftype_v4hi_v4hi,
		ARM_BUILTIN_WMACUZ);

  def_mbuiltin (FL_IWMMXT, "__builtin_arm_walign", v8qi_ftype_v8qi_v8qi_int,
		ARM_BUILTIN_WALIGN);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tmia", di_ftype_di_int_int,
		ARM_BUILTIN_TMIA);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tmiaph", di_ftype_di_int_int,
		ARM_BUILTIN_TMIAPH);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tmiabb", di_ftype_di_int_int,
		ARM_BUILTIN_TMIABB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tmiabt", di_ftype_di_int_int,
		ARM_BUILTIN_TMIABT);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tmiatb", di_ftype_di_int_int,
		ARM_BUILTIN_TMIATB);
  def_mbuiltin (FL_IWMMXT, "__builtin_arm_tmiatt", di_ftype_di_int_int,
		ARM_BUILTIN_TMIATT);
}
...
static rtx
arm_expand_binop_builtin (enum insn_code icode, tree arglist, rtx target)
{
  rtx pat;
		    ...
		    int ignore ATTRIBUTE_UNUSED)
{
  ...
  switch (fcode)
    ...
    case ARM_BUILTIN_WSADB:
      return arm_expand_binop_builtin (CODE_FOR_iwmmxt_wsadb, arglist,
				       target);
    case ARM_BUILTIN_WSADH:
      return arm_expand_binop_builtin (CODE_FOR_iwmmxt_wsadh, arglist,
				       target);
    case ARM_BUILTIN_WSADBZ:
      return arm_expand_binop_builtin (CODE_FOR_iwmmxt_wsadbz, arglist,
				       target);
    case ARM_BUILTIN_WSADHZ:
      return arm_expand_binop_builtin (CODE_FOR_iwmmxt_wsadhz, arglist,
				       target);
...
thumb_exit (FILE * f, int reg_containing_return_addr, rtx eh_ofs)
{
  ...
  if (eh_ofs)
      ...
      if (size == 0)
	{
	  ...
	  if (mode == VOIDmode)
	    regs_available_for_popping =
	      (1 << ARG_REGISTER (1))
	      | (1 << ARG_REGISTER (2)) | (1 << ARG_REGISTER (3));
	  else
	    regs_available_for_popping =
	      (1 << ARG_REGISTER (2)) | (1 << ARG_REGISTER (3));
	}
	...
	regs_available_for_popping = (1 << ARG_REGISTER (3));
    }
...
#ifdef ARM_PE
  return lookup_attribute ("interfacearm",
			   DECL_ATTRIBUTES (func)) != NULL_TREE;
#else
  return FALSE;
#endif
}
...
thumb_unexpanded_epilogue (void)
{
  ...
  if (current_function_pretend_args_size == 0 || TARGET_BACKTRACE)
    {
      if (had_to_push_lr
	  && !is_called_in_ARM_mode (current_function_decl) && !eh_ofs)
...
#define STUB_NAME ".real_start_of"
  ...
  if (high_regs_pushed)
      ...
      while (high_regs_pushed > 0)
	{
      ...
      if (pushable_regs == 0 && (THUMB_REG_PUSHED_P (LAST_ARG_REGNUM)))
	asm_fprintf (f, "\tmov\t%r, %r\n", LAST_ARG_REGNUM, IP_REGNUM);
    }
}
  ...
  if (len >= 2)
    {
      ...
      emit_insn (gen_movhi
		 (gen_rtx (MEM, HImode, plus_constant (out, offset)), reg));
      len -= 2;
      offset += 2;
    }
  ...
  if (len)
    {
      ...
      emit_insn (gen_movqi (reg, gen_rtx (MEM, QImode,
					  plus_constant (in, offset))));
      emit_insn (gen_movqi
		 (gen_rtx (MEM, QImode, plus_constant (out, offset)), reg));
    }
}
...
thumb_cmp_operand (rtx op, enum machine_mode mode)
{
  return ((GET_CODE (op) == CONST_INT
	   && INTVAL (op) < 256
	   && INTVAL (op) >= 0) || s_register_operand (op, mode));
}
...
arm_get_strip_length (int c)
{
  switch (c)
    {
    ARM_NAME_ENCODING_LENGTHS default:
      return 0;
    }
}
...
arm_output_load_gr (rtx * operands)
{
  ...
  output_asm_insn ("str%?\t%0, [sp, #-4]!\t@ Start of GR load expansion",
		   &reg);
  wcgr = operands[0];
  operands[0] = reg;
  output_asm_insn ("ldr%?\t%0, %1", operands);
