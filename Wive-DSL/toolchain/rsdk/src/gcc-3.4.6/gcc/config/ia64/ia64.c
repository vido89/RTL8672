...
static void
ia64_rwreloc_select_section (tree, int, unsigned HOST_WIDE_INT)
  ATTRIBUTE_UNUSED;
     static void ia64_rwreloc_unique_section (tree, int) ATTRIBUTE_UNUSED;
     static void ia64_rwreloc_select_rtx_section (enum machine_mode, rtx,
						  unsigned HOST_WIDE_INT)
  ATTRIBUTE_UNUSED;
     static unsigned int ia64_rwreloc_section_type_flags (tree, const char *,
							  int)
  ATTRIBUTE_UNUSED;

     static void ia64_hpux_add_extern_decl (tree decl) ATTRIBUTE_UNUSED;
     static void ia64_hpux_file_end (void) ATTRIBUTE_UNUSED;
     static void ia64_hpux_init_libfuncs (void) ATTRIBUTE_UNUSED;
     static void ia64_vms_init_libfuncs (void) ATTRIBUTE_UNUSED;

     static tree ia64_handle_model_attribute (tree *, tree, tree, int,
					      bool *);
  ...
  if (name[0] == 's')
	...
	= ((name[1] == 'e'
	   ...
	   || (name[1] == 'a' && !strcmp (name, "savectx")));
	    ...
	    && !strcmp (name, "qsetjmp"))
	   || (name[0] == 'v' && name[1] == 'f' && !strcmp (name, "vfork")))
    retval = 1;
...
ar_lc_reg_operand (register rtx op, enum machine_mode mode)
{
  return (GET_MODE (op) == DImode
	  ...
	  && GET_CODE (op) == REG && REGNO (op) == AR_LC_REGNUM);
}
...
{
  return ((GET_MODE (op) == mode || mode == VOIDmode)
	  && GET_CODE (op) == REG && REGNO (op) == AR_CCV_REGNUM);
}
...
{
  return ((GET_MODE (op) == mode || mode == VOIDmode)
	  && GET_CODE (op) == REG && REGNO (op) == AR_PFS_REGNUM);
}
  ...
  switch (TREE_CODE (decl))
    ...
    case VAR_DECL:
      if ((DECL_CONTEXT (decl) && TREE_CODE (DECL_CONTEXT (decl))
	   == FUNCTION_DECL) && !TREE_STATIC (decl))
	{
...
ia64_encode_addr_area (tree decl, rtx symbol)
{
  ...
  switch (ia64_get_addr_area (decl))
    {
    case ADDR_AREA_NORMAL:
      break;
    case ADDR_AREA_SMALL:
      flags |= SYMBOL_FLAG_SMALL_ADDR;
      break;
    default:
      abort ();
    }
...
spill_xfmode_operand (rtx in, int force)
{
  if (GET_CODE (in) == SUBREG
    ...
    {
      rtx mem =
	gen_mem_addressof (SUBREG_REG (in), NULL_TREE, /*rescan= */ true);
  ...
  else if (GET_CODE (in) == MEM && GET_CODE (XEXP (in, 0)) == ADDRESSOF)
  ...
  /* HPUX TFmode compare requires a library call to _U_Qfcmp, which takes a
	...
	case UNORDERED:
	  ...
	  /* Relational operators raise FP_INVALID when given
	     an SNaN operand.  */
	case LT:
	  magic = QCMP_LT | QCMP_INV;
	  ncode = NE;
	  break;
	case LE:
	  magic = QCMP_LT | QCMP_EQ | QCMP_INV;
	  ncode = NE;
	  break;
	case GT:
	  magic = QCMP_GT | QCMP_INV;
	  ncode = NE;
	  break;
	case GE:
	  magic = QCMP_GT | QCMP_EQ | QCMP_INV;
	  ncode = NE;
	  break;
...
ia64_reload_gp (void)
{
  ...
  if (current_frame_info.reg_save_gp)
	...
	emit_insn (gen_adddi3 (pic_offset_table_rtx, tmp, GEN_INT (offset)));
      else
	{
...
spill_restore_mem (rtx reg, HOST_WIDE_INT cfa_off)
{
  ...
  /* Micro-optimization: if we've created a frame pointer, it's at
      ...
      else
	{
	  ...
	  emit_insn (gen_adddi3 (spill_fill_data.iter_reg[iter],
				 spill_fill_data.init_reg[iter], disp_rtx));
...
ia64_expand_prologue (void)
{
  ...
  if (optimize)
	...
	if ((e->flags & EDGE_FAKE) == 0 && (e->flags & EDGE_FALLTHRU) != 0)
	  break;
      epilogue_p = (e != NULL);
    }
  else
  ...
  if (current_frame_info.n_local_regs == 0
      ...
      current_function_args_info.int_regs
...
ia64_expand_epilogue (int sibcall_p)
{
  ...
  if (!sibcall_p)
  ...
  else
    {
      ...
      if (current_frame_info.reg_fp
	  && current_frame_info.reg_fp == GR_REG (2))
			       ...
			       HOST_WIDE_INT size ATTRIBUTE_UNUSED)
{
  ...
  if (current_frame_info.reg_fp != 0
      && (grsave_prev == 0 || current_frame_info.reg_fp == grsave_prev + 1))
    {
      mask |= 2;
      if (grsave_prev == 0)
...
int
ia64_function_arg_partial_nregs (CUMULATIVE_ARGS * cum,
...
ia64_override_options (void)
{
  ...
  ia64_section_threshold =
    g_switch_set ? g_switch_value : IA64_DEFAULT_GVALUE;
...
static void
rws_update (struct reg_write_state *rws, int regno, struct reg_flags flags,
	    int pred)
{
  if (pred)
  ...
  switch (GET_CODE (src))
      ...
      else
	{
	  ...
	  if (GET_CODE (cond) != REG
	    ...
	    abort ();
	  *pcond = cond;
	  if (XEXP (src, 1) == SET_DEST (x) || XEXP (src, 2) == SET_DEST (x))
	    {
...
rtx_needs_barrier (rtx x, struct reg_flags flags, int pred)
{
  ...
  switch (GET_CODE (x))
	...
	is_complemented = 1;
      cond = XEXP (cond, 0);
      if (GET_CODE (cond) != REG && REGNO_REG_CLASS (REGNO (cond)) != PR_REGS)
	abort ();
      pred = REGNO (cond);
      if (is_complemented)
	++pred;
	  ...
	  if (GET_CODE (pat) == SET)
	    {
	      ...
	      need_barrier |=
		set_src_needs_barrier (pat, new_flags, pred, cond);
	    }
      ...
      for (i = XVECLEN (x, 0) - 1; i >= 0; --i)
	{
	  ...
	  if (GET_CODE (pat) == SET)
	      ...
	      if (GET_CODE (SET_SRC (pat)) != CALL)
		{
		  new_flags.is_write = 1;
		  need_barrier |=
		    rtx_needs_barrier (SET_DEST (pat), new_flags, pred);
		}
	    }
	  else if (GET_CODE (pat) == CLOBBER || GET_CODE (pat) == RETURN)
	    need_barrier |= rtx_needs_barrier (pat, flags, pred);
...
group_barrier_needed_p (rtx insn)
{
  ...
  switch (GET_CODE (insn))
    ...
    case CALL_INSN:
      ...
      if ((pat = prev_active_insn (insn)) && GET_CODE (pat) == CALL_INSN)
	{
	  need_barrier = 1;
	  break;
	}
  ...
  if (GET_CODE (x) == REG)
    {
      ...
      if (TEST_HARD_REG_BIT (prev_group->gr_reg_conditionally_set, REGNO (x)))
	return 1;
      return -1;
    }
  return 0;
...
errata_emit_nops (rtx insn)
{
  ...
  if ((type == TYPE_M || type == TYPE_A) && cond && set
    ...
    {
      if (GET_RTX_CLASS (GET_CODE (cond)) != '<' || !REG_P (XEXP (cond, 0)))
	abort ();

      if (TEST_HARD_REG_BIT (prev_group->p_reg_set, REGNO (XEXP (cond, 0))))
	SET_HARD_REG_BIT (this_group->gr_reg_conditionally_set,
			  REGNO (SET_DEST (set)));
    }
...
ia64_dependencies_evaluation_hook (rtx head, rtx tail)
{
  ...
  for (insn = head; insn != next_tail; insn = NEXT_INSN (insn))
    if (INSN_P (insn))
      insn->call = 0;
  for (insn = head; insn != next_tail; insn = NEXT_INSN (insn))
    if (INSN_P (insn) && ia64_safe_itanium_class (insn) == ITANIUM_CLASS_IALU)
      {
		 ...
		 int max_ready ATTRIBUTE_UNUSED)
{
  ...
  if (reload_completed)
    for (insn = NEXT_INSN (current_sched_info->prev_head);
	 insn != current_sched_info->next_tail; insn = NEXT_INSN (insn))
...
static int
ia64_sched_reorder (FILE * dump, int sched_verbose, rtx * ready,
...
ia64_sched_reorder2 (FILE * dump ATTRIBUTE_UNUSED,
		     ...
		     int *pn_ready, int clock_var)
{
  if (ia64_tune == PROCESSOR_ITANIUM && reload_completed
      && last_scheduled_insn)
...
ia64_first_cycle_multipass_dfa_lookahead_guard (rtx insn)
{
  if (insn == NULL_RTX || !INSN_P (insn))
    abort ();
  return (!reload_completed || !safe_group_barrier_needed_p (insn));
}
  ...
  if (*entry_ptr == NULL)
    {
      ...
      *entry_ptr = (void *) bundle_state;
      return TRUE;
	   ...
	   || (bundle_state->cost ==
	       ((struct bundle_state *) *entry_ptr)->cost
	       && (((struct bundle_state *) *entry_ptr)->
		   accumulated_insns_num > bundle_state->accumulated_insns_num
		   || (((struct bundle_state *) *entry_ptr)->
		       accumulated_insns_num ==
		       bundle_state->accumulated_insns_num
		       && ((struct bundle_state *) *entry_ptr)->
		       branch_deviation > bundle_state->branch_deviation))))
      ...
      if (!only_bundle_end_p && insert_bundle_state (curr_state))
	{
	  ...
	  *curr_state1 = *curr_state;
      ...
      if (!try_issue_nops (curr_state,
			   3 - curr_state->accumulated_insns_num % 3))
	return;
      curr_state->after_nops_num = 3 - curr_state->accumulated_insns_num % 3;
      curr_state->accumulated_insns_num
	+= 3 - curr_state->accumulated_insns_num % 3;
    }
...
bundling (FILE * dump, int verbose, rtx prev_head_insn, rtx tail)
{
  ...
  for (insn = NEXT_INSN (prev_head_insn);
       insn && insn != tail; insn = NEXT_INSN (insn))
    if (INSN_P (insn))
      insn_num++;
  if (insn_num == 0)
    return;
  ...
  for (insn = NEXT_INSN (prev_head_insn);
	...
	for (next_insn = NEXT_INSN (insn);
	     next_insn != tail; next_insn = NEXT_INSN (next_insn))
	  if (INSN_P (next_insn)
  ...
  for (insn = get_next_important_insn (NEXT_INSN (prev_head_insn), tail);
	...
	abort ();
      for (curr_state = index_to_bundle_states[insn_num];
	   curr_state != NULL; curr_state = curr_state->next)
	if (verbose >= 2 && dump)
	  {
		...
		? -1 : curr_state->originator->unique_num),
	       ...
	       curr_state->branch_deviation,
	       (ia64_tune ==
		PROCESSOR_ITANIUM ? ((struct DFA_chip *) curr_state->
				     dfa_state)->
		oneb_automaton_state : ((struct DFA_chip *) curr_state->
					dfa_state)->twob_automaton_state),
	       INSN_UID (insn));
	  }
    }
  if (index_to_bundle_states[insn_num] == NULL)
  ...
  for (curr_state = best_state;
       curr_state->originator != NULL; curr_state = curr_state->originator)
    {
      ...
      if (verbose >= 2 && dump)
	{
	  ...
	  fprintf
	    (dump,
	     ...
	     (ia64_tune == PROCESSOR_ITANIUM
	      ? ((struct DFA_chip *) curr_state->dfa_state)->
	      oneb_automaton_state : ((struct DFA_chip *) curr_state->
				      dfa_state)->twob_automaton_state),
	     INSN_UID (insn));
	}
	  ...
	  /* We found a MM-insn which needs additional cycles.  */
	  {
	      ...
	      if (recog_memoized (last) == CODE_FOR_bundle_selector)
		{
		  ...
		  if (template0 == 9)
		    ...
		    PATTERN (last) = gen_bundle_selector (GEN_INT (2));	/* -> MFI */
		  break;
		}
...
process_set (FILE * asm_out_file, rtx pat)
{
  ...
  if (GET_CODE (src) == UNSPEC_VOLATILE
      && XINT (src, 1) == UNSPECV_ALLOC && GET_CODE (dest) == REG)
    {
...
ia64_init_builtins (void)
{
  ...
  tree di_ftype_pdi_di_di = build_function_type_list (long_integer_type_node,
						      pdi_type_node,
						      long_integer_type_node,
						      long_integer_type_node,
						      NULL_TREE);
  /* __sync_bool_compare_and_swap_di */
  tree si_ftype_pdi_di_di = build_function_type_list (integer_type_node,
						      pdi_type_node,
						      long_integer_type_node,
						      long_integer_type_node,
						      NULL_TREE);
  ...
  tree si_ftype_psi_si = build_function_type_list (integer_type_node,
						   psi_type_node,
						   integer_type_node,
						   NULL_TREE);

  /* __sync_lock_test_and_set_di */
  tree di_ftype_pdi_di = build_function_type_list (long_integer_type_node,
						   pdi_type_node,
						   long_integer_type_node,
						   NULL_TREE);
		     ...
		     int ignore ATTRIBUTE_UNUSED)
{
  ...
  switch (fcode)
    ...
    case IA64_BUILTIN_BOOL_COMPARE_AND_SWAP_SI:
    case IA64_BUILTIN_BOOL_COMPARE_AND_SWAP_DI:
      return ia64_expand_compare_and_swap (rmode, mode, 1, arglist, target);

    case IA64_BUILTIN_VAL_COMPARE_AND_SWAP_SI:
    case IA64_BUILTIN_VAL_COMPARE_AND_SWAP_DI:
      return ia64_expand_compare_and_swap (rmode, mode, 0, arglist, target);
...
static
GTY (())
     struct extern_func_list *extern_func_head;
...
static void
ia64_rwreloc_select_section (tree exp, int reloc,
			     unsigned HOST_WIDE_INT align)
{
		      ...
		      tree function)
{
  ...
  this_parmno
    = (ia64_struct_retval_addr_is_first_parm_p (TREE_TYPE (thunk)) ? 1 : 0);
...
static rtx
ia64_struct_value_rtx (tree fntype, int incoming ATTRIBUTE_UNUSED)
{
