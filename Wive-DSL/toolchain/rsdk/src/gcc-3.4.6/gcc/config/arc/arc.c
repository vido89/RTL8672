...
arc_init (void)
{
  ...
  if (arc_cpu_string == 0 || !strcmp (arc_cpu_string, "base"))
    {
  ...
  arc_text_section = tmp =
    xmalloc (strlen (arc_text_string) + sizeof (ARC_SECTION_FORMAT) + 1);
  sprintf (tmp, ARC_SECTION_FORMAT, arc_text_string);
  arc_data_section = tmp =
    xmalloc (strlen (arc_data_string) + sizeof (ARC_SECTION_FORMAT) + 1);
  sprintf (tmp, ARC_SECTION_FORMAT, arc_data_string);
  arc_rodata_section = tmp =
    xmalloc (strlen (arc_rodata_string) + sizeof (ARC_SECTION_FORMAT) + 1);
  ...
  if (low & 0x80000000)
    {
      return (((unsigned HOST_WIDE_INT) low <= 0xffffffff && high == 0)
	      || (((low & -(unsigned HOST_WIDE_INT) 0x80000000)
		   == -(unsigned HOST_WIDE_INT) 0x80000000) && high == -1));
    }
  else
    {
...
arc_compute_function_type (tree decl)
{
  ...
  for (a = DECL_ATTRIBUTES (current_function_decl); a; a = TREE_CHAIN (a))
    {
...
arc_save_restore (FILE * file,
		  const char *base_reg,
		  unsigned int offset, unsigned int gmask, const char *op)
{
  int regno;
...
arc_output_function_prologue (FILE * file, HOST_WIDE_INT size)
{
  ...
  fprintf (file,
	   ...
	   ASM_COMMENT_START, ASM_COMMENT_START, current_frame_info.var_size,
	   current_frame_info.reg_size / 4, current_frame_info.args_size,
	   current_frame_info.extra_size);

  size = ARC_STACK_ALIGN (size);
  size = (!current_frame_info.initialized
	  ? arc_compute_frame_size (size) : current_frame_info.total_size);
  ...
  if (!noepilogue)
      ...
      if (frame_pointer_needed)
	{
	  ...
	  if (epilogue_delay != NULL_RTX
	      ...
	      || pretend_size || ARC_INTERRUPT_P (fn_type))
	    {
...
{
  if (arc_compute_function_type (current_function_decl) !=
      ARC_FUNCTION_NORMAL)
    return 0;
	  ...
	  if (final_sequence && XVECLEN (final_sequence, 0) == 2)
	      ...
	      if (INSN_ANNULLED_BRANCH_P (insn))
		{
		  if (INSN_FROM_TARGET_P (insn))
		    fprintf (file, "%s%s",
			     code == '?' ? "." : "",
			     arc_condition_codes[ARC_INVERSE_CONDITION_CODE
  ...
  if (reverse
	   ...
	   insns_skipped++)
	{
	  ...
	  if (next_must_be_target_label_p)
		...
		continue;
	      if (GET_CODE (this_insn) == CODE_LABEL && this_insn == label)
		{
		  arc_ccfsm_state = 1;
		  succeed = TRUE;
		}
      ...
      if (succeed)
	{
	  ...
	  if (!reverse)
	    arc_ccfsm_current_cc =
	      get_arc_condition_code (XEXP (SET_SRC (body), 0));
...
arc_va_start (tree valist, rtx nextarg)
{
  ...
  if (current_function_args_info < 8 && (current_function_args_info & 1))
