...
override_options (void)
{
  ...
  if (m68k_library_id_string)
	...
	error ("-mshared-library-id=%d is not between 0 and %d", id,
	       MAX_LIBRARY_ID);
	...
	if (current_frame.reg_mask & (1 << i))
	  {
		...
		if (MOTOROLA)
		  asm_fprintf (stream, "\t%Omove.l -%wd(%s,%Ra1.l),%s\n",
			       offset,
			       reg_names[FRAME_POINTER_REGNUM], reg_names[i]);
		else
...
output_btst (rtx * operands, rtx countop, rtx dataop, rtx insn, int signpos)
{
  ...
  if (GET_CODE (countop) == CONST_INT)
	...
	cc_status.flags = CC_NOT_NEGATIVE | CC_Z_IN_NOT_N;
      ...
      if (count == 31 && next_insn_tests_no_inequality (insn))
	return "tst%.l %1";
      if (count == 15 && next_insn_tests_no_inequality (insn))
	return "tst%.w %1";
      if (count == 7 && next_insn_tests_no_inequality (insn))
	return "tst%.b %1";
...
typedef enum
{ MOVL, SWAP, NEGW, NOTW, NOTB, MOVQ } CONST_METHOD;
...
#define DIVW_COST (TARGET_68020 ? 27 : TARGET_CF_HWDIV ? 11 : 12)
      ...
      if (!TARGET_68020 && !TARGET_COLDFIRE)
	{
      ...
      if (GET_CODE (XEXP (x, 1)) == CONST_INT && (INTVAL (XEXP (x, 1)) == 16))
	{
	  *total = COSTS_N_INSNS (2);	/* clrw;swap */
	  return true;
	}
      if (GET_CODE (XEXP (x, 1)) == CONST_INT
	  && !(INTVAL (XEXP (x, 1)) > 0 && INTVAL (XEXP (x, 1)) <= 8))
	{
...
output_move_simode_const (rtx * operands)
{
  if (operands[1] == const0_rtx
      && (DATA_REG_P (operands[0]) || GET_CODE (operands[0]) == MEM)
    ...
    return "clr%.l %0";
  else if (operands[1] == const0_rtx && ADDRESS_REG_P (operands[0]))
    return "sub%.l %0,%0";
...
output_move_qimode (rtx * operands)
{
  ...
  if (GET_CODE (operands[0]) == MEM
      ...
      && !ADDRESS_REG_P (operands[1]) && !TARGET_COLDFIRE)
    {
...
output_move_double (rtx * operands)
{
  ...
  /* For (set (reg:DI N) (mem:DI ... (reg:SI N) ...)),
     ...
     operand.  */
  if (optype0 == REGOP && (optype1 == OFFSOP || optype1 == MEMOP))
    {
			     ...
			     rtx target)
{
  ...
  while (p)
      ...
      if (reg_set_p (target, p))
	{
	  ...
	  if (rtx_equal_p (dest, target) && SET_SRC (set) == const0_rtx)
	    return true;
...
output_andsi3 (rtx * operands)
{
  ...
  if (GET_CODE (operands[2]) == CONST_INT
      ...
      && (DATA_REG_P (operands[0])
	  || offsettable_memref_p (operands[0])) && !TARGET_COLDFIRE)
    {
  ...
  if (GET_CODE (operands[2]) == CONST_INT
      ...
      && (DATA_REG_P (operands[0]) || offsettable_memref_p (operands[0])))
    {
	...
	{
	  operands[0] =
	    adjust_address (operands[0], SImode, 3 - (logval / 8));
