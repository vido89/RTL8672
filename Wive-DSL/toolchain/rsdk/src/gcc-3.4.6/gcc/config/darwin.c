...
static
GTY (())
     tree machopic_defined_list;

     enum machopic_addr_class machopic_classify_ident (tree ident)
{
  ...
  if (name[0] != '!')
      ...
      if (lprefix)
	{
      ...
      for (temp = machopic_defined_list;
	   temp != NULL_TREE; temp = TREE_CHAIN (temp))
	{
  ...
  for (temp = machopic_defined_list; temp != NULL_TREE;
...
GTY (())
     ...
     const char *machopic_function_base_name (void)
{
...
static
GTY (())
     const char *function_base_func_name;
     static GTY (())
     int current_pic_label_num;

     void machopic_output_function_base_name (FILE * file)
{
  const char *current_name;
...
/* Return a non-lazy pointer name corresponding to the given name,
   ...
   a new one.  */
     ...
     static const char *machopic_non_lazy_ptr_name (const char *name)
{
...
/* Return the name of the stub corresponding to the given name,
   generating a new stub name if necessary.  */
     ...
     const char *machopic_stub_name (const char *name)
{
  tree temp, ident = get_identifier (name);
  const char *tname;

  for (temp = machopic_stubs; temp != NULL_TREE; temp = TREE_CHAIN (temp))
    {
      if (ident == TREE_VALUE (temp))
	return IDENTIFIER_POINTER (TREE_PURPOSE (temp));
	...
	return IDENTIFIER_POINTER (TREE_PURPOSE (temp));
      ...
      if (name[0] != '!' && tname[0] == '!' && strcmp (name, tname + 4) == 0)
	return IDENTIFIER_POINTER (TREE_PURPOSE (temp));
    }
...
machopic_indirect_data_reference (rtx orig, rtx reg)
{
  ...
  if (GET_CODE (orig) == SYMBOL_REF)
      ...
      else if (defined)
	{
      ...
      ptr_ref = gen_rtx_SYMBOL_REF (Pmode, machopic_non_lazy_ptr_name (name));
...
machopic_legitimize_pic_address (rtx orig, enum machine_mode mode, rtx reg)
{
  ...
  if (GET_CODE (orig) == LABEL_REF || (GET_CODE (orig) == SYMBOL_REF))
    {
      ...
      if (GET_CODE (orig) == PLUS && GET_CODE (XEXP (orig, 0)) == REG)
	{
	  if (reg == 0)
		...
		sum = gen_rtx_PLUS (Pmode, pic_offset_table_rtx, sum);
	      ...
	      emit_insn (gen_rtx_SET (VOIDmode, reg,
				      gen_rtx_LO_SUM (Pmode, reg,
						      gen_rtx_CONST (Pmode,
								     offset))));
	      pic_ref = gen_rtx_PLUS (Pmode, pic_offset_table_rtx, reg);
#endif
	    }
	  else
#endif /* HAVE_lo_sum */
  ...
  else if (GET_CODE (orig) == MEM && GET_CODE (XEXP (orig, 0)) == SYMBOL_REF)
    {
			 ...
			 unsigned HOST_WIDE_INT align ATTRIBUTE_UNUSED)
{
  ...
  if (TREE_CODE (exp) == STRING_CST
	   ...
	   && flag_merge_constants)
    {
	  ...
	  TREE_INT_CST_LOW (size) == 4 && TREE_INT_CST_HIGH (size) == 0)
	literal4_section ();
      else if (TREE_CODE (size) == INTEGER_CST &&
	       TREE_INT_CST_LOW (size) == 8 && TREE_INT_CST_HIGH (size) == 0)
	literal8_section ();
      else
	base_function ();
    }
