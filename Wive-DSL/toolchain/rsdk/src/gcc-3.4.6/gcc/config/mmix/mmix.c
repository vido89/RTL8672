  ...
  if (TARGET_ABI_GNU)
	...
	call_used_regs[i] = 0;
      ...
      for (i = MMIX_RESERVED_GNU_ARG_0_REGNUM;
	   i < MMIX_RESERVED_GNU_ARG_0_REGNUM + MMIX_MAX_ARGS_IN_REGS; i++)
	fixed_regs[i] = 0;
    }
...
{
  if (class == REMAINDER_REG || class == HIMULT_REG || class == SYSTEM_REGS)
    return GENERAL_REGS;
...
{
  return (c == 'G' ? value == CONST0_RTX (GET_MODE (value)) : 0);
}
...
mmix_extra_constraint (rtx x, int c, int strict)
{
  ...
  if (c == 'U')
    return
      strict
      ? strict_memory_address_p (Pmode, x) : memory_address_p (Pmode, x);
...
{
  return count == 0 ? (MMIX_CFUN_NEEDS_SAVED_EH_RETURN_ADDRESS
...
mmix_initial_elimination_offset (int fromreg, int toreg)
{
  ...
  for (regno = MMIX_FIRST_GLOBAL_REGNUM; regno <= 255; regno++)
...
mmix_function_arg (const CUMULATIVE_ARGS * argsp,
		   ...
		   tree type, int named ATTRIBUTE_UNUSED, int incoming)
{
    ...
    return (argsp->regs < MMIX_MAX_ARGS_IN_REGS)
      ? gen_rtx_REG (mode,
		     (incoming
		      ...
		      : MMIX_FIRST_ARG_REGNUM) + argsp->regs) : NULL_RTX;
...
mmix_function_outgoing_value (tree valtype, tree func ATTRIBUTE_UNUSED)
{
  ...
  /* Return values that fit in a register need no special handling.
     ...
     registers.  */
  if (TARGET_ABI_GNU || GET_MODE_BITSIZE (mode) <= BITS_PER_WORD)
    return gen_rtx_REG (mode, MMIX_OUTGOING_RETURN_VALUE_REGNUM);
...
mmix_reorg (void)
{
  ...
  /* We put the number of the highest saved register-file register in a
     ...
     anyway.  */
  for (regno = MMIX_LAST_STACK_REGISTER_REGNUM; regno >= 0; regno--)
    if ((regs_ever_live[regno] && !call_used_regs[regno])
...
mmix_expand_builtin_va_arg (tree valist, tree type)
{
  ...
  if (AGGREGATE_TYPE_P (type)
      ...
      else
	{
	  ...
	  tree addend = fold (build (COND_EXPR, sizetype,
...
mmix_initialize_trampoline (rtx trampaddr, rtx fnaddr, rtx static_chain)
{
  ...
  emit_move_insn (gen_rtx_MEM (DImode,
			       plus_constant (trampaddr, 24)), fnaddr);
  ...
  while (string < string_end)
    {
	  ...
	  && !ISCNTRL (*string) && strchr (unwanted_chars, *string) == NULL)
	{
	  fputc ('"', stream);
	  while (*string
		 && (unsigned char) *string < 128
...
mmix_asm_output_labelref (FILE * stream, const char *name)
{
  ...
  asm_fprintf (stream, "%s%U%s",
	       is_extern && TARGET_TOPLEVEL_SYMBOLS ? ":" : "", name);
}
...
mmix_print_operand (FILE * stream, rtx x, int code)
{
  ...
  switch (code)
    ...
    case 'm':
      ...
      if (GET_CODE (x) != CONST_INT)
	{
	  fatal_insn ("MMIX Internal: Bad value for 'm', not a CONST_INT", x);
	}
    ...
    default:
      ...
      internal_error
	("MMIX Internal: Missing `%c' case in mmix_print_operand", code);
    }
...
mmix_asm_output_addr_diff_elt (FILE * stream,
			       rtx body ATTRIBUTE_UNUSED, int value, int rel)
{
...
mmix_use_simple_return (void)
{
  ...
  int stack_space_to_allocate
    = (current_function_outgoing_args_size
       + current_function_pretend_args_size + get_frame_size () + 7) & ~7;

  if (!TARGET_USE_RETURN_INSN || !reload_completed)
    return 0;

  for (regno = 255; regno >= MMIX_FIRST_GLOBAL_REGNUM; regno--)
...
mmix_expand_prologue (void)
{
  ...
  HOST_WIDE_INT stack_space_to_allocate
    = (current_function_outgoing_args_size
       + current_function_pretend_args_size + locals_size + 7) & ~7;
  ...
  if (current_function_pretend_args_size)
	...
	= (MMIX_FIRST_INCOMING_ARG_REGNUM
      ...
      for (regno
	   ...
	   regno >= mmix_first_vararg_reg; regno--)
	{
	  if (offset < 0)
	    {
  ...
  if (frame_pointer_needed)
      ...
      if (offset < 0)
	{
      ...
      insn = emit_insn (gen_adddi3 (hard_frame_pointer_rtx,
				    stack_pointer_rtx, GEN_INT (offset + 8)));
      RTX_FRAME_RELATED_P (insn) = 1;
      offset -= 8;
    }
  ...
  if (MMIX_CFUN_HAS_LANDING_PAD)
      ...
      if (offset < 0)
	{
      ...
      emit_move_insn (gen_rtx_REG (DImode, 255),
		      gen_rtx_REG (DImode, MMIX_rO_REGNUM));
      emit_move_insn (gen_rtx_MEM (DImode,
...
mmix_valid_comparison (RTX_CODE code, enum machine_mode mode, rtx op)
{
  ...
  if ((mode == CC_FPmode || mode == DFmode) && (code == GT || code == LT))
    return 1;

  if ((mode == CC_FPEQmode || mode == DFmode) && (code == EQ || code == NE))
    return 1;
...
mmix_gen_compare_reg (RTX_CODE code, rtx x, rtx y)
{
  ...
  emit_insn (gen_rtx_SET (VOIDmode, cc_reg, gen_rtx_COMPARE (ccmode, x, y)));
...
#define CCEND {NIL, NULL, NULL}
  ...
  static const struct cc_conv cc_fun_convs[] = { {ORDERED, "Z", "P"},
  {UNORDERED, "P", "Z"},
  CCEND
  };
  static const struct cc_conv cc_fp_convs[] = { {GT, "P", NULL},
  {LT, "N", NULL},
  CCEND
  };
  static const struct cc_conv cc_fpeq_convs[] = { {NE, "Z", "P"},
  {EQ, "P", "Z"},
  CCEND
  };
  static const struct cc_conv cc_uns_convs[] = { {GEU, "NN", "N"},
  {GTU, "P", "NP"},
  {LEU, "NP", "P"},
  {LTU, "N", "NN"},
  CCEND
  };
  static const struct cc_conv cc_signed_convs[] = { {NE, "NZ", "Z"},
  {EQ, "Z", "NZ"},
  {GE, "NN", "N"},
  {GT, "P", "NP"},
  {LE, "NP", "P"},
  ...
  static const struct cc_conv cc_di_convs[] = { {NE, "NZ", "Z"},
  {EQ, "Z", "NZ"},
  {GE, "NN", "N"},
  {GT, "P", "NP"},
  {LE, "NP", "P"},
...
#undef CCEND
  ...
  static const struct cc_type_conv cc_convs[] = { {CC_FUNmode, cc_fun_convs},
