...
static tree sh_handle_interrupt_handler_attribute (tree *, tree, tree, int,
						   bool *);
...
static void sh_output_mi_thunk (FILE *, tree, HOST_WIDE_INT, HOST_WIDE_INT,
				tree);
...
static void sh_setup_incoming_varargs (CUMULATIVE_ARGS *, enum machine_mode,
				       tree, int *, int);
...
print_operand (FILE * stream, rtx x, int code)
{
  switch (code)
	...
	case SUBREG:
	  if (SUBREG_BYTE (x) != 0 || GET_CODE (SUBREG_REG (x)) != REG)
	    abort ();
	...
	case REG:
	  if (FP_REGISTER_P (REGNO (x)) && GET_MODE (x) == V16SFmode)
	    fprintf ((stream), "mtrx%s", reg_names[REGNO (x)] + 2);
	  else if (FP_REGISTER_P (REGNO (x)) && GET_MODE (x) == V4SFmode)
	    fprintf ((stream), "fv%s", reg_names[REGNO (x)] + 2);
	  else if (GET_CODE (x) == REG && GET_MODE (x) == V2SFmode)
		   ...
		   enum machine_mode mode)
{
  ...
  if (GET_CODE (dst) == MEM && GET_CODE (XEXP (dst, 0)) == PRE_DEC)
    return "mov.l	%T1,%0\n\tmov.l	%1,%0";
...
output_far_jump (rtx insn, rtx op)
{
  ...
  if (TARGET_SH2
      && offset >= -32764 && offset - get_attr_length (insn) <= 32766)
    {
      far = 0;
...
static const char ashiftrt_insns[] =
  { 0, 1, 2, 3, 4, 5, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 2, 3, 4, 5, 8, 8, 8, 8, 8,
8, 8, 8, 8, 8, 8, 2 };

/* Left shift and logical right shift are the same.  */
static const char shift_insns[] =
  { 0, 1, 1, 2, 2, 3, 3, 4, 1, 2, 2, 3, 3, 4, 3, 3, 1, 2, 2, 3, 3, 4, 3, 3, 2,
3, 3, 4, 4, 4, 3, 3 };
...
static const char ext_shift_insns[] =
  { 0, 1, 1, 2, 2, 3, 2, 2, 1, 2, 2, 3, 3, 3, 2, 2, 1, 2, 2, 3, 3, 4, 3, 3, 2,
3, 3, 4, 4, 4, 3, 3 };
  ...
  if (GET_MODE_SIZE (GET_MODE (x)) > UNITS_PER_WORD)
    {
	  ...
	  && GET_CODE (XEXP (x, 1)) == CONST_INT && INTVAL (XEXP (x, 1)) == 1)
	return 2;
...
addsubcosts (rtx x)
{
  ...
  if (GET_CODE (XEXP (x, 1)) == REG || GET_CODE (XEXP (x, 1)) == SUBREG)
    return 1;
      ...
      if (TARGET_SHMEDIA)
	{
	  if (INTVAL (x) == 0)
	    ...
	    *total = 0;
		    ...
		    || outer_code == PLUS) && CONST_OK_FOR_I10 (INTVAL (x)))
	    *total = 0;
...
sh_address_cost (rtx X)
{
  return (GET_CODE (X) == PLUS
	  && !CONSTANT_P (XEXP (X, 1)) && !TARGET_SHMEDIA ? 1 : 0);
}
...
shl_sext_kind (rtx left_rtx, rtx size_rtx, int *costp)
{
  ...
  for (ext = 16; ext >= insize; ext -= 8)
	...
	cost =
	  ext_shift_insns[ext - insize] + ext_shift_insns[size - ext] + 1;
...
add_constant (rtx x, enum machine_mode mode, rtx last_value)
{
  ...
  if (last_value
      && rtx_equal_p (last_value, pool_vector[pool_size - 1].value))
    {
      lab = 0;
...
hi_const (rtx src)
{
  return (GET_CODE (src) == CONST_INT
	  && INTVAL (src) >= -32768 && INTVAL (src) <= 32767);
}
...
mova_p (rtx insn)
{
  return (GET_CODE (insn) == INSN
	  ...
	  && GET_CODE (XVECEXP (SET_SRC (PATTERN (insn)), 0, 0)) ==
	  LABEL_REF);
}
      ...
      if (broken_move (from))
	{
	  ...
	  if (mode == HImode
	  ...
	  else
	    {
	      ...
	      if (TARGET_SHCOMPACT
		  && !found_di && (mode == DFmode || mode == DImode))
		{
		  found_di = 1;
		  si_limit -= 8;
		}
...
gen_block_redirect (rtx jump, int addr, int need_block)
{
  ...
  if (dead)
    ...
    return emit_insn_before (gen_block_branch_redirect
			     (GEN_INT
			      (INSN_UID
			       (XEXP (SET_SRC (PATTERN (jump)), 0)))), jump);
  return prev;
}
    ...
    (emit_insn_after
     (gen_stuff_delay_slot
      (GEN_INT (INSN_UID (XEXP (SET_SRC (PATTERN (jump)), 0))),
       GEN_INT (recog_memoized (insn) == CODE_FOR_branch_false)), insn));
...
barrier_align (rtx barrier_or_label)
{
  ...
  if (GET_CODE (PATTERN (prev)) == ADDR_DIFF_VEC)
    {
      ...
      return ((TARGET_SMALLCODE
	       || ((unsigned) XVECLEN (pat, 1) *
		   GET_MODE_SIZE (GET_MODE (pat)) <=
		   (unsigned) 1 << (CACHE_LOG -
				    2))) ? 1 << TARGET_SHMEDIA :
	      align_jumps_log);
    }
  ...
  if (mdep_reorg_phase > SH_FIXUP_PCLOAD)
	   ...
	   prev = prev_real_insn (prev))
	{
	  ...
	  if (slot && get_attr_in_delay_slot (prev) == IN_DELAY_SLOT_YES)
	    slot = 0;
	  credit -= get_attr_length (prev);
	}
      if (prev && GET_CODE (prev) == JUMP_INSN && JUMP_LABEL (prev))
	{
	  rtx x;
	  if (jump_to_next || next_real_insn (JUMP_LABEL (prev)) == next
...
sh_reorg (void)
{
      ...
      for (insn = first; insn; insn = NEXT_INSN (insn))
	{
	  ...
	  for (scan = NEXT_INSN (link); scan; scan = NEXT_INSN (scan))
		  ...
		  && (GET_CODE (scan) == CALL_INSN || sfunc_uses_reg (scan)))
		{
	      ...
	      if (foundinsn
		  && (scanset || find_reg_note (scan, REG_DEAD, reg)))
		{
		  dies = scan;
		  break;
		}
  ...
  for (insn = first, num_mova = 0; insn; insn = NEXT_INSN (insn))
      ...
      if (mova_p (insn))
	{
	  ...
	  if (optimize
	    ...
	    mova = insn;
	       ...
	       && GET_CODE (PATTERN (insn)) == ADDR_DIFF_VEC && num_mova)
	{
	  rtx scan;
	  int total;
	      ...
	      && recog_memoized (insn) == CODE_FOR_casesi_worker_2))
	{
	  ...
	  rtx last_float_move = NULL_RTX, last_float = 0, *last_float_addr =
	    NULL;
	      ...
	      if (broken_move (scan))
		{
		  ...
		  if (GET_CODE (dst) == REG
			...
			abort ();
		      ...
		      if (last_float
			  && reg_set_between_p (r0_rtx, last_float_move,
						scan))
			last_float = 0;
		      if (last_float
			  && TARGET_SHCOMPACT
	...
	if (type == TYPE_CBRANCH)
	  {
		...
		if (!bp)
		...
		else
		  {
			...
			if (GET_CODE (PATTERN (block)) == RETURN)
			  ...
			  block = gen_block_redirect (block, bp->address, 2);
		...
		beyond
		  =
		  next_active_insn (XEXP
				    (XEXP (SET_SRC (PATTERN (insn)), 1), 0));
...
push_regs (HARD_REG_SET * mask, int interrupt_handler)
{
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
	  ...
	  && hard_regs_intersect_p (mask, &reg_class_contents[DF_REGS]))
	{
      ...
      if (i != PR_REG
	  && (i != FPSCR_REG || !skip_fpscr) && TEST_HARD_REG_BIT (*mask, i))
	push (i);
    }
...
calc_live_regs (HARD_REG_SET * live_regs_mask)
{
  ...
  for (count = 0, reg = FIRST_PSEUDO_REGISTER - 1; reg >= 0; reg--)
    {
      if (reg == (TARGET_SHMEDIA ? PR_MEDIA_REG : PR_REG) ? pr_live : (interrupt_handler && !pragma_trapa) ? (	/* Need to save all the regs ever live.  */
													       (regs_ever_live[reg]
														||
														(call_used_regs
														 [reg]
														...
														(has_call
														 ...
														 (GENERAL_REGISTER_P
														  ...
														  (reg))))
													       ...
													       && (reg != FPSCR_REG || TARGET_FPU_ANY)) : (	/* Only push those regs which are used and need to be saved.  */
																			    (TARGET_SHCOMPACT
																			     &&
																			     flag_pic
																			     &&
																			    ...
																			    (current_function_calls_eh_return
																			     ...
																			     (reg
																			      ...
																			      (3)))
																			    ||
																			    ((reg == MACL_REG || reg == MACH_REG) && regs_ever_live[reg] && sh_cfun_attr_renesas_p ())))
	{
		  ...
		  if (!TARGET_FPU_SINGLE && !regs_ever_live[reg ^ 1])
		    {
		      ...
		      count +=
			GET_MODE_SIZE (REGISTER_NATURAL_MODE (reg ^ 1));
		    }
		}
		    ...
		    int offset_base)
{
  ...
  if (!current_function_interrupt)
    for (i = FIRST_GENERAL_REG; tmpx < MAX_TEMPS && i <= LAST_GENERAL_REG;
	 i++)
      if (call_used_regs[i] && !fixed_regs[i] && i != PR_MEDIA_REG
	  && !FUNCTION_ARG_REGNO_P (i) && i != FIRST_RET_REG
	...
	if (TEST_HARD_REG_BIT (*live_regs_mask, i))
	  {
	    ...
	    if (current_function_interrupt
		&& GENERAL_REGISTER_P (i) && tmpx < MAX_TEMPS)
	      schedule->temps[tmpx++] = i;
...
sh_expand_prologue (void)
{
  ...
  if (TARGET_SHCOMPACT
	...
	if (CALL_COOKIE_STACKSEQ_GET (current_function_args_info.call_cookie)
      ...
      emit_move_insn (gen_rtx_REG (Pmode, MACL_REG), stack_pointer_rtx);
	...
	try_pre_dec:
	  do
		...
		&& (offset_in_r0 - offset == GET_MODE_SIZE (mode)
	      ...
	      {
		pre_dec = gen_rtx_MEM (mode, gen_rtx_PRE_DEC (Pmode, r0));
	  ...
	  if (offset_in_r0 == -1)
	  ...
	  else if (offset != offset_in_r0)
	    {
	      emit_move_insn (r0,
			      gen_rtx_PLUS
			      (Pmode, r0, GEN_INT (offset - offset_in_r0)));
	...
	try_post_inc:
	  do
		...
		&& (offset == offset_in_r0
		    ...
		    || reg == PR_REG || SPECIAL_REGISTER_P (reg)))
	      {
		post_inc = gen_rtx_MEM (mode, gen_rtx_POST_INC (Pmode, r0));
...
sh_builtin_saveregs (void)
{
      ...
      if (n_intregs)
	{
	  ...
	  while (pushregs < NPARM_REGS (SImode) - 1
		 && (CALL_COOKIE_INT_REG_GET
		     (current_function_args_info.call_cookie,
		      NPARM_REGS (SImode) - pushregs) == 1))
	    {
	      current_function_args_info.call_cookie
		&= ~CALL_COOKIE_INT_REG (NPARM_REGS (SImode) - pushregs, 1);
	      pushregs++;
	    }
  ...
  if (TARGET_SHMEDIA)
    regbuf = gen_rtx_MEM (BLKmode, gen_rtx_REG (Pmode, ARG_POINTER_REGNUM));
  ...
  if (TARGET_SH4)
    {
      rtx mem;
      for (regno = NPARM_REGS (DFmode) - 2; regno >= first_floatreg;
	   regno -= 2)
	{
	  emit_insn (gen_addsi3 (fpregs, fpregs,
...
sh_va_arg (tree valist, tree type)
{
  ...
  if (!TARGET_SH5 && (TARGET_SH2E || TARGET_SH4)
      ...
      if (pass_as_float)
	{
	  ...
	  if (TYPE_MODE (type) == SCmode && TARGET_SH4
		 ...
		 tree type, int named)
{
  ...
  if (!TARGET_SH5
	  ...
	  && (!FUNCTION_ARG_SCmode_WART || (ROUND_REG (*ca, mode) & 1)))
	{
	  ...
	  && ca->free_single_fp_reg && mode == SFmode)
  ...
  if (TARGET_SH5)
	  ...
	  && ca->arg_count[(int) SH_ARG_FLOAT] < NPARM_REGS (SFmode))
	{
	  ...
	  && (!TARGET_SHCOMPACT
		  ...
		  && !SH5_WOULD_BE_PARTIAL_NREGS (*ca, mode, type, named))))
	{
			 ...
			 tree type, int named)
{
  if (ca->force_mem)
    ...
    {
      tree type2 = (ca->byref && type ? TREE_TYPE (type) : type);
      ...
      if (numregs)
	{
	  ...
	  if (TARGET_SHCOMPACT && SHCOMPACT_FORCE_ON_STACK (mode2, type2))
	    {
	      ca->call_cookie
		...
		ca->call_cookie |= CALL_COOKIE_STACKSEQ (numregs);
	    }
	}
	  ...
	  && (named || !ca->prototype_p))
	{
	  ...
	  else if (ca->arg_count[(int) SH_ARG_FLOAT] < NPARM_REGS (SFmode))
	    {
	      int numfpregs = MIN ((GET_MODE_SIZE (mode2) + 7) / 8 * 2,
				   NPARM_REGS (SFmode)
				   - ca->arg_count[(int) SH_ARG_FLOAT]);
  ...
  if (!(TARGET_SH4 || ca->renesas_abi) || PASS_IN_REG_P (*ca, mode, type))
...
initial_elimination_offset (int from, int to)
{
  ...
  if (shmedia_reserve_space_for_target_registers_p
      (regs_saved, &live_regs_mask))
    {
...
{
  if (!pragma_interrupt || TREE_CODE (node) != FUNCTION_DECL)
    return;
  ...
  *attributes =
    tree_cons (get_identifier ("interrupt_handler"), NULL, *attributes);
  ...
  if (TARGET_SHMEDIA)
    {
      ...
      if (GET_CODE (op) == CONST_INT || EXTRA_CONSTRAINT_C16 (op))
	return 1;
      else
	return 0;
    }
...
and_operand (rtx op, enum machine_mode mode)
{
  ...
  if (TARGET_SHMEDIA
      && mode == DImode
      && GET_CODE (op) == CONST_INT && CONST_OK_FOR_J16 (INTVAL (op)))
    return 1;
...
{
  if (mode != DImode || GET_MODE (op) != DImode)
    return 0;
  ...
  if (REGNO (op) > LAST_VIRTUAL_REGISTER || TARGET_REGISTER_P (REGNO (op)))
    return 1;
...
extend_reg_operand (rtx op, enum machine_mode mode)
{
  return (GET_CODE (op) == TRUNCATE
	  ? arith_operand : arith_reg_operand) (op, mode);
}
...
{
  return (GET_CODE (op) == TRUNCATE
	  ? arith_operand : arith_reg_or_0_operand) (op, mode);
}
...
{
  return (GET_CODE (op) == TRUNCATE
	  ? arith_operand : nonimmediate_operand) (op, mode);
}
...
nonpic_symbol_mentioned_p (rtx x)
{
  ...
  if (GET_CODE (x) == UNSPEC
	  ...
	  || XINT (x, 1) == UNSPEC_DTPOFF || XINT (x, 1) == UNSPEC_PLT))
    return 0;
  ...
  if (flag_expensive_optimizations && simplejump_p (branch2))
    {
      ...
      for (distance = 0, insn = NEXT_INSN (branch1);
	   insn && distance < 256; insn = PREV_INSN (insn))
	{
	  if (insn == dest)
	    return 1;
	  else
      ...
      for (distance = 0, insn = NEXT_INSN (branch1);
	   insn && distance < 256; insn = NEXT_INSN (insn))
	{
	  if (insn == dest)
	    return 1;
	  else
...
sh_adjust_cost (rtx insn, rtx link ATTRIBUTE_UNUSED, rtx dep_insn, int cost)
{
  ...
  if (TARGET_SHMEDIA)
	...
	cost = 0;
      ...
      if (get_attr_is_mac_media (insn) && get_attr_is_mac_media (dep_insn))
	cost = 1;
    }
	...
	cost--;
	  ...
	  && (type = get_attr_type (insn)) != TYPE_CALL && type != TYPE_SFUNC)
	cost--;
...
sh_initialize_trampoline (rtx tramp, rtx fnaddr, rtx cxt)
{
  if (TARGET_SHMEDIA64)
      ...
      if (cxt == virtual_stack_vars_rtx && frame_offset < 0x20000)
	{
	  ...
	  insn =
	    gen_rtx_IOR (DImode, insn, gen_int_mode (0xec000010, SImode));
	...
	= force_reg (V2HImode, (simplify_gen_subreg
      ...
      emit_insn (gen_rotrdi3_mextr (quad0, quad0,
				    GEN_INT (TARGET_LITTLE_ENDIAN ? 24 :
					     56)));
  ...
  emit_move_insn (gen_rtx_MEM (SImode, plus_constant (tramp, 4)),
		  gen_int_mode (TARGET_LITTLE_ENDIAN ? 0x0009422b :
				0x422b0009, SImode));
  emit_move_insn (gen_rtx_MEM (SImode, plus_constant (tramp, 8)), cxt);
...
sh_expand_binop_v2sf (enum rtx_code code, rtx op0, rtx op1, rtx op2)
{
  rtx sel0 = const0_rtx;
  rtx sel1 = const1_rtx;
  rtx (*fn) (rtx, rtx, rtx, rtx, rtx, rtx, rtx, rtx) = gen_binary_sf_op;
		       ...
		       enum reg_class srcclass, enum reg_class dstclass)
{
  ...
  if (mode == SImode && !TARGET_SHMEDIA && TARGET_FMOVD
      && REGCLASS_HAS_FP_REG (srcclass) && REGCLASS_HAS_FP_REG (dstclass))
    return 4;
  ...
  if ((dstclass == FPUL_REGS
       && REGCLASS_HAS_GENERAL_REG (srcclass))
      || (srcclass == FPUL_REGS && REGCLASS_HAS_GENERAL_REG (dstclass)))
    return 5;
...
{
  if (GET_CODE (op) == REG && REGNO (op) == T_REG && GET_MODE (op) == SImode)
    return 1;
