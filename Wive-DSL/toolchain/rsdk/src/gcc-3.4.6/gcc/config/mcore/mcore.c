...
static void mcore_asm_named_section (const char *, unsigned int);
#endif
...
output_stack_adjust (int direction, int size)
{
  ...
  if (direction < 0 && size > mcore_stack_increment
...
mcore_const_costs (rtx exp, enum rtx_code code)
{
  ...
  if (CONST_OK_FOR_I (val)
      || CONST_OK_FOR_M (val)
      || CONST_OK_FOR_N (val) || (code == PLUS && CONST_OK_FOR_L (val)))
    return 1;
  else if (code == AND && (CONST_OK_FOR_M (~val) || CONST_OK_FOR_N (~val)))
    return 2;
  else if (code == PLUS
	   && (CONST_OK_FOR_I (-val)
	       || CONST_OK_FOR_M (-val) || CONST_OK_FOR_N (-val)))
    return 2;
...
mcore_gen_compare_reg (enum rtx_code code)
{
  ...
  emit_insn (gen_rtx
	     (SET, VOIDmode, cc_reg, gen_rtx (code, CCmode, op0, op1)));
...
output_inline_const (enum machine_mode mode, rtx operands[])
{
  ...
  switch (trick_no)
    ...
    case 1:
      ...
      break;
    case 2:			/* not */
      sprintf (buf, "%s\n\tnot\t%s\t// %d 0x%x", load_op, dst_fmt, value,
	       value);
      break;
    case 3:			/* add */
      sprintf (buf, "%s\n\taddi\t%s,%%2\t// %d 0x%x", load_op, dst_fmt, value,
	       value);
      break;
    case 4:			/* sub */
      sprintf (buf, "%s\n\tsubi\t%s,%%2\t// %d 0x%x", load_op, dst_fmt, value,
	       value);
      break;
    case 5:			/* rsub */
      /* Never happens unless -mrsubi, see try_constant_tricks().  */
      sprintf (buf, "%s\n\trsubi\t%s,%%2\t// %d 0x%x", load_op, dst_fmt,
	       value, value);
      break;
    case 6:			/* bset */
      sprintf (buf, "%s\n\tbseti\t%s,%%P2\t// %d 0x%x", load_op, dst_fmt,
	       value, value);
      break;
    case 7:			/* bclr */
      sprintf (buf, "%s\n\tbclri\t%s,%%Q2\t// %d 0x%x", load_op, dst_fmt,
	       value, value);
      break;
    case 8:			/* rotl */
      sprintf (buf, "%s\n\trotli\t%s,%%2\t// %d 0x%x", load_op, dst_fmt,
	       value, value);
      break;
    case 9:			/* lsl */
      sprintf (buf, "%s\n\tlsli\t%s,%%2\t// %d 0x%x", load_op, dst_fmt, value,
	       value);
      break;
    case 10:			/* ixh */
      sprintf (buf, "%s\n\tixh\t%s,%s\t// %d 0x%x", load_op, dst_fmt, dst_fmt,
	       value, value);
      break;
    case 11:			/* ixw */
      sprintf (buf, "%s\n\tixw\t%s,%s\t// %d 0x%x", load_op, dst_fmt, dst_fmt,
	       value, value);
      break;
    default:
      return "";
...
mcore_arith_K_operand_not_0 (rtx op, enum machine_mode mode)
{
  ...
  if (GET_CODE (op) == CONST_INT
      && CONST_OK_FOR_K (INTVAL (op)) && INTVAL (op) != 0)
    return 1;
...
int
mcore_load_multiple_operation (rtx op,
			       enum machine_mode mode ATTRIBUTE_UNUSED)
{
...
int
mcore_store_multiple_operation (rtx op,
				enum machine_mode mode ATTRIBUTE_UNUSED)
{
  int count = XVECLEN (op, 0);
  int src_regno;
...
mcore_expand_block_move (rtx dst_mem, rtx src_mem, rtx * operands)
{
  ...
  if (GET_CODE (operands[2]) == CONST_INT)
	...
	align = 4;
	  ...
	  || (align == 2 && bytes <= 4 * 2) || (align == 1 && bytes <= 4 * 1))
	{
...
mcore_expand_prolog (void)
{
  ...
  if (TARGET_CG_DATA)
	...
	free (mcore_current_function_name);
      ...
      ASM_OUTPUT_CG_NODE (asm_out_file, mcore_current_function_name,
			  space_allocated);
	...
	ASM_OUTPUT_CG_EDGE (asm_out_file, mcore_current_function_name,
      ...
      fprintf (asm_out_file,
	       ...
	       fi.local_size, fi.outbound_size, frame_pointer_needed);
    }
...
is_cond_candidate (rtx insn)
{
  ...
  if (GET_CODE (insn) == INSN)
	...
	return COND_NO;
      ...
      if ((GET_CODE (src) == REG ||
	   (GET_CODE (src) == SUBREG &&
	    GET_CODE (SUBREG_REG (src)) == REG)) && GET_MODE (src) == SImode)
