...
{
  return (GET_CODE (op) == CONST_INT
	  && abs (INTVAL (op)) > 1 && abs (INTVAL (op)) <= 4);
}
...
pdp11_output_function_epilogue (FILE * stream, HOST_WIDE_INT size)
{
  ...
  if (frame_pointer_needed)
	...
	if (regs_ever_live[i] && !call_used_regs[i])
      ...
      for (i = FIRST_PSEUDO_REGISTER; i > 7; i--)
	{
	  if (LOAD_FPU_REG_P (i) && regs_ever_live[i] && !call_used_regs[i])
	    {
	  ...
	  if (NO_LOAD_FPU_REG_P (i)
	      && regs_ever_live[i] && !call_used_regs[i])
	    {
...
{
  enum
  { REGOP, OFFSOP, MEMOP, PUSHOP, POPOP, CNSTOP, RNDOP } optype0, optype1;
  rtx latehalf[2];
...
output_move_quad (rtx * operands)
{
  ...
  if (optype0 == REGOP && optype1 == REGOP)
    ...
    if (CPU_REG_P (REGNO (operands[0])) || CPU_REG_P (REGNO (operands[1])))
      abort ();
...
*/
	...
	cc_status.value2 = 0;
	  ...
	  && cc_status.value2 && GET_CODE (cc_status.value2) == MEM)
	cc_status.value2 = 0;
    }
...
output_block_move (rtx * operands)
{
  ...
  if (GET_CODE (operands[2]) == CONST_INT && !optimize_size)
    {
      if (INTVAL (operands[2]) < 16 && INTVAL (operands[3]) == 1)
	{
	  register int i;
