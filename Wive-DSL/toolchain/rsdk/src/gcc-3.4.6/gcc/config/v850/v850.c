      ...
      if (small_memory[i].value)
	{
	  if (!ISDIGIT (*small_memory[i].value))
	    error ("%s=%s is not numeric",
		   small_memory[i].name, small_memory[i].value);
	  else
	    {
...
static bool
v850_rtx_costs (rtx x, int code, int outer_code ATTRIBUTE_UNUSED, int *total)
{
  switch (code)
    {
    case CONST_INT:
...
ep_memory_operand (rtx op, enum machine_mode mode, int unsigned_load)
{
  ...
  switch (GET_CODE (addr))
    ...
    case PLUS:
	  ...
	  && INTVAL (op1) >= 0 && (INTVAL (op1) & mask) == 0)
	{
...
substitute_ep_register (rtx first_insn,
			rtx last_insn,
			int uses, int regno, rtx * p_r1, rtx * p_ep)
{
  ...
  if (TARGET_DEBUG)
    fprintf (stderr, "\
Saved %d bytes (%d uses of register %s) in function %s, starting as insn %d, ending at %d\n", 2 * (uses - 3), uses, reg_names[regno], IDENTIFIER_POINTER (DECL_NAME (current_function_decl)), INSN_UID (first_insn), INSN_UID (last_insn));
	      ...
	      if (GET_CODE (dest) == REG)
		{
		  ...
		  if (!use_ep)
		      ...
		      for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
			{
		      ...
		      if (max_uses > 3
			  && max_regno >= regno && max_regno < endregno)
			{
			  substitute_ep_register (regs[max_regno].first_insn,
						  regs[max_regno].last_insn,
...
compute_register_save_size (long *p_reg_saved)
{
  ...
  if (interrupt_handler)
	...
	if (regs_ever_live[i] && ((!call_used_regs[i])
      ...
      if (TARGET_PROLOG_FUNCTION && (i == 2 || ((i >= 20) && (i < 30))))
	{
	  if (i == 2)
	    {
	      size += 4;
	  ...
	  if (code >= 0)
	    {
	      ...
	      if (TARGET_DEBUG)
		fprintf (stderr, "\
Saved %d bytes via prologue function (%d vs. %d) for function %s\n", save_normal_len - save_func_len, save_normal_len, save_func_len, IDENTIFIER_POINTER (DECL_NAME (current_function_decl)));
	    }
	  else
	    save_all = NULL_RTX;
	}
  ...
  if (!save_all)
      ...
      else
	{
	  ...
	  if (num_save > 0
	      && REGNO (save_regs[num_save - 1]) == LINK_POINTER_REGNUM)
	    {
	      emit_move_insn (gen_rtx_MEM (SImode,
					   plus_constant (stack_pointer_rtx,
							  offset)),
  ...
  if (actual_fsize > init_stack_alloc)
	...
	emit_insn (gen_addsi3 (stack_pointer_rtx,
			       stack_pointer_rtx, GEN_INT (-diff)));
      else
	{
...
expand_epilogue (void)
{
  ...
  if (TARGET_PROLOG_FUNCTION
      && num_restore > 0
      && actual_fsize >= default_stack && !interrupt_handler)
    {
      ...
      if (restore_func_len < restore_normal_len)
	{
	  ...
	  if (code >= 0)
	      ...
	      if (actual_fsize)
		{
	      ...
	      if (TARGET_DEBUG)
		fprintf (stderr, "\
Saved %d bytes via epilogue function (%d vs. %d) in function %s\n", restore_normal_len - restore_func_len, restore_normal_len, restore_func_len, IDENTIFIER_POINTER (DECL_NAME (current_function_decl)));
	    }
	  else
	    restore_all = NULL_RTX;
	}
...
notice_update_cc (rtx body, rtx insn)
{
  switch (get_attr_cc (insn))
    ...
    case CC_NONE_0HIT:
	  ...
	  && reg_overlap_mentioned_p (recog_data.operand[0],
				      cc_status.value1))
	cc_status.value1 = 0;
      break;
...
v850_set_data_area (tree decl, v850_data_area data_area)
{
  ...
  switch (data_area)
    {
    case DATA_AREA_SDA:
      name = get_identifier ("sda");
      break;
    case DATA_AREA_TDA:
      name = get_identifier ("tda");
      break;
    case DATA_AREA_ZDA:
      name = get_identifier ("zda");
      break;
    default:
      return;
    }

  DECL_ATTRIBUTES (decl) = tree_cons (name, NULL, DECL_ATTRIBUTES (decl));
}

const struct attribute_spec v850_attribute_table[] = {
...
v850_encode_data_area (tree decl, rtx symbol)
{
  ...
  switch (v850_get_data_area (decl))
    {
    case DATA_AREA_ZDA:
      flags |= SYMBOL_FLAG_ZDA;
      break;
    case DATA_AREA_TDA:
      flags |= SYMBOL_FLAG_TDA;
      break;
    case DATA_AREA_SDA:
      flags |= SYMBOL_FLAG_SDA;
      break;
    default:
      abort ();
    }
...
pattern_is_ok_for_epilogue (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  for (i = 3; i < count; i++)
	...
	return 0;
	  ...
	  || GET_CODE (src) != MEM || GET_MODE (src) != SImode)
	return 0;
...
construct_restore_jr (rtx op)
{
  ...
  if (TARGET_LONG_CALLS)
	...
	sprintf (buff, "jr __return_%s_%s", reg_names[first],
		 reg_names[last]);
    }
...
v850_output_aligned_bss (FILE * file,
			 tree decl, const char *name, int size, int align)
{
  switch (v850_get_data_area (decl))
    {
    case DATA_AREA_ZDA:
	  ...
	  switch (v850_get_data_area (decl))
	    ...
	    case DATA_AREA_SDA:
	      kind = ((TREE_READONLY (decl))
		      ? GHS_SECTION_KIND_ROSDATA : GHS_SECTION_KIND_SDATA);
	      break;
	    ...
	    case DATA_AREA_ZDA:
	      kind = ((TREE_READONLY (decl))
		      ? GHS_SECTION_KIND_ROZDATA : GHS_SECTION_KIND_ZDATA);
	      break;
...
construct_dispose_instruction (rtx op)
{
  ...
  if (!TARGET_DISABLE_CALLT
      && (use_callt || stack_bytes == 0 || stack_bytes == 16))
    {
      if (use_callt)
	{
	  sprintf (buff, "callt ctoff(__callt_return_r2_r%d)",
		   (mask & (1 << 31)) ? 31 : 29);
	  return buff;
	}
      else
...
construct_prepare_instruction (rtx op)
{
  ...
  if ((!TARGET_DISABLE_CALLT)
      && (use_callt || stack_bytes == 0 || stack_bytes == -16))
    {
      if (use_callt)
	{
	  sprintf (buff, "callt ctoff(__callt_save_r2_r%d)",
		   (mask & (1 << 31)) ? 31 : 29);
	  return buff;
	}
