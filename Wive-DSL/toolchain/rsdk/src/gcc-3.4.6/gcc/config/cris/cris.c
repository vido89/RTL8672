...
cris_commutative_orth_op (rtx x, enum machine_mode mode)
{
  ...
  return (GET_MODE (x) == mode &&
	  (code == PLUS || code == IOR || code == AND || code == UMIN));
}
...
cris_additive_operand_extend_operator (rtx x, enum machine_mode mode)
{
  ...
  return (GET_MODE (x) == mode && (code == PLUS || code == MINUS));
}
...
cris_plus_or_bound_operator (rtx x, enum machine_mode mode)
{
  ...
  return (GET_MODE (x) == mode && (code == UMIN || code == PLUS));
}
...
{
  return general_operand (op, mode) || (CONSTANT_P (op) && cris_symbol (op));
}
...
cris_general_operand_or_plt_symbol (rtx op, enum machine_mode mode)
{
  return general_operand (op, mode)
    || (GET_CODE (op) == CONST
	&& GET_CODE (XEXP (op, 0)) == UNSPEC && !TARGET_AVOID_GOTPLT);
}
...
cris_print_index (rtx index, FILE * file)
{
  ...
  if (REG_P (index))
    ...
    {
      fprintf (file, "$%s.", reg_names[REGNO (XEXP (index, 0))]);

      putc (INTVAL (XEXP (index, 1)) == 2 ? 'w' : 'd', file);
    }
  else if (GET_CODE (index) == SIGN_EXTEND && GET_CODE (inner) == MEM)
    {
      rtx inner_inner = XEXP (inner, 0);

      if (GET_CODE (inner_inner) == POST_INC)
	{
	  fprintf (file, "[$%s+].", reg_names[REGNO (XEXP (inner_inner, 0))]);
    ...
    cris_operand_lossage ("unexpected index-type in cris_print_index", index);
}
...
cris_print_base (rtx base, FILE * file)
{
  if (REG_P (base))
    ...
    cris_operand_lossage ("unexpected base-type in cris_print_base", base);
}
...
cris_target_asm_function_prologue (FILE * file, HOST_WIDE_INT size)
{
  ...
  if (doing_dwarf)
    {
      ...
      int cfa_offset
	= faked_args_size
	+ (return_address_on_stack ? 4 : 0) + (frame_pointer_needed ? 4 : 0);
	      ...
	      if (last_movem_reg != -1)
		{
		  ...
		  if ((last_movem_reg + 1) * 4 + size >= 64
		  ...
		  else
		    {
		      ...
		      fprintf (file,
			       "\tsub%s " HOST_WIDE_INT_PRINT_DEC ",$sp\n",
			       ADDITIVE_SIZE_MODIFIER ((last_movem_reg +
							1) * 4 + size),
			       (last_movem_reg + 1) * 4 + size);
  ...
  if (last_movem_reg != -1)
	...
	fprintf (file, "\tmovem $%s,[$sp=$sp-" HOST_WIDE_INT_PRINT_DEC "]\n",
		 reg_names[last_movem_reg], (last_movem_reg + 1) * 4 + size);
      else
	{
	...
	fprintf (file, "; frame " HOST_WIDE_INT_PRINT_DEC
		 ...
		 size,
		 last_movem_reg + 1,
		 (last_movem_reg + 1) * 4, current_function_args_size);
      ...
      if (cfoa_size)
	{
	  ...
	  fprintf (file, "\tSub%s %d,$sp\n",
		   ADDITIVE_SIZE_MODIFIER (cfoa_size), cfoa_size);
	  framesize += cfoa_size;
	}
    }
  else if ((size + cfoa_size) > 0)
    {
      ...
      fprintf (file, "\tSub%s " HOST_WIDE_INT_PRINT_DEC ",$sp\n",
	       ADDITIVE_SIZE_MODIFIER (size + cfoa_size), cfoa_size + size);
...
cris_target_asm_function_epilogue (FILE * file, HOST_WIDE_INT size)
{
  ...
  for (regno = FIRST_PSEUDO_REGISTER - 1; regno > last_movem_reg; regno--)
    if ((((regs_ever_live[regno]
	   && !call_used_regs[regno])
	  || (regno == (int) PIC_OFFSET_TABLE_REGNUM
	      && (current_function_uses_pic_offset_table
...
cris_print_operand (FILE * file, rtx x, int code)
{
  ...
  switch (code)
	...
	case MEM:
	  ...
	  {
	    rtx adj_mem = operand;
	    int size = GET_MODE_BITSIZE (GET_MODE (operand)) / BITS_PER_UNIT;
...
{
  int fp_sp_offset = cris_initial_frame_pointer_offset ();
  ...
  if (fromreg == ARG_POINTER_REGNUM && toreg == FRAME_POINTER_REGNUM)
    return ap_fp_offset;

  /* Between the frame pointer and the stack are only "normal" stack
     variables and saved registers.  */
  if (fromreg == FRAME_POINTER_REGNUM && toreg == STACK_POINTER_REGNUM)
    return fp_sp_offset;

  /* We need to balance out the frame pointer here.  */
  if (fromreg == ARG_POINTER_REGNUM && toreg == STACK_POINTER_REGNUM)
  ...
  if (GET_CODE (x) == PLUS)
    {
      ...
      if ((GET_CODE (tem1) == MULT && BIAP_INDEX_P (tem1)) || REG_P (tem1))
	return 2 / 2;
			  ...
			  int multop, int other_op)
{
  ...
  else if (code == MULT || (code == PLUS && BASE_P (val_rtx)))
    {
...
cris_override_options (void)
{
  ...
  if (cris_tune_str)
    {
      int cris_tune = (*cris_tune_str == 'v' ? atoi (cris_tune_str + 1) : -1);
  ...
  if (!integer_zerop (rounded_size))
    {
      ...
      t = build (MODIFY_EXPR, TREE_TYPE (valist), valist,
		 build (PLUS_EXPR, TREE_TYPE (valist), valist, passed_size));
...
cris_split_movdx (rtx * operands)
{
  ...
  if (GET_CODE (dest) == REG)
      ...
      if (GET_CODE (src) == REG)
	{
	  ...
	  emit_insn (gen_rtx_SET (VOIDmode,
				  ...
				  operand_subword (src, reverse, TRUE,
						   mode)));

	  emit_insn (gen_rtx_SET (VOIDmode,
				  operand_subword (dest, !reverse, TRUE,
						   mode),
				  operand_subword (src, !reverse, TRUE,
						   mode)));
	}
		...
		fatal_insn ("unexpected side-effects in address", addr);
	      ...
	      emit_insn (gen_rtx_SET
			 (VOIDmode,
			  ...
			  change_address
			  (src, SImode,
			   plus_constant (addr, reverse * UNITS_PER_WORD))));
	      emit_insn (gen_rtx_SET
			 (VOIDmode,
			  operand_subword (dest, !reverse, TRUE, mode),
			  change_address
  ...
  /* Reg-to-mem copy or clear mem.  */
	   ...
	   && (GET_CODE (src) == REG
	       || src == const0_rtx || src == CONST0_RTX (DFmode)))
    {
