			...
			rtx cur_insn)
{
  ...
  if (TARGET_DEBUG_C_MODE
      ...
      || cur_insn == 0
      || next_insn == 0
      || GET_CODE (next_insn) == CODE_LABEL || (set_reg = operands[0]) == 0)
    {
...
iq2000_va_arg (tree valist, tree type)
{
  ...
  indirect = function_arg_pass_by_reference (NULL, TYPE_MODE (type), type, 0);
  if (indirect)
    {
...

  {
    ...
    if (!indirect && TYPE_ALIGN (type) > (unsigned) BITS_PER_WORD)
      {
  ...
  if (TREE_CODE (type) == REAL_TYPE)
	...
	emit_move_insn (addr_rtx, r);
      ...
      t = build (BIT_AND_EXPR, TREE_TYPE (ovfl), ovfl, build_int_2 (7, 0));
      ...
      t = build (POSTINCREMENT_EXPR, TREE_TYPE (ovfl), ovfl, size_int (8));
      r = expand_expr (t, addr_rtx, Pmode, EXPAND_NORMAL);
      if (r != addr_rtx)
	emit_move_insn (addr_rtx, r);
  ...
  else
    {
      ...
      if (TREE_CODE (type) == INTEGER_TYPE && TYPE_PRECISION (type) == 64)
	{
...
override_options (void)
{
  ...
  if (iq2000_arch_string == 0
	...
	{
	  error ("The compiler does not support -march=%s.",
		 iq2000_arch_string);
	  iq2000_arch_string = "default";
	}
    }
...
iq2000_debugger_offset (rtx addr, HOST_WIDE_INT offset)
{
  ...
  if (reg == stack_pointer_rtx || reg == frame_pointer_rtx
	...
	? compute_frame_size (get_frame_size ()) : cfun->machine->total_size;
  ...
  if (dslots_number_nops > 0)
    {
      ...
      if (length == 0
	  || (iq2000_load_reg != 0
	      && reg_mentioned_p (iq2000_load_reg, pattern))
	  || (iq2000_load_reg2 != 0
	      && reg_mentioned_p (iq2000_load_reg2, pattern))
	  || (iq2000_load_reg3 != 0
	      && reg_mentioned_p (iq2000_load_reg3, pattern))
	  || (iq2000_load_reg4 != 0
...
iq2000_initial_elimination_offset (int from, int to ATTRIBUTE_UNUSED)
{
  ...
  if ((from) == FRAME_POINTER_REGNUM)
	...
	(offset) = cfun->machine->gp_sp_offset
...
save_restore_insns (int store_p)
{
  ...
  end_offset
    = gp_offset - (cfun->machine->gp_reg_size - GET_MODE_SIZE (gpr_mode));
  ...
  if (store_args_on_stack
      && ((TYPE_ARG_TYPES (fntype) != 0
	   && (TREE_VALUE (tree_last (TYPE_ARG_TYPES (fntype)))
	       != void_type_node)) || last_arg_is_vararg_marker))
    {
...
char *
iq2000_output_conditional_branch (rtx insn, rtx * operands,
				  ...
				  int inverted_p, int length)
{
  ...
  switch (length)
	...
	if (float_p)
	  sprintf (c, "b%s\t%%Z2%s",
		   inverted_p ? comp : inverted_comp, target);
	else
	  sprintf (c, "b%s%s\t%s%s,%s",
		   inverted_p ? comp : inverted_comp,
		   need_z_p ? "z" : "", op1, op2, target);
	strcat (c, "\n\tnop\n\tj\t%1");
	if (length == 16)
	  ...
	  if ((cum).arg_words < MAX_ARGS_IN_REGISTERS - iq2000_off)
	    {
	      ...
	      ptr = plus_constant (virtual_incoming_args_rtx,
				   -(iq2000_save_gp_regs * UNITS_PER_WORD));
	      mem = gen_rtx_MEM (BLKmode, ptr);
	      move_block_from_reg
		((cum).arg_words + GP_ARG_FIRST + iq2000_off,
		 mem, iq2000_save_gp_regs);
	    }
	}
    }
}
...
print_operand (FILE * file, rtx op, int letter)
{
  ...
  if (letter == 'C')
    ...
    {
      int value;
      if (code != CONST_INT || (value = exact_log2 (INTVAL (op))) < 0)
  ...
  switch (code)
	...
	*total = COSTS_N_INSNS (2 * num_words);
    ...
    case LSHIFTRT:
      if (mode == DImode)
	*total =
	  COSTS_N_INSNS ((GET_CODE (XEXP (x, 1)) == CONST_INT) ? 4 : 12);
      else
