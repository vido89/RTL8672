...
static rtx frv_alloc_temp_reg (frv_tmp_reg_t *,
			       enum reg_class, enum machine_mode, int, int);
...
static void frv_registers_update (rtx, unsigned char[], int[], int *, int);
...
const_small_data_p (rtx x)
{
  ...
  x1 = XEXP (XEXP (x, 0), 1);
  if (GET_CODE (x1) != CONST_INT || !IN_RANGE_P (INTVAL (x1), -2048, 2047))
    return FALSE;
...
plus_small_data_p (rtx op0, rtx op1)
{
  if (GET_MODE (op0) == SImode
      && GET_CODE (op0) == REG && REGNO (op0) == SDA_BASE_REG)
    {
...
frv_override_options (void)
{
  ...
  if (write_symbols == DWARF_DEBUG && flag_pic)
    error
      ("-fpic and -gdwarf are incompatible (-fpic and -g/-gdwarf-2 are fine)");
...
frv_stack_info (void)
{
  ...
  if (cfun->stdarg)
  ...
  else
    {
      ...
      for (cur_arg = DECL_ARGUMENTS (fndecl); cur_arg != (tree) 0;
	   cur_arg = next_arg)
	{
	  ...
	  for (regno = first; regno <= last; regno++)
	    {
		  ...
		  || (current_function_calls_eh_return
		      && (regno >= FIRST_EH_REGNUM
			  && regno <= LAST_EH_REGNUM)) || (flag_pic
							   && cfun->
  ...
  info_ptr->regs_size = ADDR_ALIGN (info_ptr->regs_size_2words
				    + info_ptr->regs_size_1word, alignment);
  ...
  info_ptr->total_size
    = (ADDR_ALIGN (info_ptr->parameter_size
       ...
       + ADDR_ALIGN (info_ptr->pretend_size
		     + info_ptr->stdarg_size, 2 * UNITS_PER_WORD));
...
{
  rtx insn = get_insns ();
  while (insn != NULL && !(GET_CODE (insn) == JUMP_INSN
...
static rtx
frv_alloc_temp_reg (frv_tmp_reg_t * info,	/* which registers are available */
...
frv_expand_prologue (void)
{
  ...
  /* Allocate the stack space.  */
  {
    ...
    frv_frame_insn (gen_stack_adjust (sp, sp, asm_offset),
		    gen_rtx_SET (Pmode,
				 sp, gen_rtx_PLUS (Pmode, sp, dwarf_offset)));
  }
...
frv_initial_elimination_offset (int from, int to)
{
  ...
  if (to == STACK_POINTER_REGNUM && from == ARG_POINTER_REGNUM)
    ...
    ret = (info->total_size
	   - info->reg_offset[FRAME_POINTER_REGNUM] - info->pretend_size);
  ...
  for (; bytes > 0; (bytes -= clear_bytes), (offset += clear_bytes))
    {
      /* Calculate the correct offset for src/dest.  */
      dest_addr = ((offset == 0)
		   ? dest_reg : plus_constant (dest_reg, offset));
...
frv_print_operand_jump_hint (rtx insn)
{
  ...
  enum
  { UNKNOWN, BACKWARD, FORWARD } jump_type = UNKNOWN;
      ...
      if (labelref)
	{
	  ...
	  jump_type =
	    (insn_current_address >
	     INSN_ADDRESSES (INSN_UID (label)) ? BACKWARD : FORWARD);
	}

      note = find_reg_note (insn, REG_BR_PROB, 0);
      if (!note)
	ret =
	  ((jump_type == BACKWARD) ? FRV_JUMP_LIKELY : FRV_JUMP_NOT_LIKELY);
  ...
  if (TARGET_DEBUG)
	...
	default:
	case UNKNOWN:
	  direction = "unknown jump direction";
	  break;
	case BACKWARD:
...
frv_init_cumulative_args (CUMULATIVE_ARGS * cum,
			  tree fntype, rtx libname, tree fndecl, int incoming)
{
		  ...
		  int named, int incoming ATTRIBUTE_UNUSED)
{
  ...
  if (TARGET_DEBUG_ARG)
    fprintf (stderr,
	     ...
	     arg_num, GET_MODE_NAME (mode), named, GET_MODE_SIZE (mode),
	     debstr);
...
frv_legitimate_address_p (enum machine_mode mode,
			  rtx x, int strict_p, int condexec_p)
{
  rtx x0, x1;
  int ret = 0;
  HOST_WIDE_INT value;
	...
	case CONST_INT:
	  ...
	  if (condexec_p)
	  ...
	  else
	    {
	      ...
	      if (ret && GET_MODE_SIZE (mode) > UNITS_PER_WORD)
		ret =
		  IN_RANGE_P (value + GET_MODE_SIZE (mode) - 1, -2048, 2047);
	    }
	  break;

	case SYMBOL_REF:
	  if (!condexec_p
	      && regno0 == SDA_BASE_REG && SYMBOL_REF_SMALL_P (x1))
	    ret = TRUE;
	  break;

	case CONST:
	  if (!condexec_p && regno0 == SDA_BASE_REG
	      && const_small_data_p (x1))
	    ret = TRUE;
	  break;
...
frv_emit_movsi (rtx dest, rtx src)
{
  ...
  if (!reload_in_progress
      && !reload_completed
      && !register_operand (dest, SImode) && (!reg_or_0_operand (src, SImode)
								 ...
								 LAST_VIRTUAL_REGISTER))))
    {
      emit_insn (gen_rtx_SET
		 (VOIDmode, dest, copy_to_mode_reg (SImode, src)));
      return TRUE;
    }
	  ...
	  if (GET_CODE (src) == REG)
		...
		return "setlos %1, %0";
	      ...
	      return "#";
		   ...
		   || GET_CODE (src) == LABEL_REF || GET_CODE (src) == CONST)
	    {
...
rtx
frv_split_scc (rtx dest, rtx test, rtx cc_reg, rtx cr_reg,
	       HOST_WIDE_INT value)
{
  rtx ret;
  ...
  if (GET_CODE (src1) == CONST_INT && GET_CODE (src2) == CONST_INT)
	       ...
	       && IN_RANGE_P (value2 - value1, -2048, 2047))
	{
	  rtx dest_si = ((GET_MODE (dest) == SImode)
			 ? dest : gen_rtx_SUBREG (SImode, dest, 0));
...
frv_split_minmax (rtx operands[])
{
  ...
  emit_insn (gen_rtx_SET (VOIDmode,
			  cc_reg,
			  gen_rtx_COMPARE (GET_MODE (cc_reg), src1, src2)));
...
frv_split_abs (rtx operands[])
{
  ...
  emit_insn (gen_rtx_SET (VOIDmode,
			  cc_reg, gen_rtx_COMPARE (CCmode, src, const0_rtx)));

  /* Set the appropriate CCR bit.  */
  emit_insn (gen_rtx_SET (VOIDmode,
			  cr_reg,
			  gen_rtx_fmt_ee (LT, CC_CCRmode, cc_reg,
					  const0_rtx)));
  ...
  if (TARGET_DEBUG_COND_EXEC)
    {
      fprintf (stderr,
	       ...
	       (before_p) ? "before" : "after", (int) INSN_UID (insn));
...
frv_ifcvt_modify_tests (ce_if_block_t * ce_info, rtx * p_true, rtx * p_false)
{
  ...
  for (j = 0; j < num_bb; j++)
	...
	fprintf (rtl_dump_file, "Scanning %s block %d, start %d, end %d\n",
		 (bb[j] ==
		  else_bb) ? "else" : ((bb[j] == then_bb) ? "then" : "test"),
		 (int) bb[j]->index, (int) INSN_UID (BB_HEAD (bb[j])),
  ...
  check_insn = gen_rtx_SET (VOIDmode, cr,
			    gen_rtx_fmt_ee (code, CC_CCRmode, cc,
					    const0_rtx));
...
frv_ifcvt_modify_multiple_tests (ce_if_block_t * ce_info,
				 basic_block bb, rtx * p_true, rtx * p_false)
{
...
rtx
frv_ifcvt_modify_insn (ce_if_block_t * ce_info, rtx pattern, rtx insn)
{
	  ...
	  if (ce_info->pass > 1
	      ...
	      && TEST_HARD_REG_BIT (frv_ifcvt.nested_cc_ok_rewrite, regno))
	    {
	      src = gen_rtx_fmt_ee (GET_CODE (src),
				    CC_CCRmode,
				    frv_ifcvt.nested_cc_reg, XEXP (src, 1));
	    }
  ...
  if (DECL_BIT_FIELD (field) && !DECL_ARTIFICIAL (field))
    {
      ...
      for (cur = TYPE_FIELDS (parent); cur && cur != field;
	   cur = TREE_CHAIN (cur))
	{
  ...
  if (flag_pic && size == UNITS_PER_WORD)
    {
	  ...
	  || GET_CODE (value) == SYMBOL_REF || GET_CODE (value) == LABEL_REF)
	{
	  if (aligned_p)
	    {
...
frv_registers_update (rtx x,
		      ...
		      int modified[], int *p_num_mod, int flag)
{
  int regno, reg_max;
  rtx reg;
  rtx cond;
  ...
  switch (GET_CODE (x))
	  ...
	  && (flag & (REGSTATE_MODIFIED | REGSTATE_IF_EITHER)) == 0)
	{
	  ...
	  flag |= ((REGNO (XEXP (cond, 0)) - CR_FIRST)
		   | ((GET_CODE (cond) == NE)
		      ? REGSTATE_IF_TRUE : REGSTATE_IF_FALSE));
	  ...
	  if (rs & REGSTATE_MODIFIED)
		...
		return TRUE;
	      ...
	      if (((rs_if == REGSTATE_IF_TRUE && flag_if == REGSTATE_IF_FALSE)
		   || (rs_if == REGSTATE_IF_FALSE
		       && flag_if == REGSTATE_IF_TRUE))
...
frv_pack_insns (void)
{
  ...
  if (!optimize
      ...
      || TARGET_NO_VLIW_BRANCH || frv_issue_rate () == 1)
    return;
  ...
  for (insn = get_insns (); insn != NULL_RTX; insn = NEXT_INSN (insn))
	...
	CLEAR_VLIW_START (insn);
      ...
      for (link = REG_NOTES (insn); link != NULL_RTX; link = XEXP (link, 1))
	{
	  rtx reg = XEXP (link, 0);
...
frv_check_constant_argument (enum insn_code icode, int opnum, rtx opval)
{
  ...
  if (!(*insn_data[icode].operand[opnum].predicate) (opval, VOIDmode))
    {
      error ("constant argument out of range for `%s'",
