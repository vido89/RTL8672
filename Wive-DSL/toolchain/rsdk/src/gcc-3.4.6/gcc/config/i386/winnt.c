			   ...
			   bool * no_add_attrs)
{
  ...
  /* Report error on dllimport ambiguities seen now before they cause
  ...
  else if (is_attribute_p ("dllimport", name))
    {
	  ...
	  && !DECL_INLINE (node))
	{
	  error ("%Jfunction `%D' definition is marked dllimport.", node,
		 node);
	  *no_add_attrs = true;
	}
...
i386_pe_dllimport_p (tree decl)
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL && TARGET_NOP_FUN_DLLIMPORT)
    return 0;
...
i386_pe_encode_section_info (tree decl, rtx rtl, int first)
{
  ...
  /* It might be that DECL has already been marked as dllimport, but a
     ...
     that. Ditto for the DECL_NON_ADDR_CONST_P flag.  */
	   ...
	   &&
	   i386_pe_dllimport_name_p (XSTR
				     (XEXP (XEXP (DECL_RTL (decl), 0), 0),
				      0)))
    {
...
{
  if (strncmp (str, DLL_IMPORT_PREFIX, strlen (DLL_IMPORT_PREFIX)) == 0)
    str += strlen (DLL_IMPORT_PREFIX);
  else if (strncmp (str, DLL_EXPORT_PREFIX, strlen (DLL_EXPORT_PREFIX)) == 0)
...
void
i386_pe_output_labelref (FILE * stream, const char *name)
{
  if (strncmp (name, DLL_IMPORT_PREFIX, strlen (DLL_IMPORT_PREFIX)) == 0)
    /* A dll import */
    {
      if (name[strlen (DLL_IMPORT_PREFIX)] == FASTCALL_PREFIX)
	/* A dllimport fastcall symbol.  */
	{
	  fprintf (stream, "__imp_%s", i386_pe_strip_name_encoding (name));
	}
      else
	/* A dllimport non-fastcall symbol.  */
	{
	  fprintf (stream, "__imp__%s", i386_pe_strip_name_encoding (name));
	}
    }
  else if ((name[0] == FASTCALL_PREFIX)
	   || (strncmp (name, DLL_EXPORT_PREFIX, strlen (DLL_EXPORT_PREFIX)
			== 0
			&& name[strlen (DLL_EXPORT_PREFIX)] ==
			FASTCALL_PREFIX)))
    /* A fastcall symbol.  */
    {
      fprintf (stream, "%s", i386_pe_strip_name_encoding (name));
    }
  else
    /* Everything else.  */
    {
...
i386_pe_declare_function_type (FILE * file, const char *name, int public)
{
  ...
  fprintf (file, ";\t.scl\t%d;\t.type\t%d;\t.endef\n",
	   public ? (int) C_EXT : (int) C_STAT, (int) DT_FCN << N_BTSHFT);
}
