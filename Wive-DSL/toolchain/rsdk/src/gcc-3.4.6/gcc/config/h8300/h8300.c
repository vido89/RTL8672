...
static tree h8300_handle_eightbit_data_attribute (tree *, tree, tree, int,
						  bool *);
static tree h8300_handle_tiny_data_attribute (tree *, tree, tree, int,
					      bool *);
...
h8300_can_use_return_insn_p (void)
{
  return (reload_completed
	  ...
	  && get_frame_size () == 0 && compute_saved_regs () == 0);
}
...
two_insn_adds_subs_operand (rtx op, enum machine_mode mode)
{
  if (GET_CODE (op) == CONST_INT)
      ...
      else
	{
	  ...
	  if (mode == HImode && (value == 2 + 1 || value == 2 + 2))
	    return 1;
	}
    }
...
h8300_shift_costs (rtx x)
{
  ...
  if (GET_MODE (x) != QImode
      && GET_MODE (x) != HImode && GET_MODE (x) != SImode)
    return 100;
...
{
  return (GET_CODE (x) == CONST_INT && abs (INTVAL (x)) > 2);
}
...
{
  return (GET_CODE (x) == CONST_INT && abs (INTVAL (x)) >= 8);
}
...
{
  return (GET_CODE (x) == CONST_INT && (INTVAL (x) & 0xff) == INTVAL (x));
}
...
{
  return (GET_CODE (x) == CONST_INT && (INTVAL (x) & 0xffff) == INTVAL (x));
}
...
incdec_operand (rtx x, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  return (GET_CODE (x) == CONST_INT
	  && (CONST_OK_FOR_M (INTVAL (x)) || CONST_OK_FOR_O (INTVAL (x))));
}
...
bit_operator (rtx x, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  return (code == XOR || code == AND || code == IOR);
}

...
output_logical_op (enum machine_mode mode, rtx * operands)
{
  ...
  switch (mode)
    ...
    case HImode:
      ...
      if ((TARGET_H8300H || TARGET_H8300S) && b0 != 0 && b1 != 0)
	{
      ...
      else
	{
	  ...
	  if (w0 == 0xffff && (TARGET_H8300 ? (code == AND) : (code != IOR)))
	    output_asm_insn ((code == AND)
			     ? "sub.w\t%f0,%f0" : "not.w\t%f0", operands);
	  else if ((TARGET_H8300H || TARGET_H8300S) && (b0 != 0) && (b1 != 0))
	    {
	  ...
	  if ((w1 == 0xffff)
	    ...
	    output_asm_insn ((code == AND)
			     ? "sub.w\t%e0,%e0" : "not.w\t%e0", operands);
	  else if ((TARGET_H8300H || TARGET_H8300S)
		   && code == IOR && w1 == 0xffff && (w0 & 0x8000) != 0)
	    {
...
compute_logical_op_length (enum machine_mode mode, rtx * operands)
{
  ...
  switch (mode)
      ...
      else
	{
	  ...
	  if (w1 == 0xffff && (TARGET_H8300 ? (code == AND) : (code != IOR)))
	    {
	      length += 2;
	    }
	  else if ((TARGET_H8300H || TARGET_H8300S)
	       ...
	       unsigned int count, struct shift_info *info)
{
  ...
  switch (shift_mode)
      ...
      if (count == 7)
	{
      ...
      else if ((8 <= count && count <= 13) || (TARGET_H8300S && count == 14))
	{
...
fix_bit_operand (rtx * operands, int what, enum rtx_code type)
{
  ...
  operands[1] = force_reg (QImode, operands[1]);
  {
    ...
    emit_insn (gen_rtx_SET (VOIDmode, res,
			    gen_rtx (type, QImode, operands[1],
				     operands[2])));
...
h8300_encode_section_info (tree decl, rtx rtl, int first)
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL && h8300_funcvec_function_p (decl))
...
#ifndef OBJECT_FORMAT_ELF
static void
h8300_asm_named_section (const char *name,
