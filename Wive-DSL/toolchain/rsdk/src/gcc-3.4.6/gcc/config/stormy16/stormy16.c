...
{
  return (GET_CODE (x) == CONST_INT ? 2 : GET_CODE (x) == PLUS ? 7 : 5);
}
...
xstormy16_emit_cbranch (enum rtx_code code, rtx loc)
{
  ...
  else if (mode == SImode && (code == NE || code == EQ) && op1 != const0_rtx)
    {
      rtx lab = NULL_RTX;
			     ...
			     rtx insn)
{
  ...
  switch (code)
	...
	sprintf (prevop, "or %s,%s", reg_names[regnum],
		 reg_names[regnum + 1]);
      }
      break;
...
{
  if (class == GENERAL_REGS && GET_CODE (x) == MEM)
    return EIGHT_REGS;
				...
				rtx x, int strict)
{
  ...
  if (GET_CODE (x) == PLUS && LEGITIMATE_ADDRESS_INTEGER_P (XEXP (x, 1), 0))
    x = XEXP (x, 0);

  if ((GET_CODE (x) == PRE_MODIFY
       && GET_CODE (XEXP (XEXP (x, 1), 1)) == CONST_INT)
      || GET_CODE (x) == POST_INC || GET_CODE (x) == PRE_DEC)
    x = XEXP (x, 0);
...
xstormy16_extra_constraint_p (rtx x, int c)
{
  switch (c)
    ...
    case 'Z':
      return (GET_CODE (x) == CONST_INT && (INTVAL (x) == 0));
...
xstormy16_split_move (enum machine_mode mode, rtx dest, rtx src)
{
  ...
  if (!reload_completed
      ...
      || !nonimmediate_operand (dest, mode) || !general_operand (src, mode))
    abort ();
  ...
  if (mem_operand == NULL_RTX)
    ...
    direction = -1;
  else if (GET_CODE (src) == MEM && reg_overlap_mentioned_p (dest, src))
    {
      int regno;
  ...
  for (i = direction < 0 ? num_words - 1 : 0; i != end; i += direction)
	...
	w_src =
	  simplify_gen_subreg (word_mode, src, mode, i * UNITS_PER_WORD);
      if (src_volatile)
	...
	MEM_VOLATILE_P (w_dest) = 1;
      ...
      if (GET_CODE (w_src) == SUBREG || GET_CODE (w_dest) == SUBREG)
	abort ();
...
xstormy16_expand_move (enum machine_mode mode, rtx dest, rtx src)
{
  if ((GET_CODE (dest) == MEM) && (GET_CODE (XEXP (dest, 0)) == PRE_MODIFY))
    {
      ...
      emit_insn (gen_rtx_PARALLEL (VOIDmode, gen_rtvec (2, set, clobber)));
	   ...
	   && (GET_CODE (XEXP (src, 0)) == PRE_MODIFY))
    {
      rtx pmv = XEXP (src, 0);
  ...
  if (reload_completed && mode != HImode && mode != QImode)
    {
...
xstormy16_compute_stack_layout (void)
{
  ...
  layout.frame_size = (layout.locals_size
		       + layout.register_save_size + layout.stdarg_save_size);
...
xstormy16_expand_prologue (void)
{
  ...
  if (layout.stdarg_save_size)
    for (regno = FIRST_ARGUMENT_REGISTER;
	 regno < FIRST_ARGUMENT_REGISTER + NUM_ARGUMENT_REGISTERS; regno++)
      {
	rtx dwarf;
	...
	XVECEXP (dwarf, 0, 1) =
	  gen_rtx_SET (Pmode, stack_pointer_rtx,
		       plus_constant (stack_pointer_rtx,
				      GET_MODE_SIZE (Pmode)));
	REG_NOTES (insn) =
	  gen_rtx_EXPR_LIST (REG_FRAME_RELATED_EXPR, dwarf, REG_NOTES (insn));
...
xstormy16_build_builtin_va_list (void)
{
  ...
  f_1 = build_decl (FIELD_DECL, get_identifier ("base"), ptr_type_node);
  f_2 = build_decl (FIELD_DECL, get_identifier ("count"), unsigned_type_node);
...
xstormy16_expand_builtin_va_arg (tree valist, tree type)
{
  ...
  if (!must_stack)
    {
      count_plus_size =
	build (PLUS_EXPR, TREE_TYPE (count), count, size_tree);
      count_plus_size_rtx =
	expand_expr (count_plus_size, NULL_RTX, HImode, EXPAND_NORMAL);
      emit_cmp_and_jump_insns (count_plus_size_rtx,
			       GEN_INT (size_of_reg_args), GTU, const1_rtx,
			       HImode, 1, lab_fromstack);
...
xstormy16_expand_call (rtx retval, rtx dest, rtx counter)
{
  ...
  if (!CONSTANT_P (dest) && GET_CODE (dest) != REG)
  ...
  for (i = 0; i < num_words; i++)
    {
      ...
      w_src0 = simplify_gen_subreg (word_mode, src0, mode,
				    i * UNITS_PER_WORD);
      w_src1 =
	simplify_gen_subreg (word_mode, src1, mode, i * UNITS_PER_WORD);
      w_dest =
	simplify_gen_subreg (word_mode, dest, mode, i * UNITS_PER_WORD);
			...
			rtx x, rtx size_r, rtx temp)
{
  ...
  if (GET_CODE (size_r) != CONST_INT || GET_CODE (x) != REG || mode != SImode)
    abort ();
  ...
  for (i = 0; s16builtins[i].name; i++)
      ...
      for (a = strlen (s16builtins[i].arg_types) - 1; a >= 0; a--)
	{
	  switch (s16builtins[i].arg_types[a])
	    {
	    case 's':
	      arg = short_integer_type_node;
	      break;
	    case 'S':
	      arg = short_unsigned_type_node;
	      break;
	    case 'l':
	      arg = long_integer_type_node;
	      break;
	    case 'L':
	      arg = long_unsigned_type_node;
	      break;
	    default:
	      abort ();
	    }
