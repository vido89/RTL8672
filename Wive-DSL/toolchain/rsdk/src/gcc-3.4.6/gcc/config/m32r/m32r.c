			     ...
			     bool * no_add_attrs)
{
  ...
  if (arg != small_ident1
      && arg != small_ident2
      && arg != medium_ident1
      && arg != medium_ident2 && arg != large_ident1 && arg != large_ident2)
    {
...
addr24_operand (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  if (TARGET_ADDR24
      && (CONSTANT_POOL_ADDRESS_P (sym) || LIT_NAME_P (XSTR (sym, 0))))
    return 1;
...
addr32_operand (rtx op, enum machine_mode mode)
{
  ...
  if (GET_CODE (op) == SYMBOL_REF)
    sym = op;
	   ...
	   && GET_CODE (XEXP (op, 0)) == PLUS
	   && GET_CODE (XEXP (XEXP (op, 0), 0)) == SYMBOL_REF
	   && GET_CODE (XEXP (XEXP (op, 0), 1)) == CONST_INT && !flag_pic)
    sym = XEXP (XEXP (op, 0), 0);
  else
    return 0;

  return (!addr24_operand (sym, mode) && !small_data_operand (sym, mode));
}
...
two_insn_const_operand (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  if (INT16_P (INTVAL (op))
      || UINT24_P (INTVAL (op)) || UPPER16_P (INTVAL (op)))
    return 0;
  return 1;
}
...
easy_di_const (rtx op)
{
  ...
  if (high >= -128 && high <= 127 && low >= -128 && low <= 127)
    return 1;
  return 0;
}
...
gen_compare (enum rtx_code code, rtx x, rtx y, int need_compare)
{
  ...
  if (need_compare)
	...
	{
	case EQ:
	  if (GET_CODE (y) == CONST_INT && CMP_INT16_P (INTVAL (y))	/* Reg equal to small const.  */
	      && y != const0_rtx)
	    {
  ...
  else
    {
      ...
      if (compare_code == EQ && register_operand (y, SImode))
	return gen_rtx (code, CCmode, x, y);

      /* Reg/zero signed comparison.  */
      if ((compare_code == EQ || compare_code == LT) && y == const0_rtx)
	return gen_rtx (code, CCmode, x, y);
	  ...
	  && GET_CODE (y) == CONST_INT && CMP_INT16_P (INTVAL (y)))
	{
      ...
      if (compare_code == EQ && CONSTANT_P (y))
	{
	  rtx tmp = force_reg (GET_MODE (x), y);
...
{
  if (m32r_is_insn (insn) && get_attr_insn_size (insn) != INSN_SIZE_SHORT)
    priority <<= 3;
		    ...
		    int *n_readyp, int clock ATTRIBUTE_UNUSED)
{
  ...
  if (stream)
    fprintf (stream,
	     ...
	     n_ready, (m32r_sched_odd_word_p) ? "odd" : "even");
...
m32r_variable_issue (FILE * stream, int verbose, rtx insn, int how_many)
{
  ...
  if (verbose > 7 && stream)
    fprintf (stream,
	     ...
	     short_p ? "short" : "long",
	     INSN_UID (insn), orig_odd_word_p ? "odd" : "even", how_many);
...
m32r_compute_function_type (tree decl)
{
  ...
  fn_type =
    (lookup_attribute ("interrupt", DECL_ATTRIBUTES (current_function_decl))
     != NULL_TREE ? M32R_FUNCTION_INTERRUPT : M32R_FUNCTION_NORMAL);
  ...
  if (current_frame_info.pretend_size != 0)
    {
      ...
      emit_insn (gen_addsi3 (stack_pointer_rtx,
			     stack_pointer_rtx, GEN_INT (-pretend_size)));
    }
...
m32r_output_function_prologue (FILE * file, HOST_WIDE_INT size)
{
  ...
  fprintf (file,
	   ...
	   current_frame_info.args_size, current_frame_info.extra_size);
}

...
static void
m32r_output_function_epilogue (FILE * file,
...
m32r_legitimize_pic_address (rtx orig, rtx reg)
{
  ...
  if (GET_CODE (orig) == SYMBOL_REF || GET_CODE (orig) == LABEL_REF)
      ...
      if (GET_CODE (XEXP (orig, 0)) == PLUS)
	{
	  ...
	  if (base == reg)
	    offset =
	      m32r_legitimize_pic_address (XEXP (XEXP (orig, 0), 1),
					   NULL_RTX);
	  else
	    offset =
	      m32r_legitimize_pic_address (XEXP (XEXP (orig, 0), 1), reg);
	}
      else
	return orig;
...
m32r_print_operand (FILE * file, rtx x, int code)
{
  ...
  switch (code)
	...
	real_to_decimal (str, CONST_DOUBLE_REAL_VALUE (x), sizeof (str), 0,
			 1);
	fprintf (file, "%s", str);
	return;
      }
	...
	case CONST_DOUBLE:
	  {
	    ...
	    fprintf (file, HOST_WIDE_INT_PRINT_HEX,
		     (code == 'B'
		      ? INTVAL (x) & 0xffff : (INTVAL (x) >> 16) & 0xffff));
	  }
	  return;
	case CONST:
	case SYMBOL_REF:
	  if (code == 'B' && small_data_operand (x, VOIDmode))
	    {
	      fputs ("sda(", file);
...
m32r_print_operand_address (FILE * file, rtx addr)
{
  ...
  switch (GET_CODE (addr))
	...
	{
	  if (index != 0 || GET_CODE (XEXP (base, 0)) != REG)
	    abort ();
...
{
  if (GET_CODE (op) != CONST_INT
      || INTVAL (op) > MAX_MOVE_BYTES || INTVAL (op) <= 0)
    return 0;
