...
pa_select_section (tree, int, unsigned HOST_WIDE_INT)
  ATTRIBUTE_UNUSED;
     ...
     static void pa_globalize_label (FILE *, const char *) ATTRIBUTE_UNUSED;
...
static
GTY ((length ("n_deferred_plabels")))
     struct deferred_plabel *deferred_plabels;
...
reg_or_0_or_nonsymb_mem_operand (rtx op, enum machine_mode mode)
{
  ...
  if (!TARGET_NO_SPACE_REGS
      ...
      && REG_P (XEXP (XEXP (op, 0), 0)) && REG_P (XEXP (XEXP (op, 0), 1)))
    return 0;
...
reg_before_reload_operand (rtx op, enum machine_mode mode)
{
  ...
  if (reload_completed
      && memory_operand (op, mode) && !symbolic_memory_operand (op, mode))
    return 1;
...
move_dest_operand (rtx op, enum machine_mode mode)
{
  ...
  return (memory_address_p (mode, op)
	  && !IS_INDEX_ADDR_P (op) && !IS_LO_SUM_DLT_ADDR_P (op));
}
...
legitimize_pic_address (rtx orig, enum machine_mode mode, rtx reg)
{
  ...
  if (GET_CODE (orig) == SYMBOL_REF)
	...
	= gen_rtx_MEM (Pmode,
      ...
      REG_NOTES (insn) =
	gen_rtx_EXPR_LIST (REG_EQUAL, orig, REG_NOTES (insn));
			 ...
			 enum machine_mode mode)
{
  ...
  if (!TARGET_NO_SPACE_REGS
      ...
      && REG_P (XEXP (x, 0))
      && REG_P (XEXP (x, 1))
      && REG_POINTER (XEXP (x, 0)) && !REG_POINTER (XEXP (x, 1)))
  ...
  if (GET_CODE (x) == PLUS
	...
	newoffset = (offset & ~mask);
      ...
      /* If the newoffset will not fit in 14 bits (ldo), then
         ...
         them.  */
      if (!VAL_14_BITS_P (newoffset) && GET_CODE (XEXP (x, 0)) == SYMBOL_REF)
	{
	  rtx const_part = plus_constant (XEXP (x, 0), newoffset);
	  rtx tmp_reg = force_reg (Pmode,
				   gen_rtx_HIGH (Pmode, const_part));
	  ptr_reg
	    = force_reg (Pmode, gen_rtx_LO_SUM (Pmode, tmp_reg, const_part));
	}
      else
	{
  ...
  if (GET_CODE (x) == PLUS
	...
	reg2 = force_reg (Pmode, force_operand (reg2, 0));
      ...
      if (GET_CODE (reg1) == REG && REG_POINTER (reg1))
	{
	  base = reg1;
      ...
      else if (GET_CODE (reg2) == REG && REG_POINTER (reg2))
	{
	  base = reg2;
	  orig_base = XEXP (x, 1);
	  idx = XEXP (x, 0);
  ...
  if (GET_CODE (x) == PLUS && symbolic_expression_p (XEXP (x, 1)))
    {
	  ...
	  if (GET_CODE (XEXP (y, 0)) == SYMBOL_REF
		...
		reg2 = force_reg (Pmode, force_operand (reg2, 0));
	      ...
	      return force_reg (Pmode,
		   ...
		   && GET_CODE (XEXP (x, 0)) == MULT
		   && GET_CODE (XEXP (y, 1)) == CONST_INT
		   && INTVAL (XEXP (y, 1)) % INTVAL (XEXP (XEXP (x, 0), 1)) ==
		   0 && GET_CODE (XEXP (XEXP (x, 0), 1)) == CONST_INT
		   && shadd_constant_p (INTVAL (XEXP (XEXP (x, 0), 1))))
	    {
	      regx1
...
emit_move_sequence (rtx * operands, enum machine_mode mode, rtx scratch_reg)
{
  ...
  if (scratch_reg
      ...
      if (!memory_address_p (Pmode, XEXP (operand0, 0)))
	{
	  ...
	  emit_move_insn (scratch_reg,
			  gen_rtx_fmt_ee (GET_CODE (XEXP (operand0, 0)),
					  Pmode, XEXP (XEXP (operand0, 0), 0),
					  scratch_reg));
	}
      else
  ...
  /* Handle secondary reloads for loads of FP registers from constant
     ...
     more problems than it solves.   Fix this for 2.5.  */
	   ...
	   && CONSTANT_P (operand1) && fp_reg_operand (operand0, mode))
    {
      rtx xoperands[2];
  ...
  /* Handle the most common case: storing into a register.  */
	  ...
	  || GET_CODE (operand1) == MEM)
	{
	  ...
	  if (REG_P (operand0) && REG_P (operand1))
	    {
		  ...
		  && !REG_POINTER (operand0) && !HARD_REGISTER_P (operand0))
	  ...
	  if (REG_P (operand0)
	      && GET_CODE (operand1) == MEM && !REG_POINTER (operand0))
	    {
  ...
  if ((GET_CODE (operand1) != HIGH && immediate_operand (operand1, mode))
      ...
      if (symbolic_operand (operand1, mode))
	{
	  ...
	  if (flag_pic)
		...
		temp = gen_reg_rtx (Pmode);
		  ...
		  && (reload_completed || reload_in_progress) && flag_pic)
		{
	  ...
	  /* On the HPPA, references to data space are supposed to use dp,
		...
		set = gen_rtx_SET (VOIDmode,
	      ...
	      emit_insn (gen_rtx_SET (VOIDmode,
				      temp, gen_rtx_HIGH (mode, operand1)));
	      emit_insn (set);
	       ...
	       || !cint_ok_for_move (INTVAL (operand1)))
	{
	  ...
	  if (GET_CODE (operand1) == CONST_INT
	      && GET_MODE_BITSIZE (mode) <= HOST_BITS_PER_WIDE_INT && !insert)
	    {
	  ...
	  if (insert)
	      ...
	      if (temp != operand0 && (insv >= 16384 || insv < -16384))
		{
		  ...
		  emit_move_insn (temp,
				  gen_rtx_LO_SUM (mode, temp, operand1));
		  emit_insn (gen_insv
			     (operand0, GEN_INT (32), const0_rtx, temp));
		}
	      else
		{
		  int len = 5, pos = 27;
...
{
  enum
  { REGOP, OFFSOP, MEMOP, CNSTOP, RNDOP } optype0, optype1;
  rtx latehalf[2];
  ...
  if (optype1 == MEMOP)
      ...
      else if (GET_CODE (addr) == PRE_INC || GET_CODE (addr) == PRE_DEC)
	{
      ...
      else if (GET_CODE (addr) == PLUS && GET_CODE (XEXP (addr, 0)) == MULT)
	{
	  rtx high_reg = gen_rtx_SUBREG (SImode, operands[0], 0);
      ...
      if (DO_FRAME_NOTES && current_function_calls_eh_return)
	{
	  ...
	  for (i = 0;; ++i)
		...
		break;
		  ...
		  && local_fsize == 0 && VAL_14_BITS_P (-actual_fsize))
...
hppa_profile_hook (int label_no)
{
  ...
  ASM_GENERATE_INTERNAL_LABEL (begin_label_name, FUNC_BEGIN_PROLOG_LABEL,
			       label_no);
  begin_label_rtx =
    gen_rtx_SYMBOL_REF (SImode, ggc_strdup (begin_label_name));
...
hppa_can_use_return_insn_p (void)
{
  return (reload_completed
	  ...
	  && !regs_ever_live[2] && !frame_pointer_needed);
}
...
pa_adjust_insn_length (rtx insn, int length)
{
  ...
  /* Millicode insn with an unfilled delay slot.  */
	   ...
	   && GET_CODE (pat) != USE
	   && GET_CODE (pat) != CLOBBER && get_attr_type (insn) == TYPE_MILLI)
    return 4;
  /* Block move pattern.  */
  else if (GET_CODE (insn) == INSN
	   && GET_CODE (pat) == PARALLEL
  ...
  /* Conditional branch with an unfilled delay slot.  */
  else if (GET_CODE (insn) == JUMP_INSN && !simplejump_p (insn))
    {
      ...
      if (GET_CODE (pat) == SET && length == 4 && !forward_branch_p (insn))
	return 4;
      else if (GET_CODE (pat) == PARALLEL
	       && get_attr_type (insn) == TYPE_PARALLEL_BRANCH && length == 4)
	return 4;
      /* Adjust dbra insn with short backwards conditional branch with
         ...
         general register register.  */
	       ...
	       && length == 4 && !forward_branch_p (insn))
	return 4;
      else
	return 0;
    }
...
print_operand (FILE * file, rtx x, int code)
{
  switch (code)
    ...
    case 'r':
      /* A register or zero.  */
      if (x == const0_rtx
	  || (x == CONST0_RTX (DFmode)) || (x == CONST0_RTX (SFmode)))
	{
	  fputs ("%r0", file);
	  return;
	}
  ...
  if (GET_CODE (x) == REG)
      ...
      if (TARGET_64BIT && FP_REG_P (x) && GET_MODE_SIZE (GET_MODE (x)) <= 4)
	{
      ...
      if (FP_REG_P (x)
	  && GET_MODE_SIZE (GET_MODE (x)) <= 4 && (REGNO (x) & 1) == 0)
	fputs ("L", file);
    }
  ...
  for (i = 0; i < n_deferred_plabels; i++)
    {
      (*targetm.asm_out.internal_label) (asm_out_file, "L",
					 CODE_LABEL_NUMBER (deferred_plabels
							    [i].
							    internal_label));
...
pa_hpux_init_libfuncs (void)
{
  ...
  set_conv_libfunc (sfix_optab, SImode, TFmode, TARGET_64BIT
		    ? "__U_Qfcnvfxt_quad_to_sgl" : "_U_Qfcnvfxt_quad_to_sgl");
...
enum millicodes
{ remI, remU, divI, divU, mulI, end1000 };
static void import_milli (enum millicodes);
static char imported[(int) end1000];
static const char *const milli_names[] =
...
emit_hpdiv_const (rtx * operands, int unsignedp)
{
  if (GET_CODE (operands[2]) == CONST_INT
      && INTVAL (operands[2]) > 0
      && INTVAL (operands[2]) < 16 && magic_milli[INTVAL (operands[2])])
    {
...
output_arg_descriptor (rtx call_insn)
{
  ...
  if (GET_CODE (call_insn) != CALL_INSN)
    abort ();
  for (link = CALL_INSN_FUNCTION_USAGE (call_insn); link;
       link = XEXP (link, 1))
    {
      rtx use = XEXP (link, 0);
...
secondary_reload_class (enum reg_class class, enum machine_mode mode, rtx in)
{
  ...
  if (GET_CODE (in) == MEM && GET_CODE (XEXP (in, 0)) == MEM)
    return NO_REGS;
  ...
  if (!flag_pic && is_symbolic && read_only_operand (in, VOIDmode))
    return NO_REGS;
...
{
  if (mode == BLKmode || (TARGET_64BIT && type && AGGREGATE_TYPE_P (type)))
    {
...
hppa_builtin_saveregs (void)
{
  ...
  int argadj = ((!(TYPE_ARG_TYPES (fntype) != 0
		   && (TREE_VALUE (tree_last (TYPE_ARG_TYPES (fntype)))
		       != void_type_node))) ? UNITS_PER_WORD : 0);
...
const char *
output_cbranch (rtx * operands, int nullify, int length, int negated,
		rtx insn)
{
  static char buf[100];
  int useskip = 0;
  ...
  switch (length)
    ...
    case 8:
      /* Handle weird backwards branch with a filled delay slot
         with is nullified.  */
      if (dbr_sequence_length () != 0 && !forward_branch_p (insn) && nullify)
	{
	  strcpy (buf, "{com%I2b,|cmp%I2b,}");
	  if (GET_MODE (operands[1]) == DImode)
	    strcat (buf, "*");
...
output_lbranch (rtx dest, rtx insn)
{
  ...
  if (dbr_sequence_length () != 0)
	...
	abort ();
      ...
      final_scan_insn (NEXT_INSN (insn), asm_out_file, optimize, 0, 0, NULL);
	   ...
	   int negated, rtx insn, int which)
{
  ...
  switch (length)
	...
	strcpy (buf, "bb,*");
      if ((which == 0 && negated) || (which == 1 && !negated))
	strcat (buf, ">=");
      else
	strcat (buf, "<");
      if (useskip)
...
attr_length_millicode_call (rtx insn)
{
  ...
  unsigned long total =
    IN_NAMED_SECTION_P (cfun->decl) ? 0 : total_code_bytes;
...
output_call (rtx insn, rtx call_dest, int sibcall)
{
  ...
  if (!TARGET_LONG_CALLS && attr_length_call (insn, sibcall) == 8)
      ...
      if (TARGET_64BIT && !local_call)
	{
	  ...
	  if (seq_length != 0
	      && GET_CODE (NEXT_INSN (insn)) != JUMP_INSN && !sibcall)
	    {
      ...
      else
	{
	  ...
	  if (seq_length != 0
	      ...
	      && !sibcall && (!TARGET_PA_20 || indirect_call))
	    {
	  ...
	  if ((TARGET_LONG_ABS_CALL || local_call) && !flag_pic)
	      ...
	      else
		{
		  ...
		  struct deferred_plabel *p =
		    get_plabel (XSTR (call_dest, 0));
  ...
  /* We are done if there isn't a jump in the delay slot.  */
  if (seq_length == 0
      || delay_insn_deleted || GET_CODE (NEXT_INSN (insn)) != JUMP_INSN)
    return "";
...
fmpyaddoperands (rtx * operands)
{
  ...
  if (!(mode == GET_MODE (operands[1])
	...
	&& mode == GET_MODE (operands[4]) && mode == GET_MODE (operands[5])))
    return 0;
  ...
  if (!(GET_CODE (operands[1]) == REG
	...
	&& GET_CODE (operands[4]) == REG && GET_CODE (operands[5]) == REG))
    return 0;
...
borx_reg_operand (rtx op, enum machine_mode mode)
{
  ...
  if (op == virtual_incoming_args_rtx
      ...
      || op == virtual_outgoing_args_rtx || op == virtual_cfa_rtx)
    return 0;
  ...
  if (!reload_completed
      ...
      && !current_function_calls_alloca && op == frame_pointer_rtx)
    return 0;
  ...
  for (anchor = get_insns (); anchor; anchor = NEXT_INSN (anchor))
	      ...
	      && !forward_branch_p (anchor)))
	{
	  ...
	  for (floater = PREV_INSN (anchor);
	       floater; floater = PREV_INSN (floater))
	    {
	  ...
	  else if (floater && anchor_attr == PA_COMBINE_TYPE_UNCOND_BRANCH)
	    {
	      rtx temp;
		  ...
		  rtx src1, rtx src2)
{
  ...
  if (insn_code_number < 0 || (extract_insn (new), !constrain_operands (1)))
    return 0;
...
function_value (tree valtype, tree func ATTRIBUTE_UNUSED)
{
  ...
  if (TREE_CODE (valtype) == REAL_TYPE
      ...
      && TYPE_MODE (valtype) != TFmode && !TARGET_SOFT_FLOAT)
	      ...
	      int named ATTRIBUTE_UNUSED)
{
  ...
  if (((TARGET_PORTABLE_RUNTIME || TARGET_64BIT || TARGET_ELF32)
      ...
      || (!TARGET_64BIT
	  && !TARGET_GAS
	  && !cum->incoming && cum->indirect && FLOAT_MODE_P (mode)))
    {
      retval
	= gen_rtx_PARALLEL
	(mode,
  ...
  else
    {
	  ...
	  || (!TARGET_PORTABLE_RUNTIME
	      && !TARGET_64BIT && !TARGET_ELF32 && cum->indirect)
...
cmpib_comparison_operator (rtx op, enum machine_mode mode)
{
  return ((mode == VOIDmode || GET_MODE (op) == mode)
	  && (GET_CODE (op) == EQ
	      ...
	      || GET_CODE (op) == GE
	      || GET_CODE (op) == LT
	      || GET_CODE (op) == LE || GET_CODE (op) == LEU));
}
...
som_text_section_asm_op (void)
{
  ...
  if (TARGET_GAS)
      ...
      if (cfun && !cfun->machine->in_nsubspa)
	{
	  ...
	  if (cfun->decl
	      && DECL_ONE_ONLY (cfun->decl) && !DECL_WEAK (cfun->decl))
	    return
		   ...
		   unsigned HOST_WIDE_INT align ATTRIBUTE_UNUSED)
{
  if (TREE_CODE (exp) == VAR_DECL
	  ...
	  || TREE_CONSTANT (DECL_INITIAL (exp))) && !reloc)
    {
      if (TARGET_SOM && DECL_ONE_ONLY (exp) && !DECL_WEAK (exp))
    ...
    readonly_data_section ();
	   ...
	   && DECL_ONE_ONLY (exp) && !DECL_WEAK (exp))
