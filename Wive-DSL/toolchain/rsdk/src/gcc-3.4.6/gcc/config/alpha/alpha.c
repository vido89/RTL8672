...
/* Costs of various operations on the different architectures.  */
     ...
     static struct alpha_rtx_cost_data const
       alpha_rtx_cost_data[PROCESSOR_MAX] = {
       {			/* EV4 */
...
small_symbolic_operand (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  return (SYMBOL_REF_LOCAL_P (op)
	  && SYMBOL_REF_SMALL_P (op) && SYMBOL_REF_TLS_MODEL (op) == 0);
}
...
signed_comparison_operator (rtx op, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  return (code == EQ || code == NE
	  || code == LE || code == LT || code == GE || code == GT);
}
...
aligned_memory_operand (rtx op, enum machine_mode mode)
{
  ...
  if (reload_in_progress)
	...
	tmp = SUBREG_REG (tmp);
      if (GET_CODE (tmp) == REG && REGNO (tmp) >= FIRST_PSEUDO_REGISTER)
	{
  ...
  if (reload_in_progress
      && GET_CODE (op) == PLUS && GET_CODE (XEXP (op, 0)) == PLUS)
...
alpha_legitimate_address_p (enum machine_mode mode, rtx x, int strict)
{
  ...
  if (mode == DImode
      && GET_CODE (x) == AND
      && GET_CODE (XEXP (x, 1)) == CONST_INT && INTVAL (XEXP (x, 1)) == -8)
    x = XEXP (x, 0);
  ...
  if (REG_P (x)
      && (strict
	  ? STRICT_REG_OK_FOR_BASE_P (x) : NONSTRICT_REG_OK_FOR_BASE_P (x)))
    return true;
  ...
  if (GET_CODE (x) == PLUS)
      ...
      if (REG_P (x))
	{
	  ...
	  if ((strict
	       ...
	       : NONSTRICT_REG_OK_FOR_BASE_P (x)) && CONSTANT_ADDRESS_P (ofs))
	    return true;
	}
      else if (GET_CODE (x) == ADDRESSOF && GET_CODE (ofs) == CONST_INT)
	return true;
    }
...
static
GTY (())
     rtx tls_get_addr_libfunc;

     static rtx get_tls_get_addr (void)
{
...
alpha_rtx_costs (rtx x, int code, int outer_code, int *total)
{
  ...
  switch (code)
	...
	*total = alpha_rtx_cost_data[alpha_cpu].int_mult_si;
    ...
    case ASHIFT:
      if (GET_CODE (XEXP (x, 1)) == CONST_INT && INTVAL (XEXP (x, 1)) <= 3)
	{
	...
	*total = alpha_rtx_cost_data[alpha_cpu].fp_div_df;
	...
	{
	  *total =
	    COSTS_N_INSNS (1) + alpha_rtx_cost_data[alpha_cpu].int_cmov;
	  return false;
	}
      /* FALLTHRU */
...
static void
alpha_set_memflags_1 (rtx x, int in_struct_p, int volatile_p,
		      int unchanging_p)
{
  int i;
			...
			HOST_WIDE_INT c, int n)
{
  ...
  if (HOST_BITS_PER_WIDE_INT != 64 || c >> 31 == -1 || c >> 31 == 0)
    {
	  ...
	  if (no_new_pseudos)
	    {
	      emit_insn (gen_rtx_SET
			 (VOIDmode, target, GEN_INT (high << 16)));
	      temp = target;
	    }
	  else
...
alpha_emit_conditional_branch (enum rtx_code code)
{
  ...
  return gen_rtx_fmt_ee (branch_code, branch_mode, tem,
			 CONST0_RTX (cmp_mode));
}
...
void
alpha_split_tfmode_frobsign (rtx operands[3],
			     rtx (*operation) (rtx, rtx, rtx))
{
  ...
  if (WORDS_BIG_ENDIAN && sign && (size == 2 || size == 4))
    {
      ...
      addr =
	expand_binop (DImode, ior_optab, extl, exth, tgt, 1, OPTAB_WIDEN);
      addr =
	expand_binop (DImode, ashr_optab, addr, GEN_INT (64 - size * 8), addr,
		      1, OPTAB_WIDEN);
    }
  else if (sign && size == 2)
    {
	...
	expand_binop (DImode, ior_optab, extl, exth, tgt, 1, OPTAB_WIDEN);
      addr =
	expand_binop (DImode, ashr_optab, addr, GEN_INT (48), addr, 1,
		      OPTAB_WIDEN);
    }
  else
    {
  ...
  if (WORDS_BIG_ENDIAN)
      ...
      if (src != const0_rtx)
	{
	  switch ((int) size)
	    {
	    case 2:
	      emit_insn (gen_inswl_be
			 (insh, gen_lowpart (HImode, src), addr));
	      break;
	    case 4:
	      emit_insn (gen_insll_be
			 (insh, gen_lowpart (SImode, src), addr));
	      break;
	    case 8:
	      emit_insn (gen_insql_be
			 (insh, gen_lowpart (DImode, src), addr));
	      break;
	    }
      ...
      if (src != const0_rtx)
	{
	  ...
	  switch ((int) size)
	    {
	    case 2:
	      emit_insn (gen_inswl_le
			 (insl, gen_lowpart (HImode, src), addr));
	      break;
	    case 4:
	      emit_insn (gen_insll_le
			 (insl, gen_lowpart (SImode, src), addr));
	      break;
	    case 8:
				    ...
				    HOST_WIDE_INT words, HOST_WIDE_INT ofs)
{
  ...
  st_addr_1 = change_address (dmem, DImode, gen_rtx_AND (DImode, dmema, im8));
  ...
  if (data_regs != NULL)
	...
	expand_binop (DImode, ior_optab, st_tmp_2, ins_tmps[words - 1],
      ...
      st_tmp_1 =
	expand_binop (DImode, ior_optab, st_tmp_1, data_regs[0], st_tmp_1, 1,
		      OPTAB_WIDEN);
    }
  ...
  if (GET_CODE (XEXP (orig_src, 0)) == ADDRESSOF)
    {
      ...
      if (GET_CODE (tmp) == REG
	  && mode != BLKmode && GET_MODE_SIZE (GET_MODE (tmp)) >= bytes)
	{
	  if (mode == TImode)
	    {
...
alpha_expand_block_clear (rtx operands[])
{
  ...
  if (align >= 32 && bytes > 16)
	...
	orig_dsta = force_reg (Pmode, orig_dsta);
      ...
      for (i = 0; i < words; ++i)
	{
	  rtx mem = change_address (orig_dst, DImode,
				    gen_rtx_AND (DImode,
						 plus_constant (orig_dsta,
								ofs + i * 8),
						 GEN_INT (-8)));
	  set_mem_alias_set (mem, 0);
...
alpha_adjust_cost (rtx insn, rtx link, rtx dep_insn, int cost)
{
  ...
  if (dep_insn_type == TYPE_ILD
      || dep_insn_type == TYPE_FLD || dep_insn_type == TYPE_LDSYM)
...
print_operand (FILE * file, rtx x, int code)
{
  ...
  switch (code)
	...
	output_operand_lossage ("invalid %%s value");
      ...
      fprintf (file, HOST_WIDE_INT_PRINT_DEC,
	       WORDS_BIG_ENDIAN ? (56 - INTVAL (x)) / 8 : INTVAL (x) / 8);
      break;
...
print_operand_address (FILE * file, rtx addr)
{
  ...
  if (GET_CODE (addr) == REG)
    basereg = REGNO (addr);
  else if (GET_CODE (addr) == SUBREG && GET_CODE (SUBREG_REG (addr)) == REG)
    basereg = subreg_regno (addr);
...
alpha_build_builtin_va_list (void)
{
  ...
  base = build_decl (FIELD_DECL, get_identifier ("__base"), ptr_type_node);
  ...
  if (!no_rtl)
    {
      ...
      tmp = gen_rtx_MEM (BLKmode,
			 plus_constant (virtual_incoming_args_rtx,
					cum * UNITS_PER_WORD));
      set_mem_alias_set (tmp, set);
      move_block_from_reg (16 + (TARGET_FPREGS ? 32 : 0) + cum, tmp, 6 - cum);
    }
...
alpha_va_arg (tree valist, tree type)
{
  ...
  addr_tree = build (PLUS_EXPR, TREE_TYPE (base_field), base_field, addend);
...
#define MAX_ARGS 2
  ...
  for (arglist = TREE_OPERAND (exp, 1), arity = 0;
       arglist; arglist = TREE_CHAIN (arglist), arity++)
    {
...
enum alpha_procedure_types
{ PT_NULL = 0, PT_REGISTER = 1, PT_STACK = 2 };
...
alpha_sa_size (void)
{
  ...
  if (TARGET_ABI_UNICOSMK)
	...
	= (sa_size || get_frame_size () != 0
	   ...
	   || frame_pointer_needed) ? PT_STACK : PT_REGISTER;
...
alpha_expand_prologue (void)
{
  ...
  if (TARGET_ABI_OPEN_VMS)
    ...
    frame_size = ALPHA_ROUND (sa_size
      ...
      + ALPHA_ROUND (frame_size + current_function_outgoing_args_size);
  else
  ...
  if (frame_size <= 32768)
  ...
  else
    {
      ...
      emit_insn (gen_adddi3 (ptr, stack_pointer_rtx,
			     GEN_INT (TARGET_ABI_UNICOSMK ? 4096 -
				      64 : 4096)));
  ...
  if (TARGET_ABI_OPEN_VMS)
	...
	{
	  rtx seq = emit_move_insn (stack_pointer_rtx,
		      ...
		      tree decl ATTRIBUTE_UNUSED)
{
  ...
  if (TARGET_ABI_UNICOSMK)
    ...
    fprintf (file, "\t.frame $%d," HOST_WIDE_INT_PRINT_DEC ",$26,"
	     ...
	     vms_unwind_regno,
	     frame_size >= (1UL << 31) ? 0 : frame_size, reg_offset);
...
void
alpha_end_function (FILE * file, const char *fnname,
		    tree decl ATTRIBUTE_UNUSED)
{
  /* End the function.  */
...
alpha_reorg (void)
{
  ...
  if (optimize && !optimize_size
      && alpha_tp != ALPHA_TP_INSN && flag_schedule_insns_after_reload)
    {
...
enum links_kind
{ KIND_UNUSED, KIND_LOCAL, KIND_EXTERN };
enum reloc_kind
{ KIND_LINKAGE, KIND_CODEADDR };
...
/* Return the VMS argument type corresponding to MODE.  */
     ...
     enum avms_arg_type alpha_arg_type (enum machine_mode mode)
{
  switch (mode)
    {
    case SFmode:
...
alpha_use_linkage (rtx linkage ATTRIBUTE_UNUSED,
		   ...
		   int lflag ATTRIBUTE_UNUSED, int rflag ATTRIBUTE_UNUSED)
{
  return NULL_RTX;
}
...
unicosmk_defer_case_vector (rtx lab, rtx vec)
{
  ...
  machine->addr_list = gen_rtx_EXPR_LIST (VOIDmode, vec, machine->addr_list);
}
...
{
  static const char *const externs[] = { "__T3E_MISMATCH" };
