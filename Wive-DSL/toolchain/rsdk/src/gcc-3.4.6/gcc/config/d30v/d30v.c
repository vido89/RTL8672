...
unsigned char hard_regno_mode_ok[(int)
...
override_options ()
{
	   ...
	   mode2 = (enum machine_mode) ((int) mode2 + 1))
	{
	  for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
		...
		&& (hard_regno_mode_ok[(int) mode1][regno]
	      ...
	      error
		("bad modes_tieable_p for register %s, mode1 %s, mode2 %s",
  ...
  for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
	...
	fprintf (stderr, "Register %s class is %s, can hold modes",
		 reg_names[regno], names[class]);
	for (mode1 = VOIDmode; (int) mode1 < NUM_MACHINE_MODES;
	     mode1 = (enum machine_mode) ((int) mode1 + 1))
	  {
     ...
     enum machine_mode mode ATTRIBUTE_UNUSED;
{
  ...
  switch (GET_CODE (op))
    ...
    case ABS:
    case NOT:
      if (GET_MODE (op) == SImode && GET_CODE (op0) == REG
	  && GPR_P (REGNO (op0)))
	return TRUE;
     ...
     enum machine_mode mode ATTRIBUTE_UNUSED;
{
  ...
  if (GET_RTX_CLASS (GET_CODE (op)) != '2'
...
d30v_stack_info ()
{
  ...
  info_ptr->varargs_size = ((varargs_p)
			    ? (GPR_ARG_LAST + 1 -
			       GPR_ARG_FIRST) * UNITS_PER_WORD : 0);
  ...
  info_ptr->save_offset = (info_ptr->total_size
			   - (current_function_pretend_args_size
			      + info_ptr->varargs_size
			      + info_ptr->gpr_size + info_ptr->accum_size));
  ...
  if (TARGET_DEBUG_ARG)
    fprintf (stderr,
	     ...
	     *cum, GET_MODE_NAME (mode), named, size, adjust,
	     (ret) ? ((ret == const0_rtx) ? "<0>" : reg_names[REGNO (ret)]) :
	     "memory");
  ...
  *cum += words + adjust;
  ...
  if (TARGET_DEBUG_ARG)
    fprintf (stderr,
	     ...
	     *cum, GET_MODE_NAME (mode), named, words * UNITS_PER_WORD,
	     adjust);
}

...
d30v_expand_prologue ()
{
  ...
  int num_memrefs = (info->memrefs_2words
		     + info->memrefs_1word + info->memrefs_varargs);
  ...
  if (info->varargs_p)
      ...
      if ((info->memrefs_1word & 1) != 0)
	{
	  ...
	  emit_insn (gen_addsi3
		     (save_tmp, save_tmp, GEN_INT (UNITS_PER_WORD)));
	}
  ...
  switch (GET_CODE (value))
	...
	abort ();
      offset = subreg_regno_offset (REGNO (SUBREG_REG (value)),
				    ...
				    SUBREG_BYTE (value), GET_MODE (value));
  ...
  if (!x1)
	...
	case SUBREG:
	  offset1 = subreg_regno_offset (REGNO (SUBREG_REG (x1)),
					 ...
					 SUBREG_BYTE (x1), GET_MODE (x1));
	  x1 = SUBREG_REG (x1);
	  if (GET_CODE (x1) != REG)
	    fatal_insn ("bad insn to d30v_print_operand_memory_reference:",
			x);
     ...
     rtx static_chain;
{
  /* The instruction space can only be accessed by ld2w/st2w.
     ...
     jmp <fnaddr> */
  d30v_build_long_insn (0x83A80000 |
			((STATIC_CHAIN_REGNUM - GPR_FIRST) << 12), 0x80000000,
			static_chain, gen_rtx (MEM, DImode, addr));
     ...
     rtx arg2;
{
  ...
  rtx rtx_test =
    gen_rtx (SET, VOIDmode, result, gen_rtx (test, CCmode, arg1, arg2));
     ...
     rtx insn;
{
  if (GET_CODE (operands[0]) == REG && GPR_P (REGNO (operands[0])))
	...
	return "mvtacc %0,%.,%.";
	   ...
	   && GET_CODE (operands[1]) == REG && GPR_P (REGNO (operands[1])))
    return "st2w %1,%M0";
