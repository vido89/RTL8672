...
const unsigned int ns32k_reg_class_contents[N_REG_CLASSES][1] =
  REG_CLASS_CONTENTS;
...
hard_regno_mode_ok (int regno, enum machine_mode mode)
{
  ...
  if (size == UNITS_PER_WORD * 2 && (regno & 1) == 0 && regno < F0_REGNUM)
    return 1;
...
secondary_memory_needed (enum reg_class CLASS1,
			 enum reg_class CLASS2, enum machine_mode M)
{
...
gen_indexed_expr (rtx base, rtx index, rtx scale)
{
  ...
  addr = gen_rtx_MULT (SImode, index, GEN_INT (1 << INTVAL (scale)));
...
{
  enum anon1
  { REGOP, OFFSOP, PUSHOP, CNSTOP, RNDOP } optype0, optype1;
  rtx latehalf[2];
  ...
  if (REG_P (operands[1]))
    optype1 = REGOP;
  else if (CONSTANT_P (operands[1]) || GET_CODE (operands[1]) == CONST_DOUBLE)
    optype1 = CNSTOP;
...
print_operand_address (register FILE * file, register rtx addr)
{
  ...
  if (base
      ...
      && GET_CODE (base) != SYMBOL_REF && GET_CODE (offset) != CONST_INT
#else
      /* This is a re-implementation of the SEQUENT_ADDRESS_BUG fix.  */
#endif
      && !indexexp && GET_CODE (base) == REG && REG_OK_FOR_INDEX_P (base))
    {
      indexexp = base;
      base = NULL;
    }
...
#ifdef UTEK_ASM
      fprintf (file, "[%c`%s]",
	       scales[scale], ns32k_out_reg_names[REGNO (indexexp)]);
#else
      fprintf (file, "[%s:%c]",
	       ns32k_out_reg_names[REGNO (indexexp)], scales[scale]);
#endif
    }
}

      ...
      if (GET_CODE (operands[0]) == REG)
	{
	  if (GET_CODE (operands[1]) == REG)
		...
		return "movd %1,%0\n\taddd %0,%0";
	      ...
	      operands[1] =
		gen_indexed_expr (const0_rtx, operands[1], operands[2]);
	      return "addr %a1,%0";
	    }
	  if (operands[2] == const1_rtx)
	    return "movd %1,%0\n\taddd %0,%0";
