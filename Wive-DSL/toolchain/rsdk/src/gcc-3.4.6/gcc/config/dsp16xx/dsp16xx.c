     ...
     enum machine_mode mode;
{
  switch ((int) mode)
    ...
    case DImode:
#endif
      if (regno == REG_A0 || regno == REG_A1 || regno == REG_Y
	  || regno == REG_PROD || (IS_YBASE_REGISTER_WINDOW (regno)
				   && ((regno & 1) == 0)))
	return 1;
      else
	return 0;
     ...
     enum reg_class class;
{
  ...
  if (REG_P (x) && !IS_ACCUM_REG (REGNO (x))
     ...
     rtx in;
{
  ...
  if (class == ACCUM_HIGH_REGS || class == A1H_REG || class == A0H_REG)
    {
  ...
  if (class == ACCUM_HIGH_REGS
      ...
      || class == A1L_REG
      || class == A0L_REG || class == A1H_REG || class == A0H_REG)
    {
      if (GET_CODE (in) == PLUS && mode == QImode)
	{
	  rtx addr0 = XEXP (in, 0);
     ...
     HOST_WIDE_INT size;
{
  ...
  fprintf (file,
	   ...
	   current_frame_info.reg_size, current_function_outgoing_args_size,
  ...
  if (current_frame_info.var_size)
	...
	{
	  if (SMALL_INTVAL (current_frame_info.var_size)
     ...
     HOST_WIDE_INT size ATTRIBUTE_UNUSED;
{
  ...
  if (current_frame_info.args_size)
	...
	{
	  fprintf (file, "\t%s=%ld\n\t*%s++%s\n",
		   reg_names[REG_J], -current_frame_info.args_size, sp,
		   reg_names[REG_J]);
	}
    }
  ...
  if (current_frame_info.var_size)
	...
	{
	  fprintf (file, "\t%s=%ld\n\t*%s++%s\n",
		   reg_names[REG_J], -current_frame_info.var_size, sp,
		   reg_names[REG_J]);
	}
    }
     ...
     int shift_amount;
{
  ...
  while (shift_amount != 0)
      ...
      if (shift_amount / 16)
	{
	  ...
	  shift_amount = shift_amount - (quotient * 16);
	  for (i = 0; i < quotient; i++)
	    output_asm_insn ((first_shift_emitted ?
			      shift_asm_ptr[SHIFT_INDEX_16] :
  ...
  switch (code)
	...
	*total = COSTS_N_INSNS (38);
      ...
      if (GET_CODE (XEXP (x, 1)) == CONST_INT)
	{
	  ...
	  if (number == 1 || number == 4 || number == 8 || number == 16)
	    *total = COSTS_N_INSNS (1);
	  else if (TARGET_BMU)
	    *total = COSTS_N_INSNS (2);
	  else
