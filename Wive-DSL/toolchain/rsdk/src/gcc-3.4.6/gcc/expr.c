...
init_expr_once (void)
{
  ...
  for (mode = VOIDmode; (int) mode < NUM_MACHINE_MODES;
	...
	for (regno = 0; regno < FIRST_PSEUDO_REGISTER
	     && (direct_load[(int) mode] == 0
		 || direct_store[(int) mode] == 0); regno++)
	  {
  ...
  if (to_real)
	...
	abort ();
      ...
      emit_libcall_block (insns, to, value,
			  tab ==
			  trunc_optab ? gen_rtx_FLOAT_TRUNCATE (to_mode,
								from) :
  ...
  if (GET_MODE_BITSIZE (from_mode) < GET_MODE_BITSIZE (to_mode)
      ...
      for (i = GET_MODE_SIZE (lowpart_mode) / UNITS_PER_WORD; i < nwords; i++)
	{
      ...
      emit_no_conflict_block (insns, to, from, NULL_RTX,
			      gen_rtx_fmt_e (equiv_code, to_mode,
					     copy_rtx (from)));
      return;
    }
	       ...
	       int unsignedp)
{
  ...
  if ((GET_CODE (x) == CONST_INT
	  ...
	  && GET_MODE_SIZE (mode) > GET_MODE_SIZE (oldmode))
	{
	  ...
	  val &= ((HOST_WIDE_INT) 1 << width) - 1;
	  if (!unsignedp && (val & ((HOST_WIDE_INT) 1 << (width - 1))))
		...
		unsigned int align, int endp)
{
  ...
  if (to)
	...
	= (GET_CODE (to_addr) == PRE_INC || GET_CODE (to_addr) == PRE_DEC
	   || GET_CODE (to_addr) == POST_INC
	   || GET_CODE (to_addr) == POST_DEC);
      data.reverse = (GET_CODE (to_addr) == PRE_DEC
		      || GET_CODE (to_addr) == POST_DEC);
    }
  else
    {
...
block_move_libcall_safe_for_call_parm (void)
{
     ...
     argument.  */
  {
	...
	if (FUNCTION_ARG_PARTIAL_NREGS (args_so_far, mode, NULL_TREE, 1))
	  return false;
#endif
      ...
      else if (GET_CODE (src) == CONCAT)
	{
	  ...
	  if ((bytepos == 0 && bytelen == slen0)
	      || (bytepos != 0 && bytepos + bytelen <= slen))
	    {
		  ...
		  && (GET_CODE (tmps[i]) != REG
		      || GET_MODE (tmps[i]) != mode))
		tmps[i] =
		  extract_bit_field (tmps[i], bytelen * BITS_PER_UNIT,
				     (bytepos % slen0) * BITS_PER_UNIT, 1,
				     NULL_RTX, mode, mode, ssize);
	    }
	  else if (bytepos == 0)
	    {
      ...
      else if (VECTOR_MODE_P (GET_MODE (dst)) && GET_CODE (src) == REG)
	{
...
emit_group_move (rtx dst, rtx src)
{
  ...
  if (GET_CODE (src) != PARALLEL
      || GET_CODE (dst) != PARALLEL || XVECLEN (src, 0) != XVECLEN (dst, 0))
    abort ();
		  ...
		  int ssize)
{
  ...
  for (i = start; i < XVECLEN (src, 0); i++)
	  ...
	  if (bytepos + bytelen <= GET_MODE_SIZE (GET_MODE (XEXP (dst, 0))))
	  ...
	  else if (bytepos == 0 && XVECLEN (src, 0))
	    {
	      ...
	      emit_move_insn (adjust_address
			      (dest, GET_MODE (tmps[i]), bytepos), tmps[i]);
	      dst = dest;
	      break;
	    }
	  else
	...
	store_bit_field (dest, bytelen * BITS_PER_UNIT,
			 bytepos * BITS_PER_UNIT, mode, tmps[i], ssize);
    }
...
use_reg (rtx * call_fusage, rtx reg)
{
  ...
  *call_fusage
    = gen_rtx_EXPR_LIST (VOIDmode, gen_rtx_USE (VOIDmode, reg), *call_fusage);
}
...
can_store_by_pieces (unsigned HOST_WIDE_INT len,
		     rtx (*constfun) (void *, HOST_WIDE_INT,
				      enum machine_mode), void *constfundata,
		     unsigned int align)
{
  ...
  for (reverse = 0;
       reverse <= (HAVE_PRE_DECREMENT || HAVE_POST_DECREMENT); reverse++)
    {
      l = len;
      mode = VOIDmode;
		   ...
		   unsigned int align ATTRIBUTE_UNUSED)
{
  ...
  /* If storing requires more than two move insns,
     copy addresses to registers (to make displacements shorter)
     and use post-increment if available.  */
  if (!data->autinc_to && move_by_pieces_ninsns (data->len, align) > 2)
    {
	...
	{
	  data->to_addr =
	    copy_addr_to_reg (plus_constant (to_addr, data->len));
	  data->autinc_to = 1;
	  data->explicit_inc_to = -1;
	}
...
GTY (())
     ...
     void init_block_clear_fn (const char *asmspec)
{
  if (!block_clear_fn)
    {
      tree fn, args;
...
emit_move_insn (rtx x, rtx y)
{
  ...
  if (y_cst && GET_CODE (x) == REG
      ...
      && SET_DEST (set) == x && !rtx_equal_p (y_cst, SET_SRC (set)))
  ...
  /* This will handle any multi-word or full-word mode that lacks a move_insn
	   ...
	   i++)
	{
      ...
      /* Show the output dies here.  This is necessary for SUBREGs
         ...
         We never want to emit such a clobber after reload.  */
      if (x != y
	  && !(reload_in_progress || reload_completed) && need_clobber != 0)
	emit_insn (gen_rtx_CLOBBER (VOIDmode, x));
...
compress_float_constant (rtx x, rtx y)
{
  ...
  for (srcmode = GET_CLASS_NARROWEST_MODE (GET_MODE_CLASS (orig_srcmode));
       srcmode != orig_srcmode; srcmode = GET_MODE_WIDER_MODE (srcmode))
    {
      enum insn_code ic;
...
emit_single_push_insn (enum machine_mode mode, rtx x, tree type)
{
  ...
  /* If we are to pad downward, adjust the stack pointer first and
  ...
  else if (FUNCTION_ARG_PADDING (mode, type) == downward)
    {
      ...
      emit_move_insn (stack_pointer_rtx, expand_binop (Pmode,
#ifdef STACK_GROWS_DOWNWARD
						       sub_optab,
#else
						       add_optab,
...
expand_assignment (tree to, tree from, int want_value)
{
  ...
  if (TREE_CODE (to) == COMPONENT_REF || TREE_CODE (to) == BIT_FIELD_REF
	...
	{
	  rtx offset_rtx =
	    expand_expr (offset, NULL_RTX, VOIDmode, EXPAND_SUM);
      ...
      if (GET_CODE (to_rtx) == MEM && !can_address_p (to))
	{
      ...
      result = store_field (to_rtx, bitsize, bitpos, mode1, from, (want_value
...
store_expr (tree exp, rtx target, int want_value)
{
  ...
  if (TREE_CODE (exp) == COMPOUND_EXPR)
	...
	dont_return_target = 1;
	   ...
	   && GET_CODE (target) == MEM
	   && !MEM_VOLATILE_P (target) && GET_MODE (target) != BLKmode)
	  ...
	  && TREE_TYPE (TREE_TYPE (exp)) == 0)
	{
	  ...
	  exp = convert ((*lang_hooks.types.type_for_mode)
			 (GET_MODE (SUBREG_REG (target)),
			  SUBREG_PROMOTED_UNSIGNED_P (target)), exp);
	...
	{
	  temp =
	    convert_modes (GET_MODE (target), TYPE_MODE (TREE_TYPE (exp)),
			   temp, SUBREG_PROMOTED_UNSIGNED_P (target));
	  temp =
	    convert_modes (GET_MODE (SUBREG_REG (target)), GET_MODE (target),
			   temp, SUBREG_PROMOTED_UNSIGNED_P (target));
	}
  ...
  if ((!rtx_equal_p (temp, target)
      ...
      && TREE_CODE (exp) != ERROR_MARK && !dont_store_target
	  ...
	  if (GET_CODE (size) == CONST_INT
	  ...
	  else
	    {
	      ...
	      tree copy_size = size_binop (MIN_EXPR,
					   ...
					   size_int (TREE_STRING_LENGTH
						     (exp)));
	      rtx copy_size_rtx = expand_expr (copy_size, NULL_RTX, VOIDmode,
					       (want_value & 2
						? EXPAND_STACK_PARM :
						EXPAND_NORMAL));
	      rtx label = 0;
...
is_zeros_p (tree exp)
{
  ...
  switch (TREE_CODE (exp))
    ...
    case VECTOR_CST:
      for (elt = TREE_VECTOR_CST_ELTS (exp); elt; elt = TREE_CHAIN (elt))
...
{
  if (TREE_CODE (exp) == CONSTRUCTOR && bitpos % BITS_PER_UNIT == 0
  ...
  if (TREE_CODE (type) == RECORD_TYPE || TREE_CODE (type) == UNION_TYPE
	       ...
	       && GET_MODE_SIZE (GET_MODE (target)) <= UNITS_PER_WORD)
	{
      ...
      /* If the constructor has fewer fields than the structure
		...
		|| mostly_zeros_p (exp)) && (GET_CODE (target) != REG
					     || ((HOST_WIDE_INT)
      ...
      else
	{
	  ...
	  for (elt = CONSTRUCTOR_ELTS (exp);
	       elt != NULL_TREE && !need_to_clear; elt = TREE_CHAIN (elt))
	    {
	   ...
	   elt; elt = TREE_CHAIN (elt), i++)
	{
	  ...
	  if (mode == BLKmode)
	    bitsize = (host_integerp (TYPE_SIZE (elttype), 1)
		       ? tree_low_cst (TYPE_SIZE (elttype), 1) : -1);
	  else
	  ...
	  if (index != NULL_TREE && TREE_CODE (index) == RANGE_EXPR)
		...
		index = convert (ssizetype,
	      ...
	      xtarget = offset_address (target,
					expand_expr (position, 0, VOIDmode,
						     0),
					highest_pow2_factor (position));
	      xtarget = adjust_address (xtarget, mode, 0);
	      store_expr (value, xtarget, 0);
		     ...
		     int *pvolatilep)
{
  ...
  while (1)
	...
	bit_offset =
	  size_binop (PLUS_EXPR, bit_offset, TREE_OPERAND (exp, 2));
      else if (TREE_CODE (exp) == COMPONENT_REF)
	{
	  ...
	  if (this_offset == 0)
	    ...
	    this_offset =
	      build (WITH_RECORD_EXPR, sizetype, this_offset, exp);
...
safe_from_p (rtx x, tree exp, int top_p)
{
  ...
  if (x == 0
	  ...
	  && (TREE_CODE (TREE_TYPE (exp)) != ARRAY_TYPE
	      ...
	      != INTEGER_CST) && GET_MODE (x) == BLKmode)
  ...
  switch (TREE_CODE_CLASS (TREE_CODE (exp)))
	...
	case ADDR_EXPR:
	  ...
	  if (DECL_P (exp))
	    {
	      if (!DECL_RTL_SET_P (exp) || GET_CODE (DECL_RTL (exp)) != MEM)
		return 0;
	      else
		  ...
		  enum expand_modifier modifier, rtx * alt_rtl)
{
  ...
  switch (code)
    ...
    case INTEGER_CST:
      ...
      if (TREE_CONSTANT_OVERFLOW (exp) && modifier != EXPAND_INITIALIZER)
    ...
    case STRING_CST:
	  ...
	  && modifier != EXPAND_INITIALIZER
	  && modifier != EXPAND_SUM
	  && (!memory_address_p (mode, XEXP (temp, 0)) || flag_force_addr))
	...
	*alt_rtl = RTL_EXPR_ALT_RTL (exp);
	...
	if (string
	    ...
	    && GET_MODE_CLASS (mode) == MODE_INT
	    && GET_MODE_SIZE (mode) == 1 && modifier != EXPAND_WRITE)
	...
	if (CONSTANT_P (op0))
	  {
	    ...
	    if (mode != BLKmode && LEGITIMATE_CONSTANT_P (op0) && offset == 0)
	...
	if (mode1 == VOIDmode
					  ...
					  bitsize)))
	  {
		...
		&& bitsize <
		(HOST_WIDE_INT) GET_MODE_BITSIZE (GET_MODE (op0)))
	      op0 =
		expand_shift (LSHIFT_EXPR, GET_MODE (op0), op0,
	...
	bit = expand_shift (RSHIFT_EXPR, byte_mode,
			    ...
			    make_tree (TREE_TYPE (index), rem), NULL_RTX, 1);
      ...
      if (CONSTANT_P (op0))
	{
      ...
      if (modifier == EXPAND_INITIALIZER)
	return gen_rtx_fmt_e (unsignedp ? ZERO_EXTEND : SIGN_EXTEND, mode,
			      op0);
	...
	&& (GET_MODE_CLASS (mode) == MODE_INT) ? addv_optab : add_optab;
	  ...
	  && TREE_CODE (TREE_OPERAND (TREE_OPERAND (exp, 0), 1)) ==
	  INTEGER_CST && TREE_CODE (TREE_OPERAND (exp, 1)) == RTL_EXPR
	  ...
	  && really_constant_p (TREE_OPERAND (exp, 1)))
	{
      ...
      this_optab = !unsignedp && flag_trapv
	&& (GET_MODE_CLASS (mode) == MODE_INT) ? subv_optab : sub_optab;
			       ...
			       (TREE_OPERAND (TREE_OPERAND (exp, 0), 0)))))))
	{
	  ...
	  if (mode == GET_MODE_WIDER_MODE (innermode))
	    {
	      if (this_optab->handlers[(int) mode].insn_code !=
		  CODE_FOR_nothing)
		{
	      ...
	      else if (other_optab->handlers[(int) mode].insn_code !=
		       CODE_FOR_nothing && innermode == word_mode)
		{
		  rtx htem;
		  ...
		  if (TREE_CODE (TREE_OPERAND (exp, 1)) == INTEGER_CST)
		    ...
		    op1 =
		      expand_expr (TREE_OPERAND (TREE_OPERAND (exp, 1), 0),
				   NULL_RTX, VOIDmode, 0);
		  temp =
		    expand_binop (mode, other_optab, op0, op1, target,
				  unsignedp, OPTAB_LIB_WIDEN);
		  htem =
		    expand_mult_highpart_adjust (innermode,
						 gen_highpart (innermode,
							       temp), op0,
						 op1, gen_highpart (innermode,
								    temp),
						 unsignedp);
	...
	if (temp && singleton && binary_op
	    ...
	    && TREE_CODE_CLASS (TREE_CODE (TREE_OPERAND (exp, 0))) == '<')
	  {
	    ...
	    optab boptab = (TREE_CODE (binary_op) == PLUS_EXPR
			    ...
			    : TREE_CODE (binary_op) ==
			    BIT_IOR_EXPR ? ior_optab : xor_optab);
	...
	if (ignore
	    ...
	    &&
	    integer_onep (DECL_SIZE
			  (TREE_OPERAND (TREE_OPERAND (rhs, 1), 1))))
	  {
      ...
      else
	{
	  ...
	  if (CONSTANT_P (op0))
	    op0 =
	      force_const_mem (TYPE_MODE (TREE_TYPE (TREE_OPERAND (exp, 0))),
			       op0);
	  ...
	  if (STRICT_ALIGNMENT && GET_MODE (op0) == BLKmode
	      && (TYPE_ALIGN (TREE_TYPE (TREE_OPERAND (exp, 0)))
		  > MEM_ALIGN (op0)) && MEM_ALIGN (op0) < BIGGEST_ALIGNMENT)
	    {
	      tree inner_type = TREE_TYPE (TREE_OPERAND (exp, 0));
	      rtx new;
...
do_store_flag (tree exp, rtx target, enum machine_mode mode, int only_cheap)
{
  ...
  if ((code == NE || code == EQ)
      ...
      && integer_pow2p (TREE_OPERAND (arg0, 1)))
    {
      ...
      return
	expand_expr (fold_single_bit_test
		     (code == NE ? NE_EXPR : EQ_EXPR, arg0, arg1, type),
  ...
  if (!get_subtarget (target) || GET_MODE (subtarget) != operand_mode)
    subtarget = 0;
  ...
  if (GET_CODE (result) == CONST_INT)
    return (((result == const0_rtx && !invert)
	     || (result != const0_rtx && invert)) ? const0_rtx : const1_rtx);
