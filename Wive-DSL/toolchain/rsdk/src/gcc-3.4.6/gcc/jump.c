...
duplicate_loop_exit_test (rtx loop_start)
{
  ...
  for (insn = loop_start, loop_depth = 0;
       insn != exitcode; insn = NEXT_INSN (insn))
    {
      if (GET_CODE (insn) != NOTE)
	continue;
...
enum rtx_code
reversed_comparison_code_parts (enum rtx_code code, rtx arg0, rtx arg1,
				rtx insn)
{
  ...
  if (GET_CODE (arg0) == CONST_INT
	  ...
	  && GET_MODE_CLASS (mode) != MODE_CC && !HONOR_NANS (mode)))
    return reverse_condition (code);
...
condjump_p (rtx insn)
{
  ...
  if (GET_CODE (x) != SET || GET_CODE (SET_DEST (x)) != PC)
    return 0;
...
follow_jumps (rtx label)
{
  ...
  for (depth = 0;
	...
	&& (next = NEXT_INSN (insn)) && GET_CODE (next) == BARRIER); depth++)
    {
...
delete_prior_computation (rtx note, rtx insn)
{
  ...
  for (our_prev = prev_nonnote_insn (insn);
	...
	break;
      ...
      if (GET_CODE (pat) == USE && GET_CODE (XEXP (pat, 0)) == INSN)
...
never_reached_warning (rtx avoided_insn, rtx finish)
{
  ...
  while (1)
    {
      rtx prev = PREV_INSN (insn);
      if (prev == NULL_RTX || GET_CODE (prev) != NOTE)
	break;
      insn = prev;
    }
