...
get_jump_table_offset (rtx insn, rtx * earliest)
{
  ...
  if (GET_CODE (x) == IF_THEN_ELSE && GET_CODE (XEXP (x, 2)) == LABEL_REF)
    x = XEXP (x, 1);
...
reg_set_p (rtx reg, rtx insn)
{
  ...
  if (INSN_P (insn)
      && (FIND_REG_INC_NOTE (insn, reg) || (GET_CODE (insn) == CALL_INSN
  ...
  for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
      ...
      else if (fmt[i] == 'E')
	{
	  ...
	  for (j = XVECLEN (x, i) - 1; j >= 0; j--)
		...
		&& refers_to_regno_p (regno, endregno, XVECEXP (x, i, j),
				      loc))
	      return 1;
	}
    }
...
reg_overlap_mentioned_p (rtx x, rtx in)
{
  ...
  if (GET_CODE (x) == STRICT_LOW_PART
      || GET_CODE (x) == ZERO_EXTRACT || GET_CODE (x) == SIGN_EXTRACT)
    x = XEXP (x, 0);
...
reg_set_last (rtx x, rtx insn)
{
  ...
  for (;
       ...
       && !(GET_CODE (insn) == CALL_INSN
	    && REGNO (x) <= FIRST_PSEUDO_REGISTER); insn = PREV_INSN (insn))
    if (INSN_P (insn))
      {
	...
	if (set)
	  {
		...
		|| ((GET_CODE (last_value) == REG
		    ...
		    && !reg_set_between_p (last_value, insn, orig_insn)))
	      return last_value;
	    else
	      return 0;
	  }
  ...
  for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e')
	XEXP (x, i) =
	  replace_regs (XEXP (x, i), reg_map, nregs, replace_dest);
      else if (fmt[i] == 'E')
	{
	  int j;
	  for (j = 0; j < XVECLEN (x, i); j++)
...
replace_label (rtx * x, void *data)
{
  ...
  if (GET_CODE (l) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P (l))
    {
  ...
  if ((GET_CODE (l) == LABEL_REF
       || GET_CODE (l) == INSN_LIST) && XEXP (l, 0) == old_label)
    {
      XEXP (l, 0) = new_label;
...
rtx_referenced_p_1 (rtx * body, void *x)
{
  ...
  if (GET_CODE (*body) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P (*body))
...
computed_jump_p_1 (rtx x)
{
  ...
  for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e' && computed_jump_p_1 (XEXP (x, i)))
	return 1;
			       ...
			       unsigned int offset, enum machine_mode ymode)
{
  ...
  if (GET_MODE_SIZE (xmode) % GET_MODE_SIZE (ymode)
      || GET_MODE_SIZE (ymode) % nregs_ymode || nregs_xmode % nregs_ymode)
    abort ();
#endif
  ...
  nregs_multiple = nregs_xmode / nregs_ymode;
#ifdef ENABLE_CHECKING
  if (offset % GET_MODE_SIZE (ymode) || mode_multiple % nregs_multiple)
    abort ();
#endif
...
hoist_update_store (rtx insn, rtx * xp, rtx val, rtx new)
{
  ...
  REG_NOTES (insn) = alloc_EXPR_LIST (REG_UNUSED, *xp, REG_NOTES (insn));
}
