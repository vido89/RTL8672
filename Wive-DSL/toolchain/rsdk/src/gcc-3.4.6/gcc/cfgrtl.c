...
delete_insn (rtx insn)
{
  ...
  if (GET_CODE (insn) == JUMP_INSN
      && JUMP_LABEL (insn) && GET_CODE (JUMP_LABEL (insn)) == CODE_LABEL)
...
delete_insn_and_edges (rtx insn)
{
  ...
  if (INSN_P (insn)
      && BLOCK_FOR_INSN (insn) && BB_END (BLOCK_FOR_INSN (insn)) == insn)
    purge = true;
  x = delete_insn (insn);
  if (purge)
...
delete_insn_chain_and_edges (rtx first, rtx last)
{
  ...
  if (INSN_P (last)
      && BLOCK_FOR_INSN (last) && BB_END (BLOCK_FOR_INSN (last)) == last)
    purge = true;
...
basic_block
create_basic_block_structure (rtx head, rtx end, rtx bb_note,
			      basic_block after)
{
  basic_block bb;
  ...
  if (bb_note
      ...
      && (bb = NOTE_BASIC_BLOCK (bb_note)) != NULL && bb->aux == NULL)
    {
  ...
  if (bb->global_live_at_start)
    {
      ...
      COPY_REG_SET (bb->global_live_at_end, new_bb->global_live_at_start);
...
rtl_can_merge_blocks (basic_block a, basic_block b)
{
  ...
  return (a->succ && !a->succ->succ_next && a->succ->dest == b
	  ...
	  && a->next_bb == b && a != ENTRY_BLOCK_PTR && b != EXIT_BLOCK_PTR
...
redirect_branch_edge (edge e, basic_block target)
{
  ...
  if (tablejump_p (insn, NULL, &tmp))
	...
	{
	  XEXP (SET_SRC (tmp), 2) = gen_rtx_LABEL_REF (VOIDmode, new_label);
	  --LABEL_NUSES (old_label);
	  ++LABEL_NUSES (new_label);
	}
    }
...
{
  if (redirect_edge_and_branch (e, target) || e->dest == target)
    return NULL;
...
tidy_fallthru_edge (edge e, basic_block b, basic_block c)
{
  ...
  if (GET_CODE (q) == JUMP_INSN
      && onlyjump_p (q)
      && (any_uncondjump_p (q) || (b->succ == e && e->succ_next == NULL)))
    {
#ifdef HAVE_cc0
      /* If this was a conditional jump, we need to also delete
         the insn that set cc0.  */
...
tidy_fallthru_edges (void)
{
  ...
  FOR_BB_BETWEEN (b, ENTRY_BLOCK_PTR->next_bb, EXIT_BLOCK_PTR->prev_bb,
		  next_bb)
  {
    edge s;
    ...
    if ((s = b->succ) != NULL
	&& !(s->flags & EDGE_COMPLEX) && s->succ_next == NULL && s->dest == c
	/* If the jump insn has side effects, we can't tidy the edge.  */
	&& (GET_CODE (BB_END (b)) != JUMP_INSN || onlyjump_p (BB_END (b))))
...
commit_one_edge_insertion (edge e, int watch_calls)
{
  ...
  if (watch_calls && (e->flags & EDGE_FALLTHRU) && !e->dest->pred->pred_next
      && e->src != ENTRY_BLOCK_PTR && GET_CODE (BB_END (e->src)) == CALL_INSN)
    {
      rtx next = next_nonnote_insn (BB_END (e->src));

      after = BB_HEAD (e->dest);
      /* The first insn after the call may be a stack pop, skip it.  */
      while (next && keep_with_call_p (next))
	{
	  after = next;
...
print_rtl_with_bb (FILE * outf, rtx rtx_first)
{
  ...
  if (rtx_first == 0)
	...
	for (x = BB_HEAD (bb); x != NULL_RTX; x = NEXT_INSN (x))
      ...
      for (tmp_rtx = rtx_first; NULL != tmp_rtx;
	   tmp_rtx = NEXT_INSN (tmp_rtx))
	{
	  ...
	  if (in_bb_p[INSN_UID (tmp_rtx)] == NOT_IN_BB
	      && GET_CODE (tmp_rtx) != NOTE && GET_CODE (tmp_rtx) != BARRIER)
...
rtl_verify_flow_info_1 (void)
{
  ...
  FOR_EACH_BB_REVERSE (bb)
  {
    ...
    if (INSN_P (BB_END (bb))
	&& (note = find_reg_note (BB_END (bb), REG_BR_PROB, NULL_RTX))
	&& bb->succ && bb->succ->succ_next && any_condjump_p (BB_END (bb)))
      {
	...
	if ((e->flags & ~(EDGE_DFS_BACK
			  | EDGE_CAN_FALLTHRU
			  | EDGE_IRREDUCIBLE_LOOP | EDGE_LOOP_EXIT)) == 0)
	  n_branch++;
    ...
    if (n_branch != 1 && any_uncondjump_p (BB_END (bb)))
      {
	error ("Wrong amount of branch edges after unconditional jump %i",
	       bb->index);
	err = 1;
      }
...
rtl_verify_flow_info (void)
{
  ...
  FOR_EACH_BB_REVERSE (bb)
  {
    ...
    else if (e->src != ENTRY_BLOCK_PTR && e->dest != EXIT_BLOCK_PTR)
      {
	rtx insn;
  ...
  if (GET_CODE (insn) == JUMP_INSN)
    {
	  ...
	  && !returnjump_p (insn) && !simplejump_p (insn))
	return purged;
  ...
  if (e->flags & EDGE_FALLTHRU)
    {
	  ...
	  && label_is_jump_target_p (BB_HEAD (e->dest), BB_END (src)))
	{
	  if (rtl_dump_file)
      ...
      if (src->succ->succ_next && !src->succ->succ_next->succ_next)
	{
	  edge s = e->succ_next ? e->succ_next : src->succ;
	  if (s->dest == dest
	      && any_condjump_p (BB_END (src)) && onlyjump_p (BB_END (src)))
...
cfg_layout_merge_blocks (basic_block a, basic_block b)
{
  ...
  /* Otherwise just re-associate the instructions.  */
  else
    {
      ...
      for (insn = BB_HEAD (b);
	   insn != NEXT_INSN (BB_END (b)); insn = NEXT_INSN (insn))
