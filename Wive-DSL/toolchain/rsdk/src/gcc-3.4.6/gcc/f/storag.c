		 ...
		 ffestorag mst)
{
  ...
  for (st = sl->first; st != (ffestorag) & sl->first; st = st->next)
    (*fn) (mst, st);
}
  ...
  if (s->equivs_.first != (ffestorag) & s->equivs_.first)
    {
      ...
      for (sq = s->equivs_.first;
	   sq != (ffestorag) & s->equivs_.first; sq = ffestorag_next_ (sq))
	{
  ...
  switch (ffesymbol_kind (s))
	...
	case FFEINFO_whereLOCAL:
	  ...
	  if (ffesymbol_rank (s) == 0)
	    ...
	    {
	      if (ffebld_op (ffesymbol_arraysize (s)) != FFEBLD_opCONTER)
	   ...
	   list != NULL; list = ffebld_trail (list))
	{
	  ...
	  if (ffesymbol_rank (var) == 0)
	    ...
	    num_elements = ffebld_constant_integerdefault (ffebld_conter
							   (ffesymbol_arraysize
							    (var)));
	  ffetarget_layout (ffesymbol_text (var), &alignment, &modulo, &size,
			    ffesymbol_basictype (var),
			    ffesymbol_kindtype (var), ffesymbol_size (var),
			    num_elements);
	  pad =
	    ffetarget_align (&st->alignment, &st->modulo, st->size, alignment,
			     modulo);
	  if (pad != 0)
...
ffestorag_report (void)
{
  ...
  for (s = ffestorag_list_.first;
       s != (ffestorag) & ffestorag_list_.first; s = s->next)
    {
      if (s->symbol == NULL)
	{
...
ffestorag_update_init (ffestorag s)
{
  ...
  if ((s->symbol != NULL) && !ffesymbol_is_init (s->symbol))
...
ffestorag_update_save (ffestorag s)
{
  ...
  if ((s->symbol != NULL) && !ffesymbol_is_save (s->symbol))
