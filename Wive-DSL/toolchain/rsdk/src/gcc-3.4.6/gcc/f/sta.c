	...
	{			/* At end of both lists!  Pick confirmed or
	  ...
	  if (ffesta_confirmed_possible_ == NULL)
	      ...
	      for (;;)
		{
		  ...
		  if (first == NULL)
		    first = possible;
		  if (possible->named && (first_named == NULL))
		...
		ffesta_current_possible_ = first_named;
	      else if (ffesta_seen_first_exec && (first_exec != NULL))
...
ffesta_first (ffelexToken t)
{
  switch (ffelex_token_type (t))
    ...
    case FFELEX_typeNUMBER:
	  ...
	  && !ffe_is_free_form () && ffe_is_pedantic ())
	{
...
void
ffesta_ffebad_1p (ffebad errnum, ffelexToken names_token,
...
void
ffesta_ffebad_2st (ffebad errnum, const char *s, ffelexToken t1,
		   ffelexToken t2)
{
  if (ffesta_ffebad_start (errnum))
    {
      ffebad_string (s);
      ffebad_here (0, ffelex_token_where_line (t1),
		   ffelex_token_where_column (t1));
      ffebad_here (1, ffelex_token_where_line (t2),
...
ffesta_zero (ffelexToken t)
{
  ...
  if (!ffesta_is_inhibited_ && ffesta_seen_first_exec)
    {
      ffestc_any ();
    }
