...
main (int argc, char **argv)
{
  ...
  while (TRUE)
      ...
      for (i = 0; i < newname->namelen; ++i)
	{
      ...
      if ((last_buf[0] != '\0') && (strcmp (last_buf, newname->name_uc) >= 0))
	{
...
", len);
    ...
    case %d:\n\
", len);
	      testname (FALSE, do_names ? 10 : 6, names[len].first,
			names[len].last);
	      if (do_names)
		fprintf (out, "\
\t  break;\n\
...
", out);
      ...
      for (len = ARRAY_SIZE (names) - 1;
	   names[len].first == (name) & names[len]; --len)
	;
...
%sreturn %s%s%s;\n\
", xspaces[indent], nhalf->name_uc, nhalf->name_lc, nhalf->name_ic, xspaces[indent + 2], prefix, nhalf->kwname, postfix);
...
%sreturn %s%s%s;\n\
", xspaces[indent], nhalf->name_uc, nhalf->name_lc, nhalf->name_ic, len, xspaces[indent + 2], prefix, nhalf->kwname, postfix);
