...
static
GTY (())
     ffewhereFile ffelex_current_wf_;
...
/* Information on the pending INCLUDE file.  */
     ...
     static GTY (())
     ffewhereFile ffelex_include_wherefile_;
...
#define wide_flag 0
  ...
  switch (state)
    {
    case 0:
      if ((c == '\\') && (ffelex_raw_mode_ != 0) && ffe_is_backslash ())
	{
	  state = 1;
	  column = col + 1;
	...
	case '%':
	  if (pedantic)
		...
		("Non-ISO-C-standard escape sequence `\\%A' at %0",
	...
	default:
	  if (c >= 040 && c < 0177)
	  ...
	  else
	    {
	      ...
	      /* xgettext:no-c-format */
	      ffebad_start_msg_lex
		("Unknown escape sequence `\\' followed by char code 0x%A at %0",
		 FFEBAD_severityPEDANTIC);
	      ffelex_bad_here_ (0, line, column);
	      ffebad_string (m);
	      ffebad_finish ();
  ...
  if (!wide_flag
      ...
      && code >= (1 << TYPE_PRECISION (char_type_node)))
    {
      /* xgettext:no-c-format */
      ffebad_start_msg_lex
	("Escape sequence at %0 out of range for character",
	 FFEBAD_severityFATAL);
      ffelex_bad_here_ (0, line, column);
      ffebad_finish ();
    }
...
static void
ffelex_bad_1_ (ffebad errnum, ffewhereLineNumber ln0,
	       ffewhereColumnNumber cn0)
{
  ffewhereLine wl0;
  ffewhereColumn wc0;
...
static void
ffelex_bad_2_ (ffebad errnum, ffewhereLineNumber ln0,
	       ffewhereColumnNumber cn0, ffewhereLineNumber ln1,
	       ffewhereColumnNumber cn1)
{
...
static void
ffelex_bad_here_ (int n, ffewhereLineNumber ln0, ffewhereColumnNumber cn0)
{
  ffewhereLine wl0;
  ffewhereColumn wc0;
      ...
      else if (c == 'i')
	{
	  if (getc (finput) == 'd'
	      ...
	      if (!flag_no_ident)
		{
		  ...
		  ASM_OUTPUT_IDENT (asm_out_file, ffelex_token_text (token));
#endif
		}
...
linenum:
  ...
  if ((token != NULL) && (ffelex_token_type (token) == FFELEX_typeNUMBER))
    {
  ...
  if (card_length != 0)
    {
      card_image = malloc_new_ks (malloc_pool_image (),
				  "FFELEX saved card image", card_length);
...
ffelex_display_token (ffelexToken t)
{
  ...
  fprintf (dmpout,
	   "; Token #%lu is %s (line %" ffewhereLineNumber_f "u, col %"
	   ffewhereColumnNumber_f "u)", t->id_, ffelex_type_string_ (t->type),
...
first_line:			/* :::::::::::::::::::: */
  ...
  while (((lextype = ffelex_first_char_[c]) == FFELEX_typeCOMMENT)
	 ...
	 || (lextype == FFELEX_typeSLASH) || (lextype == FFELEX_typeHASH))
    {
      /* Test most frequent type of line first, etc.  */
      if ((lextype == FFELEX_typeCOMMENT) || ((lextype == FFELEX_typeSLASH) && ((c = getc (f)) == '*')))	/* NOTE SIDE-EFFECT. */
	{
  ...
  ffelex_saw_tab_ = (c == '&') || (ffelex_final_nontab_column_ == 0);
  ...
  for (column = 0; column < 5; ++column)
	...
	case '&':
	  ...
	  if (ffe_is_pedantic ())
	    ffelex_bad_1_ (FFEBAD_AMPERSAND, ffelex_linecount_current_, 1);
...
stop_looking:			/* :::::::::::::::::::: */
  ...
  if (have_content || (first_label_char != FFEWHERE_columnUNKNOWN))
    {
...
ffelex_hash_kludge (FILE * finput)
{
  ...
  for (p = &match[0], q = &kludge[0], c = getc (finput);
       (c == *p) && (*p != '\0') && (c != EOF); ++p, ++q, c = getc (finput))
    *q = c;
...
ffelex_init_1 (void)
{
  ...
  if ((ffe_case_match () == FFE_caseNONE) || ((ffe_case_match () == FFE_caseUPPER) && (ffe_case_source () != FFE_caseLOWER))	/* Idiot!  :-) */
  ...
  if ((ffe_case_match () == FFE_caseNONE) || ((ffe_case_match () == FFE_caseLOWER) && (ffe_case_source () != FFE_caseUPPER))	/* Idiot!  :-) */
