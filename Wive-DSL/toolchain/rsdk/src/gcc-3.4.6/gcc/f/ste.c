...
ffeste_begin_iterdo_ (ffestw block, tree * xtvar, tree * xtincr,
		      ...
		      ffebld start, ffelexToken start_token,
		      ffebld end, ffelexToken end_token,
		      ffebld incr, ffelexToken incr_token, const char *msg)
{
  tree tvar;
  tree expr;
  tree tstart;
  ...
  if (TREE_CODE (tvar) == ERROR_MARK || TREE_CODE (tincr) == ERROR_MARK)
    {
      if (block)
	{
  ...
  if (TREE_CODE (tstart) == ERROR_MARK || TREE_CODE (tend) == ERROR_MARK)
    {
      ffeste_end_stmt_ ();
  ...
  /* For warnings only, nothing else happens here.  */
  {
    ...
    if (!ffe_is_onetrip ())
      {
	try = ffecom_2 (MINUS_EXPR, TREE_TYPE (tvar), tend, tstart);

	try = ffecom_2 (PLUS_EXPR, TREE_TYPE (tvar), try, tincr);

	if (TREE_CODE (TREE_TYPE (tvar)) != REAL_TYPE)
	  try = ffecom_2 (TRUNC_DIV_EXPR, integer_type_node, try, tincr);
	else
	  try = convert (integer_type_node,
			 ffecom_2 (RDIV_EXPR, TREE_TYPE (tvar), try, tincr));
    ...
    try = ffecom_2 (PLUS_EXPR, TREE_TYPE (tvar), tend, tincr);
  ...
  expr = ffecom_2 (MINUS_EXPR, TREE_TYPE (tvar), tend, tstart);

  if (!ffe_is_onetrip ())
    {
      expr = ffecom_2 (PLUS_EXPR, TREE_TYPE (expr),
		       expr, convert (TREE_TYPE (expr), tincr_saved));
    }

  if (TREE_CODE (TREE_TYPE (tvar)) != REAL_TYPE)
    expr = ffecom_2 (TRUNC_DIV_EXPR, TREE_TYPE (expr), expr, tincr_saved);
  else
    expr = ffecom_2 (RDIV_EXPR, TREE_TYPE (expr), expr, tincr_saved);
  ...
  if (!ffe_is_onetrip ())
	...
	(ffecom_2 (GE_EXPR, integer_type_node,
		   ...
		   convert (TREE_TYPE (niters), ffecom_integer_zero_node)));
...
ffeste_end_iterdo_ (ffestw block, tree tvar, tree tincr, tree itersvar)
{
  ...
  expr = ffecom_modify (void_type_node, tvar,
			ffecom_2 (PLUS_EXPR, TREE_TYPE (tvar), tvar, tincr));
  expand_expr_stmt (expr);
...
ffeste_io_impdo_ (ffebld impdo, ffelexToken impdo_token)
{
  ...
  ffeste_begin_iterdo_ (NULL, &tvar, &tincr, &titervar, var,
			...
			incr, impdo_token, "Implied DO loop");
...
ffeste_io_dofio_ (ffebld expr)
{
  ...
  if ((bt == FFEINFO_basictypeANY) || (kt == FFEINFO_kindtypeANY))
  ...
  if ((variable == error_mark_node) || (size == error_mark_node))
    return error_mark_node;
  ...
  if (ffeinfo_rank (ffebld_info (expr)) == 0
	...
	= size_binop (CEIL_DIV_EXPR,
      ...
      num_elements = size_binop (CEIL_DIV_EXPR, num_elements,
				 size_int (TYPE_PRECISION (char_type_node)
					   / BITS_PER_UNIT));
      num_elements = convert (ffecom_f2c_ftnlen_type_node, num_elements);
    }

  num_elements
    = ffecom_1 (ADDR_EXPR, ffecom_f2c_ptr_to_ftnlen_type_node, num_elements);
...
ffeste_io_dolio_ (ffebld expr)
{
  ...
  type_id
    = ffecom_1 (ADDR_EXPR, ffecom_f2c_ptr_to_ftnint_type_node,
		convert (ffecom_f2c_ftnint_type_node, type_id));

  variable = ffecom_arg_ptr_to_expr (expr, &size);

  if ((type_id == error_mark_node)
      || (variable == error_mark_node) || (size == error_mark_node))
    return error_mark_node;
...
static
GTY (())
     tree f2c_alist_struct;
     static tree
       ffeste_io_ialist_ (bool have_err,
			  ffestvUnit unit, ffebld unit_expr, int unit_dflt)
{
  tree t;
  tree ttype;
  tree field;
...
GTY (())
     ...
     static tree
       ffeste_io_cilist_ (bool have_err,
			  ...
			  bool have_end,
			  ffestvFormat format,
			  ffestpFile * format_spec, bool rec, ffebld rec_expr)
{
  tree t;
  tree ttype;
  tree field;
  ...
  switch (format)
    ...
    case FFESTV_formatLABEL:
      ...
      if ((formatinit == NULL_TREE) || (TREE_CODE (formatinit) == ERROR_MARK))
	break;
      formatinit = ffecom_1 (ADDR_EXPR,
			     build_pointer_type (void_type_node), formatinit);
  ...
  inits =
    build_tree_list ((field = TYPE_FIELDS (f2c_cilist_struct)), errinit);
  initn = inits;
  ffeste_f2c_init_next_ (unitinit);
...
static
GTY (())
     tree f2c_close_struct;
     static tree
       ffeste_io_cllist_ (bool have_err,
			  ffebld unit_expr, ffestpFile * stat_spec)
{
  tree t;
  tree ttype;
  tree field;
...
static
GTY (())
     tree f2c_icilist_struct;
     static tree
       ffeste_io_icilist_ (bool have_err,
			   ffebld unit_expr,
			   bool have_end,
			   ffestvFormat format, ffestpFile * format_spec)
{
  tree t;
  tree ttype;
  tree field;
  ...
  if (f2c_icilist_struct == NULL_TREE)
    {
      ...
      errfield = ffecom_decl_field (ref, NULL_TREE, "err",
				    ffecom_f2c_flag_type_node);
      unitfield = ffecom_decl_field (ref, errfield, "unit", string_type_node);
      endfield = ffecom_decl_field (ref, unitfield, "end",
				    ffecom_f2c_flag_type_node);
      formatfield = ffecom_decl_field (ref, endfield, "format",
				       string_type_node);
  ...
  if (!unitnumexp
      && unitexp != error_mark_node && unitlenexp != error_mark_node)
    {
      unitnumexp
	= size_binop (CEIL_DIV_EXPR,
		      TYPE_SIZE_UNIT (TREE_TYPE (TREE_TYPE (unitexp))),
...
GTY (())
     ...
     static tree
       ffeste_io_inlist_ (bool have_err,
			  ...
			  ffestpFile * unformatted_spec,
			  ffestpFile * recl_spec,
			  ffestpFile * nextrec_spec, ffestpFile * blank_spec)
{
  tree t;
  tree ttype;
  tree field;
  ...
  static tree errfield, unitfield, filefield, filelenfield, existfield,
    ...
    directlenfield, formfield, formlenfield, formattedfield,
    formattedlenfield, unformattedfield, unformattedlenfield, reclfield,
    nextrecfield, blankfield, blanklenfield;
  tree errinit, unitinit, fileinit, fileleninit, existinit, openinit,
    numberinit, namedinit, nameinit, nameleninit, accessinit, accessleninit,
    sequentialinit, sequentialleninit, directinit, directleninit, forminit,
    formleninit, formattedinit, formattedleninit, unformattedinit,
    unformattedleninit, reclinit, nextrecinit, blankinit, blankleninit;
  tree unitexp, fileexp, filelenexp, existexp, openexp, numberexp, namedexp,
    nameexp, namelenexp, accessexp, accesslenexp, sequentialexp,
    sequentiallenexp, directexp, directlenexp, formexp, formlenexp,
    formattedexp, formattedlenexp, unformattedexp, unformattedlenexp, reclexp,
    nextrecexp, blankexp, blanklenexp;
  static int mynumber = 0;
  ...
  ffeste_f2c_compile_char_ (sequentialfield, sequentiallenfield,
			    sequential_spec, sequentialexp, sequentiallenexp);
...
GTY (())
     ...
     static tree
       ffeste_io_olist_ (bool have_err,
			 ...
			 ffestpFile * access_spec,
			 ffestpFile * form_spec,
			 ffestpFile * recl_spec, ffestpFile * blank_spec)
{
  tree t;
  tree ttype;
  tree field;
  ...
  if (errl)
    {
      ...
      ffeste_io_err_
	= ffeste_io_abort_
	= ffecom_lookup_label (info->beru_spec[FFESTP_beruixERR].u.label);
...
ffeste_R804 (ffestw block, ffebld expr)
{
  ...
  ffestw_set_ifthen_fake_else (block, ffestw_ifthen_fake_else (block) + 1);
}
...
ffeste_R809 (ffestw block, ffebld expr)
{
  ...
  if ((expr == NULL)
      || (ffeinfo_basictype (ffebld_info (expr)) == FFEINFO_basictypeANY))
	   ...
	   == FFEINFO_basictypeCHARACTER)
    {
      ...
      ffebad_start_msg
	("SELECT CASE on CHARACTER type (at %0) not supported -- sorry",
  ...
  else
    {
      ...
      result = ffecom_make_tempvar ("select", ffecom_type_expr (expr),
				    ffeinfo_size (ffebld_info (expr)), -1);
      ...
      texpr = ffecom_modify (void_type_node, result, texpr);
...
ffeste_R810 (ffestw block, unsigned long casenum)
{
  ...
  if ((c == NULL) || (casenum != c->casenum))
	...
	if (c->low != c->high)
	  {
	    texprhigh = (c->high == NULL) ? NULL_TREE
	      :
	      ffecom_constantunion_with_type (&ffebld_constant_union
					      (c->high),
					      ffecom_tree_type[s->type][s->
									kindtype],
					      c->high->consttype);
	    pushok =
	      pushcase_range (texprlow, texprhigh, convert, tlabel,
			      &duplicate);
	  }
	else
	...
	if (pushok == 2)
	  {
	    ffebad_start_msg
	      ("SELECT (at %0) has duplicate cases -- check integer overflow of CASE(s)",
	       FFEBAD_severityFATAL);
	    ffebad_here (0, ffestw_line (block), ffestw_col (block));
	    ffebad_finish ();
	    ffestw_set_select_texpr (block, error_mark_node);
	      ...
	      ffebld incr, ffelexToken incr_token)
{
  ...
  ffeste_begin_iterdo_ (block, NULL, NULL, NULL,
			...
			incr, incr_token, "Iterative DO loop");
}
  ...
  if (expr)
    {
      ...
      mod = ffecom_modify (void_type_node,
			   result, ffecom_truth_value (ffecom_expr (expr)));
...
ffeste_R836 (ffelab label)
{
  ...
  glabel = ffecom_lookup_label (label);
  if ((glabel != NULL_TREE) && (TREE_CODE (glabel) != ERROR_MARK))
    {
      expand_goto (glabel);
      TREE_USED (glabel) = 1;
    }
  ...
  for (i = 0; i < count; ++i)
    {
      ...
      if ((tlabel == NULL_TREE) || (TREE_CODE (tlabel) == ERROR_MARK))
	continue;
...
ffeste_R838 (ffelab label, ffebld target)
{
  ...
  if ((label_tree != NULL_TREE) && (TREE_CODE (label_tree) != ERROR_MARK))
    {
      label_tree = ffecom_1 (ADDR_EXPR,
			     build_pointer_type (void_type_node), label_tree);
      ...
      if (TREE_CODE (target_tree) != ERROR_MARK)
	{
	  ...
	  expr_tree = ffecom_modify (void_type_node, target_tree, label_tree);
  ...
  if (neg == zero)
      ...
      else
	{
	  ...
	  texpr = ffecom_2 (LE_EXPR, integer_type_node,
			    texpr,
			    convert (TREE_TYPE (texpr), integer_zero_node));
  ...
  else
    {
      ...
      texpr = ffecom_2 (LT_EXPR, integer_type_node,
			expr_saved,
			convert (TREE_TYPE (expr_saved), integer_zero_node));
      expand_start_cond (ffecom_truth_value (texpr), 0);
      expand_goto (gneg);
...
ffeste_R842 (ffebld expr)
{
  ...
  callit = ffecom_call_gfrt (FFECOM_gfrtSTOP,
			     ffecom_list_ptr_to_expr (ffebld_new_item
						      (expr, NULL)),
			     NULL_TREE);
...
ffeste_R843 (ffebld expr)
{
  ...
  if ((expr == NULL)
	...
	ffelex_token_new_character (num,
				    ffelex_token_where_line (ffesta_tokens
							     [0]),
				    ffelex_token_where_column (ffesta_tokens
							       [0]));
      expr = ffebld_new_conter (ffebld_constant_new_characterdefault (msg));
      ffelex_token_kill (msg);
      ffebld_set_info (expr, ffeinfo_new (FFEINFO_basictypeCHARACTER,
  ...
  if (errl)
    {
      ffeste_io_err_
	= ffeste_io_abort_
	= ffecom_lookup_label (info->open_spec[FFESTP_openixERR].u.label);
      ffeste_io_abort_is_temp_ = FALSE;
    }
  else
    {
  ...
  if (errl)
    {
      ffeste_io_err_
	= ffeste_io_abort_
	= ffecom_lookup_label (info->close_spec[FFESTP_closeixERR].u.label);
      ffeste_io_abort_is_temp_ = FALSE;
    }
  else
    {
...
ffeste_R909_finish (void)
{
  ...
  if (ffeste_io_endgfrt_ != FFECOM_gfrt)
    ffeste_io_call_ (ffecom_call_gfrt (ffeste_io_endgfrt_, NULL_TREE,
				       NULL_TREE), !ffeste_io_abort_is_temp_);
  ...
  if (errl)
    {
      ...
      ffeste_io_err_
	= ffeste_io_abort_
	= ffecom_lookup_label (info->write_spec[FFESTP_writeixERR].u.label);
      ffeste_io_abort_is_temp_ = FALSE;
    }
  else
    {
  ...
  if (labels == NULL)
  ...
  else
    {
      ...
      for (caseno = 1, label = labels;
	   label != NULL; ++caseno, label = ffebld_trail (label))
	{
	  ...
	  tlabel = ffecom_lookup_label (ffebld_labter (ffebld_head (label)));
	  if ((tlabel == NULL_TREE) || (TREE_CODE (tlabel) == ERROR_MARK))
	    continue;
	  TREE_USED (tlabel) = 1;
	  expand_goto (tlabel);
...
ffeste_R1227 (ffestw block UNUSED, ffebld expr)
{
  ...
  if ((rtn == NULL_TREE) || (rtn == error_mark_node))
    expand_null_return ();
  else
    {
...
void
ffeste_V020_start (ffestpTypeStmt * info UNUSED, ffestvFormat format UNUSED)
{
  ffeste_check_start_ ();
}
