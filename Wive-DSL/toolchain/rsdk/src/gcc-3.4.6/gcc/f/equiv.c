...
ffeequiv_destroy_ (ffeequiv victim)
{
  ...
  for (list = victim->list; list != NULL; list = ffebld_trail (list))
    {
      for (item = ffebld_head (list); item != NULL;
...
ffeequiv_layout_local_ (ffeequiv eq)
{
  ...
  for (list = ffeequiv_list (eq); list != NULL; list = ffebld_trail (list))
	...
	{			/* For every equivalence list in the list of
	  ...
	  for (item = ffebld_head (list);
		  ...
		  || ((rooted_st = ffesymbol_storage (rooted_sym)) == NULL))
		{
	      ...
	      if (!ffeequiv_offset_
		  (&eqlist_offset, rooted_sym, rooted_exp, FALSE,
	  ...
	  for (item = ffebld_head (list);
	    ...
	    {			/* For every equivalence item in the list */
	      ...
	      if ((item_sym == NULL) || (ffesymbol_equiv (item_sym) == NULL))
		continue;	/* Ignore me. */
		...
		num_elements = ffebld_constant_integerdefault (ffebld_conter
							       (ffesymbol_arraysize
								(item_sym)));
	      ffetarget_layout (ffesymbol_text (item_sym), &alignment,
				&modulo, &size,
				ffesymbol_basictype (item_sym),
				ffesymbol_kindtype (item_sym),
				ffesymbol_size (item_sym), num_elements);
	      pad =
		ffetarget_align (ffestorag_ptr_to_alignment (st),
				 ffestorag_ptr_to_modulo (st), item_offset,
				 alignment, modulo);
	      if (pad != 0)
		{
		...
		{		/* Create new ffestorag object, extend equiv
		  ...
		  ffestorag_set_basictype (item_st,
					   ffesymbol_basictype (item_sym));
		  ffestorag_set_kindtype (item_st,
					  ffesymbol_kindtype (item_sym));
		  ...
		  if (!ffetarget_offset_add (&size, item_offset, size)
		      || !ffetarget_offset_add (&size, -ffestorag_offset (st),
						size))
		    ffetarget_offset_overflow (ffesymbol_text (s));
		  else if (size > ffestorag_size (st))
		    ffestorag_set_size (st, size);
		  ffestorag_update (st, item_sym,
				    ffesymbol_basictype (item_sym),
				    ffesymbol_kindtype (item_sym));
		}
	      else
		  ...
		  if (item_offset != ffestorag_offset (item_st))
		    {
		      ...
		      sprintf (&io1[0], "%" ffetargetOffset_f "d",
			       item_offset);
		      sprintf (&io2[0], "%" ffetargetOffset_f "d",
			       ffestorag_offset (item_st));
  ...
  if (ffestorag_offset (st) < 0)
    {
      ...
      pad = ffetarget_align (&alignment, &modulo,
			     -ffestorag_offset (st), alignment, 0);
  ...
  switch (ffebld_op (expr))
	...
	while (subscripts != NULL)
	  {
	    ...
	    highbound = ffebld_constant_integerdefault (ffebld_conter (high));
	...
	if (begin == NULL)
	...
	else
	  {
	    ...
	    if ((value < 1)
		|| ((sym != NULL) && (value > ffesymbol_size (sym))))
	      {
...
ffeequiv_add (ffeequiv eq, ffebld list, ffelexToken t)
{
  ...
  if ((common != NULL) && (ffeequiv_common (eq) == NULL))	/* Is COMMON involved already? */
	  ...
	  for (item = ffeequiv_list (seq);	/* Get list of equivs. */
	    ...
	    {			/* For every eqv list in the list of equivs
	      ...
	      for (root = ffebld_head (item);
		   root != NULL; root = ffebld_trail (root))
		{		/* For every equivalence item in the list */
	      ...
	      else if (altroot != NULL)
		{
		  ...
		  ok = ffeequiv_offset_ (&root_offset, altrootsym, root,
					 FALSE,
					 ffestorag_offset (ffesymbol_storage
							   (altrootsym)),
					 TRUE);
...
ffeequiv_update_init (ffeequiv eq)
{
  ...
  if ((eq->common != NULL) && !ffesymbol_is_init (eq->common))
...
ffeequiv_update_save (ffeequiv eq)
{
  ...
  if ((eq->common != NULL) && !ffesymbol_is_save (eq->common))
