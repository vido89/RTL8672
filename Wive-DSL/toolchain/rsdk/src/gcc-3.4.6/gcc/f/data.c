...
static ffetargetOffset ffedata_eval_offset_ (ffebld subscripts, ffebld dims);
...
ffedata_value (ffetargetIntegerDefault rpt, ffebld value, ffelexToken token)
{
  ...
  if (!ffe_is_zeros ()
    ...
    value = NULL;
  else if ((value != NULL) && (ffebld_op (value) == FFEBLD_opANY))
    value = NULL;
  else
    {
      /* Must be a constant. */
  ...
  for (i = 0; i < rpt; ++i)
    {
      if ((ffedata_symbol_ != NULL) && !ffesymbol_is_init (ffedata_symbol_))
	{
...
tail_recurse:			/* :::::::::::::::::::: */
  ...
  if (ffedata_list_ == NULL)
	...
	+ ffedata_stack_->increment;
      ...
      if (((ffedata_stack_->increment > 0) ? newval > ffedata_stack_->final : newval < ffedata_stack_->final) || (((ffesymbol_value (ffedata_stack_->itervar) < 0) == (ffedata_stack_->increment < 0)) && ((ffesymbol_value (ffedata_stack_->itervar) < 0) != (newval < 0))))	/* Overflow/underflow? */
	{			/* Done with the loop. */
  ...
  switch (ffebld_op (next))
	...
	{
	  ffetarget_aggregate_info (&ffedata_storage_bt_,
				    &ffedata_storage_kt_,
				    &ffedata_storage_units_,
				    ffestorag_basictype (ffedata_storage_),
				    ffestorag_kindtype (ffedata_storage_));
	  ffedata_storage_size_ =
	    ffestorag_size (ffedata_storage_) / ffedata_storage_units_;
	  assert (ffestorag_size (ffedata_storage_) %
		  ffedata_storage_units_ == 0);
	}
      ...
      else
	{
      ...
      ffedata_size_ =
	(ffedata_basictype_ ==
	 FFEINFO_basictypeCHARACTER) ? ffesymbol_size (ffedata_symbol_) : 1;
      ffedata_symbolsize_ = ffedata_size_ * ffedata_arraysize_;
      ffedata_charexpected_ = ffedata_size_;
      ffedata_charnumber_ = 0;
      ffedata_charoffset_ = 0;
	...
	ffedata_charexpected_ =
	  ffedata_eval_substr_end_ (ffebld_head (ffebld_trail (colon)),
				    ffedata_charoffset_,
	...
	if (ffeinfo_basictype (ffebld_info (start)) !=
	    FFEINFO_basictypeINTEGER)
	  {
	...
	if (incr == NULL)
	  ...
	  {
	    if (ffeinfo_basictype (ffebld_info (incr)) !=
		FFEINFO_basictypeINTEGER)
	      {
		ffebad_start (FFEBAD_DATA_EVAL);
		ffest_ffebad_here_current_stmt (0);
  ...
  switch (ffebld_op (expr))
    ...
    case FFEBLD_opUMINUS:
      error = ffetarget_uminus_integer1 (&result,
					 ffedata_eval_integer1_ (ffebld_left
								 (expr)));
      break;
    ...
    case FFEBLD_opADD:
      error = ffetarget_add_integer1 (&result,
				      ffedata_eval_integer1_ (ffebld_left
							      (expr)),
				      ffedata_eval_integer1_ (ffebld_right
							      (expr)));
      break;
  ...
  while (subscripts != NULL)
	  ...
	  sub1 =
	    ffebld_new_conter_with_orig (ffebld_constant_new_integer1_val
					 (sub_kind ==
					  ...
					  subscript->u.conter.expr->u.
					  integer4 : subscript->u.conter.
					  expr->u.integer1), NULL);
      ...
      else
	{
	  ...
	  if (ffebld_op (low) == FFEBLD_opCONTER)
		...
		ffebld_new_conter_with_orig (ffebld_constant_new_integer1_val
					     (low_kind ==
					      FFEINFO_kindtypeINTEGER2 ? low->
					      u.conter.expr->u.
					      integer2 : low_kind ==
					      FFEINFO_kindtypeINTEGER3 ? low->
					      u.conter.expr->u.
					      integer3 : low_kind ==
					      FFEINFO_kindtypeINTEGER4 ? low->
					      u.conter.expr->u.
					      integer4 : low->u.conter.expr->
					      u.integer1), NULL);
      ...
      if (ffebld_op (high) == FFEBLD_opCONTER)
	{
	  ...
	  hi1 =
	    ffebld_new_conter_with_orig (ffebld_constant_new_integer1_val
					 (hi_kind ==
					  FFEINFO_kindtypeINTEGER2 ? high->u.
					  conter.expr->u.integer2 : hi_kind ==
					  FFEINFO_kindtypeINTEGER3 ? high->u.
					  conter.expr->u.integer3 : hi_kind ==
					  FFEINFO_kindtypeINTEGER4 ? high->u.
					  conter.expr->u.integer4 : high->u.
					  conter.expr->u.integer1), NULL);
...
ffedata_gather_ (ffestorag mst, ffestorag st)
{
  ...
  ffedata_charexpected_ = (bt == FFEINFO_basictypeCHARACTER)
    ? ffebld_size (b) : 1;
  ffetarget_aggregate_info (&ign_bt, &ign_kt, &units, bt, kt);	/* Find out unit size of source datum. */
  ...
  if (ffestorag_accretion (mst) == NULL)
	...
	ffebld_new_accter (array,
      ...
      ffebld_set_info (accter,
		       ffeinfo_new (ffedata_storage_bt_, ffedata_storage_kt_,
				    1, FFEINFO_kindENTITY,
				    FFEINFO_whereCONSTANT,
				    (ffedata_basictype_ ==
				     FFEINFO_basictypeCHARACTER) ? 1 :
  ...
  fn =
    ffetarget_aggregate_ptr_memcpy (ffedata_storage_bt_, ffedata_storage_kt_,
				    bt, kt);

  switch (ffebld_op (b))
    {
    case FFEBLD_opCONTER:
      ffebld_constantarray_prepare (&ptr1, &ptr2, &size, array,
				    ffedata_storage_bt_, ffedata_storage_kt_,
				    offset,
				    ffebld_constant_ptr_to_union
				    (ffebld_conter (b)), bt, kt);
      (*fn) (ptr1, ptr2, size);	/* Does the appropriate memcpy-like
				   operation. */
      ffebit_count (ffebld_accter_bits (accter), offset, FALSE, units_expected, &actual);	/* How many FALSE? */
      if (units_expected != (ffetargetOffset) actual)
	{
      ...
      ffestorag_set_accretes (mst, ffestorag_accretes (mst) - actual);	/* Decrement # of values
									   actually accreted. */
    ...
    case FFEBLD_opARRTER:
      ffebld_constantarray_preparray (&ptr1, &ptr2, &size, array,
				      ffedata_storage_bt_,
				      ffedata_storage_kt_, offset,
				      ffebld_arrter (b), bt, kt);
...
ffedata_value_ (ffebld value, ffelexToken token)
{
  ...
  if (ffedata_storage_ != NULL)
      ...
      if (offset + ffedata_charexpected_ - 1 > ffedata_symbolsize_)
	{
      ...
      ffetarget_aggregate_info (&ign_bt, &ign_kt, &units, ffedata_basictype_, ffedata_kindtype_);	/* Find out unit size of
													   source datum. */
      assert (units % ffedata_storage_units_ == 0);
      units_expected = ffedata_charexpected_ * units / ffedata_storage_units_;
      ...
      if (value != NULL)
	{
		 ...
		 ffedata_storage_size_);
	      accter = ffebld_new_accter (array,
					  ffebit_new (ffe_pool_program_unit
						      ...
						      ffedata_storage_size_));
	      ffebld_set_info (accter,
			       ffeinfo_new (ffedata_storage_bt_,
					    ffedata_storage_kt_, 1,
					    FFEINFO_kindENTITY,
					    FFEINFO_whereCONSTANT,
					    (ffedata_basictype_ ==
					     FFEINFO_basictypeCHARACTER) ? 1 :
	  ...
	  ffestorag_set_accretes (ffedata_storage_, ffestorag_accretes (ffedata_storage_) - actual);	/* Decrement # of values
													   actually accreted. */
	  ffebit_set (ffebld_accter_bits (accter), offset, 1, units_expected);
		...
		(ffestorag_init (ffedata_storage_),
		 ffebld_accter (ffestorag_init (ffedata_storage_)));
	      ffebld_arrter_set_size (ffestorag_init (ffedata_storage_),
				      ffedata_storage_size_);
	      ffebld_arrter_set_pad (ffestorag_init (ffedata_storage_), 0);
  ...
  if ((ffedata_number_ != 0)
      ...
      if (value != NULL)
	{
...
#endif
	      array = ffebld_constantarray_new
		(ffedata_basictype_, ffedata_kindtype_, ffedata_symbolsize_);
	      accter = ffebld_new_accter (array,
					  ffebit_new (ffe_pool_program_unit
						      (),
						      ffedata_symbolsize_));
	      ffebld_set_info (accter,
			       ffeinfo_new (ffedata_basictype_,
					    ffedata_kindtype_, 1,
					    FFEINFO_kindENTITY,
					    FFEINFO_whereCONSTANT,
					    (ffedata_basictype_ ==
	  ...
	  ffebit_count (ffebld_accter_bits (accter), offset, FALSE, ffedata_charexpected_, &actual);	/* How many FALSE? */
	  ...
	  ffesymbol_set_accretes (ffedata_symbol_, ffesymbol_accretes (ffedata_symbol_) - actual);	/* Decrement # of values
													   actually accreted. */
      ...
      if (++ffedata_number_ < ffedata_expected_)
	{
      ...
      if ((value != NULL) && (ffesymbol_accretes (ffedata_symbol_) == 0))
	{
