  ...
  if ((g != NULL) && (g->type != FFEGLOBAL_typeNONE))
      ...
      else
	{
	  ...
	  if (ffe_is_globals () || ffe_is_warn_globals ())
	    {
	      ffebad_start (ffe_is_globals ()? FFEBAD_FILEWIDE_ALREADY_SEEN
      ...
      if (g == NULL)
	{
	  ...
	  g->intrinsic = FALSE;
	       ...
	       && !g->explicit_intrinsic && ffe_is_warn_globals ())
	{
...
ffeglobal_new_progunit_ (ffesymbol s, ffelexToken t, ffeglobalType type)
{
  ...
  if ((g != NULL)
	  ...
	  || (g->type == FFEGLOBAL_typeBDATA)) && g->u.proc.defined)
    {
      ...
      if (ffe_is_globals () || ffe_is_warn_globals ())
	{
      ...
      g->type = FFEGLOBAL_typeANY;
	   ...
	   && (g->type != FFEGLOBAL_typeEXT) && (g->type != type))
    {
      ...
      if (ffe_is_globals () || ffe_is_warn_globals ())
	{
	  ffebad_start (ffe_is_globals ()? FFEBAD_FILEWIDE_DISAGREEMENT
		       ...
		       && (ffesymbol_size (s) != g->u.proc.sz))))
	{
      ...
      if (g->intrinsic && !g->explicit_intrinsic && ffe_is_warn_globals ())
	{
		      ...
		      ffewhereColumn wc)
{
  ...
  if (!g->u.common.have_pad)
      ...
      if (pad != 0)
	{
	  ...
	  ffebad_string ((pad == 1) ? FFECOM_SIZE_UNIT : FFECOM_SIZE_UNITS);
	  ffebad_here (0, wl, wc);
	  ffebad_finish ();
	}
    }
...
void
ffeglobal_proc_def_arg (ffesymbol s, int argno, const char *name,
			ffeglobalArgSummary as, ffeinfoBasictype bt,
			ffeinfoKindtype kt, bool array)
{
  ...
  if ((ai->t != NULL) && ffe_is_warn_globals ())
    {
	...
	case FFEGLOBAL_argsummaryREF:
	  if ((ai->as != FFEGLOBAL_argsummaryREF) && (ai->as != FFEGLOBAL_argsummaryNONE) && ((ai->as != FFEGLOBAL_argsummaryDESCR)	/* Choose better message. */
											      ||
											      (ai->
											       bt
											       !=
	...
	case FFEGLOBAL_argsummaryDESCR:
	  if ((ai->as != FFEGLOBAL_argsummaryDESCR) && (ai->as != FFEGLOBAL_argsummaryNONE) && ((ai->as != FFEGLOBAL_argsummaryREF)	/* Choose better message. */
												||
												(bt
												 !=
												 FFEINFO_basictypeCHARACTER)
      ...
      if (warn)
	{
	  ...
	  if (name == NULL)
		...
		sprintf (&num[0], "%d (named `%.*s...')", argno + 1, 30,
			 name);
	    }
	  ...
	  ffebad_here (0, ffelex_token_where_line (g->t),
		       ffelex_token_where_column (g->t));
	  ffebad_here (1, ffelex_token_where_line (ai->t),
		       ffelex_token_where_column (ai->t));
	  ffebad_finish ();
	}
    }
  ...
  if ((as != FFEGLOBAL_argsummaryPROC) || (ai->t == NULL))
  ...
  if (name == NULL)
  ...
  else
    {
      ai->name = malloc_new_ks (malloc_pool_image (),
				"ffeglobalArgInfo_ name", strlen (name) + 1);
      strcpy (ai->name, name);
    }
  ai->bt = bt;
  ai->kt = kt;
...
ffeglobal_ref_intrinsic (ffesymbol s, ffelexToken t, bool explicit)
{
  ...
  if ((g != NULL) && (g->type != FFEGLOBAL_typeNONE))
    {
      if (!explicit && !g->intrinsic && ffe_is_warn_globals ())
	{
      ...
      if (g == NULL)
	{
	  ...
	  g->t = ffelex_token_use (t);
	       ...
	       && (g->tick != ffe_count_2) && ffe_is_warn_globals ())
	{
...
ffeglobal_ref_progunit_ (ffesymbol s, ffelexToken t, ffeglobalType type)
{
  ...
  if ((g != NULL)
      ...
      && (g->type != type) && (type != FFEGLOBAL_typeEXT))
    {
      ...
      if ((((type == FFEGLOBAL_typeBDATA)
	   ...
	   || ((g->type == FFEGLOBAL_typeBDATA)
	       && (type != FFEGLOBAL_typeCOMMON) && !g->u.proc.defined)))
	{
  ...
  if ((g != NULL) && (type == FFEGLOBAL_typeFUNC))
    {
		  ...
		  && g->u.proc.sz != FFETARGET_charactersizeNONE)))
	{
	  ...
	  error = (g->tick != ffe_count_2
		   && g->u.proc.defined && ffe_is_globals ());
  ...
  if ((g->type != type) && (type != FFEGLOBAL_typeEXT))
    {
		       ...
		       ffewhereColumn wc)
{
  ...
  if (!g->u.common.have_save)
      ...
      if ((g->u.common.save != save) && ffe_is_pedantic ())
	{
	  ...
	  ffebad_here (save ? 1 : 0, g->u.common.pad_where_line,
		       g->u.common.pad_where_col);
	  ffebad_finish ();
	}
    }
...
ffeglobal_size_common (ffesymbol s, ffetargetOffset size)
{
  ...
  if ((g->tick > 0) && (g->tick < ffe_count_2) && (g->u.common.size < size))
    {
      char oldsize[40];
      char newsize[40];
      ...
      ffebad_string ((g->u.common.size == 1)
		     ? FFECOM_SIZE_UNIT : FFECOM_SIZE_UNITS);
      ffebad_string ((size == 1) ? FFECOM_SIZE_UNIT : FFECOM_SIZE_UNITS);
