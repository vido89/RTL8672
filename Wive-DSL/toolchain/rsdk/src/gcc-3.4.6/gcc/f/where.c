...
struct _ffewhere_line_ ffewhere_unknown_line_ = { NULL, NULL, 0, 0, 0, {0} };
...
static
GTY (())
     struct _ffewhere_root_ll_ *ffewhere_root_ll_;
     ...
     static ffewhereLL_ ffewhere_ll_lookup_ (ffewhereLineNumber ln)
{
  ...
  for (ll = ffewhere_root_ll_->last;
       ll != (ffewhereLL_) & ffewhere_root_ll_->first; ll = ll->previous)
    {
      if (ll->line_no <= ln)
	return ll;
    }
...
ffewhere_line_new (ffewhereLineNumber ln)
{
  ...
  if (((ln == 0)		/* Presumably asking for EOF pointer. */
       || (wl->line_num != ln)) && (ffelex_line_number () == ln))
    {
#if 0
      fprintf (dmpout,
...
#if 0
      fprintf (dmpout,
	       "; ffewhere_line_new %" ffewhereLineNumber_f "u, uses=%"
	       ffewhereUses_f_ "u\n", ln, wl->uses);
#endif
      wl->uses++;
      return wl;
  ...
  wl = malloc_new_ks (ffe_pool_file (), "FFEWHERE line",
		      offsetof (struct _ffewhere_line_, content) +1);
  wl->next = (ffewhereLine) & ffewhere_root_line_;
  wl->previous = ffewhere_root_line_.last;
  wl->previous->next = wl;
  wl->next->previous = wl;
...
#if 0
  fprintf (dmpout,
	   "; ffewhere_line_use %" ffewhereLineNumber_f "u, uses=%"
	   ffewhereUses_f_ "u\n", wl->line_num, wl->uses);
#endif
  assert (ffewhere_line_is_unknown (wl) || (wl->uses != 0));
  if (!ffewhere_line_is_unknown (wl))
...
void
ffewhere_track_copy (ffewhereTrack dwt, ffewhereTrack swt,
