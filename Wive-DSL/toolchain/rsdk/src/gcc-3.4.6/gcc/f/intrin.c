...
static const struct _ffeintrin_name_ ffeintrin_names_[] = {	/* Alpha order. */
...
ffeintrin_check_ (ffeintrinImp imp, ffebldOp op,
		  ...
		  ffeinfoKindtype * xkt,
		  ffetargetCharacterSize * xsz,
		  bool * check_intrin, ffelexToken t, bool commit)
{
  ...
  for (argc = &c[colon + 3], arg = args; *argc != '\0';)
      ...
      do
	{
	  ...
	  if ((arg == NULL) || (ffebld_head (arg) == NULL))
	    {
	      if (required != '\0')
	  ...
	  switch (kind)
		...
		{
		  okay &= anynum
		    || (ffeinfo_kindtype (i) == FFEINFO_kindtypeINTEGER1
		...
		{
		  okay &= anynum
		    || (ffeinfo_kindtype (i) == FFEINFO_kindtypeLOGICAL1
	  ...
	  switch (elements)
	    ...
	    default:
		  ...
		  || (ffeinfo_kindtype (ffebld_info (b)) !=
		      FFEINFO_kindtypeINTEGERDEFAULT)
		  || (ffebld_constant_integer1 (ffebld_conter (b)) !=
		      elements))
		okay = FALSE;
	      break;
	    }
	  ...
	  if (anynum)
	    {
		  ...
		  && (akt != FFEINFO_kindtypeNONE) && commit)
		{
  ...
  if (need_col || c[colon + 1] != '-')
    {
      ...
      for (arg = args, arg_count = 0;
	   arg != NULL; arg = ffebld_trail (arg), arg_count++)
	{
  ...
  for (argno = 0, argc = &c[colon + 3], arg = args; *argc != '\0'; ++argno)
    {
      char optional = '\0';
      char required = '\0';
      char extra = '\0';
      ...
      do
	{
	  ...
	  if (anynum && commit)
	  ...
	  else if ((c[colon + 1] == '*') && commit)
	    {
	      ...
	      a = ffeexpr_convert (a, t, NULL,
				   col_bt, col_kt, 0,
				   ffeinfo_size (i), FFEEXPR_contextLET);
	      ffebld_set_head (arg, a);
	    }
  ...
  for (; arglist != NULL; arglist = ffebld_trail (arglist))
    {
      ...
      if ((item != NULL) && (ffebld_op (item) == FFEBLD_opANY))
	return TRUE;
    }
...
{
  const char *const uc =
    ((const struct _ffeintrin_name_ *) intrinsic)->name_uc;
  const char *const lc =
    ((const struct _ffeintrin_name_ *) intrinsic)->name_lc;
  const char *const ic =
    ((const struct _ffeintrin_name_ *) intrinsic)->name_ic;
  int i;
  ...
  for (i = 0;
      ...
      else
	{
	  ...
	  if (terror == FFEBAD)
		...
		{
		  if (ffebld_symter_specific (ffebld_left (*expr)) == tspec)
  ...
  if (any || (imp == FFEINTRIN_impNONE))
      ...
      if (!highly_specific && (nimp != FFEINTRIN_impNONE))
	{
	  fprintf (stderr, "lineno=%ld, gen=%s, imp=%s, timp=%s\n",
		   ...
		   ffeintrin_imps_[imp].name, ffeintrin_imps_[nimp].name);
      ...
      *info = ffeinfo_new (bt,
			   kt,
			   0, FFEINFO_kindENTITY, FFEINFO_whereFLEETING, sz);
  ...
  return (!ffe_is_pedantic () || ffeintrin_specs_[spec].is_actualarg)
    ...
    (ffe_is_f2c ()?
     (ffeintrin_imps_[ffeintrin_specs_[spec].implementation].gfrt_f2c !=
      FFECOM_gfrt) : (ffeintrin_imps_[ffeintrin_specs_[spec].implementation].
		      gfrt_gnu != FFECOM_gfrt))
  ...
  if (gen != FFEINTRIN_genNONE)
    {
      ...
      for (i = 0;
	   (((size_t) i) < ARRAY_SIZE (ffeintrin_gens_[gen].specs))
	   && ((tspec
		= ffeintrin_gens_[gen].specs[i]) != FFEINTRIN_specNONE); ++i)
	{
	  ...
	  if ((state == FFE_intrinsicstateENABLED)
	      || (explicit && (state == FFE_intrinsicstateHIDDEN)))
	    {
	      ok = TRUE;
	      break;
	    }
