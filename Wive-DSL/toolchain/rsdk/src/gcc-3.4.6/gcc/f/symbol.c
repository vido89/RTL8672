...
{
  if ((t != NULL) && ffest_ffebad_start (FFEBAD_SYMERR))
    {
  ...
  if (where == FFEINFO_whereINTRINSIC)
    {
      ffeglobal_ref_intrinsic (s, t,
			       explicit
			       || s->explicit_where
			       || ffeintrin_is_standard (s->generic,
							 s->specific));
      return;
    }
...
ffesymbol_update_init (ffesymbol s)
{
  ...
  if ((s->equiv != NULL) && !ffeequiv_is_init (s->equiv))
    ffeequiv_update_init (s->equiv);

  if ((s->storage != NULL) && !ffestorag_is_init (s->storage))
    ffestorag_update_init (s->storage);

  if ((s->common != NULL) && (!ffesymbol_is_init (s->common)))
...
ffesymbol_update_save (ffesymbol s)
{
  ...
  if ((s->equiv != NULL) && !ffeequiv_is_save (s->equiv))
    ffeequiv_update_save (s->equiv);

  if ((s->storage != NULL) && !ffestorag_is_save (s->storage))
    ffestorag_update_save (s->storage);

  if ((s->common != NULL) && (!ffesymbol_is_save (s->common)))
