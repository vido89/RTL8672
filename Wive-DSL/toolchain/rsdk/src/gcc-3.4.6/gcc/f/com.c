...
static
GTY (())
     tree ffecom_tree_fun_type_void;
...
/* _fun_type things are the f2c-specific versions.  For -fno-f2c,
   ...
   appropriate _tree_type array element.  */
     ...
     static GTY (())
     tree ffecom_tree_fun_type[FFEINFO_basictype][FFEINFO_kindtype];
     static GTY (()) tree
  ffecom_tree_ptr_to_fun_type[FFEINFO_basictype][FFEINFO_kindtype];
     static GTY (())
     tree ffecom_tree_subr_type;
     static GTY (())
     tree ffecom_tree_ptr_to_subr_type;
     static GTY (())
     tree ffecom_tree_blockdata_type;

     static GTY (())
     tree ffecom_tree_xargc_;
...
/* Static functions (internal). */
     ...
     static tree ffecom_check_size_overflow_ (ffesymbol s, tree type,
					      bool dummy);
     ...
     static ffecomConcatList_
       ffecom_concat_list_gather_ (ffecomConcatList_ catlist,
				   ffebld expr, ffetargetCharacterSize max);
     ...
     static void ffecom_f2c_make_type_ (tree * type, int tcode,
					const char *name);
     ...
     static void ffecom_prepare_let_char_ (ffetargetCharacterSize dest_size,
					   ffebld source);
     static void ffecom_push_dummy_decls_ (ffebld dumlist, bool stmtfunc);
...
/* These are static functions that parallel those found in the C front
     ...
     static void ffe_print_error_function (diagnostic_context *,
					   const char *);
     ...
     static void start_function (tree name, tree type, int nested,
				 int public);
...
/* Static objects accessed by functions in this module. */
     ...
     static GTY (())
     tree ffecom_outer_function_decl_;
     static GTY (())
     tree ffecom_previous_function_decl_;
     static GTY (())
     tree ffecom_which_entrypoint_decl_;
     static GTY (())
     tree ffecom_float_zero_;
     static GTY (())
     tree ffecom_float_half_;
     static GTY (())
     tree ffecom_double_zero_;
     static GTY (())
     tree ffecom_double_half_;
     static GTY (())
     tree ffecom_func_result_;	/* For functions. */
     static GTY (())
     tree ffecom_func_length_;	/* For CHARACTER fns. */
     ...
     static GTY (())
     tree ffecom_multi_type_node_;
     static GTY (())
     tree ffecom_multi_retval_;
     static GTY (()) tree
...
static
GTY (())
     struct f_binding_level *current_binding_level;

/* A chain of binding_level structures awaiting reuse.  */

     static GTY ((deletable ("")))
     struct f_binding_level *free_binding_level;
...
*/
     ...
     static tree
       ffecom_subscript_check_ (tree array, tree element, int dim,
				int total_dims, const char *array_name,
				tree item)
{
...

  {
    ...
    TREE_TYPE (arg1)
      = build_type_variant (build_array_type (char_type_node,
					      ...
					      (integer_type_node,
					       integer_one_node,
					       build_int_2 (len, 0))), 1, 0);
    TREE_CONSTANT (arg1) = 1;
    TREE_STATIC (arg1) = 1;
    arg1 = ffecom_1 (ADDR_EXPR, build_pointer_type (TREE_TYPE (arg1)), arg1);
    ...
    TREE_STATIC (arg3) = 1;
    arg3 = ffecom_1 (ADDR_EXPR, build_pointer_type (TREE_TYPE (arg3)), arg3);

    arg4 = convert (ffecom_f2c_ftnint_type_node, build_int_2 (input_line, 0));
  ...
  die = ffecom_call_gfrt (FFECOM_gfrtRANGE, args, NULL_TREE);
...
ffecom_arrayref_ (tree item, ffebld expr, int want_ptr)
{
  ...
  for (i = 0, list = ffebld_right (expr);
       list != NULL; ++i, list = ffebld_trail (list))
    {
  ...
  if (need_ptr)
    {
      ...
      for (--i, array = TYPE_MAIN_VARIANT (TREE_TYPE (TREE_TYPE (item)));
	   i >= 0; --i, array = TYPE_MAIN_VARIANT (TREE_TYPE (array)))
	{
...
ffecom_convert_narrow_ (tree type, tree expr)
{
  ...
  if (type == TREE_TYPE (e) || TREE_CODE (e) == ERROR_MARK)
    return e;
  if (TYPE_MAIN_VARIANT (type) == TYPE_MAIN_VARIANT (TREE_TYPE (e)))
    return fold (build1 (NOP_EXPR, type, e));
  if (TREE_CODE (TREE_TYPE (e)) == ERROR_MARK || code == ERROR_MARK)
    return error_mark_node;
  ...
  if ((code != RECORD_TYPE) && (TREE_CODE (TREE_TYPE (e)) == RECORD_TYPE))
  ...
  if (code == COMPLEX_TYPE)
    {
      ...
      assert (TYPE_PRECISION (TREE_TYPE (type)) <=
  ...
  if (ffe_is_emulate_complex ())
    {
      ...
      TREE_CHAIN (bothparts) =
	build_tree_list (TREE_CHAIN (TYPE_FIELDS (type)), imagpart);
...
ffecom_arglist_expr_ (const char *c, ffebld expr)
{
  ...
  while (expr != NULL)
	...
	*plist
	  = build_tree_list (NULL_TREE,
			     ffecom_arg_ptr_to_expr (exprh, &length));
      else
	{
	  ...
	  if (ptr)
	    {
	      item = ffecom_1 (ADDR_EXPR,
			       build_pointer_type (TREE_TYPE (item)), item);
	    }
	  *plist = build_tree_list (NULL_TREE, item);
	}
  ...
  while (*c != '\0' && *c != '0')
	...
	default:
      ...
      *plist = build_tree_list (NULL_TREE, null_pointer_node);
      plist = &TREE_CHAIN (*plist);
    }
...
ffecom_widest_expr_type_ (ffebld list)
{
  ...
  for (; list != NULL; list = ffebld_trail (list))
	...
	continue;
	  ...
	  || (ffetype_size (type) > ffetype_size (widest_type)))
	{
	  widest = item;
	  widest_type = type;
	}
...
ffecom_possible_partial_overlap_ (ffebld expr1, ffebld expr2 ATTRIBUTE_UNUSED)
{
  ...
  if (ffesymbol_where (sym) != FFEINFO_whereCOMMON
	  ...
	  || !(st = ffesymbol_storage (sym)) || !ffestorag_parent (st)))
    return FALSE;
		 ...
		 tree source_tree, ffebld source UNUSED, bool scalar_arg)
{
  ...
  switch (TREE_CODE (source_tree))
	...
	return TRUE;
      ...
      ffecom_tree_canonize_ptr_ (&source_decl, &source_offset, source_tree);
	...
	ffecom_overlap_ (dest_decl, dest_offset, dest_size,
			 ...
			 scalar_arg)
	|| ffecom_overlap_ (dest_decl, dest_offset, dest_size,
			    TREE_OPERAND (source_tree, 2), NULL, scalar_arg);


    case ADDR_EXPR:
      ffecom_tree_canonize_ref_ (&source_decl, &source_offset,
				 &source_size, TREE_OPERAND (source_tree, 0));
      break;
  ...
  t = ffecom_2 (LE_EXPR, integer_type_node,
		ffecom_2 (PLUS_EXPR, TREE_TYPE (dest_offset),
			  ...
			  convert (TREE_TYPE (dest_offset),
				   dest_size)),
		convert (TREE_TYPE (dest_offset), source_offset));
  ...
  t = ffecom_2 (LE_EXPR, integer_type_node,
		ffecom_2 (PLUS_EXPR, TREE_TYPE (source_offset),
			  ...
			  convert (TREE_TYPE (source_offset),
				   source_size)),
		convert (TREE_TYPE (source_offset), dest_offset));
			  ...
			  tree callee_commons, bool scalar_args)
{
  ...
  ffecom_tree_canonize_ref_ (&dest_decl, &dest_offset, &dest_size, dest_tree);
  ...
  if ((TREE_CODE (dest_decl) == ERROR_MARK)
      || ((callee_commons != NULL_TREE) && TREE_PUBLIC (dest_decl)))
    return TRUE;
  ...
  switch (ffebld_op (expr))
      ...
      if (with_null)
	{
      ...
      *length = build_int_2 (newlen, 0);
      ...
      item = build_string (newlen, ffetarget_text_character1 (val));
	...
	(build_array_type
	 (char_type_node,
	  ...
	  (ffecom_f2c_ftnlen_type_node,
	   ffecom_f2c_ftnlen_one_node, high)), 1, 0);
	...
	if (ffesymbol_kind (s) == FFEINFO_kindENTITY)
	  ...
	  *length = NULL_TREE;
	if (!ffesymbol_hook (s).addr && (item != error_mark_node))
	  item = ffecom_1 (ADDR_EXPR,
			   build_pointer_type (TREE_TYPE (item)), item);
      }
      break;
	...
	if (start == NULL)
	  {
		...
		if (flag_bounds_check)
		  end_tree = ffecom_subscript_check_ (array, end_tree, 1, 0,
						      char_name, NULL_TREE);
		end_tree = convert (ffecom_f2c_ftnlen_type_node, end_tree);
	...
	else
	  {
	    ...
	    if (flag_bounds_check)
	      start_tree = ffecom_subscript_check_ (array, start_tree, 0, 0,
						    char_name, NULL_TREE);
	    start_tree = convert (ffecom_f2c_ftnlen_type_node, start_tree);
	...
	*length = build_int_2 (size, 0);
	...
	if (ffeinfo_where (ffebld_info (ffebld_left (expr)))
	    == FFEINFO_whereINTRINSIC)
	  {
		...
		item = ffecom_expr_intrinsic_ (expr, NULL_TREE, NULL, NULL);
		break;
	      }
	    ix =
	      ffeintrin_gfrt_direct (ffebld_symter_implementation
				     (ffebld_left (expr)));
	...
	{			/* Possible blank-padding needed, copy into
	  ...
	  TREE_CHAIN (TREE_CHAIN (args)) =
	    build_tree_list (NULL_TREE, newlen);
		      ...
		      tree member_type UNUSED, ffetargetOffset offset)
{
  ...
  for (type_id = member_type; TREE_CODE (type_id) != IDENTIFIER_NODE;)
    {
  ...
  if (ffecom_transform_only_dummies_ || !ffe_is_debug_kludge ())
    return;			/* Can't do this yet, maybe later. */

  len = 60 + strlen (aggr_type) + IDENTIFIER_LENGTH (DECL_NAME (aggr));
#if 0
  ...
  sprintf (&buff[0], "At (%s) `%s' plus %ld bytes",
	   aggr_type,
	   IDENTIFIER_POINTER (DECL_NAME (aggr)), (long int) offset);
...
ffecom_do_entry_ (ffesymbol fn, int entrynum)
{
  ...
  switch (ffecom_primary_entry_kind_)
	...
	charfunc = TRUE, cmplxfunc = FALSE;
      else if ((bt == FFEINFO_basictypeCOMPLEX) && ffesymbol_is_f2c (fn))
	...
	type = build_function_type (ffecom_tree_type[bt][kt], NULL_TREE);
      ...
      if ((type == NULL_TREE) || (TREE_TYPE (type) == NULL_TREE))
	...
	{			/* Am _I_ altreturning? */
	  for (item = ffesymbol_dummyargs (fn);
	       item != NULL; item = ffebld_trail (item))
	    {
  ...
  start_function (ffecom_get_external_identifier_ (fn), type, 0,	/* nested/inline */
		  1);		/* TREE_PUBLIC */
...

  {
    ...
    for (list = ffecom_master_arglist_;
	 list != NULL; list = ffebld_trail (list))
      {
	...
	*plist = build_tree_list (NULL_TREE, actarg);
	...
	result = ffecom_modify (NULL_TREE,
				result, convert (TREE_TYPE (result), call));
	expand_return (result);
      }
  }
	      ...
	      bool assignp, bool widenp)
{
  ...
  switch (ffebld_op (expr))
	...
	{			/* ASSIGN'ed-label expr. */
	  if (ffe_is_ugly_assign ())
	      ...
	      if (ffesymbol_hook (s).assign_tree == NULL_TREE)
		{
		  ...
		  ffebad_start_msg
		    ("ASSIGN'ed label cannot fit into `%A' at %0 -- using wider sibling",
	  ...
	  if (t != NULL_TREE && TREE_CODE (t) == VAR_DECL)
    ...
    case FFEBLD_opUPLUS:
      left =
	ffecom_expr_ (ffebld_left (expr), NULL, NULL, NULL, FALSE, widenp);
	...
	ffecom_expr_ (ffebld_left (expr), NULL, NULL, NULL, FALSE, widenp);
      right =
	ffecom_expr_ (ffebld_right (expr), NULL, NULL, NULL, FALSE, widenp);
      if (tree_type_x)
	{
	  tree_type = tree_type_x;
	  left = convert (tree_type, left);
	...
	switch (ffeinfo_basictype (ffebld_info (right)))
	  ...
	  case FFEINFO_basictypeREAL:
		...
		FFEINFO_kindtypeREAL1)
	      left =
		ffeexpr_convert (left, NULL, NULL, FFEINFO_basictypeREAL,
		...
		FFEINFO_kindtypeREAL1)
	      right =
		ffeexpr_convert (right, NULL, NULL, FFEINFO_basictypeREAL,
				 FFEINFO_kindtypeREALDOUBLE, 0,
				 FFETARGET_charactersizeNONE,
				 FFEEXPR_contextLET);
	  ...
	  case FFEINFO_basictypeCOMPLEX:
		...
		FFEINFO_kindtypeREAL1)
	      left =
		ffeexpr_convert (left, NULL, NULL, FFEINFO_basictypeCOMPLEX,
				 FFEINFO_kindtypeREALDOUBLE, 0,
				 FFETARGET_charactersizeNONE,
				 FFEEXPR_contextLET);
	    if (ffeinfo_kindtype (ffebld_info (right)) ==
		FFEINFO_kindtypeREAL1)
	      right =
		ffeexpr_convert (right, NULL, NULL, FFEINFO_basictypeCOMPLEX,
				 FFEINFO_kindtypeREALDOUBLE, 0,
				 FFETARGET_charactersizeNONE,
				 FFEEXPR_contextLET);
	...
	{			/* Invocation of an intrinsic. */
	  item = ffecom_expr_intrinsic_ (expr, dest_tree, dest, dest_used);
	  return item;
	}
	...
	return error_mark_node;
      ...
      item = ffecom_call_ (item, kt,
			   ...
			   args,
			   dest_tree, dest, dest_used,
			   error_mark_node, FALSE, ffebld_nonter_hook (expr));
      TREE_SIDE_EFFECTS (item) = 1;
      return item;
	...
	case FFEINFO_basictypeLOGICAL:
	  item
	    = ffecom_2 (TRUTH_ANDIF_EXPR, integer_type_node,
			...
			ffecom_truth_value (ffecom_expr
					    (ffebld_right (expr))));
	  return convert (tree_type, item);
	    ...
	    item
	      = ffecom_2 (TRUTH_ANDIF_EXPR, integer_type_node,
			  ffecom_2 (EQ_EXPR, integer_type_node,
				    ...
				    ffecom_1 (REALPART_EXPR, real_type,
					      arg2)), ffecom_2 (EQ_EXPR,
								integer_type_node,
								ffecom_1
								(IMAGPART_EXPR,
								 real_type,
		  ...
		  = ffecom_2 (code, integer_type_node,
			      ffecom_2 (ARRAY_REF,
					TYPE_MAIN_VARIANT (TREE_TYPE
							   (TREE_TYPE
							    (left_tree))),
					left_tree, integer_one_node),
			      ffecom_2 (ARRAY_REF,
					TYPE_MAIN_VARIANT (TREE_TYPE
							   (TREE_TYPE
							    (right_tree))),
					right_tree, integer_one_node));
	      }
	    else
	      {
			...
			bool * dest_used)
{
  ...
  switch (codegen_imp)
    ...
    case FFEINTRIN_impIABS:
      if (ffeinfo_basictype (ffebld_info (arg1)) == FFEINFO_basictypeCOMPLEX)
	{
	...
	arg1_type = TREE_TYPE (TYPE_FIELDS (arg1_type));
      ...
      return
	convert (tree_type,
		 ffecom_1 (IMAGPART_EXPR, arg1_type, ffecom_expr (arg1)));
	...
	ffecom_2 (COMPLEX_EXPR, tree_type,
		  ...
		  convert (real_type, ffecom_expr (arg2)));

    case FFEINTRIN_impCOMPLEX:
      return
	ffecom_2 (COMPLEX_EXPR, tree_type,
		  ffecom_expr (arg1), ffecom_expr (arg2));
	...
	expr_tree
	  = ffecom_2 (COMPOUND_EXPR, tree_type,
		      convert (void_type_node, saved_expr1), expr_tree);
      }
      return expr_tree;
	...
	arg1_type = TREE_TYPE (TYPE_FIELDS (arg1_type));
      ...
      return
	convert (tree_type,
		 ffecom_1 (REALPART_EXPR, arg1_type, ffecom_expr (arg1)));
    ...
    case FFEINTRIN_impLSHIFT:
      return ffecom_2 (LSHIFT_EXPR, tree_type,
		       ffecom_expr (arg1),
		       convert (integer_type_node, ffecom_expr (arg2)));
			       ...
			       ffecom_2 (BIT_AND_EXPR, arg1_type,
					 ...
					 ffecom_2 (LSHIFT_EXPR, arg1_type,
						   ...
						   convert (integer_type_node,
							    ffecom_expr
							    (arg2)))),
			       convert (arg1_type, integer_zero_node))),
		    false_tree, true_tree);
      }
	...
	tree arg3_tree = ffecom_save_tree (convert (integer_type_node,
						    ffecom_expr (arg3)));
	tree uns_type = ffecom_tree_type[FFEINFO_basictypeHOLLERITH][kt];
	...
	expr_tree
	  = ffecom_3 (COND_EXPR, tree_type,
		      ...
		      (ffecom_2 (NE_EXPR, integer_type_node,
				 arg3_tree,
				 integer_zero_node)),
		      expr_tree, convert (tree_type, integer_zero_node));
      }
      return expr_tree;
	...
	mask_arg1
	  = ffecom_3 (COND_EXPR, tree_type,
		      ...
		      (ffecom_2 (NE_EXPR, integer_type_node,
				 arg3_tree,
				 TYPE_SIZE (uns_type))),
		      mask_arg1, convert (tree_type, integer_zero_node));
	mask_arg1 = ffecom_save_tree (mask_arg1);
	masked_arg1
	  = ffecom_2 (BIT_AND_EXPR, tree_type,
		      arg1_tree,
		      ffecom_1 (BIT_NOT_EXPR, tree_type, mask_arg1));
	...
	expr_tree
	  = ffecom_3 (COND_EXPR, tree_type,
		      ...
		      (ffecom_2 (LT_EXPR, integer_type_node,
				 arg2_tree,
				 integer_zero_node)), shift_neg, shift_pos);
	expr_tree
	  = ffecom_2 (BIT_IOR_EXPR, tree_type,
		      ffecom_2 (BIT_AND_EXPR, tree_type,
				mask_arg1,
	...
	expr_tree
	  = ffecom_2 (COMPOUND_EXPR, tree_type,
		      convert (void_type_node, arg3_tree), expr_tree);
      }
      return expr_tree;
	...
	prep_arg4
	  = ffecom_2 (LSHIFT_EXPR, arg4_type,
		      ffecom_1 (BIT_NOT_EXPR, arg4_type,
				convert (arg4_type,
					 integer_zero_node)), arg5_plus_arg3);
	/* Fix up, because LSHIFT_EXPR above can't shift over TYPE_SIZE.  */
	prep_arg4
	  = ffecom_3 (COND_EXPR, arg4_type,
		      ...
		      (ffecom_2 (NE_EXPR, integer_type_node,
				 ...
				 convert (TREE_TYPE (arg5_plus_arg3),
					  TYPE_SIZE (arg4_type)))),
		      prep_arg4, convert (arg4_type, integer_zero_node));
	prep_arg4
	  = ffecom_2 (BIT_AND_EXPR, arg4_type,
		      arg4_tree,
					  ...
					  ffecom_2 (LSHIFT_EXPR, arg4_type,
						    ffecom_1 (BIT_NOT_EXPR,
							      ...
							      (arg4_type,
							       integer_zero_node)),
						    arg5_tree))));
	prep_arg1 = ffecom_2 (BIT_IOR_EXPR, arg4_type, prep_arg1, prep_arg4);
	...
	prep_arg1
	  = ffecom_3 (COND_EXPR, arg4_type,
		      ...
		      prep_arg1, arg1_tree);
	expr_tree
	  = ffecom_2s (MODIFY_EXPR, void_type_node, arg4_tree, prep_arg1);
	/* Make sure SAVE_EXPRs get referenced early enough. */
	expr_tree
		      ...
		      ffecom_2 (COMPOUND_EXPR, void_type_node,
				...
				ffecom_2 (COMPOUND_EXPR, void_type_node,
					  ...
					  ffecom_2 (COMPOUND_EXPR,
						    ...
						    expr_tree))));
	expr_tree =
	  ffecom_2 (COMPOUND_EXPR, void_type_node, arg4_tree, expr_tree);
	...
	arg2_tree = convert (TREE_TYPE (null_pointer_node), arg2_tree);
	...
	if (arg3_tree != NULL_TREE)
	  expr_tree
	    = ffecom_modify (NULL_TREE, arg3_tree,
			     convert (TREE_TYPE (arg3_tree), expr_tree));
      }
      return expr_tree;
	...
	if (arg2_tree != NULL_TREE)
	  expr_tree
	    = ffecom_modify (NULL_TREE, arg2_tree,
			     convert (TREE_TYPE (arg2_tree), expr_tree));
      }
      return expr_tree;
	...
	arg1_tree = convert (ffecom_f2c_integer_type_node, arg1_tree);
	arg1_tree = ffecom_1 (ADDR_EXPR,
			      build_pointer_type (TREE_TYPE (arg1_tree)),
			      arg1_tree);
	arg1_tree = build_tree_list (NULL_TREE, arg1_tree);
	...
	expr_tree = ffecom_call_ (ffecom_gfrt_tree_ (gfrt),
				  ...
				  arg1_tree,
				  dest_tree, dest, dest_used,
				  NULL_TREE, TRUE, ffebld_nonter_hook (expr));
      }
      return expr_tree;
	...
	expr_tree
	  = ffecom_modify (NULL_TREE, arg1_tree,
			   convert (TREE_TYPE (arg1_tree), expr_tree));
      }
      return expr_tree;
    ...
    case FFEINTRIN_imp:	/* Hush up gcc warning. */
      fprintf (stderr, "No %s implementation.\n",
	       ffeintrin_name_implementation (ffebld_symter_implementation
					      (ffebld_left (expr))));
...
ffecom_expr_power_integer_ (ffebld expr)
{
  ...
  if (l == error_mark_node || r == error_mark_node)
    return error_mark_node;
  ...
  if (TREE_CODE (r) == INTEGER_CST)
	...
	return convert (ltype, integer_one_node);
      ...
      if ((TREE_CODE (ltype) == INTEGER_TYPE) && (sgn < 0))
	{
	  ...
	  result = ffecom_tree_divide_ (ltype,
					...
					l, NULL_TREE, NULL, NULL, NULL_TREE);
	  r = ffecom_1 (NEGATE_EXPR, rtype, r);
	  if ((TREE_INT_CST_LOW (r) & 1) == 0)
	    result = ffecom_1 (ABS_EXPR, rtype, result);
	}
	    ...
	    {			/* The "most negative" number.  */
	      r = ffecom_1 (NEGATE_EXPR, rtype,
			    ffecom_2 (RSHIFT_EXPR, rtype,
				      r, integer_one_node));
	      l = save_expr (l);
	      l = ffecom_2 (MULT_EXPR, ltype, l, l);
	    }
	}
      ...
      for (;;)
	{
	  if (TREE_INT_CST_LOW (r) & 1)
		...
		result = ffecom_2 (MULT_EXPR, ltype, result, l);
	    }

	  r = ffecom_2 (RSHIFT_EXPR, rtype, r, integer_one_node);
	  if (integer_zerop (r))
	    break;
	  assert (TREE_CODE (r) == INTEGER_CST);
...

  {
    ...
    result = ffecom_make_tempvar ("power_res", ltype,
				  FFETARGET_charactersizeNONE, -1);
    if (TREE_CODE (ltype) == COMPLEX_TYPE || TREE_CODE (ltype) == RECORD_TYPE)
      divide = ffecom_make_tempvar ("power_div", ltype,
				    FFETARGET_charactersizeNONE, -1);
    else
      divide = NULL_TREE;

    expand_expr_stmt (ffecom_modify (void_type_node, rtmp, r));
    expand_expr_stmt (ffecom_modify (void_type_node, ltmp, l));
    expand_start_cond (ffecom_truth_value
		       (ffecom_2 (EQ_EXPR, integer_type_node,
				  rtmp,
				  convert (rtype, integer_zero_node))), 0);
			   ...
			   (ffecom_2 (TRUTH_ANDIF_EXPR, integer_type_node,
				      ...
				      ffecom_2 (EQ_EXPR, integer_type_node,
						...
						convert (rtype,
							 integer_zero_node)))),
			   0);
	expand_expr_stmt (ffecom_modify
			  (void_type_node, result,
			   ffecom_1 (NEGATE_EXPR, ltype, result)));
	expand_end_cond ();
	expand_start_else ();
      }
    expand_expr_stmt (ffecom_modify (void_type_node,
    ...
    expand_expr_stmt (ffecom_modify (void_type_node,
				     ...
				     (ltype,
				      convert (ltype, integer_one_node),
				      ltmp, NULL_TREE, NULL, NULL, divide)));
    expand_expr_stmt (ffecom_modify (void_type_node,
				     rtmp,
				     ffecom_1 (NEGATE_EXPR, rtype, rtmp)));
    expand_start_cond (ffecom_truth_value
		       (ffecom_2 (LT_EXPR, integer_type_node,
				  rtmp,
				  convert (rtype, integer_zero_node))), 0);
    ...
    expand_start_cond (ffecom_truth_value
		       (ffecom_2 (BIT_AND_EXPR, rtype,
				  rtmp,
				  convert (rtype, integer_one_node))), 0);
    expand_expr_stmt (ffecom_modify (void_type_node,
				     result,
				     ffecom_2 (MULT_EXPR, ltype,
					       result, ltmp)));
	...
	result = build (BIND_EXPR, TREE_TYPE (result), NULL_TREE, result, t);
...
ffecom_finish_global_ (ffeglobal global)
{
  ...
  if ((cbt == NULL_TREE) || !ffeglobal_common_have_size (global))
  ...
  size = build_int_2 ((ffeglobal_common_size (global)
		       + ffeglobal_common_pad (global)) - 1, 0);
...
ffecom_get_appended_identifier_ (char us, const char *name)
{
  ...
  newname = xmalloc ((i = strlen (name)) + 1 + ffe_is_underscoring () + us);
...
ffecom_get_external_identifier_ (ffesymbol s)
{
  ...
  us = ffe_is_second_underscore ()? (strchr (name, '_') != NULL) : 0;
...
ffecom_get_identifier_ (const char *name)
{
  ...
  if (!ffe_is_underscoring () || (strchr (name, '_') == NULL))
    return get_identifier (name);

  return ffecom_get_appended_identifier_ (ffe_is_second_underscore (), name);
}
...
ffecom_gen_sfuncdef_ (ffesymbol s, ffeinfoBasictype bt, ffeinfoKindtype kt)
{
  ...
  start_function (ffecom_get_identifier_ (ffesymbol_text (s)), build_function_type (type, NULL_TREE), 1,	/* nested/inline */
		  0);		/* TREE_PUBLIC */
...
static
GTY (())
     struct tree_ggc_tracker *tracker_head;
...
ffecom_init_zero_ (tree decl)
{
  ...
  if ((TREE_CODE (type) != ARRAY_TYPE)
      ...
      && (TREE_CODE (type) != UNION_TYPE) && !incremental)
...
ffecom_intrinsic_ichar_ (tree tree_type, ffebld arg, tree * maybe_tree)
{
  ...
  switch (ffebld_op (arg))
    ...
    case FFEBLD_opCONTER:	/* For F90, check 0-length. */
	  ...
	  (ffebld_constant_character1 (ffebld_conter (arg))) == 0)
	{
    ...
    case FFEBLD_opSUBSTR:
      ...
      if ((expr_tree == error_mark_node) || (length_tree == error_mark_node))
	{
	...
	*maybe_tree
	  = ffecom_truth_value
	  (ffecom_2 (GT_EXPR, integer_type_node,
		     length_tree, ffecom_f2c_ftnlen_zero_node));
      return expr_tree;
	...
	*maybe_tree = ffecom_2 (TRUTH_ORIF_EXPR, integer_type_node,
	...
	expr_tree = ffecom_3 (COND_EXPR, tree_type,
			      maybe_left, expr_left, expr_right);
	return expr_tree;
      }
		  ...
		  ffetargetCharacterSize dest_size, ffebld source)
{
  ...
  if ((dest_tree == error_mark_node) || (dest_length == error_mark_node))
    return;
...

  {
	...
	if ((citem == error_mark_node) || (clength == error_mark_node))
	  {
	    ffecom_concat_list_kill_ (catlist);
	    return;
	  }
...
ffecom_make_gfrt_ (ffecomGfrt ix)
{
  ...
  t = build_decl (FUNCTION_DECL,
		  get_identifier (ffecom_gfrt_name_[ix]), ttype);
  DECL_EXTERNAL (t) = 1;
...
ffecom_member_phase2_ (ffestorag mst, ffestorag st)
{
  ...
  if ((mst == NULL)
      || ((mt = ffestorag_hook (mst)) == NULL) || (mt == error_mark_node))
    return;

  if ((st == NULL) || ((s = ffestorag_symbol (st)) == NULL))
    return;
  ...
  t = build_decl (VAR_DECL,
		  ffecom_get_identifier_ (ffesymbol_text (s)), type);
...
ffecom_push_dummy_decls_ (ffebld dummy_list, bool stmtfunc)
{
  ...
  for (dumlist = dummy_list; dumlist != NULL;
       dumlist = ffebld_trail (dumlist))
    {
...
ffecom_start_progunit_ (void)
{
  ...
  switch (ffecom_primary_entry_kind_)
	...
	charfunc = TRUE, cmplxfunc = FALSE;
	       ...
	       && ffesymbol_is_f2c (fn) && !altentries)
	charfunc = FALSE, cmplxfunc = TRUE;
      else
	charfunc = cmplxfunc = FALSE;
  ...
  start_function (id, type, 0,	/* nested/inline */
  ...
  if (!altentries
      ...
      && ((ffeglobal_type (g) == gt) || (ffeglobal_type (g) == egt)))
    {
      ffeglobal_set_hook (g, current_function_decl);
    }
...
ffecom_sym_transform_ (ffesymbol s)
{
  ...
  if (!ffecom_transform_only_dummies_
      && ffesymbol_assigned (s) && !ffesymbol_hook (s).assign_tree)
    s = ffecom_sym_transform_assign_ (s);
  ...
  switch (ffesymbol_kind (s))
	...
	case FFEINFO_whereGLOBAL:	/* Subroutine or function. */
	  ...
	  t = build_decl (FUNCTION_DECL, ffecom_get_external_identifier_ (s), ffecom_tree_subr_type);	/* Assume subr. */
	  DECL_EXTERNAL (t) = 1;
	  TREE_PUBLIC (t) = 1;
	...
	case FFEINFO_whereLOCAL:
...

	  {
	    ...
	    if ((st != NULL) && (ffestorag_size (st) == 0))
	      {
		t = error_mark_node;
		break;
	      }

	    if ((st != NULL) && (ffestorag_parent (st) != NULL))
		...
		if (init || ffesymbol_namelisted (s)
		  ...
		  TREE_STATIC (t) =
		    !ffesymbol_attr (s, FFESYMBOL_attrADJUSTABLE);
		else
	...
	case FFEINFO_whereDUMMY:
	  {
		...
		type
		  = build_array_type
		  (type,
		   build_range_type (ffecom_integer_type_node, low, high));
		type = ffecom_check_size_overflow_ (s, type, TRUE);
	      }
	    ...
	    if (ffesymbol_numentries (s) == (ffecom_num_entrypoints_ + 1))
	      break;
...

	    {
	      ...
	      for (tem = sizes; tem != old_sizes; tem = TREE_CHAIN (tem))
		{
	      ...
	      if (sizes != tem)
		{
		  ...
		  sizes = tree_cons (NULL_TREE, sizes, tem);
		}
...
#if 0
	    if (adjustable
		&& (ffesymbol_numentries (s) != ffecom_num_entrypoints_ + 1))
	      DECL_SOMETHING (t)
		= ffecom_2 (NE_EXPR, integer_type_node, t, null_pointer_node);
#else
#if 0
	    if (adjustable
		&& (ffesymbol_numentries (s) != ffecom_num_entrypoints_ + 1))
	...
	case FFEINFO_whereCOMMON:
	  {
	    ...
	    if ((cg == NULL) || (ffeglobal_type (cg) != FFEGLOBAL_typeCOMMON))
	      ct = NULL_TREE;
	    else
	      ct = ffeglobal_hook (cg);	/* The common area's tree.  */

	    if ((ct == NULL_TREE)
		|| (st == NULL) || (type == error_mark_node))
	      t = error_mark_node;
	    else
	      {
		ffetargetOffset offset;
		...
		offset = ffestorag_modulo (cst)
		  + ffestorag_offset (st) - ffestorag_offset (cst);
		...
		t = ffecom_2 (PLUS_EXPR, TREE_TYPE (t), t, toffset);
		t = convert (build_pointer_type (type), t);
		TREE_CONSTANT (t) = 1;
	...
	case FFEINFO_whereGLOBAL:
	  ...
	  t = build_decl (FUNCTION_DECL,
			  ffecom_get_external_identifier_ (s), t);
	  DECL_EXTERNAL (t) = 1;
	  TREE_PUBLIC (t) = 1;
	...
	case FFEINFO_whereDUMMY:
	  ...
	  t = build_decl (PARM_DECL,
			  ffecom_get_identifier_ (ffesymbol_text (s)), t);
	  DECL_ARTIFICIAL (t) = 1;
	  addr = TRUE;
	  break;
...
ffecom_transform_common_ (ffesymbol s)
{
  ...
  if ((g == NULL) || (ffeglobal_type (g) != FFEGLOBAL_typeCOMMON))
    return;
  ...
  if ((cbt != NULL_TREE) && (!is_init || !DECL_EXTERNAL (cbt)))
    {
      if (st->hook == NULL)
  ...
  if (cbt == NULL_TREE)
    {
      cbt
	= build_decl (VAR_DECL, ffecom_get_external_identifier_ (s), cbtype);
      TREE_STATIC (cbt) = 1;
      TREE_PUBLIC (cbt) = 1;
    }
  else
  ...
  eqt = build_decl (VAR_DECL,
		    ffecom_get_invented_identifier ("__g77_equiv_%s",
						    ...
						    (ffestorag_symbol
						     (eqst))), eqtype);
  DECL_EXTERNAL (eqt) = 0;
  if (is_init || ffecom_member_namelisted_
#ifdef FFECOM_sizeMAXSTACKITEM
...
ffecom_transform_namelist_ (ffesymbol s)
{
  ...
  nameinit = ffecom_build_f2c_string_ (i + 1, ffesymbol_text (s));
  TREE_TYPE (nameinit)
    = build_type_variant
    (build_array_type
     (char_type_node,
...
ffecom_tree_canonize_ref_ (tree * decl, tree * offset, tree * size, tree t)
{
  ...
  *decl = NULL_TREE;
  ...
  switch (TREE_CODE (t))
	...
	if ((array == NULL_TREE) || (element == NULL_TREE))
	  {
	    *decl = error_mark_node;
	    return;
	  }

	ffecom_tree_canonize_ref_ (decl, &init_offset, size, array);
	if ((*decl == NULL_TREE) || (*decl == error_mark_node))
	  return;
		     ...
		     ffebld dest, bool * dest_used, tree hook)
{
  ...
  switch (TREE_CODE (tree_type))
    {
    case INTEGER_TYPE:
      return ffecom_2 (TRUNC_DIV_EXPR, tree_type, left, right);

    case COMPLEX_TYPE:
      if (!optimize_size)
	return ffecom_2 (RDIV_EXPR, tree_type, left, right);
      {
	ffecomGfrt ix;
...
ffecom_type_localvar_ (ffesymbol s, ffeinfoBasictype bt, ffeinfoKindtype kt)
{
  ...
  if (bt == FFEINFO_basictypeCHARACTER)
	...
	(type,
	 build_range_type (ffecom_f2c_ftnlen_type_node,
			   ffecom_f2c_ftnlen_one_node, hight));
...
ffecom_vardesc_ (ffebld expr)
{
  ...
  if (ffesymbol_hook (s).vardesc_tree == NULL_TREE)
	...
	(build_array_type
	 (char_type_node,
	  build_range_type (integer_type_node,
			    integer_one_node, build_int_2 (i, 0))), 1, 0);
      TREE_CONSTANT (nameinit) = 1;
      TREE_STATIC (nameinit) = 1;
...
ffecom_vardesc_dims_ (ffesymbol s)
{
  if (ffesymbol_dims (s) == NULL)
    return convert (ffecom_f2c_ptr_to_ftnlen_type_node, integer_zero_node);

  {
    ...
    for (b = ffesymbol_dims (s), e = ffesymbol_extents (s);
	 b != NULL; b = ffebld_trail (b), e = ffebld_trail (e))
      {
	tree t;
	tree low;
	tree back;
...
ffecom_1 (enum tree_code code, tree type, tree node)
{
  ...
  if ((node == error_mark_node) || (type == error_mark_node))
    return error_mark_node;
  ...
  switch (ffe_is_emulate_complex ()? code : NOP_EXPR)
    ...
    case REALPART_EXPR:
      item =
	build (COMPONENT_REF, type, node, TYPE_FIELDS (TREE_TYPE (node)));
      break;

    case IMAGPART_EXPR:
      item =
	build (COMPONENT_REF, type, node,
	...
	ffecom_2 (COMPLEX_EXPR, type,
		  ffecom_1 (NEGATE_EXPR, realtype,
			    ffecom_1 (REALPART_EXPR, realtype,
				      node)),
		  ffecom_1 (NEGATE_EXPR, realtype,
			    ffecom_1 (IMAGPART_EXPR, realtype, node)));
      break;
...
ffecom_1_fn (tree node)
{
  ...
  item = build1 (ADDR_EXPR, build_pointer_type (type), node);
  if (TREE_SIDE_EFFECTS (node))
    TREE_SIDE_EFFECTS (item) = 1;
  if (staticp (node))
    TREE_CONSTANT (item) = 1;
...
ffecom_2 (enum tree_code code, tree type, tree node1, tree node2)
{
  ...
  if ((node1 == error_mark_node)
      || (node2 == error_mark_node) || (type == error_mark_node))
    return error_mark_node;
  ...
  switch (ffe_is_emulate_complex ()? code : NOP_EXPR)
    ...
    case COMPLEX_EXPR:
      ...
      TREE_CHAIN (item) =
	build_tree_list (TREE_CHAIN (TYPE_FIELDS (type)), node2);
	...
	ffecom_2 (COMPLEX_EXPR, type,
		  ...
		  ffecom_2 (PLUS_EXPR, realtype,
			    ffecom_1 (IMAGPART_EXPR, realtype,
				      node1),
			    ffecom_1 (IMAGPART_EXPR, realtype, node2)));
      break;
      ...
      if (TREE_CODE (type) != RECORD_TYPE)
	{
      ...
      node2 = ffecom_stabilize_aggregate_ (node2);
      realtype = TREE_TYPE (TYPE_FIELDS (type));
      a = save_expr (ffecom_1 (REALPART_EXPR, realtype, node1));
      b = save_expr (ffecom_1 (IMAGPART_EXPR, realtype, node1));
      c = save_expr (ffecom_1 (REALPART_EXPR, realtype, node2));
      d = save_expr (ffecom_1 (IMAGPART_EXPR, realtype, node2));
      item =
	ffecom_2 (COMPLEX_EXPR, type,
		  ffecom_2 (MINUS_EXPR, realtype,
		  ...
		  ffecom_2 (PLUS_EXPR, realtype,
			    ffecom_2 (MULT_EXPR, realtype,
				      a,
				      d),
			    ffecom_2 (MULT_EXPR, realtype, c, b)));
      break;
...
ffecom_2pass_advise_entrypoint (ffesymbol entry)
{
  ...
  switch (ffecom_master_bt_)
    ...
    case FFEINFO_basictypeCHARACTER:
      ok = (bt == FFEINFO_basictypeCHARACTER)
	&& (kt == ffecom_master_kt_) && (size == ffecom_master_size_);
      break;
  ...
  for (list = ffesymbol_dummyargs (entry);
	...
	continue;		/* Alternate return or some such thing. */
      ...
      for (plist = NULL, mlist = ffecom_master_arglist_;
	   mlist != NULL; plist = mlist, mlist = ffebld_trail (mlist))
...
ffecom_3 (enum tree_code code, tree type, tree node1, tree node2, tree node3)
{
  ...
  if ((node1 == error_mark_node)
      ...
      || (node3 == error_mark_node) || (type == error_mark_node))
    return error_mark_node;
...
ffecom_arg_ptr_to_const_expr (ffebld expr, tree * length)
{
  ...
  if (length && ffebld_size (expr) != FFETARGET_charactersizeNONE)
    *length = build_int_2 (ffebld_size (expr), 0);
  else if (length)
    *length = NULL_TREE;
  return NULL_TREE;
...
ffecom_arg_ptr_to_expr (ffebld expr, tree * length)
{
...

  {
    ...
    TREE_SIDE_EFFECTS (item) = 1;
    item = ffecom_2 (COMPOUND_EXPR, TREE_TYPE (temporary), item, temporary);
  ...
  switch (bt)
	...
	switch (kt)
	  ...
	  case FFEINFO_kindtypeREAL1:
	    ...
	    imag =
	      ffetarget_value_real1 (ffebld_cu_val_complex1 (*cu).imaginary);
	    break;
#endif
	  ...
	  case FFEINFO_kindtypeREAL2:
	    ...
	    imag =
	      ffetarget_value_real2 (ffebld_cu_val_complex2 (*cu).imaginary);
	    break;
#endif
	  ...
	  case FFEINFO_kindtypeREAL3:
	    ...
	    imag =
	      ffetarget_value_real3 (ffebld_cu_val_complex3 (*cu).imaginary);
	    break;
#endif
	...
	if (h.length >= FLOAT_TYPE_SIZE / CHAR_TYPE_SIZE)
	...
	else
	  {
	    ...
	    memset (&str[h.length], ' ',
		    FLOAT_TYPE_SIZE / CHAR_TYPE_SIZE - h.length);
	    item = build_string (FLOAT_TYPE_SIZE / CHAR_TYPE_SIZE, str);
	  }
	TREE_TYPE (item)
	  = build_type_variant (build_array_type (char_type_node,
						  build_range_type
				...
				ffebldConst ct)
{
  ...
  switch (ct)
	...
	item = build_int_2 ((HOST_WIDE_INT) big,
			    (HOST_WIDE_INT) (big >> HOST_BITS_PER_WIDE_INT));
      }
#else
...
ffecom_end_transition (void)
{
  ...
  for (item = ffecom_list_common_; item != NULL; item = ffebld_trail (item))
...
ffecom_expand_let_stmt (ffebld dest, ffebld source)
{
  ...
  if (ffeinfo_basictype (ffebld_info (dest)) != FFEINFO_basictypeCHARACTER)
	...
	return;
      ...
      if ((TREE_CODE (dest_tree) != VAR_DECL) || TREE_ADDRESSABLE (dest_tree))
      ...
      else if (assign_temp)
	{
	  expr_tree = ffecom_2s (MODIFY_EXPR, void_type_node,
				 assign_temp, source_tree);
	  expand_expr_stmt (expr_tree);
	  expr_tree = ffecom_2s (MODIFY_EXPR, void_type_node,
				 dest_tree, assign_temp);
	}
      else
	expr_tree = ffecom_2s (MODIFY_EXPR, void_type_node,
  ...
  ffecom_let_char_ (dest_tree, dest_length, ffebld_size_known (dest), source);
}
...
ffecom_init_0 (void)
{
  ...
  if (ffe_is_do_internal_checks ())
    {
      static const char names[][12] = { "bar", "bletch", "foo", "foobar" };
      const char *name;
      unsigned long ul;
      double fl;
 ...
 in proj.h" == NULL);
  ...
  ffecom_tree_fun_type_void = build_function_type (void_type_node, NULL_TREE);
  ...
  pushdecl (build_decl (TYPE_DECL, get_identifier ("integer"), t));
  type = ffetype_new ();
  base_type = type;
  ffeinfo_set_type (FFEINFO_basictypeINTEGER, FFEINFO_kindtypeINTEGER1, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
		    TREE_INT_CST_LOW (TYPE_SIZE (t)) / CHAR_TYPE_SIZE, type);
  ...
  ffecom_tree_type[FFEINFO_basictypeHOLLERITH][FFEINFO_kindtypeINTEGER1] = t = make_unsigned_type (FLOAT_TYPE_SIZE);	/* HOLLERITH means unsigned. */
  pushdecl (build_decl (TYPE_DECL, get_identifier ("unsigned"), t));

  ffecom_tree_type[FFEINFO_basictypeINTEGER][FFEINFO_kindtypeINTEGER2]
    = t = make_signed_type (CHAR_TYPE_SIZE);
  pushdecl (build_decl (TYPE_DECL, get_identifier ("byte"), t));
  type = ffetype_new ();
  ffeinfo_set_type (FFEINFO_basictypeINTEGER, FFEINFO_kindtypeINTEGER2, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  pushdecl (build_decl (TYPE_DECL, get_identifier ("unsigned byte"), t));

  ffecom_tree_type[FFEINFO_basictypeINTEGER][FFEINFO_kindtypeINTEGER3]
    = t = make_signed_type (CHAR_TYPE_SIZE * 2);
  pushdecl (build_decl (TYPE_DECL, get_identifier ("word"), t));
  type = ffetype_new ();
  ffeinfo_set_type (FFEINFO_basictypeINTEGER, FFEINFO_kindtypeINTEGER3, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  pushdecl (build_decl (TYPE_DECL, get_identifier ("unsigned word"), t));

  ffecom_tree_type[FFEINFO_basictypeINTEGER][FFEINFO_kindtypeINTEGER4]
    = t = make_signed_type (FLOAT_TYPE_SIZE * 2);
  pushdecl (build_decl (TYPE_DECL, get_identifier ("integer4"), t));
  type = ffetype_new ();
  ffeinfo_set_type (FFEINFO_basictypeINTEGER, FFEINFO_kindtypeINTEGER4, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  pushdecl (build_decl (TYPE_DECL, get_identifier ("unsigned4"), t));
  ...
  ffecom_tree_type[FFEINFO_basictypeLOGICAL][FFEINFO_kindtypeLOGICAL1]
    = t = make_signed_type (FLOAT_TYPE_SIZE);
  pushdecl (build_decl (TYPE_DECL, get_identifier ("logical"), t));
  type = ffetype_new ();
  base_type = type;
  ffeinfo_set_type (FFEINFO_basictypeLOGICAL, FFEINFO_kindtypeLOGICAL1, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  ffecom_tree_type[FFEINFO_basictypeLOGICAL][FFEINFO_kindtypeLOGICAL2]
    = t = make_signed_type (CHAR_TYPE_SIZE);
  pushdecl (build_decl (TYPE_DECL, get_identifier ("logical2"), t));
  type = ffetype_new ();
  ffeinfo_set_type (FFEINFO_basictypeLOGICAL, FFEINFO_kindtypeLOGICAL2, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  ffecom_tree_type[FFEINFO_basictypeLOGICAL][FFEINFO_kindtypeLOGICAL3]
    = t = make_signed_type (CHAR_TYPE_SIZE * 2);
  pushdecl (build_decl (TYPE_DECL, get_identifier ("logical3"), t));
  type = ffetype_new ();
  ffeinfo_set_type (FFEINFO_basictypeLOGICAL, FFEINFO_kindtypeLOGICAL3, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  ffecom_tree_type[FFEINFO_basictypeLOGICAL][FFEINFO_kindtypeLOGICAL4]
    = t = make_signed_type (FLOAT_TYPE_SIZE * 2);
  pushdecl (build_decl (TYPE_DECL, get_identifier ("logical4"), t));
  type = ffetype_new ();
  ffeinfo_set_type (FFEINFO_basictypeLOGICAL, FFEINFO_kindtypeLOGICAL4, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  pushdecl (build_decl (TYPE_DECL, get_identifier ("real"), t));
  layout_type (t);
  type = ffetype_new ();
  base_type = type;
  ffeinfo_set_type (FFEINFO_basictypeREAL, FFEINFO_kindtypeREAL1, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  pushdecl (build_decl (TYPE_DECL, get_identifier ("double precision"), t));
  layout_type (t);
  type = ffetype_new ();
  ffeinfo_set_type (FFEINFO_basictypeREAL, FFEINFO_kindtypeREALDOUBLE, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  ffecom_tree_type[FFEINFO_basictypeCOMPLEX][FFEINFO_kindtypeREAL1]
    = t =
    ffecom_make_complex_type_ (ffecom_tree_type[FFEINFO_basictypeREAL]
			       [FFEINFO_kindtypeREAL1]);
  pushdecl (build_decl (TYPE_DECL, get_identifier ("complex"), t));
  type = ffetype_new ();
  base_type = type;
  ffeinfo_set_type (FFEINFO_basictypeCOMPLEX, FFEINFO_kindtypeREAL1, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_star (base_type,
  ...
  pushdecl (build_decl (TYPE_DECL, get_identifier ("double complex"), t));
  type = ffetype_new ();
	...
	if ((t = ffecom_tree_type[i][j]) != NULL_TREE)
	  {
		...
		if (GET_MODE_SIZE (TYPE_MODE (t))
		    ...
		    GET_MODE_SIZE (TYPE_MODE (TREE_TYPE (null_pointer_node))))
		  {
			...
			(GET_MODE_SIZE
			 (TYPE_MODE
			  (ffecom_tree_type[i][ffecom_pointer_kind_])) >
  ...
  ffeinfo_set_type (FFEINFO_basictypeCHARACTER,
		    FFEINFO_kindtypeCHARACTER1, type);
  ffetype_set_ams (type,
		   TYPE_ALIGN (t) / BITS_PER_UNIT, 0,
		   TREE_INT_CST_LOW (TYPE_SIZE (t)) / BITS_PER_UNIT);
  ffetype_set_kind (base_type, 1, type);
  assert (ffetype_size (type) == sizeof (((ffetargetCharacter1)
					  {
					  0, NULL}
					 ).text[0]));
  ...
  ffecom_tree_ptr_to_fun_type[FFEINFO_basictypeCHARACTER]
    [FFEINFO_kindtypeCHARACTER1] = ffecom_tree_ptr_to_fun_type_void;
  ffecom_f2c_typecode_[FFEINFO_basictypeCHARACTER][FFEINFO_kindtypeCHARACTER1]
    = FFETARGET_f2cTYCHAR;

  ffecom_f2c_typecode_[FFEINFO_basictypeANY][FFEINFO_kindtypeANY] = 0;
  ...
  for (i = 0; ((size_t) i) < ARRAY_SIZE (ffecom_tree_type); ++i)
	...
	DECL_CONTEXT (ffecom_multi_fields_[i][j]) = ffecom_multi_type_node_;
  ...
  ffecom_tree_subr_type = build_function_type (integer_type_node, NULL_TREE);
  ffecom_tree_ptr_to_subr_type = build_pointer_type (ffecom_tree_subr_type);
  ffecom_tree_blockdata_type
  ...
  if ((FLOAT_TYPE_SIZE != 32)
      || (TREE_INT_CST_LOW (TYPE_SIZE (TREE_TYPE (null_pointer_node))) != 32))
    {
      ...
      warning ("and pointers are %d bits wide, but g77 doesn't yet work",
	       (int)
	       TREE_INT_CST_LOW (TYPE_SIZE (TREE_TYPE (null_pointer_node))));
...
ffecom_prepare_expr_ (ffebld expr, ffebld dest UNUSED)
{
  ...
  switch (ffebld_op (expr))
	...
	for (item = expr; item != NULL; item = ffebld_trail (item))
...
ffecom_prepare_return_expr (ffebld expr)
{
  ...
  if (ffecom_primary_entry_kind_ == FFEINFO_kindSUBROUTINE
      && ffecom_is_altreturning_ && expr != NULL)
    ffecom_prepare_expr (expr);
}
...
ffecom_return_expr (ffebld expr)
{
  ...
  switch (ffecom_primary_entry_kind_)
	...
	{			/* Value is returned by direct assignment
      ...
      if ((rtn == NULL_TREE) || !TREE_USED (rtn))
	{
...
duplicate_decls (tree newdecl, tree olddecl)
{
	  ...
	  if (TYPE_MODE (oldreturntype) == TYPE_MODE (newreturntype))
	    {
	      ...
	      tree newtype = build_function_type (newreturntype,
  ...
  if (types_match)
	...
	TREE_TYPE (newdecl) = TREE_TYPE (olddecl) = TREE_TYPE (newdecl);
	...
	DECL_SECTION_NAME (newdecl) = DECL_SECTION_NAME (olddecl);
      ...
      if (TREE_CODE (newdecl) == FUNCTION_DECL)
	{
	  DECL_STATIC_CONSTRUCTOR (newdecl) |=
	    DECL_STATIC_CONSTRUCTOR (olddecl);
	  DECL_STATIC_DESTRUCTOR (newdecl) |=
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL
      && DECL_BUILT_IN (olddecl) && (!types_match || new_is_definition))
    {
      TREE_TYPE (olddecl) = TREE_TYPE (newdecl);
...
finish_decl (tree decl, tree init, bool is_top_level)
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE
      && TYPE_DOMAIN (type) == 0 && TREE_CODE (decl) != TYPE_DECL)
    {
      assert (top_level);
...
/*  TREE_READONLY (fndecl) = 1;
  ...
  if (TREE_CODE (fndecl) != ERROR_MARK
      && !nested && DECL_SAVED_INSNS (fndecl) == 0)
    {
...
static void
ffe_print_error_function (diagnostic_context * context
...
start_function (tree name, tree type, int nested, int public)
{
  ...
  if (TREE_CODE (type) == ERROR_MARK)
    ...
    {
      decl1 = build_decl (FUNCTION_DECL, name, type);
...
convert (tree type, tree expr)
{
  ...
  if ((code != RECORD_TYPE) && (TREE_CODE (TREE_TYPE (e)) == RECORD_TYPE))
    e = ffecom_1 (REALPART_EXPR, TREE_TYPE (TYPE_FIELDS (TREE_TYPE (e))), e);
...
poplevel (int keep, int reverse, int functionbody)
{
  ...
  for (decl = decls; decl; decl = TREE_CHAIN (decl))
	...
	&& DECL_INITIAL (decl) != 0 && TREE_ADDRESSABLE (decl))
      {
  ...
  if (functionbody && current_function_decl != error_mark_node)
    ...
    return (ffecom_2
	    ((TREE_SIDE_EFFECTS (expr)
	     ...
	     ffe_truthvalue_conversion (ffecom_1 (REALPART_EXPR,
						  TREE_TYPE (TREE_TYPE
							     (expr)), expr)),
	     ffe_truthvalue_conversion (ffecom_1
					(IMAGPART_EXPR,
					 TREE_TYPE (TREE_TYPE (expr)),
					 expr))));

  return ffecom_2 (NE_EXPR, integer_type_node,
		   expr, convert (TREE_TYPE (expr), integer_zero_node));
}
	  ...
	  if (f == NULL && errno == EACCES)
	    {
	      ...
	      /* xgettext:no-c-format */
	      ffebad_start_msg
		("At %0, INCLUDE file %A exists, but is not readable",
		 FFEBAD_severityWARNING);
	      ffebad_string (fname);
	      ffebad_here (0, l, c);
	      ffebad_finish ();
