...
static void dumpspec (int menu, const char *name, const char *name_uc,
		      ffeintrinSpec spec);
static void dumpimp (int menu, const char *name, const char *name_uc,
...
dumpif (ffeintrinFamily fam)
{
  ...
  if ((in_ifset != 2) || (fam != latest_family))
    {
      if (in_ifset == 2)
	printf ("@end ifset\n");
      latest_family = fam;
...
dumpclearif (void)
{
  if ((in_ifset == 2) || (latest_family != FFEINTRIN_familyNONE))
    printf ("@end ifset\n");
...
dumpem (void)
{
  ...
  for (i = 0; ((size_t) i) < ARRAY_SIZE (names); ++i)
	...
	dumpgen (1, names[i].name_ic, names[i].name_uc, names[i].generic);
      if (names[i].specific != FFEINTRIN_specNONE)
	dumpspec (1, names[i].name_ic, names[i].name_uc, names[i].specific);
    }
  dumpclearif ();
  ...
  for (i = 0; ((size_t) i) < ARRAY_SIZE (names); ++i)
    {
      if (names[i].generic != FFEINTRIN_genNONE)
	dumpgen (0, names[i].name_ic, names[i].name_uc, names[i].generic);
      if (names[i].specific != FFEINTRIN_specNONE)
	dumpspec (0, names[i].name_ic, names[i].name_uc, names[i].specific);
    }
  dumpclearif ();
}
...
dumpgen (int menu, const char *name, const char *name_uc, ffeintrinGen gen)
{
  ...
  for (i = 0; i < ARRAY_SIZE (gens[gen].specs); ++i)
	...
	continue;
      ...
      dumpimp (menu, name, name_uc, i, specs[spec].family,
	       specs[spec].implementation, spec);
      if (!menu && (total > 0))
	{
	  if (total == 1)
	    {
...
{
  dumpif (specs[spec].family);
  dumpimp (menu, name, name_uc, 0, specs[spec].family,
	   specs[spec].implementation, FFEINTRIN_specNONE);
  dumpendif ();
}
...
%s%s(", (subr ? "CALL " : ""), name);
  ...
  for (argno = 0;; ++argno)
	...
	printf (", ");
      ...
      if ((argi[0] == '*')
	  || (argi[0] == 'n') || (argi[0] == '+') || (argi[0] == 'p'))
	printf ("-1, @var{%s}-2, @dots{}, @var{%s}-n", argc, argc);
    }
...
%s: ", name);
      ...
      if ((c[0] == 'R') && (c[1] == 'C'))
	{
	  ...
	  if ((arg_info[0] == '?') || (arg_info[0] == '!')
	      || (arg_info[0] == '+') || (arg_info[0] == '*')
	      || (arg_info[0] == 'n') || (arg_info[0] == 'p'))
	    ++arg_info;
...
on the target system (typically @code{INTEGER(KIND=1)} or @code{INTEGER(KIND=4)}).\n\n");
      ...
      else if (c[1] == '=' && ISDIGIT (c[colon + 1]))
	{
	  ...
	  if (((c[0] == arg_info[0])
	      ...
	      || ((c[0] == 'C') && (arg_info[0] == 'R')))
	    printf
	      (", the @samp{KIND=} value of the type being that of argument @var{%s}.\n\n",
	       arg_string);
	  else if ((c[0] == 'S')
		   && ((arg_info[0] == 'C') || (arg_info[0] == 'F')
		       || (arg_info[0] == 'N')))
	    printf (".\n\
...
@var{");
	...
	case 'I':
	  switch (kind)
	    ...
	    case 'A':
	      printf
		("@code{INTEGER} with same @samp{KIND=} value as for @var{%s}",
		 argument_name_string (imp, 0));
	      break;
	...
	case 'L':
	  switch (kind)
	    ...
	    case 'A':
	      printf
		("@code{LOGICAL} with same @samp{KIND=} value as for @var{%s}",
		 argument_name_string (imp, 0));
	      break;
	...
	case 'R':
	  switch (kind)
	    ...
	    case 'A':
	      printf
		("@code{REAL} with same @samp{KIND=} value as for @var{%s}",
		 argument_name_string (imp, 0));
	      break;
	...
	case 'B':
	  switch (kind)
	    ...
	    case 'N':
	      printf
		("@code{INTEGER} or @code{LOGICAL} not wider than the default kind");
	      break;
	...
	case 'F':
	  switch (kind)
	    ...
	    case 'A':
	      printf ("Same type as @var{%s}", argument_name_string (imp, 0));
	      break;
	...
	case 'N':
	  switch (kind)
	    ...
	    case '9':
	      printf
		("@code{INTEGER(KIND=%d)}, @code{REAL(KIND=%d)}, or @code{COMPLEX(KIND=%d)}",
	...
	case 'S':
	  switch (kind)
	    ...
	    case 'A':
	      printf
		("@code{INTEGER} or @code{REAL} with same @samp{KIND=} value as for @var{%s}",
		 argument_name_string (imp, 0));
	      break;
...
of an executable statement");
	...
	case 's':
	  printf
	    ("Signal handler (@code{INTEGER FUNCTION} or @code{SUBROUTINE})\n\
or dummy/global @code{INTEGER(KIND=1)} scalar");
	...
	case '!':
	  printf
	    ("; OPTIONAL (must be omitted if @var{%s} is @code{COMPLEX})",
