		 ...
		 ffetargetAlign alignment, ffetargetAlign modulo)
{
  ...
  for (ua = *updated_alignment, ucnt = 1;
       ua % alignment != 0; ua += *updated_alignment)
    ++ucnt;
		      ...
		      mallocPool pool)
{
  ...
  if (val->length == 0)
    ...
    {
      val->text =
	malloc_new_kp (pool, "ffetargetCharacter1", val->length + 1);
...
ffetarget_concatenate_character1 (ffetargetCharacter1 * res,
				  ...
				  ffetargetCharacter1 r, mallocPool pool,
				  ffetargetCharacterSize * len)
{
  ...
  if (*len == 0)
    ...
    {
      res->text =
	malloc_new_kp (pool, "ffetargetCharacter1(CONCAT)", *len + 1);
      if (l.length != 0)
			     ...
			     ffetargetCharacterSize * len)
{
  if (last < first)
  ...
  else
    {
      ...
      res->text =
	malloc_new_kp (pool, "ffetargetCharacter1(SUBSTR)", *len + 1);
  ...
  if (size <= (size_t) l.length)
    {
      ...
      for (p = &l.text[0] + size, i = l.length - size; i > 0; ++p, --i)
	if (*p != ' ')
...
void
ffetarget_integer_bad_magical_binary (ffelexToken integer, ffelexToken minus)
{
					       ...
					       ffetargetIntegerDefault r)
{
  ...
  if (ffetarget_iszero_real1 (l.real) && ffetarget_iszero_real1 (l.imaginary))
    {
      ffetarget_real1_zero (&res->real);
      ffetarget_real1_zero (&res->imaginary);
      return FFEBAD;
  ...
  *res = l;
  ...
  while (r != 0)
      ...
      if ((r & 1) == 1)
	{
	  ...
	  if (bad != FFEBAD)
	    return bad;
	  bad = ffetarget_multiply_real1 (&tmp2, res->imaginary, l.imaginary);
	  if (bad != FFEBAD)
	    return bad;
	  bad = ffetarget_subtract_real1 (&tmp, tmp1, tmp2);
	  if (bad != FFEBAD)
					      ...
					      ffetargetIntegerDefault r)
{
  ...
  if (ffetarget_iszero_real2 (l.real) && ffetarget_iszero_real2 (l.imaginary))
    {
      ffetarget_real2_zero (&res->real);
      ffetarget_real2_zero (&res->imaginary);
      return FFEBAD;
  ...
  *res = l;
  ...
  while (r != 0)
      ...
      if ((r & 1) == 1)
	{
	  ...
	  if (bad != FFEBAD)
	    return bad;
	  bad = ffetarget_multiply_real2 (&tmp2, res->imaginary, l.imaginary);
	  if (bad != FFEBAD)
	    return bad;
	  bad = ffetarget_subtract_real2 (&tmp, tmp1, tmp2);
	  if (bad != FFEBAD)
