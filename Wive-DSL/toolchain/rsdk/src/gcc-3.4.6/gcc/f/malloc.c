...
malloc_kill_area_ (mallocPool pool UNUSED, mallocArea_ a)
{
  ...
  malloc_kill_ (a,
		offsetof (struct _malloc_area_, name) + strlen (a->name) + 1);
}
...
=%lu,\n   allocations=%lu, frees=%lu, resizes=%lu, uses=%lu\n   Subpools:\n",
	   p->name, p->allocated, p->freed, p->old_sizes, p->new_sizes, p->allocations, p->frees, p->resizes,
	   p->uses);
...
malloc_pool_kill (mallocPool p)
{
  ...
  malloc_kill_ (p,
		offsetof (struct _malloc_pool_, name) + strlen (p->name) + 1);
}
...
malloc_display_ (mallocArea_ a UNUSED)
{
  ...
  fprintf (dmpout, "At %08lX, size=%" mallocSize_f "u, type=%s, \"%s\"\n",
	   (unsigned long) a->where, a->size, malloc_types_[a->type],
	   a->name);
#endif
}
...
void *
malloc_new_inpool_ (mallocPool pool, mallocType_ type, const char *name,
		    mallocSize s)
{
  void *ptr;
  mallocArea_ a;
  ...
  ptr =
    malloc_new_ (ROUNDED_AREA_SIZE + s +
		 (i = (MALLOC_DEBUG ? strlen (name) + 1 : 0)));
#if MALLOC_DEBUG
...
void *
malloc_new_zinpool_ (mallocPool pool, mallocType_ type, const char *name,
		     mallocSize s, int z)
{
  void *ptr;
...
#endif
  ptr =
    malloc_resize_ (ptr - ROUNDED_AREA_SIZE,
		    ROUNDED_AREA_SIZE + ns +
		    (MALLOC_DEBUG ? strlen (a->name) + 1 : 0));
  temp = (mallocArea_ *) ptr;
  *temp = a;
  ptr = ptr + ROUNDED_AREA_SIZE;
  a->where = ptr;
