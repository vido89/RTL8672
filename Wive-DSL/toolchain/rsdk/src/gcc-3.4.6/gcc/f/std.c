...
ffestd_stmt_pass_ (void)
{
  ...
  for (stmt = ffestd_stmt_list_.first;
       stmt != (ffestdStmt_) & ffestd_stmt_list_.first; stmt = stmt->next)
    {
      switch (stmt->id)
	{
	...
	case FFESTD_stmtidR840_:
	  ffestd_subr_line_restore_ (stmt);
	  if (okay)
	    ffeste_R840 (stmt->u.R840.expr, stmt->u.R840.neg,
			 stmt->u.R840.zero, stmt->u.R840.pos);
  ...
  for (h = ffelab_handle_first (); h != NULL; h = ffelab_handle_next (h))
	...
	{			/* Undefined label. */
	  ...
	  if (ffelab_type (l) == FFELAB_typeLOOPEND)
	    ...
	    ffebad_here (0, ffelab_firstref_line (l),
	    ...
	    ffebad_here (0, ffelab_definition_line (l),
...
void
ffestd_R547_item_object (ffelexToken name UNUSED, ffesttDimList dims UNUSED)
{
  ffestd_check_item_ ();
}
...
void
ffestd_R819B (ffelexToken construct_name UNUSED, ffelab label, ffebld expr)
{
  ffestdStmt_ stmt;
...
#define specified(something) \
  ...
  if (specified (FFESTP_openixACTION)
      ...
      || specified (FFESTP_openixSHARED) || specified (FFESTP_openixUSEROPEN))
    {
...
#define specified(something) \
  ...
  if (specified (FFESTP_readixADVANCE)
      ...
      || specified (FFESTP_readixNULLS) || specified (FFESTP_readixSIZE))
    {
...
ffestd_R909_item (ffebld expr, ffelexToken expr_token)
{
  ...
  item =
    malloc_new_kp (ffesta_output_pool, "ffestdExprItem_", sizeof (*item));
...
#define specified(something) \
  ...
  /* Warn if there are any thing we don't handle via f2c libraries. */
  if (specified (FFESTP_writeixADVANCE) || specified (FFESTP_writeixEOR))
    {
...
ffestd_R1001error_ (ffesttFormatList f)
{
  ...
  ffebad_here (0, ffelex_token_where_line (f->t),
	       ffelex_token_where_column (f->t));
  ffebad_finish ();
}
