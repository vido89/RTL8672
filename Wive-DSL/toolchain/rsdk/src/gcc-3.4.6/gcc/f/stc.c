			    ...
			    ffelexToken lent)
{
  ...
  if (((len == NULL) && (lent == NULL)) || (bt != FFEINFO_basictypeCHARACTER))
			    ...
			    ffelexToken kindt, ffebld len, ffelexToken lent)
{
  ...
  if (((len == NULL) && (lent == NULL)) || (type != FFESTP_typeCHARACTER))
  ...
  if (!ok)
    {
      ...
      ffebad_here (0, ffelex_token_where_line (first),
...
{
  if ((ffesta_label_token == NULL) || !ffestc_labeldef_begin_ ())
    return;
  ...
  if (label == NULL)
    {
      ...
      ffelab_set_definition_line (label,
				  ffewhere_line_use (ffelex_token_where_line
						     (ffesta_label_token)));
...
ffestc_labeldef_branch_begin_ (void)
{
  if ((ffesta_label_token == NULL)
      || (ffestc_shriek_after1_ != NULL) || !ffestc_labeldef_begin_ ())
    return;
  ...
  switch (ffelab_type (ffestc_label_))
	...
	{			/* Unterminated block. */
	  ...
	  ffebad_here (1, ffestw_line (ffestw_stack_top ()),
		       ffestw_col (ffestw_stack_top ()));
...
ffestc_labeldef_endif_ (void)
{
  ...
  switch (ffelab_type (ffestc_label_))
    ...
    case FFELAB_typeASSIGNABLE:
      ...
      ffelab_set_blocknum (ffestc_label_,
			   ffestw_blocknum (ffestw_previous
					    (ffestw_stack_top ())));
...
{
  if ((ffesta_label_token == NULL) || (ffestc_shriek_after1_ != NULL))
    {
  ...
  if (label == NULL)
    {
      ...
      ffelab_set_doref_column (label,
			       ffewhere_column_use (ffelex_token_where_column
						    (label_token)));
    }
  ...
  if (sa & FFESYMBOL_attrsANY)
	...
	na = sa;
      sfref_ok = TRUE;		/* Ok for sym to be ref'd in sfuncdef
		    ...
		    | FFESYMBOL_attrsSFARG | FFESYMBOL_attrsTYPE)))
    na = sa | FFESYMBOL_attrsDUMMY;
  else
    na = FFESYMBOL_attrsetNONE;

  if (!ffesymbol_is_specable (s)
      && (!sfref_ok || (ffesymbol_where (s) != FFEINFO_whereDUMMY)))
  ...
  if (((l = ffestw_label (ffestw_stack_top ())) != NULL)
      ...
      if (ffelab_type (l) != FFELAB_typeANY)
	{
	  ffelab_set_definition_line (l,
				      ffewhere_line_use (ffelab_doref_line
							 (l)));
...
static int
ffestc_subr_speccmp_ (const char *string, ffestpFile * spec,
		      const char **target, int *length)
{
  ffebldConstant c;
  int i;
...
ffestc_try_shriek_do_ (void)
{
  ...
  while ((ffestw_state (ffestw_stack_top ()) == FFESTV_stateDO)
	 ...
	 && (((ty = (ffelab_type (lab)))
	     ...
	     || (ty == FFELAB_typeNOTLOOP) || (ty == FFELAB_typeENDIF)))
...
ffestc_decl_attrib (ffestpAttrib attrib UNUSED,
		    ...
		    ffestrOther intent_kw UNUSED, ffesttDimList dims UNUSED)
{
...
recurse:
  ...
  switch (ffestw_state (b))
	  ...
	  && (ffestw_state (ffestw_previous (b)) != FFESTV_stateINTERFACE0))
	{
	  ...
	  ffebad_here (0, ffelex_token_where_line (ffesta_tokens[0]),
		       ffelex_token_where_column (ffesta_tokens[0]));
	  ffebad_here (1, ffestw_line (ffestw_previous (b)),
  ...
  if (ffestw_state (ffestw_stack_top ()) != FFESTV_stateNIL)
    {
      ...
      ffebad_here (0, ffestw_line (ffestw_stack_top ()),
		   ffestw_col (ffestw_stack_top ()));
      ffebad_finish ();
      do
  ...
  for (block = ffestw_top_do (ffestw_stack_top ());
      ...
      if (ffestw_do_iter_var (block) == s)
	{
	  ffebad_here (i,
		       ...
		       ffelex_token_where_column (ffestw_do_iter_var_t
						  (block)));
	  return;
	}
    }
		  ...
		  ffebld init, ffelexToken initt, bool clist)
{
  ...
  if (is_init)
	...
	;
		     ...
		     | FFESYMBOL_attrsADJUSTABLE | FFESYMBOL_attrsANYSIZE))
	na = FFESYMBOL_attrsetNONE;
      else
    ...
    na = sa;
	   ...
	   || ((sa & (FFESYMBOL_attrsSAVE
	       ...
	       && (na & (FFESYMBOL_attrsADJUSTABLE
			 | FFESYMBOL_attrsANYLEN | FFESYMBOL_attrsANYSIZE))))
    na = FFESYMBOL_attrsetNONE;
  ...
  if (na == FFESYMBOL_attrsetNONE)
      ...
      else if (clist)
	{
	  ...
	  symter = ffebld_new_symter (s, FFEINTRIN_genNONE,
				      FFEINTRIN_specNONE, FFEINTRIN_impNONE);
...
ffestc_R524_item (ffelexToken name, ffesttDimList dims)
{
  ...
  if (!ffesymbol_is_specable (s))
  ...
  else if (!ffesta_is_entry_valid && (sa & FFESYMBOL_attrsANYLEN))
    na = FFESYMBOL_attrsetNONE;
  else if ((sa & FFESYMBOL_attrsARRAY)
    ...
    na = FFESYMBOL_attrsetNONE;
		    ...
		    | FFESYMBOL_attrsEQUIV
		    | FFESYMBOL_attrsNAMELIST
		    | FFESYMBOL_attrsSAVE | FFESYMBOL_attrsTYPE)))
    na |= sa;
  else
    na = FFESYMBOL_attrsetNONE;
...
ffestc_R542_start (void)
{
  ...
  if (ffe_is_f2c_library () && (ffe_case_source () == FFE_caseNONE))
    {
...
ffestc_R547_item_object (ffelexToken name, ffesttDimList dims)
{
  ...
  if (na == FFESYMBOL_attrsetNONE)
    ...
    na = FFESYMBOL_attrsANY;
		  ...
		  | FFESYMBOL_attrsARRAY
		  | FFESYMBOL_attrsINIT
		  | FFESYMBOL_attrsSFARG)) && (na & FFESYMBOL_attrsARRAY))
    na = FFESYMBOL_attrsetNONE;
  else if (!(sa & ~(FFESYMBOL_attrsADJUSTS
		    | FFESYMBOL_attrsARRAY
		    | FFESYMBOL_attrsEQUIV
...
void
ffestc_R804 (ffebld expr, ffelexToken expr_token UNUSED, ffelexToken name)
{
  ffestc_check_simple_ ();
  if (ffestc_order_ifthen_ () != FFESTC_orderOK_)
    return;
  ...
  if (name != NULL)
				    ...
				    ffestw_name (ffestw_stack_top ())) != 0)
	{
	  ...
	  ffebad_here (0, ffelex_token_where_line (name),
		       ffelex_token_where_column (name));
	  ffebad_here (1,
		       ffelex_token_where_line (ffestw_name
						(ffestw_stack_top ())),
		       ffelex_token_where_column (ffestw_name
						  (ffestw_stack_top ())));
	  ffebad_finish ();
	}
    }
...
ffestc_R805 (ffelexToken name)
{
  ...
  if (name != NULL)
      ...
      if (ffestw_name (ffestw_stack_top ()) == NULL)
	{
      ...
      else if (ffelex_token_strcmp (name, ffestw_name (ffestw_stack_top ()))
	       != 0)
	{
	  ffebad_start (FFEBAD_CONSTRUCT_WRONG_NAME);
	  ffebad_here (0, ffelex_token_where_line (name),
  ...
  if (cases == NULL)
      ...
      for (caseobj = cases->next; caseobj != cases; caseobj = caseobj->next)
	{
	  if ((caseobj->expr1 == NULL)
	      && (!caseobj->range || (caseobj->expr2 == NULL)))
	    {			/* "CASE (:)". */
	  ...
	  if (((caseobj->expr1 != NULL)
		   ...
		   || ((ffeinfo_kindtype (ffebld_info (caseobj->expr1))
			!= s->kindtype)
		       && (ffeinfo_kindtype (ffebld_info (caseobj->expr1)) !=
			   FFEINFO_kindtypeINTEGER1)) || ((caseobj->range)
							  && (caseobj->
							      expr2 != NULL)
							  &&
							  ((ffeinfo_basictype
...
ffestc_R819A (ffelexToken construct_name, ffelexToken label_token, ffebld var,
	      ffelexToken var_token, ffebld start, ffelexToken start_token,
	      ffebld end, ffelexToken end_token, ffebld incr,
	      ffelexToken incr_token)
{
  ...
  ffestd_R819A (construct_name, label, var,
		start, start_token, end, end_token, incr, incr_token);
}
...
ffestc_R820A (ffelexToken construct_name, ffebld var, ffelexToken var_token,
	      ffebld start, ffelexToken start_token, ffebld end,
	     ...
	     ffelexToken expr_token UNUSED)
{
  ...
  for (ti = label_toks->first, i = 0;
       ti != (ffesttTokenItem) & label_toks->first; ti = ti->next, ++i)
    {
...
ffestc_R904 (void)
{
  ...
  if (ffestc_subr_is_branch_
      ...
      && ffestc_subr_is_present_ ("UNIT",
				  &ffestp_file.open.
				  open_spec[FFESTP_openixUNIT]))
    {
      i = ffestc_subr_binsrch_ (status_strs,
				ARRAY_SIZE (status_strs),
				&ffestp_file.open.
				open_spec[FFESTP_openixSTATUS],
	  ...
	  && ffestp_file.open.open_spec[FFESTP_openixFILE].kw_or_val_present)
	{
	  ...
	  assert (ffestp_file.open.open_spec[FFESTP_openixFILE].
		  kw_or_val_present);
	  if (ffestp_file.open.open_spec[FFESTP_openixFILE].kw_present)
	    {
	      ffebad_here (0, ffelex_token_where_line
			   ...
			   ffelex_token_where_column
			   (ffestp_file.open.open_spec[FFESTP_openixFILE].
			    kw));
	    }
	  else
	    {
	      ffebad_here (0, ffelex_token_where_line
			   (ffestp_file.open.open_spec[FFESTP_openixFILE].
			    value),
			   ffelex_token_where_column (ffestp_file.open.
						      open_spec
						      [FFESTP_openixFILE].
						      value));
	    }
	  assert (ffestp_file.open.open_spec[FFESTP_openixSTATUS].
		  kw_or_val_present);
	       ...
	       kw_or_val_present)
	{
      ...
      ffestc_subr_binsrch_ (carriagecontrol_strs,
			    ...
			    &ffestp_file.open.
			    open_spec[FFESTP_openixCARRIAGECONTROL],
      ...
      ffestc_subr_binsrch_ (recordtype_strs, ARRAY_SIZE (recordtype_strs),
			    &ffestp_file.open.
			    open_spec[FFESTP_openixRECORDTYPE],
...
ffestc_R907 (void)
{
  ...
  if (ffestc_subr_is_branch_
      ...
      && ffestc_subr_is_present_ ("UNIT",
				  &ffestp_file.close.
				  close_spec[FFESTP_closeixUNIT]))
    {
      ffestc_subr_binsrch_ (status_strs, ARRAY_SIZE (status_strs),
			    &ffestp_file.close.
			    close_spec[FFESTP_closeixSTATUS],
			    "DELETE, KEEP, PRINT, PRINT/DELETE, SAVE, SUBMIT, or SUBMIT/DELETE");
...
ffestc_R909_start (bool only_format)
{
  ...
  unit = ffestc_subr_unit_ (&ffestp_file.read.read_spec[FFESTP_readixUNIT]);
  ...
  if (rec)
	...
	{			/* EOR= specified. */
	  ...
	  if (ffestc_subr_speccmp_ ("No",
				    &ffestp_file.read.
				    read_spec[FFESTP_readixADVANCE], NULL,
				    NULL) != 0)
	    {
	      goto whine_advance;	/* :::::::::::::::::::: */
	    }
	...
	{			/* SIZE= specified. */
	  ...
	  if (ffestc_subr_speccmp_ ("No",
		  ...
		  kw_or_val_present)
		{
		  ...
		  ffebad_here (1, ffelex_token_where_line
			       (ffestp_file.read.
				read_spec[FFESTP_readixADVANCE].kw),
			       ffelex_token_where_column (ffestp_file.read.
							  read_spec
...
ffestc_R910_start (void)
{
  ...
  if (rec)
	...
	{			/* EOR= specified. */
	  ...
	  if (ffestc_subr_speccmp_ ("No",
				    &ffestp_file.write.
				    write_spec[FFESTP_writeixADVANCE], NULL,
				    NULL) != 0)
	    {
	      if (ffestp_file.write.write_spec[FFESTP_writeixADVANCE].
		  kw_or_val_present)
		{
		  ...
		  ffebad_here (1, ffelex_token_where_line
			       (ffestp_file.write.
				write_spec[FFESTP_writeixADVANCE].kw),
			       ffelex_token_where_column (ffestp_file.write.
							  write_spec
...
ffestc_R923A (void)
{
  ...
  if (ffestc_subr_is_branch_
    ...
    {
      by_file =
	ffestp_file.inquire.inquire_spec[FFESTP_inquireixFILE].
	kw_or_val_present;
      by_unit =
	ffestp_file.inquire.inquire_spec[FFESTP_inquireixUNIT].
	kw_or_val_present;
      if (by_file && by_unit)
	{
	  ffebad_start (FFEBAD_CONFLICTING_SPECS);
	  assert (ffestp_file.inquire.inquire_spec[FFESTP_inquireixUNIT].
		  kw_or_val_present);
	  if (ffestp_file.inquire.inquire_spec[FFESTP_inquireixUNIT].
	      kw_present)
	    {
	      ffebad_here (0, ffelex_token_where_line
			   (ffestp_file.inquire.
			    inquire_spec[FFESTP_inquireixUNIT].kw),
			   ffelex_token_where_column (ffestp_file.inquire.
						      inquire_spec
	  ...
	  else
	    {
	      ffebad_here (0, ffelex_token_where_line
			   (ffestp_file.inquire.
			    inquire_spec[FFESTP_inquireixUNIT].value),
			   ffelex_token_where_column (ffestp_file.inquire.
						      inquire_spec
						      [FFESTP_inquireixUNIT].
						      value));
	    }
	  assert (ffestp_file.inquire.inquire_spec[FFESTP_inquireixFILE].
		  kw_or_val_present);
	  if (ffestp_file.inquire.inquire_spec[FFESTP_inquireixFILE].
	      kw_present)
	    {
	      ffebad_here (1, ffelex_token_where_line
			   (ffestp_file.inquire.
			    inquire_spec[FFESTP_inquireixFILE].kw),
			   ffelex_token_where_column (ffestp_file.inquire.
						      inquire_spec
	  ...
	  else
	    {
	      ffebad_here (1, ffelex_token_where_line
			   (ffestp_file.inquire.
			    inquire_spec[FFESTP_inquireixFILE].value),
			   ffelex_token_where_column (ffestp_file.inquire.
						      inquire_spec
						      [FFESTP_inquireixFILE].
						      value));
...
ffestc_R1207_item (ffelexToken name)
{
  ...
  if (!ffesymbol_is_specable (s))
    ...
    na = FFESYMBOL_attrsANY;
  else if (!(sa & ~(FFESYMBOL_attrsDUMMY | FFESYMBOL_attrsTYPE)))
  ...
  if (!ffesymbol_is_specable (s))
  ...
  else if (!(sa & ~FFESYMBOL_attrsTYPE))
    {
	  ...
	  && ((imp == FFEINTRIN_impNONE)
		  ...
		  && (ffeintrin_kindtype (spec) == ffesymbol_kindtype (s)))
#else
	      || 1
#endif
	      ...
	      ffelexToken final UNUSED)
{
  ...
  if (in_func)
	...
	na = FFESYMBOL_attrsANY;
      else if (!(sa & ~(FFESYMBOL_attrsANYLEN | FFESYMBOL_attrsTYPE)))
