  ...
  for (next = list->next; next != list; next = next->next)
	  ...
	  && (ffebld_op (next->upper) == FFEBLD_opCONTER))
	{
	  if (next->lower == NULL)
	    ...
	    low =
	      ffebld_constant_integerdefault (ffebld_conter (next->lower));
	  ...
	  if ((next->next == list)
	      && is_ugly_assumed
	      && (next->lower == NULL)
	      && (high == 1) && (ffebld_conter_orig (next->upper) == NULL))
	    {
	      star = TRUE;
	      ffebld_append_item (&bottom,
				  ffebld_new_bounds (NULL,
						     ffebld_new_star ()));
	      continue;
	    }
	}
  ...
  if (zero)
      ...
      for (next = list->next; next != list; next = next->next)
	{
      ...
      as = ffeexpr_convert (as, list->next->t, NULL,
			    ...
			    FFETARGET_charactersizeNONE, FFEEXPR_contextLET);
    }
  ...
  for (next = list->next; next != list; next = next->next)
    {
	  ...
	  && (ffebld_constant_integerdefault (ffebld_conter (next->upper))
	      == 1) && (ffebld_conter_orig (next->upper) == NULL))
	ugly_assumed = TRUE;
      ...
      if (next->lower != NULL)
	{
      ...
      if (next->upper != NULL)
	{
	  if (ugly_assumed || (ffebld_op (next->upper) == FFEBLD_opSTAR))
	    {
...
void
ffestt_implist_append (ffesttImpList list, ffelexToken first,
		       ffelexToken last)
{
  ffesttImpList new;

  new =
    malloc_new_kp (ffesta_scratch_pool, "FFEST token list", sizeof (*new));
  new->next = list->previous->next;
  new->previous = list->previous;
  new->next->previous = new;
  new->previous->next = new;
...
ffestt_implist_drive (ffesttImpList list,
		      void (*fn) (ffelexToken, ffelexToken))
{
  ffesttImpList next;
