...
/* Static hunks of RTL used by the aliasing code; these are initialized
   once per function to avoid unnecessary RTL allocations.  */
     static GTY (())
     rtx static_reg_base_value[FIRST_PSEUDO_REGISTER];
...
/* Vector of known invariant relationships between registers.  Set in
   ...
   after reload.  */
     static GTY ((length ("alias_invariant_size")))
     rtx *alias_invariant;
     unsigned GTY (())
     int alias_invariant_size;

/* Vector indexed by N giving the initial (unchanging) value known for
   pseudo-register N.  This array is initialized in init_alias_analysis,
   and does not change until end_alias_analysis is called.  */
     static GTY ((length ("reg_known_value_size")))
     rtx *reg_known_value;
...
/* The splay-tree used to store the various alias set entries.  */
     static GTY ((param_is (struct alias_set_entry)))
...
alias_sets_conflict_p (HOST_WIDE_INT set1, HOST_WIDE_INT set2)
{
  ...
  if (ase != 0
      && (ase->has_zero_child
	  || splay_tree_lookup (ase->children, (splay_tree_key) set1)))
    return 1;
...
readonly_fields_p (tree type)
{
  ...
  for (field = TYPE_FIELDS (type); field != 0; field = TREE_CHAIN (field))
	...
	&& (TREE_READONLY (field) || readonly_fields_p (TREE_TYPE (field))))
      return 1;
	  ...
	  if (decl && DECL_POINTER_ALIAS_SET_KNOWN_P (decl))
	      ...
	      if (DECL_POINTER_ALIAS_SET (decl) == -2)
		{
		  ...
		  if (pointed_to_alias_set == 0)
		    ...
		    DECL_POINTER_ALIAS_SET (decl) = pointed_to_alias_set;
		  else
		    {
  ...
  if (superset_entry == 0)
    {
      ...
      superset_entry->children = splay_tree_new_ggc (splay_tree_compare_ints);
...
record_base_value (unsigned int regno, rtx val, int invariant)
{
  ...
  if (GET_CODE (val) == REG)
    {
      VARRAY_RTX (reg_base_value, regno) = REG_BASE_VALUE (val);
      return;
    }
  VARRAY_RTX (reg_base_value, regno) = find_base_value (val);
}
...
memrefs_conflict_p (int xsize, rtx x, int ysize, rtx y, HOST_WIDE_INT c)
{
  ...
  if (GET_CODE (x) == ADDRESSOF)
    {
      if (y == frame_pointer_rtx || GET_CODE (y) == ADDRESSOF)
	return xsize <= 0 || ysize <= 0;
    }
...
fixed_scalar_and_varying_struct_p (rtx mem1, rtx mem2, rtx mem1_addr,
				   rtx mem2_addr, int (*varies_p) (rtx, int))
{
  if (!flag_strict_aliasing)
    return NULL_RTX;
...
nonoverlapping_component_refs_p (tree x, tree y)
{
  ...
  while (x && y
	 && TREE_CODE (x) == COMPONENT_REF && TREE_CODE (y) == COMPONENT_REF);
...
nonoverlapping_memrefs_p (rtx x, rtx y)
{
  ...
  if (TREE_CODE (exprx) == COMPONENT_REF)
  ...
  else if (TREE_CODE (exprx) == INDIRECT_REF)
    {
      ...
      if (flag_argument_noalias < 2 || TREE_CODE (exprx) != PARM_DECL)
	return 0;
    }
  ...
  if (TREE_CODE (expry) == COMPONENT_REF)
  ...
  else if (TREE_CODE (expry) == INDIRECT_REF)
    {
      ...
      if (flag_argument_noalias < 2 || TREE_CODE (expry) != PARM_DECL)
	return 0;
    }
  ...
  sizex = (GET_CODE (rtlx) != MEM ? (int) GET_MODE_SIZE (GET_MODE (rtlx))
	   : MEM_SIZE (rtlx) ? INTVAL (MEM_SIZE (rtlx)) : -1);
  sizey = (GET_CODE (rtly) != MEM ? (int) GET_MODE_SIZE (GET_MODE (rtly))
	   : MEM_SIZE (rtly) ? INTVAL (MEM_SIZE (rtly)) : -1);
...
write_dependence_p (rtx mem, rtx x, int writep, int constp)
{
  ...
  if (!base_alias_check (x_addr, mem_addr, GET_MODE (x), GET_MODE (mem)))
    return 0;
...
memory_modified_1 (rtx x, rtx pat ATTRIBUTE_UNUSED, void *data)
{
  if (GET_CODE (x) == MEM)
    {
      if (anti_dependence (x, (rtx) data)
	  || output_dependence (x, (rtx) data))
	  ...
	  if (INSN_P (insn))
	    {
	      ...
	      if (reload_completed && prologue_epilogue_contains (insn))
		continue;
#endif
		  ...
		  && REGNO (SET_DEST (set)) >= FIRST_PSEUDO_REGISTER)
		{
		  ...
		  if (REG_NOTES (insn) != 0
						   ...
						   XEXP (note, 0)))
		    {
		      ...
		      set_reg_known_equiv_p (regno,
			   ...
			   && (t =
			       get_reg_known_value (REGNO (XEXP (src, 0))))
		  ...
		  else if (REG_N_SETS (regno) == 1 && !rtx_varies_p (src, 1))
		    {
