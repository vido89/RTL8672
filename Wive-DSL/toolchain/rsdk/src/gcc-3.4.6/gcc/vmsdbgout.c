  ...
  switch (GET_CODE (x))
      ...
      if (GET_MODE (x) == VOIDmode)
	{
	  ...
	  if (CONST_DOUBLE_HIGH (x))
	    ...
	    sprintf (buf1, HOST_WIDE_INT_PRINT_DEC, CONST_DOUBLE_LOW (x));
	  strcat (str, buf1);
	}
      else
...
write_modbeg (int dosizeonly)
{
  ...
  totsize +=
    write_debug_string (module_producer, "compiler name", dosizeonly);
	       ...
	       int dosizeonly)
{
  ...
  if (fileid == 1)
	...
	= DST_K_SOURCE;
      ...
      totsize += write_debug_data1 (src_command.dst_b_src_command,
				    "source_corr (SRC FORMFEED)", dosizeonly);
    }

  src_command_size = DST_K_SRC_COMMAND_SIZE + flen + DST_K_SRC_CMDTRLR_SIZE;
  ...
  totsize += write_debug_data1
    (src_command.dst_a_src_cmd_fields.dst_a_src_decl_src.
     dst_b_src_df_filename, "source_corr (filename length)", dosizeonly);
...
vmsdbgout_end_block (register unsigned line, register unsigned blocknum)
{
  ...
  if (debug_info_level > DINFO_LEVEL_TERSE)
    (*targetm.asm_out.internal_label) (asm_out_file, BLOCK_END_LABEL,
				       blocknum);
}
