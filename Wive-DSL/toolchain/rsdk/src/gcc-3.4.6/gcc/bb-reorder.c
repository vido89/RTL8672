...
rotate_loop (edge back_edge, struct trace *trace, int trace_n)
{
  ...
  do
	...
	if (e->dest != EXIT_BLOCK_PTR
	    ...
	    && (e->flags & EDGE_CAN_FALLTHRU) && !(e->flags & EDGE_COMPLEX))
	  {
	    if (is_preferred)
	      {
      ...
      do
	{
	  ...
	  for (e = bb->succ; e; e = e->succ_next)
		...
		continue;
	      ...
	      if (e->dest->rbi->visited && e->dest->rbi->visited != *n_traces)
		continue;
	  ...
	  for (e = bb->succ; e; e = e->succ_next)
	    {
	      if (e == best_edge
		  || e->dest == EXIT_BLOCK_PTR || e->dest->rbi->visited)
		continue;
      ...
      for (e = bb->succ; e; e = e->succ_next)
	{
	  if (e->dest == EXIT_BLOCK_PTR || e->dest->rbi->visited)
	    continue;
