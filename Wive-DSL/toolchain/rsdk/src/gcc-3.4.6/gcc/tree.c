...
int_size_in_bytes (tree type)
{
  ...
  if (t == 0
      ...
      || TREE_OVERFLOW (t) || TREE_INT_CST_HIGH (t) != 0
...
array_type_nelts (tree type)
{
  ...
  return (integer_zerop (min)
	  ? max : fold (build (MINUS_EXPR, TREE_TYPE (max), max, min)));
}

  ...
  switch (TREE_CODE (arg))
    ...
    case VAR_DECL:
      return ((TREE_STATIC (arg) || DECL_EXTERNAL (arg))
	      && !DECL_THREAD_LOCAL (arg) && !DECL_NON_ADDR_CONST_P (arg));
...
save_expr (tree expr)
{
  ...
  t =
    build (SAVE_EXPR, TREE_TYPE (expr), t, current_function_decl, NULL_TREE);
...
build (enum tree_code code, tree tt, ...)
{
  ...
  if (code == CALL_EXPR && !TREE_SIDE_EFFECTS (t))
	...
	for (node = TREE_OPERAND (t, 1); node; node = TREE_CHAIN (node))
...
build1 (enum tree_code code, tree type, tree node)
{
  ...
  if (TREE_CODE_CLASS (code) == '2'
      || TREE_CODE_CLASS (code) == '<' || TREE_CODE_LENGTH (code) != 1)
    abort ();
...
is_attribute_p (const char *attr, tree ident)
{
  ...
  if (attr[0] == '_')
    {
      if (attr[1] != '_'
	  || attr[attr_len - 2] != '_' || attr[attr_len - 1] != '_')
	abort ();
...
tree
build_method_type_directly (tree basetype, tree rettype, tree argtypes)
{
  tree t;
  tree ptype;
  int hashcode;
  ...
  if (TREE_CODE (op) == COMPONENT_REF
	...
	= tree_low_cst (DECL_SIZE (TREE_OPERAND (op, 1)), 1);
	  ...
	  && INT_CST_LT_UNSIGNED (TYPE_SIZE (type),
				  TYPE_SIZE (TREE_TYPE (op))) && (for_type
								  ||
								  !DECL_BIT_FIELD
								  (TREE_OPERAND
								   (op, 1)))
...
variably_modified_type_p (tree type)
{
  ...
  switch (TREE_CODE (type))
	...
	for (parm = TYPE_ARG_TYPES (type);
	     parm && parm != void_list_node; parm = TREE_CHAIN (parm))
...
build_common_tree_nodes (int signed_char)
{
  ...
  unsigned_intHI_type_node =
    make_or_reuse_type (GET_MODE_BITSIZE (HImode), 1);
  unsigned_intSI_type_node =
    make_or_reuse_type (GET_MODE_BITSIZE (SImode), 1);
  unsigned_intDI_type_node =
    make_or_reuse_type (GET_MODE_BITSIZE (DImode), 1);
  unsigned_intTI_type_node =
    make_or_reuse_type (GET_MODE_BITSIZE (TImode), 1);
