...
/* For a REG or SUBREG expression X return the size/offset pair
   as an integer.  */
     ...
     unsigned int rtx_to_bits (rtx x)
{
...
copy_insn_p (rtx insn, rtx * source, rtx * target)
{
  ...
  if ((s_regno < FIRST_PSEUDO_REGISTER
      ...
      || s_regno >= max_normal_pseudo || d_regno >= max_normal_pseudo)
    return 0;
...
undef_to_size_word (rtx reg, unsigned HOST_WIDE_INT * undefined)
{
  ...
  if (*undefined <= 0xffff)
    switch ((int) *undefined)
      ...
      case 0x00f0:
	*undefined = 0;
	return BL_TO_WORD (4, 4);
      case 0x00ff:
	*undefined = 0;
	return BL_TO_WORD (0, 8);
      case 0x0f00:
	*undefined = 0;
	return BL_TO_WORD (8, 4);
      case 0x0ff0:
	*undefined = 0xf0;
	return BL_TO_WORD (8, 4);
      case 0x0fff:
	if (INTEGRAL_MODE_P (GET_MODE (reg)))
	  ...
	  {
	    *undefined = 0;
	    return BL_TO_WORD (0, 12);	/* XFmode */
	  }
      case 0xf000:
	*undefined = 0;
	return BL_TO_WORD (12, 4);
      case 0xff00:
	*undefined = 0;
	return BL_TO_WORD (8, 8);
      case 0xfff0:
	*undefined = 0xf0;
	return BL_TO_WORD (8, 8);
      case 0xffff:
	*undefined = 0;
	return BL_TO_WORD (0, 16);
      }
...
livethrough_conflicts_bb (basic_block bb)
{
  ...
  if (deaths > 0 || bitmap_first_set_bit (all_defs) >= 0)
    EXECUTE_IF_SET_IN_BITMAP (info->live_throughout, first, use_id,
			      {
			      struct web_part *wp =
			      &web_parts[df->def_id + use_id];
			      unsigned int bl =
			      rtx_to_bits (DF_REF_REG (wp->ref));
			      bitmap conflicts; wp = find_web_part (wp);
			      wp->spanned_deaths += deaths;
			      wp->crosses_call |= contains_call;
			      conflicts = get_sub_conflicts (wp, bl);
			      bitmap_operation (conflicts, conflicts,
						all_defs, BITMAP_IOR);}
  );
		 ...
		 struct df_link *all_refs)
{
  ...
  for (i = 0; i < def_id + use_id; i++)
      ...
      if (def2web[i] != NULL)
	{
	  ...
	  if (i >= def_id && TEST_BIT (live_over_abnormal, ref_id))
	...
	if (cl->conflicts)
	  {
	    ...
	    EXECUTE_IF_SET_IN_BITMAP (cl->conflicts, 0, j,
				      {
...
{
  basic_block bb;
  FOR_EACH_BB (bb) if (GET_CODE (BB_END (bb)) == JUMP_INSN)
    {
      struct df_link *link;
...
handle_asm_insn (struct df *df, rtx insn)
{
  ...
  for (i = 0; i < noperands; i++)
	...
	{
	  int c;
	  ra_debug_msg (DUMP_ASM, " ASM constrain Web %d conflicts with:",
			web->id);
	  for (c = 0; c < FIRST_PSEUDO_REGISTER; c++)
	    if (TEST_HARD_REG_BIT (conflict, c))
