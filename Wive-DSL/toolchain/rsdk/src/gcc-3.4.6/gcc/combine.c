	...
	if (GET_CODE (insn) == CODE_LABEL)
	...
	else if (INSN_P (insn))
	  {
		...
		for (nextlinks = LOG_LINKS (link);
		     nextlinks; nextlinks = XEXP (nextlinks, 1))
	    ...
	    for (links = LOG_LINKS (insn); links; links = XEXP (links, 1))
		  ...
		  && reg_mentioned_p (cc0_rtx,
				      SET_SRC (PATTERN (XEXP (links, 0))))
		  && (prev = prev_nonnote_insn (XEXP (links, 0))) != 0
		  && GET_CODE (prev) == INSN && sets_cc0_p (PATTERN (prev))
		  && (next =
		      try_combine (insn, XEXP (links, 0), prev,
				   &new_direct_jump_p)) != 0)
		goto retry;
#endif
...
static void
set_nonzero_bits_and_sign_copies (rtx x, rtx set, void *data ATTRIBUTE_UNUSED)
{
  unsigned int num;
...
try_combine (rtx i3, rtx i2, rtx i1, int *new_direct_jump_p)
{
  ...
  if (cant_combine_insn_p (i3)
      || cant_combine_insn_p (i2) || (i1 && cant_combine_insn_p (i1))
  ...
  i2pat = (GET_CODE (PATTERN (i2)) == PARALLEL
	   ? gen_rtx_SET (VOIDmode, i2dest, i2src) : PATTERN (i2));
...
validate_replacement:
...

  {
	...
	*new_direct_jump_p = 1;
	...
	&& SET_SRC (newpat) == pc_rtx && SET_DEST (newpat) == pc_rtx)
      *new_direct_jump_p = 1;
  }
  ...
  switch (code)
	  ...
	  /* If that didn't work, perhaps the first operand is complex and
	     ...
	     and have a constant moved through some previous computation.  */
		   ...
		   && !(GET_CODE (XEXP (XEXP (x, 0), 0)) == SUBREG
			&&
			(GET_RTX_CLASS
			 (GET_CODE (SUBREG_REG (XEXP (XEXP (x, 0), 0)))) ==
			 'o')))
	    return &XEXP (XEXP (x, 0), 0);
	}
      break;
	...
	case AND:
	  ...
	  if (GET_CODE (XEXP (SET_SRC (x), 1)) == CONST_INT
	      ...
	      && (split =
		  find_single_use (SET_DEST (x), insn, (rtx *) 0)) != 0
      ...
      if (len && pos >= 0 && pos + len <= GET_MODE_BITSIZE (GET_MODE (inner)))
	{
	  ...
	  && (GET_RTX_CLASS (GET_CODE (XEXP (SET_SRC (x), 0))) == 'o'
		  ...
		  &&
		  (GET_RTX_CLASS
		   (GET_CODE (SUBREG_REG (XEXP (SET_SRC (x), 0)))) == 'o'))))
...
#define COMBINE_RTX_EQUAL_P(X,Y)			\
  ...
  if (code == PARALLEL
      ...
      for (i = XVECLEN (x, 0) - 1; i >= 1; i--)
	{
	  ...
	  if (GET_CODE (dest) != REG
	      && GET_CODE (dest) != CC0 && GET_CODE (dest) != PC)
	    {
	  ...
	  if (fmt[i] == 'E')
	  ...
	  else if (fmt[i] == 'e')
	    {
		  ...
		  && (((code == SUBREG || code == ZERO_EXTRACT)
		       && GET_CODE (new) == REG) || code == STRICT_LOW_PART))
		;
  ...
  switch (code)
	  ...
	  && (flag_unsafe_math_optimizations
		  ...
		  >= (GET_MODE_BITSIZE (GET_MODE (XEXP (XEXP (x, 0), 0)))
		      - num_sign_bit_copies (XEXP (XEXP (x, 0), 0),
					     GET_MODE (XEXP
						       (XEXP (x, 0), 0)))))))
	return simplify_gen_unary (FLOAT, mode, XEXP (XEXP (x, 0), 0),
	...
	return simplify_shift_const (NULL_RTX, ASHIFTRT, mode,
	  ...
	  && (reversed = reversed_comparison (XEXP (x, 0), mode,
					      XEXP (XEXP (x, 0), 0),
					      XEXP (XEXP (x, 0), 1))))
	return simplify_gen_unary (NEG, mode, reversed, mode);
	  ...
	  && GET_CODE (XEXP (XEXP (x, 1), 0)) == NEG)
	{
      ...
      if (GET_CODE (XEXP (x, 1)) == MULT && GET_CODE (XEXP (x, 0)) == NEG)
	{
	  rtx in1, in2;
	...
	return simplify_shift_const (NULL_RTX, LSHIFTRT, mode, XEXP (x, 0),
				     i);
      break;
	      ...
	      && !CC0_P (XEXP (x, 0))))
	{
	  ...
	  if (STORE_FLAG_VALUE == 1
	      ...
	      && op1 == const0_rtx
	      && mode == GET_MODE (op0) && nonzero_bits (op0, mode) == 1)
	  ...
	  if (STORE_FLAG_VALUE == -1
	      && new_code == NE && GET_MODE_CLASS (mode) == MODE_INT
	      && op1 == const0_rtx
	      && (num_sign_bit_copies (op0, mode) == GET_MODE_BITSIZE (mode)))
	    return gen_lowpart_for_combine (mode,
					    expand_compound_operation (op0));
	...
	if (len == 1
	  ...
	  {
	    int offset =
	      INTVAL (XVECEXP (op1, 0, 0)) * GET_MODE_SIZE (GET_MODE (x));
...
simplify_if_then_else (rtx x)
{
  ...
  if (comparison_p
	  ...
	  != UNKNOWN) && GET_CODE (XEXP (cond, 0)) == REG)
    {
      HOST_WIDE_INT nzb;
	...
	true_rtx = subst (known_cond (copy_rtx (true_rtx), true_code,
				      from, true_val), pc_rtx, pc_rtx, 0, 0);
  ...
  if (GET_MODE_CLASS (mode) == MODE_INT
      ...
      && comparison_p
      && rtx_equal_p (true_rtx, XEXP (cond, 0)) && !side_effects_p (true_rtx))
    switch (true_code)
      {
      case GT:
      case GE:
  ...
  if ((!FLOAT_MODE_P (mode) || flag_unsafe_math_optimizations)
      && comparison_p
      && rtx_equal_p (XEXP (cond, 0), true_rtx)
      && rtx_equal_p (XEXP (cond, 1), false_rtx) && !side_effects_p (cond))
    switch (true_code)
      {
      case GE:
      case GT:
  ...
  if ((STORE_FLAG_VALUE == 1 || STORE_FLAG_VALUE == -1)
      && comparison_p
      && GET_MODE_CLASS (mode) == MODE_INT && !side_effects_p (x))
    {
	...
	c1 = XEXP (t, 1), op = GET_CODE (t), z = f;
      ...
      /* If an identity-zero op is commutative, check whether there
		...
		|| GET_CODE (t) == XOR) && rtx_equal_p (XEXP (t, 1), f))
  ...
  if (true_code == NE && XEXP (cond, 1) == const0_rtx
    ...
    return
      simplify_shift_const (NULL_RTX, ASHIFT, mode,
			    gen_lowpart_for_combine (mode, XEXP (cond, 0)),
			    i);
...
simplify_set (rtx x)
{
  ...
  if (GET_CODE (src) == SUBREG && subreg_lowpart_p (src)
    ...
    {
      SUBST (SET_DEST (x),
	     gen_lowpart_for_combine (GET_MODE (SUBREG_REG (src)), dest));
  ...
  if (GET_CODE (dest) != PC
	...
	term1 = false_rtx, true_rtx = XEXP (true_rtx, 1), false_rtx =
	  const0_rtx;
      else if (GET_CODE (true_rtx) == IOR
	       && rtx_equal_p (XEXP (true_rtx, 1), false_rtx))
	term1 = false_rtx, true_rtx = XEXP (true_rtx, 0), false_rtx =
	  const0_rtx;
      else if (GET_CODE (false_rtx) == IOR
	       && rtx_equal_p (XEXP (false_rtx, 0), true_rtx))
	term1 = true_rtx, false_rtx = XEXP (false_rtx, 1), true_rtx =
	  const0_rtx;
      else if (GET_CODE (false_rtx) == IOR
	       && rtx_equal_p (XEXP (false_rtx, 1), true_rtx))
	term1 = true_rtx, false_rtx = XEXP (false_rtx, 0), true_rtx =
	  const0_rtx;
...
simplify_logical (rtx x, int last)
{
  ...
  switch (GET_CODE (x))
    ...
    case AND:
      ...
      if (GET_CODE (op0) == XOR
	  && rtx_equal_p (XEXP (op0, 0), op1) && !side_effects_p (op1))
	...
	return apply_distributive_law
	  (gen_binary (XOR, mode,
		       ...
		       gen_binary (IOR, mode, copy_rtx (XEXP (op1, 0)),
				   XEXP (op0, 1))));
      break;
    ...
    case XOR:
      ...
      if (GET_MODE_BITSIZE (mode) <= HOST_BITS_PER_WIDE_INT
	  && (nonzero_bits (op0, mode) & nonzero_bits (op1, mode)) == 0)
	...
	return gen_binary (AND, mode,
			   simplify_gen_unary (NOT, mode, XEXP (op0, 0),
					       mode), op1);

      else if (GET_CODE (op0) == AND
	       && rtx_equal_p (XEXP (op0, 0), op1) && !side_effects_p (op1))
	return gen_binary (AND, mode,
			   simplify_gen_unary (NOT, mode, XEXP (op0, 1),
					       mode), op1);
...
expand_compound_operation (rtx x)
{
  ...
  if (GET_CODE (x) == SIGN_EXTEND
	  ...
	  && ((nonzero_bits (XEXP (x, 0), GET_MODE (XEXP (x, 0)))
	       & ~(((unsigned HOST_WIDE_INT)
		    GET_MODE_MASK (GET_MODE (XEXP (x, 0)))) >> 1)) == 0)))
    {
...
expand_field_assignment (rtx x)
{
  ...
  while (1)
      ...
      if (!SCALAR_INT_MODE_P (compute_mode))
	{
	  ...
	  imode =
	    mode_for_size (GET_MODE_BITSIZE (compute_mode), MODE_INT, 0);
	  if (imode == BLKmode)
	    break;
		 ...
		 int in_dest, int in_compare)
{
  ...
  if (wanted_inner_mode != VOIDmode
      ...
      if (pos_rtx == 0)
	{
	  ...
	  && !spans_byte && is_mode != wanted_inner_mode)
      ...
      && GET_MODE_SIZE (pos_mode) > GET_MODE_SIZE (GET_MODE (pos_rtx)))
    {
	  ...
	  && (GET_MODE_BITSIZE (GET_MODE (pos_rtx)) <= HOST_BITS_PER_WIDE_INT
	      && ((nonzero_bits (pos_rtx, GET_MODE (pos_rtx))
		   & ~(((unsigned HOST_WIDE_INT)
			GET_MODE_MASK (GET_MODE (pos_rtx))) >> 1)) == 0)))
	{
...
extract_left_shift (rtx x, int count)
{
  ...
  switch (code)
    ...
    case AND:
      ...
      if (GET_CODE (XEXP (x, 1)) == CONST_INT
	  && (INTVAL (XEXP (x, 1)) & ((((HOST_WIDE_INT) 1 << count)) - 1)) ==
	  0 && (tem = extract_left_shift (XEXP (x, 0), count)) != 0)
...
make_compound_operation (rtx x, enum rtx_code in_code)
{
  ...
  switch (code)
    ...
    case LSHIFTRT:
	  ...
	  && !have_insn_for (LSHIFTRT, mode)
	  && mode_width <= HOST_BITS_PER_WIDE_INT
	  && (nonzero_bits (XEXP (x, 0), mode) & (1 << (mode_width - 1))) ==
	  0)
	{
	  ...
	  && subreg_lowpart_p (x))
	{
      ...
      if ((GET_CODE (tem) == SIGN_EXTEND
	   || GET_CODE (tem) == ZERO_EXTEND) && subreg_lowpart_p (x))
	{
	       ...
	       rtx reg, int just_select)
{
  ...
  op_mode = ((GET_MODE_CLASS (mode) == GET_MODE_CLASS (GET_MODE (x))
	      && have_insn_for (code, mode)) ? mode : GET_MODE (x));
  ...
  switch (code)
	  ...
	  && GET_MODE_BITSIZE (op_mode) <= HOST_BITS_PER_WIDE_INT)
	{
	  ...
	  inner =
	    force_to_mode (inner, op_mode, inner_mask, reg, next_select);
	  ...
	  && INTVAL (XEXP (x, 1)) < HOST_BITS_PER_WIDE_INT)
	{
	  ...
	  if (GET_MODE_BITSIZE (GET_MODE (x)) > HOST_BITS_PER_WIDE_INT)
		...
		nonzero >>= INTVAL (XEXP (x, 1))
		  + HOST_BITS_PER_WIDE_INT - GET_MODE_BITSIZE (GET_MODE (x));
	    }
	  else
	    {
	...
	{
	  temp =
	    simplify_binary_operation (code == ROTATE ? ROTATERT : ROTATE,
				       GET_MODE (x), GEN_INT (mask), XEXP (x,
									   1));
	  ...
	  && INTVAL (XEXP (XEXP (x, 0), 1)) < HOST_BITS_PER_WIDE_INT)
	{
	  ...
	  x =
	    gen_binary (LSHIFTRT, GET_MODE (x), temp, XEXP (XEXP (x, 0), 1));
    ...
    case IF_THEN_ELSE:
      ...
      SUBST (XEXP (x, 2),
	     gen_lowpart_for_combine (GET_MODE (x),
				      force_to_mode (XEXP (x, 2), mode, mask,
						     reg, next_select)));
      break;
  ...
  /* If X is a SUBREG, we can narrow both the true and false values
					       ...
					       &true0, &false0)))
    {
      true0 = simplify_gen_subreg (mode, true0,
				   ...
				   SUBREG_BYTE (x));
      false0 =
	simplify_gen_subreg (mode, false0, GET_MODE (SUBREG_REG (x)),
			     SUBREG_BYTE (x));
      if (true0 && false0)
	{
	  *ptrue = true0;
...
known_cond (rtx x, enum rtx_code cond, rtx reg, rtx val)
{
  ...
  if (cond == EQ
      && rtx_equal_p (x, reg)
      && !FLOAT_MODE_P (GET_MODE (x)) && !FLOAT_MODE_P (GET_MODE (val)))
    return val;
...
make_field_assignment (rtx x)
{
  ...
  src = force_to_mode (simplify_shift_const (NULL_RTX, LSHIFTRT,
		       ...
		       GET_MODE_BITSIZE (mode) >= HOST_BITS_PER_WIDE_INT
		       ? ~(unsigned HOST_WIDE_INT) 0
		       : ((unsigned HOST_WIDE_INT) 1 << len) - 1, dest, 0);
...
apply_distributive_law (rtx x)
{
  ...
  if (FLOAT_MODE_P (GET_MODE (x)) && !flag_unsafe_math_optimizations)
    return x;
			...
			unsigned HOST_WIDE_INT constop)
{
  ...
  if (GET_CODE (varop) == PLUS && exact_log2 (constop + 1) >= 0)
    {
      rtx o0, o1;
  ...
  if (x && GET_CODE (XEXP (x, 0)) == SUBREG
      && GET_MODE (XEXP (x, 0)) == mode && SUBREG_REG (XEXP (x, 0)) == varop)
    varop = XEXP (x, 0);
  else
...
nonzero_bits1 (rtx x, enum machine_mode mode, rtx known_x,
	       enum machine_mode known_mode, unsigned HOST_WIDE_INT known_ret)
{
  ...
  switch (code)
	...
	nonzero &= GET_MODE_MASK (ptr_mode);
      ...
      if ((x == stack_pointer_rtx
	   || x == frame_pointer_rtx
	   || x == arg_pointer_rtx) && REGNO_POINTER_ALIGN (REGNO (x)))
	{
	  ...
	  && (reg_last_set_label[REGNO (x)] == label_tick
		  ...
		  && !REGNO_REG_SET_P (ENTRY_BLOCK_PTR->next_bb->
				       global_live_at_start, REGNO (x))))
	...
	if ((nonzero & nonzero0) != nonzero)
	  nonzero &= (nonzero0 | nonzero_bits_with_known (XEXP (x, 1), mode));
      }
      break;
	      ...
	      <= HOST_BITS_PER_WIDE_INT))
	{
	  ...
	  if ((LOAD_EXTEND_OP (GET_MODE (SUBREG_REG (x))) == SIGN_EXTEND
	       ? (((nonzero
			...
			<< (GET_MODE_BITSIZE (GET_MODE (SUBREG_REG (x))) -
			    1)))) !=
		   0)) : LOAD_EXTEND_OP (GET_MODE (SUBREG_REG (x))) !=
	       ZERO_EXTEND) || GET_CODE (SUBREG_REG (x)) != MEM)
#endif
	    {
	  ...
	  && INTVAL (XEXP (x, 1)) < HOST_BITS_PER_WIDE_INT)
	{
	  ...
	  if (code == LSHIFTRT)
		...
		inner |=
		  (((HOST_WIDE_INT) 1 << count) - 1) << (width - count);
	    }
	  else if (code == ASHIFT)
	    inner <<= count;
	  else
...
num_sign_bit_copies1 (rtx x, enum machine_mode mode, rtx known_x,
		      enum machine_mode known_mode, unsigned int known_ret)
{
  enum rtx_code code = GET_CODE (x);
  ...
  switch (code)
      ...
      if (SUBREG_PROMOTED_VAR_P (x) && !SUBREG_PROMOTED_UNSIGNED_P (x))
	{
	  ...
	  return MAX ((int) bitwidth
		      - (int) GET_MODE_BITSIZE (GET_MODE (x)) + 1, num0);
	}
	...
	return bitwidth;
      ...
      if (num0 > 1 && (((HOST_WIDE_INT) 1 << (bitwidth - 1)) & nonzero))
	num0--;
	...
	result = MAX ((int) (GET_MODE_BITSIZE (Pmode)
			     - GET_MODE_BITSIZE (ptr_mode) + 1), result);
#endif
      return result;
...
extended_count (rtx x, enum machine_mode mode, int unsignedp)
{
  ...
  return (unsignedp
	  ? (GET_MODE_BITSIZE (mode) <= HOST_BITS_PER_WIDE_INT
	     ...
	     : 0) : num_sign_bit_copies (x, mode) - 1);
}

...
merge_outer_ops (enum rtx_code *pop0, HOST_WIDE_INT * pconst0,
		 ...
		 enum machine_mode mode, int *pcomp_p)
{
  ...
  while (count != 0)
	...
	case SUBREG:
	  ...
	  if (subreg_lowpart_p (varop)
	      ...
	      && (unsigned
		  int) ((GET_MODE_SIZE (GET_MODE (SUBREG_REG (varop))) +
			 (UNITS_PER_WORD - 1)) / UNITS_PER_WORD) ==
	      mode_words)
	    {
...
gen_binary (enum rtx_code code, enum machine_mode mode, rtx op0, rtx op1)
{
  ...
  if (GET_RTX_CLASS (code) == 'c' && swap_commutative_operands_p (op0, op1))
	       ...
	       && GET_CODE (XEXP (op1, 1)) == CONST_INT)
	{
      ...
      /* If both operands are NOT, we can strip off the outer operation
         ...
         NEG, except that this must be an equality comparison.  */
		   ...
		   && (code == EQ || code == NE)))
	op0 = XEXP (op0, 0), op1 = XEXP (op1, 0), code =
	  swap_condition (code);
  ...
  while (GET_CODE (op1) == CONST_INT)
	...
	case LEU:
	  ...
	  /* (unsigned) <= 0x7fffffff is equivalent to >= 0.  */
		   ...
		   && (const_op ==
		       ((HOST_WIDE_INT) 1 << (mode_width - 1)) - 1))
	    {
	      const_op = 0, op1 = const0_rtx;
	      code = GE;
	    }
	  ...
	  if (const_op == 0 && equality_comparison_p
	      ...
	      && INTVAL (XEXP (XEXP (op0, 0), 1)) < HOST_BITS_PER_WIDE_INT)
	    {
		  ...
		  && (GET_RTX_CLASS (GET_CODE (XEXP (XEXP (op0, 0), 0))) ==
		      '<'
		      || ((tem = get_last_value (XEXP (XEXP (op0, 0), 0))) !=
			  0 && GET_RTX_CLASS (GET_CODE (tem)) == '<')))
		{
		  op0 = XEXP (XEXP (op0, 0), 0);
		  continue;
		}
	  ...
	  if (GET_CODE (XEXP (op0, 0)) == SUBREG)
	    {
		  ...
		  && c1 != mask && c1 != GET_MODE_MASK (tmode))
		{
	...
	case ASHIFT:
	  ...
	  if (GET_CODE (XEXP (op0, 1)) == CONST_INT
		  ...
		  0) && mode_width <= HOST_BITS_PER_WIDE_INT
	      && (nonzero_bits (XEXP (op0, 0), mode) &
		  ~(mask >>
		    (INTVAL (XEXP (op0, 1)) + !equality_comparison_p))) == 0)
	    {
...
count_rtxs (rtx x)
{
  ...
  if (GET_RTX_CLASS (code) == '2' || GET_RTX_CLASS (code) == 'c')
	...
	return 2 + 2 * count_rtxs (x0)
	  + count_rtxs (x == XEXP (x1, 0) ? XEXP (x1, 1) : XEXP (x1, 0));

      if ((GET_RTX_CLASS (GET_CODE (x0)) == '2'
	   || GET_RTX_CLASS (GET_CODE (x0)) == 'c')
	  && (x1 == XEXP (x0, 0) || x1 == XEXP (x0, 1)))
	return 2 + 2 * count_rtxs (x1)
	  + count_rtxs (x == XEXP (x0, 0) ? XEXP (x0, 1) : XEXP (x0, 0));
    }
...
update_table_tick (rtx x)
{
  ...
  for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
	...
	/* Check for identical subexpressions.  If x contains
	   ...
	   them.  */
	if (i == 0
	    && (GET_RTX_CLASS (code) == '2' || GET_RTX_CLASS (code) == 'c'))
	  {
  ...
  if (GET_CODE (x) == REG)
	...
	if (reg_last_set_invalid[j]
	    ...
	    || (!(regno >= FIRST_PSEUDO_REGISTER
		  && REG_N_SETS (regno) == 1
		  && (!REGNO_REG_SET_P
		      (ENTRY_BLOCK_PTR->next_bb->global_live_at_start,
		       regno))) && reg_last_set_label[j] > tick))
	  {
	    if (replace)
...
reg_dead_at_p (rtx reg, rtx insn)
{
  ...
  if (insn == 0)
    ...
    {
      FOR_EACH_BB (block) if (insn == BB_HEAD (block))
	break;
	     ...
	     rtx * pnotes)
{
		  ...
		  && INSN_CUID (after_dead) == INSN_CUID (to_insn))))
	{
	  ...
	  if (note != 0 && regno < FIRST_PSEUDO_REGISTER
		  ...
		  > GET_MODE_SIZE (GET_MODE (x))))
	    {
	      ...
	      unsigned int deadend = (deadregno + HARD_REGNO_NREGS (deadregno,
								    GET_MODE
								    (XEXP
								     (note,
								      0))));
  ...
  for (i = 0; i < len; i++)
	...
	move_deaths (XEXP (x, i), maybe_kill_insn, from_cuid, to_insn,
		     pnotes);
    }
}

  ...
  for (note = notes; note; note = next_note)
	...
	case REG_DEAD:
	  ...
	  if (place && REG_NOTE_KIND (note) == REG_DEAD)
		...
		{
		  unsigned int endregno = regno + HARD_REGNO_NREGS (regno,
								    GET_MODE
								    (XEXP
								     (note,
								      0)));
