...
reload_cse_simplify (rtx insn, rtx testreg)
{
  ...
  if (GET_CODE (body) == SET)
      ...
      if (!count && reload_cse_noop_set_p (body))
	{
	  ...
	  if (REG_P (value) && !REG_FUNCTION_VALUE_P (value))
	    value = 0;
      ...
      if (CONSTANT_P (this_rtx) && !references_value_p (this_rtx, 0))
	{
	  ...
	  if (extend_op != NIL)
		...
		case SIGN_EXTEND:
		  ...
		  if (this_val ==
		      trunc_int_for_mode (this_val, GET_MODE (src)))
		    break;
		default:
		  abort ();
		}
...
reload_combine (void)
{
  ...
  FOR_EACH_BB_REVERSE (bb)
  {
	...
	REG_SET_TO_HARD_REG_SET (live, bb->global_live_at_start);
	compute_use_by_pseudos (&live, bb->global_live_at_start);
  ...
  for (insn = get_last_insn (); insn; insn = PREV_INSN (insn))
	  ...
	  && last_label_ruid < reg_state[REGNO (SET_DEST (set))].use_ruid)
	{
	      ...
	      for (i = first_index_reg; i <= last_index_reg; i++)
		{
		  if (TEST_HARD_REG_BIT (reg_class_contents[INDEX_REG_CLASS],
		      ...
		      && HARD_REGNO_NREGS (i, GET_MODE (reg)) == 1)
		    {
		      ...
		      const_reg = index_reg;
		      reg_sum =
			gen_rtx_PLUS (GET_MODE (reg), index_reg, base);
		      break;
		    }
		}
	    }
	  ...
	  if (prev_set != 0
		  ...
		  <= reg_state[regno].use_ruid) && reg_sum != 0)
	    {
	      int i;
...
reload_cse_move2add (rtx first)
{
  ...
  for (insn = first; insn; insn = NEXT_INSN (insn), move2add_luid++)
		  ...
		  if (new_src == const0_rtx)
			   ...
			   narrow_mode = GET_MODE_WIDER_MODE (narrow_mode))
			{
			  if (have_insn_for (STRICT_LOW_PART, narrow_mode)
				...
				GEN_INT (trunc_int_for_mode (INTVAL (src),
							     narrow_mode));
			      rtx new_set = gen_rtx_SET (VOIDmode,
      ...
      if (any_condjump_p (insn) && onlyjump_p (insn))
	{
	  ...
	  if (cnd != NULL_RTX
	      && GET_CODE (cnd) == NE && GET_CODE (XEXP (cnd, 0)) == REG
	      /* The following two checks, which are also in
	         move2add_note_store, are intended to reduce the
	         number of calls to gen_rtx_SET to avoid memory
	         allocation if possible.  */
	      && SCALAR_INT_MODE_P (GET_MODE (XEXP (cnd, 0)))
	      && HARD_REGNO_NREGS (REGNO (XEXP (cnd, 0)),
				   GET_MODE (XEXP (cnd, 0))) == 1
...
move2add_note_store (rtx dst, rtx set, void *data ATTRIBUTE_UNUSED)
{
  ...
  if (SCALAR_INT_MODE_P (mode)
      ...
      if (reg_set_luid[base_regno] <= move2add_last_label_luid)
	{
      ...
      else if (!MODES_OK_FOR_MOVE2ADD (dst_mode, reg_mode[base_regno]))
	goto invalidate;
