  ...
  if (loop->desc.niter > npeel - 1)
      ...
      if (rtl_dump_file)
	{
	  ...
	  fprintf (rtl_dump_file, HOST_WIDEST_INT_PRINT_DEC,
		   (HOST_WIDEST_INT) loop->desc.niter);
...
unroll_loop_constant_iterations (struct loops *loops, struct loop *loop)
{
  ...
  if (desc->postincr)
      ...
      if (exit_mod != (unsigned) max_unroll || desc->may_be_zero)
	{
	  ...
	  if (!duplicate_loop_to_header_edge
	      (loop, loop_preheader_edge (loop), loops, exit_mod + 1,
  ...
  if (!duplicate_loop_to_header_edge (loop, loop_latch_edge (loop),
				      loops, max_unroll,
				      wont_exit, desc->out_edge, remove_edges,
				      &n_remove_edges,
				      DLTHE_FLAG_UPDATE_FREQ))
    abort ();
...
unroll_loop_runtime_iterations (struct loops *loops, struct loop *loop)
{
  ...
  swtch = loop_split_edge_with (loop_preheader_edge (loop), NULL_RTX);
  ...
  for (i = 0; i < n_peel; i++)
	...
	preheader = loop_split_edge_with (loop_preheader_edge (loop),
      ...
      REG_NOTES (jump)
	= gen_rtx_EXPR_LIST (REG_BR_PROB, GEN_INT (p), REG_NOTES (jump));
...
peel_loop_simple (struct loops *loops, struct loop *loop)
{
  ...
  if (!duplicate_loop_to_header_edge (loop, loop_preheader_edge (loop),
				      loops, npeel, wont_exit, NULL, NULL,
...
unroll_loop_stupid (struct loops *loops, struct loop *loop)
{
  ...
  if (!duplicate_loop_to_header_edge (loop, loop_latch_edge (loop),
				      loops, nunroll, wont_exit, NULL, NULL,
				      NULL, DLTHE_FLAG_UPDATE_FREQ))
    abort ();
...
discard_increment (struct loop * loop)
{
  ...
  if (desc->out_edge->src != loop->latch)
	...
	return false;
      ...
      for (insn = BB_HEAD (loop->latch);
	   insn != NEXT_INSN (BB_END (loop->latch)); insn = NEXT_INSN (insn))
	if (INSN_P (insn))
	  return false;
    }
