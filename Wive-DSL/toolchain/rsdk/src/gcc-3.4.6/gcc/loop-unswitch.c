...
static bool may_unswitch_on_p (basic_block, struct loop *, basic_block *);
...
may_unswitch_on_p (basic_block bb, struct loop *loop, basic_block * body)
{
  ...
  for (i = 0; i < loop->num_nodes; i++)
	...
	(test, BB_HEAD (body[i]), NEXT_INSN (BB_END (body[i]))))
      return false;
		      ...
		      rtx cond_checked, int num)
{
  ...
  do
	...
	{
	  /* Remove false path.  */
	  for (e = bbs[i]->succ; !(e->flags & EDGE_FALLTHRU);
	       e = e->succ_next);
	  remove_path (loops, e);
	  free (bbs);
	  repeat = 1;
...
/* Unswitch a LOOP w.r. to given basic block UNSWITCH_ON.  We only support
   ...
   loop otherwise.  */
static struct loop *
unswitch_loop (struct loops *loops, struct loop *loop,
	       basic_block unswitch_on)
{
  ...
  for (latch_edge = loop->latch->rbi->copy->succ;
       latch_edge->dest != loop->header; latch_edge = latch_edge->succ_next);
