...
static bool validate_pch (cpp_reader *, _cpp_file * file,
			  const char *pchname);
  ...
  for (f = pfile->all_files; f; f = f->next_file)
	  ...
	  && f->st.st_size == file->st.st_size)
	{
	  ...
	  same_file_p = read_file (pfile, ref_file)
	    ...
	    && !memcmp (ref_file->buffer, file->buffer, file->st.st_size);
...
open_file_failed (cpp_reader * pfile, _cpp_file * file, int angle_brackets)
{
  ...
  bool print_dep = CPP_OPTION (pfile, deps.style) > (angle_brackets
						     || !!sysp);
...
make_cpp_dir (cpp_reader * pfile, const char *dir_name, int sysp)
{
  ...
  hash_slot = (struct file_hash_entry **)
    htab_find_slot_with_hash (pfile->dir_hash, dir_name,
			      htab_hash_string (dir_name), INSERT);
...
void
cpp_set_include_chains (cpp_reader * pfile, cpp_dir * quote,
			cpp_dir * bracket, int quote_ignores_source_dir)
{
	  ...
	  if (count + 2 > room)
	    {
	      room += 8;
	      dir->name_map =
		xrealloc (dir->name_map, room * sizeof (char *));
	    }
