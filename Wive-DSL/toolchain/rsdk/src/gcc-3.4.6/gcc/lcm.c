		 ...
		 sbitmap * antloc, sbitmap * later, sbitmap * laterin)
{
  ...
  for (e = EXIT_BLOCK_PTR->pred; e != NULL; e = e->pred_next)
    sbitmap_a_and_b (laterin[last_basic_block],
		     laterin[last_basic_block], later[(size_t) e->aux]);
	      ...
	      sbitmap ** insert, sbitmap ** delete)
{
  ...
  if (file)
    {
      dump_sbitmap_vector (file, "laterin", "", laterin,
			   last_basic_block + 1);
		   ...
		   sbitmap * avin)
{
  ...
  while (qlen)
      ...
      else
	{
      ...
      if (sbitmap_union_of_diff_cg
	  (avout[bb->index], avloc[bb->index], avin[bb->index],
	   kill[bb->index]))
		   ...
		   sbitmap * st_avloc, sbitmap * nearer, sbitmap * nearerout)
{
  ...
  for (e = ENTRY_BLOCK_PTR->succ; e != NULL; e = e->succ_next)
    sbitmap_a_and_b (nearerout[last_basic_block],
		     nearerout[last_basic_block], nearer[(size_t) e->aux]);
			   ...
			   sbitmap * insert, sbitmap * delete)
{
  ...
  for (x = 0; x < NUM_EDGES (edge_list); x++)
	...
	sbitmap_difference (insert[x], nearer[x],
  ...
  if (file)
    {
      ...
      dump_sbitmap_vector (file, "st_antloc", "", st_antloc,
			   last_basic_block);
      dump_sbitmap_vector (file, "st_antin", "", st_antin, last_basic_block);
      dump_sbitmap_vector (file, "st_antout", "", st_antout,
			   last_basic_block);
...
#define N_ENTITIES ARRAY_SIZE (num_modes)
  ...
  for (j = n_entities - 1; j >= 0; j--)
	...
	REG_SET_TO_HARD_REG_SET (live_now, bb->global_live_at_start);
	for (insn = BB_HEAD (bb);
  ...
  for (i = 0; i < max_num_modes; i++)
      ...
      for (j = n_entities - 1; j >= 0; j--)
	{
      ...
      FOR_EACH_BB (bb) sbitmap_not (kill[bb->index], transp[bb->index]);
      ...
      for (j = n_entities - 1; j >= 0; j--)
	{
	  ...
	  FOR_EACH_BB_REVERSE (bb) if (TEST_BIT (delete[bb->index], j))
	    {
