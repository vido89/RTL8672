...

     ...
     static hashval_t htab_counts_entry_hash (const void *of)
{
  ...
  while ((tag = gcov_read_unsigned ()))
      ...
      else if (GCOV_TAG_IS_COUNTER (tag) && fn_ident)
	{
	  ...
	  if (!entry)
		...
		("coverage mismatch for function %u while reading execution counters.",
		 fn_ident);
	      error ("number of counters is %d instead of %d",
		     entry->summary.num, n_counts);
	      htab_delete (counts_hash);
	      break;
	    }
		     ...
		     const struct gcov_ctr_summary **summary)
{
  ...
  if (entry->checksum != checksum)
    {
      error
	("coverage mismatch for function '%s' while reading counter '%s'.",
	...
	("coverage mismatch for function '%s' while reading counter '%s'.",
	 IDENTIFIER_POINTER (DECL_ASSEMBLER_NAME (current_function_decl)),
	 ctr_names[counter]);
      error ("number of counters is %d instead of %d", entry->summary.num,
	     expected);
      return 0;
    }
...
build_fn_info_value (const struct function_list *function, tree type)
{
  ...
  value = tree_cons (fields,
		     convert (unsigned_intSI_type_node,
			      build_int_2 (function->ident, 0)), value);
  fields = TREE_CHAIN (fields);

  /* checksum */
  value = tree_cons (fields,
		     convert (unsigned_intSI_type_node,
			      build_int_2 (function->checksum, 0)), value);
  fields = TREE_CHAIN (fields);
  ...
  array_value =
    build_constructor (TREE_TYPE (fields), nreverse (array_value));
...
build_ctr_info_type (void)
{
  ...
  gcov_merge_fn_type =
    build_function_type_list (void_type_node,
			      gcov_ptr_type, unsigned_type_node, NULL_TREE);
...
build_ctr_info_value (unsigned int counter, tree type)
{
  ...
  value = tree_cons (fields,
		     convert (unsigned_intSI_type_node,
			      build_int_2 (prg_n_ctrs[counter], 0)), value);
  fields = TREE_CHAIN (fields);
  ...
  if (prg_n_ctrs[counter])
    {
      ...
      array_type =
	build_index_type (build_int_2 (prg_n_ctrs[counter] - 1, 0));
      array_type =
	build_array_type (TREE_TYPE (TREE_TYPE (fields)), array_type);
  ...
  TREE_NOTHROW (fn) = 1;
  value = tree_cons (fields,
		     build1 (ADDR_EXPR, TREE_TYPE (fields), fn), value);
...
build_gcov_info (void)
{
  ...
  value = tree_cons (field, convert (unsigned_intSI_type_node,
				     build_int_2 (GCOV_VERSION, 0)), value);
  ...
  fields = field;
  value = tree_cons (field, convert (unsigned_intSI_type_node,
				     build_int_2 (local_tick, 0)), value);
  ...
  filename = (filename && da_file_name[0] != '/'
	      ? concat (filename, "/", da_file_name, NULL) : da_file_name);
  ...
  if (n_fns)
    {
      ...
      fn_info_value =
	build_constructor (array_type, nreverse (fn_info_value));
  ...
  value = tree_cons (field,
		     convert (unsigned_type_node,
			      build_int_2 (prg_ctr_mask, 0)), value);
