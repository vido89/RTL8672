...
align_fuzz (rtx start, rtx end, int known_align_log, unsigned int growth)
{
  ...
  for (align_label = uid_align[uid]; align_label;
       align_label = uid_align[uid])
    {
...
compute_alignments (void)
{
  ...
  FOR_EACH_BB (bb)
  {
    ...
    if (GET_CODE (label) != CODE_LABEL || probably_never_executed_bb_p (bb))
      continue;
...
#define MAX_CODE_ALIGN 16
  ...
  for (insn_current_address = 0, insn = first;
	...
	insn_lengths[uid] =
	  asm_insn_count (body) * insn_default_length (insn);
      else if (GET_CODE (body) == SEQUENCE)
	{
	  int i;
	   ...
	   insn != 0; insn = NEXT_INSN (insn))
	{
	  ...
	  if (GET_CODE (insn) == CODE_LABEL)
	      ...
	      if (log > insn_current_align)
		{
		  int align = 1 << log;
		  int new_address =
		    (insn_current_address + align - 1) & -align;
		  insn_lengths[uid] = new_address - insn_current_address;
...
# define NO_PROFILE_COUNTERS	0
  ...
  rtx svrtx =
    targetm.calls.struct_value_rtx (TREE_TYPE (current_function_decl), 1);
  ...
  if (!NO_PROFILE_COUNTERS)
    {
      ...
      (*targetm.asm_out.internal_label) (file, "LP",
...
final_scan_insn (rtx insn, FILE * file, int optimize ATTRIBUTE_UNUSED,
		 int prescan, int nopeepholes ATTRIBUTE_UNUSED, int *seen)
{
#ifdef HAVE_cc0
  rtx set;
#endif
	...
	if (GET_CODE (body) == ADDR_VEC || GET_CODE (body) == ADDR_DIFF_VEC)
	  {
	    ...
	    ASM_OUTPUT_CASE_END (file,
				 CODE_LABEL_NUMBER (PREV_INSN (insn)), insn);
#endif
#endif
	...
	if (GET_CODE (body) == SEQUENCE)
	  {
	    ...
	    next =
	      final_scan_insn (XVECEXP (body, 0, 0), file, 0, prescan, 1,
			       seen);
...
#endif
	    && final_sequence == 0 && dwarf2out_do_frame ())
	  dwarf2out_frame_debug (insn);
#endif
  ...
  if (debug_insn)
    {
      ...
      fprintf (asm_out_file, "\t%s %d\t%s",
	       ASM_COMMENT_START, INSN_UID (debug_insn), insn_data[num].name);
  ...
  for (i = 0; i < nops; i++)
    {
      ...
      if (expr)
	{
	  fprintf (asm_out_file, "%s", addressp ? "*" : "");
