...
remap_decl (tree decl, inline_data * id)
{
  ...
  if (!n)
	  ...
	  && (*lang_hooks.tree_inlining.anon_aggr_type_p) (TREE_TYPE (t)))
	{
	  ...
	  for (src = DECL_ANON_UNION_ELEMS (t); src; src = TREE_CHAIN (src))
	    {
      ...
      n = splay_tree_insert (id->decl_map,
			     (splay_tree_key) decl, (splay_tree_value) t);
    }
  ...
  if (SCOPE_BEGIN_P (scope_stmt) && SCOPE_STMT_BLOCK (scope_stmt))
    {
      ...
      for (old_var = decls ? decls : BLOCK_VARS (old_block);
	   old_var; old_var = TREE_CHAIN (old_var))
	{
	  tree new_var;
...
copy_body_r (tree * tp, int *walk_subtrees, void *data)
{
  ...
  if (TREE_CODE (*tp) == RETURN_EXPR && id->ret_label)
	...
	{
	  *tp = build_stmt (EXPR_STMT, RETURN_STMT_EXPR (return_stmt));
  ...
  /* For a SCOPE_STMT, we must copy the associated block so that we
  ...
  else if (TREE_CODE (*tp) == LABELED_BLOCK_EXPR)
    {
      ...
      splay_tree_insert (id->decl_map,
			 (splay_tree_key) * tp, (splay_tree_value) new_block);
      *tp = new_block;
    }
  else if (TREE_CODE (*tp) == EXIT_BLOCK_EXPR)
    {
      splay_tree_node n = splay_tree_lookup (id->decl_map,
					     (splay_tree_key)
					     TREE_OPERAND (*tp, 0));
...
#endif				/* INLINER_FOR_JAVA */
{
  ...
  for (p = parms, a = args; p; a = a ? TREE_CHAIN (a) : a, p = TREE_CHAIN (p))
	...
	(p, a ? TREE_VALUE (a) : NULL_TREE, fn, argnum);
      ...
      if (TREE_READONLY (p)
	  && !TREE_ADDRESSABLE (p) && value && !TREE_SIDE_EFFECTS (value))
	{
	...
	var_sub = var;
      ...
      splay_tree_insert (id->decl_map,
			 (splay_tree_key) p, (splay_tree_value) var_sub);
...
#endif				/* INLINER_FOR_JAVA */
{
  ...
  /* Register the VAR_DECL as the equivalent for the RESULT_DECL; that
     ...
     automatically replaced by the VAR_DECL.  */
  splay_tree_insert (id->decl_map,
		     (splay_tree_key) result, (splay_tree_value) var);
...
expand_call_inline (tree * tp, int *walk_subtrees, void *data)
{
  ...
  /* Build a statement-expression containing code to initialize the
     ...
     function call.  */
  expr =
    build1 (STMT_EXPR, TREE_TYPE (TREE_TYPE (fn)), make_node (COMPOUND_STMT));
  ...
  if (!DECL_INITIAL (fn) || TREE_CODE (DECL_INITIAL (fn)) != BLOCK)
    abort ();
  ...
  *inlined_body = copy_body (id);
...
#ifndef INLINER_FOR_JAVA
  COMPOUND_BODY (stmt)
    = chainon (COMPOUND_BODY (stmt), build_stmt (LABEL_STMT, id->ret_label));
#else /* INLINER_FOR_JAVA */
  {
  ...
  COMPOUND_BODY (stmt) = chainon (COMPOUND_BODY (stmt), scope_stmt);
#else /* INLINER_FOR_JAVA */
  if (retvar)
    {
...
tail_recurse:
  ...
  if (code != EXIT_BLOCK_EXPR
      && code != SAVE_EXPR && IS_EXPR_CODE_CLASS (TREE_CODE_CLASS (code)))
#endif /* INLINER_FOR_JAVA */
    {
      int i, len;
	...
	{
	  if (code == DECL_STMT
	      && DECL_STMT_DECL (*tp) && DECL_P (DECL_STMT_DECL (*tp)))
	    {
  ...
  result =
    (*lang_hooks.tree_inlining.walk_subtrees) (tp, &walk_subtrees, func, data,
					       htab);
  ...
  if (!n)
    {
      ...
      n = splay_tree_insert (st, (splay_tree_key) * tp, (splay_tree_value) t);
