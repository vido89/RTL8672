...
static cpp_num num_bitwise_op (cpp_reader *, cpp_num, cpp_num,
			       enum cpp_ttype);
...
interpret_float_suffix (const uchar * s, size_t len)
{
  ...
  return ((i ? CPP_N_IMAGINARY : 0)
	  | (f ? CPP_N_SMALL : l ? CPP_N_LARGE : CPP_N_MEDIUM));
}
...
interpret_int_suffix (const uchar * s, size_t len)
{
  ...
  return ((i ? CPP_N_IMAGINARY : 0)
	  | (u ? CPP_N_UNSIGNED : 0)
	  | ((l == 0) ? CPP_N_SMALL : (l == 1) ? CPP_N_MEDIUM : CPP_N_LARGE));
}
...
cpp_classify_number (cpp_reader * pfile, const cpp_token * token)
{
  ...
  enum
  { NOT_FLOAT = 0, AFTER_POINT, AFTER_EXPON } float_flag;
  ...
  if (float_flag != NOT_FLOAT)
      ...
      if (result == 0)
	{
      ...
      if (limit != str
	  && CPP_WTRADITIONAL (pfile) && !cpp_sys_macro_p (pfile))
      ...
      if (CPP_WTRADITIONAL (pfile) && !cpp_sys_macro_p (pfile))
	{
	  ...
	  && !CPP_OPTION (pfile, c99) && CPP_OPTION (pfile, warn_long_long))
	cpp_error (pfile, CPP_DL_PEDWARN,
...
static cpp_num
num_binary_op (cpp_reader * pfile, cpp_num lhs, cpp_num rhs,
	       enum cpp_ttype op)
{
  cpp_num result;
  size_t precision = CPP_OPTION (pfile, precision);
  ...
  switch (op)
    ...
    default:			/* case CPP_COMMA: */
      if (CPP_PEDANTIC (pfile) && (!CPP_OPTION (pfile, c99)
				   || !pfile->state.skip_eval))
	cpp_error (pfile, CPP_DL_PEDWARN, "comma operator in operand of #if");
      lhs = rhs;
      break;
    }
