...
function_cannot_inline_p (tree fndecl)
{
  ...
  for (parms = DECL_ARGUMENTS (fndecl); parms; parms = TREE_CHAIN (parms))
	...
	return
	  N_("no prototype, and parameter address used; cannot be inline");
    }
#endif
  ...
  for (parms = DECL_ARGUMENTS (fndecl); parms; parms = TREE_CHAIN (parms))
	...
	return
	  N_("function with transparent unit parameter cannot be inline");
    }

  if (get_max_uid () > max_insns)
    {
      for (ninsns = 0, insn = get_first_nonparm_insn ();
	   insn && ninsns < max_insns; insn = NEXT_INSN (insn))
	if (INSN_P (insn))
	  ninsns++;
  ...
  if (DECL_RTL_SET_P (DECL_RESULT (fndecl)))
	...
	return
	  N_("inline functions not supported for this return value type");
    }
...
initialize_for_inline (tree fndecl)
{
  ...
  for (parms = DECL_ARGUMENTS (fndecl), i = 0;
       parms; parms = TREE_CHAIN (parms), i++)
    {
...
static void
note_modified_parmregs (rtx reg, rtx x ATTRIBUTE_UNUSED,
			...
			tree type, rtx structure_value_addr)
{
  ...
  rtx insns = (inl_f->inl_last_parm_insn
	       ? NEXT_INSN (inl_f->inl_last_parm_insn) : parm_insns);
  tree *arg_trees;
  rtx *arg_vals;
  int max_regno;
  int i;
  ...
  for (formal = DECL_ARGUMENTS (fndecl), actual = parms;
       formal; formal = TREE_CHAIN (formal), actual = TREE_CHAIN (actual))
    {
      tree arg;
	...
	return (rtx) (size_t) - 1;
      ...
      if (arg == error_mark_node || mode != TYPE_MODE (TREE_TYPE (arg))
  ...
  for (formal = DECL_ARGUMENTS (fndecl), actual = parms, i = 0;
	  ...
	  if (GET_MODE (loc) != TYPE_MODE (TREE_TYPE (arg)))
	    {
	      ...
	      pmode = promote_mode (TREE_TYPE (formal), pmode, &unsignedp, 0);
	...
	RTX_UNCHANGING_P (arg_vals[i]) = 0;
      ...
      if (arg_vals[i] != 0 && (!TREE_READONLY (formal)
				   ...
				   && reg_overlap_mentioned_p (arg_vals[i],
							       target))
  ...
  VARRAY_CONST_EQUIV_INIT (map->const_equiv_varray,
			   (max_reg_num ()
			    ...
			    + 15 * nargs + 10), "expand_inline_function");
  map->const_age = 0;
  ...
  if (TYPE_MODE (type) == VOIDmode)
	...
	abort ();
      if (GET_CODE (XEXP (loc, 0)) != REG || GET_CODE (XEXP (loc, 1)) != REG)
	abort ();
  ...
  qsort (&VARRAY_TREE (map->block_map, 0),
	 map->block_map->elements_used, sizeof (tree), compare_blocks);
  ...
  if (target
    ...
    target =
      copy_blkmode_from_reg (0, target, TREE_TYPE (TREE_TYPE (fndecl)));
	...
	case INSN:
	  ...
	  if (map->inline_target == 0
	      ...
	      if (volatile_refs_p (SET_SRC (set)))
		{
		  ...
		  copy =
		    emit_insn (copy_rtx_and_substitute (pattern, map, 0));
	  ...
	  /* If this is the virtual frame pointer, make space in current
		...
		= force_reg (Pmode, force_operand (loc, NULL_RTX));
	      ...
	      return temp;
		   ...
		   || (map->integrating
		       && (DECL_SAVED_INSNS (map->fndecl)->
			   internal_arg_pointer == orig)))
	    {
...
#endif
	{
	  rtx copy = gen_rtx_MEM (GET_MODE (XEXP (orig, 0)),
				  copy_rtx_and_substitute (XEXP
							   (XEXP (orig, 0),
							    0),
							   map, 0));
...
subst_constants (rtx * loc, rtx insn, struct inline_remap *map, int memonly)
{
  ...
  switch (code)
	...
	if (GET_CODE (SET_SRC (x)) == COMPARE)
	  {
	    src = SET_SRC (x);
	    if (GET_MODE_CLASS (GET_MODE (src)) == MODE_CC || CC0_P (dest))
	      {
	...
	if (code == IF_THEN_ELSE)
	  {
		...
		rtx temp = simplify_relational_operation (GET_CODE (op0),
							  map->compare_mode,
							  XEXP (op0, 0),
							  XEXP (op0, 1));
  ...
  if (regno >= 0)
    {
      ...
      unsigned int last_reg = (uregno >= FIRST_PSEUDO_REGISTER ? uregno
			       : uregno + HARD_REGNO_NREGS (uregno,
							    mode) - 1);
      unsigned int i;
...
set_block_origin_self (tree stmt)
{
  if (BLOCK_ABSTRACT_ORIGIN (stmt) == NULL_TREE)
	...
	for (local_decl = BLOCK_VARS (stmt);
	     local_decl != NULL_TREE; local_decl = TREE_CHAIN (local_decl))
	...
	for (subblock = BLOCK_SUBBLOCKS (stmt);
	     subblock != NULL_TREE; subblock = BLOCK_CHAIN (subblock))
...
static void
setup_initial_hard_reg_value_integration (struct function *inl_f,
