...
haifa_classify_insn (rtx insn)
{
  ...
  if (GET_CODE (pat) == PARALLEL)
	...
	case SET:
	  ...
	  /* Test if it is a load.  */
	  tmp_class =
	    WORST_CLASS (tmp_class, may_trap_exp (SET_SRC (pat), 0));
	  break;
	case COND_EXEC:
	case TRAP_IF:
	  tmp_class = TRAP_RISKY;
...
HAIFA_INLINE int
actual_hazard_this_instance (int unit, int instance, rtx insn, int clock,
			     int cost)
{
  ...
  if (ready->first == ready->n_ready)
    {
      memmove (ready->vec + ready->veclen - ready->n_ready,
	       ready_lastpos (ready), ready->n_ready * sizeof (rtx));
...
advance_one_cycle (void)
{
  if (targetm.sched.use_dfa_pipeline_interface
	...
	state_transition (curr_state, (*targetm.sched.dfa_pre_cycle_insn) ());
...
schedule_insn (rtx insn, struct ready_list *ready, int clock)
{
  ...
  if (targetm.sched.use_dfa_pipeline_interface
      && (*targetm.sched.use_dfa_pipeline_interface) () && sched_verbose >= 1)
    {
      char buf[2048];
...
{
  if (GET_CODE (x) == CLOBBER && register_operand (SET_DEST (x), VOIDmode))
    return 1;
  if (GET_CODE (x) == SET && register_operand (SET_DEST (x), VOIDmode))
    {
...
schedule_block (int b, int rgn_n_insns)
{
  ...
  while ((*current_sched_info->schedule_more_p) ())
      ...
      for (;;)
	{
	  ...
	  if (sched_verbose >= 2)
	    {
	      fprintf (sched_dump, ";;\tReady list (t =%3d):  ", clock_var);
	      debug_ready_list (&ready);
	    }
	  ...
	  if (!targetm.sched.use_dfa_pipeline_interface
	    ...
	    {
	      if (ready.n_ready == 0 && can_issue_more && reload_completed)
		{
...
set_priorities (rtx head, rtx tail)
{
  rtx insn;
  int n_insn;
  int sched_max_insns_priority = current_sched_info->sched_max_insns_priority;
  rtx prev_head;
...
sched_init (FILE * dump_file)
{
  ...
  if ((!targetm.sched.use_dfa_pipeline_interface
       || !(*targetm.sched.use_dfa_pipeline_interface) ()) && sched_verbose)
  ...
  if (NEXT_INSN (insn) == 0
	  ...
	  && GET_CODE (NEXT_INSN (insn)) != BARRIER))
    {
      ...
      BB_END (EXIT_BLOCK_PTR->prev_bb) =
	PREV_INSN (BB_END (EXIT_BLOCK_PTR->prev_bb));
    }

  /* Compute INSN_REG_WEIGHT for all blocks.  We must do this before
     removing death notes.  */
  FOR_EACH_BB_REVERSE (b) find_insn_reg_weight (b->index);
}
