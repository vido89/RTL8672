...
et_recomp_min (struct et_occ *occ)
{
  ...
  if (!mson || (occ->next && mson->min > occ->next->min))
    mson = occ->next;
...
et_check_occ_sanity (struct et_occ *occ)
{
  ...
  if (occ->parent && occ->parent->prev != occ && occ->parent->next != occ)
    abort ();
}
...
check_path_after_1 (struct et_occ *occ, int depth)
{
  ...
  len--;
  if (depths[len] != depth || datas[len] != occ->of)
    abort ();
...
et_new_occ (struct et_node *node)
{
  ...
  if (!et_occurences)
    et_occurences =
      create_alloc_pool ("et_occ pool", sizeof (struct et_occ), 300);
...
et_new_tree (void *data)
{
  ...
  if (!et_nodes)
    et_nodes =
      create_alloc_pool ("et_node pool", sizeof (struct et_node), 300);
