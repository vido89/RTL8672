...
find_unreachable_blocks (void)
{
  ...
  FOR_EACH_BB (bb) bb->flags &= ~BB_REACHABLE;
...
verify_edge_list (FILE * f, struct edge_list *elist)
{
    ...
    FOR_BB_BETWEEN (s, ENTRY_BLOCK_PTR->next_bb, NULL, next_bb)
  {
    ...
    if (EDGE_INDEX (elist, p, s) == EDGE_INDEX_NO_EDGE && found_edge != 0)
...
int
find_edge_index (struct edge_list *edge_list, basic_block pred,
		 basic_block succ)
{
  int x;
...
void
flow_edge_list_print (const char *str, const edge * edge_list, int num_edges,
		      FILE * file)
{
  int i;
...
add_noreturn_fake_exit_edges (void)
{
  ...
  FOR_EACH_BB (bb) if (bb->succ == NULL)
...
flow_depth_first_order_compute (int *dfs_order, int *rc_order)
{
  ...
  while (sp)
	...
	{
	  if (!e->succ_next && src != ENTRY_BLOCK_PTR && rc_order)
...
flow_dfs_compute_reverse_init (depth_first_search_ds data)
{
  ...
  /* Allocate bitmap to track nodes that have been visited.  */
  data->visited_blocks =
    sbitmap_alloc (last_basic_block - (INVALID_BLOCK + 1));
