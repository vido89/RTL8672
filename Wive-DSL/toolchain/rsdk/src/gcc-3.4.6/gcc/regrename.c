	...
	for (new_reg = 0; new_reg < FIRST_PSEUDO_REGISTER; new_reg++)
	  {
	    ...
	    for (i = nregs - 1; i >= 0; --i)
		  ...
		  || fixed_regs[new_reg + i] || global_regs[new_reg + i]
	      ...
	      enum scan_actions action, enum op_type type, int earlyclobber)
{
  ...
  for (p = &open_chains; *p;)
      ...
      else
	{
	  ...
	  if (regno + nregs <= this_regno || this_regno + this_nregs <= regno)
	    {
	      p = &this->next_chain;
	      continue;
	    }
		  ...
		  enum scan_actions action, enum machine_mode mode)
{
  ...
  switch (code)
	...
	if (locB)
	  scan_rtx_address (insn, locB, MODE_BASE_REG_CLASS (mode), action,
			    mode);
	return;
      }
...
static void set_value_regno (unsigned, enum machine_mode,
			     struct value_data *);
  ...
  if (vd->e[regno].oldest_regno != regno)
    {
      for (i = vd->e[regno].oldest_regno;
	   vd->e[i].next_regno != regno; i = vd->e[i].next_regno)
	continue;
...
copyprop_hardreg_forward (void)
{
  ...
  FOR_EACH_BB (bb)
  {
	...
	&& bb->pred->src != ENTRY_BLOCK_PTR && bbp)
...
debug_value_data (struct value_data *vd)
{
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; ++i)
	...
	for (j = vd->e[i].next_regno;
	     j != INVALID_REGNUM; j = vd->e[j].next_regno)
	  {
...
validate_value_data (struct value_data *vd)
{
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; ++i)
	...
	for (j = vd->e[i].next_regno;
	     j != INVALID_REGNUM; j = vd->e[j].next_regno)
	  {
	    ...
	    if (vd->e[j].oldest_regno != i)
	      internal_error
		("validate_value_data: [%u] Bad oldest_regno (%u)", j,
		 vd->e[j].oldest_regno);
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; ++i)
	...
	&& (vd->e[i].mode != VOIDmode
	    ...
	    || vd->e[i].next_regno != INVALID_REGNUM))
      internal_error
	("validate_value_data: [%u] Non-empty reg in chain (%s %u %i)", i,
