...
static const cpp_token *new_string_token (cpp_reader *, uchar *,
					  unsigned int);
static const cpp_token *stringify_arg (cpp_reader *, macro_arg *);
static void paste_all_tokens (cpp_reader *, const cpp_token *);
static bool paste_tokens (cpp_reader *, const cpp_token **,
			  const cpp_token *);
...
static bool
paste_tokens (cpp_reader * pfile, const cpp_token ** plhs,
	      const cpp_token * rhs)
{
...
bool
_cpp_arguments_ok (cpp_reader * pfile, cpp_macro * macro,
		   const cpp_hashnode * node, unsigned int argc)
{
	  ...
	  if (dest != first)
	    {
		  ...
		  && macro->variadic && src->val.arg_no == macro->paramc)
		{
...
static void
push_ptoken_context (cpp_reader * pfile, cpp_hashnode * macro,
		     _cpp_buff * buff, const cpp_token ** first,
		     unsigned int count)
{
...
bool
_cpp_save_parameter (cpp_reader * pfile, cpp_macro * macro,
		     cpp_hashnode * node)
{
  unsigned int len;
...
_cpp_create_definition (cpp_reader * pfile, cpp_hashnode * node)
{
  ...
  if (node->type == NT_MACRO)
	...
	{
	  cpp_error_with_line (pfile, CPP_DL_PEDWARN, pfile->directive_line,
			       0, "\"%s\" redefined", NODE_NAME (node));
