...
bitmap_element_link (bitmap head, bitmap_element * element)
{
  ...
  /* If this index is less than that of the current element, it goes someplace
  ...
  else if (indx < head->indx)
    {
      for (ptr = head->current;
	   ptr->prev != 0 && ptr->prev->indx > indx; ptr = ptr->prev)
	;
  ...
  /* Otherwise, it must go someplace after the current element.  */
    ...
    {
      for (ptr = head->current;
	   ptr->next != 0 && ptr->next->indx < indx; ptr = ptr->next)
	;
...
bitmap_find_bit (bitmap head, unsigned int bit)
{
  ...
  if (head->current == 0 || head->indx == indx)
    return head->current;

  if (head->indx > indx)
    for (element = head->current;
	 element->prev != 0 && element->indx > indx; element = element->prev)
      ;

  else
    for (element = head->current;
	 element->next != 0 && element->indx < indx; element = element->next)
      ;
...
#error "Fill out the table."
  ...
  return (ptr->indx * BITMAP_ELEMENT_ALL_BITS
	  + word_num * BITMAP_WORD_BITS + bit_num);
}
	      ...
	      const char *suffix)
{
  ...
  EXECUTE_IF_SET_IN_BITMAP (head, 0, i,
			    {
			    fprintf (file, "%s%d", comma, i); comma = ", ";});
  fputs (suffix, file);
}
