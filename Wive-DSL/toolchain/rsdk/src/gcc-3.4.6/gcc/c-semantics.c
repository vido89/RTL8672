...
genrtl_decl_stmt (tree t)
{
  ...
  else if (TREE_CODE (decl) == LABEL_DECL && C_DECLARED_LABEL_FLAG (decl))
  ...
  if (block && SCOPE_END_P (t))
      ...
      for (fn = BLOCK_VARS (block); fn; fn = TREE_CHAIN (fn))
	{
	  if (TREE_CODE (fn) == FUNCTION_DECL
	      ...
	      && !TREE_ASM_WRITTEN (fn) && TREE_ADDRESSABLE (fn))
	    {
...
genrtl_case_label (tree case_label)
{
  ...
  if (cleanup)
	...
	{
	  warning
	    ("(enclose actions of previous case statements requiring destructors in their own scope.)");
	  explained = 1;
	}
    }
