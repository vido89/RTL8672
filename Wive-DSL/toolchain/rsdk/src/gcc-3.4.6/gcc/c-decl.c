...
static
GTY (())
     struct stmt_tree_s c_stmt_tree;

/* The current scope statement stack.  */

     static GTY (())
     tree c_scope_stmt_stack;
...
/* A list of external DECLs that appeared at block scope when there was
   some other global meaning for that identifier.  */
     static GTY (())
     tree truly_local_externals;

/* All the builtins; this is a subset of the entries of global_scope.  */

     static GTY (())
     tree first_builtin_decl;
     static GTY (())
     tree last_builtin_decl;
  ...
  if (TREE_CODE (decl) == VAR_DECL)
    {
	  ...
	  && !DECL_EXTERNAL (decl) && TYPE_DOMAIN (type) == 0)
	{
...
objc_mark_locals_volatile (void *enclosing_blk)
{
  ...
  for (scope = current_scope;
       scope && scope != enclosing_blk; scope = scope->outer)
    {
      tree decl;
...
#define END_OF_ARGLIST(t) (TYPE_MAIN_VARIANT (t) == void_type_node)
  ...
  for (;;)
	...
	{
	  error
	    ("%Jprototype for '%D' declares arg %d with incompatible type",
	     newdecl, newdecl, i);
	  return false;
	}
			   ...
			   tree * newtypep, tree * oldtypep)
{
  ...
  *newtypep = newtype = TREE_TYPE (newdecl);
  ...
  if (!warned && !pedwarned && warn_redundant_decls
...
merge_decls (tree newdecl, tree olddecl, tree newtype, tree oldtype)
{
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
      ...
      else
	{
	  ...
	  if (DECL_DECLARED_INLINE_P (newdecl) && !DECL_UNINLINABLE (newdecl))
	    DECL_INLINE (newdecl) = 1;
	}
    }
  ...
  if (name)
    {
	  ...
	  && scope != global_scope
	  && DECL_EXTERNAL (x) && !DECL_IN_SYSTEM_HEADER (x))
...
groktypename (tree typename)
{
  ...
  typename = grokdeclarator (TREE_VALUE (typename), specs, TYPENAME, 0, NULL);
...
groktypename_in_parm_context (tree typename)
{
  ...
  return grokdeclarator (TREE_VALUE (typename),
			 TREE_PURPOSE (typename), PARM, 0, NULL);
}
...
start_decl (tree declarator, tree declspecs, int initialized, tree attributes)
{
  ...
  decl = grokdeclarator (declarator, declspecs, NORMAL, initialized, NULL);
  ...
  if (TREE_CODE (decl) == VAR_DECL
      && !initialized
      && TREE_PUBLIC (decl) && !DECL_THREAD_LOCAL (decl) && !flag_no_common)
    DECL_COMMON (decl) = 1;
  ...
  if (current_scope != global_scope
      ...
      && !DECL_RTL_SET_P (tem) && DECL_FILE_SCOPE_P (tem))
    {
...
finish_decl (tree decl, tree init, tree asmspec_tree)
{
  ...
  if (TREE_CODE (decl) == VAR_DECL)
	...
	layout_decl (decl, 0);
	  ...
	  && TREE_TYPE (decl) != error_mark_node && (TREE_STATIC (decl) ?
	  ...
	  if (asmspec)
		...
		DECL_C_HARD_REGISTER (decl) = 1;
	      ...
	      /* If this is not a static variable, issue a warning.
	         ...
	         this case.  */
	      if (TREE_CODE (decl) == VAR_DECL
		  && !DECL_REGISTER (decl) && !TREE_STATIC (decl))
...
push_parm_decl (tree parm)
{
  ...
  decl = grokdeclarator (TREE_VALUE (TREE_PURPOSE (parm)),
			 TREE_PURPOSE (TREE_PURPOSE (parm)), PARM, 0, NULL);
...
static void
check_bitfield_type_and_width (tree * type, tree * width,
			       const char *orig_name)
{
  ...
  if (pedantic
      ...
      && type_mv != unsigned_type_node && type_mv != boolean_type_node)
		...
		enum decl_context decl_context, int initialized, tree * width)
{
  ...
  if (specbits & ((1 << (int) RID_LONG) | (1 << (int) RID_SHORT)
	...
	error ("both long and short specified for `%s'", name);
		...
		|| (specbits & 1 << (int) RID_SHORT)) && explicit_char)
...

  {
    ...
    if (nclasses > 1)
      error ("multiple storage classes in declaration of `%s'", name);
		 ...
		 & ((1 << (int) RID_REGISTER)
		    | (1 << (int) RID_AUTO)
		    | (1 << (int) RID_TYPEDEF) | (1 << (int) RID_THREAD))))
      {
	if (specbits & 1 << (int) RID_AUTO
	...
	if (TREE_CODE (type) == ARRAY_TYPE)
	  {
		...
		int erred = 0;
		split_specs_attrs (array_ptr_quals, &new_ptr_quals,
				   &new_ptr_attrs);
		...
		restrictp = 0;
		for (; new_ptr_quals;
		     new_ptr_quals = TREE_CHAIN (new_ptr_quals))
		  {
...
get_parm_info (int void_at_end)
{
  ...
  if (void_at_end && parms != 0
      && TREE_CHAIN (parms) == 0
      && VOID_TYPE_P (TREE_TYPE (parms)) && !DECL_NAME (parms))
    {
      if (TREE_THIS_VOLATILE (parms)
	  || TREE_READONLY (parms) || DECL_REGISTER (parms))
  ...
  if (values != error_mark_node)
      ...
      for (pair = values; pair; pair = TREE_CHAIN (pair))
	{
	  ...
	  if (int_fits_type_p (DECL_INITIAL (enu), enum_value_type))
	    DECL_INITIAL (enu) =
	      convert (enum_value_type, DECL_INITIAL (enu));
	  else
...
merge_translation_unit_decls (void)
{
  ...
  for (tu = tu_list; tu; tu = TREE_CHAIN (tu))
    for (decl = BLOCK_VARS (DECL_INITIAL (tu)); decl;
      ...
      if (TREE_PUBLIC (decl) && !DECL_EXTERNAL (decl))
	{
	  ...
	  if (*slot != NULL)
	      ...
	      else
		{
		  ...
		  error ("%J'%D' previously defined here", old_decl,
			 old_decl);
		}
	    }
	  else
...
c_reset_state (void)
{
  ...
  for (link = first_builtin_decl;
       link != TREE_CHAIN (last_builtin_decl); link = TREE_CHAIN (link))
