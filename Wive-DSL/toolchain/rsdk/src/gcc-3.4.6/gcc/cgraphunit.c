...
decide_is_function_needed (struct cgraph_node *node, tree decl)
{
  ...
  if (DECL_COMDAT (decl))
    return false;
  if (!DECL_INLINE (decl) || (!node->local.disregard_inline_limits
...
cgraph_create_edges (tree decl, tree body)
{
  ...
  visited_nodes = htab_create (37, htab_hash_pointer, htab_eq_pointer, NULL);
		    ...
		    int ninlined_callees)
{
  ...
  if (!called && !what->needed && !what->origin
      && flag_unit_at_a_time && !DECL_EXTERNAL (what->decl))
    {
			    ...
			    const char **reason)
{
  ...
  if (newsize > PARAM_VALUE (PARAM_LARGE_FUNCTION_INSNS) && newsize > limit)
    {
	...
	if (e->inline_failed)
	  {
	    ...
	    if (cgraph_dump_file)
	      fprintf (cgraph_dump_file,
		       ...
		       cgraph_node_name (e->caller), e->caller->global.insns);
	  }
		  ...
		  if (cgraph_check_inline_limits
			...
			cgraph_inlined_callees (node, inlined_callees);
		      cgraph_mark_inline (node->callers->caller, node,
					  inlined, ninlined, inlined_callees,
					  ninlined_callees);
		      for (y = 0; y < ninlined_callees; y++)
			inlined_callees[y]->output =
			  0, inlined_callees[y]->aux = 0;
		      if (cgraph_dump_file)
			fprintf (cgraph_dump_file,
				 " Inlined into %s which now has %i insns"
