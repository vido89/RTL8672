...
/* Counter for sdbout_source_line.  */
     ...
     static GTY (())
     int sdbout_source_line_counter;
...
plain_type_1 (tree type, int level)
{
  ...
  switch (TREE_CODE (type))
	...
	return ((TREE_CODE (type) == RECORD_TYPE) ? T_STRUCT
		...
		: (TREE_CODE (type) == QUAL_UNION_TYPE) ? T_UNION : T_ENUM);
      }
    case POINTER_TYPE:
    case REFERENCE_TYPE:
      {
...
sdbout_symbol (tree decl, int local)
{
  ...
  switch (TREE_CODE (decl))
	  ...
	  || TREE_CODE (type) == QUAL_UNION_TYPE)
	{
      ...
      if (!local && GET_CODE (value) == MEM && DECL_INITIAL (decl))
	return;
	...
	name = IDENTIFIER_POINTER (DECL_ASSEMBLER_NAME (decl));
      ...
      if (GET_CODE (value) == MEM && GET_CODE (XEXP (value, 0)) == SYMBOL_REF)
	{
	  PUT_SDB_DEF (name);
	  if (TREE_PUBLIC (decl))
	    {
...
sdbout_toplevel_data (tree decl)
{
  ...
  if (!(TREE_CODE (decl) == VAR_DECL
	&& GET_CODE (DECL_RTL (decl)) == MEM && DECL_INITIAL (decl)))
    abort ();
...
sdbout_reg_parms (tree parms)
{
  for (; parms; parms = TREE_CHAIN (parms))
	...
	/* Report parms that live in memory but not where they were passed.  */
		 ...
		 && GET_CODE (XEXP (DECL_RTL (parms), 0)) == PLUS
		 && GET_CODE (XEXP (XEXP (DECL_RTL (parms), 0), 1)) ==
		 CONST_INT && PARM_PASSED_IN_MEMORY (parms)
		 && !rtx_equal_p (DECL_RTL (parms),
				  DECL_INCOMING_RTL (parms)))
	  {
	    ...
	    if (BYTES_BIG_ENDIAN
		&& offset != -1 && TREE_TYPE (parms) != DECL_ARG_TYPE (parms))
...
{
  if (TREE_CODE (decl) == VAR_DECL
      && !DECL_EXTERNAL (decl) && DECL_RTL_SET_P (decl))
    {
  ...
  for (i = 0; i < len; i++)
    {
	  ...
	  && !DECL_EXTERNAL (decl)
	  && DECL_INITIAL (decl)
	  && TREE_PUBLIC (decl) && DECL_RTL_SET_P (decl))
	sdbout_symbol (decl, 0);
    }
