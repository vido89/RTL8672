...
/* This must be in the same order as format_types, with format_type_error
   last.  */
enum format_type
{ printf_format_type, asm_fprintf_format_type,
			     ...
			     tree args, int flags, bool * no_add_attrs)
{
  ...
  if (TREE_CODE (TREE_TYPE (type)) != POINTER_TYPE
      || (TYPE_MAIN_VARIANT (TREE_TYPE (TREE_TYPE (type))) != char_type_node))
    {
...
static const format_flag_spec printf_flag_specs[] = {
  ...
  {'L', 0, 0, N_("length modifier"), N_("length modifier in printf format"),
   STD_C89},
  {0, 0, 0, NULL, NULL, 0}
};
...
static const format_flag_spec scanf_flag_specs[] = {
  {'*', 0, 0, N_("assignment suppression"),
   N_("the assignment suppression scanf feature"), STD_C89},
  {'a', 0, 0, N_("`a' flag"), N_("the `a' scanf flag"), STD_EXT},
  {'w', 0, 0, N_("field width"), N_("field width in scanf format"), STD_C89},
  {'L', 0, 0, N_("length modifier"), N_("length modifier in scanf format"),
   STD_C89},
...
static const format_flag_spec strftime_flag_specs[] = {
  ...
  {'^', 0, 0, N_("`^' flag"), N_("the `^' strftime flag"), STD_EXT},
  {'#', 0, 0, N_("`#' flag"), N_("the `#' strftime flag"), STD_EXT},
  {'w', 0, 0, N_("field width"), N_("field width in strftime format"),
   STD_EXT},
...
static const format_flag_spec strfmon_flag_specs[] = {
  {'=', 0, 1, N_("fill character"), N_("fill character in strfmon format"),
   STD_C89},
  ...
  {'!', 0, 0, N_("`!' flag"), N_("the `!' strfmon flag"), STD_C89},
  {'-', 0, 0, N_("`-' flag"), N_("the `-' strfmon flag"), STD_C89},
  {'w', 0, 0, N_("field width"), N_("field width in strfmon format"),
   STD_C89},
  {'#', 0, 0, N_("left precision"), N_("left precision in strfmon format"),
   STD_C89},
  {'p', 0, 0, N_("right precision"), N_("right precision in strfmon format"),
   STD_C89},
  {'L', 0, 0, N_("length modifier"), N_("length modifier in strfmon format"),
   STD_C89},
  {0, 0, 0, NULL, NULL, 0}
};
...
static const format_char_info print_char_table[] = {
  ...
  {"di", 0, STD_C89,
   {T89_I, T99_SC, T89_S, T89_L, T9L_LL, TEX_LL, T99_SST, T99_PD, T99_IM},
   "-wp0 +'I", "i"},
  {"oxX", 0, STD_C89,
   {T89_UI, T99_UC, T89_US, T89_UL, T9L_ULL, TEX_ULL, T99_ST, T99_UPD,
    T99_UIM}, "-wp0#", "i"},
  {"u", 0, STD_C89,
   {T89_UI, T99_UC, T89_US, T89_UL, T9L_ULL, TEX_ULL, T99_ST, T99_UPD,
    T99_UIM}, "-wp0'I", "i"},
...
static const format_char_info asm_fprintf_char_table[] = {
  /* C89 conversion specifiers.  */
  {"di", 0, STD_C89,
   {T89_I, BADLEN, BADLEN, T89_L, T9L_LL, BADLEN, BADLEN, BADLEN, BADLEN},
   "-wp0 +", "i"},
  {"oxX", 0, STD_C89,
   {T89_UI, BADLEN, BADLEN, T89_UL, T9L_ULL, BADLEN, BADLEN, BADLEN, BADLEN},
   "-wp0#", "i"},
  {"u", 0, STD_C89,
   {T89_UI, BADLEN, BADLEN, T89_UL, T9L_ULL, BADLEN, BADLEN, BADLEN, BADLEN},
   "-wp0", "i"},
  {"c", 0, STD_C89,
   {T89_I, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN},
   "-w", ""},
  {"s", 1, STD_C89,
   {T89_C, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN},
   "-wp", "cR"},
...
static const format_char_info scan_char_table[] = {
  ...
  {"oxX", 1, STD_C89,
   {T89_UI, T99_UC, T89_US, T89_UL, T9L_ULL, TEX_ULL, T99_ST, T99_UPD,
    T99_UIM}, "*w", "W"},
  {"efgEG", 1, STD_C89,
   {T89_F, BADLEN, BADLEN, T89_D, BADLEN, T89_LD, BADLEN, BADLEN, BADLEN},
   "*w'", "W"},
  {"c", 1, STD_C89,
   {T89_C, BADLEN, BADLEN, T94_W, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN},
   "*w", "cW"},
  ...
  /* C99 conversion specifiers.  */
  {"FaA", 1, STD_C99,
   {T99_F, BADLEN, BADLEN, T99_D, BADLEN, T99_LD, BADLEN, BADLEN, BADLEN},
   "*w'", "W"},
  /* X/Open conversion specifiers.  */
  {"C", 1, STD_EXT,
   {TEX_W, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN, BADLEN},
...
static const format_char_info monetary_char_table[] = {
  {"in", 0, STD_C89,
   {T89_D, BADLEN, BADLEN, BADLEN, BADLEN, T89_LD, BADLEN, BADLEN, BADLEN},
   "=^+(!-w#p", ""},
  {NULL, 0, 0, NOLENGTHS, NULL, NULL}
};
...
static const format_kind_info format_types_orig[] = {
  ...
  {"asm_fprintf", asm_fprintf_length_specs, asm_fprintf_char_table, " +#0-",
   NULL,
  ...
  {"gcc_cdiag", gcc_cdiag_length_specs, gcc_cdiag_char_table, "", NULL,
   ...
   0, 0, 'p', 0, 'L',
   NULL, &integer_type_node},
  {"gcc_cxxdiag", gcc_cxxdiag_length_specs, gcc_cxxdiag_char_table, "+#",
   NULL,
  ...
  {"scanf", scanf_length_specs, scan_char_table, "*'I", NULL,
   ...
   FMT_FLAG_ARG_CONVERT | FMT_FLAG_SCANF_A_KLUDGE | FMT_FLAG_USE_DOLLAR |
   FMT_FLAG_ZERO_WIDTH_BAD | FMT_FLAG_DOLLAR_GAP_POINTER_OK,
   'w', 0, 0, '*', 'L',
   NULL, NULL},
...
status_warning (int *, const char *, ...)
  ATTRIBUTE_PRINTF_2;
     ...
     static void finish_dollar_format_checking (int *, format_check_results *,
						int);
...
/* Decode a format type from a string, returning the type, or
   ...
   error message.  */
     static enum format_type decode_format_type (const char *s)
{
  int i;
  int slen;
  slen = strlen (s);
  ...
  if (dollar_arguments_alloc)
      ...
      if (first_arg_num > 0)
	{
	  ...
	  while (params)
		...
		(TREE_CODE (TREE_TYPE (TREE_VALUE (params))) == POINTER_TYPE);
	      params = TREE_CHAIN (params);
	      i++;
	    }
	}
...
static void
finish_dollar_format_checking (int *status, format_check_results * res,
			       int pointer_gap_ok)
{
  int i;
...
get_flag_spec (const format_flag_spec * spec, int flag,
  ...
  if (res.number_non_literal > 0)
    {
      ...
      if (!
	  (format_types[info->format_type].
	   flags & (int) FMT_FLAG_ARG_CONVERT))
	{
...
check_format_arg (void *ctx, tree format_tree, unsigned HOST_WIDE_INT arg_num)
{
  ...
  if (TREE_CODE (format_tree) == PLUS_EXPR)
      ...
      else
	{
      ...
      if (!host_integerp (arg1, 0) || (offset = tree_low_cst (arg1, 0)) < 0)
	{
	  res->number_non_literal++;
	  return;
	}
  ...
  while (1)
      ...
      if (fki->width_char != 0)
	{
	  if (fki->width_type != NULL && *format_chars == '*')
		...
		status_warning (status, "zero width in %s format", fki->name);
	      if (found_width)
		{
		  i = strlen (flag_chars);
		  flag_chars[i++] = fki->width_char;
	  ...
	  while (*format_chars != 0
	      ...
	      if (strchr (flag_chars, *format_chars) != 0)
		{
		  const format_flag_spec *s = get_flag_spec (flag_specs,
							     *format_chars,
							     NULL);
		  status_warning (status, "repeated %s in format",
				  _(s->name));
		}
	      else
		{
      ...
      for (i = 0; i < types->pointer_count; ++i)
	{
	  if (TREE_CODE (cur_type) == POINTER_TYPE)
		...
		break;
		  ...
		  && i == 0 && cur_param != 0 && integer_zerop (cur_param))
		status_warning (status,
		...
		status_warning (status,
				"writing into constant object (arg %d)",
				arg_num);
	...
	if (strcmp (this, that) != 0)
	  {
	    ...
	    if (types->name != 0)
	      status_warning (status, "%s is not type %s (arg %d)",
			      types->name, this, arg_num);
	    else
	      status_warning (status, "%s format, %s arg (arg %d)", this,
			      that, arg_num);
	  }
      }
    }
