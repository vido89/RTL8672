...
block_fallthru (basic_block bb)
{
  ...
  for (e = bb->succ;
       e != NULL_EDGE && (e->flags & EDGE_FALLTHRU) == 0; e = e->succ_next)
    ;
...
noce_try_store_flag (struct noce_if_info *if_info)
{
  ...
  if (GET_CODE (if_info->b) == CONST_INT
      && INTVAL (if_info->b) == STORE_FLAG_VALUE && if_info->a == const0_rtx)
    reversep = 0;
...
noce_try_store_flag_constants (struct noce_if_info *if_info)
{
  ...
  if (!no_new_pseudos
	...
	normalize = 1, reversep = 1;
      else if (itrue == -1 && (STORE_FLAG_VALUE == -1 || BRANCH_COST >= 2))
	normalize = -1;
...
noce_try_cmove_arith (struct noce_if_info *if_info)
{
  ...
  if (!no_new_pseudos && cse_not_expected
      && GET_CODE (a) == MEM && GET_CODE (b) == MEM && BRANCH_COST >= 5)
    {
      a = XEXP (a, 0);
      b = XEXP (b, 0);
      x = gen_reg_rtx (Pmode);
  ...
  end_sequence ();
  emit_insn_before_setloc (tmp, if_info->jump,
			   INSN_LOCATOR (if_info->insn_a));
  return TRUE;
...
noce_try_abs (struct noce_if_info *if_info)
{
  ...
  if (REG_P (c))
    {
      rtx insn, note = NULL;
      for (insn = earliest;
	   insn != BB_HEAD (if_info->test_bb); insn = PREV_INSN (insn))
	if (INSN_P (insn)
  ...
  if (target && negate)
    target =
      expand_simple_unop (GET_MODE (target), NEG, target, if_info->x, 0);
...
noce_process_if_block (struct ce_if_block *ce_info)
{
  ...
  orig_x = x;
  if (GET_CODE (x) != REG
      || (SMALL_REGISTER_CLASSES && REGNO (x) < FIRST_PSEUDO_REGISTER))
    {
  ...
  if (!set_b
      && GET_CODE (orig_x) == MEM
      && !MEM_NOTRAP_P (orig_x) && rtx_addr_can_trap_p (XEXP (orig_x, 0)))
    {
      if (HAVE_conditional_move)
	{
	  if (noce_try_cmove (&if_info))
	    goto success;
	  if (!HAVE_conditional_execution && noce_try_cmove_arith (&if_info))
	    goto success;
	}
      return FALSE;
    }
  ...
  if (noce_try_abs (&if_info))
    goto success;
  if (HAVE_conditional_move && noce_try_cmove (&if_info))
    goto success;
  ...
  if (orig_x != x)
    {
      ...
      emit_insn_after_setloc (insn_b, BB_END (test_bb),
			      INSN_LOCATOR (insn_a));
    }
...
{
  if (!reload_completed && noce_process_if_block (ce_info))
    return TRUE;
...
merge_if_block (struct ce_if_block *ce_info)
{
  ...
  if (!join_bb)
	...
	;
	       ...
	       && GET_CODE (last) == CALL_INSN && SIBLING_CALL_P (last))
	;
      else if ((combo_bb->succ->flags & EDGE_EH) && can_throw_internal (last))
	;
      else
	abort ();
    }
  ...
  /* The JOIN block may have had quite a number of other predecessors too.
     ...
     back up (as with a call to abort).  */
	    ...
	    || join_bb->pred->pred_next == NULL) && join_bb != EXIT_BLOCK_PTR)
    {
...
block_jumps_and_fallthru_p (basic_block cur_bb, basic_block target_bb)
{
  ...
  for (cur_edge = cur_bb->succ;
       cur_edge != NULL_EDGE; cur_edge = cur_edge->succ_next)
    {
...
find_if_block (struct ce_if_block *ce_info)
{
  ...
  for (cur_edge = then_bb->pred;
       cur_edge != NULL_EDGE; cur_edge = cur_edge->pred_next)
    {
      then_predecessors++;
      if (cur_edge->flags & EDGE_COMPLEX)
	return FALSE;
  ...
  if (rtl_dump_file)
	...
	fprintf (rtl_dump_file, ", else %d [%d]",
		 else_bb->index,
		 (BB_HEAD (else_bb)) ? (int) INSN_UID (BB_HEAD (else_bb)) :
		 -1);
	...
	fprintf (rtl_dump_file, ", %d %s block%s last test %d [%d]",
		 ...
		 ((BB_HEAD (ce_info->last_test_bb))
		  ? (int) INSN_UID (BB_HEAD (ce_info->last_test_bb)) : -1));
...
find_cond_trap (basic_block test_bb, edge then_edge, edge else_edge)
{
  ...
  seq = gen_cond_trap (code, XEXP (cond, 0),
		       XEXP (cond, 1), TRAP_CODE (PATTERN (trap)));
  if (seq == NULL)
    return FALSE;
...
find_if_case_1 (basic_block test_bb, edge then_edge, edge else_edge)
{
  ...
  if (!dead_or_predicable (test_bb, then_bb, else_bb, then_bb->succ->dest, 1))
    return FALSE;
...
find_if_case_2 (basic_block test_bb, edge then_edge, edge else_edge)
{
  ...
  if (note && INTVAL (XEXP (note, 0)) >= REG_BR_PROB_BASE / 2)
    ;
	   ...
	   || dominated_by_p (CDI_POST_DOMINATORS, then_bb, else_succ->dest))
    ;
  else
    return FALSE;
		    ...
		    basic_block other_bb, basic_block new_dest, int reversep)
{
  ...
  if (HAVE_conditional_execution)
	...
	{
	  EXECUTE_IF_SET_IN_BITMAP (merge_set, 0, i,
				    {
				    if (i < FIRST_PSEUDO_REGISTER
					&& !fixed_regs[i]
					&& !global_regs[i]) fail = 1;}
	  );
	}
