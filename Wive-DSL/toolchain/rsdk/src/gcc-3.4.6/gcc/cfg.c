...
struct basic_block_def entry_exit_blocks[2] = { {NULL,	/* head */
						 NULL,	/* end */
						 NULL,	/* head_tree */
						 NULL,	/* end_tree */
						 NULL,	/* pred */
...
init_flow (void)
{
  ...
  edge_pool = create_alloc_pool ("Edge pool", sizeof (struct edge_def), 100);
}

...
edge
cached_make_edge (sbitmap * edge_cache, basic_block src, basic_block dst,
		  int flags)
{
  int use_edge_cache;
  edge e;
...
clear_bb_flags (void)
{
  ...
  FOR_BB_BETWEEN (bb, ENTRY_BLOCK_PTR, NULL, next_bb) bb->flags = 0;
}

void
...
verify_flow_info (void)
{
  ...
  {
    if (bb != EXIT_BLOCK_PTR && bb != BASIC_BLOCK (bb->index))
      {
