...
/* These arrays record the INSN_UIDs of the prologue and epilogue insns.  */
     static GTY (())
     varray_type prologue;
     static GTY (())
     varray_type epilogue;

/* Array of INSN_UIDs to hold the INSN_UIDs for each sibcall epilogue
   in this function.  */
     static GTY (())
     varray_type sibcall_epilogue;

...
static void put_reg_into_stack (struct function *, rtx, tree,
				enum machine_mode, unsigned int, bool, bool,
				bool, htab_t);
static void schedule_fixup_var_refs (struct function *, rtx, tree,
				     enum machine_mode, htab_t);
...
static void fixup_var_refs_insns_with_hash (htab_t, rtx, enum machine_mode,
					    int, rtx);
		      ...
		      struct function *function)
{
  ...
  if (STACK_ALIGNMENT_NEEDED || mode != BLKmode || size != 0)
    {
...
put_var_into_stack (tree decl, int rescan)
{
  ...
  bool volatile_p = TREE_CODE (decl) != SAVE_EXPR
  ...
  reg = (TREE_CODE (decl) == SAVE_EXPR
	 ? SAVE_EXPR_RTL (decl) : DECL_RTL_IF_SET (decl));
  ...
  if (context != current_function_decl && context != inline_function_decl)
    for (function = outer_function_chain; function;
  ...
  if (!can_use_addressof_p
      && GET_CODE (reg) == MEM && GET_CODE (XEXP (reg, 0)) == ADDRESSOF)
...
static void
fixup_var_refs_insns_with_hash (htab_t ht, rtx var,
				...
				int unsignedp, rtx may_share)
{
  ...
  for (insn_list = ime->insns; insn_list != 0;
		     ...
		     int unsignedp, int toplevel, rtx no_share)
{
  ...
  else if (toplevel && (set = single_set (insn)) != 0 && SET_DEST (set) == var
  ...
  while (note)
	...
	XEXP (note, 0)
	  = walk_fixup_memory_subreg (XEXP (note, 0), insn, promoted_mode, 1);
      note = XEXP (note, 1);
    }
}

...
static void
fixup_var_refs_1 (rtx var, enum machine_mode promoted_mode, rtx * loc,
		  ...
		  rtx no_share)
{
  ...
  switch (code)
	...
	if (GET_CODE (outerdest) == ZERO_EXTRACT && dest == var
	    && mode_for_extraction (EP_insv, -1) != MAX_MACHINE_MODE)
	  {
	    ...
	    if (GET_CODE (tem) == SUBREG && SUBREG_REG (tem) == var)
	      tem = fixup_memory_subreg (tem, insn, promoted_mode, 0);
	    else
	      tem = fixup_stack_1 (tem, insn);
	...
	if ((SET_SRC (x) == var
	    ...
	    && GET_MODE (var) == promoted_mode && x == single_set (insn))
	  {
		...
		SUBREG_REG (SET_SRC (x)) = replacement->new;
	      ...
	      {
		replacement =
		  find_fixup_replacement (replacements, SET_SRC (x));
		if (replacement->new)
  ...
  for (i = GET_RTX_LENGTH (code) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e')
	fixup_var_refs_1 (var, promoted_mode, &XEXP (x, i), insn,
			  replacements, no_share);
      else if (fmt[i] == 'E')
	{
	  int j;
...
static rtx
fixup_memory_subreg (rtx x, rtx insn, enum machine_mode promoted_mode,
		     int uncritical)
{
  int offset;
  ...
  if (BYTES_BIG_ENDIAN)
    /* If the PROMOTED_MODE is wider than the mode of the MEM, adjust
       ...
       MEM.  */
    offset -=
      (GET_MODE_SIZE (promoted_mode) - GET_MODE_SIZE (GET_MODE (mem)));
	  ...
	  && !MEM_VOLATILE_P (memref))
	{
	  ...
	  if (destflag)
	      ...
	      if (!CONSTANT_ADDRESS_P (SET_SRC (body)))
		{
		  ...
		  while (GET_CODE (src) == SUBREG && SUBREG_BYTE (src) == 0)
		    src = SUBREG_REG (src);
...
static bool
purge_addressof_1 (rtx * loc, rtx insn, int force, int store,
		   int may_postpone, htab_t ht)
{
  rtx x;
  RTX_CODE code;
  int i, j;
	  ...
	  if (!insn)
	    {
	      ...
	      for (tem = purge_bitfield_addressof_replacements;
		   tem != NULL_RTX; tem = XEXP (XEXP (tem, 1), 1))
		...
		if (rtx_equal_p (XEXP (x, 0), XEXP (tem, 0)))
		  ...
		  || (GET_MODE_SIZE (GET_MODE (x)) > UNITS_PER_WORD
		      && (GET_MODE_SIZE (GET_MODE (x))
			  > GET_MODE_SIZE (GET_MODE (sub))) && libcall))
		{
...
insns_for_mem_walk (rtx * r, void *data)
{
  ...
  if (ifmwi->pass == 0 && *r && GET_CODE (*r) == ADDRESSOF
	...
	ifme->insns = gen_rtx_EXPR_LIST (VOIDmode, ifmwi->insn, ifme->insns);
    }
  ...
  for (t = BLOCK_VARS (let); t; t = TREE_CHAIN (t))
    if (DECL_RTL_SET_P (t))
      instantiate_decl (DECL_RTL (t),
			int_size_in_bytes (TREE_TYPE (t)), valid_only);
  ...
  switch (code)
      ...
      if (CONSTANT_P (XEXP (x, 1)))
	{
	  ...
	  if (GET_CODE (XEXP (x, 0)) == PLUS)
	    {
	      if ((new =
		   instantiate_new_reg (XEXP (XEXP (x, 0), 0), &offset)))
		{
...
assign_parms (tree fndecl)
{
  ...
  for (parm = fnargs; parm; parm = TREE_CHAIN (parm))
      ...
      if (stdarg)
	{
      ...
      named_arg =
	targetm.calls.strict_argument_naming (&args_so_far) ? 1 : !last_named;
      ...
      if (targetm.calls.promote_function_args (TREE_TYPE (fndecl)))
	{
	  ...
	  promoted_mode =
	    promote_mode (passed_type, promoted_mode, &unsignedp, 1);
	}
      ...
      if (partial)
	{
	  ...
	  if (GET_CODE (entry_parm) == PARALLEL)
	    ...
	    move_block_from_reg (REGNO (entry_parm),
				 validize_mem (stack_parm), partial);
	  ...
	  || GET_CODE (entry_parm) == PARALLEL)
	{
	  ...
	  if (!DECL_RTL_SET_P (parm))
	    SET_DECL_RTL (parm, stack_parm);
		  ...
		  && !DECL_REGISTER (parm)) || TREE_SIDE_EFFECTS (parm)
	       ...
	       || passed_pointer || parm == function_result_decl)
	{
	  ...
	  if (nominal_mode != passed_mode
		  ...
		  < GET_MODE_SIZE (GET_MODE (entry_parm)))
		{
	      ...
	      expand_assignment (parm, make_tree (nominal_type, tempreg), 0);
	  ...
	  if (GET_CODE (parmreg) == CONCAT)
	    regno =
	      MAX (REGNO (XEXP (parmreg, 0)), REGNO (XEXP (parmreg, 1)));
	  else
	    regno = REGNO (parmreg);
  ...
  if (DECL_RTL_SET_P (DECL_RESULT (fndecl)))
	  ...
	  : DECL_REGISTER (decl_result))
	{
	  ...
	  real_decl_rtl = FUNCTION_OUTGOING_VALUE (TREE_TYPE (decl_result),
						   fndecl);
#else
	  real_decl_rtl = FUNCTION_VALUE (TREE_TYPE (decl_result), fndecl);
#endif
...
rtx
promoted_input_arg (unsigned int regno, enum machine_mode * pmode,
		    int *punsignedp)
{
  tree arg;
...
static void
pad_below (struct args_size *offset_ptr, enum machine_mode passed_mode,
	   tree sizetree)
{
  if (passed_mode != BLKmode)
    {
...
uninitialized_vars_warning (tree block)
{
  ...
  for (decl = BLOCK_VARS (block); decl; decl = TREE_CHAIN (decl))
    {
      if (warn_uninitialized && TREE_CODE (decl) == VAR_DECL
	  /* These warnings are unreliable for and aggregates
	     because assigning the fields one by one can fail to convince
	     flow.c that the entire aggregate was initialized.
	     Unions are troublesome because members may be shorter.  */
	  && !AGGREGATE_TYPE_P (TREE_TYPE (decl))
	  && DECL_RTL_SET_P (decl) && GET_CODE (DECL_RTL (decl)) == REG
	  ...
	  && (DECL_INITIAL (decl) == NULL_TREE
	      || lang_hooks.decl_uninit (decl))
	  && regno_uninitialized (REGNO (DECL_RTL (decl))))
	warning ("%J'%D' might be used uninitialized in this function", decl,
		 decl);
      if (extra_warnings && TREE_CODE (decl) == VAR_DECL
	  && DECL_RTL_SET_P (decl) && GET_CODE (DECL_RTL (decl)) == REG
...
trampoline_address (tree function)
{
  ...
  for (fp = outer_function_chain; fp; fp = fp->outer)
	...
	{
	  tramp =
	    fix_lexical_addr (XEXP (RTL_EXPR_RTL (TREE_VALUE (link)), 0),
			      function);
	  ...
	  if (NOTE_LINE_NUMBER (insn) == NOTE_INSN_BLOCK_BEG)
	      ...
	      if (TREE_ASM_WRITTEN (block))
		{
		  ...
		  origin = (BLOCK_FRAGMENT_ORIGIN (block)
			    ? BLOCK_FRAGMENT_ORIGIN (block) : block);
...
expand_function_start (tree subr, int parms_have_cleanups)
{
  ...
  current_function_profile
    = (profile_flag && !DECL_NO_INSTRUMENT_FUNCTION_ENTRY_EXIT (subr));
  ...
  if (aggregate_value_p (DECL_RESULT (subr), subr))
  ...
  else
    {
      ...
      rtx hard_reg = hard_function_value (TREE_TYPE (DECL_RESULT (subr)),
					  subr, 1);
...
do_warn_unused_parameter (tree fn)
{
  ...
  for (decl = DECL_ARGUMENTS (fn); decl; decl = TREE_CHAIN (decl))
     ...
     void expand_function_end (void)
{
  ...
  if (warn_unused_parameter && !lang_hooks.callgraph.expand_function)
...
#ifdef FUNCTION_OUTGOING_VALUE
      rtx outgoing = FUNCTION_OUTGOING_VALUE (build_pointer_type (type),
  ...
  else
    {
      SET_SRC (set) = simplify_replace_rtx (SET_SRC (set), stack_pointer_rtx,
					    plus_constant (p->sp_equiv_reg,
							   p->sp_offset));
      SET_DEST (set) =
	simplify_replace_rtx (SET_DEST (set), stack_pointer_rtx,
			      plus_constant (p->sp_equiv_reg, p->sp_offset));
      emit_insn (set);
    }
}
  ...
  if (optimize && HAVE_return)
      ...
      if (BB_HEAD (last) == label && GET_CODE (label) == CODE_LABEL)
	{
	  ...
	  for (seq = get_last_insn ();
	       seq && !active_insn_p (seq); seq = PREV_INSN (seq))
  ...
  if (prologue_end)
      ...
      for (insn = prologue_end; insn; insn = prev)
	{
      ...
      for (insn = BB_END (ENTRY_BLOCK_PTR->next_bb);
	   insn != prologue_end && insn; insn = PREV_INSN (insn))
	if (GET_CODE (insn) == NOTE && NOTE_LINE_NUMBER (insn) > 0)
	  break;
