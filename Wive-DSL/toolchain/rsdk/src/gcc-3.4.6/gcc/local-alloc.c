...
update_equiv_regs (void)
{
  ...
  FOR_EACH_BB (bb)
  {
	...
	if (note == 0 && REG_BASIC_BLOCK (regno) >= 0
	  ...
	  REG_NOTES (insn) = note =
	    gen_rtx_EXPR_LIST (REG_EQUIV, SET_SRC (set), REG_NOTES (insn));
  ...
  FOR_EACH_BB_REVERSE (bb)
  {
    loop_depth = bb->loop_depth;
    for (insn = BB_END (bb);
	 insn != PREV_INSN (BB_HEAD (bb)); insn = PREV_INSN (insn))
      {
	rtx link;
	  ...
	  if (optimize
	      ...
	      for (i = 1; i < recog_data.n_operands; i++)
		{
		  ...
		  if (recog_data.constraints[i][0] == 'p'
		      || EXTRA_ADDRESS_CONSTRAINT (recog_data.
						   constraints[i][0],
...
#define EXCHANGE(I1, I2)  \
  ...
  for (i = 0; i < next_qty; i++)
      ...
      if (qty[q].phys_reg < 0)
	{
	  ...
	  if (flag_schedule_insns_after_reload
	      && !optimize_size
	      && !SMALL_REGISTER_CLASSES && qty[q].alternate_class != NO_REGS)
...
reg_is_born (rtx reg, int birth)
{
  ...
  if (regno < FIRST_PSEUDO_REGISTER)
	...
	post_mark_life (regno, GET_MODE (reg), 1, birth,
			2 * this_insn_number);
    }
  else
    {
	       ...
	       int born_index, int dead_index)
{
  ...
  if (current_function_has_nonlocal_label && qty[qtyno].n_calls_crossed > 0)
    return -1;
  ...
  GO_IF_HARD_REG_SUBSET (reg_class_contents[(int) ALL_REGS], first_used,
			 fail);
