...
pch_init (void)
{
  ...
  if (asm_file_name == NULL || strcmp (asm_file_name, "-") == 0)
...
c_common_valid_pch (cpp_reader * pfile, const char *name, int fd)
{
  ...
  if (v.version_length != strlen (version_string)
      || memcmp (version_string,
		 (short_strings + v.host_machine_length
		  + v.target_machine_length), v.version_length) != 0)
    {
      if (cpp_get_options (pfile)->warn_invalid_pch)
	cpp_error (pfile, CPP_DL_WARNING,
		   ...
		   (short_strings + v.host_machine_length
		    + v.target_machine_length), version_string);
      return 2;
    }
  if (v.flags_info != current_flags_info)
    {
      if (cpp_get_options (pfile)->warn_invalid_pch)
	cpp_error (pfile, CPP_DL_WARNING,
		   "%s: created using different flags", name);
      return 2;
    }
  ...
  if (v.debug_info_type != write_symbols && write_symbols != NO_DEBUG)
    {
      if (cpp_get_options (pfile)->warn_invalid_pch)
	cpp_error (pfile, CPP_DL_WARNING,
