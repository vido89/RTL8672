...
static
GTY (())
     struct align_stack *alignment_stack;
...
/* Push an alignment value onto the stack.  */
     static void push_alignment (int alignment, tree id)
{
  if (alignment_stack == NULL
      || alignment_stack->alignment != alignment || id != NULL_TREE)
    {
      align_stack *entry;
...
#pragma pack (pop) encountered without matching #pragma pack (push, <n>)");
  ...
  if (id)
	...
	if (entry->id == id)
      ...
      if (entry == NULL)
	warning ("\
#pragma pack(pop, %s) encountered without matching #pragma pack(push, %s, <n>)", IDENTIFIER_POINTER (id), IDENTIFIER_POINTER (id));
    }
...
/* #pragma pack ()
   ...
   #pragma pack (pop)
   #pragma pack (pop, ID) */
     static void handle_pragma_pack (cpp_reader * dummy ATTRIBUTE_UNUSED)
{
  tree x, id = 0;
  int align = -1;
...
GTY (())
     ...
     static void apply_pragma_weak (tree decl, tree value)
{
  if (value)
    {
  ...
  if (SUPPORTS_WEAK && DECL_EXTERNAL (decl) && TREE_USED (decl) && !DECL_WEAK (decl)	/* Don't complain about a redundant #pragma.  */
...
#else
     void maybe_apply_pragma_weak (tree decl ATTRIBUTE_UNUSED)
{
}
