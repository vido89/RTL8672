      ...
      if (modified || !qty_valid)
	{
	  if (classp)
	    for (classp = classp->first_same_value;
		 classp != 0; classp = classp->next_same_value)
...
lookup (rtx x, unsigned int hash, enum machine_mode mode)
{
  ...
  for (p = table[hash]; p; p = p->next_same_hash)
    if (mode == p->mode && ((x == p->exp && GET_CODE (x) == REG)
			    || exp_equiv_p (x, p->exp, GET_CODE (x) != REG,
					    0)))
      return p;
  ...
  if (elt->is_const && classp && GET_CODE (classp->exp) == REG
      && GET_CODE (x) != REG)
    {
      ...
      exp_ent->const_insn = this_insn;
	   ...
	   && classp
	   && !qty_table[REG_QTY (REGNO (x))].const_rtx && !elt->is_const)
    {
      struct table_elt *p;
  ...
  switch (code)
	...
	if (regno >= FIRST_PSEUDO_REGISTER)
	  record = true;
		 ...
		 || x == arg_pointer_rtx
		 || x == stack_pointer_rtx || x == pic_offset_table_rtx)
	  record = true;
    ...
    case USE:
      ...
      if (GET_CODE (XEXP (x, 0)) == MEM && !MEM_VOLATILE_P (XEXP (x, 0)))
	{
	  hash += (unsigned) USE;
	  x = XEXP (x, 0);
      ...
      else
	{
	  ...
	  if (ASM_OPERANDS_INPUT_LENGTH (x))
	      ...
	      for (i = 1; i < ASM_OPERANDS_INPUT_LENGTH (x); i++)
		{
	      ...
	      hash +=
		canon_hash_string (ASM_OPERANDS_INPUT_CONSTRAINT (x, 0));
  ...
  if (GET_CODE (x) == REG && REGNO_QTY_VALID_P (REGNO (x)))
    {
      ...
      if (GET_MODE (x) == x_ent->mode && x_ent->const_rtx != NULL_RTX)
	return 0;
    }
  ...
  if (GET_CODE (addr) != REG)
    {
      ...
      if ((addr_folded_cost < addr_cost || (addr_folded_cost == addr_cost
  ...
  if (elt)
      ...
      while (found_better)
	{
	  ...
	  for (p = elt->first_same_value; p; p = p->next_same_value)
		...
		if ((GET_CODE (p->exp) == REG
		     || exp_equiv_p (p->exp, p->exp, 1, 0))
		    && ((exp_cost = address_cost (p->exp, mode)) <
			best_addr_cost || (exp_cost == best_addr_cost
					   && ((p->cost + 1) >> 1) >
					   best_rtx_cost)))
		  {
		    found_better = 1;
  ...
  if (flag_expensive_optimizations
      ...
      while (found_better)
	{
	  ...
	  for (p = elt->first_same_value, count = 0;
	       p && count < 32; p = p->next_same_value, count++)
	    if (!p->flag
		&& (GET_CODE (p->exp) == REG
		    || exp_equiv_p (p->exp, p->exp, 1, 0)))
	      {
...
fold_rtx (rtx x, rtx insn)
{
		  ...
		  if (op0)
			...
			|| GET_RTX_CLASS (GET_CODE (elt->exp)) == 'c')
		       ...
		       && ((GET_CODE (XEXP (elt->exp, 1)) == SUBREG
				...
				== mode)) || CONSTANT_P (XEXP (elt->exp, 1))))
		{
		  ...
		  if (op0 && op1
		    ...
		    new =
		      simplify_binary_operation (GET_CODE (elt->exp), mode,
						 op0, op1);
		}
	...
	for (j = 0; j < 2 && replacements[j]; j++)
	  {
		...
		&& (code == ZERO_EXTEND
		    ...
		    || code == FLOAT
		    || code == FIX
		    || code == UNSIGNED_FLOAT || code == UNSIGNED_FIX))
	      continue;
  ...
  if (code == EQ || code == NE || GET_RTX_CLASS (code) == 'c'
      || code == LTGT || code == UNEQ || code == ORDERED || code == UNORDERED)
    {
      if (must_swap
	  ...
	  if (const_arg0 == 0 || const_arg1 == 0)
	    {
	      ...
	      if (const_arg1 == const0_rtx && nonzero_address_p (folded_arg0))
		{
		  if (code == EQ)
		    return false_rtx;
		  else if (code == NE)
		      ...
		      && p0->first_same_value == p1->first_same_value))
		{
		  ...
		  if (!HONOR_NANS (mode_arg0))
		    return ((code == EQ || code == LE || code == GE
			     ...
			     || code == UNLE || code == UNGE
			     || code == ORDERED) ? true_rtx : false_rtx);
		  ...
		  if (REGNO_QTY_VALID_P (REGNO (folded_arg0)))
		    {
			  ...
			  && (rtx_equal_p (ent->comparison_const, folded_arg1)
				  ...
				  && (REG_QTY (REGNO (folded_arg1)) ==
				      ent->comparison_qty))))
			return (comparison_dominates_p
				(ent->comparison_code,
				 code) ? true_rtx : false_rtx);
		    }
		}
	    }
	}
...
cse_insn (rtx insn, rtx libcall_insn)
{
  ...
  for (i = 0; i < n_sets; i++)
	...
	for (p = elt->first_same_value; p; p = p->next_same_value)
      ...
      if (src_const == 0 && (CONSTANT_P (src_folded)
			     /* Consider (minus (label_ref L1) (label_ref L2)) as
			        "constant" here so we will record it. This allows us
			        to fold switch statements when an ADDR_DIFF_VEC is used.  */
			     || (GET_CODE (src_folded) == MINUS
				 && GET_CODE (XEXP (src_folded, 0)) ==
				 LABEL_REF
				 && GET_CODE (XEXP (src_folded, 1)) ==
				 LABEL_REF)))
      ...
      while (1)
	{
	  ...
	  if (n_sets == 1 && dest == pc_rtx
	      && (trial == pc_rtx
		  || (GET_CODE (trial) == LABEL_REF && !condjump_p (insn))))
	    {
      ...
      if (GET_CODE (dest) == REG && REGNO_QTY_VALID_P (REGNO (dest)))
	{
	  ...
	  if (dest_ent->mode == GET_MODE (dest)
		...
		= (first >= FIRST_PSEUDO_REGISTER
		   ? regno_reg_rtx[first] : gen_rtx_REG (GET_MODE (src),
							 first));
  ...
  for (i = 0; i < n_sets; i++)
	...
	/* Don't record value if we are not supposed to risk allocating
	   ...
	   memory.  */
	if ((flag_float_store
	     && GET_CODE (dest) == MEM && FLOAT_MODE_P (GET_MODE (dest)))
  ...
  switch (code)
      ...
      if (REGNO_QTY_VALID_P (REGNO (x)))
	{
	  ...
	  if (ent->const_rtx != NULL_RTX
	    ...
	    {
	      rtx new =
		gen_lowpart_if_possible (GET_MODE (x), ent->const_rtx);
	      if (new)
		return new;
	    }
	}
...
invalidate_skipped_set (rtx dest, rtx set, void *data ATTRIBUTE_UNUSED)
{
  ...
  if (code == MEM && !addr_affects_sp_p (dest)	/* If this is not a stack push ...  */
  ...
  if (GET_CODE (set) == CLOBBER || CC0_P (dest) || dest == pc_rtx)
    return;
	  ...
	  if (GET_CODE (src_elt->exp) == REG && REG_LOOP_TEST_P (src_elt->exp)
	      && COST (src_elt->exp) < COST (SET_SRC (x)))
	    {
	      ...
	      for (p = prev_nonnote_insn (loop_start);
		   ...
		   && GET_CODE (p) != CODE_LABEL; p = prev_nonnote_insn (p))
      ...
      if (path_entry < path_size && data->path[path_entry].branch == p)
	{
      ...
      /* If this is a conditional jump, we can follow it if -fcse-follow-jumps
         ...
         registers set in the block when following the jump.  */
	       ...
	       && JUMP_LABEL (p) != 0 && LABEL_NUSES (JUMP_LABEL (p)) == 1
		 ...
		 int around_loop)
{
  ...
  if ((cse_jumps_altered == 0
      ...
      && JUMP_LABEL (insn) != 0 && LABEL_NUSES (JUMP_LABEL (insn)) == 1)
...
cse_cc_succs (basic_block bb, rtx cc_reg, rtx cc_src, bool can_change_mode)
{
  ...
  for (e = bb->succ; e; e = e->succ_next)
	...
	continue;
      ...
      if (!e->dest->pred
	  || e->dest->pred->pred_next || e->dest == EXIT_BLOCK_PTR)
	continue;
	...
	if (mode != orig_mode)
	  {
	    ...
	    /* Change the mode of CC_REG in CC_SRC_INSN to
	       GET_MODE (NEWREG).  */
	    for_each_rtx (&PATTERN (cc_src_insn), cse_change_cc_mode, newreg);
