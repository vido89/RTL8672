  ...
  for (i = 1; i < 32; i++)
    {
      ...
      strcpy (buffer, "c1");
      field_decl = create_builtin_decl (FIELD_DECL, char_type_node, buffer);
		      ...
		      if (!rproto)
			{
			  ...
			  for (rproto = rproto_list; rproto;
			       rproto = TREE_CHAIN (rproto))
			    {
			      p = TREE_VALUE (rproto);
			      lproto =
				lookup_protocol_in_reflist (lproto_list, p);
	      ...
	      for (lproto = lproto_list; lproto; lproto = TREE_CHAIN (lproto))
		{
		  ...
		  while (rinter && !rproto)
		      ...
		      if (!rproto)
			{
			  ...
			  rproto =
			    lookup_protocol_in_reflist (rproto_list, p);
			}
		  ...
		  if (!rproto)
		    warning
		      ("class `%s' does not implement the `%s' protocol",
...
lookup_and_install_protocols (tree protocols)
{
  ...
  for (proto = protocols; proto; proto = TREE_CHAIN (proto))
	...
	return_value = chainon (return_value, build_tree_list (NULL_TREE, p));
    }
...
synth_module_prologue (void)
{
  ...
  objc_class_type =
    build_pointer_type (xref_tag (RECORD_TYPE, objc_class_id));
  temp_type = get_identifier (PROTOCOL_OBJECT_CLASS_NAME);
  objc_declare_class (tree_cons (NULL_TREE, temp_type, NULL_TREE));
  protocol_type = build_pointer_type (xref_tag (RECORD_TYPE, temp_type));
  ...
  if (flag_next_runtime)
    {
      tree objc_return_struct_type = xref_tag (RECORD_TYPE,
					       get_identifier
					       (TAG_RETURN_STRUCT));

      tree stret_temp_type = build_function_type (id_type,
						  tree_cons (NULL_TREE,
							     id_type,
							     tree_cons
							     (NULL_TREE,
  ...
  objc_get_meta_class_decl
    =
    builtin_function (TAG_GETMETACLASS, temp_type, 0, NOT_BUILT_IN, NULL,
		      NULL_TREE);
  ...
  if (!flag_next_runtime)
	...
	= create_builtin_decl (VAR_DECL, temp_type, "_OBJC_SELECTOR_TABLE");
...
build_objc_symtab_template (void)
{
  ...
  field_decl = create_builtin_decl (FIELD_DECL,
				    long_integer_type_node, "sel_ref_cnt");
  field_decl_chain = field_decl;
  ...
  field_decl = create_builtin_decl (FIELD_DECL,
				    short_integer_type_node, "cls_def_cnt");
  chainon (field_decl_chain, field_decl);

  /* short cat_def_cnt; */

  field_decl = create_builtin_decl (FIELD_DECL,
				    short_integer_type_node, "cat_def_cnt");
  chainon (field_decl_chain, field_decl);
  ...
  if (imp_count || cat_count || !flag_next_runtime)
	...
	(FIELD_DECL,
	 build_array_type
	 (ptr_type_node, build_index_type (build_int_2 (index, 0))), "defs");
      chainon (field_decl_chain, field_decl);
    }
...
build_metadata_decl (const char *name, tree type)
{
  ...
  decl = define_decl (synth_id_with_class_suffix
		      (name, objc_implementation_context), decl_specs);
  TREE_USED (decl) = 1;
  DECL_ARTIFICIAL (decl) = 1;
  TREE_PUBLIC (decl) = 0;
  return decl;
...
generate_objc_symtab_decl (void)
{
  ...
  finish_decl (UOBJC_SYMBOLS_decl,
	       init_objc_symtab (TREE_TYPE (UOBJC_SYMBOLS_decl)), NULL_TREE);
}

static tree
...
generate_static_references (void)
{
  ...
  decl_spec = tree_cons (NULL_TREE, build_pointer_type (void_type_node),
			 build_tree_list (NULL_TREE,
					  ridpointers[(int) RID_STATIC]));
  static_instances_decl = start_decl (expr_decl, decl_spec, 1, NULL_TREE);
...
generate_strings (void)
{
  ...
  for (chain = class_names_chain; chain; chain = TREE_CHAIN (chain))
	...
	= tree_cons (NULL_TREE, ridpointers[(int) RID_STATIC], NULL_TREE);
      decl_specs =
	tree_cons (NULL_TREE, ridpointers[(int) RID_CHAR], sc_spec);
  ...
  for (chain = sel_ref_chain; chain; chain = TREE_CHAIN (chain))
      ...
      if (warn_selector && objc_implementation_context)
	{
	  ...
	  for (method_chain = meth_var_names_chain;
	       method_chain; method_chain = TREE_CHAIN (method_chain))
	    {
	  ...
	  if (record && !TREE_STATIC_TEMPLATE (record))
	    {
	      error ("`%s' redeclared as different kind of symbol",
		     IDENTIFIER_POINTER (ident));
	      error ("%Jprevious declaration of '%D'", record, record);
	    }
...
is_class_name (tree ident)
{
  ...
  while (ident && TREE_CODE (ident) == TYPE_DECL
...
objc_is_object_ptr (tree type)
{
  ...
  if (id_type && type && TYPE_P (type)
      && (IS_ID (type) || TREE_TYPE (type) == TREE_TYPE (objc_class_type)))
    return type;
...
get_class_ivars (tree interface, int raw)
{
  ...
  while (super_name)
      ...
      if (!super_interface)
	{
	  ...
	  error
	    ("cannot find interface declaration for `%s', superclass of `%s'",
...
objc_exit_block (void)
{
  ...
  SCOPE_STMT_BLOCK (TREE_PURPOSE (scope_stmt))
    = SCOPE_STMT_BLOCK (TREE_VALUE (scope_stmt)) = inner;
...
objc_build_try_enter_fragment (void)
{
    ...
    = tree_cons (NULL_TREE,
		 build_c_cast (TYPE_ARG_TYPES (TREE_TYPE (objc_setjmp_decl))
			       ...
			       TREE_VALUE (TYPE_ARG_TYPES
					   (TREE_TYPE (objc_setjmp_decl))) :
			       ptr_type_node, build_unary_op (ADDR_EXPR,
							      build_component_ref
							      (TREE_VALUE
  ...
  cond = build_unary_op (TRUTH_NOT_EXPR,
			 ...
			 0);
  c_expand_start_cond (c_common_truthvalue_conversion (cond), 0, if_stmt);
  objc_enter_block ();
}
...
objc_build_extract_fragment (void)
{
  ...
  c_expand_expr_stmt (build_modify_expr
		      (TREE_VALUE (objc_rethrow_exception),
		       NOP_EXPR, objc_build_extract_expr ()));
  objc_exit_block ();
  c_finish_else ();
  c_expand_end_cond ();
  if_nesting_count--;
...
objc_build_catch_stmt (tree catch_expr)
{
  ...
  for (prev_catch = objc_catch_type; TREE_VALUE (prev_catch);
	...
	warning
	  ("Exception of type `%s *' already handled by `@catch (%s *)'",
...
objc_build_catch_epilogue (void)
{
  ...
  c_expand_expr_stmt
    (build_modify_expr
     (TREE_VALUE (objc_rethrow_exception),
      NOP_EXPR, TREE_VALUE (objc_caught_exception)));
  ...
  objc_eval_once
    = tree_cons (NULL_TREE,
		 objc_declare_variable (RID_AUTO,
					...
					id_type, sync_expr), objc_eval_once);
  objc_build_try_prologue ();
  objc_enter_block ();
  func_params = tree_cons (NULL_TREE, TREE_VALUE (objc_eval_once), NULL_TREE);
...
objc_build_synchronized_epilogue (void)
{
  ...
  c_expand_expr_stmt (build_function_call (objc_sync_exit_decl, func_params));
...
build_objc_exception_stuff (void)
{
  ...
  objc_exception_extract_decl =
    builtin_function (TAG_EXCEPTIONEXTRACT, temp_type, 0, NOT_BUILT_IN, NULL,
		      NULL_TREE);
  ...
  objc_exception_try_enter_decl =
    builtin_function (TAG_EXCEPTIONTRYENTER, temp_type, 0, NOT_BUILT_IN, NULL,
		      NULL_TREE);
  objc_exception_try_exit_decl =
    builtin_function (TAG_EXCEPTIONTRYEXIT, temp_type, 0, NOT_BUILT_IN, NULL,
		      NULL_TREE);
  ...
  objc_sync_enter_decl =
    builtin_function (TAG_SYNCENTER, temp_type, 0, NOT_BUILT_IN, NULL,
		      NULL_TREE);
  objc_sync_exit_decl =
  ...
  objc_exception_match_decl
    =
    builtin_function (TAG_EXCEPTIONMATCH, temp_type, 0, NOT_BUILT_IN, NULL,
		      NULL_TREE);
...
encode_method_prototype (tree method_decl)
{
  ...
  encode_type (objc_method_parm_type (method_decl),
	       obstack_object_size (&util_obstack), OBJC_ENCODE_INLINE_DEFS);
  ...
  for (parms = METHOD_SEL_ARGS (method_decl); parms;
       parms = TREE_CHAIN (parms))
    {
      ...
      if (sz < 0)
	{
	  error ("%Jtype '%D' does not have a known size", type, type);
...
build_category_template (void)
{
  ...
  decl_specs = build_tree_list (NULL_TREE, ridpointers[(int) RID_CHAR]);
  field_decl =
    build1 (INDIRECT_REF, NULL_TREE, get_identifier ("class_name"));
  field_decl = grokfield (field_decl, decl_specs, NULL_TREE);
  chainon (field_decl_chain, field_decl);
...
build_class_template (void)
{
  ...
  if (flag_next_runtime)
  ...
  else
    {
      ...
      field_decl =
	build1 (INDIRECT_REF, NULL_TREE, get_identifier ("dtable"));
      field_decl = grokfield (field_decl, decl_specs, NULL_TREE);
      chainon (field_decl_chain, field_decl);
  ...
  if (flag_next_runtime)
    {
      ...
      decl_specs = build_tree_list (NULL_TREE, ridpointers[(int) RID_VOID]);
      field_decl =
	build1 (INDIRECT_REF, NULL_TREE, get_identifier ("sel_id"));
      field_decl = grokfield (field_decl, decl_specs, NULL_TREE);
      chainon (field_decl_chain, field_decl);
    }
  ...
  decl_specs = build_tree_list (NULL_TREE, ridpointers[(int) RID_VOID]);
  field_decl =
    build1 (INDIRECT_REF, NULL_TREE, get_identifier ("gc_object_type"));
  field_decl = grokfield (field_decl, decl_specs, NULL_TREE);
  chainon (field_decl_chain, field_decl);
...
synth_forward_declarations (void)
{
  ...
  UOBJC_CLASS_decl = build_metadata_decl ("_OBJC_CLASS", objc_class_template);
...
check_ivars (tree inter, tree imp)
{
  ...
  while (1)
      ...
      if (intdecls == 0 || impdecls == 0)
	{
      ...
      t1 = TREE_TYPE (intdecls);
      t2 = TREE_TYPE (impdecls);
      ...
      else if (DECL_NAME (intdecls) != DECL_NAME (impdecls))
	{
	  error_with_ivar ("conflicting instance variable name",
			   impdecls, rawimpdecls);
	  error_with_ivar ("previous declaration of", intdecls, rawintdecls);
	}
...
build_super_template (void)
{
  ...
  objc_super_template =
    start_struct (RECORD_TYPE, get_identifier (UTAG_SUPER));
...
build_method_list_template (tree list_type, int size)
{
  ...
  decl_specs
    = build_tree_list
    (NULL_TREE,
     xref_tag (RECORD_TYPE, get_identifier (UTAG_METHOD_PROTOTYPE_LIST)));
  field_decl
...
generate_ivars_list (tree type, const char *name, int size, tree list)
{
  ...
  decl =
    start_decl (synth_id_with_class_suffix
		(name, objc_implementation_context), decl_specs, 1,
		NULL_TREE);
...
generate_ivar_lists (void)
{
  ...
  if (CLASS_SUPER_NAME (implementation_template) == NULL_TREE
      && (chain = TYPE_FIELDS (objc_class_template)))
    {
      ...
      ivar_list_template =
	build_ivar_list_template (objc_ivar_template, size);
...
generate_dispatch_tables (void)
{
  ...
  if (chain)
	...
	UOBJC_INSTANCE_METHODS_decl =
	  generate_dispatch_table (method_list_template,
				   "_OBJC_INSTANCE_METHODS", size, initlist);
      else
	/* We have a category.  */
	UOBJC_INSTANCE_METHODS_decl
	  = generate_dispatch_table (method_list_template,
...
generate_shared_structures (void)
{
  ...
  initlist
    ...
    (TREE_TYPE (decl),
     ...
     2 /*CLS_META */ ,
     UOBJC_CLASS_METHODS_decl, UOBJC_CLASS_VARIABLES_decl, protocol_decl);

  finish_decl (decl, initlist, NULL_TREE);

  /* static struct objc_class _OBJC_CLASS_Foo={ ... }; */

  decl = start_decl (DECL_NAME (UOBJC_CLASS_decl), decl_specs, 1, NULL_TREE);
  ...
  initlist
    ...
    (TREE_TYPE (decl),
     ...
     1 /*CLS_FACTORY */ ,
     UOBJC_INSTANCE_METHODS_decl,
     UOBJC_INSTANCE_VARIABLES_decl, protocol_decl);
...
adjust_type_for_id_default (tree type)
{
  ...
  if (!type)
    return
      build_tree_list (build_tree_list (NULL_TREE, objc_object_reference),
		       build1 (INDIRECT_REF, NULL_TREE, NULL_TREE));
...
receiver_is_class_object (tree receiver, int self, int super)
{
  ...
  if (TREE_CODE (receiver) == CALL_EXPR
      ...
      && (exp = TREE_OPERAND (exp, 0)) && TREE_CODE (exp) == FUNCTION_DECL
      ...
      && TREE_CODE (arg) == TREE_LIST && (arg = TREE_VALUE (arg)))
    {
      STRIP_NOPS (arg);
      if (TREE_CODE (arg) == ADDR_EXPR
	  && (arg = TREE_OPERAND (arg, 0)) && TREE_CODE (arg) == STRING_CST)
...
lookup_method_in_hash_lists (tree sel_name, int is_class)
{
  ...
  if (!is_class)
    method_prototype = hash_lookup (nst_method_hash_list, sel_name);

  if (!method_prototype)
    {
      method_prototype = hash_lookup (cls_method_hash_list, sel_name);
      is_class = 1;
    }
...
finish_message_expr (tree receiver, tree sel_name, tree method_params)
{
  ...
  if (!rtype || IS_ID (rtype)
	...
	method_prototype
	  = lookup_method_in_hash_lists (sel_name, is_class != NULL_TREE);
    }
  else
    {
	       ...
	       || TREE_CODE (rtype) == CLASS_IMPLEMENTATION_TYPE)
	{
	  ...
	  if (!method_prototype && objc_implementation_context
		...
		: CLASS_NST_METHODS (objc_implementation_context)), sel_name);
  ...
  if (!method_prototype)
	...
	warning ("`%s' may not respond to `%c%s'",
		 ...
		 (is_class ? '+' : '-'), IDENTIFIER_POINTER (sel_name));
      if (rprotos)
			...
			tree lookup_object, tree selector, tree method_params)
{
  ...
  tree ret_type
    = (method_prototype
       ? groktypename (TREE_TYPE (method_prototype)) : id_type);
  tree sender_cast = build_pointer_type (build_function_type (ret_type,
							      get_arg_type_list
							      (method_prototype,
							       METHOD_REF,
							       super_flag)));
...
lookup_method_static (tree interface, tree ident, int is_class)
{
  ...
  while (inter)
    {
      tree chain =
	is_class ? CLASS_CLS_METHODS (inter) : CLASS_NST_METHODS (inter);
      tree category = inter;
	...
	{
	  chain =
	    is_class ? CLASS_CLS_METHODS (category) :
	  ...
	  if (CLASS_PROTOCOL_LIST (category))
	    {
	      if ((meth = (lookup_method_in_protocol_list
			   (CLASS_PROTOCOL_LIST (category), ident,
			    is_class))))
		return meth;
	    }
	}
...
objc_add_method (tree class, tree method, int is_class)
{
  ...
  if (!
      (mth =
       lookup_method (is_class ? CLASS_CLS_METHODS (class) :
		       ...
		       tree declspecs, tree width)
{
  ...
  const char *ivar_name = DECL_NAME (field_decl)
    ? IDENTIFIER_POINTER (DECL_NAME (field_decl)) : "<unnamed>";
  tree raw_decl;
  ...
  if (IS_AGGR_TYPE (field_type) && (TYPE_NEEDS_CONSTRUCTING (field_type)
      ...
      if (TYPE_POLYMORPHIC_P (field_type))
	{
	  ...
	  error
	    ("illegal aggregate type `%s' specified for instance variable `%s'",
	     type_name, ivar_name);
	  /* Return class as is without adding this ivar.  */
	  return class;
	}
...
is_public (tree expr, tree identifier)
{
  ...
  if (code == RECORD_TYPE)
      ...
      if (TREE_STATIC_TEMPLATE (basetype))
	{
      ...
      else if (objc_implementation_context
	       && (basetype == objc_object_reference))
	{
      ...
      while (context)
	{
	  ...
	  if (lookup_method (list, chain))
	    ...
	    context = (CLASS_NAME (context)
		       ? lookup_interface (CLASS_NAME (context)) : NULL_TREE);
	  else
	    abort ();
	}
...
finish_class (tree class)
{
  if (TREE_CODE (class) == CLASS_IMPLEMENTATION_TYPE)
    ...
    {
      tree category =
	lookup_category (implementation_template, CLASS_SUPER_NAME (class));
...
encode_type (tree type, int curtype, int format)
{
  ...
  if (code == INTEGER_TYPE)
	...
	{
	case 8:
	  c = TREE_UNSIGNED (type) ? 'C' : 'c';
	  break;
	case 16:
	  c = TREE_UNSIGNED (type) ? 'S' : 's';
	  break;
	case 32:
...
comp_proto_with_proto (tree proto1, tree proto2)
{
  ...
  for (type1 = get_arg_type_list (proto1, METHOD_REF, 0),
       ...
       type1 && type2; type1 = TREE_CHAIN (type1), type2 = TREE_CHAIN (type2))
    {
...
really_start_method (tree method, tree parmlist)
{
  ...
  if (implementation_template != objc_implementation_context)
    {
      tree proto = lookup_method_static (implementation_template,
      ...
      else
	{
	  ...
	  if (TREE_CODE (objc_implementation_context)
	    ...
	    interface = lookup_category
	      (interface, CLASS_SUPER_NAME (objc_implementation_context));
	...
	/* We have a category.  */
	{
	  ...
	  if (flag_next_runtime && !flag_zero_link)
	  ...
	  else
	    {
	      ...
	      super_class =
		(TREE_CODE (objc_method_context) ==
		 INSTANCE_METHOD_DECL ? objc_get_class_decl :
		 objc_get_meta_class_decl);
...
{
  if (objc_method_context && UOBJC_SUPER_scope == get_current_scope ())
    {
      UOBJC_SUPER_decl = 0;
      UOBJC_SUPER_scope = 0;
    }
...
objc_demangle (const char *mangled)
{
  ...
  if (mangled[0] == '_' &&
      (mangled[1] == 'i' || mangled[1] == 'c') && mangled[2] == '_')
    {
