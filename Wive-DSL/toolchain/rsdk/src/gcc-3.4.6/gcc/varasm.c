...
/* RTX_UNCHANGING_P in a MEM can mean it is stored into, for initialization.
   ...
   conflict with them.  */
     ...
     static void output_constructor (tree, unsigned HOST_WIDE_INT,
				     unsigned int);
...

     ...
     static GTY (())
     enum in_section in_section = no_section;
...
static
GTY ((param_is (struct in_named_entry)))
			...
			int flag_function_or_data_sections)
{
  if (DECL_SECTION_NAME (decl) == NULL_TREE
      ...
      && (flag_function_or_data_sections || DECL_ONE_ONLY (decl)))
			    ...
			    unsigned int flags ATTRIBUTE_UNUSED)
{
  ...
  if (HAVE_GAS_SHF_MERGE && flag_merge_constants
      ...
      && mode != BLKmode
      && modesize <= align
      && align >= 8 && align <= 256 && (align & (align - 1)) == 0)
    {
      char name[24];
...
make_decl_rtl (tree decl, const char *asmspec)
{
  ...
  if (reg_number >= 0 || reg_number == -3)
    error ("%Jregister name given for non-register variable '%D'", decl,
	   decl);
		   ...
		   int at_end ATTRIBUTE_UNUSED, int dont_output_data)
{
  ...
  if (!dont_output_data && !host_integerp (DECL_SIZE_UNIT (decl), 1))
    {
...
#define MAX_OFILE_ALIGNMENT BIGGEST_ALIGNMENT
  ...
  /* We don't implement common thread-local data at present.  */
	...
	rounded = 1;
      ...
      if ((unsigned HOST_WIDE_INT) DECL_ALIGN (decl) / BITS_PER_UNIT >
	  rounded)
...
enum kind
{ RTX_UNKNOWN, RTX_DOUBLE, RTX_VECTOR, RTX_INT, RTX_UNSPEC };
struct rtx_const
GTY (())
{
...
static
GTY ((param_is (struct constant_descriptor_tree)))
     htab_t const_desc_htab;

     static struct constant_descriptor_tree *build_constant_desc (tree);
     static void maybe_output_constant_def_contents (struct
						     constant_descriptor_tree
						     *, int);

/* Compute a hash code for a constant expression.  */

     static hashval_t const_desc_hash (const void *ptr)
{
  ...
  switch (TREE_CODE (t1))
      ...
      else
	{
	  ...
	  for (l1 = CONSTRUCTOR_ELTS (t1), l2 = CONSTRUCTOR_ELTS (t2);
	       l1 && l2; l1 = TREE_CHAIN (l1), l2 = TREE_CHAIN (l2))
	    {
	...
	return (value1.offset == value2.offset
		&& strcmp (XSTR (value1.base, 0),
			   XSTR (value2.base, 0)) == 0);
      }
    ...
    case RANGE_EXPR:
      return (compare_constant (TREE_OPERAND (t1, 0), TREE_OPERAND (t2, 0))
	      && compare_constant (TREE_OPERAND (t1, 1),
				   TREE_OPERAND (t2, 1)));
...
output_constructor (tree exp, unsigned HOST_WIDE_INT size, unsigned int align)
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE && TYPE_DOMAIN (type) != 0)
  ...
  for (link = CONSTRUCTOR_ELTS (exp);
       link; link = TREE_CHAIN (link), field = field ? TREE_CHAIN (field) : 0)
    {
	...
	fprintf (asm_out_file, "%s %s:\n",
		 ASM_COMMENT_START,
		 DECL_NAME (field)
		 ? IDENTIFIER_POINTER (DECL_NAME (field)) : "<anonymous>");
#endif
...
/* Mark DECL as weak.  */
     ...
     static void mark_weak (tree decl)
{
  DECL_WEAK (decl) = 1;
...
static
GTY ((param_is (struct output_def_pair)))
...
/* Output the assembler code for a define (equate) using ASM_OUTPUT_DEF
   ...
   tree node is DECL to have the value of the tree node TARGET.  */
     ...
     static void
       assemble_output_def (tree decl ATTRIBUTE_UNUSED,
			...
			unsigned HOST_WIDE_INT align ATTRIBUTE_UNUSED)
{
  ...
  if (DECL_P (decl))
  ...
  else if (TREE_CODE (decl) == CONSTRUCTOR)
    {
      if (!((flag_pic && reloc)
	    || !TREE_READONLY (decl)
	    || TREE_SIDE_EFFECTS (decl) || !TREE_CONSTANT (decl)))
	readonly = true;
    }
...
static enum section_category categorize_decl_for_section (tree, int, int);
