...
compute_branch_probabilities (void)
{
  ...
  FOR_BB_BETWEEN (bb, ENTRY_BLOCK_PTR, NULL, next_bb)
  {
      ...
      if (!EDGE_INFO (e)->ignore && !EDGE_INFO (e)->on_tree)
	{
	  ...
	  if (exec_counts)
		...
		{
		  error
		    ("corrupted profile info: edge from %i to %i exceeds maximal count",
  ...
  FOR_BB_BETWEEN (bb, ENTRY_BLOCK_PTR, NULL, next_bb)
  {
    ...
    if (bb->count < 0)
      {
	error
	  ("corrupted profile info: number of iterations for basic block %d thought to be %i",
	...
	if ((e->count < 0
	    ...
	    || (e->count > bb->count && e->dest != EXIT_BLOCK_PTR))
	  {
	    rtx insn = BB_END (bb);

	    while (GET_CODE (insn) != CALL_INSN
		   && insn != BB_HEAD (bb) && keep_with_call_p (insn))
	      insn = PREV_INSN (insn);
	...
	for (e = bb->succ; e; e = e->succ_next)
	  e->probability =
	    (e->count * REG_BR_PROB_BASE + bb->count / 2) / bb->count;
	if (bb->index >= 0 && any_condjump_p (BB_END (bb))
	    && bb->succ->succ_next)
	  {
	    int prob;
	    edge e;
	...
	if (bb->index >= 0
	    && any_condjump_p (BB_END (bb)) && bb->succ->succ_next)
  ...
  if (rtl_dump_file)
	...
	for (i = 0; i < 10; i++)
	  fprintf (rtl_dump_file, "%d%% branches in range %d-%d%%\n",
		   (hist_br_prob[i] +
		    hist_br_prob[19 - i]) * 100 / num_branches, 5 * i,
		   5 * i + 5);
...
compute_value_histograms (unsigned n_values, struct histogram_value *values)
{
  ...
  for (i = 0; i < n_values; i++)
	...
	hist_list =
	  alloc_EXPR_LIST (0, GEN_INT (aact_count[j - 1]), hist_list);
	...
	while (insn != BB_END (bb))
	  {
		...
		if (NOTE_LINE_NUMBER (insn) == NOTE_INSN_REPEATED_LINE_NUMBER)
		...
		else
		  {
		    ...
		    if (!prev_file_name
			|| strcmp (NOTE_SOURCE_FILE (insn), prev_file_name))
		      {
...
find_spanning_tree (struct edge_list *el)
{
  ...
  FOR_BB_BETWEEN (bb, ENTRY_BLOCK_PTR, NULL, next_bb) bb->aux = bb;
  ...
  FOR_BB_BETWEEN (bb, ENTRY_BLOCK_PTR, NULL, next_bb) bb->aux = NULL;
}

...
end_branch_prob (void)
{
  if (rtl_dump_file)
	...
	fprintf (rtl_dump_file,
		 ...
		 (total_num_passes +
		  (total_num_times_called >> 1)) / total_num_times_called);
      ...
      if (total_num_branches)
	{
	  ...
	  for (i = 0; i < 10; i++)
	    fprintf (rtl_dump_file, "%d%% branches in range %d-%d%%\n",
		     (total_hist_br_prob[i] +
		      total_hist_br_prob[19 - i]) * 100 / total_num_branches,
		     5 * i, 5 * i + 5);
	}
    }
}
		       ...
		       unsigned base)
{
  ...
  if (value->hdata.intvl.may_be_more)
    do_compare_rtx_and_jump (copy_rtx (val),
			     GEN_INT (value->hdata.intvl.steps), GE, 0,
			     value->mode, NULL_RTX, NULL_RTX, more_label);
  ...
  if (value->hdata.intvl.may_be_more || value->hdata.intvl.may_be_less)
    {
...
gen_pow2_profiler (struct histogram_value *value, unsigned tag, unsigned base)
{
  ...
  if (value->hdata.pow2.may_be_other)
    {
      do_compare_rtx_and_jump (copy_rtx (uval), const0_rtx, LE, 0,
			       value->mode, NULL_RTX, NULL_RTX,
			       end_of_code_label);
      tmp =
	expand_simple_binop (value->mode, PLUS, copy_rtx (uval), constm1_rtx,
			     NULL_RTX, 0, OPTAB_WIDEN);
      tmp =
	expand_simple_binop (value->mode, AND, copy_rtx (uval), tmp, NULL_RTX,
			     0, OPTAB_WIDEN);
  ...
  tmp =
    expand_simple_binop (Pmode, PLUS, copy_rtx (mr), GEN_INT (per_counter),
			 mr, 0, OPTAB_WIDEN);
  if (tmp != mr)
    emit_move_insn (copy_rtx (mr), tmp);

  tmp =
    expand_simple_binop (value->mode, ASHIFTRT, copy_rtx (uval), const1_rtx,
			 uval, 0, OPTAB_WIDEN);
  if (tmp != uval)
