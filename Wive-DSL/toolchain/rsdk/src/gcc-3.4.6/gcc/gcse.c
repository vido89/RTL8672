...
gmalloc (size_t)
  ATTRIBUTE_MALLOC;
     ...
     static int hoist_expr_reaches_here_p (basic_block, int, basic_block,
					   char *);
     ...
     static int delete_null_pointer_checks_1 (unsigned int *, sbitmap *,
					      sbitmap *,
					      struct null_pointer_info *);
     ...
     static bool store_killed_after (rtx, rtx, rtx, basic_block, int *,
				     rtx *);
  ...
  while (changed && pass < MAX_GCSE_PASSES)
      ...
      else
	{
	  ...
	  if (changed)
		...
		gcalloc (last_basic_block, sizeof (rtx));
	    }
	  free_reg_set_mem ();
...
oprs_unchanged_p (rtx x, rtx insn, int avail_p)
{
  ...
  switch (code)
    ...
    case MEM:
      if (load_killed_in_block_p (current_bb, INSN_CUID (insn), x, avail_p))
	return 0;
      else
...
hash_scan_set (rtx pat, rtx insn, struct hash_table *table)
{
  ...
  if (GET_CODE (src) == CALL)
	...
	src = XEXP (note, 0), pat = gen_rtx_SET (VOIDmode, dest, src);
      ...
      if (!table->set_p && regno >= FIRST_PSEUDO_REGISTER
	      ...
	      || GET_CODE (XEXP (note, 0)) != MEM))
	{
      ...
      /* Record sets for constant/copy propagation.  */
	       ...
	       && ((GET_CODE (src) == REG
		    ...
		    && can_copy_p (GET_MODE (dest))
		    && REGNO (src) != regno) || gcse_constant_p (src))
...
compute_hash_table_work (struct hash_table *table)
{
  ...
  FOR_EACH_BB (current_bb)
  {
    ...
    if (table->set_p && implicit_sets[current_bb->index] != NULL_RTX)
...
compute_kill_rd (void)
{
  ...
  FOR_EACH_BB (bb) for (cuid = 0; cuid < max_cuid; cuid++)
...
compute_rd (void)
{
  ...
  while (changed)
	...
	changed |=
	  sbitmap_union_of_diff_cg (rd_out[bb->index], rd_gen[bb->index],
				    reaching_defs[bb->index],
				    rd_kill[bb->index]);
      }
      passes++;
    }
...
compute_ae_gen (struct hash_table *expr_hash_table)
{
  ...
  for (i = 0; i < expr_hash_table->size; i++)
    for (expr = expr_hash_table->table[i]; expr != 0;
	 expr = expr->next_same_hash)
		 ...
		 struct hash_table *expr_hash_table)
{
  ...
  FOR_EACH_BB (bb) for (i = 0; i < expr_hash_table->size; i++)
...
static int
can_disregard_other_sets (struct reg_set **addr_this_reg, rtx insn,
			  int for_combine)
{
...
handle_avail_expr (rtx insn, struct expr *expr)
{
  ...
  if (GET_CODE (SET_SRC (expr_set)) == REG)
    {
      ...
      unsigned int regnum_for_replacing = REGNO (SET_SRC (expr_set));
  ...
  if (!found_setting)
    {
      unsigned int regnum_for_replacing = REGNO (SET_DEST (expr_set));
  ...
  /* The register that the expr is computed into is set more than once.  */
      ...
      if (gcse_file != NULL)
	{
	  fprintf (gcse_file,
		   ...
		   REGNO (SET_SRC
			  (PATTERN (NEXT_INSN (insn_computes_expr)))));
...
{
  compute_local_properties (cprop_absaltered, cprop_pavloc, NULL,
			    &set_hash_table);
  compute_available (cprop_pavloc, cprop_absaltered, cprop_avout, cprop_avin);
}

...
constprop_register (rtx insn, rtx from, rtx to, int alter_jumps)
{
  ...
  if (alter_jumps
      ...
      && any_condjump_p (NEXT_INSN (insn)) && onlyjump_p (NEXT_INSN (insn)))
    {
	  ...
	  && cprop_jump (BLOCK_FOR_INSN (insn), insn, NEXT_INSN (insn), from,
			 to))
	return 1;
    }

  /* Handle normal insns next.  */
  if (GET_CODE (insn) == INSN && try_replace_reg (from, to, insn))
    return 1;
...
fis_get_condition (rtx jump)
{
  ...
  if (!REG_P (tmp) || GET_MODE_CLASS (GET_MODE (tmp)) != MODE_INT)
    return NULL_RTX;
  tmp = canonicalize_condition (jump, cond, reverse, &earliest, tmp, false);
  if (!tmp)
    return NULL_RTX;
...
find_implicit_sets (void)
{
  ...
  FOR_EACH_BB (bb)
	  ...
	  && implicit_set_cond_p (cond))
	{
	  ...
	  if (dest && !dest->pred->pred_next && dest != EXIT_BLOCK_PTR)
	    {
	      new = gen_rtx_SET (VOIDmode, XEXP (cond, 0), XEXP (cond, 1));
...
bypass_block (basic_block bb, rtx setcc, rtx jump)
{
  ...
  for (e = bb->pred; e; e = enext)
      ...
      for (i = 0; i < reg_use_count; i++)
	{
	  ...
	  old_dest = e->dest;
	  if (dest != NULL && dest != old_dest && dest != EXIT_BLOCK_PTR)
	    {
...
compute_pre_data (void)
{
  ...
  edge_list =
    pre_edge_lcm (gcse_file, expr_hash_table.n_elems, transp, comp, antloc,
...
static int
pre_expr_reaches_here_p (basic_block occr_bb, struct expr *expr,
			 basic_block bb)
{
  int rval;
  char *visited = xcalloc (last_basic_block, 1);
...
pre_edge_insert (struct edge_list *edge_list, struct expr **index_map)
{
  ...
  for (e = 0; e < num_edges; e++)
      ...
      for (i = indx = 0; i < set_size; i++, indx += SBITMAP_ELT_BITS)
	{
	  ...
	  for (j = indx; insert && j < (int) expr_hash_table.n_elems;
	       j++, insert >>= 1)
...
pre_insert_copy_insn (struct expr *expr, rtx insn)
{
  ...
  if (GET_CODE (pat) == SET)
	...
	{
	  rtx x = XVECEXP (pat, 0, i);
	  if (GET_CODE (x) == SET && expr_equiv_p (SET_SRC (x), expr->expr))
	    {
	      set = x;
	      break;
	    }
...
pre_insert_copies (void)
{
  ...
  for (i = 0; i < expr_hash_table.size; i++)
    for (expr = expr_hash_table.table[i]; expr != NULL;
	 expr = expr->next_same_hash)
      {
...
pre_delete (void)
{
  ...
  for (i = 0; i < expr_hash_table.size; i++)
    for (expr = expr_hash_table.table[i];
	 expr != NULL; expr = expr->next_same_hash)
      {
	int indx = expr->bitmap_index;
...
invalidate_nonnull_info (rtx x, rtx setter ATTRIBUTE_UNUSED, void *data)
{
  ...
  if (GET_CODE (x) != REG
      || REGNO (x) < npi->min_reg || REGNO (x) >= npi->max_reg)
    return;
			      ...
			      struct null_pointer_info *npi)
{
  ...
  FOR_EACH_BB (current_block)
  {
    ...
    for (insn = BB_HEAD (current_block);
	 insn != stop_insn; insn = NEXT_INSN (insn))
      {
	rtx set;
	rtx reg;
	...
	if (GET_CODE (SET_SRC (set)) == MEM
	    ...
	    && REGNO (reg) >= npi->min_reg && REGNO (reg) < npi->max_reg)
  ...
  FOR_EACH_BB (bb)
  {
    ...
    if (!TEST_BIT
	(nonnull_avout[bb->index], block_reg[bb->index] - npi->min_reg))
      continue;
...
delete_null_pointer_checks (rtx f ATTRIBUTE_UNUSED)
{
  ...
  FOR_EACH_BB (bb)
  {
	...
	|| !any_condjump_p (last_insn) || !onlyjump_p (last_insn))
      continue;
  ...
  while (changed)
    {
      ...
      {
	changed |=
	  sbitmap_a_or_b_and_c_cg (hoist_vbein[bb->index], antloc[bb->index],
				   hoist_vbeout[bb->index],
				   transp[bb->index]);
	if (bb->next_bb != EXIT_BLOCK_PTR)
	  sbitmap_intersection_of_succs (hoist_vbeout[bb->index], hoist_vbein,
					 bb->index);
      }
...
static int
hoist_expr_reaches_here_p (basic_block expr_bb, int expr_index,
			   basic_block bb, char *visited)
{
  edge pred;
...
compute_ld_motion_mems (void)
{
  ...
  {
    for (insn = BB_HEAD (bb);
	 insn && insn != NEXT_INSN (BB_END (bb)); insn = NEXT_INSN (insn))
      {
	if (INSN_P (insn))
	  {
	    if (GET_CODE (PATTERN (insn)) == SET)
...
static void
reg_set_info (rtx dest, rtx setter ATTRIBUTE_UNUSED, void *data)
{
  sbitmap bb_reg = data;
...
static void
reg_clear_last_set (rtx dest, rtx setter ATTRIBUTE_UNUSED, void *data)
{
  int *dead_vec = data;
...
find_moveable_store (rtx insn, int *regs_set_before, int *regs_set_after)
{
  ...
  if (!ANTIC_STORE_LIST (ptr))
  ...
  else
    {
      ...
      if (tmp != NULL_RTX && BLOCK_FOR_INSN (tmp) != bb)
	check_anticipatable = 1;
    }
  if (check_anticipatable)
    {
      if (store_killed_before
	  (dest, ptr->pattern_regs, insn, bb, regs_set_before))
	tmp = NULL_RTX;
      else
	tmp = insn;
      ANTIC_STORE_LIST (ptr) = alloc_INSN_LIST (tmp, ANTIC_STORE_LIST (ptr));
    }
...
compute_store_table (void)
{
  ...
  FOR_EACH_BB (bb)
  {
    ...
    for (insn = BB_HEAD (bb);
	 insn != NEXT_INSN (BB_END (bb)); insn = NEXT_INSN (insn))
      {
	if (!INSN_P (insn))
	  continue;
  ...
  for (ptr = pre_ldst_mems, prev_next_ptr_ptr = &pre_ldst_mems;
       ptr != NULL; ptr = *prev_next_ptr_ptr)
    {
      if (!AVAIL_STORE_LIST (ptr))
	{
  ...
  if (GET_CODE (PATTERN (insn)) == SET)
    {
      ...
      if (GET_CODE (dest) == SIGN_EXTRACT || GET_CODE (dest) == ZERO_EXTRACT)
	dest = XEXP (dest, 0);

      /* Check for memory stores to aliased objects.  */
      if (GET_CODE (dest) == MEM && !expr_equiv_p (dest, x))
	{
	  if (after)
	    {
...
build_store_vectors (void)
{
  ...
  {
    for (regno = 0; regno < max_gcse_regno; regno++)
      regs_set_in_block[regno] =
	TEST_BIT (reg_set_in_block[bb->index], regno);
  ...
  if (gcse_file)
    {
      dump_sbitmap_vector (gcse_file, "st_antloc", "", st_antloc,
			   last_basic_block);
      dump_sbitmap_vector (gcse_file, "st_kill", "", ae_kill,
			   last_basic_block);
      dump_sbitmap_vector (gcse_file, "Transpt", "", transp,
			   last_basic_block);
      dump_sbitmap_vector (gcse_file, "st_avloc", "", ae_gen,
			   last_basic_block);
    }
}
  ...
  while (1)
      ...
      if (!act)
	{
      ...
      /* We used to continue the loop without scanning this block if the
         ...
         is invalid.  */
      if (bb == EXIT_BLOCK_PTR || TEST_BIT (visited, bb->index))
	{
	  act = act->succ_next;
	  continue;
	}
      ...
      if (TEST_BIT (st_antloc[bb->index], smexpr->index))
	{
	  for (last = ANTIC_STORE_LIST (smexpr);
	       BLOCK_FOR_INSN (XEXP (last, 0)) != bb; last = XEXP (last, 1))
	    continue;
	  last = XEXP (last, 0);
	}
      else
...
store_motion (void)
{
  ...
  for (ptr = first_ls_expr (); ptr != NULL; ptr = next_ls_expr (ptr))
    {
      FOR_EACH_BB (bb) if (TEST_BIT (pre_delete_map[bb->index], ptr->index))
	delete_store (ptr, bb);
