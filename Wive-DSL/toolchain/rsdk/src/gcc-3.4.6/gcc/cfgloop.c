...
{
  return loop->depth > outer->depth && loop->pred[outer->depth] == outer;
}
...
static basic_block
make_forwarder_block (basic_block bb, int redirect_latch,
...
canonicalize_loop_headers (void)
{
  ...
  FOR_EACH_BB (header)
  {
    ...
    max_freq = 0;
    for (e = header->pred; e; e = e->pred_next)
      if (LATCH_EDGE (e) && EDGE_FREQUENCY (e) > max_freq)
	max_freq = EDGE_FREQUENCY (e);
    for (e = header->pred; e; e = e->pred_next)
      if (LATCH_EDGE (e) && EDGE_FREQUENCY (e) >= max_freq / HEAVY_EDGE_RATIO)
	{
	  if (heavy)
	    {
	      is_heavy = 0;
...
flow_loops_find (struct loops *loops, int flags)
{
  ...
  /* Find and record information about all the natural loops
     in the CFG.  */
  loops->num = 1;
  FOR_EACH_BB (bb) bb->loop_father = loops->tree_root;
...
verify_loop_structure (struct loops *loops)
{
  ...
  FOR_EACH_BB (bb) for (loop = bb->loop_father; loop; loop = loop->outer)
    sizes[loop->num]++;
  ...
  for (i = 1; i < loops->num; i++)
	...
	if (!flow_bb_inside_loop_p (loop, bbs[j]))
	  {
	    error ("Bb %d do not belong to loop %d.", bbs[j]->index, i);
	    err = 1;
	  }
      free (bbs);
    }
  ...
  for (i = 1; i < loops->num; i++)
	...
	{
	  if (!loop->latch->succ || loop->latch->succ->succ_next)
	    {
