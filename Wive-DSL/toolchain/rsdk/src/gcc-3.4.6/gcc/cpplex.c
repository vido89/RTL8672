...
#define TK(e, s) { s,              U #e },
static const struct token_spelling token_spellings[N_TTYPES] =
  { TTYPE_TABLE };
#undef OP
#undef TK
...
static void save_comment (cpp_reader *, cpp_token *, const uchar *,
			  cppchar_t);
...
static void
save_comment (cpp_reader * pfile, cpp_token * token,
	      const unsigned char *from, cppchar_t type)
{
...
_cpp_get_fresh_line (cpp_reader * pfile)
{
  ...
  for (;;)
	...
	return false;
	  ...
	  && buffer->next_line > buffer->rlimit && !buffer->from_stage3)
	{
	  /* Only warn once.  */
	  buffer->next_line = buffer->rlimit;
		 ...
		 const cpp_token * token2)
{
  ...
  switch (a)
    ...
    case CPP_DEREF:
      return c == '*';
    case CPP_DOT:
      return c == '.' || c == '%' || b == CPP_NUMBER;
    case CPP_HASH:
      return c == '#' || c == '%';	/* Digraph form.  */
    case CPP_NAME:
      return ((b == CPP_NUMBER && name_p (pfile, &token2->val.str)) || b == CPP_NAME || b == CPP_CHAR || b == CPP_STRING);	/* L */
    case CPP_NUMBER:
      return (b == CPP_NUMBER || b == CPP_NAME
	      || c == '.' || c == '+' || c == '-');
      /* UCNs */
    case CPP_OTHER:
...
_cpp_buff *
_cpp_append_extend_buff (cpp_reader * pfile, _cpp_buff * buff,
			 size_t min_extra)
{
