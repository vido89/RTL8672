...
enum typestatus
{ TYPE_UNSEEN, TYPE_XREF, TYPE_DEFINED };
...
/* In dbx output, each type gets a unique number.
   ...
   The number, once assigned, is in the TYPE_SYMTAB_ADDRESS field.  */
     ...
     enum binclstatus
     { BINCL_NOT_REQUIRED, BINCL_PENDING, BINCL_PROCESSED };
...
{
  if (TREE_CODE (decl) == VAR_DECL && !DECL_EXTERNAL (decl) && DECL_RTL_SET_P (decl))	/* Not necessary?  */
    {
  ...
  for (tem = TYPE_FIELDS (type); tem; tem = TREE_CHAIN (tem))
	...
	return;
	  ...
	  || (TREE_CODE (tem) == FIELD_DECL
		  ...
		  || !DECL_SIZE (tem) || !host_integerp (DECL_SIZE (tem), 1)))
	  ...
	  if (use_gnu_debug_info_extensions
		  ...
		  || TREE_CODE (tem) != FIELD_DECL))
	    {
	      ...
	      putc ((TREE_PRIVATE (tem) ? '0'
		     : TREE_PROTECTED (tem) ? '1' : '2'), asmfile);
	      CHARS (2);
	    }
  ...
  while (fndecl)
    {
      ...
      for (last = NULL_TREE;
	   fndecl && (last == NULL_TREE
		      || DECL_NAME (fndecl) == DECL_NAME (last));
...
dbxout_range_type (tree type)
{
  ...
  if (TYPE_MIN_VALUE (type) != 0 && host_integerp (TYPE_MIN_VALUE (type), 0))
    {
      putc (';', asmfile);
      CHARS (1);
  ...
  if (TYPE_MAX_VALUE (type) != 0 && host_integerp (TYPE_MAX_VALUE (type), 0))
    {
      putc (';', asmfile);
      CHARS (1);
      if (print_int_cst_bounds_in_octal_p (type))
	...
	for (i = 0; i < n_baseclasses; i++)
	  {
	    ...
	    tree access = (BINFO_BASEACCESSES (binfo)
			   ? BINFO_BASEACCESS (binfo,
					       i) : access_public_node);
		...
		putc (access == access_public_node ? '2' :
		      (access == access_protected_node ? '1' : '0'), asmfile);
		CHARS (2);
...
dbxout_symbol (tree decl, int local ATTRIBUTE_UNUSED)
{
  ...
  switch (TREE_CODE (decl))
	...
	if (tag_needed && TYPE_NAME (type) != 0
	    ...
	    && COMPLETE_TYPE_P (type) && !TREE_ASM_WRITTEN (TYPE_NAME (type)))
	  {
...
dbxout_symbol_location (tree decl, tree type, const char *suffix, rtx home)
{
  ...
  if (GET_CODE (home) == MEM && GET_CODE (XEXP (home, 0)) == SYMBOL_REF)
    {
      if (TREE_PUBLIC (decl))
	{
	  letter = 'G';
      ...
      else
	{
	  ...
	  current_sym_value = DEBUGGER_AUTO_OFFSET (XEXP (XEXP (home, 0), 0));
	}
  ...
  else if (GET_CODE (home) == MEM && GET_CODE (XEXP (home, 0)) == REG)
    {
      current_sym_code = N_LSYM;
  ...
  else if (GET_CODE (home) == MEM && GET_CODE (XEXP (home, 0)) == CONST)
    {
...
dbxout_symbol_name (tree decl, const char *suffix, int letter)
{
  ...
  if (name == 0)
    name = "(anon)";
  fprintf (asmfile, "%s\"%s%s:", ASM_STABS_OP, name, (suffix ? suffix : ""));
  ...
  for (; parms; parms = TREE_CHAIN (parms))
	...
	/* Report parms that live in memory but not where they were passed.  */
		 ...
		 && !rtx_equal_p (DECL_RTL (parms),
				  DECL_INCOMING_RTL (parms)))
	  dbxout_symbol_location (parms, TREE_TYPE (parms), 0,
				  DECL_RTL (parms));
      }
  DBXOUT_DECR_NESTING;
}
...
/* Subroutine of dbxout_block.  Emit an N_LBRAC stab referencing LABEL.
   ...
   be required.  */
static void
dbx_output_lbrac (const char *label, const char *begin_label ATTRIBUTE_UNUSED)
{
...
/* Subroutine of dbxout_block.  Emit an N_RBRAC stab referencing LABEL.
   ...
   be required.  */
static void
dbx_output_rbrac (const char *label, const char *begin_label ATTRIBUTE_UNUSED)
{
