...
static rtx test_for_iteration (struct loop_desc *desc,
			       unsigned HOST_WIDE_INT);
static bool constant_iterations (struct loop_desc *, unsigned HOST_WIDE_INT *,
				 bool *);
static bool simple_loop_exit_p (struct loop *, edge, regset, rtx *,
				struct loop_desc *);
...
static bool simple_condition_p (struct loop *, rtx, regset,
				struct loop_desc *);
static basic_block simple_increment (struct loop *, rtx *,
				     struct loop_desc *);
static rtx count_strange_loop_iterations (rtx, rtx, enum rtx_code, int, rtx,
					  enum machine_mode,
					  enum machine_mode);
...
blocks_invariant_registers (basic_block * bbs, int nbbs, regset regs)
{
  ...
  for (i = 0; i < nbbs; i++)
    for (insn = BB_HEAD (bbs[i]);
	 insn != NEXT_INSN (BB_END (bbs[i])); insn = NEXT_INSN (insn))
      if (INSN_P (insn))
	note_stores (PATTERN (insn),
		     (void (*)(rtx, rtx, void *)) unmark_altered, regs);
}
...
blocks_single_set_registers (basic_block * bbs, int nbbs, rtx * regs)
{
  ...
  for (i = 0; i < nbbs; i++)
	...
	data.insn = insn;
	note_stores (PATTERN (insn),
		     (void (*)(rtx, rtx, void *)) unmark_altered_insn, &data);
      }
}
...
invariant_rtx_wrto_regs_p (rtx expr, regset invariant_regs)
{
  return !for_each_rtx (&expr,
			(rtx_function) invariant_rtx_wrto_regs_p_helper,
			invariant_regs);
}
		  ...
		  struct loop_desc *desc)
{
  ...
  if (GET_CODE (set_src) == SIGN_EXTEND || GET_CODE (set_src) == ZERO_EXTEND)
    {
	  ...
	  && ((desc->cond == LEU
	       || desc->cond == LTU
	       || desc->cond == GEU
	       || desc->cond == GTU) ^ (GET_CODE (set_src) == ZERO_EXTEND)))
	return NULL;

      if (GET_CODE (XEXP (set_src, 0)) != SUBREG
	  || SUBREG_BYTE (XEXP (set_src, 0)) != 0
	  || GET_MODE (SUBREG_REG (XEXP (set_src, 0))) !=
	  GET_MODE (desc->var))
	return NULL;
      ...
      if (insn != BB_HEAD (bb))
	{
	  ...
	  if ((GET_CODE (val) == SIGN_EXTEND
	    ...
	    ret = gen_rtx_fmt_e (GET_CODE (val),
				 GET_MODE (var),
				 gen_rtx_fmt_ei (SUBREG, inner_mode, var, 0));
			       ...
			       enum machine_mode inner_mode)
{
  ...
  switch (cond)
    ...
    case LT:
      ...
      mode_min = gen_int_mode (-((unsigned HOST_WIDEST_INT) 1 << (size - 1)),
			       mode);
      mode_max =
	gen_int_mode (((unsigned HOST_WIDEST_INT) 1 << (size - 1)) - 1, mode);
  ...
  switch (cond)
	...
	abort ();
      n_to_wrap =
	simplify_gen_binary (MINUS, mode, mode_max, copy_rtx (init));
      ...
      after_wrap = simplify_gen_binary (PLUS, mode, before_wrap, stride);
      if (GET_CODE (after_wrap) != CONST_INT)
	{
	  after_wrap = simplify_gen_binary (PLUS, mode, mode_min, stride);
	  after_wrap =
	    simplify_gen_binary (MINUS, mode, after_wrap, const1_rtx);
	}
      break;
	...
	abort ();
      ...
      n_to_wrap =
	simplify_gen_binary (MINUS, mode, copy_rtx (init), mode_min);
      n_to_wrap = simplify_gen_binary (UDIV, mode, n_to_wrap, stride);
      before_wrap = simplify_gen_binary (MULT, mode,
					 copy_rtx (n_to_wrap), stride);
      before_wrap = simplify_gen_binary (MINUS, mode,
					 copy_rtx (init), before_wrap);
      after_wrap = simplify_gen_binary (MINUS, mode, before_wrap, stride);
      if (GET_CODE (after_wrap) != CONST_INT)
	{
	  after_wrap = simplify_gen_binary (MINUS, mode, mode_max, stride);
	  after_wrap =
	    simplify_gen_binary (PLUS, mode, after_wrap, const1_rtx);
	}
      break;
    default:
      abort ();
...
fits_in_mode_p (enum machine_mode mode, rtx expr)
{
  ...
  if (GET_CODE (expr) == SIGN_EXTEND || GET_CODE (expr) == ZERO_EXTEND)
...
count_loop_iterations (struct loop_desc * desc, rtx init, rtx lim)
{
  ...
  if (stride == const1_rtx
      || stride == constm1_rtx || cond == NE || cond == EQ)
    {
  ...
  switch (cond)
      ...
      while (s % 2 != 1)
	{
      ...
      bound =
	gen_int_mode (((unsigned HOST_WIDEST_INT) 1 << (size - 1) << 1) - 1,
		      mode);
  ...
  if (cond != NE && stride != const1_rtx)
	...
	{
	  rtx stridem1 =
	    simplify_gen_binary (PLUS, mode, stride, constm1_rtx);
  ...
  FOR_BB_BETWEEN (act, ENTRY_BLOCK_PTR, EXIT_BLOCK_PTR, next_bb)
	  ...
	  || act->loop_father == e->dest->loop_father->outer)
	{
      ...
      /* Edges exiting loops remain.  They should lead from representative
         ...
         act lays.  */
      depth =
	find_common_loop (act->loop_father, e->dest->loop_father)->depth + 1;
