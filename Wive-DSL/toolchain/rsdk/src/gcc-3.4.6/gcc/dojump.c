...
/* Compare the relative costs of "(X & (1 << BITNUM))" and "(X >> BITNUM) & 1"
   ...
   is preferred.  */
     ...
     static bool prefer_and_bit_test (enum machine_mode mode, int bitnum)
{
  if (and_test == 0)
    {
  ...
  switch (code)
    ...
    case PLUS_EXPR:
      ...
      exp = build (MINUS_EXPR, TREE_TYPE (exp),
		   TREE_OPERAND (exp, 0),
		   fold (build1
			 (NEGATE_EXPR, TREE_TYPE (TREE_OPERAND (exp, 1)),
			  TREE_OPERAND (exp, 1))));
      /* Process as MINUS.  */
#endif
	...
	if (!do_rev)
	  ...
	  do_compare_and_jump (exp, rcmp, rcmp, if_true_label,
			       if_false_label);
      }
      break;
