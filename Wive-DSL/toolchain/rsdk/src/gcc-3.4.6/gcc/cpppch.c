...
write_macdef (cpp_reader * pfile, cpp_hashnode * hn, void *file_p)
{
  ...
  switch (hn->type)
	...
	if (fwrite (&s, sizeof (s), 1, f) != 1
	    || fwrite (defn, 1, s.definition_length,
		       f) != s.definition_length)
	  {
...
static int
save_idents (cpp_reader * pfile ATTRIBUTE_UNUSED, cpp_hashnode * hn,
	     void *ss_p)
{
...
static int
count_defs (cpp_reader * pfile ATTRIBUTE_UNUSED, cpp_hashnode * hn,
	    void *ss_p)
{
  struct cpp_savedstate *const ss = (struct cpp_savedstate *) ss_p;
...
static int
write_defs (cpp_reader * pfile ATTRIBUTE_UNUSED, cpp_hashnode * hn,
	    void *ss_p)
{
  struct cpp_savedstate *const ss = (struct cpp_savedstate *) ss_p;
...
cpp_valid_state (cpp_reader * r, const char *name, int fd)
{
  ...
  for (;;)
	...
	goto error;
	  ...
	  || h->type != NT_MACRO || h->flags & NODE_POISONED)
	{
  ...
  if ((size_t) read (fd, undeftab, m.definition_length) !=
      m.definition_length)
    goto error;
  ...
  while (first < last && i < nl.n_defs)
      ...
      else
	{
	  if (CPP_OPTION (r, warn_invalid_pch))
	    cpp_error (r, CPP_DL_WARNING_SYSHDR,
		       "%s: not used because `%s' is defined", name, first);
	  goto fail;
	}
    }
		...
		struct save_macro_data *data)
{
  ...
  if (deps_restore
      (r->deps, f, CPP_OPTION (r, restore_pch_deps) ? name : NULL) != 0)
    goto error;
