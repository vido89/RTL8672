...
static
GTY ((length ("fde_table_allocated")))
...
static
GTY ((param_is (struct indirect_string_node)))
...
/* Support for complex CFA locations.  */
     ...
     static struct dw_loc_descr_struct *build_cfa_loc (dw_cfa_location *);
...
static
GTY (())
     struct queued_reg_save *queued_reg_saves;
...
output_cfi (dw_cfi_ref cfi, dw_fde_ref fde, int for_eh)
{
  ...
  if (cfi->dw_cfi_opc == DW_CFA_advance_loc)
	...
	case DW_CFA_set_loc:
	  if (for_eh)
	    dw2_asm_output_encoded_addr_rtx (ASM_PREFERRED_EH_DATA_FORMAT
					     ( /*code= */ 1, /*global= */ 0),
	...
	case DW_CFA_def_cfa:
	  ...
	  dw2_asm_output_data_uleb128 (cfi->dw_cfi_oprnd2.dw_cfi_offset,
				       NULL);
	  break;
	...
	case DW_CFA_def_cfa_sf:
	  r =
	    DWARF2_FRAME_REG_OUT (cfi->dw_cfi_oprnd1.dw_cfi_reg_num, for_eh);
	  dw2_asm_output_data_uleb128 (r, NULL);
	  dw2_asm_output_data_sleb128 (cfi->dw_cfi_oprnd2.dw_cfi_offset,
				       NULL);
	  break;
	...
	case DW_CFA_def_cfa_offset:
	case DW_CFA_GNU_args_size:
	  dw2_asm_output_data_uleb128 (cfi->dw_cfi_oprnd1.dw_cfi_offset,
				       NULL);
	  break;

	case DW_CFA_def_cfa_offset_sf:
	  dw2_asm_output_data_sleb128 (cfi->dw_cfi_oprnd1.dw_cfi_offset,
				       NULL);
	  break;
...
output_call_frame_info (int for_eh)
{
  ...
  if (for_eh)
    {
      bool any_eh_needed = !flag_exceptions
  ...
  ASM_GENERATE_INTERNAL_LABEL (section_start_label, FRAME_BEGIN_LABEL,
			       for_eh);
  ...
  dw2_asm_output_data ((for_eh ? 4 : DWARF_OFFSET_SIZE),
		       (for_eh ? 0 : DW_CIE_ID), "CIE Identifier Tag");
  ...
  if (for_eh)
      ...
      if (eh_personality_libfunc && per_encoding == DW_EH_PE_aligned)
	{
	  int offset = (4	/* Length */
			...
			+ size_of_sleb128 (DWARF_CIE_DATA_ALIGNMENT) + 1	/* RA column */
			+ 1	/* Augmentation size */
			  ...
			  const char *file ATTRIBUTE_UNUSED)
{
  ...
  /* ??? current_function_func_begin_label is also used by except.c
     ...
     be used.  */
  if ((!flag_exceptions || USING_SJLJ_EXCEPTIONS) && !dwarf2out_do_frame ())
    return;
#else
...
static void
get_cfa_from_loc_descr (dw_cfa_location * cfa,
...
/* Filenames referenced by this compilation unit.  */
     static GTY (())
     varray_type file_table;
     static GTY (())
     varray_type file_table_emitted;
     static GTY (())
     size_t file_table_last_lookup_index;
...
/* Number of elements currently allocated for the decl_die_table.  */
     static GTY (())
     unsigned decl_die_table_allocated;
...
/* Number of elements currently allocated for separate_line_info_table.  */
     static GTY (())
     unsigned separate_line_info_table_allocated;

/* Number of elements in separate_line_info_table currently in use.  */
     static GTY (())
     unsigned separate_line_info_table_in_use;
...
/* A pointer to the base of a table that contains a list of publicly
   accessible names.  */
     static GTY ((length ("pubname_table_allocated")))
...
/* Array of dies for which we should generate .debug_arange info.  */
     static GTY ((length ("arange_table_allocated")))
     dw_die_ref *arange_table;

/* Number of elements currently allocated for arange_table.  */
     static GTY (())
     unsigned arange_table_allocated;

/* Number of elements in arange_table currently in use.  */
     static GTY (())
     unsigned arange_table_in_use;
...
/* Array of dies for which we should generate .debug_ranges info.  */
     static GTY ((length ("ranges_table_allocated")))
...
/* Forward declarations for functions defined in this file.  */
     ...
     static void add_AT_unsigned (dw_die_ref, enum dwarf_attribute,
				  unsigned HOST_WIDE_INT);
     static inline unsigned HOST_WIDE_INT AT_unsigned (dw_attr_ref);
     static void add_AT_long_long (dw_die_ref, enum dwarf_attribute,
				   unsigned long, unsigned long);
     static inline void add_AT_vec (dw_die_ref, enum dwarf_attribute,
				    unsigned int, unsigned int,
				    unsigned char *);
     static hashval_t debug_str_do_hash (const void *);
     static int debug_str_eq (const void *, const void *);
     static void add_AT_string (dw_die_ref, enum dwarf_attribute,
				const char *);
     static inline const char *AT_string (dw_attr_ref);
     static int AT_string_form (dw_attr_ref);
     static void add_AT_die_ref (dw_die_ref, enum dwarf_attribute,
				 dw_die_ref);
     ...
     static void add_AT_lbl_id (dw_die_ref, enum dwarf_attribute,
				const char *);
     static void add_AT_lbl_offset (dw_die_ref, enum dwarf_attribute,
				    const char *);
     ...
     static dw_loc_list_ref new_loc_list (dw_loc_descr_ref, const char *,
					  ...
					  unsigned);
     static void add_loc_descr_to_loc_list (dw_loc_list_ref *,
					    dw_loc_descr_ref, const char *,
					    const char *, const char *);
...
static inline void
add_AT_flag (dw_die_ref die, enum dwarf_attribute attr_kind,
	     unsigned int flag)
{
...
static inline void
add_AT_int (dw_die_ref die, enum dwarf_attribute attr_kind,
...
static inline void
add_AT_string (dw_die_ref die, enum dwarf_attribute attr_kind,
	       const char *str)
{
  dw_attr_ref attr = ggc_alloc (sizeof (dw_attr_node));
...
static inline void
add_AT_fde_ref (dw_die_ref die, enum dwarf_attribute attr_kind,
...
static inline void
add_AT_loc (dw_die_ref die, enum dwarf_attribute attr_kind,
	    dw_loc_descr_ref loc)
{
  dw_attr_ref attr = ggc_alloc (sizeof (dw_attr_node));
...
static inline void
add_AT_loc_list (dw_die_ref die, enum dwarf_attribute attr_kind,
...
static inline void
add_AT_lbl_offset (dw_die_ref die, enum dwarf_attribute attr_kind,
		   const char *label)
{
  dw_attr_ref attr = ggc_alloc (sizeof (dw_attr_node));
...
equate_decl_number_to_die (tree decl, dw_die_ref decl_die)
{
  ...
  if (decl_id >= decl_die_table_allocated)
	...
	= ((decl_id + 1 + DECL_DIE_TABLE_INCREMENT - 1)
	   / DECL_DIE_TABLE_INCREMENT) * DECL_DIE_TABLE_INCREMENT;
      ...
      memset (&decl_die_table[decl_die_table_allocated], 0,
	      (num_allocated -
	       decl_die_table_allocated) * sizeof (dw_die_ref));
  ...
  switch (v1->val_class)
    ...
    case dw_val_class_loc:
      for (loc1 = v1->v.val_loc, loc2 = v2->v.val_loc;
	   loc1 && loc2; loc1 = loc1->dw_loc_next, loc2 = loc2->dw_loc_next)
...
same_die_p (dw_die_ref die1, dw_die_ref die2, int *mark)
{
  ...
  for (a1 = die1->die_attr, a2 = die2->die_attr;
       a1 && a2; a1 = a1->dw_attr_next, a2 = a2->dw_attr_next)
  ...
  for (c1 = die1->die_child, c2 = die2->die_child;
       c1 && c2; c1 = c1->die_sib, c2 = c2->die_sib)
...
static void
record_comdat_symbol_number (dw_die_ref cu, htab_t htable,
  ...
  if (abbrev_id >= abbrev_die_table_in_use)
      ...
      if (abbrev_die_table_in_use >= abbrev_die_table_allocated)
	{
	  ...
	  memset (&abbrev_die_table[abbrev_die_table_allocated], 0,
		  (n_alloc -
		   abbrev_die_table_allocated) * sizeof (dw_die_ref));
...
size_of_die (dw_die_ref die)
{
  ...
  for (a = die->die_attr; a != NULL; a = a->dw_attr_next)
	...
	case dw_val_class_long_long:
	  ...
	  break;
	case dw_val_class_vec:
	  size += 1 + (a->dw_attr_val.v.val_vec.length * a->dw_attr_val.v.val_vec.elt_size);	/* block */
	  break;
	case dw_val_class_flag:
	  size += 1;
	  break;
...
static inline void
add_loc_descr_to_loc_list (dw_loc_list_ref * list_head,
			   dw_loc_descr_ref descr, const char *begin,
			   const char *end, const char *section)
{
  dw_loc_list_ref *d;
...
output_file_names (void)
{
  ...
  files =
    alloca (VARRAY_ACTIVE_SIZE (file_table) * sizeof (struct file_info));
...
output_line_info (void)
{
  ...
  dw2_asm_output_data (1, DWARF_LINE_OPCODE_BASE, "Special Opcode Base");
...
is_subrange_type (tree type)
{
  ...
  if (TREE_CODE (type) == INTEGER_TYPE && subtype != NULL_TREE)
    {
...
subrange_type_die (tree type, dw_die_ref context_die)
{
  ...
  if (TYPE_MIN_VALUE (type) != NULL)
    add_bound_info (subrange_die, DW_AT_lower_bound, TYPE_MIN_VALUE (type));
  if (TYPE_MAX_VALUE (type) != NULL)
    add_bound_info (subrange_die, DW_AT_upper_bound, TYPE_MAX_VALUE (type));
		   ...
		   dw_die_ref context_die)
{
  ...
  if (item_type)
    ...
    sub_die = modified_type_die (item_type,
				 ...
				 TYPE_VOLATILE (item_type), context_die);
  ...
  switch (TREE_CODE (loc))
    ...
    case ADDR_EXPR:
      ...
      for (loc = TREE_OPERAND (loc, 0);
	   ...
	   || TREE_CODE (loc) == SAVE_EXPR; loc = TREE_OPERAND (loc, 0))
	;
      ...
      if (rtl == NULL_RTX || is_pseudo_reg (rtl))
	{
      ...
      /* ??? This is the same series of checks that are made in dbxout.c before
         ...
         thing to do.  */
	       ...
	       && XEXP (rtl, 0) != const0_rtx && !CONSTANT_P (XEXP (rtl, 0))
...
tree_add_const_value_attribute (dw_die_ref var_die, tree decl)
{
  ...
  switch (TREE_CODE (type))
	...
	add_AT_unsigned (var_die, DW_AT_const_value, tree_low_cst (init, 0));
      else
	add_AT_long_long (var_die, DW_AT_const_value,
			  TREE_INT_CST_HIGH (init), TREE_INT_CST_LOW (init));
      break;
  ...
  switch (TREE_CODE (tree_node))
    ...
    case FIELD_DECL:
      /* For a data member of a struct or union, the DW_AT_byte_size is
         ...
         even for bit-fields.  */
      size =
	simple_type_size_in_bits (field_type (tree_node)) / BITS_PER_UNIT;
      break;
    default:
      abort ();
    }
...
{
  dw_die_ref parm_die = new_die (DW_TAG_formal_parameter, context_die, node);
  tree origin;
...
gen_formal_types_die (tree function_or_method_type, dw_die_ref context_die)
{
  ...
  for (link = first_parm_type; link;)
	...
	break;
      ...
      if ((TREE_CODE (function_or_method_type) == METHOD_TYPE
	   && link == first_parm_type) || (arg && DECL_ARTIFICIAL (arg)))
	add_AT_flag (parm_die, DW_AT_artificial, 1);
  ...
  for (link = TYPE_ARG_TYPES (function_or_method_type);
       link && TREE_VALUE (link); link = TREE_CHAIN (link))
...
gen_subprogram_die (tree decl, dw_die_ref context_die)
{
  ...
  if (declaration)
	...
	add_AT_location_description (subr_die, DW_AT_static_link,
				     loc_descriptor (lookup_static_chain
						     (decl)));
#endif
    }
...
gen_subroutine_type_die (tree type, dw_die_ref context_die)
{
  ...
  dw_die_ref subr_die = new_die (DW_TAG_subroutine_type,
				 scope_die_for (type, context_die), type);
...
decls_for_scope (tree stmt, dw_die_ref context_die, int depth)
{
  ...
  for (subblocks = BLOCK_SUBBLOCKS (stmt);
       subblocks != NULL; subblocks = BLOCK_CHAIN (subblocks))
...
gen_decl_die (tree decl, dw_die_ref context_die)
{
  ...
  switch (TREE_CODE (decl))
	...
	break;
      ...
      if (TYPE_DECL_IS_STUB (decl)
	  && decl_ultimate_origin (decl) != NULL_TREE)
	{
...
void
dwarf2out_add_library_unit_info (const char *filename,
...
static void
dwarf2out_end_block (unsigned int line ATTRIBUTE_UNUSED,
		     unsigned int blocknum)
{
  function_section (current_function_decl);
...
dwarf2out_finish (const char *filename)
{
  ...
  if (debug_info_level >= DINFO_LEVEL_VERBOSE)
    add_AT_lbl_offset (comp_unit_die, DW_AT_macro_info,
