...
static const char z_std_preamble[] = "/*  DO NOT EDIT THIS FILE.\n\n\
     ...
     int main (int argc, char **argv)
{
  ...
  for (;;)
	...
	file_name_buf = pz_end + 1;
      ...
      while ((pz_end > pz_curr_file) && ISSPACE (pz_end[-1]))
	pz_end--;
...
#undef _ENV_
    ...
    switch (*pz_verbose)
      ...
      case 's':
      case 'S':
	verbose_level = VERB_SILENT;
	break;

      case 'f':
      case 'F':
	verbose_level = VERB_FIXES;
	break;

      case 'a':
      case 'A':
	verbose_level = VERB_APPLIES;
	break;

      default:
      case 'p':
      case 'P':
	verbose_level = VERB_PROGRESS;
	break;

      case 't':
      case 'T':
	verbose_level = VERB_TESTS;
	break;
...
create_file (void)
{
  ...
  fprintf (pf, z_std_preamble,
	   (pz_machine == NULL)
	   ? "fixinc/tests/inc" : pz_input_dir, pz_curr_file);
...
quoted_file_exists (const char *pz_src_path,
		    const char *pz_file_path, const char *pz_file)
{
  char z[MAXPATHLEN];
  char *pz;
...
extract_quoted_files (char *pz_data,
		      const char *pz_fixed_file, regmatch_t * p_re_match)
{
...
fix_with_system (tFixDesc * p_fixd,
		 tCC * pz_fix_file, tCC * pz_file_source, tCC * pz_temp_file)
{
  char *pz_cmd;
  char *pz_scan;
  size_t argsize;
  ...
  if (p_fixd->fd_flags & FD_SUBROUTINE)
	...
	+ strlen (pz_fix_file)
	+ strlen (pz_file_source) + strlen (pz_temp_file);
