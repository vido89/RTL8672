...
FIX_PROC_HEAD (char_macro_use_fix)
{
  ...
  for (p = text; xregexec (&re, p, 1, rm, 0) == 0; p = limit + 1)
    {
    ...
    found:
      ...
      while ((ISSPACE (*p) || ISUPPER (*p)) && p < limit)
	p++;
      if (*p++ != '(')
	continue;
      if (!ISALPHA (*p))
...
FIX_PROC_HEAD (machine_name_fix)
{
  ...
  for (base = text; xregexec (label_re, base, 2, match, 0) == 0; base = limit)
    {
      base += match[0].rm_eo;
