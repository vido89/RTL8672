...
Average leaf depth\t\t%.1f\n", SCALE (G.objects * offsetof (struct ggc_mem, u)), LABEL (G.objects * offsetof (struct ggc_mem, u)), (unsigned long) nleaf, (double) sumdepth / (double) nleaf);

  /* Report overall memory usage.  */
  fprintf (stderr, "\n\
Total objects allocated\t\t%ld\n\
Total memory in GC arena\t%ld%c\n", (unsigned long) G.objects, SCALE (G.allocated), LABEL (G.allocated));
}

struct ggc_pch_data *
init_ggc_pch (void)
