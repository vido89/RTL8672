...
record_effective_endpoints (void)
{
  ...
  if (cfg_layout_function_footer)
    cfg_layout_function_footer =
      unlink_insn_chain (cfg_layout_function_footer, get_last_insn ());
}

...
static
GTY (())
     varray_type block_locators_locs;
     static GTY (())
     varray_type block_locators_blocks;
     static GTY (())
     varray_type line_locators_locs;
     static GTY (())
     varray_type line_locators_lines;
     static GTY (())
     varray_type file_locators_locs;
     static GTY (())
     varray_type file_locators_files;
  ...
  for (insn = get_insns (); insn; insn = next)
    {
	  ...
	  || !NEXT_INSN (insn) || (!prologue_locator && file_name))
	{
	  if (last_block != block)
	    {
	      loc++;
	  ...
	  if (any_condjump_p (bb_end_insn))
	    {
		  ...
		  || (!bb->rbi->next && e_fall->dest == EXIT_BLOCK_PTR))
		continue;
  ...
  if (rtl_dump_file)
    {
      ...
      for (bb = ENTRY_BLOCK_PTR->next_bb, index = 0; bb;
...
verify_insn_chain (void)
{
  ...
  for (prevx = NULL, insn_cnt1 = 1, x = get_insns ();
       x != 0; prevx = x, insn_cnt1++, x = NEXT_INSN (x))
  ...
  for (nextx = NULL, insn_cnt2 = 1, x = get_last_insn ();
       x != 0; nextx = x, insn_cnt2++, x = PREV_INSN (x))
...
cfg_layout_finalize (void)
{
  ...
  FOR_BB_BETWEEN (bb, ENTRY_BLOCK_PTR, NULL, next_bb) bb->rbi = NULL;
...
can_copy_bbs_p (basic_block * bbs, unsigned n)
{
  ...
  for (i = 0; i < n; i++)
	...
	if ((e->flags & EDGE_ABNORMAL) && e->dest->rbi->duplicated)
	  {
	    ret = false;
	    goto end;
	  }
...
copy_bbs (basic_block * bbs, unsigned n, basic_block * new_bbs,
	  edge * edges, unsigned n_edges, edge * new_edges, struct loop *base)
{
  unsigned i, j;
