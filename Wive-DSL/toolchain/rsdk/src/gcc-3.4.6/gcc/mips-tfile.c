...
STATIC symint_t add_string (varray_t *, shash_t **, const char *,
			    const char *, shash_t **);
	  ...
	  /* Functions push two or more aux words as follows:
	  ...
	  else
	    {
	      ...
	      type = add_aux_sym_tir (&last_func_type_info,
				      hash_no, &cur_file_ptr->thash_head[0]);
	      if (last_func_eptr)
		{
  ...
  if (debug > 1)
    {
      ...
      fprintf (stderr,
	       "\tesym\tv= %10ld, ifd= %2d, sc= %-12s", value, ifd, sc_str);
  ...
  *psym = *esym;
  ...
  psym->asym.iss = (str_start == (const char *) 0)
    ...
    : add_string (&ext_strings,
		  &ext_str_hash[0], str_start, str_end_p1, &hash_ptr);
...
add_aux_sym_tir (type_info_t * t, hash_state_t state, thash_t ** hash_tbl)
{
  ...
  if (state != hash_no
	  ...
	  || t->basic_type == bt_Enum || t->bitfield || t->num_dims > 0))
    state = hash_no;
  ...
  if (state != hash_no)
    {
      ...
      for (hash_ptr = hash_tbl[hi];
	   hash_ptr != (thash_t *) 0; hash_ptr = hash_ptr->next)
	{
  ...
  *aux_ptr = aux;
  ...
  if (t->basic_type == bt_Struct
      || t->basic_type == bt_Union || t->basic_type == bt_Enum)
    {
  ...
  for (i = 0; i < t->num_dims; i++)
    {
      (void) add_aux_sym_rndx (ST_RFDESCAPE, cur_file_ptr->int_type);
	 ...
	 bt_t basic_type)	/* bt_Struct, bt_Union, or bt_Enum */
{
  ...
  hash_ptr = hash_string (tag_start,
			  ...
			  &tag_hash[0], (symint_t *) 0);

  if (hash_ptr != (shash_t *) 0 && hash_ptr->tag_ptr != (tag_t *) 0)
    {
  ...
  (void) add_string (&tag_strings,
		     &tag_hash[0], tag_start, tag_end_p1, &hash_ptr);
...
add_unknown_tag (tag_t * ptag)
{
  ...
  sym_index = add_local_symbol (name_start,
				name_end_p1,
				st_Block,
				sc_Info, (symint_t) 0, (symint_t) 0);

  (void) add_local_symbol (name_start,
			   name_end_p1,
			   st_End, sc_Info, (symint_t) 0, (symint_t) 0);
	       ...
	       const char *func_end_p1)	/* 1st byte after func name */
{
  ...
  cur_proc_ptr = new_proc_ptr =
    &vp->last->datum->proc[vp->objects_last_page++];
  ...
  (void) add_local_symbol (func_start, func_end_p1,
			   proc_type, sc_Text, value, (symint_t) 0);
}

	  ...
	  const char *file_end_p1)	/* first byte after string */
{
  ...
  for (file_ptr = first_file;
       file_ptr != (efdr_t *) 0; file_ptr = file_ptr->next_file)
    {
	...
	= &file_desc.last->datum->file[file_desc.objects_last_page++];
      *file_ptr = init_file;
      ...
      add_string (&file_ptr->strings,
		  ...
		  &zero_bytes[0], &zero_bytes[0], (shash_t **) 0);
...
parse_begin (const char *start)
{
  ...
  hash_ptr = hash_string (start,
			  end_p1 - start, &orig_str_hash[0], (symint_t *) 0);
  ...
  (void) add_local_symbol ((const char *) 0, (const char *) 0,
			   ...
			   cur_oproc_begin->value, (symint_t) 0);
}

...
parse_bend (const char *start)
{
  ...
  if (hash_ptr == (shash_t *) 0)
    {
      error ("label %.*s not found for #.bend", (int) (end_p1 - start),
	     start);
      return;
    }
...
parse_def (const char *name_start)
{
  ...
  for (name_end_p1 = name_start; (ch = *name_end_p1) != ';' && ch != '\0';
       name_end_p1++)
    ;
  ...
  for (;;)
      ...
      if (ISDIGIT (ch) || ch == '-' || ch == '+')
	{
	  ...
	  if (arg_end_p1 != arg_start || ((ch2 = *arg_end_p1) != ';')
	      || ch2 != ',')
	    arg_was_number++;
	}
		  ...
		  if (ISDIGIT (ch) || ch == '-' || ch == '+')
		    {
		      int ch2;
		      arg_number =
			strtol (arg_start, (char **) &arg_end_p1, 0);
		      if (arg_end_p1 != arg_start
			  || ((ch2 = *arg_end_p1) != ';') || ch2 != ',')
			arg_was_number++;
	...
	case 's':
	  if (len == sizeof (".scl") - 1
	      ...
	      && arg_was_number && arg_number < ((symint_t) C_MAX))
	    {
		  ...
		  if ((orig_hash_ptr == (shash_t *) 0
		      ...
		      && eptr == (EXTR *) 0)
		    {
		      fprintf (stderr,
			       ...
			       (int) (arg_end_p1 - arg_start), arg_start);
		      value = 0;
		    }
		  else
		    {
		      SYMR *ptr = (orig_hash_ptr != (shash_t *) 0
				   && orig_hash_ptr->sym_ptr != (SYMR *) 0)
			? orig_hash_ptr->sym_ptr : &eptr->asym;
  ...
  if (symbol_type == st_Block || symbol_type == st_End)
	  ...
	  if (tag_start == (char *) 0)
	    {
	      error ("no tag specified for %.*s",
		     (int) (name_end_p1 - name_start), name_start);
	      return;
	    }
      ...
      if (is_function)
	{
      ...
      indx = add_aux_sym_tir (&t, hash_yes, &cur_file_ptr->thash_head[0]);
    }
  ...
  if (eptr == (EXTR *) 0
      || eptr->asym.st == (int) st_Nil || cur_proc_ptr != (PDR *) 0)
    {
...
bomb_out:
  if (error_line)
    error ("compiler error, badly formed #.def (internal line # = %d)",
	   error_line);
  else
...
parse_end (const char *start)
{
  ...
  for (start_func = start; ISSPACE ((unsigned char) *start_func);
       start_func++)
    ;
  ...
  (void) add_local_symbol (start_func, end_func_p1,
			   st_End, sc_Text, value, (symint_t) 0);
		    ...
		    const char *rest)	/* rest of the directive.  */
{
  ...
  if (code == (int) N_SLINE)
	...
	{
	  error
	    ("line number (%lu) for .stabs/.stabn directive cannot fit in index field (20 bits)",
	     code);
      ...
      if (ISDIGIT (ch) || ch == '-')
	{
	  ...
	  if (*p != '\n')
	    {
	      error
		("invalid .stabs/.stabn directive, stuff after numeric value");
	      return;
	    }
	}
      ...
      else
	{
	  ...
	  if (ch != '\n')
		...
		{
		  error
		    ("invalid .stabs/.stabn directive, badly formed value");
		  return;
		}
	      if (ch == '+')
...
parse_input (void)
{
  ...
  for (ptag = ptag_head->first_tag; ptag != (tag_t *) 0; ptag = ptag_next)
    {
      if (ptag->forward_ref != (forward_t *) 0)
	add_unknown_tag (ptag);
...
update_headers (void)
{
  ...
  for (file_ptr = first_file;
      ...
      for (sym = sym_start; sym < sym_end_p1; sym++)
	{
      ...
      (void) add_local_symbol ((const char *) 0, (const char *) 0,
			       st_End, sc_Text, (symint_t) 0, (symint_t) 0);
	      ...
	      const char *str)	/* string to print out when tracing */
{
  ...
  for (ptr = vp->first; ptr != (vlinks_t *) 0; ptr = ptr->next)
	...
	fatal ("wrote %d bytes to %s, system returned %d",
	       num_write, object_name, sys_write);
...
write_object (void)
{
  ...
  sys_write = fwrite (&symbolic_header,
		      1, sizeof (symbolic_header), object_stream);
  ...
  if (sys_write < 0)
    ...
    fatal ("wrote %d bytes to %s, system returned %d",
	   (int) sizeof (symbolic_header), object_name, sys_write);
  ...
  if (symbolic_header.cbLine > 0)	/* line numbers */
    {
	  ...
	  && fseek (object_stream, symbolic_header.cbLineOffset,
		    SEEK_SET) != 0)
	pfatal_with_name (object_name);
	...
	fprintf (stderr, "\twrite\tvp = " HOST_PTR_PRINTF
      ...
      sys_write = fwrite (orig_linenum,
			  1, symbolic_header.cbLine, object_stream);
	...
	fatal ("wrote %ld bytes to %s, system returned %ld",
	       (long) symbolic_header.cbLine, object_name, sys_write);
  ...
  if (symbolic_header.ioptMax > 0)	/* optimization symbols */
    {
	  ...
	  && fseek (object_stream, symbolic_header.cbOptOffset,
		    SEEK_SET) != 0)
	pfatal_with_name (object_name);
	...
	fprintf (stderr, "\twrite\tvp = " HOST_PTR_PRINTF
      ...
      sys_write = fwrite (orig_opt_syms, 1, num_write, object_stream);
  ...
  if (symbolic_header.idnMax > 0)	/* dense numbers */
    write_varray (&dense_num, (off_t) symbolic_header.cbDnOffset,
		  "Dense numbers");
  ...
  if (symbolic_header.issExtMax > 0)	/* external strings */
    write_varray (&ext_strings, symbolic_header.cbSsExtOffset,
		  "External strings");
  ...
  if (symbolic_header.ifdMax > 0)	/* file tables */
	   ...
	   file_ptr != (efdr_t *) 0; file_ptr = file_ptr->next_file)
	{
	  ...
	  sys_write = fwrite (&file_ptr->fdr, 1, sizeof (FDR), object_stream);
	  ...
	  if (sys_write < 0)
	    ...
	    fatal ("wrote %d bytes to %s, system returned %d",
		   (int) sizeof (FDR), object_name, sys_write);
  ...
  if (symbolic_header.crfd > 0)	/* relative file descriptors */
    {
	  ...
	  && fseek (object_stream, symbolic_header.cbRfdOffset,
		    SEEK_SET) != 0)
	pfatal_with_name (object_name);
	...
	fprintf (stderr, "\twrite\tvp = " HOST_PTR_PRINTF
      ...
      sys_write = fwrite (orig_rfds, 1, num_write, object_stream);
  ...
  if (symbolic_header.issExtMax > 0)	/* external symbols */
    write_varray (&ext_symbols, (off_t) symbolic_header.cbExtOffset,
		  "External symbols");
      ...
      if (difference < 8)
	{
	  ...
	  if ((symint_t) sys_read != difference)
	    fatal ("wanted to read %lu bytes from %s, system returned %ld",
		   (unsigned long) size, obj_in_name, sys_read);
	}
...
copy_object (void)
{
  ...
  sys_read = fread (&orig_file_header,
		    1, sizeof (struct filehdr), obj_in_stream);
  ...
  if (sys_read < 0)
    ...
    fatal ("wanted to read %d bytes from %s, system returned %d",
	   (int) sizeof (struct filehdr), obj_in_name, sys_read);
  ...
  sys_read = fread (&orig_sym_hdr, 1, sizeof (orig_sym_hdr), obj_in_stream);
  ...
  if (orig_sym_hdr.isymMax > 0)	/* local symbols */
    orig_local_syms =
      (SYMR *) read_seek (orig_sym_hdr.isymMax * sizeof (SYMR),
			  orig_sym_hdr.cbSymOffset, "Local symbols");
  ...
  if (orig_sym_hdr.ifdMax > 0)	/* file tables */
    orig_files = (FDR *) read_seek (orig_sym_hdr.ifdMax * sizeof (FDR),
				    orig_sym_hdr.cbFdOffset, "File tables");
  ...
  if (max_file_offset != (unsigned long) stat_buf.st_size)
    fatal
      ("symbol table is not last (symbol table ends at %ld, .o ends at %ld",
       max_file_offset, (long) stat_buf.st_size);
  ...
  if (orig_sym_hdr.ifdMax > 1
      && orig_files->csym == 2 && orig_files->caux == 0)
    {
      char *filename =
	orig_local_strs + (orig_files->issBase + orig_files->rss);
  ...
  for (fd = delete_ifd; fd < orig_sym_hdr.ifdMax; fd++)
      ...
      for (sym = sym_start; sym < sym_end_p1; sym++)
	{
	  switch ((st_t) sym->st)
		...
		if (shash_ptr != (shash_t *) 0)
		  error
		    ("internal error, %s is already in original symbol table",
		     str);
      ...
      for (proc = proc_start; proc < proc_end_p1; proc++)
	{
	  ...
	  if (shash_ptr == (shash_t *) 0)
	    error
	      ("internal error, function %s is not in original symbol table",
	       str);
  ...
  for (remaining = orig_file_header.f_symptr;
       remaining > 0; remaining -= num_write)
    {
      num_write
	...
	fatal ("wanted to read %d bytes from %s, system returned %d",
	       num_write, obj_in_name, sys_read);
...
main (int argc, char **argv)
{
  ...
  while ((option =
	  getopt_long (argc, argv, "d:i:I:o:v", options, NULL)) != -1)
    switch (option)
      {
      default:
	had_errors++;
	...
	if (alloc_counts[i].total_alloc)
	  {
	    fprintf (stderr,
		     ...
		     alloc_counts[i].total_free, alloc_counts[i].total_pages);
	  }
    }
...
allocate_scope (void)
{
  ...
  if (ptr != (scope_t *) 0)
      ...
      if (unallocated == 0)
	{
	  ...
	  alloc_counts[(int) alloc_type_scope].cur_page = cur_page =
	    allocate_page ();
  ...
  if (unallocated == 0)
    {
      ...
      alloc_counts[(int) alloc_type_vlinks].cur_page = cur_page =
	allocate_page ();
  ...
  if (unallocated == 0)
    {
      ...
      alloc_counts[(int) alloc_type_shash].cur_page = cur_page =
	allocate_page ();
  ...
  if (unallocated == 0)
    {
      ...
      alloc_counts[(int) alloc_type_thash].cur_page = cur_page =
	allocate_page ();
...
allocate_tag (void)
{
  ...
  if (ptr != (tag_t *) 0)
      ...
      if (unallocated == 0)
	{
	  ...
	  alloc_counts[(int) alloc_type_tag].cur_page = cur_page =
	    allocate_page ();
...
allocate_forward (void)
{
  ...
  if (ptr != (forward_t *) 0)
      ...
      if (unallocated == 0)
	{
	  ...
	  alloc_counts[(int) alloc_type_forward].cur_page = cur_page =
	    allocate_page ();
...
allocate_thead (void)
{
  ...
  if (ptr != (thead_t *) 0)
      ...
      if (unallocated == 0)
	{
	  ...
	  alloc_counts[(int) alloc_type_thead].cur_page = cur_page =
	    allocate_page ();
...
#ifndef MALLOC_CHECK
  ptr->free =
    (thead_t *) alloc_counts[(int) alloc_type_thead].free_list.f_thead;
