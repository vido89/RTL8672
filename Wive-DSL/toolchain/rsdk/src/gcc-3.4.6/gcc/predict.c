...
can_predict_insn_p (rtx insn)
{
  return (GET_CODE (insn) == JUMP_INSN
	  && any_condjump_p (insn) && BLOCK_FOR_INSN (insn)->succ->succ_next);
}

/* Predict edge E by given predictor if possible.  */

void
predict_edge_def (edge e, enum br_predictor predictor, enum prediction taken)
{
  int probability = predictor_info[(int) predictor].hitrate;
  ...
  for (note = REG_NOTES (insn); note; note = XEXP (note, 1))
	...
	if (d == 0)
	  ...
	  combined_probability =
	    (((double) combined_probability) * probability *
...
estimate_probability (struct loops *loops_info)
{
  ...
  for (i = 1; i < loops_info->num; i++)
      ...
      if (simple_loop_p (loop, &desc) && desc.const_iter)
	{
	  ...
	  prob = (REG_BR_PROB_BASE - (REG_BR_PROB_BASE + niter / 2) / niter);
	  ...
	  predict_edge (desc.in_edge, PRED_LOOP_ITERATIONS, prob);
	}
      ...
      for (j = 0; j < loop->num_nodes; j++)
	{
	  ...
	  for (e = bb->succ; e; e = e->succ_next)
	    if (e->dest == loop->header && e->src == loop->latch)
	      {
		header_found = 1;
	  ...
	  if (!header_found)
		...
		predict_edge
		  (e, PRED_LOOP_EXIT,
		   (REG_BR_PROB_BASE
		    - predictor_info[(int) PRED_LOOP_EXIT].hitrate) / exits);
	}
	...
	if (e->dest != EXIT_BLOCK_PTR && e->dest != bb
	    ...
	    && !dominated_by_p (CDI_POST_DOMINATORS, e->src, e->dest))
	  {
	    ...
	    /* The call heuristic claims that a guarded function call
	       ...
	       messages.  */
	    for (insn = BB_HEAD (e->dest);
		 insn != NEXT_INSN (BB_END (e->dest));
		 insn = NEXT_INSN (insn))
  ...
  FOR_EACH_BB (bb)
	...
	&& any_condjump_p (BB_END (bb)) && bb->succ->succ_next != NULL)
...
note_prediction_to_br_prob (void)
{
  ...
  FOR_EACH_BB (bb) process_note_predictions (bb, heads);
...
counts_to_freqs (void)
{
  ...
  FOR_EACH_BB (bb) count_max = MAX (bb->count, count_max);
