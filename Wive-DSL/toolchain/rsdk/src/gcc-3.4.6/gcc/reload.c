...
static struct replacement replacements[MAX_RECOG_OPERANDS *
  ...
  for (class = 1; class < N_REG_CLASSES; class++)
	...
	continue;
      ...
      if ((reg_class_size[class] > best_size
	   && (best_cost < 0 || best_cost >= cost)) || best_cost > cost)
	{
	  best_class = class;
	     ...
	     int opnum, enum reload_type type)
{
  ...
  if (i == n_reloads)
	...
	rld[i].class = class;
      rld[i].optional &= optional;
      if (MERGE_TO_OTHER (type, rld[i].when_needed, opnum, rld[i].opnum))
...
combine_reloads (void)
{
  ...
  for (i = 0; i < n_reloads; i++)
	...
	&& rld[i].inc == 0 && rld[i].reg_rtx == 0
#ifdef SECONDARY_MEMORY_NEEDED
	/* Don't combine two reloads with different secondary
	   memory locations.  */
	&&
	(secondary_memlocs_elim[(int) rld[output_reload].outmode]
	 [rld[i].opnum] == 0
	 || secondary_memlocs_elim[(int) rld[output_reload].
				   outmode][rld[output_reload].opnum] == 0
	 ||
	 rtx_equal_p (secondary_memlocs_elim[(int) rld[output_reload].outmode]
		      [rld[i].opnum],
		      secondary_memlocs_elim[(int) rld[output_reload].
					     outmode][rld[output_reload].
						      opnum]))
#endif
	...
	if (secondary_memlocs_elim[(int) rld[output_reload].outmode]
	    [rld[output_reload].opnum] != 0)
	  secondary_memlocs_elim[(int) rld[output_reload].outmode][rld[i].
								   opnum] =
	    secondary_memlocs_elim[(int) rld[output_reload].
				   outmode][rld[output_reload].opnum];
#endif
  ...
  for (note = REG_NOTES (this_insn); note; note = XEXP (note, 1))
	...
	&& HARD_REGNO_MODE_OK (REGNO (XEXP (note, 0)),
			       rld[output_reload].outmode)
	&&
	...
	(HARD_REGNO_NREGS (REGNO (XEXP (note, 0)), rld[output_reload].outmode)
	 <= HARD_REGNO_NREGS (REGNO (XEXP (note, 0)),
	...
	&& ((secondary_out = rld[output_reload].secondary_out_reload) == -1
	    || (!(TEST_HARD_REG_BIT
		  (reg_class_contents[(int) rld[secondary_out].class],
		   REGNO (XEXP (note, 0))))
		&& ((secondary_out = rld[secondary_out].secondary_out_reload)
		    == -1
		    ||
		    !(TEST_HARD_REG_BIT
		      (reg_class_contents[(int) rld[secondary_out].class],
	...
	rld[output_reload].reg_rtx
	  = gen_rtx_REG (rld[output_reload].outmode, REGNO (XEXP (note, 0)));
	return;
      }
}

		   ...
		   enum reg_class class, int for_real, int earlyclobber)
{
  ...
  if (hard_regs_live_known
      ...
      && !fixed_regs[REGNO (in)] && HARD_REGNO_MODE_OK (REGNO (in),
							 ...
							 outmode)))
    {
      ...
      if (!refers_to_regno_for_reload_p
	  (regno, regno + nwords, out, (rtx *) 0)
...
operands_match_p (rtx x, rtx y)
{
  ...
  if ((code == REG || (code == SUBREG && GET_CODE (SUBREG_REG (x)) == REG))
	...
	j = REGNO (y);
	  ...
	  && SCALAR_INT_MODE_P (GET_MODE (x)) && i < FIRST_PSEUDO_REGISTER)
	i += HARD_REGNO_NREGS (i, GET_MODE (x)) - 1;
      if (WORDS_BIG_ENDIAN && GET_MODE_SIZE (GET_MODE (y)) > UNITS_PER_WORD
	  && SCALAR_INT_MODE_P (GET_MODE (y)) && j < FIRST_PSEUDO_REGISTER)
...
immune_p (rtx x, rtx y, struct decomposition ydata)
{
  ...
  if (ydata.reg_flag)
    return !refers_to_regno_for_reload_p (ydata.start, ydata.end, x,
					  (rtx *) 0);
  if (ydata.safe)
    return 1;
	      ...
	      short *reload_reg_p)
{
  ...
  enum reload_usage
  { RELOAD_READ, RELOAD_READ_WRITE,
      RELOAD_WRITE } modified[MAX_RECOG_OPERANDS];
  ...
  if (secondary_memlocs_elim_used)
    {
      memset (secondary_memlocs_elim, 0,
	      sizeof (secondary_memlocs_elim[0]) *
  ...
  for (i = 0; i < noperands; i++)
      ...
      else if (code == SUBREG)
	{
	  ...
	  if (replace
	    ...
	    set_unique_reg_note (emit_insn_before
				 (gen_rtx_USE (VOIDmode, reg), insn),
				 REG_EQUAL,
	...
	substed_operand[i] = recog_data.operand[i]
	  = find_reloads_toplev (recog_data.operand[i], i, address_type[i],
				 ind_levels, 0, insn, &address_reloaded[i]);
      else if (code == REG)
	{
	...
	= ((code == REG && REGNO (recog_data.operand[i])
	   ...
	   ? reg_preferred_class (REGNO (recog_data.operand[i])) : NO_REGS);
      pref_or_nothing[i]
	= (code == REG
  ...
  if (set != 0 && noperands >= 2 && recog_data.operand[0] == SET_DEST (set)
	...
	=
	reg_class_subunion[(int) preferred_class[0]][(int)
						     preferred_class[1]];
...
try_swapped:
  ...
  for (this_alternative_number = 0;
       this_alternative_number < n_alternatives; this_alternative_number++)
    {
		  ...
		  && REGNO (SUBREG_REG (operand)) < FIRST_PSEUDO_REGISTER)
		{
		  if (!subreg_offset_representable_p
		      (REGNO (SUBREG_REG (operand)),
		       ...
		       SUBREG_BYTE (operand), GET_MODE (operand)))
		    force_reload = 1;
	      ...
	      if (CONSTANT_P (operand) || GET_CODE (operand) == PLUS
	  ...
	  do
		...
		if (GET_CODE (operand) == MEM
		    ...
		    && !(GET_CODE (XEXP (operand, 0)) == REG
			 && (ind_levels == 0
			     || reg_equiv_address[REGNO (XEXP (operand, 0))]
			     != 0)))
		  win = 1;
		break;
		...
		if ((GET_CODE (operand) == MEM
			...
			&& ((reg_equiv_mem[REGNO (operand)] != 0
			     &&
			     offsettable_memref_p (reg_equiv_mem
						   [REGNO (operand)]))
...
finish:
  ...
  for (i = 0; i < noperands; i++)
    if (!goal_alternative_win[i] && CONSTANT_P (recog_data.operand[i])
	/* force_const_mem does not accept HIGH.  */
	&& GET_CODE (recog_data.operand[i]) != HIGH
	&& ((PREFERRED_RELOAD_CLASS (recog_data.operand[i],
				     (enum reg_class) goal_alternative[i])
	     == NO_REGS) || no_input_reloads) && operand_mode[i] != VOIDmode)
      {
	substed_operand[i] = recog_data.operand[i]
  ...
  for (i = 0; i < noperands; i++)
	    ...
	    operand_reloadnum[i]
	      = push_reload ((modified[i] != RELOAD_WRITE
			     ...
			     (insn_code_number < 0 ? 0
			      : insn_data[insn_code_number].operand[i].
			      strict_low), 0, i, operand_type[i]);
	  }
	/* In a matching pair of operands, one must be input only
		 ...
		 && modified[goal_alternative_matched[i]] == RELOAD_WRITE)
	  {
	    operand_reloadnum[i]
	      = push_reload (recog_data.operand[i],
			     ...
			     recog_data.operand_loc[i],
			     recog_data.
			     operand_loc[goal_alternative_matched[i]],
			     (enum reg_class) goal_alternative[i],
			     operand_mode[i],
			     operand_mode[goal_alternative_matched[i]], 0, 0,
			     i, RELOAD_OTHER);
	...
	else
	  {
	    ...
	    return 0;
	     ...
	     && !address_operand_reloaded[i] && optimize)
      {
	...
	if ((GET_CODE (operand) == MEM
		...
		|| modified[i] == RELOAD_WRITE) && !no_input_reloads
	...
	/* If a memory reference remains (either as a MEM or a pseudo that
			 ...
			 && reg_renumber[REGNO (operand)] < 0)))
	  {
		...
		if (modified[i] != RELOAD_READ)
	     ...
	     && !no_input_reloads && !no_output_reloads && optimize)
      {
	...
	if ((GET_CODE (operand) == MEM
		...
		!= NO_REGS))
	  operand_reloadnum[i] =
	    operand_reloadnum[goal_alternative_matches[i]] =
  ...
  if (insn_code_number >= 0 && replace)
	...
	*recog_data.dup_loc[i] = *recog_data.operand_loc[opno];
	dup_replacements (recog_data.dup_loc[i],
  ...
  for (i = 0; i < n_reloads; i++)
    if (rld[i].reg_rtx == 0
	&& rld[i].in != 0 && GET_CODE (rld[i].in) == REG && rld[i].out == 0)
      {
	rld[i].reg_rtx
  ...
  for (i = 0; i < n_reloads; i++)
	...
	if ((rld[i].when_needed == RELOAD_FOR_INPUT_ADDRESS
		...
		|| (goal_alternative_matches[rld[j].opnum] == rld[i].opnum)))
	  {
  ...
  for (i = 0; i < n_reloads; i++)
	      ...
	      || rld[operand_reloadnum[rld[i].opnum]].optional))
	{
	  ...
	  if ((rld[i].when_needed == RELOAD_FOR_OUTPUT_ADDRESS
		...
		rld[rld[secondary_out_reload].secondary_out_reload].
	...
	if (regno < FIRST_PSEUDO_REGISTER
	    ...
	    && HARD_REGNO_MODE_OK (regno, rld[i].mode))
	  {
	    ...
	    for (nri = 1; nri < nr; nri++)
		  ...
		  (reg_class_contents[rld[i].class], regno + nri))
		ok = 0;
			...
			int ind_levels, rtx insn)
{
  ...
  switch (code)
	...
	if (context)
	  {
	    find_reloads_address (GET_MODE (x), loc, XEXP (x, 0),
	...
	if (code0 == MULT || code0 == SIGN_EXTEND || code0 == TRUNCATE
	...
	else if (code0 == REG && code1 == REG)
	  {
		...
		&& REG_MODE_OK_FOR_BASE_P (op1, mode))
	      ...
	      {
		find_reloads_address_1 (mode, orig_op0, 1, &XEXP (x, 0),
					opnum, type, ind_levels, insn);
		find_reloads_address_1 (mode, orig_op1, 0, &XEXP (x, 1),
					opnum, type, ind_levels, insn);
	      }
	  }
      ...
      if (r >= FIRST_PSEUDO_REGISTER)
	{
      ...
      return (endregno > r
	      && regno < r + (r < FIRST_PSEUDO_REGISTER
			      ? HARD_REGNO_NREGS (r, GET_MODE (x)) : 1));
...
reg_overlap_mentioned_for_reload_p (rtx x, rtx in)
{
  ...
  /* Overly conservative.  */
  if (GET_CODE (x) == STRICT_LOW_PART || GET_RTX_CLASS (GET_CODE (x)) == 'a')
    x = XEXP (x, 0);
	      ...
	      || INSN_UID (p) < reload_first_uid))
	{
	  ...
	  if (pat != 0
		  ...
		  || (goal_const && rtx_equal_p (SET_SRC (pat), goal)
		      ...
		      && !reg_overlap_mentioned_for_reload_p (SET_DEST (pat),
							      goal)
		      && (valueno =
  ...
  if (code == MEM)
    {
      ...
      if ((GET_CODE (addr) == PRE_DEC
	   ...
	   || GET_CODE (addr) == POST_INC) && XEXP (addr, 0) == inced)
  ...
  for (r = 0; r < n_reloads; r++)
    {
      ...
      if (rld[r].in != 0)
	{
	  fprintf (f, "reload_in (%s) = ", GET_MODE_NAME (rld[r].inmode));
      ...
      if (rld[r].out != 0)
	{
	  fprintf (f, "reload_out (%s) = ", GET_MODE_NAME (rld[r].outmode));
