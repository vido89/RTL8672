...
static void insn_values_to_profile (rtx, unsigned *,
				    struct histogram_value **);
  ...
  switch (GET_CODE (set_src))
      ...
      if (GET_CODE (set_src) == UMOD && !side_effects_p (op1))
	{
	  ...
	  *values = xrealloc (*values,
	  ...
	  start_sequence ();
	  tmp =
	    simplify_gen_binary (DIV, mode, copy_rtx (op1), copy_rtx (op2));
			...
			rtx target, rtx op1, rtx op2, gcov_type value)
{
  ...
  do_compare_rtx_and_jump (tmp, GEN_INT (value), NE, 0, mode, NULL_RTX,
			   NULL_RTX, neq_label);
  tmp1 =
    simplify_gen_binary (operation, mode, copy_rtx (op1), GEN_INT (value));
  ...
  tmp1 =
    simplify_gen_binary (operation, mode, copy_rtx (op1), copy_rtx (tmp));
  tmp1 = force_operand (tmp1, target);
  if (tmp1 != target)
    emit_move_insn (copy_rtx (target), copy_rtx (tmp1));
...
divmod_fixed_value_transform (rtx insn)
{
  ...
  for (histogram = REG_NOTES (insn);
       histogram; histogram = XEXP (histogram, 1))
  ...
  insert_insn_on_edge (gen_divmod_fixed_value
		       (mode, code, set_dest, op1, op2, val), e);
	      ...
	      rtx op1, rtx op2)
{
  ...
  tmp1 = expand_simple_binop (mode, PLUS, tmp, constm1_rtx, NULL_RTX,
			      0, OPTAB_WIDEN);
  tmp2 = expand_simple_binop (mode, AND, tmp, tmp1, NULL_RTX, 0, OPTAB_WIDEN);
  do_compare_rtx_and_jump (tmp2, const0_rtx, NE, 0, mode, NULL_RTX,
			   NULL_RTX, neq_label);
  tmp3 = expand_simple_binop (mode, AND, op1, tmp1, target, 0, OPTAB_WIDEN);
  if (tmp3 != target)
...
mod_pow2_value_transform (rtx insn)
{
  ...
  insert_insn_on_edge (gen_mod_pow2 (mode, code, set_dest, op1, op2), e);
		  ...
		  rtx target, rtx op1, rtx op2, int sub)
{
  ...
  tmp1 =
    simplify_gen_binary (operation, mode, copy_rtx (target), copy_rtx (tmp));
  tmp1 = force_operand (tmp1, target);
  if (tmp1 != target)
    emit_move_insn (target, tmp1);
...
mod_subtract_transform (rtx insn)
{
  ...
  insert_insn_on_edge (gen_mod_subtract (mode, code, set_dest, op1, op2, i),
		       e);
