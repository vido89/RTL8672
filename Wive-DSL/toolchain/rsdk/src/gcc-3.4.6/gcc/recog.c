  ...
  switch (code)
	  ...
	  && !MEM_VOLATILE_P (XEXP (x, 0)))
	{
	  ...
	  if (GET_CODE (x) == ZERO_EXTRACT)
	    {
	      enum machine_mode new_mode = mode_for_extraction (EP_extzv, 1);
	      if (new_mode != MAX_MACHINE_MODE)
		wanted_mode = new_mode;
	    }
	  else if (GET_CODE (x) == SIGN_EXTRACT)
	    {
	      enum machine_mode new_mode = mode_for_extraction (EP_extv, 1);
	      if (new_mode != MAX_MACHINE_MODE)
		wanted_mode = new_mode;
	    }
  ...
  if (code == SUBREG)
	...
	return 0;
      ...
      /* Avoid memories with nonzero SUBREG_BYTE, as offsetting the memory
         ...
         ??? This is a kludge.  */
      if (!reload_completed && SUBREG_BYTE (op) != 0 && GET_CODE (sub) == MEM)
	return 0;
...
win:ATTRIBUTE_UNUSED_LABEL return 1;
}

...
extract_constrain_insn_cached (rtx insn)
{
  ...
  if (which_alternative == -1 && !constrain_operands (reload_completed))
...
extract_insn (rtx insn)
{
  ...
  for (i = 0; i < noperands; i++)
    recog_data.operand_type[i]
	 ...
	 : recog_data.constraints[i][0] == '+' ? OP_INOUT : OP_IN);
  ...
  for (i = 0; i < recog_data.n_operands; i++)
      ...
      for (j = 0; j < recog_data.n_alternatives; j++)
	{
	  ...
	  for (;;)
		...
		case 'r':
		  op_alt[j].class =
		    reg_class_subunion[(int) op_alt[j].
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
	...
	continue;
      ...
      if ((regno == FRAME_POINTER_REGNUM
	   || regno == HARD_FRAME_POINTER_REGNUM) && (!reload_completed
						      ||
