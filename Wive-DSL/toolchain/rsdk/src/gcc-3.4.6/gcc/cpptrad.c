...
/* Set up state for finding the opening '(' of a function-like
   macro.  */
static void
maybe_start_funlike (cpp_reader * pfile, cpp_hashnode * node,
		     const uchar * start, struct fun_macro *macro)
{
  ...
  for (;;)
	...
	case '\n':
	  ...
	  if ((lex_state == ls_fun_open || lex_state == ls_fun_close)
	      && !pfile->state.in_directive && _cpp_get_fresh_line (pfile))
	    {
	...
	lex_state = ls_none;
	       ...
	       || lex_state == ls_predicate || lex_state == ls_defined)
	lex_state = ls_none;
  ...
  if (*CUR (context) == '(')
      ...
      else
	{
	  BUFF_FRONT (pfile->a_buff) =
	    (uchar *) & macro->params[macro->paramc];
	  macro->fun_like = 1;
	}
    }
...
static size_t
canonicalize_text (uchar * dest, const uchar * src, size_t len,
		   uchar * pquote)
{
