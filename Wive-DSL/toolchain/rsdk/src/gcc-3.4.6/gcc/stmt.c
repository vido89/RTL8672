...
force_label_rtx (tree label)
{
  ...
  if (function != current_function_decl && function != inline_function_decl)
  ...
  if (context != 0 && context != current_function_decl)
...
#endif
	{
	  ...
	  emit_insn (gen_rtx_CLOBBER
		     (VOIDmode,
		      gen_rtx_MEM (BLKmode, hard_frame_pointer_rtx)));
  ...
  if (block != end_block)
	...
	= ((block->data.block.outer_cleanups
	   ...
	   ? tree_cons (NULL_TREE, block->data.block.cleanups,
			block->data.block.outer_cleanups) : 0);
	     ...
	     tree cleanup_list, rtx first_insn, int dont_jump_in)
{
  ...
  for (prev = 0, f = goto_fixup_chain; f; prev = f, f = f->next)
      ...
      else if (PREV_INSN (f->target_rtl) != 0)
	{
	  ...
	  if (f->cleanup_list_list)
	    {
	      tree lists;
	      for (lists = f->cleanup_list_list; lists;
		   lists = TREE_CHAIN (lists))
  ...
  for (prev = 0, f = goto_fixup_chain; f; prev = f, f = f->next)
    if (f->before_jump != 0 && PREV_INSN (f->target_rtl) == 0
      ...
      if (asm_operand_ok (op, constraint) <= 0)
	{
	  if (allows_reg)
	      ...
	      if (CONSTANT_P (op))
		{
	      ...
	      if (GET_CODE (op) == REG
		  || GET_CODE (op) == SUBREG
		  || GET_CODE (op) == ADDRESSOF || GET_CODE (op) == CONCAT)
		{
  ...
  for (i = 0; i < ninout; i++)
    {
      ...
      ASM_OPERANDS_INPUT (body, ninputs - ninout + i) = output_rtx[j];
	      ...
	      if (j == -4)	/* `memory', don't cache memory across asm */
		{
		  XVECEXP (body, 0, i++)
		    = gen_rtx_CLOBBER (VOIDmode,
				       gen_rtx_MEM
				       (BLKmode, gen_rtx_SCRATCH (VOIDmode)));
		  continue;
		}
...
expand_expr_stmt_value (tree exp, int want_value, int maybe_last)
{
  ...
  if (!want_value
      ...
      && exp != error_mark_node && warn_unused_value)
    {
...
expand_return (tree retval)
{
  ...
  if (retval == error_mark_node)
    ...
    if ((TREE_CODE (retval) == MODIFY_EXPR || TREE_CODE (retval) == INIT_EXPR)
  ...
  if (optimize && retval_rhs != 0
	  ...
	  || TREE_CODE (TREE_OPERAND (retval_rhs, 2)) == CALL_EXPR))
    {
      ...
      expr =
	build (MODIFY_EXPR, TREE_TYPE (TREE_TYPE (current_function_decl)),
	       DECL_RESULT (current_function_decl), TREE_OPERAND (retval_rhs,
								  1));
  ...
  if (retval_rhs != 0
      ...
      if (tmpmode == BLKmode)
	{
	  ...
	  for (tmpmode = GET_CLASS_NARROWEST_MODE (MODE_INT);
	       tmpmode != VOIDmode; tmpmode = GET_MODE_WIDER_MODE (tmpmode))
...
tail_recursion_args (tree actuals, tree formals)
{
  ...
  for (a = actuals, f = formals, i = 0; a && f;
...
expand_end_bindings (tree vars, int mark_ends, int dont_jump_in)
{
  ...
  if (dont_jump_in > 0
      || (dont_jump_in == 0 && thisblock->data.block.stack_level != 0))
    {
      ...
      for (chain = thisblock->data.block.label_chain; chain;
	   chain = chain->next)
	{
  ...
  if (thisblock->data.block.stack_level != 0
      ...
      if (reachable && thisblock->data.block.stack_level != 0)
	{
      ...
      fixup_gotos (thisblock,
		   ...
		   thisblock->data.block.first_insn, dont_jump_in);
    }
...
expand_decl_init (tree decl)
{
  ...
  /* If this is a CONST_DECL, we don't have to generate any code.  Likewise
     for static decls.  */
  if (TREE_CODE (decl) == CONST_DECL || TREE_STATIC (decl))
    return;
  ...
  if (DECL_INITIAL (decl) == error_mark_node)
	...
	expand_assignment (decl,
			   convert (TREE_TYPE (decl), integer_zero_node), 0);
      emit_queue ();
    }
  else if (DECL_INITIAL (decl)
	   && TREE_CODE (DECL_INITIAL (decl)) != TREE_LIST)
    {
...
expand_anon_union_decl (tree decl, tree cleanup, tree decl_elts)
{
  ...
  for (t = decl_elts; t; t = TREE_CHAIN (t))
	...
	abort ();
      ...
      if (cleanup != 0)
	thisblock->data.block.cleanups
	  = tree_cons (decl_elt, cleanup_elt, thisblock->data.block.cleanups);
    }
}

...
void
expand_start_case (int exit_flag, tree expr, tree type, const char *printname)
{
		 ...
		 int sparseness)
{
  ...
  if (!root)
    ...
    {
      if (root->left)
	case_stack->data.case_stmt.case_list = root =
	  case_tree2list (root, 0);
...
expand_end_case_type (tree orig_index, tree orig_type)
{
  ...
  if (index_type != error_mark_node)
	       ...
	       || TREE_CONSTANT (index_expr))
	{
	  ...
	  if (GET_CODE (index) == CONST_INT
	  ...
	  else
	    {
	      ...
	      use_cost_table
		= (TREE_CODE (orig_type) != ENUMERAL_TYPE
		   && estimate_case_costs (thiscase->data.case_stmt.
					   case_list));
	...
	abort ();
      reorder_insns (before_case, end, thiscase->data.case_stmt.start);
    }
  else
    end_cleanup_deferral ();
