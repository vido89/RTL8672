		 ...
		 rtx value, HOST_WIDE_INT total_size)
{
  ...
  if (BYTES_BIG_ENDIAN
      && GET_CODE (op0) != MEM && unit > GET_MODE_BITSIZE (GET_MODE (op0)))
  ...
  if (HAVE_insv
	...
	xop0 =
	  gen_rtx_SUBREG (maxmode, SUBREG_REG (xop0), SUBREG_BYTE (xop0));
      ...
      if (GET_MODE (value) != maxmode)
	{
	  if (GET_MODE_BITSIZE (GET_MODE (value)) >= bitsize)
	      ...
	      if (GET_MODE_SIZE (GET_MODE (value)) < GET_MODE_SIZE (maxmode))
		{
		  ...
		  tmp =
		    simplify_subreg (maxmode, value1, GET_MODE (value), 0);
		  if (!tmp)
  ...
  if (GET_CODE (op0) == REG || GET_CODE (op0) == SUBREG)
      ...
      if (mode == VOIDmode)
	{
	  /* The only way this should occur is if the field spans word
	     boundaries.  */
	  store_split_bit_field (op0, bitsize,
				 bitpos + offset * BITS_PER_UNIT, value);
	  return;
	}
		   ...
		   HOST_WIDE_INT total_size)
{
  ...
  if (GET_CODE (op0) == REG
      && mode == GET_MODE (op0)
      && bitnum == 0 && bitsize == GET_MODE_BITSIZE (GET_MODE (op0)))
    {
  ...
  if (VECTOR_MODE_P (GET_MODE (op0))
	  ...
	  bitsize / GET_MODE_BITSIZE (GET_MODE_INNER (GET_MODE (op0)))))
    {
      ...
      enum machine_mode innermode = GET_MODE_INNER (outermode);
      int icode =
	(int) vec_extract_optab->handlers[(int) outermode].insn_code;
      int pos = bitnum / GET_MODE_BITSIZE (innermode);
      rtx rtxpos = GEN_INT (pos);
      rtx src = op0;
  ...
  mode1 = (SCALAR_INT_MODE_P (tmode)
	   ? mode_for_size (bitsize, GET_MODE_CLASS (tmode), 0) : mode);

  if (((bitsize >= BITS_PER_WORD && bitsize == GET_MODE_BITSIZE (mode)
	&& bitpos % BITS_PER_WORD == 0) || (mode1 != BLKmode
					    ...
					    && (BYTES_BIG_ENDIAN
						? bitpos + bitsize ==
						BITS_PER_WORD : bitpos == 0)))
      &&
      ((GET_CODE (op0) != MEM
	&& TRULY_NOOP_TRUNCATION (GET_MODE_BITSIZE (mode),
				  GET_MODE_BITSIZE (GET_MODE (op0)))
	&& GET_MODE_SIZE (mode1) != 0
	&& byte_offset % GET_MODE_SIZE (mode1) == 0) || (GET_CODE (op0) == MEM
							 &&
...
no_subreg_mode_swap:
  ...
  if (bitsize > BITS_PER_WORD)
      ...
      for (i = 0; i < nwords; i++)
	{
	  ...
	  rtx result_part = extract_bit_field (op0, MIN (BITS_PER_WORD,
							 bitsize -
							 i * BITS_PER_WORD),
					       bitnum + bit_offset, 1,
					       target_part, mode,
      ...
      if (unsignedp)
	{
	  ...
	  if (GET_MODE_SIZE (GET_MODE (target)) > nwords * UNITS_PER_WORD)
		...
		emit_move_insn
		  (operand_subword (target,
				    WORDS_BIG_ENDIAN ? total_words - i -
				    1 : i, 1, VOIDmode), const0_rtx);
	    }
	  return target;
	}
      ...
      target = expand_shift (LSHIFT_EXPR, mode, target,
			     build_int_2 (GET_MODE_BITSIZE (mode) - bitsize,
					  0), NULL_RTX, 0);
	       ...
	       && (bitsize + bitpos > GET_MODE_BITSIZE (extzv_mode))))
	{
	  ...
	  if (pat)
	  ...
	  else
	    {
	      delete_insns_since (last);
	      target =
		extract_fixed_bit_field (int_mode, op0, offset, bitsize,
					 bitpos, target, 1);
	    }
	}
      else
  ...
  if (unsignedp)
      ...
      if (bitpos)
	{
	  ...
	  rtx subtarget = (target != 0 && GET_CODE (target) == REG
			   && !REG_FUNCTION_VALUE_P (target) ? target : 0);
	  if (tmode != mode)
	    subtarget = 0;
			 ...
			 unsigned HOST_WIDE_INT bitpos, int unsignedp)
{
  ...
  while (bitsdone < bitsize)
	...
	result =
	  expand_binop (word_mode, ior_optab, part, result, NULL_RTX, 1,
			OPTAB_LIB_WIDEN);
	      ...
	      tree amount, rtx target, int unsignedp)
{
  ...
  if (SHIFT_COUNT_TRUNCATED)
	...
	op1 = GEN_INT ((unsigned HOST_WIDE_INT) INTVAL (op1)
      ...
      else if (GET_CODE (op1) == SUBREG && subreg_lowpart_p (op1))
	op1 = SUBREG_REG (op1);
    }
#endif
	     ...
	     int unsignedp)
{
  ...
  if (const_op1 && GET_CODE (const_op1) == CONST_INT
      && (unsignedp || !flag_trapv))
    {
      ...
      int mult_cost;
      enum
      { basic_variant, negate_variant, add_variant } variant = basic_variant;
      ...
      if (HOST_BITS_PER_INT >= GET_MODE_BITSIZE (mode))
	{
	  synth_mult (&alg2, -val,
		      (alg.cost <
		       mult_cost ? alg.cost : mult_cost) - negate_cost);
	  ...
	  for (opno = 1; opno < alg.ops; opno++)
	    {
	      ...
	      rtx add_target
		= (opno == alg.ops - 1 && target != 0
		   && variant != add_variant && !preserve) ? target : 0;
  ...
  /* Expand x*2.0 as x+x.  */
  if (GET_CODE (op1) == CONST_DOUBLE && GET_MODE_CLASS (mode) == MODE_FLOAT)
    {
      REAL_VALUE_TYPE d;
		   ...
		   int *post_shift_ptr, int *lgup_ptr)
{
  ...
  for (post_shift = lgup; post_shift > 0; post_shift--)
	...
	(mlow_hi << (HOST_BITS_PER_WIDE_INT - 1)) | (mlow_lo >> 1);
      unsigned HOST_WIDE_INT mh_lo =
	(mhigh_hi << (HOST_BITS_PER_WIDE_INT - 1)) | (mhigh_lo >> 1);
      if (ml_lo >= mh_lo)
	break;
		      ...
		      int unsignedp, int max_cost)
{
  ...
  if (mul_highpart_cost[(int) mode] < max_cost)
    {
      mul_highpart_optab =
	unsignedp ? umul_highpart_optab : smul_highpart_optab;
      target =
	expand_binop (mode, mul_highpart_optab, op0, op1, target, unsignedp,
		      OPTAB_DIRECT);
      if (target)
	return target;
    }
  ...
  if (size - 1 < BITS_PER_WORD
    ...
    {
      mul_highpart_optab =
	unsignedp ? smul_highpart_optab : umul_highpart_optab;
      target =
	expand_binop (mode, mul_highpart_optab, op0, op1, target, unsignedp,
		      OPTAB_DIRECT);
      if (target)
	       ...
	       rtx op0, rtx op1, rtx target, int unsignedp)
{
  ...
  if (!unsignedp && op1 == constm1_rtx)
    {
      if (rem_flag)
	return const0_rtx;
      return expand_unop (mode, flag_trapv
			  && GET_MODE_CLASS (mode) ==
			  MODE_INT ? negv_optab : neg_optab, op0, target, 0);
    }
  ...
  optab2 = ((op1_is_pow2 && op1 != const0_rtx)
	    ? optab1 : (unsignedp ? udivmod_optab : sdivmod_optab));
  ...
  if (op1 != const0_rtx)
	...
	if (op1_is_constant)
	  {
			...
			if (mh != 0)
			  {
			    ...
			    t3 = expand_shift (RSHIFT_EXPR, compute_mode, t2,
					       build_int_2 (1, 0), NULL_RTX,
					       1);
			    t4 =
			      force_operand (gen_rtx_PLUS
					     (compute_mode, t1, t3),
					     NULL_RTX);
			    quotient =
		...
		if (d == 1)
		...
		else if (EXACT_POWER_OF_2_OR_ZERO_P (abs_d))
		  {
			...
			emit_label (label);
			quotient =
			  expand_shift (RSHIFT_EXPR, compute_mode, t1,
					build_int_2 (lgup, 0), tquotient, 0);
		      }
		    else
		      {
			rtx t1, t2, t3;
			...
			t3 = force_operand (gen_rtx_PLUS (compute_mode,
							  op0, t2), NULL_RTX);
			quotient =
			  expand_shift (RSHIFT_EXPR, compute_mode, t3,
					build_int_2 (lgup, 0), tquotient, 0);
		      }
		...
		else if (size <= HOST_BITS_PER_WIDE_INT)
		  {
			...
			t2 = expand_shift (RSHIFT_EXPR, compute_mode, t1,
					   ...
					   NULL_RTX, 0);
			t3 =
			  expand_shift (RSHIFT_EXPR, compute_mode, op0,
					build_int_2 (size - 1, 0), NULL_RTX,
					0);
			if (d < 0)
			  quotient
	...
	if (op1_is_constant && HOST_BITS_PER_WIDE_INT >= size)
	  {
		...
		t1 = force_operand (gen_rtx_PLUS (compute_mode,
				    ...
				    NULL_RTX);
		t2 =
		  expand_binop (compute_mode, ior_optab, op0, t1, NULL_RTX, 0,
				OPTAB_WIDEN);
		nsign =
		  expand_shift (RSHIFT_EXPR, compute_mode, t2,
				build_int_2 (size - 1, 0), NULL_RTX, 0);
		t3 =
		  force_operand (gen_rtx_MINUS (compute_mode, t1, nsign),
				 NULL_RTX);
		t4 =
		  expand_divmod (0, TRUNC_DIV_EXPR, compute_mode, t3, op1,
				 NULL_RTX, 0);
		if (t4)
		  {
		    rtx t5;
	   ...
	   by conditionally adjusting op0 *and* the result.  */
	{
	  ...
	  do_cmp_and_jump (adjusted_op0, const0_rtx, LT, compute_mode,
			   label1);
	  tem =
	    expand_binop (compute_mode, sdiv_optab, adjusted_op0, op1,
	...
	if (op1_is_constant && HOST_BITS_PER_WIDE_INT >= size)
	  {
	    ...
	    t1 = expand_shift (RSHIFT_EXPR, compute_mode, op0,
			       build_int_2 (pre_shift, 0), NULL_RTX,
			       unsignedp);
	    quotient =
	      expand_mult (compute_mode, t1, gen_int_mode (ml, compute_mode),
			   NULL_RTX, 1);
  ...
  if (quotient == 0)
      ...
      if (rem_flag)
	{
	  ...
	  remainder
	    = sign_expand_binop (compute_mode, umod_optab, smod_optab,
				 op0, op1, target,
				 unsignedp,
				 ((optab2->handlers[(int) compute_mode].
				   insn_code !=
				   CODE_FOR_nothing) ? OPTAB_DIRECT :
				  OPTAB_WIDEN));
	  if (remainder == 0)
	    {
      ...
      if (quotient == 0)
	{
	  ...
	  if (!expand_twoval_binop (unsignedp ? udivmod_optab : sdivmod_optab,
				    op0, op1, quotient, NULL_RTX, unsignedp))
	    {
	      quotient = 0;
	      if (!rem_flag)
...
bool
const_mult_add_overflow_p (rtx x, rtx mult, rtx add, enum machine_mode mode,
			   int unsignedp)
{
  ...
  add_type = (GET_MODE (add) == VOIDmode ? mult_type
	      : (*lang_hooks.types.type_for_mode) (GET_MODE (add),
						   unsignedp));
...
expand_and (enum machine_mode mode, rtx op0, rtx op1, rtx target)
{
  ...
  if (tem == 0)
    tem =
      expand_binop (mode, and_optab, op0, op1, target, 0, OPTAB_LIB_WIDEN);
		 ...
		 enum machine_mode mode, int unsignedp, int normalizep)
{
  ...
  if (op1 == const0_rtx && (code == LT || code == GE)
	  ...
	  || (GET_MODE_BITSIZE (mode) <= HOST_BITS_PER_WIDE_INT
		  ...
		  ==
		  (unsigned HOST_WIDE_INT) 1 << (GET_MODE_BITSIZE (mode) -
						 1)))))
    {
      subtarget = target;
  ...
  if (icode != CODE_FOR_nothing)
	...
	return (comparison == const0_rtx ? const0_rtx
      ...
      if (preserve_subexpressions_p () || !(*pred) (subtarget, compare_mode))
      ...
      if (pattern)
	{
	  ...
	  /* We don't want to use STORE_FLAG_VALUE < 0 below since this
	    ...
	    op0 = expand_shift (RSHIFT_EXPR, compare_mode, op0,
				size_int (GET_MODE_BITSIZE (compare_mode) -
					  1), subtarget, normalizep == 1);
