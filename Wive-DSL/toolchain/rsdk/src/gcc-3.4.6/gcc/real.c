...
lshift_significand_1 (REAL_VALUE_TYPE * r, const REAL_VALUE_TYPE * a)
{
  ...
  for (i = SIGSZ - 1; i > 0; --i)
    r->sig[i] =
      (a->sig[i] << 1) | (a->sig[i - 1] >> (HOST_BITS_PER_LONG - 1));
  r->sig[0] = a->sig[0] << 1;
}
...
void
real_from_target (REAL_VALUE_TYPE * r, const long *buf,
		  enum machine_mode mode)
{
  const struct real_format *fmt;
...
/* Convert from the internal format to the 12-byte Motorola format
   for an IEEE extended real.  */
static void
decode_ieee_extended_motorola (const struct real_format *fmt,
...
/* Convert from the internal format to the 12-byte Intel format for
   an IEEE extended real.  */
static void
decode_ieee_extended_intel_96 (const struct real_format *fmt,
			       REAL_VALUE_TYPE * r, const long *buf)
{
  if (FLOAT_WORDS_BIG_ENDIAN)
    {
...
/* Convert from the internal format to the 16-byte Intel format for
   an IEEE extended real.  */
static void
decode_ieee_extended_intel_128 (const struct real_format *fmt,
				REAL_VALUE_TYPE * r, const long *buf)
{
  /* All the padding in an Intel-format extended real goes at the high end.  */
...
static void
decode_ibm_extended (const struct real_format *fmt ATTRIBUTE_UNUSED,
		     REAL_VALUE_TYPE * r, const long *buf)
{
  REAL_VALUE_TYPE u, v;
  const struct real_format *base_fmt;
