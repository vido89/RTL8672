...
unsigned_conversion_warning (tree result, tree operand)
{
  ...
  if (TREE_CODE (operand) == INTEGER_CST
      ...
      && TREE_UNSIGNED (type)
      && skip_evaluation == 0 && !int_fits_type_p (operand, type))
    {
...
static void
add_tlist (struct tlist **to, struct tlist *add, tree exclude_writer,
	   int copy)
{
  while (add)
    {
  ...
  while (list)
    {
	  ...
	  && list->writer != writer && (!only_writes || list->writer))
	{
...
pointer_int_sum (enum tree_code resultcode, tree ptrop, tree intop)
{
  ...
  if ((TREE_CODE (intop) == PLUS_EXPR || TREE_CODE (intop) == MINUS_EXPR)
      && !TREE_CONSTANT (intop)
      && TREE_CONSTANT (TREE_OPERAND (intop, 1)) && TREE_CONSTANT (size_exp)
  ...
  switch (TREE_CODE (expr))
    ...
    case COMPLEX_EXPR:
      return build_binary_op ((TREE_SIDE_EFFECTS (TREE_OPERAND (expr, 1))
			      ...
			      c_common_truthvalue_conversion (TREE_OPERAND
							      (expr, 0)),
			      c_common_truthvalue_conversion (TREE_OPERAND
							      (expr, 1)), 0);
    ...
    case COND_EXPR:
      ...
      return
	fold (build
	      (COND_EXPR, truthvalue_type_node, TREE_OPERAND (expr, 0),
  ...
  if (TREE_CODE (TREE_TYPE (expr)) == COMPLEX_TYPE)
    {
      ...
      return (build_binary_op
		...
		? TRUTH_OR_EXPR : TRUTH_ORIF_EXPR),
	       c_common_truthvalue_conversion (build_unary_op
					       (REALPART_EXPR, t, 0)),
	       c_common_truthvalue_conversion (build_unary_op
					       (IMAGPART_EXPR, t, 0)), 0));
    }
...
static tree builtin_function_2 (const char *, const char *, tree, tree,
				int, enum built_in_class, int, int, tree);
...
c_common_get_alias_set (tree t)
{
  ...
  if (t == char_type_node
      || t == signed_char_type_node || t == unsigned_char_type_node)
    return 0;
...
#undef DEF_POINTER_TYPE
  ...
  (*lang_hooks.decls.pushdecl)
    (build_decl (TYPE_DECL, get_identifier ("__builtin_size_t"), sizetype));
		  ...
		  tree high_value)
{
  ...
  if (low_value && high_value && !tree_int_cst_lt (low_value, high_value))
  ...
  if (!node && (low_value || high_value))
    {
      ...
      low_bound = splay_tree_predecessor (cases, (splay_tree_key) low_value);
      high_bound = splay_tree_successor (cases, (splay_tree_key) low_value);
	       ...
	       rtx * alt_rtl)
{
  switch (TREE_CODE (exp))
	...
	result =
	  expand_expr_real (rtl_expr, target, tmode, modifier, alt_rtl);
...
boolean_increment (enum tree_code code, tree arg)
{
  ...
  switch (code)
    ...
    case PREDECREMENT_EXPR:
      val =
	build (MODIFY_EXPR, TREE_TYPE (arg), arg, invert_truthvalue (arg));
      break;
    case POSTDECREMENT_EXPR:
      val =
	build (MODIFY_EXPR, TREE_TYPE (arg), arg, invert_truthvalue (arg));
  ...
  if (DECL_P (*node))
    {
	  ...
	  || TREE_CODE (decl) == LABEL_DECL || TREE_CODE (decl) == TYPE_DECL)
	TREE_USED (decl) = 1;
      else
	{
	  warning ("`%s' attribute ignored", IDENTIFIER_POINTER (name));
  ...
  if (TREE_CODE (TREE_VALUE (args)) != IDENTIFIER_NODE)
      ...
      else
	{
	  ...
	  if (TREE_CODE (type) == POINTER_TYPE)
	    {
	      ptr_type = build_pointer_type_for_mode (TREE_TYPE (type), mode);
	      *node = ptr_type;
	    }
			  ...
			  int flags ATTRIBUTE_UNUSED, bool * no_add_attrs)
{
  ...
  if (targetm.have_named_sections)
	...
	{
	  if (TREE_CODE (decl) == VAR_DECL
	      && current_function_decl != NULL_TREE && !TREE_STATIC (decl))
	    {
			     ...
			     int flags ATTRIBUTE_UNUSED, bool * no_add_attrs)
{
  ...
  *no_add_attrs = true;
  ...
  if (strcmp (TREE_STRING_POINTER (id), "default") == 0)
    ...
    error
      ("visibility arg must be one of \"default\", \"hidden\", \"protected\" or \"internal\"");
			    ...
			    int flags ATTRIBUTE_UNUSED, bool * no_add_attrs)
{
  ...
  if (!DECL_THREAD_LOCAL (decl))
	...
	{
	  error
	    ("tls_model arg must be one of \"local-exec\", \"initial-exec\", \"local-dynamic\" or \"global-dynamic\"");
	  *no_add_attrs = true;
	  return NULL_TREE;
	}
    }
			  ...
			  bool * no_add_attrs)
{
  ...
  for (attr_arg_num = 1; args; args = TREE_CHAIN (args))
      ...
      if (argument)
	{
	  ...
	  if (!argument || TREE_CODE (TREE_VALUE (argument)) == VOID_TYPE)
	    {
	      error
  ...
  if (TREE_CODE (param) == CALL_EXPR)
    {
      ...
      for (attrs = TYPE_ATTRIBUTES (type); attrs; attrs = TREE_CHAIN (attrs))
	if (is_attribute_p ("format_arg", TREE_PURPOSE (attrs)))
	  {
	    ...
	    for (inner_args = TREE_OPERAND (param, 1), i = 1;
		 inner_args != 0; inner_args = TREE_CHAIN (inner_args), i++)
	      if (i == format_num)
		{
		  check_function_arguments_recurse (callback, ctx,
		      ...
		      gt_pointer_operator new_value, void *cookie)
{
  ...
  qsort (&sf->elts[0], sf->len, sizeof (tree), resort_field_decl_cmp);
}
