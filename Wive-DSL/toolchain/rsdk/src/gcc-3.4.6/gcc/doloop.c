...
doloop_valid_p (const struct loop *loop, rtx jump_insn)
{
  ...
  if (!any_condjump_p (jump_insn) || !onlyjump_p (jump_insn))
    {
      if (loop_dump_stream)
	fprintf (loop_dump_stream, "Doloop: Invalid jump at loop end.\n");
      return 0;
    }
  ...
  if (loop_info->n_iterations == loop_info->unroll_number)
    {
      if (loop_dump_stream)
	fprintf (loop_dump_stream, "Doloop: Loop completely unrolled.\n");
      return 0;
    }
  ...
  if (loop_info->has_indirect_jump)
    {
      if (loop_dump_stream)
	fprintf (loop_dump_stream, "Doloop: Indirect jump in function.\n");
      return 0;
    }
  ...
  if (loop_info->has_call)
    {
      if (loop_dump_stream)
	fprintf (loop_dump_stream, "Doloop: Function call in loop.\n");
      return 0;
    }
  ...
  if (loop_info->has_tablejump)
    {
      if (loop_dump_stream)
	fprintf (loop_dump_stream, "Doloop: Computed branch in the loop.\n");
      return 0;
    }
		       ...
		       enum machine_mode mode, rtx condition)
{
  ...
  unsigned_p = (comparison_code == LTU
		|| comparison_code == LEU
		|| comparison_code == GTU
		|| comparison_code == GEU || comparison_code == NE);
  ...
  diff = expand_simple_binop (mode, MINUS,
			      copy_rtx (neg_inc ? initial_value :
					final_value),
			      copy_rtx (neg_inc ? final_value :
					initial_value), NULL_RTX, unsigned_p,
			      OPTAB_LIB_WIDEN);
...
doloop_optimize (const struct loop *loop)
{
  ...
  if (!precondition_loop_p (loop, &initial_value, &final_value,
    ...
    {
      if (loop_dump_stream)
	fprintf (loop_dump_stream, "Doloop: Cannot precondition loop.\n");
      return 0;
    }
  ...
  if (n_iterations)
      ...
      if (!n_iterations_max)
	{
	  ...
	  if (loop_dump_stream)
	    fprintf (loop_dump_stream, "Doloop: Not normal loop.\n");
	  return 0;
	}
    }
  ...
  if (!doloop_pat || !(condition = doloop_condition_get (doloop_pat)))
    {
      if (loop_dump_stream)
	fprintf (loop_dump_stream,
