...
enum operator
{ PLUS_OP, MINUS_OP, POS_MINUS_OP, EQ_OP, OR_OP, ORX_OP, MAX_OP, MIN_OP,
    RANGE_OP };
...
static void write_attr_set (struct attr_desc *, int, rtx,
			    const char *, const char *, rtx, int, int);
...
attr_rtx_1 (enum rtx_code code, va_list p)
{
  ...
  if (GET_RTX_CLASS (code) == '1')
	...
	if (h->hashcode == hashcode
	    && GET_CODE (h->u.rtl) == code && XEXP (h->u.rtl, 0) == arg0)
	  return h->u.rtl;
      ...
      if (h == 0)
	{
	  ...
	  XEXP (rt_val, 0) = arg0;
	   ...
	   || GET_RTX_CLASS (code) == '2' || GET_RTX_CLASS (code) == '<')
    {
      rtx arg0 = va_arg (p, rtx);
	...
	if (h->hashcode == hashcode
	    ...
	    && XEXP (h->u.rtl, 0) == arg0 && XEXP (h->u.rtl, 1) == arg1)
	  return h->u.rtl;
  ...
  else if (GET_RTX_LENGTH (code) == 1 && GET_RTX_FORMAT (code)[0] == 's')
    {
      ...
      for (h = attr_hash_table[hashcode % RTL_HASH_SIZE]; h; h = h->next)
	if (h->hashcode == hashcode
	    && GET_CODE (h->u.rtl) == code && XSTR (h->u.rtl, 0) == arg0)
	  return h->u.rtl;
	...
	if (h->hashcode == hashcode
	    && GET_CODE (h->u.rtl) == code
	    && XSTR (h->u.rtl, 0) == arg0 && XSTR (h->u.rtl, 1) == arg1)
	  return h->u.rtl;
	  ...
	  if (unit->needs_range_function)
	    {
	      ...
	      make_internal_attr (str, newexp,
				  (ATTR_STATIC | ATTR_BLOCKAGE |
				   ATTR_UNSIGNED));
	    }
...
fill_attr (struct attr_desc *attr)
{
  ...
  for (id = defs; id; id = id->next)
	...
	for (i = 0; i < XVECLEN (id->def, id->vec_idx); i++)
	  if (!strcmp_check
	      (XSTR (XEXP (XVECEXP (id->def, id->vec_idx, i), 0), 0),
	       attr->name))
...
make_length_attrs (void)
{
  ...
  {
  identity_fn, zero_fn, zero_fn};
  static rtx (*const address_fn[]) (rtx) =
  {
  max_fn, one_fn, identity_fn};
  size_t i;
...
static rtx
insert_right_side (enum rtx_code code, rtx exp, rtx term, int insn_code,
		   int insn_index)
{
  rtx newexp;
  ...
  if (GET_CODE (value) == CONST_STRING)
	...
	*p = TOUPPER (*p);
      ...
      newexp = attr_rtx (EQ, value,
			 attr_rtx (SYMBOL_REF, DEF_ATTR_STRING (string)));
    }
...
simplify_test_exp (rtx exp, int insn_code, int insn_index)
{
  ...
  switch (GET_CODE (exp))
	...
	return false_rtx;
      ...
      if (GET_CODE (left) == EQ_ATTR_ALT && GET_CODE (right) == EQ_ATTR_ALT)
	{
	...
	left_alt = (GET_CODE (left) == EQ_ATTR_ALT && XINT (left, 1));

      if (GET_CODE (right) == NOT)
	right_alt = (GET_CODE (XEXP (right, 0)) == EQ_ATTR
		     && XSTR (XEXP (right, 0), 0) == alternative_name);
      else
	right_alt = (GET_CODE (right) == EQ_ATTR_ALT && XINT (right, 1));

      if (insn_code >= 0
	  && (GET_CODE (left) == AND
	      || left_alt || GET_CODE (right) == AND || right_alt))
	{
    ...
    case EQ_ATTR_ALT:
      if (current_alternative_string)
	return attr_alt_bit_p (exp,
			       atoi (current_alternative_string)) ? true_rtx :
	  false_rtx;
...
simplify_by_exploding (rtx exp)
{
  ...
  for (ndim = 0; list; ndim++)
    {
      ...
      if (space[ndim].attr == 0 || space[ndim].attr->is_numeric)
	{
  ...
  for (i = num_marks = 0; i < total; i++)
    if (GET_CODE (condval[i]) == CONST_STRING && !ATTR_EQ_ATTR_P (condval[i]))
      {
...
write_eligible_delay (const char *kind)
{
  ...
  printf ("int\n");
  printf
    ("eligible_for_%s (rtx delay_insn ATTRIBUTE_UNUSED, int slot, rtx candidate_insn, int flags ATTRIBUTE_UNUSED)\n",
     kind);
  printf ("{\n");
  printf ("  rtx insn;\n");
  printf ("\n");
...
find_attr (const char **name_p, int create)
{
  ...
  attr->is_numeric = attr->negative_ok = attr->is_const = attr->is_special =
    0;
  attr->unsigned_p = attr->func_units_p = attr->blockage_p = attr->static_p =
    0;
