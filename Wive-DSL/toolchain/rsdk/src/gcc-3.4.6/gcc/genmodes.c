...
validate_mode (struct mode_data *m,
	       ...
	       enum requirement r_ncomponents, enum requirement r_format)
{
...
complete_all_modes (void)
{
  ...
  for_all_modes (c, m) complete_mode (m);
}
...
make_float_mode (const char *name,
		 unsigned int precision, unsigned int bytesize,
		 const char *format, const char *file, unsigned int line)
{
		  ...
		  const char *file, unsigned int line)
{
  ...
  if ((size_t) snprintf (namebuf, sizeof namebuf, "V%u%s",
    ...
    {
      error ("%s:%d: mode name \"%s\" is too long", base, file, line);
      return;
    }
...
emit_insn_modes_h (void)
{
  ...
  printf ("/* Generated automatically from machmode.def%s%s\n",
	  HAVE_EXTRA_MODES ? " and " : "", EXTRA_MODES_FILE);
...
emit_mode_precision (void)
{
  ...
  for_all_modes (c, m) if (m->precision != (unsigned int) -1)
...
emit_mode_size (void)
{
  ...
  for_all_modes (c, m) tagged_printf ("%u", m->bytesize, m->name);
...
emit_mode_nunits (void)
{
  ...
  for_all_modes (c, m) tagged_printf ("%u", m->ncomponents, m->name);
...
emit_mode_wider (void)
{
  ...
  for_all_modes (c, m)
    tagged_printf ("%smode",
		   m->wider ? m->wider->name : void_mode->name, m->name);
...
emit_mode_base_align (void)
{
  ...
  print_maybe_const_decl ("%sunsigned char",
			  "mode_base_align", "NUM_MACHINE_MODES", alignment);

  for_all_modes (c, m) tagged_printf ("%u", m->alignment, m->name);
...
emit_real_format_for_mode (void)
{
  ...
  print_maybe_const_decl ("const struct real_format *%s",
			  ...
			  "MAX_MODE_FLOAT - MIN_MODE_FLOAT + 1", format);
#else
...
main (int argc, char **argv)
{
  ...
  modes_by_name =
    htab_create_alloc (64, hash_mode, eq_mode, 0, xcalloc, free);
