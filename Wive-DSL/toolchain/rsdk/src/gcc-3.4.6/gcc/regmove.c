...
struct match
{
  ...
  enum
  { READ, WRITE, READWRITE } use[MAX_RECOG_OPERANDS];
      ...
      if (use != 0 && use != (rtx) (size_t) 1)
	{
	  ...
	  if (0
	      ...
	      if (apply_change_group ())
		{
		  ...
		  REG_NOTES (insn)
		    = gen_rtx_EXPR_LIST (REG_INC, reg, REG_NOTES (insn));
		  if (!inc_insn_set)
...
{
  if (GET_CODE (pat) == SET && reg_overlap_mentioned_p (x, flags_set_1_rtx))
    flags_set_1_set = 1;
}

  ...
  for (p = NEXT_INSN (insn); p; p = NEXT_INSN (p))
	  ...
	  && GET_MODE (XEXP (note, 0)) == GET_MODE (src))
	{
	  ...
	  for (q = next_real_insn (insn);
	       q != next_real_insn (p); q = next_real_insn (q))
	    {
	      ...
	      if (q != p && GET_CODE (q) == CALL_INSN)
		{
	      ...
	      if (dest_death == 0
		  && (dest_death =
		      find_regno_note (q, REG_DEAD, dregno)) != 0)
		{
...
optimize_reg_copy_3 (rtx insn, rtx dest, rtx src)
{
  ...
  if (src_no < FIRST_PSEUDO_REGISTER
      ...
      || REG_N_DEATHS (src_no) != 1 || REG_N_SETS (src_no) != 1)
    return;
  ...
  if (!(set = single_set (p)) || GET_CODE (SET_SRC (set)) != MEM
      ...
      || find_reg_note (p, REG_EQUIV, NULL_RTX) || SET_DEST (set) != src_reg)
    return;
  ...
  /* Do not use a SUBREG to truncate from one mode to another if truncation
     is not a nop.  */
  if (GET_MODE_BITSIZE (GET_MODE (src_reg)) <=
...
regmove_optimize (rtx f, int nregs, FILE * regmove_dump_file)
{
  ...
  for (i = old_max_uid; i >= 0; i--)
    regmove_bb_head[i] = -1;
  FOR_EACH_BB (bb) regmove_bb_head[INSN_UID (BB_HEAD (bb))] = bb->index;
	  ...
	  for (op_no = 0; op_no < recog_data.n_operands; op_no++)
		...
		continue;
	      ...
	      if (fixup_match_1 (insn, set, src, src_subreg, dst, pass,
				 op_no, match_no, regmove_dump_file))
		break;
	    }
	}
    }
	       ...
	       FILE * regmove_dump_file)
{
  ...
  for (length = s_length = 0, p = NEXT_INSN (insn); p; p = NEXT_INSN (p))
	  ...
	  && (GET_MODE (XEXP (dst_note, 0)) == GET_MODE (dst)))
	{
	  ...
	  validate_change (insn, recog_data.operand_loc[match_number], src,
			   1);
  ...
  if (code == MINUS)
	...
	post_inc = 0;
      validate_change (insn, &XEXP (SET_SRC (set), 1), GEN_INT (insn_const),
		       0);
...
combine_stack_adjustments (void)
{
  ...
  FOR_EACH_BB (bb) combine_stack_adjustments_for_block (bb);
}
  ...
  for (i = 1; i < XVECLEN (tmp, 0); ++i)
    {
      ...
      if (GET_CODE (this) == SET && SET_SRC (this) == SET_DEST (this))
	;
      else if (GET_CODE (this) != CLOBBER && GET_CODE (this) != USE)
	return NULL_RTX;
    }
...
record_one_stack_memref (rtx insn, rtx * mem,
...
static int
try_apply_stack_adjustment (rtx insn, struct csa_memlist *memlist,
