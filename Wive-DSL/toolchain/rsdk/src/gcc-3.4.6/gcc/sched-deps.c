...
get_condition (rtx insn)
{
  ...
  pat = SET_DEST (pat);
  cond = XEXP (pat, 0);
  if (GET_CODE (XEXP (cond, 1)) == LABEL_REF && XEXP (cond, 2) == pc_rtx)
    return cond;
  else if (GET_CODE (XEXP (cond, 2)) == LABEL_REF && XEXP (cond, 1) == pc_rtx)
    return gen_rtx_fmt_ee (reverse_condition (GET_CODE (cond)),
			   GET_MODE (cond), XEXP (cond, 0), XEXP (cond, 1));
  else
    return 0;
}
...
add_dependence (rtx insn, rtx elem, enum reg_note dep_type)
{
  ...
  if (GET_CODE (insn) == CALL_INSN && (INSN_BB (elem) != INSN_BB (insn)))
    return 0;
#endif
...
static void
flush_pending_lists (struct deps *deps, rtx insn, int for_read, int for_write)
{
  if (for_write)
    {
...
sched_analyze_1 (struct deps *deps, rtx x, rtx insn)
{
  ...
  while (GET_CODE (dest) == STRICT_LOW_PART || GET_CODE (dest) == SUBREG
	 ...
	 || GET_CODE (dest) == SIGN_EXTRACT)
    {
	  ...
	  || GET_CODE (dest) == ZERO_EXTRACT
	  || GET_CODE (dest) == SIGN_EXTRACT || read_modify_subreg_p (dest))
	{
...
sched_analyze_2 (struct deps *deps, rtx x, rtx insn)
{
  ...
  switch (code)
	...
	for (u = deps->last_pending_memory_flush; u; u = XEXP (u, 1))
	  if (GET_CODE (XEXP (u, 0)) != JUMP_INSN || deps_may_trap_p (x))
  ...
  if (reg_pending_barrier)
      ...
      else
	{
	  EXECUTE_IF_SET_IN_REG_SET (&deps->reg_last_in_use, 0, i,
				     ...
				     struct deps_reg *reg_last =
				     &deps->reg_last[i];
				     add_dependence_list_and_free (insn,
								   &reg_last->
								   uses,
								   REG_DEP_ANTI);
				     add_dependence_list_and_free (insn,
      ...
      if (GET_CODE (PATTERN (insn)) == COND_EXEC)
	{
	  EXECUTE_IF_SET_IN_REG_SET (reg_pending_uses, 0, i,
				     ...
				     add_dependence_list (insn,
							  ...
							  0);
				     reg_last->uses =
				     alloc_INSN_LIST (insn, reg_last->uses);
				     reg_last->uses_length++;});
	  ...
	  EXECUTE_IF_SET_IN_REG_SET (reg_pending_sets, 0, i,
				     ...
				     add_dependence_list (insn,
							  ...
							  REG_DEP_ANTI);
				     reg_last->sets =
				     alloc_INSN_LIST (insn, reg_last->sets);
				     SET_REGNO_REG_SET (&deps->
