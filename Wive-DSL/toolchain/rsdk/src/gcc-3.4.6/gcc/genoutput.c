...
get_insn_name (int index)
{
  ...
  if (last_named)
    sprintf (buf, "%s+%d", last_named->name,
	     index - last_named->index_number);
  else
	       ...
	       int this_strict_low)
{
  ...
  switch (GET_CODE (part))
	...
	{
	  message_with_line (d->lineno, "repeated operand number %d\n", opno);
	  have_error = 1;
	}
  ...
  if (template[0] == '*')
    {
      ...
      puts ("\nstatic const char *");
      printf
	("output_%d (rtx *operands ATTRIBUTE_UNUSED, rtx insn ATTRIBUTE_UNUSED)\n",
	 d->code_number);
      puts ("{");
