...
static const char *process_brace_body (const char *, const char *,
				       const char *, int, int);
...
static void
pfatal_pexecute (const char *, const char *)
  ATTRIBUTE_NORETURN;
...
/* NB: This is shared amongst all front-ends.  */
 ...
 %{g*} %{O*} %{W*&pedantic*} %{w} %{plinux} %{pros} %{std*&ansi&trigraphs}\
...
/* This is the table of mappings.  Mappings are tried sequentially
   for each option encountered; the first one that matches, wins.  */
     ...
     static const struct option_map
       option_map[] = {
       {"--all-warnings", "-Wall", 0},
       {"--ansi", "-ansi", 0},
       {"--assemble", "-S", 0},
  ...
  while (i < argc)
    {
      ...
      for (tott_idx = 0;
	   target_option_translations[tott_idx].option_found; tott_idx++)
	{
...
set_spec (const char *name, const char *spec)
{
  ...
  *(sl->ptr_spec) = ((spec[0] == '+' && ISSPACE ((unsigned char) spec[1]))
		     ? concat (old_spec, spec + 1, NULL) : xstrdup (spec));
  ...
  while (1)
      ...
      if (*p == '%' && !main_p)
	{
	  ...
	  if (!strncmp (p1, "%include", sizeof ("%include") - 1)
	    ...
	    if (!strncmp (p1, "%include_noerr", sizeof "%include_noerr" - 1)
...
See %s for instructions.", strsignal (WTERMSIG (status)), commands[j].prog, bug_report_url);
		  signal_count++;
		  ret_code = -1;
		}
		  ...
		  int do_obj ATTRIBUTE_UNUSED)
{
  ...
  if (do_obj && len > 2 && name[len - 2] == '.' && name[len - 1] == 'o')
    {
      obstack_grow (&obstack, name, len - 2);
      obstack_grow0 (&obstack, TARGET_OBJECT_SUFFIX,
      ...
      else if (argv[i][0] == '-' && argv[i][1] != 0)
	{
	  ...
	  switch (c)
		...
		add_prefix (&startfile_prefixes, value, NULL,
			    PREFIX_PRIORITY_B_OPT, 0, &warn_B, 0);
		add_prefix (&include_prefixes,
			    concat (value, "include", NULL), NULL,
			    PREFIX_PRIORITY_B_OPT, 0, NULL, 0);
		n_switches++;
	      }
	      break;
		  ...
		  while (j < argc)
		      ...
		      if (argv[j][0] == '-')
			{
			  if (SWITCH_CURTAILS_COMPILATION (argv[j][1])
			    ...
			    if ((skip = WORD_SWITCH_TAKES_ARG (argv[j] + 1)))
			    j += skip;
			}
		      j++;
		    }
  ...
  for (i = 1; i < argc; i++)
      ...
      else if (argv[i][0] == '-' && argv[i][1] != 0)
	{
	  ...
	  if (!strcmp (p, "save-temps")
	      ...
	      || !strcmp (p, "shared-libgcc") || !strcmp (p, "pipe"))
...
do_spec_1 (const char *spec, int inswitch, const char *soft_matched_part)
{
  ...
  while ((c = *p++))
	...
	switch (c = *p++)
	  ...
	  case 'O':
	    obstack_grow (&obstack, TARGET_OBJECT_SUFFIX,
			  strlen (TARGET_OBJECT_SUFFIX));
	    arg_going = 1;
	    break;
	  ...
	  case 'C':
		...
		= (input_file_compiler->cpp_spec
		   ? input_file_compiler->cpp_spec : cpp_spec);
...
main (int argc, const char **argv)
{
  ...
  obstack_grow (&collect_obstack, "COLLECT_GCC=",
		sizeof ("COLLECT_GCC=") - 1);
  ...
  if (*sysroot_suffix_spec != 0 && do_spec_2 (sysroot_suffix_spec) == 0)
    {
      if (argbuf_index > 1)
  ...
  if (gcc_exec_prefix)
    gcc_exec_prefix =
      concat (gcc_exec_prefix, spec_machine, dir_separator_str, spec_version,
	      dir_separator_str, NULL);
  ...
  return (signal_count != 0 ? 2
	  : error_count > 0 ? (pass_exit_codes ? greatest_status : 1) : 0);
}
		...
		while (*q != ';')
		  {
		    ...
		    if (!strncmp
			(arg, this_arg, (len < q - arg) ? q - arg : len)
      ...
      if (!skip)
	{
	  ...
	  while (*q != ';')
	      ...
	      while (*q != ' ' && *q != ';')
		{
	      ...
	      if (arg != NULL && default_arg (arg, q - arg))
		{
		  skip = 1;
		  break;
		}
