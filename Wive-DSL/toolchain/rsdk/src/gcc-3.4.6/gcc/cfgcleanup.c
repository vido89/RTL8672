...
thread_jump (int mode, edge e, basic_block b)
{
  ...
  for (insn = NEXT_INSN (BB_HEAD (e->src));
       insn != NEXT_INSN (BB_END (e->src)); insn = NEXT_INSN (insn))
    if (INSN_P (insn))
  ...
  for (insn = NEXT_INSN (BB_HEAD (b));
       insn != NEXT_INSN (BB_END (b)) && !failed; insn = NEXT_INSN (insn))
    {
      if (INSN_P (insn))
	{
      ...
      else
	{
	  ...
	  do
	      ...
	      if (first->succ->succ_next)
		{
		  ...
		  if (first->frequency)
		    prob =
		      edge_frequency * REG_BR_PROB_BASE / first->frequency;
		  else
		    prob = 0;
		      ...
		      basic_block bb2, rtx * f1, rtx * f2)
{
  ...
  if (ninsns)
	...
	last1 = PREV_INSN (last1);
      ...
      if (last1 != BB_HEAD (bb1)
	  && GET_CODE (PREV_INSN (last1)) == CODE_LABEL)
	last1 = PREV_INSN (last1);

      while (last2 != BB_HEAD (bb2) && !INSN_P (PREV_INSN (last2)))
	last2 = PREV_INSN (last2);

      if (last2 != BB_HEAD (bb2)
	  && GET_CODE (PREV_INSN (last2)) == CODE_LABEL)
	last2 = PREV_INSN (last2);
...
outgoing_edges_match (int mode, basic_block bb1, basic_block bb2)
{
  ...
  if (bb1->succ && !bb1->succ->succ_next
      ...
      && (GET_CODE (BB_END (bb1)) != JUMP_INSN
	  || simplejump_p (BB_END (bb1))))
    return (bb2->succ && !bb2->succ->succ_next
	    && (bb2->succ->flags & (EDGE_COMPLEX | EDGE_FAKE)) == 0
	    && (GET_CODE (BB_END (bb2)) != JUMP_INSN
  ...
  if (bb1->succ
      ...
      && any_condjump_p (BB_END (bb1)) && onlyjump_p (BB_END (bb1)))
    {
      edge b1, f1, b2, f2;
      bool reverse, match;
	  ...
	  || !any_condjump_p (BB_END (bb2)) || !onlyjump_p (BB_END (bb2)))
	return false;
	...
	f2 = f2->dest->succ;
	  ...
	  || FORWARDER_BLOCK_P (b1->dest) || FORWARDER_BLOCK_P (b2->dest))
	return false;
	...
	return false;
      ...
      /* If we return true, we will join the blocks.  Which means that
         ...
         roughly similar.  */
      if (match
	  && !optimize_size && maybe_hot_bb_p (bb1) && maybe_hot_bb_p (bb2))
	{
	  int prob2;
  ...
  if (e1 || e2
      || nehedges1 != nehedges2 || (fallthru1 != 0) != (fallthru2 != 0))
    return false;
...
try_crossjump_to_edge (int mode, edge e1, edge e2)
{
  ...
  if (src1->pred && !src1->pred->pred_next && FORWARDER_BLOCK_P (src1))
    e1 = src1->pred, src1 = e1->src;

  if (src2->pred && !src2->pred->pred_next && FORWARDER_BLOCK_P (src2))
     ...
     so replace the references to TABLE1 by references to TABLE2.  */
  {
	...
	&& tablejump_p (BB_END (src2), &label2, &table2) && label1 != label2)
      {
	replace_label_data rr;
	rtx insn;
...
try_optimize_cfg (int mode)
{
  ...
  FOR_EACH_BB (bb) update_forwarder_flag (bb);
      ...
      do
	{
	  ...
	  for (b = ENTRY_BLOCK_PTR->next_bb; b != EXIT_BLOCK_PTR;)
						  ...
						  BB_END (b->pred->src))))
		{
		  ...
		  && (b->succ->flags & EDGE_FALLTHRU) && n_basic_blocks > 1)
		{
		  if (rtl_dump_file)
		    fprintf (rtl_dump_file,
		  ...
		  && c->pred->pred_next == NULL && b != c)
		{
		  ...
		  if ((mode & CLEANUP_CFGLAYOUT) && can_merge_blocks_p (b, c))
		    {
		      merge_blocks (b, c);
		      update_forwarder_flag (b);
		      changed_here = true;
		  ...
		  && !(mode & CLEANUP_CFGLAYOUT) && try_simplify_condjump (b))
		changed_here = true;
