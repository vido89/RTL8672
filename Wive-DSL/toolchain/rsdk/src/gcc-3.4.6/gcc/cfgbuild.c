...
static void
make_edges (rtx label_value_list, basic_block min, basic_block max,
	    int update_p)
{
  basic_block bb;
  ...
  if (min == ENTRY_BLOCK_PTR->next_bb)
    cached_make_edge (edge_cache, ENTRY_BLOCK_PTR, min, EDGE_FALLTHRU);
		   ...
		   FILE * file ATTRIBUTE_UNUSED)
{
  ...
  make_edges (label_value_list, ENTRY_BLOCK_PTR->next_bb,
	      EXIT_BLOCK_PTR->prev_bb, 0);
...
enum state
{ BLOCK_NEW = 0, BLOCK_ORIGINAL, BLOCK_TO_SPLIT };
...
find_many_sub_basic_blocks (sbitmap blocks)
{
  ...
  FOR_EACH_BB (bb)
    SET_STATE (bb,
	       TEST_BIT (blocks,
			 bb->index) ? BLOCK_TO_SPLIT : BLOCK_ORIGINAL);

  FOR_EACH_BB (bb) if (STATE (bb) == BLOCK_TO_SPLIT)
    find_bb_boundaries (bb);

  FOR_EACH_BB (bb) if (STATE (bb) != BLOCK_ORIGINAL)
    break;
