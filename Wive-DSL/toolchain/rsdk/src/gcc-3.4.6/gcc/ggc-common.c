		    ...
		    gt_note_pointers note_ptr_fn)
{
  ...
  slot = (struct ptr_data **)
    htab_find_slot_with_hash (saving_htab, obj, POINTER_HASH (obj), INSERT);
  if (*slot != NULL)
    {
		     ...
		     gt_handle_reorder reorder_fn)
{
  ...
  if (data == NULL || data->note_ptr_cookie != note_ptr_cookie)
    abort ();
...
call_alloc (void **slot, void *state_p)
{
  ...
  d->new_addr =
    ggc_pch_alloc_object (state->d, d->obj, d->size,
			  d->note_ptr_fn == gt_pch_p_S);
...
gt_pch_save (FILE * f)
{
  ...
  if (fwrite (&mmi, sizeof (mmi), 1, state.f) != 1)
    fatal_error ("can't write PCH file: %m");
  if (mmi.offset != 0 && fseek (state.f, mmi.offset, SEEK_SET) != 0)
