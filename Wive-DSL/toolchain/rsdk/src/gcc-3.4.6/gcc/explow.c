...
restart:
  ...
  switch (code)
	...
	{
	  x =
	    gen_rtx_PLUS (mode, XEXP (x, 0), plus_constant (XEXP (x, 1), c));
	  c = 0;
	}
  ...
  switch (GET_CODE (x))
    ...
    case MULT:
	  ...
	  || (GET_CODE (x) == PLUS
	      ...
	      && XEXP (x, 1) == convert_memory_address (to_mode,
							XEXP (x, 1))))
...
stabilize (rtx x)
{
  if (GET_CODE (x) != MEM || !rtx_unstable_p (XEXP (x, 0)))
    return x;
...
force_reg (enum machine_mode mode, rtx x)
{
  ...
  if (CONSTANT_P (x)
      ...
      && SET_DEST (set) == temp && !rtx_equal_p (x, SET_SRC (set)))
...
#define MUST_ALIGN (PREFERRED_STACK_BOUNDARY < BIGGEST_ALIGNMENT)
  ...
  if (flag_stack_check && !STACK_CHECK_BUILTIN)
    probe_stack_range (STACK_CHECK_MAX_FRAME_SIZE + STACK_CHECK_PROTECT,
		       size);

  /* Don't use a TARGET that isn't a pseudo or is the wrong mode.  */
  if (target == 0 || GET_CODE (target) != REG
      || REGNO (target) < FIRST_PSEUDO_REGISTER || GET_MODE (target) != Pmode)
...
GTY (())
     ...
     void set_stack_check_libfunc (rtx libfunc)
{
...
probe_stack_range (HOST_WIDE_INT first, rtx size)
{
  ...
  /* In the variable case, do the same as above, but in a loop.  We emit loop
    ...
    {
      rtx test_addr = force_operand (gen_rtx_fmt_ee (STACK_GROW_OP, Pmode,
						     stack_pointer_rtx,
						     GEN_INT (first +
