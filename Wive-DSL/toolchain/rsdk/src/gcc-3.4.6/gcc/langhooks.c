...
lhd_expand_expr (tree t ATTRIBUTE_UNUSED, rtx r ATTRIBUTE_UNUSED,
		 ...
		 int em ATTRIBUTE_UNUSED, rtx * a ATTRIBUTE_UNUSED)
{
  abort ();
}
...
tree
lhd_tree_inlining_add_pending_fn_decls (void *vafnp ATTRIBUTE_UNUSED,
					tree pfn)
{
  return pfn;
}
...
{
  if (TREE_CODE_CLASS (TREE_CODE (exp)) == 'd' && DECL_SIZE_UNIT (exp) != 0)
