...
GTY (())
     ...
     static void prepare_float_lib_cmp (rtx *, rtx *, enum rtx_code *,
					enum machine_mode *, int *);
     static rtx expand_vector_binop (enum machine_mode, optab, rtx, rtx, rtx,
...
/* Add a REG_EQUAL note to the last insn in INSNS.  TARGET is being set to
   ...
   again, ensuring that TARGET is not one of the operands.  */
     ...
     static int
       add_equal_note (rtx insns, rtx target, enum rtx_code code, rtx op0,
		       rtx op1)
{
  rtx last_insn, insn, set;
  rtx note;

  if (!insns || !INSN_P (insns) || NEXT_INSN (insns) == NULL_RTX)
    abort ();
  ...
  for (last_insn = insns;
       NEXT_INSN (last_insn) != NULL_RTX; last_insn = NEXT_INSN (last_insn))
    ;
  ...
  if (GET_RTX_CLASS (code) == '1')
    ...
    note =
      gen_rtx_fmt_ee (code, GET_MODE (target), copy_rtx (op0),
		      copy_rtx (op1));
		      ...
		      optab binoptab)
{
  ...
  mode = GET_MODE (temp1);
  lab1 = gen_label_rtx ();
  emit_cmp_and_jump_insns (temp1, temp2, LT, NULL_RTX, mode, unsignedp, lab1);
	      ...
	      rtx target, int unsignedp, enum optab_methods methods)
{
  ...
  if (GET_RTX_CLASS (binoptab->code) == 'c'
      ...
      || binoptab == smul_highpart_optab || binoptab == umul_highpart_optab)
    {
      commutative_op = 1;
      ...
      if (((target == 0 || GET_CODE (target) == REG)
	   ? ((GET_CODE (op1) == REG
	       && GET_CODE (op0) != REG)
	      || target == op1)
	   : rtx_equal_p (op1, target)) || GET_CODE (op0) == CONST_INT)
	{
	  temp = op1;
	  op1 = op0;
	  op0 = temp;
  ...
  if (methods != OPTAB_MUST_WIDEN
	...
	xop0 = convert_modes (mode0,
			      ...
			      ? GET_MODE (op0) : mode, xop0, unsignedp);

      if (GET_MODE (op1) != mode1 && mode1 != VOIDmode)
	xop1 = convert_modes (mode1,
			      GET_MODE (op1) != VOIDmode
			      ? GET_MODE (op1) : mode, xop1, unsignedp);
  ...
  if ((class == MODE_INT || class == MODE_FLOAT
       || class == MODE_COMPLEX_FLOAT) && methods != OPTAB_DIRECT
      && methods != OPTAB_LIB)
	...
	if (binoptab->handlers[(int) wider_mode].insn_code != CODE_FOR_nothing
		  ...
		  insn_code) != CODE_FOR_nothing)))
	  {
	    ...
	    xop0 =
	      widen_operand (xop0, wider_mode, mode, unsignedp, no_extend);
  ...
  if ((binoptab == and_optab || binoptab == ior_optab
       || binoptab == xor_optab) && class == MODE_INT
  ...
  if ((binoptab == rotl_optab || binoptab == rotr_optab)
      ...
      else
	{
	  ...
	  if (into_temp1 != 0 && into_temp2 != 0)
	    inter =
	      expand_binop (word_mode, ior_optab, into_temp1, into_temp2,
			    into_target, unsignedp, next_methods);
	  else
	    inter = 0;
  ...
  if (binoptab == smul_optab
	  ...
	  != CODE_FOR_nothing)
	{
	  ...
	  if (op0_xhigh)
	    ...
	    {
	      op0_xhigh =
		expand_binop (word_mode, ashr_optab, op0_low, wordm1,
	  ...
	  if (op1_xhigh)
	    ...
	    {
	      op1_xhigh =
		expand_binop (word_mode, ashr_optab, op1_low, wordm1,
			      NULL_RTX, 0, next_methods);
	      if (op1_xhigh)
      ...
      if (product && op0_xhigh && op1_xhigh)
	{
	  ...
	  if (temp != 0)
	    temp = expand_binop (word_mode, add_optab, temp,
				 product_high, product_high, 0, next_methods);

	  if (temp != 0 && temp != product_high)
	    emit_move_insn (product_high, temp);

	  emit_move_insn (operand_subword (product, high, 1, mode),
			  product_high);

	  if (temp != 0)
	    {
	      if (mov_optab->handlers[(int) mode].insn_code !=
		  CODE_FOR_nothing)
		{
	...
	case MINUS:
	  ...
	  if (imag0 != 0 && imag1 != 0)
	    ...
	    res = expand_unop (submode,
			       binoptab ==
			       subv_optab ? negv_optab : neg_optab, imag1,
			       imagr, unsignedp);
	  else
	    res = imag1;
		   ...
		   enum optab_methods methods)
{
  ...
  temp =
    expand_binop (mode, direct_optab, op0, op1, target, unsignedp, OPTAB_LIB);
	   ...
	   wider_mode = GET_MODE_WIDER_MODE (wider_mode))
	{
	  if (clz_optab->handlers[(int) wider_mode].insn_code
		...
		target = gen_reg_rtx (mode);
	      ...
	      temp =
		expand_unop (wider_mode, clz_optab, xop0, NULL_RTX, true);
	      if (temp != 0)
  ...
  /* Open-code the complex negation operation.  */
	...
	target = gen_reg_rtx (mode);
      ...
      x = expand_unop (submode, unoptab,
		       gen_imagpart (submode, op0), target_piece, unsignedp);
      if (target_piece != x)
	emit_move_insn (target_piece, x);

      target_piece = gen_realpart (submode, target);
      x = expand_unop (submode, unoptab,
		       gen_realpart (submode, op0), target_piece, unsignedp);
      if (target_piece != x)
	emit_move_insn (target_piece, x);
	  ...
	  if (temp != 0)
		...
		target = gen_reg_rtx (mode);
	      ...
	      set_unique_reg_note (insn, REG_EQUAL,
				   gen_rtx_fmt_e (NEG, mode, copy_rtx (op0)));
	      return target;
	    }
	  delete_insns_since (last);
	}
		    ...
		    int unsignedp)
{
  ...
  this_abs_optab = !unsignedp && flag_trapv
    && (GET_MODE_CLASS (mode) == MODE_INT) ? absv_optab : abs_optab;
  ...
  if (this_abs_optab->handlers[(int) mode].libfunc)
    {
      ...
      /* Pass 1 for NO_QUEUE so we don't lose any increments
         if the libcall is cse'd or moved.  */
      value =
	emit_library_call_value (abs_optab->handlers[(int) mode].libfunc,
	       ...
	       enum can_compare_purpose purpose)
{
  do
      ...
      if (cmp_optab->handlers[(int) mode].insn_code != CODE_FOR_nothing)
	{
      ...
      if (purpose == ccp_jump
	  && cbranch_optab->handlers[(int) mode].insn_code !=
	  CODE_FOR_nothing)
	return 1;
      if (purpose == ccp_cmov
  ...
  if (mode == BLKmode)
	...
	abort ();
      ...
      for (cmp_mode = GET_CLASS_NARROWEST_MODE (MODE_INT);
	   cmp_mode != VOIDmode; cmp_mode = GET_MODE_WIDER_MODE (cmp_mode))
	{
  ...
  *py = y;
  ...
  if (cmp_optab->handlers[(int) mode].libfunc && class != MODE_FLOAT)
	...
	libfunc = ucmp_optab->handlers[(int) mode].libfunc;
      ...
      result =
	emit_library_call_value (libfunc, NULL_RTX, LCT_CONST_MAKE_BLOCK,
		       ...
		       enum machine_mode mode, int unsignedp)
{
  ...
  comparison = compare_from_rtx (op0, op1, code, unsignedp, cmode, NULL_RTX);
...
expand_float (rtx to, rtx from, int unsignedp)
{
  ...
  for (fmode = GET_MODE (to); fmode != VOIDmode;
	...
	if (icode == CODE_FOR_nothing && imode != GET_MODE (from)
	    && unsignedp)
  ...
  if (unsignedp)
	  ...
	  if ((significand_size (fmode) + 1)
		...
		target = gen_reg_rtx (fmode);
	      ...
	      temp = expand_binop (imode, and_optab, from, const1_rtx,
				   NULL_RTX, 1, OPTAB_LIB_WIDEN);
	      temp1 =
		expand_shift (RSHIFT_EXPR, imode, from, integer_one_node,
			      NULL_RTX, 1);
	      temp =
		expand_binop (imode, ior_optab, temp, temp1, temp, 1,
			      OPTAB_LIB_WIDEN);
	...
	target = gen_reg_rtx (fmode);
      ...
      do_pending_stack_adjust ();
      emit_cmp_and_jump_insns (from, const0_rtx, GE, NULL_RTX,
			       GET_MODE (from), 0, label);
  ...
  /* No hardware instruction available; call a library routine.  */
  {
    ...
    value = emit_library_call_value (libfunc, NULL_RTX, LCT_CONST,
				     GET_MODE (to), 1, from, GET_MODE (from));
    insns = get_insns ();
    end_sequence ();
...
expand_fix (rtx to, rtx from, int unsignedp)
{
  ...
  for (fmode = GET_MODE (from); fmode != VOIDmode;
	...
	if (icode == CODE_FOR_nothing && imode != GET_MODE (to) && unsignedp)
	  icode = can_fix_p (imode, fmode, 0, &must_trunc), doing_unsigned =
	    0;
					 ...
					 &must_trunc))
	{
	  ...
	  target = expand_binop (GET_MODE (to), xor_optab, to,
				 ...
				 ((HOST_WIDE_INT) 1 << (bitsize - 1),
				  GET_MODE (to)), to, 1, OPTAB_LIB_WIDEN);
...
init_integral_libfuncs (optab optable, const char *opname, int suffix)
{
  ...
  init_libfuncs (optable, word_mode,
		 mode_for_size (maxsize, MODE_INT, 0), opname, suffix);
}
			       ...
			       enum mode_class to_class)
{
  ...
  for (fmode = first_from_mode;
       fmode != VOIDmode; fmode = GET_MODE_WIDER_MODE (fmode))
...
init_optabs (void)
{
  ...
  ffs_optab->handlers[(int) mode_for_size (INT_TYPE_SIZE, MODE_INT, 0)].
    libfunc = init_one_libfunc ("ffs");
