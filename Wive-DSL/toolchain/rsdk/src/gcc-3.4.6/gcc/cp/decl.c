...
current_tmpl_spec_kind (int n_class_scopes)
{
  ...
  for (b = current_binding_level;
       b->kind == sk_template_parms; b = b->level_chain)
    {
...
poplevel (int keep, int reverse, int functionbody)
{
  ...
  my_friendly_assert (!current_binding_level->class_shadowed, 19990414);
  ...
  if (kind == sk_cleanup)
    {
      ...
      scope_stmts = add_scope_stmt ( /*begin_p= */ 0, /*partial_p= */ 1);
      if (block)
	{
...
decls_match (tree newdecl, tree olddecl)
{
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
	...
	types_match = comptypes (TREE_TYPE (newdecl),
				 TREE_TYPE (olddecl), COMPARE_REDECLARATION);
    }
...
warn_extern_redeclared_static (tree newdecl, tree olddecl)
{
  ...
  if (TREE_CODE (olddecl) == FUNCTION_DECL && DECL_ARTIFICIAL (olddecl))
    return;
...
duplicate_decls (tree newdecl, tree olddecl)
{
      ...
      if (TREE_CODE (newdecl) != FUNCTION_DECL)
	{
	  ...
	  /* If the built-in is not ansi, then programs can override
	  ...
	  else
	    {
	      ...
	      error ("conflicts with built-in declaration `%#D'", olddecl);
	    }
	  return NULL_TREE;
	}
      else if (!types_match)
	{
	  ...
	  if (DECL_ANTICIPATED (olddecl))
	      ...
	      if (TREE_PUBLIC (newdecl))
		{
		  ...
		  warning ("ambiguates built-in declaration `%#D'", olddecl);
		}
	      else if (warn_shadow)
		warning ("shadowing %s function `%#D'",
			 DECL_BUILT_IN (olddecl) ? "built-in" : "library",
	  ...
	  if (TREE_CODE (DECL_TEMPLATE_RESULT (olddecl)) == TYPE_DECL
	      || TREE_CODE (DECL_TEMPLATE_RESULT (newdecl)) == TYPE_DECL)
	    {
	      ...
	      cp_error_at ("conflicts with previous declaration `%#D'",
		   ...
		   && TREE_CODE (DECL_TEMPLATE_RESULT (newdecl)) ==
		   FUNCTION_DECL
		   &&
      ...
      else if (TREE_CODE (olddecl) == FUNCTION_DECL)
	{
	  ...
	  for (; t1 && t1 != void_list_node;
		...
		if (1 == simple_cst_equal (TREE_PURPOSE (t1),
		  ...
		  {
		    pedwarn
		      ("default argument given for parameter %d of `%#D'", i,
		       newdecl);
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
	...
	DECL_CONTEXT (newdecl) = DECL_CONTEXT (olddecl);
      ...
      DECL_NEEDS_FINAL_OVERRIDER_P (newdecl) |=
  ...
  if (types_match)
		   ...
		   != TYPE_RAISES_EXCEPTIONS (TREE_TYPE (olddecl))))
	{
	  ...
	  if ((pedantic || !DECL_IN_SYSTEM_HEADER (olddecl))
	      ...
	      && flag_exceptions
	      &&
	      !comp_except_specs (TYPE_RAISES_EXCEPTIONS
				  (TREE_TYPE (newdecl)),
				  TYPE_RAISES_EXCEPTIONS (TREE_TYPE
							  (olddecl)), 1))
	    {
	  ...
	  && DECL_INITIAL (olddecl) != NULL_TREE)
	{
	  ...
	  if (CAN_HAVE_FULL_LANG_DECL_P (newdecl)
	      && DECL_LANG_SPECIFIC (newdecl) && DECL_LANG_SPECIFIC (olddecl))
	    {
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
	  ...
	  && !DECL_TEMPLATE_INSTANTIATION (newdecl))
	{
	  ...
	  my_friendly_assert (DECL_TEMPLATE_SPECIALIZATION (newdecl), 0);
	  ...
	  if (TREE_USED (olddecl))
	    ...
	    error ("explicit specialization of %D after first use", olddecl);
  ...
  if (TREE_CODE (newdecl) == FUNCTION_DECL)
	...
	reregister_specialization (newdecl,
				   DECL_TI_TEMPLATE (newdecl), olddecl);
    }
  else
    {
      memcpy ((char *) olddecl + sizeof (struct tree_common),
...
decl_jump_unsafe (tree decl)
{
  ...
  if (DECL_INITIAL (decl) == NULL_TREE && pod_type_p (TREE_TYPE (decl)))
    return 0;
  ...
  if (DECL_INITIAL (decl) || (TYPE_NEEDS_CONSTRUCTING (TREE_TYPE (decl))))
    return 2;
  return 1;
}
  ...
  for (; b; b = b->level_chain)
    {
      ...
      for (; new_decls != old_decls; new_decls = TREE_CHAIN (new_decls))
	{
	  ...
	  if (problem > 1)
	    cp_error_at ("  crosses initialization of `%#D'", new_decls);
	  else
	    cp_pedwarn_at ("  enters scope of non-POD `%#D'", new_decls);
	}
...
define_label (location_t location, tree name)
{
  ...
  for (p = current_binding_level;
       p->kind != sk_function_parms; p = p->level_chain)
...
GTY ((param_is (union tree_node)))
     ...
     static tree build_typename_type (tree context, tree name, tree fullname)
{
  tree t;
  tree d;
  void **e;
...
make_typename_type (tree context, tree name, tsubst_flags_t complain)
{
  ...
  if (name == error_mark_node
      || context == NULL_TREE || context == error_mark_node)
    return error_mark_node;
...
cxx_init_decl_processing (void)
{
  ...
  vtbl_type_node = build_cplus_array_type (vtable_entry_type, NULL_TREE);
  ...
  abort_fndecl = build_library_fn_ptr ("__cxa_pure_virtual", void_ftype);
...
builtin_function (const char *name,
		  tree type,
		  int code,
		  enum built_in_class class, const char *libname, tree attrs)
{
...
check_tag_decl (tree declspecs)
{
  ...
  /* Check for an anonymous union.  */
	...
	error ("`%D' can only be specified for constructors", ob_modifier);
      else
...
groktypename (tree typename)
{
  ...
  type = grokdeclarator (TREE_VALUE (typename), specs, TYPENAME, 0, &attrs);
  if (attrs)
...
start_decl (tree declarator,
	    tree declspecs,
	    int initialized, tree attributes, tree prefix_attributes)
{
  tree decl;
  tree type, tem;
  tree context;
  ...
  if (initialized)
    {
      if (!toplevel_bindings_p () && DECL_EXTERNAL (decl))
		  ...
		  if (!same_type_p (DECL_CONTEXT (field), context))
		    pedwarn
		      ("ISO C++ does not permit `%T::%D' to be defined as `%T::%D'",
		       DECL_CONTEXT (field), DECL_NAME (decl), context,
		       DECL_NAME (decl));
  ...
  if (init == NULL_TREE)
    {
      if ((DECL_LANG_SPECIFIC (decl) == 0
	   || DECL_IN_AGGR_P (decl) == 0) && !DECL_THIS_EXTERN (decl))
...
maybe_deduce_size_from_array_init (tree decl, tree init)
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE
      && TYPE_DOMAIN (type) == NULL_TREE && TREE_CODE (decl) != TYPE_DECL)
    {
...
layout_var_decl (tree decl)
{
  ...
  if (!DECL_SIZE (decl)
	  ...
	  || (TREE_CODE (type) == ARRAY_TYPE
	      && !TYPE_DOMAIN (type) && COMPLETE_TYPE_P (TREE_TYPE (type)))))
    layout_decl (decl, 0);
  ...
  if ((DECL_EXTERNAL (decl) || TREE_STATIC (decl))
      && DECL_SIZE (decl) != NULL_TREE && !TREE_CONSTANT (DECL_SIZE (decl)))
    {
...
maybe_commonize_var (tree decl)
{
  ...
  if (TREE_STATIC (decl)
      ...
      && !DECL_ARTIFICIAL (decl) && DECL_FUNCTION_SCOPE_P (decl)
      ...
      else
	{
	  if (DECL_INITIAL (decl) == NULL_TREE
	  ...
	  else
	    {
	      ...
	      DECL_COMMON (decl) = 0;
	      cp_warning_at
		("sorry: semantics of inline function static data `%#D' are wrong (you'll wind up with multiple copies)",
		 decl);
	      warning
...
check_for_uninitialized_const_var (tree decl)
{
  ...
  if (TREE_CODE (decl) == VAR_DECL
      ...
      && !TYPE_NEEDS_CONSTRUCTING (type) && !DECL_INITIAL (decl))
		    ...
		    tree * initp, tree new_init)
{
  ...
  for (index = 0;
       *initp && (!sized_array_p || index <= max_index_cst); ++index)
    {
      tree element_init;
  ...
  if (!CP_AGGREGATE_TYPE_P (type))
    {
      *initp = TREE_CHAIN (old_init);
      ...
      if (brace_enclosed_p)
	{
	  error ("brace-enclosed initializer used to initialize `%T'", type);
  ...
  if (TREE_CODE (old_init_value) == STRING_CST
	  ...
	  if (!field)
	    {
	      ...
	      if (!brace_enclosed_p)
		{
		  error ("initializer for `%T' must be brace-enclosed", type);
		  return error_mark_node;
		}
	    }
	  else
	       ...
	       || (TREE_CODE (type) == VECTOR_TYPE))
	{
	  ...
	  max_index =
	    ((TYPE_DOMAIN (type)
	      && (TREE_CODE (type) ==
		  ARRAY_TYPE)) ? array_type_nelts (type) : NULL_TREE);
	  if (!reshape_init_array
	      (TREE_TYPE (type), max_index, initp, new_init))
	    return error_mark_node;
	}
      else
	abort ();
...
check_initializer (tree decl, tree init, int flags, tree * cleanup)
{
  ...
  if (TREE_CODE (decl) == CONST_DECL)
	...
	error ("structure `%D' with uninitialized reference members", decl);
...
initialize_local_var (tree decl, tree init)
{
  ...
  my_friendly_assert (TREE_CODE (decl) == VAR_DECL
		      || TREE_CODE (decl) == RESULT_DECL, 20021010);
  ...
  if (DECL_SIZE (decl) && type != error_mark_node)
      ...
      if (init)
	{
      ...
      if (TYPE_NEEDS_CONSTRUCTING (type)
	  && !already_used
	  && TYPE_HAS_TRIVIAL_DESTRUCTOR (type) && DECL_NAME (decl))
	TREE_USED (decl) = 0;
...
cp_finish_decl (tree decl, tree init, tree asmspec_tree, int flags)
{
  ...
  if (init && TREE_CODE (init) == NAMESPACE_DECL)
    {
      error ("cannot initialize `%D' to namespace `%D'", decl, init);
      init = NULL_TREE;
    }
  ...
  if (TREE_CODE (decl) == VAR_DECL
      ...
      && DECL_CONTEXT (decl) != current_namespace && init)
    {
  ...
  if (TREE_CODE (decl) == TYPE_DECL)
    {
      if (type != error_mark_node && IS_AGGR_TYPE (type) && DECL_NAME (decl))
	{
  ...
  if (TREE_CODE (decl) == VAR_DECL || TREE_CODE (decl) == FUNCTION_DECL)
	...
	abstract_virtuals_error (decl, strip_array_types (TREE_TYPE (type)));
	...
	if (context && TYPE_P (context) && (TREE_CODE (decl) == VAR_DECL
	    ...
	    /* If size hasn't been set, we're still defining it,
	       ...
	       the binding level..  */
	    && COMPLETE_TYPE_P (context) && context == current_class_type)
	  pop_nested_class ();
      }
    }
...
start_handler_parms (tree declspecs, tree declarator)
{
  ...
  if (declspecs)
    {
      decl = grokdeclarator (declarator, declspecs, CATCHPARM, 1, NULL);
      if (decl == NULL_TREE)
...
member_function_or_else (tree ctype, tree cur_type, enum overload_flags flags)
{
  if (ctype && ctype != cur_type)
    {
      if (flags == DTOR_FLAG)
	error ("destructor for alien class `%T' cannot be a member", ctype);
      else
	error ("constructor for alien class `%T' cannot be a member", ctype);
      return 0;
    }
  return 1;
}
...
bad_specifiers (tree object,
		const char *type,
		int virtualp, int quals, int inlinep, int friendp, int raises)
{
  if (virtualp)
...
grokfndecl (tree ctype,
	    ...
	    int publicp,
	    int inlinep,
	    int funcdef_flag, int template_count, tree in_namespace)
{
  tree decl;
  ...
  if ((MAIN_NAME_P (declarator)
	   ...
	   && strncmp (IDENTIFIER_POINTER (declarator) + 2, "builtin_",
		       8) == 0)) && current_lang_name == lang_name_cplusplus
      && ctype == NULL_TREE
	  ...
	  if (TYPE_ANONYMOUS_P (t))
	      ...
	      else
		{
		  ...
		  if (DECL_ORIGINAL_TYPE (TYPE_NAME (t)))
		    cp_pedwarn_at ("\
`%#D' does not refer to the unqualified type, so it is not used for linkage", TYPE_NAME (t));
		}
	    }
	  else
  ...
  for (t = TYPE_ARG_TYPES (TREE_TYPE (decl)); t; t = TREE_CHAIN (t))
    if (TREE_PURPOSE (t) && TREE_CODE (TREE_PURPOSE (t)) == DEFAULT_ARG)
      {
	has_default_arg = 1;
	break;
      }
  ...
  if (friendp && TREE_CODE (orig_declarator) == TEMPLATE_ID_EXPR)
      ...
      else
	{
	  ...
	  if (has_default_arg)
	    {
	      error
		("default arguments are not allowed in declaration of friend template specialization `%D'",
		 decl);
	      return NULL_TREE;
	    }
  ...
  if (ctype != NULL_TREE
      && (!TYPE_FOR_JAVA (ctype) || check_java_method (decl)) && check)
    {
      tree old_decl;
	     ...
	     int initialized, int constp, tree scope)
{
  ...
  my_friendly_assert (!name || TREE_CODE (name) == IDENTIFIER_NODE, 20020808);
  ...
  if (scope && (		/* If the variable is a namespace-scope variable declared in a
...
build_ptrmemfunc_type (tree type)
{
  ...
  if (cp_type_quals (type) != TYPE_UNQUALIFIED)
    unqualified_variant = build_ptrmemfunc_type (TYPE_MAIN_VARIANT (type));
...
build_ptrmem_type (tree class_type, tree member_type)
{
  if (TREE_CODE (member_type) == METHOD_TYPE)
    ...
    {
      my_friendly_assert (TREE_CODE (member_type) != FUNCTION_TYPE, 20030716);
...
check_static_variable_definition (tree decl, tree type)
{
  ...
  if (!ARITHMETIC_TYPE_P (type) && TREE_CODE (type) != ENUMERAL_TYPE)
    {
      error
	("invalid in-class initialization of static data member of non-integral type `%T'",
	 type);
    ...
    pedwarn
      ("ISO C++ forbids initialization of member constant `%D' of non-integral type `%T'",
       decl, type);
...
get_scope_of_declarator (tree declarator)
{
  ...
  switch (TREE_CODE (declarator))
    ...
    case ADDR_EXPR:
      ...
      return get_scope_of_declarator (TREE_OPERAND (declarator, 0));

    case SCOPE_REF:
      /* For a pointer-to-member, continue descending.  */
      if (TREE_CODE (TREE_OPERAND (declarator, 1)) == INDIRECT_REF)
	return get_scope_of_declarator (TREE_OPERAND (declarator, 1));
  ...
  if (TREE_CODE (type) == ARRAY_TYPE && !TYPE_DOMAIN (type))
    {
      if (name)
	error
	  ("declaration of `%D' as multidimensional array must have bounds for all dimensions except the first",
	   name);
      else
	error
		...
		int initialized, tree * attrlist)
{
     ...
     and get it as a string, for an error message.  */
  {
	...
	switch (TREE_CODE (decl))
	  ...
	  case TEMPLATE_ID_EXPR:
		...
		{
		  my_friendly_assert (is_overloaded_fn (dname), 19990331);
  ...
  if (((dname && IDENTIFIER_OPNAME_P (dname)) || flags == TYPENAME_FLAG)
      && innermost_code != CALL_EXPR && !(ctype && declspecs == NULL_TREE))
    {
  ...
  for (spec = declspecs; spec; spec = TREE_CHAIN (spec))
      ...
      if (TREE_CODE (id) == IDENTIFIER_NODE)
	{
	  ...
	  for (i = (int) RID_FIRST_MODIFIER; i <= (int) RID_LAST_MODIFIER;
	       i++)
	    {
	      if (ridpointers[i] == id)
		{
  ...
  if (sfk != sfk_none)
    type = check_special_function_return_type (sfk, type, ctor_return_type);
	...
	pedwarn ("ISO C++ forbids declaration of `%s' with no type", name);
      else if (warn_return_type)
	warning ("ISO C++ forbids declaration of `%s' with no type", name);
  ...
  if (RIDBIT_SETP (RID_UNSIGNED, specbits)
	...
	error ("long and short specified together for `%s'", name);
		...
		|| RIDBIT_SETP (RID_SHORT, specbits)) && explicit_char)
  ...
  else if (RIDBIT_SETP (RID_SIGNED, specbits) && type == char_type_node)
  ...
  if (RIDBIT_ANY_SET (specbits))
    {
      if (RIDBIT_SETP (RID_STATIC, specbits))
	nclasses++;
      if (RIDBIT_SETP (RID_EXTERN, specbits) && !extern_langp)
	nclasses++;
      if (RIDBIT_SETP (RID_THREAD, specbits))
	nclasses++;
  ...
  if (virtualp && (current_class_name == NULL_TREE || decl_context != FIELD))
    {
  ...
  while (declarator && TREE_CODE (declarator) != IDENTIFIER_NODE
      ...
      if (type == error_mark_node)
	{
      ...
      if (quals != NULL_TREE
	  && (declarator == NULL_TREE || TREE_CODE (declarator) != SCOPE_REF))
	{
	...
	case CALL_EXPR:
	  {
	    ...
	    if (TREE_CODE (type) == FUNCTION_TYPE)
	      {
		error ("`%s' declared as function returning a function",
		       name);
		type = integer_type_node;
	      }
	    if (TREE_CODE (type) == ARRAY_TYPE)
  ...
  if (TREE_CODE (type) == ARRAY_TYPE
      && COMPLETE_TYPE_P (type) && TREE_OVERFLOW (TYPE_SIZE (type)))
    {
  ...
  if ((decl_context == FIELD || decl_context == PARM)
      && !processing_template_decl && variably_modified_type_p (type))
    {
      if (decl_context == FIELD)
  ...
  if (RIDBIT_SETP (RID_TYPEDEF, specbits) && decl_context != TYPENAME)
	  ...
	  && cp_type_quals (type) == TYPE_UNQUALIFIED)
	{
	  ...
	  if (TYPE_LANG_SPECIFIC (type) && CLASSTYPE_TEMPLATE_INFO (type))
	    DECL_NAME (CLASSTYPE_TI_TEMPLATE (type)) = TYPE_IDENTIFIER (type);
  ...
  if (decl_context == TYPENAME)
	  ...
	  if (type != integer_type_node)
	    {
	      /* A friendly class?  */
	      if (current_class_type)
		make_friend_class (current_class_type,
      ...
      else if (quals)
	{
      ...
      return type;
	   ...
	   && TREE_CODE (type) != UNION_TYPE && !bitfield)
    {
...

  {
	...
	if (type == error_mark_node)
	...
	else if (TREE_CODE (type) == FUNCTION_TYPE)
	  {
	    ...
	    if (friendp && declarator == ridpointers[(int) RID_SIGNED])
	      {
		error ("function `%D' cannot be declared friend", declarator);
		friendp = 0;
	      }
	    ...
	    publicp = (!friendp || !staticp) && function_context == NULL_TREE;
	...
	else
	  {
	    if (friendp)
	      {
		error
		  ("`%s' is neither function nor member function; cannot be declared friend",
	...
	if (decl == NULL_TREE)
	  {
		...
		if (uses_template_parms (type))
		  ...
		  ;
		else if (check_static_variable_definition (declarator, type))
	...
	if (!toplevel_bindings_p ()
	    && (RIDBIT_SETP (RID_STATIC, specbits)
		|| RIDBIT_SETP (RID_INLINE, specbits)) && pedantic)
	  {
	    if (RIDBIT_SETP (RID_STATIC, specbits))
	      pedwarn
...
local_variable_p (tree t)
{
  if ((TREE_CODE (t) == VAR_DECL
       ...
       && !DECL_NAMESPACE_SCOPE_P (t)) || (TREE_CODE (t) == PARM_DECL))
    return 1;
...
nonstatic_local_decl_p (tree t)
{
  return ((local_variable_p (t) && !TREE_STATIC (t))
	  || TREE_CODE (t) == LABEL_DECL || TREE_CODE (t) == RESULT_DECL);
}
...
check_default_argument (tree decl, tree arg)
{
  ...
  if (arg == error_mark_node
      ...
      || TREE_TYPE (arg) == error_mark_node || decl_type == error_mark_node)
  ...
  /* [dcl.fct.default]
     ...
     parameter type.  */
  if (!TREE_TYPE (arg) || !can_convert_arg (decl_type, TREE_TYPE (arg), arg))
    {
      if (decl)
  ...
  /* [dcl.fct.default]
     ...
     member function.  */
  var = walk_tree_without_duplicates (&arg, local_variable_p_walkfn, NULL);
  if (var)
    {
      error ("default argument `%E' uses local variable `%D'", arg, var);
      return error_mark_node;
    }
  ...
  for (parm = first_parm; parm != NULL_TREE; parm = chain)
    {
      ...
      if (TREE_CODE (decl) != VOID_TYPE && TREE_CODE (decl) != TREE_LIST)
	{
...
unary_op_p (enum tree_code code)
{
  return (code == TRUTH_NOT_EXPR
	  || code == BIT_NOT_EXPR
	  || code == COMPONENT_REF || code == TYPE_EXPR);
}
...
grok_op_properties (tree decl, bool complain)
{
  ...
  for (argtype = argtypes, arity = 0;
       argtype && argtype != void_list_node; argtype = TREE_CHAIN (argtype))
    ++arity;
  ellipsis_p = !argtype;
  ...
  if (operator_code == NEW_EXPR || operator_code == VEC_NEW_EXPR)
      ...
      if (!methodp || DECL_STATIC_FUNCTION_P (decl))
	{
	  if (operator_code == TYPE_EXPR
	      ...
	      || operator_code == ARRAY_REF || operator_code == NOP_EXPR)
	...
	return ok;
	  ...
	  && !DECL_TEMPLATE_INFO (decl) && warn_conversion
...
check_elaborated_type_specifier (enum tag_types tag_code,
				 tree decl, bool allow_template_p)
{
  tree type;
  ...
  if (!DECL_IMPLICIT_TYPEDEF_P (decl) && tag_code != typename_type)
    {
  ...
  if (TREE_CODE (type) == TEMPLATE_TYPE_PARM)
    {
      ...
      return error_mark_node;
	   ...
	   && tag_code != enum_type && tag_code != typename_type)
    {
      error ("`%T' referred to as `%s'", type, tag_name (tag_code));
      return error_mark_node;
    }
  else if (TREE_CODE (type) != ENUMERAL_TYPE && tag_code == enum_type)
    {
      error ("`%T' referred to as enum", type);
      return error_mark_node;
    }
  else if (!allow_template_p
	   && TREE_CODE (type) == RECORD_TYPE && CLASSTYPE_IS_TEMPLATE (type))
    {
      ...
      error ("template argument required for `%s %T'",
	     tag_name (tag_code), DECL_NAME (CLASSTYPE_TI_TEMPLATE (type)));
      return error_mark_node;
    }
	  ...
	  bool globalize, bool template_header_p)
{
  ...
  if (!globalize)
	...
	t = NULL_TREE;
	  ...
	  && template_class_depth (current_class_type) && template_header_p)
	{
  ...
  if (!t)
	...
	redeclare_class_template (t, current_template_parms);
	       ...
	       && CLASS_TYPE_P (t) && CLASSTYPE_IS_TEMPLATE (t))
	{
...
xref_basetypes (tree ref, tree base_list)
{
  ...
  if (i)
      ...
      for (i = 0; base_list; base_list = TREE_CHAIN (base_list))
	{
	  ...
	  if (CLASS_TYPE_P (basetype))
	    {
	      TYPE_HAS_NEW_OPERATOR (ref) |= TYPE_HAS_NEW_OPERATOR (basetype);
...
finish_enum (tree enumtype)
{
  ...
  for (itk = (flag_short_enums ? itk_char : itk_int); itk != itk_none; itk++)
    {
...
check_function_type (tree decl, tree current_function_parms)
{
  ...
  if (!COMPLETE_OR_VOID_TYPE_P (return_type))
	...
	TREE_TYPE (decl)
      ...
      TREE_TYPE (decl)
	= build_exception_variant (fntype, TYPE_RAISES_EXCEPTIONS (fntype));
    }
  else
...
start_function (tree declspecs, tree declarator, tree attrs, int flags)
{
  ...
  /* Sanity check.  */
  my_friendly_assert (TREE_CODE (TREE_VALUE (void_list_node)) == VOID_TYPE,
		      160);
  ...
  if (DECL_DECLARED_INLINE_P (decl1) && lookup_attribute ("noinline", attrs))
  ...
  if (!warn_implicit
    ...
    cp_warning_at ("`%D' implicitly declared before its definition",
  ...
  if (ctype && !doing_friend && !DECL_STATIC_FUNCTION_P (decl1))
    {
      ...
      my_friendly_assert (t != NULL_TREE && TREE_CODE (t) == PARM_DECL, 162);
      my_friendly_assert (TREE_CODE (TREE_TYPE (t)) == POINTER_TYPE,
			  19990811);

      cp_function_chain->x_current_class_ref = build_indirect_ref (t, NULL);
  ...
  else if (interface_unknown == 0 && !DECL_TEMPLATE_INSTANTIATION (decl1))
    {
      if (DECL_DECLARED_INLINE_P (decl1)
	  || DECL_TEMPLATE_INSTANTIATION (decl1) || processing_template_decl)
	{
	  DECL_EXTERNAL (decl1)
	    = (interface_only
	       || (DECL_DECLARED_INLINE_P (decl1)
		   && !flag_implement_inlines && !DECL_VINDEX (decl1)));
...
save_function_data (tree decl)
{
  ...
  my_friendly_assert (!DECL_PENDING_INLINE_P (decl), 19990908);
...
finish_function (int flags)
{
  ...
  if (DECL_NONSTATIC_MEMBER_FUNCTION_P (fndecl)
      && DECL_VIRTUAL_P (fndecl) && !processing_template_decl)
    {
  ...
  if (!processing_template_decl
      && !cp_function_chain->can_throw && !flag_non_call_exceptions)
...
start_method (tree declspecs, tree declarator, tree attrlist)
{
  ...
  if (DECL_IN_AGGR_P (fndecl))
    {
      if (IDENTIFIER_ERROR_LOCUS (DECL_ASSEMBLER_NAME (fndecl)) !=
	  current_class_type)
	{
...
cxx_maybe_build_cleanup (tree decl)
{
  ...
  if (type != error_mark_node && TYPE_HAS_NONTRIVIAL_DESTRUCTOR (type))
	...
	flags |= LOOKUP_NONVIRTUAL;
      ...
      if (TYPE_USES_VIRTUAL_BASECLASSES (type) && !TYPE_HAS_DESTRUCTOR (type))
...
revert_static_member_fn (tree decl)
{
  ...
  if (cp_type_quals (TREE_TYPE (TREE_VALUE (args))) != TYPE_UNQUALIFIED)
  ...
  tmp = build_exception_variant (tmp, TYPE_RAISES_EXCEPTIONS (function));
  TREE_TYPE (decl) = tmp;
