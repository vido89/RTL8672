...
cxx_print_decl (FILE * file, tree node, int indent)
{
  ...
  if (!DECL_LANG_SPECIFIC (node))
    return;
  indent_to (file, indent + 3);
  if (TREE_CODE (node) == FUNCTION_DECL && DECL_PENDING_INLINE_INFO (node))
    fprintf (file, " pending-inline-info " HOST_PTR_PRINTF,
	     (void *) DECL_PENDING_INLINE_INFO (node));
  if (TREE_CODE (node) == TYPE_DECL && DECL_SORTED_FIELDS (node))
...
cxx_print_type (FILE * file, tree node, int indent)
{
  switch (TREE_CODE (node))
	...
	print_node (file, "throws", TYPE_RAISES_EXCEPTIONS (node),
		    indent + 4);
      return;
...
{
  fprintf (stream, "%s <" HOST_PTR_PRINTF ">", prefix, (void *) binding);
}
