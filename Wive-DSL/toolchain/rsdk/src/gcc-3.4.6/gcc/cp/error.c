...
dump_qualifiers (tree t, enum pad p)
{
  ...
  static const char *const names[] = { "const", "volatile", "__restrict" };
  int ix;
...
dump_template_decl (tree t, int flags)
{
  ...
  if (TREE_CODE (DECL_TEMPLATE_RESULT (t)) == TYPE_DECL)
	...
	dump_type (TREE_TYPE (t),
		   (flags & ~TFF_CLASS_KEY_OR_ENUM) | TFF_TEMPLATE_NAME
		   | (flags & TFF_DECL_SPECIFIERS ? TFF_CLASS_KEY_OR_ENUM :
		      0));
      }
}
...
dump_function_name (tree t, int flags)
{
  ...
  if (DECL_LANG_SPECIFIC (t) && DECL_TEMPLATE_INFO (t)
    ...
    dump_template_parms (DECL_TEMPLATE_INFO (t), !DECL_USE_TEMPLATE (t),
			 flags);
}
...
dump_expr (tree t, int flags)
{
  ...
  switch (TREE_CODE (t))
    ...
    case ADDR_EXPR:
	  ...
	  || (TREE_TYPE (t) && TREE_CODE (TREE_TYPE (t)) == REFERENCE_TYPE))
	dump_expr (TREE_OPERAND (t, 0), flags | TFF_EXPR_IN_PARENS);
      ...
      if (TREE_TYPE (t) && TYPE_PTRMEMFUNC_P (TREE_TYPE (t)))
	{
	  ...
	  if (integer_zerop (idx))
	      ...
	      while (n > 0 && virtuals)
		{
	      ...
	      if (virtuals)
		{
		  dump_expr (BV_FN (virtuals), flags | TFF_EXPR_IN_PARENS);
		  break;
		}
	    }
	}
			 ...
			 diagnostic_info * diagnostic)
{
  if (diagnostic_last_function_changed (context))
	...
	? file_name_as_prefix (diagnostic->location.file) : NULL;
...
#define next_int     va_arg (*text->args_ptr, int)
  ...
  switch (*text->format_spec)
    {
    case 'A':
      result = args_to_string (next_tree, verbose);
      break;
    case 'C':
      result = code_to_string (next_tcode);
      break;
    case 'D':
      result = decl_to_string (next_tree, verbose);
      break;
    case 'E':
      result = expr_to_string (next_tree);
      break;
    case 'F':
      result = fndecl_to_string (next_tree, verbose);
      break;
    case 'L':
      result = language_to_string (next_lang);
      break;
    case 'O':
      result = op_to_string (next_tcode);
      break;
    case 'P':
      result = parm_to_string (next_int);
      break;
    case 'Q':
      result = assop_to_string (next_tcode);
      break;
    case 'T':
      result = type_to_string (next_tree, verbose);
      break;
    case 'V':
      result = cv_to_string (next_tree, verbose);
      break;
      ...
      if (*f == '%')
	{
	  ...
	  switch (*f)
	    ...
	    case 's':
	      ...
	      break;
	    case 'L':
	      (void) va_arg (ap, enum languages);
	      break;
	    case 'C':
	    case 'O':
	    case 'Q':
	      (void) va_arg (ap, enum tree_code);
	      break;
...
cp_error_at (const char *msgid, ...)
{
  ...
  diagnostic_set_info (&diagnostic, msgid, &ap, location_of (here), DK_ERROR);
