...
decl_is_java_type (tree decl, int err)
{
  ...
  if (err)
	  ...
	  && TYPE_FOR_JAVA (TREE_TYPE (decl)))
	{
	  ...
	  error ("type `%T' is disallowed in Java `throw' or `catch'", decl);
	}
...
initialize_handler_parm (tree decl, tree exp)
{
  ...
  if (!TYPE_PTR_P (init_type) && TREE_CODE (init_type) != REFERENCE_TYPE)
  ...
  start_decl_1 (decl);
  cp_finish_decl (decl, init, NULL_TREE, LOOKUP_ONLYCONVERTING | DIRECT_BIND);
}
...
is_admissible_throw_operand (tree expr)
{
  ...
  /* 10.4/3 An abstract class shall not be used as a parameter type,
    ...
    {
      error
	("expression '%E' of abstract class type '%T' cannot be used in throw-expression",
	 expr, type);
      return false;
    }
...
nothrow_libfn_p (tree fn)
{
  ...
  if (TREE_PUBLIC (fn)
      && DECL_EXTERNAL (fn)
      && DECL_NAMESPACE_SCOPE_P (fn) && DECL_EXTERN_C_P (fn))
    /* OK */ ;
  else
