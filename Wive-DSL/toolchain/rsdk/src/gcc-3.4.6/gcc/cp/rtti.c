...
init_rtti_processing (void)
{
  ...
  type_info_type_node
    = xref_tag (class_type, get_identifier ("type_info"), true, false);
  pop_namespace ();
...
build_typeid (tree exp)
{
  ...
  if (TREE_CODE (exp) == INDIRECT_REF
      ...
      && !resolves_to_fixed_type_p (exp, &nonnull) && !nonnull)
    {
...
get_tinfo_ptr (tree type)
{
  ...
  mark_used (decl);
  return build_nop (type_info_ptr_type, build_address (decl));
}
...
ifnonnull (tree test, tree result)
{
  return build (COND_EXPR, TREE_TYPE (result),
		...
		cp_convert (TREE_TYPE (result), integer_zero_node), result);
}
...
build_dynamic_cast_1 (tree type, tree expr)
{
  ...
  if (!at_least_as_qualified_p (TREE_TYPE (type), TREE_TYPE (exprtype)))
    {
  ...
  if (TYPE_POLYMORPHIC_P (TREE_TYPE (exprtype)))
      ...
      else
	{
	  ...
	  elems = tree_cons
	    (NULL_TREE, expr1, tree_cons
	     (NULL_TREE, td3, tree_cons
	      (NULL_TREE, td2, tree_cons (NULL_TREE, boff, NULL_TREE))));
	  ...
	  if (!dcast_fn)
		...
		(build_qualified_type (tinfo_ptr, TYPE_QUAL_CONST));
	      name = "__dynamic_cast";
	      tmp = tree_cons
  ...
  if (!vtable_ptr)
    {
      ...
      real_type = xref_tag (class_type, TINFO_REAL_NAME (desc), true, false);
...
ptr_initializer (tree desc, tree target, bool * non_public_ptr)
{
  ...
  init = tree_cons (NULL_TREE, get_tinfo_ptr (TYPE_MAIN_VARIANT (to)), init);
...
ptm_initializer (tree desc, tree target, bool * non_public_ptr)
{
  ...
  init = tree_cons (NULL_TREE, build_int_2 (flags, 0), init);
  init = tree_cons (NULL_TREE, get_tinfo_ptr (TYPE_MAIN_VARIANT (to)), init);
  init = tree_cons (NULL_TREE, get_tinfo_ptr (klass), init);
      ...
      else
	{
	  ...
	  if (num_bases == 1
		...
		("__vmi_class_type_info", num_bases,
		 ...
		 build_decl (FIELD_DECL, NULL_TREE, integer_type_node),
		 build_decl (FIELD_DECL, NULL_TREE, base_array), NULL);
	      pop_nested_namespace (abi_node);
...
create_tinfo_types (void)
{
  ...
  bltn_desc_type_node = create_pseudo_type_info
    ("__fundamental_type_info", 0, NULL);

  /* Array, function and enum type_info. No additional fields.  */
  ary_desc_type_node = create_pseudo_type_info ("__array_type_info", 0, NULL);
  func_desc_type_node = create_pseudo_type_info
    ("__function_type_info", 0, NULL);
  enum_desc_type_node = create_pseudo_type_info ("__enum_type_info", 0, NULL);

  /* Class type_info. Add a flags field.  */
  class_desc_type_node = create_pseudo_type_info
    ("__class_type_info", 0, NULL);

  /* Single public non-virtual base class. Add pointer to base class. 
     This is really a descendant of __class_type_info.  */
  si_class_desc_type_node = create_pseudo_type_info
    ("__si_class_type_info", 0,
     build_decl (FIELD_DECL, NULL_TREE, type_info_ptr_type), NULL);
     ...
     flags.  */
  {
    ...
    finish_builtin_struct (base_desc_type_node,
			   "__base_class_type_info_pseudo", fields,
			   NULL_TREE);
