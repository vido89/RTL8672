...
static cp_token_cache *cp_token_cache_new (void);
static void cp_token_cache_push_token (cp_token_cache *, cp_token *);
...
static void
cp_token_cache_push_token (cp_token_cache * cache, cp_token * token)
{
...
static cp_lexer *cp_lexer_new_main (void);
static cp_lexer *cp_lexer_new_from_tokens (struct cp_token_cache *);
static int cp_lexer_saving_tokens (const cp_lexer *);
static cp_token *cp_lexer_next_token (cp_lexer *, cp_token *);
static cp_token *cp_lexer_prev_token (cp_lexer *, cp_token *);
static ptrdiff_t cp_lexer_token_difference
  (cp_lexer *, cp_token *, cp_token *);
static cp_token *cp_lexer_read_token (cp_lexer *);
static void cp_lexer_maybe_grow_buffer (cp_lexer *);
static void cp_lexer_get_preprocessor_token (cp_lexer *, cp_token *);
static cp_token *cp_lexer_peek_token (cp_lexer *);
static cp_token *cp_lexer_peek_nth_token (cp_lexer *, size_t);
static inline bool cp_lexer_next_token_is (cp_lexer *, enum cpp_ttype);
static bool cp_lexer_next_token_is_not (cp_lexer *, enum cpp_ttype);
static bool cp_lexer_next_token_is_keyword (cp_lexer *, enum rid);
static cp_token *cp_lexer_consume_token (cp_lexer *);
static void cp_lexer_purge_token (cp_lexer *);
static void cp_lexer_purge_tokens_after (cp_lexer *, cp_token *);
static void cp_lexer_save_tokens (cp_lexer *);
static void cp_lexer_commit_tokens (cp_lexer *);
static void cp_lexer_rollback_tokens (cp_lexer *);
static inline void cp_lexer_set_source_position_from_token
  (cp_lexer *, const cp_token *);
static void cp_lexer_print_token (FILE *, cp_token *);
static inline bool cp_lexer_debugging_p (cp_lexer *);
static void cp_lexer_start_debugging (cp_lexer *) ATTRIBUTE_UNUSED;
...
static ptrdiff_t
cp_lexer_token_difference (cp_lexer * lexer, cp_token * start,
			   cp_token * finish)
{
  if (finish >= start)
    return finish - start;
  else
    return ((lexer->buffer_end - lexer->buffer) - (start - finish));
}
  ...
  if (lexer->last_token == lexer->first_token)
    {
      ...
      memcpy (new_buffer + buffer_length,
	      new_buffer, num_tokens_to_copy * sizeof (cp_token));
      ...
      new_first_token = new_buffer + (lexer->first_token - old_buffer);
				 ...
				 cp_token * token)
{
  ...
  if (token->type == CPP_NAME && C_IS_RESERVED_WORD (token->value))
    {
...
cp_lexer_consume_token (cp_lexer * lexer)
{
  ...
  lexer->next_token = cp_lexer_next_token (lexer, lexer->next_token);
...
static cp_parser_context *cp_parser_context_new (cp_parser_context *);

/* Class variables.  */

static
GTY ((deletable ("")))
     cp_parser_context *cp_parser_context_free_list;
...
static cp_parser *cp_parser_new (void);
...
static tree cp_parser_identifier (cp_parser *);

/* Basic concepts [gram.basic]  */

static bool cp_parser_translation_unit (cp_parser *);

/* Expressions [gram.expr]  */

static tree cp_parser_primary_expression (cp_parser *, cp_id_kind *, tree *);
static tree cp_parser_id_expression (cp_parser *, bool, bool, bool *, bool);
static tree cp_parser_unqualified_id (cp_parser *, bool, bool, bool);
...
static tree cp_parser_postfix_expression (cp_parser *, bool);
static tree cp_parser_parenthesized_expression_list
  (cp_parser *, bool, bool *);
static void cp_parser_pseudo_destructor_name (cp_parser *, tree *, tree *);
static tree cp_parser_unary_expression (cp_parser *, bool);
static enum tree_code cp_parser_unary_operator (cp_token *);
static tree cp_parser_new_expression (cp_parser *);
static tree cp_parser_new_placement (cp_parser *);
static tree cp_parser_new_type_id (cp_parser *);
static tree cp_parser_new_declarator_opt (cp_parser *);
static tree cp_parser_direct_new_declarator (cp_parser *);
static tree cp_parser_new_initializer (cp_parser *);
static tree cp_parser_delete_expression (cp_parser *);
static tree cp_parser_cast_expression (cp_parser *, bool);
static tree cp_parser_pm_expression (cp_parser *);
static tree cp_parser_multiplicative_expression (cp_parser *);
static tree cp_parser_additive_expression (cp_parser *);
static tree cp_parser_shift_expression (cp_parser *);
static tree cp_parser_relational_expression (cp_parser *);
static tree cp_parser_equality_expression (cp_parser *);
static tree cp_parser_and_expression (cp_parser *);
static tree cp_parser_exclusive_or_expression (cp_parser *);
static tree cp_parser_inclusive_or_expression (cp_parser *);
static tree cp_parser_logical_and_expression (cp_parser *);
static tree cp_parser_logical_or_expression (cp_parser *);
static tree cp_parser_question_colon_clause (cp_parser *, tree);
static tree cp_parser_assignment_expression (cp_parser *);
static enum tree_code cp_parser_assignment_operator_opt (cp_parser *);
static tree cp_parser_expression (cp_parser *);
static tree cp_parser_constant_expression (cp_parser *, bool, bool *);

/* Statements [gram.stmt.stmt]  */

static void cp_parser_statement (cp_parser *, bool);
static tree cp_parser_labeled_statement (cp_parser *, bool);
static tree cp_parser_expression_statement (cp_parser *, bool);
static tree cp_parser_compound_statement (cp_parser *, bool);
static void cp_parser_statement_seq_opt (cp_parser *, bool);
static tree cp_parser_selection_statement (cp_parser *);
static tree cp_parser_condition (cp_parser *);
static tree cp_parser_iteration_statement (cp_parser *);
static void cp_parser_for_init_statement (cp_parser *);
static tree cp_parser_jump_statement (cp_parser *);
static void cp_parser_declaration_statement (cp_parser *);

static tree cp_parser_implicitly_scoped_statement (cp_parser *);
static void cp_parser_already_scoped_statement (cp_parser *);

/* Declarations [gram.dcl.dcl] */

static void cp_parser_declaration_seq_opt (cp_parser *);
static void cp_parser_declaration (cp_parser *);
static void cp_parser_block_declaration (cp_parser *, bool);
static void cp_parser_simple_declaration (cp_parser *, bool);
static tree cp_parser_decl_specifier_seq
  (cp_parser *, cp_parser_flags, tree *, int *);
static tree cp_parser_storage_class_specifier_opt (cp_parser *);
static tree cp_parser_function_specifier_opt (cp_parser *);
static tree cp_parser_type_specifier
  (cp_parser *, cp_parser_flags, bool, bool, int *, bool *);
static tree cp_parser_simple_type_specifier
  (cp_parser *, cp_parser_flags, bool);
static tree cp_parser_type_name (cp_parser *);
static tree cp_parser_elaborated_type_specifier (cp_parser *, bool, bool);
static tree cp_parser_enum_specifier (cp_parser *);
static void cp_parser_enumerator_list (cp_parser *, tree);
static void cp_parser_enumerator_definition (cp_parser *, tree);
static tree cp_parser_namespace_name (cp_parser *);
static void cp_parser_namespace_definition (cp_parser *);
static void cp_parser_namespace_body (cp_parser *);
static tree cp_parser_qualified_namespace_specifier (cp_parser *);
static void cp_parser_namespace_alias_definition (cp_parser *);
static void cp_parser_using_declaration (cp_parser *);
static void cp_parser_using_directive (cp_parser *);
static void cp_parser_asm_definition (cp_parser *);
static void cp_parser_linkage_specification (cp_parser *);
...
static enum tree_code cp_parser_ptr_operator (cp_parser *, tree *, tree *);
static tree cp_parser_cv_qualifier_seq_opt (cp_parser *);
static tree cp_parser_cv_qualifier_opt (cp_parser *);
static tree cp_parser_declarator_id (cp_parser *);
static tree cp_parser_type_id (cp_parser *);
static tree cp_parser_type_specifier_seq (cp_parser *);
static tree cp_parser_parameter_declaration_clause (cp_parser *);
static tree cp_parser_parameter_declaration_list (cp_parser *);
static tree cp_parser_parameter_declaration (cp_parser *, bool, bool *);
static void cp_parser_function_body (cp_parser *);
static tree cp_parser_initializer (cp_parser *, bool *, bool *);
static tree cp_parser_initializer_clause (cp_parser *, bool *);
static tree cp_parser_initializer_list (cp_parser *, bool *);

static bool cp_parser_ctor_initializer_opt_and_function_body (cp_parser *);

/* Classes [gram.class] */

static tree cp_parser_class_name
  (cp_parser *, bool, bool, bool, bool, bool, bool);
static tree cp_parser_class_specifier (cp_parser *);
static tree cp_parser_class_head (cp_parser *, bool *, tree *);
static enum tag_types cp_parser_class_key (cp_parser *);
static void cp_parser_member_specification_opt (cp_parser *);
static void cp_parser_member_declaration (cp_parser *);
static tree cp_parser_pure_specifier (cp_parser *);
static tree cp_parser_constant_initializer (cp_parser *);

/* Derived classes [gram.class.derived] */

static tree cp_parser_base_clause (cp_parser *);
static tree cp_parser_base_specifier (cp_parser *);

/* Special member functions [gram.special] */

static tree cp_parser_conversion_function_id (cp_parser *);
static tree cp_parser_conversion_type_id (cp_parser *);
static tree cp_parser_conversion_declarator_opt (cp_parser *);
static bool cp_parser_ctor_initializer_opt (cp_parser *);
static void cp_parser_mem_initializer_list (cp_parser *);
static tree cp_parser_mem_initializer (cp_parser *);
static tree cp_parser_mem_initializer_id (cp_parser *);

/* Overloading [gram.over] */

static tree cp_parser_operator_function_id (cp_parser *);
static tree cp_parser_operator (cp_parser *);

/* Templates [gram.temp] */

static void cp_parser_template_declaration (cp_parser *, bool);
static tree cp_parser_template_parameter_list (cp_parser *);
static tree cp_parser_template_parameter (cp_parser *);
static tree cp_parser_type_parameter (cp_parser *);
static tree cp_parser_template_id (cp_parser *, bool, bool, bool);
static tree cp_parser_template_name (cp_parser *, bool, bool, bool, bool *);
static tree cp_parser_template_argument_list (cp_parser *);
static tree cp_parser_template_argument (cp_parser *);
static void cp_parser_explicit_instantiation (cp_parser *);
static void cp_parser_explicit_specialization (cp_parser *);

/* Exception handling [gram.exception] */

static tree cp_parser_try_block (cp_parser *);
static bool cp_parser_function_try_block (cp_parser *);
static void cp_parser_handler_seq (cp_parser *);
static void cp_parser_handler (cp_parser *);
static tree cp_parser_exception_declaration (cp_parser *);
static tree cp_parser_throw_expression (cp_parser *);
static tree cp_parser_exception_specification_opt (cp_parser *);
static tree cp_parser_type_id_list (cp_parser *);

/* GNU Extensions */

static tree cp_parser_asm_specification_opt (cp_parser *);
static tree cp_parser_asm_operand_list (cp_parser *);
static tree cp_parser_asm_clobber_list (cp_parser *);
static tree cp_parser_attributes_opt (cp_parser *);
static tree cp_parser_attribute_list (cp_parser *);
static bool cp_parser_extension_opt (cp_parser *, int *);
static void cp_parser_label_declaration (cp_parser *);

/* Utility Routines */

static tree cp_parser_lookup_name (cp_parser *, tree, bool, bool, bool, bool);
static tree cp_parser_lookup_name_simple (cp_parser *, tree);
static tree cp_parser_maybe_treat_template_as_class (tree, bool);
static bool cp_parser_check_declarator_template_parameters
  (cp_parser *, tree);
static bool cp_parser_check_template_parameters (cp_parser *, unsigned);
static tree cp_parser_simple_cast_expression (cp_parser *);
static tree cp_parser_binary_expression
  (cp_parser *, const cp_parser_token_tree_map, cp_parser_expression_fn);
static tree cp_parser_global_scope_opt (cp_parser *, bool);
static bool cp_parser_constructor_declarator_p (cp_parser *, bool);
static tree cp_parser_function_definition_from_specifiers_and_declarator
  (cp_parser *, tree, tree, tree);
static tree cp_parser_function_definition_after_declarator
  (cp_parser *, bool);
static void cp_parser_template_declaration_after_export (cp_parser *, bool);
static tree cp_parser_single_declaration (cp_parser *, bool, bool *);
static tree cp_parser_functional_cast (cp_parser *, tree);
static tree cp_parser_save_member_function_body
  (cp_parser *, tree, tree, tree);
static tree cp_parser_enclosed_template_argument_list (cp_parser *);
...
static tree cp_parser_sizeof_operand (cp_parser *, enum rid);
static bool cp_parser_declares_only_class_p (cp_parser *);
static bool cp_parser_friend_p (tree);
static bool cp_parser_typedef_p (tree);
static cp_token *cp_parser_require
  (cp_parser *, enum cpp_ttype, const char *);
static cp_token *cp_parser_require_keyword
  (cp_parser *, enum rid, const char *);
static bool cp_parser_token_starts_function_definition_p (cp_token *);
static bool cp_parser_next_token_starts_class_definition_p (cp_parser *);
static bool cp_parser_next_token_ends_template_argument_p (cp_parser *);
static bool cp_parser_nth_token_starts_template_argument_list_p
  (cp_parser *, size_t);
static enum tag_types cp_parser_token_is_class_key (cp_token *);
static void cp_parser_check_class_key (enum tag_types, tree type);
static void cp_parser_check_access_in_redeclaration (tree type);
static bool cp_parser_optional_template_keyword (cp_parser *);
static void cp_parser_pre_parsed_nested_name_specifier (cp_parser *);
static void cp_parser_cache_group
  (cp_parser *, cp_token_cache *, enum cpp_ttype, unsigned);
static void cp_parser_parse_tentatively (cp_parser *);
static void cp_parser_commit_to_tentative_parse (cp_parser *);
static void cp_parser_abort_tentative_parse (cp_parser *);
static bool cp_parser_parse_definitely (cp_parser *);
static inline bool cp_parser_parsing_tentatively (cp_parser *);
static bool cp_parser_committed_to_tentative_parse (cp_parser *);
static void cp_parser_error (cp_parser *, const char *);
static void cp_parser_name_lookup_error
  (cp_parser *, tree, tree, const char *);
static bool cp_parser_simulate_error (cp_parser *);
static void cp_parser_check_type_definition (cp_parser *);
static void cp_parser_check_for_definition_in_return_type (tree, tree);
static void cp_parser_check_for_invalid_template_id (cp_parser *, tree);
static bool cp_parser_non_integral_constant_expression
  (cp_parser *, const char *);
static bool cp_parser_diagnose_invalid_type_name (cp_parser *);
static int cp_parser_skip_to_closing_parenthesis
  (cp_parser *, bool, bool, bool);
static void cp_parser_skip_to_end_of_statement (cp_parser *);
static void cp_parser_consume_semicolon_at_end_of_statement (cp_parser *);
static void cp_parser_skip_to_end_of_block_or_statement (cp_parser *);
static void cp_parser_skip_to_closing_brace (cp_parser *);
static void cp_parser_skip_until_found
  (cp_parser *, enum cpp_ttype, const char *);
static bool cp_parser_error_occurred (cp_parser *);
static bool cp_parser_allow_gnu_extensions_p (cp_parser *);
static bool cp_parser_is_string_literal (cp_token *);
static bool cp_parser_is_keyword (cp_token *, enum rid);
...
cp_parser_name_lookup_error (cp_parser * parser,
			     tree name, tree decl, const char *desired)
{
  ...
  if (decl == error_mark_node)
	...
	error ("`%D::%D' has not been declared", parser->scope, name);
...
static void
cp_parser_check_for_invalid_template_id (cp_parser * parser, tree type)
{
  ptrdiff_t start;
  cp_token *token;
  ...
  if (cp_lexer_next_token_is (parser->lexer, CPP_LESS))
      ...
      if (start >= 0)
	{
	  token = cp_lexer_advance_token (parser->lexer,
					  parser->lexer->first_token, start);
...
cp_parser_skip_to_closing_parenthesis (cp_parser * parser,
				       bool recovering,
				       bool or_comma, bool consume_paren)
{
...
cp_parser_primary_expression (cp_parser * parser,
			      cp_id_kind * idk, tree * qualifying_class)
{
  cp_token *token;
	...
	/* Look up the name.  */
	else
	  {
		...
		if (local_variable_p (decl))
		  {
		    error
		      ("local variable `%D' may not appear in this context",
		       decl);
		    return error_mark_node;
		  }
	      }
			 ...
			 bool * template_p, bool declarator_p)
{
  ...
  nested_name_specifier_p = (cp_parser_nested_name_specifier_opt (parser,
								  /*typename_keyword_p= */
								  false,
								  check_dependency_p,
								  /*type_p= */
...
cp_parser_unqualified_id (cp_parser * parser,
			  bool template_keyword_p,
			  bool check_dependency_p, bool declarator_p)
{
  cp_token *token;
  ...
  switch (token->type)
	...
	id = cp_parser_template_id (parser, template_keyword_p,
				    check_dependency_p, declarator_p);
...
cp_parser_nested_name_specifier_opt (cp_parser * parser,
				     ...
				     bool check_dependency_p,
				     bool type_p, bool is_declaration)
{
  bool success = false;
  ...
  if (cp_parser_parsing_tentatively (parser)
      && !cp_parser_committed_to_tentative_parse (parser))
    {
      token = cp_lexer_peek_token (parser->lexer);
      start = cp_lexer_token_difference (parser->lexer,
					 parser->lexer->first_token, token);
    }
  else
    start = -1;
  ...
  while (true)
	...
	template_keyword_p = false;
      ...
      if (is_declaration
	  && !typename_keyword_p
	  && parser->scope && TREE_CODE (parser->scope) == TYPENAME_TYPE)
	  ...
	  while (cp_lexer_next_token_is (parser->lexer, CPP_NAME)
	      ...
	      if (!error_p)
		{
		  ...
		  if (TREE_CODE (decl) == TEMPLATE_DECL)
		    error ("`%D' used without template parameters", decl);
		  else
...
cp_parser_postfix_expression (cp_parser * parser, bool address_p)
{
  ...
  switch (keyword)
	...
	switch (keyword)
	  {
	  case RID_DYNCAST:
	    postfix_expression = build_dynamic_cast (type, expression);
	    break;
	  case RID_STATCAST:
	    postfix_expression = build_static_cast (type, expression);
	    break;
	  case RID_REINTCAST:
	    postfix_expression = build_reinterpret_cast (type, expression);
	    break;
	  case RID_CONSTCAST:
	    postfix_expression = build_const_cast (type, expression);
	    break;
	  default:
	    abort ();
	  }
	...
	type = cp_parser_simple_type_specifier (parser, CP_PARSER_FLAGS_NONE,
						/*identifier_p= */ false);
  ...
  while (true)
	  ...
	  /* postfix-expression ( expression-list [opt] ) */
	  {
		...
		if (BASELINK_P (fn))
	      ...
	      /* A call to a static class member, or a namespace-scope
	         function.  */
	      postfix_expression = finish_call_expr (postfix_expression, args,
						     /*disallow_virtual= */
						     true,
						     koenig_p);
	    else
	...
	case CPP_PLUS_PLUS:
	  ...
	  postfix_expression
	    = finish_increment_expr (postfix_expression, POSTINCREMENT_EXPR);
	...
	case CPP_MINUS_MINUS:
	  ...
	  /* Generate a representation for the complete expression.  */
	  postfix_expression
	    = finish_increment_expr (postfix_expression, POSTDECREMENT_EXPR);
...
cp_parser_new_expression (cp_parser * parser)
{
  ...
  /* Look for the optional `::' operator.  */
  global_scope_p = (cp_parser_global_scope_opt (parser,
...
cp_parser_new_declarator_opt (cp_parser * parser)
{
  ...
  if (cp_parser_parse_definitely (parser))
	...
	declarator = make_pointer_declarator (cv_qualifier_seq, declarator);
      else
	declarator = make_reference_declarator (cv_qualifier_seq, declarator);
  ...
  if (cp_lexer_next_token_is (parser->lexer, CPP_OPEN_PAREN))
      ...
      if (cp_parser_parse_definitely (parser))
	{
	  ...
	  if (warn_old_style_cast
	      && !in_system_header
	      && !VOID_TYPE_P (type) && current_lang_name != lang_name_c)
...
cp_parser_multiplicative_expression (cp_parser * parser)
{
  ...
  return cp_parser_binary_expression (parser, map, cp_parser_pm_expression);
}
...
cp_parser_shift_expression (cp_parser * parser)
{
  ...
  return cp_parser_binary_expression (parser,
				      map, cp_parser_additive_expression);
}
...
cp_parser_relational_expression (cp_parser * parser)
{
  ...
  return cp_parser_binary_expression (parser,
				      map, cp_parser_shift_expression);
}
...
cp_parser_equality_expression (cp_parser * parser)
{
  ...
  return cp_parser_binary_expression (parser,
				      map, cp_parser_relational_expression);
}
...
cp_parser_and_expression (cp_parser * parser)
{
  ...
  return cp_parser_binary_expression (parser,
				      map, cp_parser_equality_expression);
}
...
cp_parser_exclusive_or_expression (cp_parser * parser)
{
  ...
  return cp_parser_binary_expression (parser, map, cp_parser_and_expression);
}
...
cp_parser_inclusive_or_expression (cp_parser * parser)
{
  ...
  return cp_parser_binary_expression (parser,
				      map, cp_parser_exclusive_or_expression);
}
...
cp_parser_logical_and_expression (cp_parser * parser)
{
  ...
  return cp_parser_binary_expression (parser,
				      map, cp_parser_inclusive_or_expression);
}
...
cp_parser_logical_or_expression (cp_parser * parser)
{
  ...
  return cp_parser_binary_expression (parser,
				      map, cp_parser_logical_and_expression);
}
...
cp_parser_question_colon_clause (cp_parser * parser, tree logical_or_expr)
{
  ...
  return build_x_conditional_expr (logical_or_expr, expr, assignment_expr);
}
  ...
  /* Otherwise, it must be that we are looking at a
      ...
      else
	{
	  ...
	  /* If it's an assignment-operator, we're using the second
	     production.  */
	  assignment_operator = cp_parser_assignment_operator_opt (parser);
  ...
  while (true)
    {
      ...
      /* Parse the next assignment-expression.  */
      assignment_expression = cp_parser_assignment_expression (parser);
...
cp_parser_statement (cp_parser * parser, bool in_statement_expr_p)
{
  ...
  if (!statement)
      ...
      if (cp_lexer_next_token_is_not (parser->lexer, CPP_SEMICOLON))
	{
      ...
      statement =
	cp_parser_expression_statement (parser, in_statement_expr_p);
    }
	...
	if (keyword == RID_IF)
	  {
		...
		else_stmt = cp_parser_implicitly_scoped_statement (parser);
...
cp_parser_jump_statement (cp_parser * parser)
{
  ...
  switch (keyword)
    {
    case RID_BREAK:
      if (!parser->in_switch_statement_p && !parser->in_iteration_statement_p)
	{
...
cp_parser_declaration (cp_parser * parser)
{
  ...
  if (token1.keyword == RID_EXTERN && cp_parser_is_string_literal (&token2))
  ...
  /* If the next token is `namespace', check for a named or unnamed
     namespace definition.  */
  else if (token1.keyword == RID_NAMESPACE && (	/* A named namespace definition.  */
...
static void
cp_parser_block_declaration (cp_parser * parser, bool statement_p)
{
  cp_token *token1;
  int saved_pedantic;
			      ...
			      bool function_definition_allowed_p)
{
  ...
  decl_specifiers
    = cp_parser_decl_specifier_seq (parser,
				    ...
				    &attributes, &declares_class_or_enum);
...
cp_parser_decl_specifier_seq (cp_parser * parser,
			      ...
			      tree * attributes, int *declares_class_or_enum)
{
  ...
  *attributes = NULL_TREE;
  ...
  while (true)
      ...
      if (!decl_spec && !constructor_p)
	{
	  ...
	  decl_spec = cp_parser_type_specifier (parser, flags, friend_p,
						/*is_declaration= */ true,
...
cp_parser_conversion_declarator_opt (cp_parser * parser)
{
  ...
  code = cp_parser_ptr_operator (parser, &class_type, &cv_qualifier_seq);
  ...
  while (true)
    {
      ...
      /* Add it to the list.  */
      parameter_list = process_template_parm (parameter_list, parameter);
...
cp_parser_template_parameter (cp_parser * parser)
{
  ...
  if (token->keyword == RID_TYPENAME || token->keyword == RID_CLASS)
	...
	token = cp_lexer_peek_nth_token (parser->lexer, 3);
      /* Now, see if the token looks like the end of a template
         parameter.  */
      if (token->type == CPP_COMMA
	  || token->type == CPP_EQ || token->type == CPP_GREATER)
	return cp_parser_type_parameter (parser);
    }
  ...
  /* Otherwise, it is a non-type parameter.  
     ...
     operator.  */
  return cp_parser_parameter_declaration (parser, /*template_parm_p= */ true,
					  /*parenthesized_p= */ NULL);
}
	...
	if (cp_lexer_next_token_is (parser->lexer, CPP_EQ))
	  {
	    ...
	    cp_lexer_consume_token (parser->lexer);
	    /* Parse the id-expression.  */
	    default_argument = cp_parser_id_expression (parser,
							/*template_keyword_p= */
							false,
							/*check_dependency_p= */
							true,
...
cp_parser_template_id (cp_parser * parser,
		       bool template_keyword_p,
		       bool check_dependency_p, bool is_declaration)
{
  tree template;
  tree arguments;
  tree template_id;
...
cp_parser_template_name (cp_parser * parser,
			 bool template_keyword_p,
			 bool check_dependency_p,
			 bool is_declaration, bool * is_identifier)
{
  tree identifier;
  tree decl;
  tree fns;
  ...
  if (processing_template_decl
      && cp_parser_nth_token_starts_template_argument_list_p (parser, 1))
    {
	  ...
	  && check_dependency_p && dependent_type_p (parser->scope)
...
cp_parser_template_argument (cp_parser * parser)
{
  ...
  if (token->type == CPP_NAME
	...
	{
	  if (qualifying_class)
	    argument = finish_qualified_id_expr (qualifying_class, argument,
						 /*done= */ true,
						 address_p);
...
cp_parser_explicit_instantiation (cp_parser * parser)
{
  ...
  if (cp_parser_allow_gnu_extensions_p (parser))
    {
      extension_specifier = cp_parser_storage_class_specifier_opt (parser);
...
cp_parser_type_specifier (cp_parser * parser,
			  ...
			  bool is_friend,
			  bool is_declaration,
			  int *declares_class_or_enum, bool * is_cv_qualifier)
{
				 ...
				 bool identifier_p)
{
  ...
  if (!(flags & CP_PARSER_FLAGS_NO_USER_DEFINED_TYPES))
	...
	cp_parser_parse_tentatively (parser);
      ...
      /* Look for the optional `::' operator.  */
      global_p = (cp_parser_global_scope_opt (parser,
					      /*current_scope_valid_p= */
					      false)
		  != NULL_TREE);
      /* Look for the nested-name specifier.  */
      qualified_p = (cp_parser_nested_name_specifier_opt (parser,
							  /*typename_keyword_p= */
							  false,
							  /*check_dependency_p= */
							  true,
      ...
      if (parser->scope && cp_parser_optional_template_keyword (parser))
	{
  ...
  if (!cp_parser_parse_definitely (parser))
      ...
      if (TREE_CODE (type_decl) != TYPE_DECL)
	{
      ...
      /* Remember that the name was used in the definition of the
         ...
         entirely defined.  */
      else if (type_decl != error_mark_node && !parser->scope)
				     ...
				     bool is_friend, bool is_declaration)
{
  ...
  if (tag_type == typename_type)
    {
      if (cp_parser_nested_name_specifier (parser,
					   ...
					   /*check_dependency_p= */ true,
					   /*type_p= */ true,
					   is_declaration) == error_mark_node)
	return error_mark_node;
    }
  else
  ...
  if (!type)
      ...
      if (parser->scope)
	{
	  ...
	  decl = (cp_parser_maybe_treat_template_as_class (decl, /*tag_name_p= */ is_friend
							   && parser->
...
cp_parser_asm_definition (cp_parser * parser)
{
  ...
  if (cp_parser_allow_gnu_extensions_p (parser)
	...
	inputs_p = true;
      ...
      /* Look for inputs.  */
      if (inputs_p || cp_lexer_next_token_is (parser->lexer, CPP_COLON))
	{
	  /* Consume the `:' or `::'.  */
	  cp_lexer_consume_token (parser->lexer);
	  /* Parse the output-operands.  */
	...
	clobbers_p = true;
      ...
      /* Look for clobbers.  */
      if (clobbers_p || cp_lexer_next_token_is (parser->lexer, CPP_COLON))
	{
	  /* Consume the `:' or `::'.  */
	  cp_lexer_consume_token (parser->lexer);
	  /* Parse the clobbers.  */
			   ...
			   bool * function_definition_p)
{
  ...
  is_initialized = (token->type == CPP_EQ || token->type == CPP_OPEN_PAREN);
  /* If the init-declarator isn't initialized and isn't followed by a
     `,' or `;', it's not a valid init-declarator.  */
  if (!is_initialized
      && token->type != CPP_COMMA && token->type != CPP_SEMICOLON)
    {
  ...
  /* Finish processing the declaration.  But, skip friend
     declarations.  */
  if (!friend_p && decl)
    cp_finish_decl (decl, initializer, asm_specification,
...
cp_parser_declarator (cp_parser * parser,
		      ...
		      int *ctor_dtor_or_conv_p,
		      bool * parenthesized_p, bool member_p)
{
  cp_token *token;
  tree declarator;
  enum tree_code code;
...
cp_parser_direct_declarator (cp_parser * parser,
			     cp_parser_declarator_kind dcl_kind,
			     int *ctor_dtor_or_conv_p, bool member_p)
{
  cp_token *token;
  tree declarator = NULL_TREE;
  tree scope = NULL_TREE;
  ...
  while (true)
      ...
      else if (first && dcl_kind != CP_PARSER_DECLARATOR_ABSTRACT)
	{
	  ...
	  if (TREE_CODE (declarator) == SCOPE_REF && !current_scope ())
	    {
	      ...
	      if (TREE_CODE (scope) == TYPENAME_TYPE)
		{
		  ...
		  if (type == error_mark_node)
		    error ("`%T::%D' is not a type",
			   TYPE_CONTEXT (scope), TYPE_IDENTIFIER (scope));
		  /* Build a new DECLARATOR.  */
		  declarator = build_nt (SCOPE_REF,
					 type, TREE_OPERAND (declarator, 1));
		}
	    }
  ...
  if (token->type == CPP_MULT || token->type == CPP_AND)
    {
      ...
      if (code == INDIRECT_REF || cp_parser_allow_gnu_extensions_p (parser))
...
cp_parser_cv_qualifier_seq_opt (cp_parser * parser)
{
  ...
  while (true)
	...
	break;
      ...
      cv_qualifiers = tree_cons (NULL_TREE, cv_qualifier, cv_qualifiers);
    }
...
cp_parser_type_specifier_seq (cp_parser * parser)
{
  ...
  while (true)
	...
	cp_parser_parse_tentatively (parser);
      ...
      type_specifier = cp_parser_type_specifier (parser, CP_PARSER_FLAGS_NONE,
						 /*is_friend= */ false,
						 /*is_declaration= */ false,
						 NULL, NULL);
  ...
  while (true)
    {
      ...
      /* Parse the parameter.  */
      parameter = cp_parser_parameter_declaration (parser,
						   /*template_parm_p= */
						   false,
						   &parenthesized_p);
...
cp_parser_parameter_declaration (cp_parser * parser,
				 bool template_parm_p, bool * parenthesized_p)
{
  int declares_class_or_enum;
  bool greater_than_is_operator_p;
  tree decl_specifiers;
  ...
  if (token->type == CPP_CLOSE_PAREN
      ...
      || token->type == CPP_EQ
      || token->type == CPP_ELLIPSIS || token->type == CPP_GREATER)
    {
      declarator = NULL_TREE;
      if (parenthesized_p)
  ...
  parameter = build_tree_list (default_argument,
			       build_tree_list (decl_specifiers, declarator));
  ...
  *non_constant_p = false;
  ...
  if (cp_lexer_next_token_is_not (parser->lexer, CPP_OPEN_BRACE))
	...
	switch (token->type)
      ...
      if (!initializer)
	{
	  initializer = cp_parser_constant_expression (parser,
						       /*allow_non_constant_p= */
						       true,
						       non_constant_p);
	  if (!*non_constant_p)
...
cp_parser_class_name (cp_parser * parser,
		      ...
		      bool type_p,
		      bool check_dependency_p,
		      bool class_head_p, bool is_declaration)
{
  tree decl;
  tree scope;
  bool typename_p;
  ...
  if (token->type == CPP_NAME
      ...
      else
	{
	  ...
	  if (cp_lexer_next_token_is (parser->lexer, CPP_SCOPE))
	    type_p = true;
	  /* Look up the name.  */
	  decl = cp_parser_lookup_name (parser, identifier, type_p,
					/*is_template= */ false,
					/*is_namespace= */ false,
					check_dependency_p);
	}
  ...
  else
    {
      ...
      decl = cp_parser_template_id (parser, template_keyword_p,
				    check_dependency_p, is_declaration);
      if (decl == error_mark_node)
	return error_mark_node;
    }
...
cp_parser_class_specifier (cp_parser * parser)
{
  ...
  type = cp_parser_class_head (parser, &nested_name_specifier_p, &attributes);
  ...
  /* Inside the class, surrounding template-parameter-lists do not
     apply.  */
  saved_num_template_parameter_lists = parser->num_template_parameter_lists;
  parser->num_template_parameter_lists = 0;
...
cp_parser_class_head (cp_parser * parser,
		      bool * nested_name_specifier_p, tree * attributes_p)
{
  cp_token *token;
  ...
  *nested_name_specifier_p = false;
  ...
  nested_name_specifier = cp_parser_nested_name_specifier_opt (parser,
							       /*typename_keyword_p= */
							       false,
							       /*check_dependency_p= */
							       false,
  ...
  if (nested_name_specifier)
      ...
      else
	{
	  ...
	  for (scope = TREE_TYPE (type);
	       ...
	       scope = (TYPE_P (scope)
			? TYPE_CONTEXT (scope) : DECL_CONTEXT (scope)))
	    if (TYPE_P (scope)
		&& CLASS_TYPE_P (scope)
  ...
  if (qualified_p)
    cp_parser_error (parser, "global qualification of class name is invalid");
  else if (invalid_nested_name_p)
    cp_parser_error (parser, "qualified name does not name a class");
  ...
  if (at_namespace_scope_p ()
      && parser->num_template_parameter_lists == 0 && template_id_p)
    {
	  ...
	  if (friend_p)
	      ...
	      if (!type)
		{
		  ...
		  for (specifier = decl_specifiers;
		       specifier; specifier = TREE_CHAIN (specifier))
		    {
      ...
      while (cp_lexer_next_token_is_not (parser->lexer, CPP_SEMICOLON))
	{
	  ...
	  if (token->type == CPP_COLON
		...
		identifier = NULL_TREE;
	      ...
	      decl = grokbitfield (identifier, decl_specifiers, width);
...
cp_parser_base_specifier (cp_parser * parser)
{
  ...
  type = cp_parser_class_name (parser, class_scope_p, template_p,
			       /*type_p= */ true,
			       /*check_dependency_p= */ true,
			       /*class_head_p= */ false,
			       /*is_declaration= */ true);
...
cp_parser_throw_expression (cp_parser * parser)
{
  ...
  if (token->type == CPP_COMMA
      ...
      || token->type == CPP_CLOSE_SQUARE
      || token->type == CPP_CLOSE_BRACE || token->type == CPP_COLON)
		       ...
		       bool check_dependency)
{
  ...
  my_friendly_assert (TREE_CODE (name) == IDENTIFIER_NODE, 20000619);
  ...
  if (parser->scope)
	...
	return error_mark_node;
      ...
      dependent_p = (TYPE_P (parser->scope)
		     ...
		     && dependent_type_p (parser->scope));
      if ((check_dependency || !CLASS_TYPE_P (parser->scope)) && dependent_p)
	{
	  if (is_type)
	    ...
	    decl = make_unbound_class_template (parser->scope, name,
						/*complain= */ 1);
	  else
	...
	object_decl = lookup_member (object_type, name,
				     /*protect= */ 0, is_type);
  ...
  if (!decl
      || decl == error_mark_node
      || (TREE_CODE (decl) == FUNCTION_DECL && DECL_ANTICIPATED (decl)))
    return error_mark_node;
						...
						tree declarator)
{
  ...
  switch (TREE_CODE (declarator))
	...
	++num_templates;
      ...
      return cp_parser_check_template_parameters (parser, num_templates);
    }
}
  ...
  while (true)
    {
      ...
      if (token->type == CPP_GREATER && !parser->greater_than_is_operator_p)
	break;
      /* If we find one of the tokens we want, build the corresponding
         tree representation.  */
      for (map_node = token_tree_map;
	   map_node->token_type != CPP_EOF; ++map_node)
...
cp_parser_constructor_declarator_p (cp_parser * parser, bool friend_p)
{
  ...
  cp_parser_global_scope_opt (parser,
			      /*current_scope_valid_p= */ false);
  /* Look for the nested-name-specifier.  */
  nested_name_p = (cp_parser_nested_name_specifier_opt (parser,
							/*typename_keyword_p= */
							false,
							/*check_dependency_p= */
							false,
  ...
  /* If we're still considering a constructor, we have to see a `(',
     ...
     `S::S'.  */
  if (constructor_p && cp_parser_require (parser, CPP_OPEN_PAREN, "`('"))
    {
      if (cp_lexer_next_token_is_not (parser->lexer, CPP_CLOSE_PAREN)
	  ...
	  && !cp_parser_storage_class_specifier_opt (parser))
	{
	  ...
	  /* Look for the type-specifier.  */
	  cp_parser_type_specifier (parser, CP_PARSER_FLAGS_NONE,
				    /*is_friend= */ false,
				    /*is_declarator= */ true,
...
static tree
  cp_parser_function_definition_from_specifiers_and_declarator
  (cp_parser * parser, tree decl_specifiers, tree attributes, tree declarator)
{
  tree fn;
  bool success_p;
						...
						bool inline_p)
{
  ...
  /* Finish the function.  */
  fn = finish_function ((ctor_initializer_p ? 1 : 0) | (inline_p ? 2 : 0));
...
static void
cp_parser_template_declaration_after_export (cp_parser * parser,
					     bool member_p)
{
  tree decl = NULL_TREE;
  tree parameter_list;
  ...
  if (cp_lexer_next_token_is_keyword (parser->lexer, RID_TEMPLATE))
    ...
    {
      decl = cp_parser_single_declaration (parser, member_p, &friend_p);
			      ...
			      bool member_p, bool * friend_p)
{
  ...
  if (!decl
    ...
    decl = cp_parser_init_declarator (parser, decl_specifiers, attributes,
				      /*function_definition_allowed_p= */
				      true,
				      member_p, declares_class_or_enum,
				      &function_definition_p);
...
cp_parser_save_member_function_body (cp_parser * parser,
				     tree decl_specifiers,
				     tree declarator, tree attributes)
{
...
cp_parser_enclosed_template_argument_list (cp_parser * parser)
{
  ...
  if (cp_lexer_next_token_is (parser->lexer, CPP_RSHIFT))
      ...
      else
	{
	  ...
	  error
	    ("spurious `>>', use `>' to terminate a template argument list");
	  cp_lexer_consume_token (parser->lexer);
	}
    }
  else
...
cp_parser_late_parsing_default_args (cp_parser * parser, tree fn)
{
  ...
  for (parameters = TYPE_ARG_TYPES (TREE_TYPE (fn));
       parameters; parameters = TREE_CHAIN (parameters))
    {
	...
	pop_nested_class ();
      ...
      for (default_arg = TREE_CHAIN (default_arg);
	   default_arg; default_arg = TREE_CHAIN (default_arg))
...
cp_parser_sizeof_operand (cp_parser * parser, enum rid keyword)
{
  ...
  if (cp_lexer_next_token_is (parser->lexer, CPP_OPEN_PAREN))
      ...
      if (cp_parser_parse_definitely (parser))
	{
	  ...
	  expr = grokdeclarator (NULL_TREE, type, TYPENAME,
				 /*initialized= */ 0,
				 /*attrlist= */ NULL);
	}
    }
			    ...
			    enum cpp_ttype type, const char *token_desc)
{
  ...
  while (true)
	...
	return;
      if (token->type == CPP_OPEN_BRACE
	  || token->type == CPP_OPEN_PAREN || token->type == CPP_OPEN_SQUARE)
	++nesting_depth;
...
cp_parser_check_class_key (enum tag_types class_key, tree type)
{
  if ((TREE_CODE (type) == UNION_TYPE) != (class_key == union_type))
    pedwarn ("`%s' tag used in naming `%#T'",
	     ...
	     : class_key == record_type ? "struct" : "class", type);
}
...
cp_parser_cache_group (cp_parser * parser,
		       cp_token_cache * cache,
		       enum cpp_ttype end, unsigned depth)
{
  while (true)
    {
      cp_token *token;
