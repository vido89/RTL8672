		  ...
		  int size)
{
  ...
  stack =
    (struct stack_level *) ((char *) obstack_next_free (obstack) - size);
...
static base_kind
lookup_base_r (tree binfo, tree base, base_access access, bool is_virtual,	/* inside a virtual part */
	       tree * binfo_ptr)
{
  int i;
  tree bases, accesses;
...
lookup_base (tree t, tree base, base_access access, base_kind * kind_ptr)
{
  ...
  if (access != ba_any)
	...
	if ((access & ~ba_quiet) != ba_ignore
	    ...
	    && COMPLETE_TYPE_P (base) && !accessible_base_p (t, base))
	  {
	    if (!(access & ba_quiet))
	      {
...
dfs_accessible_p (tree binfo, void *data ATTRIBUTE_UNUSED)
{
  ...
  if (access != ak_none && is_friend (BINFO_TYPE (binfo), current_scope ()))
    return binfo;
...
lookup_field_r (tree binfo, void *data)
{
  ...
  if (lfi->rval_binfo && !is_subobject_of_p (lfi->rval_binfo, binfo))
...
build_baselink (tree binfo, tree access_binfo, tree functions, tree optype)
{
  ...
  my_friendly_assert (TREE_CODE (functions) == FUNCTION_DECL
		      ...
		      || TREE_CODE (functions) == OVERLOAD, 20020730);
...
bfs_walk (tree binfo,
	  tree (*fn) (tree, void *),
	  tree (*qfn) (tree, int, void *), void *data)
{
  tree rval = NULL_TREE;
  ...
  if (!BINFO_PRIMARY_P (binfo))
    {
      ...
      for (virtuals = BINFO_VIRTUALS (binfo);
	   virtuals; virtuals = TREE_CHAIN (virtuals))
...
setup_class_bindings (tree name, int type_binding_p)
{
  ...
  if (type_binding_p
	  ...
	  || (TREE_CODE (value_binding) == TREE_LIST
	      ...
	      && (TREE_CODE (TREE_VALUE (value_binding)) == TYPE_DECL))))
...
dfs_push_decls (tree binfo, void *data)
{
  ...
  for (fields = TYPE_FIELDS (type); fields; fields = TREE_CHAIN (fields))
	...
	&& TREE_CODE (fields) != USING_DECL && !DECL_ARTIFICIAL (fields))
