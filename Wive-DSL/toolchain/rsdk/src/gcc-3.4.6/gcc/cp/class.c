...
static tree resolve_address_of_overloaded_function (tree, tree,
						    tsubst_flags_t, bool,
						    tree);
...
tree
build_base_path (enum tree_code code, tree expr, tree binfo, int nonnull)
{
  ...
  if (v_binfo && fixed_type_p <= 0)
	...
	v_offset = build_vfield_ref (build_indirect_ref (expr, NULL),
      ...
      v_offset = build1 (NOP_EXPR,
			 build_pointer_type (ptrdiff_type_node), v_offset);
  ...
  if (null_test)
    expr = build (COND_EXPR, target_type, null_test,
		  build1 (NOP_EXPR, target_type, integer_zero_node), expr);
...
convert_to_base (tree object, tree type, bool check_access)
{
  ...
  binfo = lookup_base (TREE_TYPE (object), type,
		       check_access ? ba_check : ba_ignore, NULL);
...
build_vtable (tree class_type, tree name, tree vtable_type)
{
  ...
  DECL_ALIGN (decl) = MAX (TYPE_ALIGN (double_type_node), DECL_ALIGN (decl));
...
modify_vtable_entry (tree t,
		     tree binfo, tree fndecl, tree delta, tree * virtuals)
{
  tree v;

  v = *virtuals;

  if (fndecl != BV_FN (v) || !tree_int_cst_equal (delta, BV_DELTA (v)))
    {
...
add_method (tree type, tree method, int error_p)
{
  ...
  if (processing_template_decl)
  ...
  else
    {
      ...
      for (fns = TREE_VEC_ELT (method_vec, slot); fns; fns = OVL_NEXT (fns))
	{
  ...
  /* Add the new binding.  */
  if (!DECL_CONSTRUCTOR_P (method) && !DECL_DESTRUCTOR_P (method))
...
handle_using_decl (tree using_decl, tree t)
{
  ...
  if (!old_value)
    ...
    {
      cp_error_at ("`%D' invalid in `%#T'", using_decl, t);
      cp_error_at ("  because of local member `%#D' with same name",
		   old_value);
      return;
    }
...
check_bases (tree t,
	     ...
	     int *cant_have_const_ctor_p, int *no_const_asn_ref_p)
{
  int n_baseclasses;
  int i;
  ...
  for (i = 0; i < n_baseclasses; ++i)
	...
	warning ("base class `%#T' has a non-virtual destructor", basetype);
...
determine_primary_base (tree t)
{
  ...
  for (vbases = CLASSTYPE_VBASECLASSES (t);
      ...
      for (i = 0; i < n_baseclasses; ++i)
	{
	  ...
	  for (v = CLASSTYPE_VBASECLASSES (basetype); v; v = TREE_CHAIN (v))
	    {
  ...
  if (!CLASSTYPE_HAS_PRIMARY_BASE_P (t))
    {
      ...
      for (base_binfo = TYPE_BINFO (t);
	   base_binfo; base_binfo = TREE_CHAIN (base_binfo))
	{
	  tree basetype = BINFO_TYPE (base_binfo);
...
maybe_warn_about_overly_private_class (tree t)
{
  ...
  if (TYPE_HAS_DESTRUCTOR (t) && TREE_PRIVATE (CLASSTYPE_DESTRUCTORS (t)))
    {
  ...
  if (TYPE_HAS_CONSTRUCTOR (t))
	...
	{
	  warning
	    ("`%#T' only defines private constructors and has no friends", t);
	  return;
	}
    }
}
...
dfs_find_final_overrider_1 (tree binfo,
			    tree * vpath, find_final_overrider_data * ffod)
{
  tree method;
...
find_final_overrider (tree derived, tree binfo, tree fn)
{
  ...
  dfs_walk_real (derived,
		 ...
		 dfs_find_final_overrider_q, &ffod);
  ...
  if (POINTER_TYPE_P (over_return)
	       ...
	       (over_return, base_return))
	{
	  ...
	  for (base_binfo = TYPE_BINFO (base_return),
	       ...
	       thunk_binfo; thunk_binfo = TREE_CHAIN (thunk_binfo))
  ...
  for (; b; b = BINFO_INHERITANCE_CHAIN (b))
    {
      /* If we find the final overrider, then we can stop
         walking.  */
      if (same_type_p (BINFO_TYPE (b), BINFO_TYPE (TREE_VALUE (overrider))))
	break;
...
dfs_modify_vtables (tree binfo, void *data)
{
  if (				/* There's no need to modify the vtable for a non-virtual
	...
	update_vtable_entry_for_fn (t,
				    binfo,
				    BV_FN (old_virtuals), &virtuals, ix);
    }
...
get_basefndecls (tree name, tree t)
{
  ...
  if (i != -1)
    for (methods = TREE_VEC_ELT (CLASSTYPE_METHOD_VEC (t), i);
	 methods; methods = OVL_NEXT (methods))
      {
  ...
  for (i = 0; i < n_baseclasses; i++)
    {
      tree basetype = TYPE_BINFO_BASETYPE (t, i);
      base_fndecls = chainon (get_basefndecls (name, basetype), base_fndecls);
    }
...
check_for_override (tree decl, tree ctype)
{
  ...
  if ((DECL_DESTRUCTOR_P (decl)
       ...
       || DECL_CONV_FN_P (decl))
      && look_for_overrides (ctype, decl) && !DECL_STATIC_FUNCTION_P (decl))
				 ...
				 int cant_have_const_assignment)
{
  ...
  if (!TYPE_HAS_CONSTRUCTOR (t) && !cant_have_default_ctor)
    {
      default_fn =
	implicitly_declare_fn (sfk_constructor, t, /*const_p= */ 0);
      TREE_CHAIN (default_fn) = implicit_fns;
      implicit_fns = default_fn;
    }
  ...
  if (!TYPE_HAS_INIT_REF (t) && !TYPE_FOR_JAVA (t))
    {
      /* ARM 12.18: You get either X(X&) or X(const X&), but
         not both.  --Chip  */
      default_fn = implicitly_declare_fn (sfk_copy_constructor, t,
					  /*const_p= */
  ...
  if (!TYPE_HAS_ASSIGN_REF (t) && !TYPE_FOR_JAVA (t))
    {
      default_fn = implicitly_declare_fn (sfk_assignment_operator, t,
					  /*const_p= */
  ...
  for (f = &implicit_fns; *f; f = &TREE_CHAIN (*f))
    {
      ...
      maybe_add_class_template_decl_list (current_class_type, *f,
					  /*friend_p= */ 0);
    }
  ...
  for (x = fields; x; x = TREE_CHAIN (x))
    {
      if (TREE_CODE (x) == FIELD_DECL && ANON_AGGR_TYPE_P (TREE_TYPE (x)))
	idx =
	  add_fields_to_record_type (TYPE_FIELDS (TREE_TYPE (x)), field_vec,
				     idx);
      else
...
check_bitfield_decl (tree field)
{
  ...
  if (DECL_INITIAL (field) && !INTEGRAL_TYPE_P (TREE_TYPE (field)))
    {
  ...
  if (DECL_INITIAL (field))
	...
	{
	  cp_error_at ("bit-field `%D' width not an integer constant", field);
	  w = error_mark_node;
	}
...
check_field_decl (tree field,
		  ...
		  int *cant_have_default_ctor,
		  int *no_const_asn_ref, int *any_default_members)
{
  ...
  /* Check members with class type for constructors, destructors,
	...
	*no_const_asn_ref = 1;
      ...
      if (TYPE_HAS_CONSTRUCTOR (type) && !TYPE_HAS_DEFAULT_CONSTRUCTOR (type))
  ...
  for (field = &TYPE_FIELDS (t); *field; field = next)
      ...
      if (TREE_CODE (x) == USING_DECL)
	{
	  ...
	  *access_decls = tree_cons (NULL_TREE, x, *access_decls);
      ...
      if (TREE_CODE (x) == TYPE_DECL || TREE_CODE (x) == TEMPLATE_DECL)
	continue;
	...
	{
	  cp_error_at ("field `%D' invalidly declared function type", x);
	...
	check_field_decl (x, t,
			  cant_have_const_ctor_p,
			  cant_have_default_ctor_p,
			  no_const_asn_ref_p, &any_default_members);
    }
...
record_subobject_offset (tree type, tree offset, splay_tree offsets)
{
  ...
  n->value = ((splay_tree_value)
	      tree_cons (NULL_TREE, type, (tree) n->value));
...
walk_subobject_offsets (tree type,
			...
			tree offset,
			splay_tree offsets, tree max_offset, int vbases_p)
{
  int r = 0;
  ...
  if (CLASS_TYPE_P (type))
      ...
      for (i = 0; i < BINFO_N_BASETYPES (type_binfo); ++i)
	{
	  ...
	  if (abi_version_at_least (2) && TREE_VIA_VIRTUAL (binfo))
	    continue;

	  if (!vbases_p
	      && TREE_VIA_VIRTUAL (binfo) && !BINFO_PRIMARY_P (binfo))
	    continue;
	  ...
	  if (!abi_version_at_least (2))
	  ...
	  else
	    {
	      ...
	      binfo_offset = size_binop (PLUS_EXPR,
					 offset, BINFO_OFFSET (orig_binfo));
	    }
...
record_subobject_offsets (tree type,
			  tree offset, splay_tree offsets, int vbases_p)
{
...
static int
layout_conflict_p (tree type, tree offset, splay_tree offsets, int vbases_p)
{
  ...
  return walk_subobject_offsets (type, check_subobject_offset, offset,
				 offsets, (tree) (max_node->key), vbases_p);
}
...
layout_nonempty_base_or_field (record_layout_info rli,
			       tree decl, tree binfo, splay_tree offsets)
{
...
layout_empty_base (tree binfo, tree eoc, splay_tree offsets)
{
  ...
  if (!integer_zerop (BINFO_OFFSET (binfo)))
	...
	warning ("offset of empty base `%T' may not be ABI-compliant and may"
		 "change in a future version of GCC", BINFO_TYPE (binfo));
    }

  /* This is an empty base class.  We first try to put it at offset
     zero.  */
  if (layout_conflict_p (binfo, BINFO_OFFSET (binfo), offsets,
			 /*vbases_p= */ 0))
    {
		  ...
		  splay_tree offsets, tree * next_field)
{
  ...
  record_subobject_offsets (binfo, BINFO_OFFSET (binfo), offsets,
			    /*vbases_p= */ 0);
		   ...
		   splay_tree offsets, tree * next_field)
{
  ...
  for (i = 0; i < n_baseclasses; ++i)
	...
	continue;
      ...
      next_field = build_base_field (rli, base_binfo, offsets, next_field);
    }
}
  ...
  if (DECL_HAS_IN_CHARGE_PARM_P (clone))
    {
      ...
      if (DECL_HAS_VTT_PARM_P (fn) && !DECL_NEEDS_VTT_PARM_P (clone))
	parmtypes = TREE_CHAIN (parmtypes);
...
clone_function_decl (tree fn, int update_method_vec_p)
{
  ...
  if (TREE_CHAIN (fn) && DECL_CLONED_FUNCTION (TREE_CHAIN (fn)))
    return;
...
type_requires_array_cookie (tree type)
{
  ...
  /* If the usual deallocation function is a two-argument whose second
     ...
     a cookie.  */
  fns = lookup_fnfields (TYPE_BINFO (type), ansi_opname (VEC_DELETE_EXPR),
			 /*protect= */ 0);
...
check_bases_and_members (tree t)
{
  ...
  check_field_decls (t, &access_decls,
		     ...
		     &cant_have_const_ctor, &no_const_asn_ref);
  ...
  TYPE_HAS_COMPLEX_INIT_REF (t)
    |= (TYPE_HAS_INIT_REF (t)
	|| TYPE_USES_VIRTUAL_BASECLASSES (t) || TYPE_POLYMORPHIC_P (t));
  ...
  if (!TYPE_VFIELD (t) && (*virtuals_p || TYPE_CONTAINS_VPTR_P (t)))
    {
      ...
      field =
	build_decl (FIELD_DECL, get_vfield_name (t), vtbl_ptr_type_node);
...
layout_virtual_bases (record_layout_info rli, splay_tree offsets)
{
  ...
  if (!abi_version_at_least (2))
    {
      ...
      if (!TYPE_PACKED (t))
	TYPE_ALIGN (t) =
	  MAX (TYPE_ALIGN (t), (unsigned) STRUCTURE_SIZE_BOUNDARY);
#endif
  ...
  for (vbase = TYPE_BINFO (t); vbase; vbase = TREE_CHAIN (vbase))
      ...
      if (!BINFO_PRIMARY_P (vbase))
	{
	  ...
	  next_field = build_base_field (rli, vbase, offsets, next_field);
	  ...
	  if (warn_abi
		  ...
		  (size_binop (CEIL_DIV_EXPR,
			       ...
			       bitsize_unit_node), BINFO_OFFSET (vbase))))
	    warning
	      ("offset of virtual base `%T' is not ABI-compliant and may change in a future version of GCC",
	       basetype);
...
layout_class_type (tree t, tree * virtuals_p)
{
  ...
  for (field = non_static_data_members; field; field = TREE_CHAIN (field))
	  ...
	  && INT_CST_LT (TYPE_SIZE (type), DECL_SIZE (field)))
	{
	  ...
	  else if (warn_abi && DECL_MODE (field) != TYPE_MODE (type))
	...
	record_subobject_offsets (TREE_TYPE (field),
				  byte_position (field), empty_base_offsets,
				  /*vbases_p= */ 1);
      ...
      if (padding)
	{
	  ...
	  padding_field = build_decl (FIELD_DECL, NULL_TREE, char_type_node);
	  DECL_BIT_FIELD (padding_field) = 1;
	  DECL_SIZE (padding_field) = padding;
	  DECL_CONTEXT (padding_field) = t;
	  DECL_ARTIFICIAL (padding_field) = 1;
	  layout_nonempty_base_or_field (rli, padding_field,
					 NULL_TREE, empty_base_offsets);
	}
      ...
      if (!abi_version_at_least (2) && CLASSTYPE_EMPTY_P (t))
	{
	  ...
	  if (warn_abi && !integer_zerop (rli_size_unit_so_far (rli)))
	    warning ("layout of classes derived from empty class `%T' "
		     "may change in a future version of GCC", t);
	}
      else
	{
	  tree eoc;
  ...
  if (integer_zerop (rli_size_unit_so_far (rli)) && CLASSTYPE_EMPTY_P (t))
    place_field (rli, build_decl (FIELD_DECL, NULL_TREE, char_type_node));
...
key_method (tree type)
{
  ...
  for (method = TYPE_METHODS (type); method != NULL_TREE;
	...
	&& !DECL_DECLARED_INLINE_P (method) && !DECL_PURE_VIRTUAL_P (method))
      return method;
...
finish_struct_1 (tree t)
{
  ...
  if (vfield && CLASSTYPE_HAS_PRIMARY_BASE_P (t))
    ...
    my_friendly_assert (!vfield
			|| DECL_FIELD_CONTEXT (vfield) == t, 20010726);
  ...
  if (TYPE_CONTAINS_VPTR_P (t))
	...
	my_friendly_assert (DECL_VIRTUAL_P (TYPE_BINFO_VTABLE (t)), 20000116);
      if (!CLASSTYPE_HAS_PRIMARY_BASE_P (t))
	my_friendly_assert (TYPE_BINFO_VIRTUALS (t) == NULL_TREE, 20000116);
...
unreverse_member_declarations (tree t)
{
  ...
  prev = NULL_TREE;
  for (x = TYPE_FIELDS (t); x && TREE_CODE (x) != TYPE_DECL; x = next)
    {
      next = TREE_CHAIN (x);
      TREE_CHAIN (x) = prev;
      prev = x;
	...
	*nonnull = 1;
      ...
      else if (TREE_CODE (TREE_TYPE (instance)) == REFERENCE_TYPE)
	{
	  ...
	  if (TREE_CODE (instance) == VAR_DECL
	      && DECL_INITIAL (instance) && !DECL_VAR_MARKED_P (instance))
	    {
	      tree type;
					...
					tree explicit_targs)
{
  ...
  if (TYPE_PTRFN_P (target_type))
  ...
  else
    {
      if (flags & tf_error)
	error ("\
cannot resolve overloaded function `%D' based on conversion to type `%T'", DECL_NAME (OVL_FUNCTION (overload)), target_type);
      return error_mark_node;
    }
  ...
  if (!matches)
      ...
      for (fns = overload; fns; fns = OVL_NEXT (fns))
	{
	  ...
	  if (can_convert_arg
	      (target_type, instantiation_type, instantiation))
  ...
  if (matches == NULL_TREE)
      ...
      if (flags & tf_error)
	{
	  ...
	  for (; overload; overload = OVL_NEXT (overload))
	    matches = tree_cons (NULL_TREE, OVL_CURRENT (overload), matches);
      ...
      if (flags & tf_error)
	{
	  ...
	  error
	    ("converting overloaded function `%D' to type `%#T' is ambiguous",
	     DECL_NAME (OVL_FUNCTION (overload)), target_type);
...
instantiate_type (tree lhstype, tree rhs, tsubst_flags_t flags)
{
  ...
  switch (TREE_CODE (rhs))
	...
	return resolve_address_of_overloaded_function (lhstype, fns, flags_in,
						       /*template_only= */
						       true,
						       args);
      }
    ...
    case OVERLOAD:
    case FUNCTION_DECL:
      return resolve_address_of_overloaded_function (lhstype, rhs, flags_in,
						     /*template_only= */
						     false,
						     /*explicit_targs= */
						     NULL_TREE);
...
contains_empty_class_p (tree type)
{
  ...
  if (CLASS_TYPE_P (type))
	...
	if (contains_empty_class_p (TYPE_BINFO_BASETYPE (type, i)))
      ...
      for (field = TYPE_FIELDS (type); field; field = TREE_CHAIN (field))
	if (TREE_CODE (field) == FIELD_DECL
	    && !DECL_ARTIFICIAL (field) && is_empty_class (TREE_TYPE (field)))
	  return true;
    }
  else if (TREE_CODE (type) == ARRAY_TYPE)
...
maybe_note_name_used_in_class (tree name, tree decl)
{
  ...
  splay_tree_insert (names_used,
		     (splay_tree_key) name, (splay_tree_value) decl);
}
...
dump_class_hierarchy_r (FILE * stream,
			int flags, tree binfo, tree igo, int indent)
{
  int indented = 0;
  tree base_binfos;
...
dump_class_hierarchy_1 (FILE * stream, int flags, tree t)
{
  ...
  fprintf (stream, "   base size=%lu base align=%lu\n",
	   (unsigned
	    long) (tree_low_cst (TYPE_SIZE (CLASSTYPE_AS_BASE (t)),
				 0) / BITS_PER_UNIT),
...
build_vtt_inits (tree binfo, tree t, tree * inits, tree * index)
{
  ...
  *index = size_binop (PLUS_EXPR, *index, TYPE_SIZE_UNIT (ptr_type_node));
  ...
  for (i = 0; i < BINFO_N_BASETYPES (binfo); ++i)
    {
      ...
      if (!TREE_VIA_VIRTUAL (b))
	inits = build_vtt_inits (BINFO_BASETYPE (binfo, i), t, inits, index);
    }
  ...
  dfs_walk_real (binfo,
		 ...
		 NULL, dfs_ctor_vtable_bases_queue_p, secondary_vptrs);
  ...
  if (!top_level_p)
    {
      ...
      dfs_walk (binfo, dfs_fixup_binfo_vtbls,
		dfs_ctor_vtable_bases_queue_p, data);
    }
...
static tree
dfs_ctor_vtable_bases_queue_p (tree derived, int ix, void *data)
{
  tree binfo = BINFO_BASETYPE (derived, ix);
...
dfs_fixup_binfo_vtbls (tree binfo, void *data)
{
  ...
  if (BINFO_VTABLE (binfo)
      ...
      && (TREE_PURPOSE (BINFO_VTABLE (binfo)) == TREE_VALUE ((tree) data)))
...
accumulate_vtbl_inits (tree binfo,
		       tree orig_binfo, tree rtti_binfo, tree t, tree inits)
{
  int i;
  int ctor_vtbl_p = !same_type_p (BINFO_TYPE (rtti_binfo), t);

  my_friendly_assert (same_type_p (BINFO_TYPE (binfo),
				   BINFO_TYPE (orig_binfo)), 20000517);
...
dfs_accumulate_vtbl_inits (tree binfo,
			   tree orig_binfo, tree rtti_binfo, tree t, tree l)
{
  ...
  if (!vtbl)
    {
      ...
      index = size_binop (MULT_EXPR,
			  TYPE_SIZE_UNIT (vtable_entry_type), index);
...
build_vtbl_initializer (tree binfo,
			tree orig_binfo,
			tree t, tree rtti_binfo, int *non_fn_entries_p)
{
  tree v, b;
  tree vfun_inits;
  tree vbase;
  ...
  for (v = BINFO_VIRTUALS (orig_binfo); v; v = TREE_CHAIN (v))
      ...
      if (!init)
	{
	  ...
	  if (DECL_PURE_VIRTUAL_P (fn_original))
	    ...
	    {
	      fn =
		make_thunk (fn, /*this_adjusting= */ 1, delta, vcall_index);
	      if (!DECL_NAME (fn))
		finish_thunk (fn);
	    }
  ...
  for (vbase = TYPE_BINFO (BINFO_TYPE (binfo));
      ...
      if (binfo != TYPE_BINFO (t))
	{
      ...
      /* The initializer is the delta from BINFO to this virtual base.
         ...
         the right order.  */
      delta =
	size_diffop (BINFO_OFFSET (b), BINFO_OFFSET (non_primary_binfo));
  ...
  if (abi_version_at_least (2))
    {
      ...
      for (orig_fn = TYPE_METHODS (BINFO_TYPE (binfo));
	   orig_fn; orig_fn = TREE_CHAIN (orig_fn))
...
build_rtti_vtbl_entries (tree binfo, vtbl_init_data * vid)
{
  ...
  while (CLASSTYPE_HAS_PRIMARY_BASE_P (BINFO_TYPE (b))
	 && !BINFO_LOST_PRIMARY_P (b))
    {
      ...
      my_friendly_assert (BINFO_PRIMARY_BASE_OF (primary_base) == b,
			  20010127);
      b = primary_base;
    }
