...
operator_name_info_t assignment_operator_name_info[(int)
...
init_reswords (void)
{
  unsigned int i;
  tree id;
  int mask = ((flag_no_asm ? D_ASM : 0) | (flag_no_gnu_keywords ? D_EXT : 0));
...
retrofit_lang_decl (tree t)
{
  ...
  if (current_lang_name == lang_name_cplusplus || decl_linkage (t) == lk_none)
...
cxx_make_type (enum tree_code code)
{
  ...
  if (IS_AGGR_TYPE_CODE (code) || code == BOUND_TEMPLATE_TEMPLATE_PARM)
    {
      struct lang_type *pi;
  ...
  if (IS_AGGR_TYPE_CODE (code) || code == TEMPLATE_TYPE_PARM
      || code == BOUND_TEMPLATE_TEMPLATE_PARM || code == TYPENAME_TYPE)
