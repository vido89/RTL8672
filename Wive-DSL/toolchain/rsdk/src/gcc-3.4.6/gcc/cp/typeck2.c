...
readonly_error (tree arg, const char *string, int soft)
{
  ...
  if (TREE_CODE (arg) == COMPONENT_REF)
  ...
  else if (TREE_CODE (arg) == VAR_DECL)
    {
	  ...
	  && DECL_IN_AGGR_P (arg) && !TREE_STATIC (arg))
...
abstract_virtuals_error (tree decl, tree type)
{
  ...
  if (decl)
	...
	error ("cannot declare variable `%D' to be of type `%T'", decl, type);
      else if (TREE_CODE (decl) == PARM_DECL)
	error ("cannot declare parameter `%D' to be of type `%T'",
	       decl, type);
      else if (TREE_CODE (decl) == FIELD_DECL)
	error ("cannot declare field `%D' to be of type `%T'", decl, type);
  ...
  if (code == ARRAY_TYPE)
	      ...
	      || (element && TREE_CODE (element) == STRING_CST)))
	{
	  ...
	  if ((TYPE_MAIN_VARIANT (TREE_TYPE (TREE_TYPE (string)))
	       != char_type_node) && TYPE_PRECISION (typ1) == BITS_PER_UNIT)
	    {
  ...
  if (TREE_CODE (type) == ARRAY_TYPE || TREE_CODE (type) == VECTOR_TYPE)
	      ...
	      if (TREE_VALUE (tail) != 0)
		{
		  ...
		  my_friendly_assert
		    (same_type_ignoring_top_level_qualifiers_p
		     (TREE_TYPE (type), TREE_TYPE (next1)), 981123);
      ...
      if (tail)
	{
	  if (TYPE_USES_VIRTUAL_BASECLASSES (type))
	    {
	      sorry
		("initializer list for object of class with virtual base classes");
	      return error_mark_node;
	    }
	  ...
	  if (TYPE_BINFO_BASETYPES (type))
	    {
	      sorry
		("initializer list for object of class with base classes");
	      return error_mark_node;
	    }
      ...
      for (field = TYPE_FIELDS (type); field; field = TREE_CHAIN (field))
	{
	  ...
	  if (tail)
		...
		next1 = build_functional_cast (TREE_TYPE (field), NULL_TREE);
	      else
		{
		  next1 = build_constructor (NULL_TREE, NULL_TREE);
		  if (init)
	      ...
	      if (extra_warnings && (!init || TREE_HAS_CONSTRUCTOR (init)))
		...
		if (CLASSTYPE_READONLY_FIELDS_NEED_INIT (TREE_TYPE (field)))
		error ("member `%D' with uninitialized const fields", field);
      ...
      if (TREE_PURPOSE (tail) != NULL_TREE)
	{
	  ...
	  if (TREE_CODE (TREE_PURPOSE (tail)) == FIELD_DECL)
	    ...
	    {
	      tree temp;
	      for (temp = TYPE_FIELDS (type); temp; temp = TREE_CHAIN (temp))
      ...
      if (TREE_VALUE (tail) != 0)
	{
	  ...
	  next1 = digest_init (TREE_TYPE (field), TREE_VALUE (tail), &tail1);
