...
target_type (tree type)
{
  ...
  while (TREE_CODE (type) == POINTER_TYPE
	 ...
	 || TREE_CODE (type) == METHOD_TYPE || TYPE_PTRMEM_P (type))
    type = TREE_TYPE (type);
  return type;
}
...
type_after_usual_arithmetic_conversions (tree t1, tree t2)
{
  ...
  my_friendly_assert (ARITHMETIC_TYPE_P (t1)
		      ...
		      || TREE_CODE (t1) == ENUMERAL_TYPE, 19990725);
  my_friendly_assert (ARITHMETIC_TYPE_P (t2)
		      || TREE_CODE (t2) == COMPLEX_TYPE
		      || TREE_CODE (t2) == VECTOR_TYPE
		      || TREE_CODE (t2) == ENUMERAL_TYPE, 19990725);
  ...
  if (code1 != REAL_TYPE)
	...
	return build_type_attribute_variant (double_type_node, attributes);
      if (same_type_p (TYPE_MAIN_VARIANT (t1), float_type_node)
	  || same_type_p (TYPE_MAIN_VARIANT (t2), float_type_node))
	return build_type_attribute_variant (float_type_node, attributes);
...
merge_types (tree t1, tree t2)
{
  ...
  switch (code1)
	...
	t1 = build_ptrmem_type (TYPE_PTRMEM_CLASS_TYPE (t1), pointee);
	t1 = cp_build_qualified_type (t1, quals);
	break;
      }
...
comp_except_types (tree a, tree b, bool exact)
{
  if (same_type_p (a, b))
	...
	return false;
      ...
      if (TREE_CODE (a) == POINTER_TYPE && TREE_CODE (b) == POINTER_TYPE)
	{
	  a = TREE_TYPE (a);
	  b = TREE_TYPE (b);
	  if (cp_type_quals (a) || cp_type_quals (b))
...
comptypes (tree t1, tree t2, int strict)
{
  ...
  if (TREE_CODE (t1) != ARRAY_TYPE && TYPE_QUALS (t1) != TYPE_QUALS (t2))
    return false;
...
cxx_sizeof_or_alignof_expr (tree e, enum tree_code op)
{
  ...
  if (TREE_CODE (e) == COMPONENT_REF
    ...
    {
      pedwarn
	("ISO C++ forbids applying `%s' to an expression of function type",
	 op_name);
      e = char_type_node;
    }
...
string_conv_p (tree totype, tree exp, int warn)
{
  ...
  t = TREE_TYPE (totype);
  if (!same_type_p (t, char_type_node) && !same_type_p (t, wchar_type_node))
    return 0;
...
rationalize_conditional_expr (enum tree_code code, tree t)
{
  ...
  if (TREE_CODE (t) == MIN_EXPR || TREE_CODE (t) == MAX_EXPR)
	...
	build_conditional_expr (build_x_binary_op ((TREE_CODE (t) == MIN_EXPR
				...
				build_unary_op (code, TREE_OPERAND (t, 1),
						0));
    }
				...
				tree access_path, bool preserve_reference)
{
  ...
  my_friendly_assert (DECL_P (member) || BASELINK_P (member), 20020801);
  ...
  if (TREE_CODE (member) == VAR_DECL)
			...
			TYPE_MAIN_VARIANT (member_scope)))
	{
	  ...
	  if (null_object_p && kind == bk_via_virtual)
	    {
	      error
		("invalid access to non-static data member `%D' of NULL object",
		 member);
	  ...
	  my_friendly_assert (object != error_mark_node, 20020801);
	}
	      ...
	      (TREE_TYPE (object), DECL_CONTEXT (member))))
	{
	  ...
	  anonymous_union = lookup_anon_field (TREE_TYPE (object),
					       DECL_CONTEXT (member));
	  object = build_class_member_access_expr (object, anonymous_union,
						   /*access_path= */
						   NULL_TREE,
						   preserve_reference);
	}
...
lookup_destructor (tree object, tree scope, tree dtor_name)
{
  ...
  if (!DERIVED_FROM_P (dtor_type, TYPE_MAIN_VARIANT (object_type)))
    {
      error
	("the type being destroyed is `%T', but the destructor refers to `%T'",
	 TYPE_MAIN_VARIANT (object_type), dtor_type);
      return error_mark_node;
    }
  if (!TYPE_HAS_DESTRUCTOR (dtor_type))
    return build (PSEUDO_DTOR_EXPR, void_type_node, object, scope, dtor_type);
...
get_member_function_from_ptrfunc (tree * instance_ptrptr, tree function)
{
  ...
  if (TYPE_PTRMEMFUNC_P (TREE_TYPE (function)))
	...
	e1 = build (COMPOUND_EXPR, TREE_TYPE (e1), instance_save_expr, e1);
...
build_function_call (tree function, tree params)
{
  ...
  if (!((TREE_CODE (fntype) == POINTER_TYPE
	...
	|| is_method || TREE_CODE (function) == TEMPLATE_ID_EXPR))
    {
...
convert_arguments (tree typelist, tree values, tree fndecl, int flags)
{
  ...
  for (valtail = values, typetail = typelist;
       valtail; valtail = TREE_CHAIN (valtail), i++)
    {
      ...
      if (type != 0)
	{
	  ...
	  if (!COMPLETE_TYPE_P (complete_type (type)))
		...
		(NULL_TREE, type, val, flags, "argument passing", fndecl, i);
  ...
  if (typetail != 0 && typetail != void_list_node)
	  ...
	  && TREE_CODE (TREE_PURPOSE (typetail)) != DEFAULT_ARG)
	{
	  for (; typetail != void_list_node; ++i)
	    {
	      tree parmval = convert_default_arg (TREE_VALUE (typetail),
		 ...
		 int convert_p ATTRIBUTE_UNUSED)
{
  ...
  switch (code)
	       ...
	       && same_type_p (type0, type1))
	{
	  ...
	  delta0 = build_ptrmemfunc_access_expr (op0, delta_identifier);
	  delta1 = build_ptrmemfunc_access_expr (op1, delta_identifier);
  ...
  if (				/* It's reasonable to use pointer values as operands of &&
       ...
       !(code == TRUTH_ANDIF_EXPR || code == TRUTH_ORIF_EXPR) && (	/* If OP0 is NULL and OP1 is not a pointer, or vice versa.  */
								   (orig_op0
								    ==
								    null_node
								    &&
...
pointer_diff (tree op0, tree op1, tree ptrtype)
{
  ...
  if (!COMPLETE_TYPE_P (TREE_TYPE (TREE_TYPE (op1))))
    error
      ("invalid use of a pointer to an incomplete type in pointer arithmetic");

  op1 = (TYPE_PTROB_P (ptrtype)
	 ? size_in_bytes (target_type) : integer_one_node);
  ...
  if (!exp && code == ADDR_EXPR)
      ...
      if (TREE_CODE (xarg) == OFFSET_REF)
	{
	  ...
	  if (!ptrmem && !flag_ms_extensions
	      && TREE_CODE (TREE_TYPE (TREE_OPERAND (xarg, 1))) ==
	      METHOD_TYPE)
	    {
...
build_unary_op (enum tree_code code, tree xarg, int noconvert)
{
  ...
  switch (code)
    ...
    case NEGATE_EXPR:
      if (!
	  (arg =
	   build_expr_type_conversion (WANT_ARITH | WANT_ENUM, arg, true)))
	...
	return
	  fold (build1 (REALPART_EXPR, TREE_TYPE (TREE_TYPE (arg)), arg));
      else
	return arg;
	...
	return
	  fold (build1 (IMAGPART_EXPR, TREE_TYPE (TREE_TYPE (arg)), arg));
      else
					      ...
					      arg, true)))
	{
      ...
      if (CP_TYPE_CONST_P (TREE_TYPE (arg)) || TREE_READONLY (arg))
	...
	switch (TREE_CODE (arg))
	  ...
	  case FIX_CEIL_EXPR:
		...
		value = save_expr (arg);
	      ...
	      compound =
		build (COMPOUND_EXPR, TREE_TYPE (arg), modify, value);
	...
	{
	  arg = build1
	    (CONVERT_EXPR, build_pointer_type (TREE_TYPE (argtype)), arg);
	  TREE_CONSTANT (arg) = TREE_CONSTANT (TREE_OPERAND (arg, 0));
	  return arg;
	}
      ...
      if (TREE_CODE (arg) == ARRAY_REF && !processing_template_decl)
	{
      ...
      if (TREE_CODE (arg) == IDENTIFIER_NODE && IDENTIFIER_OPNAME_P (arg))
	{
	  abort ();
	  ...
	  if (!flag_ms_extensions)
		...
		pedwarn
		  ("ISO C++ forbids taking the address of an unqualified"
		   ...
		   " a pointer to member function.  Say `&%T::%D'", base,
		   name);
	      else
		pedwarn
	...
	if (TREE_CODE (arg) != COMPONENT_REF
	...
	else if (TREE_CODE (TREE_OPERAND (arg, 1)) == BASELINK)
	  {
	    ...
	    my_friendly_assert (TREE_CODE (fn) == FUNCTION_DECL
				&& DECL_STATIC_FUNCTION_P (fn), 20030906);
	    mark_used (fn);
	  ...
	  {
	    error
	      ("attempt to take address of bit-field structure member `%D'",
	       TREE_OPERAND (arg, 1));
	    return error_mark_node;
	  }
	else
...
unary_complex_lvalue (enum tree_code code, tree arg)
{
  ...
  if (TREE_CODE (arg) == MODIFY_EXPR || TREE_CODE (arg) == INIT_EXPR)
    {
...
cxx_mark_addressable (tree exp)
{
  ...
  while (1)
	...
	if (x == current_class_ptr)
	  {
	    error
	      ("cannot take the address of `this', which is an rvalue expression");
	...
	my_friendly_assert (DECL_LANG_SPECIFIC (x) == 0
			    || DECL_IN_AGGR_P (x) == 0
			    || TREE_STATIC (x) || DECL_EXTERNAL (x), 314);
	/* Fall through.  */
...
tree
build_x_compound_expr_from_list (tree list, const char *msg)
{
...
build_static_cast (tree type, tree expr)
{
  ...
  if (result)
    {
      ...
      if (TREE_CODE (type) != REFERENCE_TYPE && real_lvalue_p (result))
  ...
  if ((TYPE_PTRMEM_P (type) && TYPE_PTRMEM_P (intype))
      ...
      if (TYPE_PTRMEM_P (type))
	{
	  ...
	  t2 = (build_ptrmem_type
		(c2, TYPE_MAIN_VARIANT (TYPE_PTRMEM_POINTED_TO_TYPE (type))));
	}
      else
	{
	  t1 = intype;
  ...
  if (TREE_CODE (intype) == POINTER_TYPE
      && VOID_TYPE_P (TREE_TYPE (intype)) && TYPE_PTROB_P (type))
    {
      check_for_casting_away_constness (intype, type, "static_cast");
      return build_nop (type, expr);
    }
  ...
  if (processing_template_decl)
    {
      ...
      if (!TREE_SIDE_EFFECTS (t) && type_dependent_expression_p (expr))
...
build_const_cast (tree type, tree expr)
{
  ...
  if (!POINTER_TYPE_P (type) && !TYPE_PTRMEM_P (type))
    error
      ("invalid use of const_cast with type `%T', which is not a pointer, reference, nor a pointer-to-data-member type",
       type);
  else if (TREE_CODE (TREE_TYPE (type)) == FUNCTION_TYPE)
    {
      error
	("invalid use of const_cast with type `%T', which is a pointer or reference to a function type",
	 type);
      return error_mark_node;
    }
  ...
  if (same_type_ignoring_top_level_qualifiers_p (intype, type))
  ...
  else if (TREE_CODE (type) == REFERENCE_TYPE)
    {
      if (!real_lvalue_p (expr))
	{
	  error ("invalid const_cast of an rvalue of type `%T' to type `%T'",
		 intype, type);
	  return error_mark_node;
	}
...
build_c_cast (tree type, tree expr)
{
  ...
  if (warn_cast_qual
      ...
      && !at_least_as_qualified_p (TREE_TYPE (type), TREE_TYPE (otype)))
    warning
      ("cast from `%T' to `%T' discards qualifiers from pointer target type",
       otype, type);
  ...
  if (STRICT_ALIGNMENT && warn_cast_align
    ...
    warning
      ("cast from `%T' to `%T' increases required alignment of target type",
       otype, type);
...
build_modify_expr (tree lhs, enum tree_code modifycode, tree rhs)
{
  ...
  if (modifycode == INIT_EXPR)
      ...
      else
	{
	  ...
	  my_friendly_assert (!PROMOTES_TO_AGGR_TYPE
			      (lhstype, REFERENCE_TYPE), 978652);
  ...
  if (modifycode != INIT_EXPR
	  ...
	  /* If it's an aggregate and any field is const, then it is
	     effectively const.  */
	  || (CLASS_TYPE_P (lhstype) && C_TYPE_FIELDS_READONLY (lhstype))))
...
build_x_modify_expr (tree lhs, enum tree_code modifycode, tree rhs)
{
  if (processing_template_decl)
    return build_min_nt (MODOP_EXPR, lhs,
			 build_min_nt (modifycode, NULL_TREE, NULL_TREE),
			 rhs);
...
build_ptrmemfunc1 (tree type, tree delta, tree pfn)
{
  ...
  u = tree_cons (pfn_field, pfn, build_tree_list (delta_field, delta));
  ...
  if (TYPE_PTRMEMFUNC_P (pfn_type))
    {
      ...
      if (!force && !can_convert_arg (to_type, TREE_TYPE (pfn), pfn))
	error ("invalid conversion to type `%T' from type `%T'",
	       to_type, pfn_type);

      n = get_delta_difference (TYPE_PTRMEMFUNC_OBJECT_TYPE (pfn_type),
				TYPE_PTRMEMFUNC_OBJECT_TYPE (to_type), force);
  ...
  if (integer_zerop (pfn))
    {
      ...
      return build_ptrmemfunc1 (to_type, integer_zero_node, pfn);
    }
  ...
  if (!DECL_VIRTUAL_P (fn))
    *pfn = convert (TYPE_PTRMEMFUNC_FN_TYPE (type), build_addr_func (fn));
	...
	default:
      ...
      *pfn = fold (build1 (NOP_EXPR, TYPE_PTRMEMFUNC_FN_TYPE (type), *pfn));
    }
}
  ...
  if (!can_convert_arg_bad (type, rhstype, rhs))
    {
      ...
      if (!warn_pmf2ptr && TYPE_PTR_P (type) && TYPE_PTRMEMFUNC_P (rhstype))
  ...
  if (codel == REFERENCE_TYPE)
      ...
      if (fndecl)
	{
	  if (warningcount > savew)
	    cp_warning_at ("in passing argument %P of `%+D'", parmnum,
			   fndecl);
...
maybe_warn_about_returning_address_of_local (tree retval)
{
  ...
  if (TREE_CODE (whats_returned) == VAR_DECL
      ...
      && !(TREE_STATIC (whats_returned) || TREE_PUBLIC (whats_returned)))
    {
      if (TREE_CODE (valtype) == REFERENCE_TYPE)
...
check_return_expr (tree retval)
{
  ...
  if (DECL_DESTRUCTOR_P (current_function_decl))
	...
	error
	  ("cannot return from a handler of a function-try-block of a constructor");
      else if (retval)
  ...
  if ((DECL_OVERLOADED_OPERATOR_P (current_function_decl) == NEW_EXPR
      ...
      && !flag_check_new && null_ptr_cst_p (retval))
    warning
  ...
  for (;; to = TREE_TYPE (to), from = TREE_TYPE (from))
    {
      ...
      if (TREE_CODE (to) == VOID_TYPE || TREE_CODE (from) == VOID_TYPE)
	return 1;
...
casts_away_constness_r (tree * t1, tree * t2)
{
  ...
  if ((!TYPE_PTR_P (*t1) && !TYPE_PTRMEM_P (*t1))
    ...
    {
      *t1 = cp_build_qualified_type (void_type_node, cp_type_quals (*t1));
      *t2 = cp_build_qualified_type (void_type_node, cp_type_quals (*t2));
      return;
    }
