...
#define GTB_IGNORE_TYPE 2	/* We don't need to try to unify the current
     ...
     static int maybe_adjust_types_for_deduction (unification_kind_t, tree *,
						  tree *);
     static int type_unification_real (tree, tree, tree, tree, int,
     ...
     static tree tsubst_call_declarator_parms (tree, tree, tsubst_flags_t,
					       tree);
     static tree get_template_base_recursive (tree, tree, tree, tree, tree,
					      int);
...
/* Make the current scope suitable for access checking when we are
   ...
   instantiate_decl).  */
     ...
     static void push_access_scope (tree t)
{
  my_friendly_assert (TREE_CODE (t) == FUNCTION_DECL
		      || TREE_CODE (t) == VAR_DECL, 0);
...
maybe_end_member_template_processing (void)
{
  ...
  --inline_parm_levels_used;
  for (i = 0;
       i < VARRAY_INT (inline_parm_levels, inline_parm_levels_used); ++i)
    {
      --processing_template_decl;
...
check_specialization_scope (void)
{
  ...
  if (scope && TREE_CODE (scope) != NAMESPACE_DECL)
    error ("explicit specialization in non-namespace scope `%D'", scope);
...
retrieve_specialization (tree tmpl, tree args)
{
  ...
  my_friendly_assert (TMPL_ARGS_DEPTH (args)
		      == TMPL_PARMS_DEPTH (DECL_TEMPLATE_PARMS (tmpl)), 0);
...
is_specialization_of_friend (tree decl, tree friend)
{
  ...
  if (TREE_CODE (friend) == FUNCTION_DECL
      && DECL_TEMPLATE_INFO (friend) && !DECL_USE_TEMPLATE (friend))
    {
  ...
  for (s = DECL_TEMPLATE_SPECIALIZATIONS (tmpl);
      ...
      else if (comp_template_args (TREE_PURPOSE (s), args))
	{
	  if (DECL_TEMPLATE_SPECIALIZATION (spec))
		...
		{
		  if (TREE_USED (fn) || DECL_EXPLICIT_INSTANTIATION (fn))
		    {
		      error ("specialization of %D after instantiation", fn);
		      return spec;
		    }
		  else
		    {
...
determine_specialization (tree template_id,
			  tree decl,
			  tree * targs_out, int need_member_template)
{
  tree fns;
  tree targs;
  tree explicit_targs;
  ...
  *targs_out = NULL_TREE;
  ...
  for (; fns; fns = OVL_NEXT (fns))
      ...
      else
	{
	  ...
	  if (compparms (TYPE_ARG_TYPES (TREE_TYPE (fn)), decl_arg_types))
	    /* They match!  */
...
static tree
copy_default_args_to_explicit_spec_1 (tree spec_types, tree tmpl_types)
{
  tree new_spec_types;
  ...
  return hash_tree_cons (TREE_PURPOSE (tmpl_types),
			 TREE_VALUE (spec_types), new_spec_types);
}
...
copy_default_args_to_explicit_spec (tree decl)
{
  ...
  if (object_type)
    ...
    new_type = build_function_type (TREE_TYPE (old_type), new_spec_types);
...
check_explicit_specialization (tree declarator,
			       tree decl, int template_count, int flags)
{
  ...
  switch (tsk)
      ...
      if (template_header_count)
	{
	  ...
	  return decl;
	       ...
	       && CLASSTYPE_TEMPLATE_INSTANTIATION (ctype) && !is_friend)
	{
  ...
  if (specialization || member_specialization)
	...
	if (TREE_PURPOSE (t))
	  {
	    pedwarn ("default argument specified in explicit specialization");
	    break;
	  }
  ...
  if (specialization || member_specialization || explicit_instantiation)
		   ...
		   IDENTIFIER_NODE))
	{
	  ...
	  if (fns == NULL_TREE)
	    {
	      error ("no member function `%D' declared in `%T'", name, ctype);
	      return error_mark_node;
	    }
	  else
...
check_template_shadow (tree decl)
{
  ...
  if (decl == olddecl || TEMPLATE_PARMS_FOR_INLINE (current_template_parms))
    return;
...
build_template_parm_index (int index,
			   int level, int orig_level, tree decl, tree type)
{
...
process_partial_specialization (tree decl)
{
  ...
  for (i = 0; i < ntparms; ++i)
	...
	error ("        `%D'", TREE_VALUE (TREE_VEC_ELT (inner_parms, i)));
      }
  ...
  if (comp_template_args
      (inner_args,
       INNERMOST_TEMPLATE_ARGS (CLASSTYPE_TI_ARGS (TREE_TYPE (maintmpl)))))
    error
  ...
  for (i = 0; i < nargs; ++i)
	   ...
	   && TREE_CODE (arg) != TEMPLATE_PARM_INDEX)
	{
	  if (tpd.arg_uses_template_parms[i])
	    error ("template argument `%E' involves template parameter(s)",
		   arg);
	  else
	    {
	      ...
	      if (!tpd2.parms)
		{
	      ...
	      for_each_template_parm (type, &mark_template_parm, &tpd2, NULL);
	      ...
	      if (tpd2.arg_uses_template_parms[i])
		{
		  ...
		  int j;
		  for (j = 0; j < nargs; ++j)
		    if (tpd2.parms[j] != 0 && tpd.arg_uses_template_parms[j])
		      {
			error
			  ("type `%T' of template argument `%E' depends on template parameter(s)",
			   type, arg);
			break;
		      }
		}
			 ...
			 int is_partial)
{
  ...
  if (current_class_type
      && !TYPE_BEING_DEFINED (current_class_type) && DECL_LANG_SPECIFIC (decl)
...
push_template_decl_real (tree decl, int is_friend)
{
  ...
  check_default_tmpl_args (decl, current_template_parms, primary, is_partial);
  ...
  if (!ctx
    ...
    {
      if (DECL_LANG_SPECIFIC (decl)
	  && DECL_TEMPLATE_INFO (decl) && DECL_TI_TEMPLATE (decl))
	tmpl = DECL_TI_TEMPLATE (decl);
	       ...
	       && TYPE_TI_TEMPLATE (TREE_TYPE (decl)))
	{
	  ...
	  redeclare_class_template (TREE_TYPE (decl), current_template_parms);
	...
	tmpl = DECL_TI_TEMPLATE (decl);
	  ...
	  && DECL_TEMPLATE_SPECIALIZATION (decl) && is_member_template (tmpl))
	{
	  tree new_tmpl;
	  ...
	  new_tmpl = build_template_decl (decl, current_template_parms);
	  DECL_TEMPLATE_RESULT (new_tmpl) = decl;
	  TREE_TYPE (new_tmpl) = TREE_TYPE (decl);
	  DECL_TI_TEMPLATE (decl) = new_tmpl;
	  SET_DECL_TEMPLATE_SPECIALIZATION (new_tmpl);
	  DECL_TEMPLATE_INFO (new_tmpl) = tree_cons (tmpl, args, NULL_TREE);

	  register_specialization (new_tmpl,
				   most_general_template (tmpl), args);
	  return decl;
	}
  ...
  if (primary)
      ...
      if (DECL_CONV_FN_P (tmpl))
	{
	  ...
	  if (uses_template_parms_level (TREE_TYPE (TREE_TYPE (tmpl)), depth))
  ...
  if (TREE_VEC_LENGTH (parms) != TREE_VEC_LENGTH (tmpl_parms))
    {
      ...
      error ("used %d template parameter(s) instead of %d",
	     TREE_VEC_LENGTH (tmpl_parms), TREE_VEC_LENGTH (parms));
      return;
    }
...
convert_nontype_argument (tree type, tree expr)
{
  ...
  if (INTEGRAL_TYPE_P (expr_type) && TREE_CODE (type) != REFERENCE_TYPE)
	...
	/* In a template, the initializer for a VAR_DECL may not be
	   ...
	   here.  */
	if (expr == const_expr && DECL_INTEGRAL_CONSTANT_VAR_P (expr)
  ...
  if (is_overloaded_fn (expr))
	...
	{
	  error
	    ("string literal %E is not a valid template argument because it is the address of an object with static linkage",
	     referent);
	  return NULL_TREE;
	}
  ...
  switch (TREE_CODE (type))
	...
	if (!TYPE_PTRMEMFUNC_P (expr_type) && expr_type != unknown_type_node)
	  return error_mark_node;
...
convert_template_argument (tree parm,
			   tree arg,
			   tree args,
			   tsubst_flags_t complain, int i, tree in_decl)
{
  tree val;
  tree inner_args;
  ...
  requires_type = (TREE_CODE (parm) == TYPE_DECL || requires_tmpl_type);
	...
	("to refer to a type member of a template parameter, use `typename %E'",
	 arg);
      ...
      arg = make_typename_type (TREE_OPERAND (arg, 0),
				TREE_OPERAND (arg, 1), complain & tf_error);
      is_type = 1;
    }
  ...
  if (is_type)
	  ...
	  if (TREE_CODE (TREE_TYPE (arg)) == UNBOUND_CLASS_TEMPLATE)
	      ...
	      else
		{
		  if (in_decl && (complain & tf_error))
			...
			("type/value mismatch at argument %d in template parameter list for `%D'",
			 i + 1, in_decl);
		      error ("  expected a template of type `%D', got `%D'",
			     parm, arg);
		    }
	...
	error ("could not convert template argument `%E' to `%T'", arg, t);
    }
...
coerce_template_parms (tree parms,
		       tree args,
		       tree in_decl,
		       tsubst_flags_t complain, int require_all_arguments)
{
  ...
  for (i = 0; i < nparms; i++)
	...
	arg = convert_template_argument (TREE_VALUE (parm),
					 arg, new_args, complain, i, in_decl);
	  ...
	  if (TREE_CODE (arg) == TEMPLATE_DECL)
	      ...
	      if (context)
		{
		  ...
		  my_friendly_assert (TREE_CODE (context) == NAMESPACE_DECL
				      || CLASS_TYPE_P (context), 980422);
		  cat (decl_as_string
		       (DECL_CONTEXT (arg), TFF_PLAIN_IDENTIFIER));
		  cat ("::");
		}
...
{
  tree ti = (TYPE_P (d)
	     ? CLASSTYPE_TEMPLATE_INFO (d) : DECL_TEMPLATE_INFO (d));
  tree pt;
  int level;
  ...
  if (BASELINK_P (fns))
    {
      BASELINK_FUNCTIONS (fns) = build (TEMPLATE_ID_EXPR,
					unknown_type_node,
					BASELINK_FUNCTIONS (fns), arglist);
      return fns;
    }
  ...
  if (DECL_TEMPLATE_TEMPLATE_PARM_P (template))
      ...
      if (parm_depth > 1)
	{
	  ...
	  for (i = saved_depth,
	       ...
	       i > 0 && t != NULL_TREE; --i, t = TREE_CHAIN (t))
	    {
	...
	POP_TIMEVAR_AND_RETURN (TV_NAME_LOOKUP, error_mark_node);
      ...
      if (comp_template_args (TYPE_TI_ARGS (template_type), arglist))
	{
	  found = template_type;
	...
	if (comp_template_args (TREE_PURPOSE (*tp), arglist))
	  {
	    ...
	    /* Use the move-to-front heuristic to speed up future
	       searches.  */
	    *tp = TREE_CHAIN (*tp);
	    TREE_CHAIN (found) = DECL_TEMPLATE_INSTANTIATIONS (template);
	  ...
	  && TREE_CODE (CP_DECL_CONTEXT (template)) == NAMESPACE_DECL)
	{
      ...
      context = tsubst (DECL_CONTEXT (template), arglist, complain, in_decl);
      if (!context)
	...
	= tree_cons (arglist, t, DECL_TEMPLATE_INSTANTIATIONS (template));

      if (TREE_CODE (t) == ENUMERAL_TYPE && !is_partial_instantiation)
...
for_each_template_parm_r (tree * tp, int *walk_subtrees, void *d)
{
  ...
  switch (TREE_CODE (t))
	...
	*walk_subtrees = 0;
    ...
    case TYPEOF_TYPE:
      if (for_each_template_parm (TYPE_FIELDS (t), fn, data, pfd->visited))
	return error_mark_node;
      break;
    ...
    case CONSTRUCTOR:
	  ...
	  && for_each_template_parm (TYPE_PTRMEMFUNC_FN_TYPE
				     (TREE_TYPE (t)), fn, data, pfd->visited))
	return error_mark_node;
      break;
...
for_each_template_parm (tree t, tree_fn_t fn, void *data, htab_t visited)
{
  ...
  if (visited)
    ...
    pfd.visited = htab_create (37, htab_hash_pointer, htab_eq_pointer, NULL);
  result = walk_tree (&t,
		      for_each_template_parm_r,
		      &pfd, pfd.visited) != NULL_TREE;
...
push_tinst_level (tree d)
{
  ...
  if (tinst_depth >= max_tinst_depth)
	...
	return 0;
      ...
      error
	("template instantiation depth exceeds maximum of %d (use -ftemplate-depth-NN to increase the maximum) instantiating `%D'",
	 max_tinst_depth, d);
...
tsubst_friend_function (tree decl, tree args)
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL
       ...
       instantiation of what template.  */
    {
      ...
      tmpl = determine_specialization (template_id, new_friend, &new_args,
...
instantiate_class_template (tree type)
{
  ...
  if (TYPE_BEING_DEFINED (type)
      || COMPLETE_TYPE_P (type) || dependent_type_p (type))
    return type;
  ...
  if (t)
    {
      ...
      tree inner_args = get_class_bindings (TREE_VALUE (t), TREE_PURPOSE (t),
					    args);
  ...
  TYPE_HAS_ABSTRACT_ASSIGN_REF (type) =
	  ...
	  if (TYPE_P (t))
	      ...
	      if (TREE_CODE (t) != CONST_DECL)
		{
		  ...
		  if (TREE_CODE (r) == VAR_DECL)
			...
			(r, init, /*asmspec_tree= */ NULL_TREE, /*flags= */
			 0);
...
tsubst_template_parms (tree parms, tree args, tsubst_flags_t complain)
{
  ...
  for (new_parms = &r;
       ...
       new_parms = &(TREE_CHAIN (*new_parms)), parms = TREE_CHAIN (parms))
    {
      tree new_vec = make_tree_vec (TREE_VEC_LENGTH (TREE_VALUE (parms)));
      int i;
	...
	tree_cons (size_int (TMPL_PARMS_DEPTH (parms)
			     - TMPL_ARGS_DEPTH (args)), new_vec, NULL_TREE);
    }
...
tsubst_aggr_type (tree t,
		  tree args,
		  tsubst_flags_t complain, tree in_decl, int entering_scope)
{
  if (t == NULL_TREE)
    return NULL_TREE;
...
tsubst_default_arguments (tree fn)
{
  ...
  for (arg = TYPE_ARG_TYPES (TREE_TYPE (fn)); arg; arg = TREE_CHAIN (arg))
    if (TREE_PURPOSE (arg))
...
tsubst_decl (tree t, tree args, tree type, tsubst_flags_t complain)
{
  ...
  switch (TREE_CODE (t))
	...
	DECL_TEMPLATE_PARMS (r)
	  = tsubst_template_parms (DECL_TEMPLATE_PARMS (t), args, complain);
	...
	DECL_ARGUMENTS (r) = tsubst (DECL_ARGUMENTS (t), args, complain, t);
	...
	if (gen_tmpl)
	  {
	    DECL_TEMPLATE_INFO (r) = tree_cons (gen_tmpl, argvec, NULL_TREE);
	    SET_DECL_IMPLICIT_INSTANTIATION (r);
	...
	DECL_NAME (r) = tsubst_copy (DECL_NAME (t), args, complain, in_decl);
	TREE_CHAIN (r) = NULL_TREE;
      }
      break;
	...
	if (TYPE_P (CP_DECL_CONTEXT (t)))
	  ctx = tsubst_aggr_type (DECL_CONTEXT (t), args,
				  complain, in_decl, /*entering_scope= */ 1);
		      ...
		      tree args, tsubst_flags_t complain, tree in_decl)
{
  ...
  arg_types = tsubst_arg_types (TYPE_ARG_TYPES (t), args, complain, in_decl);
  ...
  if (TREE_CODE (t) == FUNCTION_TYPE)
      ...
      if (!IS_AGGR_TYPE (r))
	{
	  ...
	  if (complain & tf_error)
	    error
	      ("creating pointer to member function of non-class type `%T'",
	       r);
	  return error_mark_node;
	}
...
tsubst (tree t, tree args, tsubst_flags_t complain, tree in_decl)
{
  ...
  if (t == NULL_TREE || t == error_mark_node
      ...
      || t == void_type_node
      || t == char_type_node
      || t == unknown_type_node || TREE_CODE (t) == NAMESPACE_DECL)
    return t;
  ...
  if (type && TREE_CODE (t) != FUNCTION_DECL
      ...
      && TREE_CODE (t) != TEMPLATE_DECL
      && TREE_CODE (t) != IDENTIFIER_NODE
      && TREE_CODE (t) != FUNCTION_TYPE && TREE_CODE (t) != METHOD_TYPE)
    type = tsubst (type, args, complain, in_decl);
  if (type == error_mark_node)
    return error_mark_node;
  ...
  switch (TREE_CODE (t))
	...
	if (purpose == TREE_PURPOSE (t)
	    && value == TREE_VALUE (t) && chain == TREE_CHAIN (t))
	  return t;
	if (TREE_PARMLIST (t))
	  {
	...
	if (CLASS_TYPE_P (type) && CLASSTYPE_PURE_VIRTUALS (type))
	  {
	    if (complain & tf_error)
	      error
		("creating array of `%T', which is an abstract class type",
		 type);
	    return error_mark_node;
	  }
	...
	if (!IS_AGGR_TYPE (ctx))
	  {
	    if (complain & tf_error)
	      error ("`%T' is not a class, struct, or union type", ctx);
	    return error_mark_node;
	  }
    ...
    default:
      sorry ("use of `%s' in template", tree_code_name[(int) TREE_CODE (t)]);
      return error_mark_node;
    }
}
...
tsubst_copy (tree t, tree args, tsubst_flags_t complain, tree in_decl)
{
  ...
  switch (code)
	...
	enum_type = tsubst_aggr_type (TREE_TYPE (t), args, complain, in_decl,
				      /*entering_scope= */ 0);

	for (v = TYPE_VALUES (enum_type); v != NULL_TREE; v = TREE_CHAIN (v))
...
tsubst_copy_and_build (tree t,
		       tree args,
		       tsubst_flags_t complain, tree in_decl, bool function_p)
{
#define RECUR(NODE) \
  ...
  switch (TREE_CODE (t))
	...
	(TREE_CODE (t),
	 RECUR (TREE_OPERAND (t, 0)), RECUR (TREE_OPERAND (t, 1)),
	 /*overloaded_p= */ NULL);
	...
	(RECUR (TREE_OPERAND (t, 0)),
	 TREE_CODE (TREE_OPERAND (t, 1)), RECUR (TREE_OPERAND (t, 2)));
	...
	(RECUR (TREE_OPERAND (t, 0)),
	 ...
	 RECUR (TREE_OPERAND (t, 2)), NEW_EXPR_USE_GLOBAL (t));
	...
	if (koenig_p && ((is_overloaded_fn (function)
	...
	(RECUR (TREE_OPERAND (t, 0)),
	 RECUR (TREE_OPERAND (t, 1)), RECUR (TREE_OPERAND (t, 2)));
	...
	else if (!CLASS_TYPE_P (TREE_TYPE (object)))
	  {
	    if (TREE_CODE (member) == BIT_NOT_EXPR)
	      return finish_pseudo_destructor_expr (object,
		     ...
		     && (TREE_CODE (TREE_OPERAND (member, 1)) ==
			 BIT_NOT_EXPR))
	      return finish_pseudo_destructor_expr (object, object,
						    TREE_TYPE (object));
	  }
	else if (TREE_CODE (member) == SCOPE_REF
    ...
    case THROW_EXPR:
      return build_throw (RECUR (TREE_OPERAND (t, 0)));
    ...
    case PARM_DECL:
      return
	convert_from_reference (tsubst_copy (t, args, complain, in_decl));
...
fn_type_unification (tree fn,
		     ...
		     tree targs,
		     tree args,
		     tree return_type, unification_kind_t strict, int len)
{
  tree parms;
  tree fntype;
  int result;
  ...
  if (result == 0)
    ...
    if (tsubst (TREE_TYPE (fn), targs, tf_none, NULL_TREE) == error_mark_node)
      return 1;
...
again:
  ...
  while (parms && parms != void_list_node && args && args != void_list_node)
    {
	  ...
	  if (type_unknown_p (arg))
	    {
		  ...
		  (tparms, targs, parm, arg, strict, sub_strict) != 0)
		return 1;
	      continue;
	    }
	  arg = TREE_TYPE (arg);
  ...
  if (parms && parms != void_list_node && TREE_PURPOSE (parms) == NULL_TREE)
    return 1;
...
resolve_overloaded_unification (tree tparms,
				...
				tree parm,
				tree arg,
				unification_kind_t strict, int sub_strict)
{
...
try_one_overload (tree tparms,
		  ...
		  tree parm,
		  tree arg,
		  unification_kind_t strict, int sub_strict, bool addr_p)
{
  int nargs;
  tree tempargs;
  int i;
...
get_template_base_recursive (tree tparms,
			     tree targs,
			     tree parm, tree arg_binfo, tree rval, int flags)
{
  tree binfos;
  int i, n_baselinks;
...
unify (tree tparms, tree targs, tree parm, tree arg, int strict)
{
  ...
  if (TREE_CODE (arg) == TREE_CODE (parm) && TYPE_P (arg)
  ...
  switch (TREE_CODE (parm))
    ...
    case BOUND_TEMPLATE_TEMPLATE_PARM:
      ...
      if (TEMPLATE_TYPE_LEVEL (parm) != template_decl_level (tparm))
      ...
      if (TREE_CODE (parm) == BOUND_TEMPLATE_TEMPLATE_PARM)
	{
...

	  {
		...
		if (unify (tparms, targs, t,
			   TREE_VEC_ELT (argvec, i), UNIFY_ALLOW_NONE))
		  return 1;
	      }
	  }
    ...
    case TEMPLATE_PARM_INDEX:
      ...
      if (TEMPLATE_PARM_LEVEL (parm) != template_decl_level (tparm))
	/* The PARM is not one we're trying to unify.  Just check
	   to see if it matches ARG.  */
	return !(TREE_CODE (arg) == TREE_CODE (parm)
	  ...
	  if (strict_in & UNIFY_ALLOW_DERIVED)
	    {
	      ...
	      t = try_class_unification (tparms, targs, parm, arg);
	...
	return 1;
      return unify (tparms, targs, TREE_TYPE (parm), TREE_TYPE (arg), strict);
...
more_specialized_class (tree pat1, tree pat2, tree full_args)
{
  ...
  targs = get_class_bindings (TREE_VALUE (pat1), TREE_PURPOSE (pat1),
			      add_outermost_template_args (full_args,
							   TREE_PURPOSE
							   (pat2)));
  if (targs)
    --winner;
...
get_bindings_real (tree fn,
		   tree decl,
		   tree explicit_args, int check_rettype, int deduce, int len)
{
...
do_decl_instantiation (tree decl, tree storage)
{
  ...
  if (storage == NULL_TREE)
    ...
    error ("storage class `%D' applied to template instantiation", storage);
...
instantiate_decl (tree d, int defer_ok)
{
  ...
  if (pattern_defined)
	  ...
	  || DECL_MAYBE_IN_CHARGE_DESTRUCTOR_P (d))
	{
	  ...
	  for (t = TREE_CHAIN (d);
	       t && DECL_CLONED_FUNCTION_P (t); t = TREE_CHAIN (t))
  ...
  /* Defer all other templates, unless we have been explicitly
  ...
  else if (!pattern_defined || defer_ok)
    {
      ...
      if (at_eof && !pattern_defined && DECL_EXPLICIT_INSTANTIATION (d))
	/* [temp.explicit]
      ...
      else
	{
	  ...
	  cp_finish_decl (d,
			  (!DECL_INITIALIZED_IN_CLASS_P (d)
			   ? DECL_INITIAL (d) : NULL_TREE), NULL_TREE, 0);
...
get_mostly_instantiated_function_type (tree decl)
{
  ...
  if (parm_depth == 1)
	...
	SET_TMPL_ARGS_LEVEL (partial_args, i, TMPL_ARGS_LEVEL (targs, i));
...
dependent_type_p_r (tree type)
{
  ...
  if (TYPE_PTR_TO_MEMBER_P (type))
    return (dependent_type_p (TYPE_PTRMEM_CLASS_TYPE (type))
	    || dependent_type_p (TYPE_PTRMEM_POINTED_TO_TYPE (type)));
	...
	return true;
      for (arg_type = TYPE_ARG_TYPES (type);
	   arg_type; arg_type = TREE_CHAIN (arg_type))
...
dependent_scope_ref_p (tree expression, bool criterion (tree))
{
  ...
  /* The suggested resolution to Core Issue 2 implies that if the
     ...
     inside it.  */
  if (DECL_P (name) && currently_open_class (scope) && !criterion (name))
    return false;
  if (dependent_type_p (scope))
    return true;
...
value_dependent_expression_p (tree expression)
{
  ...
  if (TREE_CODE (expression) == IDENTIFIER_NODE
      || (DECL_P (expression) && type_dependent_expression_p (expression)))
    return true;
...
type_dependent_expression_p (tree expression)
{
  ...
  if (TREE_CODE (expression) == SCOPE_REF
      && dependent_scope_ref_p (expression, type_dependent_expression_p))
    return true;
...
dependent_template_p (tree tmpl)
{
  ...
  if (TREE_CODE (tmpl) == SCOPE_REF || TREE_CODE (tmpl) == IDENTIFIER_NODE)
    return true;
...
resolve_typename_type (tree type, bool only_current_p)
{
  ...
  my_friendly_assert (TREE_CODE (type) == TYPENAME_TYPE, 20010702);
...
build_non_dependent_expr (tree expr)
{
  ...
  if (TREE_CODE (expr) == COMPOUND_EXPR && !COMPOUND_EXPR_OVERLOADED (expr))
