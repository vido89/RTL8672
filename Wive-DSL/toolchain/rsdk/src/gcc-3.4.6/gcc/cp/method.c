  ...
  if (this_adjusting && virtual_offset)
    virtual_offset
      = size_binop (MULT_EXPR,
		    virtual_offset,
		    convert (ssizetype, TYPE_SIZE_UNIT (vtable_entry_type)));
...
/* Create a static alias to function.  */
     ...
     static tree make_alias_for_thunk (tree function)
{
  tree alias;
  char buf[256];
...
locate_dtor (tree type, void *client ATTRIBUTE_UNUSED)
{
  ...
  if (!TYPE_HAS_DESTRUCTOR (type))
    return NULL_TREE;
  fns = TREE_VEC_ELT (CLASSTYPE_METHOD_VEC (type), CLASSTYPE_DESTRUCTOR_SLOT);
  return fns;
}
