...
static bool lookup_using_namespace (tree, cxx_binding *, tree, tree, int);
...
/* A free list of "binding_entry"s awaiting for re-use.  */
     ...
     binding_entry free_binding_entry = NULL;

/* Create a binding_entry object for (NAME, TYPE).  */

     static inline binding_entry binding_entry_make (tree name, tree type)
{
  binding_entry entry;
...
supplement_binding (cxx_binding * binding, tree decl)
{
  ...
  if (TREE_CODE (decl) == TYPE_DECL && DECL_ARTIFICIAL (decl))
    ...
    binding->type = decl;
	    ...
	    || (TREE_CODE (bval) == FUNCTION_DECL && DECL_ANTICIPATED (bval)))
    binding->value = decl;
...
pushdecl (tree x)
{
  ...
  if (DECL_TEMPLATE_PARM_P (x))
	...
	DECL_CONTEXT (x) = current_function_decl;
	  ...
	  && current_function_decl && x != current_function_decl)
  ...
  if (name)
      ...
      else if (t != NULL_TREE)
	{
	  if (different_binding_level)
		...
		POP_TIMEVAR_AND_RETURN (TV_NAME_LOOKUP, t);
		    ...
		    || DECL_FUNCTION_TEMPLATE_P (x)) && is_overloaded_fn (t))
	    /* Don't do anything just yet.  */ ;
	  else if (t == wchar_decl_node)
	    {
	      if (pedantic && !DECL_IN_SYSTEM_HEADER (x))
		pedwarn ("redeclaration of `wchar_t' as `%T'", TREE_TYPE (x));
      ...
      if (TREE_PUBLIC (x) && TREE_CODE (x) != FUNCTION_DECL)
	{
	  ...
	  if (decl && decl != error_mark_node
	    ...
	    {
	      pedwarn ("type mismatch with previous external decl of `%#D'",
		       x);
      ...
      else
	{
	  ...
	  if (oldlocal != NULL_TREE && !DECL_EXTERNAL (x)
	      ...
	      && !DECL_FROM_INLINE (x) && TREE_CODE (oldlocal) == PARM_DECL
	  ...
	  /* Maybe warn if shadowing something else.  */
		   ...
		   && !DECL_FROM_INLINE (x))
	    {
		  ...
		  && current_class_ptr && !TREE_STATIC (name))
		{
...
maybe_push_decl (tree decl)
{
  ...
  if (decl == error_mark_node
      || (TREE_CODE (decl) != PARM_DECL && DECL_CONTEXT (decl) != NULL_TREE
...
maybe_inject_for_scope_var (tree decl)
{
  ...
  if (current_binding_level->kind == sk_for)
    {
      struct cp_binding_level *outer = current_binding_level->level_chain;
...
check_for_out_of_scope_variable (tree decl)
{
  ...
  if (shadowed)
	...
	{
	  warning ("name lookup of `%D' changed", DECL_NAME (decl));
...
namespace_scope_ht_size (tree ns)
{
  ...
  return name == std_identifier
    ...
    : (name == global_scope_name
       ? GLOBAL_SCOPE_HT_SIZE : NAMESPACE_ORDINARY_HT_SIZE);
}
...
static
GTY ((deletable ("")))
     struct cp_binding_level *free_binding_level;

/* Create a new KIND scope and make it the top of the active scopes stack.
   ENTITY is the scope of the associated C++ entity (namespace, class,
   function); it is NULL otherwise.  */

     cxx_scope *begin_scope (scope_kind kind, tree entity)
{
  cxx_scope *scope;
...
leave_scope (void)
{
  ...
  if (scope->kind != sk_namespace)
	...
	binding_table_free (scope->type_decls);
      my_friendly_assert (!ENABLE_SCOPE_CHECKING
			  || scope->binding_depth == binding_depth, 20030529);
  ...
  for (scope = current_binding_level;
       scope && scope->kind != sk_class; scope = scope->level_chain)
    ;
	  ...
	  for (tmp = old; tmp; tmp = OVL_NEXT (tmp))
	    {
		  ...
		  && !decls_match (fn, decl))
		error
		  ("`%#D' conflicts with previous using declaration `%#D'",
		   decl, fn);
  ...
  if (doing_global)
      ...
      if (TREE_CODE (new_binding) == OVERLOAD && old)
	{
	  ...
	  for (d = &IDENTIFIER_BINDING (name)->scope->names;
	       *d; d = &TREE_CHAIN (*d))
	    if (*d == old
		|| (TREE_CODE (*d) == TREE_LIST && TREE_VALUE (*d) == old))
	      {
		if (TREE_CODE (*d) == TREE_LIST)
		  ...
		  *d = tree_cons (NULL_TREE, new_binding, TREE_CHAIN (*d));
  ...
  if (TREE_CODE (decl) == TEMPLATE_ID_EXPR)
    {
      ...
      error
	("a using-declaration cannot specify a template-id.  Try `using %D'",
	 name);
      return NULL_TREE;
    }
  ...
  if (newval)
      ...
      if (is_overloaded_fn (newval))
	{
	  ...
	  for (fn = newval; fn && OVL_CURRENT (fn) != term;
	       fn = OVL_NEXT (fn))
	    push_overloaded_decl (OVL_CURRENT (fn), PUSH_LOCAL | PUSH_USING);
	}
      else
...
push_scope (tree t)
{
  ...
  if (TREE_CODE (t) == NAMESPACE_DECL)
    ...
    {
      if (!at_class_scope_p () || !same_type_p (current_class_type, t))
	push_nested_class (t);
      else
...
poplevel_class (void)
{
  ...
  if (current_class_depth != 1)
    {
      ...
      for (shadowed = level->class_shadowed;
	   shadowed; shadowed = TREE_CHAIN (shadowed))
  ...
  for (shadowed = level->type_shadowed;
       shadowed; shadowed = TREE_CHAIN (shadowed))
    SET_IDENTIFIER_TYPE_VALUE (TREE_PURPOSE (shadowed),
			       TREE_VALUE (shadowed));
...
clear_identifier_class_values (void)
{
  ...
  for (t = class_binding_level->class_shadowed; t; t = TREE_CHAIN (t))
...
push_class_level_binding (tree name, tree x)
{
  ...
  if ((TREE_CODE (x) == VAR_DECL
       || TREE_CODE (x) == CONST_DECL
       || (TREE_CODE (x) == TYPE_DECL
	   && !DECL_SELF_REFERENCE_P (x)) || DECL_CLASS_TEMPLATE_P (x)
  ...
  if (binding && binding->value)
      ...
      if (old_decl)
	{
	  ...
	  for (shadow = class_binding_level->class_shadowed;
	       shadow; shadow = TREE_CHAIN (shadow))
  ...
  if (push_class_binding (name, x))
	...
	= tree_cons (name, NULL, class_binding_level->class_shadowed);
...
do_class_using_decl (tree decl)
{
  ...
  if (TREE_CODE (decl) != SCOPE_REF
      || !TREE_OPERAND (decl, 0) || !TYPE_P (TREE_OPERAND (decl, 0)))
    {
...
complain:
  error ("`%D' should have been declared inside `%D'", decl, scope);
}
...
ambiguous_decl (tree name, cxx_binding * old, cxx_binding * new, int flags)
{
  ...
  if (val)
	...
	if (LOOKUP_QUALIFIERS_ONLY (flags) && !DECL_CLASS_TEMPLATE_P (val))
	  val = NULL_TREE;
	break;
      case TYPE_DECL:
      ...
      else
	{
	  ...
	  if (flags & LOOKUP_COMPLAIN)
	      ...
	      if (old->value != error_mark_node)
		{
		  ...
		  cp_error_at ("  first declared as `%#D' here", old->value);
		}
  ...
  if (binding.value)
      ...
      if (template_id)
	{
	  if (DECL_CLASS_TEMPLATE_P (val))
	    val = lookup_template_class (val, TREE_OPERAND (template_id, 1),
					 /*in_decl= */ NULL_TREE,
					 /*context= */ NULL_TREE,
					 /*entering_scope= */ 0,
					 tf_error | tf_warning);
	...
	val = OVL_FUNCTION (val);
      ...
      if (!val || !DECL_P (val)
	  || !DECL_LANG_SPECIFIC (val) || !DECL_ANTICIPATED (val))
  ...
  for (; !val; scope = CP_DECL_CONTEXT (scope))
      ...
      if (b)
	{
      ...
      for (level = current_binding_level;
	   level->kind != sk_namespace; level = level->level_chain)
				  ...
				  int flags)
{
  ...
  while (scope && result->value != error_mark_node)
      ...
      if (todo)
	{
      ...
      else if (todo_maybe && (!result->value && !result->type))
	{
  ...
  if (IDENTIFIER_TYPENAME_P (name))
    {
      ...
      for (level = current_binding_level;
	   level && level->kind != sk_namespace; level = level->level_chain)
	{
	  tree class_type;
	  tree operators;
...
lookup_type_current_level (tree name)
{
  ...
  timevar_push (TV_NAME_LOOKUP);
  my_friendly_assert (current_binding_level->kind != sk_namespace, 980716);
...
pushtag (tree name, tree type, int globalize)
{
  ...
  if (name)
      ...
      if (IDENTIFIER_TYPE_VALUE (name) != type)
	{
	  ...
	  d = maybe_process_template_type_declaration (type, globalize, b);
      ...
      if (b->kind == sk_class && !COMPLETE_TYPE_P (current_class_type))
	{
	  maybe_add_class_template_decl_list (current_class_type,
					      type, /*friend_p= */ 0);
