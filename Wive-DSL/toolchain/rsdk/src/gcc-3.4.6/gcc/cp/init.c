...
dfs_initialize_vtbl_ptrs (tree binfo, void *data)
{
  if ((!BINFO_PRIMARY_P (binfo) || TREE_VIA_VIRTUAL (binfo))
      && CLASSTYPE_VFIELDS (BINFO_TYPE (binfo)))
    {
      ...
      base_ptr =
	build_base_path (PLUS_EXPR, base_ptr, binfo, /*nonnull= */ 1);
  ...
  if (type == error_mark_node)
      ...
      for (field = TYPE_FIELDS (type); field; field = TREE_CHAIN (field))
	{
	  ...
	  if (static_storage_p && !zero_init_p (TREE_TYPE (field)))
	    inits = tree_cons (field, build_zero_init (TREE_TYPE (field),
						       /*nelts= */ NULL_TREE,
						       static_storage_p),
			       inits);
...
perform_member_init (tree member, tree init)
{
  ...
  if (ANON_AGGR_TYPE_P (type))
	  ...
	  && TREE_CODE (TREE_TYPE (TREE_VALUE (init))) == ARRAY_TYPE)
	{
	  ...
	  finish_expr_stmt (build_vec_init
			    (decl, NULL_TREE, TREE_VALUE (init),
			     /* from_array= */ 1));
	}
      else
  ...
  for (fields = TYPE_FIELDS (t); fields; fields = TREE_CHAIN (fields))
	...
	*uses_unions_p = 1;
      ...
      if (ANON_AGGR_TYPE_P (TREE_TYPE (fields)))
	{
	  ...
	  list = build_field_list (TREE_TYPE (fields), list, uses_unions_p);
	}
      /* Add this field.  */
  ...
  for (init = mem_inits; init; init = TREE_CHAIN (init))
    {
      ...
      for (subobject_init = next_subobject;
	   subobject_init; subobject_init = TREE_CHAIN (subobject_init))
      ...
      if (TREE_VALUE (subobject_init))
	{
	  if (TREE_CODE (subobject) == FIELD_DECL)
	    ...
	    error ("multiple initializations given for base `%T'", subobject);
	}
...
emit_mem_initializers (tree mem_inits)
{
  ...
  while (mem_inits && TREE_CODE (TREE_PURPOSE (mem_inits)) != FIELD_DECL)
    {
	...
	warning ("base class `%#T' should be explicitly initialized in the "
		 "copy constructor", BINFO_TYPE (subobject));
  ...
  while (mem_inits)
    {
      perform_member_init (TREE_PURPOSE (mem_inits), TREE_VALUE (mem_inits));
      mem_inits = TREE_CHAIN (mem_inits);
    }
}
  ...
  if (vtt_index)
    {
      ...
      vtbl2 = build (PLUS_EXPR, TREE_TYPE (vtt_parm), vtt_parm, vtt_index);
  ...
  if (TREE_CODE (field) == VAR_DECL)
    {
      error ("`%#D' is a static data member; it can only be "
	     "initialized at its definition", field);
      return 0;
    }
  if (TREE_CODE (field) != FIELD_DECL)
    {
      error ("`%#D' is not a non-static data member of `%T'", field, type);
      return 0;
    }
...
expand_member_init (tree name)
{
  ...
  if (!name)
	...
	default:
	  error
	    ("unnamed initializer for `%T', which uses multiple inheritance",
	     current_class_type);
	  return NULL_TREE;
	}
    }
...
static void
expand_default_init (tree binfo, tree true_exp, tree exp, tree init,
		     int flags)
{
  tree type = TREE_TYPE (exp);
  tree ctor_name;
  ...
  if (init && TREE_CODE (init) != TREE_LIST
	...
	/* Do nothing.  We hit this in two cases:  Reference initialization,
      ...
      else if (TREE_CODE (init) == CONSTRUCTOR && TREE_HAS_CONSTRUCTOR (init))
	{
	...
	init =
	  ocp_convert (type, init, CONV_IMPLICIT | CONV_FORCE_TEMP, flags);
...
expand_aggr_init_1 (tree binfo, tree true_exp, tree exp, tree init, int flags)
{
  ...
  my_friendly_assert (init != error_mark_node
		      && type != error_mark_node, 211);
...
build_offset_ref (tree type, tree name, bool address_p)
{
  ...
  if (!COMPLETE_TYPE_P (complete_type (type)) && !TYPE_BEING_DEFINED (type))
    {
      error ("incomplete type `%T' does not have member `%D'", type, name);
      return error_mark_node;
    }
  ...
  if (BASELINK_P (member))
      ...
      if (TREE_CODE (t) != TEMPLATE_ID_EXPR && !really_overloaded_fn (t))
	{
	  ...
	  if (address_p && DECL_P (t) && DECL_NONSTATIC_MEMBER_P (t))
...
decl_constant_value (tree decl)
{
  ...
  if (TREE_CODE (decl) == COND_EXPR)
	...
	return build (COND_EXPR,
		      TREE_TYPE (decl), TREE_OPERAND (decl, 0), d1, d2);
    }

  while (DECL_P (decl) && (	/* Enumeration constants are constant.  */
...
build_new (tree placement, tree decl, tree init, int use_global_new)
{
  ...
  if (processing_template_decl)
	...
	t = tree_cons (tree_cons (NULL_TREE, type, NULL_TREE),
		       build_min_nt (ARRAY_REF, NULL_TREE, nelts), NULL_TREE);
      else
	t = type;
...
build_vec_init (tree base, tree maxindex, tree init, int from_array)
{
  ...
  if (init != NULL_TREE && TREE_CODE (init) == CONSTRUCTOR)
      ...
      for (elts = CONSTRUCTOR_ELTS (init); elts; elts = TREE_CHAIN (elts))
	{
	  ...
	  if (IS_AGGR_TYPE (type) || TREE_CODE (type) == ARRAY_TYPE)
	    ...
	    finish_expr_stmt (build_modify_expr (baseref, NOP_EXPR, elt));
	  current_stmt_tree ()->stmts_are_full_exprs_p = 0;
  ...
  if (from_array
	  ...
	  && !(host_integerp (maxindex, 0)
	       && (num_initialized_elts == tree_low_cst (maxindex, 0) + 1))))
    {
      ...
      finish_for_cond (build (NE_EXPR, boolean_type_node,
			      iterator, integer_minus_one_node), for_stmt);
	      ...
	      int flags, int use_global_delete)
{
  ...
  if (TYPE_HAS_TRIVIAL_DESTRUCTOR (type))
	...
	(DELETE_EXPR, addr, cxx_sizeof_nowarn (type),
	 LOOKUP_NORMAL | (use_global_delete * LOOKUP_GLOBAL), NULL_TREE);
    }
  else
    {
	...
	expr = build (COND_EXPR, void_type_node, ifexp, expr, void_zero_node);
