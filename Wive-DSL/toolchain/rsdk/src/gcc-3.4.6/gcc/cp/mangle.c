  ...
  for (i = 0; i < VARRAY_ACTIVE_SIZE (G.substitutions); ++i)
	...
	name = IDENTIFIER_POINTER (DECL_NAME (TYPE_NAME (el)));
	  ...
	  (CP_TYPE_RESTRICT_P (el)
	   || CP_TYPE_VOLATILE_P (el) || CP_TYPE_CONST_P (el)))
	fprintf (stderr, "CV-");
			  ...
			  const substitution_identifier_index_t index)
{
  ...
  return
    TREE_VEC_LENGTH (args) == 1 && TREE_VEC_ELT (args, 0) == char_type_node;
}
      ...
      else
	{
      ...
      write_bare_function_type (fn_type,
				(!DECL_CONSTRUCTOR_P (decl)
				 ...
				 && !DECL_CONV_FN_P (decl)
				 && decl_is_template_id (decl, NULL)), d);
    }
}
...
write_unscoped_name (const tree decl)
{
  ...
  /* If not, it should be either in the global namespace, or directly
     in a local function scope.  */
	   ...
	   || context == NULL || TREE_CODE (context) == FUNCTION_DECL)
    write_unqualified_name (decl);
  else
    abort ();
}
...
write_prefix (const tree node)
{
  ...
  if (node == NULL || node == global_namespace)
    return;
  ...
  if (TREE_CODE (node) == TEMPLATE_TYPE_PARM && !abi_version_at_least (2))
    G.need_abi_warning = true;
  ...
  if (abi_version_at_least (2))
    {
      ...
      real_to_target (target_real, &TREE_REAL_CST (value), TYPE_MODE (type));
			  ...
			  const tree decl)
{
  ...
  write_method_parms (TYPE_ARG_TYPES (type),
		      TREE_CODE (type) == METHOD_TYPE, decl);
}
...
write_method_parms (tree parm_types, const int method_p, const tree decl)
{
  ...
  for (first_parm_type = parm_types;
       parm_types; parm_types = TREE_CHAIN (parm_types))
    {
...
write_expression (tree expr)
{
  ...
  /* Skip NOP_EXPRs.  They can occur when (say) a pointer argument
     ...
     type.  */
  while (TREE_CODE (expr) == NOP_EXPR || TREE_CODE (expr) == NON_LVALUE_EXPR)
    {
  ...
  else if (TREE_CODE (expr) == SIZEOF_EXPR && TYPE_P (TREE_OPERAND (expr, 0)))
    {
      write_string ("st");
...
write_template_template_param (const tree parm)
{
  ...
  if (TREE_CODE (parm) == BOUND_TEMPLATE_TEMPLATE_PARM)
    {
      template = TI_TEMPLATE (TEMPLATE_TEMPLATE_PARM_TEMPLATE_INFO (parm));
...
/* Hash a node (VAL1) in the table.  */
     ...
     static hashval_t hash_type (const void *val)
{
