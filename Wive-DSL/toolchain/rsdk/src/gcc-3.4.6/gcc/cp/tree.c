...
lvalue_p_1 (tree ref, int treat_class_rvalues_as_lvalues)
{
  ...
  switch (TREE_CODE (ref))
    ...
    case VAR_DECL:
	  ...
	  && DECL_LANG_SPECIFIC (ref) && DECL_IN_AGGR_P (ref))
	return clk_none;
    case INDIRECT_REF:
    case ARRAY_REF:
    case PARM_DECL:
...
lvalue_p (tree ref)
{
  return (lvalue_p_1 (ref, /*class rvalue ok */ 1) != clk_none);
}
			      ...
			      int type_quals, tsubst_flags_t complain)
{
  ...
  if ((type_quals & TYPE_QUAL_RESTRICT)
      ...
      && TREE_CODE (type) != TYPENAME_TYPE && !POINTER_TYPE_P (type))
    {
...
/* Compare ENTRY (an entry in the hash table) with DATA (a list_proxy
   for a node we are thinking about adding).  */
     ...
     static int list_hash_eq (const void *entry, const void *data)
{
  tree t = (tree) entry;
...
{
  tree t = (tree) p;
  return list_hash_pieces (TREE_PURPOSE (t), TREE_VALUE (t), TREE_CHAIN (t));
}
...
hash_tree_cons (tree purpose, tree value, tree chain)
{
  ...
  slot = htab_find_slot_with_hash (list_hash_table, &proxy, hashcode, INSERT);
...
is_overloaded_fn (tree x)
{
  ...
  return (TREE_CODE (x) == FUNCTION_DECL
	  ...
	  || DECL_FUNCTION_TEMPLATE_P (x) || TREE_CODE (x) == OVERLOAD);
}
...
cxx_printable_name (tree decl, int v)
{
  ...
  if (v < 2
      || TREE_CODE (decl) != FUNCTION_DECL || DECL_LANG_SPECIFIC (decl) == 0)
...
static tree
find_tree_r (tree * tp, int *walk_subtrees ATTRIBUTE_UNUSED, void *data)
{
  if (*tp == (tree) data)
    return (tree) data;
		   ...
		   void *data ATTRIBUTE_UNUSED)
{
  ...
  if (TYPE_P (t)
      ...
      && (decl_function_context (TYPE_MAIN_DECL (t)) || TYPE_ANONYMOUS_P (t)))
    return t;
  return NULL_TREE;
}
...
array_type_nelts_top (tree type)
{
  return fold (build (PLUS_EXPR, sizetype,
		      array_type_nelts (type), integer_one_node));
}
...
static tree
bot_replace (tree * t, int *walk_subtrees ATTRIBUTE_UNUSED, void *data)
{
  splay_tree target_remap = ((splay_tree) data);
...
cp_tree_equal (tree t1, tree t2)
{
  ...
  for (code1 = TREE_CODE (t1);
       ...
       || code1 == NON_LVALUE_EXPR; code1 = TREE_CODE (t1))
    t1 = TREE_OPERAND (t1, 0);
  for (code2 = TREE_CODE (t2);
       code2 == NOP_EXPR || code2 == CONVERT_EXPR
       || code1 == NON_LVALUE_EXPR; code2 = TREE_CODE (t2))
...
{
  if (DECL_P (*node) || !CLASS_TYPE_P (*node) || !TYPE_FOR_JAVA (*node))
    {
				...
				bool * no_add_attrs)
{
  ...
  *no_add_attrs = true;
  ...
  if (DECL_P (*node)
      || !CLASS_TYPE_P (*node) || *node != TYPE_MAIN_VARIANT (*node))
    {
  ...
  if (!warned++)
    warning
      ("`%s' is obsolete; g++ vtables are now COM-compatible by default",
       IDENTIFIER_POINTER (name));
				...
				bool * no_add_attrs)
{
  ...
  if (decl == NULL_TREE
    ...
    {
      error
	("can only use `%s' attribute on file-scope definitions of objects of class type",
	 IDENTIFIER_POINTER (name));
      *no_add_attrs = true;
      return NULL_TREE;
    }
...
cp_build_type_attribute_variant (tree type, tree attributes)
{
  ...
  if (TREE_CODE (new_type) == FUNCTION_TYPE
      && (TYPE_RAISES_EXCEPTIONS (new_type) != TYPE_RAISES_EXCEPTIONS (type)))
...
cp_copy_res_decl_for_inlining (tree result,
			       ...
			       tree caller,
			       void *decl_map_,
			       int *need_decl, tree return_slot_addr)
{
  ...
  if (DECL_SAVED_FUNCTION_DATA (fn))
      ...
      if (nrv)
	{
	  ...
	  splay_tree_insert (decl_map,
			     (splay_tree_key) nrv, (splay_tree_value) var);
	}
    }
  ...
  if (decl)
    {
      ...
      /* Remember the copy.  */
      splay_tree_insert (st, (splay_tree_key) decl, (splay_tree_value) copy);
    }
...
static tree
cp_unsave_r (tree * tp, int *walk_subtrees, void *data)
{
  splay_tree st = (splay_tree) data;
  splay_tree_node n;
...
stabilize_expr (tree exp, tree * initp)
{
  ...
  else if (!real_lvalue_p (exp) || !TYPE_NEEDS_CONSTRUCTING (TREE_TYPE (exp)))
    {
...
stabilize_call (tree call, tree * initp)
{
  ...
  if (TREE_CODE (call) != CALL_EXPR && TREE_CODE (call) != AGGR_INIT_EXPR)
    abort ();
...
stabilize_init (tree init, tree * initp)
{
  ...
  if (TREE_CODE (t) == INIT_EXPR
	...
	t = expr_last (t);
      if (TREE_CODE (t) == CONSTRUCTOR && CONSTRUCTOR_ELTS (t) == NULL_TREE)
	{
