...
static
GTY (())
     deferred_access *deferred_access_stack;
     static GTY (())
     deferred_access *deferred_access_free_list;
...
perform_deferred_access_checks (void)
{
  ...
  for (deferred_check = deferred_access_stack->deferred_access_checks;
       deferred_check; deferred_check = TREE_CHAIN (deferred_check))
    /* Check access.  */
...
{
  return (cfun
	  ? &cfun->language->base.x_stmt_tree : &scope_chain->x_stmt_tree);
}
...
begin_for_stmt (void)
{
  ...
  r = build_stmt (FOR_STMT, NULL_TREE, NULL_TREE, NULL_TREE, NULL_TREE);
		 ...
		 tree output_operands, tree input_operands, tree clobbers)
{
  ...
  if (!processing_template_decl)
	...
	{
	  tree converted_operand = decay_conversion (TREE_VALUE (t));
...
finish_non_static_data_member (tree decl, tree object, tree qualifying_scope)
{
  ...
  if (processing_template_decl && !qualifying_scope)
      ...
      if (qualifying_scope)
	{
	  ...
	  object = build_scoped_ref (object, qualifying_scope, &binfo);
	}
				     ...
				     tree nested_name_specifier)
{
  ...
  if (object_type
      /* OBJECT_TYPE might not be a class type; consider:
         ...
         OBJECT_TYPE.  */
      && CLASS_TYPE_P (object_type) && DERIVED_FROM_P (scope, object_type))
  ...
  else if (nested_name_specifier)
    {
	  ...
	  && current_class_ptr && DERIVED_FROM_P (scope, current_class_type))
  ...
  if (TREE_CODE (expr) == FIELD_DECL)
	...
	expr = (build_class_member_access_expr
		(maybe_dummy_object (qualifying_class, NULL),
		 expr, BASELINK_ACCESS_BINFO (expr),
		 /*preserve_reference= */ false));
      else if (done)
	/* The expression is a qualified name whose address is not
	   being taken.  */
	expr =
	  build_offset_ref (qualifying_class, expr, /*address_p= */ false);
    }
...
finish_stmt_expr (tree rtl_expr, bool has_no_scope)
{
  ...
  /* If we created a statement-tree for this statement-expression,
     remove it now.  */
  if (!cfun && TREE_CHAIN (scope_chain->x_saved_tree) == NULL_TREE)
...
finish_call_expr (tree fn, tree args, bool disallow_virtual, bool koenig_p)
{
  ...
  if (!BASELINK_P (fn) && is_overloaded_fn (fn))
      ...
      if (DECL_FUNCTION_MEMBER_P (f))
	{
	  ...
	  fn = build_baselink (TYPE_BINFO (type),
			       TYPE_BINFO (type), fn, /*optype= */ NULL_TREE);
	}
    }
  ...
  if (BASELINK_P (fn))
	...
	error ("arguments to destructor are not allowed");
      ...
      result = build1 (NOP_EXPR, void_type_node, TREE_OPERAND (fn, 0));
      TREE_SIDE_EFFECTS (result) = 1;
    }
...
finish_member_declaration (tree decl)
{
  ...
  TREE_PRIVATE (decl) = (current_access_specifier == access_private_node);
  TREE_PROTECTED (decl) = (current_access_specifier == access_protected_node);
...
check_multiple_declarators (void)
{
  ...
  if (PROCESSING_REAL_TEMPLATE_DECL_P ()
      || processing_explicit_instantiation || processing_specialization)
		      ...
		      const char **error_msg)
{
  ...
  *error_msg = NULL;
  ...
  if (TREE_CODE (decl) == TEMPLATE_DECL && !DECL_FUNCTION_TEMPLATE_P (decl))
    {
  ...
  /* Similarly, we resolve enumeration constants to their 
  ...
  else
    {
      ...
      *idk = (scope
		 ...
		 ? CP_ID_KIND_TEMPLATE_ID : CP_ID_KIND_UNQUALIFIED));
	...
	dependent_p = dependent_type_p (TREE_TYPE (decl));
      ...
      else if (is_overloaded_fn (decl))
	{
	  ...
	  if (TREE_CODE (first_fn) == FUNCTION_DECL
	      && DECL_FUNCTION_MEMBER_P (first_fn) && !shared_member_p (decl))
	    {
...
simplify_aggr_init_exprs_r (tree * tp,
			    int *walk_subtrees, void *data ATTRIBUTE_UNUSED)
{
...
expand_or_defer_fn (tree fn)
{
  ...
  walk_tree_without_duplicates (&DECL_SAVED_TREE (fn),
				simplify_aggr_init_exprs_r, NULL);
...
nullify_returns_r (tree * tp, int *walk_subtrees, void *data)
{
  ...
  else if (TREE_CODE (*tp) == CLEANUP_STMT && CLEANUP_DECL (*tp) == nrv)
    CLEANUP_EH_ONLY (*tp) = 1;
  /* Replace the DECL_STMT for the NRV with an initialization of the
     RESULT_DECL, if needed.  */
  else if (TREE_CODE (*tp) == DECL_STMT && DECL_STMT_DECL (*tp) == nrv)
    {
      tree init;
      if (DECL_INITIAL (nrv) && DECL_INITIAL (nrv) != error_mark_node)
	{
