  ...
  if (declp)
    ...
    /* It's a type.  */
    {
      ...
      for (context = supplicant;
	   context && TYPE_P (context); context = TYPE_CONTEXT (context))
	if (type == context)
	  return 1;
...
add_friend (tree type, tree decl, bool complain)
{
  ...
  while (list)
      ...
      if (name == FRIEND_NAME (list))
	{
	  ...
	  TREE_VALUE (list) = tree_cons (NULL_TREE, decl, TREE_VALUE (list));
	  return;
	}
...
make_friend_class (tree type, tree friend_type, bool complain)
{
  ...
  if (processing_template_decl > template_class_depth (type))
    ...
    {
      if (complain)
	pedwarn ("class `%T' is implicitly friends with itself", type);
      return;
    }
  else
  ...
  for (classes = CLASSTYPE_FRIEND_CLASSES (type);
       classes; classes = TREE_CHAIN (classes))
    {
      ...
      if (TREE_CODE (friend_type) == TEMPLATE_DECL)
	{
	  if (friend_type == probe)
	    {
	      if (complain)
		warning ("`%D' is already a friend of `%T'", probe, type);
	      break;
	    }
	}
      else if (TREE_CODE (probe) != TEMPLATE_DECL)
	{
	  if (same_type_p (probe, friend_type))
	    {
	      if (complain)
		warning ("`%T' is already a friend of `%T'", probe, type);
	      break;
	    }
	}
    }
  ...
  if (!classes)
    {
      maybe_add_class_template_decl_list (type, friend_type, /*friend_p= */
					  1);
	   ...
	   int funcdef_flag)
{
  ...
  if (ctype)
      ...
      if (friend_depth)
	{
	  ...
	  || COMPLETE_TYPE_P (ctype) || TYPE_BEING_DEFINED (ctype))
	{
	  ...
	  if (warn)
	    {
	      ...
	      warning
		("friend declaration `%#D' declares a non-template function",
		 decl);
	      if (!explained)
		{
		  warning
		    ("(if this is not what you intended, make sure the function template has already been declared and add <> after the function name here) -Wno-non-template-friend disables this warning");
		  explained = 1;
		}
	    }
	}
