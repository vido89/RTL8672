...
standard_conversion (tree to, tree from, tree expr, int flags)
{
  ...
  if ((tcode == POINTER_TYPE || TYPE_PTR_TO_MEMBER_P (to))
  ...
  else if (tcode == BOOLEAN_TYPE)
    {
	  ...
	  || fcode == ENUMERAL_TYPE
	  || fcode == POINTER_TYPE || TYPE_PTR_TO_MEMBER_P (from))
	{
	  conv = build_conv (STD_CONV, to, conv);
...
convert_class_to_reference (tree t, tree s, tree expr)
{
  ...
  cand->args = tree_cons (NULL_TREE,
			  build_this (expr), TREE_CHAIN (cand->args));
...
implicit_conversion (tree to, tree from, tree expr, int flags)
{
  ...
  if (expr != NULL_TREE
      && (IS_AGGR_TYPE (from)
	  || IS_AGGR_TYPE (to)) && (flags & LOOKUP_NO_CONVERSION) == 0)
    {
      struct z_candidate *cand;
  ...
  if (args[2])
    {
      ...
      t =
	implicit_conversion (boolean_type_node, argtypes[2], args[2], flags);
      if (t)
...
/* 12For every quintuple C1, C2, T, CV1, CV2), where C2 is a class type, C1
    ...
    case MEMBER_REF:
      if (TREE_CODE (type1) == POINTER_TYPE && TYPE_PTR_TO_MEMBER_P (type2))
	{
...
/* 19For  every  triple  L, VQ, R), where L is an arithmetic or enumeration
  ...
  if (type2 && !same_type_p (type1, type2)
	  ...
	  || (TYPE_PTRMEM_P (type1) && TYPE_PTRMEM_P (type2))
	  || TYPE_PTRMEMFUNC_P (type1)
	  || IS_AGGR_TYPE (type1) || TREE_CODE (type1) == ENUMERAL_TYPE))
    {
      build_builtin_candidate
			     ...
			     unification_kind_t strict)
{
  ...
  i = fn_type_unification (tmpl, explicit_targs, targs,
			   args_without_in_chrg, return_type, strict, -1);
  ...
  if (DECL_CONSTRUCTOR_P (fn) && list_length (arglist) == 2)
    {
      ...
      if (arg_types
	  && same_type_p (TYPE_MAIN_VARIANT (TREE_VALUE (arg_types)), ctype))
	return NULL;
    }
...
static struct z_candidate *
add_template_candidate (struct z_candidate **candidates, tree tmpl,
			tree ctype, tree explicit_targs, tree arglist,
			tree return_type, tree access_path,
...
merge_conversion_sequences (tree user_seq, tree std_seq)
{
  ...
  my_friendly_assert (TREE_CODE (user_seq) == USER_CONV, 20030306);
...
build_user_type_conversion_1 (tree totype, tree expr, int flags)
{
  ...
  cand->second_conv = merge_conversion_sequences (convs, cand->second_conv);
			     ...
			     bool * any_viable_p)
{
  ...
  *any_viable_p = true;
  ...
  my_friendly_assert (TREE_CODE (fn) == FUNCTION_DECL
		      ...
		      || TREE_CODE (fn) == TEMPLATE_ID_EXPR, 20020712);
  my_friendly_assert (!args || TREE_CODE (args) == TREE_LIST, 20020712);
...
tree
build_operator_new_call (tree fnname, tree args, tree * size,
			 tree * cookie_size)
{
  tree fns;
  struct z_candidate *candidates;
	...
	for (; fns; fns = OVL_NEXT (fns))
	  {
	    tree fn = OVL_CURRENT (fns);
	    if (TREE_CODE (fn) == TEMPLATE_DECL)
	      add_template_conv_candidate (&candidates, fn, obj, args, totype,
					   /*access_path= */ NULL_TREE,
					   /*conversion_path= */ NULL_TREE);
	    else
...
conditional_conversion (tree e1, tree e2)
{
  ...
  if (CLASS_TYPE_P (t1) && CLASS_TYPE_P (t2)
      ...
      if (good_base && at_least_as_qualified_p (t2, t1))
	{
	  ...
	  if (!same_type_p (TYPE_MAIN_VARIANT (t1), TYPE_MAIN_VARIANT (t2)))
...
build_conditional_expr (tree arg1, tree arg2, tree arg3)
{
  ...
  if (error_operand_p (arg1)
      || error_operand_p (arg2) || error_operand_p (arg3))
    return error_mark_node;
  ...
  if (VOID_TYPE_P (arg2_type) || VOID_TYPE_P (arg3_type))
	...
	arg3 = decay_conversion (arg3);
      ...
      /* [expr.cond]
         ...
         used.   */
      if (TREE_CODE (arg2) == THROW_EXPR && TREE_CODE (arg3) != THROW_EXPR)
	{
	  if (!VOID_TYPE_P (arg3_type))
  ...
  if (real_lvalue_p (arg2)
      && real_lvalue_p (arg3) && same_type_p (arg2_type, arg3_type))
    {
      result_type = arg2_type;
      goto valid_operands;
    }
  ...
  if (!same_type_p (arg2_type, arg3_type)
      && (CLASS_TYPE_P (arg2_type) || CLASS_TYPE_P (arg3_type)))
    {
      ...
      add_builtin_candidates (&candidates,
			      COND_EXPR,
			      NOP_EXPR,
			      ansi_opname (COND_EXPR), args, LOOKUP_NORMAL);
...
add_candidates (tree fns, tree args,
		...
		int flags, struct z_candidate **candidates)
{
  tree ctype;
  ...
  while (fns)
	...
	add_template_candidate (candidates,
				...
				NULL_TREE,
				access_path,
				conversion_path, flags, DEDUCE_CALL);
      else if (!template_only)
	add_function_candidate (candidates,
				fn,
				ctype,
				fn_args, access_path, conversion_path, flags);
      fns = OVL_NEXT (fns);
    }
}
	      ...
	      bool * overloaded_p)
{
  ...
  if (CLASS_TYPE_P (TREE_TYPE (arg1)))
	...
	add_candidates (BASELINK_FUNCTIONS (fns), arglist,
			NULL_TREE, false,
			BASELINK_BINFO (fns),
			TYPE_BINFO (TREE_TYPE (arg1)), flags, &candidates);
    }
	  ...
	  if (flags & LOOKUP_COMPLAIN)
	    pedwarn
	      ("no `%D(int)' declared for postfix `%s', trying prefix operator instead",
	       fnname, operator_name_info[code].name);
		      ...
		      int flags, tree placement)
{
  ...
  if (fn)
	...
	args = tree_cons (NULL_TREE, addr, build_tree_list (NULL_TREE, size));
		   ...
		   bool issue_conversion_warnings)
{
  ...
  if (ICS_BAD_FLAG (convs)
      ...
      && TREE_CODE (convs) != AMBIG_CONV && TREE_CODE (convs) != REF_BIND)
    {
      tree t = convs;
      for (; t; t = TREE_OPERAND (t, 0))
	{
      ...
      pedwarn ("invalid conversion from `%T' to `%T'", TREE_TYPE (expr),
	       totype);
      if (fn)
  ...
  switch (TREE_CODE (convs))
	...
	if (IS_AGGR_TYPE (totype) && (inner >= 0 || !lvalue_p (expr)))
	  {
	    expr = (build_temp (expr, totype,
				/* Core issue 84, now a DR, says that we don't
				   ...
				   auto_ptr<Derived>).  */
				LOOKUP_NORMAL | LOOKUP_ONLYCONVERTING |
				LOOKUP_NO_CONVERSION, &diagnostic_fn));
	...
	expr = instantiate_type (totype, expr, tf_error | tf_warning);
      ...
      if (inner >= 0 && TREE_CODE (TREE_TYPE (expr)) != ARRAY_TYPE)
  ...
  switch (TREE_CODE (convs))
	...
	if (NEED_TEMPORARY_P (convs) || !lvalue_p (expr))
	  {
		...
		if (lvalue & clk_bitfield)
		  error ("cannot bind bitfield `%E' to `%T'", expr, ref_type);
		...
		&& CLASS_TYPE_P (type) && !TYPE_HAS_TRIVIAL_INIT_REF (type))
	      {
		error ("cannot bind packed field `%E' to `%T'",
		       expr, ref_type);
		return error_mark_node;
	...
	expr = cp_convert (build_pointer_type (TREE_TYPE (ref_type)), expr);
...
convert_arg_to_ellipsis (tree arg)
{
  ...
  if (arg != error_mark_node && !pod_type_p (TREE_TYPE (arg)))
	...
	warning ("cannot pass objects of non-POD type `%#T' through `...'; "
      ...
      arg = build (COMPOUND_EXPR, integer_type_node, arg, integer_zero_node);
    }
  ...
  if (TREE_CODE (arg) == DEFAULT_ARG)
    {
      error ("the default argument for parameter %d of `%D' has "
	     "not yet been parsed", parmnum, fn);
      return error_mark_node;
    }
...
build_over_call (struct z_candidate *cand, int flags)
{
  ...
  if (DECL_CONSTRUCTOR_P (fn))
    {
      converted_args =
	tree_cons (NULL_TREE, TREE_VALUE (arg), converted_args);
      arg = TREE_CHAIN (arg);
  ...
  /* Bypass access control for 'this' parameter.  */
	...
	pedwarn
	  ("passing `%T' as `this' argument of `%#D' discards qualifiers",
      ...
      converted_arg = build_base_path (PLUS_EXPR,
				       TREE_VALUE (arg),
				       cand->conversion_path, 1);
      /* Check that the base class is accessible.  */
      if (!accessible_base_p (TREE_TYPE (argtype),
			      BINFO_TYPE (cand->conversion_path)))
	error ("`%T' is not an accessible base of `%T'",
	       BINFO_TYPE (cand->conversion_path), TREE_TYPE (argtype));
  ...
  for (; arg && parm; parm = TREE_CHAIN (parm), arg = TREE_CHAIN (arg), ++i)
    {
  ...
  else if (TREE_VEC_LENGTH (convs) == 1 && DECL_COPY_CONSTRUCTOR_P (fn))
    {
      tree targ;
...
build_cxx_call (tree fn, tree args, tree converted_args)
{
  ...
  if ((!fndecl || !TREE_NOTHROW (fndecl)) && at_function_scope_p () && cfun)
			   ...
			   tree binfo, int flags)
{
  ...
  my_friendly_assert (name == complete_ctor_identifier
		      ...
		      || name == ansi_assopname (NOP_EXPR), 20020712);
  ...
  if (name == complete_ctor_identifier && !instance)
  ...
  else
    {
	  ...
	  || name == base_dtor_identifier || name == deleting_dtor_identifier)
  ...
  return build_new_method_call (instance, fns, args,
				TYPE_BINFO (BINFO_TYPE (binfo)), flags);
}
...
name_as_c_string (tree name, tree type, bool * free_p)
{
  ...
  *free_p = false;
  ...
  if (IDENTIFIER_CTOR_OR_DTOR_P (name))
    {
      pretty_name = (char *) IDENTIFIER_POINTER (constructor_name (type));
		       ...
		       tree conversion_path, int flags)
{
  ...
  if (error_operand_p (instance)
      || error_operand_p (fns) || args == error_mark_node)
    return error_mark_node;
  ...
  my_friendly_assert (TREE_CODE (fns) == FUNCTION_DECL
		      ...
		      || TREE_CODE (fns) == OVERLOAD, 20020712);
  ...
  for (fn = fns; fn; fn = OVL_NEXT (fn))
    {
      ...
      if ((flags & LOOKUP_ONLYCONVERTING) && DECL_NONCONVERTING_P (t))
	continue;
	...
	add_function_candidate (&candidates, t,
				class_type,
				this_arglist,
				access_binfo, conversion_path, flags);
    }
  ...
  if (DECL_PURE_VIRTUAL_P (cand->fn)
    ...
    error ((DECL_CONSTRUCTOR_P (current_function_decl) ?
	    ...
	    : "abstract virtual `%#D' called from destructor"), cand->fn);
  ...
  if (processing_template_decl && call != error_mark_node)
    return build_min_non_dep
      (CALL_EXPR, call,
       build_min_nt (COMPONENT_REF, orig_instance, orig_fns), orig_args);
  return call;
}
...
is_subseq (tree ics1, tree ics2)
{
  ...
  while (TREE_CODE (ics1) == RVALUE_CONV || TREE_CODE (ics1) == LVALUE_CONV)
...
compare_ics (tree ics1, tree ics2)
{
  ...
  if (TYPE_PTR_P (from_type1)
      ...
      && TYPE_PTR_P (to_type1) && TYPE_PTR_P (to_type2))
    {
  ...
  for (;; t = TREE_OPERAND (t, 0))
    {
	  ...
	  || TREE_CODE (t) == AMBIG_CONV || TREE_CODE (t) == IDENTITY_CONV)
	return TREE_TYPE (t);
    }
  abort ();
}
...
add_warning (struct z_candidate *winner, struct z_candidate *loser)
{
  winner->warnings = tree_cons (NULL_TREE,
				build_zc_wrapper (loser), winner->warnings);
}
...
joust (struct z_candidate *cand1, struct z_candidate *cand2, bool warn)
{
  ...
  if (cand1->fn == cand2->fn && (TYPE_P (cand1->fn) || DECL_P (cand1->fn)))
    return 1;
...
perform_implicit_conversion (tree type, tree expr)
{
  ...
  if (error_operand_p (expr))
    return error_mark_node;
  conv = implicit_conversion (type, TREE_TYPE (expr), expr, LOOKUP_NORMAL);
  if (!conv)
    {
...
initialize_reference (tree type, tree expr, tree decl, tree * cleanup)
{
  ...
  if (!conv || ICS_BAD_FLAG (conv))
	...
	error ("invalid initialization of reference of type "
	       "'%T' from expression of type '%T'", type, TREE_TYPE (expr));
      return error_mark_node;
    }
