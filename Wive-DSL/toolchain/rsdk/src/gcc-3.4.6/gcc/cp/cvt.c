...
cp_convert_to_pointer (tree type, tree expr, bool force)
{
  ...
  if (POINTER_TYPE_P (intype))
      ...
      if (bk == bk_via_virtual)
	{
	  if (force)
	    warning
	      ("pointer to member cast from `%T' to `%T' is via virtual base",
	       intype, type);
	  else
	    {
	      error
	...
	expr = size_binop (code,
			   build_nop (sizetype, expr), BINFO_OFFSET (binfo));
      return build_nop (type, expr);
    }
...
build_up_reference (tree type, tree arg, int flags, tree decl)
{
  ...
  if ((flags & LOOKUP_PROTECT)
      ...
      && IS_AGGR_TYPE (argtype) && IS_AGGR_TYPE (target_type))
    {
    ...
    rval = convert_to_pointer_force (build_pointer_type (target_type), rval);
		      ...
		      int flags, tree decl)
{
  ...
  if (!can_convert_intype_to_type
      ...
      && !(flags & LOOKUP_NO_CONVERSION))
    {
      ...
      rval_as_conversion = build_type_conversion (reftype, expr);
  ...
  if (((convtype & CONV_STATIC) && can_convert (intype, type))
      ...
      if (flags & LOOKUP_COMPLAIN)
	{
	  ...
	  if (!(convtype & CONV_CONST) && !at_least_as_qualified_p (ttl, ttr))
...
convert_to_void (tree expr, const char *implicit)
{
  ...
  switch (TREE_CODE (expr))
	...
	if (is_volatile && !is_complete)
	  warning
	    ("object of incomplete type `%T' will not be accessed in %s",
	...
	if (TYPE_VOLATILE (type) && !is_complete)
	  warning
	    ("object `%E' of incomplete type `%T' will not be accessed in %s",
...
convert_force (tree type, tree expr, int convtype)
{
  ...
  if (((TREE_CODE (TREE_TYPE (e)) == POINTER_TYPE
	...
	&& TREE_CODE (TREE_TYPE (TREE_TYPE (e))) == METHOD_TYPE)
       || integer_zerop (e) || TYPE_PTRMEMFUNC_P (TREE_TYPE (e)))
...
build_expr_type_conversion (int desires, tree expr, bool complain)
{
  ...
  if (expr == null_node && (desires & WANT_INT) && !(desires & WANT_NULL))
  ...
  if (!IS_AGGR_TYPE (basetype))
	...
	return (desires & WANT_POINTER) ? expr : NULL_TREE;
      ...
      case FUNCTION_TYPE:
      case ARRAY_TYPE:
	return (desires & WANT_POINTER) ? decay_conversion (expr) : NULL_TREE;
      default:
	return NULL_TREE;
      }
...
type_promotes_to (tree type)
{
  ...
  /* Normally convert enums to int, but convert wide enums to something
     wider.  */
  else if (TREE_CODE (type) == ENUMERAL_TYPE || type == wchar_type_node)
    {
