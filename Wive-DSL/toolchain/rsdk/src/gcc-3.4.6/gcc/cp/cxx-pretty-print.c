...
static inline void
pp_cxx_template_keyword_if_needed (cxx_pretty_printer * pp, tree scope,
				   tree t)
{
	...
	if (TREE_CODE (fun) != FUNCTION_DECL)
	...
	else if (DECL_NONSTATIC_MEMBER_FUNCTION_P (fun))
	  {
	    tree object = code == AGGR_INIT_EXPR && AGGR_INIT_VIA_CTOR_P (t)
	      ? TREE_OPERAND (t, 2) : TREE_VALUE (args);
...
pp_cxx_pretty_printer_init (cxx_pretty_printer * pp)
{
  ...
  pp->c_base.multiplicative_expression =
    (pp_fun) pp_cxx_multiplicative_expression;
