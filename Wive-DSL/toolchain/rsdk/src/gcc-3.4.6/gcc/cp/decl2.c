...
delete_sanity (tree exp, tree size, bool doing_vec, int use_global_delete)
{
  ...
  if (doing_vec)
    return build_vec_delete (t, /*maxindex= */ NULL_TREE,
			     sfk_deleting_destructor, use_global_delete);
  else
...
check_member_template (tree tmpl)
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL
      || (TREE_CODE (decl) == TYPE_DECL && IS_AGGR_TYPE (TREE_TYPE (decl))))
    {
...
check_classfn (tree ctype, tree function, bool template_header_p)
{
  ...
  is_template = (TREE_CODE (function) == TEMPLATE_DECL || template_header_p);
	   ...
	   tree attrlist)
{
  ...
  if (DECL_IN_AGGR_P (value))
    {
      error ("`%D' is already defined in `%T'", value, DECL_CONTEXT (value));
  ...
  if (TREE_CODE (value) == VAR_DECL)
    {
      finish_static_data_member_decl (value, init, asmspec_tree, flags);
      return value;
    }
...
build_anon_union_vars (tree type, tree object)
{
  ...
  for (field = TYPE_FIELDS (type);
	...
	{
	  cp_pedwarn_at ("\
`%#D' invalid; an anonymous union can only have non-static data members", field);
	  continue;
	}
	...
	{
	  decl =
	    build_decl (ALIAS_DECL, DECL_NAME (field), TREE_TYPE (field));
...
coerce_new_type (tree type)
{
  ...
  if (!args || args == void_list_node
	...
	args = TREE_CHAIN (args);
      pedwarn ("`operator new' takes type `size_t' (`%T') as first parameter",
	       size_type_node);
    }
  switch (e)
    {
...
coerce_delete_type (tree type)
{
  ...
  if (!args || args == void_list_node
      || !same_type_p (TREE_VALUE (args), ptr_type_node))
    {
      ...
      if (args && args != void_list_node)
	args = TREE_CHAIN (args);
      error ("`operator delete' takes type `%T' as first parameter",
	     ptr_type_node);
    }
  switch (e)
    {
  ...
  for (; entries; entries = TREE_CHAIN (entries))
    {
      ...
      if (TREE_CODE (fnaddr) != ADDR_EXPR && TREE_CODE (fnaddr) != FDESC_EXPR)
...
import_export_class (tree ctype)
{
  ...
  if (import_export == 0
      && CLASSTYPE_IMPLICIT_INSTANTIATION (ctype) && !flag_implicit_templates)
    import_export = -1;

  /* Base our import/export status on that of the first non-inline,
     non-pure virtual function, if any.  */
  if (import_export == 0 && TYPE_POLYMORPHIC_P (ctype))
    {
  ...
  if (!vtbl)
    {
      /* If the references to this class' vtables are optimized away,
         ...
         dfs_debug_mark.  */
      if (DECL_COMDAT (primary_vtbl) && CLASSTYPE_DEBUG_REQUESTED (ctype))
      ...
      if (!DECL_DECLARED_INLINE_P (decl))
	{
	  ...
	  if (CLASSTYPE_INTERFACE_KNOWN (ctype))
		...
		= !(CLASSTYPE_INTERFACE_ONLY (ctype)
		    || (DECL_DECLARED_INLINE_P (decl)
			&& !flag_implement_inlines && !DECL_VINDEX (decl)));
...
get_guard_bits (tree guard)
{
  ...
  guard = build1 (ADDR_EXPR, build_pointer_type (TREE_TYPE (guard)), guard);
  guard = build1 (NOP_EXPR, build_pointer_type (char_type_node), guard);
...
start_objects (int method_type, int initp)
{
  ...
  start_function (void_list_node,
		  make_call_declarator (fnname, void_list_node, NULL_TREE,
					NULL_TREE), NULL_TREE, SF_DEFAULT);
...
/* Begins the generation of the function that will handle all
   ...
   translation unit.  */
     ...
     static tree start_static_storage_duration_function (unsigned count)
{
  tree parm_types;
  tree type;
  tree body;
  ...
  ssdf_decl = build_lang_decl (FUNCTION_DECL, get_identifier (id), type);
...
get_priority_info (int priority)
{
  ...
  n = splay_tree_lookup (priority_info_map, (splay_tree_key) priority);
  if (!n)
    {
      ...
      splay_tree_insert (priority_info_map,
			 (splay_tree_key) priority, (splay_tree_value) pi);
    }
  else
...
start_static_initialization_or_destruction (tree decl, int initp)
{
  ...
  cond = cp_build_binary_op (EQ_EXPR,
			     priority_decl, build_int_2 (priority, 0));
  ...
  if (TREE_PUBLIC (decl) && (DECL_COMMON (decl)
	...
	guard_cond
	  = cp_build_binary_op (EQ_EXPR,
				build_unary_op (PREINCREMENT_EXPR, guard,
						/*noconvert= */ 1),
				integer_one_node);
      else
	guard_cond
	  = cp_build_binary_op (EQ_EXPR,
				build_unary_op (PREDECREMENT_EXPR, guard,
						/*noconvert= */ 1),
				integer_zero_node);
  ...
  do
      ...
      for (i = 0; i < deferred_fns_used; ++i)
	{
	  ...
	  /* We lie to the back-end, pretending that some functions
	     ...
	     function twice.  */
	  if (DECL_NOT_REALLY_EXTERN (decl)
	      && DECL_INITIAL (decl) && DECL_NEEDED_P (decl))
	    DECL_EXTERNAL (decl) = 0;
  ...
  if (processing_template_decl)
    {
      my_friendly_assert (TREE_CODE (fn) == DOTSTAR_EXPR
			  || TREE_CODE (fn) == MEMBER_REF, 20030708);
...
mark_used (tree decl)
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL
      ...
      && !DECL_THUNK_P (decl) && !DECL_INITIAL (decl)
