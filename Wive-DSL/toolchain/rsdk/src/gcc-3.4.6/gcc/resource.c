...
find_basic_block (rtx insn, int search_limit)
{
  ...
  for (insn = next_nonnote_insn (insn);
       insn && GET_CODE (insn) == CODE_LABEL; insn = next_nonnote_insn (insn))
    {
      FOR_EACH_BB (bb) if (insn == BB_HEAD (bb))
	return bb->index;
    }
			   ...
			   int include_delayed_effects)
{
  ...
  switch (code)
    ...
    case JUMP_INSN:
      ...
      if (!include_delayed_effects && INSN_REFERENCES_ARE_DELAYED (x))
	return;
#endif
...
restart:
  ...
  switch (code)
    ...
    case INSN:
      ...
      if (mark_type != MARK_SRC_DEST_CALL && INSN_SETS_ARE_DELAYED (x))
	return;
#endif
    ...
    case SET:
      ...
      mark_set_resources (SET_DEST (x), res,
			  (mark_type == MARK_SRC_DEST_CALL
			   || GET_CODE (SET_SRC (x)) != CALL), mark_type);
  ...
  if (b != -1)
	   ...
	   insn = next_insn_no_annul (insn))
	{
	  ...
	  if ((GET_CODE (real_insn) == INSN
		...
		if (REG_NOTE_KIND (link) == REG_DEAD
		    ...
		    && REGNO (XEXP (link, 0)) < FIRST_PSEUDO_REGISTER)
		  {
		    ...
		    unsigned int last_regno
		      = (first_regno + HARD_REGNO_NREGS (first_regno,
							 GET_MODE (XEXP
								   (link,
								    0))));
	  ...
	  /* The beginning of the epilogue corresponds to the end of the
	    ...
	    IOR_HARD_REG_SET (current_live_regs,
			      start_of_epilogue_needs.regs);
	}
