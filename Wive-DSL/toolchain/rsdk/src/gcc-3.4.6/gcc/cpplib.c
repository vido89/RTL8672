...
/* Output diagnostics for a directive DIR.  INDENTED is nonzero if
   the '#' was indented.  */
static void
directive_diagnostics (cpp_reader * pfile, const directive * dir,
		       int indented)
{
  /* Issue -pedantic warnings for extensions.  */
  if (CPP_PEDANTIC (pfile)
      && !pfile->state.skipping && dir->origin == EXTENSION)
...
glue_header_name (cpp_reader * pfile)
{
  ...
  for (;;)
	...
	buffer[total_len++] = ' ';
      ...
      total_len =
	(cpp_spell_token (pfile, token, (uchar *) & buffer[total_len]) -
	 (uchar *) buffer);
    }
...
static int
strtoul_for_line (const uchar * str, unsigned int len,
...
_cpp_init_internal_pragmas (cpp_reader * pfile)
{
  ...
  cpp_register_pragma (pfile, "GCC", "system_header",
