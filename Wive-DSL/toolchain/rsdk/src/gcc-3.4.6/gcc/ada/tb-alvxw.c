...
create_new_frame (CORE_ADDR addr, CORE_ADDR pc)
{
  ...
  fi = (struct frame_info *) trace_alloc (sizeof (struct frame_info));
...
get_prev_frame (struct frame_info *next_frame)
{
  ...
  if (next_frame != NULL)
    {
      if (prev->frame == next_frame->frame && prev->pc == next_frame->pc)
	{
		  ...
		  void *exclude_min, void *exclude_max, int skip_frames)
{
  ...
  asm volatile (SAVE (9, 72)
		...
		SAVE (27, 216)
		SAVE (28, 224)
		SAVE (29, 232) SAVE (30, 240)::"r" (&theRegisters));
