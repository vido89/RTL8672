...
gnat_to_gnu_entity (Entity_Id gnat_entity, tree gnu_expr, int definition)
{
  ...
  unsigned int esize
	...
	&& UI_Is_In_Int_Range (Esize (gnat_entity)))
       ? MIN (UI_To_Int (Esize (gnat_entity)),
	      ...
	      : LONG_LONG_TYPE_SIZE) : LONG_LONG_TYPE_SIZE);
  tree gnu_size = 0;
  int imported_p
  ...
  if ((IN (kind, Numeric_Kind) || IN (kind, Enumeration_Kind)
      ...
      && Unknown_Esize (gnat_entity) && !Has_Size_Clause (gnat_entity))
    gigi_abort (109);
  ...
  switch (kind)
	...
	goto object;
      ...
      gnu_decl = create_var_decl (gnu_entity_id,
				  (Present (Interface_Name (gnat_entity))
				   ...
				   : NULL_TREE),
				  char_type_node, NULL_TREE, 0, 0, 1, 1, 0);
	...
	if (Is_Constr_Subt_For_UN_Aliased (Etype (gnat_entity))
	    && Is_Array_Type (Etype (gnat_entity)) && !type_annotate_only)
	  {
	    tree gnu_fat
	      =
	...
	if (definition
	    ...
	    && !Is_Imported (gnat_entity) && gnu_expr == 0)
	...
	DECL_POINTS_TO_READONLY_P (gnu_decl) = used_by_ref
	  && inner_const_flag;
      ...
      if (Is_Packed_Array_Type (gnat_entity) && BYTES_BIG_ENDIAN)
	{
	  ...
	  gnu_field = create_field_decl (get_identifier ("OBJECT"),
					 gnu_field_type, gnu_type, 1, 0, 0,
					 1), finish_record_type (gnu_type,
								 gnu_field, 0,
								 0);
	...
	if (TREE_CODE (gnu_max_size_unit) == INTEGER_CST
	  ...
	  TYPE_SIZE_UNIT (tem)
	    = size_binop (MIN_EXPR, gnu_max_size_unit, TYPE_SIZE_UNIT (tem));
      ...
      else
	{
	  ...
	  tree *gnu_index_type =
	    (tree *) alloca (array_dim * sizeof (tree *));
	  tree gnu_comp_size = 0;
	  tree gnu_max_size = size_one_node;
	  tree gnu_max_size_unit;
	  ...
	  for (index = first_dim, gnat_ind_subtype =
	       First_Index (gnat_entity), gnat_ind_base_subtype =
	       First_Index (Implementation_Base_Type (gnat_entity));
	       index < array_dim && index >= 0;
	       index += next_dim, gnat_ind_subtype =
	       Next_Index (gnat_ind_subtype), gnat_ind_base_subtype =
	  ...
	  if (CONTAINS_PLACEHOLDER_P (TYPE_SIZE (gnu_type))
	      ...
	      && !(TREE_CODE (gnu_max_size_unit) == INTEGER_CST
		   && TREE_OVERFLOW (gnu_max_size_unit)) && !max_overflow)
	    {
	...
	tree gnu_lower_bound = convert (gnu_string_index_type,
					gnat_to_gnu (String_Literal_Low_Bound
						     (gnat_entity)));
	...
	tree gnu_range_type = build_range_type (gnu_string_index_type,
						gnu_lower_bound,
						gnu_upper_bound);
	tree gnu_index_type = create_index_type (convert (sizetype,
							  TYPE_MIN_VALUE
							  (gnu_range_type)),
						 convert (sizetype,
							  TYPE_MAX_VALUE
	  ...
	  if (IN (Ekind (gnat_base_type), Record_Kind)
		...
		for (gnu_temp = gnu_subst_list;
		  ...
		  SET_TYPE_ADA_SIZE (gnu_type,
				     substitute_in_expr (TYPE_ADA_SIZE
							 (gnu_type),
							 TREE_PURPOSE
							 (gnu_temp),
							 TREE_VALUE
	...
	Entity_Id gnat_desig_full = ((IN (Ekind (Etype (gnat_desig_type)),
  ...
  if ((gnu_decl == 0 || this_made_decl) && IN (kind, Type_Kind))
	...
	align = MIN (BIGGEST_ALIGNMENT,
	       ...
	       && host_integerp (gnu_size, 1) && integer_pow2p (gnu_size))
	align = MIN (BIGGEST_ALIGNMENT, tree_low_cst (gnu_size, 1));
      ...
      if (TREE_CODE (gnu_type) == RECORD_TYPE && TYPE_IS_PADDING_P (gnu_type))
	{
  ...
  if (Is_Packed_Array_Type (gnat_entity)
    ...
    gnat_to_gnu_entity (Associated_Node_For_Itype (gnat_entity), NULL_TREE,
			0);
...
substitution_list (Entity_Id gnat_subtype,
		   Entity_Id gnat_type, tree gnu_list, int definition)
{
...
static
GTY ((length ("max_gnat_nodes")))
     tree *dummy_node_table;

/* Initialize the above table.  */

     void init_dummy_type (void)
{
  Node_Id gnat_node;
...
elaborate_expression (Node_Id gnat_expr,
		      ...
		      tree gnu_name,
		      int definition, int need_value, int need_debug)
{
  tree gnu_expr;
...
elaborate_expression_1 (Node_Id gnat_expr,
			Entity_Id gnat_entity,
			tree gnu_expr,
			tree gnu_name, int definition, int need_debug)
{
  tree gnu_decl = 0;
  ...
  TYPE_SIZE (record_type)
    = size_binop (PLUS_EXPR,
		  size_binop (MULT_EXPR, convert (bitsizetype, size),
			      bitsize_unit_node), bitsize_int (align));
		...
		int is_user_type, int definition, int same_rm_size)
{
  ...
  if (size != 0 && Present (gnat_entity)
	...
	post_error_ne_tree ("{^ }bits of & unused?", gnat_error_node,
			    gnat_entity, size_diffop (size, orig_size));
...
gnat_to_gnu_field (Entity_Id gnat_field,
		   tree gnu_record_type, int packed, int definition)
{
  ...
  gnu_field =
    create_field_decl (gnu_field_id, gnu_field_type, gnu_record_type, packed,
		       gnu_size, gnu_pos, Is_Aliased (gnat_field));
...
components_to_record (tree gnu_record_type,
		      ...
		      int definition,
		      tree * p_gnu_rep_list,
		      int cancel_alignment, int all_rep)
{
  ...
  if (TREE_CODE (gnu_record_type) == UNION_TYPE && Present (variant_part))
    for (variant = First_Non_Pragma (Variants (variant_part));
	 Present (variant); variant = Next_Non_Pragma (variant))
      {
	component_decl
  ...
  /* We create a QUAL_UNION_TYPE for the variant part since the variants are
	   ...
	   Present (variant); variant = Next_Non_Pragma (variant))
	{
	  ...
	  components_to_record (gnu_variant_type, Component_List (variant),
				...
				&gnu_our_rep_list, !all_rep_and_size,
				all_rep);
...
annotate_value (tree gnu_size)
{
  ...
  switch (TREE_CODE (gnu_size))
    ...
    case MULT_EXPR:
      ...
      break;
    case TRUNC_DIV_EXPR:
      tcode = Trunc_Div_Expr;
      break;
    case CEIL_DIV_EXPR:
      tcode = Ceil_Div_Expr;
      break;
    case FLOOR_DIV_EXPR:
      tcode = Floor_Div_Expr;
      break;
    case TRUNC_MOD_EXPR:
      tcode = Trunc_Mod_Expr;
      break;
    case CEIL_MOD_EXPR:
      tcode = Ceil_Mod_Expr;
      break;
    case FLOOR_MOD_EXPR:
      tcode = Floor_Mod_Expr;
      break;
    case EXACT_DIV_EXPR:
      tcode = Exact_Div_Expr;
      break;
    case NEGATE_EXPR:
      tcode = Negate_Expr;
      break;
    ...
    case TRUTH_OR_EXPR:
      ...
      break;
    case TRUTH_XOR_EXPR:
      tcode = Truth_Xor_Expr;
      break;
    case TRUTH_NOT_EXPR:
      tcode = Truth_Not_Expr;
      break;
    case LT_EXPR:
      tcode = Lt_Expr;
      break;
	       ...
	       enum tree_code kind, int component_p, int zero_ok)
{
  ...
  if (kind == VAR_DECL
				     ...
				     bitsize_unit_node)))
    {
      if (component_p)
	post_error_ne
	  ("component size for& is not a multiple of Storage_Unit",
	   gnat_error_node, gnat_object);
      else
  ...
  if (TREE_CODE (type_size) != INTEGER_CST
      || TREE_OVERFLOW (type_size) || tree_int_cst_lt (size, type_size))
    {
      if (component_p)
	post_error_ne_tree
...
make_type_from_size (tree type, tree size_tree, int biased_p)
{
  ...
  switch (TREE_CODE (type))
	...
	break;
      ...
      TREE_TYPE (new_type) = TREE_TYPE (type) != 0 ? TREE_TYPE (type) : type;
...
#define MAX_OFILE_ALIGNMENT BIGGEST_ALIGNMENT
  ...
  if (!UI_Is_In_Int_Range (alignment)
    ...
    post_error_ne_num ("alignment for& must be at least ^",
		       gnat_error_node, gnat_entity, align / BITS_PER_UNIT);
  else
...
check_ok_for_atomic (tree object, Entity_Id gnat_entity, int comp_p)
{
  ...
  if (size != 0 && TREE_CODE (size) == INTEGER_CST
      && compare_tree_int (size, align) == 0 && align <= BITS_PER_WORD)
    return;
	  ...
	  || CONTAINS_PLACEHOLDER_P (TYPE_MAX_VALUE (t)))
	{
	  ...
	  if (TYPE_INDEX_TYPE (t))
	    SET_TYPE_INDEX_TYPE (new,
				 gnat_substitute_in_type (TYPE_INDEX_TYPE (t),
							  f, r));
	  return new;
	}
