...
{
  if (TREE_CODE (type) == RECORD_TYPE && TYPE_LEFT_JUSTIFIED_MODULAR_P (type))
...
compare_arrays (tree result_type, tree a1, tree a2)
{
  ...
  while (TREE_CODE (t1) == ARRAY_TYPE && TREE_CODE (t2) == ARRAY_TYPE)
      ...
      else if (TREE_CODE (length2) == INTEGER_CST)
	{
	  ...
	  if (CONTAINS_PLACEHOLDER_P (comparison))
	    comparison =
	      build (WITH_RECORD_EXPR, result_type, comparison, a1);
	  if (CONTAINS_PLACEHOLDER_P (length1))
	    length1 = build (WITH_RECORD_EXPR, bt, length1, a1);
		 ...
		 tree result_type, tree left_operand, tree right_operand)
{
  ...
  /* If either operand is a NULL_EXPR, just return a new one.  */
    ...
    return build1 (NULL_EXPR, operation_type,
		   TREE_OPERAND (right_operand, 0));
  else
	  ...
	  if (op_code == ATTR_ADDR_EXPR)
	    {
	      ...
	      inner =
		get_inner_reference (operand, &bitsize, &bitpos, &offset,
  ...
  return
    build_call_2_expr (fndecl,
		       build1 (ADDR_EXPR, build_pointer_type (char_type_node),
			       filename), build_int_2 (input_line, 0));
}

  ...
  if (DECL_CONTEXT (field) != record_type)
	...
	for (new_field = TYPE_FIELDS (record_type); new_field != 0;
	  ...
	  if (DECL_INTERNAL_P (new_field))
	    {
	      tree field_ref = build_simple_component_ref (record_variable,
							   NULL_TREE,
							   new_field,
							   no_fold_p);
	      ref =
		build_simple_component_ref (field_ref, NULL_TREE, field,
					    no_fold_p);
			  ...
			  Entity_Id gnat_pool, Node_Id gnat_node)
{
  ...
  if (Present (gnat_proc))
      ...
      if (Is_Tagged_Type (Etype (gnat_pool)))
	{
	  ...
	  tree gnu_proc_addr =
	    build_unary_op (ADDR_EXPR, NULL_TREE, gnu_proc);
	  tree gnu_pool = gnat_to_gnu (gnat_pool);
	  tree gnu_pool_addr =
	    build_unary_op (ADDR_EXPR, NULL_TREE, gnu_pool);
...
build_allocator (tree type,
		 tree init,
		 tree result_type,
		 Entity_Id gnat_proc, Entity_Id gnat_pool, Node_Id gnat_node)
{
  ...
  /* If RESULT_TYPE is a fat or thin pointer, set SIZE to be the sum of the
	...
	= (TYPE_FAT_POINTER_P (result_type)
      ...
      tree storage_type = build_unc_object_type (template_type, type,
