...
void
  ...
  (OS_Time * p_time,
   int *p_year,
   int *p_month, int *p_day, int *p_hours, int *p_mins, int *p_secs)
{
  struct tm *res;
...
__gnat_readlink (char *path ATTRIBUTE_UNUSED,
		 char *buf ATTRIBUTE_UNUSED, size_t bufsiz ATTRIBUTE_UNUSED)
{
...
__gnat_open_read (char *path, int fmode)
{
  ...
  fd = open (path, O_RDONLY | o_fmode, 0444, "mbc=16", "deq=64", "fop=tef");
#elif defined (__vxworks)
...
#if defined (VMS)
  fd = open (path, O_RDWR | o_fmode, PERM, "mbc=16", "deq=64", "fop=tef");
#else
...
__gnat_get_libraries_from_registry (void)
{
  ...
  if (res == ERROR_SUCCESS)
    res =
      RegOpenKeyExA (reg_key, "Standard Libraries", 0, KEY_READ, &reg_key);
  ...
  if (strlen (dirspec))
      ...
      if (strchr (dirspec, ']') || strchr (dirspec, ':'))
	{
	  strncpy (new_canonical_dirspec,
		   (char *) decc$translate_vms (dirspec), MAXPATH);
	}
      else if (!strchr (dirspec, '/') && (dirspec1 = getenv (dirspec)) != 0)
	{
	  strncpy (new_canonical_dirspec,
		   (char *) decc$translate_vms (dirspec1), MAXPATH);
	}
      else
	{
...
convert_addresses (void *addrs ATTRIBUTE_UNUSED,
		   ...
		   void *buf ATTRIBUTE_UNUSED, int *len ATTRIBUTE_UNUSED)
{
  *len = 0;
}
#endif
