	     ...
	     Entity_Id standard_exception_type, Int gigi_operating_mode)
{
  ...
  if (!Stack_Check_Probes_On_Target)
    set_stack_check_libfunc (gen_rtx
			     (SYMBOL_REF, Pmode, "_gnat_stack_check"));
...
tree_transform (Node_Id gnat_node)
{
  ...
  switch (Nkind (gnat_node))
	...
	gigi_abort (304);
	  ...
	  && Present (Full_View (gnat_temp)) && !Is_Type (gnat_temp))
	{
	...
	gnu_result = convert (gnu_result_type,
			      build_int_2 (Char_Literal_Value (gnat_node),
					   0));
      break;
	  ...
	  if (Present (Freeze_Node (gnat_temp)))
		...
		gnu_expr
		  = create_var_decl (create_concat_name (gnat_temp, "init"),
				     ...
				     gnu_expr, 0, Is_Public (gnat_temp), 0, 0,
				     0);
	      else
	...
	if (TYPE_CONVENTION_FORTRAN_P (TREE_TYPE (gnu_array_object)))
	  for (i = ndim - 1, gnat_temp = First (Expressions (gnat_node));
	       i >= 0; i--, gnat_temp = Next (gnat_temp))
	    gnat_expr_array[i] = gnat_temp;
	else
	  for (i = 0, gnat_temp = First (Expressions (gnat_node));
	       i < ndim; i++, gnat_temp = Next (gnat_temp))
	    gnat_expr_array[i] = gnat_temp;
	...
	switch (attribute)
	  ...
	  case Attr_Pool_Address:
		  ...
		  && TYPE_CONTAINS_TEMPLATE_P (gnu_obj_type))
		{
		  ...
		  tree gnu_byte_offset = convert (gnu_char_ptr_type,
	...
	gnu_result =
	  build_binary_op (code, gnu_result_type, gnu_lhs, gnu_rhs);
      }
      break;
	      ...
	      Modular_Integer_Kind))
	{
	  enum tree_code code
	    = (Nkind (gnat_node) == N_Op_Or ? BIT_IOR_EXPR
	       : Nkind (gnat_node) == N_Op_And ? BIT_AND_EXPR : BIT_XOR_EXPR);
	...
	if (IN (Nkind (gnat_node), N_Op_Shift) && !Shift_Count_OK (gnat_node))
	  {
	    ...
	    (gnu_type,
	     build_binary_op (GE_EXPR, integer_type_node,
			      ...
			      convert (TREE_TYPE (gnu_rhs),
				       TYPE_SIZE (gnu_type))),
	     convert (gnu_type, integer_zero_node), gnu_result);
      }
      break;
	...
	for (gnat_when = First_Non_Pragma (Alternatives (gnat_node));
	     Present (gnat_when); gnat_when = Next_Non_Pragma (gnat_when))
	  {
	...
	for (gnat_statement = First (Statements (gnat_node));
	     Present (gnat_statement); gnat_statement = Next (gnat_statement))
	  gnat_to_code (gnat_statement);
	...
	if (Present (Name (gnat_node)))
	  loop_id = TREE_LOOP_ID (get_gnu_tree (Entity (Name (gnat_node))));
	...
	for (gnat_actual = First_Actual (gnat_node);
	     ...
	     gnat_actual = Next_Actual (gnat_actual))
	  {
	    ...
	    if (Ekind (gnat_formal) != E_Out_Parameter
		&& !unchecked_convert_p && Do_Range_Check (gnat_actual))
	...
	for (gnu_expr = gnu_after_list;
	     gnu_expr; gnu_expr = TREE_CHAIN (gnu_expr))
	  expand_expr_stmt (build_binary_op (MODIFY_EXPR, NULL_TREE,
	    ...
	    gnu_except_ptr_stack
	      = tree_cons (NULL_TREE,
			   ...
			   (get_identifier ("EXCEPT_PTR"), NULL_TREE,
			    ...
			    0, 0, 0, 0, 0), gnu_except_ptr_stack);
      ...
      if (Exception_Mechanism == GCC_ZCX)
	{
...

	  {
	    ...
	    tree gnu_current_exc_ptr = build (EXC_PTR_EXPR, ptr_type_node);
      ...
      if (!type_annotate_only)
	{
	  ...
	  for (; gnu_output_list;
	    ...
	    if (TREE_VALUE (gnu_orig_out_list) !=
  ...
  while ((TREE_CODE (gnu_result) == NOP_EXPR
	 ...
	 && TREE_TYPE (TREE_OPERAND (gnu_result, 0)) ==
	 TREE_TYPE (gnu_result))
...
elaborate_all_entities (Node_Id gnat_node)
{
  ...
  for (gnat_with_clause = First (Context_Items (gnat_node));
       Present (gnat_with_clause); gnat_with_clause = Next (gnat_with_clause))
	...
	else if (Ekind (Entity (Name (gnat_with_clause))) ==
		 E_Generic_Package)
	  {
	    Node_Id gnat_body
...
process_freeze_entity (Node_Id gnat_node)
{
  ...
  gnu_old = present_gnu_tree (gnat_entity) ? get_gnu_tree (gnat_entity) : 0;
		     ...
		     && Present (Freeze_Node
				 (Corresponding_Spec
				  (Proper_Body (Unit
						(Library_Unit
						 (gnat_decl)))))))
	      record_code_position (Proper_Body
				    (Unit (Library_Unit (gnat_decl))));
...
emit_index_check (tree gnu_array_object,
		  tree gnu_expr, tree gnu_low, tree gnu_high)
{
  tree gnu_expr_check;
...
convert_with_check (Entity_Id gnat_type,
		    tree gnu_expr,
		    int overflow_p, int range_p, int truncate_p)
{
  tree gnu_type = get_unpadded_type (gnat_type);
  ...
  if (IN (Ekind (gnat_entity), Record_Kind)
	...
	= get_gnu_tree (Corresponding_Concurrent_Type (gnat_entity));
      ...
      save_gnu_tree (Corresponding_Concurrent_Type (gnat_entity),
		     NULL_TREE, 0);
      save_gnu_tree (Corresponding_Concurrent_Type (gnat_entity), gnu_new, 0);
...
pos_to_constructor (Node_Id gnat_expr,
		    tree gnu_array_type, Entity_Id gnat_component_type)
{
  tree gnu_expr;
...
post_error_ne_tree_2 (const char *msg,
		      Node_Id node, Entity_Id ent, tree t, int num)
{
