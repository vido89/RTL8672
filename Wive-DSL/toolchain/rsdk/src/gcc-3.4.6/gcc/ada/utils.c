...
/* This variable keeps a table for types for each precision so that we only
   ...
   goes later.  */
     static GTY (())
     tree signed_and_unsigned_types[2 * MAX_BITS_PER_WORD + 1][2];

/* Likewise for float types, but record these by mode.  */
     static GTY (())
     tree float_types[NUM_MACHINE_MODES];
...
/* A chain of binding_level structures awaiting reuse.  */
     ...
     struct binding_level *free_binding_level;
...
finish_record_type (tree record_type,
		    tree fieldlist, int has_rep, int defer_debug)
{
  ...
  if (has_rep)
	...
	MAX (BITS_PER_UNIT, TYPE_ALIGN (record_type));
  ...
  for (field = fieldlist; field; field = TREE_CHAIN (field))
	...
	var_size = 1;
	  ...
	  && !TYPE_CONTAINS_TEMPLATE_P (type) && TYPE_ADA_SIZE (type) != 0)
	...
	case QUAL_UNION_TYPE:
	  ...
	  size_unit =
	    fold (build
		  (COND_EXPR, sizetype, DECL_QUALIFIER (field),
  ...
  if (!defer_debug)
	       ...
	       && TYPE_IS_PADDING_P (record_type)))
	{
	  ...
	  tree new_id = concat_id_with_name (orig_id,
					     TREE_CODE (record_type) ==
					     QUAL_UNION_TYPE ? "XVU" : "XVE");
...
merge_sizes (tree last_size,
	     tree first_bit, tree size, int special, int has_rep)
{
  ...
  *pvar = convert (bitsizetype, in);
  ...
  if (TREE_CODE (in) == INTEGER_CST)
  ...
  else if (TREE_CODE (in) == PLUS_EXPR || TREE_CODE (in) == MINUS_EXPR)
    {
      ...
      if (lhs_var == TREE_OPERAND (in, 0) && rhs_var == TREE_OPERAND (in, 1))
...
create_subprog_type (tree return_type,
		     ...
		     int returns_by_ref, int returns_with_dsp)
{
...
create_var_decl (tree var_name,
		 ...
		 int const_flag,
		 int public_flag,
		 int extern_flag, int static_flag, struct attrib * attr_list)
{
  int init_const
    = (var_init == 0
       ? 0
  ...
  tree var_decl = build_decl ((const_flag && init_const
  ...
  if (global_bindings_p () && var_init != 0 && !init_const)
    {
      ...
      var_init = 0;
	   ...
	   && ((TYPE_MAIN_VARIANT (TREE_TYPE (var_init))
		!= TYPE_MAIN_VARIANT (type)) || (static_flag && !init_const)))
...
create_subprog_decl (tree subprog_name,
		     ...
		     int inline_flag,
		     int public_flag,
		     int extern_flag, struct attrib * attr_list)
{
...
build_vms_descriptor (tree type, Mechanism_Type mech, Entity_Id gnat_entity)
{
  ...
  if (mech != By_Descriptor_NCA
    ...
    for (i = ndim - 1, inner_type = type;
	 i >= 0; i--, inner_type = TREE_TYPE (inner_type))
      idx_arr[i] = TYPE_DOMAIN (inner_type);
  else
    for (i = 0, inner_type = type;
	 i < ndim; i++, inner_type = TREE_TYPE (inner_type))
      idx_arr[i] = TYPE_DOMAIN (inner_type);
  ...
  switch (TREE_CODE (type))
	...
	dtype =
	  GET_MODE_BITSIZE (TYPE_MODE (TREE_TYPE (type))) == 32 ? 54 : 55;
      break;
  ...
  switch (mech)
	...
	= chainon (field_list,
		   ...
		   ("AFLAGS", gnat_type_for_size (8, 1), record_type,
		    size_int (mech == By_Descriptor_NCA ? 0
	...
	= chainon (field_list,
		   ...
		   ("A0", build_pointer_type (inner_type), record_type,
		    build1 (ADDR_EXPR, build_pointer_type (inner_type),
			    tem)));
	...
	{
	  char fname[3];
	  tree idx_length = size_binop (MULT_EXPR, tem,
...
update_pointer_to (tree old_type, tree new_type)
{
  ...
  /* Now deal with the unconstrained array case. In this case the "pointer"
	...
	= size_binop (PLUS_EXPR,
		      ...
		      DECL_SIZE_UNIT (TREE_CHAIN
				      (TYPE_FIELDS (new_obj_rec))));
  ...
  if (!notrunc_p
      ...
      && !(TREE_UNSIGNED (type) && TREE_UNSIGNED (etype)))
    {
      tree base_type = gnat_type_for_mode (TYPE_MODE (type),
					   TREE_UNSIGNED (type));
      tree shift_expr = convert (base_type,
				 size_binop (MINUS_EXPR,
					     bitsize_int
					     (GET_MODE_BITSIZE
					      (TYPE_MODE (type))),
