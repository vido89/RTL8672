...
open_dump_file (enum dump_file_index index, tree decl)
{
  ...
  if (!dump_file[index].initialized)
    {
      ...
      if (graph_dump_format != no_graph && dump_file[index].graph_dump_p)
	{
  ...
  dump_name = concat (dump_base_name, seq, dump_file[index].extension, NULL);
  ...
  if (decl)
    fprintf (rtl_dump_file, "\n;; Function %s%s\n\n",
	     ...
	     FUNCTION_FREQUENCY_UNLIKELY_EXECUTED ? " (unlikely executed)" :
	     "");
...
close_dump_file (enum dump_file_index index,
		 void (*func) (FILE *, rtx), rtx insns)
{
  if (!rtl_dump_file)
    return;

  timevar_push (TV_DUMP);
  if (insns && graph_dump_format != no_graph && dump_file[index].graph_dump_p)
    {
      char seq[16];
      char *suffix;
...
check_global_declarations (tree * vec, int len)
{
  ...
  for (i = 0; i < len; i++)
	...
	SET_DECL_RTL (decl, NULL_RTX);
	  ...
	  && DECL_INITIAL (decl) == 0
	  && DECL_EXTERNAL (decl)
	  && !DECL_ARTIFICIAL (decl) && !TREE_PUBLIC (decl))
	{
	  if (TREE_SYMBOL_REFERENCED (DECL_ASSEMBLER_NAME (decl)))
	    pedwarn ("%J'%F' used but never defined", decl, decl);
	  else
	    warning ("%J'%F' declared `static' but never defined", decl,
		     decl);
	  ...
	  && !DECL_IN_SYSTEM_HEADER (decl) && !TREE_USED (decl)
...
rest_of_decl_compilation (tree decl,
			  const char *asmspec, int top_level, int at_end)
{
  ...
  )
{
  ...
  if ((write_symbols == DWARF2_DEBUG
       || write_symbols == VMS_AND_DWARF2_DEBUG) && toplev)
...
rest_of_handle_inlining (tree decl)
{
  ...
  for (parent = DECL_CONTEXT (current_function_decl);
       parent != NULL_TREE; parent = get_containing_scope (parent))
  ...
  if ((DECL_INLINE (decl) && !flag_no_inline) || flag_inline_functions)
    {
...
rest_of_compilation (tree decl)
{
  ...
  if (optimize > 0
      || profile_arc_flag || flag_test_coverage || flag_branch_probabilities)
    {
	  ...
	  && flag_profile_values && flag_value_profile_transformations)
  ...
  if (optimize > 0
      && (flag_unswitch_loops || flag_peel_loops || flag_unroll_loops))
...
exit_rest_of_compilation:
  ...
  if ((*targetm.binds_local_p) (current_function_decl))
	...
	pref = cfun->stack_alignment_needed;
      cgraph_rtl_info (current_function_decl)->
...
#endif
	   , indent, *indent != 0 ? " " : "",
	   lang_hooks.name, version_string, TARGET_NAME, indent, __VERSION__);
  fnotice (file,
	   "%s%sGGC heuristics: --param ggc-min-expand=%d --param ggc-min-heapsize=%d\n",
	   indent, *indent != 0 ? " " : "", PARAM_VALUE (GGC_MIN_EXPAND),
...
default_tree_printer (pretty_printer * pp, text_info * text)
{
  switch (*text->format_spec)
	...
	const char *n = DECL_NAME (t)
	  ? (*lang_hooks.decl_printable_name) (t, 2) : "<anonymous>";
	pp_string (pp, n);
      }
      return true;
...
#endif
		  || flag_test_coverage || warn_notreached);
