...
static
GTY (())
     rtx top_of_stack[MAX_MACHINE_MODE];
     ...
     void init_reg_sets (void)
{
  ...
  for (i = 0; i < N_REG_CLASSES; i++)
	...
	if (int_reg_class_contents[i][j / 32] & ((unsigned) 1 << (j % 32)))
...
int
memory_move_secondary_cost (enum machine_mode mode, enum reg_class class,
			    int in)
{
  ...
  for (i = FIRST_PSEUDO_REGISTER; i < max_regno; i++)
      ...
      if (REG_N_REFS (i))
	{
	  ...
	  for (class = 0; class < (int) N_REG_CLASSES; class++)
	    if (contains_reg_of_mode[(enum reg_class) class]
		[PSEUDO_REGNO_MODE (i)]
#ifdef FORBIDDEN_INC_DEC_CLASSES
		&& (!in_inc_dec[i]
...
scan_one_insn (rtx insn, int pass)
{
  ...
  if (pass == 0 && optimize
			  ...
			  recog_data.operand_mode[1]))
    {
      ...
      rtx dest = gen_lowpart (recog_data.operand_mode[1],
	...
	{
	  basic_block b;
	  FOR_EACH_BB (b) if (insn == BB_HEAD (b))
	    BB_HEAD (b) = newinsn;
	}
	...
	if (TEST_HARD_REG_BIT (reg_class_contents[i], j))
	  {
	      ...
	      if (HARD_REGNO_MODE_OK (j, m))
		{
		  ...
		  if ((0
			...
			(MODE_BASE_REG_CLASS (VOIDmode), m, r) != NO_REGS)
#else
#ifdef SECONDARY_INPUT_RELOAD_CLASS
		       ||
      ...
      for (i = 0; i < n_ops; i++)
	{
	  ...
	  /* Otherwise, if this alternative wins, either because we
	     ...
	     the proper class, there is no cost for this alternative.  */
		   ...
		   || (GET_CODE (op) == REG
		       && reg_fits_class_p (op, classes[i], 0,
					    GET_MODE (op))))
	    ;
  ...
  switch (code)
	...
	/* If both operands are registers but one is already a hard register
	  ...
	  record_address_regs (arg1,
			       ...
			       ? INDEX_REG_CLASS :
			       MODE_BASE_REG_CLASS (VOIDmode), scale);
	else if (code0 == REG && code1 == REG
		 && REGNO (arg1) < FIRST_PSEUDO_REGISTER
		 && (REG_OK_FOR_BASE_P (arg1) || REG_OK_FOR_INDEX_P (arg1)))
	  record_address_regs (arg0,
			       REG_OK_FOR_BASE_P (arg1) ? INDEX_REG_CLASS :
			       MODE_BASE_REG_CLASS (VOIDmode), scale);
#endif
	...
	for (i = 0; i < N_REG_CLASSES; i++)
	  pp->cost[i] +=
	    (may_move_in_cost[Pmode][i][(int) class] * scale) / 2;
      }
      break;
  ...
  if (min < num_regs)
    {
      ...
      for (reg_data = reg_info_head;
	   reg_data && reg_data->max_index >= min; reg_data = reg_data->next)
	{
...
reg_scan (rtx f, unsigned int nregs, int repeat ATTRIBUTE_UNUSED)
{
  ...
  for (insn = f; insn; insn = NEXT_INSN (insn))
	...
	if (GET_CODE (pat) == PARALLEL && XVECLEN (pat, 0) > max_parallel)
...
reg_scan_mark_refs (rtx x, rtx insn, int note_flag, unsigned int min_regno)
{
  ...
  switch (code)
    ...
    case CLOBBER:
      {
	rtx reg = XEXP (x, 0);
	if (REG_P (reg) && REGNO (reg) >= min_regno)
	  {
    ...
    case SET:
      ...
      for (dest = SET_DEST (x);
	   ...
	   || GET_CODE (dest) == ZERO_EXTEND; dest = XEXP (dest, 0))
	;
