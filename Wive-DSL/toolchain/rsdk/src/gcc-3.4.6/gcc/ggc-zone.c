...
alloc_large_page (size_t size, struct alloc_zone *zone)
{
  ...
  page =
    (char *) xmalloc (size + CHUNK_OVERHEAD + sizeof (struct page_entry));
  ...
  if (GGC_DEBUG_LEVEL >= 2)
    fprintf (G.debug_file,
	     ...
	     entry->zone->name, (PTR) entry, page, page + size - 1);
...
free_page (page_entry * entry)
{
  if (GGC_DEBUG_LEVEL >= 2)
    fprintf (G.debug_file,
	     ...
	     (PTR) entry, entry->page, entry->page + entry->bytes - 1);
...
free_chunk (struct alloc_chunk *chunk, size_t size, struct alloc_zone *zone)
{
  ...
  VALGRIND_DISCARD (VALGRIND_MAKE_READABLE
		    (chunk, sizeof (struct alloc_chunk)));
}
  ...
  if (size >= G.pagesize - 2 * CHUNK_OVERHEAD - FREE_BIN_DELTA)
    {
      ...
      VALGRIND_DISCARD (VALGRIND_MAKE_WRITABLE
			(chunk, sizeof (struct alloc_chunk)));
      chunk->large = 1;
  ...
  if (lsize >= CHUNK_OVERHEAD + FREE_BIN_DELTA)
    {
      ...
      VALGRIND_DISCARD (VALGRIND_MAKE_WRITABLE
			(lchunk, sizeof (struct alloc_chunk)));
#ifdef COOKIE_CHECKING
...
ggc_pop_context_1 (struct alloc_zone *zone)
{
  ...
  if (!
      ((zone->context_depth_allocations | zone->
	context_depth_collections) & omask))
    return;

  zone->context_depth_allocations |=
  ...
  if (!zone->dead)
    {
      /* Avoid frequent unnecessary work by skipping collection if the
         ...
         collection.  */
      float allocated_last_gc = MAX (zone->allocated_last_gc,
				     (size_t) PARAM_VALUE (GGC_MIN_HEAPSIZE) *
				     1024);

      float min_expand =
	allocated_last_gc * PARAM_VALUE (GGC_MIN_EXPAND) / 100;
	  ...
	  if (!p->large_p)
	    {
	      ...
	      struct alloc_chunk *end =
		(struct alloc_chunk *) (p->page + G.pagesize);
	      do
		{
		  if (chunk->magic != CHUNK_MAGIC
		      && chunk->magic != DEADCHUNK_MAGIC)
		    abort ();
		  chunk =
		    (struct alloc_chunk *) (chunk->u.data + chunk->size);
		}
	      while (chunk < end);
	    }
	}
      ...
      for (zone = G.zones; zone; zone = zone->next_zone)
	{
	  ...
	  for (p = zone->pages; p; p = p->next)
	      ...
	      if (!p->large_p)
		{
		  ...
		  do
		    {
		      prefetch ((struct alloc_chunk *) (chunk->u.data +
							chunk->size));
		      if (chunk->mark
			  || p->context_depth < zone->context_depth)
			{
			  chunk->mark = 0;
			}
		      chunk =
		      ...
		      size_t size, bool is_string)
{
  ...
  if (!is_string)
    {
      struct alloc_chunk *chunk =
	(struct alloc_chunk *) ((char *) x - CHUNK_OVERHEAD);
      if (chunk->large)
