...
static void
write_units (int num_units, struct range *multiplicity,
	     struct range *simultaneity, struct range *ready_cost,
	     struct range *issue_delay, struct range *blockage)
{
  int i, q_size;
      ...
      else if (GET_CODE (desc) == DEFINE_DELAY)
	{
	  ...
	  for (i = 0; i < XVECLEN (desc, 1); i += 3)
	      ...
	      if (XVECEXP (desc, 1, i + 1) && !have_annul_true)
		{
		  ...
		  printf
		    ("extern int eligible_for_annul_true (rtx, int, rtx, int);\n");
		  have_annul_true = 1;
		}
  ...
  if (num_units > 0 || num_insn_reservations > 0)
      ...
      for (unit = units; unit; unit = unit->next)
	{
	  ...
	  blockage.max = MAX (unit->ready_cost.max
			      - (unit->ready_cost.min - 1), blockage.max);
