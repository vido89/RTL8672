...
basic_block
nearest_common_dominator (enum cdi_direction dir, basic_block bb1,
			  basic_block bb2)
{
  if (!dom_computed[dir])
    abort ();
...
verify_dominators (enum cdi_direction dir)
{
  ...
  FOR_EACH_BB (bb)
  {
	...
	error ("dominator of %d should be %d, not %d",
	       bb->index, dom_bb->index, get_immediate_dominator (dir,
								  bb)->index);
	err = 1;
      }
  }
...
{
  basic_block bb, bb2;
  FOR_EACH_BB (bb) if ((bb2 = get_immediate_dominator (dir, bb)))
    fprintf (stderr, "%i %i\n", bb->index, bb2->index);
}
