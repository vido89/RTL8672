...
file_hash_lookup (const char *string)
{
  ...
  e = htab_find_slot_with_hash (file_table, string,
				(*htab_hash_string) (string), INSERT);
  if (*e == NULL)
    {
...
file_push (file * p)
{
  ...
  ep = obstack_alloc (&file_stack_obstack, sizeof (struct file_stack_entry));
  ep->value = p;
  ep->next = file_stack;
  file_stack = ep;
  p->tweaking = 1;
...
tlink_init (void)
{
  ...
  file_table = htab_create (500, hash_string_hash, hash_string_eq, NULL);
  demangled_table = htab_create (500, hash_string_hash, hash_string_eq, NULL);
...
maybe_tweak (char *line, file * f)
{
  ...
  if ((sym->file == f && sym->tweaking) || (sym->file != f && line[0] == 'C'))
    {
      sym->tweaking = 0;
      sym->tweaked = 1;
