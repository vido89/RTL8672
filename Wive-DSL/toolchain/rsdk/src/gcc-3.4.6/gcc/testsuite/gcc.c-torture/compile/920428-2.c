...
typedef struct fd_set
{
  fd_mask
    fds_bits[(((256) +
	       (((sizeof (fd_mask) * 8)) - 1)) / ((sizeof (fd_mask) * 8)))];
}
fd_set;
typedef struct qhdr
{
...
gs_grestore (gs_state *), gs_grestoreall (gs_state *);
...
int gs_currentscreen (gs_state *, float *, float *,
		      float (**)(floatp, floatp));
...
struct gx_device_procs_s
{
  ...
  int (*copy_mono) (gx_device * dev, unsigned char *data, int data_x,
		    int raster, gx_bitmap_id id, int x, int y, int width,
		    int height, gx_color_index color0, gx_color_index color1);
  int (*copy_color) (gx_device * dev, unsigned char *data, int data_x,
		     int raster, gx_bitmap_id id, int x, int y, int width,
		     int height);
  int (*draw_line) (gx_device * dev, int x0, int y0, int x1, int y1,
		    gx_color_index color);
  int (*get_bits) (gx_device * dev, int y, unsigned char *data,
...
int gx_default_map_color_rgb (gx_device * dev, gx_color_index color,
			      gx_color_value rgb[3]);
int gx_default_tile_rectangle (gx_device * dev, gx_bitmap * tile, int x,
...
gs_rotate (gs_state *, floatp), gs_concat (gs_state *, const gs_matrix *);
...
{
  int code =
    gs_matrix_invert (&*(gs_matrix *) & (pgs)->ctm, &pgs->ctm_inverse);
  0;
  if (code < 0)
    return code;
  pgs->inverse_valid = 1;
...
gs_update_matrix_fixed (gs_matrix_fixed * pmat)
{
  (*pmat).tx =
    ((float)
     (((*pmat).tx_fixed =
       ((fixed) (((*pmat).tx) * (float) (1 << 12)))) * (1.0 / (1 << 12)))),
    (*pmat).ty =
    ((float)
     (((*pmat).ty_fixed =
       ((fixed) (((*pmat).ty) * (float) (1 << 12)))) * (1.0 / (1 << 12))));
}
...
gs_initmatrix (gs_state * pgs)
{
  ...
  (pgs->ctm).tx =
    ((float)
     (((pgs->ctm).tx_fixed =
       ((fixed) (((pgs->ctm).tx) * (float) (1 << 12)))) * (1.0 / (1 << 12)))),
    (pgs->ctm).ty =
    ((float)
     (((pgs->ctm).ty_fixed =
       ((fixed) (((pgs->ctm).ty) * (float) (1 << 12)))) * (1.0 / (1 << 12)))),
...
gs_translate (gs_state * pgs, floatp dx, floatp dy)
{
  ...
  int code;
  if ((code =
       gs_distance_transform (dx, dy, &*(gs_matrix *) & (pgs)->ctm, &pt)) < 0)
    return code;
  pgs->ctm.tx += pt.x;
  pgs->ctm.ty += pt.y;
  (pgs->ctm).tx =
...
{
  int code =
    gs_matrix_rotate (&*(gs_matrix *) & (pgs)->ctm, ang,
		      &*(gs_matrix *) & (pgs)->ctm);
  pgs->inverse_valid = 0, pgs->char_tm_valid = 0;
  return code;
}
...
{
  int code =
    gs_matrix_multiply (pmat, &*(gs_matrix *) & (pgs)->ctm,
			&*(gs_matrix *) & (pgs)->ctm);
  (pgs->ctm).tx =
    ((float)
     (((pgs->ctm).tx_fixed =
...
gs_itransform (gs_state * pgs, floatp x, floatp y, gs_point * pt)
{
  ...
  if (!!
      (((*(long *) (&((&pgs->ctm)->xy)) | *(long *) (&((&pgs->ctm)->yx))) <<
	1) == 0))
    {
      return gs_point_transform_inverse (x, y, &*(gs_matrix *) & (pgs)->ctm,
					 pt);
    }
  else
    {
			  ...
			  int max_bits)
{
  ...
  if (pfc->skewed)
    {
      pfc->xy =
	(((*(long *) (&(ctm.xy)) << 1) == 0) ? 0 : (long) ldexp (ctm.xy,
								 scale));
      pfc->yx =
	(((*(long *) (&(ctm.yx)) << 1) == 0) ? 0 : (long) ldexp (ctm.yx,
								 scale));
    }
  else
    pfc->xy = pfc->yx = 0;
