...
foo (int *ip, unsigned int *uip, short int *hp, unsigned short int *uhp,
     ...
     void **pp, int *n, long long *llp, unsigned long long *ullp,
     ...
     unsigned_ptrdiff_t * utp, ptrdiff_t * tn)
{
  ...
  scanf ("%d%i%o%u%x%X%a%A%e%E%f%F%g%G%s%[abc]%c%p%n%%", ip, ip, uip, uip,
	 uip, uip, fp, fp, fp, fp, fp, fp, fp, fp, s, s, s, pp, n);
