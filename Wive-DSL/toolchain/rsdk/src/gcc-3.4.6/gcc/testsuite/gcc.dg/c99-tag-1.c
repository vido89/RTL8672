...
foo (void)
{
     ...
     not.  */
  {
    ...
    enum e2
    { E2A = sizeof (enum e2 *) };	/* { dg-bogus "warning" "warning in place of error" } */
     ...
     may redeclare the tag in different scopes.  */
  {
    ...
    enum e0
    { E0B };			/* { dg-bogus "warning" "warning in place of error" } */
  ...
  /* Structure, union and enumerated types have a single namespace of tags.  */
  {
    ...
    enum u0
    { U0A };			/* { dg-bogus "warning" "warning in place of error" } */
    /* { dg-error "wrong" "wrong tag type" { target *-*-* } 78 } */
    enum u2
    { U2A };			/* { dg-bogus "warning" "warning in place of error" } */
