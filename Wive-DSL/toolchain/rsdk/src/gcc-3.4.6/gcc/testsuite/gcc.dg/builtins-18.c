...
main (void)
{
  /* For each type, test both runtime and compile time (constant folding)
     optimization.  */
  float _Complex fc = 3.0F + 4.0i F;
  double _Complex dc = 3.0 + 4.0i;
  long double _Complex ldc = 3.0L + 4.0i L;
  ...
  if (__builtin_cabsf (fc) != 5.0F)
    link_error ();
  if (cabsf (3.0F + 4.0i F) != 5.0F)
    link_failure ();
  if (__builtin_cabsf (3.0F + 4.0i F) != 5.0F)
    link_failure ();
#endif
  ...
  if (__builtin_cabsl (ldc) != 5.0L)
    link_error ();
  if (cabsl (3.0L + 4.0i L) != 5.0L)
    link_failure ();
  if (__builtin_cabsl (3.0L + 4.0i L) != 5.0L)
    link_failure ();
#endif
