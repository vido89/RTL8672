...
/* This table maps from register number to values.  It does not
   ...
   element.  */
     static GTY (())
     varray_type reg_values;
     static GTY ((deletable ("")))
     varray_type reg_values_old;
...
/* Here the set of indices I with REG_VALUES(I) != 0 is saved.  This is used
   in clear_table() for fast emptying.  */
     static GTY (())
     varray_type used_regs;
     static GTY ((deletable ("")))
     varray_type used_regs_old;
...
/* Allocate a struct elt_list and fill in its two elements with the
   arguments.  */
     ...
     static inline struct elt_list *new_elt_list (struct elt_list *next,
						  cselib_val * elt)
{
  struct elt_list *el;
      ...
      while (*p)
	{
	  ...
	  if (num_mems < PARAM_VALUE (PARAM_MAX_CSELIB_MEMORY_LOCATIONS)
	      && !canon_true_dependence (mem_rtx, GET_MODE (mem_rtx),
					 mem_addr, x, cselib_rtx_varies_p))
	    {
	      has_mem = true;
	      num_mems++;
	      p = &(*p)->next;
...
cselib_init (void)
{
  ...
  value_pool = create_alloc_pool ("value", RTX_SIZE (VALUE), 10);
