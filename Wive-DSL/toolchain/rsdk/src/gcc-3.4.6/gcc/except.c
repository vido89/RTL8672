  ...
  if (USING_SJLJ_EXCEPTIONS)
    {
      ...
      tmp =
	build_array_type ((*lang_hooks.types.type_for_mode) (word_mode, 1),
			  tmp);
	...
	= (tree_low_cst (DECL_FIELD_OFFSET (f_data), 1)
	   + tree_low_cst (DECL_FIELD_BIT_OFFSET (f_data),
			   1) / BITS_PER_UNIT);
...
expand_eh_region_end_cleanup (tree handler)
{
  ...
  if (flag_non_call_exceptions || region->may_contain_throw)
	...
	= (lang_protect_cleanup_actions
	   ? (*lang_protect_cleanup_actions) () : NULL_TREE);
...
remove_fixup_regions (void)
{
  ...
  for (insn = get_insns (); insn; insn = NEXT_INSN (insn))
	...
	if (fixup->u.fixup.real_region)
	  XEXP (note, 0) =
	    GEN_INT (fixup->u.fixup.real_region->region_number);
	else
  ...
  for (i = cfun->eh->last_region_number; i > 0; --i)
      ...
      if (r && r->region_number == i && !reachable[i])
	{
	  /* Don't remove ERT_THROW regions if their outer region
	     is reachable.  */
	  if (r->type == ERT_THROW
	      && r->outer && reachable[r->outer->region_number])
	    continue;
...
convert_from_eh_region_ranges_1 (rtx * pinsns, int *orig_sp, int cur)
{
  ...
  for (insn = *pinsns; insn; insn = next)
	...
	{
	  if (cur > 0 && !find_reg_note (insn, REG_EH_REGION, NULL_RTX)
	    ...
	    {
	      REG_NOTES (insn) =
		alloc_EXPR_LIST (REG_EH_REGION, GEN_INT (cur),
				 REG_NOTES (insn));
	    }
...
static void
sjlj_assign_call_site_values (rtx dispatch_label,
  ...
  if (region->type == ERT_CATCH)
    {
      ...
      for (try = region->next_peer;
	   try->type == ERT_CATCH; try = try->next_peer)
	continue;
      if (try->type != ERT_TRY)
	abort ();
