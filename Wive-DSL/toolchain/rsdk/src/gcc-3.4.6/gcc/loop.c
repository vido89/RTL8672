...
scan_loop (struct loop *loop, int flags)
{
  ...
  if (GET_CODE (p) == JUMP_INSN
      ...
      && any_uncondjump_p (p) && JUMP_LABEL (p) != 0
  ...
  for (in_libcall = 0, p = next_insn_in_loop (loop, loop->scan_start);
       p != NULL_RTX; p = next_insn_in_loop (loop, p))
    {
      ...
      if (GET_CODE (p) == INSN)
	{
	  ...
	  if (temp)
	    in_libcall++;
	  if (!in_libcall
	      && (set = single_set (p)) && GET_CODE (SET_DEST (set)) == REG
	      ...
	      if (GET_CODE (PATTERN (p)) == PARALLEL)
		{
	      ...
	      if (		/* The register is used in basic blocks other
		   ...
		   && (maybe_never || loop_reg_used_before_p (loop, set, p)))
		...
		;
			   ...
			   1
			   || (tem1 =
			       consec_sets_invariant_p (loop, SET_DEST (set),
							regs->
							array[REGNO
							      (SET_DEST
							       (set))].
		       ...
		       && !((maybe_never || call_passed) && may_trap_p (src)))
		{
		  ...
		  if (loop_info->has_call
			...
			= (replace_rtx
		      ...
		      delete_insn (p);
		      for (i = 0;
			   i < LOOP_REGNO_NREGS (regno, SET_DEST (set)); i++)
...
ignore_some_movables (struct loop_movables *movables)
{
  ...
  for (m = movables->head; m; m = m->next)
      ...
      if (note)
	{
	  ...
	  for (insn = XEXP (note, 0); insn != m->insn;
	       insn = NEXT_INSN (insn))
  ...
  for (m = movables->head; m; m = m->next)
	...
	for (m1 = m->next; m1; m1 = m1->next)
	  if (m != m1 && m1->match == 0
	      ...
	      &&!m1->partial && (matched_regs[m1->regno] || (
	  ...
	  if (already_moved[regno]
		  ...
		  for (count = m->consec; count >= 0; count--)
			  ...
			  && (temp = find_reg_note (p, REG_RETVAL, NULL_RTX)))
			{
			  ...
			  for (temp = XEXP (temp, 0); temp != p;
				...
				loop_insn_emit_after (loop, 0,
						      fn_address_insn,
						      gen_move_insn (fn_reg,
								     fn_address));
			      ...
			      if (GET_CODE (temp) == CALL_INSN)
				{
				  ...
				  if (CALL_INSN_FUNCTION_USAGE (temp))
				    CALL_INSN_FUNCTION_USAGE (i1)
				      =
				      copy_rtx (CALL_INSN_FUNCTION_USAGE
						(temp));
				}
			      else
		      ...
		      else if (count == m->consec && m->move_insn_first)
			{
			  ...
			  start_sequence ();
			  emit_move_insn (m->insert_temp ? newreg : m->
					  set_dest, m->set_src);
			  seq = get_insns ();
			  end_sequence ();
	      ...
	      if (!m->insert_temp)
		{
		  ...
		  if (!m->partial)
		    {
		      int i;
		      for (i = 0; i < LOOP_REGNO_NREGS (regno, m->set_dest);
			   i++)
			regs->array[regno + i].set_in_loop = 0;
		    }
  ...
  for (insn = f; insn; insn = NEXT_INSN (insn))
	...
	if (this_loop
		...
		|| (any_uncondjump_p (insn)
			...
			!= this_loop))) && get_max_uid () < max_uid_for_loop)
	  {
	    rtx p;
	    ...
	    for (p = PREV_INSN (insn);
		 ...
		 && !(GET_CODE (p) == NOTE
		      && NOTE_LINE_NUMBER (p) == NOTE_INSN_LOOP_BEG)
		 && GET_CODE (p) != JUMP_INSN; p = PREV_INSN (p))
	      ;
	    ...
	    if (!outer_loop
		&& GET_CODE (p) == JUMP_INSN && JUMP_LABEL (p) != 0
			...
			if (JUMP_LABEL (insn))
			  {
			      ...
			      if (XEXP (r, 0) == JUMP_LABEL (insn))
				{
				  ...
				  if (q)
				    ...
				    this_loop->exit_labels =
				      LABEL_NEXTREF (r);
				  break;
				}

			    for (loop = this_loop;
				 loop && loop != target_loop;
				 loop = loop->outer)
			      loop->exit_count--;
...
static void
note_addr_stored (rtx x, rtx y ATTRIBUTE_UNUSED, void *data ATTRIBUTE_UNUSED)
{
...
note_set_pseudo_multiple_uses (rtx x, rtx y ATTRIBUTE_UNUSED, void *data)
{
  ...
  while (GET_CODE (x) == STRICT_LOW_PART
	 ...
	 || GET_CODE (x) == ZERO_EXTRACT || GET_CODE (x) == SUBREG)
    x = XEXP (x, 0);
...
find_single_use_in_loop (struct loop_regs *regs, rtx insn, rtx x)
{
  ...
  if (code == REG)
    regs->array[REGNO (x)].single_usage
	 ...
	 && regs->array[REGNO (x)].single_usage != insn) ? const0_rtx : insn;
	  ...
	  for (i = 0; i < LOOP_REGNO_NREGS (regno, dest); i++)
	    {
	      ...
	      if (regs->array[regno].set_in_loop > 0 && last_set[regno] == 0)
...
for_each_insn_in_loop (struct loop *loop, loop_insn_callback fncall)
{
  ...
  for (p = next_insn_in_loop (loop, loop->scan_start);
		   ...
		   || (NEXT_INSN (p) == loop->end && any_condjump_p (p)))))
	{
	  ...
	  for (label = loop->exit_labels; label;
  ...
  if (loop_dump_stream)
    {
      ...
      fprintf (loop_dump_stream,
	       ...
	       REGNO_FIRST_UID (bl->regno), REGNO_LAST_UID (bl->regno));
    }
  return 0;
}
      ...
      if (!v->ignore && v->same == 0)
	{
	  ...
	  if (v->giv_type == DEST_ADDR && bl->biv_count == 1
		...
		if ((auto_inc_opt == 1 && sets_cc0_p (PATTERN (v->insn)))
			...
			&& INSN_P (prev) && sets_cc0_p (PATTERN (prev))))
		  auto_inc_opt = 0;
	      }
#endif
...
loop_givs_dead_check (struct loop *loop ATTRIBUTE_UNUSED, struct iv_class *bl)
{
  ...
  for (v = bl->giv; v; v = v->next_iv)
    {
      if (v->ignore || (v->same && v->same->ignore))
	continue;
...
loop_givs_rescan (struct loop *loop, struct iv_class *bl, rtx * reg_map)
{
  ...
  for (v = bl->giv; v; v = v->next_iv)
	...
	v->new_reg = replace_rtx (v->new_reg,
	  ...
	  && v->giv_type == DEST_REG && REG_POINTER (v->dest_reg))
	mark_reg_pointer (v->new_reg,
			  REGNO_POINTER_ALIGN (REGNO (v->dest_reg)));
      else if (GET_CODE (v->new_reg) == REG && REG_POINTER (v->src_reg))
	{
	  ...
	  mark_reg_pointer (v->new_reg, align);
	       ...
	       && GET_CODE (v->add_val) == REG && REG_POINTER (v->add_val))
	{
			 ...
			 rtx test_reg)
{
  ...
  if (v->giv_type == DEST_ADDR
      ...
      && benefit > 0 && GET_CODE (v->mult_val) == CONST_INT)
    {
      int size = GET_MODE_SIZE (GET_MODE (v->mem));

      if (HAVE_POST_INCREMENT && INTVAL (v->mult_val) == size)
	benefit += add_cost * bl->biv_count;
      else if (HAVE_PRE_INCREMENT && INTVAL (v->mult_val) == size)
	benefit += add_cost * bl->biv_count;
      else if (HAVE_POST_DECREMENT && -INTVAL (v->mult_val) == size)
	benefit += add_cost * bl->biv_count;
      else if (HAVE_PRE_DECREMENT && -INTVAL (v->mult_val) == size)
	benefit += add_cost * bl->biv_count;
    }
#endif
  ...
  for (bl = ivs->list; bl; bl = bl->next)
	  ...
	  && maybe_eliminate_biv (loop, bl, 1, threshold, insn_count))
	{
	  ...
	  if (bl->final_value && !bl->reversed)
	    loop_insn_sink_or_swim (loop,
				    gen_load_of_final_value (bl->biv->
							     dest_reg,
							     bl->
							     final_value));
		     ...
		     int maybe_multiple)
{
  ...
  if (GET_CODE (p) == INSN)
    find_mem_givs (loop, PATTERN (p), p, not_every_iteration, maybe_multiple);
	    ...
	    int not_every_iteration, int maybe_multiple, rtx * location)
{
  ...
  if (temp && !(GET_CODE (add_val) == MULT && GET_CODE (temp) == ASHIFT))
    add_val = temp;
      ...
      while (1)
	{
	  ...
	  if (GET_CODE (p) == INSN || GET_CODE (p) == JUMP_INSN
	      || GET_CODE (p) == CALL_INSN)
	    {
	      ...
	      if (!biv_increment_seen && reg_set_p (v->src_reg, PATTERN (p)))
...
static rtx
simplify_giv_expr (const struct loop *loop, rtx x, rtx * ext_val,
		   int *benefit)
{
  struct loop_ivs *ivs = LOOP_IVS (loop);
  struct loop_regs *regs = LOOP_REGS (loop);
  ...
  switch (GET_CODE (x))
	...
	return NULL_RTX;
	  ...
	  && !((GET_CODE (arg0) == USE
		&& GET_CODE (arg1) == USE) || GET_CODE (arg1) == CONST_INT))
			...
			if (GET_CODE (tem) == PLUS
			    ...
			    || GET_CODE (tem) == SYMBOL_REF)
			  {
			    ...
			    if (tem)
				 ...
				 && GET_CODE (XEXP (XEXP (tem, 0), 0)) ==
				 SYMBOL_REF
				 && GET_CODE (XEXP (XEXP (tem, 0), 1)) ==
				 CONST_INT)
			  {
		 ...
		 rtx * ext_val, rtx * last_consec_insn)
{
  ...
  while (count > 0)
      ...
      else if (code != NOTE)
	{
	  ...
	  if (code == INSN
	      && (set = single_set (p))
	      && SET_DEST (set) != dest_reg && CONSTANT_P (SET_SRC (set)))
	    continue;
  ...
  if (INSN_P (tmp))
      ...
      while (tmp != NULL_RTX)
	{
	  ...
	  if (++n_insns > 3
		  ...
		  && GET_CODE (SET_SRC (XVECEXP (PATTERN (tmp), 0, 0))) ==
		  MULT))
	    {
	      win = 0;
	      break;
...
check_dbra_loop (struct loop *loop, int insn_count)
{
  ...
  if (((GET_CODE (comparison) == GT && XEXP (comparison, 1) == constm1_rtx)
	   ...
	   && INTVAL (bl->biv->add_val) > 0)
    {
      ...
      if (bl->giv_count == 0
	  && !loop->exit_count && !loop_info->has_multiple_exit_targets)
	{
	...
	return 0;
      ...
      if ((num_nonfixed_reads <= 1
	   ...
	   && (bl->giv_count + bl->biv_count + loop_info->num_mem_sets
	       + num_unmoved_movables (loop) + compare_and_branch ==
	       insn_count) && (bl == ivs->list && bl->next == 0))
	  || (no_use_except_counting && !loop_info->has_prefetch))
	{
	  ...
	  if (comparison
		...
		{
		  comparison_val =
		    comparison_val - INTVAL (bl->initial_value);
		  ...
		  &&
		  !(((comparison_val -
		      add_val) ^ INTVAL (comparison_value)) &
		    comparison_sign_mask))
		{
		...
		add_adjust -= add_val;
		  ...
		  && GET_CODE (comparison_value) == CONST_INT)
		{
		  if (((unsigned HOST_WIDE_INT) comparison_val % add_val) !=
		      0)
		    return 0;
		}
	      else
	      ...
	      else if (GET_CODE (initial_value) == CONST_INT)
		{
		  ...
		  rtx add_insn =
		    gen_add3_insn (reg, comparison_value, offset);
...
loop_insn_first_p (rtx insn, rtx reference)
{
  ...
  for (p = insn, q = reference;;)
	...
	return 1;
	  ...
	  && INSN_UID (q) < max_uid_for_loop && GET_CODE (p) != NOTE)
	return INSN_LUID (p) <= INSN_LUID (q);

      if (INSN_UID (p) >= max_uid_for_loop || GET_CODE (p) == NOTE)
	p = NEXT_INSN (p);
  ...
  switch (code)
      ...
      if (SET_DEST (x) == cc0_rtx && SET_SRC (x) == reg)
	{
	  ...
	  for (v = bl->giv; v; v = v->next_iv)
		...
		&& v->mult_val != const0_rtx && v->add_val == const0_rtx
      ...
      if (CONSTANT_P (arg))
	{
	  ...
	  for (v = bl->giv; v; v = v->next_iv)
		...
		validate_change (insn, &XEXP (x, 1 - arg_operand), v->new_reg,
				 1);
	  ...
	  if (loop_invariant_p (loop, arg) == 1)
	    {
	      ...
	      for (v = bl->giv; v; v = v->next_iv)
		if (GET_CODE (v->mult_val) == CONST_INT
		    && INTVAL (v->mult_val) > 0 && !v->ignore
		    && !v->maybe_dead && v->always_computable
		    && v->mode == mode && 0)
		  {
		    rtx tem;
			...
			rtx want_reg, int allow_cc_mode)
{
  ...
  while (GET_RTX_CLASS (code) == '<'
	...
	break;
      ...
      if ((prev = prev_nonnote_insn (prev)) == 0
	  || GET_CODE (prev) != INSN || FIND_REG_INC_NOTE (prev, NULL_RTX))
	break;

      set = set_of (op0, prev);

      if (set
	  && (GET_CODE (set) != SET || !rtx_equal_p (SET_DEST (set), op0)))
	break;
  ...
  if (!allow_cc_mode && GET_MODE_CLASS (GET_MODE (op0)) == MODE_CC)
    return 0;
...
get_condition (rtx jump, rtx * earliest, int allow_cc_mode)
{
  ...
  if (GET_CODE (jump) != JUMP_INSN || !any_condjump_p (jump))
    return 0;
  set = pc_set (jump);
...
load_mems (const struct loop *loop)
{
  ...
  for (p = loop->start;
       PREV_INSN (p) && GET_CODE (p) != CODE_LABEL; p = PREV_INSN (p))
    ;
  prev_ebb_head = p;
  ...
  for (i = 0; i < loop_info->mems_idx; ++i)
    {
      ...
      if (MEM_VOLATILE_P (mem) || loop_invariant_p (loop, XEXP (mem, 0)) != 1)
      ...
      if (loop_info->mems[i].optimize && written)
	{
	  ...
	  for (j = 0; j < loop_info->mems_idx; ++j)
		...
		continue;
					...
					VOIDmode,
					loop_info->mems[j].mem, rtx_varies_p))
		{
      ...
      else
	{
	  ...
	  /* Attempt a bit of copy propagation.  This helps untangle the
	     ...
	     more bivs/givs.  */
	  EXECUTE_IF_SET_IN_REG_SET (&load_copies, FIRST_PSEUDO_REGISTER, j,
				     {
				     try_copy_prop (loop, reg, j);}
	  );
	  CLEAR_REG_SET (&load_copies);

	  EXECUTE_IF_SET_IN_REG_SET (&store_copies, FIRST_PSEUDO_REGISTER, j,
				     {
...
try_copy_prop (const struct loop *loop, rtx replacement, unsigned int regno)
{
  ...
  for (insn = next_insn_in_loop (loop, loop->scan_start);
       insn != NULL_RTX; insn = next_insn_in_loop (loop, insn))
    {
      rtx set;
		    ...
		    unsigned int regno)
{
  ...
  if (insn != NULL_RTX)
	  ...
	  && REGNO (SET_DEST (prev_set)) == regno)
	{
	  ...
	  validate_change (prev_insn, &SET_DEST (prev_set), replacement, 1);
	  validate_change (insn, &SET_DEST (set), SET_SRC (set), 1);
	  validate_change (insn, &SET_SRC (set), replacement, 1);
...
loop_biv_dump (const struct induction *v, FILE * file, int verbose)
{
  ...
  fprintf (file, "Biv %d: insn %d", REGNO (v->dest_reg), INSN_UID (v->insn));
  fprintf (file, " const ");
  print_simple_rtl (file, v->add_val);
...
loop_giv_dump (const struct induction *v, FILE * file, int verbose)
{
  ...
  if (v->giv_type == DEST_REG)
    ...
    fprintf (file, "Dest address: insn %d", INSN_UID (v->insn));

  fprintf (file, " src reg %d benefit %d", REGNO (v->src_reg), v->benefit);
  fprintf (file, " lifetime %d", v->lifetime);
  ...
  if (loop->start)
    {
      fprintf (file,
	       ...
	       LOOP_BLOCK_NUM (loop->end), LOOP_INSN_UID (loop->end));
      ...
      if (loop->exit_count)
	{
	  ...
	  for (label = loop->exit_labels; label;
	       label = LABEL_NEXTREF (label))
	    {
	      fprintf (file, " %d ", LOOP_INSN_UID (XEXP (label, 0)));
	    }
	}
      fputs ("\n", file);
