...
static
GTY (())
     varray_type stack_regs_mentioned_data;
...
/* This is the register file for all register after conversion.  */
     static rtx
       FP_mode_reg[LAST_STACK_REG + 1 -
		   FIRST_STACK_REG][(int) MAX_MACHINE_MODE];
...
reg_to_stack (rtx first, FILE * file)
{
  ...
  FOR_EACH_BB_REVERSE (bb)
  {
    ...
    for (e = bb->pred; e; e = e->pred_next)
      if (!(e->flags & EDGE_DFS_BACK) && e->src != ENTRY_BLOCK_PTR)
  ...
  if (code == LABEL_REF)
	...
	return;
      ...
      for (ref = LABEL_REFS (label);
	   ref && ref != label; ref = LABEL_NEXTREF (ref))
...
emit_swap_insn (rtx insn, stack regstack, rtx reg)
{
  ...
  if (current_block && insn != BB_HEAD (current_block))
      ...
      while (tmp != limit)
	{
	  if (GET_CODE (tmp) == CODE_LABEL
	      ...
	      || (GET_CODE (tmp) == INSN && stack_regs_mentioned (tmp)))
	    {
	      i1 = tmp;
	      break;
	    }
  ...
  if (i1 != NULL_RTX && (i1set = single_set (i1)) != NULL_RTX)
    {
...
compare_for_stack_reg (rtx insn, stack regstack, rtx pat_src)
{
  ...
  if (src2_note
      ...
      else
	{
	  ...
	  emit_pop_insn (insn, regstack, XEXP (src2_note, 0), EMIT_AFTER);
	}
    }
}

		...
		if ((REGNO (*src1) == regstack->reg[regstack->top]
			...
			&& src2_note))
		  {
		    ...
		    PUT_CODE (XEXP (pat_src, 0),
			      reversed_comparison_code (XEXP (pat_src, 0),
							insn));
		  }
		else
		  emit_swap_insn (insn, regstack, *dest);
	...
	if (TEST_HARD_REG_BIT (bi->stack_in.reg_set, reg))
	  {
	    ...
	    init = gen_rtx_SET (VOIDmode,
				FP_MODE_REG (FIRST_STACK_REG, SFmode), nan);
...
convert_regs_1 (FILE * file, basic_block block)
{
  ...
  for (e = block->pred; e; e = e->pred_next)
	...
	;
      else if ((EDGE_CRITICAL_P (e) != 0) != (EDGE_CRITICAL_P (beste) != 0))
	{
  ...
  do
      ...
      if (stack_regs_mentioned (insn) || GET_CODE (insn) == CALL_INSN)
	{
	  if (file)
	    {
	      fprintf (file, "  insn %d input stack: ", INSN_UID (insn));
  ...
  for (reg = FIRST_STACK_REG; reg <= LAST_STACK_REG; ++reg)
	  ...
	  && !TEST_HARD_REG_BIT (regstack.reg_set, reg))
	{
	  ...
	  set = gen_rtx_SET (VOIDmode, FP_MODE_REG (reg, SFmode), nan);
...
win:
  ...
  for (e = block->succ; e; e = e->succ_next)
    {
      if (e->flags & EDGE_DFS_BACK || (e->dest == EXIT_BLOCK_PTR))
	{
	  if (!BLOCK_INFO (e->dest)->done && e->dest != block)
	    abort ();
