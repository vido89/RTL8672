  ...
  for (i = 0; i < num_webs; i++)
    {
      ...
      if (web->type != INITIAL && web->type != FREE
	  && web->type != PRECOLORED)
	abort ();
    }
		...
		HARD_REG_SET free_colors, enum machine_mode mode)
{
  if (!TEST_HARD_REG_BIT (dont_begin_colors, c)
      && TEST_HARD_REG_BIT (free_colors, c) && HARD_REGNO_MODE_OK (c, mode))
    {
      int i, size;
...
try_recolor_web (struct web *web)
{
  ...
  for (c = 0; c < FIRST_PSEUDO_REGISTER; c++)
	...
	&& (newcol == -1 || cost_neighbors[c] < cost_neighbors[newcol]))
      newcol = c;
  ...
  for (i = 0; i < num_webs - num_subwebs; i++)
	...
	if (wl->t->regno >= max_normal_pseudo)
	...
	else if (!wl->sub)
	  {
	    ...
	    if (web2->type == COLORED)
	      nregs2 =
		HARD_REGNO_NREGS (web2->color, GET_MODE (web2->orig_x));
	...
	else
	  {
		...
		int sofs = 0, tofs = 0;
		if (SUBWEB_P (sl->t)
		    && GET_MODE_SIZE (GET_MODE (sl->t->orig_x)) >=
		    UNITS_PER_WORD)
...
aggressive_coalesce (void)
{
  ...
  while ((d = pop_list (&mv_worklist)) != NULL)
	...
	if (s != t
	  ...
	  {
	    if ((s->type == PRECOLORED && ok (t, s)) || s->type != PRECOLORED)
	      {
		put_move (m, MV_COALESCED);
	  ...
	  if (dest->type != PRECOLORED && dest->regno < max_normal_pseudo)
	      ...
	      for (n2 = 0; n2 < info.num_uses; n2++)
		{
		  ...
		  if (source->type != PRECOLORED
		      && source != dest && source->regno < max_normal_pseudo
