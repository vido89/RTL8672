...
ra_print_rtx_object (FILE * file, rtx x)
{
  ...
  switch (code)
	...
	if (GET_CODE (sub) == REG && REGNO (sub) < FIRST_PSEUDO_REGISTER)
	  {
	    int regno = REGNO (sub);
	    int i, nregs = HARD_REGNO_NREGS (regno, mode);
	    regno += subreg_regno_offset (regno, GET_MODE (sub), ofs, mode);
	    if (nregs > 1)
	      fputs ("[", file);
	    for (i = 0; i < nregs; i++)
	      {
	...
	if (def1 != def2 && TEST_BIT (igraph, igraph_index (def1, def2)))
	  {
	    ...
	    if (SUBWEB_P (ID2WEB (def2)))
	      ra_debug_msg (DUMP_IGRAPH, "%d(%d,%d) ", def2,
			    ID2WEB (def2)->regno,
			    SUBREG_BYTE (ID2WEB (def2)->orig_x));
	    else
	      ra_debug_msg (DUMP_IGRAPH, "%d(%d) ", def2,
			    ID2WEB (def2)->regno);
	  }
      if (num1 != num)
	ra_debug_msg (DUMP_IGRAPH, "\n  ");
  ...
  for (i = 0; i < num_webs - num_subwebs; i++)
	...
	if (cl->t->id < web->id)
      ...
      ra_debug_msg (DUMP_IGRAPH_M, "n %d %d %d %d %d %d %d\n",
		    ...
		    (unsigned int) web->spill_cost, web->num_defs,
		    web->num_uses, numc);
      if (web->type != PRECOLORED)
	{
