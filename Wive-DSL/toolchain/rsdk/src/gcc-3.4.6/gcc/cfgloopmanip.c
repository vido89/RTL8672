  ...
  while (qbeg != qend)
      ...
      for (e = from->pred; e; e = e->pred_next)
	{
	  ...
	  if (pred->loop_father != base_loop
	      && (nca == base_loop || nca != pred->loop_father))
  ...
  for (i = 0; i < n_bord_bbs; i++)
	...
	continue;
      ...
      for (ldom = first_dom_son (CDI_DOMINATORS, bb);
	   ldom; ldom = next_dom_son (CDI_DOMINATORS, ldom))
...
static void
duplicate_subloops (struct loops *loops, struct loop *loop,
		    struct loop *target)
{
  ...
  for (i = 0; i < n; i++)
      ...
      for (j = 0; j < n_dom_bbs; j++)
	{
	  ...
	  if (flow_bb_inside_loop_p (loop, dominated))
	    continue;
	  dom_bb =
	    nearest_common_dominator (CDI_DOMINATORS, first_active[i],
				      first_active_latch);
  ...
  if (nentry == 1)
    {
      ...
      if (!(flags & CP_SIMPLE_PREHEADERS) || !e->src->succ->succ_next)
	return NULL;
    }
...
force_single_succ_latches (struct loops *loops)
{
  ...
  for (i = 1; i < loops->num; i++)
    {
      loop = loops->parray[i];
      if (loop->latch != loop->header && !loop->latch->succ->succ_next)
	continue;
