...
delete_from_delay_slot (rtx insn)
{
  ...
  for (trial = insn;
       PREV_INSN (NEXT_INSN (trial)) == trial; trial = NEXT_INSN (trial))
    ;
  ...
  if (delay_list)
    trial = emit_delay_sequence (trial, delay_list, XVECLEN (seq, 0) - 2);
	   ...
	   || GET_CODE (trial) == CALL_INSN || GET_CODE (trial) == INSN)
...
static int num_filled_delays[NUM_REORG_FUNCTIONS][MAX_DELAY_HISTOGRAM +
						  1][MAX_REORG_PASSES];
...
get_jump_flags (rtx insn, rtx label)
{
  ...
  if (GET_CODE (insn) == JUMP_INSN
      ...
      && label != 0 && INSN_UID (label) <= max_uid)
    flags
  ...
  if (GET_CODE (insn) == JUMP_INSN
      && (condjump_p (insn) || condjump_in_parallel_p (insn)))
    {
      ...
      prediction =
	mostly_true_jump (insn, get_branch_condition (insn, label));
      switch (prediction)
	{
	case 2:
  ...
  for (; insn; insn = next)
	...
	case JUMP_INSN:
	  if (GET_CODE (PATTERN (insn)) == RETURN)
	    return 1;
	  else if (simplejump_p (insn) && jump_count++ < 10)
...
mostly_true_jump (rtx jump_insn, rtx condition)
{
  ...
  if (GET_CODE (PATTERN (jump_insn)) == SET
	  ...
	  || (GET_CODE (XEXP (SET_SRC (PATTERN (jump_insn)), 2)) == LABEL_REF
	      &&
	      LABEL_OUTSIDE_LOOP_P (XEXP
				    (SET_SRC (PATTERN (jump_insn)), 2)))))
    return -1;
...
fill_simple_delay_slots (int non_jumps_p)
{
  ...
  for (i = 0; i < num_unfilled_slots; i++)
	...
	flags = get_jump_flags (insn, NULL_RTX);
	  ...
	  && no_labels_between_p (insn, trial) && !can_throw_internal (trial))
	{
	  rtx *tmp;
	  slots_filled++;
	  delay_list = add_to_delay_list (trial, delay_list);
	  ...
	  && (condjump_p (insn) || condjump_in_parallel_p (insn)))
	{
	  ...
	  && (GET_CODE (insn) != JUMP_INSN
	      || ((condjump_p (insn) || condjump_in_parallel_p (insn))
		  && !simplejump_p (insn) && JUMP_LABEL (insn) != 0)))
	{
	  ...
	  if (slots_to_fill != slots_filled
	      ...
	      && (next_trial =
		  try_split (PATTERN (next_trial), next_trial, 0))
      ...
      /* If this is an unconditional jump, then try to get insns from the
         target of the jump.  */
      if (GET_CODE (insn) == JUMP_INSN
	  && simplejump_p (insn) && slots_filled != slots_to_fill)
	delay_list
...
#endif
      if (!EXIT_IGNORE_STACK || current_function_sp_is_unchanging)
...
dbr_schedule (rtx first, FILE * file)
{
  ...
  for (reorg_pass_number = 0;
       reorg_pass_number < MAX_REORG_PASSES; reorg_pass_number++)
    {
      fill_simple_delay_slots (1);
      fill_simple_delay_slots (0);
