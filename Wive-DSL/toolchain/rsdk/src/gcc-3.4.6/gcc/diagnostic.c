...
/* Prototypes.  */
static char *
build_message_string (const char *, ...)
  ATTRIBUTE_PRINTF_1;
...
/* Return a malloc'd string containing MSG formatted a la printf.  The
   caller is responsible for freeing the memory.  */
     static char *build_message_string (const char *msg, ...)
{
  char *str;
  va_list ap;
...
diagnostic_set_info (diagnostic_info * diagnostic, const char *msgid,
		     va_list * args, location_t location, diagnostic_t kind)
{
...
trim_filename (const char *name)
{
  ...
  while (p[0] == '.' && p[1] == '.' && (p[2] == DIR_SEPARATOR
  ...
  while (q[0] == '.' && q[1] == '.' && (q[2] == DIR_SEPARATOR
#ifdef DIR_SEPARATOR_2
					|| p[2] == DIR_SEPARATOR_2
#endif
	 ))
