...
identify_call_return_value (rtx cp, rtx * p_hard_return, rtx * p_soft_return)
{
  ...
  /* Allow this first destination to be copied to a second register,
     ...
     we'd been expecting.  */
  if (insn && (set = single_set (insn)) != NULL_RTX && SET_SRC (set) == soft)
    {
      soft = SET_DEST (set);
      insn = NEXT_INSN (insn);
    }
...
skip_copy_to_return_value (rtx orig_insn)
{
  ...
  if (return_value_pseudo)
    {
      if (SET_DEST (set) == return_value_pseudo && SET_SRC (set) == softret)
	return insn;
      return orig_insn;
    }
...
#define OUTGOING_REGNO(N) (N)
  ...
  if (REG_P (SET_DEST (set)) && SET_SRC (set) == softret)
    {
      rtx x = SET_DEST (set);
...
skip_jump_insn (rtx orig_insn)
{
  ...
  if (insn && GET_CODE (insn) == JUMP_INSN && any_uncondjump_p (insn))
    return insn;
...
optimize_sibling_and_tail_recursive_calls (void)
{
  ...
  for (e = EXIT_BLOCK_PTR->pred;
       e && alternate_exit == EXIT_BLOCK_PTR; e = e->pred_next)
    {
      rtx insn;
