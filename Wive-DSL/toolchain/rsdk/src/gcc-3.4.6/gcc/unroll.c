	...
	if (local_regno[r])
	  {
	    ...
	    record_base_value (REGNO (map->reg_map[r]), regno_reg_rtx[r], 0);
	  }
  ...
  if (GET_CODE (increment) != CONST_INT)
	       ...
	       || GET_CODE (increment) == LSHIFTRT)
	{
	  ...
	  if (code == IOR)
	    ...
	    increment =
	      GEN_INT ((unsigned HOST_WIDE_INT) INTVAL (increment) >>
		       INTVAL (second_part));
	}
  ...
  do
    {
      insn = NEXT_INSN (insn);
      if ((GET_CODE (insn) == INSN || GET_CODE (insn) == JUMP_INSN
	   || GET_CODE (insn) == CALL_INSN) && map->insn_map[INSN_UID (insn)])
	final_reg_note_copy (&REG_NOTES (map->insn_map[INSN_UID (insn)]),
			     map);
    }
  while (insn != copy_end);
  ...
  for (v = bl->biv; v; v = v->next_iv)
    {
	  ...
	  && !v->maybe_multiple && SCALAR_INT_MODE_P (v->mode))
	{
	  ...
	  result =
	    fold_rtx_mult_add (result, const1_rtx, v->add_val, v->mode);
	}
      else
	return 0;
    }
...
final_biv_value (const struct loop * loop, struct iv_class * bl)
{
  ...
  if (n_iterations != 0
      && !loop->exit_count && loop_invariant_p (loop, bl->initial_value))
    {
      increment = biv_total_increment (bl);
...
final_giv_value (const struct loop * loop, struct induction * v)
{
  ...
  if (n_iterations != 0 && !loop->exit_count && v->always_executed)
    {
...
loop_iterations (struct loop *loop)
{
  ...
  abs_diff &= ((unsigned HOST_WIDE_INT) 1
	       << (GET_MODE_BITSIZE (GET_MODE (iteration_var)) - 1) << 1) - 1;
