...
{
  return MIN (BIGGEST_ALIGNMENT,
	      MAX (1, mode_base_align[mode] * BITS_PER_UNIT));
}
...
layout_decl (tree decl, unsigned int known_align)
{
  ...
  if (warn_larger_than
      && (code == VAR_DECL || code == PARM_DECL) && !DECL_EXTERNAL (decl))
    {
...
start_record_layout (tree t)
{
  ...
  if (!TYPE_PACKED (t))
    rli->record_align =
      MAX (rli->record_align, (unsigned) STRUCTURE_SIZE_BOUNDARY);
#endif
...
place_field (record_layout_info rli, tree field)
{
  ...
  if ((*targetm.ms_bitfield_layout_p) (rli->t)
	      ...
	      : !integer_zerop (DECL_SIZE (field))))
	{
	  ...
	  if (prev_saved && DECL_BIT_FIELD_TYPE (prev_saved)
...
compute_record_mode (tree type)
{
  ...
  for (field = TYPE_FIELDS (type); field; field = TREE_CHAIN (field))
	...
	continue;
	  ...
	  || DECL_SIZE (field) == 0 || !host_integerp (DECL_SIZE (field), 1))
	return;
  ...
  if (TYPE_NEXT_VARIANT (type) || type != TYPE_MAIN_VARIANT (type))
    {
      ...
      for (variant = TYPE_MAIN_VARIANT (type);
	   variant != 0; variant = TYPE_NEXT_VARIANT (variant))
	{
  ...
  switch (TREE_CODE (type))
	...
	= mode_for_size (2 * TYPE_PRECISION (TREE_TYPE (type)),
			 (TREE_CODE (TREE_TYPE (type)) == INTEGER_TYPE
			  ? MODE_COMPLEX_INT : MODE_COMPLEX_FLOAT), 0);
      TYPE_SIZE (type) = bitsize_int (GET_MODE_BITSIZE (TYPE_MODE (type)));
      TYPE_SIZE_UNIT (type) = size_int (GET_MODE_SIZE (TYPE_MODE (type)));
      break;
...
set_min_and_max_values_for_integral_type (tree type,
					  int precision, bool is_unsigned)
{
  tree min_value;
  tree max_value;
