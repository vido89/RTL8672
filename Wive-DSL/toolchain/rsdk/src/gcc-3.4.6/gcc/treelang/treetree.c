			  ...
			  tree op3 ATTRIBUTE_UNUSED)
{
  ...
  switch (exp_type)
	...
	abort ();
      ...
      ret1 = build (operator, type, op1, build1 (CONVERT_EXPR, type, op2));
...
tree_code_get_numeric_type (unsigned int size1, unsigned int sign1)
{
  ...
  else if (size1 == tree_code_char_size)
    {
      if (sign1)
...
static
GTY (())
     tree signed_and_unsigned_types[MAX_BITS_PER_WORD + 1][2];
...
treelang_init_decl_processing (void)
{
  ...
  tree_push_atomic_type_decl (get_identifier ("long double"),
