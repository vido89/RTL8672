...
is_cfg_nonregular (void)
{
  ...
  FOR_EACH_BB (b) for (insn = BB_HEAD (b);; insn = NEXT_INSN (insn))
    {
      code = GET_CODE (insn);
...
build_control_flow (struct edge_list *edge_list)
{
  ...
  FOR_EACH_BB (b)
  {
    if (b->pred == NULL || (b->pred->src == b && b->pred->pred_next == NULL))
      unreachable = 1;
  }
  ...
  for (i = 0; i < num_edges; i++)
    {
      ...
      if (e->dest != EXIT_BLOCK_PTR && e->src != ENTRY_BLOCK_PTR)
...
new_edge (int source, int target)
{
  ...
  while (curr_edge)
    {
      if (FROM_BLOCK (curr_edge) == source && TO_BLOCK (curr_edge) == target)
	{
	  return;
	}
...
find_rgns (struct edge_list *edge_list)
{
  ...
  unreachable = 0;
  FOR_EACH_BB (bb) if (dfs_nr[bb->index] == 0)
    {
      unreachable = 1;
      break;
    }
  ...
  FOR_EACH_BB (bb) degree[bb->index] = 0;
  for (i = 0; i < num_edges; i++)
    {
      edge e = INDEX_EDGE (edge_list, i);
	...
	if (TEST_BIT (header, bb->index) && TEST_BIT (inner, bb->index))
	  {
	    ...
	    num_bbs = 1;
	    num_insns = (INSN_LUID (BB_END (bb)) - INSN_LUID (BB_HEAD (bb)));
  ...
  /* Any block that did not end up in a region is placed into a region
     by itself.  */
  FOR_EACH_BB (bb) if (degree[bb->index] >= 0)
    {
      rgn_bb_table[idx] = bb->index;
      RGN_NR_BLOCKS (nr_regions) = 1;
      RGN_BLOCKS (nr_regions) = idx++;
  ...
  do
    {
      ...
      sbitmap_a_or_b (ancestor_edges[bb], ancestor_edges[bb],
		      ancestor_edges[BLOCK_TO_BB (pred)]);
      ...
      sbitmap_a_or_b (pot_split[bb], pot_split[bb],
		      pot_split[BLOCK_TO_BB (pred)]);
	  ...
	  for (i = 0; i < candidate_table[src].split_bbs.nr_members; i++)
	    {
	      ...
	      if (REGNO_REG_SET_P
		  (BASIC_BLOCK (b)->global_live_at_start, regno))
		{
		  return 0;
		}
	    }
  ...
  for (link = LOG_LINKS (load_insn); link; link = XEXP (link, 1))
    {
      ...
      if (GET_MODE (link) != VOIDmode || GET_CODE (insn1) == JUMP_INSN)
	continue;
...
init_ready_list (struct ready_list *ready)
{
  ...
  for (insn = NEXT_INSN (prev_head); insn != next_tail;
       insn = NEXT_INSN (insn))
    {
...
propagate_deps (int bb, struct deps *pred_deps)
{
  ...
  if (e > 0)
	...
	if (CONTAINING_RGN (b) != CONTAINING_RGN (b_succ) || bb_succ <= bb)
	  {
	    e = NEXT_OUT (e);
	    continue;
	  }
	...
	EXECUTE_IF_SET_IN_REG_SET (&pred_deps->reg_last_in_use, 0, reg,
				   {
				   struct deps_reg *pred_rl =
				   &pred_deps->reg_last[reg];
				   struct deps_reg *succ_rl =
				   &succ_deps->reg_last[reg];
				   succ_rl->uses =
				   concat_INSN_LIST (pred_rl->uses,
						     succ_rl->uses);
				   succ_rl->sets =
				   concat_INSN_LIST (pred_rl->sets,
						     succ_rl->sets);
				   succ_rl->clobbers =
				   concat_INSN_LIST (pred_rl->clobbers,
						     succ_rl->clobbers);
				   succ_rl->uses_length +=
				   pred_rl->uses_length;
				   succ_rl->clobbers_length +=
				   pred_rl->clobbers_length;});
	IOR_REG_SET (&succ_deps->reg_last_in_use,
	  ...
	  for (insn = head; insn != next_tail; insn = NEXT_INSN (insn))
		  ...
		  && (*targetm.sched.use_dfa_pipeline_interface) ())
		{
		  fprintf (sched_dump,
			   ...
			   INSN_BB (insn),
			   INSN_DEP_COUNT (insn),
			   INSN_PRIORITY (insn), insn_cost (insn, 0, 0));
...
init_regions (void)
{
  ...
  if (reload_completed || n_basic_blocks == 1 || !flag_schedule_interblock)
    {
...
schedule_insns (FILE * dump_file)
{
  ...
  FOR_EACH_BB (bb) SET_BIT (large_region_blocks, bb->index);
