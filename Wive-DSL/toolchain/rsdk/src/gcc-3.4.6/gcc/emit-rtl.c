...
#define first_label_num (cfun->emit->x_first_label_num)
     ...
     static mem_attrs *get_mem_attrs (HOST_WIDE_INT, tree, rtx, rtx,
...
/* Returns a hash code for X (which is a really a CONST_INT).  */
     ...
     static hashval_t const_int_htab_hash (const void *x)
{
...
mem_attrs_htab_hash (const void *x)
{
  ...
  return (p->alias ^ (p->align * 1000)
	  ...
	  ^ ((p->size ? INTVAL (p->size) : 0) * 2500000) ^ (size_t) p->expr);
}
...
rtx
immed_double_const (HOST_WIDE_INT i0, HOST_WIDE_INT i1,
		    enum machine_mode mode)
{
  ...
  if (mode != VOIDmode)
	...
	i0 &= ((HOST_WIDE_INT) 1 << width) - 1, i1 = 0;
      else if (width == HOST_BITS_PER_WIDE_INT && !(i1 == ~0 && i0 < 0))
	i1 = 0;
...
gen_rtx_REG (enum machine_mode mode, unsigned int regno)
{
  ...
  if (cfun
      && cfun->emit
      && regno_reg_rtx
      && regno < FIRST_PSEUDO_REGISTER && reg_raw_mode[regno] == mode)
...
gen_lowpart_SUBREG (enum machine_mode mode, rtx reg)
{
  ...
  if (inmode == VOIDmode)
    inmode = mode;
  return gen_rtx_SUBREG (mode, reg, subreg_lowpart_offset (mode, inmode));
}

		    ...
		    int offset)
{
  ...
  REG_ATTRS (new) = get_reg_attrs (REG_EXPR (reg), REG_OFFSET (reg) + offset);
  return new;
}
  ...
  if (GET_CODE (parm_rtx) == REG)
      ...
      for (; i < XVECLEN (parm_rtx, 0); i++)
	{
	  ...
	  if (GET_CODE (XEXP (x, 0)) == REG)
	    REG_ATTRS (XEXP (x, 0))
	      = get_reg_attrs (MEM_EXPR (mem), INTVAL (XEXP (x, 1)));
	}
    }
}
...
set_decl_rtl (tree t, rtx x)
{
  ...
  if (GET_CODE (x) == REG)
    ...
    REG_ATTRS (SUBREG_REG (x)) = get_reg_attrs (t, -SUBREG_BYTE (x));
...
subreg_hard_regno (rtx x, int check_mode)
{
  ...
  if (GET_CODE (x) != SUBREG || GET_CODE (reg) != REG)
    abort ();
...
gen_complex_constant_part (enum machine_mode mode, rtx x, int imagpart_p)
{
  ...
  if (GET_CODE (x) == MEM && GET_CODE (XEXP (x, 0)) == SYMBOL_REF)
      ...
      if (decl != NULL_TREE && TREE_CODE (decl) == COMPLEX_CST)
	{
	  ...
	  if (TREE_CODE (part) == REAL_CST || TREE_CODE (part) == INTEGER_CST)
...
rtx
gen_highpart_mode (enum machine_mode outermode, enum machine_mode innermode,
		   rtx exp)
{
...
unsigned int
subreg_lowpart_offset (enum machine_mode outermode,
		       enum machine_mode innermode)
{
...
/* Return offset in bytes to get OUTERMODE high part
   of the value in mode INNERMODE stored in memory in target format.  */
unsigned int
subreg_highpart_offset (enum machine_mode outermode,
			enum machine_mode innermode)
{
  unsigned int offset = 0;
  int difference = (GET_MODE_SIZE (innermode) - GET_MODE_SIZE (outermode));
...
rtx
operand_subword (rtx op, unsigned int offset, int validate_address,
		 enum machine_mode mode)
{
  if (mode == VOIDmode)
    mode = GET_MODE (op);
  ...
  if (mode != BLKmode && (offset + 1) * UNITS_PER_WORD > GET_MODE_SIZE (mode))
    return const0_rtx;
...
mem_expr_equal_p (tree expr1, tree expr2)
{
  ...
  if (TREE_CODE (expr1) == COMPONENT_REF)
    return mem_expr_equal_p (TREE_OPERAND (expr1, 0), TREE_OPERAND (expr2, 0)) && mem_expr_equal_p (TREE_OPERAND (expr1, 1),	/* field decl */
												    TREE_OPERAND
												    (expr2,
												     1));
  ...
  if (!TYPE_P (t))
      ...
      else if (TREE_CODE (t) == ARRAY_REF)
	{
	  ...
	  do
		...
		index =
		  build (WITH_RECORD_EXPR, TREE_TYPE (index), index, t2);
  ...
  if (new == memref)
    {
	  ...
	  || (MEM_EXPR (memref) == NULL
	      ...
	      && MEM_SIZE (memref) == size && MEM_ALIGN (memref) == align))
	return new;
...
offset_address (rtx memref, rtx offset, unsigned HOST_WIDE_INT pow2)
{
  ...
  if (!memory_address_p (GET_MODE (memref), new)
      && GET_CODE (addr) == PLUS && XEXP (addr, 0) == pic_offset_table_rtx)
    {
...
get_first_nonnote_insn (void)
{
  ...
  if (insn)
	...
	for (insn = next_insn (insn);
	     insn && NOTE_P (insn); insn = next_insn (insn))
	  continue;
      else
	{
...
get_last_nonnote_insn (void)
{
  ...
  if (insn)
	...
	for (insn = previous_insn (insn);
	     insn && NOTE_P (insn); insn = previous_insn (insn))
	  continue;
      else
	{
	  if (GET_CODE (insn) == INSN
...
last_call_insn (void)
{
  ...
  for (insn = get_last_insn ();
       insn && GET_CODE (insn) != CALL_INSN; insn = PREV_INSN (insn))
    ;
...
try_split (rtx pat, rtx trial, int last)
{
  ...
  /* Return either the first or the last insn, depending on which was
     requested.  */
  return last ? (after ? PREV_INSN (after) : last_insn) : NEXT_INSN (before);
}

...
make_insn_raw (rtx pattern)
{
  ...
  if (insn
      && INSN_P (insn)
      && (returnjump_p (insn)
	  || (GET_CODE (insn) == SET && SET_DEST (insn) == pc_rtx)))
    {
...
add_insn_after (rtx insn, rtx after)
{
  ...
  if (GET_CODE (after) != BARRIER
      && GET_CODE (insn) != BARRIER && (bb = BLOCK_FOR_INSN (after)))
    {
...
add_insn_before (rtx insn, rtx before)
{
  ...
  if (GET_CODE (before) != BARRIER
      && GET_CODE (insn) != BARRIER && (bb = BLOCK_FOR_INSN (before)))
    {
      set_block_for_insn (insn, bb);
      if (INSN_P (insn))
	bb->flags |= BB_DIRTY;
...
remove_insn (rtx insn)
{
  ...
  if (GET_CODE (insn) != BARRIER && (bb = BLOCK_FOR_INSN (insn)))
    {
      if (INSN_P (insn))
	bb->flags |= BB_DIRTY;
      if (BB_HEAD (bb) == insn)
...
reorder_insns (rtx from, rtx to, rtx after)
{
  ...
  if (GET_CODE (after) != BARRIER && (bb = BLOCK_FOR_INSN (after)))
    {
      rtx x;
      bb->flags |= BB_DIRTY;

      if (GET_CODE (from) != BARRIER && (bb2 = BLOCK_FOR_INSN (from)))
	{
	  if (BB_END (bb2) == to)
	    BB_END (bb2) = prev;
	  bb2->flags |= BB_DIRTY;
...
{
  if (INSN_P (before))
    return emit_jump_insn_before_setloc (pattern, before,
					 INSN_LOCATOR (before));
  else
...
{
  if (INSN_P (before))
    return emit_call_insn_before_setloc (pattern, before,
					 INSN_LOCATOR (before));
  else
...
init_emit (void)
{
  ...
  memcpy (regno_reg_rtx,
	  static_regno_reg_rtx, FIRST_PSEUDO_REGISTER * sizeof (rtx));
...
init_emit_once (int line_numbers)
{
  ...
  virtual_stack_vars_rtx = gen_raw_REG (Pmode, VIRTUAL_STACK_VARS_REGNUM);
  ...
  for (i = 0; i < (int) ARRAY_SIZE (const_tiny_rtx); i++)
    {
      REAL_VALUE_TYPE *r = (i == 0 ? &dconst0 : i == 1 ? &dconst1 : &dconst2);
