...
{
  return c_build_qualified_type (type, TYPE_QUALS (type) | TYPE_QUALS (like));
}

...
common_type (tree t1, tree t2)
{
  ...
  switch (code1)
	...
	{
	  t1 = build_qualified_type (long_double_type_node, TYPE_QUALS (t1));
	  return build_type_attribute_variant (t1, attributes);
	}
...
comptypes (tree type1, tree type2, int flags)
{
  ...
  switch (TREE_CODE (t1))
	...
	if (TREE_TYPE (t1) != TREE_TYPE (t2)
	    && 0 == (val = comptypes (TREE_TYPE (t1), TREE_TYPE (t2), flags)))
	  return 0;
...
tagged_types_tu_compatible_p (tree t1, tree t2, int flags)
{
  ...
  switch (TREE_CODE (t1))
		...
		if (DECL_NAME (s1) == DECL_NAME (s2))
		  {
		    int result;
		    result =
		      comptypes (TREE_TYPE (s1), TREE_TYPE (s2), flags);
		    if (result == 0)
		      break;
		    if (result == 2)
		      needs_warning = true;
	...
	for (s1 = TYPE_FIELDS (t1), s2 = TYPE_FIELDS (t2);
	     s1 && s2; s1 = TREE_CHAIN (s1), s2 = TREE_CHAIN (s2))
	  {
	    int result;
	  ...
	  if (TREE_CODE (TREE_VALUE (args1)) == UNION_TYPE
				     ...
				     TYPE_SIZE (TREE_VALUE (args2))))
	    {
	      ...
	      for (memb = TYPE_FIELDS (TREE_VALUE (args1));
		   memb; memb = TREE_CHAIN (memb))
		if (comptypes (TREE_TYPE (memb), TREE_VALUE (args2), flags))
		  break;
	      if (memb == 0)
		return 0;
	    }
					  ...
					  TYPE_SIZE (TREE_VALUE (args1))))
	    {
	      ...
	      for (memb = TYPE_FIELDS (TREE_VALUE (args2));
		   memb; memb = TREE_CHAIN (memb))
		if (comptypes (TREE_TYPE (memb), TREE_VALUE (args1), flags))
		  break;
	      if (memb == 0)
		return 0;
	    }
...
convert_arguments (tree typelist, tree values, tree name, tree fundecl)
{
  ...
  for (valtail = values, typetail = typelist, parmnum = 0;
      ...
      if (type != 0)
	{
	  ...
	  if (!COMPLETE_TYPE_P (type))
	    {
	      error ("type of formal parameter %d is incomplete",
		     parmnum + 1);
	      parmval = val;
	    }
	  else
	      ...
	      if (warn_conversion || warn_traditional)
		{
		  ...
		  if (INTEGRAL_TYPE_P (type)
		    ...
		    warn_for_assignment
		      ("%s as integer rather than complex due to prototype",
		       (char *) 0, name, parmnum + 1);
		  ...
		  /* Detect integer changing in width or signedness.
			...
			;
		      /* Likewise for a constant in a NOP_EXPR.  */
			       ...
			       INTEGER_CST
			       && int_fits_type_p (TREE_OPERAND (val, 0),
						   type))
			;
		      ...
		      else if (TYPE_PRECISION (TREE_TYPE (val)) <
			       TYPE_PRECISION (type)
			       && TREE_UNSIGNED (TREE_TYPE (val)))
			;
		      else if (TREE_UNSIGNED (type))
			warn_for_assignment
			  ("%s as unsigned due to prototype", (char *) 0,
			   name, parmnum + 1);
		      else
			warn_for_assignment ("%s as signed due to prototype",
					     (char *) 0, name, parmnum + 1);
		    }
		}
	      ...
	      if (targetm.calls.
		  promote_prototypes (fundecl ? TREE_TYPE (fundecl) : 0)
	...
	result =
	  tree_cons (NULL_TREE, convert (double_type_node, val), result);
      else
...
build_conditional_expr (tree ifexp, tree op1, tree op2)
{
  ...
  if (TYPE_MAIN_VARIANT (type1) == TYPE_MAIN_VARIANT (type2))
      ...
      else if (VOID_TYPE_P (TREE_TYPE (type1)))
	{
	  ...
	  result_type =
	    build_pointer_type (qualify_type
      ...
      else if (VOID_TYPE_P (TREE_TYPE (type2)))
	{
	  ...
	  result_type =
	    build_pointer_type (qualify_type
				(TREE_TYPE (type2), TREE_TYPE (type1)));
	}
      else
	{
...
build_modify_expr (tree lhs, enum tree_code modifycode, tree rhs)
{
  ...
  switch (TREE_CODE (lhs))
	...
	tree cond = build_conditional_expr (TREE_OPERAND (lhs, 0),
					    build_modify_expr (TREE_OPERAND
							       (lhs, 1),
							       modifycode,
							       rhs),
					    build_modify_expr (TREE_OPERAND
  ...
  /* Conversion to a transparent union from its member types.
      ...
      if (memb_types || marginal_memb_type)
	{
	  if (!memb_types)
		...
		warn_for_assignment
		  ("%s discards qualifiers from pointer target type", errtype,
		   funname, parmnum);
	    }
  ...
  /* Conversions among pointers */
	   ...
	   && (coder == codel))
    {
      ...
      is_opaque_pointer = ((*targetm.vector_opaque_p) (type)
			   || (*targetm.vector_opaque_p) (rhstype))
	&& TREE_CODE (ttl) == VECTOR_TYPE && TREE_CODE (ttr) == VECTOR_TYPE;
	      ...
	      == c_common_unsigned_type (TYPE_MAIN_VARIANT (ttr))))
	{
	  ...
	  /* Const and volatile mean something different for function types,
		...
		warn_for_assignment
		  ("pointer targets in %s differ in signedness", errtype,
		   funname, parmnum);
	    }
	  ...
	  if (function)
	    {
	      ...
	      sprintf (new_opname, argstring, IDENTIFIER_POINTER (function));
	    }
	  else
	    {
	      /* Function name unknown (call through ptr).  */
	      const char *const argnofun =
...
store_init_value (tree decl, tree init)
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE
      && TYPE_DOMAIN (type) == 0 && value != error_mark_node)
    {
...
digest_init (tree type, tree init, int require_constant)
{
  ...
  if (type == error_mark_node
      || init == error_mark_node || TREE_TYPE (init) == error_mark_node)
    return error_mark_node;
...
output_init_element (tree value, tree type, tree field, int pending)
{
  ...
  else if (TREE_CODE (constructor_type) == UNION_TYPE && constructor_elements)
    {
  ...
  constructor_elements = tree_cons (field, value, constructor_elements);

  /* Advance the variable that indicates sequential elements output.  */
  if (TREE_CODE (constructor_type) == ARRAY_TYPE)
    constructor_unfilled_index
      = size_binop (PLUS_EXPR, constructor_unfilled_index, bitsize_one_node);
  else if (TREE_CODE (constructor_type) == RECORD_TYPE)
    {
  ...
  if (constructor_range_stack)
    {
      ...
      if (TREE_CODE (value) != COMPOUND_LITERAL_EXPR
	  || !require_constant_value || flag_isoc99)
  ...
  if (exp != error_mark_node)
    {
      ...
      if (!INTEGRAL_TYPE_P (orig_type) && code != ERROR_MARK)
	{
...
do_case (tree low_value, tree high_value)
{
  ...
  if (switch_stack)
    {
      bool switch_was_empty_p =
	(SWITCH_BODY (switch_stack->switch_stmt) == NULL_TREE);
      ...
      else if (switch_was_empty_p)
	{
	  ...
	  SWITCH_BODY (switch_stack->switch_stmt) =
	    TREE_CHAIN (switch_stack->switch_stmt);
