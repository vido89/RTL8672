...
calls_function_1 (tree exp, int which)
{
  ...
  switch (code)
	...
	for (subblock = BLOCK_SUBBLOCKS (exp);
	     subblock; subblock = TREE_CHAIN (subblock))
...
static void
emit_call_1 (rtx funexp, tree fndecl ATTRIBUTE_UNUSED,
	     ...
	     CUMULATIVE_ARGS * args_so_far ATTRIBUTE_UNUSED)
{
  ...
  if ((ecf_flags & ECF_SIBCALL) && HAVE_sibcall && HAVE_sibcall_value)
    {
      if (valreg)
...
special_function_p (tree fndecl, int flags)
{
  if (!(flags & ECF_MALLOC)
      ...
      && TREE_PUBLIC (fndecl))
    {
      ...
      if (((IDENTIFIER_LENGTH (DECL_NAME (fndecl)) == 6
	   ...
	   || (IDENTIFIER_LENGTH (DECL_NAME (fndecl)) == 16
	       && name[0] == '_' && !strcmp (name, "__builtin_alloca"))))
	...
	flags |= ECF_LONGJMP;
      ...
      else if ((tname[0] == 'f' && tname[1] == 'o' && !strcmp (tname, "fork"))
...
static void
restore_fixed_argument_area (rtx save_area, rtx argblock, int high_to_save,
			     int low_to_save)
{
...
static void
store_unaligned_arguments_into_pseudos (struct arg_data *args,
					int num_actuals)
{
  int i, j;
  ...
  for (i = 0; i < num_actuals; i++)
	...
	args[i].aligned_regs =
	  xmalloc (sizeof (rtx) * args[i].n_aligned_regs);
				 ...
				 bool call_from_thunk_p)
{
  ...
  for (p = actparms, argpos = 0; p; p = TREE_CHAIN (p), i += inc, argpos++)
	...
	type = TREE_TYPE (TYPE_FIELDS (type));
      ...
      if (CONTAINS_PLACEHOLDER_P (TYPE_SIZE (type)) || TREE_ADDRESSABLE (type)
	...
	)
	{
      ...
      if (targetm.calls.
	  promote_function_args (fndecl ? TREE_TYPE (fndecl) : 0))
	...
	*ecf_flags &= ~ECF_LIBCALL_BLOCK;
	  ...
	  || reg_parm_stack_space > 0 || args[i].pass_on_stack)
  ...
  if (args_size->var)
      ...
      if (preferred_stack_boundary > 1)
	{
	  ...
	  if (stack_pointer_delta & (preferred_stack_boundary - 1))
	    abort ();
	  args_size->var =
	    round_up (args_size->var, preferred_stack_boundary);
	}
...
static void
compute_argument_addresses (struct arg_data *args, rtx argblock,
			    int num_actuals)
{
  if (argblock)
    {
	...
	arg_reg = XEXP (argblock, 0), arg_offset =
	  INTVAL (XEXP (argblock, 1));
		  ...
		  tree type, rtx structure_value_addr)
{
  ...
  temp = expand_inline_function (fndecl, actparms, target,
				 ignore, type, structure_value_addr);
...
static int
check_sibcall_argument_overlap (rtx insn, struct arg_data *arg,
				int mark_stored_args_map)
{
  ...
  for (; insn; insn = NEXT_INSN (insn))
    if (INSN_P (insn) && check_sibcall_argument_overlap_1 (PATTERN (insn)))
      break;
...
expand_call (tree exp, rtx target, int ignore)
{
  ...
  if (structure_value_addr
	  ...
	  || reg_mentioned_p (virtual_outgoing_args_rtx,
			      structure_value_addr))
      && (args_size.var || (!ACCUMULATE_OUTGOING_ARGS && args_size.constant)))
  ...
  if (currently_expanding_call++ != 0
      ...
      || any_pending_cleanups () || args_size.var)
  ...
  for (pass = try_tail_call ? 0 : 1; pass < 2; pass++)
      ...
      if (pass == 0)
	{
	  ...
	  || (pending_stack_adjust > 0
	      && flag_exceptions && !(flags & ECF_NOTHROW)) || pass == 0)
	  ...
	  && adjusted_args_size.constant != unadjusted_args_size)
	{
	  ...
	  if (pending_stack_adjust
	      && !(flags & ECF_LIBCALL_BLOCK) && !inhibit_defer_pop)
	    {
      ...
      if (TYPE_MODE (TREE_TYPE (exp)) != VOIDmode && !structure_value_addr)
	{
	  if (pcc_struct_value)
	    ...
	    valreg =
	      hard_function_value (TREE_TYPE (exp), fndecl, (pass == 0));
	}
	...
	if (args[i].reg == 0 || args[i].pass_on_stack)
	  {
	    ...
	    if (flags & ECF_CONST
		&& args[i].stack && args[i].value == args[i].stack)
      ...
      if (pass && (flags & ECF_LIBCALL_BLOCK))
	{
	  ...
	  if (failed)
		...
		note = gen_rtx_fmt_e (SQRT,
				      GET_MODE (temp), args[0].initial_value);
	      else
		{
			   ...
			   enum machine_mode outmode, int nargs, va_list p)
{
  ...
  args_size.constant = (((args_size.constant
			  + stack_pointer_delta
			  + STACK_BYTES - 1)
			 / STACK_BYTES * STACK_BYTES) - stack_pointer_delta);
  ...
  for (count = 0; count < nargs; count++, argnum += inc)
      ...
      if (!(reg != 0 && partial == 0))
	{
	      ...
	      if (i < upper_bound)
		{
		  ...
		  if (save_mode == BLKmode)
			...
			= assign_stack_temp (BLKmode,
					     argvec[argnum].locate.size.
					     constant, 0);

		      emit_block_move (validize_mem
				       (argvec[argnum].save_area), stack_area,
				       GEN_INT (argvec[argnum].locate.size.
						constant),
				       BLOCK_OP_CALL_PARM);
		    }
		  else
	  ...
	  emit_push_insn (val, mode, NULL_TREE, NULL_RTX, PARM_BOUNDARY,
			  ...
			  reg_parm_stack_space,
			  ARGS_SIZE_RTX (argvec[argnum].locate.
					 alignment_pad));
  ...
  if (outmode != VOIDmode && retval)
      ...
      else if (GET_CODE (valreg) == PARALLEL)
	{
	  if (value == 0)
	    value = gen_reg_rtx (outmode);
	  emit_group_store (value, valreg, NULL_TREE,
			    GET_MODE_SIZE (outmode));
	}
      else if (value != 0)
