  ...
  for (PC = 0;;)
    {
      tree type, tmp;
      if (((PC != INVALID_PC
	    && instruction_bits[PC] & BCODE_TARGET) != 0) || PC == 0)
	{
	  ...
	  if (current_subr)
	      ...
	      while (--len >= 0)
		{
		  if (TREE_VEC_ELT (vec, index) == TYPE_UNUSED)
		    TREE_VEC_ELT (vec, index) =
		      TREE_VEC_ELT (subr_vec, index);
		}
	    }
	  break;
	case OPCODE_iadd:
	...
	ldc:
	  if (index <= 0 || index >= JPOOL_SIZE (current_jcf))
	    VERIFICATION_ERROR_WITH_INDEX
	      ("bad constant pool index %d in ldc");
	  int_value = -1;
	...
	case OPCODE_invokeinterface:
	  {
		...
		if (tag != CONSTANT_Methodref)
		  VERIFICATION_ERROR
		    ("invoke does not reference a Methodref");
	      }
	...
	case OPCODE_aaload:
	  ...
	  goto aload;
	case OPCODE_baload:
	  type = promote_type (byte_type_node);
	  goto aload;
	case OPCODE_caload:
	  type = promote_type (char_type_node);
	  goto aload;
	case OPCODE_saload:
	  type = promote_type (short_type_node);
	  goto aload;
	aload:
	  pop_type (int_type_node);
	...
	case OPCODE_lookupswitch:
	  {
	    ...
	    if (npairs < 0)
	      VERIFICATION_ERROR
		("invalid number of targets in lookupswitch");
	...
	case OPCODE_ret:
	  if (current_subr == NULL)
		...
		DECL_MAX_LOCALS (current_function_decl) + stack_pointer;
	      ...
	      INVALIDATE_PC;
	      if (index < 0
		  || index >= DECL_MAX_LOCALS (current_function_decl)
	...
	case OPCODE_ret_w:
	default:
	  error ("unknown opcode %d@pc=%d during verification", op_code,
		 PC - 1);
	  return 0;
	}
