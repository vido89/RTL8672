...
/* The stack pointer of the Java virtual machine.
   This does include the size of the quick_stack. */
     ...
     void init_expr_processing (void)
{
...
java_truthvalue_conversion (tree expr)
{
  ...
  switch (TREE_CODE (expr))
    ...
    case COND_EXPR:
      ...
      return
	fold (build
	      (COND_EXPR, boolean_type_node, TREE_OPERAND (expr, 0),
    ...
    default:
      return
	fold (build (NE_EXPR, boolean_type_node, expr, boolean_false_node));
    }
}
      ...
      else
	{
	  ...
	  if (CLASS_INTERFACE (TYPE_NAME (target_type)))
		...
		if (can_widen_reference_to
		    (TREE_TYPE (TREE_VEC_ELT (basetype_vec, i)), target_type))
		  return 1;
	      if (n == 0)
		return 0;
	    }
...
java_stack_swap (void)
{
  ...
  emit_move_insn (DECL_RTL (find_stack_slot (stack_pointer - 1, type2)),
		  DECL_RTL (decl2));
  emit_move_insn (DECL_RTL (find_stack_slot (stack_pointer - 2, type1)),
		  temp);
...
build_java_athrow (tree node)
{
  ...
  call = build (CALL_EXPR,
		...
		build_tree_list (NULL_TREE, node), NULL_TREE);
  ...
  if (!flag_syntax_only && check)
    {
      ...
      expr = build (COND_EXPR, TREE_TYPE (expr),
		    build (EQ_EXPR, boolean_type_node, expr,
			   null_pointer_node), build (CALL_EXPR,
						      void_type_node,
						      build_address_of
...
build_java_arrayaccess (tree array, tree type, tree index)
{
  ...
  ref = build (COMPONENT_REF, TREE_TYPE (data_field),
	       build_java_indirect_ref (array_type, array,
					flag_check_references), data_field);
...
build_java_arraystore_check (tree array, tree object)
{
  ...
  check = build (CALL_EXPR, void_type_node,
		 ...
		 tree_cons (NULL_TREE, array,
			    build_tree_list (NULL_TREE, object)), NULL_TREE);
...
build_newarray (int atype_value, tree length)
{
  ...
  tree type = build_java_array_type (prim_type,
  ...
  return build (CALL_EXPR, promote_type (type),
		...
		tree_cons (NULL_TREE,
			   type_arg,
			   build_tree_list (NULL_TREE, length)), NULL_TREE);
}
...
expand_java_arrayload (tree lhs_type_node)
{
  ...
  if (TREE_TYPE (array_node) == ptr_type_node)
    ...
    {
      lhs_type_node =
	build_java_check_indexed_type (array_node, lhs_type_node);
      load_node =
	build_java_arrayaccess (array_node, lhs_type_node, index_node);
    }
  ...
  if (type == object_type_node || inherits_from_p (valtype, type))
    {
      ...
      expr = build (NE_EXPR, itype, value, null_pointer_node);
	   ...
	   && (CLASS_FINAL (klass) || !inherits_from_p (valtype, type)))
    {
  ...
  else if (DECL_P (klass) && CLASS_FINAL (klass))
    {
      ...
      expr = build (COND_EXPR, itype,
		    ...
		    build (EQ_EXPR, itype,
			   ...
			   build_class_ref (type)), boolean_false_node);
    }
  else
    {
...
expand_java_CHECKCAST (tree type)
{
  ...
  value = build (CALL_EXPR, promote_type (type),
		 ...
		 tree_cons (NULL_TREE, build_class_ref (type),
			    build_tree_list (NULL_TREE, value)), NULL_TREE);
  push_value (value);
}
...
build_java_soft_divmod (enum tree_code op, tree type, tree op1, tree op2)
{
  ...
  call = build (CALL_EXPR, type,
		...
		tree_cons (NULL_TREE, arg1,
			   build_tree_list (NULL_TREE, arg2)), NULL_TREE);
...
build_java_binop (enum tree_code op, tree type, tree arg1, tree arg2)
{
  ...
  switch (op)
	...
	return fold (build (COND_EXPR, int_type_node, ifexp1,
			    ...
			    : integer_minus_one_node, second_compare));
      }
    case COMPARE_EXPR:
      arg1 = save_expr (arg1);
      arg2 = save_expr (arg2);
    ...
    case TRUNC_DIV_EXPR:
    case TRUNC_MOD_EXPR:
      if (TREE_CODE (type) == REAL_TYPE && op == TRUNC_MOD_EXPR)
	{
	  tree call;
	  ...
	  && flag_use_divide_subroutine && !flag_syntax_only)
...
build_field_ref (tree self_value, tree self_class, tree name)
{
  ...
  if (self_value == NULL_TREE)
	...
	self_value = fold (build1 (NOP_EXPR, base_type, self_value));
      if (flag_indirect_dispatch && output_class != self_class)
	   ...
	   we can make a direct reference.  */
	{
	  ...
	  return
	    fold (build1 (INDIRECT_REF, TREE_TYPE (field_decl), address));
	}
	...
	build_java_indirect_ref (TREE_TYPE (TREE_TYPE (self_value)),
      ...
      return
	fold (build
	      (COMPONENT_REF, TREE_TYPE (field_decl), self_value,
	       field_decl));
    }
}
...
{
  tree decl;
  decl = build_decl (LABEL_DECL, name, TREE_TYPE (return_address_type_node));
...
build_class_init (tree clas, tree expr)
{
  ...
  if ((!CLASS_INTERFACE (TYPE_NAME (clas))
       && inherits_from_p (current_class, clas)) || current_class == clas)
    return expr;
			...
			tree arg_list ATTRIBUTE_UNUSED)
{
  ...
  if (is_compiled_class (self_type))
	...
	methods_ident = get_identifier ("methods");
      ...
      for (meth = TYPE_METHODS (self_type);; meth = TREE_CHAIN (meth))
	{
	  if (method == meth)
	    break;
	  if (meth == NULL_TREE)
...
expand_invoke (int opcode, int method_ref_index, int nargs ATTRIBUTE_UNUSED)
{
  ...
  tree method_name =
    COMPONENT_REF_NAME (&current_jcf->cpool, method_ref_index);
  tree self_type =
  ...
  call =
    build (CALL_EXPR, TREE_TYPE (method_type), func, arg_list, NULL_TREE);
  TREE_SIDE_EFFECTS (call) = 1;
...
build_jni_stub (tree method)
{
  ...
  block = build_block (env_var, NULL_TREE, NULL_TREE, method_args, NULL_TREE);
  ...
  body = build (MODIFY_EXPR, ptr_type_node, env_var,
		build (CALL_EXPR, ptr_type_node,
		       ...
		       build_tree_list (NULL_TREE, klass), NULL_TREE));
  ...
  call = build (CALL_EXPR, TREE_TYPE (TREE_TYPE (method)),
		build1 (NOP_EXPR, jni_func_type, jnifunc), args, NULL_TREE);

  /* If the JNI call returned a result, capture it here.  If we had to
     unwrap JNI object results, we would do that here.  */
  if (res_var != NULL_TREE)
    call = build (MODIFY_EXPR, TREE_TYPE (TREE_TYPE (method)), res_var, call);
  ...
  call = build (CALL_EXPR, ptr_type_node,
		...
		build_tree_list (NULL_TREE, env_var), NULL_TREE);
  TREE_SIDE_EFFECTS (call) = 1;
  CAN_COMPLETE_NORMALLY (call) = 1;
  body = build (COMPOUND_EXPR, void_type_node, body, call);
  TREE_SIDE_EFFECTS (body) = 1;
...
{
  tree self_type = get_class_constant (current_jcf,
  ...
  if (is_putting)
      ...
      if (FIELD_FINAL (field_decl))
	{
	  if (DECL_CONTEXT (field_decl) != current_class)
		...
		warning
		  ("%Jassignment to final field '%D' not in constructor",
		   field_decl, field_decl);
	    }
	}
		  ...
		  rtx * alt_rtl ATTRIBUTE_UNUSED)
{
  ...
  switch (TREE_CODE (exp))
	...
	tmp = expand_assignment (array_decl,
				 build_new_array (element_type, length), 1);
      ...
      if (BLOCK_EXPR_BODY (exp))
	{
	  ...
	  if (!always_initialize_class_p
	      && current_function_decl && found_class_initialization_flag)
	    htab_traverse
...
force_evaluation_order (tree node)
{
  ...
  if (TREE_CODE (node) == CALL_EXPR
	...
	{
	  cmp =
	    save_expr (build (COMPOUND_EXPR, TREE_TYPE (node), cmp, node));
...
emit_init_test_initialization (void **entry, void *x ATTRIBUTE_UNUSED)
{
  ...
  expand_expr_stmt (build (MODIFY_EXPR, boolean_type_node, ite->value, rhs));
  return true;
}
