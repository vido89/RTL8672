...
caching_stat (char *filename, struct stat *buf)
{
  ...
  if (!memoized_dirlists)
    memoized_dirlists = htab_create (37,
				     htab_hash_string,
				     memoized_dirlist_lookup_eq, NULL);
  ...
  if (!*slot)
    {
      ...
      dent->num_files = scandir (filename, &dent->files,
				 (void *) java_or_class_file, alphasort);
      *slot = dent;
    }
  else
  ...
  if (!java && !class && java_buf.st_mtime > class_buf.st_mtime)
    {
      if (flag_newer)
	warning
	  ("source file for class `%s' is newer than its matching class file.  Source file `%s' used instead",
	   classname, java_buffer);
      class = -1;
    }
