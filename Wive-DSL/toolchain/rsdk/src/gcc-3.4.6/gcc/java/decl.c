...
{
  if (pc >= DECL_LOCAL_START_PC (decl) && pc < DECL_LOCAL_END_PC (decl))
    {
      if (best == NULL_TREE
...
#error "unsupported size"
  ...
  atable_type = build_array_type (ptr_type_node, one_elt_array_domain_type);
  ...
  add_predefined_file (get_identifier
  ...
  PUSH_FIELD (class_type_node, field, "otable_syms", symbols_array_ptr_type);
  PUSH_FIELD (class_type_node, field, "atable", atable_ptr_type);
  PUSH_FIELD (class_type_node, field, "atable_syms", symbols_array_ptr_type);
...
#define jint_ptr_type ptr_type_node
  ...
  PUSH_FIELD (lineNumberEntry_type, field, "line_nr",
	      unsigned_short_type_node);
...
#define lineNumbers_ptr_type_node build_pointer_type(lineNumbers_type)
  ...
  soft_anewarray_node =
    builtin_function ("_Jv_NewObjectArray",
		      build_function_type (ptr_type_node, t), 0, NOT_BUILT_IN,
		      NULL, NULL_TREE);
  ...
  if (name)
      ...
      else
	{
	  ...
	  if (oldlocal != 0 && !DECL_EXTERNAL (x)
	      ...
	      && chain_member (oldlocal,
			       current_binding_level->level_chain->names))
	    {
  ...
  int block_previously_created;
  {
    ...
    if (current_binding_level->end_pc != LARGEST_PC)
      fprintf (stderr, "pop  %s level 0x%08x pc %d (end pc %d)\n",
	       (is_class_level) ? "class" : "block", current_binding_level,
	       current_pc, current_binding_level->end_pc);
    else
