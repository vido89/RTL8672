...
static void append_innerclasses_attribute_entry (struct jcf_partial *, tree,
						 tree);
...
localvar_alloc (tree decl, struct jcf_partial *state)
{
  ...
  for (index = 0; ptr < limit; index++, ptr++)
    {
      if (ptr[0] == NULL && (!wide || ((ptr + 1) < limit && ptr[1] == NULL)))
	break;
    }
  if (ptr == limit)
    {
  ...
  if (debug_info_level > DINFO_LEVEL_TERSE && DECL_NAME (decl) != NULL_TREE)
    {
...
{
  int flags = 0;
  int isfield = TREE_CODE (decl) == FIELD_DECL
...
static void
push_long_const (HOST_WIDE_INT lo, HOST_WIDE_INT hi,
		 struct jcf_partial *state)
{
...
generate_bytecode_insns (tree exp, int target, struct jcf_partial *state)
{
  ...
  switch (TREE_CODE (exp))
	...
	if (target != IGNORE_TARGET)
	  {
		...
		OP1 (OPCODE_arraylength);
	      ...
	      {
		field_op (field,
			  is_static ? OPCODE_getstatic : OPCODE_getfield,
			  state);
		if (!is_static)
		  NOTE_POP (1);
	...
	generate_bytecode_conditional (exp, then_label, else_label, 1, state);
	define_jcf_label (then_label, state);
	...
	struct jcf_relocation *reloc
	  =
	  obstack_alloc (state->chunk_obstack,
	...
	if (sw_state.num_cases <= 1)
	...
	else
	  {
		...
		int index = 0;
		RESERVE (13 +
			 4 * (sw_state.max_case - sw_state.min_case + 1));
	...
	for (; body_block != sw_state.default_label;
	     body_block = body_block->next)
    ...
    increment:
	  ...
	  && !TREE_STATIC (exp)
	  && TREE_CODE (type) == INTEGER_TYPE && TYPE_PRECISION (type) == 32)
	{
	...
	{
	  generate_bytecode_insns (TREE_OPERAND (exp, 0), STACK_TARGET,
				   state);
	  emit_dup (1, 0, state);
      ...
      else if (TREE_CODE (exp) == ARRAY_REF)
	{
	  generate_bytecode_insns (TREE_OPERAND (exp, 0), STACK_TARGET,
				   state);
	  generate_bytecode_insns (TREE_OPERAND (exp, 1), STACK_TARGET,
				   state);
	  emit_dup (2, 0, state);
	...
	emit_dup (size, offset, state);
      ...
      emit_binop ((value >= 0 ? OPCODE_iadd : OPCODE_isub)
		  + adjust_typed_op (type, 3), type, state);
      if (target != IGNORE_TARGET && !post_op)
	emit_dup (size, offset, state);
	...
	if (TREE_CODE_CLASS (TREE_CODE (rhs)) == '2'
	    && lhs == TREE_OPERAND (rhs, 0))
	  {
		...
		NOTE_POP (2);
		jopcode =
		  OPCODE_iaload + adjust_typed_op (TREE_TYPE (lhs), 7);
		RESERVE (1);
		OP1 (jopcode);
		NOTE_PUSH (TYPE_IS_WIDE (TREE_TYPE (lhs)) ? 2 : 1);
	      }
	...
	if (arg0 != NULL_TREE && operand_equal_p (arg0, arg1, 0))
	  {
	    ...
	    emit_dup (TYPE_PRECISION (TREE_TYPE (arg0)) > 32 ? 2 : 1, 0,
		      state);
	  }
	else
	  {
    ...
    case INSTANCEOF_EXPR:
      {
	int index =
	  find_class_constant (&state->cpool, TREE_OPERAND (exp, 1));
	generate_bytecode_insns (TREE_OPERAND (exp, 0), target, state);
	RESERVE (3);
	...
	for (; values != NULL_TREE; values = TREE_CHAIN (values), offset++)
	  {
	    ...
	    NOTE_PUSH (1);
	    generate_bytecode_insns (TREE_VALUE (values), STACK_TARGET,
				     state);
	    RESERVE (1);
	    OP1 (jopcode);
	...
	x = build (COND_EXPR, TREE_TYPE (exp),
		   build (code, boolean_type_node, op0, op1), op0, op1);
	...
	if (f == soft_newarray_node)
	...
	else if (f == soft_anewarray_node)
	  {
	    ...
	    break;
		 ...
		 || f == soft_monitorexit_node || f == throw_node)
	  {
	...
	if (TREE_CODE (f) == FUNCTION_DECL && DECL_CONTEXT (f) != NULL_TREE)
	  {
	    ...
	    index =
	      find_methodref_with_class_index (&state->cpool, f, context);
	    OP2 (index);
	    if (interface)
	      {
		if (nargs <= 0)
  ...
  for (block = state->blocks; block != NULL; block = block->next)
	     ...
	     && reloc->label->v.chunk->data[0] == OPCODE_goto)
	{
	  ...
	  for (first_reloc = reloc->label->u.relocations;
	       first_reloc; first_reloc = first_reloc->next)
	    {
...
GTY (())
     ...
     static struct chunk *generate_classfile (tree clas,
					      struct jcf_partial *state)
{
  ...
  if (!(i & ACC_INTERFACE))
    i |= ACC_SUPER;
  PUT2 (i);			/* access_flags */
  i = find_class_constant (&state->cpool, clas);
  PUT2 (i);			/* this_class */
  if (clas == object_type_node)
    {
      PUT2 (0);			/* super_class */
  ...
  for (part = TYPE_FIELDS (clas); part; part = TREE_CHAIN (part))
	...
	continue;
      ptr = append_chunk (NULL, 8, state);
      i = get_access_flags (part);
      PUT2 (i);
      i = find_utf8_constant (&state->cpool, DECL_NAME (part));
      PUT2 (i);
      ...
      if (have_value)
	{
	  ...
	  i = find_constant_index (init, state);
	  PUT2 (i);
	}
  ...
  for (part = TYPE_METHODS (clas); part; part = TREE_CHAIN (part))
	...
	continue;
      ...
      i = get_access_flags (part);
      PUT2 (i);
      i = find_utf8_constant (&state->cpool, name);
      PUT2 (i);
      ...
      if (body != NULL_TREE)
	{
	  ...
	  i = find_utf8_constant (&state->cpool, Code_node);
	  PUT2 (i);
	  attr_len_ptr = ptr;
	  ...
	  if (state->lvar_count > 0)
	      ...
	      for (; lvar != NULL; lvar = lvar->next)
		{
		  ...
		  i = lvar->start_label->pc;
		  PUT2 (i);
		  i = lvar->end_label->pc - i;
		  PUT2 (i);
		  i = find_utf8_constant (&state->cpool, name);
		  PUT2 (i);
		  i = find_utf8_constant (&state->cpool, sig);
		  PUT2 (i);
		  i = DECL_LOCAL_INDEX (lvar->decl);
		  PUT2 (i);
		}
	    }
	}
