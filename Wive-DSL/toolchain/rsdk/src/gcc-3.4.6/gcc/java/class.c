...
/* The compiler generates different code depending on whether or not
   ...
   represents either a package or a specific class.  */
     ...
     static assume_compiled_node
       *find_assume_compiled_node (assume_compiled_node *, const char *);
...
/* Return the node that most closely represents the class whose name
   ...
   appropriate node does not exist.  */
     ...
     static assume_compiled_node
       *find_assume_compiled_node (assume_compiled_node * node,
				   const char *ident)
{
  while (node)
    {
		  ...
		  || ident[node_ident_length] == '.')))
	{
	  ...
	  assume_compiled_node *found =
	    find_assume_compiled_node (node->child,
				       ident);
	  if (found)
	    return found;
	  else
...
assume_compiled (const char *ident)
{
  ...
  i = find_assume_compiled_node (assume_compiled_tree, ident);
...
ident_subst (const char *old_name,
	     int old_length,
	     const char *prefix,
	     int old_char, int new_char, const char *suffix)
{
...
set_class_decl_access_flags (int access_flags, tree class_decl)
{
  if (access_flags & ACC_PUBLIC)
    CLASS_PUBLIC (class_decl) = 1;
  if (access_flags & ACC_FINAL)
    CLASS_FINAL (class_decl) = 1;
  if (access_flags & ACC_SUPER)
    CLASS_SUPER (class_decl) = 1;
  if (access_flags & ACC_INTERFACE)
    CLASS_INTERFACE (class_decl) = 1;
  if (access_flags & ACC_ABSTRACT)
    CLASS_ABSTRACT (class_decl) = 1;
  if (access_flags & ACC_STATIC)
    CLASS_STATIC (class_decl) = 1;
  if (access_flags & ACC_PRIVATE)
    CLASS_PRIVATE (class_decl) = 1;
  if (access_flags & ACC_PROTECTED)
    CLASS_PROTECTED (class_decl) = 1;
  if (access_flags & ACC_STRICT)
    CLASS_STRICTFP (class_decl) = 1;
}
...
tree
add_method_1 (tree this_class, int access_flags, tree name,
	      tree function_type)
{
  ...
  DECL_LANG_SPECIFIC (fndecl) = ggc_alloc_cleared (sizeof (struct lang_decl));
  DECL_LANG_SPECIFIC (fndecl)->desc = LANG_DECL_FUNC;

  /* Initialize the static initializer test table.  */

  DECL_FUNCTION_INIT_TEST_TABLE (fndecl) = java_treetreehash_create (10, 1);
  ...
  if (access_flags & ACC_PUBLIC)
    METHOD_PUBLIC (fndecl) = 1;
  if (access_flags & ACC_PROTECTED)
    METHOD_PROTECTED (fndecl) = 1;
  if (access_flags & ACC_PRIVATE)
  ...
  if (access_flags & ACC_SYNCHRONIZED)
    METHOD_SYNCHRONIZED (fndecl) = 1;
  if (access_flags & ACC_ABSTRACT)
    METHOD_ABSTRACT (fndecl) = 1;
  if (access_flags & ACC_STRICT)
    METHOD_STRICTFP (fndecl) = 1;
  return fndecl;
}
...
add_field (tree class, tree name, tree field_type, int flags)
{
  ...
  if (flags & ACC_PUBLIC)
    FIELD_PUBLIC (field) = 1;
  if (flags & ACC_PROTECTED)
    FIELD_PROTECTED (field) = 1;
  if (flags & ACC_PRIVATE)
    FIELD_PRIVATE (field) = 1;
  if (flags & ACC_FINAL)
    FIELD_FINAL (field) = 1;
  if (flags & ACC_VOLATILE)
    FIELD_VOLATILE (field) = 1;
  if (flags & ACC_TRANSIENT)
    FIELD_TRANSIENT (field) = 1;
  if (is_static)
    {
...
build_utf8_ref (tree name)
{
  ...
  str_type = build_prim_array_type (unsigned_byte_type_node, name_len + 1);	/* Allow for final '\0'. */
...
build_static_field_ref (tree fdecl)
{
  ...
  if (flag_indirect_dispatch)
	...
	= build (ARRAY_REF, build_pointer_type (TREE_TYPE (fdecl)),
      ...
      return fold (build1 (INDIRECT_REF, TREE_TYPE (fdecl), field_address));
    }
  else
    {
      /* Compile as:
...
#define ACC_TRANSLATED          0x4000
...

  {
	...
	for (iter = DECL_FUNCTION_THROWS (mdecl);
	     iter != NULL_TREE; iter = TREE_CHAIN (iter))
	  {
...
get_dispatch_table (tree type, tree this_class_addr)
{
  ...
  arraysize =
    (TARGET_VTABLE_USES_DESCRIPTORS ? nvirtuals + 1 : nvirtuals + 2);
  if (TARGET_VTABLE_USES_DESCRIPTORS)
...
make_class_data (tree type)
{
  ...
  tree dtable_start_offset =
    build_int_2 (2 * POINTER_SIZE / BITS_PER_UNIT, 0);
  ...
  if (super == NULL_TREE)
    super = null_pointer_node;
	    ...
	    &&
	    assume_compiled (IDENTIFIER_POINTER
			     (DECL_NAME (TYPE_NAME (super)))))
  ...
  PUSH_FIELD_VALUE (temp, "vtable",
		    build (PLUS_EXPR, dtable_ptr_type,
			   build1 (ADDR_EXPR, dtable_ptr_type,
				   class_dtable_decl), dtable_start_offset));
  ...
  if (flag_indirect_dispatch)
    ...
    PUSH_FIELD_VALUE (cons, "vtable",
		      ...
		      : build (PLUS_EXPR, dtable_ptr_type,
			       build1 (ADDR_EXPR, dtable_ptr_type,
				       dtable_decl), dtable_start_offset));
  if (TYPE_OTABLE_METHODS (type) == NULL_TREE)
  ...
  else
    {
      PUSH_FIELD_VALUE (cons, "otable",
			build1 (ADDR_EXPR, otable_ptr_type,
				TYPE_OTABLE_DECL (type)));
  ...
  if (TYPE_ATABLE_METHODS (type) == NULL_TREE)
  ...
  else
    {
      PUSH_FIELD_VALUE (cons, "atable",
			build1 (ADDR_EXPR, atable_ptr_type,
				TYPE_ATABLE_DECL (type)));
  ...
  PUSH_FIELD_VALUE (cons, "catch_classes",
		    build1 (ADDR_EXPR, ptr_type_node,
			    TYPE_CTABLE_DECL (type)));
      ...
      if (basetype_vec)
	{
	  ...
	  for (i = n; i > 0; i--)
		...
		= maybe_layout_super_class (super_interface, NULL_TREE);
		  ...
		  || TREE_CODE (TYPE_SIZE (maybe_super_interface)) ==
		  ERROR_MARK)
		{
		  TYPE_SIZE (this_class) = error_mark_node;
		  CLASS_BEING_LAIDOUT (this_class) = 0;
      ...
      if (super_method != NULL_TREE && !METHOD_PRIVATE (super_method))
	{
	  ...
	  if (DECL_VINDEX (method_decl) == NULL_TREE
	    ...
	    error ("%Jnon-static method '%D' overrides static method",
	       ...
	       && !CLASS_FINAL (TYPE_NAME (this_class)) && dtable_count)
	{
...
emit_catch_table (tree this_class)
{
  ...
  array_type =
    build_array_type (TREE_TYPE (TREE_TYPE (TYPE_CTABLE_DECL (this_class))),
		      table_size);
  table =
    build_decl (VAR_DECL, DECL_NAME (TYPE_CTABLE_DECL (this_class)),
		array_type);
  DECL_INITIAL (table) =
