...
makelong (const uch * sig)
{
   ...
   */
  return (((ulg) sig[3]) << 24)
    + (((ulg) sig[2]) << 16) + (((ulg) sig[1]) << 8) + ((ulg) sig[0]);
}
...
read_zip_archive (ZipFile * zipf)
{
  ...
  if (zipf->size < (ECREC_SIZE + 4)
      || lseek (zipf->fd, (long) (-(ECREC_SIZE + 4)), SEEK_CUR) <= 0)
    return -1;
...
#define ALLOC xmalloc
  ...
  for (i = 0; i < zipf->count; i++)
	...
	makelong (&dir_ptr[4 + C_RELATIVE_OFFSET_LOCAL_HEADER]);
      ...
      if ((dir_ptr - zipf->central_directory) + filename_length + CREC_SIZE +
	  4 > zipf->dir_size)
	return -1;
  ...
  for (i = 0; i < zipf->count; i++, zipd = ZIPDIR_NEXT (zipd))
    {
      printf ("%d: size:%d, name(#%d)%s, offset:%d\n",
	      ...
	      ZIPDIR_FILENAME (zipd), zipd->filestart);
    }
}
#endif
