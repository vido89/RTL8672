	  ...
	  if (TYPE_ARRAY_P (type))
	    {
	      ...
	      sig =
		ident_subst (IDENTIFIER_POINTER (t), IDENTIFIER_LENGTH (t),
			     "[", 0, 0, "");
	    }
	  else
	    {
		     ...
		     tree signature, tree (*signature_builder) (tree))
{
  ...
  for (method = TYPE_METHODS (searched_class);
      ...
      if (DECL_NAME (method) == method_name && method_sig == signature)
	{
	  ...
	  if (!(flags & SEARCH_VISIBLE) || !METHOD_INVISIBLE (method))
	    return method;
	}
    }
  return NULL_TREE;
			   ...
			   tree signature, tree (*signature_builder) (tree))
{
  ...
  for (i = interface_len; i > 0; i--)
    {
      tree child = TREE_VEC_ELT (TYPE_BINFO_BASETYPES (searched_class), i);
