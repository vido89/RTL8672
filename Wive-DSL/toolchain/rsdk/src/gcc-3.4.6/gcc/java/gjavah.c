...
static void print_stub_or_jni (FILE *, JCF *, int, int, int, const char *,
			       int);
static void print_full_cxx_name (FILE *, JCF *, int, int, int, const char *,
				 int);
...
static const unsigned char *decode_signature_piece (FILE *,
						    const unsigned char *,
						    const unsigned char *,
						    int *);
...
/* Print a single-precision float, suitable for parsing by g++.  */
     static void jni_print_float (FILE * stream, jfloat f)
{
...
jni_print_double (FILE * stream, jdouble f)
{
  ...
  if (JDOUBLE_FINITE (f))
	...
	fprintf (stream, "0x1.%.5x%.8xp%+d",
		 f.mantissa0, f.mantissa1, f.exponent - JDOUBLE_EXP_BIAS);
      else
	/* Exponent of 0x001 is -1022; exponent of 0x000 is *also* -1022,
	   because the implicit leading 1 bit is no longer present.  */
	fprintf (stream, "0x0.%.5x%.8xp%+d",
		 f.mantissa0, f.mantissa1, f.exponent + 1 - JDOUBLE_EXP_BIAS);
    }
  fputs (";\n", stream);
}
...
cxx_keyword_subst (const unsigned char *str, int length)
{
  ...
  for (mid = (last + first) / 2;
       mid != old; old = mid, mid = (last + first) / 2)
    {
  ...
  if (!stream)
    {
      ...
      memcpy (nn->signature, JPOOL_UTF_DATA (jcf, sig_index), nn->sig_length);
      method_name_list = nn;
...
decompile_method (FILE * out, JCF * jcf, int code_len)
{
  ...
  if (code_len == 5
	  ...
	  || codes[4] == OPCODE_ireturn || codes[4] == OPCODE_lreturn))
    {
      ...
      else
	{
      ...
      decompiled = 1;
	   ...
	   && codes[0] == OPCODE_aload_0 && codes[1] == OPCODE_areturn
  ...
  else if (code_len == 1 && codes[0] == OPCODE_return)
    {
      ...
      decompiled = 1;
	   ...
	   && codes[0] == OPCODE_aconst_null && codes[1] == OPCODE_areturn)
    {
  ...
  if (JPOOL_TAG (jcf, signature_index) != CONSTANT_Utf8)
	...
	fputs (" ", stream);
      print_full_cxx_name (stream, jcf, name_index,
			   signature_index, is_init, name_override, flags);
    }
}
...
static void
print_mangled_classname (FILE * stream, JCF * jcf, const char *prefix,
			 int index)
{
  int name_index = JPOOL_USHORT1 (jcf, index);
  fputs (prefix, stream);
  jcf_print_utf8_replace (out,
			  ...
			  JPOOL_UTF_LENGTH (jcf, name_index), '/', '_');
}
...
print_include (FILE * out, const unsigned char *utf8, int len)
{
  ...
  fputs ("#include <", out);
  jcf_print_utf8_replace (out, utf8, len, '/', flag_jni ? '_' : '/');
  fputs (".h>\n", out);
}

...
%s overwrite any edits you have made to this file.%s\n\n", cstart, jflag, mode, cstart2, cstart2, cstart2, what, cstart2, jflag, cstart2, cend);
	}
    }
  ...
  if (out && !flag_jni)
	...
	fputc ('\n', out);
      ...
      if (!stubs)
	{
	  if (!print_cxx_classname (out, "class ", jcf, jcf->this_class, 0))
	    {
...
main (int argc, char **argv)
{
  ...
  while ((opt =
	  getopt_long_only (argc, argv, "I:d:o:v", options, NULL)) != -1)
    {
      switch (opt)
	{
	case 0:
