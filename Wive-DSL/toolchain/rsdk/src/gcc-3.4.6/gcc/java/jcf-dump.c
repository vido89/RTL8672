...
usage (void)
  ATTRIBUTE_NORETURN;
     ...
     static void print_exception_table (struct JCF *,
					const unsigned char *entries, int);

#define PRINT_SIGNATURE_RESULT_ONLY 1
#define PRINT_SIGNATURE_ARGS_ONLY 2

     static int utf8_equal_string (JCF * jcf, int index, const char *value)
{
...
print_access_flags (FILE * stream, uint16 flags, char context)
{
  ...
  if (flags & ACC_TRANSIENT)
    fprintf (stream, " transient");
  if (flags & ACC_VOLATILE)
    fprintf (stream, " volatile");
  if (flags & ACC_NATIVE)
