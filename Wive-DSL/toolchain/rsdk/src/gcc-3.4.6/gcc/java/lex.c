	  ...
	  while (1)
	      ...
	      if (lex->first == 0 || lex->first >= lex->last)
		{
		  ...
		  r = fread (&lex->buffer[lex->last], 1,
			     sizeof (lex->buffer) - lex->last, lex->finput);
		  lex->last += r;
		}
	      ...
	      if (ir == (size_t) - 1)
		{
		  if (errno == EINVAL)
		  ...
		  else
		    {
		      ...
		      java_lex_error
			("unrecognized character in input stream", 0);
		      return UEOF;
		    }
		}
	    }
...
error_if_numeric_overflow (tree value)
{
  if (TREE_CODE (value) == INTEGER_CST
      && JAVA_RADIX10_FLAG (value) && tree_int_cst_sgn (value) < 0)
    {
...
static void
java_lex_error (const char *msg ATTRIBUTE_UNUSED,
