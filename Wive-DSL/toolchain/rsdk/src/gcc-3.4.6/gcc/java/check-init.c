...
static void check_bool2_init (enum tree_code, tree, tree, words, words,
			      words);
struct alternatives;
...
get_variable_decl (tree exp)
{
  ...
  /* See if exp is this.field. */
  else if (TREE_CODE (exp) == COMPONENT_REF)
    {
	  ...
	  && !METHOD_STATIC (mdecl) && FIELD_FINAL (op1))
	{
...
check_final_reassigned (tree decl, words before)
{
  ...
  if (DECL_FINAL (decl) && index != -2 && (index < loop_current_locals	/* I.e. -1, or outside current loop. */
  ...
  switch (TREE_CODE (exp))
    ...
    case COND_EXPR:
      check_cond_init (TREE_OPERAND (exp, 0), TREE_OPERAND (exp, 1),
		       TREE_OPERAND (exp, 2), before, when_false, when_true);
      return;
...
again:
  switch (TREE_CODE (exp))
	...
	if (TREE_CODE (alt->block) != LOOP_EXPR)
	  abort ();
#endif
	check_bool_init (TREE_OPERAND (exp, 0), before, when_false,
			 when_true);
