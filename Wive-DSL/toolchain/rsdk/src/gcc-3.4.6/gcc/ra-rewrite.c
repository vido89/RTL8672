...
spill_propagation (sbitmap to_prop, sbitmap spilled, sbitmap processed)
{
  ...
  EXECUTE_IF_SET_IN_SBITMAP (to_prop, 0, id,
			     {
			     spill_prop_insert (ID2WEB (id), list,
						processed);});
  ...
  for (d = WEBS (SPILLED); d; d = d->next)
	...
	if ((bestc < 0 || costs[bestc] > costs[c])
	    ...
	    && HARD_REGNO_MODE_OK (c, PSEUDO_REGNO_MODE (web->regno)))
	  {
	    ...
	    for (i = 1; i < size && TEST_HARD_REG_BIT (avail, c + i); i++);
	    if (i == size)
	      bestc = c;
	  }
      web->color = bestc;
  ...
  for (insn = get_last_insn (); insn; insn = PREV_INSN (insn))
	...
	break;
      ...
      EXECUTE_IF_SET_IN_BITMAP (live_at_end[i - 2], 0, j,
				...
				if (aweb->type !=
				    SPILLED /*|| aweb->color >= 0 */ )
				{
				SET_BIT (ri.live, web->id);
				if (aweb->type != SPILLED)
				update_spill_colors (&(ri.colors_in_use), web,
						     1);}
				}
      );
      ...
      if (flag_ra_ir_spilling)
	{
	  ...
	  for (pass = 0; pass < 2; pass++)
	    for (d = (pass) ? WEBS (SPILLED) : WEBS (COALESCED); d;
		 d = d->next)
	      {
		struct web *web = DLIST_WEB (d);
		struct web *aweb = alias (web);
      ...
      if (ri.num_reloads)
	{
	  ...
	  for (e = bb->pred; e && num < 5; e = e->pred_next, num++)
	    {
	      ...
	      EXECUTE_IF_SET_IN_BITMAP (live_at_end[e->src->index], 0, j,
					...
					if (aweb->type !=
					    SPILLED)
					update_spill_colors (&colors, web,
							     1);}
	      );
	  ...
	  EXECUTE_IF_SET_IN_BITMAP (ri.need_reload, 0, j,
				    ...
				    if (((ra_pass > 0
					  || supweb2->target_of_spilled_move)
					 && (1 || in_ir
...
delete_useless_defs (void)
{
  ...
  EXECUTE_IF_SET_IN_BITMAP (useless_defs, 0, i,
			    ...
			    rtx set = single_set (insn);
			    struct web *web =
			    find_web_for_subweb (def2web[i]);
			    if (set && web->type == SPILLED
				&& web->stack_slot == NULL)
			    {
			    deleted_def_insns++;
			    deleted_def_cost +=
			    BLOCK_FOR_INSN (insn)->frequency + 1;
			    PUT_CODE (insn, NOTE);
  ...
  for (i = 0; i < num_webs - num_subwebs; i++)
      ...
      else
	{
	  ...
	  if (web->num_uses == 0 && web->num_defs == 1)
	    web->reg_rtx =
	      gen_reg_rtx (GET_MODE (DF_REF_REAL_REG (web->defs[0])));
	  else
	  ...
	  if (web->type == COALESCED)
	    bitmap_set_bit (regnos_coalesced_to_hardregs,
			    REGNO (web->reg_rtx));
	}
    }
