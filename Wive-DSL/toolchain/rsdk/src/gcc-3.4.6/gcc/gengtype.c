  ...
  for (i = 0; i < ARRAY_SIZE (data); i++)
    {
      ...
      for (r_index = 0;
	   r_index < data[i].first_rtl + data[i].num_rtl; r_index++)
	{
	  pair_p old_subf = subfields;
	  subfields = xmalloc (sizeof (*subfields));
	  subfields->next = old_subf;
		...
		struct fileloc *pos)
{
  ...
  for (p = typeinfo; p; p = p->next)
	...
	if (strcmp (m->name, p->name) == 0)
      ...
      if (p->type == NULL)
	{
	  error_at_line (&p->line, "couldn't match fieldname `%s'", p->name);
	  p->name = NULL;
	}
    }
  ...
  while (*p_p)
    {
      ...
      if (p->name == NULL || p->type == (type_p) 1)
	*p_p = p->next;
      else
	p_p = &p->next;
    }
...
get_base_file_bitmap (const char *input_file)
{
  ...
  if (slashpos)
	...
	if ((size_t) (slashpos - basename) == strlen (lang_dir_names[i])
	    && memcmp (basename, lang_dir_names[i],
		       strlen (lang_dir_names[i])) == 0)
	  {
...
get_output_file_with_visibility (const char *input_file)
{
  ...
  if ((len > 2 && memcmp (basename + len - 2, ".c", 2) == 0)
	...
	if (memcmp (basename, lang_dir_names[i], strlen (lang_dir_names[i]))
	    == 0 && basename[strlen (lang_dir_names[i])] == '/')
	  return base_files[i];
...
main (int argc ATTRIBUTE_UNUSED, char **argv ATTRIBUTE_UNUSED)
{
  ...
  do_typedef ("HARD_REG_SET",
	      create_array (create_scalar_type
			    ("unsigned long", strlen ("unsigned long")), "2"),
	      &pos);
