...
void
__objc_register_instance_methods_to_class (Class class)
{
  MethodList_t method_list;
...
sel_types_match (const char *t1, const char *t2)
{
  ...
  while (*t1 && *t2)
	...
	t1++;
      if (*t2 == '+')
	t2++;
      while (isdigit ((unsigned char) *t1))
	t1++;
      while (isdigit ((unsigned char) *t2))
	t2++;
...
sel_get_name (SEL selector)
{
  ...
  if ((soffset_decode ((sidx) selector->sel_id) > 0)
      && (soffset_decode ((sidx) selector->sel_id) <=
			   ...
			   struct objc_selector *orig, BOOL is_const)
{
  ...
  if (soffset_decode (i) != 0)
    {
      for (l =
	   (struct objc_list *) sarray_get_safe (__objc_selector_array, i); l;
	   l = l->tail)
	{
	  SEL s = (SEL) l->head;
	  if (types == 0 || s->sel_types == 0)
