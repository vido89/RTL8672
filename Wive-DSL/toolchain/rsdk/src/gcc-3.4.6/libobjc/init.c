...
*/
static Class
class_superclass_of_class (Class class)
{
...
create_tree_of_subclasses_inherited_from (Class bottom_class, Class upper)
{
  Class superclass = bottom_class->super_class ?
    objc_lookup_class ((char *) bottom_class->super_class) : Nil;
...
objc_preorder_traverse (objc_class_tree * tree,
			int level, void (*function) (objc_class_tree *, int))
{
...
__objc_exec_class (Module_t module)
{
  ...
  for (i = 0; i < symtab->cat_def_cnt; ++i)
      ...
      if (class)
	{
	  ...
	  DEBUG_PRINTF
	    ("processing categories from (module,object): %s, %s\n",
