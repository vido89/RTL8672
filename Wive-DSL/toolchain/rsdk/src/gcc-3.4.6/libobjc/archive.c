...
static int objc_read_class (struct objc_typed_stream *stream, Class * class);
			  ...
			  unsigned char *string, unsigned int nbytes)
{
  ...
  if ((key = PTR2LONG (hash_value_for_key (stream->stream_table, string))))
    ...
    {
      int length;
      hash_add (&stream->stream_table, LONG2PTR (key = PTR2LONG (string)),
		string);
...
objc_write_object (struct objc_typed_stream *stream, id object)
{
  ...
  if ((key = PTR2LONG (hash_value_for_key (stream->object_table, object))))
    ...
    {
      int length;
      hash_add (&stream->object_table, LONG2PTR (key = PTR2LONG (object)),
		object);
      if ((length = objc_write_register_common (stream, key)))
...
__inline__ int
__objc_write_class (struct objc_typed_stream *stream,
...
objc_write_class (struct objc_typed_stream *stream, struct objc_class *class)
{
  ...
  if ((key = PTR2LONG (hash_value_for_key (stream->stream_table, class))))
    ...
    {
      int length;
      hash_add (&stream->stream_table, LONG2PTR (key = PTR2LONG (class)),
		class);
      if ((length = objc_write_register_common (stream, key)))
...
objc_read_object (struct objc_typed_stream *stream, id * object)
{
  ...
  if ((len = (*stream->read) (stream->physical, buf, 1)))
      ...
      else if ((buf[0] & _B_CODE) == _B_UCOMM)
	{
	  ...
	  len = __objc_read_nbyte_ulong (stream, (buf[0] & _B_VALUE), &key);
	  (*object) =
	    hash_value_for_key (stream->object_table, LONG2PTR (key));
	}
      ...
      else if (buf[0] == (_B_EXT | _BX_OBJREF))	/* a forward reference */
	{
	  ...
	  other =
	    (struct objc_list *) hash_value_for_key (stream->object_refs,
						     LONG2PTR (key));
...
objc_read_selector (struct objc_typed_stream *stream, SEL * selector)
{
  ...
  if ((len = (*stream->read) (stream->physical, buf, 1)))
      ...
      if (buf[0] == (_B_EXT | _BX_SEL))	/* selector! */
	{
	  ...
	  /* register */
	  if (key)
	    hash_add (&stream->stream_table, LONG2PTR (key),
		      (void *) *selector);
	}
...
objc_write_types (TypedStream * stream, const char *type, ...)
{
  ...
  for (c = type; *c; c = objc_skip_typespec (c))
	...
	case _C_UINT:
	  res =
	    objc_write_unsigned_int (stream, *va_arg (args, unsigned int *));
	  break;
...
objc_read_types (TypedStream * stream, const char *type, ...)
{
  ...
  for (c = type; *c; c = objc_skip_typespec (c))
	...
	case _C_UCHR:
	  res =
	    objc_read_unsigned_char (stream, va_arg (args, unsigned char *));
	  break;
	...
	case _C_UINT:
	  res =
	    objc_read_unsigned_int (stream, va_arg (args, unsigned int *));
	  break;
	...
	case _C_ULNG:
	  res =
	    objc_read_unsigned_long (stream, va_arg (args, unsigned long *));
	  break;
...
static void
__objc_finish_write_root_object (struct objc_typed_stream *stream)
{
...
static void
__objc_finish_read_root_object (struct objc_typed_stream *stream)
{
  node_ptr node;
