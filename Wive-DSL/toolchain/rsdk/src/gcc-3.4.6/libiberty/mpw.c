...
warn_if_spin_delay (char *file, int line)
{
  ...
  if (diff > warning_threshold)
    fprintf (stderr,
	     ...
	     (current_progress ? current_progress : ""), diff / 1000000,
	     diff % 1000000, last_spin_file, last_spin_line, file, line);
  if (dump_spin_data)
    {
      if (diff >= 0)
	{
...
long
ReadCharsFromConsole (char *buf, long n)
{
#pragma unused (buf, n)
  return 0;
}
