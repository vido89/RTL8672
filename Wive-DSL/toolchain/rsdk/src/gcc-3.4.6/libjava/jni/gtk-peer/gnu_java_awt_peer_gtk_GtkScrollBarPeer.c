...
static void
post_change_event (GtkRange * range, struct range_scrollbar *rs)
{
  GtkAdjustment *adj;
  adj = gtk_range_get_adjustment (range);
  (*gdk_env)->CallVoidMethod (gdk_env, *(rs->scrollbar),
			      postAdjustmentEventID, AWT_ADJUSTMENT_TRACK,
			      (jint) adj->value);
		       ...
		       struct range_scrollbar *rs)
{
  ...
  (*gdk_env)->CallVoidMethod (gdk_env, *(rs->scrollbar),
			      postAdjustmentEventID, type, (jint) adj->value);
}
   ...
   jint min, jint max, jint step_incr, jint page_incr, jint visible_amount)
{
  ...
  adj = gtk_adjustment_new (value, min, max,
			    step_incr, page_incr, visible_amount);
  ...
  (JNIEnv * env, jobject obj)
{
  ...
  g_signal_connect (G_OBJECT (GTK_RANGE (ptr)),
		    "value-changed", GTK_SIGNAL_FUNC (post_change_event), rs);
