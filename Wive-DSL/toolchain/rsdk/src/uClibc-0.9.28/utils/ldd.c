...
foo:
	...
	if (ehdr->e_type != ET_EXEC && ehdr->e_type != ET_DYN) {
		printf("%s: not a dynamic executable\n", filename);
		return -1;
	}
	if (ehdr->e_type == ET_EXEC || ehdr->e_type == ET_DYN) {
...
int main( int argc, char** argv)
{
	...
	if (argc < 2) {
		printf("ldd: missing file arguments\n");
		printf("Try `ldd --help' for more information.\n");
		exit(EXIT_FAILURE);
	}
	if (argc > 2) {
		multi++;
