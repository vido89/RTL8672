     ...
     asymbol **psyms;
{
  ...
  for (; elsym < elsymend;
       elsym += bfd_xcoff_ldsymsz (abfd), symbuf++, psyms++)
    {
     ...
     bfd_boolean *pneeded;
{
  ...
  *pneeded = FALSE;
  ...
  if ((abfd->flags & DYNAMIC) != 0
      && !info->static_link && info->hash->creator == abfd->xvec)
     ...
     bfd_vma address;
{
  ...
  while (min > 0 && relocs[min - 1].r_vaddr == address)
    --min;
     ...
     struct bfd_link_info *info;
{
  ...
  if ((abfd->flags & DYNAMIC) != 0 && !info->static_link)
    {
  ...
  while (esym < esym_end)
      ...
      if (sym.n_sclass != C_EXT && sym.n_sclass != C_HIDEXT)
	{
	  ...
	  if (sym.n_sclass == C_FILE && csect != NULL)
		...
		((esym
		  - (bfd_byte *) obj_coff_external_syms (abfd)) / symesz);
	      csect = NULL;
	    }
	...
	goto error_return;
      ...
      /* If this symbol has line number information attached to it,
         ...
         garbage collection.  */
      if ((info->strip == strip_none || info->strip == strip_some)
	  && sym.n_numaux > 1 && csect != NULL && ISFCN (sym.n_type))
	{
	  ...
	  if (auxlin.x_sym.x_fcnary.x_fcn.x_lnnoptr != 0)
	      ...
	      if (enclosing == NULL)
		{
	      ...
	      if (linoff <
		  (bfd_signed_vma) (enclosing->lineno_count * linesz))
		{
		  ...
		  linpstart = (reloc_info[enclosing->target_index].linenos
			       + linoff);
		  bfd_coff_swap_lineno_in (abfd, (PTR) linpstart,
					   (PTR) & lin);
		  if (lin.l_lnno == 0
      ...
      if (sym.n_numaux == 0)
	{
      ...
      bfd_coff_swap_aux_in (abfd,
			    ...
			    sym.n_type, sym.n_sclass,
			    sym.n_numaux - 1, sym.n_numaux, (PTR) & aux);
	...
	case XTY_ER:
	  ...
	  if (sym.n_sclass == C_HIDEXT
	      || sym.n_scnum != N_UNDEF || aux.x_csect.x_scnlen.l != 0)
	    {
	      (*_bfd_error_handler)
	...
	case XTY_SD:
	  ...
	  if (aux.x_csect.x_smclas == XMC_TC0)
	    {
	      if (sym.n_sclass != C_HIDEXT || aux.x_csect.x_scnlen.l != 0)
		{
		  (*_bfd_error_handler)
	  ...
	  if (aux.x_csect.x_smclas == XMC_TC
		...
		goto error_return;
		  ...
		  && ((bfd_xcoff_is_xcoff32 (abfd)
		       && rel->r_size == 31)
		      || (bfd_xcoff_is_xcoff64 (abfd) && rel->r_size == 63)))
		{
		  bfd_byte *erelsym;
...

	  {
		...
		&& ((bfd_vma) sym.n_value < enclosing->vma
	      ...
	      {
		(*_bfd_error_handler)
		  (_("%B: csect `%s' not in enclosing section"), abfd, name);
		bfd_set_error (bfd_error_bad_value);
		goto error_return;
	      }
	    csect->vma = sym.n_value;
	    csect->filepos = (enclosing->filepos
			      + sym.n_value - enclosing->vma);
	...
	case XTY_LD:
	     ...
	     cause the XTY_LD to not follow the XTY_SD symbol. */
	  {
	    ...
	    if (bad)
	      {
		(*_bfd_error_handler)
		  (_("%B: misplaced XTY_LD `%s'"), abfd, name);
		bfd_set_error (bfd_error_bad_value);
		goto error_return;
	      }
	    csect = section;
	...
	case XTY_CM:
	  ...
	  && aux.x_csect.x_smclas != XMC_TC && aux.x_csect.x_smclas != XMC_TD)
	{
      ...
      if (sym.n_sclass == C_EXT)
	{
	  ...
	  if (info->hash->creator == abfd->xvec)
		...
		flag = XCOFF_DEF_REGULAR;
	      ...
	      if ((*sym_hash)->smclas == XMC_UA || flag == XCOFF_DEF_REGULAR)
  ...
  for (o = abfd->sections; o != first_csect; o = o->next)
      ...
      if ((o->flags & SEC_RELOC) != 0)
	{
	  ...
	  for (i = 0; i < o->reloc_count; i++, rel++, rel_csect++)
		...
		{
		  (*_bfd_error_handler)
		    (_("%B: reloc %s:%d not in csect"), abfd, o->name, i);
		  bfd_set_error (bfd_error_bad_value);
		  goto error_return;
		}
		  ...
		  && obj_xcoff_sym_hashes (abfd)[rel->r_symndx] != NULL)
		{
		  ...
		  if (h->root.root.string[0] == '.' && h->descriptor == NULL)
		    {
     ...
     struct bfd_link_info *info;
{
  ...
  for (; elsym < elsymend; elsym += bfd_xcoff_ldsymsz (abfd))
	      ...
	      || h->root.type == bfd_link_hash_undefweak))
	{
      ...
      if (h->smclas == XMC_DS || (h->smclas == XMC_XO && name[0] != '.'))
     ...
     struct xcoff_link_hash_entry *h;
{
  ...
  if (h->root.type == bfd_link_hash_defined
      || h->root.type == bfd_link_hash_defweak)
    {
      ...
      if (!bfd_is_abs_section (hsec) && (hsec->flags & SEC_MARK) == 0)
	{
  ...
  if (h->toc_section != NULL && (h->toc_section->flags & SEC_MARK) == 0)
    {
...
xcoff_mark (info, sec)
     ...
     asection *sec;
{
  if (bfd_is_abs_section (sec) || (sec->flags & SEC_MARK) != 0)
    return TRUE;
  ...
  if (sec->owner->xvec == info->hash->creator
      ...
      for (; hp < hpend; hp++)
	{
	  ...
	  h = *hp;
	  if (h != NULL && (h->flags & XCOFF_MARK) == 0)
	    {
      ...
      if ((sec->flags & SEC_RELOC) != 0 && sec->reloc_count > 0)
	{
	  ...
	  for (; rel < relend; rel++)
	      ...
	      if (h != NULL && (h->flags & XCOFF_MARK) == 0)
		{
	      ...
	      if (rsec != NULL && (rsec->flags & SEC_MARK) == 0)
		{
     ...
     unsigned int syscall_flag;
{
  ...
  if (h->root.root.string[0] == '.'
      && h->root.type == bfd_link_hash_undefined && val == (bfd_vma) - 1)
    {
      struct xcoff_link_hash_entry *hds;
     ...
     struct bfd_link_hash_entry *harg;
{
  ...
  /* See if this is a function descriptor.  It may be one even though
     it is not so marked.  */
  if ((h->flags & XCOFF_DESCRIPTOR) == 0 && h->root.root.string[0] != '.')
    {
      char *fnname;
  ...
  if (info->init_function || info->fini_function || rtld)
    {
      ...
      if (hsym == NULL)
	{
	  (*_bfd_error_handler) (_("error: undefined symbol __rtinit"));
	  return FALSE;
	}
  ...
  for (i = 0; i < XCOFF_NUMBER_OF_SPECIAL_SECTIONS; i++)
    {
      ...
      if (sec != NULL && gc && (sec->flags & SEC_MARK) == 0)
	{
	  sec = NULL;
	}
  ...
  for (fl = xcoff_hash_table (info)->imports; fl != NULL; fl = fl->next)
    {
      ...
      impsize += (strlen (fl->path)
		  + strlen (fl->file) + strlen (fl->member) + 3);
    }
  ...
  *out++ = '\0';
  ...
  for (sub = info->input_bfds; sub != NULL; sub = sub->link_next)
	...
	continue;
	  ...
	  || info->strip == strip_debugger || info->discard == discard_all)
	{
	  subdeb->size = 0;
	  continue;
	}
     ...
     PTR p;
{
  ...
  if (ldinfo->export_defineds
      && (h->flags & XCOFF_DEF_REGULAR) != 0 && h->root.root.string[0] != '.')
    {
      bfd_boolean export;
  ...
  if (((h->flags & XCOFF_LDREL) == 0
       ...
       || h->root.type == bfd_link_hash_defweak
       || h->root.type == bfd_link_hash_common)
      && (h->flags & XCOFF_ENTRY) == 0 && (h->flags & XCOFF_EXPORT) == 0)
    {
      h->ldsym = NULL;
      return TRUE;
    }
  ...
  if (xcoff_hash_table (ldinfo->info)->gc && (h->flags & XCOFF_MARK) == 0)
    {
      h->ldsym = NULL;
      return TRUE;
    }
     ...
     struct bfd_link_info *info;
{
	  ...
	  for (o = abfd->sections; o != NULL; o = o->next)
	      ...
	      else
		{
		  if ((o->flags & SEC_HAS_CONTENTS) != 0)
		    sofar += BFD_ALIGN (o->size, 1 << o->alignment_power);
		}
	    }
	}
  ...
  xcoff_link_hash_traverse (xcoff_hash_table (info),
			    xcoff_write_global_symbol, (PTR) & finfo);
  ...
  for (o = abfd->sections; o != NULL; o = o->next)
      ...
      for (; irel < irelend; irel++, rel_hash++, erel += relsz)
	{
      ...
      for (toc_rel_hash = finfo.section_info[o->target_index].toc_rel_hashes;
	   toc_rel_hash != NULL; toc_rel_hash = toc_rel_hash->next)
	{
  ...
  if (bfd_seek (abfd, pos, SEEK_SET) != 0)
    goto error_return;
  H_PUT_32 (abfd,
	    _bfd_stringtab_size (finfo.strtab) + STRING_SIZE_SIZE, strbuf);
     ...
     bfd *input_bfd;
{
  ...
  if ((input_bfd->flags & DYNAMIC) != 0 && !finfo->info->static_link)
    return TRUE;
...
#define N_BTSHFT n_btshft
  ...
  while (esym < esym_end)
	...
	skip = TRUE;
      ...
      if (!skip && isymp->n_sclass == C_STAT)
	skip = TRUE;

      /* We skip all but the first TOC anchor.  */
      if (!skip
	  && isymp->n_sclass == C_HIDEXT && aux.x_csect.x_smclas == XMC_TC0)
	{
      ...
      if (!skip && finfo->info->strip == strip_all)
	skip = TRUE;
	...
	skip = TRUE;
      ...
      /* If we stripping debugging symbols, and this is a debugging
         symbol, then skip it.  */
      if (!skip
	  && finfo->info->strip == strip_debugger && isym.n_scnum == N_DEBUG)
	skip = TRUE;
	  ...
	  && (debug_index == NULL || *debug_index == (unsigned long) -1))
	{
      ...
      if (skip && require && finfo->info->strip != strip_all)
	skip = FALSE;
      ...
      if (!skip)
	{
	  ...
	  if (isym._n._n_n._n_zeroes == 0 && isym._n._n_n._n_offset != 0)
	    {
	  ...
	  if (isym.n_sclass != C_BSTAT
	      ...
	      && isym.n_sclass != C_DECL && isym.n_scnum > 0)
	    {
	      isym.n_scnum = (*csectpp)->output_section->target_index;
	      isym.n_value += ((*csectpp)->output_section->vma
			       + (*csectpp)->output_offset - (*csectpp)->vma);
	    }
	  ...
	  if (isym.n_sclass == C_BINCL || isym.n_sclass == C_EINCL)
	    {
	  ...
	  *indexp = output_index;
	  ...
	  if (isym.n_sclass == C_HIDEXT
	      && aux.x_csect.x_smclas == XMC_TC && *sym_hash != NULL)
	    {
		  ...
		  && aux.x_sym.x_fcnary.x_fcn.x_lnnoptr != 0)
		{
		  if (finfo->info->strip != strip_none
		      ...
		      else
			{
			  ...
			  linpend = (finfo->linenos + enc_count * linesz);
			  offset = (o->output_section->vma
				    + o->output_offset - o->vma);
  ...
  for (o = input_bfd->sections; o != NULL; o = o->next)
      ...
      if (!o->linker_mark)
	{
      ...
      if ((o->flags & SEC_HAS_CONTENTS) == 0
	  || o->size == 0 || (o->flags & SEC_IN_MEMORY) != 0)
	continue;
      ...
      else
	{
	  ...
	  if (!bfd_get_section_contents
	      (input_bfd, o, finfo->contents, 0, sz))
	    return FALSE;
	  ...
	  for (; irel < irelend; irel++, rel_hash++)
	      ...
	      if (r_symndx != -1 && finfo->info->strip != strip_all)
		{
		  if (h != NULL
			  ...
			  || irel->r_type == R_GL
			  || irel->r_type == R_TCL
			  || irel->r_type == R_TRL || irel->r_type == R_TRLA))
		    {
		      ...
		      if (indx == -1)
			{
			  ...
			  if (is->n_sclass == C_HIDEXT && is->n_numaux > 0)
			    {
			      PTR auxptr;
			      union internal_auxent aux;
     ...
     PTR inf;
{
  ...
  if (xcoff_hash_table (finfo->info)->gc && (h->flags & XCOFF_MARK) == 0)
    return TRUE;
  ...
  if (h->ldsym != NULL)
	       ...
	       || h->root.type == bfd_link_hash_defweak)
	{
	  ...
	  ldsym->l_value = (sec->output_section->vma
			    + sec->output_offset + h->root.u.def.value);
  ...
  if (h->root.type == bfd_link_hash_defined
      ...
      if ((h->descriptor->flags & XCOFF_SET_TOC) != 0)
	{
      ...
      bfd_put_32 (output_bfd,
		  bfd_xcoff_glink_code (output_bfd, 0) | (tocoff & 0xffff),
		  p);
  ...
  if ((h->flags & XCOFF_SET_TOC) != 0)
    {
      ...
      irel->r_vaddr = (osec->vma + tocsec->output_offset + h->u.toc_offset);
  ...
  if ((h->flags & XCOFF_DESCRIPTOR) != 0
      ...
      else
	{
      ...
      irel = finfo->section_info[oindx].relocs + osec->reloc_count;
      irel->r_vaddr = (osec->vma + sec->output_offset + h->root.u.def.value);
      ...
      else
	{
	  (*_bfd_error_handler)
	    (_("%s: loader reloc in unrecognized section `%s'"),
	     bfd_get_filename (output_bfd), esec->output_section->name);
	  bfd_set_error (bfd_error_nonrepresentable_section);
	  return FALSE;
	}
      ...
      else
	{
	  (*_bfd_error_handler)
	    (_("%s: loader reloc in unrecognized section `%s'"),
	     bfd_get_filename (output_bfd), tsec->output_section->name);
	  bfd_set_error (bfd_error_nonrepresentable_section);
	  return FALSE;
	}
      ldrel.l_rtype = (reloc_size << 8) | R_POS;
  ...
  if (h->root.type == bfd_link_hash_undefined
      || h->root.type == bfd_link_hash_undefweak)
    {
      ...
      aux.x_csect.x_smtyp = XTY_ER;
	    ...
	    || h->root.type == bfd_link_hash_defweak) && h->smclas == XMC_XO)
    {
     ...
     struct bfd_link_order *link_order;
{
  ...
  if (hsec != NULL)
    addend += (hsec->output_section->vma + hsec->output_offset + hval);
