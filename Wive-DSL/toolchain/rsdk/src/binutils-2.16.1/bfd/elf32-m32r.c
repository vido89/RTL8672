...
void _bfd_m32r_elf_symbol_processing PARAMS ((bfd *, asymbol *));
...
static bfd_boolean m32r_elf_object_p PARAMS ((bfd *));
static void m32r_elf_final_write_processing PARAMS ((bfd *, bfd_boolean));
static bfd_boolean m32r_elf_set_private_flags PARAMS ((bfd *, flagword));
static bfd_boolean m32r_elf_merge_private_bfd_data PARAMS ((bfd *, bfd *));
static bfd_boolean m32r_elf_print_private_bfd_data PARAMS ((bfd *, PTR));
     ...
     bfd_vma addend;
{
  ...
  x =
    (x & ~howto->dst_mask) | (((x & howto->src_mask) + relocation) & howto->
			      dst_mask);
     ...
     bfd_vma addend;
{
  ...
  if (type == R_M32R_HI16_SLO && (addend & 0x8000) != 0)
    addend += 0x10000;
     ...
     char **error_message;
{
  ...
  if (m32r_hi16_list != NULL)
      ...
      while (l != NULL)
	{
	  ...
	  vallo =
	    ((bfd_get_32 (input_bfd, (bfd_byte *) data + reloc_entry->address)
     ...
     bfd_reloc_code_real_type code;
{
...
#ifdef USE_M32R_OLD_RELOC
  for (i = 0;
       i < sizeof (m32r_reloc_map_old) / sizeof (struct m32r_reloc_map); i++)
    {
  ...
  for (i = 0;
       i < sizeof (m32r_reloc_map) / sizeof (struct m32r_reloc_map); i++)
    {
     ...
     bfd_vma *valp;
{
  if (!info->relocatable
      && (*namep)[0] == '_' && (*namep)[1] == 'S'
      && strcmp (*namep, "_SDA_BASE_") == 0 && is_elf_hash_table (info->hash))
    {
...
static struct bfd_hash_entry *m32r_elf_link_hash_newfunc (struct
							  bfd_hash_entry *,
							  struct
							  bfd_hash_table *,
							  const char *);
  ...
  if (bed->want_dynbss)
    {
      ...
      htab->sdynbss = s;
      if (s == NULL || !bfd_set_section_flags (abfd, s, SEC_ALLOC))
	return FALSE;
  ...
  if (h->plt.offset != (bfd_vma) - 1)
      ...
      if (!info->shared)
	{
	  ...
	  bfd_put_32 (output_bfd,
		      (PLT_ENTRY_WORD4
		       +
		       (((unsigned int) ((-(h->plt.offset + 16)) >> 2)) &
			0xffffff)), splt->contents + h->plt.offset + 16);
	}
      else
	{
	  bfd_put_32 (output_bfd,
      ...
      bfd_put_32 (output_bfd, (splt->output_section->vma + splt->output_offset + h->plt.offset + 12),	/* same offset */
     ...
     bfd *abfd;
{
  switch (elf_elfheader (abfd)->e_flags & EF_M32R_ARCH)
    ...
    default:
    case E_M32R_ARCH:
      (void) bfd_default_set_arch_mach (abfd, bfd_arch_m32r, bfd_mach_m32r);
      break;
    case E_M32RX_ARCH:
      (void) bfd_default_set_arch_mach (abfd, bfd_arch_m32r, bfd_mach_m32rx);
      break;
    case E_M32R2_ARCH:
      (void) bfd_default_set_arch_mach (abfd, bfd_arch_m32r, bfd_mach_m32r2);
      break;
    }
  return TRUE;
}
     ...
     PTR ptr;
{
  ...
  switch (elf_elfheader (abfd)->e_flags & EF_M32R_ARCH)
    ...
    default:
    case E_M32R_ARCH:
      fprintf (file, _(": m32r instructions"));
      break;
    case E_M32RX_ARCH:
      fprintf (file, _(": m32rx instructions"));
      break;
    case E_M32R2_ARCH:
      fprintf (file, _(": m32r2 instructions"));
      break;
    }
	...
	case R_M32R_26_PCREL_RELA:
	  ...
	  if ((info->shared
	      ...
	      if (p == NULL || p->sec != sec)
		{
		  ...
		  p =
		    ((struct elf_m32r_dyn_relocs *) bfd_alloc (dynobj, amt));
		  if (p == NULL)
		    return FALSE;
		  p->next = *head;
		  *head = p;
