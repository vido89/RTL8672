...
static void ppcboot_get_symbol_info
...
ppcboot_get_section_contents (abfd, section, location, offset, count)
     ...
     bfd_size_type count;
{
  if (bfd_seek (abfd, offset + (file_ptr) sizeof (ppcboot_hdr_t), SEEK_SET) !=
      0 || bfd_bread (location, count, abfd) != count)
    return FALSE;
  return TRUE;
}

     ...
     char *suffix;
{
  ...
  size = (strlen (bfd_get_filename (abfd))
	  + strlen (suffix) + sizeof "_ppcboot__");
     ...
     PTR farg;
{
  ...
  for (i = 0; i < 4; i++)
    {
      long sector_begin =
	bfd_getl_signed_32 ((PTR) tdata->header.partition[i].sector_begin);
      long sector_length =
	bfd_getl_signed_32 ((PTR) tdata->header.partition[i].sector_length);
	...
	continue;
      ...
      fprintf (f,
	       ...
	       i, tdata->header.partition[i].partition_begin.ind,
	       tdata->header.partition[i].partition_begin.head,
	       tdata->header.partition[i].partition_begin.sector,
	       tdata->header.partition[i].partition_begin.cylinder);
      ...
      fprintf (f,
	       ...
	       i, tdata->header.partition[i].partition_end.ind,
	       tdata->header.partition[i].partition_end.head,
	       tdata->header.partition[i].partition_end.sector,
	       tdata->header.partition[i].partition_end.cylinder);

      fprintf (f, _("Partition[%d] sector = 0x%.8lx (%ld)\n"), i,
	       sector_begin, sector_begin);
