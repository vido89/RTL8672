     ...
     arelent *rptr;
{
  ...
  if (!intern->r_extern
      && (intern->r_type == MIPS_R_GPREL || intern->r_type == MIPS_R_LITERAL))
     ...
     char **error_message;
{
  ...
  if (gp == 0 && (!relocatable || (symbol->flags & BSF_SECTION_SYM) != 0))
    {
      if (relocatable)
	{
	  /* Make up a value.  */
  ...
  for (i = 0; ext_rel < ext_rel_end; ext_rel++, i++)
      ...
      if (int_rel.r_type == MIPS_R_REFHI)
	{
	  ...
	  for (lo_ext_rel = ext_rel + 1;
	       lo_ext_rel < ext_rel_end; lo_ext_rel++)
	    {
      ...
      else
	{
      ...
      /* The GPREL reloc uses an addend: the difference in the GP
         values.  */
      if (int_rel.r_type != MIPS_R_GPREL && int_rel.r_type != MIPS_R_LITERAL)
	addend = 0;
      else
	{
	  if (gp_undefined)
      ...
      if (info->relocatable)
	{
	  ...
	  if (int_rel.r_extern)
		  ...
		  && !bfd_is_abs_section (h->root.u.def.section))
		{
		  ...
		  name = bfd_get_section_name (output_bfd, s->output_section);
	  ...
	  && (((relocation
		...
		+ (int_rel.r_extern ? 0 : s->vma))
	      ...
	      != ((input_section->output_section->vma
		   + input_section->output_offset
		   + (int_rel.r_vaddr - input_section->vma)) & 0xf0000000)))
