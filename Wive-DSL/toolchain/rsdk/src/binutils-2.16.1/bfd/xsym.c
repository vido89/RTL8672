     ...
     const char *name;
{
  fprintf (f, "%-6s %13ld %13ld %13ld\n",
	   name,
	   dti->dti_first_page, dti->dti_page_count, dti->dti_object_count);
}
     ...
     unsigned long index;
{
  ...
  offset = compute_offset (sdata->header.dshb_rte.dti_first_page,
			   sdata->header.dshb_page_size, entry_size, index);
     ...
     enum bfd_sym_storage_kind kind;
{
  switch (kind)
    ...
    case BFD_SYM_STORAGE_KIND_VALUE:
      return "VALUE";
    case BFD_SYM_STORAGE_KIND_REFERENCE:
      return "REFERENCE";
    case BFD_SYM_STORAGE_KIND_WITH:
      return "WITH";
    default:
      return "[UNKNOWN]";
    }
     ...
     enum bfd_sym_storage_class kind;
{
  switch (kind)
    ...
    case BFD_SYM_STORAGE_CLASS_ABSOLUTE:
      return "ABSOLUTE";
    case BFD_SYM_STORAGE_CLASS_CONSTANT:
      return "CONSTANT";
    case BFD_SYM_STORAGE_CLASS_RESOURCE:
      return "RESOURCE";
    case BFD_SYM_STORAGE_CLASS_BIGCONSTANT:
      return "BIGCONSTANT";
    default:
      return "[UNKNOWN]";
    }
     ...
     enum bfd_sym_module_kind kind;
{
  switch (kind)
    {
    case BFD_SYM_MODULE_KIND_NONE:
      return "NONE";
    case BFD_SYM_MODULE_KIND_PROGRAM:
      return "PROGRAM";
    case BFD_SYM_MODULE_KIND_UNIT:
      return "UNIT";
    case BFD_SYM_MODULE_KIND_PROCEDURE:
      return "PROCEDURE";
    case BFD_SYM_MODULE_KIND_FUNCTION:
      return "FUNCTION";
    case BFD_SYM_MODULE_KIND_DATA:
      return "DATA";
    case BFD_SYM_MODULE_KIND_BLOCK:
      return "BLOCK";
    default:
      return "[UNKNOWN]";
    }
     ...
     enum bfd_sym_symbol_scope scope;
{
  switch (scope)
    {
    case BFD_SYM_SYMBOL_SCOPE_LOCAL:
      return "LOCAL";
    case BFD_SYM_SYMBOL_SCOPE_GLOBAL:
      return "GLOBAL";
    default:
      return "[UNKNOWN]";
    }
  ...
  switch (entry->generic.type)
    ...
    default:
      fprintf (f, "\"%.*s\" (MTE %lu), offset %lu",
	       ...
	       entry->entry.mte_index, entry->entry.file_offset);
      break;
    }
}
  ...
  switch (entry->generic.type)
    ...
    default:
      fprintf (f, "\"%.*s\" (MTE %lu, NTE %lu)",
	       bfd_sym_module_name (abfd, entry->entry.mte_index)[0],
	       &bfd_sym_module_name (abfd, entry->entry.mte_index)[1],
	       entry->entry.mte_index, entry->entry.nte_index);
      break;
    }
}
     ...
     bfd_sym_contained_variables_table_entry *entry;
{
  ...
  if (entry->entry.la_size == BFD_SYM_CVTE_SCA)
    fprintf (f, ", latype %s, laclass %s, laoffset %lu",
	     bfd_sym_unparse_storage_kind (entry->entry.address.scstruct.
					   sca_kind),
	     bfd_sym_unparse_storage_class (entry->entry.address.scstruct.
					    sca_class),
     ...
     bfd_sym_contained_statements_table_entry *entry;
{
  ...
  fprintf (f, "\"%.*s\" (MTE %lu), offset %lu, delta %lu",
	   ...
	   entry->entry.mte_offset, entry->entry.file_delta);
}
     ...
     unsigned char num;
{
  switch (num)
    ...
    case 3:
      return "signed long";
    case 4:
      return "extended (10 bytes)";
    case 5:
      return "pascal boolean (1 byte)";
    case 6:
      return "unsigned byte";
    case 7:
      return "signed byte";
    case 8:
      return "character (1 byte)";
    case 9:
      return "wide character (2 bytes)";
    case 10:
    ...
    case 13:
      return "double";
    case 14:
      return "extended (12 bytes)";
    case 15:
      return "computational (8 bytes)";
    case 16:
      return "c string";
    case 17:
      return "as-is string";
     ...
     unsigned long *offsetptr;
{
  ...
  switch (type & 0x3f)
	...
	if (value <= 0)
	  ...
	  {
	    if (bfd_sym_fetch_type_table_information (abfd, &tinfo, value) <
		0)
	      fprintf (f, "[INVALID]");
	    else
	      fprintf (f, "\"%.*s\"",
     ...
     bfd_sym_type_information_table_entry *entry;
{
  ...
  bfd_sym_print_type_information (abfd, f, buf, entry->physical_size, 0,
				  &offset);
     ...
     unsigned char *entry;
{
  ...
  if (sdata->version >= BFD_SYM_VERSION_3_4 && entry[0] == 255
      && entry[1] == 0)
    {
     ...
     FILE *f;
{
  ...
  name_table_len =
    sdata->header.dshb_nte.dti_page_count * sdata->header.dshb_page_size;
     ...
     FILE *f;
{
  ...
  for (i = 1; i <= sdata->header.dshb_csnte.dti_object_count; i++)
    {
      if (bfd_sym_fetch_contained_statements_table_entry (abfd, &entry, i) <
	  0)
	fprintf (f, " [%8lu] [INVALID]\n", i);
      else
	{
     ...
     FILE *f;
{
  ...
  for (i = 1; i <= sdata->header.dshb_fite.dti_object_count; i++)
    {
      if (bfd_sym_fetch_file_references_index_table_entry (abfd, &entry, i) <
	  0)
	fprintf (f, " [%8lu] [INVALID]\n", i);
      else
	{
     ...
     FILE *f;
{
  ...
  for (i = 100; i <= sdata->header.dshb_tte.dti_object_count; i++)
      ...
      else
	{
	  ...
	  if (bfd_sym_fetch_type_information_table_entry (abfd, &entry, index)
	      < 0)
	    fprintf (f, "[INVALID]");
	  else
	    bfd_sym_print_type_information_table_entry (abfd, f, &entry);
     ...
     bfd *abfd;
{
  ...
  if (preserve.marker == NULL || !bfd_preserve_save (abfd, &preserve))
    goto fail;
...
const bfd_target sym_vec = {
  ...
  (SEC_ALLOC | SEC_LOAD | SEC_READONLY | SEC_CODE | SEC_DATA | SEC_ROM | SEC_HAS_CONTENTS),	/* section_flags */
