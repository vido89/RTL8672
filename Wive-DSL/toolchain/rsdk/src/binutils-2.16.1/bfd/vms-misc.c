     ...
     bfd *abfd;
{
  ...
  if (PRIV (file_format) == FF_UNKNOWN)
    {				/* record length repeats ? */
      if (vms_buf[0] == vms_buf[4] && vms_buf[1] == vms_buf[5])
	{
  ...
  if (PRIV (stackptr) >= STACKSIZE)
    {
      ...
      (*_bfd_error_handler) (_("Stack overflow (%d) in _bfd_vms_push"),
			     PRIV (stackptr));
      exit (1);
    }
  return;
...
#if VMS_DEBUG
  vms_debug (6,
	     "_bfd_vms_output_flush(real_size = %d, pushed_size %d at lenpos %d)\n",
     ...
     int maxlen;
{
  ...
  if ((in_len > maxlen) && PRIV (flag_hash_long_names))
  ...
  if (in_len > maxlen
      && PRIV (flag_hash_long_names) && PRIV (flag_show_after_trunc))
