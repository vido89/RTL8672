...
#ifdef OLDFORMAT
static bfd_boolean nlm_powerpc_backend_object_p PARAMS ((bfd *));
static bfd_boolean nlm_powerpc_write_prefix PARAMS ((bfd *));
#endif
...
PARAMS ((bfd *, nlmNAME (symbol_type) *));
static bfd_vma nlm_powerpc_get_public_offset PARAMS ((bfd *, asymbol *));
#endif

#ifdef OLDFORMAT
...
static reloc_howto_type nlm_powerpc_howto = HOWTO (0,	/* type */
						   0,	/* rightshift */
     ...
     int indx;
{
  ...
  for (howto = nlm_powerpc_howto_table;
       howto < nlm_powerpc_howto_table + HOWTO_COUNT; howto++)
    {
     ...
     struct reloc_and_sec *relocs;
{
  ...
  for (i = 0; i < count; i++)
	...
	return FALSE;
#else
      if (!nlm_powerpc_write_reloc (abfd, relocs[i].sec, relocs[i].rel, indx))
	return FALSE;
#endif
    }
  ...
  if (sym->symbol.value & NLM_HIBIT)
    {
      ...
      sym->symbol.section = bfd_get_section_by_name (abfd, NLM_CODE_NAME);
    }
  else
    {
      sym->symbol.section =
