...
static bfd_boolean elf64_alpha_mkobject PARAMS ((bfd *));
static bfd_boolean elf64_alpha_object_p PARAMS ((bfd *));
...
static bfd_boolean elf64_alpha_can_merge_gots PARAMS ((bfd *, bfd *));
static void elf64_alpha_merge_gots PARAMS ((bfd *, bfd *));
...
PARAMS ((bfd *, struct bfd_link_info *));
static int alpha_dynamic_entries_for_reloc PARAMS ((int, int, int));
     ...
     bfd_byte *p_lda;
{
  ...
  if (((i_ldah >> 26) & 0x3f) != 0x09 || ((i_lda >> 26) & 0x3f) != 0x08)
     ...
     Elf_Internal_Rela *irel;
{
  ...
  if (lit_insn >> 26 != OP_LDQ)
	...
	info->abfd, info->sec, (unsigned long) irel->r_offset));
      return TRUE;
    }
	...
	case LITUSE_ALPHA_TLSLDM:
	  {
	    ...
	    org = (info->sec->output_section->vma
		   + info->sec->output_offset + urel->r_offset + 4);
     ...
     unsigned long r_type;
{
  ...
  if (insn >> 26 != OP_LDQ)
	...
	info->abfd, info->sec, (unsigned long) irel->r_offset, howto->name));
      return TRUE;
    }
     ...
     bfd_boolean is_gd;
{
  ...
  if (ELF64_R_TYPE (irel[1].r_info) != R_ALPHA_LITERAL
      ...
      || irel[2].r_addend !=
      (is_gd ? LITUSE_ALPHA_TLSGD : LITUSE_ALPHA_TLSLDM))
    return TRUE;
	...
	if (disp >= -0x8000 && disp < 0x8000)
	  {
	    ...
	    break;
		 ...
		 && disp < (bfd_signed_vma) 0x7fff8000 && !pos1_unusable)
	  {
     ...
     bfd_boolean *again;
{
  ...
  *again = FALSE;
  ...
  if (link_info->relocatable
	  ...
	  != (SEC_CODE | SEC_RELOC | SEC_ALLOC)) || sec->reloc_count == 0)
    return TRUE;
     ...
     bfd_vma r_addend;
{
  ...
  for (gotent = *slot; gotent; gotent = gotent->next)
	...
	&& gotent->reloc_type == r_type && gotent->addend == r_addend)
      break;
     ...
     const Elf_Internal_Rela *relocs;
{
  ...
  for (rel = relocs; rel < relend; ++rel)
	...
	case R_ALPHA_LITERAL:
	  ...
	  while (++rel < relend
		 && ELF64_R_TYPE (rel->r_info) == R_ALPHA_LITUSE)
      ...
      if (need & NEED_DYNREL)
	{
	  ...
	  if (h)
	      ...
	      if (!rent)
		{
		  ...
		  rent =
		    (struct alpha_elf_reloc_entry *) bfd_alloc (abfd, amt);
		  if (!rent)
		    return FALSE;
     ...
     struct elf_link_hash_entry *h;
{
  ...
  if (alpha_elf_dynamic_symbol_p (h, info)
	...
	s->size = PLT_HEADER_SIZE;
      ...
      if (!info->shared && h->root.type != bfd_link_hash_defweak)
	{
	...
	if (td == NULL)
	  {
	    _bfd_error_handler (_
				("Symbol %s has no GOT subsection for offset 0x%x"),
     ...
     PTR data;
{
  ...
  for (gotent = h->got_entries; gotent; gotent = gotent->next)
    if (gotent->reloc_type == R_ALPHA_LITERAL && gotent->use_count > 0)
      break;
     ...
     struct bfd_link_info *info;
{
  ...
  alpha_elf_link_hash_traverse (alpha_elf_hash_table (info),
				elf64_alpha_merge_ind_symbols, NULL);
...
static void
elf64_alpha_emit_dynrel (abfd, info, sec, srel, offset, dynindx, rtype,
			 addend)
     bfd *abfd;
     struct bfd_link_info *info;
     asection *sec, *srel;
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (r_symndx == 0
		  ...
		  || r_type == R_ALPHA_TPRELLO || r_type == R_ALPHA_TPREL16))
	    value = tp_base;
      ...
      else
	{
	  ...
	  RELOC_FOR_GLOBAL_SYMBOL (info, input_bfd, input_section, rel,
				   ...
				   hh, sec, value, unresolved_reloc, warned);
	...
	if (gotent->gotobj == gotobj
	    && gotent->reloc_type == r_type && gotent->addend == addend)
	  break;
	...
	case R_ALPHA_LITERAL:
	  ...
	  value = (sgot->output_section->vma
		   + sgot->output_offset + gotent->got_offset);
	  value -= gp;
	  goto default_reloc;
	  ...
	  if (!gotent->reloc_done)
	      ...
	      else
		{
		  ...
		  if (r_type == R_ALPHA_GOTDTPREL)
		    ...
		    {
		      elf64_alpha_emit_dynrel (output_bfd, info, sgot,
					       srelgot, gotent->got_offset, 0,
					       R_ALPHA_TPREL64,
					       value - dtp_base);
		      value = 0;
		    }
     ...
     struct bfd_link_info *info;
{
  ...
  for (o = abfd->sections; o != (asection *) NULL; o = o->next)
      ...
      if (strcmp (o->name, ".mdebug") == 0)
	{
	  ...
	  elf_link_hash_traverse (elf_hash_table (info),
				  elf64_alpha_output_extsym, (PTR) & einfo);
	  if (einfo.failed)
	    return FALSE;
  ...
  /* The .got subsections...  */
  {
    ...
    for (i = alpha_elf_hash_table (info)->got_list;
	 i != NULL; i = alpha_elf_tdata (i)->got_link_next)
      {
	asection *sgot;
...
/* ECOFF swapping routines.  These are used when dealing with the
   ...
   from elf32-mips.c.  */
static const struct ecoff_debug_swap elf64_alpha_ecoff_debug_swap = {
