...
static char valid_mmo_symbol_character_set[	/* A-Z a-z (we assume consecutive
...
static void
mmo_find_sec_w_addr_grow (bfd * abfd ATTRIBUTE_UNUSED, asection * sec,
			  void *p)
{
  struct mmo_find_sec_info *infop = (struct mmo_find_sec_info *) p;
  bfd_vma vma = bfd_get_section_vma (abfd, sec);
  ...
  if (abfd->tdata.mmo_data->byte_no != 0)
    {
      ...
      mmo_write_tetra (abfd, bfd_get_32 (abfd, abfd->tdata.mmo_data->buf));
      abfd->tdata.mmo_data->byte_no = 0;
    }
...
| 81    The serial number is 1.  */
  ...
  if (m & MMO3_SYMBITS)
      ...
      if (m & MMO3_WCHAR)
	{
      ...
      abfd->tdata.mmo_data->lop_stab_symbol[abfd->tdata.mmo_data->
					    symbol_position++] = c;
      abfd->tdata.mmo_data->lop_stab_symbol[abfd->tdata.mmo_data->
					    symbol_position] = 0;
...
mmo_get_loc (asection * sec, bfd_vma vma, int size)
{
  ...
  for (; datap != NULL; datap = datap->next)
    {
      if (datap->where <= vma && datap->where + datap->size >= vma + size)
  ...
  allocated_size
    =
    (size + MMO_SEC_CONTENTS_CHUNK_SIZE - 1) & ~(MMO_SEC_CONTENTS_CHUNK_SIZE -
						 1);
  entry =
    (mmo_data_list_type *) bfd_zalloc (sec->owner,
		  ...
		  if (fname == NULL)
		    {
		      (*_bfd_error_handler)
			(_
			 ("%s: cannot allocate file name for file number %d, %d bytes\n"),
...
mmo_write_symbols_and_terminator (bfd * abfd)
{
  ...
  for (i = 0, serno = 1; i < count && table[i] != NULL; i++)
		     ...
		     valid_mmo_symbol_character_set) == strlen (s->name))
	{
	  ...
	  sym.value =
	    s->value
	    + s->section->output_section->vma + s->section->output_offset;
...
mmo_write_object_contents (bfd * abfd)
{
	  ...
	  if (sec->size == 0)
	    ...
	    (*_bfd_error_handler)
	      (_("%s: too many initialized registers; section length %ld\n"),
	       bfd_get_filename (abfd), (long) sec->size);
	  else
	    (*_bfd_error_handler)
  ...
  else if (!mmo_internal_write_post (abfd, 255, NULL))
    return FALSE;
