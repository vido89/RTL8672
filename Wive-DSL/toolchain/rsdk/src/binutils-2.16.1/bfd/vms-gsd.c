...
static struct sec_flags_struct vax_section_flags[] = {
  ...
  {VAX_ADDRESS_DATA_NAME,
   ...
   (GPS_S_M_PIC | GPS_S_M_REL | GPS_S_M_RD),
   (SEC_IN_MEMORY | SEC_DATA | SEC_HAS_CONTENTS | SEC_ALLOC | SEC_READONLY |
    SEC_LOAD)},
  {NULL,
...
static flagword vms_secflag_by_name
PARAMS ((bfd *, struct sec_flags_struct *, char *, int));
static flagword vms_esecflag_by_name
PARAMS ((struct sec_flags_struct *, char *, int));
	...
	case GSD_S_C_PSC:
	  {
		...
		symbol->flags =
		  (BSF_GLOBAL | BSF_SECTION_SYM | BSF_OLD_COMMON);
	      }
	    ...
	    vms_debug (4, "gsd psc %d (%s, flags %04x=%s) ",
		       section->index, name, old_flags, flag2str (gpsflagdesc,
								  old_flags));
	    vms_debug (4, "%d bytes at 0x%08lx (mem %p)\n", section->size,
		       section->vma, section->contents);
#endif
	...
	case GSD_S_C_SYMW:
	  {
		...
		if ((gsd_type == GSD_S_C_SYMW) || (gsd_type == GSD_S_C_EPMW))
		...
		vms_debug (4, "gsd sym def #%d (%s, %d [%p], %04x=%s)\n",
			   abfd->symcount, symbol->name,
			   (int) symbol->section, symbol->section, old_flags,
		...
		vms_debug (4, "gsd sym ref #%d (%s, %s [%p], %04x=%s)\n",
			   abfd->symcount, symbol->name,
			   symbol->section->name, symbol->section, old_flags,
			   flag2str (gsyflagdesc, old_flags));
#endif
	      }
	...
	case EGSD_S_C_SYM + EVAX_OFFSET:
	  {
		...
		symbol->section =
		  (asection *) ((unsigned long) bfd_getl32 (vms_rec + 28));
#if VMS_DEBUG
		vms_debug (4, "egsd sym def #%d (%s, %d, %04x=%s)\n",
			   abfd->symcount, symbol->name,
			   (int) symbol->section, old_flags,
			   flag2str (gsyflagdesc, old_flags));
#endif
	      }
	    else		/* symbol reference */
	    ...
	    entry =
	      (vms_symbol_entry *) bfd_hash_lookup (PRIV (vms_symbol_table),
						    symbol->name, TRUE,
						    FALSE);
	    if (entry == (vms_symbol_entry *) NULL)
	      {
     ...
     int objtype ATTRIBUTE_UNUSED;
{
  ...
  while (section != 0)
	...
	{
	  new_flags =
	    (EGPS_S_V_OVR | EGPS_S_V_REL | EGPS_S_V_GBL | EGPS_S_V_RD |
  ...
  for (symnum = 0; symnum < abfd->symcount; symnum++)
      ...
      if (old_flags & (BSF_GLOBAL | BSF_WEAK))	/* symbol definition */
	{
      ...
      hash =
	_bfd_vms_length_hash_symbol (abfd, symbol->name, EOBJ_S_C_SYMSIZ);
