...
ERROR: %B is compiled for the EP9312, whereas %B is compiled for XScale"), ibfd, obfd);
    ...
    {
      _bfd_error_handler (_("\
ERROR: %B is compiled for the EP9312, whereas %B is compiled for XScale"), obfd, ibfd);
      bfd_set_error (bfd_error_wrong_format);
      return FALSE;
    }
  else if (in > out)
...
arm_check_note (bfd * abfd,
		...
		const char *expected_name, char **description_return)
{
...
bfd_arm_update_notes (bfd * abfd, const char *note_section)
{
  ...
  /* Parse the note.  */
  if (!arm_check_note
      (abfd, buffer, buffer_size, NOTE_ARCH_STRING, &arch_string))
    goto FAIL;
