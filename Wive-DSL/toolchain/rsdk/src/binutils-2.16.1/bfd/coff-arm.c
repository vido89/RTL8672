...
typedef enum
{ bunknown, b9, b12, b23 } thumb_pcrel_branchtype;
...
#endif
#endif
static bfd_boolean coff_arm_merge_private_bfd_data PARAMS ((bfd *, bfd *));
static bfd_boolean coff_arm_print_private_bfd_data PARAMS ((bfd *, PTR));
static bfd_boolean _bfd_coff_arm_set_private_flags PARAMS ((bfd *, flagword));
static bfd_boolean coff_arm_copy_private_bfd_data PARAMS ((bfd *, bfd *));
     ...
     bfd_vma reloc_offset;
{
  bfd_vma addr = reloc_offset
    - input_section->vma
    + input_section->output_offset + input_section->output_section->vma;
     ...
     asection **sections;
{
  ...
  for (; rel < relend; rel++)
	...
	{
	  static reloc_howto_type fake_arm26_reloc = HOWTO (ARM_26,
							    2,
							    2,
							    24,
							    TRUE,
	  ...
	  if (!info->relocatable
		  ...
		  || h->root.type == bfd_link_hash_defweak))
	    {
	      ...
	      h_val = (h->root.u.def.value
		       + h_sec->output_section->vma + h_sec->output_offset);
		...
		{
		  if (h->class == C_EXT
		      || h->class == C_STAT || h->class == C_LABEL)
		    {
		      ...
		      if ((my_offset & 0x01) == 0x01)
			{
			  ...
			  if (globals->support_old_code)
			    ...
			    {
			      bfd_put_16 (output_bfd,
					  (bfd_vma) t2a1_bx_pc_insn,
					  s->contents + my_offset);

			      bfd_put_16 (output_bfd,
					  (bfd_vma) t2a2_noop_insn,
					  s->contents + my_offset + 2);

			      ret_offset = ((bfd_signed_vma) h_val)	/* Address of destination of the stub.  */
				- ((bfd_signed_vma) (s->output_offset	/* Offset from the start of the current section to the start of the stubs.  */
			...
			s->output_offset
			+ my_offset
			- (input_section->output_offset + rel->r_vaddr) - 4;
	...
	rstat = bfd_reloc_ok;
      ...
      else if (!info->relocatable && howto->type == ARM_THUMB23)
	{
	  ...
	  if (address > high_address)
		...
		signed_add -= (((~src_mask) >> 1) & src_mask) << 1;
	      ...
	      BFD_ASSERT (howto->complain_on_overflow ==
			  complain_overflow_signed);
...
#endif
      else if (info->relocatable && !howto->partial_inplace)
	rstat = bfd_reloc_ok;
      else
	  ...
	  && (rel->r_type == ARM_32 || rel->r_type == ARM_RVA32))
	{
	  ...
	  if (h != NULL
	      && (h->class == C_THUMBSTATFUNC || h->class == C_THUMBEXTFUNC))
	    {
	      patchit = TRUE;
	    }
	  else if (sym != NULL && sym->n_scnum > N_UNDEF)
	    {
...
#define BACK_FROM_ARM "__%s_back_from_arm"
  ...
  sprintf (tmp_name,
	   globals->support_old_code ? BACK_FROM_ARM : CHANGE_TO_ARM, name);
     ...
     bfd *obfd;
{
  ...
  if (APCS_SET (ibfd))
      ...
      if (APCS_SET (obfd))
	{
	  ...
	  if (APCS_26_FLAG (obfd) != APCS_26_FLAG (ibfd))
		...
		(_
		 ...
		 ibfd, obfd, APCS_26_FLAG (ibfd) ? 26 : 32,
	...
	{
	  SET_APCS_FLAGS (obfd,
			  APCS_26_FLAG (ibfd) | APCS_FLOAT_FLAG (ibfd) |
			  PIC_FLAG (ibfd));
      ...
      if (INTERWORK_SET (dest))
	{
	  ...
	  if (INTERWORK_FLAG (dest) != INTERWORK_FLAG (src))
	      ...
	      if (INTERWORK_FLAG (dest))
		{
		  /* xgettext:c-format */
		  _bfd_error_handler (("\
Warning: Clearing the interworking flag of %B because non-interworking code in %B has been linked with it"), dest, src);
		}
