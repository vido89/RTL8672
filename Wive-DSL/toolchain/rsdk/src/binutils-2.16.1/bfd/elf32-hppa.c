...
elf32_hppa_link_hash_table_create (bfd * abfd)
{
  ...
  if (!_bfd_elf_link_hash_table_init
      (&ret->elf, abfd, hppa_link_hash_newfunc))
    {
      free (ret);
      return NULL;
    }
		     ...
		     struct elf32_hppa_link_hash_table *htab)
{
  ...
  if (hash != NULL && hash->stub_cache != NULL
      && hash->stub_cache->h == hash && hash->stub_cache->id_sec == id_sec)
    {
...
hppa_add_stub (const char *stub_name,
	       asection * section, struct elf32_hppa_link_hash_table *htab)
{
  asection *link_sec;
  asection *stub_sec;
  struct elf32_hppa_stub_hash_entry *stub_entry;
...
hppa_type_of_stub (asection * input_sec,
		   const Elf_Internal_Rela * rel,
		   struct elf32_hppa_link_hash_entry *hash,
		   bfd_vma destination, struct bfd_link_info *info)
{
  bfd_vma location;
	      ...
	      || sym_value - 8 + (1 << (22 + 1)) >= (1 << (22 + 2))))
	{
	  (*_bfd_error_handler)
	    (_
	     ...
	     stub_entry->target_section->owner, stub_sec,
	     (long) stub_entry->stub_offset, stub_entry->root.string);
  ...
  if (strcmp (bfd_get_target (abfd), "elf32-hppa-linux") == 0)
  ...
  else if (strcmp (bfd_get_target (abfd), "elf32-hppa-netbsd") == 0)
    {
      /* GCC on hppa-netbsd produces binaries with OSABI=NetBSD,
         but the kernel produces corefiles with OSABI=SysV.  */
      if (i_ehdrp->e_ident[EI_OSABI] != ELFOSABI_NETBSD && i_ehdrp->e_ident[EI_OSABI] != ELFOSABI_NONE)	/* aka SYSV */
	return FALSE;
    }
  else
    {
			 ...
			 asection * sec, const Elf_Internal_Rela * relocs)
{
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_PARISC_DPREL21L:
	  if (info->shared)
		...
		(_
		 ...
		 abfd, elf_hppa_howto_table[r_type].name);
	      bfd_set_error (bfd_error_bad_value);
	      return FALSE;
	    }
	  /* Fall through.  */
	...
	case R_PARISC_GNU_VTINHERIT:
	  if (!bfd_elf_gc_record_vtinherit
	      (abfd, sec, &h->elf, rel->r_offset))
	    return FALSE;
	  continue;
      ...
      if (need_entry & NEED_GOT)
	{
	  ...
	  if (htab->sgot == NULL)
		...
		htab->elf.dynobj = abfd;
	      if (!elf32_hppa_create_dynamic_sections
		  (htab->elf.dynobj, info))
		return FALSE;
	    }
  ...
  if (htab->elf.dynamic_sections_created && h->plt.refcount > 0)
    {
      ...
      if (h->dynindx == -1 && !h->forced_local && h->type != STT_PARISC_MILLI)
	{
...
allocate_dynrelocs (struct elf_link_hash_entry *h, void *inf)
{
  ...
  if (h->got.refcount > 0)
      ...
      if (h->dynindx == -1 && !h->forced_local && h->type != STT_PARISC_MILLI)
	{
      ...
      if (htab->elf.dynamic_sections_created
	  && (info->shared || (h->dynindx != -1 && !h->forced_local)))
	{
  ...
  if (info->shared)
		      ...
		      || h->root.type == bfd_link_hash_undefined))))
	{
	  /* Make sure this symbol is output as a dynamic symbol.
	     Undefined weak syms won't yet be marked as dynamic.  */
	  if (h->dynindx == -1
	      && !h->forced_local && h->type != STT_PARISC_MILLI)
	    {
	      if (!bfd_elf_link_record_dynamic_symbol (info, h))
		return FALSE;
	    }
			   ...
			   struct bfd_link_info *info)
{
  ...
  if (h->type == STT_PARISC_MILLI && !h->forced_local)
    {
				  ...
				  struct bfd_link_info *info)
{
  ...
  if (htab->elf.dynamic_sections_created)
      ...
      if (info->executable)
	{
      ...
      elf_link_hash_traverse (&htab->elf, clobber_millicode_symbols, info);
    }
		     ...
		     struct bfd_link_info *info)
{
  ...
  switch (r_type)
      ...
      if (value + addend + max_branch_offset >= 2 * max_branch_offset)
	{
	  ...
	  value = (stub_entry->stub_offset
		   + stub_entry->stub_sec->output_offset
		   + stub_entry->stub_sec->output_section->vma - location);
	  addend = -8;
	}
      break;
  ...
  for (; rel < relend; rel++)
	...
	case R_PARISC_SEGREL32:
	  ...
	  if (htab->text_segment_base == (bfd_vma) - 1)
	    bfd_map_over_sections (output_bfd, hppa_record_segment_addr,
				   htab);
	  break;
	...
	case R_PARISC_PLABEL32:
	  if (htab->elf.dynamic_sections_created)
		  ...
		  if (info->shared)
		  ...
		  else
		    {
		      bfd_put_32 (output_bfd,
				  relocation, htab->splt->contents + off);
		      bfd_put_32 (output_bfd,
				  elf_gp (htab->splt->output_section->owner),
		      ...
		      && h->elf.root.type != bfd_link_hash_undefined))
		{
		  relocation = (off
				+ htab->splt->output_offset
				+ htab->splt->output_section->vma + 2);
		}
	      plabel = 1;
	    }
	  ...
	  if ((info->shared
	      ...
	      if (skip)
		{
		  memset (&outrel, 0, sizeof (outrel));
			   ...
			   || !IS_ABSOLUTE_RELOC (r_type)
			   || !info->shared
			   || !info->symbolic || !h->elf.def_regular))
		{
				  ...
				  Elf_Internal_Sym * sym)
{
  ...
  if (h->plt.offset != (bfd_vma) - 1)
      ...
      else
	{
      ...
      bfd_elf32_swap_reloca_out (htab->splt->output_section->owner, &rel,
				 loc);
