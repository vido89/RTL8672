...
PARAMS ((bfd *, unsigned int, struct orl *, unsigned int, int));
static bfd_boolean xcoff_write_archive_contents_old PARAMS ((bfd *));
static bfd_boolean xcoff_write_archive_contents_big PARAMS ((bfd *));
...
static bfd_boolean xcoff_is_lineno_count_overflow PARAMS ((bfd *, bfd_vma));
static bfd_boolean xcoff_is_reloc_count_overflow PARAMS ((bfd *, bfd_vma));
...
static bfd_boolean do_copy PARAMS ((bfd *, bfd *));
static bfd_boolean do_shared_object_padding
PARAMS ((bfd *, bfd *, file_ptr *, int));

/* Relocation functions */
static bfd_boolean xcoff_reloc_type_br PARAMS ((XCOFF_RELOC_FUNCTION_ARGS));
     ...
     bfd *abfd;
{
  ...
  if (!xcoff_big_format_p (abfd))
      ...
      if (hdrp == NULL)
	{
      ...
      if (bfd_bread ((char *) hdrp + SIZEOF_AR_HDR_BIG, namlen, abfd) !=
	  namlen)
	{
	  free (ret);
	  return NULL;
  ...
  if (bfd_seek (abfd, (file_ptr) ((namlen & 1) + SXCOFFARFMAG), SEEK_CUR) !=
      0)
    return NULL;
     ...
     bfd *last_file;
{
  ...
  if (!xcoff_big_format_p (archive))
	...
	filestart =
	  strtol (arch_xhdr_big (last_file)->nextoff, (char **) NULL, 10);
     ...
     int stridx;
{
  ...
  while (sub != NULL && i < orl_count)
      ...
      while (map[i].u.abfd == sub)
	{
      ...
      fileoff += (SIZEOF_AR_HDR + namlen + SXCOFFARFMAG + arelt_size (sub));
      fileoff = (fileoff + 1) & ~1;
      sub = sub->next;
    }
  ...
  while (remaining >= DEFAULT_BUFFERSIZE)
    {
	  ...
	  || bfd_bwrite (buffer, DEFAULT_BUFFERSIZE,
			 out_bfd) != DEFAULT_BUFFERSIZE)
	return FALSE;
     ...
     int stridx;
{
  ...
  if (sym_32)
	...
	SIZEOF_AR_HDR_BIG
	+ SXCOFFARFMAG + 8 + 8 * sym_32 + str_32 + (str_32 & 1);
      ...
      while (current_bfd != NULL && i < orl_count)
	{
	  ...
	  fileoff += (SIZEOF_AR_HDR_BIG
		      + string_length
		      + SXCOFFARFMAG + arelt_size (current_bfd));
	  fileoff += fileoff & 1;
	  current_bfd = current_bfd->next;
	  if (current_bfd != NULL)
	    arch_info = bfd_get_arch_info (current_bfd);
  ...
  if (sym_64)
    {
      ...
      bfd_vma symbol_table_size =
	SIZEOF_AR_HDR_BIG
	+ SXCOFFARFMAG + 8 + 8 * sym_64 + str_64 + (str_64 & 1);
     ...
     bfd *abfd;
{
  ...
  for (sub = abfd->archive_head, i = 0; sub != NULL; sub = sub->next, i++)
      ...
      if (ahdrp == NULL)
	{
      ...
      size = (SIZEOF_AR_HDR + namlen + SXCOFFARFMAG + remaining);
  ...
  size = (SIZEOF_AR_HDR
	  ...
	  + count * XCOFFARMAG_ELEMENT_SIZE + total_namlen + SXCOFFARFMAG);
     ...
     bfd *abfd;
{
  ...
  for (current_bfd = abfd->archive_head, count = 0, total_namlen = 0;
       current_bfd != NULL; current_bfd = current_bfd->next, count++)
    {
      total_namlen += strlen (normalize_filename (current_bfd)) + 1;

      if (makemap
	  && !hasobjects && bfd_check_format (current_bfd, bfd_object))
	hasobjects = TRUE;
    }
  ...
  for (current_bfd = abfd->archive_head, i = 0;
       current_bfd != NULL; current_bfd = current_bfd->next, i++)
    {
      const char *name;
      bfd_size_type namlen;
      ...
      if (ahdrp == NULL)
	{
      ...
      size = (SIZEOF_AR_HDR_BIG + namlen + SXCOFFARFMAG + remaining);
  ...
  member_table_size = (SIZEOF_AR_HDR_BIG
		       ...
		       + count * XCOFFARMAGBIG_ELEMENT_SIZE + total_namlen);
      ...
      if (h->toc_section == NULL)
	{
	  (*_bfd_error_handler)
	    (_("%s: TOC reloc at 0x%x to symbol `%s' with no TOC entry"),
	     bfd_get_filename (input_bfd), rel->r_vaddr, h->root.root.string);
	  bfd_set_error (bfd_error_bad_value);
	  return FALSE;
	}
...
xcoff_ppc_relocate_section (output_bfd, info, input_bfd,
			    input_section, contents, relocs, syms, sections)
     bfd *output_bfd;
	  ...
	  if (NULL == h)
	    {
	      ...
	      if (sec->name[3] == '0' && strcmp (sec->name, ".tc0") == 0)
     ...
     const char *symbol_name;
{
  ...
  if ((19 >= aux->x_csect.x_smclas) && (NULL != names[aux->x_csect.x_smclas]))
    {
...
static reloc_howto_type xcoff_dynamic_reloc = HOWTO (0,	/* type */
						     0,	/* rightshift */
						     2,	/* size (0 = byte, 1 = short, 2 = long) */
						     32,	/* bitsize */
						     FALSE,	/* pc_relative */
