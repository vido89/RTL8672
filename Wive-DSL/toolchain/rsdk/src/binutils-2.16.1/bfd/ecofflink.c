  ...
  if (bigend)
  ...
  else
    {
      ext->t_bits1[0] = ((intern->fBitfield ? TIR_BITS1_FBITFIELD_LITTLE : 0)
			 | (intern->
			    continued ? TIR_BITS1_CONTINUED_LITTLE : 0) |
			 ((intern->
			   bt << TIR_BITS1_BT_SH_LITTLE) &
			  TIR_BITS1_BT_LITTLE));
      ext->t_tq45[0] =
	(((intern->
	   tq4 << TIR_BITS_TQ4_SH_LITTLE) & TIR_BITS_TQ4_LITTLE) | ((intern->
								     tq5 <<
								     TIR_BITS_TQ5_SH_LITTLE)
								    &
								    TIR_BITS_TQ5_LITTLE));
      ext->t_tq01[0] =
	(((intern->
	   tq0 << TIR_BITS_TQ0_SH_LITTLE) & TIR_BITS_TQ0_LITTLE) | ((intern->
								     tq1 <<
								     TIR_BITS_TQ1_SH_LITTLE)
								    &
								    TIR_BITS_TQ1_LITTLE));
      ext->t_tq23[0] =
	(((intern->
	   tq2 << TIR_BITS_TQ2_SH_LITTLE) & TIR_BITS_TQ2_LITTLE) | ((intern->
								     tq3 <<
								     TIR_BITS_TQ3_SH_LITTLE)
								    &
								    TIR_BITS_TQ3_LITTLE));
     ...
     RNDXR *intern;
{
  ...
  *ext = *ext_copy;		/* Make it reasonable to do in-place.  */
  ...
  if (bigend)
	...
	| ((ext->r_bits[1] & RNDX_BITS1_RFD_BIG) >> RNDX_BITS1_RFD_SH_BIG);
...
bfd_ecoff_debug_accumulate (handle, output_bfd, output_debug, output_swap,
			    input_bfd, input_debug, input_swap, info)
     PTR handle;
     bfd *output_bfd;
     struct ecoff_debug_info *output_debug;
...
#undef SET
  ...
  for (; rfd_in < rfd_end; rfd_in += input_swap->external_rfd_size)
    {
      RFDT rfd;
  ...
  if (!add_memory_shuffle (ainfo, &ainfo->fdr, &ainfo->fdr_end, fdr_out, sz))
    return FALSE;
  for (fdr_ptr = fdr_start, i = 0; fdr_ptr < fdr_end; fdr_ptr += fdr_add, i++)
    {
      FDR fdr;
      bfd_vma fdr_adr;
      bfd_byte *sym_out;
      ...
      for (; lraw_src < lraw_end; lraw_src += input_swap->external_sym_size)
	{
	  ...
	  if (!info->relocatable)
	      ...
	      else
		{
		  ...
		  sh =
		    string_hash_lookup (&ainfo->str_hash, name, TRUE, TRUE);
      ...
      else
	{
	  ...
	  in = ((bfd_byte *) input_debug->external_pdr + fdr.ipdFirst * insz);
	  end = in + fdr.cpd * insz;
	  sz = fdr.cpd * outsz;
	  ...
	  in = ((bfd_byte *) input_debug->external_opt + fdr.ioptBase * insz);
     ...
     const char *string;
{
  ...
  if (info->relocatable)
    {
      if (!add_memory_shuffle
	  (ainfo, &ainfo->ss, &ainfo->ss_end, (PTR) string, len + 1))
	return -1;
      ret = symhdr->issMax;
      symhdr->issMax += len + 1;
      fdr->cbSs += len + 1;
      ...
      if (sh->val == -1)
	{
	  ...
	  if (ainfo->ss_hash == (struct string_hash_entry *) NULL)
	    ainfo->ss_hash = sh;
	  if (ainfo->ss_hash_end != (struct string_hash_entry *) NULL)
	    ainfo->ss_hash_end->next = sh;
	  ainfo->ss_hash_end = sh;
	}
      ret = sh->val;
  ...
  for (c = bfd_get_symcount (abfd); c > 0; c--, sym_ptr_ptr++)
	  ...
	  || sym_ptr->section->output_section == (asection *) NULL)
	{
	  ...
	  if (esym.asym.sc != scSUndefined
	      || esym.asym.value == 0 || sym_ptr->value != 0)
  ...
  if (info->relocatable)
	...
	goto error_return;
      ...
      for (sh = ainfo->ss_hash;
	   sh != (struct string_hash_entry *) NULL; sh = sh->next)
	{
	  size_t len;
  ...
  for (fdr_ptr = fdr_start; fdr_ptr < fdr_end; fdr_ptr++)
      ...
      if (fdr_ptr->csym >= 2)
	{
	  ...
	  sym_ptr = ((char *) debug_info->external_sym
		     + (fdr_ptr->isymBase +
			1) * debug_swap->external_sym_size);
  ...
  if (!stabs)
	  ...
	  if (pdr.isym == -1)
		...
		(abfd,
		 ((char *) debug_info->external_ext
		  + pdr.isym * debug_swap->external_ext_size), &proc_ext);
      ...
      else
	{
	  ...
	  line_info->cache.filename = (debug_info->ss
				       + fdr_ptr->issBase + fdr_ptr->rss);
	  (*debug_swap->swap_sym_in)
	    (abfd,
	     ((char *) debug_info->external_sym
	      + ((fdr_ptr->isymBase + pdr.isym)
		 * debug_swap->external_sym_size)), &proc_sym);
	  line_info->cache.functionname = (debug_info->ss
					   + fdr_ptr->issBase + proc_sym.iss);
	}
      if (lineno == ilineNil)
	lineno = 0;
	...
	len = funclen = strlen (function_name) + 1;
	  ...
	  && directory_name != NULL && main_file_name[0] != '/')
...
_bfd_ecoff_locate_line (abfd, section, offset, debug_info, debug_swap,
			line_info, filename_ptr, functionname_ptr,
			retline_ptr)
     bfd *abfd;
     asection *section;
     bfd_vma offset;
     ...
     unsigned int *retline_ptr;
{
  ...
  if (line_info->cache.sect == NULL
      ...
      || offset < line_info->cache.start || offset >= line_info->cache.stop)
    {
      line_info->cache.sect = section;
