...
static bfd_boolean elf64_hppa_object_p PARAMS ((bfd *));
...
static int elf64_hppa_elf_get_symbol_type PARAMS ((Elf_Internal_Sym *, int));
       ...
       info;
{
  (bfd_hash_traverse
   (&table->root,
    (bfd_boolean (*)PARAMS ((struct bfd_hash_entry *, PTR))) func, info));
}

  ...
  if (strcmp (bfd_get_target (abfd), "elf64-hppa-linux") == 0)
    {
      ...
      if (i_ehdrp->e_ident[EI_OSABI] != ELFOSABI_LINUX && i_ehdrp->e_ident[EI_OSABI] != ELFOSABI_NONE)	/* aka SYSV */
	return FALSE;
    }
  else
    {
  ...
  if (h)
  ...
  else
    {
      nlen = sprintf (buf, "%x:%lx",
		      sec->id & 0xffffffff, (long) ELF64_R_SYM (rel->r_info));
      if (rel->r_addend)
	{
	  buf[nlen++] = '+';
	  sprintf_vma (buf + nlen, rel->r_addend);
     ...
     const Elf_Internal_Rela *relocs;
{
  ...
  for (rel = relocs; rel < relend; ++rel)
	...
	{
	  if (!hppa_info->dlt_sec && !get_dlt (abfd, info, hppa_info))
	    goto err_out;
	  dyn_h->want_dlt = 1;
	}
	...
	{
	  if (!hppa_info->plt_sec && !get_plt (abfd, info, hppa_info))
	    goto err_out;
	  dyn_h->want_plt = 1;
	}
	...
	{
	  if (!hppa_info->stub_sec && !get_stub (abfd, info, hppa_info))
	    goto err_out;
	  dyn_h->want_stub = 1;
	}
	...
	{
	  if (!hppa_info->opd_sec && !get_opd (abfd, info, hppa_info))
	    goto err_out;
     ...
     PTR data;
{
  ...
  if (h
      ...
      && h->root.u.def.section->output_section != NULL && h->type == STT_FUNC)
    {
      struct elf64_hppa_dyn_hash_entry *dyn_h;
...
allocate_global_data_dlt (dyn_h, data)
     ...
     PTR data;
{
  struct elf64_hppa_allocate_data *x =
  ...
  if (dyn_h->want_dlt)
      ...
      if (x->info->shared)
	{
	  /* Possibly add the symbol to the local dynamic symbol
	     ...
	     against it.  */
	  if (!h || (h->dynindx == -1 && h->type != STT_PARISC_MILLI))
	    {
	      bfd *owner;
     ...
     PTR data;
{
  ...
  if (dyn_h->want_opd)
		   ...
		   || h->root.type == bfd_link_hash_defweak))
	{
	  ...
	  if (x->info->shared && (h == NULL || (h->dynindx == -1)))
	    {
	      bfd *owner;
	      owner = (h ? h->root.u.def.section->owner : dyn_h->owner);
     ...
     struct bfd_link_info *info;
{
  ...
  elf_link_hash_traverse (elf_hash_table (info),
			  (elf_hash_table (info)->dynamic_sections_created
			   ...
			   : elf64_hppa_mark_exported_functions), info);
	  ...
	  if (s->size == 0)
	      ...
	      if (strcmp (name, ".rela.plt") != 0)
		{
	      ...
	      s->reloc_count = 0;
	       ...
	       && strcmp (name, ".stub") != 0 && strcmp (name, ".got") != 0)
	{
     ...
     Elf_Internal_Sym *sym;
{
  ...
  if (dyn_h && dyn_h->want_opd)
    {
      ...
      sym->st_value = (dyn_h->opd_offset
		       + sopd->output_offset + sopd->output_section->vma);
  ...
  if (dyn_h && dyn_h->want_plt
	...
	value = (h->root.u.def.value + h->root.u.def.section->vma);
      ...
      bfd_put_64 (splt->owner, value,
		  splt->contents + dyn_h->plt_offset + 0x8);
  ...
  if (dyn_h && dyn_h->want_stub
      ...
      if ((value & 7) || value + max_offset >= 2 * max_offset - 8)
	{
      ...
      value += 8;
      insn =
	bfd_get_32 (stub->owner, stub->contents + dyn_h->stub_offset + 8);
      if (output_bfd->arch_info->mach >= 25)
	{
	  insn &= ~0xfff1;
	  insn |= re_assemble_16 ((int) value);
  ...
  if (h && dyn_h->want_opd)
    {
      ...
      bfd_put_64 (sopd->owner, value,
		  sopd->contents + dyn_h->opd_offset + 16);

      /* The last word is our local __gp value.  */
      value = _bfd_get_gp_value (sopd->output_section->owner);
      bfd_put_64 (sopd->owner, value,
		  sopd->contents + dyn_h->opd_offset + 24);
    }
  ...
  if (dyn_h->reloc_entries)
      ...
      for (rent = dyn_h->reloc_entries; rent; rent = rent->next)
	{
	  ...
	  loc += (hppa_info->other_rel_sec->reloc_count++
		  * sizeof (Elf64_External_Rela));
	  bfd_elf64_swap_reloca_out (hppa_info->other_rel_sec->
				     output_section->owner, &rel, loc);
	}
    }
     ...
     struct bfd_link_info *info;
{
  ...
  elf64_hppa_dyn_hash_traverse (&hppa_info->dyn_hash_table,
				elf64_hppa_finalize_opd, info);

  elf64_hppa_dyn_hash_traverse (&hppa_info->dyn_hash_table,
				elf64_hppa_finalize_dynreloc, info);

  /* Finalize the contents of the .dlt section.  */
  dynobj = elf_hash_table (info)->dynobj;
  /* Finalize the contents of the .dlt section.  */
  elf64_hppa_dyn_hash_traverse (&hppa_info->dyn_hash_table,
				elf64_hppa_finalize_dlt, info);
