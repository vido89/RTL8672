...
#define	MAX_UBYTE	((unsigned) 0x000000FF)
  ...
  switch (format)
	...
	case R_S_16C_20:	/* word1(0-3),word2.  */
	  ...
	  value = ((value & 0xFFFF0000) >> 16) | ((value & 0x0000000F) << 16);
	  break;
	case R_S_16C_24:	/* word2(0-3,8-11),word3.  */
	  value = bfd_get_32 (abfd, (bfd_byte *) data + octets);
	  left_val = value & 0x0000F0F0;
  ...
  switch (format)
	...
	case R_S_16C_16:	/* Two bytes.  */
	  ...
	  value &= 0xFFFF;
	  sword = value;
	  bfd_put_16 (abfd, (bfd_vma) sword, (unsigned char *) data + octets);
	  break;
	...
	case R_S_16C_20:	/* word1(0-3),word2.  */
	  if (value > 1048575 || value < 0)
	    return bfd_reloc_overflow;
	  value &= 0xFFFFF;
	  value = ((value & 0x0000FFFF) << 16) | ((value & 0x000F0000) >> 16);
	  value |= left_val;
	  bfd_put_32 (abfd, (bfd_vma) value, (bfd_byte *) data + octets);
	  break;
...
static void
elf32_cr16c_symbol_processing (bfd * abfd ATTRIBUTE_UNUSED, asymbol * asym)
{
...
static bfd_boolean
elf32_cr16c_link_output_symbol_hook (struct bfd_link_info *info
				     ATTRIBUTE_UNUSED,
