...
static long bfd_mach_o_canonicalize_symtab PARAMS ((bfd *, asymbol **));
...
static bfd_boolean bfd_mach_o_write_contents PARAMS ((bfd *));
static int bfd_mach_o_sizeof_headers PARAMS ((bfd *, bfd_boolean));
     ...
     bfd *abfd;
{
  ...
  if (!((abfd->xvec == &mach_o_be_vec)
	|| (abfd->xvec == &mach_o_le_vec) || (abfd->xvec == &mach_o_fat_vec)))
    return 0;
     ...
     asymbol **alocation;
{
  ...
  for (i = 0; i < mdata->header.ncmds; i++)
      ...
      if (mdata->commands[i].type == BFD_MACH_O_LC_SYMTAB)
	{
	  ...
	  if (bfd_mach_o_scan_read_symtab_symbols
	      (abfd, &mdata->commands[i].command.symtab) != 0)
	    {
	      fprintf (stderr,
  ...
  for (i = 0; i < mdata->header.ncmds; i++)
    {
      ...
      typeflag =
	cur->type_required ? cur->type & BFD_MACH_O_LC_REQ_DYLD : cur->type;
     ...
     bfd_mach_o_section *section;
{
  ...
  snamelen = strlen (prefix) + 1
    + strlen (section->segname) + 1 + strlen (section->sectname) + 1;
     ...
     bfd_mach_o_symtab_command *sym;
{
  ...
  for (i = 0; i < sym->nsyms; i++)
    {
      ret =
	bfd_mach_o_scan_read_symtab_symbol (abfd, sym, &sym->symbols[i], i);
      if (ret != 0)
	return ret;
    }
