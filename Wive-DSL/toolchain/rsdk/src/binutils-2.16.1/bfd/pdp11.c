...
{
  TARGETNAME,			/* name */
    bfd_target_aout_flavour, BFD_ENDIAN_LITTLE,	/* target byte order (little) */
    BFD_ENDIAN_LITTLE,		/* target headers byte order (little) */
    (HAS_RELOC | EXEC_P |	/* object flags */
     HAS_LINENO | HAS_DEBUG | HAS_SYMS | HAS_LOCALS | WP_TEXT), (SEC_HAS_CONTENTS | SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_CODE | SEC_DATA), MY_symbol_leading_char, AR_PAD_CHAR,	/* ar_pad_char */
    15,				/* ar_max_namelen */
    bfd_getl64, bfd_getl_signed_64, bfd_putl64, bfd_getp32, bfd_getp_signed_32, bfd_putp32, bfd_getl16, bfd_getl_signed_16, bfd_putl16,	/* data */
    bfd_getl64, bfd_getl_signed_64, bfd_putl64, bfd_getp32, bfd_getp_signed_32, bfd_putp32, bfd_getl16, bfd_getl_signed_16, bfd_putl16,	/* hdrs */
  {
  ...
  _bfd_write_archive_contents, bfd_false},
    ...
    BFD_JUMP_TABLE_LINK (MY), BFD_JUMP_TABLE_DYNAMIC (MY),
    /* Alternative_target */
NULL, (PTR) MY_backend_data,};
...
reloc_howto_type howto_table_pdp11[] = {
  ...
  HOWTO (0, 0, 1, 16, FALSE, 0, complain_overflow_signed, 0, "16", TRUE,
	 0x0000ffff, 0x0000ffff, FALSE),
  HOWTO (1, 0, 1, 16, TRUE, 0, complain_overflow_signed, 0, "DISP16", TRUE,
	 0x0000ffff, 0x0000ffff, FALSE),
};

#define TABLE_SIZE(TABLE)	(sizeof(TABLE)/sizeof(TABLE[0]))

reloc_howto_type *NAME (aout, reloc_type_lookup) (abfd, code)
     bfd *
       abfd
       ATTRIBUTE_UNUSED;
...
static bfd_boolean MY (write_object_contents) (abfd)
     bfd *
       abfd;
{
...
void NAME (aout, swap_exec_header_in) (abfd, raw_bytes, execp)
     bfd *
       abfd;
...
*/
void NAME (aout, swap_exec_header_out) (abfd, execp, raw_bytes)
     bfd *
       abfd;
     struct internal_exec *
       execp;
...
bfd_boolean NAME (aout, make_sections) (abfd)
     bfd *
       abfd;
{
...
const bfd_target *NAME (aout, some_aout_object_p) (abfd, execp,
...
bfd_boolean NAME (aout, mkobject) (abfd)
     bfd *
       abfd;
{
...
enum machine_type NAME (aout, machine_type) (arch, machine, unknown)
       ...
       unknown;
{
  ...
  *unknown = TRUE;
  ...
  switch (arch)
	...
	{
	case 0:
	  arch_flags = M_NS32532;
	  break;
	case 32032:
	  arch_flags = M_NS32032;
	  break;
	case 32532:
	  arch_flags = M_NS32532;
	  break;
...
bfd_boolean NAME (aout, set_arch_mach) (abfd, arch, machine)
     bfd *
       abfd;
     enum bfd_architecture
       arch;
...
bfd_boolean NAME (aout, adjust_sizes_and_vmas) (abfd, text_size, text_end)
     bfd *
       abfd;
     bfd_size_type *
       text_size;
  ...
  *text_size = obj_textsec (abfd)->size;
  ...
  fprintf (stderr, "%s text=<%x,%x,%x> data=<%x,%x,%x> bss=<%x,%x,%x>\n", (
									    ...
									    *str;
									    ...
									    {
case n_magic:
str = "NMAGIC"; break; case o_magic:
str = "OMAGIC"; break; case z_magic:
str = "ZMAGIC"; break; default:
									    abort ();}
									    str;}
	   ),
	   obj_textsec (abfd)->vma, obj_textsec (abfd)->size,
...
DESCRIPTION
	...
	request.
*/
bfd_boolean NAME (aout, new_section_hook) (abfd, newsect)
     bfd *
       abfd;
     asection *
       newsect;
     ...
     bfd_size_type count;
{
  ...
  if (section != obj_textsec (abfd) && section != obj_datasec (abfd))
    {
      (*_bfd_error_handler)
     ...
     bfd *abfd;
{
  if (obj_aout_external_syms (abfd) == (struct external_nlist *) NULL)
	...
	return FALSE;
      ...
      syms =
	(struct external_nlist *) bfd_malloc (count * EXTERNAL_NLIST_SIZE);
...
asymbol *NAME (aout, make_empty_symbol) (abfd)
     bfd *
       abfd;
{
...
bfd_boolean
NAME (aout, translate_symbol_table) (abfd, in, ext, count, str, strsize,
				     dynamic)
     bfd *abfd;
     aout_symbol_type *in;
...
bfd_boolean NAME (aout, slurp_symbol_table) (abfd)
     bfd *
       abfd;
{
  ...
  if (!(NAME (aout, translate_symbol_table)
	(abfd, cached,
	 ...
	 obj_aout_external_string_size (abfd), FALSE)))
    {
      free (cached);
      return FALSE;
    }
...

bfd_boolean NAME (aout, write_syms) (abfd)
     bfd *
       abfd;
{
  unsigned int count;
...
long NAME (aout, canonicalize_symtab) (abfd, location)
     bfd *
       abfd;
     asymbol **
       location;
...
void
pdp11_aout_swap_reloc_in (abfd, bytes, cache_ptr, offset, symbols, symcount)
     bfd *abfd;
     bfd_byte *bytes;
     arelent *cache_ptr;
...
bfd_boolean NAME (aout, slurp_reloc_table) (abfd, asect, symbols)
     bfd *
       abfd;
     sec_ptr
       asect;
       ...
       symbols;
{
  ...
  rptr = relocs;
  for (counter = 0;
       counter < count; counter++, rptr += RELOC_SIZE, cache_ptr++)
    {
...
bfd_boolean NAME (aout, squirt_out_relocs) (abfd, section)
     bfd *
       abfd;
     asection *
       section;
...
long NAME (aout, canonicalize_reloc) (abfd, section, relptr, symbols)
     bfd *
       abfd;
     sec_ptr
       section;
...
long NAME (aout, get_reloc_upper_bound) (abfd, asect)
     bfd *
       abfd;
     sec_ptr
       asect;
...
long NAME (aout, get_symtab_upper_bound) (abfd)
     bfd *
       abfd;
{
  if (!NAME (aout, slurp_symbol_table) (abfd))
...
alent *NAME (aout, get_lineno) (abfd, symbol)
     bfd *
       abfd
       ATTRIBUTE_UNUSED;
     asymbol *
...
void NAME (aout, get_symbol_info) (abfd, symbol, ret)
     bfd *
       abfd
       ATTRIBUTE_UNUSED;
     asymbol *
...
void NAME (aout, print_symbol) (abfd, afile, symbol, how)
     bfd *
       abfd;
     PTR
       afile;
...
long NAME (aout, read_minisymbols) (abfd, dynamic, minisymsp, sizep)
     bfd *
       abfd;
     bfd_boolean
       dynamic;
...
asymbol *NAME (aout, minisymbol_to_symbol) (abfd, dynamic, minisym, sym)
     bfd *
       abfd;
     bfd_boolean
       dynamic;
     ...
     asymbol *
       sym;
{
  if (dynamic || obj_aout_external_sym_count (abfd) < MINISYM_THRESHOLD)
     ...
     unsigned int *line_ptr;
{
  ...
  *line_ptr = 0;
	  ...
	  switch (q->type)
	    ...
	    case N_TEXT:
		  ...
		  && ((q->symbol.value > low_line_vma
		       && (line_file_name != NULL
			   || *line_ptr != 0))
		      || (q->symbol.value > low_func_vma && func != NULL)))
		{
		  const char *symname;
  ...
  if (main_file_name == NULL
      || main_file_name[0] == '/' || directory_name == NULL)
    filelen = 0;
  else
...
int NAME (aout, sizeof_headers) (abfd, execable)
     bfd *
       abfd;
     bfd_boolean execable
       ATTRIBUTE_UNUSED;
{
...
bfd_boolean NAME (aout, bfd_free_cached_info) (abfd)
     bfd *
       abfd;
{
  asection *o;
...
struct bfd_hash_entry *NAME (aout, link_hash_newfunc) (entry, table, string)
...
bfd_boolean NAME (aout, link_hash_table_init) (table, abfd, newfunc)
...
struct bfd_link_hash_table *NAME (aout, link_hash_table_create) (abfd)
     bfd *
       abfd;
{
...
bfd_boolean NAME (aout, link_add_symbols) (abfd, info)
     bfd *
       abfd;
  ...
  *pneeded = FALSE;
  ...
  for (; p < pend; p++)
	...
	continue;
	  ...
	  || type == (N_DATA | N_EXT)
	  || type == (N_BSS | N_EXT) || type == (N_ABS | N_EXT))
	{
...
static INLINE asection *aout_reloc_type_to_section PARAMS ((bfd *, int));
static bfd_boolean aout_link_reloc_link_order
PARAMS ((struct aout_final_link_info *, asection *, struct bfd_link_order *));
static bfd_boolean pdp11_aout_link_input_section
PARAMS ((struct aout_final_link_info * finfo,
	 bfd * input_bfd,
	 asection * input_section,
	 bfd_byte * relocs, bfd_size_type rel_size, bfd_byte * contents));
...
bfd_boolean NAME (aout, final_link) (abfd, info, callback)
     bfd *
       abfd;
     struct bfd_link_info *
       info;
...
PARAMS ((bfd *, file_ptr *, file_ptr *, file_ptr *));
{
  ...
  if (!bfd_hash_table_init_n (&aout_info.includes.root,
			      aout_link_includes_newfunc, 251))
    goto error_return;
      ...
      if (info->relocatable)
	{
	  if (bfd_get_flavour (sub) == bfd_target_aout_flavour)
		...
		("%s: relocatable link from %s to %s not supported",
		 bfd_get_filename (abfd), sub->xvec->name, abfd->xvec->name);
	      bfd_set_error (bfd_error_invalid_operation);
	      goto error_return;
	    }
	}
  ...
  if (info->relocatable)
    {
      if (obj_textsec (abfd) != (asection *) NULL)
	trsize +=
	  (_bfd_count_link_order_relocs (obj_textsec (abfd)->link_order_head)
	   * obj_reloc_entry_size (abfd));
      if (obj_datasec (abfd) != (asection *) NULL)
	drsize +=
	  (_bfd_count_link_order_relocs (obj_datasec (abfd)->link_order_head)
	   * obj_reloc_entry_size (abfd));
    }
  ...
  aout_link_hash_traverse (aout_hash_table (info),
			   aout_link_write_other_symbol, (PTR) & aout_info);
     ...
     bfd *input_bfd;
{
	...
	return FALSE;
      ...
      PUT_WORD (output_bfd,
		(bfd_get_section_vma (output_bfd,
		 ...
		 + obj_textsec (input_bfd)->output_offset), outsym->e_value);
  ...
  for (; sym < sym_end; sym++, sym_hash++, symbol_map++)
      ...
      else
	{
	  ...
	  if (h != (struct aout_link_hash_entry *) NULL
	      && h->root.type != bfd_link_hash_warning && h->written)
	    {
	      if ((type & N_TYPE) == N_INDR || type == N_WARNING)
		skip_next = TRUE;
	      *symbol_map = h->indx;
	      continue;
	    }
	  ...
	  if ((type & N_TYPE) == N_TEXT || type == N_WEAKT)
		       ...
		       || hresolve->root.type == bfd_link_hash_defweak)
		{
		  ...
		  /* Get the correct type based on the section.  If
		     ...
		     globally visible.  */
		  if (type == N_SETT
		      || type == N_SETD || type == N_SETB || type == N_SETA)
		    type |= N_EXT;
	  ...
	  if (symsec != (asection *) NULL)
	    val = (symsec->output_section->vma
		   ...
		   + (GET_WORD (input_bfd, sym->e_value) - symsec->vma));
	  ...
	  if (h != (struct aout_link_hash_entry *) NULL)
	    {
	      ...
	      h->indx = obj_aout_external_sym_count (output_bfd);
		   ...
		   && (type & N_TYPE) != N_SETD
		   && (type & N_TYPE) != N_SETB && (type & N_TYPE) != N_SETA)
	    {
	      switch (discard)
		{
		case discard_none:
	  ...
	  if (type == N_BINCL)
	      ...
	      else
		{
		  ...
		  for (incl_sym = sym + 1, incl_map = symbol_map + 1;
		       incl_sym < sym_end; incl_sym++, incl_map++)
		    {
		      int incl_type;
      ...
      if (!finfo->info->keep_memory)
	{
      ...
      strtab_index = add_to_stringtab (output_bfd, finfo->strtab, name, copy);
      if (strtab_index == (bfd_size_type) - 1)
	return FALSE;
      PUT_WORD (output_bfd, strtab_index, outsym->e_strx);
     ...
     PTR data;
{
  ...
  switch (h->root.type)
	...
	val = (h->root.u.def.value
	       + sec->vma + h->root.u.def.section->output_offset);
      }
      break;
	  ...
	  if (r_extern)
	  ...
	  else
	    {
	      ...
	      relocation = (section->output_section->vma
			    + section->output_offset - section->vma);
	    }
     ...
     struct bfd_link_order *p;
{
  ...
  if (p->type == bfd_section_reloc_link_order)
  ...
  else
    {
      ...
      if (h != (struct aout_link_hash_entry *) NULL && h->indx >= 0)
	r_index = h->indx;
      else if (h != NULL)
	{
  ...
  if (pr->addend != 0)
	...
	case bfd_reloc_overflow:
      ...
      ok = bfd_set_section_contents (finfo->output_bfd, o,
				     (PTR) buf, (file_ptr) p->offset, size);
      free (buf);
      if (!ok)
	return FALSE;
    }
