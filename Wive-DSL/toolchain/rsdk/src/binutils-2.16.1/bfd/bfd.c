	      ...
	      else
		{
		  ...
		  if (abfd != NULL
		    ...
		    group = elf_group_name (sec);
			   ...
			   && (ci =
			       bfd_coff_get_comdat_section (sec->owner,
							    sec)) != NULL)
		    group = ci->name;
		  if (group != NULL)
...
bfd_set_reloc (bfd * ignore_abfd ATTRIBUTE_UNUSED,
	       sec_ptr asect, arelent ** location, unsigned int count)
{
...
bfd_get_sign_extend_vma (bfd * abfd)
{
  ...
  if (strncmp (name, "coff-go32", sizeof ("coff-go32") - 1) == 0
      || strcmp (name, "pe-i386") == 0 || strcmp (name, "pei-i386") == 0)
    return 1;
...
bfd_scan_vma (const char *string, const char **end, int base)
{
  ...
  if (base == 16
      && string[0] == '0'
      && (string[1] == 'x' || string[1] == 'X') && ISXDIGIT (string[2]))
    {
      string += 2;
    }
...
void
bfd_preserve_finish (bfd * abfd ATTRIBUTE_UNUSED,
