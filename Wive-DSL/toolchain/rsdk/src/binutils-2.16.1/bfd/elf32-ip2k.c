...
static int ip2k_is_opcode PARAMS ((bfd_byte *, const struct ip2k_opcode *));
static bfd_vma symbol_value
PARAMS ((bfd *, Elf_Internal_Shdr *, Elf_Internal_Sym *,
	 Elf_Internal_Rela *));
static void ip2k_get_mem PARAMS ((bfd *, bfd_byte *, int, bfd_byte *));
...
PARAMS ((bfd *, asection *, Elf_Internal_Rela *, bfd_boolean *,
	 struct misc *));
...
PARAMS ((bfd *, asection *, bfd_boolean *, struct misc *, unsigned long,
	 unsigned long));
...
#define IP2K_HOWTO(t,rs,s,bs,pr,bp,name,sm,dm) \
  ...
  IP2K_HOWTO (R_IP2K_ADDR16CJP, 1, 1, 13, FALSE, 0, "R_IP2K_ADDR16CJP", 0,
	      0x1fff),
  ...
  IP2K_HOWTO (R_IP2K_FR_OFFSET, 0, 1, 9, FALSE, 0, "R_IP2K_FR_OFFSET", 0x180,
	      0x007f),
  /* Bits 23:16 of an address.  */
  IP2K_HOWTO (R_IP2K_EX8DATA, 16, 1, 8, FALSE, 0, "R_IP2K_EX8DATA", 0,
	      0x00ff),
};
     ...
     bfd_byte *contents;
{
  ...
  if ((!IS_PAGE_OPCODE (code + 0)) || (!IS_JMP_OPCODE (code + 2)))
    return -1;
     ...
     struct misc *misc;
{
  ...
  if ((!IS_ADD_W_WREG_OPCODE (code + 0)) || (!IS_ADD_PCL_W_OPCODE (code + 2)))
    {
     ...
     bfd_byte *contents;
{
  ...
  while (1)
	...
	return -1;
	  ...
	  && (IS_PAGE_OPCODE (code + 12)) && (IS_JMP_OPCODE (code + 14)))
	return index;
	  ...
	  && (IS_INC_1SP_OPCODE (code + 12)) && (IS_JMP_OPCODE (code + 14)))
	return index;
     ...
     struct misc *misc;
{
  ...
  if ((!IS_ADD_W_WREG_OPCODE (code + 0))
      ...
      || (!IS_INC_1SP_OPCODE (code + 4))
      || (!IS_ADD_2SP_W_OPCODE (code + 6))
      || (!IS_SNC_OPCODE (code + 8)) || (!IS_INC_1SP_OPCODE (code + 10)))
    {
      _bfd_error_handler (_("ip2k relaxer: switch table header corrupt."));
      return FALSE;
    }
     ...
     bfd_boolean *again;
{
  ...
  *again = FALSE;
  ...
  if ((pass == 1) || (new_pass && !changed))
	...
	{
	  if (!ip2k_elf_relax_section_page
	      (abfd, sec, &changed, &misc, page_start, page_end))
	    return FALSE;
	}
      *again = TRUE;
    }
     ...
     unsigned long page_end;
{
  ...
  for (irel = misc->irelbase; irel < irelend; irel++)
	...
	ip2k_is_switch_table_128 (abfd, sec, irel->r_offset, misc->contents);
      switch_table_256 =
	ip2k_is_switch_table_256 (abfd, sec, irel->r_offset, misc->contents);
     ...
     int noadj;
{
  ...
  for (isym = isymbuf; isym < isymend; isym++)
    {
      if (isym->st_shndx == shndx
	  && addr <= isym->st_value && isym->st_value < endaddr)
	isym->st_value += count;
    }
     ...
     int count;
{
  ...
  memmove (contents + addr, contents + addr + count, endaddr - addr - count);
     ...
     bfd_vma relocation;
{
  ...
  switch (howto->type)
      ...
      else if (ip2k_relaxed)
	{
	  ...
	  if (!ip2k_is_switch_table_128
	      (input_bfd, input_section, rel->r_offset - 2, contents)
	    ...
	    _bfd_error_handler (_
				...
				page_addr, relocation + rel->r_addend);
	}
      if ((relocation & IP2K_INSN_MASK) == IP2K_INSN_VALUE)
	relocation &= ~IP2K_INSN_MASK;
      else
