...
bfd_boolean
_bfd_add_bfd_to_archive_cache (bfd * arch_bfd, file_ptr filepos,
			       bfd * new_elt)
{
...
_bfd_generic_read_ar_hdr_mag (bfd * abfd, const char *mag)
{
  ...
  bfd_size_type allocsize =
    sizeof (struct areltdata) + sizeof (struct ar_hdr);
  char *allocptr = 0;

  if (bfd_bread (hdrp, sizeof (struct ar_hdr), abfd) !=
  ...
  if (strncmp (hdr.ar_fmag, ARFMAG, 2) != 0
      && (mag == NULL || strncmp (hdr.ar_fmag, mag, 2) != 0))
    {
      bfd_set_error (bfd_error_malformed_archive);
      return NULL;
    }
...
bfd_slurp_armap (bfd * abfd)
{
  ...
  if (!strncmp (nextname, "__.SYMDEF       ", 16) || !strncmp (nextname, "__.SYMDEF/      ", 16))	/* old Linux archives */
...
bfd_slurp_bsd_armap_f2 (bfd * abfd)
{
  ...
  stringbase = ((char *) raw_armap
		+ HPUX_SYMDEF_COUNT_SIZE + BSD_STRING_COUNT_SIZE);
...
_bfd_construct_extended_name_table (bfd * abfd,
				    ...
				    char **tabloc, bfd_size_type * tablen)
{
...
bfd_dont_truncate_arname (bfd * abfd, const char *pathname, char *arhdr)
{
  ...
  if (length < maxlen || (length == maxlen && length < sizeof hdr->ar_name))
  ...
  for (current = arch->archive_head; current; current = current->next)
    {
      ...
      /* Write ar header.  */
      if (bfd_bwrite (hdr, sizeof (*hdr), arch) != sizeof (*hdr))
	return FALSE;
...
_bfd_compute_and_write_armap (bfd * arch, unsigned int elength)
{
  ...
  for (current = arch->archive_head;
       current != NULL; current = current->next, elt_no++)
    {
      if (bfd_check_format (current, bfd_object)
	  ...
	  if (storage != 0)
	      ...
	      for (src_count = 0; src_count < symcount; src_count++)
		{
		  ...
		  if ((flags & BSF_GLOBAL ||
		       flags & BSF_WEAK ||
		       flags & BSF_INDIRECT ||
		       bfd_is_com_section (sec)) && !bfd_is_und_section (sec))
		    {
  ...
  ret = BFD_SEND (arch, write_armap, (arch, elength, map, orl_count, stridx));
		 ...
		 struct orl * map, unsigned int orl_count, int stridx)
{
  ...
  for (count = 0; count < orl_count; count++)
      ...
      if (map[count].u.abfd != last_elt)
	{
      ...
      if (bfd_bwrite (buf, BSD_SYMDEF_SIZE, arch) != BSD_SYMDEF_SIZE)
	return FALSE;
    }
...
coff_write_armap (bfd * arch,
		  unsigned int elength,
		  struct orl * map, unsigned int symbol_count, int stridx)
{
  ...
  archive_member_file_ptr = (mapsize
			     + elength + sizeof (struct ar_hdr) + SARMAG);
