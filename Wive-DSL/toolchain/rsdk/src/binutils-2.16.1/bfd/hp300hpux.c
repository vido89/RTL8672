...
PARAMS ((bfd *));
     ...
     bfd_boolean MY (slurp_symbol_table) PARAMS ((bfd *));
     void MY (swap_std_reloc_in)
  PARAMS ((bfd *, struct hp300hpux_reloc *, arelent *, asymbol **,
	   bfd_size_type));
     bfd_boolean MY (slurp_reloc_table) PARAMS ((bfd *, sec_ptr, asymbol **));
     long MY (canonicalize_symtab) PARAMS ((bfd *, asymbol **));
...
/* Set parameters about this a.out file that are machine-dependent.
   This routine is called from some_aout_object_p just before it returns.  */
     static const bfd_target *MY (callback) (abfd)
     bfd *abfd;
{
...
extern bfd_boolean aout_32_write_syms PARAMS ((bfd * abfd));
...
bfd_boolean MY (slurp_symbol_table) (abfd)
     bfd *
       abfd;
{
...
void MY (swap_std_reloc_in) (abfd, bytes, cache_ptr, symbols, symcount)
     bfd *
       abfd;
...
bfd_boolean MY (slurp_reloc_table) (abfd, asect, symbols)
     bfd *
       abfd;
     sec_ptr
       asect;
...
PARAMS ((bfd * abfd, sec_ptr section, arelent ** relptr, asymbol ** symbols));

long MY (canonicalize_symtab) (abfd, location)
     bfd *
       abfd;
     asymbol **
       location;
...
long MY (get_symtab_upper_bound) (abfd)
     bfd *
       abfd;
{
  if (obj_aout_subformat (abfd) == gnu_encap_format)
...
long MY (canonicalize_reloc) (abfd, section, relptr, symbols)
     bfd *
       abfd;
     sec_ptr
       section;
