...
static reloc_howto_type elf32_arm_vtinherit_howto = HOWTO (R_ARM_GNU_VTINHERIT,	/* type */
							   0,	/* rightshift */
							   2,	/* size (0 = byte, 1 = short, 2 = long) */
							   0,	/* bitsize */
							   FALSE,	/* pc_relative */
...
static reloc_howto_type elf32_arm_vtentry_howto = HOWTO (R_ARM_GNU_VTENTRY,	/* type */
							 0,	/* rightshift */
							 2,	/* size (0 = byte, 1 = short, 2 = long) */
							 0,	/* bitsize */
							 FALSE,	/* pc_relative */
  ...
  /* 12 bit pc relative */
static reloc_howto_type elf32_arm_thm_pc11_howto = HOWTO (R_ARM_THM_PC11,	/* type */
							  1,	/* rightshift */
							  1,	/* size (0 = byte, 1 = short, 2 = long) */
							  11,	/* bitsize */
							  TRUE,	/* pc_relative */
  ...
  /* 12 bit pc relative */
static reloc_howto_type elf32_arm_thm_pc9_howto = HOWTO (R_ARM_THM_PC9,	/* type */
							 1,	/* rightshift */
							 1,	/* size (0 = byte, 1 = short, 2 = long) */
							 8,	/* bitsize */
							 TRUE,	/* pc_relative */
...
static reloc_howto_type elf32_arm_got_prel = HOWTO (R_ARM_GOT_PREL,	/* type */
						    0,	/* rightshift */
						    2,	/* size (0 = byte, 1 = short, 2 = long) */
						    32,	/* bitsize */
						    TRUE,	/* pc_relative */
...
elf32_arm_create_dynamic_sections (bfd * dynobj, struct bfd_link_info *info)
{
  ...
  if (!htab->splt
      || !htab->srelplt
      || !htab->sdynbss || (!info->shared && !htab->srelbss))
    abort ();
			  ...
			  struct elf_link_hash_entry *h)
{
  ...
  tmp_name =
    bfd_malloc ((bfd_size_type) strlen (name) +
		strlen (ARM2THUMB_GLUE_ENTRY_NAME) + 1);
  ...
  myh = elf_link_hash_lookup (&(globals)->root, tmp_name, FALSE, FALSE, TRUE);
...
#define BACK_FROM_ARM "__%s_back_from_arm"
  ...
  val = hash_table->thumb_glue_size + 4,
    _bfd_generic_link_add_one_symbol (link_info,
				      hash_table->bfd_of_glue_owner, tmp_name,
...
bfd_boolean
bfd_elf32_arm_get_bfd_for_interworking (bfd * abfd,
					struct bfd_link_info * info)
{
  struct elf32_arm_link_hash_table *globals;
					 ...
					 int byteswap_code)
{
  ...
  for (; sec != NULL; sec = sec->next)
      ...
      for (irel = internal_relocs; irel < irelend; irel++)
	{
	  ...
	  if (r_type != R_ARM_PC24 && r_type != R_ARM_PLT32
#ifndef OLD_ARM_ABI
	      && r_type != R_ARM_CALL && r_type != R_ARM_JUMP24
#endif
	  ...
	  h = (struct elf_link_hash_entry *) elf_sym_hashes (abfd)[r_index];
...
bfd_elf32_arm_set_target_relocs (struct bfd_link_info *link_info,
				 int target1_is_rel,
				 char *target2_type, int fix_v4bx)
{
  struct elf32_arm_link_hash_table *globals;
...
elf32_thumb_to_arm_stub (struct bfd_link_info *info,
			 ...
			 bfd_byte * hit_data,
			 asection * sym_sec,
			 bfd_vma offset, bfd_signed_vma addend, bfd_vma val)
{
  asection *s = 0;
  bfd_vma my_offset;
  ...
  if ((my_offset & 0x01) == 0x01)
    {
      if (sym_sec != NULL
	  && sym_sec->owner != NULL && !INTERWORK_FLAG (sym_sec->owner))
	{
	  (*_bfd_error_handler)
	...
	((bfd_signed_vma) val) - ((bfd_signed_vma)
...
static int
arm_real_reloc_type (struct elf32_arm_link_hash_table *globals, int r_type)
{
  switch (r_type)
    {
    case R_ARM_TARGET1:
			       ...
			       bfd_boolean * unresolved_reloc_p)
{
  ...
  switch (r_type)
	...
	return bfd_reloc_ok;
	  ...
	  && h != NULL && splt != NULL && h->plt.offset != (bfd_vma) - 1)
	{
...
#ifndef OLD_ARM_ABI
	  && r_type != R_ARM_CALL
	  && r_type != R_ARM_JUMP24 && r_type != R_ARM_PREL31
#endif
	  && r_type != R_ARM_PLT32)
	{
	  ...
	  *unresolved_reloc_p = FALSE;
	  ...
	  if (skip)
	      ...
	      if (globals->symbian_p)
		{
		  ...
		  symbol =
		    elf_section_data (sym_sec->output_section)->dynindx;
		  BFD_ASSERT (symbol != 0);
		}
	      else
	...
	switch (r_type)
	  ...
	  case R_ARM_PLT32:
		...
		if (signed_addend > ((bfd_signed_vma) (howto->dst_mask >> 1))
		    || signed_addend <
		    -((bfd_signed_vma) ((howto->dst_mask + 1) >> 1)))
...
#ifndef OLD_ARM_ABI
	if (r_type == R_ARM_THM_XPC22 && ((lower_insn & 0x1800) == 0x0800))
	...
	upper_insn =
	  (upper_insn & ~(bfd_vma) 0x7ff) | ((relocation >> 12) & 0x7ff);
	lower_insn =
	  (lower_insn & ~(bfd_vma) 0x7ff) | ((relocation >> 1) & 0x7ff);
...
arm_add_to_rel (bfd * abfd,
		bfd_byte * address,
		reloc_howto_type * howto, bfd_signed_vma increment)
{
  ...
  for (; rel < relend; rel++)
    {
      ...
      if (r_type == R_ARM_GNU_VTENTRY || r_type == R_ARM_GNU_VTINHERIT)
	continue;
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (globals->use_rel)
		  ...
		  && ELF_ST_TYPE (sym->st_info) == STT_SECTION)
		{
		  ...
		  value =
		    (value & ~howto->dst_mask) | (addend & howto->dst_mask);
	  ...
	  && !((input_section->flags & SEC_DEBUGGING) != 0 && h->def_dynamic))
	{
	  (*_bfd_error_handler)
	    (_
	     ("%B(%A+0x%lx): warning: unresolvable relocation %d against symbol `%s'"),
	     input_bfd, input_section, (long) rel->r_offset, r_type,
	     h->root.root.string);
	  return FALSE;
	}
...
elf32_arm_merge_private_bfd_data (bfd * ibfd, bfd * obfd)
{
  ...
  if (!(ibfd->flags & DYNAMIC))
      ...
      for (sec = ibfd->sections; sec != NULL; sec = sec->next)
	{
	  ...
	  if (strcmp (sec->name, ".glue_7") && strcmp (sec->name, ".glue_7t"))
	    {
  ...
  if (EF_ARM_EABI_VERSION (in_flags) != EF_ARM_EABI_VERSION (out_flags))
	...
	(_
	 ...
	 ibfd, obfd, (in_flags & EF_ARM_EABIMASK) >> 24,
  ...
  if (EF_ARM_EABI_VERSION (in_flags) == EF_ARM_EABI_UNKNOWN)
      ...
      if ((in_flags & EF_ARM_APCS_26) != (out_flags & EF_ARM_APCS_26))
	{
	  _bfd_error_handler
	    (_
	     ...
	     ibfd, obfd, in_flags & EF_ARM_APCS_26 ? 26 : 32,
      ...
      if ((in_flags & EF_ARM_VFP_FLOAT) != (out_flags & EF_ARM_VFP_FLOAT))
	{
	  ...
	  (out_flags & EF_ARM_MAVERICK_FLOAT))
	{
	  if (in_flags & EF_ARM_MAVERICK_FLOAT)
	    _bfd_error_handler
	...
	case R_ARM_THM_PC22:
	  ...
	  if (h != NULL)
		...
		h->non_got_ref = 1;
		  ...
		  || r_type == R_ARM_JUMP24 || r_type == R_ARM_PREL31
#endif
		  || r_type == R_ARM_PLT32 || r_type == R_ARM_THM_PC22)
		h->needs_plt = 1;
	  ...
	  if ((info->shared || htab->root.is_relocatable_executable)
		...
		{
		  head =
		    &((struct elf32_arm_link_hash_entry *) h)->relocs_copied;
		}
	      else
		{
...
static bfd_boolean
elf32_arm_is_target_special_symbol (bfd * abfd ATTRIBUTE_UNUSED,
				    asymbol * sym)
{
				 ...
				 struct elf_link_hash_entry *h)
{
  ...
  if (h->type == STT_FUNC || h->type == STT_ARM_TFUNC || h->needs_plt)
    {
      if (h->plt.refcount <= 0
	  || SYMBOL_CALLS_LOCAL (info, h)
	  || (ELF_ST_VISIBILITY (h->other) != STV_DEFAULT
				 ...
				 Elf_Internal_Sym * sym)
{
  ...
  if (h->plt.offset != (bfd_vma) - 1)
      ...
      else
	{
	  ...
	  got_displacement = (sgot->output_section->vma
			      ...
			      - splt->output_offset - h->plt.offset - 8);
	  ...
	  bfd_put_32 (output_bfd,
		      ...
		      splt->contents + h->plt.offset + 4);
	  bfd_put_32 (output_bfd,
		      elf32_arm_plt_entry[2] | (got_displacement &
						0x00000fff),
	  ...
	  bfd_put_32 (output_bfd,
		      (splt->output_section->vma
		       + splt->output_offset), sgot->contents + got_offset);
  ...
  if (h->got.offset != (bfd_vma) - 1)
      ...
      else
	{
      ...
      loc =
	srel->contents + srel->reloc_count++ * sizeof (Elf32_External_Rel);
      bfd_elf32_swap_reloc_out (output_bfd, &rel, loc);
    }
...
static void
elf32_arm_post_process_headers (bfd * abfd,
				struct bfd_link_info *link_info
				ATTRIBUTE_UNUSED)
{
...
static void
elf32_arm_symbol_processing (bfd * abfd ATTRIBUTE_UNUSED, asymbol * asym)
{
...
elf32_arm_swap_symbol_in (bfd * abfd,
			  const void *psrc,
			  const void *pshn, Elf_Internal_Sym * dst)
{
  bfd_elf32_swap_symbol_in (abfd, psrc, pshn, dst);

  /* New EABI objects mark thumb function symbols by setting the low bit of
     the address.  Turn these into STT_ARM_TFUNC.  */
  if (ELF_ST_TYPE (dst->st_info) == STT_FUNC && (dst->st_value & 1))
    {
