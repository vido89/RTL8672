			     ...
			     int is_local ATTRIBUTE_UNUSED)
{
  ...
  if (((bfd_vma) check & ~reloc_bits) != 0
      && (((bfd_vma) check & ~reloc_bits) != (-(bfd_vma) 1 & ~reloc_bits)))
    {
      ...
      if (howto->rightshift && (bfd_signed_vma) Rvalue < 0)
	{
	  ...
	  if (((bfd_vma) check & ~reloc_bits) != (-(bfd_vma) 1 & ~reloc_bits))
  ...
  switch (howto->size)
    ...
    case 2:
	  ...
	  || r_type == R_CRX_REGREL22 || r_type == R_CRX_REGREL28)
	  ...
	  if ((long) value < 0x10000 && (long) value > -0x10002)
	    {
	      ...
	      /* Get the opcode.  */
	      code =
		(unsigned short) bfd_get_16 (abfd, contents + irel->r_offset);
