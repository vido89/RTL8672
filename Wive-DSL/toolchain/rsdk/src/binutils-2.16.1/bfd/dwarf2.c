...
read_n_bytes (bfd * abfd ATTRIBUTE_UNUSED,
	      bfd_byte * buf, unsigned int size ATTRIBUTE_UNUSED)
{
...
read_string (bfd * abfd ATTRIBUTE_UNUSED,
	     bfd_byte * buf, unsigned int *bytes_read_ptr)
{
  ...
  if (offset >= stash->dwarf_str_size)
    {
      (*_bfd_error_handler) (_
			     ("Dwarf Error: DW_FORM_strp offset (%lu) greater than or equal to .debug_str size (%lu)."),
...
read_abbrevs (bfd * abfd, bfd_uint64_t offset, struct dwarf2_debug *stash)
{
  ...
  if (offset >= stash->dwarf_abbrev_size)
    {
      (*_bfd_error_handler) (_
			     ("Dwarf Error: Abbrev offset (%lu) greater than or equal to .debug_abbrev size (%lu)."),
...
add_line_info (struct line_info_table *table,
	       bfd_vma address,
	       char *filename,
	       unsigned int line, unsigned int column, int end_sequence)
{
...
decode_line_info (struct comp_unit *unit, struct dwarf2_debug *stash)
{
  ...
  if (unit->line_offset >= stash->dwarf_line_size)
    {
      (*_bfd_error_handler) (_
			     ("Dwarf Error: Line offset (%lu) greater than or equal to .debug_line size (%lu)."),
				  ...
				  const char **functionname_ptr)
{
  ...
  for (each_func = table; each_func; each_func = each_func->prev_func)
    {
  ...
  if (abbrev_number)
      ...
      else
	{
	  for (i = 0; i < abbrev->num_attrs && !name; ++i)
	    {
	      info_ptr =
		read_attribute (&attr, &abbrev->attrs[i], unit, info_ptr);
	      switch (attr.name)
		{
		case DW_AT_name:
		  name = attr.u.str;
...
parse_comp_unit (bfd * abfd,
		 struct dwarf2_debug *stash,
		 bfd_vma unit_length,
		 bfd_byte * info_ptr_unit, unsigned int offset_size)
{
  ...
  if (version != 2)
    {
      (*_bfd_error_handler) (_
			     ("Dwarf Error: found dwarf version '%u', this reader only handles version 2 information."),
			     version);
      bfd_set_error (bfd_error_bad_value);
      return 0;
    }
...
find_debug_info (bfd * abfd, asection * after_sec)
{
  ...
  while (msec)
	...
	return msec;
      ...
      if (strncmp (msec->name, GNU_LINKONCE_INFO, strlen (GNU_LINKONCE_INFO))
	  == 0)
	return msec;
...
_bfd_dwarf2_find_nearest_line (bfd * abfd,
			       ...
			       const char **functionname_ptr,
			       unsigned int *linenumber_ptr,
			       unsigned int addr_size, void **pinfo)
{
  ...
  if (!stash)
	...
	return FALSE;
      ...
      for (msec = find_debug_info (abfd, NULL);
	   msec; msec = find_debug_info (abfd, msec))
	{
	  bfd_size_type size;
	  bfd_size_type start;
