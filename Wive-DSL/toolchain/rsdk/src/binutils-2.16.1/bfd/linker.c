...
void
  ...
  (struct bfd_link_hash_table *table,
   bfd_boolean (*func) (struct bfd_link_hash_entry *, void *), void *info)
{
  ...
  while (*pun != NULL)
    {
      ...
      if (h->type == bfd_link_hash_new || h->type == bfd_link_hash_undefweak)
	{
	  *pun = h->u.undef.next;
	  h->u.undef.next = NULL;
...
bfd_boolean
_bfd_generic_link_add_symbols_collect (bfd * abfd,
				       struct bfd_link_info * info)
{
...
generic_link_add_symbols (bfd * abfd,
			  struct bfd_link_info *info, bfd_boolean collect)
{
  bfd_boolean ret;
...
generic_link_add_symbol_list (bfd * abfd,
			      ...
			      asymbol ** symbols, bfd_boolean collect)
{
  asymbol **pp, **ppend;
  ...
  for (; pp < ppend; pp++)
	  ...
	  || bfd_is_ind_section (bfd_get_section (p)))
	{
	  ...
	  if (((p->flags & BSF_INDIRECT) != 0
	       || bfd_is_ind_section (p->section)) && pp + 1 < ppend)
	    {
	      pp++;
	      string = bfd_asymbol_name (*pp);
	    }
	  else if ((p->flags & BSF_WARNING) != 0 && pp + 1 < ppend)
	    {
				  ...
				  struct bfd_link_hash_entry ** hashp)
{
  ...
  if (bfd_is_ind_section (section) || (flags & BSF_INDIRECT) != 0)
    row = INDR_ROW;
...
#define CONS_PREFIX_LEN (sizeof CONS_PREFIX - 1)
	...
	case MDEF:
	  ...
	  if (!info->allow_multiple_definition)
		...
		default:
		  ...
		  && bfd_is_abs_section (section) && value == mval)
		break;
	...
	case IND:
	  /* Create an indirect symbol.  */
	  {
	    ...
	    if (inh == NULL)
	      return FALSE;
	    if (inh->type == bfd_link_hash_indirect && inh->u.i.link == h)
	      {
		(*_bfd_error_handler)
...
static bfd_boolean
generic_add_output_symbol (bfd * output_bfd, size_t * psymalloc,
			   asymbol * sym)
{
      ...
      for (sec = input_bfd->sections; sec != NULL; sec = sec->next)
	{
	  if (sec->output_section == info->create_object_symbols_section)
		...
		return FALSE;
	      ...
	      if (!generic_add_output_symbol (output_bfd, psymalloc, newsym))
		return FALSE;
      ...
      else if ((sym->flags & BSF_LOCAL) != 0)
	{
	  if ((sym->flags & BSF_WARNING) != 0)
		...
		case discard_sec_merge:
		  output = TRUE;
		  if (info->relocatable || !(sym->section->flags & SEC_MERGE))
		    break;
		  /* FALLTHROUGH */
		case discard_l:
				       ...
				       void *data)
{
  ...
  if (!generic_add_output_symbol (wginfo->output_bfd, wginfo->psymalloc, sym))
    {
...
_bfd_default_link_order (bfd * abfd,
			 struct bfd_link_info * info,
			 asection * sec, struct bfd_link_order * link_order)
{
  switch (link_order->type)
    ...
    default:
      abort ();
    case bfd_indirect_link_order:
      return default_indirect_link_order (abfd, info, sec, link_order, FALSE);
