	...
	case R_386_PC32:
	  ...
	  if ((info->shared
	      ...
	      if (sreloc == NULL)
		{
		  ...
		  name =
		    bfd_elf_string_from_elf_section (abfd, strndx, shnam);
		  if (name == NULL)
		    return FALSE;
...
allocate_dynrelocs (struct elf_link_hash_entry *h, void *inf)
{
  ...
  if (h->got.refcount > 0
      && !info->shared
      && h->dynindx == -1 && (elf_i386_hash_entry (h)->tls_type & GOT_TLS_IE))
  ...
  for (; rel < relend; rel++)
    {
      ...
      r_type = ELF32_R_TYPE (rel->r_info);
      if (r_type == R_386_GNU_VTINHERIT || r_type == R_386_GNU_VTENTRY)
	continue;
	...
	case R_386_TLS_GOTIE:
	  ...
	  if ((off & 1) != 0)
		...
		bfd_put_32 (output_bfd, 0, htab->sgot->contents + off);
	      ...
	      loc +=
		htab->srelgot->reloc_count++ * sizeof (Elf32_External_Rel);
	      bfd_elf32_swap_reloc_out (output_bfd, &outrel, loc);
		  ...
		  if (indx == 0)
		  ...
		  else
		    {
		      bfd_put_32 (output_bfd, 0,
				  htab->sgot->contents + off + 4);
		      outrel.r_info = ELF32_R_INFO (indx, R_386_TLS_DTPOFF32);
		      outrel.r_offset += 4;
	  ...
	  if (r_type == ELF32_R_TYPE (rel->r_info))
	      ...
	      else
		{
	      ...
	      memcpy (contents + roff, "\x65\xa1\0\0\0\0\x2b\x80\0\0\0", 12);
  ...
  if (h->plt.offset != (bfd_vma) - 1)
      ...
      else
	{
	  memcpy (htab->splt->contents + h->plt.offset,
		  elf_i386_pic_plt_entry, PLT_ENTRY_SIZE);
      ...
      bfd_put_32 (output_bfd,
		  (htab->splt->output_section->vma
		   ...
		   + h->plt.offset
		   + 6), htab->sgotplt->contents + got_offset);
