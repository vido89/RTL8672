...
bfd_is_local_label (bfd * abfd, asymbol * sym)
{
  ...
  if ((sym->flags & (BSF_GLOBAL | BSF_WEAK | BSF_FILE | BSF_SECTION_SYM)) !=
      0)
    return FALSE;
  if (sym->name == NULL)
    return FALSE;
  ...
  if (info != NULL)
      ...
      if (info->stabsec == NULL || info->strsec == NULL)
	{
      ...
      stabsize = (info->stabsec->rawsize
		  ? info->stabsec->rawsize : info->stabsec->size);
      strsize = (info->strsec->rawsize
		 ? info->strsec->rawsize : info->strsec->size);
    }
  else
    {
	...
	free (reloc_vector);
      ...
      saw_fun = 1;
      for (stab = info->stabs; stab < info->stabs + stabsize;
	   stab += STABSIZE)
	{
	  ...
	  switch (stab[TYPEOFF])
	      ...
	      if (saw_fun == 0)
		{
		  info->indextable[i].val =
		    bfd_get_32 (abfd, last_stab + VALOFF);
