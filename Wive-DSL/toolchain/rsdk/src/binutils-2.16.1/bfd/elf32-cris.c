		...
		if (!elf_hash_table (info)->dynamic_sections_created
			...
			&& h->def_regular))
		  {
		    ...
		    BFD_ASSERT (!elf_hash_table (info)->
				dynamic_sections_created || info->shared
				|| h->def_regular || h->type == STT_FUNC
				|| h->needs_plt
				|| h->root.type == bfd_link_hash_undefweak);
		...
		if (h == NULL)
		  (*_bfd_error_handler)
		    (_("%B, section %A: relocation %s with non-zero addend %d"
		     ...
		     input_bfd,
		     input_section,
		     cris_elf_howto_table[r_type].name, rel->r_addend);
		else
		  (*_bfd_error_handler)
		    (_("%B, section %A: relocation %s with non-zero addend %d"
		       " against symbol `%s'"),
		     ...
		     rel->r_addend,
		     symname[0] !=
		     '\0' ? symname : _("[whose name is lost]"));
	...
	case R_CRIS_32_GOTREL:
	  ...
	  if (h != NULL
		...
		(_("%B, section %A: relocation %s is"
		 ...
		 input_bfd,
		 input_section, cris_elf_howto_table[r_type].name, symname);
	      bfd_set_error (bfd_error_bad_value);
	      return FALSE;
	    }
		  ...
		  if (sreloc == NULL)
		    {
		      (*_bfd_error_handler)
			(_
			 ("%B: Internal inconsistency; no relocation section %s"),
			 input_bfd, name);
      ...
      if (has_gotplt)
	{
	  ...
	  bfd_put_32 (output_bfd,
		      (splt->output_section->vma
		       + splt->output_offset
		       + h->plt.offset
		       + plt_stub_offset), sgotplt->contents + got_offset);
	  ...
	  if (bfd_get_mach (output_bfd) == bfd_mach_cris_v32)
	      ...
	      else
		{
		  ...
		  bfd_put_32 (output_bfd,
			      ...
			      + sgot->output_offset + 4, splt->contents + 4);
	      ...
	      else
		{
		  ...
		  bfd_put_32 (output_bfd,
			      sgot->output_section->vma
			      + sgot->output_offset + 4, splt->contents + 6);
		  bfd_put_32 (output_bfd,
			      sgot->output_section->vma
			      + sgot->output_offset + 8, splt->contents + 14);
  ...
  for (rel = relocs; rel < rel_end; rel++)
	      ...
	      if (bfd_get_mach (dynobj) == bfd_mach_cris_v10_v32)
		{
		  (*_bfd_error_handler)
		    (_("%B, section %A:\n  v10/v32 compatible object %s"
		       " must not contain a PIC relocation"), abfd, sec);
		  return FALSE;
		}
	  ...
	  if (local_got_refcounts == NULL)
	    {
	      ...
	      local_got_refcounts =
		((bfd_signed_vma *) bfd_zalloc (abfd, amt));
	      if (local_got_refcounts == NULL)
		return FALSE;
	...
	case R_CRIS_32_PCREL:
	  ...
	  if (r_type == R_CRIS_8_PCREL
	      || r_type == R_CRIS_16_PCREL || r_type == R_CRIS_32_PCREL)
	    {
	  ...
	  if ((r_type == R_CRIS_8_PCREL
	       || r_type == R_CRIS_16_PCREL
	       || r_type == R_CRIS_32_PCREL) && info->symbolic)
	    {
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
  ...
  else
    {
      ...
      elf_cris_link_hash_traverse (elf_cris_hash_table (info),
				   elf_cris_adjust_gotplt_to_got, (PTR) info);
     ...
     PTR inf;
{
  ...
  /* If a symbol has been forced local or we have found a regular
     ...
     any relocs.  */
  if (h->root.def_regular && (h->root.forced_local || info->symbolic))
    {
     ...
     PTR inf;
{
  ...
  if (!h->root.def_dynamic || h->root.plt.refcount > 0)
	  ...
	  && elf_hash_table (info)->dynamic_sections_created)
	{
	  ...
	  && h->root.dynindx != -1
	  && !h->root.def_dynamic && !h->root.ref_dynamic)
	{
	  h->root.dynindx = -1;
