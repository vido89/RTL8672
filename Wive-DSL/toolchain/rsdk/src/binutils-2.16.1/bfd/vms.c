...
static bfd_boolean vms_initialize PARAMS ((bfd *));
static unsigned int priv_section_count;
static bfd_boolean fill_section_ptr PARAMS ((struct bfd_hash_entry *, PTR));
static bfd_boolean vms_fixup_sections PARAMS ((bfd *));
static bfd_boolean copy_symbols PARAMS ((struct bfd_hash_entry *, PTR));
...
static bfd_boolean vms_mkobject PARAMS ((bfd * abfd));
static bfd_boolean vms_write_object_contents PARAMS ((bfd * abfd));
static bfd_boolean vms_close_and_cleanup PARAMS ((bfd * abfd));
static bfd_boolean vms_bfd_free_cached_info PARAMS ((bfd * abfd));
...
static bfd_boolean vms_slurp_armap PARAMS ((bfd * abfd));
static bfd_boolean vms_slurp_extended_name_table PARAMS ((bfd * abfd));
...
static bfd_boolean vms_update_armap_timestamp PARAMS ((bfd * abfd));
static int vms_generic_stat_arch_elt PARAMS ((bfd *, struct stat *));
static long vms_get_symtab_upper_bound PARAMS ((bfd * abfd));
static long vms_canonicalize_symtab PARAMS ((bfd * abfd, asymbol ** symbols));
...
PARAMS ((bfd * abfd, asection * section, const PTR location, file_ptr offset,
	 bfd_size_type count));
static int vms_sizeof_headers PARAMS ((bfd * abfd, bfd_boolean reloc));
...
const bfd_target vms_alpha_vec = {
  ...
  (HAS_RELOC | HAS_SYMS | WP_TEXT | D_PAGED),	/* object flags */
  (SEC_ALLOC | SEC_LOAD | SEC_RELOC | SEC_READONLY | SEC_CODE | SEC_DATA | SEC_HAS_CONTENTS | SEC_IN_MEMORY),	/* sect flags */
...
const bfd_target vms_vax_vec = {
  ...
  (HAS_RELOC | HAS_SYMS		/* object flags */
   | WP_TEXT | D_PAGED | HAS_LINENO | HAS_DEBUG | HAS_LOCALS),
     ...
     bfd *abfd;
{
  ...
  while ((prev_type != EOBJ_S_C_EEOM) && (prev_type != OBJ_S_C_EOM)
	 && (prev_type != OBJ_S_C_EOMW));
...
#if VMS_DEBUG
  vms_debug (1,
	     "vms_get_section_contents_in_window(%p, %s, %p, off %ld, count %d)\n",
...
#if VMS_DEBUG
  vms_debug (1,
	     "vms_find_nearest_line(%p, %s, %p, %ld, <ret>, <ret>, <ret>)\n",
     ...
     bfd_reloc_code_real_type code;
{
  ...
  switch (code)
    ...
    case BFD_RELOC_ALPHA_LINKAGE:
      alpha_type = ALPHA_R_LINKAGE;
      break;
    case BFD_RELOC_ALPHA_CODEADDR:
      alpha_type = ALPHA_R_CODEADDR;
      break;
    default:
...
#if VMS_DEBUG
  vms_debug (1,
	     "vms_set_section_contents(%p, sec %s, loc %p, off %ld, count %d)\n",
...
#if VMS_DEBUG
  vms_debug (1,
	     "vms_bfd_get_relocated_section_contents(%p, %p, %p, %p, %s, %p)\n",
	     abfd, link_info, link_order, data,
	     (relocatable) ? "True" : "False", symbols);
#endif
  return 0;
}
