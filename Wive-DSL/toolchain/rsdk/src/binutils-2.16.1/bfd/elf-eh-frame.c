...
write_value (bfd * abfd, bfd_byte * buf, bfd_vma value, int width)
{
  switch (width)
    {
    case 2:
      bfd_put_16 (abfd, value, buf);
      break;
    case 4:
      bfd_put_32 (abfd, value, buf);
      break;
    case 8:
      bfd_put_64 (abfd, value, buf);
      break;
    default:
      BFD_FAIL ();
    }
...
static int
cie_compare (struct cie *c1, struct cie *c2)
{
  if (c1->hdr.length == c2->hdr.length
      ...
      && memcmp (c1->initial_instructions,
		 c2->initial_instructions, c1->initial_insn_length) == 0)
    return 0;
...
skip_cfa_op (bfd_byte ** iter, bfd_byte * end, unsigned int encoded_ptr_width)
{
  ...
  switch (op & 0x80 ? op & 0xc0 : op)
    ...
    case DW_CFA_def_cfa_sf:
      ...
      return (skip_leb128 (iter, end) && skip_leb128 (iter, end));
...
#define REQUIRE(COND)					\
  ...
  if (sec->size >= 4
      && bfd_get_32 (abfd, ehbuf) == 0 && cookie->rel == cookie->relend)
    {
...
#define GET_RELOC(buf)					\
  ...
  for (;;)
      ...
      if (hdr.id == 0 || hdr.id == (unsigned int) -1)
	{
	  ...
	  if (aug[0] != 'e' || aug[1] != 'h')
		...
		switch (*aug++)
		  ...
		  case 'L':
		    ...
		    ENSURE_NO_RELOCS (buf);
		    REQUIRE (get_DW_EH_PE_width
			     (cie.lsda_encoding, ptr_size));
		    break;
		  case 'R':
	  ...
	  if (info->shared
	      && (get_elf_backend_data (abfd)->
		  elf_backend_can_make_lsda_relative_eh_frame (abfd, info,
							       sec))
...
bfd_boolean
_bfd_elf_discard_section_eh_frame_hdr (bfd * abfd,
  ...
  for (ent = sec_info->entry; ent < sec_info->entry + sec_info->count; ++ent)
      ...
      if (ent->cie)
	{
	  ...
	  if (ent->make_relative
	      || ent->need_lsda_relative || ent->per_encoding_relative)
	    {
	      char *aug;
...
_bfd_elf_write_section_eh_frame_hdr (bfd * abfd, struct bfd_link_info * info)
{
  ...
  contents[1] = get_elf_backend_data (abfd)->elf_backend_encode_eh_address (abfd, info, eh_frame_sec, 0, sec, 4, &encoded_eh_frame);	/* .eh_frame offset.  */
