...
static void remember_hi16s_reloc PARAMS ((bfd *, bfd_vma, bfd_byte *));
...
static bfd_boolean v850_elf_object_p PARAMS ((bfd *));
static bfd_boolean v850_elf_fake_sections
PARAMS ((bfd *, Elf_Internal_Shdr *, asection *));
static void v850_elf_final_write_processing PARAMS ((bfd *, bfd_boolean));
static bfd_boolean v850_elf_set_private_flags PARAMS ((bfd *, flagword));
static bfd_boolean v850_elf_merge_private_bfd_data PARAMS ((bfd *, bfd *));
static bfd_boolean v850_elf_print_private_bfd_data PARAMS ((bfd *, PTR));
static bfd_boolean v850_elf_section_from_bfd_section
PARAMS ((bfd *, asection *, int *));
static void v850_elf_symbol_processing PARAMS ((bfd *, asymbol *));
     ...
     bfd_reloc_code_real_type code;
{
  ...
  for (i = ARRAY_SIZE (v850_elf_reloc_map); i--;)
	...
	BFD_ASSERT (v850_elf_howto_table[elf_reloc_val].type ==
		    elf_reloc_val);
...
#define V850_OTHER_MASK (V850_OTHER_TDA | V850_OTHER_SDA | V850_OTHER_ZDA)
	...
	small_data_common:
	  ...
	  if (h && h->root.type == bfd_link_hash_common
	      && h->root.u.c.p
	      && !strcmp (bfd_get_section_name (abfd, h->root.u.c.p->section),
			  "COMMON"))
	    {
	      asection *section;

	      section = h->root.u.c.p->section =
	  ...
	  fprintf (stderr,
		   ...
		   v850_elf_howto_table[(int) r_type].name, (h
							     && h->root.root.
							     string) ? h->
		   root.root.string : "<unknown>",
		   (h->root.type ==
     ...
     char **err ATTRIBUTE_UNUSED;
{
  ...
  if (obfd != (bfd *) NULL
      ...
      && (!reloc->howto->partial_inplace || reloc->addend == 0))
    {
  ...
  if (bfd_is_und_section (symbol->section)
      && (symbol->flags & BSF_WEAK) == 0 && obfd == NULL)
  ...
  for (; rel < relend; rel++)
    {
      ...
      if (r_type == R_V850_GNU_VTENTRY || r_type == R_V850_GNU_VTINHERIT)
	continue;
     ...
     PTR ptr;
{
  ...
  switch (elf_elfheader (abfd)->e_flags & EF_V850_ARCH)
    ...
    case E_V850_ARCH:
      ...
      break;
    case E_V850E_ARCH:
      fprintf (file, _("v850e architecture"));
      break;
    case E_V850E1_ARCH:
      fprintf (file, _("v850e1 architecture"));
      break;
    }
  ...
  for (; irel < irelend; irel++)
	...
	continue;
      ...
      bfd_elf32_swap_symbol_in (abfd,
				...
				shndx ? shndx +
				ELF32_R_SYM (irel->r_info) : NULL, &isym);
  ...
  for (; esym < esymend; esym++, shndx = (shndx ? shndx + 1 : NULL))
    {
      ...
      if (isym.st_shndx == sec_shndx
	  && isym.st_value >= addr + count && isym.st_value < toaddr)
	{
	  isym.st_value -= count;
      ...
      else if (isym.st_shndx == sec_shndx && isym.st_value < addr + count)
	{
	  if (isym.st_value + isym.st_size >= addr + count
	      && isym.st_value + isym.st_size < toaddr)
	    isym.st_size -= count;

	  if (isym.st_value >= addr && isym.st_value < addr + count)
	    isym.st_value = addr;
     ...
     bfd_boolean *again;
{
  ...
  *again = FALSE;
  ...
  if (link_info->relocatable
      || (sec->flags & SEC_RELOC) == 0 || sec->reloc_count == 0)
    return TRUE;
  ...
  while (addr < sec->size)
      ...
      if (irelalign)
	{
	  ...
	  alignmoveto =
	    BFD_ALIGN (addr - align_pad_size, 1 << irelalign->r_addend);
      ...
      for (irel = internal_relocs; irel < irelend; irel++)
	{
	  ...
	  if (ELF32_R_TYPE (irel->r_info) == (int) R_V850_LONGCALL)
	      ...
	      if (laddr + 16 <= (bfd_vma) sec->size)
		{
		  ...
		  if (no_match < 0 && (insn[2] & JARL_4_MASK) != JARL_4)
		    no_match = 2;
	      ...
	      if (no_match >= 0)
		{
	      ...
	      for (hi_irelfn = internal_relocs; hi_irelfn < irelend;
		   hi_irelfn++)
		if (hi_irelfn->r_offset == laddr + 2
	      ...
	      for (lo_irelfn = internal_relocs; lo_irelfn < irelend;
		   lo_irelfn++)
		...
		if (irelcall->r_offset == laddr + 8
		  ...
		  || lo_irelfn == irelend || irelcall == irelend)
		{
		  ((*_bfd_error_handler)
	      ...
	      if (ELF32_R_SYM (hi_irelfn->r_info) < symtab_hdr->sh_info)
		{
		  ...
		  symval =
		    (isym->st_value + sym_sec->output_section->vma +
	      ...
	      else
		{
		  ...
		  /* An external symbol.  */
		  indx =
		    ELF32_R_SYM (hi_irelfn->r_info) - symtab_hdr->sh_info;
		  h = elf_sym_hashes (abfd)[indx];
		  BFD_ASSERT (h != NULL);
	      ...
	      fprintf (stderr,
			...
			sec->output_offset), symval, addend, foff);
#endif
	      ...
	      else
		{
		  ...
		  fprintf (stderr,
			   ...
			   sec->output_offset, irel->r_addend);
#endif
		}
	      ...
	      if (foff < -0x100 || foff >= 0x100)
		{
		  ...
		  if (ELF32_R_SYM (hi_irelfn->r_info) < symtab_hdr->sh_info)
		    /* If this needs to be changed because of future relaxing,
		       ...
		       relocs.  */
		    bfd_put_32 (abfd,
				0x00000780 | ((addend << 15) & 0xffff0000) |
				((addend >> 17) & 0xf),
				contents + irel->r_offset);
		  else
		    /* We can't fully resolve this yet, because the external
		  ...
		  lo_irelfn->r_info =
		    ELF32_R_INFO (ELF32_R_SYM (lo_irelfn->r_info),
				  R_V850_NONE);
		  if (!v850_elf_relax_delete_bytes
		      (abfd, sec, irel->r_offset + 4, toaddr, 6))
		    goto error_return;
	      ...
	      else
		{
		  ...
		  lo_irelfn->r_info =
		    ELF32_R_INFO (ELF32_R_SYM (lo_irelfn->r_info),
				  R_V850_NONE);
		  if (!v850_elf_relax_delete_bytes
		      (abfd, sec, irel->r_offset + 2, toaddr, 8))
		    goto error_return;
  ...
  if (!irelalign)
    {
      ...
      fprintf (stderr, "relax pad %d shorten %d -> %d\n",
	       align_pad_size, sec->size, sec->size - align_pad_size);
#endif
...
static struct bfd_elf_special_section const v850_elf_special_sections[] = {
  {".sdata", 6, -2, SHT_PROGBITS, (SHF_ALLOC + SHF_WRITE + SHF_V850_GPREL)},
  {".rosdata", 8, -2, SHT_PROGBITS, (SHF_ALLOC + SHF_V850_GPREL)},
  {".sbss", 5, -2, SHT_NOBITS, (SHF_ALLOC + SHF_WRITE + SHF_V850_GPREL)},
  {".scommon", 8, -2, SHT_V850_SCOMMON, (SHF_ALLOC + SHF_WRITE
					 + SHF_V850_GPREL)},
  {".tdata", 6, -2, SHT_PROGBITS, (SHF_ALLOC + SHF_WRITE + SHF_V850_EPREL)},
  {".tbss", 5, -2, SHT_NOBITS, (SHF_ALLOC + SHF_WRITE + SHF_V850_EPREL)},
  {".tcommon", 8, -2, SHT_V850_TCOMMON, (SHF_ALLOC + SHF_WRITE
					 + SHF_V850_R0REL)},
  {".zdata", 6, -2, SHT_PROGBITS, (SHF_ALLOC + SHF_WRITE + SHF_V850_R0REL)},
  {".rozdata", 8, -2, SHT_PROGBITS, (SHF_ALLOC + SHF_V850_R0REL)},
  {".zbss", 5, -2, SHT_NOBITS, (SHF_ALLOC + SHF_WRITE + SHF_V850_R0REL)},
  {".zcommon", 8, -2, SHT_V850_ZCOMMON, (SHF_ALLOC + SHF_WRITE
					 + SHF_V850_R0REL)},
  {".call_table_data", 16, 0, SHT_PROGBITS, (SHF_ALLOC + SHF_WRITE)},
