			 ...
			 sh64_elf_crange * rangep)
{
  ...
  if ((bfd_get_section_flags (cranges->owner, cranges) & SEC_IN_MEMORY)
	...
	goto error_return;
      ...
      /* Is it sorted?  */
      if (elf_section_data (cranges)->this_hdr.sh_type != SHT_SH5_CR_SORTED)
	/* Nope.  Lets sort it.  */
  ...
  if (found_rangep)
    {
      enum sh64_elf_cr_type cr_type = bfd_get_16 (cranges->owner,
						  SH64_CRANGE_CR_TYPE_OFFSET +
						  found_rangep);
      bfd_vma cr_addr = bfd_get_32 (cranges->owner,
				    SH64_CRANGE_CR_ADDR_OFFSET
				    + (char *) found_rangep);
      bfd_size_type cr_size = bfd_get_32 (cranges->owner,
...
sh64_get_contents_type (asection * sec, bfd_vma addr,
			sh64_elf_crange * rangep)
{
  asection *cranges;
