     ...
     struct dwarf1_debug *stash;
{
  ...
  struct dwarf1_unit *x =
    (struct dwarf1_unit *) bfd_zalloc (stash->abfd, amt);
     ...
     struct dwarf1_unit *aUnit;
{
  ...
  struct dwarf1_func *x =
    (struct dwarf1_func *) bfd_zalloc (stash->abfd, amt);
     ...
     char *aDiePtrEnd;
{
  ...
  xptr += 4;
  if (aDieInfo->length == 0 || (this_die + aDieInfo->length) >= aDiePtrEnd)
    return FALSE;
  ...
  while (xptr < (this_die + aDieInfo->length))
	...
	case FORM_REF:
	  if (attr == AT_sibling)
	    ...
	    {
	      aDieInfo->stmt_list_offset =
		bfd_get_32 (abfd, (bfd_byte *) xptr);
     ...
     struct dwarf1_unit *aUnit;
{
  ...
  if (aUnit->first_child)
    for (eachDie = aUnit->first_child; eachDie < stash->debug_section_end;)
      {
  ...
  if (aUnit->low_pc <= addr && addr < aUnit->high_pc)
      ...
      if (aUnit->has_stmt_list)
	{
	  ...
	  for (eachFunc = aUnit->func_list;
	       eachFunc; eachFunc = eachFunc->prev)
	    {
	      if (eachFunc->low_pc <= addr && addr < eachFunc->high_pc)
		{
     ...
     unsigned int *linenumber_ptr;
{
  ...
  *linenumber_ptr = 0;
  ...
  while (stash->currentDie < stash->debug_section_end)
	...
	{
	  struct dwarf1_unit *aUnit = alloc_dwarf1_unit (stash);
