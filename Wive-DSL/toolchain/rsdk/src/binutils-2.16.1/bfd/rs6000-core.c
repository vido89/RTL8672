     ...
     bfd *abfd;
{
  ...
  if (!(c_flag & UBLOCK_VALID) || !(c_flag & LE_VALID))
    {
      bfd_set_error (bfd_error_wrong_format);
      return NULL;
    }
  ...
  if (!make_bfd_asection (abfd, ".reg",
			  SEC_HAS_CONTENTS, c_regsize, (bfd_vma) 0, c_regoff))
    goto fail;
  ...
  if (!make_bfd_asection (abfd, ".ldinfo",
			  SEC_HAS_CONTENTS, c_lsize, (bfd_vma) 0, c_loader))
    goto fail;
     ...
     regions.  */
  {
	...
	if (!make_bfd_asection (abfd, ".data",
				SEC_ALLOC | SEC_LOAD | SEC_HAS_CONTENTS,
				c_datasize,
				(bfd_vma) CDATA_ADDR (c_datasize), c_data))
	  goto fail;
      }
	...
	for (i = 0; i < c_vmregions; i++)
	  {
	    ...
	    size =
	      CORE_NEW (core) ? sizeof (vminfo.new) : sizeof (vminfo.old);
