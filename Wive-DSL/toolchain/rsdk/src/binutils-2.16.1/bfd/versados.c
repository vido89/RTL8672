...
static long versados_get_reloc_upper_bound PARAMS ((bfd *, sec_ptr));
     ...
     int pass;
{
  ...
  unsigned int bits = (otr->map[0] << 24)
    | (otr->map[1] << 16) | (otr->map[2] << 8) | (otr->map[3] << 0);
	      ...
	      for (j = 0; j < esdids; j++)
		{
		  ...
		  if (esdid)
		      ...
		      else
			{
			  arelent *n =
			    EDATA (abfd,
				   otr->esdid - 1).section->relocation + rn;
			  n->address = dst_idx;

			  n->sym_ptr_ptr = (asymbol **) (size_t) esdid;
			  n->addend = 0;
			  n->howto =
			    versados_howto_table + ((j & 1) * 2) +
			    (sizeinwords - 1);
			}
		    }
		}
     ...
     bfd *abfd;
{
  ...
  if (ext.type != VHEADER || ext.lang > 10)
    {
      bfd_set_error (bfd_error_wrong_format);
      return NULL;
    }
     ...
     asymbol **alocation;
{
  ...
  for (i = 0, s = VDATA (abfd)->symbols; i < symcount; s++, i++)
    {
      *alocation++ = s;
    }
