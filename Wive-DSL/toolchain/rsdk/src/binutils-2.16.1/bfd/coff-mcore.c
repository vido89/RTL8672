  ...
  switch (code)
    {
      ...
      HOW2MAP (BFD_RELOC_MCORE_PCREL_IMM11BY2,
      ...
      HOW2MAP (BFD_RELOC_MCORE_PCREL_JSR_IMM11BY2,
  ...
  for (; rel < relend; rel++)
      ...
      if (h == NULL)
	{
	  if (symndx == -1)
		...
		my_name =
		  obj_coff_strings (input_bfd) + sym->_n._n_n._n_offset;
	      else
		{
...
CREATE_BIG_COFF_TARGET_VEC (TARGET_BIG_SYM, TARGET_BIG_NAME, D_PAGED,
			    (SEC_CODE | SEC_DATA | SEC_DEBUGGING |
			     SEC_READONLY | SEC_LINK_ONCE |
			     SEC_LINK_DUPLICATES), 0, &TARGET_LITTLE_SYM,
			    COFF_SWAP_TABLE)
CREATE_LITTLE_COFF_TARGET_VEC (TARGET_LITTLE_SYM, TARGET_LITTLE_NAME, D_PAGED,
			       (SEC_CODE | SEC_DATA | SEC_DEBUGGING |
				SEC_READONLY | SEC_LINK_ONCE |
				SEC_LINK_DUPLICATES), 0, &TARGET_BIG_SYM,
			       COFF_SWAP_TABLE)
