...
static bfd_boolean elf32_vax_set_private_flags PARAMS ((bfd *, flagword));
static bfd_boolean elf32_vax_merge_private_bfd_data PARAMS ((bfd *, bfd *));
static bfd_boolean elf32_vax_print_private_bfd_data PARAMS ((bfd *, PTR));
		  ...
		  if (eh->got_addend != (bfd_vma) rel->r_addend)
		    (*_bfd_error_handler)
		      (_
		       ...
		       h->root.root.string, eh->got_addend);
     ...
     struct bfd_link_info *info;
{
  ...
  if (info->shared && info->symbolic)
    elf_vax_link_hash_traverse (elf_vax_hash_table (info),
				elf_vax_discard_copies, (PTR) NULL);
  ...
  elf_link_hash_traverse (elf_hash_table (info),
			  elf_vax_instantiate_got_entries, (PTR) info);
      ...
      else
	{
	  ...
	  if ((h->root.type == bfd_link_hash_defined
		  ...
		  || (info->shared
			  ...
			  || r_type == R_VAX_32
			  || r_type == R_VAX_PC8
			  || r_type == R_VAX_PC16 || r_type == R_VAX_PC32))))
		  ...
		  if (h != NULL)
		    ...
		    (*_bfd_error_handler)
		      (_
		       ...
		       bfd_get_filename (input_bfd), howto->name,
		       outrel.r_addend, bfd_get_section_name (input_bfd,
							      input_section));
		}
	      loc = sreloc->contents;
  ...
  if (h->plt.offset != (bfd_vma) - 1)
    {
      ...
      bfd_put_32 (output_bfd,
		  -(h->plt.offset + 8), splt->contents + h->plt.offset + 4);
      ...
      bfd_put_32 (output_bfd,
		  (splt->output_section->vma
		   + splt->output_offset
		   + h->plt.offset) + addend, sgot->contents + got_offset);
