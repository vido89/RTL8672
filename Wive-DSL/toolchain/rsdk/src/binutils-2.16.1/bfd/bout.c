...
PARAMS ((bfd *, asection *, arelent *, unsigned int, struct bfd_link_info *));
...
PARAMS ((bfd *, struct bfd_link_info *, struct bfd_link_order *, bfd_byte *,
	 bfd_boolean, asymbol **));
static int b_out_sizeof_headers PARAMS ((bfd *, bfd_boolean));
...
static bfd_boolean b_out_write_object_contents PARAMS ((bfd *));
static int b_out_symbol_cmp PARAMS ((const void *, const void *));
static bfd_boolean b_out_mkobject PARAMS ((bfd *));
     ...
     bfd *abfd;
{
  ...
  bfd_set_arch_mach (abfd, bfd_arch_i960,	/* B.out only used on i960 */
  ...
  if (IS_CALLNAME (symbol->other))
    {
      ...
      symbol = balsym;
      value = (symbol->symbol.value + output_addr (symbol->symbol.section));
    }
  ...
  if (IS_OTHER (symbol->other))
  ...
  else if (IS_CALLNAME (symbol->other))
    {
      ...
      word = BAL | ((word
		     ...
		     - dstidx - output_addr (input_section)) & BAL_MASK);
    }
  ...
  else
    {
      word = CALL | (((word & BAL_MASK)
		      ...
		      + reloc_entry->addend
		      - (shrinking ? dstidx : 0)
		      - output_addr (input_section)) & BAL_MASK);
    }
...
HOWTO (CALLJ, 0, 2, 24, TRUE, 0, complain_overflow_signed, 0, "callj", TRUE,
       0x00ffffff, 0x00ffffff, FALSE);
static reloc_howto_type howto_reloc_abs32 =
HOWTO (ABS32, 0, 2, 32, FALSE, 0, complain_overflow_bitfield, 0, "abs32",
       TRUE, 0xffffffff, 0xffffffff, FALSE);
static reloc_howto_type howto_reloc_pcrel24 =
HOWTO (PCREL24, 0, 2, 24, TRUE, 0, complain_overflow_signed, 0, "pcrel24",
       TRUE, 0x00ffffff, 0x00ffffff, FALSE);

static reloc_howto_type howto_reloc_pcrel13 =
HOWTO (PCREL13, 0, 2, 13, TRUE, 0, complain_overflow_signed, 0, "pcrel13",
       TRUE, 0x00001fff, 0x00001fff, FALSE);

static reloc_howto_type howto_reloc_abs32codeshrunk =
HOWTO (ABS32CODE_SHRUNK, 0, 2, 24, TRUE, 0, complain_overflow_signed, 0,
       "callx->callj", TRUE, 0x00ffffff, 0x00ffffff, FALSE);

static reloc_howto_type howto_reloc_abs32code =
HOWTO (ABS32CODE, 0, 2, 32, FALSE, 0, complain_overflow_bitfield, 0, "callx",
       TRUE, 0xffffffff, 0xffffffff, FALSE);

static reloc_howto_type howto_align_table[] = {
  HOWTO (ALIGNER, 0, 0x1, 0, FALSE, 0, complain_overflow_dont, 0, "align16",
	 FALSE, 0, 0, FALSE),
  HOWTO (ALIGNER, 0, 0x3, 0, FALSE, 0, complain_overflow_dont, 0, "align32",
	 FALSE, 0, 0, FALSE),
  HOWTO (ALIGNER, 0, 0x7, 0, FALSE, 0, complain_overflow_dont, 0, "align64",
	 FALSE, 0, 0, FALSE),
  HOWTO (ALIGNER, 0, 0xf, 0, FALSE, 0, complain_overflow_dont, 0, "align128",
	 FALSE, 0, 0, FALSE),
};

static reloc_howto_type howto_done_align_table[] = {
  HOWTO (ALIGNDONE, 0x1, 0x1, 0, FALSE, 0, complain_overflow_dont, 0,
	 "donealign16", FALSE, 0, 0, FALSE),
  HOWTO (ALIGNDONE, 0x3, 0x3, 0, FALSE, 0, complain_overflow_dont, 0,
	 "donealign32", FALSE, 0, 0, FALSE),
  HOWTO (ALIGNDONE, 0x7, 0x7, 0, FALSE, 0, complain_overflow_dont, 0,
	 "donealign64", FALSE, 0, 0, FALSE),
  HOWTO (ALIGNDONE, 0xf, 0xf, 0, FALSE, 0, complain_overflow_dont, 0,
	 "donealign128", FALSE, 0, 0, FALSE),
};
...
doit:
  ...
  for (rptr = relocs, cache_ptr = reloc_cache, counter = 0;
       counter < count; counter++, rptr++, cache_ptr++)
    {
      ...
      else
	{
	  ...
	  switch (s)
		...
		{
		  cache_ptr->howto =
		    &howto_align_table[(raw[7] >> length_shift) & 3];
  ...
  for (natptr = native; count > 0; --count, ++natptr, ++generic)
	...
	raw[7] = len_2 + incode_mask;
	       ...
	       && g->howto <= (howto_align_table
			       ...
			       sizeof (howto_align_table[0]) - 1))
	{
	  /* symnum == -2; extern_mask not set, pcrel_mask set.  */
	  r_idx = -2;
	  r_extern = 0;
	  raw[7] = (pcrel_mask | ((g->howto - howto_align_table) << 1));
	}
      else
	raw[7] = len_2;
      ...
      while (dst_address < link_order->size)
	{
	  ...
	  if (reloc)
		...
		case ALIGNDONE:
		  ...
		  BFD_ASSERT ((bfd_vma) reloc->addend <= input_section->size);
		...
		case PCREL24:
		  {
		    ...
		    word = ((word & ~BAL_MASK)
				...
				+ reloc->addend) & BAL_MASK));

		    bfd_put_32 (input_bfd, (bfd_vma) word,
				data + dst_address);
		    dst_address += 4;
		    src_address += 4;
