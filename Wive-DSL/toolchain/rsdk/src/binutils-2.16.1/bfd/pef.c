...
PARAMS ((unsigned long, enum bfd_architecture *, unsigned long *));
static bfd_boolean bfd_pef_mkobject PARAMS ((bfd *));
static int bfd_pef_parse_traceback_table
PARAMS ((bfd *, asection *, unsigned char *, size_t, size_t, asymbol *,
	 FILE *));
...
PARAMS ((bfd *, asection *, unsigned char *, size_t, long *, asymbol **));
static int bfd_pef_parse_function_stub
PARAMS ((bfd *, unsigned char *, size_t, unsigned long *));
static int bfd_pef_parse_function_stubs
PARAMS ((bfd *, asection *, unsigned char *, size_t, unsigned char *, size_t,
...
static void bfd_pef_get_symbol_info
     ...
     bfd_print_symbol_type how;
{
  ...
  switch (how)
    ...
    default:
      ...
      if (strncmp (symbol->name, "__traceback_", strlen ("__traceback_")) ==
	  0)
	{
...
int
bfd_pef_parse_loader_header (abfd, buf, len, header)
     bfd *abfd ATTRIBUTE_UNUSED;
     unsigned char *buf;
     size_t len;
...
int
bfd_pef_parse_imported_library (abfd, buf, len, header)
     bfd *abfd ATTRIBUTE_UNUSED;
     unsigned char *buf;
     size_t len;
...
int
bfd_pef_parse_imported_symbol (abfd, buf, len, symbol)
     bfd *abfd ATTRIBUTE_UNUSED;
     unsigned char *buf;
     size_t len;
...
int
bfd_pef_scan_section (abfd, section)
     bfd *abfd;
     bfd_pef_section *section;
{
...
static int
bfd_pef_parse_traceback_tables (abfd, sec, buf, len, nsym, csym)
     bfd *abfd;
     asection *sec;
     unsigned char *buf;
     size_t len;
...
static int
bfd_pef_parse_function_stub (abfd, buf, len, offset)
     bfd *abfd ATTRIBUTE_UNUSED;
     unsigned char *buf;
     size_t len;
     ...
     asymbol **csym;
{
  ...
  for (;;)
	...
	max =
	  loaderlen - (header.loader_strings_offset + imports[index].name);
...
static long
bfd_pef_parse_symbols (abfd, csym)
     bfd *abfd;
     asymbol **csym;
{
  unsigned long count = 0;
