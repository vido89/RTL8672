     ...
     int crash_info_loc;
{
  ...
  abfd->tdata.cisco_core_data =
    (struct cisco_core_struct *) bfd_zmalloc (amt);
  ...
  switch ((crashreason) bfd_get_32 (abfd, crashinfo.reason))
      ...
      switch (bfd_get_32 (abfd, crashinfo.cpu_vector))
	{
	  /* bus error           */
	case 2:
	  abfd->tdata.cisco_core_data->sig = SIGBUS;
	  break;
	  /* address error       */
	case 3:
	  abfd->tdata.cisco_core_data->sig = SIGBUS;
	  ...
	  /* illegal instruction */
	case 4:
	  abfd->tdata.cisco_core_data->sig = SIGILL;
	  break;
	  /* zero divide         */
	case 5:
	  abfd->tdata.cisco_core_data->sig = SIGFPE;
	  break;
	  /* chk instruction     */
	case 6:
	  abfd->tdata.cisco_core_data->sig = SIGFPE;
	...
	case 7:
	  ...
	  /* privilege violation */
	case 8:
	  abfd->tdata.cisco_core_data->sig = SIGSEGV;
	  break;
	  /* trace trap          */
	case 9:
	  abfd->tdata.cisco_core_data->sig = SIGTRAP;
	  break;
	  /* line 1010 emulator  */
	case 10:
	  abfd->tdata.cisco_core_data->sig = SIGILL;
	...
	case 13:
	  ...
	  /* interrupt           */
	case 31:
	  abfd->tdata.cisco_core_data->sig = SIGINT;
	  break;
	  /* breakpoint          */
	case 33:
	  abfd->tdata.cisco_core_data->sig = SIGTRAP;
     ...
     bfd *abfd;
{
  ...
  for (crash_info_locp = crash_info_locs;
       *crash_info_locp != -1 && target == NULL; crash_info_locp++)
    {
