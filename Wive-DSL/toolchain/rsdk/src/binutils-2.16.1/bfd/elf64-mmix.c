...
static bfd_boolean mmix_elf_new_section_hook PARAMS ((bfd *, asection *));
...
extern bfd_boolean mmix_elf_final_link
     ...
     bfd_reloc_code_real_type code;
{
  ...
  for (i = 0; i < sizeof (mmix_reloc_map) / sizeof (mmix_reloc_map[0]); i++)
    {
     ...
     bfd_vma value;
{
  ...
  switch (howto->type)
      ...
      else
	{
	  ...
	  bfd_byte *stubcontents
	    = ((bfd_byte *) datap
	       ...
	       + size + mmix_elf_section_data (isec)->pjs.stub_offset);
	  bfd_vma stubaddr;
	  ...
	  r = mmix_elf_perform_relocation (isec,
					   ...
					   + size
					   +
					   (mmix_elf_section_data (isec)->pjs.
					    stub_offset) - addr);
	  if (r != bfd_reloc_ok)
	    return r;
	...
	bfd_put_32 (abfd,
		    ...
		    | (inreg << 16) | 0xff00, (bfd_byte *) datap + 16);
				      ...
				      value)) == bfd_reloc_ok)
	{
	  ...
	  bfd_put_32 (abfd,
		      (in1 & howto->src_mask)
		      | highbit
		      | (value & howto->dst_mask), (bfd_byte *) datap);
	...
	asection *bpo_greg_section = bpodata->bpo_greg_section;
	struct bpo_greg_section_info *gregdata
	  = mmix_elf_section_data (bpo_greg_section)->bpo.greg;
	size_t bpo_index = gregdata->bpo_reloc_indexes[bpodata->bpo_index++];
 ...
 linker-allocated global register: linked: 0x%lx%08lx != relaxed: 0x%lx%08lx\n"), bfd_get_filename (isec->owner), (unsigned long) (value >> 32), (unsigned long) value, (unsigned long) (gregdata->reloc_request[bpo_index].value >> 32), (unsigned long) gregdata->reloc_request[bpo_index].value);
	...
	bfd_put_8 (abfd,
		   ...
		   + bpo_greg_section->output_section->vma / 8, datap);
	bfd_put_8 (abfd,
  ...
  bfd_put_32 (abfd,
	      ...
	      (bfd_byte *) datap + offs);
  bfd_put_32 (abfd,
	      (INCML_INSN_BYTE << 24) | ((value >> 16) & 0xffff) | (reg <<
								    16),
	      (bfd_byte *) datap + offs + 4);
  bfd_put_32 (abfd,
	      (INCMH_INSN_BYTE << 24) | ((value >> 32) & 0xffff) | (reg <<
								    16),
     ...
     char **error_message ATTRIBUTE_UNUSED;
{
  ...
  if (bfd_is_und_section (symbol->section)
      && (symbol->flags & BSF_WEAK) == 0 && output_bfd == (bfd *) NULL)
     ...
     asection **local_sections;
{
  ...
  size =
    input_section->rawsize ? input_section->rawsize : input_section->size;
  ...
  for (rel = relocs; rel < relend; rel++)
    {
      ...
      if (r_type == R_MMIX_GNU_VTINHERIT || r_type == R_MMIX_GNU_VTENTRY)
	continue;
      ...
      if (info->relocatable)
	{
	  ...
	  if (r_type == R_MMIX_PUSHJ_STUBBABLE)
		  ...
		  != 0)
		{
		  ...
		  if (mmix_elf_section_data (input_section)->pjs.
		      stub_size[pjsno] != MAX_PUSHJ_STUB_SIZE)
		    abort ();
		  ...
		  bfd_put_32 (output_bfd, JMP_INSN_BYTE << 24,
			      ...
			      + size
			      +
			      mmix_elf_section_data (input_section)->pjs.
			      stub_offset);
     ...
     asection *symsec;
{
  ...
  switch (howto->type)
    ...
    default:
      r =
	_bfd_final_link_relocate (howto, input_section->owner, input_section,
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_MMIX_BASE_PLUS_OFFSET:
	  ...
	  if (bpodata == NULL)
		...
		= bpodata->bpo_index = gregdata->n_max_bpo_relocs;
     ...
     const Elf_Internal_Rela *relocs;
{
  ...
  sym_hashes_end =
    sym_hashes + symtab_hdr->sh_size / sizeof (Elf64_External_Sym);
     ...
     asymbol *asym;
{
  ...
  switch (elfsym->internal_elf_sym.st_shndx)
      ...
      if (mmix_elf_reg_section.name == NULL)
	{
	  ...
	  mmix_elf_reg_section.symbol_ptr_ptr =
     ...
     struct bfd_link_info *info;
{
  ...
  if (reg_section != NULL)
	...
	_bfd_abort (__FILE__, __LINE__, _("Register section has contents\n"));
	   ...
	   *secpp != reg_section; secpp = &(*secpp)->next)
	;
     ...
     struct bfd_link_info *info;
{
  ...
  gregdata->bpo_reloc_indexes
    = bpo_reloc_indexes
    = bfd_alloc (bpo_greg_owner,
		 gregdata->n_max_bpo_relocs * sizeof (size_t));
     ...
     struct bfd_link_info *link_info;
{
  ...
  Please report this bug."), gregdata->n_remaining_bpo_relocs_this_relaxation_round, gregdata->n_bpo_relocs);
      return FALSE;
    }
     ...
     bfd_error_handler_type pf;
{
  ...
  (*pf) (" n_bpo_relocs: %u\n n_max_bpo_relocs: %u\n n_remain...round: %u\n\
 n_allocated_bpo_gregs: %u\n", gregdata->n_bpo_relocs, gregdata->n_max_bpo_relocs, gregdata->n_remaining_bpo_relocs_this_relaxation_round, gregdata->n_allocated_bpo_gregs);
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	  ...
	  if (ELF64_R_TYPE (irel->r_info) == R_MMIX_PUSHJ_STUBBABLE)
	    {
	      ...
	      if (bfd_check_overflow (complain_overflow_signed,
				      19, 0, bfd_arch_bits_per_address (abfd),
				      /* Output-stub location.  */
				      sec->output_section->rawsize
				      +
				      (mmix_elf_section_data
				       (sec->output_section)->pjs.
				       stubs_size_sum)
				      /* Location of this PUSHJ reloc.  */
				      - (sec->output_offset + irel->r_offset)
				      /* Don't count *this* stub twice.  */
				      -
				      (mmix_elf_section_data (sec)->pjs.
				       stub_size[pjsno] +
				       MAX_PUSHJ_STUB_SIZE)) == bfd_reloc_ok)
      ...
      if (ELF64_R_TYPE (irel->r_info) == (int) R_MMIX_PUSHJ_STUBBABLE)
	{
	  ...
	  bfd_vma stubaddr
	    = (sec->output_section->vma
	       + sec->output_offset
	       + size + mmix_elf_section_data (sec)->pjs.stubs_size_sum);
	...
	= symval + irel->r_addend;
      gregdata->reloc_request[gregdata->bpo_reloc_indexes[bpono++]].valid =
	TRUE;
      gregdata->n_remaining_bpo_relocs_this_relaxation_round--;
    }
  ...
  if (gregdata != NULL
	...
	= gregdata->n_bpo_relocs;
      ...
      qsort ((PTR) gregdata->reloc_request,
	     gregdata->n_max_bpo_relocs,
	     sizeof (struct bpo_reloc_request), bpo_reloc_request_sort_fn);
      ...
      for (i = 0; i < gregdata->n_max_bpo_relocs; i++)
	if (gregdata->
	    bpo_reloc_indexes[gregdata->reloc_request[i].bpo_reloc_no] != i)
	  {
	    gregdata->bpo_reloc_indexes[gregdata->reloc_request[i].
					bpo_reloc_no] = i;
	    *again = TRUE;
	  }
