...
static bfd_boolean ihex_mkobject PARAMS ((bfd *));
static INLINE int ihex_get_byte PARAMS ((bfd *, bfd_boolean *));
static void ihex_bad_byte PARAMS ((bfd *, unsigned int, int, bfd_boolean));
static bfd_boolean ihex_scan PARAMS ((bfd *));
static const bfd_target *ihex_object_p PARAMS ((bfd *));
static bfd_boolean ihex_read_section PARAMS ((bfd *, asection *, bfd_byte *));
...
static bfd_boolean ihex_write_object_contents PARAMS ((bfd *));
static bfd_boolean ihex_set_arch_mach
PARAMS ((bfd *, enum bfd_architecture, unsigned long));
static int ihex_sizeof_headers PARAMS ((bfd *, bfd_boolean));
      ...
      else
	{
	  ...
	  if (((-chksum) & 0xff) != (unsigned int) HEX2 (buf + 2 * i))
		...
		(_
		 ...
		 abfd, lineno, (-chksum) & 0xff,
		 (unsigned int) HEX2 (buf + 2 * i));
	      bfd_set_error (bfd_error_bad_value);
	      goto error_return;
	    }
	  ...
	  switch (type)
	      ...
	      if (len != 2)
		{
		  (*_bfd_error_handler)
		    (_
		     ("%B:%u: bad extended address record length in Intel Hex file"),
		     abfd, lineno);
		  bfd_set_error (bfd_error_bad_value);
		  goto error_return;
		}
	      ...
	      if (len != 2)
		{
		  (*_bfd_error_handler)
		    (_
		     ("%B:%u: bad extended linear address record length in Intel Hex file"),
		     abfd, lineno);
		  bfd_set_error (bfd_error_bad_value);
		  goto error_return;
		}
     ...
     bfd_size_type count;
{
  ...
  if (tdata->tail != NULL && n->where >= tdata->tail->where)
    {
      tdata->tail->next = n;
      n->next = NULL;
      tdata->tail = n;
  ...
  else
    {
      ...
      for (pp = &tdata->head;
	   *pp != NULL && (*pp)->where < n->where; pp = &(*pp)->next)
	;
      n->next = *pp;
      *pp = n;
      if (n->next == NULL)
