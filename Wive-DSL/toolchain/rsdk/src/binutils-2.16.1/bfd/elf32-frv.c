...
static bfd_boolean elf32_frv_object_p PARAMS ((bfd *));
static bfd_boolean frv_elf_set_private_flags PARAMS ((bfd *, flagword));
static bfd_boolean frv_elf_copy_private_bfd_data PARAMS ((bfd *, bfd *));
static bfd_boolean frv_elf_merge_private_bfd_data PARAMS ((bfd *, bfd *));
static bfd_boolean frv_elf_print_private_bfd_data PARAMS ((bfd *, PTR));
...
static reloc_howto_type elf32_frv_vtinherit_howto = HOWTO (R_FRV_GNU_VTINHERIT,	/* type */
							   0,	/* rightshift */
							   2,	/* size (0 = byte, 1 = short, 2 = long) */
							   0,	/* bitsize */
							   FALSE,	/* pc_relative */
...
static reloc_howto_type elf32_frv_vtentry_howto = HOWTO (R_FRV_GNU_VTENTRY,	/* type */
							 0,	/* rightshift */
							 2,	/* size (0 = byte, 1 = short, 2 = long) */
							 0,	/* bitsize */
							 FALSE,	/* pc_relative */
...
static reloc_howto_type elf32_frv_rel_32_howto = HOWTO (R_FRV_32,	/* type */
							0,	/* rightshift */
							2,	/* size (0 = byte, 1 = short, 2 = long) */
							32,	/* bitsize */
							FALSE,	/* pc_relative */
...
static reloc_howto_type elf32_frv_rel_funcdesc_howto = HOWTO (R_FRV_FUNCDESC,	/* type */
							      0,	/* rightshift */
							      2,	/* size (0 = byte, 1 = short, 2 = long) */
							      32,	/* bitsize */
							      FALSE,	/* pc_relative */
...
static reloc_howto_type elf32_frv_rel_funcdesc_value_howto = HOWTO (R_FRV_FUNCDESC_VALUE,	/* type */
								    0,	/* rightshift */
								    2,	/* size (0 = byte, 1 = short, 2 = long) */
								    64,	/* bitsize */
								    FALSE,	/* pc_relative */
...
frvfdpic_relocs_info_for_global (struct htab *ht,
				 bfd * abfd,
				 struct elf_link_hash_entry *h,
				 bfd_vma addend, enum insert_option insert)
{
...
_frvfdpic_osec_to_segment (bfd * output_bfd, asection * osec)
{
  ...
  for (m = elf_tdata (output_bfd)->segment_map,
       p = elf_tdata (output_bfd)->phdr; m != NULL; m = m->next, p++)
    {
      int i;
...
_frvfdpic_emit_got_relocs_plt_entries (struct frvfdpic_relocs_info *entry,
				       ...
				       struct bfd_link_info *info,
				       asection * sec,
				       Elf_Internal_Sym * sym, bfd_vma addend)
{
  ...
  if (entry->got_entry)
	...
	_frvfdpic_add_dyn_reloc (output_bfd, frvfdpic_gotrel_section (info),
      ...
      bfd_put_32 (output_bfd, ad,
		  ...
		  + frvfdpic_got_initial_offset (info) + entry->got_entry);
    }
	    ...
	    && FRVFDPIC_SYM_LOCAL (info, entry->d.h)))
	{
	  ...
	  if (entry->symndx == -1
	      ...
	      && !(info->executable && !info->pie))
	    {
	      ...
	      idx =
		elf_section_data (entry->d.h->root.u.def.section->
	      ...
	      if (ad)
		{
		  (*info->callbacks->reloc_dangerous)
		    (info, _("relocation requires zero addend"),
		     ...
		     frvfdpic_got_section (info), entry->fdgot_entry);
		  return FALSE;
		}
	    }
	  else
	  ...
	  if (info->executable && !info->pie
	    ...
	    _frvfdpic_add_dyn_reloc (output_bfd,
				     ...
				     output_section->vma +
				     frvfdpic_got_section (info)->
				     output_offset, reloc, idx, ad, entry);
	}

      bfd_put_32 (output_bfd, ad,
		  frvfdpic_got_section (info)->contents
		  + frvfdpic_got_initial_offset (info) + entry->fdgot_entry);
    }
  ...
  if (entry->fd_entry)
      ...
      else
	{
	  ofst =
	    _frvfdpic_add_dyn_reloc (output_bfd,
				     ...
				     output_section->vma +
				     frvfdpic_got_section (info)->
				     output_offset, R_FRV_FUNCDESC_VALUE, idx,
				     ad, entry);
	}
      ...
      else if (entry->lazyplt)
	{
	  if (ad)
		...
		(info, _("relocation requires zero addend"),
		 elf_hash_table (info)->dynobj,
		 frvfdpic_got_section (info), entry->fd_entry);
	      return FALSE;
	    }
      ...
      else
	{
      ...
      bfd_put_32 (output_bfd, highword,
		  frvfdpic_got_section (info)->contents
		  + frvfdpic_got_initial_offset (info) + entry->fd_entry + 4);
    }
  ...
  if (entry->tlsoff_entry)
    {
      ...
      if (entry->symndx != -1 || FRVFDPIC_SYM_LOCAL (info, entry->d.h))
	{
	  /* If the symbol is dynamic but binds locally, use
	     section+offset.  */
	  if (sec)
      ...
      if (info->executable
	  && idx == 0
	  && (bfd_is_abs_section (sec) || bfd_is_und_section (sec)))
	;
      ...
      else
	{
	  if (idx == 0
	      ...
	      if (!elf_hash_table (info)->tls_sec)
		{
	      ...
	      idx =
		elf_section_data (elf_hash_table (info)->tls_sec)->dynindx;
      ...
      bfd_put_32 (output_bfd, ad,
		  frvfdpic_got_section (info)->contents
		  + frvfdpic_got_initial_offset (info) + entry->tlsoff_entry);
    }
  ...
  for (rel = relocs; rel < relend; rel++)
    {
      ...
      if (r_type == R_FRV_GNU_VTINHERIT || r_type == R_FRV_GNU_VTENTRY)
	continue;
	  ...
	  if ((h->root.type == bfd_link_hash_defined
	       || h->root.type == bfd_link_hash_defweak))
	    {
	      if (		/* TLSMOFF forces local binding.  */
		   r_type != R_FRV_TLSMOFF && !FRVFDPIC_SYM_LOCAL (info, h))
		{
		  sec = NULL;
		  relocation = 0;
		}
	...
	case R_FRV_TLSMOFF:
	  if (h != NULL)
	    ...
	    picrel = frvfdpic_relocs_info_for_local (frvfdpic_relocs_info
						     (info), input_bfd,
						     r_symndx, orig_addend,
						     INSERT);
	  if (!picrel)
	    return FALSE;

	  if (!_frvfdpic_emit_got_relocs_plt_entries
	      (picrel, output_bfd, info, osec, sym, rel->r_addend))
	    {
	      (*_bfd_error_handler)
		(_
  ...
  (LOCAL_EXEC_P ((info), (picrel)) \
   && TLSMOFF_IN_RANGE_FOR_SETLOS_P((value), (info)))
	...
	case R_FRV_FUNCDESC:
	  {
		...
		if (h && !FRVFDPIC_FUNCDESC_LOCAL (info, h)
		...
		else
		  {
		    ...
		    addend =
		      frvfdpic_got_section (info)->output_offset +
		      frvfdpic_got_initial_offset (info) + picrel->fd_entry;
		  }
		...
		if (info->executable && !info->pie
		    && (!h || FRVFDPIC_FUNCDESC_LOCAL (info, h)))
		  {
		    ...
		    if ((bfd_get_section_flags
			 (output_bfd,
			  input_section->
			  output_section) & (SEC_ALLOC | SEC_LOAD)) ==
			(SEC_ALLOC | SEC_LOAD))
		      {
	...
	case R_FRV_TLSMOFF:
	  ...
	  if (!sec)
	    check_segment[1] = -1;
	  else if (bfd_is_abs_section (sec) || bfd_is_und_section (sec))
	    {
	      relocation = 0;
	...
	r = elf32_frv_relocate_gprelhi (info, input_bfd, input_section, rel,
      ...
      else if (r_type == R_FRV_TLSOFF || r_type == R_FRV_TLSDESC_VALUE)
	r = bfd_reloc_notsupported;

      else
	r =
	  frv_final_link_relocate (howto, input_bfd, input_section, contents,
				   rel, relocation);
...
static bfd_boolean
elf32_frvfdpic_create_dynamic_sections (bfd * abfd,
					struct bfd_link_info *info)
{
  /* This is mostly copied from
     elflink.c:_bfd_elf_create_dynamic_sections().  */
      ...
      && ((entry->symndx == -1 && FRVFDPIC_SYM_LOCAL (dinfo->info, entry->d.h)
	   ...
	   && (entry->d.h->root.type == bfd_link_hash_undefweak
	       || (bfd_vma) (entry->d.h->root.u.def.value
			     +
			     (entry->d.h->root.u.def.section->output_section->
			      vma) +
			     entry->d.h->root.u.def.section->output_offset +
			     entry->addend - tls_biased_base (dinfo->info) +
			     32768) < (bfd_vma) 65536))
	  || (entry->symndx != -1
...
_frvfdpic_compute_got_alloc_data (struct _frvfdpic_dynamic_got_alloc_data
				  *gad, bfd_signed_vma fdcur,
				  bfd_signed_vma odd, bfd_signed_vma cur,
				  bfd_vma got, bfd_vma fd, bfd_vma fdplt,
				  bfd_vma tlsd, bfd_vma tlsdplt, bfd_vma wrap)
{
  ...
  if (entry->tlsplt)
    {
      ...
      entry->tlsplt_entry = frvfdpic_plt_section (dinfo->g.info)->size;
			...
			struct _frvfdpic_dynamic_got_plt_info *gpinfop)
{
  ...
  /* This will be non-NULL during relaxation.  The assumption is that
    ...
    {
      frvfdpic_got_section (info)->contents =
	(bfd_byte *) bfd_zalloc (dynobj, frvfdpic_got_section (info)->size);
  ...
  if (frvfdpic_plt_section (info))
    frvfdpic_plt_initial_offset (info) = frvfdpic_plt_section (info)->size;
				      ...
				      struct bfd_link_info *info)
{
  ...
  frvfdpic_dynamic_got_plt_info (info) =
    bfd_alloc (dynobj, sizeof (gpinfo.g));
				     ...
				     struct bfd_link_info *info)
{
  if (!info->relocatable)
	...
	elf_tdata (output_bfd)->stack_flags = PF_R | PF_W | PF_X;
	  ...
	  || h->type != STT_OBJECT || !h->def_regular)
	{
	  struct bfd_link_hash_entry *bh = NULL;
      ...
      if (sec == NULL || !bfd_set_section_alignment (output_bfd, sec, 3))
	return FALSE;
    }
			      ...
			      struct bfd_link_info *info, bfd_boolean * again)
{
  ...
  *again = FALSE;
  ...
  htab_traverse (frvfdpic_relocs_info (info),
		 _frvfdpic_relax_got_plt_entries, &gpinfo.g);
  ...
  if (memcmp (frvfdpic_dynamic_got_plt_info (info),
	      &gpinfo.g, sizeof (gpinfo.g)) != 0)
    {
      ...
      htab_traverse (frvfdpic_relocs_info (info),
		     _frvfdpic_reset_got_plt_entries, NULL);
	  ...
	  if (elf_tdata (obfd)->phdr[i].p_type == PT_GNU_STACK)
	    {
	      ...
	      if (bfd_seek
		  (obfd,
		   (bfd_signed_vma) get_elf_backend_data (obfd)->s->
		   sizeof_ehdr, SEEK_SET) != 0
		  || get_elf_backend_data (obfd)->s->write_out_phdrs (obfd,
								      elf_tdata
								      (obfd)->
								      phdr,
								      elf_elfheader
     ...
     bfd *obfd;
{
  ...
  if (!elf_flags_init (obfd))	/* First call, no flags set.  */
      ...
      else
	{
	  switch (new_partial)
	    ...
	    default:
	      ...
	      break;
	    case EF_FRV_GPR_32:
	      strcat (new_opt, " -mgpr-32");
	      break;
	    case EF_FRV_GPR_64:
	      strcat (new_opt, " -mgpr-64");
	      break;
	    }
	  ...
	  switch (old_partial)
	    ...
	    default:
	      ...
	      break;
	    case EF_FRV_GPR_32:
	      strcat (old_opt, " -mgpr-32");
	      break;
	    case EF_FRV_GPR_64:
	      strcat (old_opt, " -mgpr-64");
	      break;
	    }
	}
      ...
      else
	{
	  switch (new_partial)
	    ...
	    default:
	      ...
	      break;
	    case EF_FRV_FPR_32:
	      strcat (new_opt, " -mfpr-32");
	      break;
	    case EF_FRV_FPR_64:
	      strcat (new_opt, " -mfpr-64");
	      break;
	    case EF_FRV_FPR_NONE:
	      strcat (new_opt, " -msoft-float");
	      break;
	    }
	  ...
	  switch (old_partial)
	    ...
	    default:
	      ...
	      break;
	    case EF_FRV_FPR_32:
	      strcat (old_opt, " -mfpr-32");
	      break;
	    case EF_FRV_FPR_64:
	      strcat (old_opt, " -mfpr-64");
	      break;
	    case EF_FRV_FPR_NONE:
	      strcat (old_opt, " -msoft-float");
	      break;
	    }
	}
      ...
      else
	{
	  switch (new_partial)
	    {
	    default:
	      strcat (new_opt, " -mdword-?");
	      break;
	    case EF_FRV_DWORD_YES:
	      strcat (new_opt, " -mdword");
	      break;
	    case EF_FRV_DWORD_NO:
	      strcat (new_opt, " -mno-dword");
	      break;
	    }
	  ...
	  switch (old_partial)
	    {
	    default:
	      strcat (old_opt, " -mdword-?");
	      break;
	    case EF_FRV_DWORD_YES:
	      strcat (old_opt, " -mdword");
	      break;
	    case EF_FRV_DWORD_NO:
	      strcat (old_opt, " -mno-dword");
	      break;
	    }
	}
      ...
      old_flags |= new_flags & (EF_FRV_DOUBLE
				| EF_FRV_MEDIA
				| EF_FRV_MULADD | EF_FRV_NON_PIC_RELOCS);
      ...
      else
	{
	  switch (new_partial)
	    ...
	    case EF_FRV_CPU_GENERIC:
	      strcat (new_opt, " -mcpu=frv");
	      break;
	    case EF_FRV_CPU_SIMPLE:
	      strcat (new_opt, " -mcpu=simple");
	      break;
	    case EF_FRV_CPU_FR550:
	      strcat (new_opt, " -mcpu=fr550");
	      break;
	    case EF_FRV_CPU_FR500:
	      strcat (new_opt, " -mcpu=fr500");
	      break;
	    case EF_FRV_CPU_FR450:
	      strcat (new_opt, " -mcpu=fr450");
	      break;
	    case EF_FRV_CPU_FR405:
	      strcat (new_opt, " -mcpu=fr405");
	      break;
	    case EF_FRV_CPU_FR400:
	      strcat (new_opt, " -mcpu=fr400");
	      break;
	    case EF_FRV_CPU_FR300:
	      strcat (new_opt, " -mcpu=fr300");
	      break;
	    case EF_FRV_CPU_TOMCAT:
	      strcat (new_opt, " -mcpu=tomcat");
	      break;
	    }
	  ...
	  switch (old_partial)
	    ...
	    default:
	      ...
	      break;
	    case EF_FRV_CPU_GENERIC:
	      strcat (old_opt, " -mcpu=frv");
	      break;
	    case EF_FRV_CPU_SIMPLE:
	      strcat (old_opt, " -mcpu=simple");
	      break;
	    case EF_FRV_CPU_FR550:
	      strcat (old_opt, " -mcpu=fr550");
	      break;
	    case EF_FRV_CPU_FR500:
	      strcat (old_opt, " -mcpu=fr500");
	      break;
	    case EF_FRV_CPU_FR450:
	      strcat (old_opt, " -mcpu=fr450");
	      break;
	    case EF_FRV_CPU_FR405:
	      strcat (old_opt, " -mcpu=fr405");
	      break;
	    case EF_FRV_CPU_FR400:
	      strcat (old_opt, " -mcpu=fr400");
	      break;
	    case EF_FRV_CPU_FR300:
	      strcat (old_opt, " -mcpu=fr300");
	      break;
	    case EF_FRV_CPU_TOMCAT:
	      strcat (old_opt, " -mcpu=tomcat");
	      break;
	    }
	}
  ...
  if (((new_flags & EF_FRV_FDPIC) == 0) != (!IS_FDPIC (ibfd)))
    {
      error = TRUE;
      if (IS_FDPIC (obfd))
	(*_bfd_error_handler)
     ...
     PTR ptr;
{
  ...
  switch (flags & EF_FRV_CPU_MASK)
    ...
    default:
      break;
    case EF_FRV_CPU_SIMPLE:
      fprintf (file, " -mcpu=simple");
      break;
    case EF_FRV_CPU_FR550:
      fprintf (file, " -mcpu=fr550");
      break;
    case EF_FRV_CPU_FR500:
      fprintf (file, " -mcpu=fr500");
      break;
    case EF_FRV_CPU_FR450:
      fprintf (file, " -mcpu=fr450");
      break;
    case EF_FRV_CPU_FR405:
      fprintf (file, " -mcpu=fr405");
      break;
    case EF_FRV_CPU_FR400:
      fprintf (file, " -mcpu=fr400");
      break;
    case EF_FRV_CPU_FR300:
      fprintf (file, " -mcpu=fr300");
      break;
    case EF_FRV_CPU_TOMCAT:
      fprintf (file, " -mcpu=tomcat");
      break;
    }
  ...
  switch (flags & EF_FRV_GPR_MASK)
    ...
    default:
      break;
    case EF_FRV_GPR_32:
      fprintf (file, " -mgpr-32");
      break;
    case EF_FRV_GPR_64:
      fprintf (file, " -mgpr-64");
      break;
    }
  ...
  switch (flags & EF_FRV_FPR_MASK)
    ...
    default:
      break;
    case EF_FRV_FPR_32:
      fprintf (file, " -mfpr-32");
      break;
    case EF_FRV_FPR_64:
      fprintf (file, " -mfpr-64");
      break;
    case EF_FRV_FPR_NONE:
      fprintf (file, " -msoft-float");
      break;
    }
