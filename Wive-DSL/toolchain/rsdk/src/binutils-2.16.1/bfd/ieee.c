...
static bfd_boolean ieee_write_2bytes PARAMS ((bfd *, int));
...
static bfd_boolean ieee_write_int5_out PARAMS ((bfd *, bfd_vma));
...
static bfd_boolean ieee_slurp_symbol_table PARAMS ((bfd *));
...
static bfd_boolean ieee_slurp_debug PARAMS ((bfd *));
...
PARAMS ((ieee_data_type *, ieee_per_section_type *, unsigned char *,
	 asection *, int));
static bfd_boolean ieee_slurp_section_data PARAMS ((bfd *));
...
static bfd_boolean ieee_write_section_part PARAMS ((bfd *));
static bfd_boolean do_with_relocs PARAMS ((bfd *, asection *));
static bfd_boolean do_as_repeat PARAMS ((bfd *, asection *));
static bfd_boolean do_without_relocs PARAMS ((bfd *, asection *));
static bfd_boolean ieee_mkobject PARAMS ((bfd *));
...
static bfd_boolean ieee_write_debug_part PARAMS ((bfd *));
static bfd_boolean ieee_write_data_part PARAMS ((bfd *));
static bfd_boolean init_for_output PARAMS ((bfd *));
static bfd_boolean ieee_set_section_contents
PARAMS ((bfd *, sec_ptr, const PTR, file_ptr, bfd_size_type));
static bfd_boolean ieee_write_external_part PARAMS ((bfd *));
static bfd_boolean ieee_write_me_part PARAMS ((bfd *));
static bfd_boolean ieee_write_processor PARAMS ((bfd *));
static bfd_boolean ieee_write_object_contents PARAMS ((bfd *));
...
static int ieee_sizeof_headers PARAMS ((bfd *, bfd_boolean));
...
static reloc_howto_type abs32_howto = HOWTO (1,
					     0,
					     2,
					     32,
					     FALSE,
...
static reloc_howto_type abs16_howto = HOWTO (1,
					     0,
					     1,
					     16,
					     FALSE,
...
static reloc_howto_type abs8_howto = HOWTO (1,
					    0,
					    0,
					    8,
					    FALSE,
...
static reloc_howto_type rel32_howto = HOWTO (1,
					     0,
					     2,
					     32,
					     TRUE,
...
static reloc_howto_type rel16_howto = HOWTO (1,
					     0,
					     1,
					     16,
					     TRUE,
...
static reloc_howto_type rel8_howto = HOWTO (1,
					    0,
					    0,
					    8,
					    TRUE,
...
#define POP(x,y,z) DEC;x=TOS.symbol;y=TOS.section;z=TOS.value;
  ...
  while (loop && ieee->h.input_p < ieee->h.last_byte)
	...
	case ieee_variable_L_enum:
	  ...
	  next_byte (&(ieee->h));
	  PUSH (NOSYMBOL, ieee->section_table[must_parse_int (&(ieee->h))],
		0);
	  break;
	...
	case ieee_variable_S_enum:
	  ...
	  next_byte (&(ieee->h));
	  PUSH (NOSYMBOL,
		0, ieee->section_table[must_parse_int (&(ieee->h))]->size);
	  break;
     ...
     bfd *abfd;
{
  ...
  if (ieee->external_symbol_max_index != 0)
	...
	ieee->external_symbol_max_index - ieee->external_symbol_min_index + 1;
    }
  else
    {
  ...
  if (abfd->symcount)
      ...
      if (!ieee->symbol_table_full)
	{
      ...
      for (symp = IEEE_DATA (abfd)->external_symbols;
	   symp != (ieee_symbol_type *) NULL; symp = symp->next)
	/* Place into table at correct index locations.  */
	location[symp->index + ieee->external_symbol_base_offset] =
	  &symp->symbol;
	  ...
	  switch (this_byte (&(ieee->h)))
		...
		switch (t)
		  ...
		  case ieee_section_size_enum:
		    section =
		      ieee->section_table[must_parse_int (&(ieee->h))];
...
/*			    abort();*/
		  ...
		  switch (extra)
			...
			{
#if KEEPMINUSPCININST
			  bfd_put_16 (ieee->h.abfd,
				      (bfd_vma) - current_map->pc,
				      location_ptr + current_map->pc);
			  r->relent.addend -= current_map->pc;
			...
			{
#if KEEPMINUSPCININST
			  bfd_put_8 (ieee->h.abfd, (int) (-current_map->pc),
				     location_ptr + current_map->pc);
			  r->relent.addend -= current_map->pc;
     ...
     bfd *abfd;
{
  ...
  for (s = abfd->sections; s != (asection *) NULL; s = s->next)
	...
	continue;
      ...
      if (!per->data)
	return FALSE;
      per->reloc_tail_ptr = (ieee_reloc_type **) & (s->relocation);
    }
  ...
  while (TRUE)
	...
	case ieee_e2_first_byte_enum:
	  ...
	  switch (this_byte (&(ieee->h)))
		...
		parse_expression (ieee, &value, &symbol, &pcrel, &extra, 0);
     ...
     bfd *abfd;
{
  ...
  for (s = abfd->sections; s != (asection *) NULL; s = s->next)
    {
      if (!bfd_is_abs_section (s) && (s->flags & SEC_DEBUGGING) == 0)
	{
      ...
      while (current_byte_index < s->size)
	{
	  ...
	  if (relocs_to_go && (*p) && (*p)->address == current_byte_index)
		     ...
		     && (*p) && (*p)->address == current_byte_index)
		{
		  ...
		  if (r->howto->pc_relative && !r->howto->pcrel_offset)
		    ov += r->address;
     ...
     asection *s;
{
  if (s->size)
      ...
      else
	{
	  ...
	  || !ieee_write_byte (abfd, 1) || !ieee_write_byte (abfd, 0))
	return FALSE;
    }
     ...
     bfd_size_type count;
{
  ...
  memcpy ((PTR) (ieee_per_section (section)->data + offset),
	  (PTR) location, (unsigned int) count);
  return TRUE;
}
      ...
      for (q = abfd->outsymbols; *q != (asymbol *) NULL; q++)
	{
	  ...
	  if (bfd_is_und_section (p->section))
	  ...
	  else if (p->flags & BSF_GLOBAL)
	    {
	      ...
	      if (!ieee_write_byte (abfd, ieee_external_symbol_enum) || !ieee_write_int (abfd, (bfd_vma) public_index) || !ieee_write_id (abfd, p->name) || !ieee_write_2bytes (abfd, ieee_attribute_record_enum) || !ieee_write_int (abfd, (bfd_vma) public_index) || !ieee_write_byte (abfd, 15)	/* instruction address */
		  ...
		  else
		    {
		      if (!(ieee_write_expression
			    (abfd,
			     ...
			     p->section->output_section->symbol, FALSE, 0)))
			return FALSE;
		    }
		}
	      else
...
const bfd_target ieee_vec = {
  ...
  (SEC_CODE | SEC_DATA | SEC_ROM | SEC_HAS_CONTENTS | SEC_ALLOC | SEC_LOAD | SEC_RELOC),	/* section flags */
