     ...
     struct bfd_link_hash_entry **hashp;
{
  ...
  if ((abfd->flags & DYNAMIC) != 0 && bfd_is_com_section (section))
  ...
  if (!_bfd_generic_link_add_one_symbol (info, abfd, name, flags, section,
					 value, string, copy, collect, hashp))
    return FALSE;
  ...
  if (abfd->xvec == info->hash->creator)
	  ...
	  && (h->flags & (SUNOS_DEF_REGULAR | SUNOS_REF_REGULAR)) != 0)
	{
      ...
      if ((flags & BSF_CONSTRUCTOR) != 0 && (abfd->flags & DYNAMIC) == 0)
     ...
     asection **srulesptr;
{
  ...
  *srulesptr = NULL;
  ...
  for (sub = info->input_bfds; sub != NULL; sub = sub->link_next)
    {
      if ((sub->flags & DYNAMIC) == 0 && sub->xvec == output_bfd->xvec)
	{
  ...
  if (sunos_hash_table (info)->dynamic_sections_needed)
	...
	PUT_WORD (output_bfd, (bfd_vma) - 1,
		  s->contents + i * HASH_ENTRY_SIZE);
      ...
      sunos_link_hash_traverse (sunos_hash_table (info),
				sunos_scan_dynamic_symbol, (PTR) info);
      ...
      if ((s->size & 7) != 0)
	{
	  ...
	  add = 8 - (s->size & 7);
	  contents = (bfd_byte *) bfd_realloc (s->contents, s->size + add);
	  if (contents == NULL)
	    return FALSE;
     ...
     bfd_size_type rel_size;
{
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      if (r_extern)
	{
	  ...
	  || r_type == RELOC_BASE13 || r_type == RELOC_BASE22)
	{
	  if (dynobj == NULL)
	    {
	      if (!sunos_create_dynamic_sections (abfd, info, FALSE))
      ...
      if (dynobj == NULL)
	{
      ...
      BFD_ASSERT (r_type == RELOC_JMP_TBL
		  || info->shared || (h->flags & SUNOS_REF_REGULAR) != 0);
      BFD_ASSERT (r_type == RELOC_JMP_TBL
		  || info->shared
		  || h->plt_offset != 0
		  || ((h->root.root.type == bfd_link_hash_defined
  ...
  if ((h->flags & (SUNOS_DEF_REGULAR | SUNOS_REF_REGULAR)) != 0)
      ...
      else
	{
	  ...
	  next = GET_WORD (dynobj,
			   (s->contents
			    + hash * HASH_ENTRY_SIZE + BYTES_IN_WORD));
     ...
     struct aout_link_hash_entry *harg;
{
  ...
  if (h->plt_offset != 0)
    {
      ...
      r_address = (splt->output_section->vma
		   + splt->output_offset + h->plt_offset);
	...
	case bfd_arch_sparc:
	  if (info->shared || (h->flags & SUNOS_DEF_REGULAR) == 0)
	  ...
	  else
	    {
	      ...
	      bfd_put_32 (output_bfd,
			  SPARC_PLT_PIC_WORD0 + ((val >> 10) & 0x3fffff), p);
	      bfd_put_32 (output_bfd,
			  SPARC_PLT_PIC_WORD1 + (val & 0x3ff), p + 4);
      ...
      if (info->shared || (h->flags & SUNOS_DEF_REGULAR) == 0)
	{
	  ...
	  BFD_ASSERT (s->reloc_count * obj_reloc_entry_size (dynobj)
		      < s->size);
	  p =
	    s->contents + s->reloc_count * obj_reloc_entry_size (output_bfd);
  ...
  switch (h->root.root.type)
	...
	if (h->plt_offset != 0 && (h->flags & SUNOS_DEF_REGULAR) == 0)
	  {
	    type = N_UNDF | N_EXT;
	    val = 0;
	  }
     ...
     bfd_vma *relocationp;
{
  ...
  *skip = FALSE;
  ...
  if (h != NULL
      && h->plt_offset != 0
      && (info->shared || (h->flags & SUNOS_DEF_REGULAR) == 0))
    {
      asection *splt;
  ...
  if (obj_reloc_entry_size (input_bfd) == RELOC_STD_SIZE)
	...
	r_type = ((erel->r_type[0] & RELOC_EXT_BITS_TYPE_LITTLE)
      ...
      pcrel = (r_type == RELOC_DISP8
	       || r_type == RELOC_DISP16
	       || r_type == RELOC_DISP32
	       || r_type == RELOC_WDISP30 || r_type == RELOC_WDISP22);
      ...
      else
	{
	  ...
	  if (obj_reloc_entry_size (input_bfd) == RELOC_STD_SIZE)
		...
		r_index = ((srel->r_index[0] << 16)
			   | (srel->r_index[1] << 8) | srel->r_index[2]);
	      else
		r_index = ((srel->r_index[2] << 16)
			   | (srel->r_index[1] << 8) | srel->r_index[0]);
	    }
	  else
	    {
	      struct reloc_ext_external *erel;
		...
		r_index = ((erel->r_index[0] << 16)
			   | (erel->r_index[1] << 8) | erel->r_index[2]);
	      else
		r_index = ((erel->r_index[2] << 16)
			   | (erel->r_index[1] << 8) | erel->r_index[0]);
	    }
	  ...
	  if (info->shared
	      ...
	      if (obj_reloc_entry_size (dynobj) == RELOC_STD_SIZE)
		{
		  ...
		  PUT_WORD (dynobj,
			    (*got_offsetp
			     ...
			     + sgot->output_offset), srel->r_address);
	      ...
	      else
		{
		  ...
		  PUT_WORD (dynobj,
			    (*got_offsetp
			     + sgot->output_section->vma
			     + sgot->output_offset), erel->r_address);
		  if (bfd_header_big_endian (dynobj))
		    {
  ...
  if (obj_reloc_entry_size (dynobj) == RELOC_STD_SIZE)
    {
      ...
      PUT_WORD (dynobj,
		(GET_WORD (dynobj, srel->r_address)
		 ...
		 + input_section->output_offset), srel->r_address);
      if (bfd_header_big_endian (dynobj))
	{
	  srel->r_index[0] = (bfd_byte) (indx >> 16);
	  srel->r_index[1] = (bfd_byte) (indx >> 8);
  ...
  else
    {
      ...
      PUT_WORD (dynobj,
		(GET_WORD (dynobj, erel->r_address)
		 + input_section->output_section->vma
		 + input_section->output_offset), erel->r_address);
      if (bfd_header_big_endian (dynobj))
	{
	  erel->r_index[0] = (bfd_byte) (indx >> 16);
	  erel->r_index[1] = (bfd_byte) (indx >> 8);
      ...
      if (pcrel && h != NULL)
	{
	  ...
	  PUT_WORD (dynobj,
			...
			- input_section->vma)), erel->r_addend);
	}
    }
     ...
     struct bfd_link_info *info;
{
  ...
  for (o = dynobj->sections; o != NULL; o = o->next)
    {
      if ((o->flags & SEC_HAS_CONTENTS) != 0 && o->contents != NULL)
	{
  ...
  if (sdyn->size > 0)
    {
      ...
      PUT_WORD (dynobj,
		(sdyn->output_section->vma
		 + sdyn->output_offset
		 + sizeof esd + EXTERNAL_SUN4_DYNAMIC_DEBUGGER_SIZE), esd.ld);
	...
	PUT_WORD (dynobj, s->output_section->filepos + s->output_offset,
      ...
      PUT_WORD (dynobj,
		BFD_ALIGN (obj_textsec (abfd)->size, 0x2000), esdl.ld_text);
