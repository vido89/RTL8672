...
static bfd_boolean tic30_aout_write_object_contents PARAMS ((bfd *));
static bfd_boolean tic30_aout_set_sizes PARAMS ((bfd *));
...
reloc_howto_type tic30_aout_howto_table[] = {
  ...
  HOWTO (5, 2, 1, 16, TRUE, 0, complain_overflow_signed,
	 tic30_aout_fix_pcrel_16, "PCREL", TRUE, 0x0000FFFF, 0x0000FFFF,
	 TRUE),
  EMPTY_HOWTO (-1),
  EMPTY_HOWTO (-1),
  EMPTY_HOWTO (-1),
...
NAME (aout, reloc_type_lookup)
PARAMS ((bfd *, bfd_reloc_code_real_type));

     reloc_howto_type *tic30_aout_reloc_type_lookup (abfd, code)
...
tic30_aout_fix_16 (abfd, reloc_entry, symbol, data, input_section, output_bfd,
		   error_message)
     bfd *abfd;
     arelent *reloc_entry;
     asymbol *symbol;
     ...
     char **error_message ATTRIBUTE_UNUSED;
{
  ...
  bfd_byte offset_data =
    bfd_get_8 (abfd, (bfd_byte *) data + reloc_entry->address - 1);
  ...
  if (howto->complain_on_overflow != complain_overflow_dont)
      ...
      else
	{
	  ...
	  if ((bfd_signed_vma) relocation >= 0)
	    ...
	    signed_check =
	      (check |
	       ((bfd_vma) - 1 & ~((bfd_vma) - 1 >> howto->rightshift)));
	}
     ...
     struct bfd_link_info *info;
{
  ...
  if (abfd->flags & D_PAGED)
	...
	obj_datasec (abfd)->vma =
	  info->create_object_symbols_section->next->vma;
      else
