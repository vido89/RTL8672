     ...
     struct bfd_link_hash_entry **hashp;
{
  ...
  if (!info->relocatable
      ...
      && (flags & BSF_CONSTRUCTOR) != 0 && abfd->xvec == info->hash->creator)
    {
  ...
  if (bfd_is_abs_section (section) && abfd->xvec == info->hash->creator)
    {
  ...
  if (is_plt || IS_GOT_SYM (h->root.root.root.string))
	      ...
	      || h2->root.root.type == bfd_link_hash_indirect))
	{
	  ...
	  for (f1 = linux_hash_table (info)->fixup_list;
	       f1 != NULL; f1 = f1->next)
	    {
	      if ((f1->h != h && f1->h != h1) || (!f1->builtin && !f1->jump))
		continue;
	      if (f1->h == h1)
		exists = TRUE;
	      if (!exists && bfd_is_abs_section (h->root.root.u.def.section))
		{
     ...
     struct bfd_link_info *info;
{
  ...
  printf ("Fixup table file offset: %x  VMA: %x\n",
	  os->filepos + s->output_offset, os->vma + s->output_offset);
#endif
  ...
  h = linux_link_hash_lookup (linux_hash_table (info),
			      "__BUILTIN_FIXUPS__", FALSE, FALSE, FALSE);
