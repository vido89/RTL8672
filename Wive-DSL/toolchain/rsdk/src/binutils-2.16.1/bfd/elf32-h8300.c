			...
			Elf_Internal_Rela * elf_reloc)
{
  ...
  for (i = 0; i < sizeof (h8_elf_howto_table) / sizeof (reloc_howto_type);
       i++)
	...
	case R_H8_DIR24R8:
	  {
		...
		if (code == 0x5a
		    ...
		    && ELF32_R_SYM (last_reloc->r_info) < symtab_hdr->sh_info)
		  {
			...
			code = bfd_get_8 (abfd,
					  contents + last_reloc->r_offset -
					  1);
			code ^= 1;
			bfd_put_8 (abfd,
				   code, contents + last_reloc->r_offset - 1);
...
static bfd_boolean
elf32_h8_relax_delete_bytes (bfd * abfd, asection * sec, bfd_vma addr,
			     int count)
{
  Elf_Internal_Shdr *symtab_hdr;
