     ...
     int objtype;
{
  ...
  switch (subtype)
	...
	_bfd_vms_save_sized_string (vms_rec, PRIV (rec_length - 2));
      break;
     ...
     int objtype;
{
  ...
  _bfd_vms_output_dump (abfd,
			(unsigned char *)
			"GNU BFD ported by Klaus Kämpf 1994-1996", 39);
  _bfd_vms_output_flush (abfd);
     ...
     int objtype;
{
  ...
  if ((objtype == OBJ_S_C_EOM) || (objtype == OBJ_S_C_EOMW))
    {
    }
  else
    {
     ...
     int objtype;
{
  ...
  _bfd_vms_output_long (abfd,
			(unsigned long) (PRIV (vms_linkage_index) >> 1));
