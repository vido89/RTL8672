...
static bfd_boolean ecoff_slurp_symbolic_header PARAMS ((bfd *));
...
static bfd_boolean ecoff_compute_section_file_positions PARAMS ((bfd *));
static bfd_size_type ecoff_compute_reloc_file_positions PARAMS ((bfd *));
static bfd_boolean ecoff_get_extr PARAMS ((asymbol *, EXTR *));
     ...
     flagword *flags_ptr;
{
  ...
  if ((styp_flags & STYP_TEXT)
      ...
      || (styp_flags & STYP_DYNSYM) || (styp_flags & STYP_HASH))
    {
	...
	sec_flags |= SEC_CODE | SEC_LOAD | SEC_ALLOC;
	   ...
	   || (styp_flags & STYP_GOT) || styp_flags == STYP_RCONST)
    {
      if (sec_flags & SEC_NEVER_LOAD)
	...
	sec_flags |= SEC_DATA | SEC_LOAD | SEC_ALLOC;
	  ...
	  || styp_flags == STYP_PDATA || styp_flags == STYP_RCONST)
    ...
    sec_flags |= SEC_NEVER_LOAD;
	   ...
	   || (styp_flags & STYP_LIT8) || (styp_flags & STYP_LIT4))
...
#define UPDATE_RAW_END(start, count, size) \
  ...
  UPDATE_RAW_END (cbSymOffset, isymMax,
  ...
  UPDATE_RAW_END (cbExtOffset, iextMax,
     ...
     int weak;
{
  ...
  if (weak)
  ...
  else
    {
	  ...
	  || ecoff_sym->st == stLabel || ECOFF_IS_STAB (ecoff_sym))
	asym->flags |= BSF_DEBUGGING;
    }
  ...
  for (; fdr_ptr < fdr_end; fdr_ptr++)
    {
      ...
      for (;
	   lraw_src < lraw_end; lraw_src += external_sym_size, internal_ptr++)
	{
	  SYMR internal_sym;

	  (*swap_sym_in) (abfd, (PTR) lraw_src, &internal_sym);
	  internal_ptr->symbol.name = (ecoff_data (abfd)->debug_info.ss
				       + fdr_ptr->issBase + internal_sym.iss);
     ...
     const char *which;
{
  ...
  if (ifd == 0xffffffff || (rndx->rfd == 0xfff && indx == 0))
    name = "<undefined>";
  ...
  sprintf (string,
	   "%s %s { ifd = %u, index = %lu }",
	   which, name, ifd,
	   ((long) indx + debug_info->symbolic_header.iextMax));
}
		...
		if (ECOFF_IS_STAB (&ecoff_ext.asym))
		  ...
		  fprintf (file, _("\n      Local symbol: %ld"),
			   ((long) indx
			    + (long) sym_base
			    +
			    (ecoff_data (abfd)->debug_info.symbolic_header.
			     iextMax)));
		break;
     ...
     asymbol **symbols;
{
  ...
  if (section->relocation != (arelent *) NULL
      || section->reloc_count == 0 || (section->flags & SEC_CONSTRUCTOR) != 0)
    return TRUE;
  ...
  if (internal_relocs == (arelent *) NULL || external_relocs == (char *) NULL)
    return FALSE;
      ...
      else
	{
	  ...
	  switch (intern.r_symndx)
	    ...
	    case RELOC_SECTION_TEXT:
	      sec_name = ".text";
	      break;
	    case RELOC_SECTION_RDATA:
	      sec_name = ".rdata";
	      break;
	    case RELOC_SECTION_DATA:
	      sec_name = ".data";
	      break;
	    ...
	    case RELOC_SECTION_BSS:
	      sec_name = ".bss";
	      break;
	    case RELOC_SECTION_INIT:
	      sec_name = ".init";
	      break;
	    case RELOC_SECTION_LIT8:
	      sec_name = ".lit8";
	      break;
	    case RELOC_SECTION_LIT4:
	      sec_name = ".lit4";
	      break;
	    case RELOC_SECTION_XDATA:
	      sec_name = ".xdata";
	      break;
	    case RELOC_SECTION_PDATA:
	      sec_name = ".pdata";
	      break;
	    case RELOC_SECTION_FINI:
	      sec_name = ".fini";
	      break;
	    case RELOC_SECTION_LITA:
	      sec_name = ".lita";
	      break;
	    case RELOC_SECTION_RCONST:
	      sec_name = ".rconst";
	      break;
	    default:
	      abort ();
  ...
  if (section->flags & SEC_CONSTRUCTOR)
    {
      ...
      for (count = 0, chain = section->constructor_chain;
	   count < section->reloc_count; count++, chain = chain->next)
	*relptr++ = &chain->relent;
    }
  else
    {
     ...
     bfd_boolean reloc ATTRIBUTE_UNUSED;
{
  ...
  for (current = abfd->sections;
       current != (asection *) NULL; current = current->next)
    ++c;

  ret = (bfd_coff_filhsz (abfd)
	 + bfd_coff_aoutsz (abfd) + c * bfd_coff_scnhsz (abfd));
     ...
     bfd *abfd;
{
  ...
  for (current = abfd->sections, i = 0;
       current != NULL; current = current->next, i++)
  ...
  for (i = 0; i < abfd->section_count; i++)
	...
	file_sofar = BFD_ALIGN (file_sofar, 1 << alignment_power);
      ...
      if ((abfd->flags & D_PAGED) != 0 && (current->flags & SEC_ALLOC) != 0)
	{
     ...
     bfd *abfd;
{
  ...
  /* The .bss section of a demand paged executable must receive an
     ...
     hand.  */
  if (bfd_get_symcount (abfd) == 0
      && (abfd->flags & EXEC_P) != 0 && (abfd->flags & D_PAGED) != 0)
    {
      char c;
     ...
     bfd *abfd;
{
...
#ifdef CHECK_ARMAP_HASH
  {
	...
	for (srch = (hash + rehash) & (count - 1);
	     srch != hash && srch != i; srch = (srch + rehash) & (count - 1))
  ...
  for (i = 0; i < orl_count; i++)
      ...
      if (H_GET_32 (abfd, (hashtable + (hash * 8) + 4)) != 0)
	{
	  ...
	  for (srch = (hash + rehash) & (hashsize - 1);
	       srch != hash; srch = (srch + rehash) & (hashsize - 1))
     ...
     bfd *abfd;
{
  ...
  if (ret == NULL)
    return NULL;
  if (!_bfd_link_hash_table_init (&ret->root, abfd, ecoff_link_hash_newfunc))
    {
      free (ret);
     ...
     struct bfd_link_info *info;
{
  ...
  while (*pundef != (struct bfd_link_hash_entry *) NULL)
      ...
      if (file_offset == 0)
	{
      ...
      if (name[0] != h->root.string[0] || strcmp (name, h->root.string) != 0)
	{
	  unsigned int srch;
	  bfd_boolean found;
	  ...
	  for (srch = (hash + rehash) & (armap_count - 1);
	       srch != hash; srch = (srch + rehash) & (armap_count - 1))
	    {
  ...
  for (; ext_ptr < ext_end; ext_ptr += external_ext_size)
    {
	  ...
	  && esym.asym.st != stLabel && esym.asym.st != stProc)
	continue;
     ...
     struct bfd_link_info *info;
{
  ...
  for (input_bfd = info->input_bfds;
       input_bfd != (bfd *) NULL; input_bfd = input_bfd->link_next)
    {
      bfd_boolean ret;
  ...
  ecoff_link_hash_traverse (ecoff_hash_table (info),
			    ecoff_link_write_external, (PTR) & einfo);
     ...
     PTR data;
{
  ...
  switch (h->root.type)
    ...
    case bfd_link_hash_undefweak:
      if (h->esym.asym.sc != scUndefined && h->esym.asym.sc != scSUndefined)
     ...
     struct bfd_link_order *link_order;
{
  ...
  rel.howto =
    bfd_reloc_type_lookup (output_bfd, link_order->u.reloc.p->reloc);
  if (rel.howto == 0)
    {
      bfd_set_error (bfd_error_bad_value);
      return FALSE;
  ...
  if (type == bfd_symbol_reloc_link_order)
    {
      ...
      h = ((struct ecoff_link_hash_entry *)
	   bfd_wrapped_link_hash_lookup (output_bfd, info,
					 link_order->u.reloc.p->u.name,
					 FALSE, FALSE, TRUE));
      if (h != (struct ecoff_link_hash_entry *) NULL && h->indx != -1)
	in.r_symndx = h->indx;
      else
	{
