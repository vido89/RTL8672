...
xtensa_read_table_entries (bfd * abfd,
			   ...
			   const char *sec_name, bfd_boolean output_addr)
{
  ...
  if (!section
      || !(section->flags & SEC_ALLOC) || (section->flags & SEC_DEBUGGING))
    {
      *table_p = NULL;
      return 0;
    }
...
elf_xtensa_find_property_entry (property_table_entry * property_table,
				int property_table_size, bfd_vma addr)
{
...
elf_xtensa_in_literal_pool (property_table_entry * lit_table,
			    int lit_table_size, bfd_vma addr)
{
...
elf_xtensa_create_dynamic_sections (bfd * dynobj, struct bfd_link_info *info)
{
  ...
  if (s == NULL || !bfd_set_section_flags (dynobj, s, flags))
    return FALSE;
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
	...
	abort ();
      ...
      elf_link_hash_traverse (elf_hash_table (info),
			      elf_xtensa_fix_refcounts, (void *) info);
	...
	abort ();
      elf_link_hash_traverse (elf_hash_table (info),
			      elf_xtensa_allocate_got_size, (void *) srelgot);
	...
	abort ();
      elf_link_hash_traverse (elf_hash_table (info),
			      elf_xtensa_allocate_plt_size, (void *) srelplt);
	  ...
	  for (s = abfd->sections; s != NULL; s = s->next)
	    {
		  ...
		  && xtensa_is_littable_section (s) && s != spltlittbl)
		sgotloc->size += s->size;
	    }
	}
    }
		     ...
		     bfd_boolean is_weak_undef, char **error_message)
{
  ...
  switch (howto->type)
      ...
      if (!is_weak_undef)
	{
	  ...
	  xtensa_opcode opcode = get_expanded_call_opcode (contents + address,
							   input_size -
							   address, 0);
  ...
  if (is_alt_relocation (howto->type))
      ...
      if (opcode == get_l32r_opcode ())
	{
	  ...
	  self_address = ((lit4_sec->vma & ~0xfff) + 0x40000 - 3);	/* -3 to compensate for do_reloc */
	  newval = relocation;
	  opnd = 1;
	}
  ...
  if (is_direct_call_opcode (opcode) && is_windowed_call_opcode (opcode))
    {
      if ((self_address >> CALL_SEGMENT_BITS)
	  != (relocation >> CALL_SEGMENT_BITS))
	{
...
elf_xtensa_create_plt_entry (bfd * dynobj,
			     bfd * output_bfd, unsigned reloc_index)
{
  ...
  memcpy (splt->contents + code_offset,
	  (bfd_big_endian (output_bfd)
	   ...
	   : elf_xtensa_le_plt_entry), PLT_ENTRY_SIZE);
			     ...
			     asection ** local_sections)
{
  ...
  for (; rel < relend; rel++)
      ...
      else
	{
	  ...
	  if (relocation == 0
	      && !unresolved_reloc && h->root.type == bfd_link_hash_undefweak)
	    is_weak_undef = TRUE;
	}
...
elf_xtensa_combine_prop_entries (bfd * output_bfd,
				 asection * sxtlit, asection * sgotloc)
{
  bfd_byte *contents;
...
is_indirect_call_opcode (xtensa_opcode opcode)
{
  ...
  return (opcode == callx0_op
	  || opcode == callx4_op
	  || opcode == callx8_op || opcode == callx12_op);
}
...
is_direct_call_opcode (xtensa_opcode opcode)
{
  ...
  return (opcode == call0_op
	  || opcode == call4_op || opcode == call8_op || opcode == call12_op);
}
...
get_asm_simplify_size (bfd_byte * contents,
		       bfd_size_type content_len, bfd_size_type offset)
{
...
{
  return (r_type >= R_XTENSA_SLOT0_ALT && r_type <= R_XTENSA_SLOT14_ALT);
}
...
insn_decode_opcode (bfd_byte * contents,
		    bfd_size_type content_len, bfd_size_type offset, int slot)
{
  xtensa_isa isa = xtensa_default_isa;
  xtensa_format fmt;
...
narrow_instruction (bfd_byte * contents,
		    bfd_size_type content_length,
		    bfd_size_type offset, bfd_boolean do_it)
{
  xtensa_opcode opcode;
  ...
  for (opi = 0; opi < (sizeof (narrowable) / sizeof (struct string_pair));
       ++opi)
    {
		   ...
		   bfd_size_type offset, bfd_boolean do_it)
{
  ...
  for (opi = 0; opi < (sizeof (widenable) / sizeof (struct string_pair));
       ++opi)
    {
...
elf_xtensa_do_asm_simplify (bfd_byte * contents,
			    bfd_vma address,
			    bfd_vma content_length, char **error_message)
{
  static xtensa_insnbuf insnbuf = NULL;
  static xtensa_insnbuf slotbuf = NULL;
...
contract_asm_expansion (bfd_byte * contents,
			...
			Elf_Internal_Rela * irel, char **error_message)
{
  bfd_reloc_status_type retval =
			  ...
			  bfd_boolean * p_uses_l32r)
{
  ...
  if (opcode == get_l32r_opcode ())
	...
	*p_uses_l32r = FALSE;
	...
	return XTENSA_UNDEFINED;
	  ...
	  || xtensa_operand_decode (isa, opcode, CONST16_TARGET_REG_OPERAND,
				    &const16_regno) || const16_regno != regno)
	return XTENSA_UNDEFINED;
    }
  else
    return XTENSA_UNDEFINED;
  ...
  if (fmt == XTENSA_UNDEFINED
    ...
    return XTENSA_UNDEFINED;
  opcode = xtensa_opcode_decode (isa, fmt, 0, slotbuf);
  if (opcode == XTENSA_UNDEFINED || !is_indirect_call_opcode (opcode))
    return XTENSA_UNDEFINED;
...
r_reloc_is_defined (const r_reloc * r_rel)
{
  ...
  if (sec == bfd_abs_section_ptr
      || sec == bfd_com_section_ptr || sec == bfd_und_section_ptr)
    return FALSE;
  return TRUE;
}
...
r_reloc_init (r_reloc * r_rel,
	      ...
	      bfd_byte * contents, bfd_size_type content_length)
{
  int r_type;
  reloc_howto_type *howto;
...
init_source_reloc (source_reloc * reloc,
		   asection * source_sec,
		   const r_reloc * r_rel,
		   xtensa_opcode opcode, int opnd, bfd_boolean is_abs_literal)
{
...
find_source_reloc (source_reloc * src_relocs,
		   int src_count, asection * sec, Elf_Internal_Rela * irel)
{
  int i;
...
init_literal_value (literal_value * lit,
		    const r_reloc * r_rel,
		    unsigned long value, bfd_boolean is_abs_literal)
{
  lit->r_rel = *r_rel;
  lit->value = value;
...
add_value_map (value_map_hash_table * map,
	       const literal_value * val,
	       const r_reloc * loc, bfd_boolean final_static_link)
{
  value_map **bucket_p;
  unsigned idx;
...
compute_removed_action_diff (const text_action * ta,
			     asection * sec,
			     bfd_vma offset, int removed, int removable_space)
{
  int new_removed;
...
text_action_add (text_action_list * l,
		 text_action_t action,
		 asection * sec, bfd_vma offset, int removed)
{
  text_action **m_p;
  text_action *ta;
...
text_action_add_literal (text_action_list * l,
			 text_action_t action,
			 const r_reloc * loc,
			 const literal_value * value, int removed)
{
  text_action **m_p;
  text_action *ta;
...
add_removed_literal (removed_literal_list * removed_list,
		     const r_reloc * from, const r_reloc * to)
{
...
reloc_bfd_fix_init (asection * src_sec,
		    ...
		    bfd * target_abfd,
		    asection * target_sec,
		    bfd_vma target_offset, bfd_boolean translated)
{
  reloc_bfd_fix *fix;
...
section_cache_section (section_cache_t * sec_cache,
		       asection * sec, struct bfd_link_info *link_info)
{
  bfd *abfd;
...
init_ebb (ebb_t * ebb,
	  ...
	  unsigned ptblsize,
	  Elf_Internal_Rela * internal_relocs, unsigned reloc_count)
{
...
insn_block_decodable_len (bfd_byte * contents,
			  ...
			  bfd_vma block_offset, bfd_size_type block_len)
{
...
ebb_propose_action (ebb_constraint * c,
		    ...
		    bfd_vma alignment_pow,
		    text_action_t action,
		    bfd_vma offset, int removed_bytes, bfd_boolean do_action)
{
  proposed_action *act;
	      ...
	      || target_relax_info->is_relaxable_asm_section))
	{
	  ...
	  if (is_alt_relocation (ELF32_R_TYPE (irel->r_info)))
	  ...
	  else if (is_operand_relocation (ELF32_R_TYPE (irel->r_info)))
	    {
	      opcode = get_relocation_opcode (abfd, sec, contents, irel);
	      opnd =
		get_relocation_opnd (opcode, ELF32_R_TYPE (irel->r_info));
	    }
	  ...
	  if (opcode != XTENSA_UNDEFINED)
		...
		&target_relax_info->src_relocs[src_next];
...
text_action_add_proposed (text_action_list * l,
			  const ebb_constraint * ebb_table, asection * sec)
{
  unsigned i;
...
get_irel_at_offset (asection * sec,
		    Elf_Internal_Rela * internal_relocs, bfd_vma offset)
{
  unsigned i;
  Elf_Internal_Rela *irel;
  unsigned r_type;
...
is_removable_literal (const source_reloc * rel,
		      int i, const source_reloc * src_relocs, int src_count)
{
...
remove_dead_literal (bfd * abfd,
		     ...
		     Elf_Internal_Rela * irel,
		     source_reloc * rel,
		     property_table_entry * prop_table, int ptblsize)
{
  property_table_entry *entry;
  xtensa_relax_info *relax_info;
...
relocations_reach (source_reloc * reloc,
		   int remaining_relocs, const r_reloc * r_rel)
{
  ...
  for (i = 0; i < remaining_relocs; i++)
      ...
      if (reloc[i].opnd != -1)
	{
	  ...
	  dest_address = (sec->output_section->vma
			  + sec->output_offset + r_rel->target_offset);
...
coalesce_shared_literal (asection * sec,
			 source_reloc * rel,
			 property_table_entry * prop_table,
			 int ptblsize, value_map * val_map)
{
  property_table_entry *entry;
  text_action *fa;
  property_table_entry *the_add_entry;
      ...
      for (i = 0; i < sec->reloc_count; i++)
	{
	  ...
	  if (relax_info->is_relaxable_literal_section
					   ...
					   irel->r_offset))
		{
		  /* Remove this relocation.  */
		  if (elf_hash_table (link_info)->dynamic_sections_created)
		    shrink_dynamic_reloc_sections (link_info, abfd, sec,
						   irel);
		  irel->r_info = ELF32_R_INFO (0, R_XTENSA_NONE);
	  ...
	  if (target_relax_info
		  ...
		  || target_relax_info->is_relaxable_asm_section))
	    {
		  ...
		  || r_type == R_XTENSA_DIFF16 || r_type == R_XTENSA_DIFF32)
		{
	  ...
	  if (action->offset > orig_dot)
	    {
	      ...
	      memmove (&dup_contents[dup_dot], &contents[orig_dot],
		       copy_size);
	      orig_dot += copy_size;
	      dup_dot += copy_size;
...
do_fix_for_final_link (Elf_Internal_Rela * rel,
		       bfd * input_bfd,
		       asection * input_section,
		       bfd_byte * contents, bfd_vma * relocationp)
{
  asection *sec;
  int r_type = ELF32_R_TYPE (rel->r_info);
  reloc_bfd_fix *fix;
  ...
  *relocationp = (sec->output_section->vma
		  + sec->output_offset + fix->target_offset - fixup_diff);
}

...
pcrel_reloc_fits (xtensa_opcode opc,
		  int opnd, bfd_vma self_address, bfd_vma dest_address)
{
  xtensa_isa isa = xtensa_default_isa;
...
xtensa_is_littable_section (asection * sec)
{
  ...
  if (strncmp (".gnu.linkonce.", sec->name, linkonce_len) == 0
      && sec->name[linkonce_len] == 'p' && sec->name[linkonce_len + 1] == '.')
    return TRUE;
...
xtensa_get_property_predef_flags (asection * sec)
{
  if (strcmp (sec->name, XTENSA_INSN_SEC_NAME) == 0
    ...
    return (XTENSA_PROP_INSN
	    | XTENSA_PROP_INSN_NO_TRANSFORM | XTENSA_PROP_INSN_NO_REORDER);
...
xtensa_callback_required_dependence (bfd * abfd,
				     asection * sec,
				     struct bfd_link_info * link_info,
				     deps_callback_t callback, void *closure)
{
  Elf_Internal_Rela *internal_relocs;
  bfd_byte *contents;
  unsigned i;
