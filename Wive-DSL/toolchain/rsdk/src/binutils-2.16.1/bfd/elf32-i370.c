...
static bfd_boolean i370_elf_set_private_flags PARAMS ((bfd *, flagword));

...
i370_elf_howto_init ()
{
  ...
  for (i = 0;
       i < sizeof (i370_elf_howto_raw) / sizeof (i370_elf_howto_raw[0]); i++)
    {
      type = i370_elf_howto_raw[i].type;
      BFD_ASSERT (type <
		  sizeof (i370_elf_howto_table) /
		  sizeof (i370_elf_howto_table[0]));
     ...
     bfd_reloc_code_real_type code;
{
  ...
  switch ((int) code)
    ...
    case BFD_RELOC_NONE:
      i370_reloc = R_I370_NONE;
      break;
    case BFD_RELOC_32:
      i370_reloc = R_I370_ADDR31;
      break;
    case BFD_RELOC_16:
      i370_reloc = R_I370_ADDR16;
      break;
    case BFD_RELOC_32_PCREL:
      i370_reloc = R_I370_REL31;
      break;
    case BFD_RELOC_CTOR:
      i370_reloc = R_I370_ADDR31;
      break;
    case BFD_RELOC_I370_D12:
      i370_reloc = R_I370_ADDR12;
      break;
    }
...
static bfd_boolean i370_elf_merge_private_bfd_data PARAMS ((bfd *, bfd *));
      ...
      for (s = output_bfd->sections; s != NULL; s = s->next)
	{
	  ...
	  if (dindx != -1)
		...
		maxdindx = dindx;
	      ...
	      bfd_elf32_swap_symbol_out (output_bfd, &sym, (PTR) esym,
					 (PTR) 0);
	    }
	}
...
#ifdef DEBUG
  _bfd_error_handler
    ("i370_elf_relocate_section called for %B section %A, %ld relocations%s",
     input_bfd, input_section, (long) input_section->reloc_count,
	  ...
	  if (info->shared && r_symndx != 0)
	    {
	      ...
	      fprintf (stderr,
		       ...
		       (h
			&& h->root.root.string) ? h->root.root.
		       string : "<unknown>");
#endif
		  ...
		  if (r_type == R_I370_ADDR31)
		      ...
		      else
			{
			  ...
			  if (indx <= 0)
			    {
			      printf
				("indx=%d section=%s flags=%08x name=%s\n",
	...
	case (int) R_I370_RELATIVE:
	  (*_bfd_error_handler)
	    ("%B: Relocation %s is not yet supported for symbol %s.",
	     input_bfd, i370_elf_howto_table[(int) r_type]->name, sym_name);
