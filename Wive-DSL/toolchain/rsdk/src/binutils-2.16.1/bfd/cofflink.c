...
_bfd_coff_link_hash_table_init (struct coff_link_hash_table *table,
				bfd * abfd,
				struct bfd_hash_entry *(*newfunc) (struct
								   bfd_hash_entry
								   *,
								   struct
								   bfd_hash_table
...
coff_link_add_object_symbols (bfd * abfd, struct bfd_link_info *info)
{
  ...
  if (!info->keep_memory && !_bfd_coff_free_symbols (abfd))
    return FALSE;
...
coff_link_check_ar_symbols (bfd * abfd,
			    struct bfd_link_info *info, bfd_boolean * pneeded)
{
  ...
  *pneeded = FALSE;
  ...
  while (esym < esym_end)
	  ...
	  || classification == COFF_SYMBOL_COMMON)
	{
	  ...
	  /* Auto import.  */
	  if (!h && info->pei386_auto_import && !strncmp (name, "__imp_", 6))
	    h =
	      bfd_link_hash_lookup (info->hash, name + 6, FALSE, FALSE, TRUE);
				 ...
				 bfd_boolean * pneeded)
{
  ...
  if (*pneeded && !coff_link_add_symbols (abfd, info))
    return FALSE;

  if ((!info->keep_memory || !*pneeded) && !_bfd_coff_free_symbols (abfd))
    return FALSE;
...
static bfd_boolean
coff_link_add_symbols (bfd * abfd, struct bfd_link_info *info)
{
      ...
      if (classification != COFF_SYMBOL_LOCAL)
	{
	  ...
	  if (obj_pe (abfd)
		...
		*sym_hash = coff_link_hash_lookup (coff_hash_table (info),
		  ...
		  comdat != NULL
		  &&
		  strcmp (coff_section_data
			  (abfd,
			   (*sym_hash)->root.u.def.section)->comdat->name,
			  coff_section_data (abfd,
					     section)->comdat->name) == 0)
		addit = FALSE;
	    }
		  ...
		  if (sym.n_numaux != 0)
			...
			goto error_return;
		      for (i = 0, eaux = esym + symesz, iaux = alloc;
			   i < sym.n_numaux; i++, eaux += symesz, iaux++)
...
bfd_boolean
_bfd_coff_final_link (bfd * abfd, struct bfd_link_info * info)
{
  bfd_size_type symesz;
  ...
  for (o = abfd->sections; o != NULL; o = o->next)
      ...
      else
	{
      ...
      if (bfd_coff_long_section_names (abfd) && strlen (o->name) > SCNNMLEN)
	{
  ...
  if ((finfo.internal_syms == NULL && max_sym_count > 0)
      ...
      || (!info->relocatable
	  && finfo.internal_relocs == NULL && max_reloc_count > 0))
    goto error_return;
  ...
  if (finfo.last_file_index != -1
      && (unsigned int) finfo.last_file.n_value !=
      obj_raw_syment_count (abfd))
    {
      file_ptr pos;

      finfo.last_file.n_value = obj_raw_syment_count (abfd);
      bfd_coff_swap_sym_out (abfd, &finfo.last_file, finfo.outsyms);
...
mark_relocs (struct coff_final_link_info *finfo, bfd * input_bfd)
{
  ...
  for (a = input_bfd->sections; a != (asection *) NULL; a = a->next)
	...
	(input_bfd, a, FALSE,
	 ...
	 (finfo->info->relocatable
	  ? (finfo->section_info[a->output_section->target_index].relocs +
	     a->output_section->reloc_count) : finfo->internal_relocs));
...
_bfd_coff_link_input_bfd (struct coff_final_link_info *finfo, bfd * input_bfd)
{
  ...
  if ((finfo->info->strip != strip_none
       || finfo->info->discard != discard_none) && finfo->info->relocatable)
    {
  ...
  while (esym < esym_end)
	      ...
	      || finfo->info->discard == discard_l))
	{
	  ...
	  && (isym.n_sclass == C_ENTAG
	      ...
	      || isym.n_sclass == C_UNTAG) && isym.n_numaux == 1)
	{
	  const char *name;
	  char buf[SYMNMLEN + 1];
	  ...
	  while (esl < eslend)
		...
		return FALSE;
	      ...
	      (*epp)->tagndx = 0;
	      if (islp->n_numaux >= 1
		  && islp->n_type != T_NULL && islp->n_sclass != C_EOS)
		{
      ...
      if (!skip)
	{
	  ...
	  switch (isym.n_sclass)
	    ...
	    case C_FCN:
	      if (obj_pe (input_bfd)
		  && strcmp (isym.n_name, ".bf") != 0 && isym.n_scnum > 0)
		{
  ...
  while (esym < esym_end)
    {
	  ...
	  && (*sym_hash == NULL || (*sym_hash)->auxbfd != input_bfd))
	esym += add * isymesz;
      else
	{
	  struct coff_link_hash_entry *h;
  ...
  if (finfo->info->strip == strip_none || finfo->info->strip == strip_some)
    {
		  ...
		  if (indx < 0)
		  ...
		  else
		    {
		      ...
		      bfd_coff_swap_sym_in (output_bfd,
						...
						* osymesz)), &is);
		      if ((ISFCN (is.n_type)
			   || is.n_sclass == C_BLOCK) && is.n_numaux >= 1)
			{
			  void *auxptr;

			  auxptr = (finfo->outsyms
				    + ((indx - syment_base + 1) * osymesz));
  ...
  for (o = input_bfd->sections; o != NULL; o = o->next)
	...
	{
	  if ((o->flags & SEC_RELOC) != 0 && o->reloc_count != 0)
	    {
	      (*_bfd_error_handler)
  ...
  if (!finfo->info->keep_memory && !_bfd_coff_free_symbols (input_bfd))
    return FALSE;
	  ...
	  if (sec != NULL)
	    {
		  ...
		  && (!obj_pe (output_bfd) || finfo->info->relocatable))
		(*_bfd_error_handler)
		  (_("%s: %s: reloc overflow: 0x%lx > 0xffff"),
		   bfd_get_filename (output_bfd),
		   bfd_get_section_name (output_bfd, sec), sec->reloc_count);
		...
		(*_bfd_error_handler)
		  (_("%s: warning: %s: line number overflow: 0x%lx > 0xffff"),
		   bfd_get_filename (output_bfd),
		   bfd_get_section_name (output_bfd, sec), sec->lineno_count);
	  ...
	  if (h->root.type == bfd_link_hash_defined
	      ...
	      if (h->class == C_NT_WEAK && h->numaux == 1)
		{
		  ...
		  struct coff_link_hash_entry *h2 =
		    input_bfd->tdata.coff_obj_data->sym_hashes[h->aux->x_sym.
							       x_tagndx.l];
