...
static struct elf32_m68hc11_stub_hash_entry *m68hc12_add_stub
  (const char *stub_name,
   asection * section, struct m68hc11_elf_link_hash_table *htab);
...
int
elf32_m68hc11_setup_section_lists (bfd * output_bfd,
	   ...
	   section != NULL; section = section->next)
	{
	  ...
	  for (; irela < irelaend; irela++)
	      ...
	      else
		{
		  ...
		  hash = (struct elf_link_hash_entry *) (sym_hashes[e_indx]);
		...
		(htab->stub_hash_table, stub_name, FALSE, FALSE);
	      if (stub_entry == NULL)
		{
...
m68hc11_elf_export_one_stub (struct bfd_hash_entry *gen_entry, void *in_arg)
{
  ...
  m68hc11_elf_set_symbol (htab->stub_bfd, info, name,
			  stub_entry->stub_offset, stub_entry->stub_sec);
  return result;
}
...
elf32_m68hc11_build_stubs (bfd * abfd, struct bfd_link_info *info)
{
  ...
  for (stub_sec = htab->stub_bfd->sections;
       stub_sec != NULL; stub_sec = stub_sec->next)
    {
      ...
      size = stub_sec->size;
      stub_sec->contents =
	(unsigned char *) bfd_zalloc (htab->stub_bfd, size);
  ...
  if (param.use_memory_banks)
    {
      m68hc11_elf_set_symbol (abfd, info, BFD_M68HC11_BANK_START_NAME,
			      htab->pinfo.bank_physical, bfd_abs_section_ptr);
      m68hc11_elf_set_symbol (abfd, info, BFD_M68HC11_BANK_VIRTUAL_NAME,
			      htab->pinfo.bank_virtual, bfd_abs_section_ptr);
      m68hc11_elf_set_symbol (abfd, info, BFD_M68HC11_BANK_SIZE_NAME,
			      htab->pinfo.bank_size, bfd_abs_section_ptr);
    }
...
static void
scan_sections_for_abi (bfd * abfd ATTRIBUTE_UNUSED,
...
void
elf32_m68hc11_post_process_headers (bfd * abfd,
