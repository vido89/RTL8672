     ...
     struct internal_reloc *intern;
{
  ...
  if (intern->r_type == ALPHA_R_LITUSE || intern->r_type == ALPHA_R_GPDISP)
    {
  ...
  if (intern->r_type == ALPHA_R_LITUSE || intern->r_type == ALPHA_R_GPDISP)
    {
      ...
      size = 0;
	   ...
	   && !intern->r_extern && intern->r_symndx == RELOC_SECTION_ABS)
    {
  ...
  if (h->root.type == bfd_link_hash_defined
	...
	abort ();
      ...
      relocation = (h->root.u.def.value
		    + hsec->output_section->vma + hsec->output_offset);
    }
  else
    {
  ...
  for (; ext_rel < ext_rel_end; ext_rel++)
      ...
      if (info->relocatable && adjust_addrp)
	{
	  ...
	  H_PUT_64 (input_bfd,
		    (input_section->output_section->vma
		     + input_section->output_offset
		     - input_section->vma + r_vaddr), ext_rel->r_vaddr);
	}
