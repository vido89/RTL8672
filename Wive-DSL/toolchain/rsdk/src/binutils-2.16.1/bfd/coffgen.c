...
static char *build_debug_section PARAMS ((bfd *));
static char *copy_name PARAMS ((bfd *, char *, size_t));
     ...
     unsigned int target_index;
{
  ...
  if (bfd_coff_long_section_names (abfd) && hdr->s_name[0] == '/')
    {
      char buf[SCNNMLEN];
      long strindex;
      char *p;
  ...
  for (p = abfd->outsymbols, i = 0; i < limit; i++, p++)
      ...
      if (bfd_family_coff (bfd_asymbol_bfd (q_maybe)))
	{
	  ...
	  /* The AIX 4.1 compiler can sometimes generate line numbers
	     ...
	     those here.  */
	  if (q->lineno != NULL && q->symbol.section->owner != NULL)
	    {
     ...
     asymbol *symbol;
{
  ...
  if (bfd_asymbol_bfd (symbol)->tdata.coff_obj_data ==
      (coff_data_type *) NULL)
    return (coff_symbol_type *) NULL;
  ...
  if (native->u.syment.n_numaux > 0)
      ...
      for (j = 0; j < native->u.syment.n_numaux; j++)
	{
	  bfd_coff_swap_aux_out (abfd,
				 ...
				 type,
				 class,
				 (int) j, native->u.syment.n_numaux, buf);
...
#error Change H_PUT_32
      ...
      for (p = abfd->outsymbols, i = 0; i < limit; i++, p++)
	{
	  ...
	  if (c_symbol == NULL || c_symbol->native == NULL)
	    {
	      ...
	      maxlen =
		bfd_coff_force_symnames_in_strings (abfd) ? 0 : SYMNMLEN;
	    }
	  ...
	  if (name_length > maxlen)
	    {
	      if (bfd_bwrite
		  ((PTR) (q->name), (bfd_size_type) name_length + 1,
		   abfd) != name_length + 1)
		return FALSE;
	    }
	}
     ...
     bfd *abfd;
{
  ...
  for (internal_ptr = internal;
       raw_src < raw_end; raw_src += symesz, internal_ptr++)
    {
     ...
     union internal_auxent *pauxent;
{
  ...
  if (csym == NULL
      || csym->native == NULL || indx >= csym->native->u.syment.n_numaux)
    {
      bfd_set_error (bfd_error_invalid_operation);
      return FALSE;
    }
  ...
  switch (how)
      ...
      if (coffsymbol (symbol)->native)
	{
	  ...
	  fprintf (file,
		   ...
		   combined->u.syment.n_type,
		   combined->u.syment.n_sclass,
		   combined->u.syment.n_numaux, val, symbol->name);
#endif
      ...
      else
	{
	  ...
	  fprintf (file, " %-5s %s %s %s",
		   symbol->section->name,
		   coffsymbol (symbol)->native ? "n" : "g",
		   coffsymbol (symbol)->lineno ? "l" : " ", symbol->name);
	}
    }
}
  ...
  *line_ptr = 0;
  ...
  if (p < pend)
      ...
      while (1)
	{
	  ...
	  for (p2 = p + 1 + p->u.syment.n_numaux;
	       p2 < pend; p2 += 1 + p2->u.syment.n_numaux)
	    {
  ...
  if (section->lineno != NULL)
      ...
      for (; i < section->lineno_count; i++)
	{
	  ...
	  && last_value != 0 && offset - last_value > 0x100)
	{
