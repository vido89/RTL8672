     ...
     PTR out;
{
  ...
  memcpy (scnhdr_ext->s_name, scnhdr_int->s_name,
...

  {
    ...
    pe_required_section_flags known_sections[] = {
      ...
      {".rsrc",
       ...
       IMAGE_SCN_MEM_WRITE},
      {".text",
       IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_CODE | IMAGE_SCN_MEM_EXECUTE},
      {".tls",
       IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_INITIALIZED_DATA |
       IMAGE_SCN_MEM_WRITE},
      ...
      if (hint_addr != 0)
	{
	  ...
	  if (first_thunk != hint_addr)
	    {
	      ...
	      for (ft_section = abfd->sections;
		   ft_section != NULL; ft_section = ft_section->next)
		{
	  ...
	  for (j = 0; j < datasize; j += 4)
	      ...
	      else
		{
	      ...
	      if (time_stamp != 0
		  && first_thunk != 0 && first_thunk != hint_addr)
		fprintf (file, "\t%04lx",
  ...
  for (i = 0; i < edt.num_names; ++i)
    {
      bfd_vma name_ptr = bfd_get_32 (abfd,
				     data + edt.npt_addr + (i * 4) - adj);

      char *name = (char *) data + name_ptr - adj;

      bfd_vma ord = bfd_get_16 (abfd,
				data + edt.ot_addr + (i * 2) - adj);
      fprintf (file, "\t[%4ld] %s\n", (long) ord, name);
    }
