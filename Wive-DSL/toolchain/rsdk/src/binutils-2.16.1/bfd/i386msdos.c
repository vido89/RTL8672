...
static int msdos_sizeof_headers PARAMS ((bfd *, bfd_boolean));
static bfd_boolean msdos_write_object_contents PARAMS ((bfd *));
  ...
  for (sec = abfd->sections; sec != (asection *) NULL; sec = sec->next)
      ...
      if (bfd_get_section_flags (abfd, sec) & SEC_LOAD)
	{
	  file_ptr sec_end = (sizeof (hdr)
			      + bfd_get_section_vma (abfd, sec) + sec->size);
...
const bfd_target i386msdos_vec = {
  ...
  (EXEC_P),			/* object flags */
  (SEC_CODE | SEC_DATA | SEC_HAS_CONTENTS | SEC_ALLOC | SEC_LOAD),	/* section flags */
