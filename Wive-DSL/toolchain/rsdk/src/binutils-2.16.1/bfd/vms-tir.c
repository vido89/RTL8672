...
static bfd_boolean etir_sta PARAMS ((bfd *, int, unsigned char *));
static bfd_boolean etir_sto PARAMS ((bfd *, int, unsigned char *));
static bfd_boolean etir_opr PARAMS ((bfd *, int, unsigned char *));
static bfd_boolean etir_ctl PARAMS ((bfd *, int, unsigned char *));
static bfd_boolean etir_stc PARAMS ((bfd *, int, unsigned char *));
     ...
     unsigned char *ptr;
{
  ...
  switch (*ptr++)
    ...
    case TIR_S_C_OPR_DFLIT:	/* Define a literal.  */
      (*_bfd_error_handler) (_("%s: not supported"), tir_cmd_name (ptr[-1]));
      break;
...
tir_cmd (bfd * abfd, unsigned char *ptr)
{
  ...
  if (*ptr & 0x80)		/* store immediate */
	...
	{
	  if ((tir_table[i].mincod <= *ptr) && (*ptr <= tir_table[i].maxcod))
	    {
     ...
     unsigned char *ptr;
{
  ...
  while (etir_table[i].mincod >= 0)
    {
      if ((etir_table[i].mincod <= cmd) && (cmd <= etir_table[i].maxcod))
	{
     ...
     int objtype;
{
  ...
  switch (objtype)
    ...
    case EOBJ_S_C_ETIR:
      ...
      PRIV (rec_size) -= 4;
      result =
	analyze_etir (abfd, PRIV (vms_rec), (unsigned) PRIV (rec_size));
      break;
    case OBJ_S_C_TIR:
  ...
  while (section != NULL)
      ...
      if (section->flags & SEC_RELOC)
	{
	  ...
	  if ((i = section->reloc_count) <= 0)
		...
		{
		  _bfd_vms_debug (4,
				  "sym %s in sec %s, value %08lx, addr %08lx, off %08lx, len %d: %s\n",
				...
				if (bfd_is_und_section (sym->section))
				  {
				    ...
				    if (_bfd_vms_output_check (abfd, slen) <
					0)
				      {
					end_etir_record (abfd);
