...
static bfd_boolean mips_elf_stub_section_p (bfd *, asection *);
			     ...
			     void *data ATTRIBUTE_UNUSED)
{
  ...
  if (h->fn_stub != NULL && !h->need_fn_stub)
    {
  ...
  if (h->call_stub != NULL && h->root.other == STO_MIPS16)
    {
  ...
  if (h->call_fp_stub != NULL && h->root.other == STO_MIPS16)
    {
      /* We don't need the call_stub; this is a 16 bit function, so
         calls from other 16 bit functions are OK.  Clobber the size
         to 0 to prevent it from being included in the link.  */
...
_bfd_mips_elf_gprel16_with_gp (bfd * abfd, asymbol * symbol,
			       ...
			       bfd_boolean relocatable, void *data,
			       bfd_vma gp)
{
  bfd_vma relocation;
  bfd_signed_vma val;
...
bfd_reloc_status_type
_bfd_mips_elf_got16_reloc (bfd * abfd, arelent * reloc_entry,
...
bfd_reloc_status_type
_bfd_mips_elf_generic_reloc (bfd * abfd ATTRIBUTE_UNUSED,
...
void
bfd_mips_elf64_swap_reginfo_out (bfd * abfd,
...
mips_elf_output_extsym (struct mips_elf_link_hash_entry *h, void *data)
{
  ...
  if (h->root.root.type == bfd_link_hash_common)
	...
	h->esym.asym.value = (h->root.root.u.def.value
			      + sec->output_offset + output_section->vma);
      else
	h->esym.asym.value = 0;
    }
			       ...
			       bfd_vma value)
{
  ...
  if (*tls_type_p & GOT_TLS_GD)
	...
	{
	  MIPS_ELF_PUT_WORD (abfd, 1, sgot->contents + offset);
	  MIPS_ELF_PUT_WORD (abfd, value - dtprel_base (info),
			     sgot->contents + offset2);
	}
  ...
  if (*tls_type_p & GOT_TLS_IE)
	...
	{
	  if (indx == 0)
	    MIPS_ELF_PUT_WORD (abfd,
			       value - elf_hash_table (info)->tls_sec->vma,
			       sgot->contents + offset);
	  else
	    MIPS_ELF_PUT_WORD (abfd, 0, sgot->contents + offset);
  ...
  if (*tls_type_p & GOT_TLS_LDM)
    {
      ...
      if (!info->shared)
	MIPS_ELF_PUT_WORD (abfd, 1, sgot->contents + got_offset);
      else
	mips_elf_output_dynamic_relocation
	  (abfd, sreloc, indx,
	   ABI_64_P (abfd) ? R_MIPS_TLS_DTPMOD64 : R_MIPS_TLS_DTPMOD32,
...
static bfd_vma
mips_elf_global_got_index (bfd * abfd, bfd * ibfd,
		      ...
		      bfd_vma value, bfd_boolean external)
{
  ...
  entry =
    mips_elf_create_local_got_entry (abfd, ibfd, g, sgot, value, 0, NULL,
				     R_MIPS_GOT16);
  if (entry)
    return entry->gotidx;
  else
				 ...
				 int r_type)
{
  ...
  if (TLS_RELOC_P (r_type))
	...
	entry.d.h = h;
      ...
      p = (struct mips_got_entry *) htab_find (g->got_entries, &entry);
  ...
  if (g->assigned_gotno >= g->local_gotno)
    {
      ...
      (*_bfd_error_handler) (_("not enough GOT space for local GOT entries"));
      bfd_set_error (bfd_error_bad_value);
      return NULL;
    }

  MIPS_ELF_PUT_WORD (abfd, value, (sgot->contents + entry.gotidx));
...
mips_elf_sort_hash_table (struct bfd_link_info *info, unsigned long max_local)
{
  ...
  mips_elf_link_hash_traverse (((struct mips_elf_link_hash_table *)
				elf_hash_table (info)),
			       mips_elf_sort_hash_table_f, &hsd);
...
mips_elf_make_got_per_bfd (void **entryp, void *p)
{
  ...
  struct mips_elf_got_per_bfd_arg *arg =
    (struct mips_elf_got_per_bfd_arg *) p;
...
mips_elf_merge_gots (void **bfd2got_, void *p)
{
  ...
  /* If it looks like we can merge this bfd's entries with those of
	   ...
	   && (arg->primary_count + lcount + gcount + tcount) <= maxcnt)
    {
      ...
      htab_traverse (g->got_entries, mips_elf_make_got_per_bfd, arg);
      if (arg->obfd == NULL)
	return 0;
...
mips_elf_initialize_tls_index (void **entryp, void *p)
{
  ...
  entry->gotidx =
    MIPS_ELF_GOT_SIZE (entry->abfd) * (long) g->tls_assigned_gotno;
...
mips_elf_set_global_got_offset (void **entryp, void *p)
{
  ...
  if (entry->abfd != NULL && entry->symndx == -1
      && entry->d.h->root.dynindx != -1 && entry->d.h->tls_type == GOT_NORMAL)
    {
      if (g)
	{
...
mips_elf_set_no_stub (void **entryp, void *p ATTRIBUTE_UNUSED)
{
  ...
  if (entry->abfd != NULL
      && entry->symndx == -1 && entry->d.h->root.dynindx != -1)
  ...
  do
    {
      ...
      htab_traverse (got_entries,
		     mips_elf_resolve_final_got_entry, &got_entries);
    }
			       ...
			       bfd_boolean save_addend)
{
  ...
  p = (input_section->output_section->vma
       + input_section->output_offset + relocation->r_offset);
	...
	*namep = bfd_section_name (input_bfd, sec);
	...
	h = (struct mips_elf_link_hash_entry *) h->root.root.u.i.link;
      ...
      *namep = h->root.root.root.string;
      ...
      if (strcmp (*namep, "_gp_disp") == 0 && !NEWABI_P (input_bfd))
	{
  ...
  *require_jalxp = (!info->relocatable
  ...
  switch (r_type)
	...
	goto got_disp;
      value =
	mips_elf_got_page (abfd, input_bfd, info, symbol + addend, NULL);
      if (value == MINUS_ONE)
	return bfd_reloc_outofrange;
			     ...
			     bfd_boolean require_jalx)
{
      ...
      if (!ok)
	{
	  (*_bfd_error_handler)
	    (_("%B: %A+0x%lx: jump to stub routine which is not jal"),
	     input_bfd, input_section, (unsigned long) relocation->r_offset);
	  bfd_set_error (bfd_error_bad_value);
	  return FALSE;
	}
  ...
  if (bfd_get_mach (input_bfd) == bfd_mach_mips9000 && !info->relocatable && !require_jalx && ((r_type == R_MIPS_26 && (x >> 26) == 0x3)	/* jal addr */
											       || (r_type == R_MIPS_JALR && x == 0x0320f809)))	/* jalr t9 */
    {
      ...
      addr = (input_section->output_section->vma
	      + input_section->output_offset + relocation->r_offset + 4);
      if (r_type == R_MIPS_26)
				    ...
				    asection * input_section)
{
  ...
  /* We must now calculate the dynamic symbol table index to use
     in the relocation.  */
  if (h != NULL && (!info->symbolic || !h->root.def_regular)
...
_bfd_mips_elf_symbol_processing (bfd * abfd, asymbol * asym)
{
  ...
  switch (elfsym->internal_elf_sym.st_shndx)
    ...
    case SHN_COMMON:
      ...
      if (asym->value > elf_gp_size (abfd) || IRIX_COMPAT (abfd) == ict_irix6)
	break;
      /* Fall through.  */
...
{
  if (hdr->sh_type == SHT_MIPS_REGINFO && hdr->sh_size > 0)
    {
      bfd_byte buf[4];
  ...
  if (hdr->bfd_section != NULL)
    {
	  ...
	  || strcmp (name, ".lit8") == 0 || strcmp (name, ".lit4") == 0)
	{
	  ...
	  if (ABI_64_P (abfd) && intopt.kind == ODK_REGINFO)
		...
		(abfd,
		 ((Elf64_External_RegInfo *)
		  (l + sizeof (Elf_External_Options))), &intreg);
...
bfd_boolean
_bfd_mips_elf_fake_sections (bfd * abfd, Elf_Internal_Shdr * hdr,
			     asection * sec)
{
			       ...
			       asection ** secp, bfd_vma * valp)
{
  ...
  switch (sym->st_shndx)
      ...
      if (elf_tdata (abfd)->elf_text_section == NULL)
	{
	  ...
	  elf_text_section->symbol_ptr_ptr =
	    &elf_tdata (abfd)->elf_text_symbol;
      ...
      if (elf_tdata (abfd)->elf_data_section == NULL)
	{
	  ...
	  elf_data_section->symbol_ptr_ptr =
	    &elf_tdata (abfd)->elf_data_symbol;
...
bfd_boolean
_bfd_mips_elf_create_dynamic_sections (bfd * abfd,
				       struct bfd_link_info * info)
{
  struct elf_link_hash_entry *h;
  struct bfd_link_hash_entry *bh;
  ...
  if ((IRIX_COMPAT (abfd) == ict_irix5 || IRIX_COMPAT (abfd) == ict_none)
      && !info->shared && bfd_get_section_by_name (abfd, ".rld_map") == NULL)
    {
			    ...
			    asection * sec, const Elf_Internal_Rela * relocs)
{
  ...
  if (strncmp (name, FN_STUB, sizeof FN_STUB - 1) == 0)
    {
      ...
      if (r_symndx < extsymoff || sym_hashes[r_symndx - extsymoff] == NULL)
	{
	  asection *o;
  ...
  for (rel = relocs; rel < rel_end; ++rel)
	...
	{
	  (*_bfd_error_handler)
	    (_("%B: Malformed reloc detected for section %s"), abfd, name);
	  bfd_set_error (bfd_error_bad_value);
	  return FALSE;
	}
      else
      ...
      if (dynobj == NULL || sgot == NULL)
	{
      ...
      if (!h && (r_type == R_MIPS_CALL_LO16
		 || r_type == R_MIPS_GOT_LO16 || r_type == R_MIPS_GOT_DISP))
	{
...
#define MIPS_READONLY_SECTION (SEC_ALLOC | SEC_LOAD | SEC_READONLY)
	      ...
	      if (h != NULL)
		{
		  ...
		  if (!mips_elf_create_got_section (dynobj, info, TRUE))
		    return FALSE;
		  g = mips_elf_got_info (dynobj, &sgot);
		  if (!mips_elf_record_global_got_symbol
		      (h, abfd, info, g, 0))
		    return FALSE;
		}
	    }
				     ...
				     struct elf_link_hash_entry * h)
{
  ...
  if (!hmips->no_fn_stub && h->needs_plt)
    {
				    ...
				    struct bfd_link_info * info)
{
  ...
  if (!(info->relocatable || !mips_elf_hash_table (info)->mips16_stubs_seen))
  ...
  for (sub = info->input_bfds; sub; sub = sub->link_next)
	   ...
	   subsection; subsection = subsection->next)
	{
	  if ((subsection->flags & SEC_ALLOC) == 0)
	    continue;
	  loadable_size += ((subsection->size + 0xf) & ~(bfd_size_type) 0xf);
	}
    }
  ...
  elf_link_hash_traverse (elf_hash_table (info),
			  mips_elf_count_global_tls_entries, &count_tls_arg);
				     ...
				     struct bfd_link_info * info)
{
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
      ...
      if (info->executable)
	{
	  ...
	  BFD_ASSERT (s != NULL);
	  s->size = strlen (ELF_DYNAMIC_INTERPRETER (output_bfd)) + 1;
	  s->contents = (bfd_byte *) ELF_DYNAMIC_INTERPRETER (output_bfd);
	}
    }
		  ...
		  if (info->shared)
		    {
		      ...
		      BFD_ASSERT (g->assigned_gotno == g->next->local_gotno
				  + g->next->global_gotno
				  + g->next->tls_gotno + MIPS_RESERVED_GOTNO);
		    }
		}
	    }
	  else
  ...
  for (rel = relocs; rel < relend; ++rel)
	  ...
	  if (rel_hdr->sh_entsize == MIPS_ELF_REL_SIZE (input_bfd))
						      ...
						      local_sections, FALSE)))
		{
		  ...
		  _bfd_mips16_elf_reloc_unshuffle (input_bfd, lo16_type,
						   FALSE, lo16_location);
		  l =
		    mips_elf_obtain_contents (lo16_howto, lo16_relocation,
					      input_bfd, contents);
      ...
      if (info->relocatable)
	{
	  ...
	  if (r_type == R_MIPS16_GPREL
	      ...
	      || r_type == R_MIPS_GPREL32 || r_type == R_MIPS_LITERAL)
	  ...
	  if (rela_relocation_p)
		   ...
		   but for endianness.  */
		{
		  ...
		  bfd_put_32 (input_bfd, low_bits, contents + rel->r_offset);
	   ...
	   only a 32-bit VMA.  */
	{
	  ...
	  bfd_put_32 (input_bfd, low_bits, contents + rel->r_offset);
	  bfd_put_32 (input_bfd, high_bits, contents + rel->r_offset + 4);
	  continue;
	}
				      ...
				      Elf_Internal_Sym * sym)
{
  ...
  for (i = 0; i < 2; ++i)
    for (p = (i == 0) ? text_section_symbols : data_section_symbols; *p; ++p)
      if (strcmp (*p, name) == 0)
	{
  ...
  if (g->global_gotsym != NULL && h->dynindx >= g->global_gotsym->dynindx)
    {
      ...
      value = sym->st_value;
      offset =
	mips_elf_global_got_index (dynobj, output_bfd, h, R_MIPS_GOT16, info);
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
    {
      ...
      for (b = sdyn->contents;
	   b < sdyn->contents + sdyn->size; b += MIPS_ELF_DYN_SIZE (dynobj))
	{
		...
		= index++ * MIPS_ELF_GOT_SIZE (output_bfd);
	      if (!(mips_elf_create_dynamic_relocation
		    (output_bfd, info, rel, NULL,
		     bfd_abs_section_ptr, 0, &addend, sgot)))
		return FALSE;
	      BFD_ASSERT (addend == 0);
	    }
	}
				      ...
				      bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  for (i = 1, hdrpp = elf_elfsections (abfd) + 1;
       i < elf_numsections (abfd); i++, hdrpp++)
    {
...
_bfd_mips_elf_additional_program_headers (bfd * abfd)
{
  ...
  if (IRIX_COMPAT (abfd) == ict_irix6
      && bfd_get_section_by_name (abfd, MIPS_ELF_OPTIONS_SECTION_NAME (abfd)))
    ++ret;
			    ...
			    struct bfd_link_info *info)
{
  ...
  if (o->size % PDR_SIZE != 0)
    return FALSE;
  if (o->output_section != NULL && bfd_is_abs_section (o->output_section))
    return FALSE;
				 ...
				 unsigned int *line_ptr)
{
  ...
  if (msec != NULL)
      ...
      if (fi == NULL)
	{
	  ...
	  for (; fraw_src < fraw_end;
	       fraw_src += external_fdr_size, fdr_ptr++)
...
bfd_byte *_bfd_elf_mips_get_relocated_section_contents
  (bfd * abfd,
   struct bfd_link_info * link_info,
   ...
   bfd_byte * data, bfd_boolean relocatable, asymbol ** symbols)
{
  ...
  if (reloc_count > 0)
	...
	if (abfd && input_bfd && abfd->xvec == input_bfd->xvec)
	  lh = 0;
	else
	  {
	    h =
	      bfd_hash_lookup (&link_info->hash->table, "_gp", FALSE, FALSE);
...
_bfd_mips_elf_final_link (bfd * abfd, struct bfd_link_info * info)
{
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
	...
	BFD_ASSERT ((elf_hash_table (info)->dynsymcount
		     - g->global_gotsym->dynindx) <= g->global_gotno);
    }
	  ...
	  for (p = o->link_order_head; p != NULL; p = p->next)
	      ...
	      if (p->type != bfd_indirect_link_order)
		{
	      ...
	      for (gpentry = sizeof (Elf32_External_gptab);
		   gpentry < size; gpentry += sizeof (Elf32_External_gptab))
		{
...
_bfd_mips_elf_merge_private_bfd_data (bfd * ibfd, bfd * obfd)
{
  ...
  if (mips_32bit_flags_p (old_flags) != mips_32bit_flags_p (new_flags))
    {
      (*_bfd_error_handler)
	(_("%B: linking 32-bit code with 64-bit code"), ibfd);
      ok = FALSE;
    }
      ...
      else
	{
	  ...
	  (*_bfd_error_handler)
	    (_("%B: linking %s module with previous %s modules"),
	     ibfd, bfd_printable_name (ibfd), bfd_printable_name (obfd));
	  ok = FALSE;
	}
    }
	      ...
	      != elf_elfheader (obfd)->e_ident[EI_CLASS]))
	{
	  (*_bfd_error_handler)
	    (_
	     ...
	     ibfd, elf_mips_abi_name (ibfd), elf_mips_abi_name (obfd));
	  ok = FALSE;
	}
