...
#define IS_PPI(PTR) ((bfd_get_16 (input_bfd, (PTR)) & 0xfc00) == 0xf800)
  ...
  if (input_section != symbol_section)
    x +=
      ((symbol_section->output_section->vma + symbol_section->output_offset) -
		      ...
		      Elf_Internal_Rela * dst)
{
  ...
  BFD_ASSERT (r < R_SH_FIRST_INVALID_RELOC_2
	      || r > R_SH_LAST_INVALID_RELOC_2);
  BFD_ASSERT (r < R_SH_FIRST_INVALID_RELOC_3
	      || r > R_SH_LAST_INVALID_RELOC_3);
  BFD_ASSERT (r < R_SH_FIRST_INVALID_RELOC_4
	      || r > R_SH_LAST_INVALID_RELOC_4);
  BFD_ASSERT (r < R_SH_FIRST_INVALID_RELOC_5
	      || r > R_SH_LAST_INVALID_RELOC_5);
...
movi_shori_putval (bfd * output_bfd, unsigned long value, bfd_byte * addr)
{
  bfd_put_32 (output_bfd,
	      ...
	      | ((value >> 6) & 0x3fffc00), addr);
  bfd_put_32 (output_bfd,
	      bfd_get_32 (output_bfd, addr + 4)
	      | ((value << 10) & 0x3fffc00), addr + 4);
}
			  ...
			  struct bfd_hash_table *table, const char *string)
{
  ...
  if (ret == (struct elf_sh_link_hash_entry *) NULL)
    ret = ((struct elf_sh_link_hash_entry *)
	   bfd_hash_allocate (table, sizeof (struct elf_sh_link_hash_entry)));
  if (ret == (struct elf_sh_link_hash_entry *) NULL)
  ...
  for (; rel < relend; rel++)
	  ...
	  if (h->root.type == bfd_link_hash_defined
	      || h->root.type == bfd_link_hash_defweak)
	    {
		  ...
		  || ((r_type == R_SH_GOT32
		      ...
		      && WILL_CALL_FINISH_DYNAMIC_SYMBOL (dyn, info->shared,
							  h) && (!info->shared
								 || (!info->
								     symbolic
								     && h->
	...
	case R_SH_DIR8WPL:
	  ...
	  if (input_section->output_section->vma +
	      input_section->output_offset != relocation)
	    {
	      int disp = (relocation
	      ...
	      if (disp & mask)
		{
		  ((*_bfd_error_handler)
		   (_
		    ...
		    input_section->owner, (unsigned long) rel->r_offset));
		  bfd_set_error (bfd_error_bad_value);
		  return FALSE;
		}
	      relocation -= 4;
	...
	case R_SH_IMM_HI16_PCREL:
	  ...
	  if (info->shared
	      ...
	      && r_symndx != 0
	      && (input_section->flags & SEC_ALLOC) != 0
	      && (r_type == R_SH_DIR32 || !SYMBOL_CALLS_LOCAL (info, h)))
	    {
	...
	case R_SH_GOT10BY8:
	  ...
	  if (h != NULL)
	      ...
	      if (rel->r_addend)
		{
		  ...
		  off = local_got_offsets[symtab_hdr->sh_info + r_symndx];
		}
	      else
		{
#endif
		     ...
		     Elf_Internal_Sym * sym)
{
  if (h != NULL)
	...
	default:
#ifdef INCLUDE_SHMEDIA
	  while (h->root.type == bfd_link_hash_indirect && h->root.u.i.link)
	    h = (struct elf_link_hash_entry *) h->root.u.i.link;
#endif
	  switch (h->root.type)
	    {
