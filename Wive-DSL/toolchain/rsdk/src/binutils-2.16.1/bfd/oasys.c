...
static bfd_boolean oasys_slurp_section_data PARAMS ((bfd * const));
static bfd_boolean oasys_read_record
PARAMS ((bfd *, oasys_record_union_type *));
static bfd_boolean oasys_write_sections PARAMS ((bfd *));
static bfd_boolean oasys_write_record
PARAMS ((bfd *, oasys_record_enum_type, oasys_record_union_type *, size_t));
static bfd_boolean oasys_write_syms PARAMS ((bfd *));
static bfd_boolean oasys_write_header PARAMS ((bfd *));
static bfd_boolean oasys_write_end PARAMS ((bfd *));
static bfd_boolean oasys_write_data PARAMS ((bfd *));
static size_t oasys_string_length PARAMS ((oasys_record_union_type *));
static bfd_boolean oasys_slurp_symbol_table PARAMS ((bfd * const));
static long int oasys_get_symtab_upper_bound PARAMS ((bfd * const));
static const bfd_target *oasys_archive_p PARAMS ((bfd *));
static bfd_boolean oasys_mkobject PARAMS ((bfd *));
...
static bfd_boolean oasys_new_section_hook PARAMS ((bfd *, asection *));
...
static bfd_boolean oasys_write_object_contents PARAMS ((bfd *));
...
static int oasys_sizeof_headers PARAMS ((bfd *, bfd_boolean));

long oasys_canonicalize_symtab PARAMS ((bfd *, asymbol **));
	...
	case oasys_record_is_symbol_enum:
	  {
	    int flag =
	      record.header.type ==
	      (int) oasys_record_is_local_enum ? (BSF_LOCAL) : (BSF_GLOBAL |
								BSF_EXPORT);
     ...
     bfd *abfd;
{
  ...
  if (header.version > 10000 ||
      ...
      header.sym_count > 100000 || header.xref_count > 100000)
	...
	if (0)
	  {
	    ...
	    filepos +=
	      sizeof (record_ext) +
	      record.dep_count * 4 +
	      record.depee_count * 4 + record.sect_count * 8 + 187;
	  }
	else
	  {
	    ...
	    filepos += (sizeof (record_ext)
			+ record.dep_count * 4 + record.module_name_size + 1);
	  }
...
static reloc_howto_type howto_table[] = {
  ...
  HOWTO (0, 0, 1, 16, FALSE, 0, complain_overflow_bitfield, 0, "abs16", TRUE,
	 0x0000ffff, 0x0000ffff, FALSE),
  HOWTO (0, 0, 2, 32, FALSE, 0, complain_overflow_bitfield, 0, "abs32", TRUE,
	 0xffffffff, 0xffffffff, FALSE),
  HOWTO (0, 0, 1, 16, TRUE, 0, complain_overflow_signed, 0, "pcrel16", TRUE,
	 0x0000ffff, 0x0000ffff, FALSE),
  HOWTO (0, 0, 2, 32, TRUE, 0, complain_overflow_signed, 0, "pcrel32", TRUE,
	 0xffffffff, 0xffffffff, FALSE)
};
	...
	case oasys_record_is_data_enum:
	  {
	    ...
	    bfd_byte *end_src =
	      ((bfd_byte *) & record) + record.header.length;
	    bfd_byte *dst_ptr;
		...
		if (mod_byte == 0 && gap >= 8)
		  ...
		  {
		    for (relbit = 1; count-- != 0 && src < end_src;
			 relbit <<= 1)
		      {
			if (relbit & mod_byte)
			  {
  ...
  for (count = 0; count < bfd_get_symcount (abfd); count++)
      ...
      else
	{
	  if (g->section == (asection *) NULL)
	    ...
	    {
	      symbol.relb =
		RELOCATION_TYPE_REL | g->section->output_section->
		target_index;
	    }
	  H_PUT_16 (abfd, 0, symbol.refno);
	}
     ...
     bfd *abfd;
{
  ...
  if (!oasys_write_record (abfd,
			   ...
			   (oasys_record_union_type *) & end, sizeof (end)))
    return FALSE;
...
/* Sort the reloc records so it's easy to insert the relocs into the
	   data */
	      ...
	      qsort (s->orelocation,
		     s->reloc_count, sizeof (arelent **), comp);
	    }
...
#define ADVANCE { if (++i >= 8) { i = 0; mod = dst++; *mod = 0; } current_byte_index++; }
			   ...
			   */
			  *dst++ = *raw_data++;
			  ADVANCE * dst++ = *raw_data++;
			  ADVANCE if (how->size == 2)
			    {
			      *dst++ = *raw_data++;
			      ADVANCE * dst++ = *raw_data++;
			    ADVANCE}
