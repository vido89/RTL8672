			    ...
			    bfd_reloc_code_real_type code)
{
  ...
  for (i = 0; i < sizeof (sh64_reloc_map) / sizeof (struct elf_reloc_map);
       i++)
    {
		  ...
		  || (info->shared
		      ...
		      && ((r_type == R_SH_64
			   && !(ELF_ST_VISIBILITY (h->other) == STV_INTERNAL
				...
				STV_HIDDEN)) || r_type == R_SH_64_PCREL)
...
sh_elf64_copy_private_data_internal (bfd * ibfd, bfd * obfd)
{
  ...
  for (osec = obfd->sections; osec; osec = osec->next)
    {
      int oIndex =
	((struct bfd_elf_section_data *) elf_section_data (osec))->this_idx;
  ...
  if (bfd_get_arch_size (ibfd) != bfd_get_arch_size (obfd))
    {
      ...
      if (bfd_get_arch_size (ibfd) == 32 && bfd_get_arch_size (obfd) == 64)
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_SH_GOTPLT10BY8:
	  ...
	  if (h == NULL
	      ...
	      || !info->shared
	      || info->symbolic
	      || h->dynindx == -1 || h->got.offset != (bfd_vma) - 1)
	    goto force_got;
	...
	case R_SH_64_PCREL:
	  ...
	  if (info->shared
		  ...
		  || (h != NULL && (!info->symbolic || !h->def_regular))))
	    {
...
movi_3shori_putval (bfd * output_bfd, bfd_vma value, bfd_byte * addr)
{
  bfd_put_32 (output_bfd,
	      bfd_get_32 (output_bfd, addr)
	      | ((value >> 38) & 0x3fffc00), addr);
  bfd_put_32 (output_bfd,
	      bfd_get_32 (output_bfd, addr + 4)
	      | ((value >> 22) & 0x3fffc00), addr + 4);
  bfd_put_32 (output_bfd,
	      bfd_get_32 (output_bfd, addr + 8)
	      | ((value >> 6) & 0x3fffc00), addr + 8);
  bfd_put_32 (output_bfd,
	      bfd_get_32 (output_bfd, addr + 12)
	      | ((value << 10) & 0x3fffc00), addr + 12);
}
