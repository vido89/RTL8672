     ...
     bfd *abfd;
{
  ...
  if (ret == NULL)
    return NULL;
  if (!ppc_coff_link_hash_table_init (ret, abfd, ppc_coff_link_hash_newfunc))
    {
      free (ret);
	...
	case IMAGE_REL_PPC_TOCREL16:
	  {
		...
		if ((r_flags & IMAGE_REL_PPC_TOCDEFN)
		    == IMAGE_REL_PPC_TOCDEFN)
		  {
			...
			(*_bfd_error_handler)
			  (_
			   ("%B: Relocation for %s of %lx exceeds Toc size limit"),
			   input_bfd, name, (unsigned long) our_toc_offset);
			bfd_set_error (bfd_error_bad_value);
			return FALSE;
		      }
	    ...
	    rstat =
	      _bfd_relocate_contents (howto, input_bfd, relocation, loc);
	  }
	  break;
	...
	case IMAGE_REL_PPC_ADDR32NB:
	  {
	    ...
	    if (strncmp (".idata$2", input_section->name, 8) == 0
	  ...
	  if (t->offset <= global_toc_size + thunk_size)
	  ...
	  else
	    {
	      fprintf (file,
		       ...
		       global_toc_size, global_toc_size, thunk_size,
		       thunk_size);
	      cat = _("Out of bounds!");
	    }
	}
...
static void ppc_coff_rtype2howto
