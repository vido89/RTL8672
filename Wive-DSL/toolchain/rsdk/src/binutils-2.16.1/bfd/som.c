...
static bfd_boolean som_mkobject PARAMS ((bfd *));
...
static bfd_boolean som_write_object_contents PARAMS ((bfd *));
static bfd_boolean som_slurp_string_table PARAMS ((bfd *));
...
static long som_canonicalize_symtab PARAMS ((bfd *, asymbol **));
static asymbol *som_make_empty_symbol PARAMS ((bfd *));
static void som_print_symbol
PARAMS ((bfd *, PTR, asymbol *, bfd_print_symbol_type));
static bfd_boolean som_new_section_hook PARAMS ((bfd *, asection *));
static bfd_boolean som_bfd_copy_private_symbol_data
PARAMS ((bfd *, asymbol *, bfd *, asymbol *));
static bfd_boolean som_bfd_copy_private_section_data
PARAMS ((bfd *, asection *, bfd *, asection *));
static bfd_boolean som_bfd_copy_private_bfd_data PARAMS ((bfd *, bfd *));
#define som_bfd_copy_private_header_data \
  _bfd_generic_bfd_copy_private_header_data
#define som_bfd_merge_private_bfd_data _bfd_generic_bfd_merge_private_bfd_data
#define som_bfd_set_private_flags _bfd_generic_bfd_set_private_flags
static bfd_boolean som_bfd_print_private_bfd_data (bfd *, void *);
static bfd_boolean som_bfd_is_local_label_name PARAMS ((bfd *, const char *));
...
static int exact_log2 PARAMS ((unsigned int));
...
static bfd_boolean som_prep_headers PARAMS ((bfd *));
static int som_sizeof_headers PARAMS ((bfd *, bfd_boolean));
static bfd_boolean som_finish_writing PARAMS ((bfd *));
static bfd_boolean som_build_and_write_symbol_table PARAMS ((bfd *));
...
PARAMS ((bfd *, unsigned long, asymbol **, unsigned int, unsigned *,
	 COMPUNIT *));
static bfd_boolean som_begin_writing PARAMS ((bfd *));
...
static bfd_boolean som_slurp_armap PARAMS ((bfd *));
...
PARAMS ((bfd *, unsigned int, unsigned int, struct lst_header, unsigned int));
static bfd_boolean som_is_space PARAMS ((asection *));
static bfd_boolean som_is_subspace PARAMS ((asection *));
static bfd_boolean som_is_container PARAMS ((asection *, asection *));
static bfd_boolean som_bfd_free_cached_info PARAMS ((bfd *));
static bfd_boolean som_bfd_link_split_section PARAMS ((bfd *, asection *));
     ...
     struct reloc_queue *queue;
{
  ...
  if ((skip & 3) == 0 && skip <= 0xc0000 && skip > 0)
	...
	{
	  bfd_put_8 (abfd, R_NO_RELOCATION + 24 + (((skip >> 2) - 1) >> 8),
		     p);
	...
	{
	  bfd_put_8 (abfd, R_NO_RELOCATION + 28 + (((skip >> 2) - 1) >> 16),
		     p);
  ...
  if (aux_hdrp)
      ...
      else
	{
	  bfd_get_start_address (abfd) =
	    aux_hdrp->exec_entry + current_offset;
  ...
  for (space_index = 0; space_index < file_hdr->space_total; space_index++)
	   ...
	   subspace_index++)
	{
	  ...
	  if (subspace.file_loc_init_value == 0
	    ...
	    subspace_asect->flags &=
	      ~(SEC_DATA | SEC_LOAD | SEC_HAS_CONTENTS);
	  ...
	  if (subspace.file_loc_init_value >
	      save_subspace.file_loc_init_value)
     ...
     asection *space, *subspace;
{
  return (som_section_data (subspace)->copy_data->container == space
	  || (som_section_data (subspace)->copy_data->container->
	      output_section == space));
}
  ...
  for (i = 0; i < num_spaces; i++)
	...
	section = section->next;
      ...
      for (subsection = abfd->sections;
	   subsection != NULL; subsection = subsection->next)
	{
	  int reloc_offset;
     ...
     bfd *abfd;
{
  ...
  if (obj_som_version_hdr (abfd) != NULL)
	...
	return FALSE;
      ...
      current_offset += len;
      if (bfd_bwrite
	  ((PTR) obj_som_version_hdr (abfd)->user_string, len, abfd) != len)
	return FALSE;
    }
  ...
  if (obj_som_copyright_hdr (abfd) != NULL)
	...
	return FALSE;
      ...
      current_offset += len;
      if (bfd_bwrite
	  ((PTR) obj_som_copyright_hdr (abfd)->copyright, len, abfd) != len)
	return FALSE;
    }
	   ...
	   subsection != NULL; subsection = subsection->next)
	{
	  ...
	  if (subsection->flags & SEC_LOAD)
		...
		exec_header->exec_dsize += subsection->size;
	      som_section_data (subsection)->subspace_dict->
		file_loc_init_value = current_offset;
     ...
     bfd *abfd;
{
  ...
  if (obj_som_exec_data (abfd) && obj_som_exec_data (abfd)->version_id)
    obj_som_file_hdr (abfd)->version_id =
  ...
  som_prep_for_fixups (abfd,
		       bfd_get_outsymbols (abfd), bfd_get_symcount (abfd));
  ...
  if (obj_som_compilation_unit (abfd))
	...
	return FALSE;
      ...
      amt = COMPUNITSZ;
      if (bfd_bwrite ((PTR) obj_som_compilation_unit (abfd), amt, abfd) !=
	  amt)
	return FALSE;
    }
     ...
     struct som_misc_symbol_info *info;
{
  ...
  if (som_section_data (sym->section)
      ...
      && info->symbol_scope == SS_UNIVERSAL
      && (info->symbol_type == ST_ENTRY
	  || info->symbol_type == ST_CODE || info->symbol_type == ST_DATA))
    {
      info->is_comdat
  ...
  for (bufp = buf, sym = symbase; bufp < endbufp; ++bufp)
    {
      ...
      if (bufp->symbol_type == ST_SYM_EXT || bufp->symbol_type == ST_ARG_EXT)
	continue;
		  ...
		  if (rptr->addend == 0 && !section->contents)
		    {
			  ...
			  (section->owner, section, &contents))
			{
			  if (contents != NULL)
			    free (contents);
			  return (unsigned) -1;
     ...
     int spnum;
{
  ...
  if (som_section_data (section)->copy_data == NULL)
	...
	(struct som_copyable_section_data_struct *) bfd_zalloc (section->
								owner, amt);
      if (som_section_data (section)->copy_data == NULL)
	return FALSE;
    }
     ...
     bfd_size_type count;
{
  ...
  if ((bfd_size_type) (offset + count) > section->size
      || bfd_seek (abfd, (file_ptr) (section->filepos + offset),
		   SEEK_SET) != 0
  ...
  while (curr_bfd != NULL)
      ...
      for (i = 0; i < curr_count; i++, sym++)
	{
	  ...
	  if (hash_table[curr_lst_sym->symbol_key % lst.hash_size])
		...
		=
		(curr_lst_sym -
		 lst_syms) * sizeof (struct lst_symbol_record) +
		lst.hash_size * 4 +
...
const bfd_target som_vec = {
  ...
  (SEC_CODE | SEC_DATA | SEC_ROM | SEC_HAS_CONTENTS | SEC_LINK_ONCE | SEC_ALLOC | SEC_LOAD | SEC_RELOC),	/* section flags */
