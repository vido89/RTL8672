...
PARAMS ((bfd *, arelent *, struct reloc_std_external *));
  ...
  PARAMS ((bfd *, sec_ptr, asymbol **));
     bfd_boolean NAME (lynx, squirt_out_relocs) PARAMS ((bfd *, asection *));
...
/* Output standard relocation information to a file in target byte order. */
     ...
     void NAME (lynx, swap_std_reloc_out) (abfd, g, natptr)
     bfd *abfd;
     arelent *g;
...
void NAME (lynx, swap_ext_reloc_out) (abfd, g, natptr)
     bfd *
       abfd;
     arelent *
       g;
...
bfd_boolean NAME (lynx, slurp_reloc_table) (abfd, asect, symbols)
     bfd *
       abfd;
     sec_ptr
       asect;
...
bfd_boolean NAME (lynx, squirt_out_relocs) (abfd, section)
     bfd *
       abfd;
     asection *
       section;
{
  ...
  if (each_size == RELOC_EXT_SIZE)
    {
      for (natptr = native;
	   count != 0; --count, natptr += each_size, ++generic)
	NAME (lynx, swap_ext_reloc_out) (abfd, *generic,
  ...
  else
    {
      for (natptr = native;
	   count != 0; --count, natptr += each_size, ++generic)
	NAME (lynx, swap_std_reloc_out) (abfd, *generic,
...
long NAME (lynx, canonicalize_reloc) (abfd, section, relptr, symbols)
     bfd *
       abfd;
     sec_ptr
       section;
