...
static reloc_howto_type *sh_coff_reloc_type_lookup
...
static reloc_howto_type *coff_sh_rtype_to_howto
...
#ifdef COFF_WITH_PE
      && r_type != R_SH_IMM32CE && r_type != R_SH_IMAGEBASE
#endif
      && (r_type != R_SH_PCDISP || (symbol_in->flags & BSF_LOCAL) != 0))
    return bfd_reloc_ok;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
      ...
      else
	{
      ...
      foff = (symval
	      - (irel->r_vaddr
		 - sec->vma
		 + sec->output_section->vma + sec->output_offset + 4));
	...
	if (irelcount->r_vaddr == paddr && irelcount->r_type == R_SH_COUNT)
	  break;
  ...
  for (; irel < irelend; irel++)
    {
	  ...
	  && irel->r_vaddr - sec->vma > addr && count < (1 << irel->r_offset))
	{
	  irelalign = irel;
...
#define NOP_OPCODE (0x0009)
  ...
  for (irel = coff_section_data (abfd, sec)->relocs; irel < irelend; irel++)
	...
	nraddr -= count;
	  ...
	  && irel->r_type != R_SH_DATA && irel->r_type != R_SH_LABEL)
	...
	case R_SH_SWITCH32:
	  ...
	  if (start > addr
	      && start < toaddr && (stop <= addr || stop >= toaddr))
	    irel->r_offset += count;
	  else if (stop > addr
		   && stop < toaddr && (start <= addr || start >= toaddr))
	    irel->r_offset -= count;
  ...
  if (obj_symbols (abfd) != NULL || obj_raw_syments (abfd) != NULL)
    {
      ((*_bfd_error_handler)
  ...
  while (esym < esymend)
    {
	  ...
	  && (bfd_vma) isym.n_value > addr && (bfd_vma) isym.n_value < toaddr)
	{
	  isym.n_value -= count;
     ...
     unsigned int reg;
{
  ...
  if ((f & USES1) != 0 && USES1_REG (insn) == reg)
    return TRUE;
  if ((f & USES2) != 0 && USES2_REG (insn) == reg)
    return TRUE;
  if ((f & USESR0) != 0 && reg == 0)
    return TRUE;
     ...
     unsigned int reg;
{
  ...
  if ((f & SETS1) != 0 && SETS1_REG (insn) == reg)
    return TRUE;
  if ((f & SETS2) != 0 && SETS2_REG (insn) == reg)
    return TRUE;
  if ((f & SETSR0) != 0 && reg == 0)
    return TRUE;
     ...
     unsigned int freg;
{
  ...
  if ((f & USESF1) != 0 && (USESF1_REG (insn) & 0xe) == (freg & 0xe))
    return TRUE;
  if ((f & USESF2) != 0 && (USESF2_REG (insn) & 0xe) == (freg & 0xe))
    return TRUE;
  if ((f & USESF0) != 0 && freg == 0)
    return TRUE;
     ...
     unsigned int freg;
{
  ...
  if ((f & SETSF1) != 0 && (SETSF1_REG (insn) & 0xe) == (freg & 0xe))
    return TRUE;
     ...
     const struct sh_opcode *op2;
{
  ...
  if ((f1 & (BRANCH | DELAY)) != 0 || (f2 & (BRANCH | DELAY)) != 0)
    return TRUE;

  if (((f1 | f2) & SETSSP)
      && (f1 & (SETSSP | USESSP)) && (f2 & (SETSSP | USESSP)))
    return TRUE;

  if ((f1 & SETS1) != 0 && sh_insn_uses_or_sets_reg (i2, op2, SETS1_REG (i1)))
    return TRUE;
  if ((f1 & SETS2) != 0 && sh_insn_uses_or_sets_reg (i2, op2, SETS2_REG (i1)))
    return TRUE;
  if ((f1 & SETSR0) != 0 && sh_insn_uses_or_sets_reg (i2, op2, 0))
    return TRUE;
  if ((f1 & SETSAS) && sh_insn_uses_or_sets_reg (i2, op2, SETSAS_REG (i1)))
    return TRUE;
  if ((f1 & SETSF1) != 0
      && sh_insn_uses_or_sets_freg (i2, op2, SETSF1_REG (i1)))
    return TRUE;

  if ((f2 & SETS1) != 0 && sh_insn_uses_or_sets_reg (i1, op1, SETS1_REG (i2)))
    return TRUE;
  if ((f2 & SETS2) != 0 && sh_insn_uses_or_sets_reg (i1, op1, SETS2_REG (i2)))
    return TRUE;
  if ((f2 & SETSR0) != 0 && sh_insn_uses_or_sets_reg (i1, op1, 0))
    return TRUE;
  if ((f2 & SETSAS) && sh_insn_uses_or_sets_reg (i1, op1, SETSAS_REG (i2)))
    return TRUE;
  if ((f2 & SETSF1) != 0
     ...
     const struct sh_opcode *op2;
{
  ...
  /* If both SETS1 and SETSSP are set, that means a load to a special
     ...
     about here.  */
  if ((f1 & SETS1) != 0
      && (f1 & SETSSP) == 0 && sh_insn_uses_reg (i2, op2, (i1 & 0x0f00) >> 8))
    return TRUE;

  if ((f1 & SETSR0) != 0 && sh_insn_uses_reg (i2, op2, 0))
    return TRUE;

  if ((f1 & SETSF1) != 0 && sh_insn_uses_freg (i2, op2, (i1 & 0x0f00) >> 8))
    return TRUE;
  ...
  for (; i < stop; i += 4)
    {
      ...
      if (op == NULL || (op->flags & (LOAD | STORE)) == 0)
	continue;
      ...
      if (i > start)
	{
	  ...
	  /* If the load/store instruction is in a delay slot, we
	     can't swap.  */
	  if (prev_op == NULL || (prev_op->flags & DELAY) != 0)
	    continue;
	}
      if (i > start
	  ...
	  if (i >= start + 4)
	    {
	      ...
	      /* If the instruction before PREV_INSN has a delay
	         ...
	         can not swap.  */
	      if (prev2_op == NULL || (prev2_op->flags & DELAY) != 0)
		ok = FALSE;
	...
	++ * plabel;
      ...
      if (i + 2 < stop && (*plabel >= label_end || **plabel != i + 2))
	{
	  ...
	  if (next_op != NULL
		...
		ok = FALSE;
	      ...
	      /* If INSN is a load, and it sets a register which
	         ...
	         it isn't.  */
	      if (ok && i + 4 < stop && (op->flags & LOAD) != 0)
		{
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      type = irel->r_type;
      if (type == R_SH_ALIGN
	  || type == R_SH_CODE || type == R_SH_DATA || type == R_SH_LABEL)
	continue;
...
#ifdef COFF_WITH_PE
	  && rel->r_type != R_SH_IMM32CE && rel->r_type != R_SH_IMAGEBASE
#endif
	  ...
	  if (symndx == -1)
	  ...
	  else
	    {
	      ...
	      val = (sec->output_section->vma
		     + sec->output_offset + sym->n_value - sec->vma);
	    }
	}
      else
	{
...
CREATE_BIG_COFF_TARGET_VEC (shcoff_vec, "coff-sh", BFD_IS_RELAXABLE, 0, '_',
...
#define TARGET_SHL_NAME "coff-shl"
  ...
  CREATE_LITTLE_COFF_TARGET_VEC (TARGET_SYM, TARGET_SHL_NAME, BFD_IS_RELAXABLE,
     ...
     static bfd_boolean coff_small_new_section_hook
       PARAMS ((bfd *, asection *));
...
/* Only recognize the small versions if the target was not defaulted.
   Otherwise we won't recognize the non default endianness.  */
     ...
     static const bfd_target *coff_small_object_p (abfd)
     bfd *abfd;
{
