  ...
  if (entry != NULL)
    {
      ...
      struct sec_merge_hash_entry *ret =
			...
			void **psecinfo)
{
  ...
  if ((abfd->flags & DYNAMIC) != 0 || (sec->flags & SEC_MERGE) == 0)
    abort ();

  if (sec->size == 0 || (sec->flags & SEC_EXCLUDE) != 0 || sec->entsize == 0)
    return TRUE;
...
_bfd_merged_section_offset (bfd * output_bfd ATTRIBUTE_UNUSED,
			    asection ** psec, void *psecinfo, bfd_vma offset)
{
  struct sec_merge_sec_info *secinfo;
  struct sec_merge_hash_entry *entry;
  unsigned char *p;
