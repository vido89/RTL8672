		      ...
		      char *current_name, char *new_name)
{
  ...
  new_hash =
    elf_link_hash_lookup (elf_hash_table (info), node->new_name, TRUE, FALSE,
			  TRUE);
	  ...
	  if (strncmp (s, DIRECTIVE_IMPORT, strlen (DIRECTIVE_IMPORT)))
	      ...
	      else
		{
		  ...
		  *current_name_end = 0;
		  ...
		  result =
		    sh_symbian_import_as (info, abfd, current_name, new_name);
...
bfd_boolean bfd_elf32_sh_symbian_process_directives (struct bfd_link_info *
						     info, bfd * abfd);

bfd_boolean
bfd_elf32_sh_symbian_process_directives (struct bfd_link_info *info,
					 bfd * abfd)
{
  ...
  if (!contents)
	...
	result =
	  sh_symbian_process_embedded_commands (info, abfd, sec, contents);
      free (contents);
    }
			     ...
			     asection ** local_sections)
{
  ...
  if (!info->relocatable)
      ...
      for (ptr = rename_list; ptr; ptr = ptr->next)
	{
	  ...
	  ptr->current_hash =
	    elf_link_hash_lookup (hash_table, ptr->current_name, FALSE, FALSE,
				  TRUE);
	  ...
	  new_hash =
	    elf_link_hash_lookup (hash_table, ptr->new_name, FALSE, FALSE,
				  TRUE);
	  ...
	  if (h == sym_hashes_end)
	    {
	      ...
	      ++num_global_syms;
	      new_sym_hashes =
		bfd_alloc (input_bfd, num_global_syms * sizeof *sym_hashes);
	      if (new_sym_hashes == NULL)
		{
	      ...
	      memcpy (new_sym_hashes, sym_hashes,
		      (num_global_syms - 1) * sizeof *sym_hashes);
	      new_sym_hashes[num_global_syms - 1] = new_hash;
	      elf_sym_hashes (input_bfd) = sym_hashes = new_sym_hashes;
	      sym_hashes_end = sym_hashes + num_global_syms;
	      symtab_hdr->sh_size =
		(num_global_syms +
		 num_local_syms) * sizeof (Elf32_External_Sym);
