...
bfd_boolean
_bfd_link_section_stabs (abfd, sinfo, stabsec, stabstrsec, psecinfo,
			 pstring_offset)
     bfd *abfd;
     ...
     bfd_size_type *pstring_offset;
{
  ...
  if (stabsec->size == 0 || stabstrsec->size == 0)
    {
  ...
  if (sinfo->stabstr == NULL)
	...
	goto error_return;
      ...
      if (!bfd_hash_table_init_n (&sinfo->includes,
				  stab_link_includes_newfunc, 251))
	goto error_return;
  ...
  for (sym = stabbuf, pstridx = secinfo->stridxs;
      ...
      if (type == (int) N_BINCL)
	{
	  ...
	  for (incl_sym = sym + STABSIZE;
	       incl_sym < symend; incl_sym += STABSIZE)
	    {
	      int incl_type;
	      ...
	      else if (nest == 0)
		{
		  ...
		  str = ((char *) stabstrbuf
			 + stroff + bfd_get_32 (abfd, incl_sym + STRDXOFF));
		  for (; *str != '\0'; str++)
		    {
	  ...
	  if (t == NULL)
	  ...
	  else
	    {
	      ...
	      for (incl_sym = sym + STABSIZE, incl_pstridx = pstridx + 1;
		   incl_sym < symend; incl_sym += STABSIZE, ++incl_pstridx)
		{
		  int incl_type;
     ...
     struct stab_info *sinfo;
{
  ...
  if (bfd_seek (output_bfd,
		(file_ptr) (sinfo->stabstr->output_section->filepos
			    + sinfo->stabstr->output_offset), SEEK_SET) != 0)
    return FALSE;
