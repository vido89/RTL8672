...
PARAMS ((bfd *, struct bfd_link_info *));
static bfd_boolean elf_s390_mkobject PARAMS ((bfd *));
...
static reloc_howto_type elf_howto_table[] = {
  ...
  HOWTO (R_390_GLOB_DAT, 0, 2, 32, FALSE, 0, complain_overflow_bitfield,
	 bfd_elf_generic_reloc, "R_390_GLOB_DAT", FALSE, 0, 0xffffffff,
	 FALSE),
  HOWTO (R_390_JMP_SLOT, 0, 2, 32, FALSE, 0, complain_overflow_bitfield,
	 bfd_elf_generic_reloc, "R_390_JMP_SLOT", FALSE, 0, 0xffffffff,
	 FALSE),
  HOWTO (R_390_RELATIVE, 0, 2, 32, TRUE, 0, complain_overflow_bitfield,
	 bfd_elf_generic_reloc, "R_390_RELATIVE", FALSE, 0, 0xffffffff,
	 FALSE),
  HOWTO (R_390_GOTOFF32, 0, 2, 32, FALSE, 0, complain_overflow_bitfield,
	 bfd_elf_generic_reloc, "R_390_GOTOFF32", FALSE, 0, 0xffffffff,
	 FALSE),
  ...
  HOWTO (R_390_TLS_LDM32, 0, 2, 32, FALSE, 0, complain_overflow_bitfield,
	 bfd_elf_generic_reloc, "R_390_TLS_LDM32", FALSE, 0, 0xffffffff,
	 FALSE),
  ...
  HOWTO (R_390_TLS_DTPMOD, 0, 2, 32, FALSE, 0, complain_overflow_bitfield,
	 bfd_elf_generic_reloc, "R_390_TLS_DTPMOD", FALSE, 0, 0xffffffff,
	 FALSE),
  HOWTO (R_390_TLS_DTPOFF, 0, 2, 32, FALSE, 0, complain_overflow_bitfield,
	 bfd_elf_generic_reloc, "R_390_TLS_DTPOFF", FALSE, 0, 0xffffffff,
	 FALSE),
  HOWTO (R_390_TLS_TPOFF, 0, 2, 32, FALSE, 0, complain_overflow_bitfield,
	 bfd_elf_generic_reloc, "R_390_TLS_TPOFF", FALSE, 0, 0xffffffff,
	 FALSE),
  ...
  HOWTO (R_390_TLS_GOTIE20, 0, 2, 20, FALSE, 8, complain_overflow_dont,
	 s390_elf_ldisp_reloc, "R_390_TLS_GOTIE20", FALSE, 0, 0x0fffff00,
	 FALSE),
};

/* GNU extension to record C++ vtable hierarchy.  */
static reloc_howto_type elf32_s390_vtinherit_howto =
HOWTO (R_390_GNU_VTINHERIT, 0, 2, 0, FALSE, 0, complain_overflow_dont, NULL,
       "R_390_GNU_VTINHERIT", FALSE, 0, 0, FALSE);
static reloc_howto_type elf32_s390_vtentry_howto =
HOWTO (R_390_GNU_VTENTRY, 0, 2, 0, FALSE, 0, complain_overflow_dont,
       _bfd_elf_rel_vtable_reloc_fn, "R_390_GNU_VTENTRY", FALSE, 0, 0, FALSE);
     ...
     PTR inf;
{
  ...
  if (h->got.refcount > 0
      && !info->shared
      && h->dynindx == -1 && elf_s390_hash_entry (h)->tls_type >= GOT_TLS_IE)
    {
     ...
     struct bfd_link_info *info;
{
  ...
  for (s = dynobj->sections; s != NULL; s = s->next)
	...
	continue;
      ...
      if (s == htab->splt || s == htab->sgot || s == htab->sgotplt)
	{
     ...
     asection **local_sections;
{
  ...
  for (; rel < relend; rel++)
	...
	case R_390_PLTOFF32:
	  ...
	  if (h == NULL
	      || h->plt.offset == (bfd_vma) - 1 || htab->splt == NULL)
	    {
	      relocation -= htab->sgot->output_section->vma;
	      break;
	    }
	  ...
	  relocation = (htab->splt->output_section->vma
			...
			+ h->plt.offset - htab->sgot->output_section->vma);
	  unresolved_reloc = FALSE;
	  break;
	  ...
	  || r_type == R_390_GOTPLT20 || r_type == R_390_TLS_GOTIE20)
	{
	  ...
	  relocation =
	    (relocation & 0xfff) << 8 | (relocation & 0xff000) >> 12;
	  r =
  ...
  if (h->plt.offset != (bfd_vma) - 1)
    {
      ...
      if (h->dynindx == -1
	  || htab->splt == NULL
	  || htab->sgotplt == NULL || htab->srelplt == NULL)
	abort ();
	...
	{
	  bfd_put_32 (output_bfd,
		      (bfd_vma) PLT_PIC12_ENTRY_WORD0 + got_offset,
		      htab->splt->contents + h->plt.offset);
	...
	{
	  bfd_put_32 (output_bfd,
		      (bfd_vma) PLT_PIC16_ENTRY_WORD0 + got_offset,
		      htab->splt->contents + h->plt.offset);
      ...
      else
	{
      ...
      bfd_put_32 (output_bfd,
		  (htab->splt->output_section->vma
		   + htab->splt->output_offset
		   + h->plt.offset
		   + 12), htab->sgotplt->contents + got_offset);
      ...
      rela.r_addend = 0;
      loc =
	htab->srelplt->contents + plt_index * sizeof (Elf32_External_Rela);
