...
bfd_elf_string_from_elf_section (bfd * abfd,
				 unsigned int shindex, unsigned int strindex)
{
...
bfd_elf_get_elf_syms (bfd * ibfd,
		      ...
		      void *extsym_buf, Elf_External_Sym_Shndx * extshndx_buf)
{
  ...
  if (shndx_hdr == NULL || shndx_hdr->sh_size == 0)
  ...
  else
    {
      ...
      pos =
	shndx_hdr->sh_offset + symoffset * sizeof (Elf_External_Sym_Shndx);
...
bfd_elf_sym_name (bfd * abfd,
		  Elf_Internal_Shdr * symtab_hdr, Elf_Internal_Sym * isym)
{
				 ...
				 Elf_Internal_Shdr * hdr, const char *name)
{
     ...
     any sort of flag.  */
  {
    ...
    for (i = ARRAY_SIZE (debug_sec_names); i--;)
      if (strncmp (name, debug_sec_names[i], strlen (debug_sec_names[i])) ==
	  0)
	break;
...
static void
merge_sections_remove_hook (bfd * abfd ATTRIBUTE_UNUSED, asection * sec)
{
  ...
  if (p != NULL)
      ...
      for (i = 0; i < c; i++, p++)
	{
	  ...
	  if ((p->p_flags & ~(unsigned) (PF_R | PF_W | PF_X)) != 0)
	    fprintf (f, " %lx",
		     p->p_flags & ~(unsigned) (PF_R | PF_W | PF_X));
	  fprintf (f, "\n");
	}
    }
...
bfd_section_from_r_symndx (bfd * abfd,
			   ...
			   asection * sec, unsigned long r_symndx)
{
  Elf_Internal_Shdr *symtab_hdr;
  unsigned char esym[sizeof (Elf64_External_Sym)];
  Elf_External_Sym_Shndx eshndx;
				 ...
				 int index, const char *typename)
{
  ...
  split = ((hdr->p_memsz > 0)
	   && (hdr->p_filesz > 0) && (hdr->p_memsz > hdr->p_filesz));
...
_bfd_elf_init_reloc_shdr (bfd * abfd,
			  ...
			  asection * asect, bfd_boolean use_rela_p)
{
  char *name;
  const struct elf_backend_data *bed = get_elf_backend_data (abfd);
  ...
  rel_hdr->sh_name =
    (unsigned int) _bfd_elf_strtab_add (elf_shstrtab (abfd), name, FALSE);
  if (rel_hdr->sh_name == (unsigned int) -1)
    return FALSE;
  rel_hdr->sh_type = use_rela_p ? SHT_RELA : SHT_REL;
  rel_hdr->sh_entsize = (use_rela_p
			 ? bed->s->sizeof_rela : bed->s->sizeof_rel);
...
bfd_elf_set_group_contents (bfd * abfd, asection * sec, void *failedptrarg)
{
  ...
  if (elf_section_data (sec)->this_hdr.sh_type != SHT_GROUP || *failedptr)
    return;
		      ...
		      if (elfsec == 0)
			{
			  ...
			  if (bed->link_order_error_handler)
			    bed->link_order_error_handler
			      (_
			       ("%B: warning: sh_link not set for section `%S'"),
			       abfd, s);
			}
		      else
			{
...
_bfd_elf_assign_file_position_for_section (Elf_Internal_Shdr * i_shdrp,
					   file_ptr offset, bfd_boolean align)
{
  if (align)
    {
      unsigned int al;
...
make_mapping (bfd * abfd,
	      asection ** sections,
	      unsigned int from, unsigned int to, bfd_boolean phdr)
{
...
map_sections_to_segments (bfd * abfd)
{
      ...
      else if ((abfd->flags & D_PAGED) == 0)
	{
	  ...
	  new_segment = FALSE;
	       ...
	       && (((last_hdr->lma + last_size - 1)
		    & ~(maxpagesize - 1)) != (hdr->lma & ~(maxpagesize - 1))))
	{
  ...
  for (s = abfd->sections; s != NULL; s = s->next)
    {
      if ((s->flags & SEC_LOAD) != 0 && strncmp (s->name, ".note", 5) == 0)
	{
	  amt = sizeof (struct elf_segment_map);
	  m = bfd_zalloc (abfd, amt);
	  if (m == NULL)
				    ...
				    struct bfd_link_info *link_info)
{
  ...
  for (m = elf_tdata (abfd)->segment_map, p = phdrs;
      ...
      if (p->p_type == PT_LOAD && m->count > 0)
	{
	  ...
	  if (adjust != 0
	      && !m->includes_filehdr
	      && !m->includes_phdrs && (ufile_ptr) off >= align)
	    {
	...
	p->p_paddr = m->sections[0]->lma;
      ...
      if (p->p_type == PT_LOAD && (abfd->flags & D_PAGED) != 0)
      ...
      for (i = 0, secpp = m->sections; i < m->count; i++, secpp++)
	{
	  ...
	  if (p->p_type == PT_LOAD || p->p_type == PT_TLS)
	    {
	      ...
	      if ((flags & SEC_LOAD) != 0)
		{
	      ...
	      /* .tbss is special.  It doesn't contribute to p_memsz of
	         normal segments.  */
	      else if ((flags & SEC_THREAD_LOCAL) == 0 || p->p_type == PT_TLS)
		{
	  ...
	  if (p->p_type == PT_NOTE && bfd_get_format (abfd) == bfd_core)
		...
		p->p_memsz += sec->size;
	      ...
	      if (p->p_type == PT_TLS
		  && sec->size == 0 && (sec->flags & SEC_HAS_CONTENTS) == 0)
		{
		  struct bfd_link_order *o;
		  bfd_vma tbss_size = 0;
      ...
      if (m->count == 0)
	{
	  if (m->includes_filehdr)
	      ...
	      for (lp = phdrs; lp < phdrs + count; ++lp)
		{
	      ...
	      if (lp < phdrs + count && link_info->relro_end > lp->p_vaddr)
		{
				     ...
				     struct bfd_link_info *link_info)
{
  ...
  if ((abfd->flags & (EXEC_P | DYNAMIC)) == 0
      ...
      for (i = 1, hdrpp = i_shdrpp + 1; i < num_sec; i++, hdrpp++)
	{
	  ...
	  hdr = *hdrpp;
	  if (hdr->bfd_section != NULL && hdr->bfd_section->filepos != 0)
...
_bfd_elf_symbol_from_bfd_symbol (bfd * abfd, asymbol ** asym_ptr_ptr)
{
  ...
  if (asym_ptr->udata.i == 0
      && (flags & BSF_SECTION_SYM) && asym_ptr->section)
    {
      int indx;
		  ...
		  if (map->count == 0)
		    {
			  ...
			  (map->p_paddr
			   ...
			   + (map->includes_phdrs
			      ? iehdr->e_phnum * iehdr->e_phentsize : 0)))
			abort ();
		    }
		  else
		    {
	  ...
	  if (isec < section_count)
	    {
	      ...
	      amt = sizeof (struct elf_segment_map);
	      amt +=
		((bfd_size_type) section_count - 1) * sizeof (asection *);
	      map = bfd_alloc (obfd, amt);
	      if (map == NULL)
		{
		  free (sections);
				    ...
				    bfd * obfd, asection * osec)
{
  ...
  if (ihdr->sh_type == SHT_SYMTAB
      ...
      || ihdr->sh_type == SHT_GNU_verneed || ihdr->sh_type == SHT_GNU_verdef)
				   ...
				   bfd * obfd, asymbol * osymarg)
{
  ...
  if (isym != NULL
      && osym != NULL && bfd_is_abs_section (isym->symbol.section))
    {
      unsigned int shndx;
  ...
  for (idx = 0; idx < symcount; idx++)
      ...
      else
	{
	  ...
	  if (bfd_is_abs_section (sec)
	      && type_ptr != NULL && type_ptr->internal_elf_sym.st_shndx != 0)
	    {
...
Unable to find equivalent output section for symbol '%s' from section '%s'"), syms[idx]->name ? syms[idx]->name : "<Local sym>", sec->name);
...
long
_bfd_elf_get_reloc_upper_bound (bfd * abfd ATTRIBUTE_UNUSED, sec_ptr asect)
{
...
long
_bfd_elf_canonicalize_dynamic_symtab (bfd * abfd, asymbol ** allocation)
{
  const struct elf_backend_data *bed = get_elf_backend_data (abfd);
			       ...
			       bfd_boolean default_imported_symver)
{
  ...
  if (elf_dynverdef (abfd) != 0)
      ...
      for (i = 0; i < hdr->sh_info; i++)
	{
	  ...
	  amt =
	    (bfd_size_type) iverdef->vd_cnt * sizeof (Elf_Internal_Verdaux);
...
bfd_boolean
_bfd_elf_is_local_label_name (bfd * abfd ATTRIBUTE_UNUSED, const char *name)
{
		   ...
		   const char **filename_ptr, const char **functionname_ptr)
{
  ...
  enum
  { nothing_seen, symbol_seen, file_after_symbol_seen } state;
...
fail:
  (*_bfd_error_handler)
    (_("%B: unsupported relocation type %s"), abfd, areloc->howto->name);
  bfd_set_error (bfd_error_bad_value);
  return FALSE;
}
...
_bfd_elfcore_make_pseudosection (bfd * abfd,
				 char *name, size_t size, ufile_ptr filepos)
{
  char buf[100];
  char *threaded_name;
  size_t len;
...
elfcore_make_note_pseudosection (bfd * abfd,
				 char *name, Elf_Internal_Note * note)
{
...
elfcore_grok_nto_regs (bfd * abfd,
		       Elf_Internal_Note * note, pid_t tid, char *base)
{
  char buf[100];
  char *name;
  asection *sect;
...
elfcore_write_note (bfd * abfd,
		    char *buf,
		    int *bufsiz,
		    const char *name, int type, const void *input, int size)
{
...
elfcore_write_prpsinfo (bfd * abfd,
			char *buf,
			int *bufsiz, const char *fname, const char *psargs)
{
  int note_type;
  char *note_name = "CORE";
...
elfcore_write_prstatus (bfd * abfd,
			char *buf,
			int *bufsiz, long pid, int cursig, const void *gregs)
{
  prstatus_t prstat;
  char *note_name = "CORE";
...
elfcore_write_prfpreg (bfd * abfd,
		       char *buf, int *bufsiz, const void *fpregs, int size)
{
  char *note_name = "CORE";
  return elfcore_write_note (abfd, buf, bufsiz,
...
elfcore_write_prxfpreg (bfd * abfd,
			char *buf, int *bufsiz, const void *xfpregs, int size)
{
...
bfd *bfd_elf_bfd_from_remote_memory
  (bfd * templ,
   bfd_vma ehdr_vma,
   bfd_vma * loadbasep, int (*target_read_memory) (bfd_vma, bfd_byte *, int))
{
...
bfd_elf_match_symbols_in_sections (asection * sec1, asection * sec2)
{
  ...
  /* Sort symbols by binding and section. Global definitions are at
     the beginning.  */
  qsort (isymbuf1, symcount1, sizeof (Elf_Internal_Sym), elf_sort_elf_symbol);
  qsort (isymbuf2, symcount2, sizeof (Elf_Internal_Sym), elf_sort_elf_symbol);
  ...
  symp = symtable1;
  for (isym = isymstart1, isymend = isym + count1; isym < isymend; isym++)
    {
      symp->sym = isym;
  ...
  symp = symtable2;
  for (isym = isymstart2, isymend = isym + count1; isym < isymend; isym++)
    {
      symp->sym = isym;
  ...
  /* Sort symbol by name.  */
  qsort (symtable1, count1, sizeof (struct elf_symbol), elf_sym_name_compare);
  qsort (symtable2, count1, sizeof (struct elf_symbol), elf_sym_name_compare);
