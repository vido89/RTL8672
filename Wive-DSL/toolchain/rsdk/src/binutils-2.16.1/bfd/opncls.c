...
bfd_fdopenr (const char *filename, const char *target, int fd)
{
  ...
  switch (fdflags & (O_ACCMODE))
    {
    case O_RDONLY:
      nbfd->iostream = fdopen (fd, FOPEN_RB);
      break;
    case O_WRONLY:
      nbfd->iostream = fdopen (fd, FOPEN_RUB);
      break;
    case O_RDWR:
      nbfd->iostream = fdopen (fd, FOPEN_RUB);
      break;
...
bfd_openr_iovec (const char *filename, const char *target,
		 ...
		 file_ptr (*pread) (struct bfd * abfd,
				    ...
				    file_ptr nbytes,
				    file_ptr offset),
		 int (*close) (struct bfd * nbfd, void *stream))
{
  bfd *nbfd;
  const bfd_target *target_vec;
  struct opncls *vec;
...
bfd_close (bfd * abfd)
{
  ...
  /* If the file was open for writing and is now executable,
     make it so.  */
  if (ret && abfd->direction == write_direction && abfd->flags & EXEC_P)
    {
      struct stat buf;
...
bfd_calc_gnu_debuglink_crc32 (unsigned long crc,
			      const unsigned char *buf, bfd_size_type len)
{
...
find_separate_debug_file (bfd * abfd, const char *debug_file_directory)
{
  ...
  debugfile = malloc (strlen (debug_file_directory) + 1
		      + strlen (dir)
		      + strlen (".debug/") + strlen (basename) + 1);
  if (debugfile == NULL)
    {
      free (basename);
      free (dir);
  ...
  if (i > 0 && debug_file_directory[i] != '/' && dir[0] != '/')
    strcat (debugfile, "/");
