...
_bfd_elf_create_got_section (bfd * abfd, struct bfd_link_info *info)
{
  ...
  if (bed->want_got_sym)
	...
	return FALSE;
      ...
      if (!info->executable && !bfd_elf_link_record_dynamic_symbol (info, h))
	return FALSE;
...
bfd_boolean
_bfd_elf_link_create_dynamic_sections (bfd * abfd,
  ...
  if (info->executable)
    {
      ...
      if (s == NULL || !bfd_set_section_flags (abfd, s, flags | SEC_READONLY))
	return FALSE;
    }
				...
				const char *name, bfd_boolean provide)
{
  ...
  if (provide && h->def_dynamic && !h->def_regular)
    h->root.type = bfd_link_hash_undefined;
  ...
  if ((h->def_dynamic
	...
	return FALSE;
      ...
      if (h->u.weakdef != NULL && h->u.weakdef->dynindx == -1)
	{
  ...
  switch (elf_section_data (p)->this_hdr.sh_type)
	  ...
	  || strcmp (p->name, ".plt") == 0)
	{
	  ...
	  if (dynobj != NULL
	      ...
	      && (ip->flags & SEC_LINKER_CREATED) && ip->output_section == p)
	    return TRUE;
	}
      return FALSE;
...
_bfd_elf_link_renumber_dynsyms (bfd * output_bfd, struct bfd_link_info *info)
{
  ...
  elf_link_hash_traverse (elf_hash_table (info),
			  elf_link_renumber_hash_table_dynsyms, &dynsymcount);
		       ...
		       bfd_boolean * size_change_ok)
{
  ...
  *override = FALSE;
  ...
  if (!bfd_is_und_section (sec))
    h =
      elf_link_hash_lookup (elf_hash_table (info), name, TRUE, FALSE, FALSE);
  else
  ...
  *sym_hash = h;
  ...
  if (abfd == oldbfd && ((abfd->flags & DYNAMIC) == 0 || !h->def_regular))
    return TRUE;
  ...
  if (oldweak
      || newweak || (newdef && h->root.type == bfd_link_hash_undefined))
    *type_change_ok = TRUE;

  /* It's OK to change the size if either the existing symbol or the
     new symbol is weak, or if the old symbol is undefined.  */

  if (*type_change_ok || h->root.type == bfd_link_hash_undefined)
    *size_change_ok = TRUE;
  ...
  if (newdyn
      ...
      && sym->st_size > 0 && ELF_ST_TYPE (sym->st_info) != STT_FUNC)
    newdyncommon = TRUE;
  else
    newdyncommon = FALSE;
  ...
  if (olddyncommon && newdyncommon && sym->st_size != h->size)
    {
  ...
  if (newdyn
      ...
      && (olddef
	  || (h->root.type == bfd_link_hash_common
	      && (newweak || ELF_ST_TYPE (sym->st_info) == STT_FUNC))))
    {
      *override = TRUE;
      newdef = FALSE;
      newdyncommon = FALSE;
  ...
  if (newdyncommon && h->root.type == bfd_link_hash_common)
    {
      *override = TRUE;
      newdef = FALSE;
      newdyncommon = FALSE;
  ...
  if (!newdyn && bfd_is_com_section (sec) && olddyncommon)
    {
...
_bfd_elf_add_default_symbol (bfd * abfd,
			     ...
			     asection ** psec,
			     bfd_vma * value,
			     bfd_boolean * dynsym, bfd_boolean override)
{
  bfd_boolean type_change_ok;
  bfd_boolean size_change_ok;
  bfd_boolean skip;
  ...
  if (!override)
      ...
      if (h->def_dynamic)
	{
	  h->def_dynamic = 0;
	  hi->ref_dynamic = 1;
	  if (hi->ref_regular || hi->def_regular)
	    {
...
_bfd_elf_export_symbol (struct elf_link_hash_entry * h, void *data)
{
  ...
  if (h->dynindx == -1 && (h->def_regular || h->ref_regular))
    {
					 ...
					 void *data)
{
  ...
  /* We only care about symbols defined in shared objects with version
     information.  */
  if (!h->def_dynamic
      || h->def_regular || h->dynindx == -1 || h->verinfo.verdef == NULL)
    return TRUE;

  /* See if we already know about this version.  */
  for (t = elf_tdata (rinfo->output_bfd)->verref; t != NULL;
       t = t->vn_nextref)
    {
...
_bfd_elf_link_assign_sym_version (struct elf_link_hash_entry * h, void *data)
{
      ...
      for (t = sinfo->verdefs; t != NULL; t = t->next)
	{
	  if (strcmp (t->name, p) == 0)
	      ...
	      if (d == NULL && t->locals.list != NULL)
		{
		  ...
		  if (d != NULL
		      && h->dynindx != -1
		      && info->shared && !info->export_dynamic)
  ...
  if (h->verinfo.vertree == NULL && sinfo->verdefs != NULL)
      ...
      if (local_ver != NULL)
	{
	  ...
	  if (h->dynindx != -1 && info->shared && !info->export_dynamic)
	    {
	      (*bed->elf_backend_hide_symbol) (info, h, TRUE);
	    }
	}
      ...
      if ((size_t) r_symndx >= nsyms)
	{
	  (*_bfd_error_handler)
	    (_("%B: bad reloc symbol index (0x%lx >= 0x%lx)"
	     ...
	     abfd, sec,
	     (unsigned long) r_symndx, (unsigned long) nsyms,
	     irela->r_offset);
	  bfd_set_error (bfd_error_bad_value);
	  return FALSE;
	}
			   ...
			   bfd_boolean keep_memory)
{
  ...
  if (!elf_link_read_relocs_from_section (abfd, o, rel_hdr,
					  external_relocs, internal_relocs))
    goto error_return;
  if (elf_section_data (o)->rel_hdr2
				  ...
				  Elf_Internal_Shdr * rel_hdr, asection * o)
{
  ...
  if (elf_section_data (o)->rel_hashes == NULL && num_rel_hashes)
    {
			   ...
			   struct elf_info_failed * eif)
{
  ...
  if (h->non_elf)
      ...
      else
	{
      ...
      if (h->dynindx == -1 && (h->def_dynamic || h->ref_dynamic))
	{
	  if (!bfd_elf_link_record_dynamic_symbol (eif->info, h))
	    {
	      eif->failed = TRUE;
  ...
  if (h->needs_plt
      ...
      && (eif->info->symbolic
	  || ELF_ST_VISIBILITY (h->other) != STV_DEFAULT) && h->def_regular)
    {
      const struct elf_backend_data *bed;
...
_bfd_elf_adjust_dynamic_symbol (struct elf_link_hash_entry * h, void *data)
{
  ...
  if (h->size == 0 && h->type == STT_NOTYPE && !h->needs_plt)
    (*_bfd_error_handler)
...
elf_link_is_defined_archive_symbol (bfd * abfd, carsym * symdef)
{
  ...
  result = FALSE;
  for (isym = isymbuf, isymend = isymbuf + extsymcount; isym < isymend;
       isym++)
    {
      const char *name;
...
elf_add_dt_needed_tag (bfd * abfd,
		       struct bfd_link_info *info,
		       const char *soname, bfd_boolean do_it)
{
  struct elf_link_hash_table *hash_table;
  ...
  if (oldsize == _bfd_elf_strtab_size (hash_table->dynstr))
	...
	for (extdyn = sdyn->contents;
	     ...
	     extdyn += bed->s->sizeof_dyn)
	  {
	    ...
	    if (dyn.d_tag == DT_NEEDED && dyn.d_un.d_val == strindex)
	      {
...
elf_finalize_dynstr (bfd * output_bfd, struct bfd_link_info *info)
{
  ...
  for (extdyn = sdyn->contents;
       extdyn < sdyn->contents + sdyn->size; extdyn += bed->s->sizeof_dyn)
    {
      Elf_Internal_Dyn dyn;
...
elf_link_add_object_symbols (bfd * abfd, struct bfd_link_info *info)
{
  ...
  bfd_boolean (*check_directives) (bfd *, struct bfd_link_info *);
  bfd_boolean collect;
  Elf_Internal_Shdr *hdr;
  bfd_size_type symcount;
  bfd_size_type extsymcount;
  ...
  if (info->executable)
      ...
      for (s = abfd->sections; s != NULL; s = s->next)
	{
	  ...
	  if (strncmp (name, ".gnu.warning.", sizeof ".gnu.warning." - 1) ==
	      0)
	    {
	      char *msg;
	      bfd_size_type sz;
  ...
  if (!dynamic)
	...
	goto error_return;
      ...
      add_needed = (elf_dyn_lib_class (abfd)
		    & (DYN_AS_NEEDED | DYN_DT_NEEDED | DYN_NO_NEEDED)) == 0;
      ...
      if (s != NULL)
	{
	  ...
	  for (extdyn = dynbuf;
	       extdyn < dynbuf + s->size; extdyn += bed->s->sizeof_dyn)
	    {
	      Elf_Internal_Dyn dyn;
	      ...
	      if (dyn.d_tag == DT_SONAME)
		{
		  ...
		  soname =
		    bfd_elf_string_from_elf_section (abfd, shlink, tagv);
		  if (soname == NULL)
		    goto error_free_dyn;
		}
	      ...
	      if (dyn.d_tag == DT_RUNPATH)
		{
		  ...
		  n->by = abfd;
		  n->next = NULL;
		  for (pn = &runpath; *pn != NULL; pn = &(*pn)->next)
		    ;
		  *pn = n;
		}
	      ...
	      if (!runpath && dyn.d_tag == DT_RPATH)
		{
		  ...
		  n->by = abfd;
		  n->next = NULL;
		  for (pn = &rpath; *pn != NULL; pn = &(*pn)->next)
		    ;
		  *pn = n;
		}
	    }
      ...
      if (rpath)
	{
	  ...
	  for (pn = &hash_table->runpath; *pn != NULL; pn = &(*pn)->next)
	    ;
	  *pn = rpath;
	}
  ...
  for (isym = isymbuf, isymend = isymbuf + extsymcount;
	      ...
	      if (isym->st_shndx != SHN_UNDEF)
		{
		  ...
		  if (verstr == NULL)
			...
			(_("%B: %s: invalid version %u (max %d)"),
			 abfd, name, vernum, elf_tdata (abfd)->cverdefs);
		      bfd_set_error (bfd_error_bad_value);
		      goto error_free_vers;
		    }
		}
	...
	h = (struct elf_link_hash_entry *) h->root.u.i.link;
      *sym_hash = h;
	  ...
	  && is_elf_hash_table (hash_table) && h->u.weakdef == NULL)
	{
      ...
      if (is_elf_hash_table (hash_table))
	{
	  ...
	  if (isym->st_size != 0 && (definition || h->size == 0))
	    {
	  ...
	  if (definition && !dynamic
	    ...
	    isym->st_other =
	      STV_HIDDEN | (isym->st_other & ~ELF_ST_VISIBILITY (-1));
	  ...
	  if (!dynamic)
		...
		h->def_regular = 1;
	      if (!info->executable || h->def_dynamic || h->ref_dynamic)
		dynsym = TRUE;
	    }
	  else
	    {
		...
		h->def_dynamic = 1;
		  ...
		  || h->ref_regular
		  || (h->u.weakdef != NULL
		      && !new_weakdef && h->u.weakdef->dynindx != -1))
		dynsym = TRUE;
	    }
	  ...
	  if (dynsym && h->dynindx == -1)
		...
		goto error_free_vers;
		  ...
		  && !new_weakdef && h->u.weakdef->dynindx == -1)
		{
		  if (!bfd_elf_link_record_dynamic_symbol
		      (info, h->u.weakdef))
		    goto error_free_vers;
		}
	    }
	  ...
	  if (!add_needed && definition && dynsym && h->ref_regular)
	    {
	      int ret;
  ...
  if (!add_needed && (elf_dyn_lib_class (abfd) & DYN_AS_NEEDED) != 0)
    {
  ...
  if (weaks != NULL)
      ...
      for (; hpp < hppend; hpp++)
	{
	  h = *hpp;
	  if (h != NULL
	      && h->root.type == bfd_link_hash_defined && h->type != STT_FUNC)
	    {
	      *sym_hash = h;
	      sym_hash++;
	      sym_count++;
      ...
      qsort (sorted_sym_hash, sym_count,
	     sizeof (struct elf_link_hash_entry *), elf_sort_symbol);
  ...
  if (!dynamic
      && is_elf_hash_table (hash_table)
      && hash_table->root.creator == abfd->xvec && check_relocs != NULL)
    {
      asection *o;
  ...
  *sinterpptr = NULL;
  ...
  if (info->execstack)
  ...
  else
    {
      ...
      for (inputobj = info->input_bfds;
	   inputobj; inputobj = inputobj->link_next)
	{
	  asection *s;
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
	...
	for (d = t->globals.list; d != NULL; d = d->next)
      ...
      elf_link_hash_traverse (elf_hash_table (info),
			      _bfd_elf_link_assign_sym_version, &asvinfo);
      if (asvinfo.failed)
	return FALSE;
      ...
      if (!info->allow_undefined_version)
	{
	  ...
	  for (t = verdefs; t != NULL; t = t->next)
		...
		{
		  (*_bfd_error_handler)
		    (_("%s: undefined version: %s"), d->pattern, t->name);
		  all_defined = FALSE;
		}
      ...
      elf_link_hash_traverse (elf_hash_table (info),
			      _bfd_elf_adjust_dynamic_symbol, &eif);
      if (eif.failed)
	return FALSE;
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
	...
	{
	  if (info->executable)
	    info->flags_1 &= ~(DF_1_INITFIRST | DF_1_NODELETE | DF_1_NOOPEN);
...
static size_t
elf_link_sort_relocs (bfd * abfd, struct bfd_link_info *info,
		      asection ** psec)
{
  asection *reldyn;
...
elf_link_output_sym (struct elf_final_link_info *finfo,
		     ...
		     asection * input_sec, struct elf_link_hash_entry *h)
{
  bfd_byte *dest;
				 ...
				 struct elf_link_hash_entry *h)
{
  ...
  for (loaded = elf_hash_table (info)->loaded;
       loaded != NULL; loaded = loaded->next)
    {
      bfd *input;
      Elf_Internal_Shdr *hdr;
      bfd_size_type symcount;
      ...
      if (input == abfd
	  || (input->flags & DYNAMIC) == 0 || elf_dynversym (input) == 0)
	continue;
...
elf_link_output_extsym (struct elf_link_hash_entry *h, void *data)
{
      ...
      && finfo->info->unresolved_syms_in_shared_libs != RM_IGNORE)
    {
      if (!((*finfo->info->callbacks->undefined_symbol)
	    (finfo->info, h->root.root.string, h->root.u.undef.abfd,
	     NULL, 0,
	     finfo->info->unresolved_syms_in_shared_libs ==
	     RM_GENERATE_ERROR)))
	{
	  eoinfo->failed = TRUE;
	  return FALSE;
  ...
  if (!finfo->info->relocatable
	...
	(_("%B: %s symbol `%s' in %B is referenced by DSO"),
	 ...
	 ? "internal"
	 : ELF_ST_VISIBILITY (h->other) == STV_HIDDEN
	 ? "hidden" : "local", h->root.root.string);
      eoinfo->failed = TRUE;
      return FALSE;
    }
  ...
  if (strip && h->dynindx == -1 && !h->forced_local)
    return TRUE;
		...
		if (h->type == STT_TLS)
		  {
		    /* STT_TLS symbols are relative to PT_TLS segment
		       base.  */
		    BFD_ASSERT (elf_hash_table (finfo->info)->tls_sec !=
				NULL);
		    sym.st_value -=
		      elf_hash_table (finfo->info)->tls_sec->vma;
		  }
	      }
	  }
	else
  ...
  if (!finfo->info->relocatable
      && ELF_ST_VISIBILITY (sym.st_other) != STV_DEFAULT
      && ELF_ST_BIND (sym.st_info) != STB_WEAK
      && h->root.type == bfd_link_hash_undefined && !h->def_regular)
    {
      (*_bfd_error_handler)
	(_("%B: %s symbol `%s' isn't defined"),
	 ...
	 ? "internal" : "hidden", h->root.root.string);
      eoinfo->failed = TRUE;
      return FALSE;
    }
...
elf_link_input_bfd (struct elf_final_link_info *finfo, bfd * input_bfd)
{
  ...
  for (isym = isymbuf, pindex = finfo->indices, ppsection = finfo->sections;
       isym < isymend; isym++, pindex++, ppsection++)
    {
      asection *isec;
      const char *name;
  ...
  for (o = input_bfd->sections; o != NULL; o = o->next)
      ...
      if ((o->flags & SEC_RELOC) != 0)
	{
	  ...
	  if (internal_relocs == NULL && o->reloc_count > 0)
	    return FALSE;
	      ...
	      for (; rel < relend; rel++)
		{
		  ...
		  if (r_symndx >= locsymcount
		      ...
		      if (h == NULL)
			{
			  ...
			  (*_bfd_error_handler)
			    (_
			     ("error: %B contains a reloc (0x%s) for section %A "
	  ...
	  if (emit_relocs)
		...
		irela + o->reloc_count * bed->s->int_rels_per_ext_rel;
	      rel_hash =
		(elf_section_data (o->output_section)->rel_hashes +
			  ...
			  if (!finfo->info->relocatable)
			      ...
			      if (ELF_ST_TYPE (sym.st_info) == STT_TLS)
				{
				  ...
				  BFD_ASSERT (elf_hash_table (finfo->info)->
					      tls_sec != NULL);
				  sym.st_value -=
				    (elf_hash_table (finfo->info)->tls_sec->
				     vma);
				}
			    }
...
elf_get_linked_section_vma (struct bfd_link_order *p)
{
  ...
  if (elfsec == 0)
    {
      const struct elf_backend_data *bed = get_elf_backend_data (s->owner);
...
elf_fixup_link_order (bfd * abfd, asection * o)
{
  ...
  for (p = o->link_order_head; p != NULL; p = p->next)
	  ...
	  && elf_elfheader (sub)->e_ident[EI_CLASS] == bed->s->elfclass)
	{
	  ...
	  /* 2006-03-22 tonywu: should do boundary check
	   * when elfsec is out of bound SHN_XXX, elf_eflsections(sub)[elfsec]
	   * might segment fault */
	  if (elfsec != -1 &&
	      elfsec < (int) elf_numsections (abfd) &&
...
bfd_elf_final_link (bfd * abfd, struct bfd_link_info * info)
{
  ...
  emit_relocs = (info->relocatable
		 || info->emitrelocations || bed->elf_backend_emit_relocs);
  ...
  for (o = abfd->sections; o != NULL; o = o->next)
      ...
      else
	{
      ...
      if ((o->flags & SEC_ALLOC) == 0 && !o->user_set_vma)
	o->vma = 0;
    }
  ...
  if (elf_hash_table (info)->tls_sec)
    {
      ...
      for (sec = elf_hash_table (info)->tls_sec;
	   sec && (sec->flags & SEC_THREAD_LOCAL); sec = sec->next)
	{
	...
	align_power (end, elf_hash_table (info)->tls_sec->alignment_power);
  ...
  if (dynamic && finfo.dynsym_sec->output_section != bfd_abs_section_ptr)
    {
      Elf_Internal_Sym sym;
	  ...
	  for (e = elf_hash_table (info)->dynlocal; e; e = e->next)
		      ...
		      || e->isym.st_shndx > SHN_HIRESERVE))
		{
		  ...
		  sym.st_value = (s->output_section->vma
				  + s->output_offset + e->isym.st_value);
		}
  ...
  if (dynamic)
      ...
      for (; dyncon < dynconend; dyncon += bed->s->sizeof_dyn)
	{
	  ...
	  switch (dyn.d_tag)
	    ...
	    case DT_NULL:
	      if (relativecount > 0
		  && dyncon + bed->s->sizeof_dyn < dynconend)
		{
  ...
  if (dynobj != NULL)
	...
	{
	  if ((o->flags & SEC_HAS_CONTENTS) == 0
	      || o->size == 0 || o->output_section == bfd_abs_section_ptr)
	    continue;
	  if ((o->flags & SEC_LINKER_CREATED) == 0)
	    {
...
_bfd_elf_gc_mark (struct bfd_link_info *info,
		  asection * sec, gc_mark_hook_fn gc_mark_hook)
{
  bfd_boolean ret;
  asection *group_sec;
...
elf_gc_sweep (struct bfd_link_info *info, gc_sweep_hook_fn gc_sweep_hook)
{
  ...
  for (sub = info->input_bfds; sub != NULL; sub = sub->link_next)
      ...
      for (o = sub->sections; o != NULL; o = o->next)
	{
	  ...
	  if (gc_sweep_hook && (o->flags & SEC_RELOC) && o->reloc_count > 0)
	    {
	      Elf_Internal_Rela *internal_relocs;
	      bfd_boolean r;
...
static bfd_boolean
elf_gc_propagate_vtable_entries_used (struct elf_link_hash_entry *h,
				      void *okp)
{
  if (h->root.type == bfd_link_hash_warning)
    h = (struct elf_link_hash_entry *) h->root.u.i.link;
...
elf_gc_smash_unused_vtentry_relocs (struct elf_link_hash_entry *h, void *okp)
{
  ...
  for (rel = relstart; rel < relend; ++rel)
	...
	if (h->vtable->used && (rel->r_offset - hstart) < h->vtable->size)
	  {
				...
				void *okp ATTRIBUTE_UNUSED)
{
  ...
  if ((h->root.type == bfd_link_hash_defined
       || h->root.type == bfd_link_hash_defweak) && h->ref_dynamic)
...
bfd_elf_gc_sections (bfd * abfd, struct bfd_link_info * info)
{
  ...
  if (!get_elf_backend_data (abfd)->can_gc_sections
      ...
      || info->shared || !is_elf_hash_table (info->hash))
    {
  ...
  elf_link_hash_traverse (elf_hash_table (info),
			  elf_gc_propagate_vtable_entries_used, &ok);
  if (!ok)
    return FALSE;

  /* Kill the vtable relocations that were not used.  */
  elf_link_hash_traverse (elf_hash_table (info),
			  elf_gc_smash_unused_vtentry_relocs, &ok);
  if (!ok)
    return FALSE;
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
    elf_link_hash_traverse (elf_hash_table (info),
			    elf_gc_mark_dynamic_ref_symbol, &ok);
  if (!ok)
    return FALSE;
					...
					struct bfd_link_info * info)
{
  ...
  elf_link_hash_traverse (elf_hash_table (info),
			  elf_gc_allocate_got_offsets, &gofarg);
  return TRUE;
}
  ...
  for (; rcookie->rel < rcookie->relend; rcookie->rel++)
      ...
      else
	{
	  ...
	  if (isym->st_shndx < SHN_LORESERVE
	    ...
	    {
	      isec =
		bfd_section_from_elf_index (rcookie->abfd, isym->st_shndx);
...
bfd_elf_discard_info (bfd * output_bfd, struct bfd_link_info * info)
{
  ...
  if (info->traditional_format || !is_elf_hash_table (info->hash))
    return FALSE;
  ...
  for (abfd = info->input_bfds; abfd != NULL; abfd = abfd->link_next)
	...
	continue;
      ...
      if (info->relocatable
	  || (eh != NULL
	      && (eh->size == 0 || bfd_is_abs_section (eh->output_section))))
	eh = NULL;
	...
	stab = NULL;
      ...
      if (stab == NULL && eh == NULL && bed->elf_backend_discard_info == NULL)
	continue;
	  ...
	  switch (flags & SEC_LINK_DUPLICATES)
	      ...
	      else if (sec->size != 0)
		{
		  ...
		  if (!bfd_malloc_and_get_section (abfd, sec, &sec_contents))
			...
			(l->sec->owner, l->sec, &l_sec_contents))
		    (*_bfd_error_handler) (_
					   ("%B: warning: could not read contents of section `%A'\n"),
					   l->sec->owner, l->sec);
		  else if (memcmp (sec_contents, l_sec_contents, sec->size) !=
			   0)
		    (*_bfd_error_handler) (_
