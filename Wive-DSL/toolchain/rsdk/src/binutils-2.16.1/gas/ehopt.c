...
get_cie_info (struct cie_info *info)
{
  ...
  if (f == NULL || f->fr_fix - offset < 1 || f->fr_literal[offset] != 1)
    return 0;
  ...
  if (augmentation[0] == '\0')
  ...
  else if (strcmp (augmentation, "eh") == 0)
    {
      ...
      while (fix != NULL && (fix->fx_frag != f || fix->fx_where != offset))
	fix = fix->fx_next;
      if (fix == NULL)
	offset += 4;
      else
...
check_eh_frame (expressionS * exp, unsigned int *pnbytes)
{
  ...
  switch (d->state)
    ...
    case state_saw_pc_begin:
      ...
      if (!d->cie_info_ok && !(d->cie_info_ok = get_cie_info (&d->cie_info)))
	d->state = state_error;
