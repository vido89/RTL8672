...
chain_frchains_together (bfd * abfd ATTRIBUTE_UNUSED,
			 segT section, PTR xxx ATTRIBUTE_UNUSED)
{
	...
	if (fragP->fr_subtype)
	  {
	    ...
	    for (lie = (struct broken_word *) (fragP->fr_symbol);
		 lie && lie->dispfrag == fragP; lie = lie->next_broken_word)
	      if (lie->added == 1)
  ...
  if (seginfo && seginfo->frchainP)
	...
	cvt_frag_to_fill (sec, fragp);
      for (fragp = seginfo->frchainP->frch_root;
	   fragp->fr_next; fragp = fragp->fr_next)
	/* Walk to last elt.  */
	;
		   ...
		   asection * sec, PTR xxx ATTRIBUTE_UNUSED)
{
  ...
  for (fixp = seginfo->fix_root; fixp; fixp = fixp->fx_next)
	...
	if (symsec != sec && !S_IS_LOCAL (sym))
	  {
	    if ((symsec->flags & SEC_LINK_ONCE) != 0 || (IS_ELF
							 /* The GNU toolchain uses an extension for ELF: a
							    section beginning with the magic string
							    .gnu.linkonce is a linkonce section.  */
							 &&
							 strncmp (segment_name
								  (symsec),
								  ".gnu.linkonce",
								  sizeof
								  ".gnu.linkonce"
  ...
  for (fixp = seginfo->fix_root; fixp != (fixS *) NULL; fixp = fixp->fx_next)
      ...
      if (!reloc)
	{
      ...
      s = bfd_install_relocation (stdoutput, reloc,
				  ...
				  fixp->fx_frag->fr_address, sec, &err);
      switch (s)
	{
	case bfd_reloc_ok:
	  break;
      ...
      for (j = 0; reloc[j]; j++)
	{
	  ...
	  switch (s)
	    ...
	    case bfd_reloc_notmultipleof8_ltw:
	      as_bad (_("offset in ltw instruction is not multiple of 8."));
	      break;
	    default:
	      as_fatal (_
...
write_object_file (void)
{
...
#ifndef WORKING_DOT_WORD
  {
	...
	for (m = 0;
	     lie && lie->dispfrag == fragP; m++, lie = lie->next_broken_word)
	  {
	    if (lie->added == 2)
	      continue;
  ...
  if (symbol_rootP)
      ...
      for (symp = symbol_rootP; symp; symp = symbol_next (symp))
	{
	  ...
	  if (S_IS_DEFINED (symp) == 0 && S_GET_VALUE (symp) != 0)
	  ...
	  if (symp == abs_section_sym
	      || (!EMIT_SECTION_SYMBOLS && symbol_section_p (symp))
		  ...
		  for (lie = (struct broken_word *) (fragP->fr_symbol);
			...
			continue;
		      ...
		      offset = (S_GET_VALUE (lie->add)
				+ lie->addnum - S_GET_VALUE (lie->sub));
		...
		if (symbolP)
		  {
			...
			|| S_IS_COMMON (symbolP) || !S_IS_DEFINED (symbolP))
		      {
			as_bad_where (fragP->fr_file, fragP->fr_line,
				      _
  ...
  for (; fixP; fixP = fixP->fx_next)
    {
      ...
      if (fixP->fx_addsy != NULL && symbol_mri_common_p (fixP->fx_addsy))
	{
      ...
      if (!fixP->fx_bit_fixP && !fixP->fx_no_overflow && fixP->fx_size != 0)
	{
	  ...
	  if (!flag_signed_overflow_ok
	      && fixP->fx_size == 2 && add_number > 0x7fff)
	    as_bad_where (fixP->fx_file, fixP->fx_line,
			  _
...
{
  indent_level = 1;
  fprintf (stderr, "fix %lx %s:%d", (long) fixp, fixp->fx_file,
	   fixp->fx_line);
  if (fixp->fx_pcrel)
