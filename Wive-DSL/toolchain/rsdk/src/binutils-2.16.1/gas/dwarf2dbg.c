...
get_filenum (const char *filename, unsigned int num)
{
  ...
  if (num == 0)
	...
	if (files[i].dir == dir
	    && files[i].filename && strcmp (file, files[i].filename) == 0)
	  {
	    last_used = i;
...
first_frag_for_seg (segT seg)
{
  ...
  for (f = frchain_root; f; f = f->frch_next)
    if (f->frch_seg == seg && (!first || first->frch_subseg > f->frch_subseg))
      first = f;
...
last_frag_for_seg (segT seg)
{
  ...
  for (f = frchain_root; f; f = f->frch_next)
    if (f->frch_seg == seg && (!last || last->frch_subseg < f->frch_subseg))
      last = f;
...
size_inc_line_addr (int line_delta, addressT addr_delta)
{
  ...
  if (addr_delta < 256 + MAX_SPECIAL_ADDR_DELTA)
	...
	return len + 1;
      ...
      opcode =
	tmp + (addr_delta - MAX_SPECIAL_ADDR_DELTA) * DWARF2_LINE_RANGE;
      if (opcode <= 255)
	return len + 2;
    }
