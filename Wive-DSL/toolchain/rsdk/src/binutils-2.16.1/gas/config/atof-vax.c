     ...
     int number_of_bits;
{
  ...
  if (number_of_bits >= bits_left_in_littlenum)
  ...
  else
    {
      ...
      return_value =
	mask[number_of_bits] & ((*littlenum_pointer) >>
     ...
     LITTLENUM_TYPE *words;	/* Deliver answer here.  */
{
  ...
  if (return_value != 0)
      ...
      else
	{
	  ...
	  {
	    LITTLENUM_TYPE *ltp;
	    for (ltp = f->leader + 1; ltp <= f->low + precision; ltp++)
	      {
		*ltp = 0;
	      }
	  }
	  ...
	  if (exponent_4 & ~mask[exponent_bits])
	  ...
	  else
	    {
	      ...
	      word1 =
		(((exponent_4 & mask[exponent_bits]) << (15 - exponent_bits))
		 | ((f->sign == '+') ? 0 : 0x8000) | next_bits (15 -
								exponent_bits));
	      *lp++ = word1;
  ...
  if (kind_of_float)
    {
      ...
      know (number_of_chars <=
	    MAXIMUM_NUMBER_OF_LITTLENUMS * sizeof (LITTLENUM_TYPE));
