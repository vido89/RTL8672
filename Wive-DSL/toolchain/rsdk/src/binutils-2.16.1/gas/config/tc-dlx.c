  ...
  if (the_insn.reloc != NO_RELOC)
    {
      fixP = fix_new_exp (frag_now,
			  (toP - frag_now->fr_literal +
			   the_insn.reloc_offset), the_insn.size,
     ...
     expressionS *operandp;
{
  ...
  if ((s[0] == '%' || s[0] == '$' || s[0] == 'r' || s[0] == 'R')
      && ISDIGIT ((unsigned char) s[1]))
    {
      ...
      if (operandp->X_op != O_constant || operandp->X_add_number > 31)
     ...
     char *str;
{
  ...
  if ((insn = (struct machine_opcode *) hash_find (op_hash, str)) == NULL)
      ...
      if ((strcmp (str, "ret") == 0) || (strcmp (str, "return") == 0))
	{
	  ...
	  the_insn.pcrel = 0;
	  the_insn.opcode = (unsigned long) (JROP | 0x03e00000);	/* 0x03e00000 = r31 << 21 */
	}
      else
	as_bad (_("Unknown opcode `%s'."), str);
	...
	case 'I':
	  ...
	  if (operand->X_op == O_constant)
		...
		operand->X_add_number >>= 16;
	      ...
	      if (the_insn.HI && the_insn.LO)
		as_bad (_("Both the_insn.HI and the_insn.LO are set : %s"),
			s);
	      else
		{
		  the_insn.HI = 0;
