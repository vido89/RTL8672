...
/* Unconditional branch cases "jrb"
  ...
  {
   BF, BB, 1, C (1, 1)},	/* brb B`foo        1,0 */
  {
   WF, WB, 2, C (1, 2)},	/* brw W`foo        1,1 */
  {
   0, 0, 5, 0},			/* Jmp L`foo        1,2 */
  {
   1, 1, 0, 0},			/* unused           1,3 */
/* Reversible Conditional Branch. If the branch won't reach, reverse
  ...
  {
   WF + 2, WB + 2, 4, C (2, 2)},	/* brev over, brw W`foo, over: 2,1 */
  {
   0, 0, 7, 0},			/* brev over, jmp L`foo, over: 2,2 */
  {
   1, 1, 0, 0},			/* unused           2,3 */
/* Another type of reversible branch. But this only has a word
     displacement.  */
  {
   1, 1, 0, 0},			/* unused           3,0 */
  {
   WF, WB, 2, C (3, 2)},	/* jbX W`foo        3,1 */
  {
   0, 0, 8, 0},			/* jrevX over, jmp L`foo, over:  3,2 */
  {
   1, 1, 0, 0},			/* unused           3,3 */
/* These are the non reversible branches, all of which have a word
     ...
     leap frog.  */
  {
   1, 1, 0, 0},			/* unused           4,0 */
  {
   WF, WB, 2, C (4, 2)},	/* aobl_ W`foo      4,1 */
  {
   0, 0, 10, 0},		/*aobl_ W`hop,br over,hop: jmp L^foo,over 4,2 */
  {
   1, 1, 0, 0},			/* unused           4,3 */
/* Normal displacement mode, no jumping or anything like that.
  ...
  {
   BF + 1, BB + 1, 2, C (5, 1)},	/* B^"foo"          5,0 */
  {
   WF + 1, WB + 1, 3, C (5, 2)},	/* W^"foo"          5,1 */
  {
   0, 0, 5, 0},			/* L^"foo"          5,2 */
  {
   1, 1, 0, 0},			/* unused           5,3 */
     ...
     char *arg;
{
  switch (c)
    ...
    case 'a':
      as_warn (_
	       ("The -a option doesn't exist. (Despite what the man page says!"));
      break;
     ...
     segT segment_type;		/* N_DATA or N_TEXT.  */
{
  if (RELAX_LENGTH (fragP->fr_subtype) == STATE_UNDF)
	...
	case STATE_BIG_NON_REV_BRANCH:
	  fragP->fr_subtype =
	    ENCODE_RELAX (STATE_BIG_NON_REV_BRANCH, STATE_WORD);
	  break;
	case STATE_ALWAYS_BRANCH:
...

{
  ...
  *optex = '\0';		/* This is kind of a back stop for all
  ...
  if (*point != '\0' && ((point[1] == '^') || (point[1] == '`')))
	...
	if (com_width)
	  as_warn (_
		   ("Casting a branch displacement is bad form, and is ignored."));
	else
	  {
	    c = TOLOWER (*point);
	    call_width = ((c == 'b') ? 1 : ((c == 'w') ? 2 : 4));
	  }
	point += 2;
	break;
      }
  ...
  *optex = segfault;
  ...
  disp_size = (call_width ? call_width :
	       (com_width ? com_width : abs_width ? abs_width : 0));
	  ...
	  if (*op_bad == '\0')
	    {
	      ...
	      if (width == 'q' && imreg == 12)
		as_warn (_
			 ("Using reg 14 for quadwords can tromp the FP register."));
      ...
      else
	{
	  ...
	  for (p = twP->args, operandp = titP->tit_operand;
	       !*alloperr && *p; operandp++, p += 2)
	    {
	      ...
	      if (!p[1])
		as_fatal (_
			  ("Compiler bug: ODD number of bytes in arg structure %s."),
			  twP->args);
	      else if (*instring)
		{
  ...
  else
    {
      ...
      *opcodeP = t.tit_opcode;
      ...
      for (operandP = t.tit_operand;
	   operandP < t.tit_operand + t.tit_operands; operandP++)
	{			/* for each operand */
	  ...
	  know (to_seg == SEG_UNKNOWN ||
		to_seg == SEG_ABSOLUTE ||
		to_seg == SEG_DATA ||
		to_seg == SEG_TEXT || to_seg == SEG_BSS);
	  ...
	  if (operandP->top_access == 'b')
	      ...
	      if (to_seg == now_seg || is_undefined)
		{
		  ...
		  if (dispsize)
			...
			default:
			  as_fatal (_
				    ("Compliler bug: Got a case (%d) I wasn't expecting."),
				    operandP->top_width);
			}
		    }
		}
		  ...
		  switch (operandP->top_width)
		    ...
		    case TAHOE_WIDTH_CONDITIONAL_JUMP:
		      ...
		      *p++ = (operandP->top_mode ==
		      ...
		      fix_new (frag_now, p - frag_now->fr_literal,
			       this_add_symbol, this_add_number,
			       (to_seg != SEG_ABSOLUTE) ? FX_PCREL32 : FX_32,
			       NULL);
		      /*
		       * Now (eg)       BLEQ    1f
		       *                JMP     foo
		    ...
		    case 'w':
		      as_warn (_
			       ("Real branch displacements must be expressions."));
		      break;
		    default:
		      as_fatal (_
				("Complier error: I got an unknown synthetic branch :%c"),
				operandP->top_width);
		      break;
		    }
		}
		  ...
		  if (this_add_symbol != NULL)
		    {
		      ...
		      *p++ = TAHOE_IMMEDIATE_LONGWORD;
		      fix_new (frag_now, p - frag_now->fr_literal,
			       this_add_symbol, this_add_number, FX_32, NULL);
		    }
		  else
		    {
     ...
     fixS *fixP;
{
  return (((fixP->fx_type == FX_8
	   ...
	   : ((fixP->fx_type == FX_16
		 ...
		 ? 4 : 0))) + fixP->fx_where + fixP->fx_frag->fr_address);
}				/* md_pcrel_from() */
