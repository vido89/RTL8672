...
{
  {
  "SYNTAX_3OP|OP1_MUST_BE_IMM", 26,
      SYNTAX_3OP | OP1_MUST_BE_IMM | SYNTAX_VALID},
  {
  "OP1_MUST_BE_IMM|SYNTAX_3OP", 26,
      OP1_MUST_BE_IMM | SYNTAX_3OP | SYNTAX_VALID},
  {
  "SYNTAX_2OP|OP1_IMM_IMPLIED", 26,
      SYNTAX_2OP | OP1_IMM_IMPLIED | SYNTAX_VALID},
  {
  "OP1_IMM_IMPLIED|SYNTAX_2OP", 26,
      OP1_IMM_IMPLIED | SYNTAX_2OP | SYNTAX_VALID},
  {
     ...
     int mach;
{
  ...
  for (i = 0; i < arc_suffixes_count; i++)
	...
	hash_insert (arc_suffix_hash, arc_suffixes[i].name,
		     (PTR) (arc_suffixes + i));
  ...
  if (operand->insert)
    {
      ...
      errmsg = NULL;
      insn =
	(*operand->insert) (insn, operand, mods, reg, (long) val, &errmsg);
  ...
  for (opcode = (ext_opcode ? ext_opcode : std_opcode);
      ...
      for (str = start, syn = opcode->syntax; *syn != '\0';)
	{
	  ...
	  while (ARC_MOD_P (arc_operands[arc_operand_map[(int) *syn]].flags))
	    {
	      mods |=
		arc_operands[arc_operand_map[(int) *syn]].
		flags & ARC_MOD_BITS;
	      ++syn;
	    }
	  ...
	  if (operand->flags & ARC_OPERAND_FAKE)
		...
		{
		  insn =
		    (*operand->insert) (insn, operand, mods, NULL, 0,
					&errmsg);
		  if (errmsg != (const char *) NULL)
		    {
		      last_errmsg = errmsg;
		  ...
		  if (limm_reloc_p
			  ...
			  (ARC_OPERAND_ABSOLUTE_BRANCH |
			   ARC_OPERAND_ADDRESS)))
		    {
		      fixups[fix_up_at].opindex =
			arc_operand_map[operand->fmt];
		    }
		}
	      ++syn;
	    }
	  /* Are we finished with suffixes?  */
	      ...
	      else
		{
		  *t = c;
		  ...
		  for (suffix = suf;
		       suffix < suffix_end
		       && strcmp (suffix->name, suf->name) == 0; ++suffix)
		    {
...
#define IS_REG_SHIMM_OFFSET(o) ((o) == 'd')
      ...
      if (*syn == '\0')
	{
...

	  {
	    ...
	    if (delay_slot_type != ARC_DELAY_NONE && limm_p && arc_insn_not_jl (insn))	/* except for jl  addr */
	  ...
	  for (i = 0; i < fc; ++i)
	      ...
	      else
		{
	      ...
	      fix_new_exp (frag_now,
			   ...
			   &exptmp,
			   (operand->flags & ARC_OPERAND_RELATIVE_BRANCH) !=
			   0, (bfd_reloc_code_real_type) reloc_type);
	    }
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  *p = c;
  ...
  for (i = 0; i < (int) MAXSUFFIXCLASS; i++)
    {
      if (!strncmp
	  (suffixclass[i].name, input_line_pointer, suffixclass[i].len))
	{
	  suffixcode = i;
  ...
  for (i = 0; i < (int) MAXSYNTAXCLASS; i++)
    {
      if (!strncmp
	  (syntaxclass[i].name, input_line_pointer, syntaxclass[i].len))
	{
  ...
  strcat (syntax,
	  ((opcode ==
	    0x3) ? "%a,%b" : ((class & SYNTAX_3OP) ? "%a,%b,%c" : "%b,%c")));
  if (suffixcode < 2)
    strcat (syntax, "%F");
  strcat (syntax, "%S%L");
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  *input_line_pointer = c;
  ...
  if (mach_type_specified_p && mach != arc_mach_type)
	...
	as_warn
	  ("\".option\" directive overrides command-line (default) value");
      arc_mach_type = mach;
      if (!bfd_set_arch_mach (stdoutput, bfd_arch_arc, mach))
