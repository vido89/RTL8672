  ...
  for (i = 0; i < num_opcodes; i++)
      ...
      if (!strcmp (name, machine_opcodes[i + 1].name))
	{
	  if ((machine_opcodes[i].opcode & 0x01000000) != 0
	    ...
	    goto bad_table;
	  strend =
	    machine_opcodes[i].args + strlen (machine_opcodes[i].args) - 1;
	  strend2 =
	    machine_opcodes[i + 1].args +
	    strlen (machine_opcodes[i + 1].args) - 1;
	  switch (*strend)
	    {
	    case 'b':
	      if (*strend2 != 'i')
     ...
     char *str;
{
  ...
  if (the_insn.reloc != NO_RELOC)
    {
      fix_new_exp (frag_now, (toP - frag_now->fr_literal + the_insn.reloc_offset), 4,	/* size */
		   &the_insn.exp, the_insn.pcrel, the_insn.reloc);
    }
}
     ...
     char *str;
{
  ...
  for (args = insn->args;; ++args)
	...
	case 'n':		/* Control bits for LOAD/STORE instructions */
	  if (operand->X_op == O_constant && operand->X_add_number < 128)
	    {
	      opcode |= (operand->X_add_number << 16);
	      continue;
	    }
	...
	case 'r':		/* RND bits of CONVERT */
	  if (operand->X_op == O_constant && operand->X_add_number < 8)
	    {
	...
	case 'I':		/* ID bits of INV and IRETINV.  */
	  ...
	  if (operand->X_op == O_absent)
	    continue;
	  else if (operand->X_op == O_constant && operand->X_add_number < 4)
	    {
	      opcode |= operand->X_add_number << 16;
	      continue;
	    }
	...
	case 'd':		/* FD bits of CONVERT */
	  if (operand->X_op == O_constant && operand->X_add_number < 4)
	    {
	...
	case 'F':
	  if (operand->X_op == O_constant && operand->X_add_number < 16)
	    {
     ...
     relax_addressT segment_address_in_file;
{
  ...
  where[7] =
    (((!S_IS_DEFINED (fixP->fx_addsy)) << 7) & 0x80) | (0 & 0x60) | (fixP->
								     fx_r_type
								     & 0x1F);
  /* Also easy */
     ...
     char *name;
{
  ...
  if (name[0] == 'g' || name[0] == 'G'
      || name[0] == 'l' || name[0] == 'L' || name[0] == 's' || name[0] == 'S')
    {
     ...
     expressionS *expressionP;
{
  ...
  if (input_line_pointer[0] == '%' && input_line_pointer[1] == '%')
    {
      ...
      if (expressionP->X_op != O_constant || expressionP->X_add_number > 255)
  ...
  else if (input_line_pointer[0] == '$' && ISDIGIT (input_line_pointer[1]))
    {
      long lab;
      char *name;
      symbolS *sym;
