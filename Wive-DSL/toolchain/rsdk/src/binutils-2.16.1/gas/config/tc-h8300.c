...
md_begin (void)
{
  ...
  while (p1)
      ...
      for (p = p1; p->name; p++)
	{
	  ...
	  pi->noperands = 0;
	  while (pi->noperands < 3
		 && p->args.nib[pi->noperands] != (op_type) E)
	    pi->noperands++;
...
static const struct h8_instruction *get_specific (const struct h8_instruction
						  *, struct h8_op *, int);
...
parse_reg (char *src, op_type * mode, unsigned int *reg, int direction)
{
  ...
  if (len == 3 &&
      TOLOWER (src[0]) == 'c' &&
      TOLOWER (src[1]) == 'c' && TOLOWER (src[2]) == 'r')
    {
      *mode = CCR;
      *reg = 0;
      return len;
  ...
  if (len == 3 &&
      TOLOWER (src[0]) == 'e' &&
      TOLOWER (src[1]) == 'x' && TOLOWER (src[2]) == 'r')
    {
      *mode = EXR;
      *reg = 1;
      return len;
  ...
  if (len == 3 &&
      TOLOWER (src[0]) == 'v' &&
      TOLOWER (src[1]) == 'b' && TOLOWER (src[2]) == 'r')
    {
      *mode = VBR;
      *reg = 6;
      return len;
...
get_operand (char **ptr, struct h8_op *op, int direction)
{
  ...
  if (TOLOWER (src[0]) == 'e' && TOLOWER (src[1]) == 'r' &&
      ...
      TOLOWER (src[4]) == 'e' && TOLOWER (src[5]) == 'r' && ISDIGIT (src[6]))
    {
	  ...
	  && !(low == 2
	       && (high == 3 || ((high == 4 || high == 5) && SXmode)))
	  && !(low == 3 && (high == 4 || high == 5 || high == 6) && SXmode)
	  && !(low == 4 && (high == 5 || high == 6)) && !(low == 4
							  && high == 7
							  && SXmode)
	  && !(low == 5 && (high == 6 || high == 7) && SXmode) && !(low == 6
								    && high ==
								    7
								    &&
								    SXmode))
  ...
  if (*src == '@')
      ...
      if (*src == '(')
	{
	  ...
	  if (src[0] == '.')
		...
		default:
		  as_bad (_
			  ("expected .L, .W or .B for register in indexed addressing mode"));
		}
	      src += 2;
	      op->reg &= 7;
	    }
  ...
  if (*src == '#')
    {
      ...
      *ptr = parse_exp (src + 1, op);
      return;
	   ...
	   strncmp (src, "MACH", 4) == 0 || strncmp (src, "MACL", 4) == 0)
    {
...
error:
  as_bad (_
	  ("expected valid addressing mode for mova: \"@(disp, ea.sz),ERn\""));
}
	      ...
	      struct h8_op *operands, int size)
{
	  ...
	  for (i = 0; i < this_try->noperands && found; i++)
	      ...
	      if (op_mode == LOWREG && (x_mode == REG || x_mode == LOWREG))
		{
		  if ((x_size == L_8 && (operands[i].reg & 8) == 0)
		    ...
		    as_warn (_
			     ("can't use high part of register in operand %d"),
			     i);
	      ...
	      else if (op_mode & CTRL)	/* control register */
		{
		  ...
		  switch (x_mode)
		    ...
		    case CCR:
		      if (op_mode != CCR &&
			  op_mode != CCR_EXR && op_mode != CC_EX_VB_SB)
			found = 0;
		      break;
		    case EXR:
		      if (op_mode != EXR &&
			...
			found = 0;
		      break;
		    case MACH:
		      if (op_mode != MACH && op_mode != MACREG)
			found = 0;
		      break;
		    case MACL:
		      if (op_mode != MACL && op_mode != MACREG)
			found = 0;
		      break;
		    case VBR:
		      if (op_mode != VBR &&
			  op_mode != VBR_SBR && op_mode != CC_EX_VB_SB)
			found = 0;
		      break;
		    case SBR:
		      if (op_mode != SBR &&
  ...
  if (found_other)
    {
      as_warn (_
	       ("Opcode `%s' with these operand types not available in %s mode"),
	       found_other->opcode->name, (!Hmode
					   && !Smode ? "H8/300" : SXmode ?
					   "H8sx" : Smode ? "H8/300S" :
					   "H8/300H"));
    }
...
{
  if (operand->exp.X_add_symbol == 0 && operand->exp.X_op_symbol == 0)
    {
	...
	{
	  if (width == 255 && (operand->exp.X_add_number & 0xff00) == 0xff00)
	    {
...
do_a_fix_imm (int offset, int nibble, struct h8_op *operand, int relaxmode)
{
  ...
  if (operand->exp.X_add_symbol == 0)
	...
	case L_8:
      ...
      fix_new_exp (frag_now, offset + where, size, &operand->exp, 0, idx);
    }
}
...
build_bytes (const struct h8_instruction *this_try, struct h8_op *operand)
{
  ...
  if (!Hmode && this_try->opcode->available != AV_H8)
    as_warn (_
	     ("Opcode `%s' with these operand types not available in H8/300 mode"),
	     this_try->opcode->name);
  else if (!Smode && this_try->opcode->available != AV_H8
	   && this_try->opcode->available != AV_H8H)
    as_warn (_
	     ("Opcode `%s' with these operand types not available in H8/300H mode"),
	     this_try->opcode->name);
  else if (!SXmode && this_try->opcode->available != AV_H8
	   && this_try->opcode->available != AV_H8H
	   && this_try->opcode->available != AV_H8S)
    as_warn (_
	     ("Opcode `%s' with these operand types not available in H8/300S mode"),
	     this_try->opcode->name);
  ...
  for (i = 0; i < this_try->noperands; i++)
      ...
      else if (x_mode == MEMIND)
	{
	  ...
	  fix_new_exp (frag_now,
		       ...
		       1, &operand[i].exp, 0, R_MEM_INDIRECT);
	}
  ...
  if ((instruction + 1)->idx != instruction->idx)
      ...
      for (argn = 0; argn < instruction->noperands; argn++)
	{
	  switch (instruction->opcode->args.nib[argn])
		...
		{
		  as_bad (_
			  ("destination operand must be 16bit absolute address"));
		  return;
		}
	      break;
	    case RD8:
...
md_assemble (char *str)
{
  ...
  for (op_start = op_end = str; *op_end != 0 && *op_end != ' '; op_end++)
    {
      if (*op_end == '.')
	{
	  dot = op_end + 1;
  ...
  if (OP_KIND (instruction->opcode->how) == O_LDM)
    {
	  ...
	  || (operand[0].reg != 7) || ((operand[1].reg & 0x80000000) == 0))
