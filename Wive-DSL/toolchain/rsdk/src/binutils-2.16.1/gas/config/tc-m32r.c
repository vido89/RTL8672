...
#define OPTION_NO_WARN_UNMATCHED  (OPTION_WARN_UNMATCHED + 1)
  ...
  {"warn-explicit-parallel-conflicts", no_argument, NULL,
     ...
     const int check_outputs;
{
  ...
  for (a_index = 0;
       a_operands->type != CGEN_OPINST_END; a_index++, a_operands++)
    {
	  ...
	  if (OPERAND_IS_COND_BIT (a_operands, a->indices, a_index))
		   ...
		   b_index++, b_operands++)
		{
		  if ((b_operands->type
			   ...
			   ? CGEN_OPINST_OUTPUT
			   : CGEN_OPINST_INPUT))
		      && OPERAND_IS_COND_BIT (b_operands, b->indices,
					      b_index))
		    return 1;
		}
	    }
     ...
     m32r_insn *b;
{
  ...
  if (a_pipe == PIPE_NONE || b_pipe == PIPE_NONE)
  ...
  if (a_pipe == PIPE_S
      || b_pipe == PIPE_O || (b_pipe == PIPE_O_OS && (enable_m32rx != 2)))
  ...
  *str2 = save_str2;
  ...
  if (CGEN_FIELDS_BITSIZE (&second.fields) != 16)
...
#endif
    {
      ...
      return;
	   ...
	   && CGEN_INSN_ATTR_VALUE (second.insn,
				    CGEN_INSN_MACH) == (1 << MACH_M32RX))
    {
      /* xgettext:c-format  */
      as_bad (_("instruction '%s' is for the M32RX only"), str);
      return;
  ...
  if (parallel_p && !enable_m32rx)
	  ...
	  && CGEN_INSN_NUM (second.insn) != M32R_INSN_NOP)
	{
	  /* xgettext:c-format  */
	  as_bad (_
		  ("'%s': only the NOP instruction can be issued in parallel on the m32r"),
		  str2);
	  return;
	}
    }
  ...
  if (parallel_p && warn_explicit_parallel_conflicts)
	...
	/* xgettext:c-format  */
	as_warn (_
		 ("%s: output of 1st instruction is the same as an input to 2nd instruction - is this intentional ?"),
		 str2);

      if (first_writes_to_seconds_operands (&second, &first, FALSE))
	/* xgettext:c-format  */
	as_warn (_
		 ("%s: output of 2nd instruction is the same as an input to 1st instruction - is this intentional ?"),
		 str2);
    }
...
#else
  if (!enable_special && CGEN_INSN_ATTR_VALUE (insn.insn, CGEN_INSN_SPECIAL))
#endif
    {
      /* xgettext:c-format  */
      as_bad (_("unknown instruction '%s'"), str);
      return;
	   ...
	   && CGEN_INSN_ATTR_VALUE (insn.insn,
				    CGEN_INSN_MACH) == (1 << MACH_M32RX))
    {
      /* xgettext:c-format  */
      as_bad (_("instruction '%s' is for the M32RX only"), str);
  ...
  if (S_GET_VALUE (symbolP) && S_GET_VALUE (symbolP) != (valueT) size)
    {
      /* xgettext:c-format  */
      as_bad (_
	      ("Length of .scomm \"%s\" is already %ld. Not changed to %ld."),
	...
	for (i = 0, insn = fragP->fr_cgen.insn; i < 4; i++, insn++)
	  {
	    if ((strcmp (CGEN_INSN_MNEMONIC (insn),
		 ...
		 == 0) && CGEN_INSN_ATTR_VALUE (insn, CGEN_INSN_RELAXED))
	      break;
	  }
	if (i == 4)
	  abort ();
  ...
  if (S_GET_SEGMENT (fragP->fr_symbol) != sec
      || S_IS_EXTERNAL (fragP->fr_symbol) || S_IS_WEAK (fragP->fr_symbol))
    {
      ...
      fixP = gas_cgen_record_fixup (fragP,
				    ...
				    fragP->fr_cgen.insn, 4 /* Length.  */ ,
				    /* FIXME: quick hack.  */
				    cgen_operand_lookup_by_num
				    (gas_cgen_cpu_desc, M32R_OPERAND_DISP24),
...
{
  if (exp->X_op == O_PIC_reloc && exp->X_md == BFD_RELOC_M32R_26_PLTREL)
    {
      exp->X_op = O_symbol;
      opinfo = exp->X_md;
    }
