...
static int tic4x_gen_to_words PARAMS ((FLONUM_TYPE, LITTLENUM_TYPE *, int));
static char *tic4x_atof PARAMS ((char *, char, LITTLENUM_TYPE *));
static void tic4x_insert_reg PARAMS ((char *, int));
static void tic4x_insert_sym PARAMS ((char *, int));
static char *tic4x_expression PARAMS ((char *, expressionS *));
static char *tic4x_expression_abs PARAMS ((char *, offsetT *));
static void tic4x_emit_char PARAMS ((char, int));
static void tic4x_seg_alloc PARAMS ((char *, segT, int, symbolS *));
static void tic4x_asg PARAMS ((int));
static void tic4x_bss PARAMS ((int));
static void tic4x_globl PARAMS ((int));
static void tic4x_cons PARAMS ((int));
static void tic4x_stringer PARAMS ((int));
static void tic4x_eval PARAMS ((int));
static void tic4x_newblock PARAMS ((int));
static void tic4x_sect PARAMS ((int));
static void tic4x_set PARAMS ((int));
static void tic4x_usect PARAMS ((int));
static void tic4x_version PARAMS ((int));
static void tic4x_init_regtable PARAMS ((void));
static void tic4x_init_symbols PARAMS ((void));
static int tic4x_inst_insert PARAMS ((tic4x_inst_t *));
static tic4x_inst_t *tic4x_inst_make PARAMS ((char *, unsigned long, char *));
static int tic4x_inst_add PARAMS ((tic4x_inst_t *));
void tic4x_end PARAMS ((void));
static int tic4x_indirect_parse
PARAMS ((tic4x_operand_t *, const tic4x_indirect_t *));
static char *tic4x_operand_parse PARAMS ((char *, tic4x_operand_t *));
static int tic4x_operands_match
PARAMS ((tic4x_inst_t *, tic4x_insn_t *, int));
static void tic4x_insn_check PARAMS ((tic4x_insn_t *));
static void tic4x_insn_output PARAMS ((tic4x_insn_t *));
static int tic4x_operands_parse PARAMS ((char *, tic4x_operand_t *, int));
void tic4x_cleanup PARAMS ((void));
int tic4x_unrecognized_line PARAMS ((int));
static int tic4x_pc_offset PARAMS ((unsigned int));
int tic4x_do_align PARAMS ((int, const char *, int, int));
void tic4x_start_line PARAMS ((void));
arelent *tc_gen_reloc PARAMS ((asection *, fixS *));
  ...
  if (rbit)
    {
      /* If the mantissa is going to overflow when added, lets store
         ...
         test. */
      if (smant == ((unsigned) (1 << (mantissa_bits + 1)) - 1) || smant == (unsigned) -1)	/* This is to catch E_PRECISION cases */
	mover = 1;
      smant++;
    }
     ...
     LITTLENUM_TYPE *words;
{
  ...
  tic4x_gen_to_words (generic_floating_point_number, words, precision);
  ...
  if (size)
    {
      ...
      p = frag_var (rs_fill, 1, 1, (relax_substateT) 0,
		    (symbolS *) symbolP, size * OCTETS_PER_BYTE, (char *) 0);
      *p = 0;
    }
}
     ...
     int x ATTRIBUTE_UNUSED;
{
  ...
  input_line_pointer = tic4x_expression_abs (++input_line_pointer, &size);
  if (size < 0)
    {
     ...
     int x ATTRIBUTE_UNUSED;
{
  ...
  SKIP_WHITESPACE ();
  input_line_pointer = tic4x_expression_abs (input_line_pointer, &value);
  if (*input_line_pointer++ != ',')
    {
     ...
     int x ATTRIBUTE_UNUSED;
{
  ...
  if (c == ',')
    input_line_pointer = tic4x_expression_abs (input_line_pointer, &num);
     ...
     int x ATTRIBUTE_UNUSED;
{
  ...
  input_line_pointer = tic4x_expression_abs (input_line_pointer, &temp);
...
tic4x_init_regtable ()
{
  ...
  for (i = 0; i < tic3x_num_registers; i++)
    tic4x_insert_reg (tic3x_registers[i].name, tic3x_registers[i].regno);

  if (IS_CPU_TIC4X (tic4x_cpu))
    {
      /* Add additional Tic4x registers, overriding some C3x ones.  */
      for (i = 0; i < tic4x_num_registers; i++)
	tic4x_insert_reg (tic4x_registers[i].name, tic4x_registers[i].regno);
    }
}
...
tic4x_init_symbols ()
{
  ...
  tic4x_insert_sym (".C3X", tic4x_cpu == 30 || tic4x_cpu == 31
		    || tic4x_cpu == 32 || tic4x_cpu == 33);
  tic4x_insert_sym (".C3x", tic4x_cpu == 30 || tic4x_cpu == 31
		    || tic4x_cpu == 32 || tic4x_cpu == 33);
  tic4x_insert_sym (".C4X", tic4x_cpu == 0 || tic4x_cpu == 40
		    || tic4x_cpu == 44);
  tic4x_insert_sym (".C4x", tic4x_cpu == 0 || tic4x_cpu == 40
		    || tic4x_cpu == 44);
  /* Do we need to have the following symbols also in lower case?  */
  tic4x_insert_sym (".TMS320C30", tic4x_cpu == 30 || tic4x_cpu == 31
		    || tic4x_cpu == 32 || tic4x_cpu == 33);
  tic4x_insert_sym (".tms320C30", tic4x_cpu == 30 || tic4x_cpu == 31
		    || tic4x_cpu == 32 || tic4x_cpu == 33);
  ...
  tic4x_insert_sym (".tms320C33", tic4x_cpu == 33);
  tic4x_insert_sym (".TMS320C40", tic4x_cpu == 40 || tic4x_cpu == 44
		    || tic4x_cpu == 0);
  tic4x_insert_sym (".tms320C40", tic4x_cpu == 40 || tic4x_cpu == 44
		    || tic4x_cpu == 0);
  ...
  if (insts == NULL)
    {
      ...
      insts = (tic4x_inst_t *) xmalloc (sizeof (tic4x_inst_t) * 1024);
    }
	  ...
	  for (i = 0; i < tic4x_num_conds; i++)
		...
		*e++ = *c++;
	      ...
	      do
		{
	      ...
	      while (!strcmp (insts->name, insts[k].name));
	    }
	  return ok;
	  break;
...
tic4x_end ()
{
  bfd_set_arch_mach (stdoutput, bfd_arch_tic4x,
		     IS_CPU_TIC4X (tic4x_cpu) ? bfd_mach_tic4x :
		     bfd_mach_tic3x);
}
  ...
  for (;; ++args)
	...
	case 'e':
	  ...
	  if ((reg >= REG_R0 && reg <= REG_R7)
	      || (IS_CPU_TIC4X (tic4x_cpu) && reg >= REG_R8
		  && reg <= REG_R11))
	...
	case 'i':
	  if (operand->mode == M_REGISTER && tic4x_oplevel & OP_ENH)
	    {
	      reg = exp->X_add_number;
     ...
     tic4x_insn_t *insn;
{
  ...
  if (!strcmp (insn->name, "lda"))
	...
	as_fatal ("Illegal internal LDA insn definition");
	  ...
	  && insn->operands[0].expr.X_add_number ==
	  insn->operands[1].expr.X_add_number)
	...
	as_fatal ("Illegal internal %s insn definition", insn->name);
	  ...
	  && insn->operands[1].expr.X_add_number ==
	  insn->operands[insn->num_operands - 1].expr.X_add_number)
	as_warn
	  ("Equal parallell destination registers, one result will be discarded");
    }
}
  ...
  if (insn->reloc != NO_RELOC)
    {
      ...
      fix_new_exp (frag_now, dst - frag_now->fr_literal,	/* where */
		   insn->nchars,	/* size */
		   &insn->exp, insn->pcrel, insn->reloc);
    }
}
  ...
  if (str)
      ...
      if (*s && s[1] == '|')
	{
	  if (insn->parallel)
	    {
	      as_bad
		("Parallel opcode cannot contain more than two instructions");
	      insn->parallel = 0;
	      insn->in_use = 0;
	      return;
	    }
  ...
  *sizeP = prec * sizeof (LITTLENUM_TYPE);
  ...
  for (wordP = words; wordP < (words + prec); wordP += 2)
      ...
      if (wordP < (words + prec - 1))	/* Dump wordP[1] (if we have one) */
	{
      ...
      /* Dump wordP[0] */
      md_number_to_chars (litP, (valueT) (wordP[0]), sizeof (LITTLENUM_TYPE));
      litP += sizeof (LITTLENUM_TYPE);
    }
  return 0;
}
     ...
     int max ATTRIBUTE_UNUSED;
{
  ...
  if (alignment != 0 && !need_pass_2)
      ...
      if (fill == NULL)
	{
	  ...
	  frag_align_pattern (alignment, (const char *) &nop, sizeof (nop),
			      max);
	  return 1;
	  /*else
