...
static void
get_ctrl_operand (char **ptr, struct z8k_op *mode,
...
static void
get_flags_operand (char **ptr, struct z8k_op *mode,
		   unsigned int dst ATTRIBUTE_UNUSED)
{
  char *src = *ptr;
  char c;
...
static void
get_interrupt_operand (char **ptr, struct z8k_op *mode,
		       unsigned int dst ATTRIBUTE_UNUSED)
{
  char *src = *ptr;
  int i, l;
...
static void
get_cc_operand (char **ptr, struct z8k_op *mode,
		unsigned int dst ATTRIBUTE_UNUSED)
{
  char *src = *ptr;
  int i, l;
...
static void
get_operand (char **ptr, struct z8k_op *mode,
	     unsigned int dst ATTRIBUTE_UNUSED)
{
  char *src = *ptr;
  char *end;
...
static char *
get_operands (const opcode_entry_type * opcode, char *op_end,
	      op_type * operand)
{
  char *ptr = op_end;
  char *savptr;
  ...
  while (this_index == opcode->idx && !found)
      ...
      for (i = 0; i < noperands; i++)
	{
	  ...
	  if (((mode & CLASS_MASK) == CLASS_IR)
	      && ((this_try->arg_info[i] & CLASS_MASK) == CLASS_IRO))
	    {
	      mode = operands[i].mode =
		(operands[i].mode & ~CLASS_MASK) | CLASS_IRO;
	    }
	  ...
	  switch (mode & CLASS_MASK)
	    ...
	    case CLASS_IR:
		  ...
		  || (!segmented_mode
		      && operands[i].regsize != CLASS_REG_WORD))
		as_bad (_("invalid indirect register size"));
	      reg[this_try->arg_info[i] & ARG_MASK] = operands[i].reg;
	      break;
	    case CLASS_X:
...
newfix (int ptr, int type, int size, expressionS * operand)
{
  ...
  if (operand->X_add_symbol || operand->X_op_symbol || operand->X_add_number)
    {
      switch (type)
	{
	case R_JR:
	...
	case R_CALLR:
      ...
      fix_new_exp (frag_now, ptr, size / 2, operand, is_pcrel, type);
    }
}
...
static void
build_bytes (opcode_entry_type * this_try,
...
tc_reloc_mangle (fixS * fix_ptr, struct internal_reloc *intr, bfd_vma base)
{
  ...
  if (fix_ptr->fx_addsy && fix_ptr->fx_subsy)
    {
