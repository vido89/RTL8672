...
static int get_spec_reg PARAMS ((char **, const struct cris_spec_reg **));
static int get_sup_reg PARAMS ((char **, int *));
static int get_autoinc_prefix_or_indir_op PARAMS ((char **,
						   struct cris_prefix *,
						   int *, int *, int *,
						   expressionS *));
static int get_3op_or_dip_prefix_op PARAMS ((char **, struct cris_prefix *));
...
#define HANDLE_RELAXABLE(state)						\
  ...
  switch (fragP->fr_subtype)
    ...
    case ENCODE_RELAX (STATE_LAPC, STATE_UNDF):
      if (symbolP != NULL
	  && S_GET_SEGMENT (symbolP) == segment_type && !S_IS_WEAK (symbolP))
	{
	...
	/* Unknown or not the same segment, so not relaxable.  */
	fragP->fr_subtype = ENCODE_RELAX (STATE_LAPC, STATE_DWORD);
 ...
 a multiple of two.  Use LAPC.D"));
  ...
  Please report this."), offset);
	...
	md_number_to_chars (opcodep,
			    LAPC_DWORD_OPCODE + (opcodep[1] & 0xf0) * 256, 2);
     ...
     char *str;
{
  ...
  if (output_instruction.insn_type == CRIS_INSN_BRANCH)
	      ...
	      && output_instruction.opcode == BA_QUICK_OPCODE))
	{
	  ...
	  valueT addvalue
	    = (SIMPLE_EXPR (&output_instruction.expr)
	       ? output_instruction.expr.X_add_number : 0);
	  symbolS *sym
	    = (SIMPLE_EXPR (&output_instruction.expr)
	  ...
	  frag_var (rs_machine_dependent,
		    ...
		    ENCODE_RELAX (cris_arch == arch_crisv32
				  ...
				  : (cris_arch == arch_cris_common_v10_v32
				     ...
				     : STATE_COND_BRANCH),
				  length_code), sym, addvalue, opcodep);
	}
      else
	{
    ...
    frag_variant (rs_machine_dependent, 0, 0,
		  ENCODE_RELAX (STATE_MUL, STATE_BYTE), NULL, 0, opcodep);
  else
    {
	  ...
	  switch (*args)
	      ...
	      else
		{
		  if (out_insnp->expr.X_op == O_constant
		    ...
		    as_bad (_
			    ("Immediate value not in 4 bit unsigned range: %ld"),
			    out_insnp->expr.X_add_number);
	      ...
	      else
		{
		  if (out_insnp->expr.X_op == O_constant
		    ...
		    as_bad (_
			    ("Immediate value not in 6 bit unsigned range: %ld"),
			    out_insnp->expr.X_add_number);
	      ...
	      else
		{
		  ...
		  out_insnp->opcode |= out_insnp->spec_reg->number << 12;
		  continue;
		}
	    ...
	    case 'T':
	      if (cris_arch == arch_crisv32 && get_sup_reg (&s, &regno))
		{
		  out_insnp->opcode |= regno << 12;
		  continue;
		}
		...
		case SIZE_SPEC_REG:
		  if (cris_arch == arch_crisv32)
			...
			if (out_insnp->expr.X_op == O_constant
			  ...
			  as_bad (_
				  ("Immediate value not in 8 bit range: %ld"),
				  out_insnp->expr.X_add_number);
			/* Fall through.  */
		      case 2:
			...
			if (out_insnp->expr.X_op == O_constant
			  ...
			  as_bad (_
				  ("Immediate value not in 16 bit range: %ld"),
				  out_insnp->expr.X_add_number);
		      ...
		      if (out_insnp->expr.X_op == O_constant)
			{
			  if (instruction->imm_oprnd_size == SIZE_FIELD
			    ...
			    as_bad (_
				    ("Immediate value not in 8 bit range: %ld"),
				    out_insnp->expr.X_add_number);
			  else if (instruction->imm_oprnd_size ==
				   SIZE_FIELD_SIGNED
				   && (out_insnp->expr.X_add_number < -128
				       || out_insnp->expr.X_add_number > 127))
			    as_bad (_
				    ("Immediate value not in 8 bit signed range: %ld"),
				    out_insnp->expr.X_add_number);
			  else if (instruction->imm_oprnd_size ==
				   SIZE_FIELD_UNSIGNED
				   && (out_insnp->expr.X_add_number < 0
				       || out_insnp->expr.X_add_number > 255))
			    as_bad (_
				    ("Immediate value not in 8 bit unsigned range: %ld"),
				    out_insnp->expr.X_add_number);
			}
			  ...
			  if (instruction->imm_oprnd_size == SIZE_FIELD
			    ...
			    as_bad (_
				   ...
				   && (out_insnp->expr.X_add_number < -32768
				       ...
				       32767))
			    as_bad (_
				    ("Immediate value not in 16 bit signed range: %ld"),
				    out_insnp->expr.X_add_number);
			  else if (instruction->imm_oprnd_size ==
				   SIZE_FIELD_UNSIGNED
				   && (out_insnp->expr.X_add_number < 0
				       ...
				       65535))
			    as_bad (_
				    ("Immediate value not in 16 bit unsigned range: %ld"),
				    out_insnp->expr.X_add_number);
			}
		      out_insnp->imm_oprnd_size = 2;
		      break;
      ...
      if (**cPP == '+')
	{
	  ...
	  if (**cPP == '[')
		...
		return 0;
	      ...
	      prefixp->opcode
		= (BDAP_INDIR_OPCODE
		   + (prefixp->base_reg_number << 12) + index_reg_number);
	  ...
	  /* Seen "[rN+", but not a '[', so check if we have a
	  ...
	  else if (get_gen_reg (cPP, &index_reg_number))
	    {
	      ...
	      int size_bits;
	      prefixp->kind = PREFIX_BIAP;
	      prefixp->opcode = (BIAP_OPCODE | prefixp->base_reg_number	/* << 0 */
				 | (index_reg_number << 12));
     ...
     int *flagsp;
{
  for (;;)
	...
	case 'd':
	case 'D':
	  if (!cris_insn_ver_valid_for_arch (cris_ver_v0_3, cris_arch))
	    return 0;
	  *flagsp |= 0x80;
	  break;
	...
	case 'm':
	case 'M':
	  if (!cris_insn_ver_valid_for_arch (cris_ver_v8_10, cris_arch))
	    return 0;
	  *flagsp |= 0x80;
	  break;
	...
	case 'p':
	case 'P':
	  if (!cris_insn_ver_valid_for_arch (cris_ver_v32p, cris_arch))
	    return 0;
	  *flagsp |= 0x80;
	  break;
     ...
     expressionS *exprP;
{
  ...
  if (exprP->X_op == O_constant)
    ...
    {
      /* Handle complex expressions.  */
      valueT addvalue = SIMPLE_EXPR (exprP) ? exprP->X_add_number : 0;
      symbolS *sym
	= (SIMPLE_EXPR (exprP)
     ...
     int *sizep ATTRIBUTE_UNUSED;
{
  /* FIXME:  Is this function mentioned in the internals.texi manual?  If
     not, add it.  */
  return
    _("Bad call to md_atof () - floating point formats are not supported");
}
     ...
     segT seg;
{
  ...
  if (fixP->fx_addsy && (sym_seg = S_GET_SEGMENT (fixP->fx_addsy)) != seg)
    val += sym_seg->vma;
     ...
     fixS *fixP;
{
  ...
  switch (fixP->fx_r_type)
    ...
    default:
      as_bad_where (fixP->fx_file, fixP->fx_line,
		    _
		    ("Semantics error.  This type of operand can not be relocated, it must be an assembly-time constant"));
      return 0;
    }
     ...
     char **str;
{
  ...
  for (ap = arch_table;
       ap < arch_table + sizeof (arch_table) / sizeof (arch_table[0]); ap++)
    {
     ...
     enum cris_archs arch;
{
  switch (arch)
	...
	(iver == cris_ver_version_all
	 ...
	 || iver == cris_ver_v8p
	 || iver == cris_ver_v8_10
	 || iver == cris_ver_v10 || iver == cris_ver_v10p);
	...
	(iver == cris_ver_version_all
	 || iver == cris_ver_v3p
	 || iver == cris_ver_v8p
	 || iver == cris_ver_v10p || iver == cris_ver_v32p);

    case arch_cris_common_v10_v32:
      return
	(iver == cris_ver_version_all
	 || iver == cris_ver_v3p
	 || iver == cris_ver_v8p || iver == cris_ver_v10p);

    case arch_crisv0:
      return
	(iver == cris_ver_version_all
	 || iver == cris_ver_v0_3
	 || iver == cris_ver_v0_10 || iver == cris_ver_sim_v0_10);
	...
	(iver == cris_ver_version_all
	 ...
	 || iver == cris_ver_v3p
	 || iver == cris_ver_v0_10
	 || iver == cris_ver_sim_v0_10 || iver == cris_ver_v3_10);
	...
	(iver == cris_ver_version_all
	 ...
	 || iver == cris_ver_v3_10
	 || iver == cris_ver_v8
	 || iver == cris_ver_v8p || iver == cris_ver_v8_10);
