...
s_enddual (int ignore ATTRIBUTE_UNUSED)
{
  if (target_intel_syntax)
    ...
    as_bad (_
	    ("Directive .enddual available only with -mintel-syntax option"));
}
  ...
  for (;;)
      ...
      for (args = insn->args;; ++args)
	{
	  ...
	  switch (*args)
	    ...
	    case 'T':
	      the_insn.fi[fc].fup |=
		(OP_IMM_SPLIT16 | OP_ENCODE1 | OP_ALIGN2);
	      goto immediate;

	      /* 16-bit split immediate, word aligned (st.l).  */
	    case 'U':
	      the_insn.fi[fc].fup |=
		(OP_IMM_SPLIT16 | OP_ENCODE1 | OP_ALIGN4);
	      goto immediate;
	...
	{
	  /* Args don't match.  */
	  if (insn[1].name != NULL && !strcmp (insn->name, insn[1].name))
	    {
	      ++insn;
	      s = args_start;
	      continue;
