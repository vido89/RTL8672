...
PARAMS ((const struct reg_name *, int, const char *));
     ...
     expressionS *expressionP;
{
  ...
  c = get_symbol_end ();
  reg_number =
    reg_name_search (address_registers, ADDRESS_REG_NAME_CNT, name);
     ...
     expressionS *expressionP;
{
  ...
  *input_line_pointer = c;
  ...
  /* Look to see if it's in the register table.  */
  if (reg_number == 0 || (reg_number == AM33 && HAVE_AM33))
    {
      expressionP->X_op = O_register;
...
repeat:
  switch (exp->X_op)
    ...
    case O_PIC_reloc:
	  ...
	  && fixup->opindex >= 0
	  && (mn10300_operands[fixup->opindex].flags & MN10300_OPERAND_RELAX))
	return 1;
      break;
     ...
     char *str;
{
  ...
  for (;;)
	...
	goto error;
	   ...
	   *opindex_ptr != 0; opindex_ptr++, op_idx++)
	{
	  ...
	  /* See if we can match the operands.  */
	      ...
	      while (*input_line_pointer != ']')
		{
		  ...
		  else if (HAVE_AM33 && strcasecmp (start, "exreg0") == 0)
		    {
		      value |= 0x04;
		      *input_line_pointer = c;
		    }
		  else if (HAVE_AM33 && strcasecmp (start, "exreg1") == 0)
		    {
		      value |= 0x02;
		      *input_line_pointer = c;
		    }
		  else if (HAVE_AM33 && strcasecmp (start, "exother") == 0)
		    {
		      value |= 0x01;
		      *input_line_pointer = c;
		    }
		  else if (HAVE_AM33 && strcasecmp (start, "all") == 0)
		    {
		      value |= 0xff;
		      *input_line_pointer = c;
		    }
		  ...
		  for (j = i + 1; j < MN10300_MAX_OPERANDS; j++)
		    {
			  ...
			  && mn10300_reg_operands[i] ==
			  mn10300_reg_operands[j])
			{
  ...
  if (relaxable && fc > 0)
	...
	type = 3;
      ...
      f = frag_var (rs_machine_dependent, 8, 8 - size, type,
		    ...
		    fixups[0].exp.X_add_number, (char *) fixups[0].opindex);
  ...
  else
    {
	  ...
	  || opcode->format == FMT_D10 || opcode->format == FMT_D1)
	{
	  number_to_chars_bigendian (f, insn, size);
	}
      else if (opcode->format == FMT_S2
	       && opcode->opcode != 0xdf0000 && opcode->opcode != 0xde0000)
	{
      ...
      else if (opcode->format == FMT_S2)
	{
      ...
      else if (opcode->format == FMT_S4 && opcode->opcode != 0xdc000000)
	{
     ...
     fixS *fixp;
{
  ...
  if (fixp->fx_subsy && S_GET_SEGMENT (fixp->fx_subsy) == absolute_section)
    {
     ...
     unsigned int shift;
{
  ...
  if (operand->bits != 32 && (operand->flags & MN10300_OPERAND_SPLIT) == 0)
    {
      long min, max;
      offsetT test;
      int bits;
