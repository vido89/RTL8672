...
md_estimate_size_before_relax (fragS * fragP, segT segment)
{
  ...
  as_fatal (_
	    ("Illegal Reloc type in md_estimate_size_before_relax for line : %d"),
	    frag_now->fr_line);
  return 0;
}
...
#endif
{
  ...
  if ((displacement_from_opcode_start >= -128
      ...
      if (fragP->fr_subtype != SHORT_PREFIX)
	{
	  ...
	  reloc_type = (fragP->fr_subtype ? fragP->fr_subtype : LONG_PREFIX);
...
maxq20_data (char *op_string)
{
  ...
  if (symbolP != NULL
      && S_GET_SEGMENT (symbolP) != now_seg && S_GET_SEGMENT (symbolP) !=
#ifdef BFD_ASSEMBLER
      bfd_und_section_ptr
#else
      SEG_UNKNOWN
		...
		int op_num)
{
  switch (type)
    ...
    case BIT:
      if ((arg_type & ACC_BIT) == ACC_BIT
	  && !strcmp (i.maxq20_op[op_num].r_bit->reg->reg_name, "ACC"))
	return 1;
...
output_disp (fragS * insn_start_frag, offsetT insn_start_off)
{
  ...
  *(p + 1) = i.instr[0];
  ...
  frag_var (rs_machine_dependent, 2, i.reloc[this_operand], subtype, sym, off,
	    p);
}
...
md_begin (void)
{
  ...
  for (memtab = mem_table;
       memtab < mem_table + ARRAY_SIZE (mem_table); memtab++)
    {
  ...
  for (bittab = bit_table;
       bittab < bit_table + ARRAY_SIZE (bit_table); bittab++)
    {
  ...
  for (memsyntab = mem_access_syntax_table;
       memsyntab <
       mem_access_syntax_table + ARRAY_SIZE (mem_access_syntax_table);
       memsyntab++)
    {
      hash_err =
