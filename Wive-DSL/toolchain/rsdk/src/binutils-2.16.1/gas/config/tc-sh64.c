...
sh64_handle_align (fragS * frag)
{
  ...
  if (frag->tc_frag_data.isa == sh64_isa_shmedia
	...
	{
	  memcpy (p, shmedia_big_nop_pattern, sizeof shmedia_big_nop_pattern);
...
shmedia_md_apply_fix3 (fixS * fixP, valueT * valp)
{
  ...
  if (fixP->fx_pcrel)
	...
	case BFD_RELOC_SH_IMM_HI16:
	  ...
	  fixP->fx_offset = *valp + SHMEDIA_MD_PCREL_FROM_FIX (fixP);
	  break;
  ...
  switch (fragP->fr_subtype)
    ...
    case C (SH64PCREL16PT_64, SH64PCREL32):
      ...
      md_number_to_chars (var_partp + 4,
			  ...
			  | (SHMEDIA_TEMP_REG << 10) | (insn & (7 << 4)), 4);
      ...
      if (reloc_needed)
	{
	  ...
	  fix_new (fragP, var_partp - fragP->fr_literal, 4, fragP->fr_symbol,
		   fragP->fr_offset - 4, 1,
		   reloctype == BFD_RELOC_32_PLT_PCREL
		   ? BFD_RELOC_SH_PLT_LOW16 : BFD_RELOC_SH_IMM_LOW16_PCREL);
	}
      ...
      if (reloc_needed)
	{
	  ...
	  fix_new (fragP, var_partp - fragP->fr_literal + 4, 4,
		   fragP->fr_symbol, fragP->fr_offset - 4, 1,
		   reloctype ==
		   BFD_RELOC_32_PLT_PCREL ? BFD_RELOC_SH_PLT_LOW16 :
      ...
      if (reloc_needed)
	{
	  fix_new (opc_fragP, opcodep - opc_fragP->fr_literal, 4,
		   ...
		   reloctype == BFD_RELOC_32_PLT_PCREL
		   ? BFD_RELOC_SH_PLT_HI16 : BFD_RELOC_SH_IMM_HI16_PCREL);
	  fix_new (fragP, var_partp - fragP->fr_literal, 4, fragP->fr_symbol,
	  ...
	  fix_new (fragP, var_partp - fragP->fr_literal + 4, 4,
		   fragP->fr_symbol, fragP->fr_offset - 8, 1,
		   reloctype ==
		   BFD_RELOC_32_PLT_PCREL ? BFD_RELOC_SH_PLT_MEDLOW16 :
		   BFD_RELOC_SH_IMM_MEDLOW16_PCREL);
	...
	/* A SHORI, for the medium-low part.  */
	md_number_to_chars (var_partp + 4, SHMEDIA_SHORI_OPC | (reg << 4), 4);
	fix_new (fragP, var_partp - fragP->fr_literal + 4, 4,
		 fragP->fr_symbol, fragP->fr_offset, 0,
		 reloctype ==
		 BFD_RELOC_NONE ? BFD_RELOC_SH_IMM_MEDLOW16 : reloctype ==
		 BFD_RELOC_32_GOTOFF ? BFD_RELOC_SH_GOTOFF_MEDLOW16
		 : (abort (), BFD_RELOC_SH_IMM_MEDLOW16));

	/* Fill in a SHORI for the low part.  */
	md_number_to_chars (var_partp + 8, SHMEDIA_SHORI_OPC | (reg << 4), 4);
	fix_new (fragP, var_partp - fragP->fr_literal + 8, 4,
		 fragP->fr_symbol, fragP->fr_offset, 0,
		 reloctype ==
		 BFD_RELOC_NONE ? BFD_RELOC_SH_IMM_LOW16 : reloctype ==
		 BFD_RELOC_32_GOTOFF ? BFD_RELOC_SH_GOTOFF_LOW16 : (abort (),
	...
	if (reloc_needed)
	  {
	    ...
	    fix_new (fragP, var_partp - fragP->fr_literal, 4,
		     fragP->fr_symbol, fragP->fr_offset + 4, 1,
		     reloctype ==
		     BFD_RELOC_NONE ? BFD_RELOC_SH_IMM_LOW16_PCREL : reloctype
		     ==
		     BFD_RELOC_SH_GOTPC ? BFD_RELOC_SH_GOTPC_LOW16 : reloctype
		     ==
		     BFD_RELOC_32_PLT_PCREL ? BFD_RELOC_SH_PLT_LOW16
	...
	if (reloc_needed)
	  {
	    ...
	    fix_new (fragP, var_partp - fragP->fr_literal, 4,
		     fragP->fr_symbol, fragP->fr_offset + 4, 1,
		     reloctype ==
		     BFD_RELOC_NONE ? BFD_RELOC_SH_IMM_MEDHI16_PCREL :
		     reloctype ==
		     BFD_RELOC_SH_GOTPC ? BFD_RELOC_SH_GOTPC_MEDHI16 :
		     reloctype ==
		     BFD_RELOC_32_PLT_PCREL ? BFD_RELOC_SH_PLT_MEDHI16
		     : (abort (), BFD_RELOC_SH_IMM_MEDHI16_PCREL));
	    fix_new (fragP, var_partp - fragP->fr_literal + 4, 4,
		     fragP->fr_symbol, fragP->fr_offset + 8, 1,
		     reloctype ==
		     BFD_RELOC_NONE ? BFD_RELOC_SH_IMM_MEDLOW16_PCREL :
		     reloctype ==
		     BFD_RELOC_SH_GOTPC ? BFD_RELOC_SH_GOTPC_MEDLOW16 :
		     reloctype ==
		     BFD_RELOC_32_PLT_PCREL ? BFD_RELOC_SH_PLT_MEDLOW16
		     : (abort (), BFD_RELOC_SH_IMM_MEDLOW16_PCREL));
	    fix_new (fragP, var_partp - fragP->fr_literal + 8, 4,
		      ...
		      bfd_reloc_code_real_type how)
{
  ...
  if (op->immediate.X_op != O_constant || pcrel)
    fix_new_exp (frag_now,
		 where - frag_now->fr_literal, 4, &op->immediate, pcrel, how);
  else
    {
...
shmedia_parse_reg (char *src, int *mode, int *reg, shmedia_arg_type argtype)
{
  ...
  if (l0 == 'f' && l1 == 'v')
      ...
      if (src[2] >= '1' && src[2] <= '5')
	{
      ...
      if (src[2] == '6')
	{
	  if (src[3] == '0' && !IDENT_CHAR ((unsigned char) src[4]))
	    {
	      *mode = A_FVREG_G;
	      *reg = 60 + src[3] - '0';
	      return 4;
				       ...
				       segT segment_type ATTRIBUTE_UNUSED)
{
  ...
  bfd_boolean sym_relaxable
    = (fragP->fr_symbol
       ...
       && !S_IS_EXTERNAL (fragP->fr_symbol) && !S_IS_WEAK (fragP->fr_symbol));
  ...
  switch (fragP->fr_subtype)
	  ...
	  && S_GET_SEGMENT (exp->X_op_symbol) == segment_type)
	{
	  ...
	  expressionS *opexp = symbol_get_value_expression (exp->X_op_symbol);
	  expressionS *addexp
		   ...
		   && exp->X_op == O_constant))
	{
	  ...
	  if (value >= ((offsetT) - 1 << 15) && value < ((offsetT) 1 << 15))
	  ...
	  else if (GET_WHAT (fragP->fr_subtype) == MOVI_IMM_32)
	    {
	      ...
	      as_bad_where (fragP->fr_file, fragP->fr_line,
			    _
			    ("MOVI operand is not a 32-bit signed value: 0x%8x%08x"),
			    ((unsigned int) (value >> 32) & (unsigned int)
			     0xffffffff),
	...
	case A_IMMU16:
	     ...
	     adjust operands->operands[i].reloctype.  */
	  {
		...
		&& ((right_expr
		    ...
		    X_op == O_constant) && right_expr->X_add_number == 0xffff)
	      {
		symbolS *inner = imm_expr->X_add_symbol;
		bfd_reloc_code_real_type reloctype = BFD_RELOC_SH_IMM_LOW16;
		expressionS *inner_expr = symbol_get_value_expression (inner);
		...
		if (inner_expr->X_op == O_right_shift)
		  {
			...
			&& ((inner_right
			    ...
			    X_op == O_constant))
		      {
			offsetT addnum = inner_right->X_add_number;
...
shmedia_find_cooked_opcode (char **str_p)
{
  ...
  for (op_start = op_end = str;
       *op_end
       ...
       && !ISSPACE ((unsigned char) *op_end); op_end++)
    {
	  ...
	  if ((opjp->reloctype == BFD_RELOC_NONE
	      ...
	      else if (opjp->reloctype == BFD_RELOC_32_PLT_PCREL)
		{
		  ...
		  what = (sh64_abi == sh64_abi_64
			  ? MOVI_IMM_64_PCREL : MOVI_IMM_32_PCREL);
		}
	...
	case A_PCIMMS16BY4:
	  {
	    ...
	    if (sh64_expand || opjp->reloctype != BFD_RELOC_NONE)
	      frag_var (rs_machine_dependent,
			...
			C (what, init),
			opjp->immediate.X_add_symbol,
			opjp->immediate.X_add_number, insn_loc);
	    else
...
shmedia_md_begin (void)
{
  ...
  for (shmedia_opcode = shmedia_table; shmedia_opcode->name; shmedia_opcode++)
...
s_sh64_abi (int ignore ATTRIBUTE_UNUSED)
{
  ...
  *input_line_pointer = '\0';
  ...
  if (strcmp (name, "64") == 0)
	...
	as_bad (_
		("`.abi 64' but command-line options do not specify 64-bit ABI"));
    }
  else if (strcmp (name, "32") == 0)
    {
      if (sh64_abi != sh64_abi_32)
	as_bad (_
		("`.abi 32' but command-line options do not specify 32-bit ABI"));
    }
  else
...
sh64_target_format (void)
{
  ...
  if (sh64_isa_mode == sh64_isa_shcompact && sh64_abi == sh64_abi_unspecified)
    sh64_abi = sh64_abi_32;

  if (sh64_isa_mode == sh64_isa_shmedia && sh64_abi == sh64_abi_unspecified)
    sh64_abi = sh64_abi_64;
  ...
  if ((sh64_isa_mode == sh64_isa_unspecified
       || sh64_isa_mode == sh64_isa_shmedia) && sh64_shcompact_const_crange)
		  ...
		  enum sh64_elf_cr_type cr_type)
{
  ...
  asection *cranges = bfd_make_section_old_way (stdoutput,
  ...
  if (seginfo)
	...
	= seginfo->tc_segment_info_data.contents_type;
      ...
      /* If it was just SHcompact switching between code and constant
         ...
         in SHmedia mode.  */
      if (sh64_isa_mode == sh64_isa_shcompact && !sh64_shcompact_const_crange)
      ...
      if (exp->X_op == O_symbol || exp->X_op == O_PIC_reloc)
	{
      ...
      /* Complain about other types of operands than symbol, unless they
         ...
         Complaining about it would also be wrong.  */
	       ...
	       && exp->X_op != O_absent && exp->X_op != O_constant)
      ...
      else
	{
	  ...
	  if ((S_GET_VALUE (symp) != frag_now_fix ()
		...
		= seginfo->tc_segment_info_data.contents_type;
		  ...
		  && contents_type != CRT_NONE && !seen_insn)
		{
			...
			void *countparg ATTRIBUTE_UNUSED)
{
  ...
  if (seginfo
      ...
      && seginfo->tc_segment_info_data.emitted_ranges != 0)
    {
      ...
      sh64_emit_crange (seginfo->tc_segment_info_data.mode_start_symbol,
			symp, seginfo->tc_segment_info_data.contents_type);
    }
}
...
{
  if (sh64_isa_mode != sh64_isa_unspecified
      && !seen_insn && !sh64_end_of_assembly && !emitting_crange)
    {
