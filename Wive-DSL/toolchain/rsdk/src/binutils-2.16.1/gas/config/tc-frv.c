...
static const char *frv_pic_flag =
  DEFAULT_FDPIC ? "-mfdpic" : (const char *) 0;
...
md_begin ()
{
  ...
  gas_cgen_cpu_desc = frv_cgen_cpu_open (CGEN_CPU_OPEN_MACHS, 0,
					 ...
					 CGEN_ENDIAN_BIG, CGEN_CPU_OPEN_END);
     ...
     bfd_boolean count;
{
  ...
  if (current_vliw_chain == NULL)
    {
      vliw_chain_entry =
	(struct vliw_chain *) xmalloc (sizeof (struct vliw_chain));
  ...
  vliw_insn_list_entry =
    (struct vliw_insn_list *) xmalloc (sizeof (struct vliw_insn_list));
     ...
     symbolS *label_sym;
{
  ...
  for (the_insn = this_chain->insn_list; the_insn; the_insn = the_insn->next)
    {
      if (the_insn->type == vliw_insn_type && the_insn->sym == label_sym)
	return the_insn;
    }
     ...
     struct vliw_chain *start_chain;
{
  ...
  for (this_chain = start_chain; this_chain;
       this_chain = this_chain->next, i++)
    {
      fprintf (stderr, "\nVliw Insn #%d, #insns: %d\n", i,
	       this_chain->insn_count);

      for (this_insn = this_chain->insn_list; this_insn;
     ...
     struct vliw_chain *this_chain;
{
  ...
  for (this_insn = this_chain->insn_list;
       this_insn; this_insn = this_insn->next)
    {
      if (this_insn->type != VLIW_LABEL_TYPE)
	this_chain->insn_count++;
    }
     ...
     struct vliw_insn_list *insert_before_insn;
{
  ...
  struct vliw_chain *double_nop =
    (struct vliw_chain *) xmalloc (sizeof (struct vliw_chain));
  struct vliw_chain *single_nop =
    (struct vliw_chain *) xmalloc (sizeof (struct vliw_chain));
  struct vliw_chain *second_part =
    (struct vliw_chain *) xmalloc (sizeof (struct vliw_chain));
...
workaround_top:
  ...
  if (vliw1->insn_count == 1)
	...
	{
	  temp_insn =
	    frv_find_in_vliw (VLIW_BRANCH_TYPE, vliw2, vliw1->insn_list->sym);
	  if (temp_insn)
	    {
	      vliw1 =
		frv_tomcat_shuffle (VLIW_DOUBLE_NOP, vliw2, vliw1->insn_list);
		...
		tomcat_doubles++;
	      goto workaround_top;
		   ...
		   && vliw2->insn_count == 1
		   && (temp_insn =
		       frv_find_in_vliw (VLIW_BRANCH_TYPE, vliw3,
					 vliw1->insn_list->sym)) != NULL)
	    {
	      temp_insn->snop_frag->fr_subtype = NOP_KEEP;
	      vliw1 =
		frv_tomcat_shuffle (VLIW_SINGLE_NOP, vliw3, vliw3->insn_list);
	      if (tomcat_stats)
		tomcat_singles++;
	      goto workaround_top;
	    }
  ...
  if (vliw1->insn_count == 2)
    {
      ...
      /* check vliw1 for a label. */
      for (this_insn = vliw1->insn_list; this_insn;
	   this_insn = this_insn->next)
	{
	  if (this_insn->type == VLIW_LABEL_TYPE)
	    {
	      if ((temp_insn =
		   frv_find_in_vliw (VLIW_BRANCH_TYPE, vliw2,
				     this_insn->sym)) != NULL)
		{
		  temp_insn->snop_frag->fr_subtype = NOP_KEEP;
		  vliw1 =
		    frv_tomcat_shuffle (VLIW_SINGLE_NOP, vliw2, this_insn);
		  if (tomcat_stats)
		    tomcat_singles++;
		}
	      else
  ...
  for (this_insn = vliw1->insn_list; this_insn; this_insn = this_insn->next)
	...
	{
	  if ((temp_insn =
	       frv_find_in_vliw (VLIW_LABEL_TYPE, vliw1,
				 this_insn->sym)) != NULL)
	    {
	      ...
	      vliw1 =
		frv_tomcat_shuffle (VLIW_DOUBLE_THEN_SINGLE_NOP, vliw1,
				    this_insn);
	      goto workaround_top;
	    }
	}
     ...
     char *str;
{
  ...
  if (frv_mach == bfd_mach_frvtomcat)
	  ...
	  && (frv_is_branch_insn (insn.insn) || insn.fields.f_pack))
	{
	  ...
	  buffer =
	    frag_var (rs_machine_dependent, 8, 8, NOP_DELETE, NULL, 0, 0);
	  ...
	  buffer =
	    frag_var (rs_machine_dependent, 4, 4, NOP_DELETE, NULL, 0, 0);
     ...
     fixS *fix;
{
  switch (fix->fx_r_type < BFD_RELOC_UNUSED
	  ? (int) fix->fx_r_type : fix->fx_cgen.opinfo)
    {
     ...
     PTR ptr ATTRIBUTE_UNUSED;
{
  ...
  for (fixp = seginfo->fix_root; fixp; fixp = fixp->fx_next)
      ...
      else
	{
      ...
      DPRINTF3 ("\treloc %s\t%s", bfd_get_reloc_code_name (reloc),
		S_GET_NAME (s));
	...
	case BFD_RELOC_32:
	  ...
	  if (!known_section_p
	      ...
	      && !fixp->fx_subsy && (flags & (SEC_READONLY | SEC_CODE)) == 0)
	    {
	      non_pic_p = 1;
	    }
	  break;
     ...
     expressionS *exp;
{
  ...
  if (frv_mach == bfd_mach_frvtomcat
      && current_vliw_insn
      && current_vliw_insn->type == VLIW_BRANCH_TYPE && exp != NULL)
