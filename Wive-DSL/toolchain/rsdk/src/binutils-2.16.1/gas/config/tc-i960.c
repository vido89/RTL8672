  ...
  switch (fragP->fr_subtype)
    ...
    case 1:
      ...
      fixP = fix_new (fragP,
		      ...
		      4, fragP->fr_symbol, fragP->fr_offset, 1, NO_RELOC);
     ...
     struct relocation_info *ri;
{
  ...
  where[7] = (ri->r_pcrel << 7
	      ...
	      | ri->r_extern << 4
	      | ri->r_bsr << 3
	      | ri->r_disp << 2 | ri->r_callj << 1 | ri->nuthin << 0);
#else
  where[7] = (ri->r_pcrel << 0
	      | ri->r_length << 1
	      | ri->r_extern << 3
	      | ri->r_bsr << 4
	      | ri->r_disp << 5 | ri->r_callj << 6 | ri->nuthin << 7);
#endif
}
  ...
  switch (expr.X_op)
      ...
      if (instr.disp == 12)
	{
      ...
      fixP = fix_new_exp (frag_now,
			  outP - frag_now->fr_literal, 4, &expr, 0, NO_RELOC);
     ...
     char opdesc;		/* Descriptor byte:  what's legal for this operand */
{
  ...
  if (n >= 0)
	...
	{			/* floating point literal acceptable */
	  ...
	  if ((optext[0] == '0') && (optext[1] >= 'd') && (optext[1] <= 'f'))
	    {
	      optext += 2;
	    }
	...
	{			/* fixed point literal acceptable */
	  parse_expr (optext, &e);
	  if (e.X_op != O_constant || (offs (e) < 0) || (offs (e) > 31))
	    {
				 ...
				 */
{
  ...
  instr = coj[opcode].compare | src1 | (m1 << 11) | (s2 << 6) | (src2 << 14);
  ...
  if (TC_S_IS_SYSPROC (fixP->fx_addsy))
    {
      ...
      md_number_to_chars (where, CALLS | TC_S_GET_SYSPROC (fixP->fx_addsy),
			  4);
     ...
     char *args[];		/* args[1]->1st operand, args[2]->2nd operand */
{
  ...
  if (exp.X_op != O_constant || (offs (exp) < 0) || (offs (exp) > 31))
    {
     ...
     int ic;
{
  ...
  switch (architecture)
	...
	{
	  as_warn (_
		   ("architecture of opcode conflicts with that of earlier instruction(s)"));
	  iclasses_seen &= ~ic;
	}
      return 1;
    }
     ...
     object_headers *headers ATTRIBUTE_UNUSED;
{
  ...
  for (symbolP = symbol_rootP; symbolP; symbolP = symbol_next (symbolP))
      ...
      if (TC_S_IS_SYSPROC (symbolP))
	{
	  ...
	  S_SET_DATA_TYPE (symbolP,
			   S_GET_DATA_TYPE (symbolP) | (DT_FCN << N_BTSHFT));
	  continue;
	}			/* rewrite sysproc */
#endif /* OBJ_COFF */
	  ...
	  if (S_IS_EXTERNAL (symbolP) != S_IS_EXTERNAL (balP))
	    {
	      ...
	      S_SET_EXTERNAL (balP);
	      as_warn (_
		       ("Warning: making leafproc entries %s and %s both global\n"),
     ...
     fragS *fragp ATTRIBUTE_UNUSED;
{
  ...
  if (fragp->fr_next == text_last_frag || fragp->fr_next == data_last_frag)
    return;
