  ...
  *p = c;
  ...
  if (S_GET_VALUE (symbolP) != 0)
      ...
      if (S_GET_VALUE (symbolP) != size)
	{
	  /* xgettext:c-format  */
	  as_warn (_
		   ("Length of .comm \"%s\" is already %ld. Not changed to %d."),
  ...
  *input_line_pointer = c;
  ...
  if (reg_number < 0 && accept_numbers)
      ...
      if (ISDIGIT (*input_line_pointer))
	{
	  ...
	  if (reg_number < 0
	      || (reg_number > 5 && reg_number < 16) || reg_number > 27)
	    {
	      reg_number = -1;
	    }
	}
      ...
      else if (system_register_name (&exp, TRUE, TRUE))
	{
	  if (regs == type1_regs)
	    ...
	    *insn |= 0x80000;
	    ...
	    return
	      _("High value system registers cannot be included in list");
	}
  ...
  if (fragP->fr_subtype == 0 || fragP->fr_subtype == 2)
    {
      fix_new (fragP, fragP->fr_fix, 2, fragP->fr_symbol,
	       fragP->fr_offset, 1,
	       BFD_RELOC_UNUSED + (int) fragP->fr_opcode);
      fragP->fr_fix += 2;
    }
     ...
     const struct v850_operand *operand;
{
  ...
  if (operand->bits != 6 || operand->shift != 0)
    {
      as_bad (_
	      ("ctoff() relocation used on an instruction which does not support it"));
      return BFD_RELOC_64;	/* Used to indicate an error condition.  */
    }
     ...
     const struct v850_operand *operand;
{
  ...
  if (operand->bits != 16 || operand->shift != 16)
    {
      as_bad (_
	      ("sdaoff() relocation used on an instruction which does not support it"));
      return BFD_RELOC_64;	/* Used to indicate an error condition.  */
    }
     ...
     const struct v850_operand *operand;
{
  ...
  if (operand->bits != 16 || operand->shift != 16)
    {
      as_bad (_
	      ("zdaoff() relocation used on an instruction which does not support it"));
     ...
     const struct v850_operand *operand;
{
  ...
  if (operand->bits != 7)
    {
      as_bad (_
	      ("tdaoff() relocation used on an instruction which does not support it"));
      /* Used to indicate an error condition.  */
      return BFD_RELOC_64;
    }

  return operand->insert != NULL ? BFD_RELOC_V850_TDA_7_8_OFFSET	/* sld.h/sst.h, operand: D8_7  */
      ...
      if (operand->bits != 32)
	{
	  ...
	  if (val < (offsetT) min || val > (offsetT) max)
		...
		buf[0] = 0;
	      ...
	      as_bad_value_out_of_range (buf, val, (offsetT) min,
      ...
      for (opindex_ptr = opcode->operands; *opindex_ptr != 0; opindex_ptr++)
	{
	  ...
	  if ((reloc = v850_reloc_prefix (operand)) != BFD_RELOC_UNUSED)
		       ...
		       && (operand->flags & V850_OPERAND_REG) == 0)
		{
		  ...
		  *input_line_pointer = c;
		  ...
		  if (ex.X_op != O_constant)
			...
			errmsg =
			  _
			  ("syntax error: value is missing before the register name");
		      else
	      ...
	      else
		{
		  ...
		  if (((insn & 0x07e0) == 0x0200) && operand->bits == 5	/* Do not match the CALLT instruction.  */
		      && ex.X_op == O_constant
     ...
     segT seg ATTRIBUTE_UNUSED;
{
  ...
  if (fixP->fx_addsy == (symbolS *) NULL)
    fixP->fx_addnumber = value, fixP->fx_done = 1;
