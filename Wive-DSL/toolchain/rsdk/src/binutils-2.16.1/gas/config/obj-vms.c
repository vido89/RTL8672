...
vms_resolve_symbol_redef (symbolS * sym)
{
  ...
  if (SEGMENT_TO_SYMBOL_TYPE ((int) now_seg) == (N_UNDF | N_EXT)
    ...
    {
      as_warn (_
	       ("compiler emitted zero-size common symbol `%s' already defined"),
	       S_GET_NAME (sym));
      return 1;
    }
...
generate_suffix (struct VMS_DBG_Symbol *spnt, int dbx_type)
{
  ...
  if ((total_len >= MAX_DEBUG_RECORD) || overflow)
    {
      as_warn (_
	       ("Variable descriptor %d too complicated.  Defined as `void *'."),
	       spnt->dbx_type);
...
***Warning - the assembly code generated by the compiler has placed \n\
 ...
 I didn't warn you! \n");
	      ...
	      VMS_DBG_record (spnt, Text_Psect, S_GET_VALUE (sp1), str);
	      *pnt1 = ':';
...
VMS_typedef_parse (char *str)
{
  ...
  switch (*pnt)
	...
	{
	  spnt->VMS_type =
	    vax_g_doubles ? DBG_S_C_COMPLX8_G : DBG_S_C_COMPLX8;
	  spnt->data_size = 2 * 8;
	}
      ...
      while (*++pnt1 != ';')
	{
	  ...
	  *pnt = '\0';
	  ...
	  if (spnt1 && (spnt1->advanced == BASIC || spnt1->advanced == ENUM)
		...
		{
		  as_tsktsk (_
			     ("debugger output: structure element `%s' has undefined type"),
			     pnt2);
		  continue;
		}
	      Local[i++] = 7 + len;
...
VMS_LSYM_Parse (void)
{
  ...
  if (incomplete != 0)
    as_tsktsk (_
	       ("debugger output: Unable to resolve %d circular references."),
	       incomplete);
	  ...
	  if (strncmp (Name, Attributes[i].Name, cp - Name) == 0)
	    {
	      /* Set or clear.  */
	      if (Negate)
		*Attribute_Pointer &= ~Attributes[i].Value;
	      else
		*Attribute_Pointer |= Attributes[i].Value;
	      /* Done.  */
	      break;
...
static void
VMS_Global_Symbol_Spec (const char *Name, int Psect_Number, int Psect_Offset,
			int Flags)
{
  char Local[32];
...
static int
VMS_Psect_Spec (const char *Name, int Size, enum ps_type Type,
		struct VMS_Symbol *vsp)
{
  char Local[32];
  ...
  switch (Type)
    ...
    case ps_COMMON:
      ...
      Psect_Attributes =
	(GPS_S_M_PIC | GPS_S_M_OVR | GPS_S_M_REL | GPS_S_M_GBL | GPS_S_M_RD |
	 GPS_S_M_WRT);
      break;
    case ps_CONST:
    ...
    case ps_CTORS:
      ...
      Psect_Attributes =
	(GPS_S_M_PIC | GPS_S_M_REL | GPS_S_M_GBL | GPS_S_M_RD);
      break;
    case ps_DTORS:
  ...
  for (sp = symbol_rootP; sp; sp = sp->sy_next)
    {
      ...
      if (!abstyp && typ != (N_DATA | N_EXT) && typ != (N_UNDF | N_EXT))
	continue;
				...
				int Psect, int Psect_Offset, int Record_Type)
{
  ...
  VMS_Set_Psect (Psect,
		 PC_Relative ? Psect_Offset - 1 : Psect_Offset, Record_Type);
  ...
  switch (S_GET_RAW_TYPE (Symbol))
    ...
    case N_BSS:
      ...
      vms_tir_stack_psect (vsp->Psect_Index, vsp->Psect_Offset + Offset, 0);
      break;
      /* Local text.  */
    case N_TEXT:
      /* Stack the Psect (+offset).  */
      vms_tir_stack_psect (vsp->Psect_Index,
			   S_GET_VALUE (Symbol) + Offset, 0);
      break;
			    ...
			    fragS * fragP, fragS * text_frag_root)
{
  ...
  if ((Offset < fragP->fr_address) ||
      ...
      for (fragP = text_frag_root; fragP; fragP = fragP->fr_next)
	{
      ...
      if (fragP == 0)
	error (_
	       ("Couldn't find fixup fragment when checking for indirect reference"));
    }
	  ...
	  while (frchainP)
	      ...
	      while (fragP && (fragP != frchainP->frch_last))
		{
		  ...
		  if (fragP == symbolP->sy_frag)
			...
			New_Frag->fr_literal[i + 6] = fragP->fr_literal[i];
		      ...
		      for (fixP = text_fix_root; fixP; fixP = fixP->fx_next)
			{
		      ...
		      for (symbolP = symbol_rootP;
			   symbolP; symbolP = symbol_next (symbolP))
			{
			  /* Look for references to this fragment.  */
  ...
  for (fixP = text_fix_root; fixP; fixP = fixP->fx_next)
      ...
      if (fixP->fx_subsy && fixP->fx_addsy)
	{
	  ...
	  if ((S_GET_TYPE (fixP->fx_addsy) != N_DATA) &&
	    ...
	    error (_
		   ("Fixup data addsy and subsy don't have an appropriate type"));
	  ...
	  VMS_Set_Psect (Text_Psect,
			 ...
			 OBJ_S_C_TIR);
	  VMS_Store_Immediate_Data (Local, fixP->fx_size, OBJ_S_C_TIR);
	  continue;
	}
	...
	error (_("Fixup datum is not \"fixP->fx_addsy\""));
      ...
      VMS_Store_PIC_Symbol_Reference (fixP->fx_addsy,
				      ...
				      Text_Psect,
				      fixP->fx_where +
				      fixP->fx_frag->fr_address, OBJ_S_C_TIR);
	...
	VMS_Fix_Indirect_Reference (Text_Psect,
				    fixP->fx_where +
				    fixP->fx_frag->fr_address, fixP->fx_frag,
				    text_frag_root);
    }
}

	  ...
	  if (fixP->fx_subsy && fixP->fx_addsy)
	    {
		  ...
		  S_GET_RAW_TYPE (fixP->fx_addsy))
		error (_
		       ("Fixup data addsy and subsy don't have the same type"));
	      /* And they need to be in one that we can check the psect on.  */
	      if ((S_GET_TYPE (fixP->fx_addsy) != N_DATA) &&
		  (S_GET_TYPE (fixP->fx_addsy) != N_TEXT))
		error (_
		...
		S_GET_VALUE (fixP->fx_addsy) - S_GET_VALUE (fixP->fx_subsy);
	      ...
	      VMS_Set_Psect (vsp->Psect_Index,
			     ...
			     - val + vsp->Psect_Offset, OBJ_S_C_TIR);
	      VMS_Store_Immediate_Data (Local, fixP->fx_size, OBJ_S_C_TIR);
...
global_symbol_directory (unsigned text_siz, unsigned data_siz)
{
  ...
  for (sp = symbol_rootP; sp; sp = symbol_next (sp))
	...
	case N_TEXT | N_EXT:
	  {
		...
		switch ((S_GET_NAME (sp))[10])
		  ...
		  case 'I':
		    ...
		    vsp->Psect_Offset =
		      (Ctors_Symbols ==
		       0) ? 0 : (Ctors_Symbols->Psect_Offset + XTOR_SIZE);
		  ...
		  case 'D':
		    ...
		    vsp->Psect_Offset =
		      (Dtors_Symbols ==
		       0) ? 0 : (Dtors_Symbols->Psect_Offset + XTOR_SIZE);
		...
		while (fragP && fragP->fr_type == rs_fill
		       && fragP->fr_fix == 0)
		  fragP = fragP->fr_next;
	...
	case N_UNDF:
	  ...
	  VMS_Global_Symbol_Spec (S_GET_NAME (sp), 0, 0, GBLSYM_REF);
	  break;
