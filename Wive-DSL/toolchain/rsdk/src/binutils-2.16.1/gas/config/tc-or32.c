...
static void encode
PARAMS ((const struct machine_opcode *, unsigned long *, signed long, char));
...
#if DEBUG
  printf
    ("    encode:  opcode=%.8lx  param_val=%.8lx abs=%.8lx param_ch=%c\n",
     *opcode, param_val, abs (param_val), param_ch);
#endif
  for (enc = insn->encoding; *enc != '\0'; enc++)
    if (*enc == param_ch)
      {
	if (enc - 2 >= insn->encoding && (*(enc - 2) == '0')
	    && (*(enc - 1) == 'x'))
	  continue;
	else
	  param_pos++;
...
#if DEBUG
	      printf
		("    ',' case: operand->X_add_number = %d, *args = %s, *s = %s\n",
	...
	default:
...
#if DEBUG
	  printf
	    ("    default case: operand->X_add_number = %d, *args = %s, *s = %s\n",
	     operand->X_add_number, args, s);
#endif
	  if (operand->X_op == O_constant)
		...
		operand->X_add_number =
		  ((operand->X_add_number >> 16) & 0xffff);
     ...
     relax_addressT segment_address_in_file;
{
  ...
  md_number_to_chars
    (where,
     fixP->fx_frag->fr_address + fixP->fx_where - segment_address_in_file, 4);
     ...
     expressionS *expressionP;
{
  ...
  if (input_line_pointer[0] == REGISTER_PREFIX
