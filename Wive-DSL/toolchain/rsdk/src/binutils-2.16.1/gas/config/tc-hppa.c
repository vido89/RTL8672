...
pa_get_label ()
{
  ...
  for (label_chain = label_symbols_rootp;
       label_chain; label_chain = label_chain->lss_next)
    {
#ifdef OBJ_SOM
  ...
  while (i < NUMOPCODES)
    {
      ...
      retval =
	hash_insert (op_hash, name, (struct pa_opcode *) &pa_opcodes[i]);
  ...
  if (within_procedure && last_call_info->start_symbol == NULL)
      ...
      if (label_symbol)
	{
	  if (label_symbol->lss_label)
	    ...
	    as_bad (_
		    ("Missing function name for .PROC (corrupted label chain)"));
	}
      else
  ...
  for (;;)
    {
      ...
      if (insn->arch < 20 && bfd_get_mach (stdoutput) < insn->arch)
	{
      ...
      for (args = insn->args;; ++args)
	{
	  ...
	  switch (*args)
		...
		case 'e':
		  {
			...
			if (!found && strict)
			...
			else
			  {
			    if (!found)
			      as_bad (_
				      ("Invalid Short Load/Store Completer."));
			    s += 2;
			  }
		      }
		...
		switch (*args)
		  ...
		  case 'n':
			...
			if (cmpltr < 0)
			  {
			    as_bad (_
				    ("Invalid Compare and Branch Condition"));
			    cmpltr = 0;
			  }
			else
			  {
		  ...
		  case 'l':
			...
			*s = 0x00;
			...
			/* ",*" is a valid condition.  */
			  ...
			  as_bad (_
				  ("Invalid Logical Instruction Condition."));
			*s = c;
		      }
		    opcode |= cmpltr << 13;
		    INSERT_FIELD_AND_CONTINUE (opcode, flag, 12);
		...
		flag = QUAD;
		  ...
		  || the_insn.fpof1 == DBL || the_insn.fpof1 == QUAD)
		{
		  if (the_insn.fpof2 == SGL
		      || the_insn.fpof2 == DBL || the_insn.fpof2 == QUAD)
		    flag = 0;
		  else if (the_insn.fpof2 == W
			   || the_insn.fpof2 == DW || the_insn.fpof2 == QW)
		    flag = 2;
		  else if (the_insn.fpof2 == UW
			   || the_insn.fpof2 == UDW || the_insn.fpof2 == UQW)
		    flag = 6;
		  else
		    abort ();
		}
	      else if (the_insn.fpof1 == W
		       || the_insn.fpof1 == DW || the_insn.fpof1 == QW)
		{
		  if (the_insn.fpof2 == SGL
		      || the_insn.fpof2 == DBL || the_insn.fpof2 == QUAD)
		    flag = 1;
		    ...
		    abort ();
		       ...
		       || the_insn.fpof1 == UDW || the_insn.fpof1 == UQW)
		{
		  if (the_insn.fpof2 == SGL
		      || the_insn.fpof2 == DBL || the_insn.fpof2 == QUAD)
		    flag = 5;
		...
		case 'i':
		  {
			...
			if (num < 16)
			  {
			    as_bad (_
				    ("Invalid register for single precision fmpyadd or fmpysub"));
			    break;
			  }
			num &= 0xF;
  ...
  for (i = 0; i < n_relocs; i++)
    {
      ...
      *relocs[i]->sym_ptr_ptr = symbol_get_bfdsym (fixp->fx_addsy);
      relocs[i]->howto =
	bfd_reloc_type_lookup (stdoutput, (bfd_reloc_code_real_type) code);
     ...
     segT seg ATTRIBUTE_UNUSED;
{
  ...
  if ((fixP->fx_addsy != NULL || fixP->fx_r_type == (int) R_HPPA_NONE)
#ifdef OBJ_SOM
      && fmt != 32
#endif
    )
  ...
  switch (fmt)
    {
    case 10:
      CHECK_FIELD_WHERE (new_val, 8191, -8192, fixP->fx_file, fixP->fx_line);
      val = new_val;
    ...
    case 11:
      CHECK_FIELD_WHERE (new_val, 1023, -1024, fixP->fx_file, fixP->fx_line);
      val = new_val;
     ...
     char **str;
{
  ...
  if ((*str)[1] == '\'' || (*str)[1] == '%')
    name[0] = TOLOWER ((*str)[0]), name[1] = 0;
  else if ((*str)[2] == '\'' || (*str)[2] == '%')
    name[0] = TOLOWER ((*str)[0]), name[1] = TOLOWER ((*str)[1]), name[2] = 0;
  else if ((*str)[3] == '\'' || (*str)[3] == '%')
    name[0] = TOLOWER ((*str)[0]),
      name[1] = TOLOWER ((*str)[1]),
      name[2] = TOLOWER ((*str)[2]), name[3] = 0;
  else
    return e_fsel;
     ...
     char *str;
{
  ...
  if (!(seg == absolute_section
	|| seg == undefined_section || SEG_NORMAL (seg)))
    {
     ...
     struct call_info *call_info;
{
  ...
  fix_new_hppa (frag_now, p - frag_now->fr_literal, 4,
		...
		(expressionS *) NULL, 0, reloc, e_fsel, 32, 0, 0);
     ...
     int unused ATTRIBUTE_UNUSED;
{
  ...
  current_subspace = pa_subsegment_to_subspace (current_space->sd_seg, 0);
#endif
     ...
     expressionS *rightP;
{
  if (fudge_reg_expressions
      && rightP->X_op == O_register && resultP->X_op == O_register)
    {
  ...
  if (label_symbol)
      ...
      else
	{
	  ...
	  if (exp.X_op != O_constant && exp.X_op != O_register)
		...
		as_bad (_
			("bad or irreducible absolute expression; zero assumed"));
	      exp.X_add_number = 0;
     ...
     int is_export;
{
  ...
  if (strncasecmp (input_line_pointer, "absolute", 8) == 0)
      ...
      if (S_IS_FUNCTION (symbolP))
	{
	  if (is_export)
	    as_tsktsk (_
		       ("Using ENTRY rather than CODE in export directive for %s"),
		       S_GET_NAME (symbolP));
  ...
  while (!is_end_of_statement ())
	...
	{
	  as_bad (_("Undefined .EXPORT/.IMPORT argument (ignored): %s"),
		  name);
	  p = input_line_pointer;
	  *p = c;
	}
     ...
     int unused ATTRIBUTE_UNUSED;
{
  ...
  if (within_procedure)
    {
      as_bad (_
	      ("Can\'t change spaces within a procedure definition. Ignored"));
      ignore_rest_of_line ();
    }
  else
    {
      ...
      if (strncmp (input_line_pointer, "$PRIVATE$", 9) == 0)
	{
      ...
      if (!strncasecmp (input_line_pointer,
			GDB_DEBUG_SPACE_NAME, strlen (GDB_DEBUG_SPACE_NAME)))
	{
     ...
     int create_new;
{
  ...
  if (within_procedure)
    {
      as_bad (_
	      ("Can\'t change subspaces within a procedure definition. Ignored"));
      ignore_rest_of_line ();
    }
  else
    {
      ...
      if (ssd && SUBSPACE_DEFINED (ssd))
	{
	  ...
	  if (!is_end_of_statement ())
	    as_warn (_
		     ("Parameters of an existing subspace can\'t be modified"));
	  demand_empty_rest_of_line ();
	  return;
	}
      else
...
pa_spaces_begin ()
{
  ...
  while (pa_def_subspaces[i].name)
      ...
      else if (!strcmp (pa_def_subspaces[i].name, "$BSS$"))
	{
	  bss_section = segment;
	  applicable = bfd_applicable_section_flags (stdoutput);
	  bfd_set_section_flags (stdoutput, segment, applicable & SEC_ALLOC);
	}
	...
	{
	  as_fatal (_
		    ("Internal error: Unable to find containing space for %s."),
      ...
      create_new_subspace (space, name,
			   ...
			   pa_def_subspaces[i].quadrant, segment);
      i++;
    }
}
...
update_subspace (space, name, loadable, code_only, comdat, common, dup_common,
		 sort, zero, access, space_index, alignment, quadrant,
		 section)
     sd_chain_struct *space;
     char *name;
     int loadable ATTRIBUTE_UNUSED;
     ...
     char *name;
{
  ...
  for (chain_pointer = space_dict_root;
       chain_pointer; chain_pointer = chain_pointer->sd_next)
    {
     ...
     asection *seg;
{
  ...
  for (space_chain = space_dict_root;
       space_chain; space_chain = space_chain->sd_next)
    {
  ...
  for (space_chain = space_dict_root;
       space_chain; space_chain = space_chain->sd_next)
    {
      ...
      for (subspace_chain = space_chain->sd_subspaces;
	   subspace_chain; subspace_chain = subspace_chain->ssd_next)
     ...
     fixS *fixp;
{
  ...
  if (fixp->fx_r_type == R_HPPA
      ...
      && strncmp (S_GET_NAME (fixp->fx_addsy),
		  FAKE_LABEL_NAME, strlen (FAKE_LABEL_NAME)))
    return 0;
#endif
  ...
  if (fixp->fx_addsy
      && fixp->fx_subsy && (hppa_fix->segment->flags & SEC_CODE))
    return 0;
  ...
  if (hppa_fix->fx_r_field == e_lrsel
      || hppa_fix->fx_r_field == e_rrsel || hppa_fix->fx_r_field == e_nlrsel)
    return 0;
  ...
  if (hppa_fix->fx_r_field == e_tsel
      ...
      || hppa_fix->fx_r_field == e_rpsel || hppa_fix->fx_r_field == e_lpsel)
    return 0;
  ...
  if (fixp->fx_r_type == (int) R_HPPA_PCREL_CALL)
	...
	min_stub_distance -= S_GET_VALUE (last_call_info->start_symbol);
	  ...
	  || (hppa_fixp->fx_r_format == 12
	      && distance + 8192 >= 16384 && min_stub_distance <= 8192))
	return 1;
    }
...
elf_hppa_final_processing ()
{
  ...
  for (call_info_pointer = call_info_root;
       call_info_pointer; call_info_pointer = call_info_pointer->ci_next)
    {
