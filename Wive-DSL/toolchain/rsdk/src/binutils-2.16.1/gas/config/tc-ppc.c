...
static void ppc_macro
PARAMS ((char *str, const struct powerpc_macro * macro));
...
#ifdef OBJ_ELF
static bfd_reloc_code_real_type ppc_elf_suffix
...
static void ppc_apuinfo_section_add
PARAMS ((unsigned int apu, unsigned int version));
#endif
...
static enum
{ SHLIB_NONE, SHLIB_PIC, SHLIB_MRELOCATABLE } shlib = SHLIB_NONE;
...
parse_cpu (const char *arg)
{
  ...
  /* -mppc, -mppc32, -m603, and -m604 mean to assemble for the
     PowerPC 603/604.  */
	   ...
	   || strcmp (arg, "ppc32") == 0
	   || strcmp (arg, "603") == 0 || strcmp (arg, "604") == 0)
    ppc_cpu = PPC_OPCODE_PPC | PPC_OPCODE_CLASSIC | PPC_OPCODE_32;
  /* -m403 and -m405 mean to assemble for the PowerPC 403/405.  */
  else if (strcmp (arg, "403") == 0 || strcmp (arg, "405") == 0)
    ppc_cpu = (PPC_OPCODE_PPC | PPC_OPCODE_CLASSIC
    ...
    ppc_cpu = (PPC_OPCODE_PPC | PPC_OPCODE_BOOKE | PPC_OPCODE_32
	   ...
	   || strcmp (arg, "7410") == 0
	   || strcmp (arg, "7450") == 0 || strcmp (arg, "7455") == 0)
    ppc_cpu = (PPC_OPCODE_PPC | PPC_OPCODE_CLASSIC
  ...
  for (macro = powerpc_macros; macro < macro_end; macro++)
      ...
      if ((macro->flags & ppc_cpu) != 0)
	{
	  ...
	  if (retval != (const char *) NULL)
	    {
	      as_bad (_("Internal assembler error for macro %s"),
		      macro->name);
	      dup_insn = TRUE;
	    }
	}
...
ppc_cleanup ()
{
   ...
   */
  {
    ...
    bfd_set_section_flags (stdoutput,
			   apuinfo_secp, SEC_HAS_CONTENTS | SEC_READONLY);
  ...
  for (ptr = &mapping[0]; ptr->length > 0; ptr++)
	...
	if (!ppc_obj64)
	  if (exp_p->X_add_number != 0
	    ...
	    as_warn (_
		     ("identifier+constant@got means identifier@got+constant"));
     ...
     int xxx ATTRIBUTE_UNUSED;
{
  ...
  *p = c;
  ...
  if (S_GET_VALUE (symbolP) && S_GET_VALUE (symbolP) != (valueT) size)
    {
      as_bad (_
	      ("Length of .lcomm \"%s\" is already %ld. Not changed to %ld."),
...
ppc_frob_file_before_adjust ()
{
  ...
  for (symp = symbol_rootP; symp; symp = symbol_next (symp))
	...
	continue;
      ...
      if (!S_IS_WEAK (symp) || S_IS_DEFINED (symp))
	continue;
  ...
  if (toc != NULL && bfd_section_size (stdoutput, toc) > 0x10000)
  ...
  for (opindex_ptr = opcode->operands; *opindex_ptr != 0; opindex_ptr++)
      ...
      if ((operand->flags & PPC_OPERAND_OPTIONAL) != 0)
	{
	  ...
	  for (num_operands_expected = 0, i = 0; opcode->operands[i]; i++)
	    if ((powerpc_operands[opcode->operands[i]].
		 flags & PPC_OPERAND_FAKE) == 0)
  ...
  for (opindex_ptr = opcode->operands; *opindex_ptr != 0; opindex_ptr++)
      ...
      if ((operand->flags & PPC_OPERAND_FAKE) != 0)
	{
      ...
      /* If this is an optional operand, and we are skipping it, just
         insert a zero.  */
      if ((operand->flags & PPC_OPERAND_OPTIONAL) != 0 && skip_optional)
	{
	  if (operand->insert)
	    {
	      insn = (*operand->insert) (insn, 0L, ppc_cpu, &errmsg);
	...
	{
	  as_bad (_("syntax error; found `%c' but expected `%c'"), *str,
		  endc);
	  break;
	}
  ...
  for (i = 0; i < fc; i++)
      ...
      if (fixups[i].reloc != BFD_RELOC_UNUSED)
	{
	  ...
	  fixP = fix_new_exp (frag_now,
			      ...
			      size,
			      &fixups[i].exp,
			      reloc_howto->pc_relative, fixups[i].reloc);
     ...
     int lcomm;
{
  ...
  *end_name = endc;
  ...
  if (S_IS_DEFINED (sym) || S_GET_VALUE (sym) != 0)
    {
  ...
  if (!lcomm || !S_IS_DEFINED (lcomm_sym))
    {
      symbolS *def_sym;
      offsetT def_size;
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  s = strchr (name, '[');
  if (s != (char *) NULL && strcmp (s + 1, "PR]") == 0)
    *s = '\0';
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  *input_line_pointer = c;
  ...
  md_number_to_chars (f, insn, 4);
  fix_new (frag_now,
	   f - frag_now->fr_literal, 4, sym, 0, 0, BFD_RELOC_16_GOT_PCREL);
  ...
  if (S_GET_STORAGE_CLASS (sym) == C_EXT
      ...
      else
	{
	  ...
	  if (csect == (symbolS *) NULL)
	    {
	      as_warn (_("warning: symbol %s has no csect"),
		       S_GET_NAME (sym));
	      a->x_csect.x_scnlen.l = 0;
	    }
	  else
     ...
     fixS *fix;
{
  ...
  if (ppc_toc_csect != (symbolS *) NULL
	  ...
	  || val < ppc_after_toc_frag->fr_address))
    {
      ...
      for (sy = symbol_next (ppc_toc_csect);
	   sy != (symbolS *) NULL; sy = symbol_next (sy))
	{
  ...
  if (tc->subseg == 0
      && tc->class != XMC_TC0 && tc->class != XMC_TC && symseg != bss_section
	...
	{
	  while ((next_csect =
		  symbol_get_tc (csect)->next) != (symbolS *) NULL
		  ...
		  && S_GET_VALUE (csect) == val)
		{
		  ...
		  for (scan = symbol_next (csect);
		       scan != NULL; scan = symbol_next (scan))
		    {
  ...
  /* Adjust a reloc against a .lcomm symbol to be against the base
     .lcomm.  */
  if (symseg == bss_section && !S_IS_EXTERNAL (fix->fx_addsy))
    {
     ...
     fixS *fix;
{
  ...
  if (fix->fx_pcrel
	  ...
	  || (symbol_get_tc (fix->fx_addsy)->next != NULL
	      && (symbol_get_frag (symbol_get_tc (fix->fx_addsy)->next)->
		  fr_address <= fix->fx_frag->fr_address))))
    return 1;
     ...
     segT seg ATTRIBUTE_UNUSED;
{
  ...
  if ((int) fixP->fx_r_type >= (int) BFD_RELOC_UNUSED)
	...
	return;
	  ...
	  && operand->bits == 26 && operand->shift == 0)
	fixP->fx_r_type = BFD_RELOC_PPC_B26;
      else if ((operand->flags & PPC_OPERAND_RELATIVE) != 0
	       && operand->bits == 16 && operand->shift == 0)
	{
...
#ifdef OBJ_ELF
	      if (ppc_obj64 && (operand->flags & PPC_OPERAND_DS) != 0)
	...
	case BFD_RELOC_PPC64_TOC16_HA:
	  ...
	  if (fixP->fx_pcrel)
		...
		as_bad_where (fixP->fx_file, fixP->fx_line,
			      _
			      ("cannot emit PC relative %s relocation against %s"),
	...
	case BFD_RELOC_PPC_LOCAL24PC:
	  ...
	  if (fixP->fx_done)
		...
		as_bad_where (fixP->fx_file, fixP->fx_line,
			      _
			      ("@local or @plt branch destination is too far away, %ld bytes"),
			      (long) value);
