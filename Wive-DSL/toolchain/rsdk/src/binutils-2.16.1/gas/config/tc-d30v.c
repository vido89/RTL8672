...
static bfd_reloc_code_real_type get_reloc
PARAMS ((struct d30v_operand * op, int rel_flag));
...
static struct d30v_format *find_format PARAMS ((struct d30v_opcode * opcode,
						expressionS ops[], int fsize,
						int cmp_hack));
static long long build_insn
PARAMS ((struct d30v_insn * opcode, expressionS * opers));
static void write_long
PARAMS ((struct d30v_insn * opcode, long long insn, Fixups * fx));
static void write_1_short
PARAMS ((struct d30v_insn * opcode, long long insn, Fixups * fx,
	 int use_sequential));
static int write_2_short
PARAMS ((struct d30v_insn * opcode1, long long insn1,
	 struct d30v_insn * opcode2, long long insn2,
	 exec_type_enum exec_type, Fixups * fx));
static long long do_assemble
PARAMS ((char *str, struct d30v_insn * opcode, int shortp, int is_parallel));
static int parallel_ok
PARAMS ((struct d30v_insn * opcode1, unsigned long insn1,
	 struct d30v_insn * opcode2, unsigned long insn2,
	 exec_type_enum exec_type));
static void d30v_number_to_chars
PARAMS ((char *buf, long long value, int nbytes));
...
md_begin ()
{
  ...
  for (opcode = (struct d30v_opcode *) d30v_opcode_table; opcode->name;
       opcode++)
	  ...
	  && !(flags & OPERAND_NAME))
	{
	  ...
	  fixups->fix[fixups->fc].reloc =
	    get_reloc ((struct d30v_operand *)
		       &d30v_operand_table[form->operands[i]],
		       op->reloc_flag);
  ...
  for (i = 0; i < fx->fc; i++)
      ...
      if (fx->fix[i].reloc)
	{
	  ...
	  fix_new_exp (frag_now,
		       where,
		       fx->fix[i].size,
		       &(fx->fix[i].exp), fx->fix[i].pcrel, fx->fix[i].reloc);
	}
    }
     ...
     exec_type_enum exec_type;
{
  ...
  for (j = 0; j < 3; j++)
      ...
      if (flag_explicitly_parallel)
	{
      ...
      else if ((mod_reg[0][j] & (mod_reg[1][j] | used_reg[1][j])) != 0)
	return 0;
    }
     ...
     char *str;
{
  ...
  if ((extype != EXEC_REVSEQ)
      && prev_mul32_p && (opcode.op->flags_used & (FLAG_MEM | FLAG_MUL16)))
    {
	  ...
	  && (extype == EXEC_PARALLEL
	      || (Optimizing && extype == EXEC_UNKNOWN))
	  && parallel_ok (&prev_opcode, (long) prev_insn, &opcode,
			  (long) insn, extype)
	  && write_2_short (&prev_opcode, (long) prev_insn, &opcode,
	  ...
	  if (prev_insn != -1 && (strcmp (prev_opcode.op->name, "nop") == 0))
	      ...
	      else
		{
		  ...
		  if (warn_nops == NOP_ALL || warn_nops == NOP_MULTIPLY)
			...
			as_warn (_
				 ("word of NOPs added between word multiply and load"));
		      else
			as_warn (_
				 ("word of NOPs added between word multiply and 16-bit multiply"));
		    }
		}
	    }
  ...
  if (opcode.form->form >= LONG)
    {
      if (extype != EXEC_UNKNOWN)
	as_bad (_
		("Instruction uses long version, so it cannot be mixed as specified"));
      d30v_cleanup (FALSE);
	  ...
	  if (strcmp (opcode->op->name, "mvtsys") == 0)
	    {
	      ...
	      if ((myops[0].X_op == O_register) && ((myops[0].X_add_number == OPERAND_CONTROL) ||	/* psw */
      ...
      while (fm->form == index)
	{
	  ...
	  for (j = 0; match && fm->operands[j]; j++)
	      ...
	      else if (flags & OPERAND_REG)
		{
		  if (X_op != O_register
		      ...
		      || (!(flags & (OPERAND_FLAG | OPERAND_CONTROL))
			  && (num & OPERAND_FLAG))
	  ...
	  if (match && myops[j].X_op == 0)
		...
		if (myops[j].X_op == O_register
		    ...
		    && (d30v_operand_table[fm->operands[j]].
			flags & OPERAND_2REG))
		  as_warn (_
     ...
     segT seg ATTRIBUTE_UNUSED;
{
  ...
  switch (fixP->fx_r_type)
	...
	as_bad (_
		("line %d: unable to place address of symbol '%s' into a byte"),
	...
	*(unsigned char *) where = value;
    ...
    case BFD_RELOC_16:		/* Check for a bad .short directive.  */
      if (fixP->fx_addsy != NULL)
	as_bad (_
		("line %d: unable to place address of symbol '%s' into a short"),
		fixP->fx_line, S_GET_NAME (fixP->fx_addsy));
    ...
    case BFD_RELOC_64:		/* Check for a bad .quad directive.  */
      if (fixP->fx_addsy != NULL)
	as_bad (_
		("line %d: unable to place address of symbol '%s' into a quad"),
		fixP->fx_line, S_GET_NAME (fixP->fx_addsy));
      else
	{
