...
parse_reg (char *str, struct pdp11_code *operand)
{
  ...
  else if (strncmp (str, "pc", 2) == 0 || strncmp (str, "PC", 2) == 0)
    {
      operand->code = 7;
      str += 2;
    }
...
parse_ac5 (char *str, struct pdp11_code *operand)
{
  ...
  if (strncmp (str, "fr", 2) == 0 ||
      strncmp (str, "FR", 2) == 0 ||
      strncmp (str, "ac", 2) == 0 || strncmp (str, "AC", 2) == 0)
    {
      str += 2;
      switch (*str)
	{
  ...
  switch (*str)
	...
	case O_big:
	  ...
	  if (literal_float[1] != 0)
	    as_warn (_
		     ("Low order bits truncated in immediate float operand"));
	  break;
	default:
	  operand->error = "Error in expression";
	  break;
  ...
  *p = c;
  ...
  switch (op->type)
	...
	str++;
      ...
      if (op1.error)
	break;
      if (op1.reloc.exp.X_op != O_constant
	  || op1.reloc.type != BFD_RELOC_NONE)
	{
     ...
     char *arg;
{
  ...
  if (strcmp (arg, "all-extensions") == 0 || strcmp (arg, "all") == 0)
    {
  ...
  if (strcmp (arg, "cis") == 0)	/* commersial instructions */
    ...
    pdp11_extension[PDP11_EIS] = pdp11_extension[PDP11_LEIS] = yes;
	   ...
	   strcmp (arg, "kev11") == 0 || strcmp (arg, "kev-11") == 0)
    pdp11_extension[PDP11_FIS] = yes;
	   ...
	   strcmp (arg, "fp-11") == 0 ||
	   strcmp (arg, "fpj11") == 0 ||
	   strcmp (arg, "fp-j11") == 0 || strcmp (arg, "fpj-11") == 0)
     ...
     char *arg;
{
  ...
  *model++ = 0;
  ...
  if (strncmp (buf, "a", 1) == 0)	/* KA11 (11/15/20) */
    ...
    return set_option ("eis") && set_option ("mxps") && set_option ("ucode");

  else if (strncmp (buf, "dz", 2) == 0)	/* KD11-Z (11/44) */
    return set_option ("csm") &&
      set_option ("eis") &&
      set_option ("mfpt") && set_option ("mxps") && set_option ("spl");

  else if (strncmp (buf, "f", 1) == 0)	/* F11 (11/23/24) */
    return set_option ("eis") && set_option ("mfpt") && set_option ("mxps");

  else if (strncmp (buf, "j", 1) == 0)	/* J11 (11/53/73/83/84/93/94) */
    return set_option ("csm") &&
      set_option ("eis") &&
      set_option ("mfpt") &&
      set_option ("multiproc") && set_option ("mxps") && set_option ("spl");
     ...
     char *arg;
{
  if (strncmp (arg, "pdp-11/", 7) != 0 &&
      strncmp (arg, "pdp11/", 6) != 0 && strncmp (arg, "11/", 3) != 0)
    return 0;
  ...
  if (strcmp (arg, "03") == 0)	/* 11/03 */
    ...
    return set_cpu_model ("kd11dz");	/* KD11-Z */
	   ...
	   strcmp (arg, "50") == 0 ||
	   strcmp (arg, "55") == 0 || strcmp (arg, "70") == 0)
    ...
    return set_cpu_model ("kd11k");	/* KD11-K *//* FPP? */
	   ...
	   strcmp (arg, "83") == 0 ||
	   strcmp (arg, "84") == 0 ||
	   strcmp (arg, "93") == 0 || strcmp (arg, "94") == 0)
