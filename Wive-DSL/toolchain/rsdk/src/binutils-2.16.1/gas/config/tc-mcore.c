     ...
     unsigned *reg;
{
  ...
  else if (TOLOWER (s[0]) == 's' && TOLOWER (s[1]) == 'p' && !ISALNUM (s[2]))
    {
      *reg = 0;
      return s + 2;
    }
  ...
  for (p = litpool, i = 0; i < poolsize; i++, p++)
    {
	  ...
	  && e->X_add_number == p->e.X_add_number && ispcrel == p->ispcrel)
	{
	  p->refcnt++;
	  return i;
	}
     ...
     char *str;
{
  ...
  for (op_start = op_end = str;
       nlen < 20 && !is_end_of_line[(unsigned char) *op_end]
       && *op_end != ' '; op_end++)
    {
     ...
     char *arg;
{
  switch (c)
    ...
    case OPTION_EL:
      ...
      cpu = M340;
      break;
    case OPTION_JSRI2BSR_ON:
      do_jsri2bsr = 1;
      break;
    case OPTION_JSRI2BSR_OFF:
      do_jsri2bsr = 0;
      break;
    case OPTION_SIFILTER_ON:
      sifilter_mode = 1;
      break;
    case OPTION_SIFILTER_OFF:
      sifilter_mode = 0;
      break;
    default:
      return 0;
     ...
     int nbytes;
{
  if (!target_big_endian)
    switch (nbytes)
      {
      case 4:
	ptr[3] = (use >> 24) & 0xff;	/* fall through */
      case 3:
	ptr[2] = (use >> 16) & 0xff;	/* fall through */
      case 2:
	ptr[1] = (use >> 8) & 0xff;	/* fall through */
      case 1:
	ptr[0] = (use >> 0) & 0xff;
	break;
      default:
    ...
    switch (nbytes)
      {
      case 4:
	*ptr++ = (use >> 24) & 0xff;	/* fall through */
      case 3:
	*ptr++ = (use >> 16) & 0xff;	/* fall through */
      case 2:
	*ptr++ = (use >> 8) & 0xff;	/* fall through */
      case 1:
	*ptr++ = (use >> 0) & 0xff;
	break;
      default:
  ...
  switch (fixp->fx_r_type)
	...
	default:
	  ...
	  as_bad (_("Can not do %d byte %srelocation"),
		  fixp->fx_size, fixp->fx_pcrel ? _("pc-relative") : "");
	}
      break;
    }
