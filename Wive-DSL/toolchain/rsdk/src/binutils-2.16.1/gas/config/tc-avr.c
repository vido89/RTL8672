     ...
     char **line;
{
  ...
  switch (*op)
	...
	str = input_line_pointer;
	fix_new_exp (frag_now, where, 3, &op_expr, FALSE, r_type);
      }
      break;
    ...
    case 'K':
      ...
      avr_offset_expression (&op_expr);
      str = input_line_pointer;
      fix_new_exp (frag_now, where, 3, &op_expr, FALSE, BFD_RELOC_AVR_6_ADIW);
      break;
  ...
  if (fixP->fx_done)
	...
	case BFD_RELOC_AVR_6_ADIW:
	  if ((value > 63) || (value < 0))
	    as_bad_where (fixP->fx_file, fixP->fx_line,
			  _("operand out of range: %ld"), value);
	  bfd_putl16 ((bfd_vma) insn | (value & 0xf) | ((value & 0x30) << 2),
		      where);
	  break;
