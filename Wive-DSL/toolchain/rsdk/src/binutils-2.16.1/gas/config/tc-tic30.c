...
md_begin ()
{
  ...
  op_hash = hash_new ();
  {
    ...
    for (; current_optab < tic30_optab_end; current_optab++)
      {
	hash_err =
	  hash_insert (op_hash, current_optab->name, (char *) current_optab);
	if (hash_err)
	  as_fatal ("Internal Error: Can't Hash %s: %s", current_optab->name,
		    hash_err);
      }
  }
  parop_hash = hash_new ();
  {
    ...
    for (; current_parop < tic30_paroptab_end; current_parop++)
      {
	hash_err =
	  hash_insert (parop_hash, current_parop->name,
		       (char *) current_parop);
	if (hash_err)
	  as_fatal ("Internal Error: Can't Hash %s: %s", current_parop->name,
		    hash_err);
      }
  }
  reg_hash = hash_new ();
  {
    ...
    for (; current_reg < tic30_regtab_end; current_reg++)
      {
	hash_err =
	  hash_insert (reg_hash, current_reg->name, (char *) current_reg);
	if (hash_err)
	  as_fatal ("Internal Error: Can't Hash %s: %s", current_reg->name,
		    hash_err);
      }
  }
  ind_hash = hash_new ();
  {
    ...
    for (; current_ind < tic30_indaddrtab_end; current_ind++)
      {
	hash_err =
	  hash_insert (ind_hash, current_ind->syntax, (char *) current_ind);
	if (hash_err)
	  as_fatal ("Internal Error: Can't Hash %s: %s", current_ind->syntax,
		    hash_err);
      }
  }
	...
	case OtherReg:
	  if (insn.operand_type[1]->op_type & (AllReg))
	  ...
	  else
	    {
	      ...
	      as_bad
		("Incompatible first and second operands in instruction");
	      return;
	    }
	  break;
	case Indirect:
      ...
      if ((insn.operands > 0) && (insn.tm->opcode_modifier == AddressMode))
	{
	  ...
	  if (insn.addressing_mode == AM_Indirect)
		...
		am_insn = 1;
	      ...
	      insn.opcode |=
		(insn.operand_type[am_insn]->indirect.ARnum << 8);
		...
		{		/* Unresolved direct addressing mode instruction */
		  ...
		  fix_new_exp (frag_now, p + 2 - (frag_now->fr_literal), 2,
			       &insn.operand_type[am_insn]->direct.
			       direct_expr, 0, 0);
		}
	    }
	      ...
	      if (insn.operand_type[0]->immediate.resolved == 1)
		{
		  ...
		  switch (insn.tm->imm_arg_type)
			...
			{
			  as_bad
			    ("invalid short form floating point immediate operand");
			  return;
			}
			...
			as_warn
			  ("rounding down first operand float to unsigned int");
		      if (insn.operand_type[0]->immediate.u_number > 0xFFFF)
			as_warn
			  ("only lower 16-bits of first operand are used");
		      insn.opcode |=
			(insn.operand_type[0]->immediate.
			 u_number & 0x0000FFFFL);
		      md_number_to_chars (p, (valueT) insn.opcode, INSN_SIZE);
		      break;
		    case Imm_SInt:
		      ...
		      if (insn.operand_type[0]->immediate.decimal_found)
			as_warn
			  ("rounding down first operand float to signed int");
		      if (insn.operand_type[0]->immediate.s_number < -32768
			  || insn.operand_type[0]->immediate.s_number > 32767)
			{
			  as_bad
			    ("first operand is too large for 16-bit signed int");
			  return;
			}
		      insn.opcode |=
			(insn.operand_type[0]->immediate.
			 s_number & 0x0000FFFFL);
		      md_number_to_chars (p, (valueT) insn.opcode, INSN_SIZE);
		      break;
		    }
		}
		...
		{		/* Unresolved immediate label */
		  ...
		  md_number_to_chars (p, (valueT) insn.opcode, INSN_SIZE);
		  fix_new_exp (frag_now, p + 2 - (frag_now->fr_literal), 2,
			       &insn.operand_type[0]->immediate.imm_expr, 0,
			       0);
		}
	    }
	}
      else if (insn.tm->opcode_modifier == PCRel)
	{
	  ...
	  if ((insn.tm->operand_types[0] & (AllReg | Disp)) ==
	  ...
	  else if ((insn.tm->operand_types[0] & ARn) == ARn)
	    {
	      ...
	      insn.opcode |=
		((insn.operand_type[0]->reg.opcode - 0x08) << 22);
	      if (insn.operand_type[1]->op_type & (AllReg))
		{
	      ...
	      else
		{
		  ...
		  md_number_to_chars (p, (valueT) insn.opcode, INSN_SIZE);
		  fix_new_exp (frag_now, p + 2 - frag_now->fr_literal, 2,
			       &insn.operand_type[1]->immediate.imm_expr, 1,
			       0);
		}
	    }
	}
	       ...
	       (Abs24 | Direct))
	{
	  ...
	  if (insn.operand_type[0]->op_type & Direct)
	      ...
	      else
		{
		  ...
		  md_number_to_chars (p, (valueT) insn.opcode, INSN_SIZE);
		  fix =
		    fix_new_exp (frag_now, p + 3 - (frag_now->fr_literal), 1,
				 &insn.operand_type[0]->direct.direct_expr, 0,
				 0);
	  ...
	  if (insn.operand_type[0]->immediate.resolved == 1)
	    {
	      if (insn.operand_type[0]->immediate.u_number > 0x00FFFFFF)
		as_warn
		  ("first operand is too large for a 24-bit displacement");
	      insn.opcode |=
...
tic30_parallel_insn (char *token)
{
  ...
  {				/* Find operands */
	...
	if (current_posn != token_start)
	  {			/* yes, we've read in another operand */
	    ...
	    save_char = *current_posn;
	    *current_posn = '\0';
	    p_insn.operand_type[found_separator][p_insn.
						 operands[found_separator] -
						 1] =
	      tic30_operand (token_start);
	    *current_posn = save_char;
	    if (!p_insn.
		operand_type[found_separator][p_insn.
					      operands[found_separator] - 1])
	      return 1;
	  }
	else
	  {
     ...
     addressing, so there are no labels to resolve.  */
  {
	...
	p_insn.opcode |= (p_insn.operand_type[0][1]->reg.opcode << 22);
	if (p_insn.operand_type[1][1]->reg.opcode ==
	    p_insn.operand_type[0][1]->reg.opcode)
  ...
  if (*token == DIRECT_REFERENCE)
	...
	{			/* Strip operand */
	  ...
	  if ((*(token + count - 1) == 'a' || *(token + count - 1) == 'A') &&
		...
		{
		  as_bad
		    ("More than one AR register found in indirect reference");
		  return NULL;
		}
	  ...
	  if (*(token + count) == '(')
	      ...
	      if (is_digit_char (*(token + count + 1)))
		{
		  ...
		  if (found_disp)
		    {
		      as_bad
			("More than one displacement found in indirect reference");
		      return NULL;
		    }
		  count++;
		  while (*(token + count) != ')')
			...
			{
			  as_bad
			    ("Invalid displacement in indirect reference");
			  return NULL;
			}
      ...
      if (ind_addr_op)
	{
	  ...
	  else if ((ind_addr_op->displacement == DISP_REQUIRED)
		   && !found_disp)
	    {
	      /* Maybe an implied displacement of 1 again */
	      as_bad
		("required displacement wasn't given in indirect reference");
	      return 0;
	    }
	}
      else
	...
	{
	  if (!is_digit_char (*token) || *(token + 1) == 'x'
	      || strchr (token, 'h'))
	    {
	      char *save_input_line_pointer;
	      segT retval;
	      debug ("Probably a label: %s\n", token);
	      current_op->immediate.label =
	      ...
	      if (current_op->immediate.imm_expr.X_op == O_constant)
		{
		  ...
		  current_op->immediate.u_number =
		    (unsigned int) current_op->immediate.imm_expr.
		    X_add_number;
     ...
     char *next_line;
{
  ...
  parallel_insn =
    (char *) malloc (strlen (first_opcode) + strlen (first_operands) +
		     strlen (second_opcode) + strlen (second_operands) + 8);
  sprintf (parallel_insn, "q_%s_%s %s | %s", first_opcode, second_opcode,
