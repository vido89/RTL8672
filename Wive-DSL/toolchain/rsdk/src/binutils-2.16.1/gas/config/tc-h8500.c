  ...
  if (*src == '#')
  ...
  else
    {
      *ptr = skip_colonthing (exp_signed, src, op,
			      ispage ? ABS24 : PCREL8, PCREL8, PCREL16,
			      ABS24);
    }
}
  ...
  while (this_index == opcode->idx && !found)
      ...
      for (i = 0; i < noperands; i++)
	{
	  ...
	  switch (this_try->arg_type[i])
	    ...
	    case IMM16:
	      if (user->type == IMM16 || user->type == IMM8)
		{
     ...
     int high;
{
  if (operand->X_op != O_constant
      || operand->X_add_number < low || operand->X_add_number > high)
    {
...
insert (output, index, exp, reloc, pcrel)
     ...
     int pcrel;
{
  fix_new_exp (frag_now, output - frag_now->fr_literal + index, 4,	/* always say size is 4, but we know better */
	       exp, pcrel, reloc);
}
     ...
     h8500_operand_info *operand ATTRIBUTE_UNUSED;
{
  ...
  p = frag_var (rs_machine_dependent,
		...
		displacement.X_add_symbol, displacement.X_add_number, 0);
     ...
     char *str;
{
  ...
  for (op_start = op_end = str;
       !is_end_of_line[(unsigned char) *op_end] && *op_end != ' '; op_end++)
    {
      if (			/**op_end != '.'
	  && *op_end != ':'
	   	   	   	  && */ nlen < 10)
  ...
  switch (fragP->fr_subtype)
    ...
    case C (SCB_TST, UNDEF_WORD_DISP):
      ...
      fix_new (fragP,
	       ...
	       4, fragP->fr_symbol, fragP->fr_offset, 0, R_H8500_PCREL16);
  ...
  if (inst_size)
    {
      ...
      int next_inst =
	fragP->fr_fix + fragP->fr_address + disp_size + inst_size;
