     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  SA_SET_SYM_TAGNDX (def_symbol_in_progress, tag_find_or_make (symbol_name));
     ...
     int *punt;
{
  ...
  if (S_GET_STORAGE_CLASS (symp) == C_NT_WEAK
      && !S_IS_WEAK (symp) && weak_is_altname (S_GET_NAME (symp)))
    {
  ...
  if (!S_IS_DEFINED (symp)
      && !S_IS_WEAK (symp) && S_GET_STORAGE_CLASS (symp) != C_STAT)
    S_SET_STORAGE_CLASS (symp, C_EXT);
  ...
  if (!SF_GET_DEBUG (symp))
    {
	  ...
	  && S_GET_STORAGE_CLASS (real) == C_NULL && real != symp)
	{
     ...
     PTR x ATTRIBUTE_UNUSED;
{
  ...
  if (bfd_get_section_size (sec) == 0
      && nrelocs == 0
      && nlnno == 0
      && sec != text_section && sec != data_section && sec != bss_section)
    return;
  ...
  if (*input_line_pointer == ',')
      ...
      else
	{
	  ...
	  while (*input_line_pointer != '"'
		...
		case 's':
		  flags |= SEC_COFF_SHARED;	/* fall through */
		case 'd':
		  flags |= SEC_DATA | SEC_LOAD;	/* fall through */
		case 'w':
		  flags &= ~SEC_READONLY;
		  break;

		case 'a':
		  break;	/* For compatibility with ELF.  */
		case 'x':
		  flags |= SEC_CODE | SEC_LOAD;
		  break;
		case 'r':
		  flags |= SEC_DATA | SEC_LOAD | SEC_READONLY;
		  break;
     ...
     segT sec;
{
  ...
  bfd_vma align_power =
    (bfd_vma) sec->alignment_power + OCTETS_PER_BYTE_POWER;
  ...
  for (symbolP = symbol_rootP; symbolP; symbolP = symbol_next (symbolP))
    {
      printf (_("0x%lx: \"%s\" type = %ld, class = %d, segment = %d\n"),
	      ...
	      S_GET_STORAGE_CLASS (symbolP), (int) S_GET_SEGMENT (symbolP));
    }
}
...
static void do_linenos_for
PARAMS ((bfd *, object_headers *, unsigned long *));
		  ...
		  if (fix_ptr->fx_done == 0 && TC_COUNT_RELOC (fix_ptr))
		    {
		      ...
		      intr.r_vaddr =
			base + fix_ptr->fx_frag->fr_address +
			fix_ptr->fx_where;
			  ...
			  if (!symbol_ptr->sy_resolved)
			    {
				  ...
				  (symbol_ptr, &file, &line))
				as_bad_where (file, line,
					      _("unresolved relocation"));
			      else
				as_bad (_
					("bad relocation: symbol `%s' not in symbol table"),
		      ...
		      if (intr.r_type == R_IHIHALF)
			{
			  ...
			  (void) bfd_coff_swap_reloc_out (abfd, &intr,
							  ext_ptr);
			  ext_ptr++;
			}
#endif
		  ...
		  if (frag->fr_fix)
		    {
		      memcpy (buffer + frag->fr_address,
			      frag->fr_literal, (unsigned int) frag->fr_fix);
		      offset += frag->fr_fix;
		    }
     ...
     object_headers *h;
{
  ...
  H_SET_SIZEOF_OPTIONAL_HEADER (h,
				bfd_coff_swap_aouthdr_out (abfd, &h->aouthdr,
							   buffero));
  ...
  bfd_bwrite (buffero, (bfd_size_type) H_GET_SIZEOF_OPTIONAL_HEADER (h),
	      abfd);
     ...
     symbolS *symbolP;
{
  ...
  where += bfd_coff_swap_sym_out (abfd, &symbolP->sy_symbol.ost_entry, where);
     ...
     int what ATTRIBUTE_UNUSED;
{
  ...
  def_symbol_in_progress =
    (symbolS *) obstack_alloc (&notes, sizeof (*def_symbol_in_progress));
...
yank_symbols ()
{
  ...
  for (symbolP = symbol_rootP;
       symbolP; symbolP = symbolP ? symbol_next (symbolP) : symbol_rootP)
    {
		   ...
		   || S_GET_STORAGE_CLASS (symbolP) == C_WEAKEXT))
	{
	  ...
	  symbol_append (symbolP, symbol_extern_lastP, &symbol_externP,
			 &symbol_extern_lastP);
	  symbolP = hold;
	}
...
tie_tags ()
{
  ...
  for (symbolP = symbol_rootP; symbolP; symbolP = symbol_next (symbolP))
	...
	{
	  SA_SET_SYM_TAGNDX
	    (symbolP, ((symbolS *) SA_GET_SYM_TAGNDX (symbolP))->sy_number);
	}
  ...
  for (idx = SEG_E0; idx < SEG_LAST; idx++)
      ...
      if (s->scnhdr.s_nlnno != 0)
	{
	  ...
	  for (line_ptr = s->lineno_list_head;
		...
		line_ptr->line.l_addr.l_paddr +=
		  ((struct frag *) (line_ptr->frag))->fr_address;
	  ...
	  bfd_bwrite (buffer, (bfd_size_type) s->scnhdr.s_nlnno * LINESZ,
		      abfd);
	  free (buffer);
  ...
  for (fixP = segP->fix_root; fixP; fixP = fixP->fx_next)
    {
      ...
      if (linkrelax && (!sub_symbolP || pcrel))
	continue;
      ...
      if (add_symbolP != NULL)
	{
	  ...
	  if (add_symbolP->sy_value.X_op == O_symbol
	      && (!S_IS_DEFINED (add_symbolP) || S_IS_COMMON (add_symbolP)))
	    {
	...
	resolve_symbol_value (sub_symbolP);
      ...
      if (add_symbolP != NULL && add_symbolP->sy_mri_common)
	{
      ...
      if (!fixP->fx_bit_fixP && !fixP->fx_no_overflow)
	{
	  ...
	  /* Warn if a .word value is too large when treated as a
	     ...
	     the 68k.  */
	  if (!flag_signed_overflow_ok && size == 2 && add_number > 0x7fff)
	    as_bad_where (fixP->fx_file, fixP->fx_line,
			  _
