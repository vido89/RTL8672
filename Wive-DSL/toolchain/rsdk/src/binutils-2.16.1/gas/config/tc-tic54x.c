...
static void stag_add_field_symbols
PARAMS ((struct stag *, const char *, bfd_vma, symbolS *, const char *));
static void stag_add_field
PARAMS ((struct stag *, const char *, bfd_vma, struct stag *));
...
static int operands_match
PARAMS ((tic54x_insn *, struct opstruct *, int, const enum optype *, int,
	 int));
...
static int encode_integer
PARAMS ((tic54x_insn *, struct opstruct *, int, int, int, unsigned short));
...
static int encode_operand
PARAMS ((tic54x_insn *, enum optype, struct opstruct *));
...
static int tic54x_parse_parallel_insn_firstline
PARAMS ((tic54x_insn *, char *));
static int tic54x_parse_parallel_insn_lastline
PARAMS ((tic54x_insn *, char *));
     ...
     segT seg;
{
  while (frag != NULL)
    {
      if (frag->fr_fix == 0
	  && frag->fr_opcode == NULL && frag->tc_frag_data == 0)
     ...
     const char *root_stag_name;
{
  ...
  while (field != NULL)
	...
	stag_add_field_symbols (field->stag, name,
				field->offset, rootsym, root_stag_name);
      field = field->next;
    }
}
  ...
  if (!current_stag || current_stag->is_union != is_union)
    {
      as_bad (_(".end%s without preceding .%s"),
	      is_union ? "union" : "struct", is_union ? "union" : "struct");
      ignore_rest_of_line ();
      return;
    }
  ...
  if (current_stag->outer == NULL)
    {
      ...
      stag_add_field_symbols (current_stag, path,
			      S_GET_VALUE (current_stag->sym), NULL, NULL);
    }
     ...
     int type;
{
  ...
  do
      ...
      else
	{
	  ...
	  if (exp.X_op != O_constant && amode == c_mode && octets == 4)
	    {
  ...
  if (exp.X_op != O_constant || frag_bit_offset (frag_now, now_seg) == -1)
    {
      ...
      p = frag_var (rs_machine_dependent,
		    ...
		    make_expr_symbol (&exp), (offsetT) 0, (char *) bi);
      if (p)
	*p = 0;
     ...
     const char *ver;
{
  ...
  if (ver[0] == '5' && ver[1] == '4')
	...
	version = ver[2] - '0';
	       ...
	       && TOUPPER (ver[3]) == 'L'
	       && TOUPPER (ver[4]) == 'P' && (ver[2] == '5' || ver[2] == '6'))
	version = ver[2] - '0' + 10;
    }
...
md_begin ()
{
  ...
  for (tm = (template *) tic54x_optab; tm->name; tm++)
	...
	continue;
      ...
      if (hash_err)
	as_fatal ("Internal Error: Can't hash %s: %s", tm->name, hash_err);
    }
  parop_hash = hash_new ();
  ...
  for (math_proc = math_procs; math_proc->name; math_proc++)
    {
      ...
      hash_err = hash_insert (subsym_hash[0], math_proc->name,
			      (char *) math_proc);
      hash_err = hash_insert (math_hash, math_proc->name, (char *) math_proc);
    }
  ...
  if (is_indirect (operand) && strncasecmp (operand->buf, "*ar", 3) == 0)
    {
      ...
      valid_mod = *tmp == '\0' ||
	strcasecmp (tmp, "-") == 0 ||
	strcasecmp (tmp, "+") == 0 || strcasecmp (tmp, "+0%") == 0;
     ...
     enum optype type;
{
  switch (type)
    ...
    case OP_SBIT:
      return hash_find (sbit_hash, operand->buf) != 0
	|| is_absolute (operand);
    case OP_CC:
	...
	&& operand->exp.X_add_number >= 0 && operand->exp.X_add_number < 256;
    case OP_lk:
    case OP_lku:
	  ...
	  if ((insn->opcode[0].word & CC_ACC) != (cc->value & CC_ACC))
	    {
	      as_bad (_("Condition \"%s\" uses a different accumulator from "
			"a preceding condition"), operand->buf);
	      return 0;
	    }
     ...
     struct opstruct *operand;
{
  ...
  switch (type)
	...
	if (insn->opcount == 1)
	  {
	    if (!sbit)
	      {
		as_bad (_
			("A status register or status bit name is required"));
		return 0;
	      }
	...
	{
	  as_bad (_("Operand \"%s\" out of range (use 1 or 2)"),
		  operand->buf);
	  return 0;
	}
     ...
     int forced;
{
  ...
  if (strstr (line, ".if")
      || strstr (line, ".elseif") || strstr (line, ".break"))
    line_conditional = 1;
	  ...
	  || (!forced && is_name_beginner (current_char)))
	{
	  ...
	  if ((*name == '$' && ISDIGIT (name[1]) && name[2] == '\0')
	      ...
	      if (value == NULL)
		{
		  ...
		  value =
		    strcpy (xmalloc (strlen (name) + sizeof (digit) + 1),
			    name);
		  if (*value != '$')
	  ...
	  /* Check for built-in subsym and math functions.  */
		...
		{
		  as_bad (_
			  ("Missing '(' after substitution symbol function"));
		  break;
		}
	      ++ptr;
	      if (math_entry != NULL)
		{
		  ...
		  if (*ptr++ != ')')
		    {
		      as_bad (_
			      ("Extra junk in function call, expecting ')'"));
		      break;
		    }
	  ...
	  if (value != NULL && !eval_symbol)
	      ...
	      if (forced)
		{
		  if (c != ':')
		    {
		      as_bad (_
			      ("Missing forced substitution terminator ':'"));
		      break;
		    }
		  ++tail;
		}
      ...
      if (repeat_slot)
	{
	  if (insn.tm->flags & FL_NR)
	    as_warn (_("'%s' is not repeatable. "
		       "Resulting behavior is undefined."), insn.tm->name);
     ...
     char *name;
{
  ...
  if ((sym = (symbol *) hash_find (reg_hash, name)) != NULL ||
    ...
    {
      return symbol_new (name, reg_section,
			 (valueT) sym ? sym->value : 0, &zero_address_frag);
    }
...
#define MAX_LITTLENUMS 2
  ...
  *sizeP = 4;
  ...
  for (word = words; prec--;)
    {
      md_number_to_chars (literalP, (long) (*word++),
  ...
  switch (fixP->fx_r_type)
    ...
    case BFD_RELOC_TIC54X_PARTLS7:
      bfd_put_16 (stdoutput,
		  (bfd_get_16 (stdoutput, buf) & 0xFF80) | (val & 0x7F), buf);
      /* Indicate what we're actually writing, so that we don't get warnings
         about exceeding available space.  */
      *valP = val & 0x7F;
      break;
    case BFD_RELOC_TIC54X_PARTMS9:
      ...
      bfd_put_16 (stdoutput,
		  (bfd_get_16 (stdoutput, buf) & 0xFE00) | (val >> 7), buf);
      break;
    case BFD_RELOC_32:
    case BFD_RELOC_TIC54X_23:
      bfd_put_32 (stdoutput,
		  (bfd_get_32 (stdoutput, buf) & 0xFF800000) | val, buf);
      break;
    }
  ...
  if (sym != NULL)
    {
	  ...
	  || S_IS_COMMON (sym) || !S_IS_DEFINED (sym))
     ...
     fragS *frag;
{
  ...
  frag->fr_offset = (frag->fr_next->fr_address
		     - frag->fr_address - frag->fr_fix) / frag->fr_var;
  if (frag->fr_offset < 0)
    {
      as_bad_where (frag->fr_file, frag->fr_line,
