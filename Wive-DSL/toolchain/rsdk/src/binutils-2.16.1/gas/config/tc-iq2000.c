...
iq2000_add_macro (const char *name,
		  const char *semantics, const char **arguments)
{
  macro_entry *macro;
  sb macro_name;
  const char *namestr;
...
md_assemble (char *str)
{
  ...
  if (delayed_load_register
      && (last_insn_has_load_delay || last_insn_was_ldw))
    {
...
md_convert_frag (bfd * abfd ATTRIBUTE_UNUSED,
		 segT sec ATTRIBUTE_UNUSED, fragS * fragP ATTRIBUTE_UNUSED)
{
}

