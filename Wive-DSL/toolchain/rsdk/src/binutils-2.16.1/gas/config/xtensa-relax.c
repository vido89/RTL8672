...
static string_pattern_pair widen_spec_list[] = {
  ...
  {"addmi %ar,%as,%imm",
   "addmi %ar,%as,HI24S(%imm); addi %ar,%ar,LOW8(%imm)"},
  ...
  {"loop %as,%label | %as!=1 ? IsaUseLoops",
   "loop %as,%LABEL0;" "rsr.lend    %as;"	/* LEND */
   "wsr.lbeg    %as;"		/* LBEG */
   "addi    %as, %as, 0;"	/* lo8(%label-%LABEL1) */
   "addmi   %as, %as, 0;"	/* mid8(%label-%LABEL1) */
   "wsr.lend    %as;" "isync;" "rsr.lcount    %as;"	/* LCOUNT */
   "addi    %as, %as, 1;"	/* density -> addi.n %as, %as, 1 */
   "LABEL0"},
  {"loopgtz %as,%label | %as!=1 ? IsaUseLoops",
   "beqz    %as,%label;" "bltz    %as,%label;" "loopgtz %as,%LABEL0;" "rsr.lend    %as;"	/* LEND */
   "wsr.lbeg    %as;"		/* LBEG */
   "addi    %as, %as, 0;"	/* lo8(%label-%LABEL1) */
   "addmi   %as, %as, 0;"	/* mid8(%label-%LABEL1) */
   "wsr.lend    %as;" "isync;" "rsr.lcount    %as;"	/* LCOUNT */
  ...
  {"loopnez %as,%label | %as!=1 ? IsaUseLoops",
   "beqz     %as,%label;" "loopnez %as,%LABEL0;" "rsr.lend    %as;"	/* LEND */
   "wsr.lbeg    %as;"		/* LBEG */
   "addi    %as, %as, 0;"	/* lo8(%label-%LABEL1) */
   "addmi   %as, %as, 0;"	/* mid8(%label-%LABEL1) */
   "wsr.lend    %as;" "isync;" "rsr.lcount    %as;"	/* LCOUNT */
  ...
  {"beqz %as,%label ? IsaUseDensityInstruction",
   "bnez.n %as,%LABEL0;j %label;LABEL0"},
  {"bnez %as,%label ? IsaUseDensityInstruction",
   "beqz.n %as,%LABEL0;j %label;LABEL0"},
  {"beqz %as,%label", "bnez %as,%LABEL0;j %label;LABEL0"},
  {"bnez %as,%label", "beqz %as,%LABEL0;j %label;LABEL0"},
  {"beqzt %as,%label ? IsaUsePredictedBranches",
   "bnez %as,%LABEL0;j %label;LABEL0"},
  {"bnezt %as,%label ? IsaUsePredictedBranches",
   "beqz %as,%LABEL0;j %label;LABEL0"},
  ...
  {"beqt %as,%at,%label ? IsaUsePredictedBranches",
   "bne %as,%at,%LABEL0;j %label;LABEL0"},
  {"bnet %as,%at,%label ? IsaUsePredictedBranches",
   "beq %as,%at,%LABEL0;j %label;LABEL0"},
...
string_pattern_pair simplify_spec_list[] = {
  ...
  {"or %ar,%as,%at | %ar==%as | %as==%at ? IsaUseDensityInstruction",
   "nop.n"},
  {"or %ar,%as,%at | %ar!=%as | %as==%at ? IsaUseDensityInstruction",
   "mov.n %ar,%as"},
...
append_transition (TransitionTable * tt,
		   ...
		   TransitionRule * t, transition_cmp_fn cmp)
{
...
append_value_condition (TransitionRule * tr,
			CmpOp cmp, unsigned op1, unsigned op2)
{
...
append_constant_value_condition (TransitionRule * tr,
				 CmpOp cmp, unsigned op1, unsigned cnst)
{
  Precondition *cond = (Precondition *) xmalloc (sizeof (Precondition));
...
append_user_fn_field_op (BuildInstr * bi,
			 unsigned op1, OpType typ, unsigned src_op)
{
  BuildOp *b_op = (BuildOp *) xmalloc (sizeof (BuildOp));
...
parse_special_fn (const char *name,
		  const char **fn_name_p, const char **arg_name_p)
{
  char *p_start;
  const char *p_end;
...
split_string (split_rec * rec,
	      const char *in, char c, bfd_boolean elide_whitespace)
{
  int cnt = 0;
  int i;
  const char *p = in;
		    ...
		    const char *to_string ATTRIBUTE_UNUSED)
{
  ...
  for (req_option = initial_insn->options;
       req_option != NULL; req_option = req_option->next)
    {
      ReqOrOption *req_or_option = req_option->or_option_terms;

      if (req_or_option == NULL || req_or_option->next != NULL)
	continue;
      ...
      if (strncmp (req_or_option->option_name, "IsaUse", 6) == 0)
	{
	  ...
	  if (!strcmp (option_name, "DensityInstruction"))
	    ...
	    as_warn (_
		     ("invalid configuration option '%s' in transition rule '%s'"),
...
build_transition (insn_pattern * initial_insn,
		  ...
		  const char *from_string, const char *to_string)
{
  ...
  for (precond = initial_insn->preconds.head;
       precond != NULL; precond = precond->next)
    {
      op1 = NULL;
      op2 = NULL;
...
build_transition_table (const string_pattern_pair * transitions,
			int transition_count, transition_cmp_fn cmp)
{
