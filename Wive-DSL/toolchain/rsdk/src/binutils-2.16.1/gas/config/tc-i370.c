...
static void i370_make_relative
PARAMS ((expressionS * exp, expressionS * baseaddr));

#ifdef OBJ_ELF
static bfd_reloc_code_real_type i370_elf_suffix
     ...
     expressionS *expressionP;
{
  ...
  if (!ISALPHA (name[0]))
  ...
  else
    {
      c = get_symbol_end ();
      reg_number =
	reg_name_search (pre_defined_registers, REG_NAME_CNT, name);
...
static enum
{ SHLIB_NONE, SHLIB_PIC, SHILB_MRELOCATABLE } shlib = SHLIB_NONE;
#endif
...
i370_set_cpu ()
{
  ...
  if (i370_cpu == 0)
	...
	as_fatal ("Unknown default cpu = %s, os = %s", default_cpu,
		  default_os);
    }
}
  ...
  for (op = i370_opcodes; op < op_end; op++)
      ...
      if ((op->flags & i370_cpu) != 0)
	{
	  ...
	  if (retval != (const char *) NULL)
	    {
	      as_bad ("Internal assembler error for instruction %s",
		      op->name);
	      dup_insn = TRUE;
	    }
	}
  ...
  if (S_GET_VALUE (symbolP) && S_GET_VALUE (symbolP) != (valueT) size)
    {
      as_bad ("Length of .lcomm \"%s\" is already %ld. Not changed to %ld.",
	      S_GET_NAME (symbolP),
	      (long) S_GET_VALUE (symbolP), (long) size);
...
add_to_lit_pool (expressionS * exx, char *name, int sz)
{
  ...
  while (lit_count < next_literal_pool_place)
	...
	break;
	       ...
	       && name && !strcmp (name, literals[lit_count].sym_name))
	break;
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  if (strncmp (now_seg->name, ".text", 5))
	...
	{
	  as_bad (".ltorg without prior .using in section %s", now_seg->name);
	}
  ...
  if (!longlong_poolP && !word_poolP && !short_poolP && !byte_poolP)
    {
      /* Nothing to do */
  ...
  if (1 == biggest_literal_size)
    biggest_align = 0;
  else if (2 == biggest_literal_size)
    biggest_align = 1;
  else if (4 == biggest_literal_size)
    biggest_align = 2;
  else if (8 == biggest_literal_size)
    biggest_align = 3;
  else
    as_bad ("bad alignment of %d bytes in literal pool",
	    biggest_literal_size);
  if (0 == biggest_align)
    biggest_align = 1;
  ...
  for (litsize = 8; litsize; litsize /= 2)
	...
	{
	case 8:
	  current_poolP = longlong_poolP;
	  break;
	case 4:
	  current_poolP = word_poolP;
	  break;
	case 2:
	  current_poolP = short_poolP;
	  break;
	case 1:
	  current_poolP = byte_poolP;
	  break;
	default:
...
#define EMIT_ADDR_CONS_SYMBOLS
	      ...
	      if (literals[lit_count].sym_name)
		{
	      ...
	      emit_expr (&(literals[lit_count].exp),
			 literals[lit_count].size);
	    }
	  lit_count++;
	}
    }
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  if (O_constant != baseaddr.X_op
      && O_symbol != baseaddr.X_op && O_uminus != baseaddr.X_op)
    {
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  if (0 == strncmp (now_seg->name, ".text", 5))
	...
	{
	  as_bad
	    ("droping register %d in section %s does not match using register %d",
	...
	{
	  as_bad
	    ("droping register %d in section %s previously used in section %s",
     ...
     char *str;
{
  ...
  for (opindex_ptr = opcode->operands; *opindex_ptr != 0; opindex_ptr++)
    {
      ...
      /* If this is an index operand, and we are skipping it,
         just insert a zero.  */
      if (skip_optional_index && ((operand->flags & I370_OPERAND_INDEX) != 0))
	{
  ...
  for (i = 0; i < fc; i++)
	...
	{
	  reloc_howto_type *reloc_howto =
	    bfd_reloc_type_lookup (stdoutput, fixups[i].reloc);
	  int size;
	  fixS *fixP;
     ...
     segT seg;
{
  ...
  if (fixP->fx_addsy != NULL)
	...
	("\nmd_apply_fix3: symbol %s at 0x%x (%s:%d) val=0x%x addend=0x%x\n",
	 ...
	 fixP->fx_frag->fr_address + fixP->fx_where, fixP->fx_file,
	 fixP->fx_line, S_GET_VALUE (fixP->fx_addsy), value);
#endif
    }
  else
    fixP->fx_done = 1;
  ...
  if ((int) fixP->fx_r_type >= (int) BFD_RELOC_UNUSED)
	...
	("\nmd_apply_fix3: fixup operand %s at 0x%x in %s:%d addend=0x%x\n",
	 operand->name, fixP->fx_frag->fr_address + fixP->fx_where,
	 fixP->fx_file, fixP->fx_line, value);
#endif
  ...
  else
    {
      ...
      printf ("md_apply_fix3: reloc case %d in segment  %s %s:%d\n",
	      fixP->fx_r_type, segment_name (seg), fixP->fx_file,
	      fixP->fx_line);
	...
	default:
	  fprintf (stderr, "Gas failure, reloc value %d\n", fixP->fx_r_type);
	  fflush (stderr);
	  abort ();
	}
    }
     ...
     fixS *fixp;
{
  ...
  *reloc->sym_ptr_ptr = symbol_get_bfdsym (fixp->fx_addsy);
  ...
  printf ("\ngen_reloc(): sym %s (%s:%d) at addr 0x%x addend=0x%x\n",
	  ...
	  fixp->fx_file, fixp->fx_line, reloc->address, reloc->addend);
#endif
