...
#define gpr_interlocks \
  ( mips_opts.isa == ISA_MIPS5280 || \
   (mips_opts.isa != ISA_MIPS1 && \
        mips_opts.isa != ISA_MIPS4180 && \
        mips_opts.isa != ISA_MIPS4181 && \
	mips_opts.isa != ISA_MIPS5181) || \
...
#if 0
static inline int rlx_is_char(char c);
static inline void rlx_translate_alias(char *);
static inline int rlx_translate_operand(char *p, char *q);
static void rlx_print_expression(expressionS *);
#endif
  ...
  while (i < bfd_mips16_num_opcodes)
    {
      ...
      if (retval != NULL)
	as_fatal (_("internal: can't hash `%s': %s"), mips16_opcodes[i].name,
		  retval);
      do
	{
  ...
  for (i = 0; i < 32; i++)
    {
      ...
      sprintf (buf, "$%d", i);
      symbol_table_insert (symbol_new
			   (buf, reg_section, i, &zero_address_frag));
    }
  symbol_table_insert (symbol_new
		       ("$ra", reg_section, RA, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$fp", reg_section, FP, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$sp", reg_section, SP, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$gp", reg_section, GP, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$at", reg_section, AT, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$kt0", reg_section, KT0, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$kt1", reg_section, KT1, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$zero", reg_section, ZERO, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$pc", reg_section, -1, &zero_address_frag));

  /* 2006-11-28 tonywu: add radiax alias name translation */
  symbol_table_insert (symbol_new
		       ("$m0l", reg_section, M0L, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m0h", reg_section, M0H, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m0", reg_section,  M0, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m1l", reg_section, M1L, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m1h", reg_section, M1H, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m1", reg_section,  M1, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m2l", reg_section, M2L, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m2h", reg_section, M2H, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m2", reg_section,  M2, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m3l", reg_section, M3L, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m3h", reg_section, M3H, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$m3", reg_section,  M3, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$estatus", reg_section, ESTATUS, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$ecause", reg_section, ECAUSE, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$intvec", reg_section, INTVEC, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$mmd", reg_section,  MMD, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$cbs0", reg_section, CBS0, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$cbs1", reg_section, CBS1, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$cbs2", reg_section, CBS2, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$cbe0", reg_section, CBE0, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$cbe1", reg_section, CBE1, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$cbe2", reg_section, CBE2, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$lps0", reg_section, LPS0, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$lpe0", reg_section, LPE0, &zero_address_frag));
  symbol_table_insert (symbol_new
		       ("$lpc0", reg_section, LPC0, &zero_address_frag));
  /* 2006-11-28 tonywu: add radiax alias name translation */
  ...
  for (i = 0; i < 8; i++)
    {
      ...
      sprintf (buf, "$fcc%i", i);
      symbol_table_insert (symbol_new
			   (buf, reg_section, -1, &zero_address_frag));
    }
...
/* 2006-01-16 tonywu: md_assemble is called from read.c to
		   ...
		   const struct mips_cl_insn *insn)
{
  ...
  for (i = 0; i < MAX_DELAY_NOPS; i++)
    {
      pinfo = history[i].insn_mo->pinfo;
      if (pinfo == INSN_MACRO) {
        return 1;
    }

  if (pinfo & INSN_LOAD_MEMORY_DELAY ||
      pinfo & INSN_WRITE_GPR_T) { 
      if (insn_uses_reg(insn, EXTRACT_OPERAND(RT, history[i]), 
             MIPS_GR_REG)) {
          return 1;
	}
    }
  ...
  if (pinfo1 & INSN_LOAD_MEMORY_DELAY ||
      pinfo1 & INSN_WRITE_GPR_T) {
    if (insn_uses_reg(insn, EXTRACT_OPERAND(RT, history[0]), 
		      MIPS_GR_REG)) {
	return 1;
    }
  }

  if (pinfo2 & INSN_LOAD_MEMORY_DELAY ||
      pinfo1 & INSN_WRITE_GPR_T) {
    if (insn_uses_reg(insn, EXTRACT_OPERAND(RT, history[1]), 
		      MIPS_GR_REG)) {
	return 1;
    }
  }
	     ...
	     bfd_reloc_code_real_type *reloc_type)
{
      ...
      if (address_expr->X_op == O_constant)
	{
	  ...
	  switch (*reloc_type)
		...
		as_bad (_("jump address range overflow (0x%lx)"),
	      ...
	      ip->insn_opcode |=
		(((address_expr->
		   X_add_number & 0x7c0000) << 3) | ((address_expr->
						      X_add_number &
						      0xf800000) >> 7) |
  ...
  /* 2006-01-06 tonywu: branch delay slot filling starts here */
  if (mips_relax.sequence != 2 && !mips_opts.noreorder)
    {
...
macro (struct mips_cl_insn *ip)
{
  ...
  switch (mask)
      ...
      if (HAVE_64BIT_GPRS && sizeof (maxnum) > 4)
	{
      ...
      if (imm_expr.X_op == O_constant
	  && imm_expr.X_add_number >= maxnum && (HAVE_32BIT_GPRS
						 || sizeof (maxnum) > 4))
	{
	do_false:
...
validate_mips_insn (const struct mips_opcode *opc)
{
  ...
  if ((used_bits & opc->match) != opc->match)
    {
      as_bad (_("internal: bad mips opcode (mask error): %s %s"), opc->name,
	      opc->args);
      return 0;
    }
#define USE_BITS(mask,shift)	(used_bits |= ((mask) << (shift)))
  while (*p)
	...
	break;
      ...
      case '`':
	USE_BITS (OP_MASK_EVENREG, OP_SH_EVENREG);
	break;
      case '~':
        USE_BITS (OP_MASK_IMMIDATE88, OP_SH_IMMIDATE88);
        break;
      case '#':
        USE_BITS (OP_MASK_IMMIDATE74, OP_SH_IMMIDATE74);
        break;
      case '@':
        USE_BITS (OP_MASK_IMMIDATE6b, OP_SH_IMMIDATE6b);
        break;
      case '-':
        USE_BITS (OP_MASK_OFFSET6A, OP_SH_OFFSET6A);
        break;
      /* dbb: new instructions support */
#ifdef DVR
      /* DVR taro: modified for supporting audio engine instructions */
      case '$':
	USE_BITS (OP_MASK_IMMIDATEa6, OP_SH_IMMIDATEa6);
	break;
	/* DVR taro: modified for supporting audio engine instructions */
	/* DVR jacky: modified for supporting video engine instructions */
      case '&':
	USE_BITS (OP_MASK_IMMIDATE6A, OP_SH_IMMIDATE6A);
	break;
	/* DVR jacky: modified for supporting video engine instructions */
#endif
...
do_msb:
		  ...
		  if ((long) imm_expr.X_add_number < 0
		      || ((unsigned long) imm_expr.X_add_number
			  + lastpos) < limlo
		      || ((unsigned long) imm_expr.X_add_number + lastpos) >
		      limhi)
		    {
		      as_bad (_("Improper insert size (%lu, position %lu)"),
			      (unsigned long) imm_expr.X_add_number,
...
do_msbd:
		...
		as_bad (_("(%d) is out of range 0-32"), imm_expr.X_add_number);
	      ip->insn_opcode |= 
		    ((imm_expr.X_add_number & OP_MASK_IMMIDATEa6) << OP_SH_IMMIDATEa6);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
	    /* DVR taro: modified for supporting audio engine instructions */
		...
		as_bad (_("(%d) is too large to stay in 10 bits"),imm_expr.X_add_number);
              ip->insn_opcode |= 
		   ((imm_expr.X_add_number & OP_MASK_IMMIDATE6A) << OP_SH_IMMIDATE6A);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
	    /* DVR jacky: modified for supporting video engine instructions */
	      ...
	      if (imm_expr.X_op == O_constant)
		{
		  ...
		  ip->insn_opcode |=
		    (((imm_expr.
		       X_add_number >> 3) & OP_MASK_OFFSET6A) <<
		     OP_SH_OFFSET6A);
		  imm_expr.X_op = O_absent;
		}
	      else
		...
		as_bad (_("(%d) is too large to stay in 14 bits"),
			(int) imm_expr.X_add_number);
	      ip->insn_opcode |=
		(((imm_expr.
		   X_add_number >> 3) & OP_MASK_IMMIDATE6b) <<
		 OP_SH_IMMIDATE6b);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
		...
		as_bad (_("(%d) is out of range 0-8"),
	      ...
	      ip->insn_opcode |=
		((imm_expr.
		  X_add_number & OP_MASK_IMMIDATE74) << OP_SH_IMMIDATE74);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
		...
		case 0:
		  if (((imm_expr.X_add_number + 128) & ~0xff) != 0)
		    as_bad (_("(%d) is too large to stay in 8 bits"),
			    (int) imm_expr.X_add_number);
		  break;
		case 2:
	      ...
	      if (s[0] == '$')
		{
		  ...
		  if (ISDIGIT (s[1]))
		    ...
		    as_bad (_("register number needed"));
		}
	      else
		{
	      ...
	      if (s[0] == '$')
		{
		  ...
		  if (regno == AT
		      ...
		      && *args != 'G'
		      && *args != 'K') {
		      if ((strncmp(ip->insn_mo->name, "mta2", 4) != 0 || *args != 'd') &&
			  (strncmp(ip->insn_mo->name, "mfa", 3) != 0 || *args != 't') &&
			  (strncmp(ip->insn_mo->name, "diva", 4) != 0 || *args != 'd') && 
			  (strncmp(ip->insn_mo->name, "mtru", 4) != 0 || *args != 'd') && 
			  (strncmp(ip->insn_mo->name, "mfru", 4) != 0 || *args != 'd') && 
			  (strncmp(ip->insn_mo->name, "multa", 5) != 0 || *args != 'd') &&
			  (strncmp (ip->insn_mo->name, "mulna2", 6) != 0 || *args != 'd') && 
			  (strncmp (ip->insn_mo->name, "cmulta", 6) != 0 || *args != 'd') && 
			  (strncmp (ip->insn_mo->name, "madda", 5) != 0 || *args != 'd') && 
			  (strncmp (ip->insn_mo->name, "msuba", 5) != 0 || *args != 'd') && 
	 ...
	 * to figure out where it goes in the instruction.  */
		  switch (c)
		    ...
		    case 's':
			    ...
			    || strncmp(ip->insn_mo->name, "subma", 5) == 0) {
                         if ((regno & 3) == 0 || (regno & 3) == 3)
                           as_bad (_("Illegal reg number (%d) in %s"), regno, str);
                      }
		    ...
		    case 't':
		      if (strncmp (ip->insn_mo->name, "addma", 5) == 0 || \
			  strncmp (ip->insn_mo->name, "subma", 5) == 0 || \
			  strncmp (ip->insn_mo->name, "mfa", strlen(ip->insn_mo->name)) == 0) {
			if (((regno & 3) == 0) || ((regno & 3) == 3))
			  as_bad (_("Illegal reg number (%d) in %s"), regno, str);
		      }
...
#define OPTION_MARCH (OPTION_ARCH_BASE + 0)
  ...
  {"mcpu",  required_argument, NULL, OPTION_MARCH},
...
mips_after_parse_args (void)
{
  ...
  if (file_mips_isa != ISA_UNKNOWN)
      ...
      if (arch_info != 0)
	{
	  ...
	  { 
	      /* 2006-02-14 tonywu: make -march overrides -mipsN */
	      char *tmp_isa1 = (char *) mips_cpu_info_from_isa(file_mips_isa)->name;
	      char *tmp_isa2 = (char *) mips_cpu_info_from_isa(arch_info->isa)->name;

	      as_warn (_("march|mtune|mcpu %s overrides %s"), tmp_isa2, tmp_isa1);
	      file_mips_isa = arch_info->isa;
	  }
	}
      else
  ...
  /* 2006-02-14 tonywu: add default cpu */
  if (arch_info == 0)
    arch_info = mips_parse_cpu ("internal use", "lx4180");
#if 0
...
md_apply_fix3 (fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  /* 2006-01-13 tonywu: remove this for binutils to support old gcc */
  ...
  switch (fixP->fx_r_type)
    ...
    case BFD_RELOC_MIPS16_JMP:
      /* We currently always generate a reloc against a symbol, which
         ...
         defined.  */
      /* 2006-01-17 tonywu: fall through and let assembler bitch about it */
      /**valP = 0;*/
      break;
...
relaxed_branch_length (fragS *fragp, asection *sec, int update)
{
  ...
  if (fragp && S_IS_DEFINED (fragp->fr_symbol)
...
tc_gen_reloc (asection *section ATTRIBUTE_UNUSED, fixS *fixp)
{
  ...
  *reloc->sym_ptr_ptr = symbol_get_bfdsym (fixp->fx_addsy);
  ...
  /* 2006-01-13 tonywu: add this for binutils to support old gcc */
#if 0
  assert (! fixp->fx_pcrel);
...
md_convert_frag (bfd *abfd ATTRIBUTE_UNUSED, segT asec, fragS *fragp)
{
  ...
  if (RELAX_MIPS16_P (fragp->fr_subtype))
      ...
      if (op->pcrel)
	{
      ...
      if (ext && (RELAX_MIPS16_JAL_DSLOT (fragp->fr_subtype)
...
static const struct mips_cpu_info mips_cpu_info_table[] =
{
  ...
  /*{ "4180",           1,      ISA_MIPS1,      CPU_R3000},*/

  { "lx4180",         1,      ISA_MIPS4180,   CPU_LX4180 },
  { "rlx4181",        1,      ISA_MIPS4181,   CPU_RLX4181 },
  { "rlx5181",        1,      ISA_MIPS5181,   CPU_RLX5181 },
  { "lx5280",         1,      ISA_MIPS5280,   CPU_LX5280 },
...
mips_matching_cpu_name_p (const char *canonical, const char *given)
{
  ...
  /* 2006-01-19 tonywu: add to parse LX/RLX CPUs */
  if (TOLOWER (canonical[0]) == 'l' && 
	  TOLOWER (canonical[1]) == 'x')
    canonical += 2;
  else if (TOLOWER (canonical[0]) == 'r' && 
           TOLOWER (canonical[1]) == 'l' &&
           TOLOWER (canonical[2]) == 'x') 
    canonical += 3;
...
-warnforneedednops      Warn when nops needed but can't be inserted for\n                        .set noreorder\n"));
#endif
}
...
static inline int
rlx_is_insn_lt(const struct mips_cl_insn *insn1, const struct mips_cl_insn *insn2)
{
  int is_lt = 0;
...
static inline int
rlx_is_insn_st(const struct mips_cl_insn *insn1, const struct mips_cl_insn *insn2)
{
  int is_st = 0;
