	  ...
	  if (trap_id_warn_once == 0)
	    {
	      trap_id_warn_once = 1;
	      as_bad (_
		      ("Trap id must be within [0x30..0x39] or [0x40..0xff]."));
	    }
	}
  ...
  if (oper->X_op == O_constant)
	...
	reloc = BFD_RELOC_16;
      ...
      fixp = fix_new_exp (frag_now, f - frag_now->fr_literal, 2,
			  oper, reloc == BFD_RELOC_16_PCREL, reloc);
		 ...
		 int nb_operands, int jmp_mode)
{
  ...
  if (operands[0].exp.X_op == O_constant)
	...
	{
	  as_bad (_("Operand out of range for a relative branch: `%ld'"), n);
	}
	  ...
	  if (move_insn)
	    {
	      ...
	      number_to_chars_bigendian (f, byte, 1);
	      fix_new (frag_now, f - frag_now->fr_literal, 1,
		       op->exp.X_add_symbol, op->exp.X_add_number, 0,
	  ...
	  else
	    {
	      ...
	      frag_var (rs_machine_dependent, 2, 2,
			...
			op->exp.X_add_symbol, op->exp.X_add_number, f);
	    }
	}
      return 3;
    }
	    ...
	    int nb_operands ATTRIBUTE_UNUSED)
{
  ...
  if (format & (M6811_OP_DIRECT | M6811_OP_IMM8))
  ...
  else if (IS_CALL_SYMBOL (format) && nb_operands == 1)
    {
      ...
      fixup24 (&operands[i].exp, format & M6811_OP_IND16, operands[i].mode);
      i++;
    }
  ...
  if (RELAX_LENGTH (fragP->fr_subtype) == STATE_UNDF)
	      ...
	      && RELAX_STATE (fragP->fr_subtype) == STATE_INDEXED_OFFSET))
	{
	  ...
	  switch (RELAX_STATE (fragP->fr_subtype))
	    ...
	    default:
	      as_fatal (_("Subtype %d is not recognized."),
			fragP->fr_subtype);
	    }
	  frag_wane (fragP);
	...
	case STATE_INDEXED_PCREL:
	  ...
	  fragP->fr_subtype = ENCODE_RELAX (STATE_INDEXED_PCREL, STATE_BITS5);
	  break;
...
md_apply_fix3 (fixS * fixP, valueT * valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  switch (fixP->fx_r_type)
	...
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _
		      ("Auto increment/decrement offset '%ld' is out of range."),
		      value);
      if (where[0] & 0x8)
	value = 8 - value;
      else
