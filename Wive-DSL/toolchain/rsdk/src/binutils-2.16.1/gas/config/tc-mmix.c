     ...
     expressionS exp[];
{
  ...
  *sregend = c;
  ...
  if (expp_reg->X_op != O_illegal
      && expp_reg->X_op != O_absent && regno != -1)
    {
  ...
  if (now_seg == data_section)
      ...
      if (lowest_data_loc != (bfd_vma) - 1 && (lowest_data_loc & 3) != 0)
	{
	  if (data_has_contents)
	    ...
	    as_bad (_
		    ("unaligned data at an absolute location is not supported"));
  ...
  switch (instruction->operands)
	...
	if ((n_operands != 2 && n_operands != 3)
		...
		|| (exp[op2no].X_op == O_constant
		    && (exp[op2no].X_add_number > 255
			|| exp[op2no].X_add_number < 0))) || (n_operands == 3
    ...
    case mmix_operands_save:
      /* "$X,0"; SAVE.  */
      if (n_operands != 2
	  || exp[1].X_op != O_constant || exp[1].X_add_number != 0)
	{
	  as_bad (_("invalid operands to opcode %s: `%s'"),
		  instruction->name, operands);
	  return;
      ...
      else if (n_operands == 1 && exp[0].X_op != O_register)
	{
	  if (exp[0].X_op == O_constant)
	    ...
	    fix_new_exp (opc_fragP, opcodep - opc_fragP->fr_literal + 1,
	       ...
	       && exp[0].X_op != O_register && exp[1].X_op != O_register)
	{
	  /* Two operands.  */

	  if (exp[0].X_op == O_constant)
	    {
	      if (exp[0].X_add_number > 255 || exp[0].X_add_number < 0)
		{
		  as_bad (_("invalid operands to opcode %s: `%s'"),
			  instruction->name, operands);
		  return;
	  ...
	  if (exp[1].X_op == O_constant)
	    {
	      if (exp[1].X_add_number > 255 * 255 || exp[1].X_add_number < 0)
		{
		  as_bad (_("invalid operands to opcode %s: `%s'"),
			  instruction->name, operands);
		  return;
	    ...
	    fix_new_exp (opc_fragP, opcodep - opc_fragP->fr_literal + 2,
	       ...
	       && exp[0].X_op != O_register
	       && exp[1].X_op != O_register && exp[2].X_op != O_register)
	{
	  /* Three operands.  */
	  ...
	  if (exp[1].X_op == O_constant)
	    {
	      if (exp[1].X_add_number > 255 || exp[1].X_add_number < 0)
		{
		  as_bad (_("invalid operands to opcode %s: `%s'"),
			  instruction->name, operands);
		  return;
	  ...
	  if (exp[2].X_op == O_constant)
	    {
	      if (exp[2].X_add_number > 255 || exp[2].X_add_number < 0)
		{
		  as_bad (_("invalid operands to opcode %s: `%s'"),
			  instruction->name, operands);
		  return;
	...
	break;
	  ...
	  || exp[0].X_op == O_register
	  || (exp[0].X_op == O_constant
	      && (exp[0].X_add_number < 0 || exp[0].X_add_number > 255)))
	{
	  as_bad (_("invalid operands to opcode %s: `%s'"),
		  instruction->name, operands);
	  return;
     ...
     int unused ATTRIBUTE_UNUSED;
{
  ...
  if (exp.X_op != O_constant
      ...
      || exp.X_add_number < 0 || exp.X_add_number > 65535)
    {
     ...
     segT segment;
{
  ...
  switch (fixP->fx_r_type)
	...
	{
	  as_bad_where (fixP->fx_file, fixP->fx_line, _("invalid operands"));
     ...
     fixS *fixP;
{
  bfd_signed_vma val
    ...
    + (fixP->fx_addsy != NULL
       && !S_IS_WEAK (fixP->fx_addsy)
       && !S_IS_COMMON (fixP->fx_addsy) ? S_GET_VALUE (fixP->fx_addsy) : 0);
  arelent *relP;
  ...
  if (bfd_section_size (section->owner, section) == 0)
    as_bad_where
      (fixP->fx_file, fixP->fx_line, fixP->fx_r_type == BFD_RELOC_MMIX_LOCAL
  ...
  switch (fixP->fx_r_type)
			 ...
			 MMIX_REG_CONTENTS_SECTION_NAME) == 0))
	{
	  ...
	  && (S_GET_SEGMENT (addsy) != real_reg_section
	      || val > 255 || val < 0) && !bfd_is_abs_section (addsec))
	goto badop;
      ...
      if (addsy == NULL || addsec != real_reg_section)
	{
	  ...
	  if (S_IS_DEFINED (addsy)
	      && !bfd_is_com_section (addsec) && !S_IS_WEAK (addsy))
	    {
	      if (!symbol_section_p (addsy) && !bfd_is_abs_section (addsec))
		as_fatal (_
			  ("internal: BFD_RELOC_MMIX_BASE_PLUS_OFFSET not resolved to section"));
			 ...
			 MMIX_REG_CONTENTS_SECTION_NAME) == 0))
	{
	  ...
	  && (addsec != real_reg_section
	      || val > 255 || val < 0) && !bfd_is_und_section (addsec))
...
mmix_handle_mmixal ()
{
  ...
  if (ISDIGIT (input_line_pointer[0])
      && input_line_pointer[1] == 'H' && ISSPACE (input_line_pointer[2]))
    {
      char *s;
  ...
  if (s0 != input_line_pointer && s0[-1] == ':'
      && (s0 - 2 != input_line_pointer || !ISDIGIT (s0[-2])))
    s0[-1] = ' ';
  else if (label != NULL)
    {
  ...
  /* Skip the insn.  */
  while (*s
	 && !ISSPACE (*s) && *s != ';' && !is_end_of_line[(unsigned int) *s])
    s++;
  ...
  while ((c = *s) != 0
	 && !ISSPACE (c) && c != ';' && !is_end_of_line[(unsigned int) c])
    {
      if (c == '"')
	{
	  s++;
	...
	{
	  if ((s[1] != 'B' && s[1] != 'F')
	      || is_part_of_name (s[-1]) || is_part_of_name (s[2]))
	    s++;
	  else
	    {
	      s[0] = (s[1] == 'B'
     ...
     long stretch;
{
  switch (fragP->fr_subtype)
	...
	know (lastfrag->fr_type == rs_fill
	      && lastfrag->fr_fix == 0 && lastfrag->fr_var == 0);
	...
	if ((lastfrag->fr_address
	    ...
	    - (fragP->fr_address + fragP->fr_fix) > GETA_0F || !pushj_stubs)
  ...
  if (n_of_raw_gregs != 0)
    {
      ...
      asection *regsec = bfd_make_section_old_way (stdoutput,
						   MMIX_REG_CONTENTS_SECTION_NAME);
      subseg_set (regsec, 0);
...
mmix_frob_file ()
{
  ...
  for (i = 0; i < n_of_cooked_gregs; i++)
      ...
      if (fixP->fx_subsy != NULL)
	{
      ...
      if (S_IS_DEFINED (sym)
	  && !bfd_is_com_section (S_GET_SEGMENT (sym)) && !S_IS_WEAK (sym))
	{
  ...
  for (i = 0; i < n_greg_symbols; i++)
    qsort (all_greg_symbols[i]->greg_fixes, all_greg_symbols[i]->n_gregs,
	   sizeof (all_greg_symbols[i]->greg_fixes[0]),
	   cmp_greg_symbol_fixes);
  ...
  if (real_reg_section != NULL)
    {
      ...
      if (bfd_get_section_flags (stdoutput, real_reg_section) &
	  SEC_HAS_CONTENTS)
	as_fatal (_("register section has contents\n"));

      /* Really remove the section.  */
      for (secpp = &stdoutput->sections;
	   *secpp != real_reg_section; secpp = &(*secpp)->next)
	;
...
mmix_md_elf_section_change_hook ()
{
  if (doing_bspec)
    as_bad (_
	    ("section change from within a BSPEC/ESPEC pair is not supported"));
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  if (doing_bspec)
    {
      as_bad (_
	      ("directive LOC from within a BSPEC/ESPEC pair is not supported"));
      return;
    }
  ...
  if (exp.X_op == O_illegal
      || exp.X_op == O_absent
      || exp.X_op == O_big || section == undefined_section)
    {
  ...
  if (section == absolute_section)
      ...
      if (exp.X_add_number < ((offsetT) 0x20 << 56))
	{
	  ...
	  if (exp.X_add_number < 0
	    ...
	    {
	      as_bad (_
		      ("LOC expression stepping backwards is not supported"));
	      exp.X_op = O_absent;
	    }
	  else
	    {
	...
	default:
	  {
		...
		|| (exp.X_op != O_constant
		    && (exp.X_op != O_register || exp.X_add_number <= 255)))
	      {
