...
static char *parse_exp_save_ilp PARAMS ((char *, expressionS *));
static int c_to_r PARAMS ((char));
static void ipush_code PARAMS ((pj_opc_info_t *, char *));
static void fake_opcode
PARAMS ((const char *, void (*)(struct pj_opc_info_t *, char *)));
static void alias PARAMS ((const char *, const char *));
  ...
  if (opcode->opcode == -1)
      ...
      for (an = 0; opcode->arg[an]; an++)
	{
	  ...
	  fix_new_exp (frag_now,
		       ...
		       &arg,
		       PCREL (opcode->arg[an]),
		       pending_reloc ? pending_reloc : c_to_r (opcode->
							       arg[an]));
