     ...
     unsigned int line;
{
  ...
  if (operand->flags & (S390_OPERAND_SIGNED | S390_OPERAND_PCREL))
      ...
      if (val < min || val > max)
	{
      ...
      uval =
	(addressT) val & ((((addressT) 1 << (operand->bits - 1)) << 1) - 1);
      ...
      if (uval < min || uval > max)
	{
	  ...
	  as_bad_value_out_of_range (_("operand"), uval, (offsetT) min,
  ...
  do
    {
	  ...
	  && *input_line_pointer == '@'
	  && (suffix =
	      s390_elf_suffix (&input_line_pointer, &exp)) != ELF_SUFFIX_NONE)
	{
	  bfd_reloc_code_real_type reloc;
      ...
      else if (ex.X_op == O_register || ex.X_op == O_constant)
	{
	  ...
	  if (ex.X_op != O_register && ex.X_op != O_constant)
	  ...
	  else
	    {
		  ...
		  && ex.X_add_number == 0 && warn_areg_zero)
	  ...
	  if (*str != '(')
	    {
	      ...
	      operand = s390_operands + *(++opindex_ptr);
	      if (!
		  (operand->flags & (S390_OPERAND_INDEX | S390_OPERAND_BASE)))
  ...
  *s++ = '\0';
  ...
  if (exp.X_op == O_constant)
  ...
  else if (exp.X_op == O_big)
    {
	  ...
	  && opformat->oplen == 6 && generic_bignum[3] == 0)
	{
