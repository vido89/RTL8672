  ...
  if (opcode->amode == ADDR_IMPLIED)
  ...
  else if (opcode->amode == ADDR_PC_REL)
    {
      ...
      output = frag_var (rs_machine_dependent,
			 ...
			 immediate.X_add_symbol, immediate.X_add_number, 0);
    }
  else
    {
      switch (opcode->amode)
      ...
      if (opcode->amode == ADDR_BLOCK_MOVE)
	{
	  ...
	  fix_new_exp (frag_now,
		       ...
		       1, &immediate, 0, R_W65_ABS8S16);

	  fix_new_exp (frag_now,
		       output + 2 - frag_now->fr_literal,
		       1, &immediate1, 0, R_W65_ABS8S16);
	}
      else if (type >= 0
	       && opcode->amode != ADDR_IMPLIED
	       && opcode->amode != ADDR_ACC && opcode->amode != ADDR_STACK)
	{
	  fix_new_exp (frag_now,
		       output + 1 - frag_now->fr_literal,
		       size, &immediate, pcrel, type);
	}
    }
  output[0] = opcode->val;
}
     ...
     char *str;
{
  ...
  if (opcode->amode != ADDR_IMPLIED && opcode->amode != ADDR_STACK)
    {
     ...
     fragS *fragP;
{
  ...
  switch (fragP->fr_subtype)
    ...
    case C (COND_BRANCH, UNDEF_WORD_DISP):
      ...
      fix_new (fragP,
	       fragP->fr_fix + 3,
	       4, fragP->fr_symbol, fragP->fr_offset, 0, R_W65_PCR16);
