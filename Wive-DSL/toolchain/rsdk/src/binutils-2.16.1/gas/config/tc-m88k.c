...
static char *get_bf_offset_expression
PARAMS ((char *param, unsigned *offsetp));
...
PARAMS ((char *, struct field_val_assoc *, unsigned *));
static char *get_reg
PARAMS ((char *param, unsigned *regnop, unsigned int reg_prefix));
  ...
  switch (insn.reloc)
    ...
    case RELOC_HI16:
      fix_new_exp (frag_now,
		   ...
		   2, &insn.exp, 0, insn.reloc);
      break;

    case RELOC_IW16:
      fix_new_exp (frag_now,
		   thisfrag - frag_now->fr_literal,
		   4, &insn.exp, 0, insn.reloc);
      break;

    case RELOC_PC16:
      fix_new_exp (frag_now,
		   thisfrag - frag_now->fr_literal + 2,
		   2, &insn.exp, 1, insn.reloc);
      break;

    case RELOC_PC26:
      fix_new_exp (frag_now,
		   thisfrag - frag_now->fr_literal,
		   4, &insn.exp, 1, insn.reloc);
      break;
     ...
     symbolS *to_symbol;
{
  ...
  ptr[2] = 0x00;
  ptr[3] = 0x00;
  fix_new (frag, ptr - frag->fr_literal, 4, to_symbol, (offsetT) 0, 0, RELOC_PC26);	/* Botch: Shouldn't this be RELOC_PC16? */
}
     ...
     symbolS *to_symbol;
{
  ...
  ptr[3] = 0x00;
  fix_new (frag,
	   ptr - frag->fr_literal, 4, to_symbol, (offsetT) 0, 0, RELOC_PC26);
}
