...
static struct d10v_opcode *find_opcode
PARAMS ((struct d10v_opcode * opcode, expressionS ops[]));
static unsigned long build_insn
PARAMS ((struct d10v_opcode * opcode, expressionS * opers,
	 unsigned long insn));
static void write_long PARAMS ((unsigned long insn, Fixups * fx));
static void write_1_short
PARAMS ((struct d10v_opcode * opcode, unsigned long insn, Fixups * fx));
static int write_2_short
PARAMS ((struct d10v_opcode * opcode1, unsigned long insn1,
	 struct d10v_opcode * opcode2, unsigned long insn2,
	 packing_type exec_type, Fixups * fx));
static unsigned long do_assemble
PARAMS ((char *str, struct d10v_opcode ** opcode));
static unsigned long d10v_insert_operand
PARAMS ((unsigned long insn, int op_type, offsetT value, int left,
	 fixS * fix));
  ...
  for (i = 0; i < fx->fc; i++)
      ...
      if (fx->fix[i].reloc)
	{
	  ...
	  fix_new_exp (frag_now,
		       ...
		       fx->fix[i].size,
		       &(fx->fix[i].exp),
		       fx->fix[i].pcrel, fx->fix[i].operand | 2048);
	}
    }
  fx->fc = 0;
}
     ...
     Fixups *fx;
{
  ...
  if (opcode->exec_type & PARONLY)
    as_fatal (_
	      ("Instruction must be executed in parallel with another instruction."));
     ...
     Fixups *fx;
{
  ...
  switch (exec_type)
      ...
      if (opcode1->unit == IU)
	{
	  if (opcode2->unit == IU)
	    as_fatal (_
		      ("Two IU instructions may not be executed in parallel"));
      ...
      else if (opcode2->unit == MU)
	{
	  if (opcode1->unit == MU)
	    as_fatal (_
		      ("Two MU instructions may not be executed in parallel"));
	  if (!flag_warn_suppress_instructionswap)
	    as_warn (_("Swapping instruction order"));
	  insn = FM00 | (insn2 << 15) | insn1;
	}
     ...
     char *str;
{
  ...
  if (prev_opcode
      && prev_seg && ((prev_seg != now_seg) || (prev_subseg != now_subseg)))
    d10v_cleanup ();
     ...
     segT seg ATTRIBUTE_UNUSED;
{
  ...
  switch (fixP->fx_r_type)
    ...
    case BFD_RELOC_D10V_18_PCREL:
      ...
      if (fixP->fx_addsy != NULL && S_IS_EXTERN (fixP->fx_addsy))
	{
