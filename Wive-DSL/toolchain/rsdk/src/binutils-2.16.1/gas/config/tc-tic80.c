...
static struct tic80_opcode *find_opcode
PARAMS ((struct tic80_opcode *, expressionS[]));
	  ...
	  if (*p == 'm')
	    {
	      ...
	      exp[numexp].X_add_number =
		TIC80_OPERAND_M_SI | TIC80_OPERAND_M_LI;
	    }
	  else if (*p == 's')
	    {
	      p++;
      ...
      for (expi = 0, opi = -1; (expi < numexp) && match; expi++)
	{
	  ...
	  switch (X_op)
	    ...
	    case O_register:
		  ...
		  || ((flags & TIC80_OPERAND_FPA) !=
		      (num & TIC80_OPERAND_FPA))
  ...
  for (expi = 0, opi = -1; opers[expi].X_op != O_illegal; expi++)
	...
	case O_symbol:
	  if (bits == 32)
	    ...
	    {
	      fix_new_exp (ffrag, f - (ffrag->fr_literal), 4,	/* FIXME! how is this used?  */
			   &opers[expi], 1, R_MPPCR15W);
	    }
	  else
	    {
	      internal_error (_
			      ("symbol reloc that is not PC relative or 32 bits"));
	    }
	  break;
	case O_absent:
