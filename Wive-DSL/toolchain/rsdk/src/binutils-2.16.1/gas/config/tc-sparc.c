...
/* The currently selected v9 memory model.  Currently only used for
   ELF.  */
static enum
{ MM_TSO, MM_PSO, MM_RMO } sparc_memory_model = MM_RMO;
#endif
...
enum sparc_arch_types
{ v6, v7, v8, sparclet, sparclite, sparc86x, v8plus,
  ...
  for (i = 0; native_op_table[i].name; i++)
    {
      ...
      char *name = ((sparc_arch_size == 32)
		    ? native_op_table[i].name32 : native_op_table[i].name64);
  ...
  if (warn_on_bump && architecture_requested)
    {
      ...
      for (max_architecture = SPARC_OPCODE_ARCH_MAX;
	   max_architecture > warn_after_architecture; --max_architecture)
...
sparc_md_end ()
{
  ...
  if (sparc_arch_size == 64)
    switch (current_architecture)
      {
      case SPARC_OPCODE_ARCH_V9A:
	mach = bfd_mach_sparc_v9a;
	break;
      case SPARC_OPCODE_ARCH_V9B:
    ...
    switch (current_architecture)
      {
      case SPARC_OPCODE_ARCH_SPARCLET:
	mach = bfd_mach_sparc_sparclet;
	break;
      case SPARC_OPCODE_ARCH_V9:
	mach = bfd_mach_sparc_v8plus;
	break;
      case SPARC_OPCODE_ARCH_V9A:
	mach = bfd_mach_sparc_v8plusa;
	break;
      case SPARC_OPCODE_ARCH_V9B:
     ...
     char *str;
{
  ...
  if (insn != NULL
      && last_insn != NULL
      && (insn->flags & F_FBR) != 0 && (last_insn->flags & F_DELAYED) != 0
  ...
  for (;;)
	  ...
	  switch (*args)
		  ...
		  if (p->name[0] != s[0])
		    {
		      error_message =
			_
			(": unrecognizable v9a or v9b ancillary state register");
		      goto error;
		    }
	      ...
	      if (s[0] == ',' && s[1] == '%')
		{
		  ...
		  if (s[o->len + 2] != '(')
		    {
		      as_bad (_
			      ("Illegal operands: %%%s requires arguments in ()"),
			      o->name);
		      return special_case;
		    }
		  ...
		  if (o->call
		    ...
		    {
		      as_bad (_
			      ("Illegal operands: %%%s can be only used with call __tls_get_addr"),
			      o->name);
		      return special_case;
		    }
	      ...
	      if (*s++ == '%')
		{
		  ...
		  if ((mask & ~1) == 2 && sparc_arch_size == 64
		    ...
		    as_bad (_
			    ("detected global register use not covered by .register pseudo-op"));
		...
		if (*s++ == '%' && ((format = *s) == 'f') && ISDIGIT (*++s))
		  {
		    ...
		    if ((*args == 'v'
			 || *args == 'B' || *args == 'H') && (mask & 1))
		      {
			break;
		      }		/* register must be even numbered */

		    if ((*args == 'V'
			 || *args == 'R' || *args == 'J') && (mask & 3))
		      {
			break;
		...
		if (*s == '%')
		  {
		    ...
		    } ops[] =
		    {
		      ...
		      {
		      "tldo_hix22", 10, BFD_RELOC_SPARC_TLS_LDO_HIX22, 0, 0},
		      {
		      "tldo_lox10", 10, BFD_RELOC_SPARC_TLS_LDO_LOX10, 0, 0},
		      {
		...
		if (s1 != s && ISDIGIT (s1[-1]))
		  {
		    if (s1[-2] == '%' && s1[-3] == '+')
		      s1 -= 3;
		    else if (strchr ("goli0123456789", s1[-2])
			     && s1[-3] == '%' && s1[-4] == '+')
		      s1 -= 4;
		    else
		      s1 = NULL;
		...
		if (op_arg)
		  {
			...
			the_insn.reloc = old_reloc;
		      ...
		      {
			as_bad (_
				("Illegal operands: Can't add non-constant expression to %%%s()"),
				op_arg);
			return special_case;
		      }
		    else
			...
			if (old_reloc != BFD_RELOC_SPARC13
			  ...
			  {
			    as_bad (_
				    ("Illegal operands: Can't do arithmetics involving %%%s() of a relocatable symbol"),
				    op_arg);
			    return special_case;
			  }
	    ...
	    case 'S':
	      if (strcmp (str, "set") == 0 || strcmp (str, "setuw") == 0)
		{
		...
		if (e.X_op == O_constant)
		  {
		    ...
		    if (n != e.X_add_number || (n & ~0x1ff) != 0)
		      as_bad (_
			      ("OPF immediate operand out of range (0-0x1ff)"));
		    else
		...
		int cpreg;
		if (!parse_keyword_arg
		    (sparc_encode_sparclet_cpreg, &s, &cpreg))
		  {
      ...
      else
	{
	  ...
	  /* Can we bump up the architecture?  */
		...
		{
		  as_warn (_
			   ("architecture bumped from \"%s\" to \"%s\" on \"%s\""),
			   sparc_opcode_archs[current_architecture].name,
			   sparc_opcode_archs[needed_architecture].name, str);
     ...
     segT segment ATTRIBUTE_UNUSED;
{
  ...
  if (fixP->fx_addsy != NULL && fixP->fx_r_type != BFD_RELOC_32_PCREL_S2)
    val = 0;
  ...
  if (sparc_pic_code
      ...
      && fixP->fx_addsy != NULL
      && !S_IS_COMMON (fixP->fx_addsy) && symbol_section_p (fixP->fx_addsy))
  ...
  if (sparc_pic_code
      ...
      && S_IS_DEFINED (fixP->fx_addsy) && !S_IS_COMMON (fixP->fx_addsy))
    {
      val = 0;
      fixP->fx_addnumber -= 2 * S_GET_VALUE (fixP->fx_addsy);
    }
...
#define GOT_NAME "__GLOBAL_OFFSET_TABLE_"
  ...
  if (reloc->howto->pc_relative == 0
      || code == BFD_RELOC_SPARC_PC10 || code == BFD_RELOC_SPARC_PC22)
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  if (input_line_pointer[0] != '%'
    ...
    as_bad (_
	    ("register syntax is .register %%g[2367],{#scratch|symbolname|#ignore}"));
      ...
      else
	{
	  if (regname == NULL)
		...
		flags |= BSF_GLOBAL;
	      ...
	      elf_symbol (symbol_get_bfdsym (globals[reg]))->internal_elf_sym.
		st_info = ELF_ST_INFO (STB_GLOBAL, STT_REGISTER);
	      elf_symbol (symbol_get_bfdsym (globals[reg]))->internal_elf_sym.
		st_shndx = SHN_UNDEF;
	    }
	}
...
sparc_adjust_symtab ()
{
  ...
  for (sym = symbol_rootP; sym != NULL; sym = symbol_next (sym))
    {
      if (ELF_ST_TYPE
	  (elf_symbol (symbol_get_bfdsym (sym))->internal_elf_sym.st_info) !=
	  STT_REGISTER)
	continue;

      if (ELF_ST_TYPE
	  (elf_symbol (symbol_get_bfdsym (sym))->internal_elf_sym.st_shndx !=
	   SHN_UNDEF))
	continue;
     ...
     int size;
{
  ...
  if (input_line_pointer[0] == '%'
      && input_line_pointer[1] == 'r' && input_line_pointer[2] == '_')
    {
	...
	{
	  if (size != 4 && size != 8)
	    as_bad (_("Illegal operands: %%r_plt in %d-byte data field"),
		    size);
	  else
	    {
	      input_line_pointer += 6;
      ...
      if (sparc_cons_special_reloc)
	{
	  ...
	  switch (size)
		...
		bad = 1;
	      ...
	      break;
	    case 2:
	      if (input_line_pointer[0] != '1'
		  || input_line_pointer[1] != '6')
		bad = 1;
	      break;
	    case 4:
	      if (input_line_pointer[0] != '3'
		  || input_line_pointer[1] != '2')
		bad = 1;
	      break;
	    case 8:
	      if (input_line_pointer[0] != '6'
		  || input_line_pointer[1] != '4')
		bad = 1;
	      break;
	    default:
	      bad = 1;
	  ...
	  if (bad)
	    {
	      as_bad (_
		      ("Illegal operands: Only %%r_%s%d allowed in %d-byte data fields"),
		...
		{
		  as_bad (_
			  ("Illegal operands: %%r_%s%d requires arguments in ()"),
	      ...
	      else
		{
		  ...
		  *end = c;
		  if (input_line_pointer != end)
			...
			as_bad (_
				("Illegal operands: garbage after %%r_%s%d()"),
				sparc_cons_special_reloc, size * 8);
		    }
		}
	    }
