      ...
      if (S_GET_WEAK (symbolP))
	{
	  switch (S_GET_TYPE (symbolP))
	    ...
	    case N_BSS:
	      ...
	      break;
	    default:
	      as_bad (_("%s: bad type for weak symbol"), temp);
	      break;
	    }
	}
  ...
  while ((symbolP = *symbolPP) != NULL)
      ...
      if (flag_readonly_data_in_text && (S_GET_SEGMENT (symbolP) == SEG_DATA))
	{
	  ...
	  && (!S_IS_DEFINED (symbolP)
	      || S_IS_COMMON (symbolP) || symbol_resolved_p (symbolP)))
	{
	  *symbolPP = symbol_next (symbolP);
	  continue;
	}
...
static void obj_aout_process_stab
PARAMS ((segT, int, const char *, int, int, int));
