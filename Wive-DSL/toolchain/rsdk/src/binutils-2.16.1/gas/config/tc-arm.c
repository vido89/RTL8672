...
static const struct reg_entry rn_table[] = {
  ...
  {"r12", 12, TRUE}, {"r13", REG_SP, TRUE}, {"r14", REG_LR, TRUE}, {"r15",
								    REG_PC,
								    TRUE},
  /* ATPCS Synonyms.  */
  ...
  {"ip", 12, TRUE}, {"sp", REG_SP, TRUE}, {"lr", REG_LR, TRUE}, {"pc", REG_PC,
								 TRUE},
  {NULL, 0, TRUE}
};
...
static const struct reg_entry cn_table[] = {
  ...
  {"cr12", 12, TRUE}, {"cr13", 13, TRUE}, {"cr14", 14, TRUE}, {"cr15", 15,
							       TRUE},
  {NULL, 0, TRUE}
};
...
static const struct reg_entry mav_mvf_table[] = {
  ...
  {"mvf8", 8, TRUE}, {"mvf9", 9, TRUE}, {"mvf10", 10, TRUE}, {"mvf11", 11,
							      TRUE},
  {"mvf12", 12, TRUE}, {"mvf13", 13, TRUE}, {"mvf14", 14, TRUE}, {"mvf15", 15,
								  TRUE},
  {NULL, 0, TRUE}
};
...
static const struct reg_entry mav_mvd_table[] = {
  ...
  {"mvd8", 8, TRUE}, {"mvd9", 9, TRUE}, {"mvd10", 10, TRUE}, {"mvd11", 11,
							      TRUE},
  {"mvd12", 12, TRUE}, {"mvd13", 13, TRUE}, {"mvd14", 14, TRUE}, {"mvd15", 15,
								  TRUE},
  {NULL, 0, TRUE}
};

static const struct reg_entry mav_mvfx_table[] = {
  {"mvfx0", 0, TRUE}, {"mvfx1", 1, TRUE}, {"mvfx2", 2, TRUE}, {"mvfx3", 3,
							       TRUE},
  {"mvfx4", 4, TRUE}, {"mvfx5", 5, TRUE}, {"mvfx6", 6, TRUE}, {"mvfx7", 7,
							       TRUE},
  {"mvfx8", 8, TRUE}, {"mvfx9", 9, TRUE}, {"mvfx10", 10, TRUE}, {"mvfx11", 11,
								 TRUE},
  {"mvfx12", 12, TRUE}, {"mvfx13", 13, TRUE}, {"mvfx14", 14, TRUE}, {"mvfx15",
								     15,
								     TRUE},
  {NULL, 0, TRUE}
};
...
static const struct reg_entry mav_mvdx_table[] = {
  {"mvdx0", 0, TRUE}, {"mvdx1", 1, TRUE}, {"mvdx2", 2, TRUE}, {"mvdx3", 3,
							       TRUE},
  {"mvdx4", 4, TRUE}, {"mvdx5", 5, TRUE}, {"mvdx6", 6, TRUE}, {"mvdx7", 7,
							       TRUE},
  {"mvdx8", 8, TRUE}, {"mvdx9", 9, TRUE}, {"mvdx10", 10, TRUE}, {"mvdx11", 11,
								 TRUE},
  {"mvdx12", 12, TRUE}, {"mvdx13", 13, TRUE}, {"mvdx14", 14, TRUE}, {"mvdx15",
								     15,
								     TRUE},
  {NULL, 0, TRUE}
};
...
static const struct reg_entry mav_mvax_table[] = {
  {"mvax0", 0, TRUE}, {"mvax1", 1, TRUE}, {"mvax2", 2, TRUE}, {"mvax3", 3,
							       TRUE},
  {NULL, 0, TRUE}
};
...
static void
symbol_locate (symbolS * symbolP, const char *name,	/* It is copied, the caller can modify.  */
...
static unsigned int
validate_immediate_twopart (unsigned int val, unsigned int *highpart)
{
  unsigned int a;
  unsigned int i;
  ...
  *input_line_pointer = 0;
  ...
  if (*name)
      ...
      if (req_type != REG_TYPE_MAX)
	{
	  char *temp_name = name;
	  int req_no =
	    arm_reg_parse (&temp_name, all_reg_maps[req_type].htab);
...
s_thumb_set (int equiv)
{
  ...
  *end_name = delim;
  ...
  if (equiv
      && S_IS_DEFINED (symbolP) && S_GET_SEGMENT (symbolP) != reg_section)
  ...
  if (symbol_get_value_expression (sp)->X_add_symbol)
    {
      return (walk_no_bignums (symbol_get_value_expression (sp)->X_add_symbol)
		  ...
		  && walk_no_bignums (symbol_get_value_expression (sp)->
				      X_op_symbol)));
    }
...
wreg_required_here (char **str, int shift, enum wreg_type reg_type)
{
  ...
  if ((reg = arm_reg_parse (str, all_reg_maps[REG_TYPE_IWMMXT].htab)) != FAIL)
	  ...
	  && (reg_type == IWMMXT_REG_WR || reg_type == IWMMXT_REG_WR_OR_WC))
	{
	  ...
	  return reg;
	       ...
	       && (reg_type == IWMMXT_REG_WC
		   || reg_type == IWMMXT_REG_WR_OR_WC))
	{
	  if (shift >= 0)
  ...
  *str = start;
  ...
  sprintf (buff,
	   _("Intel Wireless MMX technology register expected, not '%.100s'"),
	   start);
  inst.error = buff;
      ...
      if (*p == ']')
	{
	  ...
	  if (*p == '#')
	      ...
	      else
		{
		  inst.error =
		    _
		    ("non-constant expressions for 'option' field not supported");
		  return FAIL;
		}
	    }
	  else
...
do_mrs (char *str)
{
  ...
  if (streq (str, "CPSR") || streq (str, "SPSR")
      /* Lower case versions for backwards compatibility.  */
      || streq (str, "cpsr") || streq (str, "spsr"))
    skip = 4;

  /* This is for backwards compatibility with older toolchains.  */
  else if (streq (str, "cpsr_all") || streq (str, "spsr_all"))
    skip = 8;
  else
    {
...
do_co_reg2c (char *str)
{
  ...
  if (skip_past_comma (&str) == FAIL || cp_opc_expr (&str, 4, 4) == FAIL)
    {
      if (!inst.error)
	inst.error = BAD_ARGS;
      return;
  ...
  if (rd == REG_PC || rn == REG_PC)
    as_tsktsk (_("Warning: instruction unpredictable when using r15"));
...
do_cdp2 (char *str)
{
  ...
  if (skip_past_comma (&str) == FAIL || cp_opc_expr (&str, 20, 4) == FAIL)
    {
      if (!inst.error)
	inst.error = BAD_ARGS;
      return;
...
do_co_reg2 (char *str)
{
  ...
  if (skip_past_comma (&str) == FAIL || cp_opc_expr (&str, 21, 3) == FAIL)
    {
      if (!inst.error)
	inst.error = BAD_ARGS;
      return;
  ...
  if (skip_past_comma (&str) == FAIL || reg_required_here (&str, 12) == FAIL)
    {
      if (!inst.error)
	inst.error = BAD_ARGS;
      return;
...
do_sat (char **str, int bias)
{
  ...
  if (expr.X_add_number + bias < 0 || expr.X_add_number + bias > 31)
    {
...
do_sat16 (char **str, int bias)
{
  ...
  if (expr.X_add_number + bias < 0 || expr.X_add_number + bias > 15)
    {
      inst.error = _("immediate value out of range");
      return;
    }
...
#undef MAP
  ...
  for (i = 0, ip = input_line_pointer;
       i < sizeof (id) && (ISALNUM (*ip) || ISPUNCT (*ip)); i++, ip++)
    id[i] = TOLOWER (*ip);
...
do_branch25 (char *str)
{
...
#ifdef OBJ_ELF
  {
	...
	&& *str == '(' && arm_parse_reloc () == BFD_RELOC_ARM_PLT32)
      {
...
check_iwmmxt_insn (char *str,
		   enum iwmmxt_insn_type insn_type, int immediate_size)
{
  int reg = 0;
...
do_iwmmxt_textrc (char *str)
{
  ...
  if ((number =
       check_iwmmxt_insn (str, check_textrc, 7)) == (unsigned long) FAIL)
    return;
...
do_iwmmxt_textrm (char *str)
{
  ...
  if ((number =
       check_iwmmxt_insn (str, check_textrm, 7)) == (unsigned long) FAIL)
    return;
...
do_iwmmxt_tinsr (char *str)
{
  ...
  if ((number =
       check_iwmmxt_insn (str, check_tinsr, 7)) == (unsigned long) FAIL)
    return;
...
do_iwmmxt_waligni (char *str)
{
  ...
  if ((number =
       check_iwmmxt_insn (str, check_waligni, 7)) == (unsigned long) FAIL)
    return;
...
do_iwmmxt_word_addr (char *str)
{
  ...
  if (wc_register (reg))
	...
	as_bad (_
		("conditional execution not supported with control register"));
      if (op != 2)
	as_bad (_("non-word size not supported with control register"));
      inst.instruction |= 0xf0000100;
      inst.instruction &= ~0x00400000;
...
do_iwmmxt_wshufh (char *str)
{
  ...
  if ((number =
       check_iwmmxt_insn (str, check_wshufh, 0xff)) == (unsigned long) FAIL)
    return;
...
do_iwmmxt_wzero (char *str)
{
  ...
  inst.instruction |=
    ((inst.instruction & 0xf) << 12) | ((inst.instruction & 0xf) << 16);
}
...
do_ldrd (char *str)
{
  ...
  if (((rd == rn) || (rd + 1 == rn))
	  ...
	  || (!(inst.instruction & PRE_INDEX))))
    as_warn (_
	     ("pre/post-indexing used when modified address register is destination"));
  ...
  if ((inst.instruction & V4_STR_BIT) == 0
      && (inst.instruction & HWOFFSET_IMM) == 0)
    {
      ...
      if (rm == rd || (rm == rd + 1))
	as_warn (_
		 ("ldrd destination registers must not overlap index register"));
    }
...
my_get_float_expression (char **str)
{
  ...
  if (expression (&exp) == absolute_section
      && exp.X_op == O_big && exp.X_add_number < 0)
    {
...
static int
negate_data_op (unsigned long *instruction, unsigned long value)
{
  int op, new_inst;
...
do_arit (char *str)
{
  ...
  if (reg_required_here (&str, 12) == FAIL
      || skip_past_comma (&str) == FAIL
      || reg_required_here (&str, 16) == FAIL
      || skip_past_comma (&str) == FAIL || data_op2 (&str) == FAIL)
    {
      if (!inst.error)
	inst.error = BAD_ARGS;
      return;
...
do_cmp (char *str)
{
  ...
  if (skip_past_comma (&str) == FAIL || data_op2 (&str) == FAIL)
    {
      if (!inst.error)
	inst.error = BAD_ARGS;
      return;
	  ...
	  if (value != FAIL)
	    {
	      ...
	      inst.instruction &= LITERAL_MASK;
	      inst.instruction |=
		INST_IMMEDIATE | (OPCODE_MOV << DATA_OP_SHIFT);
	      inst.instruction |= value & 0xfff;
	      end_of_line (str);
	      return;
	    }
	  ...
	  if (value != FAIL)
	    {
	      ...
	      inst.instruction &= LITERAL_MASK;
	      inst.instruction |=
		INST_IMMEDIATE | (OPCODE_MVN << DATA_OP_SHIFT);
	      inst.instruction |= value & 0xfff;
	      end_of_line (str);
	      return;
	    }
...
reg_list (char **strp)
{
  ...
  do
      ...
      else
	{
	  ...
	  if (expr.X_op == O_constant)
	    {
	      if (expr.X_add_number != (expr.X_add_number & 0x0000ffff))
		{
...
do_ldmstm (char *str)
{
  ...
  if (skip_past_comma (&str) == FAIL || (range = reg_list (&str)) == FAIL)
    {
      if (!inst.error)
	inst.error = BAD_ARGS;
      return;
...
do_swap (char *str)
{
  ...
  if (skip_past_comma (&str) == FAIL || *str++ != '[')
    {
      inst.error = BAD_ARGS;
      return;
    }
...
do_fpa_dyadic (char *str)
{
  ...
  if (skip_past_comma (&str) == FAIL || fp_op2 (&str) == FAIL)
    {
      if (!inst.error)
	inst.error = BAD_ARGS;
      return;
...
vfp_parse_reg_list (char **str, int *pbase, int dp)
{
  ...
  do
      ...
      if (**str == '-')		/* We have the start of a range expression */
	{
	  ...
	  if ((high_range
	       = arm_reg_parse (str, all_reg_maps[regtype].htab)) == FAIL)
	    {
	      inst.error = _(all_reg_maps[regtype].expected);
	      return FAIL;
	    }
...
vfp_psr_parse (char **str)
{
  ...
  *--p = 0;
  ...
  for (vreg = vfp_regs + 0;
       vreg < vfp_regs + sizeof (vfp_regs) / sizeof (struct vfp_reg); vreg++)
    {
...
do_vfp_reg_from_ctrl (char *str)
{
  ...
  if (skip_past_comma (&str) == FAIL || vfp_psr_required_here (&str) == FAIL)
    {
      if (!inst.error)
	inst.error = BAD_ARGS;
      return;
...
thumb_shift (char *str, int shift)
{
  ...
  if (Rn != FAIL)
	...
	{
	case THUMB_ASR:
	  inst.instruction = T_OPCODE_ASR_R;
	  break;
	case THUMB_LSL:
	  inst.instruction = T_OPCODE_LSL_R;
	  break;
	case THUMB_LSR:
	  inst.instruction = T_OPCODE_LSR_R;
	  break;
	}
...
mav_reg_required_here (char **str, int shift, enum arm_reg_type regtype)
{
  ...
  *str = start;
  ...
  if (regtype == REG_TYPE_MVF ||
      ...
      regtype == REG_TYPE_MVFX || regtype == REG_TYPE_MVDX)
    {
      if ((reg = arm_reg_parse (str, all_reg_maps[REG_TYPE_CN].htab)) != FAIL)
	{
	  if (shift >= 0)
...
do_mav_binops (char *str,
	       int mode, enum arm_reg_type reg0, enum arm_reg_type reg1)
{
  int shift0, shift1;
...
do_mav_triple (char *str,
	       ...
	       enum arm_reg_type reg1, enum arm_reg_type reg2)
{
...
do_mav_quad (char *str,
	     ...
	     enum arm_reg_type reg2, enum arm_reg_type reg3)
{
...
static void
do_mav_shift (char *str, enum arm_reg_type reg0, enum arm_reg_type reg1)
{
  int error;
  int imm, neg = 0;
...
do_mav_ldst (char *str, enum arm_reg_type reg0)
{
  ...
  if (mav_reg_required_here (&str, 12, reg0) == FAIL
      || skip_past_comma (&str) == FAIL
      || *str++ != '[' || reg_required_here (&str, 16) == FAIL)
    goto fail_ldst;
...
do_t_arit (char *str)
{
  ...
  if (inst.instruction == T_OPCODE_MUL && Rs == Rd)
...
do_t_lds (char *str)
{
  ...
  if ((Rd = thumb_reg (&str, THUMB_REG_LO)) == FAIL
      ...
      || skip_past_comma (&str) == FAIL
      || (Ro = thumb_reg (&str, THUMB_REG_LO)) == FAIL || *str++ != ']')
    {
      if (!inst.error)
...
do_t_adr (char *str)
{
  ...
  if ((reg = reg_required_here (&str, 4)) == FAIL || (reg > 7)	/* For Thumb reg must be r0..r7.  */
      || skip_past_comma (&str) == FAIL
      || my_get_expression (&inst.reloc.exp, &str))
    {
      if (!inst.error)
...
static void
insert_reg_alias (char *str, int regnum, struct hash_control *htab)
{
  const char *error;
  ...
  if (error)
    {
      as_bad (_("failed to create an alias for %s, reason: %s"), str, error);
      free ((char *) name);
      free (new);
    }
}
...
md_begin (void)
{
...
#if defined OBJ_COFF || defined OBJ_ELF
  {
	...
	if (pic_code)
	  flags |= F_PIC;
	if ((cpu_variant & FPU_ANY) == FPU_NONE || (cpu_variant & FPU_ANY) == FPU_ARCH_VFP)	/* VFP layout only.  */
	  flags |= F_SOFT_FLOAT;
	...
	if (sec != NULL)
	  {
	    bfd_set_section_flags
	      (stdoutput, sec,
	       SEC_READONLY | SEC_DEBUGGING /* | SEC_HAS_CONTENTS */ );
...
md_apply_fix3 (fixS * fixP, valueT * valP, segT seg)
{
  ...
  switch (fixP->fx_r_type)
	...
	if (newimm == (unsigned int) FAIL
	    && (newimm =
		negate_data_op (&temp, value)) == (unsigned int) FAIL)
	  {
	    ...
	    else if ((newimm = validate_immediate_twopart (-value, &highpart))
		     != (unsigned int) FAIL)
	      temp = newinsn =
		(temp & OPCODE_MASK) | OPCODE_SUB << DATA_OP_SHIFT;
	    /* Otherwise - give up.  */
	    else
	      {
		as_bad_where (fixP->fx_file, fixP->fx_line,
...
#define SEXT24(x)	((((x) & 0xffffff) ^ (~ 0x7fffff)) + 0x800000)
	  ...
	  && ((value & ~((offsetT) 0x1ffffff)) != ~((offsetT) 0x1ffffff)))
	{
	  ...
	  if (!fixP->fx_done)
	    ...
	    as_bad_where (fixP->fx_file, fixP->fx_line,
			  _
			  ("GAS can't handle same-section branch dest >= 0x04000000"));
	}
	...
	as_bad_where (fixP->fx_file, fixP->fx_line, _("out of range branch"));
...
tc_gen_reloc (asection * section ATTRIBUTE_UNUSED, fixS * fixp)
{
  ...
  *reloc->sym_ptr_ptr = symbol_get_bfdsym (fixp->fx_addsy);
  ...
  switch (fixp->fx_r_type)
    ...
    case BFD_RELOC_ARM_OFFSET_IMM:
      if (fixp->fx_addsy != NULL
	  && !S_IS_DEFINED (fixp->fx_addsy) && S_IS_LOCAL (fixp->fx_addsy))
	{
	  as_bad_where (fixp->fx_file, fixp->fx_line,
...
struct arm_option_table arm_opts[] = {
  ...
  {"mapcs-32", N_("code uses 32-bit program counter"), &uses_apcs_26, 0,
   NULL},
  {"mapcs-26", N_("code uses 26-bit program counter"), &uses_apcs_26, 1,
   NULL},
...
void
cons_fix_new_arm (fragS * frag, int where, int size, expressionS * exp)
{
  bfd_reloc_code_real_type type;
  int pcrel = 0;
	  ...
	  if (THUMB_IS_FUNC (sym))
	    {
	      ...
	      if (S_GET_STORAGE_CLASS (sym) == C_STAT || S_GET_STORAGE_CLASS (sym) == C_LABEL)	/* This can happen!  */
...
arm_validate_fix (fixS * fixP)
{
  ...
  if (fixP->fx_r_type == BFD_RELOC_THUMB_PCREL_BRANCH23
      && fixP->fx_addsy != NULL
      && S_IS_DEFINED (fixP->fx_addsy) && !THUMB_IS_FUNC (fixP->fx_addsy))
    {
...
arm_fix_adjustable (fixS * fixP)
{
  ...
  if (THUMB_IS_FUNC (fixP->fx_addsy) && fixP->fx_subsy == NULL)
    return 0;
...
elf32_arm_target_format (void)
{
  ...
  return (target_big_endian
	  ? "elf32-bigarm-symbian" : "elf32-littlearm-symbian");
#elif defined (TE_VXWORKS)
  return (target_big_endian
	  ? "elf32-bigarm-vxworks" : "elf32-littlearm-vxworks");
#else
  if (target_big_endian)
    return "elf32-bigarm";
  else
...
void
armelf_frob_symbol (symbolS * symp, int *puntp)
{
  ...
  if (unwind.opcode_count + length > unwind.opcode_alloc)
    {
      ...
      if (unwind.opcodes)
	unwind.opcodes = xrealloc (unwind.opcodes, unwind.opcode_alloc);
      else
...
start_unwind_section (const segT text_seg, int idx)
{
  ...
  if (prefix != prefix_once && (text_seg->flags & SEC_LINK_ONCE) != 0)
	...
	prefix_len = 16;
      ...
      len = (sec_name_len + prefix_len	/* ,"aG",%sectiontype,  */
	...
	memcpy (section + sec_name_len, ",\"aG\",%progbits,", 16);
      memcpy (section + sec_name_len + prefix_len, group_name,
	      group_name_len);
...
s_arm_unwind_fnend (int ignored ATTRIBUTE_UNUSED)
{
  ...
  fix_new (frag_now, where, 4, unwind.proc_start, 0, 1, BFD_RELOC_ARM_PREL31);
...
s_arm_unwind_save_core (void)
{
  ...
  if (unwind.sp_restored && unwind.fp_reg == 12 && (range & 0x3000) == 0x1000)
    {
      unwind.opcode_count--;
      unwind.sp_restored = 0;
...
s_arm_unwind_raw (int ignored ATTRIBUTE_UNUSED)
{
  ...
  SKIP_WHITESPACE ();
  expression (&exp);
  if (exp.X_op == O_constant && skip_past_comma (&input_line_pointer) != FAIL)
    {
