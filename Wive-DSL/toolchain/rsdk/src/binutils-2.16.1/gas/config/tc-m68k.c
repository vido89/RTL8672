...
static const enum m68k_register m68000_control_regs[] =
{ 0 };
...
static const struct m68k_cpu archs[] = {
  ...
  {mcfisa_a | mcfhwdiv | mcfisa_b | mcfemac | mcfusp | cfloat, mcf5470,
   "547x", 0},
  {mcfisa_a | mcfhwdiv | mcfisa_b | mcfemac | mcfusp | cfloat, mcf5480,
   "548x", 0},
...
tc_coff_fix2rtype (fixS * fixP)
{
  ...
  return (fixP->fx_size == 1 ? R_RELBYTE
	  : fixP->fx_size == 2 ? R_DIR16 : R_DIR32);
#else
  return (fixP->fx_pcrel
	  ...
	  : (fixP->fx_size == 1 ? R_RELBYTE
	     : fixP->fx_size == 2 ? R_RELWORD : R_RELLONG));
#endif
}
  ...
  for (losing = 0;;)
      ...
      else
	{
	  for (s = opcode->m_operands, opP = &the_ins.operands[0];
		...
		case '*':
		  if (opP->mode == CONTROL
		      || opP->mode == FPREG || opP->mode == REGLST)
		    losing++;
		  break;
		...
		case '4':
		  if (opP->mode != AINDR && opP->mode != AINC
		      && opP->mode != ADEC && (opP->mode != DISP
					       || opP->reg < ADDR0
					       || opP->reg > ADDR7))
		    ++losing;
		  break;
		...
		case 'J':
		  if (opP->mode != CONTROL
		      || opP->reg < USP || opP->reg > last_movec_reg)
		    losing++;
		  else
		    {
		...
		case 'M':
		  if (opP->mode != IMMED)
		    ...
		    losing++;
			   ...
			   && instring[3] != 'q' && instring[4] != 'q')
		    losing++;
		  break;

		case 'O':
		  if (opP->mode != DREG
		      && opP->mode != IMMED && opP->mode != ABSL)
		    losing++;
		  break;
		...
		case 'x':
		  if (opP->mode != IMMED)
		    losing++;
			   ...
			   || (TRUNC (opP->disp.exp.X_add_number) !=
			       0xffffffff
		...
		case '2':
		  if (opP->mode != CONTROL
		      || (opP->reg != CAL
			  && opP->reg != VAL && opP->reg != SCC))
		    losing++;
		  break;

		case 'V':
		  if (opP->mode != CONTROL || opP->reg != VAL)
		    losing++;
		  break;
	...
	break;
      ...
      if (!opcode)
	{
	  if (ok_arch && !(ok_arch & current_architecture))
	    {
	      char buf[200], *cp;
		...
		default:
		  {
		    ...
		    for (idx = 0; idx < n_archs; idx++)
		      {
			if ((archs[idx].arch & ok_arch) && !archs[idx].alias)
			  {
			    if (got_one)
			      {
				strcpy (cp, " or ");
  ...
  for (s = the_ins.args, opP = &the_ins.operands[0]; *s; s += 2, opP++)
	...
	case '|':
	  ...
	  switch (opP->mode)
		  ...
		  && (opP->reg >= ADDR0) && (opP->reg <= ADDR7))
		{
	      ...
	      if (opP->reg == PC && !isvar (&opP->disp) && m68k_abspcadd)
		{
		...
		siz2 = SIZE_UNSPEC;
	      ...
	      if (opP->index.reg != 0
		  && opP->index.reg >= DATA && opP->index.reg <= ADDR7)
		{
		  ...
		  if (opP->mode == BASE
		      && ((opP->reg >= ADDR0
			   && opP->reg <= ADDR7) || opP->reg == PC))
		    {
		      if (siz1 == SIZE_BYTE
			  || cpu_of_arch (current_architecture) < m68020
			  || arch_coldfire_p (current_architecture)
			  || (siz1 == SIZE_UNSPEC
			      && !isvar (&opP->disp) && issbyte (baseo)))
			{
			  nextword += baseo & 0xff;
			  addword (nextword);
			  if (isvar (&opP->disp))
	      ...
	      else
		{
		  ...
		  if (opP->index.reg >= ZDATA0 && opP->index.reg <= ZDATA7)
		...
		{
		case SIZE_UNSPEC:
		  if (isvar (&opP->disp) ? m68k_rel32 : !issword (baseo))
		    {
		      siz1 = SIZE_LONG;
		      nextword |= 0x30;
		    }
	      ...
	      if (opP->mode == POST || opP->mode == PRE)
		{
		  ...
		  switch (siz2)
		    {
		    case SIZE_UNSPEC:
		      if (isvar (&opP->odisp) ? m68k_rel32 : !issword (outro))
			{
			  siz2 = SIZE_LONG;
			  nextword |= 0x3;
			}
		  ...
		  if (opP->mode == POST && (nextword & 0x40) == 0)
		    nextword |= 0x04;
		}
	      addword (nextword);
		...
		case SIZE_UNSPEC:
		  ...
		  if (isvar (&opP->disp)
		      && !subs (&opP->disp) && adds (&opP->disp)
#ifdef OBJ_ELF
		      /* If the displacement needs pic relocation it
		         cannot be relaxed.  */
		      && opP->disp.pic_reloc == pic_none
#endif
		      && !flag_long_jumps && !strchr ("~%&$?", s[0]))
		    {
		      tmpreg = 0x3A;	/* 7.2 */
		      add_frag (adds (&opP->disp),
				SEXT (offs (&opP->disp)),
	...
	case 'B':
	  ...
	  switch (s[1])
		...
		add_frag (adds (&opP->disp),
			  SEXT (offs (&opP->disp)), TAB (BRANCHBW, SZ_UNDEF));
	      break;
	    case 'w':
	      if (isvar (&opP->disp))
		{
		...
		add_frag (adds (&opP->disp),
			  SEXT (offs (&opP->disp)), TAB (FBRANCH, SZ_UNDEF));
	      break;
	    default:
	      abort ();
	    }
  ...
  if (the_ins.nfrag == 0)
      ...
      for (m = 0; m < the_ins.nrel; m++)
	{
	  switch (the_ins.reloc[m].wid)
	    ...
	    default:
	      as_fatal (_
			("Don't know how to figure width of %c in md_assemble()"),
			the_ins.reloc[m].wid);
	    }
  ...
  for (n = 0, fromP = &the_ins.opcode[0]; n < the_ins.nfrag; n++)
      ...
      for (m = 0; m < the_ins.nrel; m++)
	{
      ...
      (void) frag_var (rs_machine_dependent, 10, 0,
		       ...
		       the_ins.fragb[n].fadd, the_ins.fragb[n].foff,
		       to_beg_P);
    }
...
md_begin (void)
{
  ...
  m68k_sorted_opcodes =
    xmalloc (m68k_numopcodes * sizeof (*m68k_sorted_opcodes));
  ...
  for (i = 0; i < m68k_numopcodes; i++)
    {
      hack = slak =
	(struct m68k_incant *) obstack_alloc (&robyn,
  ...
  if (flag_mri)
	...
	{"bsr", "jbsr",},
      ...
      for (i = 0; i < (int) (sizeof mri_aliases / sizeof mri_aliases[0]); i++)
	{
     ...
     gas expects pseudo ops to start with a dot.  */
  {
	...
	hash_insert (op_hash, mote_pseudo_table[n].poc_name, (char *) hack);
	hack->m_operands = 0;
	hack->m_opnum = n;
	n++;
      }
...
m68k_init_after_args (void)
{
  ...
  if (current_architecture & m68851)
	...
	as_warn (_
		 ("68040 and 68851 specified; mmu instructions may assemble incorrectly"));
    }
  /* What other incompatibilities could we check for?  */

  /* Toss in some default assumptions about coprocessors.  */
  if (!no_68881 && (cpu_of_arch (current_architecture)
...
{
  if (S_GET_SEGMENT (sym) == reg_section && (int) S_GET_VALUE (sym) < 0)
    {
...
md_apply_fix3 (fixS * fixP, valueT * valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  if ((addressT) val > upper_limit && (val > 0 || val < lower_limit))
  ...
  if ((fixP->fx_pcrel
      ...
      && fixP->fx_size == 1
      && (fixP->fx_addsy == NULL
	  || S_IS_DEFINED (fixP->fx_addsy)) && (val == 0 || val == -1))
    as_bad_where (fixP->fx_file, fixP->fx_line,
...
md_convert_frag_1 (fragS * fragP)
{
  ...
  switch (fragP->fr_subtype)
      ...
      else if (fragP->fr_opcode[0] == 0x60)	/* jbra */
	{
	  if (flag_keep_pcrel)
	    as_fatal (_
		      ("Tried to convert PC relative branch to absolute jump"));
	  fragP->fr_opcode[0] = 0x4E;
...
md_convert_frag (object_headers * headers ATTRIBUTE_UNUSED,
		 segT sec ATTRIBUTE_UNUSED, fragS * fragP)
{
		      ...
		      relax_addressT segment_address_in_file)
{
  ...
  r_symbolnum = (S_IS_DEFINED (fixP->fx_addsy)
		 ? S_GET_TYPE (fixP->fx_addsy) : fixP->fx_addsy->sy_number);
...
{
  return (pop->poc_handler == s_mri_if || pop->poc_handler == s_mri_else);
}
...
s_reg (int ignore ATTRIBUTE_UNUSED)
{
  ...
  *input_line_pointer = c;
  ...
  else if (rop.mode == CONTROL && rop.reg == FPI)
    mask = 1 << 24;
  else if (rop.mode == CONTROL && rop.reg == FPS)
    mask = 1 << 25;
  else if (rop.mode == CONTROL && rop.reg == FPC)
    mask = 1 << 26;
  else
    {
      as_bad (_("bad register list"));
...
swap_mri_condition (int cc)
{
  switch (cc)
    ...
    case MCC ('n', 'e'):
      return MCC ('n', 'e');	// no problem here
    case MCC ('e', 'q'):
      return MCC ('e', 'q');	// also no problem
    case MCC ('v', 'c'):
    case MCC ('v', 's'):
    default:
      as_warn (_
  ...
  if (strncasecmp (input_line_pointer, "AND", 3) == 0)
	...
	flab = mri_control_label ();
      ...
      build_mri_control_operand (qual, cc, leftstart, leftstop, rightstart,
				 rightstop, (const char *) NULL, flab,
				 extent);

      input_line_pointer += 3;
      if (*input_line_pointer != '.' || input_line_pointer[1] == '\0')
	qual = '\0';
      else
	{
	...
	tlab = mri_control_label ();
      ...
      build_mri_control_operand (qual, cc, leftstart, leftstop, rightstart,
				 rightstop, tlab, (const char *) NULL,
				 extent);
...
s_mri_if (int qual)
{
  ...
  if (s - input_line_pointer > 1 && s[-1] == '.')
    s -= 2;

  if (s - input_line_pointer < 3 || strncasecmp (s - 3, "THEN", 4) != 0)
    {
      if (qual != '\0')
	{
...
s_mri_else (int qual)
{
  ...
  *input_line_pointer = c;
  ...
  if (mri_control_stack == NULL
      || mri_control_stack->type != mri_if || mri_control_stack->else_seen)
    {
...
{
  if (mri_control_stack == NULL || mri_control_stack->type != mri_if)
    {
...
s_mri_break (int extent)
{
  ...
  while (n != NULL
	 && n->type != mri_for
	 && n->type != mri_repeat && n->type != mri_while)
    n = n->outer;
  if (n == NULL)
    {
...
s_mri_for (int qual)
{
  ...
  if (varstop > varstart && (varstop[-1] == ' ' || varstop[-1] == '\t'))
    --varstop;
  ...
  if (initstop > initstart && (initstop[-1] == ' ' || initstop[-1] == '\t'))
    --initstop;
  ...
  if (endstop > endstart && (endstop[-1] == ' ' || endstop[-1] == '\t'))
    --endstop;
  ...
  if (!by)
      ...
      if (bystop == NULL)
	{
      ...
      if (bystop > bystart && (bystop[-1] == ' ' || bystop[-1] == '\t'))
	--bystop;
    }
...
{
  if (mri_control_stack == NULL || mri_control_stack->type != mri_for)
    {
...
s_mri_until (int qual)
{
  ...
  if (mri_control_stack == NULL || mri_control_stack->type != mri_repeat)
    {
...
s_mri_while (int qual)
{
  ...
  if (s - input_line_pointer > 1 && s[-1] == '.')
    s -= 2;
  if (s - input_line_pointer < 2 || strncasecmp (s - 1, "DO", 2) != 0)
    {
      as_bad (_("missing do"));
      ignore_rest_of_line ();
      return;
...
s_mri_endw (int ignore ATTRIBUTE_UNUSED)
{
  ...
  if (mri_control_stack == NULL || mri_control_stack->type != mri_while)
    {
...
md_show_usage (FILE * stream)
{
  ...
  for (i = 0; i < n_archs; i++)
      ...
      if (strcasecmp (default_cpu, archs[i].name) == 0)
	{
	  ...
	  for (i = 0; i < n_archs; i++)
	    {
	      if (archs[i].arch == default_arch && !archs[i].alias)
		{
  ...
  for (;;)
      ...
      for (n = 0; n < strlen (the_ins.args) / 2; n++)
	{
	  ...
	  if (the_ins.operands[n].b_const)
	    printf ("Constant: '%.*s', ",
		    1 + the_ins.operands[n].e_const -
		    the_ins.operands[n].b_const, the_ins.operands[n].b_const);
	  printf ("ireg %d, isiz %d, imul %d, ", the_ins.operands[n].ireg,
		  the_ins.operands[n].isiz, the_ins.operands[n].imul);
	  if (the_ins.operands[n].b_iadd)
	    printf ("Iadd: '%.*s',",
		    1 + the_ins.operands[n].e_iadd -
		    the_ins.operands[n].b_iadd, the_ins.operands[n].b_iadd);
	  putchar ('\n');
	}
    }
  m68k_ip_end ();
...
tc_m68k_frame_initial_instructions (void)
{
  ...
  cfi_add_CFA_offset (DWARF2_DEFAULT_RETURN_COLUMN,
