...
get_section (bfd * abfd ATTRIBUTE_UNUSED, asection * sec, void *inf)
{
  ...
  return (group_name == gname
	  || (group_name != NULL
	      && gname != NULL && strcmp (group_name, gname) == 0));
}
...
obj_elf_change_section (const char *name,
			...
			int attr,
			int entsize,
			const char *group_name, int linkonce, int push)
{
  asection *old_sec;
  segT sec;
  flagword flags;
...
obj_elf_section (int push)
{
  ...
  obj_elf_change_section (name, type, attr, entsize, group_name, linkonce,
			  push);
}
...
obj_elf_vtable_inherit (int ignore ATTRIBUTE_UNUSED)
{
  ...
  *input_line_pointer = c;
  ...
  if (input_line_pointer[0] == '0'
      && (input_line_pointer[1] == '\0' || ISSPACE (input_line_pointer[1])))
    {
...
elf_frob_file (void)
{
  ...
  for (i = 0; i < list.num_group; i++)
	...
	if ((s->flags ^ flags) & SEC_LINK_ONCE)
      ...
      if (sy != NULL
	  && (sy == symbol_lastP
	      || (sy->sy_next != NULL && sy->sy_next->sy_previous == sy)))
	{
	  has_sym = 1;
	  sec_name = ".group";
	}
