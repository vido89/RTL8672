...
static void create_literal_section
PARAMS ((const char *, segT *, symbolS **));
#ifndef OBJ_ELF
  ...
  for (i = 0; i < alpha_num_opcodes;)
    {
      ...
      retval =
	hash_insert (alpha_opcode_hash, name, (PTR) & alpha_opcodes[i]);
      if (retval)
...
{
  if (alpha_debug != 0 && !ecoff_debugging_seen)
    flag_keep_locals = 1;
}
	  ...
	  if (fixp->tc_fix_data.info
	      ...
	      && !fixp->tc_fix_data.info->multi_section_p)
	    {
	      for (slave = fixp->tc_fix_data.info->slaves;
		   slave != (fixS *) 0; slave = slave->tc_fix_data.next_reloc)
		{
     ...
     offsetT addend;
{
  ...
  if (seginfo->frchainP)
    for (fixp = seginfo->frchainP->fix_root;
	 fixp != (fixS *) NULL; fixp = fixp->fx_next, offset += 8)
      {
  ...
  if (!range_signed_32 (addend))
      ...
      if (alpha_lit8_section == NULL)
	{
	  create_literal_section (".lit8",
				  &alpha_lit8_section, &alpha_lit8_symbol);
      ...
      lit =
	add_to_literal_pool (NULL, addend, alpha_lit8_section, 8) - 0x8000;
      if (lit >= 0x8000)
  ...
  *p = c;
  ...
  if (bfd_section_size (stdoutput, new_seg) > 0)
	...
	as_bad (_
		...
		S_GET_NAME (symbolP), (long) bfd_section_size (stdoutput,
							       new_seg),
		(long) temp);
    }
#else
     ...
     int which;
{
  ...
  ecoff_directive_begin,
      ...
      ecoff_directive_scl, ecoff_directive_tag, ecoff_directive_val,};
