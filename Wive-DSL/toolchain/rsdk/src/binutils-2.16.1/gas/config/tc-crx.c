  ...
  if (reloc->howto == (reloc_howto_type *) NULL)
    {
      as_bad_where (fixP->fx_file, fixP->fx_line,
		    _
		    ("internal error: reloc %d (`%s') not supported by object file format"),
		    fixP->fx_r_type,
...
md_apply_fix3 (fixS * fixP, valueT * valP, segT seg)
{
  ...
  if (fixP->fx_pcrel == 1
      && fixP->fx_addsy != NULL && S_GET_SEGMENT (fixP->fx_addsy) == seg)
    fixP->fx_done = 1;
}
...
md_begin (void)
{
...

  {
    ...
    for (regtab = crx_regtab; regtab < (crx_regtab + NUMREGS); regtab++)
      {
...
process_label_constant (char *str, ins * crx_ins)
{
  ...
  switch (crx_ins->exp.X_op)
	...
	case arg_cr:
	  ...
	  else if (IS_INSN_TYPE (CSTBIT_INS) || IS_INSN_TYPE (STOR_IMM_INS))
...
get_cinv_parameters (char *operand)
{
  ...
  return ((b_used ? 8 : 0)
	  + (d_used ? 4 : 0) + (i_used ? 2 : 0) + (u_used ? 1 : 0));
}
...
print_operand (int nbits, int shift, argument * arg)
{
  switch (arg->type)
	...
	as_bad (_
		("Illegal Co-processor special register in Instruction `%s' "),
		ins_parse);
      CRX_PRINT (0, getreg_image (arg->cr), shift);
      break;
      ...
      for (i = 0; i < insn->nargs; i++)
	{
	  ...
	  /* For symbols, we make sure the relocation size (which was already 
	     determined) is sufficient.  */
		   ...
		   && ((bfd_reloc_type_lookup (stdoutput, insn->rtype))->
		       bitsize > cur_size[j]))
	    goto next_insn;
	}
      ...
      else if (!found_const_within_range)
	{
	  switch (const_err)
	    ...
	    case OP_ILLEGAL_CST4:
	      as_bad (_("Invalid CST4 operand value (arg %d)"),
		      invalid_const);
	      break;
...
warn_if_needed (ins * insn)
{
  ...
  if (IS_INSN_TYPE (LD_STOR_INS_INC))
	...
	if (insn->arg[0].r == insn->arg[1].r)
	  as_bad (_
		  ("Same src/dest register is used (`r%d'), result is undefined"),
		  insn->arg[0].r);
    }
  ...
  *allocated = 1;
  ...
  while (*paramP != '}')
      ...
      if (IS_INSN_TYPE (COP_REG_INS))
	{
	  if (((cr = get_copregister (reg_name)) == nullcopregister)
	    ...
	    as_fatal (_("Illegal register `%s' in cop-register list"),
		      reg_name);
      ...
      else if (instruction->flags & USER_REG)
	{
	  if (streq (reg_name, "uhi"))
	    ...
	    as_fatal (_("Illegal register `%s' in user register list"),
		      reg_name);
  ...
  switch (hi_found + lo_found)
    ...
    case 2:
      ...
      if (mask != 0)
	as_bad (_
		("HI/LO registers should be specified without additional registers"));
