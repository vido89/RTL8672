...
const relax_typeS md_relax_table[C (END, 0)] = {
  ...
  EMPTY,
  {SH64PCREL16_F, SH64PCREL16_M, SH64PCREL16_LENGTH,
  ...
  /* C (SH64PCREL16_64, SH64PCREL32) */
  {SH64PCREL32_F, SH64PCREL32_M, SH64PCREL32_LENGTH,
   C (SH64PCREL16_64, SH64PCREL48)},
  /* C (SH64PCREL16_64, SH64PCREL48) */
  {SH64PCREL48_F, SH64PCREL48_M, SH64PCREL48_LENGTH,
...
sh_check_fixup (expressionS * main_exp, bfd_reloc_code_real_type * r_type_p)
{
  ...
  if (exp->X_op == O_subtract)
	...
	return 1;
      ...
      if (exp && exp->X_op == O_add && sh_PIC_related_p (exp->X_add_symbol))
	{
...
parse_reg (char *src, int *mode, int *reg)
{
  ...
  if (l0 == 'f' && l1 == 'p' && TOLOWER (src[2]) == 'u'
      && TOLOWER (src[3]) == 'l' && !IDENT_CHAR ((unsigned char) src[4]))
    {
      *mode = FPUL_N;
      return 4;
    }
  ...
  while (opcode->name)
      ...
      for (n = 0; this_try->arg[n]; n++)
	{
	  ...
	  if (SH_MERGE_ARCH_SET_VALID (valid_arch, arch_sh2a_nofpu_up)
	      && (arg == A_DISP_REG_M || arg == A_DISP_REG_N))
	    {
	  ...
	  switch (arg)
		...
		goto fail;
	      if (user->reg != A_A0_NUM && user->reg != A_A1_NUM)
		goto fail;
	      reg_m = user->reg;
	      break;
...
insert (char *where, int how, int pcrel, sh_operand_info * op)
{
  fix_new_exp (frag_now,
	       where - frag_now->fr_literal, 2, &op->immediate, pcrel, how);
}
  ...
  if (sh_relax)
    {
      ...
      fix_new (frag_now, frag_now_fix (), 2, end_sym, 0, 1,
	       BFD_RELOC_SH_LABEL);
    }
...
assemble_ppi (char *op_end, sh_opcode_info * opcode)
{
  ...
  for (;;)
	...
	case MOVX_NOPY:
	  if (movx)
	    as_bad (_("multiple movx specifications"));
	  if ((reg_n < 4 || reg_n > 5) && (reg_n < 0 || reg_n > 1))
	  ...
	  movx = ((((reg_n & 1) != 0) << 9)
		  ...
		  + (reg_x << 6)
		  + (opcode->nibbles[2] << 4)
		  + opcode->nibbles[3] + DDT_BASE);
	  break;

	case MOVY_NOPX:
	  if (movy)
	    as_bad (_("multiple movy specifications"));
	  if ((reg_n < 6 || reg_n > 7) && (reg_n < 2 || reg_n > 3))
	  ...
	  if (field_b)
		...
		field_b -= 0x8100;
	      /* pclr Dz pmuls Se,Sf,Dg */
		       ...
		       &&
		       (SH_MERGE_ARCH_SET_VALID
			(valid_arch, arch_sh4al_dsp_up)))
		{
		  valid_arch =
		    SH_MERGE_ARCH_SET (valid_arch, arch_sh4al_dsp_up);
		  field_b -= 0x8cf0;
		}
	      else
		...
		as_warn (_
			 ("destination register is same for parallel insns"));
	    }
...
md_assemble (char *str)
{
  ...
  if (sh_relax && !seg_info (now_seg)->tc_segment_info_data.in_code)
    {
  ...
  if (opcode->nibbles[0] == PPI)
    ...
    {
      if (opcode->arg[0] == A_BDISP12 || opcode->arg[0] == A_BDISP8)
	{
  ...
  if (sh_relax && seg_info (now_seg)->tc_segment_info_data.in_code)
    {
      ...
      offset = frag_now_fix ();
      if (frag_now != last_label_frag || offset != last_label_offset)
	{
	  fix_new (frag_now, offset, 2, &abs_symbol, 0, 0,
		   BFD_RELOC_SH_LABEL);
...
#define OPTION_PT32                   (OPTION_NO_EXPAND + 1)
  ...
  {"shcompact-const-crange", no_argument, NULL,
  ...
  switch (c)
      ...
      else if (strcasecmp (arg, "shmedia") == 0)
	{
	  if (sh64_isa_mode == sh64_isa_shcompact)
	    as_bad (_
		    ("Invalid combination: --isa=SHcompact with --isa=SHmedia"));
      ...
      else if (strcasecmp (arg, "shcompact") == 0)
	{
	  if (sh64_isa_mode == sh64_isa_shmedia)
	    as_bad (_
		    ("Invalid combination: --isa=SHmedia with --isa=SHcompact"));
		 ...
		 void *ignore ATTRIBUTE_UNUSED)
{
  ...
  for (fix = seginfo->fix_root; fix != NULL; fix = fix->fx_next)
	...
	continue;
      ...
      if (sym == NULL
	  || fix->fx_subsy != NULL
	  || fix->fx_addnumber != 0 || S_GET_SEGMENT (sym) != sec
	  ...
	  || S_IS_EXTERNAL (sym))
	{
      ...
      val = S_GET_VALUE (sym);
      for (fscan = seginfo->fix_root; fscan != NULL; fscan = fscan->fx_next)
      ...
      if (fscan->fx_tcbit)
	{
      ...
      if (sym == NULL
	  || fscan->fx_subsy != NULL
	  || fscan->fx_addnumber != 0 || S_GET_SEGMENT (sym) != sec
#if ! defined (BFD_ASSEMBLER) && defined (OBJ_COFF)
	  || S_GET_STORAGE_CLASS (sym) == C_EXT
#endif
	  || S_IS_EXTERNAL (sym))
...
#endif
{
  ...
  switch (fragP->fr_subtype)
	...
	as_bad_where (fragP->fr_file, fragP->fr_line,
		      _
		      ("displacement to defined symbol %s overflows 12-bit field"),
		      S_GET_NAME (fragP->fr_symbol));
      else
	as_bad_where (fragP->fr_file, fragP->fr_line,
		      _
		      ("displacement to undefined symbol %s overflows 12-bit field"),
		      S_GET_NAME (fragP->fr_symbol));
	...
	as_bad_where (fragP->fr_file, fragP->fr_line,
		      _
		      ("displacement to defined symbol %s overflows 8-bit field"),
		      S_GET_NAME (fragP->fr_symbol));
      else
	as_bad_where (fragP->fr_file, fragP->fr_line,
		      _
		      ("displacement to undefined symbol %s overflows 8-bit field "),
		      S_GET_NAME (fragP->fr_symbol));
      /* Stabilize this frag, so we don't trip an assert.  */
      fragP->fr_fix += fragP->fr_var;
      fragP->fr_var = 0;
  ...
  if (donerelax && !sh_relax)
    as_warn_where (fragP->fr_file, fragP->fr_line,
		   ...
		   (fragP->fr_symbol !=
		    NULL ? S_GET_NAME (fragP->fr_symbol) : ""));
}
...
sh_handle_align (fragS * frag)
{
  ...
  if (sh_relax
      ...
      && frag->fr_offset > 1 && now_seg != bss_section)
...
md_apply_fix3 (fixS * fixP, valueT * valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  if (OUTPUT_FLAVOR != bfd_target_elf_flavour
      && fixP->fx_addsy != NULL && S_IS_WEAK (fixP->fx_addsy))
  ...
  if (shift != 0)
	...
	val = ((val >> shift) | ((long) -1 & ~((long) -1 >> shift)));
    }
