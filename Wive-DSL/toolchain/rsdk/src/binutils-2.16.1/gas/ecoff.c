...
static symint_t add_string (varray_t * vp,
			    ...
			    const char *str, shash_t ** ret_hash);
...
static symint_t add_aux_sym_tir (type_info_t * t,
				 hash_state_t state, thash_t ** hash_tbl);
...
ecoff_symbol_new_hook (symbolS * symbolP)
{
  ...
  /* Make sure that we have a file pointer, but only if we have seen a
     ...
     wrong.  */
  if (cur_file_ptr == (efdr_t *) NULL && seen_at_least_1_file ())
	    ...
	    shash_t ** ret_hash /* return hash pointer */ )
{
  ...
  if (hash_ptr == (shash_t *) NULL)
      ...
      if (vp->objects_last_page + len >= PAGE_USIZE)
	{
      ...
      if (err)
	as_fatal (_("inserting \"%s\" into string hash table: %s"), str, err);
    }
	  ...
	  if (begin_type != st_File && storage != sc_Info)
	    {
	      ...
	      for (ptag = ptag_head->first_tag;
		   ptag != (tag_t *) NULL; ptag = ptag_next)
		{
		 ...
		 thash_t ** hash_tbl /* pointer to hash table to use */ )
{
  ...
  if (t->basic_type == bt_Struct
	...
	= &vp->last->datum->aux[vp->objects_last_page - 1];
      ...
      forward_ref->ifd_ptr = &vp->last->datum->aux[vp->objects_last_page - 1];
    }
	 ...
	 bt_t basic_type /* bt_Struct, bt_Union, or bt_Enum */ )
{
  ...
  if (hash_ptr != (shash_t *) NULL && hash_ptr->tag_ptr != (tag_t *) NULL)
    {
  ...
  if (hash_ptr == (shash_t *) NULL)
    {
      ...
      if (err)
	as_fatal (_("inserting \"%s\" into tag hash table: %s"), tag, err);
...
add_file (const char *file_name, int indx ATTRIBUTE_UNUSED, int fake)
{
  ...
  for (fil_ptr = first_file;
       fil_ptr != (efdr_t *) NULL; fil_ptr = fil_ptr->next_file)
    {
      if (first_ch == fil_ptr->name[0]
	  && strcmp (file_name, fil_ptr->name) == 0 && fil_ptr->fdr.fMerge)
	{
  ...
  if (fil_ptr == (efdr_t *) NULL)
	...
	&file_desc.last->datum->file[file_desc.objects_last_page++];
      *fil_ptr = init_file;
      ...
      add_string (&fil_ptr->strings, fil_ptr->str_hash, "", (shash_t **) 0);
...
ecoff_directive_endef (int ignore ATTRIBUTE_UNUSED)
{
  ...
  if (coff_symbol_typ == st_Block || coff_symbol_typ == st_End)
      ...
      if (coff_is_function)
	{
      ...
      indx = add_aux_sym_tir (&coff_type,
			      hash_yes, &cur_file_ptr->thash_head[0]);
    }
  ...
  sym = add_ecoff_symbol (name,
			  ...
			  coff_storage_class,
			  coff_sym_value,
			  coff_sym_addend, (symint_t) coff_value, indx);
...
ecoff_stab (segT sec ATTRIBUTE_UNUSED,
	    int what, const char *string, int type, int other, int desc)
{
...
static char *
ecoff_add_bytes (char **buf, char **bufend, char *bufptr, unsigned long need)
{
  unsigned long at;
  unsigned long want;
...
ecoff_padding_adjust (const struct ecoff_debug_swap *backend,
		      char **buf,
		      char **bufend, unsigned long offset, char **bufptrptr)
{
  bfd_size_type align;
...
ecoff_build_lineno (const struct ecoff_debug_swap *backend,
		    char **buf,
		    char **bufend, unsigned long offset, long *linecntptr)
{
  char *bufptr;
  ...
  if (debug_type != DEBUG_ECOFF
      ...
      && ((S_GET_VALUE (first_proc_ptr->sym->as_sym)
	   + bfd_get_section_vma (stdoutput,
				  S_GET_SEGMENT (first_proc_ptr->sym->
						 as_sym))) != 0))
    {
...
ecoff_build_symbols (const struct ecoff_debug_swap *backend,
		     char **buf, char **bufend, unsigned long offset)
{
  ...
  for (file_link = file_desc.first;
       file_link != (vlinks_t *) NULL; file_link = file_link->next)
    {
      int ifilesym;
      int fil_cnt;
      efdr_t *fil_ptr;
      ...
      for (; fil_ptr < fil_end; fil_ptr++)
	{
	  ...
	  for (sym_link = fil_ptr->symbols.first;
	       sym_link != (vlinks_t *) NULL; sym_link = sym_link->next)
	    {
	      int sym_cnt;
	      localsym_t *sym_ptr;
	      localsym_t *sym_end;
	      ...
	      for (; sym_ptr < sym_end; sym_ptr++)
		{
		  ...
		  if (as_sym != (symbolS *) NULL)
			...
			sym_ptr->ecoff_sym.asym.value =
			  ...
			  && S_IS_DEFINED (as_sym)
			  && !S_IS_EXTERNAL (as_sym) && !S_IS_WEAK (as_sym))
			  ...
			  if (!S_IS_DEFINED (as_sym))
			    {
			      ...
			      if (s == 0 || s > bfd_get_gp_size (stdoutput))
				sc = sc_Undefined;
			      else
				{
				  sc = sc_SUndefined;
		  ...
		  if (local)
		    {
		      if ((bfd_size_type) (*bufend - sym_out) <
			  external_sym_size)
			sym_out =
  ...
  for (file_link = file_desc.first;
      ...
      for (; fil_ptr < fil_end; fil_ptr++)
	{
	  ...
	  for (proc_link = fil_ptr->procs.first;
	       proc_link != (vlinks_t *) NULL; proc_link = proc_link->next)
	    {
	      int prc_cnt;
	      proc_t *proc_ptr;
	      proc_t *proc_end;
  ...
  for (file_link = file_desc.first;
      ...
      for (; fil_ptr < fil_end; fil_ptr++)
	{
	  ...
	  for (aux_link = fil_ptr->aux_syms.first;
	       aux_link != (vlinks_t *) NULL; aux_link = aux_link->next)
	    {
	      int aux_cnt;
	      aux_t *aux_ptr;
	      aux_t *aux_end;
	      ...
	      for (; aux_ptr < aux_end; aux_ptr++)
		{
		  ...
		  switch (aux_ptr->type)
		    ...
		    case aux_rndx:
		      ...
		      break;
		    case aux_dnLow:
		      AUX_PUT_DNLOW (bigendian, aux_ptr->data.dnLow, aux_out);
		      break;
		    case aux_dnHigh:
		      ...
		      break;
		    case aux_isym:
		      AUX_PUT_ISYM (bigendian, aux_ptr->data.isym, aux_out);
		      break;
		    case aux_iss:
		      AUX_PUT_ISS (bigendian, aux_ptr->data.iss, aux_out);
		      break;
		    case aux_width:
		      AUX_PUT_WIDTH (bigendian, aux_ptr->data.width, aux_out);
		      break;
		    case aux_count:
		      AUX_PUT_COUNT (bigendian, aux_ptr->data.count, aux_out);
		      break;
		    }
...
ecoff_build_strings (char **buf,
		     char **bufend, unsigned long offset, varray_t * vp)
{
  unsigned long istr;
  char *str_out;
  vlinks_t *str_link;
  ...
  for (str_link = vp->first;
       str_link != (vlinks_t *) NULL; str_link = str_link->next)
    {
...
ecoff_setup_ext (void)
{
  ...
  for (sym = symbol_rootP; sym != (symbolS *) NULL; sym = symbol_next (sym))
	...
	continue;
      ...
      if (!S_IS_EXTERNAL (sym) && !S_IS_WEAK (sym) && S_IS_DEFINED (sym))
	{
		   ...
		   char **bufp, const struct ecoff_debug_swap *backend)
{
  ...
  if ((bfd_size_type) (bufend - (buf + offset)) <
...
allocate_vlinks (void)
{
  ...
  register int unallocated =
    alloc_counts[(int) alloc_type_vlinks].unallocated;
  register page_type *cur_page =
    alloc_counts[(int) alloc_type_vlinks].cur_page;
...
allocate_shash (void)
{
  ...
  register page_type *cur_page =
    alloc_counts[(int) alloc_type_shash].cur_page;
...
allocate_thash (void)
{
  ...
  register page_type *cur_page =
    alloc_counts[(int) alloc_type_thash].cur_page;
...
allocate_tag (void)
{
  ...
  if (ptr != (tag_t *) NULL)
    ...
    {
      register int unallocated =
	alloc_counts[(int) alloc_type_tag].unallocated;
      register page_type *cur_page =
	alloc_counts[(int) alloc_type_tag].cur_page;
...
allocate_forward (void)
{
  ...
  register int unallocated =
    alloc_counts[(int) alloc_type_forward].unallocated;
  register page_type *cur_page =
    alloc_counts[(int) alloc_type_forward].cur_page;
...
allocate_thead (void)
{
  ...
  if (ptr != (thead_t *) NULL)
    ...
    {
      register int unallocated =
	alloc_counts[(int) alloc_type_thead].unallocated;
      register page_type *cur_page =
	alloc_counts[(int) alloc_type_thead].cur_page;
...
allocate_lineno_list (void)
{
  ...
  register int unallocated =
    alloc_counts[(int) alloc_type_lineno].unallocated;
  register page_type *cur_page =
    alloc_counts[(int) alloc_type_lineno].cur_page;

  if (unallocated == 0)
    {
      unallocated = PAGE_SIZE / sizeof (lineno_list_t);
      alloc_counts[(int) alloc_type_lineno].cur_page = cur_page =
	allocate_page ();
