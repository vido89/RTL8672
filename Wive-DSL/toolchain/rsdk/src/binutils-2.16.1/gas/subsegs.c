...
subsegs_begin (void)
{
  ...
  absolute_frchain.frch_root = absolute_frchain.frch_last =
    &zero_address_frag;
}

/*
...
subseg_set_rest (segT seg, subsegT subseg)
{
  ...
  for (frcP = *(lastPP = &frchain_root);
       frcP && frcP->frch_seg <= seg; frcP = *(lastPP = &frcP->frch_next))
    {
      if (frcP->frch_seg == seg && frcP->frch_subseg >= subseg)
	{
	  break;
	}
    }
   ...
   *
   */
  if (!frcP || (frcP->frch_seg > seg || frcP->frch_subseg > subseg))	/* Kinky logic only works with 2 segments.  */
    {
      /*
  ...
  for (i = 0; i < (int) SEG_MAXIMUM_ORDINAL; i++)
    {
	  ...
	  || (segname[0] == '.' && strcmp (segname + 1, s) == 0))
	{
     ...
     register subsegT subseg;
{
  ...
  know (seg == SEG_DATA
	|| seg == SEG_TEXT || seg == SEG_BSS || seg == SEG_ABSOLUTE);
#endif
...
subseg_get (const char *segname, int force_new)
{
  ...
  if (!force_new
      && now_seg_name
      && (now_seg_name == segname || !strcmp (now_seg_name, segname)))
    return now_seg;
...
#define EMIT_SECTION_SYMBOLS 1
  ...
  if (!EMIT_SECTION_SYMBOLS || symbol_table_frozen)
  ...
  else
    {
      ...
      if (s == NULL
	  || ((seg = S_GET_SEGMENT (s)) != sec && seg != undefined_section))
