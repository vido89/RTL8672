...
static int sub_actual (int, sb *, sb *, struct hash_control *, int, sb *,
		       int);
static const char *macro_expand_body (sb *, sb *, formal_entry *,
				      struct hash_control *, int);
		 ...
		 int (*get_line) (sb *))
{
  ...
  while (more)
	...
	i++;
      ...
      if (i < ptr->len && (ptr->ptr[i] == '.' || NO_PSEUDO_DOT || macro_mri))
	{
...
getstring (int idx, sb * in, sb * acc)
{
  ...
  while (idx < in->len
      ...
      if (in->ptr[idx] == '<')
	{
	  int nest = 0;
	  idx++;
	  while ((in->ptr[idx] != '>' || nest) && idx < in->len)
	    {
	      if (in->ptr[idx] == '!')
		{
		  idx++;
...
get_any_string (int idx, sb * in, sb * out, int expand, int pretend_quoted)
{
  ...
  if (idx < in->len)
    {
      if (in->len > idx + 2 && in->ptr[idx + 1] == '\''
	  && ISBASE (in->ptr[idx]))
	{
	  while (!ISSEP (in->ptr[idx]))
	    sb_add_char (out, in->ptr[idx++]);
	}
      else if (in->ptr[idx] == '%' && macro_alternate && expand)
	{
	  int val;
	  char buf[20];
	...
	{
	  if (macro_alternate && !macro_strip_at && expand)
	    {
	      /* Keep the quotes.  */
	      sb_add_char (out, '\"');
...
get_apost_token (int idx, sb * in, sb * name, int kind)
{
  ...
  if (idx < in->len
      && in->ptr[idx] == kind
      && (!macro_mri || macro_strip_at) && (!macro_strip_at || kind == '@'))
    idx++;
  return idx;
}
		   ...
		   struct hash_control *formal_hash, int locals)
{
  ...
  while (src < in->len)
		   ...
		   || (src > 0 && in->ptr[src - 1] == '@')))
	{
      ...
      else if (in->ptr[src] == '"' || (macro_mri && in->ptr[src] == '\''))
	{
	  inquote = !inquote;
	  sb_add_char (out, in->ptr[src++]);
	}
...
macro_expand (int idx, sb * in, macro_entry * m, sb * out)
{
  ...
  while (idx < in->len)
      ...
      if (scan < in->len && !macro_alternate && in->ptr[scan] == '=')
	{
	  ...
	  ptr =
	    (formal_entry *) hash_find (m->formal_hash, sb_terminate (&t));
	  if (!ptr)
