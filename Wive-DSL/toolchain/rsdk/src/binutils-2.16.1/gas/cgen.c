...
fixS *
gas_cgen_record_fixup (frag, where, insn, length, operand, opinfo, symbol,
		       offset)
     fragS *frag;
     int where;
     ...
     offsetT offset;
{
  ...
  fixP = fix_new (frag, where, length / 8, symbol, offset,
		  ...
		  ((int) BFD_RELOC_UNUSED + (int) operand->type));
     ...
     expressionS *exp;
{
  ...
  fixP = fix_new_exp (frag, where, length / 8, exp,
		      CGEN_OPERAND_ATTR_VALUE (operand,
					       CGEN_OPERAND_PCREL_ADDR),
		      (bfd_reloc_code_real_type) ((int) BFD_RELOC_UNUSED +
						  (int) operand->type));
  fixP->fx_cgen.insn = insn;
  fixP->fx_cgen.opinfo = opinfo;
     ...
     finished_insnS *result;
{
      ...
      for (i = 0; i < num_fixups; ++i)
	{
	  if (CGEN_OPERAND_ATTR_VALUE
	      (cgen_operand_lookup_by_num
	       (gas_cgen_cpu_desc, fixups[i].opindex), CGEN_OPERAND_RELAX))
	    {
	      relax_operand = i;
	      break;
	    }
  ...
  if (relax_operand != -1)
      ...
      if (exp->X_op != O_constant && exp->X_op != O_symbol)
	{
      ...
      frag_var (rs_machine_dependent, max_len - byte_len /* max chars */ ,
