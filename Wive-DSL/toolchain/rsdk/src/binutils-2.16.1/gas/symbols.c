	...
	/* We copy this string: OK to alter later.  */ )
{
  ...
  if (new_broken_words)
	...
	{
	  as_bad (_("cannot define symbol `%s' in absolute section"),
		  sym_name);
	  return NULL;
	}
      ...
      frag_opcode = frag_var (rs_broken_word,
			      ...
			      possible_bytes,
			      (relax_substateT) 0,
			      (symbolS *) broken_words, (offsetT) 0, NULL);

      /* We want to store the pointer to where to insert the jump
         table in the fr_opcode of the rs_broken_word frag.  This
         requires a little hackery.  */
      while (frag_tmp
	     && (frag_tmp->fr_type != rs_broken_word || frag_tmp->fr_opcode))
      ...
      else if (!S_IS_DEFINED (symbolP) || S_IS_COMMON (symbolP))
	{
     ...
     && !defined (OBJ_BOUT) && !defined (OBJ_MAYBE_BOUT))
		  ...
		  as_bad (_("symbol `%s' is already defined as \"%s\"/%s%ld"),
			  ...
			  od_buf, (long) S_GET_VALUE (symbolP));
		}
...
symbol_table_insert (symbolS * symbolP)
{
  ...
  if ((error_string =
       hash_jam (sy_hash, S_GET_NAME (symbolP), (PTR) symbolP)))
    {
      as_fatal (_("inserting \"%s\" into symbol table failed: %s"),
		S_GET_NAME (symbolP), error_string);
    }				/* on error  */
...
report_op_error (symbolS * symp, symbolS * left, symbolS * right)
{
  ...
  if (expr_symbol_where (symp, &file, &line))
	...
	as_bad_where (file, line,
		      _("undefined symbol `%s' in operation"),
		      S_GET_NAME (right));
      if (seg_left != undefined_section && seg_right != undefined_section)
	{
	  if (right)
	    as_bad_where (file, line,
			  _
...
resolve_symbol_value (symbolS * symp)
{
  ...
  if (LOCAL_SYMBOL_CHECK (symp))
	...
	return final_val;
      ...
      final_val +=
	local_symbol_get_frag (locsym)->fr_address / OCTETS_PER_BYTE;
	  ...
	  switch (symp->sy_value.X_op)
	    ...
	    case O_ne:
	      left = (left == right && seg_left == seg_right
		      && (seg_left != undefined_section
			  || add_symbol == op_symbol) ? ~(offsetT) 0 : 0);
...
define_dollar_label (long label)
{
  ...
  if (dollar_labels == NULL)
	...
	xrealloc (dollar_label_defines, dollar_label_max);
...
decode_local_label_name (char *s)
{
  ...
  sprintf (symbol_decode, message_format, label_number, instance_number,
	   type);
  ...
  if (s->bsym->flags & BSF_SECTION_SYM)
    {
      ...
      as_where (&file, &line);
      as_warn_where (file, line, _("section symbols are already global"));
      return;
    }
...
S_SET_THREAD_LOCAL (symbolS * s)
{
  ...
  if ((s->bsym->flags & BSF_FUNCTION) != 0)
    ...
    as_bad (_("Accessing `%s' as thread-local object"), S_GET_NAME (s));
}
  ...
  if (symbol_resolved_p (sym))
    {
      ...
      if (s != undefined_section && s != expr_section)
