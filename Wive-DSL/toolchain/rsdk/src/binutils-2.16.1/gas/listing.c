...
static void print_lines (list_info_type *, unsigned int, char *,
			 unsigned int);
static void list_symbol_table (void);
static void print_source (file_info_type *, list_info_type *, char *,
			  unsigned int);
...
listing_newline (char *ps)
{
  ...
  if ((listing & LISTING_NODEBUG) != 0
      && listing_tail != NULL && !listing_tail->debugging)
    {
      const char *segname;
  ...
  if (ps == NULL)
	  ...
	  && input_line_pointer != NULL)
	{
	  ...
	  for (copy = input_line_pointer - 1;
	       *copy && (seen_quote
			 || (!is_end_of_line[(unsigned char) *copy])); copy++)
	    if (*copy == '"' && copy[-1] != '\\')
...
listing_prev_line (void)
{
  ...
  if (head == (list_info_type *) NULL || head == listing_tail)
    return;
  ...
  while (frag_ptr != (fragS *) NULL && frag_ptr->line == first)
	     ...
	     && data_buffer_size < MAX_BYTES - 3)
	{
	  ...
	  sprintf (data_buffer + data_buffer_size,
		   "%02X", (frag_ptr->fr_literal[octet_in_frag]) & 0xff);
	  data_buffer_size += 2;
	  octet_in_frag++;
	}
      if (frag_ptr->fr_type == rs_fill)
	{
	  ...
	  while (((offsetT) octet_in_frag
		...
		address = frag_ptr->fr_address / OCTETS_PER_BYTE;
	      ...
	      sprintf (data_buffer + data_buffer_size,
		       "%02X", (frag_ptr->fr_literal[var_rep_idx]) & 0xff);
	      data_buffer_size += 2;

	      var_rep_idx++;
	      octet_in_frag++;

	      if ((offsetT) var_rep_idx >=
		  frag_ptr->fr_fix + frag_ptr->fr_var)
	     ...
	     char *string, unsigned int address)
{
  ...
  for (lines = 0;
       lines < (unsigned int) listing_lhs_cont_lines && src[cur]; lines++)
    {
	      ...
	      char *buffer, unsigned int width)
{
  if (!current_file->at_end)
    {
      while (current_file->linenum < list->hll_line && !current_file->at_end)
	{
  ...
  if (*line != '.')
    {
      ...
      /* The ELF compiler sometimes emits blank lines after switching
         ...
         general case.  */
      if (was_debug
	  && *line == '\0' && list->next != NULL && list->next->debugging)
	{
	  in_debug = 1;
	  return 1;
	}
...
listing_listing (char *name ATTRIBUTE_UNUSED)
{
  ...
  while (list)
	...
	default:
      ...
      if (show_listing <= 0)
	{
	  while (list->file->linenum < list_line && !list->file->at_end)
