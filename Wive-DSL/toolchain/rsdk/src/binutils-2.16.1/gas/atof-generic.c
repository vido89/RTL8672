...
#define IS_DECIMAL_MARK(c)	(0 != strchr (string_of_decimal_marks, (c)))
  ...
  for (p = first_digit;
       (((c = *p) != '\0')
	&& (!c || !IS_DECIMAL_MARK (c))
	&& (!c || !strchr (string_of_decimal_exponent_marks, c))); p++)
    {
      if (ISDIGIT (c))
	{
  ...
  *address_of_string_pointer = p;
  ...
  if (number_of_digits_available == 0)
  ...
  else
    {
      ...
      precision = (address_of_generic_floating_point_number->high - address_of_generic_floating_point_number->low + 1);	/* Number of destination littlenums.  */

      /* Includes guard bits (two littlenums worth) */
      maximum_useful_digits = (((precision - 2)) * ((LITTLENUM_NUMBER_OF_BITS)) * 1000000 / 3321928) + 2;	/* 2 :: guard digits.  */
	...
	= (more_than_enough_bits_for_digits / LITTLENUM_NUMBER_OF_BITS) + 2;
	...
	* sizeof (LITTLENUM_TYPE);
      ...
      digits_binary_low = (LITTLENUM_TYPE *) alloca (size_of_digits_in_chars);
      ...
      for (p = first_digit, count = number_of_digits_to_use; count;
	   p++, --count)
	{
	  c = *p;
	  if (ISDIGIT (c))
		...
		+ more_than_enough_littlenums_for_digits - 1;

	      carry = c - '0';	/* char -> binary */

	      for (littlenum_pointer = digits_binary_low;
		   littlenum_pointer <= littlenum_limit; littlenum_pointer++)
		{
		  long work;
	...
	size_of_digits_in_littlenums--;
      ...
      digits_flonum.high =
	digits_binary_low + size_of_digits_in_littlenums - 1;
	...
	temporary_binary_low =
	  (LITTLENUM_TYPE *) alloca (size_of_power_in_chars);
	memset ((char *) power_binary_low, '\0', size_of_power_in_chars);
	*power_binary_low = 1;
	power_of_10_flonum.exponent = 0;
	power_of_10_flonum.low = power_binary_low;
	power_of_10_flonum.leader = power_binary_low;
	power_of_10_flonum.high =
	  power_binary_low + size_of_power_in_littlenums - 1;
	power_of_10_flonum.sign = '+';
	temporary_flonum.low = temporary_binary_low;
	temporary_flonum.high =
	  temporary_binary_low + size_of_power_in_littlenums - 1;
	/*
	 * (power) == 1.
	 ...
	 */
	{
      ...
      flonum_multip (&power_of_10_flonum, &digits_flonum,
