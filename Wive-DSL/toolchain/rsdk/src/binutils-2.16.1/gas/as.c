...
#endif
static enum debug_info_type (*md_debug_format_selector) (int *) =
...
parse_args (int *pargc, char ***pargv)
{
  ...
  static const struct option std_longopts[] = {
    ...
    , {"listing-lhs-width2", required_argument, NULL,
       OPTION_LISTING_LHS_WIDTH2}
    , {"listing-rhs-width", required_argument, NULL, OPTION_LISTING_RHS_WIDTH}
    , {"listing-cont-lines", required_argument, NULL,
       OPTION_LISTING_CONT_LINES}
  ...
  longopts =
    xmalloc (sizeof (std_longopts) + md_longopts_size +
	     sizeof (struct option));
  memcpy (longopts, std_longopts, sizeof (std_longopts));
  memcpy (((char *) longopts) + sizeof (std_longopts), md_longopts,
	  md_longopts_size);
  memset (((char *) longopts) + sizeof (std_longopts) + md_longopts_size, 0,
	  sizeof (struct option));
  ...
  while (1)
	...
	case OPTION_VERBOSE:
	    ...
	    as_bad (_("unrecognized option -%c%s"), optc,
		    optarg ? optarg : "");
	  /* Fall through.  */
