...
typedef void (*callback_t) (lang_wild_statement_type *,
			    struct wildcard_list *, asection *,
...
unique_section_p (const asection * sec)
{
  ...
  if (link_info.relocatable
      && sec->owner != NULL && bfd_is_group_section (sec->owner, sec))
    return TRUE;
  ...
  for (s = file->the_bfd->sections; s != NULL; s = s->next)
      ...
      while (sec != NULL)
	{
	  ...
	  for (list_tmp = sec->spec.exclude_name_list;
	       list_tmp; list_tmp = list_tmp->next)
	    {
...
walk_wild_file (lang_wild_statement_type * s,
		...
		callback_t callback, void *data)
{
  if (f->the_bfd == NULL || !bfd_check_format (f->the_bfd, bfd_archive))
...
new_statement (enum statement_enum type,
	       size_t size, lang_statement_list_type * list)
{
...
new_afile (const char *name,
	   ...
	   const char *target, bfd_boolean add_to_list)
{
  ...
  lang_statement_append (&input_file_chain,
			 (lang_statement_union_type *) p, &p->next_real_file);
  return p;
}

lang_input_statement_type *
lang_add_input_file (const char *name,
		     lang_input_file_enum_type file_type, const char *target)
{
  lang_has_input_file = TRUE;
...
lang_memory_default (asection * section)
{
  ...
  for (p = lang_memory_region_list; p != NULL; p = p->next)
    {
      if ((p->flags & sec_flags) != 0 && (p->not_flags & sec_flags) == 0)
	{
	  return p;
	}
    }
...
lang_output_section_find_1 (const char *const name, int constraint)
{
  ...
  for (lookup = &lang_output_section_statement.head->output_section_statement;
       lookup != NULL; lookup = lookup->next)
    {
...
static lang_output_section_statement_type *
lang_output_section_statement_lookup_1 (const char *const name,
					int constraint)
{
  lang_output_section_statement_type *lookup;
		    ...
		    lang_statement_list_type * add_child)
{
  ...
  if (after != NULL && after->load_base != NULL)
    {
      ...
      load_base = exp_binop ('+', lma_from_vma, exp_nameop (ADDR, secname));
    }
  ...
  if (config.build_constructors && *ps == '\0')
	...
	stat_ptr = &add;
      ...
      lang_add_assignment (exp_assop ('=', symname, exp_nameop (NAME, ".")));
    }
	  ...
	  if (place->stmt == NULL)
	    {
		   ...
		   *where != NULL; where = &(*where)->header.next)
		{
...
wild_sort (lang_wild_statement_type * wild,
	   struct wildcard_list *sec,
	   lang_input_statement_type * file, asection * section)
{
  const char *section_name;
  lang_statement_union_type *l;

  if (!wild->filenames_sorted && (sec == NULL || sec->spec.sorted == none))
    return NULL;
  ...
  for (l = wild->children.head; l != NULL; l = l->header.next)
      ...
      if (wild->filenames_sorted)
	{
	  ...
	  if (file->the_bfd != NULL && bfd_my_archive (file->the_bfd) != NULL)
	    {
...
output_section_callback (lang_wild_statement_type * ptr,
			 struct wildcard_list *sec,
			 asection * section,
			 lang_input_statement_type * file, void *output)
{
  ...
  if (before == NULL)
    lang_add_section (&ptr->children, section,
		      (lang_output_section_statement_type *) output, file);
  else
    {
...
lookup_name (const char *name)
{
  ...
  for (search = (lang_input_statement_type *) input_file_chain.head;
	...
	return search;
      if (filename != NULL && name != NULL && strcmp (filename, name) == 0)
	break;
    }
  ...
  if (search->loaded || !search->real || search->filename == NULL)
    return search;
	      ...
	      lang_statement_list_type * place)
{
  ...
  if (!bfd_check_format (entry->the_bfd, bfd_archive)
      ...
      if (err == bfd_error_file_ambiguously_recognized)
	{
      ...
      else if (err != bfd_error_file_not_recognized || place == NULL)
...
get_first_input_target (void)
{
  ...
  {
    if (s->header.type == lang_input_statement_enum && s->real)
      {
	ldfile_open_file (s);

	if (s->the_bfd != NULL && bfd_check_format (s->the_bfd, bfd_object))
	  {
	  ...
	  if (target->byteorder != desired_endian)
	    {
		  ...
		  && (target->alternative_target->byteorder ==
		      desired_endian))
...
lang_update_definedness (const char *name, struct bfd_link_hash_entry *h)
{
  ...
  if (h->type != bfd_link_hash_undefined
      ...
      && h->type != bfd_link_hash_new && defentry->iteration == -1)
    return;
   ...
   lang_output_section_statement_type * os)
{
  for (; s != NULL; s = s->header.next)
	...
	case lang_output_section_statement_enum:
	  if (s->output_section_statement.constraint)
		...
		break;
		  ...
		  || (!s->output_section_statement.all_input_readonly
		      && s->output_section_statement.constraint ==
		      ONLY_IF_RO))
		{
	  ...
	  map_input_to_output_sections (s->output_section_statement.children.
					head, target,
					&s->output_section_statement);
	  break;
	case lang_output_statement_enum:
...
os_region_check (lang_output_section_statement_type * os,
		 ...
		 etree_type * tree, bfd_vma base)
{
  if ((region->current < region->origin
       || (region->current - region->origin > region->length))
      && ((region->current != region->origin + region->length) || base == 0))
    {
      if (tree != NULL)
	{
	  einfo (_("%X%P: address 0x%v of %B section %s"
		 ...
		 region->current,
		 os->bfd_section->owner, os->bfd_section->name, region->name);
	}
      else
	{
	  einfo (_("%X%P: region %s is full (%B section %s)\n"),
		 region->name, os->bfd_section->owner, os->bfd_section->name);
	}
	...
	case lang_output_section_statement_enum:
	  {
	    ...
	    after = ((os->bfd_section->vma
		      ...
		      + os->block_value - 1) & -(bfd_vma) os->block_value);
	...
	case lang_assignment_statement_enum:
	  {
	    ...
	    exp_fold_tree (s->assignment_statement.exp,
			   output_section_statement,
			   lang_allocating_phase_enum, dot, &newdot);
...
lang_check (void)
{
  ...
  for (file = file_chain.head; file != NULL;
...
lang_common (void)
{
  if (command_line.inhibit_common_definition)
    return;
  if (link_info.relocatable && !command_line.force_common_definition)
    return;
...
lang_one_common (struct bfd_link_hash_entry *h, void *info)
{
  ...
  if (config.sort_common && power_of_two < (unsigned int) *(int *) info)
    return TRUE;
...
lang_for_each_input_file (void (*func) (lang_input_statement_type *))
{
  ...
  for (f = (lang_input_statement_type *) input_file_chain.head;
       f != NULL; f = (lang_input_statement_type *) f->next_real_file)
    func (f);
}
...
ldlang_add_file (lang_input_statement_type * entry)
{
  ...
  lang_statement_append (&file_chain,
			 (lang_statement_union_type *) entry, &entry->next);
...
lang_output_section_statement_type *
lang_enter_output_section_statement (const char
				     *output_section_statement_name,
				     etree_type * address_exp,
				     enum section_type sectype,
				     etree_type * align,
				     etree_type * subalign,
				     etree_type * ebase, int constraint)
{
  lang_output_section_statement_type *os;
...
lang_process (void)
{
  ...
  if (!link_info.relocatable && command_line.check_section_addresses)
...
lang_add_wild (struct wildcard_spec *filespec,
	       struct wildcard_list *section_list, bfd_boolean keep_sections)
{
  ...
  for (curr = section_list, section_list = NULL;
       curr != NULL; section_list = curr, curr = next)
    {
...
lang_add_reloc (bfd_reloc_code_real_type reloc,
		...
		asection * section,
		const char *name, union etree_union *addend)
{
...
lang_get_regions (lang_memory_region_type ** region,
		  ...
		  bfd_boolean have_lma, bfd_boolean have_vma)
{
  *lma_region = lang_memory_region_lookup (lma_memspec, FALSE);
  ...
  /* If no runtime region or VMA has been specified, but the load region
     ...
     as well.  */
  if (lma_memspec != NULL
      && !have_vma && strcmp (memspec, DEFAULT_MEMORY_REGION) == 0)
    *region = *lma_region;
  else
...
lang_abs_symbol_at_beginning_of (const char *secname, const char *name)
{
  ...
  if (h->type == bfd_link_hash_new || h->type == bfd_link_hash_undefined)
    {
      asection *sec;
...
lang_add_output_format (const char *format,
			const char *big, const char *little, int from_script)
{
  if (output_target == NULL || !from_script)
    {
      if (command_line.endian == ENDIAN_BIG && big != NULL)
	format = big;
      else if (command_line.endian == ENDIAN_LITTLE && little != NULL)
	format = little;
...
lang_new_phdr (const char *name,
	       etree_type * type,
	       bfd_boolean filehdr,
	       bfd_boolean phdrs, etree_type * at, etree_type * flags)
{
...
lang_record_phdrs (void)
{
  ...
  for (os = &lang_output_section_statement.head->output_section_statement;
       os != NULL; os = os->next)
    {
      ...
      if (os->constraint == -1 || os->bfd_section == NULL)
	continue;

      for (pl = os->phdrs; pl != NULL; pl = pl->next)
...
lang_enter_overlay (etree_type * vma_expr, etree_type * subalign)
{
  ...
  ASSERT (overlay_vma == NULL
	  && overlay_subalign == NULL && overlay_max == NULL);
			    ...
			    lang_output_section_phdr_list * phdrs)
{
  ...
  *s2 = '\0';
  ...
  lang_add_assignment (exp_assop ('=', buf, exp_nameop (LOADADDR, name)));
		    ...
		    const char *lma_memspec)
{
  ...
  lang_get_regions (&region, &lma_region,
		    memspec, lma_memspec, lma_expr != NULL, FALSE);
...
lang_new_vers_pattern (struct bfd_elf_version_expr *orig,
		       const char *new, const char *lang)
{
