...
add_cref (const char *name,
	  bfd * abfd, asection * section, bfd_vma value ATTRIBUTE_UNUSED)
{
...
check_nocrossref (struct cref_hash_entry *h, void *ignore ATTRIBUTE_UNUSED)
{
  ...
  if (hl->type != bfd_link_hash_defined && hl->type != bfd_link_hash_defweak)
    return TRUE;
...
check_refs (const char *name,
	    asection * sec, bfd * abfd, struct lang_nocrossrefs *ncrs)
{
  lang_input_statement_type *li;
  asymbol **asymbols;
