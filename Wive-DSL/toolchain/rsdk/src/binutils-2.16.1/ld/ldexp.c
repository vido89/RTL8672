...
new_rel (bfd_vma value,
	 char *str, lang_output_section_statement_type * section)
{
  etree_value_type new;
  new.valid_p = TRUE;
  new.value = value;
...
fold_unary (etree_type * tree,
	    ...
	    lang_phase_type allocation_done, bfd_vma dot, bfd_vma * dotp)
{
...
#define BOP(x,y) case x : result.value = result.value y other.value; break;
		      ...
		      || allocation_done != lang_allocating_phase_enum))
		{
		  if (exp_data_seg.phase == exp_dataseg_align_seen
		      || exp_data_seg.phase == exp_dataseg_relro_adjust)
		    exp_data_seg.relro_end = result.value + other.value;
		  if (exp_data_seg.phase == exp_dataseg_relro_adjust
...
fold_name (etree_type * tree,
	   lang_output_section_statement_type * current_section,
	   lang_phase_type allocation_done, bfd_vma dot)
{
  etree_value_type result;
	       ...
	       lang_phase_type allocation_done, bfd_vma dot, bfd_vma * dotp)
{
  ...
  switch (tree->type.node_class)
    ...
    case etree_unary:
      result = fold_unary (tree, current_section, allocation_done, dot, dotp);
      break;
...
exp_binop (int code, etree_type * lhs, etree_type * rhs)
{
  ...
  r = exp_fold_tree_no_dot (&value,
			    abs_output_section, lang_first_phase_enum);
  if (r.valid_p)
    {
...
exp_get_vma (etree_type * tree,
	     bfd_vma def, char *name, lang_phase_type allocation_done)
{
  etree_value_type r;
...
exp_get_value_int (etree_type * tree,
		   int def, char *name, lang_phase_type allocation_done)
{
...
exp_get_fill (etree_type * tree,
	      fill_type * def, char *name, lang_phase_type allocation_done)
{
  fill_type *fill;
  etree_value_type r;
  size_t len;
...
exp_get_abs_int (etree_type * tree,
		 ...
		 char *name, lang_phase_type allocation_done)
{
