...
static void set_scripts_dir (void);
  ...
  if (link_info.wrap_hash == NULL)
    {
      ...
      if (!bfd_hash_table_init_n (link_info.wrap_hash, bfd_hash_newfunc, 61))
	einfo (_("%P%F: bfd_hash_table_init failed: %E\n"));
    }
  ...
  if (config.map_file != NULL)
      ...
      else
	{
	  ...
	  len = (strlen (bfd_get_filename (bfd_my_archive (abfd)))
		 + strlen (bfd_get_filename (abfd)) + 2);
	}
...
multiple_definition (struct bfd_link_info *info ATTRIBUTE_UNUSED,
		     ...
		     bfd * obfd,
		     asection * osec,
		     bfd_vma oval, bfd * nbfd, asection * nsec, bfd_vma nval)
{
		 ...
		 bfd * nbfd, enum bfd_link_hash_type ntype, bfd_vma nsize)
{
  ...
  if (ntype == bfd_link_hash_defined
      || ntype == bfd_link_hash_defweak || ntype == bfd_link_hash_indirect)
    {
...
add_to_set (struct bfd_link_info *info ATTRIBUTE_UNUSED,
	    ...
	    bfd * abfd, asection * section, bfd_vma value)
{
...
undefined_symbol (struct bfd_link_info *info ATTRIBUTE_UNUSED,
		  const char *name,
		  bfd * abfd,
		  asection * section, bfd_vma address, bfd_boolean error)
{
...
#define MAX_ERRORS_IN_A_ROW 5
  ...
  /* We never print more than a reasonable number of errors in a row
     for a single symbol.  */
  if (error_name != NULL && strcmp (name, error_name) == 0)
    ++error_count;
  else
    {
      error_count = 0;
		...
		bfd * abfd, asection * section, bfd_vma address)
{
  ...
  if (overflow_cutoff_limit >= 0 && overflow_cutoff_limit-- == 0)
    {
  ...
  if (entry)
	...
	case bfd_link_hash_defweak:
	  einfo (_
		 ...
		 reloc_name, entry->root.string, entry->u.def.section,
...
notice (struct bfd_link_info *info,
	const char *name, bfd * abfd, asection * section, bfd_vma value)
{
  if (!info->notice_all
