...
#define NE pe_def_file->num_exports
  ...
  for (i = 0, j = 0; i < NE; i++)
      ...
      if (i > 0 && strcmp (e[i].name, e[i - 1].name) == 0)
	{
	  /* This is a duplicate.  */
	  if (e[j - 1].ordinal != -1
	      && e[i].ordinal != -1 && e[j - 1].ordinal != e[i].ordinal)
	    {
  ...
  for (i = 0; i < NE; i++)
	...
	strcpy (name, pe_def_file->exports[i].internal_name);
      ...
      blhe = bfd_link_hash_lookup (info->hash, name, FALSE, FALSE, TRUE);
  ...
  if (include_edata)
    {
	  ...
	  || !bfd_set_section_flags (filler_bfd, edata_s,
				     (SEC_HAS_CONTENTS
				      | SEC_ALLOC
				      | SEC_LOAD | SEC_KEEP | SEC_IN_MEMORY)))
	{
...
static void
fill_exported_offsets (bfd * abfd ATTRIBUTE_UNUSED,
...
pe_walk_relocs_of_symbol (struct bfd_link_info *info,
			  const char *name, int (*cb) (arelent *, asection *))
{
  bfd *b;
  asection *s;
...
#define BITS_AND_SHIFT(bits, shift) (bits * 1000 | shift)
		  ...
		  switch BITS_AND_SHIFT
		    (relocs[i]->howto->bitsize, relocs[i]->howto->rightshift)
		    {
      ...
      if (pe_def_file->name)
	{
	  ...
	  if (pe_data (output_bfd)->pe_opthdr.ImageBase)
	    fprintf (out, " BASE=0x%lx",
		     (unsigned long) pe_data (output_bfd)->pe_opthdr.
		     ImageBase);
	  fprintf (out, "\n");
	}
...
quick_symbol (bfd * abfd,
	      const char *n1,
	      const char *n2,
	      const char *n3, asection * sec, int flags, int addr)
{
  asymbol *sym;
...
make_import_fixup_mark (arelent * rel)
{
  ...
  bh = NULL;
  bfd_coff_link_add_one_symbol (&link_info, abfd, fixup_name, BSF_GLOBAL, current_sec,	/* sym->section, */
...
make_import_fixup_entry (const char *name,
			 ...
			 const char *dll_symname, bfd * parent)
{
  asection *id3;
  unsigned char *d3;
  char *oname;
...
make_runtime_pseudo_reloc (const char *name ATTRIBUTE_UNUSED,
			   const char *fixup_name, int addend, bfd * parent)
{
  asection *rt_rel;
	  ...
	  if (pe_dll_extra_pe_debug)
	    printf
	      ("creating runtime pseudo-reloc entry for %s (addend=%d)\n",
	       fixup_name, addend);
	  b =
	    make_runtime_pseudo_reloc (name, fixup_name, addend, output_bfd);
	  add_bfd_to_link (b, b->filename, &link_info);
		 ...
		 struct bfd_link_info *link_info)
{
  ...
  fake_file = lang_add_input_file (name, lang_input_file_is_fake_enum, NULL);
...
void
pe_exe_build_sections (bfd * abfd,
