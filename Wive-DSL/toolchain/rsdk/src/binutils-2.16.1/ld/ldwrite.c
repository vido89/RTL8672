	...
	if ((output_section->flags & SEC_HAS_CONTENTS) != 0)
	  {
	    ...
	    link_order->u.data.contents =
	      statement->padding_statement.fill->data;
...
ds (asection * s)
{
  ...
  while (l)
	...
	{
	  printf ("%8x %s\n", l->offset,
		  l->u.indirect.section->owner->filename);
	}
      else
	{
...
split_sections (bfd * abfd, struct bfd_link_info *info)
{
  ...
  for (original_sec = abfd->sections;
       original_sec && nsecs; original_sec = original_sec->next, nsecs--)
    {
      int count = 0;
