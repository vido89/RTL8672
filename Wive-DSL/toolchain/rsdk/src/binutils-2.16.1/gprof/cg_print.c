...
cmp_arc (Arc * left, Arc * right)
{
  ...
  DBG (TIMEDEBUG,
       ...
       printf (" %f + %f %lu/%lu\n", right->time, right->child_time,
	       right->count, right_child->ncalls); printf ("\n"););
  ...
  for (index = 0; index < symtab.len + num_cycles; ++index)
    {
	  ...
	  || !parent->cg.print_flag || (line_granularity && !parent->is_func))
	continue;
  ...
  for (index = 0; index < scratch_arc_count; index++)
    {
      ...
      if (arc->child->has_been_placed && arc->parent->has_been_placed)
	{
...
cg_print_file_ordering ()
{
  ...
  for (index = 0; index < numarcs; index++)
    {
      if (!arcs[index]->parent->mapped || !arcs[index]->child->mapped)
  ...
  for (index = 0; index < symtab.len; index++)
    {
      if (symtab.base[index].mapped && !symtab.base[index].has_been_placed)
