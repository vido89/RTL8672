...
#endif
static int gmon_read_raw_arc (FILE *, bfd_vma *, bfd_vma *, unsigned long *);
static int gmon_write_raw_arc (FILE *, bfd_vma, bfd_vma, unsigned long);
...
static int
gmon_read_raw_arc (FILE * ifp, bfd_vma * fpc, bfd_vma * spc,
		   unsigned long *cnt)
{
#ifdef BFD_HOST_U_64_BIT
  ...
  if (gmon_io_read_vma (ifp, fpc) || gmon_io_read_vma (ifp, spc))
    return 1;
...
gmon_write_raw_arc (FILE * ofp, bfd_vma fpc, bfd_vma spc, unsigned long cnt)
{
  ...
  if (gmon_io_write_vma (ofp, fpc) || gmon_io_write_vma (ofp, spc))
    return 1;
  ...
  if ((file_format == FF_MAGIC)
      ...
      while (fread (&tag, sizeof (tag), 1, ifp) == 1)
	{
	  switch (tag)
	   ...
	   || file_format == FF_BSD || file_format == FF_BSD44)
    {
      struct hdr
      {
	bfd_vma low_pc;
  ...
  if (file_format == FF_AUTO || file_format == FF_MAGIC)
    {
      ...
      bfd_put_32 (core_bfd, (bfd_vma) GMON_VERSION,
		  (bfd_byte *) ghdr.version);
  ...
  else if (file_format == FF_BSD || file_format == FF_BSD44)
    {
      ...
      if (file_format == FF_BSD44 || hz != hertz ())
	{
	  padsize = 3 * 4;
	  switch (gmon_get_ptr_size ())
	    {
      ...
      if (file_format == FF_BSD44 || hz != hertz ())
	{
      ...
      /* Now write out any necessary padding after the meaningful
         header bits.  */
      if (padsize != 0 && fwrite (pad, 1, padsize, ofp) != padsize)
	{
	  perror (filename);
	  done (1);
	}
