...
#ifdef MBS_SUPPORT
static reg_errcode_t wcs_regex_compile
_RE_ARGS ((const char *pattern, size_t size, reg_syntax_t syntax,
	   struct re_pattern_buffer * bufp));
...
static int wcs_re_match_2_internal PARAMS ((struct re_pattern_buffer * bufp,
					    ...
					    int *mbs_offset1,
					    int *mbs_offset2));
static int wcs_re_search_2
PARAMS ((struct re_pattern_buffer * bufp, const char *string1, int size1,
...
# ifdef DEBUG
static void
PREFIX (extract_number)
_RE_ARGS ((int *dest, UCHAR_T * source));
     static void PREFIX (extract_number) (dest, source)
     int *dest;
     UCHAR_T *source;
{
#  ifdef WCHAR
...
# ifdef DEBUG
static void
PREFIX (extract_number_and_incr)
_RE_ARGS ((int *destination, UCHAR_T ** source));
     static void PREFIX (extract_number_and_incr) (destination, source)
     int *destination;
     UCHAR_T **source;
{
...
void PREFIX (print_partial_compiled_pattern) (start, end)
     UCHAR_T *
       start;
     UCHAR_T *
       end;
...
void PREFIX (print_compiled_pattern) (bufp)
...
void PREFIX (print_double_string) (where, string1, size1, string2, size2)
     const CHAR_T *
       where;
     const CHAR_T *
       string1;
...
static void
PREFIX (store_op1)
_RE_ARGS ((re_opcode_t op, UCHAR_T * loc, int arg));
     ...
     static void PREFIX (insert_op2) _RE_ARGS ((re_opcode_t op, UCHAR_T * loc,
						int arg1, int arg2,
						UCHAR_T * end));
     static boolean PREFIX (at_begline_loc_p)
  _RE_ARGS ((const CHAR_T * pattern, const CHAR_T * p, reg_syntax_t syntax));
     static boolean PREFIX (at_endline_loc_p)
  _RE_ARGS ((const CHAR_T * p, const CHAR_T * pend, reg_syntax_t syntax));
# ifdef WCHAR
     ...
     static void insert_space
       _RE_ARGS ((int num, CHAR_T * loc, CHAR_T * end));
# else /* BYTE */
     static reg_errcode_t byte_compile_range
       _RE_ARGS ((unsigned int range_start, const char **p_ptr,
...
/* Make the register vectors big enough for NUM_REGS registers,
   but don't make them smaller.  */
     ...
     static void PREFIX (regex_grow_registers) (num_regs)
     int num_regs;
{
	...
	case '[':
	  {
		...
		if (c == '-'
		  ...
		  {		/* This handles ranges made up of characters only.  */
		    ...
		    b += 2;
		    ret =
		      wcs_compile_range (c, &p, pend, translate, syntax, b,
					 laststart);
		    if (ret != REG_NOERROR)
		      FREE_STACK_RETURN (ret);
		    range_start = 0xffffffff;
...
# endif
		...
		if (c == '-'
		  ...
		  {		/* This handles ranges made up of characters only.  */
		    ...
		    ret =
		      byte_compile_range (c, &p, pend, translate, syntax, b);
		    if (ret != REG_NOERROR)
		      FREE_STACK_RETURN (ret);
		    range_start = 0xffffffff;
		  }
...
# endif
	...
	case '\\':
	  ...
	  switch (c)
		...
		fixup_alt_jump =
		  COMPILE_STACK_TOP.fixup_alt_jump ? COMPILED_BUFFER_VAR +
...
static void PREFIX (store_op1) (op, loc, arg)
     re_opcode_t
       op;
     UCHAR_T *
       loc;
...
static void PREFIX (store_op2) (op, loc, arg1, arg2)
     re_opcode_t
       op;
     UCHAR_T *
       loc;
...
static void PREFIX (insert_op1) (op, loc, arg, end)
     re_opcode_t
       op;
     UCHAR_T *
       loc;
...
static void PREFIX (insert_op2) (op, loc, arg1, arg2, end)
     re_opcode_t
       op;
     UCHAR_T *
       loc;
...
static boolean PREFIX (at_begline_loc_p) (pattern, p, syntax)
     const CHAR_T *
     pattern, *
       p;
     reg_syntax_t
...
static boolean PREFIX (at_endline_loc_p) (p, pend, syntax)
     const CHAR_T *
     p, *
       pend;
     reg_syntax_t
...
# if _LIBC
  ...
  for (this_char = 0; this_char <= (unsigned char) -1; ++this_char)
    {
      unsigned int this_colseq =
	collseq[(unsigned char) TRANSLATE (this_char)];
...
#endif
     static int
       PREFIX (re_search_2) (bufp, string1, size1, string2, size2, startpos,
			     range, regs, stop)
     struct re_pattern_buffer *bufp;
     const char *string1, *string2;
     int size1, size2;
...
# endif
  ...
  _RE_ARGS ((UCHAR_T ** p, UCHAR_T * end,
  ...
  _RE_ARGS ((UCHAR_T ** p, UCHAR_T * end,
	     PREFIX (register_info_type) * reg_info));
     static int PREFIX (bcmp_translate)
  _RE_ARGS ((const CHAR_T * s1, const CHAR_T * s2, int len, char *translate));
#else /* not INSIDE_RECURSION */
...
#ifdef MBS_SUPPORT
		     || (re_opcode_t) p1[1 + OFFSET_ADDRESS_SIZE] ==
		     exactn_bin
#endif
...
static boolean PREFIX (group_match_null_string_p) (p, end, reg_info)
     UCHAR_T **
     p, *
       end;
PREFIX (register_info_type) * reg_info;
...
static boolean PREFIX (alt_match_null_string_p) (p, end, reg_info)
     UCHAR_T *
     p, *
       end;
PREFIX (register_info_type) * reg_info;
...
static boolean PREFIX (common_op_match_null_string_p) (p, end, reg_info)
     UCHAR_T **
     p, *
       end;
PREFIX (register_info_type) * reg_info;
...
static int PREFIX (bcmp_translate) (s1, s2, len, translate)
     const CHAR_T *
     s1, *
       s2;
     register int
