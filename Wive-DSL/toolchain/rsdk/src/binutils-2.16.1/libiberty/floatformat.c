...
static int floatformat_i387_ext_is_valid
PARAMS ((const struct floatformat * fmt, const char *from));
     ...
     double *to;
{
  ...
  if ((unsigned long) exponent == fmt->exp_nan)
	...
	dto = INFINITY;
      ...
      if (get_field
	  (ufrom, fmt->byteorder, fmt->totalsize, fmt->sign_start, 1))
	dto = -dto;
  ...
  special_exponent = exponent == 0
    || (unsigned long) exponent == fmt->exp_nan;
  ...
  while (mant_bits_left > 0)
	...
	dto += ldexp ((double) mant,
		      (-fmt->exp_bias
		       - mant_bits - (mant_off - fmt->man_start) + 1));
      else
  ...
  if (dfrom != dfrom)
    {
      ...
      put_field (uto, fmt->byteorder, fmt->totalsize, fmt->man_start, 32, 1);
      return;
    }
