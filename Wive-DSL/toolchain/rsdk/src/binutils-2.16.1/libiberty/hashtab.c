...
htab_t
htab_create_alloc_ex (size, hash_f, eq_f, del_f, alloc_arg, alloc_f, free_f)
     size_t size;
     htab_hash hash_f;
     htab_eq eq_f;
     htab_del del_f;
...
hashval_t
iterative_hash (k_in, length, initval)
     ...
     register hashval_t initval;	/* the previous hash, or an arbitrary value */
{
  ...
  if (sizeof (hashval_t) == 4 && (((size_t) k) & 3) == 0)
    ...
    while (len >= 12)
      {
	a +=
	  (k[0] + ((hashval_t) k[1] << 8) + ((hashval_t) k[2] << 16) +
	   ((hashval_t) k[3] << 24));
	b +=
	  (k[4] + ((hashval_t) k[5] << 8) + ((hashval_t) k[6] << 16) +
	   ((hashval_t) k[7] << 24));
	c +=
	  (k[8] + ((hashval_t) k[9] << 8) + ((hashval_t) k[10] << 16) +
	   ((hashval_t) k[11] << 24));
	mix (a, b, c);
	k += 12;
	len -= 12;
