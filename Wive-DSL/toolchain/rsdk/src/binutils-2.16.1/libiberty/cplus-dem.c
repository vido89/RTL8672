...
static void delete_non_B_K_work_stuff PARAMS ((struct work_stuff *));
...
#if 0
static int
  demangle_method_args
PARAMS ((struct work_stuff *, const char **, string *));
#endif

static char *internal_cplus_demangle
...
static int
  demangle_template
PARAMS ((struct work_stuff * work, const char **, string *, string *, int,
	 int));
...
static void string_append_template_idx PARAMS ((string *, int));
...
PARAMS ((struct work_stuff *, const char **, string *, const char *));
  ...
  if (opname[0] == '_' && opname[1] == '_'
      ...
      else
	{
	  if (opname[2] == 'a' && opname[5] == '\0')
	   ...
	   && opname[0] == 'o'
	   && opname[1] == 'p' && strchr (cplus_markers, opname[2]) != NULL)
    {
     ...
     int option ATTRIBUTE_UNUSED;
{
  ...
  if (ISDIGIT ((unsigned char) mangled[len0 - 1]))
    {
      for (i = len0 - 2; i >= 0 && ISDIGIT ((unsigned char) mangled[i]);
	   i -= 1)
	;
     ...
     struct work_stuff *from;
{
  ...
  if (from->ksize)
    to->ktypevec = (char **) xmalloc (from->ksize * sizeof (to->ktypevec[0]));
  ...
  if (from->bsize)
    to->btypevec = (char **) xmalloc (from->bsize * sizeof (to->btypevec[0]));
  ...
  if (scan == NULL)
	   ...
	   && (scan[2] != 't'))
    {
	  ...
	  (ARM_DEMANGLING || LUCID_DEMANGLING || HP_DEMANGLING
	   || EDG_DEMANGLING) || (arm_special (mangled, declp) == 0))
	{
	  while (*scan == '_')
	    {
	      scan++;
     ...
     const char *scan;
{
  ...
  *(declp->p) = '\0';
  ...
  if (declp->p - declp->b >= 3
      && declp->b[0] == 'o'
      && declp->b[1] == 'p' && strchr (cplus_markers, declp->b[2]) != NULL)
    {
      /* see if it's an assignment expression */
