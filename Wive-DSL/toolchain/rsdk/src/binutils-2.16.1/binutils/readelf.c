...
static void *
get_data (void *var, FILE * file, long offset, size_t size,
	  const char *reason)
{
  void *mvar;
...
byte_get_little_endian (unsigned char *field, int size)
{
  switch (size)
	...
	| (((bfd_vma) (field[4])) << 32)
	| (((bfd_vma) (field[5])) << 40)
	| (((bfd_vma) (field[6])) << 48) | (((bfd_vma) (field[7])) << 56);
#endif
    default:
...
byte_get_big_endian (unsigned char *field, int size)
{
  switch (size)
	...
	| (((bfd_vma) (field[3])) << 32)
	| (((bfd_vma) (field[2])) << 40)
	| (((bfd_vma) (field[1])) << 48) | (((bfd_vma) (field[0])) << 56);
#endif
...
dump_relocations (FILE * file,
		  ...
		  unsigned long nsyms,
		  char *strtab, unsigned long strtablen, int is_rela)
{
  unsigned int i;
  Elf_Internal_Rela *rels;
  ...
  for (i = 0; i < rel_size; i++)
      ...
      if (is_32bit_elf)
	{
	  ...
	  printf ("%8.8lx  %8.8lx ", _bfd_int64_low (offset),
		  _bfd_int64_low (info));
#else
      ...
      else
	{
	  ...
	  printf (do_wide
		  ...
		  _bfd_int64_high (info), _bfd_int64_low (info));
#else
	  printf (do_wide
		  ? "%16.16lx  %16.16lx "
		  : "%12.12lx  %12.12lx ", offset, info);
#endif
	}
      ...
      if (symtab_index)
	{
	  if (symtab == NULL || symtab_index >= nsyms)
		...
		printf (_("<corrupt string table index: %3ld>"),
			psym->st_name);
	      else
  ...
  for (i = 0, segment = program_headers;
	...
	case PT_DYNAMIC:
	  ...
	  if (section_headers != NULL)
		...
		warn (_
		      ("the .dynamic section is not contained within the dynamic segment"));
...
offset_from_vma (FILE * file, bfd_vma vma, bfd_size_type size)
{
  ...
  for (seg = program_headers;
       seg < program_headers + elf_header.e_phnum; ++seg)
    {
...
process_section_headers (FILE * file)
{
  ...
  for (i = 0, section = section_headers;
       i < elf_header.e_shnum; i++, section++)
    {
      ...
      if (section->sh_type == SHT_GROUP)
	{
	  ...
	  if (ELF_ST_TYPE (sym->st_info) == STT_SECTION)
	      ...
	      if (strtab_sec != sec)
		{
		  ...
		  strtab = get_data (NULL, file, strtab_sec->sh_offset,
				     strtab_sec->sh_size, _("string table"));
		}
	  ...
	  for (j = 0; j < size; j++)
		  ...
		  != NULL)
		{
	      ...
	      section_headers_groups[SECTION_HEADER_INDEX (entry)] = group;
	  ...
	  if (rel_size)
		...
		printf (_("'%s'"), SECTION_NAME (section));
	      ...
	      printf (_(" at offset 0x%lx contains %lu entries:\n"),
		      rel_offset,
		      (unsigned long) (rel_size / section->sh_entsize));
...
find_symbol_for_address (Elf_Internal_Sym * symtab,
			 ...
			 struct absaddr addr,
			 const char **symname, bfd_vma * offset)
{
			 ...
			 Elf_Internal_Shdr * sec)
{
  ...
  for (relsec = section_headers;
       relsec < section_headers + elf_header.e_shnum; ++relsec)
    {
...
ia64_process_unwind (FILE * file)
{
  ...
  while (unwcount-- > 0)
	...
	if (strneq (SECTION_NAME (unwsec), ELF_STRING_ia64_unwind_once, len))
	{
	  ...
	  for (i = 0, sec = section_headers; i < elf_header.e_shnum;
	       ++i, ++sec)
	    if (strneq
		(SECTION_NAME (sec), ELF_STRING_ia64_unwind_info_once, len2)
...
/* SGI's ELF has more than one section in the DYNAMIC segment, and we
  ...
  for (ext = edyn, dynamic_nent = 0;
       (char *) ext < (char *) edyn + dynamic_size; ext++)
    {
      dynamic_nent++;
  ...
  for (ext = edyn, entry = dynamic_section;
       entry < dynamic_section + dynamic_nent; ext++, entry++)
    {
  ...
  if (dynamic_symbols == NULL)
    {
      for (entry = dynamic_section;
	   entry < dynamic_section + dynamic_nent; ++entry)
	{
	  Elf_Internal_Shdr section;
  ...
  if (do_syms
      ...
      for (hn = 0; hn < nbuckets; hn++)
	{
	  ...
	  for (si = buckets[hn]; si < nchains && si > 0; si = chains[si])
		...
		fputs ("     " + n, stdout);
	      ...
	      printf (" %3s",
		      get_symbol_visibility (ELF_ST_VISIBILITY
					     (psym->st_other)));
	   ...
	   i < elf_header.e_shnum; i++, section++)
	{
	  ...
	  for (si = 0, psym = symtab;
	       si < section->sh_size / section->sh_entsize; si++, psym++)
	    {
	      printf ("%6d: ", si);
	      print_vma (psym->st_value, LONG_HEX);
	      putchar (' ');
  ...
  for (i = 0; i < dynamic_syminfo_nent; ++i)
	...
	default:
	  if (dynamic_syminfo[i].si_boundto > 0
	      ...
	      &&
	      VALID_DYNAMIC_NAME (dynamic_section
				  [dynamic_syminfo[i].si_boundto].d_un.d_val))
	    {
	      print_symbol (10,
			    GET_DYNAMIC_NAME (dynamic_section
					      [dynamic_syminfo[i].si_boundto].
					      d_un.d_val));
	      putchar (' ');
	    }
	  else
...
get_FORM_name (unsigned long form)
{
  switch (form)
	...
	snprintf (buffer, sizeof (buffer), _("Unknown FORM value: %lx"),
		  form);
	return buffer;
      }
    }
			    ...
			    unsigned long length, unsigned long cu_offset)
{
  ...
  while (data < end)
	...
	case DW_OP_const4s:
	  ...
	  break;
	case DW_OP_const8u:
	  printf ("DW_OP_const8u: %lu %lu",
		  (unsigned long) byte_get (data, 4),
	...
	case DW_OP_call2:
	  ...
	  printf ("DW_OP_call2: <%lx>",
		  (long) byte_get (data, 2) + cu_offset);
	  data += 2;
	  break;
	case DW_OP_call4:
	  /* XXX: Strictly speaking for 64-bit DWARF3 files
	     this ought to be an 8-byte wide computation.  */
	  printf ("DW_OP_call4: <%lx>",
		  (long) byte_get (data, 4) + cu_offset);
	  data += 4;
	  break;
	case DW_OP_call_ref:
			     ...
			     debug_info * debug_info_p, int do_loc)
{
  ...
  switch (attribute)
	...
	case DW_ATE_imaginary_float:
	  ...
	  break;
	default:
	  if (uvalue >= DW_ATE_lo_user && uvalue <= DW_ATE_hi_user)
		       ...
		       debug_info * debug_info_p, int do_loc)
{
  ...
  data = read_and_display_attr_value (attribute, form, data, cu_offset,
				      ...
				      dwarf_version, debug_info_p, do_loc);
  if (!do_loc)
    printf ("\n");
  return data;
}
  ...
  for (section_begin = start, unit = 0; start < end; unit++)
      ...
      while (tags < start)
	{
	  ...
	  for (entry = first_abbrev;
	       entry && entry->entry != abbrev_number; entry = entry->next)
	    continue;
	  ...
	  if (!do_loc)
	    printf (_(" <%d><%lx>: Abbrev Number: %lu (%s)\n"),
		    ...
		    (unsigned long) (tags - section_begin
				     - bytes_read),
		    abbrev_number, get_TAG_name (entry->tag));
					  ...
					  unsigned long *offset_return)
{
  ...
  if (num_debug_info_entries == 0)
	...
	{
	  warn (_
		("%s section has more comp units than .debug_info section\n"),
		section_name);
	  warn (_
	  ...
	  if (op_code >= info.li_opcode_base)
	    {
	      ...
	      printf (_
		      ("  Special opcode %d: advance Address by %d to 0x%lx"),
		...
		data +=
		  process_extended_line_op (data, info.li_default_is_stmt,
					    pointer_size);
		break;
		...
		adv =
		  info.li_min_insn_length * read_leb128 (data, &bytes_read,
							 0);
		data += bytes_read;
		state_machine_regs.address += adv;
...
display_debug_pubnames (Elf_Internal_Shdr * section,
			unsigned char *start, FILE * file ATTRIBUTE_UNUSED)
{
		      ...
		      unsigned char *start, FILE * file ATTRIBUTE_UNUSED)
{
  ...
  for (i = 0; i < num_debug_info_entries; i++)
      ...
      if (use_debug_info && num != 0)
	{
	  ...
	  for (; j < num; j++)
	    {
	      if (last_offset > debug_information[i].range_lists[j])
		{
		  use_debug_info = 0;
		  break;
		}
  ...
  while (start < end)
      ...
      if (is_eh ? (cie_id == 0) : (cie_id == DW_CIE_ID))
	{
	  ...
	  if (do_debug_frames_interp)
	    printf ("\n%08lx %08lx %08lx CIE \"%s\" cf=%d df=%d ra=%d\n",
		    (unsigned long) (saved_start - section_start), length,
		    cie_id, fc->augmentation, fc->code_factor,
		    fc->data_factor, fc->ra);
	  else
	    {
	      printf ("\n%08lx %08lx %08lx CIE\n",
		      (unsigned long) (saved_start - section_start), length,
      ...
      else
	{
	  ...
	  printf ("\n%08lx %08lx %08lx FDE cie=%08lx pc=%08lx..%08lx\n",
		  (unsigned long) (saved_start - section_start), length,
		  cie_id, (unsigned long) (cie->chunk_start - section_start),
      ...
      while (start < block_end)
	{
	  ...
	  switch (op)
	    ...
	    case DW_CFA_GNU_negative_offset_extended:
	      ...
	      if (!do_debug_frames_interp)
		printf
		  ("  DW_CFA_GNU_negative_offset_extended: r%ld at cfa%+ld\n",
		   reg, l * fc->data_factor);
	      fc->col_type[reg] = DW_CFA_offset;
	      fc->col_offset[reg] = l * fc->data_factor;
	      break;
  ...
  if (liblist_offset != 0 && liblistno != 0 && do_dynamic)
    {
      ...
      elib = get_data (NULL, file, liblist_offset,
		       liblistno * sizeof (Elf32_External_Lib), _("liblist"));
      if (elib)
	{
	  ...
	  for (cnt = 0; cnt < liblistno; ++cnt)
	      ...
	      else
		{
		  ...
		  for (fcnt = 0;
		       fcnt <
		       sizeof (l_flags_vals) / sizeof (l_flags_vals[0]);
		       ++fcnt)
      ...
      if (eopt)
	{
	  ...
	  while (cnt-- > 0)
		...
		case ODK_NULL:
		  ...
		  printf (" NULL       %d %lx", option->section,
			  option->info);
		  break;
		case ODK_REGINFO:
		  printf (" REGINFO    ");
		  if (elf_header.e_machine == EM_MIPS)
		  ...
		  else
		    {
		      ...
		      printf ("GPR %08lx  GP 0x", reginfo.ri_gprmask);
		...
		case ODK_EXCEPTIONS:
		  ...
		  process_mips_fpe_exception ((option->
					       info & OEX_FPU_MAX) >> 8);
		  fputs (")", stdout);
...
get_netbsd_elfcore_note_type (unsigned e_type)
{
  ...
  if (e_type < NT_NETBSDCORE_FIRSTMACH)
    {
      snprintf (buff, sizeof (buff), _("Unknown note type: (0x%08x)"),
		e_type);
      return buff;
    }
...
process_note (Elf_Internal_Note * pnote)
{
  ...
  printf ("  %s\t\t0x%08lx\t%s\n",
	  pnote->namesz ? pnote->namedata : "(NONE)", pnote->descsz, nt);
  return 1;
}
  ...
  while (external < (Elf_External_Note *) ((char *) pnotes + length))
    {
      ...
      next =
	(Elf_External_Note *) (inote.descdata +
			       align_power (inote.descsz, 2));
