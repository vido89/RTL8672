  ...
  for (; from < fromend; from += size)
    {
      ...
      sym =
	bfd_minisymbol_to_symbol (abfd, dynamic, (const void *) from, store);
      if (sym == NULL)
	bfd_fatal (bfd_get_filename (abfd));
	...
	keep = 1;
      ...
      if (keep && !print_debug_syms && (sym->flags & BSF_DEBUGGING) != 0)
	keep = 0;
		      ...
		      struct size_sym **symsizesp)
{
  ...
  *symsizesp = symsizes;
  ...
  for (; from < fromend; from += size)
	...
	{
	  if (from + size < fromend && sec == bfd_get_section (next))
	    sz = valueof (next) - valueof (sym);
	  else
	    sz = (bfd_get_section_vma (abfd, sec)
		  + bfd_section_size (abfd, sec) - valueof (sym));
	}
  ...
  qsort ((void *) *symsizesp, symcount, sizeof (struct size_sym),
	 size_forward2);
...
print_symbol_info_sysv (struct extended_symbol_info *info, bfd * abfd)
{
  ...
  if (SYM_TYPE (info) == '-')
  ...
  else
    {
      ...
      if (info->elfinfo)
	printf ("%18s|",
		get_symbol_type (ELF_ST_TYPE
				 (info->elfinfo->internal_elf_sym.st_info)));
      else
	printf ("                  |");
