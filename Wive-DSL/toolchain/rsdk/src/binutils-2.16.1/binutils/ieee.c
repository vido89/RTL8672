...
bfd_boolean
parse_ieee (void *dhandle, bfd * abfd, const bfd_byte * bytes,
	    bfd_size_type len)
{
...
parse_ieee_ty (struct ieee_info *info, const bfd_byte ** pp)
{
	...
	if (!present)
	  {
	    ...
	    type = debug_make_int_type (dhandle, 4, signedp ? FALSE : TRUE);
	  }
	else
	  {
	    *pp = hold;
  ...
  if ((tag || typdef) && info->vars.vars[varindx].namlen > 0)
    {
      const char *name;
      ...
      if (pvar != NULL)
	{
      ...
      return debug_record_variable (dhandle, namcopy, type,
				    (blocktype == 4 || blocktype == 6
				     ? DEBUG_LOCAL_STATIC : DEBUG_STATIC), v);
	...
	case 'd':
	  {
		...
		if (debug_get_type_kind (dhandle, ftype) ==
		    DEBUG_KIND_POINTER)
		  {
			...
			for (pfcnt = structfields;
			     *pfcnt != DEBUG_FIELD_NULL; pfcnt++)
			  ++fcnt;
			it->fslots = ((debug_type *)
				      xmalloc (fcnt * sizeof *it->fslots));
			memset (it->fslots, 0, fcnt * sizeof *it->fslots);
		      }
  ...
  if (count > 0)
    {
      ...
      dhandle = info->dhandle;
      arg_slots =
	info->types.types[info->blockstack.bsp[-1].fnindx].arg_slots;
      while (count-- > 0)
	{
	  bfd_vma indx;
	  debug_type target;
      ...
      for (pass = 0; pass < 2; pass++)
	{
	  ...
	  for (i = (int) vars->alloc - 1; i >= 0; i--)
		...
		case 1:
		  ...
		  if (pv->kind == IEEE_STATIC || pv->kind == IEEE_FUNCTION)
		    found = TRUE;
		  break;
...
ieee_append_buffer (struct ieee_handle *info ATTRIBUTE_UNUSED,
		    struct ieee_buflist *mainbuf, struct ieee_buflist *newbuf)
{
  if (newbuf->head != NULL)
    {
...
{
  return (ieee_write_byte (info, i >> 8) && ieee_write_byte (info, i & 0xff));
}
...
ieee_write_asn (struct ieee_handle *info, unsigned int indx, bfd_vma val)
{
  return (ieee_write_2bytes (info, (int) ieee_asn_record_enum)
	  && ieee_write_number (info, indx) && ieee_write_number (info, val));
}
...
ieee_write_atn65 (struct ieee_handle *info, unsigned int indx, const char *s)
{
  return (ieee_write_2bytes (info, (int) ieee_atn_record_enum)
	  ...
	  && ieee_write_number (info, 65) && ieee_write_id (info, s));
}
  ...
  if (buflist != NULL)
      ...
      else
	{
	  if (!ieee_change_buffer (info, &info->global_types)
	      || !ieee_write_byte (info, (int) ieee_bb_record_enum)
	      || !ieee_write_byte (info, 2)
	      || !ieee_write_number (info, 0) || !ieee_write_id (info, ""))
	    return FALSE;
	}
    }
...
write_ieee_debugging_info (bfd * abfd, void *dhandle)
{
  ...
  ieee_name_type_hash_traverse (&info.tags,
				ieee_write_undefined_tag, (void *) &info);
  if (info.error)
    return FALSE;
...
ieee_write_undefined_tag (struct ieee_name_type_hash_entry *h, void *p)
{
  ...
  for (nt = h->types; nt != NULL; nt = nt->next)
	...
	case DEBUG_KIND_ENUM:
      ...
      if (!ieee_write_number (info, code) || !ieee_write_number (info, 0))
	{
	  info->error = TRUE;
	  return FALSE;
	}
...
ieee_start_compilation_unit (void *p, const char *filename)
{
  ...
  if (!ieee_change_buffer (info, &info->types)
      || !ieee_write_byte (info, (int) ieee_bb_record_enum)
      || !ieee_write_byte (info, 1)
      || !ieee_write_number (info, 0) || !ieee_write_id (info, info->modname))
    return FALSE;
...
ieee_finish_compilation_unit (struct ieee_handle *info)
{
  ...
  if (!ieee_write_byte (info, (int) ieee_bb_record_enum)
      ...
      || !ieee_write_id (info, info->modname)
      || !ieee_write_id (info, "")
      || !ieee_write_number (info, 0) || !ieee_write_id (info, "GNU objcopy"))
    return FALSE;
...
ieee_complex_type (void *p, unsigned int size)
{
  ...
  if (!ieee_define_type (info, size * 2, FALSE, FALSE)
      || !ieee_write_number (info, code) || !ieee_write_id (info, ""))
    return FALSE;
		...
		bfd_signed_vma * vals)
{
  ...
  for (e = info->enums; e != NULL; e = e->next)
	...
	{
	  if (e->tag == NULL
	      || tag[0] != e->tag[0] || strcmp (tag, e->tag) != 0)
	    continue;
	}
      ...
      if (names != NULL && e->names != NULL)
	{
      ...
      if ((names == NULL && e->names == NULL)
	  || (names != NULL
	      && e->names != NULL && names[i] == NULL && e->names[i] == NULL))
	{
...
ieee_pointer_type (void *p)
{
  ...
  if (!ieee_define_type (info, 4, TRUE, localp)
      || !ieee_write_number (info, 'P') || !ieee_write_number (info, indx))
    return FALSE;
...
ieee_const_type (void *p)
{
  ...
  if (!ieee_define_type (info, size, unsignedp, localp)
      ...
      || !ieee_write_number (info, 1) || !ieee_write_number (info, indx))
    return FALSE;
...
ieee_volatile_type (void *p)
{
  ...
  if (!ieee_define_type (info, size, unsignedp, localp)
      || !ieee_write_number (info, 'n')
      || !ieee_write_number (info, 2) || !ieee_write_number (info, indx))
    return FALSE;
...
ieee_typdef (void *p, const char *name)
{
  ...
  if (!ieee_define_named_type (info, name, (unsigned int) -1, type.size,
			       ...
			       (struct ieee_buflist *) NULL)
      || !ieee_write_number (info, 'T') || !ieee_write_number (info, indx))
    return FALSE;
...
static bfd_boolean
ieee_int_constant (void *p ATTRIBUTE_UNUSED,
	       ...
	       bfd_vma val)
{
  ...
  switch (kind)
	...
	return FALSE;
      ...
      break;
    case DEBUG_LOCAL:
      if (!ieee_write_number (info, 1) || !ieee_write_number (info, val))
	return FALSE;
      refflag = 2;
      asn = FALSE;
      break;
...
ieee_start_function (void *p, const char *name, bfd_boolean global)
{
  ...
  if (!ieee_define_named_type (info, name, (unsigned int) -1, 0, FALSE, TRUE,
      ...
      || !ieee_write_number (info, 0)
      || !ieee_write_number (info, 0) || !ieee_write_number (info, retindx))
    return FALSE;
...
ieee_start_block (void *p, bfd_vma addr)
{
  ...
  if (info->block_depth == 1)
  ...
  else
    {
	  ...
	  || !ieee_write_id (info, "")
	  || !ieee_write_number (info, 0)
	  || !ieee_write_number (info, 0) || !ieee_write_number (info, addr))
	return FALSE;
    }
...
static bfd_boolean
ieee_lineno (void *p, const char *filename, unsigned long lineno,
	     bfd_vma addr)
{
  struct ieee_handle *info = (struct ieee_handle *) p;
