...
read_coff_data_entry (const bfd_byte * data,
		 ...
		 const struct res_directory *resources)
{
  ...
  length = (cwi.dirsize
	    + cwi.dirstrsize + cwi.dataentsize + cwi.resources.length);
...
coff_res_to_bin (const struct res_resource *res, struct coff_write_info *cwi)
{
  ...
  putcwi_32 (cwi,
	     (cwi->dirsize
	      + cwi->dirstrsize
	      + cwi->dataentsize + cwi->resources.length), erd->rva);
