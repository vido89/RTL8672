				...
				enum debug_object_linkage linkage)
{
  ...
  if (info->current_unit == NULL || info->current_file == NULL)
    {
...
debug_start_source (void *handle, const char *name)
{
  ...
  for (f = info->current_unit->files; f != NULL; f = f->next)
    {
	  ...
	  && f->filename[1] == name[1] && strcmp (f->filename, name) == 0)
	{
	  info->current_file = f;
	  return TRUE;
	}
  ...
  for (pf = &info->current_file->next; *pf != NULL; pf = &(*pf)->next)
    ;
  *pf = f;
			...
			enum debug_parm_kind kind, bfd_vma val)
{
  ...
  if (info->current_unit == NULL || info->current_function == NULL)
    {
...
debug_end_function (void *handle, bfd_vma addr)
{
  ...
  if (info->current_unit == NULL
      || info->current_block == NULL || info->current_function == NULL)
    {
...
debug_start_block (void *handle, bfd_vma addr)
{
  ...
  /* We must always have a current block: debug_record_function sets
     one up.  */
  if (info->current_unit == NULL || info->current_block == NULL)
    {
  ...
  for (pb = &info->current_block->children; *pb != NULL; pb = &(*pb)->next)
    ;
  *pb = b;
		       ...
		       enum debug_var_kind kind, bfd_vma val)
{
  ...
  n =
    debug_add_to_namespace (info, nsp, name, DEBUG_OBJECT_VARIABLE, linkage);
  if (n == NULL)
    return FALSE;
			...
			debug_field * fields)
{
  ...
  t = debug_make_type (info,
		       structp ? DEBUG_KIND_STRUCT : DEBUG_KIND_UNION, size);
  if (t == NULL)
    return DEBUG_TYPE_NULL;
	  ...
	  for (n = b->locals->list; n != NULL; n = n->next)
	    {
		  ...
		  && n->name[0] == name[0] && strcmp (n->name, name) == 0)
		return n->u.type;
	    }
	}
    }
...
debug_get_type_name (void *handle, debug_type type)
{
  ...
  if (type->kind == DEBUG_KIND_NAMED || type->kind == DEBUG_KIND_TAGGED)
		  ...
		  struct debug_type *type, struct debug_name *name)
{
  ...
  if (name != NULL
      && type->kind != DEBUG_KIND_NAMED && type->kind != DEBUG_KIND_TAGGED)
    {
		    ...
		    struct debug_type *type)
{
  ...
  for (l = info->id_list; l != NULL; l = l->next)
	...
	{
	  if (l->tag == NULL
	      || l->tag[0] != tag[0] || strcmp (l->tag, tag) != 0)
	    continue;
	}
		  ...
		  struct debug_type *t2)
{
  ...
  /* As a special case, permit a typedef to match a tag, since C++
     ...
     output will not.  */
  if (t1->kind == DEBUG_KIND_NAMED && t2->kind == DEBUG_KIND_TAGGED)
    return debug_type_samep (info, t1->u.knamed->type, t2);
  else if (t1->kind == DEBUG_KIND_TAGGED && t2->kind == DEBUG_KIND_NAMED)
  ...
  switch (t1->kind)
      ...
      else
	{
	  ...
	  while (*pn1 != NULL && *pn2 != NULL)
	    {
	      if (**pn1 != **pn2 || *pv1 != *pv2 || strcmp (*pn1, *pn2) != 0)
		break;
	      ++pn1;
	      ++pn2;
	      ++pv1;
