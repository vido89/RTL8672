...
do_sections_p1 (struct coff_ofile *head)
{
  ...
  for (idx = 0, section = abfd->sections; section;
       section = section->next, idx++)
    {
      long relsize;
...
{
  struct coff_line *res =
    (struct coff_line *) xcalloc (sizeof (struct coff_line), 1);
  asection *s;
  unsigned int l;
  ...
  for (s = abfd->sections; s; s = s->next)
      ...
      for (l = 0; l < s->lineno_count; l++)
	{
	  if (s->lineno[l].line_number == 0)
	    {
	      if (rawsyms + i ==
		  ((coff_symbol_type *) (&(s->lineno[l].u.sym[0])))->native)
		{
		  ...
		  int start_line =
		    rawsyms[i + 3].u.auxent.x_sym.x_misc.x_lnsz.x_lnno;
		  ...
		  for (c = 0; s->lineno[l + c + 1].line_number; c++)
		    {
		      res->lines[c + 1] =
			s->lineno[l + c].line_number + start_line - 1;
...
do_define (int i, struct coff_scope *b)
{
  ...
  if (s->where->where == coff_where_memory
	...
	cur_sfile->section + s->where->section->number;
...
doit (void)
{
  ...
  for (i = 0; i < rawcount;)
	...
	case C_FILE:
	  {
	    ...
	    n->section =
	      (struct coff_isection *) xcalloc (sizeof (struct coff_isection),
	...
	case C_ARG:
	  i =
	    do_define (i, last_function_symbol->type->u.function.parameters);
	  break;
	case C_MOS:
	case C_MOU:
	case C_FIELD:
...
coff_grok (bfd * inabfd)
{
  ...
  tindex =
    (struct coff_symbol
     **) (xcalloc (sizeof (struct coff_symbol *), rawcount));
