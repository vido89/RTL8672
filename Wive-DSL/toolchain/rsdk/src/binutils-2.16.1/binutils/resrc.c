...
static enum
{ ISTREAM_PIPE, ISTREAM_FILE } istream_type;
  ...
  if (preprocessor)
    {
      cmd = xmalloc (strlen (preprocessor)
		     + strlen (preprocargs) + strlen (filename) + 10);
      ...
      if (dash)
	{
	  ...
	  cpp_pipe =
	    look_for_default (cmd, program_name, dash - program_name + 1,
      ...
      if (slash && !cpp_pipe)
	{
	  ...
	  cpp_pipe =
	    look_for_default (cmd, program_name, slash - program_name + 1,
			      preprocargs, filename);
	}
	     ...
	     struct menuitem *menuitems)
{
  ...
  r =
    define_standard_resource (&resources, RT_MENU, id, resinfo->language, 0);
...
write_rc_file (const char *filename, const struct res_directory *resources)
{
  ...
  if (filename == NULL)
  ...
  else
    {
      ...
      if (e == NULL)
	fatal (_("can't open `%s' for output: %s"), filename,
	       strerror (errno));
    }
		   ...
		   int *language)
{
  ...
  if (rt != 0
      && type != NULL && (type->named || type->u.id != (unsigned long) rt))
    {
  ...
  if ((res->res_info.language != 0 && res->res_info.language != *language)
	...
	fprintf (e, "%sCHARACTERISTICS %lu\n",
		 modifiers ? "// " : "", res->res_info.characteristics);
      if (res->res_info.version != 0)
	fprintf (e, "%sVERSION %lu\n",
		 modifiers ? "// " : "", res->res_info.version);
    }
  ...
  for (acc = accelerators; acc != NULL; acc = acc->next)
    {
	  ...
	  && ISPRINT (acc->key) && (acc->flags & ACC_VIRTKEY) == 0)
	{
	  fprintf (e, "\"%c\"", acc->key);
	  printable = 1;
	}
  ...
  if (dialog->font != NULL)
    {
      ...
      if (dialog->ex != NULL
	  && (dialog->ex->weight != 0
	      || dialog->ex->italic != 0 || dialog->ex->charset != 1))
	fprintf (e, ", %d, %d, %d",
		    ...
		    int ind)
{
  ...
  for (mi = menuitems; mi != NULL; mi = mi->next)
	...
	fprintf (e, "POPUP");
      ...
      if (!menuex
	  && mi->popup == NULL
	  && mi->text == NULL && mi->type == 0 && mi->id == 0)
	{
	  fprintf (e, " SEPARATOR\n");
	  continue;
	}
