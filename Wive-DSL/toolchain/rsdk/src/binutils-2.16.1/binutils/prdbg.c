...
static bfd_boolean tg_start_class_type
  (void *, const char *, unsigned int, bfd_boolean, unsigned int, bfd_boolean,
   bfd_boolean);
static bfd_boolean tg_class_static_member (void *, const char *, const char *,
					   enum debug_visibility);
static bfd_boolean tg_class_baseclass (void *, bfd_vma, bfd_boolean,
				       enum debug_visibility);
static bfd_boolean tg_class_method_variant (void *, const char *,
					    enum debug_visibility,
					    bfd_boolean, bfd_boolean, bfd_vma,
					    bfd_boolean);
static bfd_boolean tg_class_static_method_variant (void *, const char *,
						   enum debug_visibility,
						   bfd_boolean, bfd_boolean);
...
substitute_type (struct pr_handle *info, const char *s)
{
  ...
  if (strchr (s, '|') != NULL
	  ...
	  || strchr (info->stack->type, '(') != NULL))
    {
      if (!prepend_type (info, "(") || !append_type (info, ")"))
	return FALSE;
    }
	      ...
	      bfd_signed_vma * values)
{
  ...
  if (tag != NULL)
    {
      if (!append_type (info, tag) || !append_type (info, " "))
	return FALSE;
    }
  if (!append_type (info, "{ "))
    return FALSE;
	  ...
	  if (values[i] != val)
	    {
	      ...
	      if (!append_type (info, " = ") || !append_type (info, ab))
		return FALSE;
	      val = values[i];
	    }
...
pr_range_type (void *p, bfd_signed_vma lower, bfd_signed_vma upper)
{
  ...
  return (prepend_type (info, "range (")
	  ...
	  && append_type (info, ":") && append_type (info, abu));
}
	       ...
	       bfd_boolean stringp)
{
  ...
  if (strcmp (range_type, "int") != 0)
    {
      if (!append_type (info, ":") || !append_type (info, range_type))
	return FALSE;
    }
...
pr_set_type (void *p, bfd_boolean bitstringp)
{
  ...
  if (!prepend_type (info, "set { ") || !append_type (info, " }"))
    return FALSE;
...
pr_offset_type (void *p)
{
  ...
  return (substitute_type (info, "")
	  && prepend_type (info, " ")
	  && prepend_type (info, t) && append_type (info, "::|"));
}

/* Make a method type.  */

static bfd_boolean
pr_method_type (void *p, bfd_boolean domain, int argcount,
		bfd_boolean varargs)
{
  struct pr_handle *info = (struct pr_handle *) p;
  unsigned int len;
  ...
  if (bitsize != 0)
    {
	  ...
	  || !append_type (info, ab) || !append_type (info, ", "))
	return FALSE;
    }
		     ...
		     bfd_boolean vptr, bfd_boolean ownvptr)
{
  ...
  if (size != 0 || vptr || ownvptr || tag != NULL)
      ...
      if (size != 0)
	{
	  ...
	  sprintf (ab, "%u", size);
	  if (!append_type (info, " size ") || !append_type (info, ab))
	    return FALSE;
	}
      ...
      if (vptr)
	{
	  ...
	  if (ownvptr)
	    ...
	    {
	      if (!append_type (info, tv) || !append_type (info, " "))
		return FALSE;
	    }
	}
			 ...
			 bfd_vma voffset, bfd_boolean context)
{
  ...
  if (!append_type (info, method_type)
      || !append_type (info, " /* ")
      || !append_type (info, physname) || !append_type (info, " "))
    return FALSE;
  if (context || voffset != 0)
      ...
      if (context)
	{
      ...
      if (!append_type (info, "voffset ") || !append_type (info, ab))
	return FALSE;
    }
...
static bfd_boolean
pr_variable (void *p, const char *name, enum debug_var_kind kind, bfd_vma val)
{
  struct pr_handle *info = (struct pr_handle *) p;
  char *t;
  char ab[20];
		       ...
		       enum debug_parm_kind kind, bfd_vma val)
{
  ...
  if (kind == DEBUG_PARM_REFERENCE || kind == DEBUG_PARM_REF_REG)
    {
