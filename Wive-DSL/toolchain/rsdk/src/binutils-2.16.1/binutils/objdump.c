...
/* 2006-01-12 tonywu: fix objdump signed address overflow */
#define bfd_add_overflow(a,b) (((a) & (b) & ~((a)+(b))) | (~(a) & ~(b) & ((a)+(b))))
#undef bfd_asymbol_value
#define bfd_asymbol_value(x) (bfd_add_overflow(bfd_asymbol_base(x), (x)->value) ? \
		        (unsigned long) (bfd_asymbol_base(x) + (x)->value) : \
				        (bfd_asymbol_base(x) + (x)->value))
...
static int print_hex_int = 0;	/* --add-hex-prefix */
...
static struct option long_options[] = {
  ...
  {"add-hex-prefix", no_argument, &print_hex_int, 1},
...
find_symbol_for_address (bfd_vma vma,
			 struct disassemble_info *info, long *place)
{
...
objdump_print_addr (bfd_vma vma,
		    struct disassemble_info *info, bfd_boolean skip_zeroes)
{
  struct objdump_disasm_info *aux;
  asymbol *sym;
#ifdef DISASSEMBLER_NEEDS_RELOCS
...
#ifdef DISASSEMBLER_NEEDS_RELOCS
  if (aux->reloc != NULL
      && aux->reloc->sym_ptr_ptr != NULL && *aux->reloc->sym_ptr_ptr != NULL)
    {
...
static void
skip_to_line (struct print_file_list *p, unsigned int line, bfd_boolean show)
{
  while (p->line < line)
    {
      char buf[100];
...
show_line (bfd * abfd, asection * section, bfd_vma addr_offset)
{
  ...
  if (with_source_code && filename != NULL && line > 0)
    {
  ...
  if (!prefix_addresses)
    {
      ...
      bfd_sprintf_vma
	(aux->abfd, buf,
	 (section->vma + bfd_section_size (section->owner, section) / opb));
      s = buf;
      ...
      else
	{
	  ...
	  if (insns)
	      ...
	      if (*relppp < relppend)
		{
		  ...
		  if (distance_to_rel == 0
		      ...
		      || (distance_to_rel > 0
			  && distance_to_rel <
			  (bfd_signed_vma) (previous_octets / opb)))
		    {
	  ...
	  if (prefix_addresses ? show_raw_insn > 0 : show_raw_insn >= 0)
		...
		bpc = 1;
	      ...
	      for (j = addr_offset * opb; j < addr_offset * opb + pb;
		   j += bpc)
		{
		  int k;
...
disassemble_section (bfd * abfd, asection * section, void *info)
{
	  ...
	  if (relsize > 0)
	    {
	      ...
	      rel_count =
		bfd_canonicalize_reloc (abfd, section, rel_pp, syms);
	      if (rel_count < 0)
		bfd_fatal (bfd_get_filename (abfd));
  ...
  if (start_address == (bfd_vma) - 1 || start_address < pinfo->buffer_vma)
    addr_offset = 0;
  else
  ...
  /* Skip over the relocs belonging to addresses below the
     start address.  */
  while (rel_pp < rel_ppend && (*rel_pp)->address < rel_offset + addr_offset)
    ++rel_pp;
  ...
  while (addr_offset < stop_offset)
      ...
      if (sym != NULL && bfd_asymbol_value (sym) <= addr)
	{
	  ...
	  for (x = place;
	       (x < sorted_symcount
		&& (bfd_asymbol_value (sorted_syms[x]) <= addr)); ++x)
	    continue;
...
static char *
read_section_stabs (bfd * abfd, const char *sect_name,
...
print_section_stabs (bfd * abfd,
		     const char *stabsect_name, unsigned *string_offset_ptr)
{
  int i;
...
dump_section (bfd * abfd, asection * section, void *dummy ATTRIBUTE_UNUSED)
{
  ...
  if (start_address == (bfd_vma) - 1 || start_address < section->vma)
    start_offset = 0;
  else
  ...
  for (p = relpp; relcount && *p != NULL; p++, relcount--)
    {
      ...
      if (start_address != (bfd_vma) - 1 && q->address < start_address)
	continue;
      if (stop_address != (bfd_vma) - 1 && q->address > stop_address)
	continue;
      ...
      else
	{
	  ...
	  bfd_printf_vma (abfd, q->address);
	  printf (" %-16s  [%s]", q->howto->name, section_name);
	}
...
dump_relocs_in_section (bfd * abfd,
			asection * section, void *dummy ATTRIBUTE_UNUSED)
{
  arelent **relpp;
  long relcount;
  long relsize;
