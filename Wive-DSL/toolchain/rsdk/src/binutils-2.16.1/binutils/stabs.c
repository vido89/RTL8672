...
static bfd_boolean parse_stab_struct_fields
  (void *, struct stab_handle *, const char **, debug_field **,
   bfd_boolean *);
static bfd_boolean parse_stab_cpp_abbrev (void *, struct stab_handle *,
					  const char **, debug_field *);
static bfd_boolean parse_stab_one_struct_field (void *, struct stab_handle *,
						const char **, const char *,
						debug_field *, bfd_boolean *);
static bfd_boolean parse_stab_members (void *, struct stab_handle *,
				       const char *, const char **,
				       const int *, debug_method **);
static debug_type parse_stab_argtypes (void *, struct stab_handle *,
				       debug_type, const char *, const char *,
				       debug_type, const char *, bfd_boolean,
				       bfd_boolean, const char **);
static bfd_boolean parse_stab_tilde_field (void *, struct stab_handle *,
					   const char **, const int *,
					   debug_type *, bfd_boolean *);
static debug_type parse_stab_array_type (void *, struct stab_handle *,
  ...
  switch (type)
      ...
      while (*p == ';')
	{
	  ...
	  if (parse_stab_type (dhandle, info, (const char *) NULL, &p,
			       (debug_type **) NULL) == DEBUG_TYPE_NULL)
	    return FALSE;
	}
...
parse_stab_type (void *dhandle, struct stab_handle *info,
		 const char *typename, const char **pp, debug_type ** slotp)
{
  const char *orig;
  int typenums[2];
  int size;
...
parse_stab_range_type (void *dhandle, struct stab_handle *info,
		       const char *typename, const char **pp,
		       const int *typenums)
{
  const char *orig;
  int rangenums[2];
...
#define ULLHIGH "01777777777777777777777;"
      if (index_type == DEBUG_TYPE_NULL)
	{
	  ...
	  if (!ov2
	      && n2 == 0 && strncmp (s3, ULLHIGH, sizeof ULLHIGH - 1) == 0)
...
parse_stab_sun_floating_type (void *dhandle, const char **pp)
{
  ...
  if (details == NF_COMPLEX
      || details == NF_COMPLEX16 || details == NF_COMPLEX32)
...
parse_stab_enum_type (void *dhandle, const char **pp)
{
  ...
  while (**pp != '\0' && **pp != ';' && **pp != ',')
	...
	{
	  alloc += 10;
	  names = ((const char **) xrealloc (names, alloc * sizeof *names));
			...
			bfd_boolean structp, const int *typenums)
{
  ...
  if (!statics
      && baseclasses == NULL
      && methods == NULL && vptrbase == DEBUG_TYPE_NULL && !ownvptr)
	  ...
	  if (cvars + 1 >= allocvars)
	    {
	      allocvars += 10;
	      variants = ((debug_method_variant *)
			  xrealloc (variants, allocvars * sizeof *variants));
	    }
		     ...
		     bfd_boolean volatilep, const char **pphysname)
{
    ...
    *pphysname = argtypes;
	...
	sprintf (buf, "__%s%s%d", const_prefix, volatile_prefix, len);
      ...
      mangled_name_len = ((is_constructor ? 0 : strlen (fieldname))
			  + strlen (buf) + len + strlen (argtypes) + 1);
  ...
  args =
    stab_demangle_argtypes (dhandle, info, *pphysname, &varargs,
			    physname_len);
  if (args == NULL)
    return DEBUG_TYPE_NULL;
...
static debug_type
stab_xcoff_builtin_type (void *dhandle, struct stab_handle *info, int typenum)
{
  debug_type rettype;
  const char *name;
		      ...
		      unsigned int physname_len)
{
  ...
  else if (scan == *pp && !ISDIGIT (scan[2]) && scan[2] != 't')
    {
			     ...
			     const char **pp, const char *scan)
{
  ...
  *pp = scan + 2;
  ...
  if (*pp - name >= 5
      && strncmp (name, "type", 4) == 0 && (name[4] == '$' || name[4] == '.'))
    {
      const char *tem;
