...
normalize (const char *file, bfd * abfd)
{
  ...
  if (ar_truncate
      && abfd != NULL && strlen (filename) > abfd->xvec->ar_max_namelen)
    {
      char *s;
...
main (int argc, char **argv)
{
  ...
  if (is_ranlib)
	...
	usage (0);
	  ...
	  || strcmp (argv[1], "-v") == 0 || strncmp (argv[1], "--v", 3) == 0)
	print_version ("ranlib");
      arg_index = 1;
  ...
  if (mri_mode)
	...
	operation = replace;
      ...
      if ((operation == none || operation == print_table) && write_armap == 1)
	{
	  ranlib_only (argv[arg_index]);
	  xexit (0);
	}
...
/* KLUDGE ALERT! Temporary fix until I figger why
   stat() is wrong ... think it's buried in GO32's IDT - Jax */
      ...
      if (file != NULL)
	{
      ...
      if (arch == NULL
	  || !bfd_set_format (arch, bfd_archive) || !bfd_close (arch))
	bfd_fatal (archive_filename);
  ...
  for (next_one = bfd_openr_next_archived_file (arch, NULL);
       next_one; next_one = bfd_openr_next_archived_file (arch, next_one))
    {
      PROGRESS (1);
      *last_one = next_one;
