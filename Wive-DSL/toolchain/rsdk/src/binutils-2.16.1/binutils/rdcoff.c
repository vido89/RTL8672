		      ...
		      union internal_auxent *pauxent, void *dhandle)
{
  ...
  if (ntype >= 0 && ntype <= T_MAX && types->basic[ntype] != DEBUG_TYPE_NULL)
  ...
  switch (ntype)
    ...
    case T_STRUCT:
      if (pauxent == NULL)
	ret = debug_make_struct_type (dhandle, TRUE, 0, (debug_field *) NULL);
      else
    ...
    case T_UNION:
      if (pauxent == NULL)
	ret =
	  debug_make_struct_type (dhandle, FALSE, 0, (debug_field *) NULL);
      else
	ret = parse_coff_struct_type (abfd, symbols, types, ntype, pauxent,
				      dhandle);
  ...
  if (set_basic && ntype >= 0 && ntype <= T_MAX)
			...
			union internal_auxent *pauxent, void *dhandle)
{
  ...
  return debug_make_struct_type (dhandle, ntype == T_STRUCT,
				 pauxent->x_sym.x_misc.x_lnsz.x_size, fields);
}
  ...
  switch (psyment->n_sclass)
    ...
    case C_STAT:
      if (!debug_record_variable (dhandle, bfd_asymbol_name (sym), type,
				  (within_function
				   ? DEBUG_LOCAL_STATIC
				   : DEBUG_STATIC), bfd_asymbol_value (sym)))
	return FALSE;
      break;
