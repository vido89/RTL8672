...
#ifdef NLMCONV_I386
static void i386_mangle_relocs (bfd *, asection *, arelent ***, long *,
				char *, bfd_size_type);
#endif

#ifdef NLMCONV_ALPHA
static void alpha_mangle_relocs (bfd *, asection *, arelent ***, long *,
				 char *, bfd_size_type);
#endif
...
main (int argc, char **argv)
{
  ...
  if (header_file != NULL)
    {
      if (!nlmlex_file (header_file) || yyparse () != 0 || parse_errors != 0)
	exit (1);
    }
  ...
  *nlm_extended_header (outbfd) = extended_hdr_struct;
  ...
  if (custom_file != NULL)
    {
      ...
      if (custom_data == NULL || fstat (fileno (custom_data), &st) < 0)
	{
  ...
  if (help_file != NULL)
    {
      ...
      if (help_data == NULL || fstat (fileno (help_data), &st) < 0)
	{
  ...
  if (message_file != NULL)
    {
      ...
      if (message_data == NULL || fstat (fileno (message_data), &st) < 0)
	{
  ...
  if (rpc_file != NULL)
    {
      ...
      if (rpc_data == NULL || fstat (fileno (rpc_data), &st) < 0)
	{
  ...
  if (sharelib_file != NULL)
    {
      ...
      if (sharedbfd == NULL || !bfd_check_format (sharedbfd, bfd_object))
	{
      ...
      else
	{
	  ...
	  if (shared_data == NULL || (fstat (fileno (shared_data), &st) < 0))
	    {
	      fprintf (stderr, "%s:%s: %s\n", program_name, sharelib_file,
		       strerror (errno));
	      sharelib_file = NULL;
  ...
  if (rpc_file != NULL)
      ...
      else
	{
	  if (!bfd_set_section_contents (outbfd, rpc_section, data,
					 (file_ptr) 0, rpc_size))
	    bfd_fatal (_("rpc section"));
	  nlm_extended_header (outbfd)->RPCDataOffset = rpc_section->filepos;
  ...
  if (sharelib_file != NULL)
	...
	(sharedhdr.relocationFixupOffset
	 - shared_offset + shared_section->filepos);
...
static void
setup_sections (bfd * inbfd ATTRIBUTE_UNUSED, asection * insec,
		void *data_ptr)
{
  ...
  if (!bfd_set_section_size (outbfd, outsec,
			     (bfd_section_size (outbfd, outsec)
			      + bfd_section_size (inbfd, insec) + add)))
...
static reloc_howto_type nlm_i386_pcrel_howto = HOWTO (1,	/* type */
						      0,	/* rightshift */
		    ...
		    bfd_size_type contents_size)
{
  ...
  for (i = 0; i < reloc_count; i++)
	...
	continue;
      ...
      if (sym == insec->symbol
	  && rel->howto != NULL
	  && rel->howto->pc_relative && !rel->howto->pcrel_offset)
	{
	  --*reloc_count_ptr;
	  --relocs;
...
static reloc_howto_type nlm32_alpha_nw_howto = HOWTO (ALPHA_R_NW_RELOC,	/* type */
						      0,	/* rightshift */
  ...
  for (l = powerpc_stubs; l != (struct powerpc_stub *) NULL; l = l->next)
    {
      ...
      bfd_put_32 (outbfd, (bfd_vma) powerpc_stub_insns[0] + l->toc_index,
		  buf);

      /* Write this stub out.  */
      if (!bfd_set_section_contents (outbfd,
				     bfd_get_section (l->start),
				     buf, l->start->value, POWERPC_STUB_SIZE))
		       ...
		       bfd_size_type contents_size ATTRIBUTE_UNUSED)
{
  ...
  if (strcmp (bfd_get_section_name (insec->owner, insec), ".got") == 0)
    memset (contents + powerpc_initial_got_size, 0,
	    (size_t) (bfd_get_section_size (insec) -
  ...
  for (i = 0; i < reloc_count; i++)
      ...
      if (rel->howto->pc_relative)
	{
	  ...
	  if (bfd_get_section (sym) != insec->output_section)
	  ...
	  else
	    {
	      ...
	      if (bfd_asymbol_name (sym)[0] == '.' && (sym->flags & BSF_DYNAMIC) != 0 && (bfd_get_32 (outbfd, (bfd_byte *) contents + rel->address + 4) == 0x4ffffb82))	/* cror 31,31,31 */
      ...
      if (sym_value + rel->addend != 0)
	{
	  ...
	  switch (rel->howto->size)
	    {
	    case 1:
	      val = bfd_get_16 (outbfd, (bfd_byte *) contents + rel->address);
	      val = ((val & ~rel->howto->dst_mask)
		     | (((val & rel->howto->src_mask)
			 + sym_value + rel->addend) & rel->howto->dst_mask));
...
link_inputs (struct string_list *inputs, char *ld, char *map_file)
{
  ...
  if (map_file && *map_file && strcmp (map_file, "stderr") == 0)
    {
      argv[4] = (char *) "-Map";
      argv[5] = map_file;
      i = 6;
