...
wr_rl (struct coff_ofile *ptr ATTRIBUTE_UNUSED, struct coff_section *sec)
{
  ...
  for (i = 0; i < nr; i++)
      ...
      else
	{
      ...
      if (rl.op == OP_SEC_REF || rl.op == OP_EXT_REF)
  ...
  if (symbol->type->type == coff_function_type)
    {
      ...
      wr_dps_start (sfile, symbol->where->section,
		    symbol->type->u.function.code, BLOCK_TYPE_BLOCK, nest);
      ...
      wr_dps_end (symbol->where->section,
		  symbol->type->u.function.code, BLOCK_TYPE_BLOCK);
      wr_dps_end (symbol->where->section,
		  ...
		  struct coff_symbol *symbol, int nest)
{
  ...
  switch (symbol->where->where)
    ...
    case coff_where_memory:
      dsy.address =
	symbol->where->offset - find_base (sfile, symbol->where->section);
      break;
		 ...
		 struct coff_scope *scope, int nest, int type)
{
  ...
  if (scope->vars_head || (scope->list_head && scope->list_head->vars_head))
    {
...
wr_du (struct coff_ofile *p, struct coff_sfile *sfile, int n)
{
  ...
  for (j = 0; j < lim; j++)
      ...
      if (sfile->section[src].init)
	{
	  ...
	  du.address[dst] = sfile->section[src].low;
	}
      else
	{
	  du.length[dst] = 0;
	  ...
	  if (sfile->section[src].parent)
	    {
	      printf (" section %6s 0x%08x..0x%08x\n",
		      ...
		      du.address[dst], du.address[dst] + du.length[dst] - 1);
	    }
	}
	...
	int n ATTRIBUTE_UNUSED)
{
  ...
  for (sy = sfile->scope->vars_head; sy; sy = sy->next)
    {
	    ...
	    int n ATTRIBUTE_UNUSED)
{
  ...
  for (sy = p->symbol_list_head; sy; sy = sy->next_in_ofile_list)
    {
...
wr_debug (struct coff_ofile *p)
{
  ...
  for (sfile = p->source_head; sfile; sfile = sfile->next)
    {
      if (debug)
	printf ("%s\n", sfile->name);
...
wr_sc (struct coff_ofile *ptr, struct coff_sfile *sfile)
{
  ...
  for (symbol = sfile->scope->vars_head; symbol; symbol = symbol->next)
    {
...
wr_unit_info (struct coff_ofile *ptr)
{
  ...
  for (sfile = ptr->source_head; sfile; sfile = sfile->next)
    {
      long p1;
      long p2;
      int nsecs;
...
prescan (struct coff_ofile *tree)
{
  ...
  for (s = tree->symbol_list_head; s; s = s->next_in_ofile_list)
    {
  ...
  if (!output_file)
    {
      ...
      if (len > 3
	  && output_file[len - 2] == '.' && output_file[len - 1] == 'o')
	{
	  output_file[len] = 'b';
	  output_file[len + 1] = 'j';
	  output_file[len + 2] = 0;
