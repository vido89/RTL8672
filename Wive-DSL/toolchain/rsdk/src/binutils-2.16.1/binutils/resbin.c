		    ...
		    int big_endian)
{
  ...
  while (length > 0)
      ...
      if (ch == 'S')
	{
	  ...
	  if (vallen != 0)
	    fatal (_("unexpected version stringtable value length %d"),
		   vallen);
...
static struct bindata *res_to_bin_cursor (const struct cursor *, int);
static struct bindata *res_to_bin_group_cursor
  (const struct group_cursor *, int);
static struct bindata *res_to_bin_dialog (const struct dialog *, int);
static struct bindata *res_to_bin_fontdir (const struct fontdir *, int);
static struct bindata *res_to_bin_group_icon (const struct group_icon *, int);
static struct bindata *res_to_bin_menu (const struct menu *, int);
static struct bindata *res_to_bin_menuitems (const struct menuitem *, int);
static struct bindata *res_to_bin_menuexitems (const struct menuitem *, int);
static struct bindata *res_to_bin_rcdata (const struct rcdata_item *, int);
  ...
  for (a = accelerators; a != NULL; a = a->next)
    {
      ...
      put_16 (big_endian,
	      a->flags | (a->next != NULL ? 0 : ACC_LAST), d->data);
