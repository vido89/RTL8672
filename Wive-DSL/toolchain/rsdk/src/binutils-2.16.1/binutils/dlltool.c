	     ...
	     int noname, int constant, int data, int private)
{
  ...
  if ((internal_name != NULL) && (strchr (internal_name, '.') != NULL))
...
static void
append_import (const char *symbol_name, const char *dll_name,
	       int func_ordinal)
{
  iheadtype **pq;
  iheadtype *q;
  ...
  for (; from < fromend; from += size)
    {
      ...
      sym =
	bfd_minisymbol_to_symbol (abfd, FALSE, (const void *) from, store);
      if (sym == NULL)
	bfd_fatal (bfd_get_filename (abfd));
  ...
  for (i = 0, exp = d_exports; exp; i++, exp = exp->next)
    {
      fprintf (f, "%s  %d = %s %s @ %d %s%s%s%s\n",
	       ...
	       exp->noname ? "NONAME " : "",
	       exp->private ? "PRIVATE " : "",
	       exp->constant ? "CONSTANT" : "", exp->data ? "DATA" : "");
    }
}
...
flush_page (FILE * f, long *need, int page_addr, int on_page)
{
  ...
  fprintf (f, "\t%s\t0x%08x\t%s Starting RVA for chunk\n",
	   ASM_LONG, page_addr, ASM_C);
  fprintf (f, "\t%s\t0x%x\t%s Size of block\n",
	   ASM_LONG, (on_page * 2) + (on_page & 1) * 2 + 8, ASM_C);
  ...
  for (i = 0, exp = d_exports; exp; i++, exp = exp->next)
      ...
      if (strcmp (exp->name, exp->internal_name) == 0)
	{
	  fprintf (output_def, "\t%s%s%s @ %d%s%s%s\n",
		   ...
		   exp->ordinal,
		   exp->noname ? " NONAME" : "",
		   exp->private ? "PRIVATE " : "", exp->data ? " DATA" : "");
	}
      else
	{
  ...
  if (d_exports)
    {
      ...
      fprintf (f, "\t%s	%d	%s Number of names\n", ASM_LONG,
	       show_allnames ? d_high_ord - d_low_ord +
	       1 : d_named_nfuncs, ASM_C);
...
#define EXTRA    0
  ...
  for (i = 0; i < NSECS; i++)
	...
	abort ();
      ...
      bfd_set_section_flags (abfd, si->sec, si->flags & applicable);
	...
	case PDATA:
	  {
	    ...
	    imglue->addend = 0;
	    imglue->howto = bfd_reloc_type_lookup (abfd, BFD_RELOC_32_GOTOFF);
  ...
  for (i = 0; i < NSECS; i++)
	...
	continue;
      ...
      bfd_set_section_contents (abfd, si->sec, si->data, 0, si->size);
    }
...
process_duplicates (export_type ** d_export_vec)
{
  ...
  while (more)
	...
	{
	  if (strcmp (d_export_vec[i]->name, d_export_vec[i + 1]->name) == 0)
	    {
