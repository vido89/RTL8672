...
look_for_prog (const char *prog_name, const char *prefix, int end_prefix)
{
  ...
  cmd = xmalloc (strlen (prefix) + strlen (prog_name)
...
run (const char *what, char *args)
{
  ...
  if (pid == -1)
	...
	{
	  warn (_("%s exited with status %d"), what,
...
Creating one, but that may not be what you want"));
  ...
  for (i = 1; i < argc; ++i)
      ...
      if (dlltool_arg_indices[i])
	{
	  ...
	  dyn_string_append_cstr (dlltool_cmdline, (quote) ? " \"" : " ");
  ...
  for (i = 1; i < argc; ++i)
      ...
      if (driver_arg_indices[i])
	{
	  char *arg = saved_argv[i];
	  int quote = (strchr (arg, ' ') || strchr (arg, '\t'));
	  dyn_string_append_cstr (driver_cmdline, (quote) ? " \"" : " ");
  ...
  if (!def_file_seen)
      ...
      for (i = 1; i < argc; ++i)
	{
	  if (driver_arg_indices[i])
		...
		{
		  int quote = (strchr (arg, ' ') || strchr (arg, '\t'));
		  dyn_string_append_cstr (step_pre1, (quote) ? " \"" : " ");
...

  {
    ...
    dyn_string_t step1 = dyn_string_new (driver_cmdline->length
					 + strlen (base_file_name) + 20);
    dyn_string_append_cstr (step1, "-Wl,--base-file,");
    quote = (strchr (base_file_name, ' ') || strchr (base_file_name, '\t'));
    dyn_string_append_cstr (step1, (quote) ? "\"" : "");
...

  {
    ...
    quote = (strchr (base_file_name, ' ') || strchr (base_file_name, '\t'));
    dyn_string_append_cstr (step3, (quote) ? "\"" : "");
...

  {
    ...
    quote = (strchr (base_file_name, ' ') || strchr (base_file_name, '\t'));
    dyn_string_append_cstr (step4, (quote) ? "\"" : "");
...

  {
    ...
    dyn_string_t step5 = dyn_string_new (driver_cmdline->length
					 + strlen (exp_file_name) + 20);
    quote = (strchr (exp_file_name, ' ') || strchr (exp_file_name, '\t'));
    dyn_string_append_cstr (step5, (quote) ? "\"" : "");
