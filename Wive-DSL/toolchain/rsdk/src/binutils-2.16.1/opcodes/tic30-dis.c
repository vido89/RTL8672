     ...
     disassemble_info *info;
{
  ...
  insn_word =
    (*(info->buffer + bufaddr) << 24) | (*(info->buffer + bufaddr + 1) << 16)
	...
	for (; current_optab < tic30_optab_end; current_optab++)
	  {
		...
		else if ((current_optab->base_opcode & NORMAL_IDEN) ==
			 (insn_word & NORMAL_IDEN))
		  {
		    insn->tm = current_optab;
		    break;
		  }
	...
	for (; current_optab < tic30_optab_end; current_optab++)
	  {
		...
		if (current_optab->operand_types[0] & Imm24)
		  ...
		  {
		    if ((current_optab->
			 base_opcode & (BR_COND_IDEN | 0x00800000)) ==
     ...
     struct instruction *insn;
{
  ...
  if (insn->tm->opcode_modifier == AddressMode)
	...
	get_register_operand ((insn_word & 0x001F0000) >> 16,
			      operand[dest_op]);
	...
	case AM_IMM:
	  ...
	  switch (insn->tm->imm_arg_type)
	    ...
	    case Imm_Float:
	      ...
	      break;
	    case Imm_SInt:
	      sprintf (operand[src_op], "%d",
		       (short) (insn_word & 0x0000FFFF));
	      break;
	    case Imm_UInt:
     ...
     struct instruction *insn;
{
  ...
  switch (insn->ptm->oporder)
	...
	case 0x00000000:
	  get_indirect_operand ((insn_word & 0x000000FF), 1, operand[0][1]);
	  get_indirect_operand ((insn_word & 0x0000FF00) >> 8, 1,
				operand[0][0]);
     ...
     struct instruction *insn;
{
  ...
  /* Get the operand for the trap instruction.  */
  ...
  else
    {
      ...
      if (insn->tm->operands == 2)
	{
	  get_register_operand (((insn_word & 0x01C00000) >> 22) + REG_AR0,
				operand[0]);
	  if (insn_word & PCRel)
	    {
