  ...
  while (op->name)
    {
      ...
      if (op->format == FMT_1)
	mysize = 1;
      else if (op->format == FMT_2 || op->format == FMT_4)
	mysize = 2;
      else if (op->format == FMT_3 || op->format == FMT_5)
	mysize = 3;
	...
	extra_shift = 8;
	       ...
	       || op->format == FMT_6 || op->format == FMT_7)
	extra_shift = 16;
      else
	extra_shift = 0;

      if ((op->mask & insn) == op->opcode && size == (unsigned int) mysize)
	{
	  ...
	  for (opindex_ptr = op->operands, nocomma = 1;
		...
		(*info->
		 print_address_func) ((value + memaddr + mysize) & 0xffffff,
				      info);
