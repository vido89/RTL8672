  ...
  for (mem_acc = &mem_table[0]; mem_acc->name != NULL || !mem_acc; ++mem_acc)
      ...
      if (reg_code == mem_acc->opcode)
	{
	  for (syntax = mem_access_syntax_table;
	       mem_access_syntax_table != NULL
	       || mem_access_syntax_table->name; ++syntax)
  ...
  if (insn0 & 0x80)
      ...
      else
	{
	  for (reg_x = &system_reg_table[0];
	       reg_x->reg_name != NULL && reg_x; ++reg_x)
	    {
...
maxq_print_arg (MAX_ARG_TYPE arg,
		struct disassemble_info *info, group_info grp)
{
  switch (arg)
    {
    case FLAG_C:
		  ...
		  if (format == 1)
		    info->fprintf_func (info->stream, " %s",
					get_reg_name (grp.src,
						      (type1) 1 /*SRC*/));
		  if (format == 0)
		    info->fprintf_func (info->stream, " #%02xh", (grp.src));
		}
	    }
