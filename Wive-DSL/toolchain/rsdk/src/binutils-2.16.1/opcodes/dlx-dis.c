  ...
  for (idx = 0; idx < dlx_load_opcode_num; idx++)
	...
	if (opc == OPC (LHIOP))
	...
	else
	  {
	    ...
	    operand_deliminator (info, dlx_load_opcode[idx].name);
	    (*info->fprintf_func) (info->stream, "r%d,", (int) rs2);
	    (*info->fprintf_func) (info->stream, "0x%04x[r%d]", (int) imm16,
				   (int) rs1);
	  }
     ...
     struct disassemble_info *info;
{
  ...
  int dlx_br_opcode_num = (sizeof dlx_br_opcode) / (sizeof dlx_br_opcode[0]);
  int idx;
     ...
     struct disassemble_info *info;
{
  ...
  int dlx_jr_opcode_num = (sizeof dlx_jr_opcode) / (sizeof dlx_jr_opcode[0]);
  int idx;
     ...
     struct disassemble_info *info;
{
  ...
  int dlx_insn_type_num =
    ((sizeof dlx_insn_type) / (sizeof (unsigned long))) - 1;
  int status =
