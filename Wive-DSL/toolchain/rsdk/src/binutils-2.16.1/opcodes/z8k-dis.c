...
output_instr (instr_data_s * instr_data,
	      unsigned long addr ATTRIBUTE_UNUSED, disassemble_info * info)
{
  int num_bytes;
  char out_str[100];
...
static void
unpack_instr (instr_data_s * instr_data, int is_segmented,
...
unparse_instr (instr_data_s * instr_data, int is_segmented)
{
  ...
  for (loop = 0; loop < loop_limit; loop++)
	...
	case CLASS_BA:
	  if (is_segmented)
	    sprintf (tmp_str, "rr%ld(#0x%lx)",
		     instr_data->arg_reg[datum_value], instr_data->immediate);
	  else
	    sprintf (tmp_str, "r%ld(#0x%lx)",
		     instr_data->arg_reg[datum_value], instr_data->immediate);
	  strcat (out_str, tmp_str);
	  break;
	case CLASS_BX:
