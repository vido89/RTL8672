     ...
     struct disassemble_info *info;
{
  ...
  if (((insn & 0xc) == 0 && (insn & 0x2a0))
      || ((insn & 3) == 0 && (insn & 0x150)))
    if (info->mach != bfd_mach_sh_dsp && info->mach != bfd_mach_sh3_dsp)
      {
	...
	if (!first_movx)
	  {
	    for (first_movx = sh_table; first_movx->nibbles[1] != MOVX_NOPY;)
	      first_movx++;
	    for (first_movy = first_movx;
		 first_movy->nibbles[1] != MOVY_NOPX;)
	      first_movy++;
	  }
  ...
  if ((field_b & 0xe800) == 0)
    {
      fprintf_fn (stream, "psh%c\t#%d,",
		  field_b & 0x1000 ? 'a' : 'l', (field_b >> 4) & 127);
  ...
  if ((field_b & 0xc000) == 0x4000 && (field_b & 0x3000) != 0x1000)
      ...
      if (field_b & 0x2000)
	{
	  fprintf_fn (stream, "p%s %s,%s,%s\t",
		      ...
		      sx_tab[(field_b >> 6) & 3],
		      sy_tab[(field_b >> 4) & 3], du_tab[(field_b >> 0) & 3]);
	}
      ...
      else if ((field_b & 0xf3) != 0)
	{
      ...
      fprintf_fn (stream, "pmuls%c%s,%s,%s",
		  field_b & 0x2000 ? ' ' : '\t',
		  se_tab[(field_b >> 10) & 3],
		  sf_tab[(field_b >> 8) & 3], sg_tab[(field_b >> 2) & 3]);
      return;
    }
  ...
  for (op = sh_table; op->name; op++)
    {
	  ...
	  && op->nibbles[2] == nib2 && op->nibbles[3] == nib3)
	{
	  int n;
  ...
  for (op = sh_table; op->name; op++)
    {
      ...
      if (!allow_op32 && SH_MERGE_ARCH_SET (op->arch, arch_op32))
	goto fail;
