...
const CGEN_HW_ENTRY ip2k_cgen_hw_table[] = {
  ...
  {"h-registers", HW_H_REGISTERS, CGEN_ASM_NONE, 0,
...
const CGEN_IFLD ip2k_cgen_ifld_table[] = {
  ...
  {IP2K_F_ADDR16CJP, "f-addr16cjp", 0, 16, 12, 13,
  ...
  {IP2K_F_SKIPB, "f-skipb", 0, 16, 12, 1,
   {0 | A (ABS_ADDR), {(1 << MACH_BASE)}}},
	...
	if (mach->insn_chunk_bitsize != 0)
	  {
		...
		&& cd->insn_chunk_bitsize != mach->insn_chunk_bitsize)
	      {
		fprintf (stderr,
			 "ip2k_cgen_rebuild_tables: conflicting insn-chunk-bitsize values: `%d' vs. `%d'\n",
			 cd->insn_chunk_bitsize, mach->insn_chunk_bitsize);
		abort ();
	      }
