...
struct rdep
{
  ...
  unsigned mode:2,		/* RAW, WAW, or WAR.  */
...
static short insert_dependencies (int, unsigned short *, int,
				  unsigned short *);
...
static void add_dep (const char *, const char *, const char *, int, int,
		     char *, int);
...
static int in_iclass_mov_x (struct ia64_opcode *, struct iclass *,
			    const char *, const char *);
static int in_iclass (struct ia64_opcode *, struct iclass *, const char *,
		      const char *, int *);
...
static void insert_bit_table_ent (struct bittree *, int, ia64_insn, ia64_insn,
				  int, int, int);
static void add_dis_entry (struct bittree *, ia64_insn, ia64_insn, int,
			   struct completer_entry *, int);
		     ...
		     int nregs, unsigned short *regs)
{
  ...
  for (i = 0; i < opdeplen; i++)
    if (opdeps[i]->chk == chkind && opdeps[i]->reg == regind)
      return i;
  ...
  if (ic->nsubs == 0 && ic->nxsubs == 0)
    {
      ...
      resolved = ((strncmp (ic->name, idesc->name, len) == 0)
		  && (idesc->name[len] == '\0' || idesc->name[len] == '.'));
...
print_dis_table (void)
{
  ...
  while (cent != NULL)
	...
	{
	  printf ("{ 0x%x, %d, %d, %d },\n", ent->completer_index,
		  ent->insn, (ent->nexte != NULL ? 1 : 0), ent->priority);
	  ent = ent->nexte;
	}
      cent = cent->next_ent;
    }
...
generate_disassembler (void)
{
  ...
  for (i = 0; i < otlen; i++)
	...
	add_dis_entry (bittree,
		       ...
		       ptr->main_index, ptr->completers, 1);
    }
  ...
  if (ent != NULL)
      ...
      else
	{
	  ...
	  while (1)
		...
		{
		  while (i > 0 && ent->name->num == glist[i - 1]->name->num)
		    i--;
     ...
     struct completer_entry *cmp ATTRIBUTE_UNUSED;
{
  ...
  for (i = 0; i < rdepslen; i++)
    {
	  ...
	  && strncmp (rs->name, "PR%", 3) == 0 && rs->mode == 1)
	no_class_found = 99;
      ...
      for (j = 0; j < rs->nregs; j++)
	{
	  ...
	  if (in_iclass (opc, ics[rs->regs[j]], NULL, NULL, &ic_note))
		...
		warn (_
		      ...
		      ic_note, opc->name, ics[rs->regs[j]]->name, rs->name,
		      rs->regnotes[j]);
      ...
      for (j = 0; j < rs->nchks; j++)
	{
	  ...
	  if (in_iclass (opc, ics[rs->chks[j]], NULL, NULL, &ic_note))
		...
		warn (_
		      ...
		      ic_note, opc->name, ics[rs->chks[j]]->name, rs->name,
		      rs->chknotes[j]);
	      if (ic_note)
  ...
  if (no_class_found)
    warn (_("opcode %s has no class (ops %d %d %d)\n"),
	  opc->name, opc->operands[0], opc->operands[1], opc->operands[2]);
     ...
     struct completer_entry *ent;
{
  ...
  printf ("  { 0x%x, 0x%x, %d, %d, %d, %d, %d, %d },\n",
	  ...
	  moffset, ent->is_terminal ? 1 : 0, ent->dependencies);
}

static void
...
print_main_table (void)
{
  ...
  while (ptr != NULL)
    {
      printf ("  { %d, %d, %d, 0x",
	      ptr->name->num, ptr->opcode->type, ptr->opcode->num_outputs);
      ...
      printf ("ull, { %d, %d, %d, %d, %d }, 0x%x, %d, },\n",
	      ...
	      ptr->opcode->flags, ptr->completers->num);
