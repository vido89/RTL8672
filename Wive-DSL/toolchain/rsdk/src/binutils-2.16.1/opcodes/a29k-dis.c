     ...
     int opcode;
{
  return (opcode == 0xa8 || opcode == 0xa9 || opcode == 0xa0 || opcode == 0xa1
	  ...
	  || opcode == 0xac || opcode == 0xad || opcode == 0xcc);
}
     ...
     struct disassemble_info *info;
{
  ...
  if ((insn24 == 0x70) && (insn16 == 0x40) && (insn8 == 0x01)
      && (insn0 == 0x01))
    {
  ...
  for (opcode = &a29k_opcodes[0];
       opcode < &a29k_opcodes[num_opcodes]; ++opcode)
    {
      if (((unsigned long) insn24 << 24) == opcode->opcode)
	{
	  ...
	  if (insn24 == 2)	/* consth */
	      ...
	      if (errcode == 0)
		{
		  ...
		  if (prev_insn24 == 3 && prev_insn8 == insn8)
		    {
		      (*info->fprintf_func) (info->stream, "\t; ");
		      (*info->print_address_func)
			(((insn16 << 24) + (insn0 << 16)
			  + (prev_insn16 << 8) + (prev_insn0)), info);
		    }
		}
	    }
