  ...
  while (instruction >= crx_instruction)
    {
      mask = build_mask ();
      if ((doubleWord & mask) ==
	  BIN (instruction->match, instruction->match_bits))
	return 1;
      else
	instruction--;
    }
  ...
  switch (a->type)
      ...
      else
	{
	  ...
	  p = makelongparameter (allWords, 32 - (start_bits + 4),
				 32 - start_bits);
	  a->r = p.val;
	  p = makelongparameter (allWords, 32 - start_bits, inst_bit_size);
	}
      if ((p.nbits == 4) && cst4flag)
	{
...
print_arg (argument * a, bfd_vma memaddr, struct disassemble_info *info)
{
  ...
  switch (a->type)
	...
	a->constant = 2 * a->constant + 2;
	       ...
	       || IS_INSN_TYPE (STOR_IMM_INS) || IS_INSN_TYPE (CSTBIT_INS))
	{
...
static void
print_arguments (ins * currInsn, bfd_vma memaddr,
