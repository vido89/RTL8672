...
static int lookup_opcode
PARAMS ((struct d30v_insn * insn, long num, int is_long));
static void print_insn
PARAMS ((struct disassemble_info * info, bfd_vma memaddr, long long num,
	 struct d30v_insn * insn, int is_long, int show_ext));
static int extract_value
PARAMS ((long long num, struct d30v_operand * oper, int is_long));
     ...
     int show_ext;
{
  ...
  if (d30v_operand_table[insn->form->operands[0]].flags & OPERAND_NAME)
    {
      ...
      val =
	extract_value (num,
		       (struct d30v_operand *) &d30v_operand_table[insn->
								   form->
								   operands
								   [0]],
		       is_long);
  ...
  while ((opnum = insn->form->operands[opind++]) != 0)
	...
	bits += 3;
      ...
      if (need_comma
	  && oper->flags != OPERAND_PLUS && oper->flags != OPERAND_MINUS)
	{
	  need_comma = 0;
	  (*info->fprintf_func) (info->stream, ", ");
	}
      ...
      if (oper->flags & OPERAND_REG)
	{
	  ...
	  if (oper->flags & OPERAND_CONTROL)
		...
		(struct d30v_operand *) &d30v_operand_table[insn->form->
							    operands[2]];
     ...
     int is_long;
{
  ...
  if (is_long)
      ...
      if (oper->bits == 32)
	{
	  ...
	  val = ((num & 0x3FFFF)
		 | ((num & 0xFF00000) >> 2) | ((num & 0x3F00000000LL) >> 6));
	}
      else
