  ...
  switch (looking_for & SIZE)
      ...
      if ((looking_for & MODE) == DISP)
	{
	  if (OP_KIND (q->how) == O_MOVAB ||
	      OP_KIND (q->how) == O_MOVAW || OP_KIND (q->how) == O_MOVAL)
	    {
     ...
     int len;
{
  ...
  if ((x & SIZE) == L_3 || (x & SIZE) == L_3NZ)
    ...
    {
      if ((x & SIZE) == L_16 || (x & SIZE) == L_16U)
	{
	  outfn (stream, ".%s%d (0x%x)",
		 (short) cst > 0 ? "+" : "",
		 (short) cst, addr + (short) cst + len);
	}
      else
	{
	  outfn (stream, ".%s%d (0x%x)",
		 (char) cst > 0 ? "+" : "",
		 (char) cst, addr + (char) cst + len);
	}
    }
  else if ((x & MODE) == DISP)
    {
	      ...
	      else if (looking_for == (op_type) E)
		{
		  ...
		  /* Fill in the args.  */
		  {
			...
			|| strcmp (qi->opcode->name, "subs") == 0)
		      {
			outfn (stream, "#%d,%s", cst[0],
			       pregnames[regno[1] & 0x7]);
			return qi->length;
		      }

		    for (nargs = 0;
			 nargs < 3 && args[nargs] != (op_type) E; nargs++)
		      {
			int x = args[nargs];
