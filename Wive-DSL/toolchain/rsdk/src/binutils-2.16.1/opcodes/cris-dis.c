...
static char *format_sup_reg PARAMS ((unsigned int, char *, bfd_boolean));
static int cris_constraint
PARAMS ((const char *, unsigned int, unsigned int,
	 struct cris_disasm_data *));
static unsigned bytes_to_skip
PARAMS ((unsigned int, const struct cris_opcode *, enum cris_disass_family));
     ...
     struct cris_disasm_data *disdata;
{
  ...
  if (opc_table == NULL)
	...
	return NULL;
      ...
      dip_prefixes = malloc (65536 * sizeof (const struct cris_opcode **));
	...
	return NULL;
      ...
      rest_prefixes = malloc (65536 * sizeof (const struct cris_opcode **));
  ...
  if (prefix_insn != NO_CRIS_PREFIX && prefix_opc_table[insn] != NULL)
  ...
  else
    {
      ...
      for (opcodep = cris_opcodes; opcodep->name != NULL; opcodep++)
	{
	  int level_of_match;
     ...
     bfd_boolean with_reg_prefix;
{
  ...
  for (; *s; s++)
	...
	case 'o':
	  {
	    ...
	    target =
	      addr + ((disdata->distype == cris_dis_v32) ? 0 : 2) + offset;
	    info->target = target;
	    *tp = 0;
	    tp = temp;
	    (*info->fprintf_func) (info->stream, "%s", temp);
	...
	case 'i':
	  tp =
	    format_dec ((insn & 32) ? (insn & 31) | ~31 : insn & 31, tp, 1);
	  break;
