...
static void dummy_print_address PARAMS ((bfd_vma, struct disassemble_info *));

static int fetch_arg PARAMS ((unsigned char *, int, int, disassemble_info *));

static void print_base PARAMS ((int, bfd_vma, disassemble_info *));
     ...
     disassemble_info *info;
{
  ...
  if (!opcodes[0])
      ...
      for (i = 1; i < 16; i++)
	{
      ...
      for (i = 0; i < m68k_numopcodes; i++)
	*opc_pointer[(m68k_opcodes[i].opcode >> 28) & 15]++ =
	  &m68k_opcodes[i];
    }
     ...
     disassemble_info *info;
{
  ...
  switch (*d)
	...
	(*info->fprintf_func)
	  (info->stream,
	   "%s@", reg_names[fetch_arg (buffer, place, 3, info) + 8]);
	break;
      }
	...
	} names[] =
	{
	  ...
	  "%flashbar", 0xc04},
	  {
	  "%rambar", 0xc05},	/* mcf528x added these.  */
    ...
    case 'R':
      (*info->fprintf_func)
	(info->stream, "%s", reg_names[fetch_arg (buffer, place, 4, info)]);
      break;
    ...
    case 'F':
      (*info->fprintf_func)
	(info->stream, "%%fp%d", fetch_arg (buffer, place, 3, info));
      break;
    ...
    case 'g':
      val = fetch_arg (buffer, place, 1, info);
      (*info->fprintf_func) (info->stream, "%%accext%s",
			     val == 0 ? "01" : "23");
      break;
	...
	(*info->fprintf_func) (info->stream, "%s%s",
			       reg_half_names[reg], is_upper ? "u" : "l");
      }
      break;
     ...
     disassemble_info *info;
{
  ...
  switch (code)
	...
	| ((buffer[0] >> 1) & 0x7) | (buffer[3] & 0x80 ? 0x10 : 0);
      break;
     ...
     disassemble_info *info;
{
  ...
  sprintf (buf, "%s:%c%s",
	   ...
	   (word & 0x800) ? 'l' : 'w', scales[(word >> 9) & 3]);
