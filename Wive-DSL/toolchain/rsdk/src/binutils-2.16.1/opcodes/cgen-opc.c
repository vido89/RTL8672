...
static unsigned int hash_keyword_value (const CGEN_KEYWORD *, unsigned int);
static void build_keyword_hash_tables (CGEN_KEYWORD *);
...
cgen_keyword_add (CGEN_KEYWORD * kt, CGEN_KEYWORD_ENTRY * ke)
{
  ...
  for (i = 1; i < strlen (ke->name); i++)
    if (!ISALNUM (ke->name[i]) && !strchr (kt->nonalpha_chars, ke->name[i]))
      {
...
cgen_keyword_search_next (CGEN_KEYWORD_SEARCH * search)
{
  ...
  while (search->current_hash < search->table->hash_table_size)
    {
      search->current_entry =
	search->table->name_hash_table[search->current_hash];
      if (search->current_entry != NULL)
	return search->current_entry;
      ++search->current_hash;
    }
...
cgen_get_insn_value (CGEN_CPU_DESC cd, unsigned char *buf, int length)
{
  ...
  if (insn_chunk_bitsize != 0 && insn_chunk_bitsize < length)
      ...
      for (i = 0; i < length; i += insn_chunk_bitsize)	/* NB: i == bits */
	{
	  ...
	  this_value =
	    bfd_get_bits (&buf[index / 8], insn_chunk_bitsize, big_p);
...
cgen_put_insn_value (CGEN_CPU_DESC cd,
		     unsigned char *buf, int length, CGEN_INSN_INT value)
{
  int big_p = (cd->insn_endian == CGEN_ENDIAN_BIG);
  int insn_chunk_bitsize = cd->insn_chunk_bitsize;
  ...
  if (insn_chunk_bitsize != 0 && insn_chunk_bitsize < length)
      ...
      for (i = 0; i < length; i += insn_chunk_bitsize)	/* NB: i == bits */
	{
	  ...
	  bfd_put_bits ((bfd_vma) value, &buf[index / 8], insn_chunk_bitsize,
			big_p);
...
cgen_lookup_insn (CGEN_CPU_DESC cd,
		  const CGEN_INSN * insn, CGEN_INSN_INT insn_int_value,
		  /* ??? CGEN_INSN_BYTES would be a nice type name to use here.  */
		  unsigned char *insn_bytes_value,
		  int length, CGEN_FIELDS * fields, int alias_p)
{
  unsigned char *buf;
  ...
  if (!insn)
  ...
  else
    {
      ...
      if (!alias_p && CGEN_INSN_ATTR_VALUE (insn, CGEN_INSN_ALIAS))
	abort ();
			...
			const CGEN_FIELDS * fields, int *indices)
{
  ...
  if (insn->opinst == NULL)
    abort ();
  for (i = 0, opinst = insn->opinst; opinst->type != CGEN_OPINST_END;
       ++i, ++opinst)
    {
...
cgen_lookup_get_insn_operands (CGEN_CPU_DESC cd,
			       ...
			       /* ??? CGEN_INSN_BYTES would be a nice type name to use here.  */
			       unsigned char *insn_bytes_value,
			       int length, int *indices, CGEN_FIELDS * fields)
{
