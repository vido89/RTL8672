...
static void my_sprintf
VPARAMS ((struct arcDisState * state, char *buf, const char *format, ...))
{
  char *bp;
  const char *p;
...
static void
write_instr_name_ (state, instrName, cond, condCodeIsPartOfName, flag,
		   signExtend, addrWriteBack, directMem)
     struct arcDisState *state;
     ...
     struct arcDisState *state;
{
  ...
  switch (state->_opcode)
      ...
      if (!instrName)
	{
      ...
      decodingClass =
	((state->_opcode == op_JC) ? CLASS_A4_JC : CLASS_A4_BRANCH);
      state->isBranch = 1;
      break;
  ...
  switch (decodingClass)
      ...
      else
	{
      ...
      write_instr_name ();
      strcat (formatString, IS_REG (B) ? "[%r]" : "%s");	/* address/label name  */
      if (fieldA != 0)
	{
	  fieldAisReg = 0;
