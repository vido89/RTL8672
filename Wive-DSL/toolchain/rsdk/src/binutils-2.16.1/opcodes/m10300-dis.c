     ...
     struct disassemble_info *info;
{
  ...
  else if ((insn & 0xff) == 0xcd || (insn & 0xff) == 0xdc)
    {
      status = (*info->read_memory_func) (memaddr, buffer, 4, info);
      if (status != 0)
	{
  ...
  else if ((insn & 0xff) == 0xfd || (insn & 0xff) == 0xfc)
    {
      status = (*info->read_memory_func) (memaddr, buffer, 4, info);
      if (status != 0)
	{
  ...
  while (op->name)
    {
      ...
      if (op->format == FMT_S0)
	mysize = 1;
      else if (op->format == FMT_S1 || op->format == FMT_D0)
	mysize = 2;
      else if (op->format == FMT_S2 || op->format == FMT_D1)
	mysize = 3;
	      ...
	      || (op->machine == AM30 && HAVE_AM30)))
	{
	  ...
	  if (op->format == FMT_D1 || op->format == FMT_S1)
	    ...
	    extra_shift = 16;
		   ...
		   || op->format == FMT_D8 || op->format == FMT_D9)
	    extra_shift = 8;
	  else
	    extra_shift = 0;
	  ...
	  if (size == 1 || size == 2)
	    {
	      extension = 0;
		   ...
		   && (op->format == FMT_D1
		       || op->opcode == 0xdf0000 || op->opcode == 0xde0000))
	    {
	      extension = 0;
	    }
	  else if (size == 3 && op->format == FMT_D6)
	    {
	      extension = 0;
	    }
	  else if (size == 3)
	    {
	      insn &= 0xff0000;
	      status =
		(*info->read_memory_func) (memaddr + 1, buffer, 2, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
		       ...
		       || op->opcode == 0xfaf40000))
	    {
	      extension = 0;
		   ...
		   && (op->format == FMT_D7 || op->format == FMT_D10))
	    {
	      extension = 0;
	    }
	  else if (size == 4)
	    {
	      insn &= 0xffff0000;
	      status =
		(*info->read_memory_func) (memaddr + 2, buffer, 2, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
	  ...
	  else if (size == 5 && op->opcode == 0xdc000000)
	    {
	      ...
	      status =
		(*info->read_memory_func) (memaddr + 1, buffer, 4, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
	      ...
	      if (status != 0)
		{
	      ...
	      status =
		(*info->read_memory_func) (memaddr + 4, buffer, 1, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
	  ...
	  else if (size == 6 && op->format == FMT_D8)
	    {
	      insn &= 0xffffff00;
	      status =
		(*info->read_memory_func) (memaddr + 5, buffer, 1, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
	      ...
	      status =
		(*info->read_memory_func) (memaddr + 3, buffer, 2, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
	  ...
	  else if (size == 6)
	    {
	      unsigned long temp = 0;
	      status =
		(*info->read_memory_func) (memaddr + 2, buffer, 4, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
	  ...
	  else if (size == 7 && op->format == FMT_D9)
	    {
	      insn &= 0xffffff00;
	      status =
		(*info->read_memory_func) (memaddr + 3, buffer, 4, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
	      ...
	      if (status != 0)
		{
	      ...
	      status =
		(*info->read_memory_func) (memaddr + 5, buffer, 2, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
	      ...
	      if (status != 0)
		{
	      ...
	      status =
		(*info->read_memory_func) (memaddr + 6, buffer, 1, info);
	      if (status != 0)
		{
		  (*info->memory_error_func) (status, memaddr, info);
		  return;
	  ...
	  for (opindex_ptr = op->operands, nocomma = 1;
		...
		value =
		  ((value ^ (((unsigned long) 1) << (operand->bits - 1))) -
	      ...
	      else if ((operand->flags & MN10300_OPERAND_RREG) != 0)
		{
		  ...
		  if (value < 8)
		    ...
		    (*info->fprintf_func) (info->stream, "a%d",
					   (int) value - 8);
		  else
