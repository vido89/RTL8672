...
static CGEN_INSN_LIST *hash_insn_array (CGEN_CPU_DESC, const CGEN_INSN *, int,
					int, CGEN_INSN_LIST **,
					CGEN_INSN_LIST *);
static CGEN_INSN_LIST *hash_insn_list (CGEN_CPU_DESC, const CGEN_INSN_LIST *,
  ...
  for (i = count - 1; i >= 0; --i, ++hentbuf)
	...
	continue;
      ...
      bfd_put_bits ((bfd_vma) value,
		    buf, CGEN_INSN_MASK_BITSIZE (insn), big_p);
		...
		CGEN_INSN_LIST ** htable, CGEN_INSN_LIST * hentbuf)
{
  ...
  for (ilist = insns; ilist != NULL; ilist = ilist->next, ++hentbuf)
	...
	continue;
      ...
      bfd_put_bits ((bfd_vma) value,
		    buf, CGEN_INSN_MASK_BITSIZE (ilist->insn), big_p);
      hash = (*cd->dis_hash) (buf, value);
