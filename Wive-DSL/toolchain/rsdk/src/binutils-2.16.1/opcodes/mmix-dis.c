     ...
     struct disassemble_info *info;
{
  ...
  if ((info->section != NULL && info->section->owner != NULL)
	...
	? info->section->owner : bfd_asymbol_bfd (info->symbols[0]);
     ...
     unsigned long insn;
{
  ...
  do
	...
	case mmix_operands_put:
	  ...
	  if ((insn & INSN_IMMEDIATE_BIT) || ((insn >> 16) & 255) < 32)
	    return opcodep;
	  break;
     ...
     struct disassemble_info *info;
{
  ...
  /* FIXME: Is -1 suitable?  */
  if (info->private_data == NULL && !initialize_mmix_dis_info (info))
    return -1;
  ...
  switch (opcodep->operands)
    ...
    case mmix_operands_regs:
      ...
      (*info->fprintf_func) (info->stream, "%s,%s,%s",
			     minfop->reg_name[x],
			     minfop->reg_name[y], minfop->reg_name[z]);
      break;
      ...
      if (y != 0)
	{
	  if (insn & INSN_IMMEDIATE_BIT)
	    (*info->fprintf_func) (info->stream, "%s,%s,%d",
				   minfop->reg_name[x], ROUND_MODE (y), z);
	  else
	    (*info->fprintf_func) (info->stream, "%s,%s,%s",
				   minfop->reg_name[x],
				   ROUND_MODE (y), minfop->reg_name[z]);
	}
      else
	{
	  if (insn & INSN_IMMEDIATE_BIT)
	    ...
	    (*info->fprintf_func) (info->stream, "%s,%s",
				   minfop->reg_name[x], minfop->reg_name[z]);
	}
      break;
    ...
    case mmix_operands_sync:
      /* Like SYNC - "XYZ".  */
      (*info->fprintf_func) (info->stream, "%u", x * 65536 + y * 256 + z);
      break;
	...
	(*info->fprintf_func) (info->stream, "%d,%s,%s",
			       x, minfop->reg_name[y], minfop->reg_name[z]);
      break;
	...
	(*info->fprintf_func) (info->stream, "%s,%d,%s",
			       minfop->reg_name[x], y, minfop->reg_name[z]);
      break;
    ...
    case mmix_operands_get:
      /* GET - "X,spec_reg".  */
      (*info->fprintf_func) (info->stream, "%s,%s",
			     minfop->reg_name[x], minfop->spec_reg_name[z]);
      break;
	...
	(*info->fprintf_func) (info->stream, "%s,%s",
			       minfop->spec_reg_name[x], minfop->reg_name[z]);
      break;

    case mmix_operands_set:
      /*  Two registers, "$X,$Y".  */
      (*info->fprintf_func) (info->stream, "%s,%s",
			     minfop->reg_name[x], minfop->reg_name[y]);
      break;
