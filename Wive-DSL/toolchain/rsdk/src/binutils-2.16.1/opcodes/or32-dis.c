...
static void or32_print_register
PARAMS ((char, char *, unsigned long, struct disassemble_info *));
static void or32_print_immediate
PARAMS ((char, char *, unsigned long, struct disassemble_info *));
     ...
     unsigned long *insn;
{
  *insn =
    ...
    ((unsigned long) insn_ch[2] << 8) + ((unsigned long) insn_ch[3]);
#if DEBUG
     ...
     unsigned long *insn;
{
  *insn =
    ...
    ((unsigned long) insn_ch[1] << 8) + ((unsigned long) insn_ch[0]);
}
     ...
     struct disassemble_info *info;
{
  ...
  find_byte_func_type find_byte_func =
    (find_byte_func_type) info->private_data;
  ...
  for (opcode = &or32_opcodes[0];
       opcode < &or32_opcodes[or32_num_opcodes]; ++opcode)
    {
