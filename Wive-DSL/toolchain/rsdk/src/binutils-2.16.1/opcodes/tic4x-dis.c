...
static int tic4x_print_char PARAMS ((struct disassemble_info *, char));
static int tic4x_print_str PARAMS ((struct disassemble_info *, char *));
...
void tic4x_print_ftoa PARAMS ((unsigned int, FILE *, fprintf_ftype));
      ...
      if (IS_CPU_TIC4X (tic4x_version))
	{
	  ...
	  for (i = 0; i < tic4x_num_registers; i++)
	    registertable[tic4x_registers[i].regno] =
	      (void *) &tic4x_registers[i];
	}
    }
  if ((int) regno >
      (IS_CPU_TIC4X (tic4x_version) ? TIC4X_REG_MAX : TIC3X_REG_MAX))
    return 0;
  if (info != NULL)
     ...
     unsigned long arg;
{
  ...
  switch (type)
    ...
    default:
      (*info->fprintf_func) (info->stream,
			     "# internal error: Unknown indirect type %d",
			     type);
      return 0;
    }
     ...
     unsigned long pc;
{
  ...
  while (*s)
	...
	case 'F':		/* 16-bit float immediate 0--15 */
	  tic4x_print_immed (info, IMMED_SFLOAT, EXTRU (instruction, 15, 0));
	  break;
	...
	case 'S':		/* 16-bit signed immediate 0--15 */
	  tic4x_print_immed (info, IMMED_SINT, EXTRS (instruction, 15, 0));
	  break;
     ...
     const unsigned long tic4x_oplevel;
{
  ...
  for (j = opcode; j < opmask; j++)
    if ((j & opmask) == opcode && inst->oplevel & tic4x_oplevel)
      {
#if TIC4X_DEBUG
     ...
     struct disassemble_info *info;
{
  ...
  if (optable == NULL)
	...
	tic4x_hash_opcode (optable, optable_special, &tic4x_insts[i],
      ...
      for (i = 0; i < TIC4X_SPESOP_SIZE; i++)
	if (optable_special[i] != NULL)
	  optable[optable_special[i]->opcode >> (32 - TIC4X_HASH_SIZE)] =
	    NULL;
    }
