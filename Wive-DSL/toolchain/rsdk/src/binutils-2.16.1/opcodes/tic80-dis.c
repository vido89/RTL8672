...
static void print_operand_condition_code
PARAMS ((struct disassemble_info *, long));
static void print_operand_control_register
PARAMS ((struct disassemble_info *, long));
...
PARAMS ((struct disassemble_info *, long, unsigned long,
	 const struct tic80_operand *, bfd_vma));
static int print_one_instruction
PARAMS ((struct disassemble_info *, bfd_vma, unsigned long,
	 const struct tic80_opcode *));
static int print_instruction
PARAMS ((struct disassemble_info *, bfd_vma, unsigned long,
	 const struct tic80_opcode *));
static int fill_instruction
PARAMS ((struct disassemble_info *, bfd_vma, unsigned long *));

     ...
     const struct tic80_opcode *vec_opcode;
{
  ...
  for (opcode = tic80_opcodes; opcode < opcode_end; opcode++)
    {
      if ((insn & opcode->mask) == opcode->opcode && opcode != vec_opcode)
	{
	  break;
	}
    }
  ...
  if (opcode == opcode_end)
  ...
  else
    {
      ...
      if (opcode->flags & TIC80_VECTOR && vec_opcode == NULL
	  && TWO_INSN (insn))
	{
