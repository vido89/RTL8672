 ...
 * for more details.
 *
 * RCSID $Id: connections.c,v 1.1.1.1 2003/08/18 05:39:35 kaohj Exp $
 */
...
, bool initiator, bool aggrmode)
{
	 ...
	 */
	if (psk == NULL) {
	    log("refine_host_connection - cannot determine PSK of %s and %s!", c->this.id.name, c->that.id.name);
	    return NULL;	/* cannot determine PSK! */
	}
	if (c->dnshostname != NULL) {
			d = c;
			d->that.id = *peer_id;
			log("refine_host_connection - get_preshared_secret a");
			psk_check = get_preshared_secret(d);
			log("refine_host_connection - get_preshared_secret b");
			if (psk_check != NULL && !strcmp(psk_check->ptr, psk->ptr) && (psk_check->len == psk->len)) {
				ip_address new_addr;
				d->that.host_addr = new_addr;
				log("refine_host_connection - get_preshared_secret 1");
				if (ttoaddr(inet_ntoa(d->that.id.ip_addr.u.v4.sin_addr), 0, c->addr_family, &new_addr) == NULL) {
					for (; d != NULL; d = d->hp_next)
					{
						d->that.host_addr = new_addr;
						log("refine_host_connection - get_preshared_secret 2");
						state_rehash(c);
						log("refine_host_connections - get_preshared_secret 3");
					}
				}
			}
	}
